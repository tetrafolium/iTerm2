...
static NSTimeInterval DelayInGifProperties(NSDictionary *gifProperties) {
  ...
  if (gifProperties) {
    NSNumber *number = (id)CFDictionaryGetValue(
        (CFDictionaryRef)gifProperties, kCGImagePropertyGIFUnclampedDelayTime);
...
static NSImage *DecodeSixelData(sixel_decoder_t *decoder, NSData *data) {
  ...
  SIXELSTATUS status = sixel_decode_raw(
      (unsigned char *)[[data mutableCopy] mutableBytes], (int)data.length,
      &pixels, &width, &height, &palette, &ncolors, NULL);
...
static NSImage *ImageFromSixelData(NSData *data) {
  ...
  NSRange range = [data rangeOfData:newlineData
                            options:0
  ...
  NSData *payload =
      [data subdataWithRange:NSMakeRange(NSMaxRange(range),
                                         data.length - NSMaxRange(range))];
  NSString *paramString = [[NSString alloc] initWithData:params
  ...
  NSArray<NSString *> *paramParts =
      [paramString componentsSeparatedByString:@";"];
  [paramParts
      enumerateObjectsUsingBlock:^(NSString *_Nonnull value, NSUInteger index,
                                   BOOL *_Nonnull stop) {
        sixel_decoder_setopt(decoder, (int)index, value.UTF8String);
      }];
...
int main(int argc, const char *argv[]) {
  ...
  @autoreleasepool {
    ...
    iTermSerializableImage *serializableImage =
        [[iTermSerializableImage alloc] init];
    ...
    CGImageSourceRef source =
        CGImageSourceCreateWithData((CFDataRef)data, (CFDictionaryRef) @{});
    ...
    if (count > 1) {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        // TIFF and PDF files may have multiple pages, so make sure it's an
        // animated GIF.
        if (gifProperties) {
