...
static NSString *const iTermRPCTriggerPathCapturedStrings =
    @"trigger.captured_strings";
static NSString *const iTermRPCTriggerPathCapturedRanges =
    @"trigger.captured_ranges";
...
- (NSArray<NSString *> *)allPaths {
  return @[
    iTermRPCTriggerPathCapturedStrings, iTermRPCTriggerPathCapturedRanges,
    iTermRPCTriggerPathInput, iTermRPCTriggerPathLineNumber
  ];
}
...
- (BOOL)performActionWithCapturedStrings:(NSString *const *)capturedStrings
  ...
  NSArray<NSString *> *captureStringArray =
      [[NSArray alloc] initWithObjects:capturedStrings count:captureCount];
  NSMutableArray<NSArray<NSNumber *> *> *captureRangeArray =
      [NSMutableArray array];
  for (NSInteger i = 0; i < captureCount; i++) {
    [captureRangeArray addObject:@[
      @(capturedRanges[i].location), @(capturedRanges[i].length)
    ]];
  }

  NSDictionary *temporaryVariables = @{
    iTermRPCTriggerPathCapturedStrings : captureStringArray,
...
- (id<NSTextFieldDelegate>)newParameterDelegateWithPassthrough:
