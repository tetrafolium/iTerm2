...
+ (NSString *)stringForSelectionMovementUnit:
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
  ...
  if (self) {
    ...
    _parameter =
        [dictionary[iTermKeyBindingDictionaryKeyParameter] ?: @"" copy];
...
- (NSString *)displayName {
  ...
  case KEY_ACTION_HEX_CODE:
    actionString =
        [NSString stringWithFormat:@"%@ %@", @"Send Hex Codes:", _parameter];
    break;
  case KEY_ACTION_VIM_TEXT:
    actionString =
        [NSString stringWithFormat:@"%@ \"%@\"", @"Send:", _parameter];
    break;
  ...
  case KEY_ACTION_RUN_COPROCESS:
    actionString =
        [NSString stringWithFormat:@"Run Coprocess \"%@\"", _parameter];
    break;
  case KEY_ACTION_SELECT_MENU_ITEM: {
    NSArray *parts = [_parameter componentsSeparatedByString:@"\n"];
    actionString = [NSString
        stringWithFormat:@"%@ “%@”", @"Select Menu Item", parts.firstObject];
    break;
  }
  case KEY_ACTION_NEW_WINDOW_WITH_PROFILE:
    actionString = [NSString stringWithFormat:@"New Window with \"%@\" Profile",
                                              GetProfileName(_parameter)];
    break;
  case KEY_ACTION_NEW_TAB_WITH_PROFILE:
    actionString = [NSString stringWithFormat:@"New Tab with \"%@\" Profile",
                                              GetProfileName(_parameter)];
    break;
  case KEY_ACTION_SPLIT_HORIZONTALLY_WITH_PROFILE:
    actionString =
        [NSString stringWithFormat:@"Split Horizontally with \"%@\" Profile",
                                   GetProfileName(_parameter)];
    break;
  case KEY_ACTION_SPLIT_VERTICALLY_WITH_PROFILE:
    actionString =
        [NSString stringWithFormat:@"Split Vertically with \"%@\" Profile",
                                   GetProfileName(_parameter)];
    break;
  case KEY_ACTION_SET_PROFILE:
    actionString = [NSString stringWithFormat:@"Change Profile to \"%@\"",
                                              GetProfileName(_parameter)];
    break;
  case KEY_ACTION_LOAD_COLOR_PRESET:
    actionString =
        [NSString stringWithFormat:@"Load Color Preset \"%@\"", _parameter];
    break;
  ...
  case KEY_ACTION_PASTE_SPECIAL_FROM_SELECTION: {
    NSString *pasteDetails = [iTermPasteSpecialViewController
        descriptionForCodedSettings:_parameter];
    if (pasteDetails.length) {
      actionString =
          [NSString stringWithFormat:@"Paste from Selection: %@", pasteDetails];
    } else {
  ...
  case KEY_ACTION_MOVE_END_OF_SELECTION_LEFT:
    actionString =
        [NSString stringWithFormat:@"Move End of Selection Left %@",
                                   [self.class stringForSelectionMovementUnit:
                                                   _parameter.integerValue]];
    break;
  ...
  default:
    actionString =
        [NSString stringWithFormat:@"%@ %d", @"Unknown Action ID", _keyAction];
    break;
