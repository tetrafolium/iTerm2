...
- (NSString *)displayString {
  return [[iTermKeyBindingAction withAction:_action
                                  parameter:_parameter ?: @""] displayName];
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _actions = [[[NSArray castFrom:[[NSUserDefaults standardUserDefaults]
                                       objectForKey:kPreferenceKeyActions]]
...
- (void)addAction:(iTermAction *)action {
  ...
  [[iTermActionsDidChangeNotification
      notificationWithMutationType:iTermActionsDidChangeMutationTypeInsertion
...
- (void)replaceAction:(iTermAction *)actionToReplace
  ...
  [self save];
  [[iTermActionsDidChangeNotification
      notificationWithMutationType:iTermActionsDidChangeMutationTypeEdit
                             index:index] post];
}

- (NSInteger)indexOfActionWithIdentifier:(NSInteger)identifier {
  return [_actions
      indexOfObjectPassingTest:^BOOL(iTermAction *_Nonnull action,
...
- (void)moveActionsWithIdentifiers:(NSArray<NSNumber *> *)identifiers
  ...
  NSArray<iTermAction *> *actions =
      [_actions filteredArrayUsingBlock:^BOOL(iTermAction *action) {
        return [identifiers containsObject:@(action.identifier)];
      }];
  NSInteger countBeforeRow =
      [[actions filteredArrayUsingBlock:^BOOL(iTermAction *action) {
...
+ (instancetype)notificationWithMutationType:
                    (iTermActionsDidChangeMutationType)mutationType
...
+ (void)subscribe:(NSObject *)owner
            block:(void (^)(iTermActionsDidChangeNotification
