...
+ (instancetype)subSelectionWithAbsRange:(VT100GridAbsWindowedRange)unsafeRange
  ...
  VT100GridAbsWindowedRange range =
      VT100GridAbsWindowedRangeClampedToWidth(unsafeRange, width);
  sub.absRange = range;
...
+ (instancetype)subSelectionWithDictionary:(NSDictionary *)dict
  ...
  VT100GridAbsWindowedRange range;
  range = VT100GridAbsWindowedRangeFromWindowedRange(
      [dict[kiTermSubSelectionRange] gridWindowedRange],
...
- (NSDictionary *)dictionaryValueWithYOffset:(int)yOffset
  ...
  const VT100GridWindowedRange range =
      VT100GridWindowedRangeFromAbsWindowedRange(absrange,
                                                 totalScrollbackOverflow);
  return @{
    kiTermSubSelectionRange :
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p range=%@ mode=%@>", [self class], self,
...
- (BOOL)containsAbsCoord:(VT100GridAbsCoord)coord {
  const VT100GridAbsCoord start =
      VT100GridAbsCoordRangeMin(_absRange.coordRange);
  ...
  if (_selectionMode == kiTermSelectionModeBox) {
    ...
    contained = (coord.x >= left && coord.x < right && coord.y >= top &&
                 coord.y <= bottom);
  } else {
  ...
  if (_absRange.columnWindow.length) {
    contained =
        contained && VT100GridRangeContains(_absRange.columnWindow, coord.x);
  }
  return contained;
}
...
- (NSArray *)nonwindowedComponentsWithWidth:(int)width {
  ...
  [[self class
  ] enumerateAbsoluteRangesInAbsWindowedRange:self.absRange
                                        block:^(
                                            VT100GridAbsCoordRange subrange) {
                                          iTermSubSelection *sub = [iTermSubSelection
                                              subSelectionWithAbsRange:
...
+ (void)
    enumerateAbsoluteRangesInAbsWindowedRange:
        (VT100GridAbsWindowedRange)absWindowedRange
                                        block:(void (^)(VT100GridAbsCoordRange))
                                                  block {
  if (absWindowedRange.columnWindow.length) {
    const int right = absWindowedRange.columnWindow.location +
                      absWindowedRange.columnWindow.length;
    int startX = VT100GridAbsWindowedRangeStart(absWindowedRange).x;
    for (long long y = absWindowedRange.coordRange.start.y;
         y < absWindowedRange.coordRange.end.y; y++) {
      block(VT100GridAbsCoordRangeMake(startX, y, right, y));
      startX = absWindowedRange.columnWindow.location;
    }
    block(VT100GridAbsCoordRangeMake(
        startX, absWindowedRange.coordRange.end.y,
...
- (BOOL)isEqual:(id)object {
  ...
  return (VT100GridAbsWindowsRangeEqualsAbsWindowedRange(self.absRange,
                                                         other.absRange) &&
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@: %p liveRange=%@ initialRange=%@ live=%d extend=%d "
          @"resumable=%d mode=%@ subselections=%@ delegate=%@>",
          [self class], self, VT100GridAbsWindowedRangeDescription(_absRange),
          VT100GridAbsCoordRangeDescription(_initialAbsRange.coordRange), _live,
          _extend, _resumable, [[self class] nameForMode:_selectionMode],
          _subSelections, _delegate];
}

- (void)flip {
  _absRange.coordRange = VT100GridAbsCoordRangeMake(
      _absRange.coordRange.end.x, _absRange.coordRange.end.y,
...
- (void)beginExtendingSelectionAt:(VT100GridAbsCoord)absCoord {
  ...
  VT100GridAbsWindowedRange absRange =
      [self absRangeForCurrentModeAtAbsCoord:absCoord
  ...
  if (absRange.coordRange.start.x != -1) {
    ...
    if ([self absCoord:absCoord isInAbsRange:_absRange]) {
      ...
      long long distanceToStart = VT100GridAbsCoordDistance(
          _absRange.coordRange.start, absRange.coordRange.start, width);
      long long distanceToEnd = VT100GridAbsCoordDistance(
          _absRange.coordRange.end, absRange.coordRange.end, width);
      if (distanceToEnd < distanceToStart) {
        // Move the end point
        _absRange.coordRange.end = absRange.coordRange.end;
        _initialAbsRange =
            [self absRangeForCurrentModeAtAbsCoord:_absRange.coordRange.start
        ...
        const VT100GridAbsCoord anchor = [_delegate
            selectionPredecessorOfAbsCoord:_absRange.coordRange.start];
      ...
      if ([self absCoord:determinant.start isEqualToAbsCoord:determinant.end]) {
        // The initial range was empty, so use the live selection range to
        // decide whether to move the start or end point of the live range.
        determinant = [self unflippedLiveAbsRange].coordRange;
      }
      if ([self absCoord:absRange.coordRange.end
              isAfterAbsCoord:determinant.end]) {
        _absRange.coordRange.end = absRange.coordRange.end;
        _initialAbsRange =
            [self absRangeForCurrentModeAtAbsCoord:_absRange.coordRange.start
                             includeParentheticals:NO
      ...
      if ([self absCoord:absRange.coordRange.start
              isBeforeAbsCoord:determinant.start]) {
        [self flip];
        _absRange.coordRange.end = absRange.coordRange.start;
        VT100GridAbsCoord lastSelectedCharAbsCoord = [_delegate
            selectionPredecessorOfAbsCoord:_absRange.coordRange.start];
        _initialAbsRange =
            [self absRangeForCurrentModeAtAbsCoord:lastSelectedCharAbsCoord
                             includeParentheticals:NO
                                needAccurateWindow:NO];
      }
    }
...
- (VT100GridAbsWindowedRange)
    absRangeForCurrentModeAtAbsCoord:(VT100GridAbsCoord)rawAbsCoord
  ...
  if (_absRange.columnWindow.length > 0) {
    absCoord.x = MAX(
        _absRange.columnWindow.location,
        MIN(_absRange.columnWindow.location + _absRange.columnWindow.length - 1,
            absCoord.x));
  }
  VT100GridAbsWindowedRange absWindowedRange = VT100GridAbsWindowedRangeMake(
      VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
  switch (_selectionMode) {
  case kiTermSelectionModeWord:
    if (includeParentheticals) {
      absWindowedRange =
          [_delegate selectionAbsRangeForParentheticalAt:absCoord];
    }
  ...
  case kiTermSelectionModeSmart:
    absWindowedRange =
        [_delegate selectionAbsRangeForSmartSelectionAt:absCoord];
    break;
  ...
  case kiTermSelectionModeBox:
    ...
    absWindowedRange.coordRange = VT100GridAbsCoordRangeMake(
        absCoord.x, absCoord.y, absCoord.x, absCoord.y);
    break;
  }
...
- (void)beginSelectionAtAbsCoord:(VT100GridAbsCoord)absCoord
  ...
  const long long totalScrollbackOverflow =
      _delegate.selectionTotalScrollbackOverflow;
  ...
  DLog(@"Begin new selection. coord=%@, extend=%d",
       VT100GridAbsCoordDescription(absCoord), extend);
  _live = YES;
  _extend = NO;
...
- (void)endLiveSelection {
  ...
  if (_selectionMode == kiTermSelectionModeBox) {
    const int left =
        MIN(_absRange.coordRange.start.x, _absRange.coordRange.end.x);
    const int right =
        MAX(_absRange.coordRange.start.x, _absRange.coordRange.end.x);
    const long long top =
        MIN(_absRange.coordRange.start.y, _absRange.coordRange.end.y);
    const long long bottom =
        MAX(_absRange.coordRange.start.y, _absRange.coordRange.end.y);
    _absRange = VT100GridAbsWindowedRangeMake(
        VT100GridAbsCoordRangeMake(left, top, right, bottom), 0, 0);
    for (long long i = top; i <= bottom; i++) {
      VT100GridAbsWindowedRange theRange = VT100GridAbsWindowedRangeMake(
          VT100GridAbsCoordRangeMake(left, i, right, i), 0, 0);
      theRange = [self absRangeByExtendingRangePastNulls:theRange];
      iTermSubSelection *sub = [iTermSubSelection
          subSelectionWithAbsRange:theRange
...
- (BOOL)absCoord:(VT100GridAbsCoord)absCoord
    isInAbsRange:(VT100GridAbsWindowedRange)absRange {
  iTermSubSelection *temp =
...
- (BOOL)moveSelectionEndpointTo:(VT100GridAbsCoord)coord {
  ...
  const long long totalScrollbackOverflow =
      [self.delegate selectionTotalScrollbackOverflow];
  if (coord.y < totalScrollbackOverflow) {
    coord.x = 0;
    coord.y = totalScrollbackOverflow;
  }
  NSArray<iTermSubSelection *> *subselectionsBefore =
      [[self.allSubSelections copy] autorelease] ?: @[];
...
- (void)moveSelectionEndpointToAbsRange:(VT100GridAbsWindowedRange)range {
  ...
  if ([self absCoord:_absRange.coordRange.start
          isBeforeAbsCoord:range.coordRange.end]) {
    // The word you clicked on ends after the start of the existing range.
    if ([self liveRangeIsFlipped]) {
      // The range is flipped. This happens when you were selecting backwards
      // and turned around and started going forwards.
      //
      // end<---------selection--------->start
      //                                |...<-----------range----------->
      //                 <-initialRange->
      ...
      //       ...<---range--->
      // start<---newRange---->end
      if ([self absCoord:range.coordRange.start
              isBeforeAbsCoord:_absRange.coordRange.start]) {
        // This happens with smart selection, where the new range is not just a
        // subset of the old range.
    ...
    if (![self liveRangeIsFlipped]) {
      // This happens when you were selecting forwards and turned around and
...
- (void)scrollbackOverflowDidChange {
  BOOL notifyDelegateOfChange =
      _subSelections.count > 0 || [self haveLiveSelection];
  const long long totalScrollbackOverflow =
      [self.delegate selectionTotalScrollbackOverflow];
  if ([self haveLiveSelection]) {
...
- (BOOL)absRangeIsFlipped:(VT100GridAbsWindowedRange)range {
  return VT100GridAbsCoordOrder(range.coordRange.start, range.coordRange.end) ==
         NSOrderedDescending;
}
...
- (long long)length {
  ...
  const int width = [self width];
  [self enumerateSelectedAbsoluteRanges:^(VT100GridAbsWindowedRange range,
                                          BOOL *stop, BOOL eol) {
...
- (VT100GridAbsWindowedRange)
    unflippedAbsRangeForAbsRange:(VT100GridAbsWindowedRange)absRange
                            mode:(iTermSelectionMode)mode {
  if (mode == kiTermSelectionModeBox) {
    ...
    absRange.coordRange = VT100GridAbsCoordRangeMake(
        MIN(absRange.coordRange.start.x, absRange.coordRange.end.x),
  ...
  } else if ([self absCoord:absRange.coordRange.end
                 isBeforeAbsCoord:absRange.coordRange.start]) {
    ...
    // reading order.
    absRange.coordRange = VT100GridAbsCoordRangeMake(
        absRange.coordRange.end.x, absRange.coordRange.end.y,
        absRange.coordRange.start.x, absRange.coordRange.start.y);
  }
  return absRange;
}

- (VT100GridAbsWindowedRange)absRangeByExtendingRangePastNulls:
    (VT100GridAbsWindowedRange)range {
  const long long totalScrollbackOverflow =
      [self.delegate selectionTotalScrollbackOverflow];
  if (range.coordRange.start.y < totalScrollbackOverflow) {
  ...
  VT100GridAbsWindowedRange unflippedRange =
      [self unflippedAbsRangeForAbsRange:range mode:_selectionMode];
  VT100GridRange nulls = [_delegate
      selectionRangeOfTerminalNullsOnAbsoluteLine:unflippedRange.coordRange
                                                      .start.y];
  ...
  nulls = [_delegate
      selectionRangeOfTerminalNullsOnAbsoluteLine:unflippedRange.coordRange.end
                                                      .y];
...
- (NSArray *)allSubSelections {
  if ([self haveLiveSelection]) {
    ...
    iTermSubSelection *temp =
        [iTermSubSelection subSelectionWithAbsRange:[self unflippedLiveAbsRange]
                                               mode:_selectionMode
                                              width:self.width];
    [subs addObject:temp];
    return subs;
...
- (VT100GridAbsWindowedRange)lastAbsRange {
  VT100GridAbsWindowedRange best = VT100GridAbsWindowedRangeMake(
      VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
  for (iTermSubSelection *sub in [self allSubSelections]) {
    VT100GridAbsWindowedRange absRange = sub.absRange;
    if (best.coordRange.start.x < 0 ||
        [self absCoord:absRange.coordRange.end
...
- (VT100GridAbsWindowedRange)firstAbsRange {
  ...
  for (iTermSubSelection *sub in [self allSubSelections]) {
    ...
    if (best.coordRange.start.x < 0 ||
        [self absCoord:range.coordRange.start
            isAfterAbsCoord:best.coordRange.start]) {
      best = range;
    }
  }
  return best;
  ...
  if ([_subSelections count] == 0) {
    ...
    [_subSelections
        replaceObjectAtIndex:0
                  withObject:[iTermSubSelection
                                 subSelectionWithAbsRange:firstRange
                                                     mode:mode
                                                    width:self.width]];
  }
  [_delegate selectionDidChange:[[self retain] autorelease]];
...
- (void)setLastAbsRange:(VT100GridAbsWindowedRange)lastRange
  ...
  if (_live) {
    ...
    [_subSelections removeLastObject];
    [_subSelections
        addObject:[iTermSubSelection subSelectionWithAbsRange:lastRange
                                                         mode:mode
                                                        width:self.width]];
  }
  [_delegate selectionDidChange:[[self retain] autorelease]];
...
- (void)removeWindowsWithWidth:(int)width {
  _initialAbsRange = VT100GridAbsWindowedRangeMake(
      VT100GridAbsCoordRangeMake(0, 0, 0, 0), 0, 0);
  if (_live) {
  ...
  for (iTermSubSelection *sub in _subSelections) {
    if (sub.absRange.columnWindow.location == 0 &&
      ...
      for (iTermSubSelection *subsub in
           [sub nonwindowedComponentsWithWidth:self.width]) {
...
- (NSRange)rangeOfIndexesInAbsRange:(VT100GridAbsWindowedRange)range
  ...
  if (range.coordRange.start.y == line) {
    if (range.coordRange.end.y == line) {
      if (range.columnWindow.length) {
        ...
        NSRange result;
        result.location =
            MAX(range.columnWindow.location, range.coordRange.start.x);
  ...
  if (range.coordRange.end.y == line) {
    if (range.columnWindow.length) {
      int limit = VT100GridRangeMax(range.columnWindow) + 1;
      return iTermMakeRange(range.columnWindow.location,
                            MIN(limit, range.coordRange.end.x) -
...
- (NSIndexSet *)selectedIndexesOnAbsoluteLine:(long long)line {
  ...
  if (_live && numberOfSubSelections == 0) {
    // Fast path
    NSRange theRange =
        [self rangeOfIndexesInAbsRange:[self unflippedLiveAbsRange]
                        onAbsoluteLine:line
                                  mode:_selectionMode];
    return [NSIndexSet it_indexSetWithIndexesInRange:theRange];
  }
  ...
  for (iTermSubSelection *sub in [self allSubSelections]) {
    ...
    // Any values in theRange that intersect indexes should be removed from
    // indexes. And values in theSet that don't intersect indexes should be
    // added to indexes.
    NSMutableIndexSet *indexesToAdd =
        [NSMutableIndexSet it_indexSetWithIndexesInRange:theRange];
...
- (NSIndexSet *)selectedIndexesIncludingTabFillersInAbsoluteLine:(long long)y {
  ...
  NSRange range;
  if (_absRange.columnWindow.length > 0) {
    range = NSMakeRange(_absRange.columnWindow.location,
                        _absRange.columnWindow.length);
  } else {
  ...
  NSIndexSet *tabs = [_delegate selectionIndexesOnAbsoluteLine:y
                                           containingCharacter:'\t'
                                                       inRange:range];
  NSIndexSet *tabFillers = [_delegate selectionIndexesOnAbsoluteLine:y
...
- (void)enumerateSelectedAbsoluteRanges:(void (^)(VT100GridAbsWindowedRange,
                                                  BOOL *, BOOL))block {
  if (_live) {
    // Live ranges can have box subs, which is just a pain to deal with, so make
    // a copy, end live selection in the copy (which converts boxes to
    // individual selections), and then try again on the copy.
  ...
  // NOTE: This assumes a 64-bit platform, otherwise the NSUInteger type used by
  // index set would overflow too quickly.
  ...
  for (iTermSubSelection *outer in [self allSubSelections]) {
    if (outer.connected) {
      const long long thePosition = outer.absRange.coordRange.end.x +
                                    outer.absRange.coordRange.end.y * width;
    ...
    [iTermSubSelection
        enumerateAbsoluteRangesInAbsWindowedRange:outer.absRange
                                            block:^(VT100GridAbsCoordRange
                                                        outerRange) {
                                              theRange = NSMakeRange(
                                                  outerRange.start.x +
                                                      outerRange.start.y *
                                                          width,
                                                  VT100GridAbsCoordRangeLength(
                                                      outerRange, width));
                                              ...
                                              [indexes removeIndexes:
                                                           indexesToRemove];
                                              [indexes addIndexes:indexesToAdd];

                                              // In multipart windowed ranges,
                                              // add connectors for the endpoint
                                              // of all but the last range. Each
                                              // enumerated range is on its own
                                              // line.
                                              if (outer.absRange.columnWindow
                                                      .length &&
                                                  !VT100GridAbsCoordEquals(
                                                      outerRange.end,
                                                      outer.absRange.coordRange
                                                          .end) &&
                                                  theRange.length > 0) {
                                                [connectors
                                                    addIndex:NSMaxRange(
  ...
  // enumerateRangesUsingBlock doesn't guarantee the ranges come in order, so
  // put them in an array and then sort it.
  NSMutableArray<NSValue *> *allRanges = [NSMutableArray array];
  [indexes enumerateRangesUsingBlock:^(NSRange range, BOOL *stop) {
    VT100GridAbsCoordRange coordRange = VT100GridAbsCoordRangeMake(
        range.location % width, range.location / width,
        (range.location + range.length) % width,
        (range.location + range.length) / width);
    [allRanges addObject:[NSValue valueWithGridAbsCoordRange:coordRange]];
  }];

  NSArray<NSValue *> *sortedRanges = [allRanges
      sortedArrayUsingSelector:@selector(compareGridAbsCoordRangeStart:)];
  for (NSValue *value in sortedRanges) {
    ...
    block(VT100GridAbsWindowedRangeMake(theRange, 0, 0), &stop,
          ![connectors containsIndex:endIndex] &&
              value != [sortedRanges lastObject]);
    if (stop) {
      break;
    }
  }
...
- (void)setFromDictionaryValue:(NSDictionary *)dict
  ...
  NSMutableArray<iTermSubSelection *> *subSelectionsToAdd =
      [NSMutableArray array];
  for (NSDictionary *subDict in subs) {
    iTermSubSelection *sub =
        [iTermSubSelection subSelectionWithDictionary:subDict
                                                width:width
                              totalScrollbackOverflow:totalScrollbackOverflow];
    if (sub) {
