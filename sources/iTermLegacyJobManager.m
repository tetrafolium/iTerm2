...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p fd=%d tty=%@ childPid=%@>",
                                    NSStringFromClass([self class]), self, _fd,
                                    _tty, @(self.childPid)];
}
...
- (void)forkAndExecWithTtyState:(iTermTTYState)ttyState
                     ...
                     completion:(void (^)(iTermJobManagerForkAndExecStatus))
                                    completion {
  __block iTermJobManagerForkAndExecStatus status =
...
- (iTermJobManagerForkAndExecStatus)
    queueForkAndExecWithTtyState:(iTermTTYState)ttyState
                         argpath:(NSString *)argpath
                            argv:(NSArray<NSString *> *)argv
                      initialPwd:(NSString *)initialPwd
                      newEnviron:(NSArray<NSString *> *)newEnviron
  ...
  self.fd = iTermForkAndExecToRunJobDirectly(&forkState, &ttyState,
                                             argpath.UTF8String, cArgv, YES,
...
- (void)attachToServer:(iTermGeneralServerConnection)serverConnection
  ...
  completion(iTermJobManagerAttachResultsAttached |
...
- (iTermJobManagerAttachResults)
    attachToServer:(iTermGeneralServerConnection)serverConnection
     withProcessID:(NSNumber *)thePid
              task:(id<iTermTask>)task {
  return iTermJobManagerAttachResultsAttached |
         iTermJobManagerAttachResultsRegistered;
}
