...
public:
  ...
  void merge_from(const Sampler &other) {
    ...
    // Shuffle the two values array because we want to sample from them. Make
    // copies so this method can take a const argument and not have unnecessary
    // side-effects.
    ...
    // been sampled with the same probability. If one sampler has Ni values with
    // a weight of Wi then each value was selected with probability Ni/Wi. After
    // merging, each value should be selected with probability T=Nm/(W1+W2)
    // where Nm is the number of elements in the merged vector and W1 and W2 are
    // the weights of the two samplers.
    const double Nm = MIN(_capacity, this_values.size() + other_values.size());
    // Values from vector i (with Si elements) have already been selected with
    // probability Si/Wi. If we pick an element from that vector with
    // probability Pi then its total probability of having been selected is T =
    // Pi * Si/Wi. We want T = Nm/(W1+W2) for selected elements, so we can solve
    // for Pi.
    //
    // T = Nm / (W1 + W2)
    // T = P * (Si / Wi)
    ...
    // Pi = Nm / ((W1 + W2) * (Si / Wi))
    //
    // The number of elements to select is floor(_capacity * Pi). Use floor to
    // avoid selecting more elements than can fit. It introduces a bit of error,
    // but this thing is approximate anyway.
    ...
    merged_values.insert(std::end(merged_values), std::begin(this_values),
                         std::begin(this_values) + N1);
    merged_values.insert(std::end(merged_values), std::begin(other_values),
...
#if ENABLE_STATS
// 3.2.0beta1 had a TON of crashes in dtoa. Somehow I'm producing doubles that
// are so broken they can't be converted to ASCII.
...
- (NSString *)sparklines {
  ...
  [sparklines appendString:[self sparklineGraphWithPrecision:4
                                                  multiplier:1
                                                       units:@""]];

  return [NSString
      stringWithFormat:@"%@ %@ %@  Count=%@ Mean=%@ p50=%@ p95=%@ Sum=%@",
                       @(_min), sparklines, @(_max), @(_count),
                       @(_sum / _count), @(_sampler->value_for_percentile(0.5)),
...
- (NSString *)floatingPointFormatWithPrecision:(int)precision
...
- (NSString *)sparklineGraphWithPrecision:(int)precision
  ...
  NSString *format = [self floatingPointFormatWithPrecision:precision
                                                      units:units];
  ...
  NSMutableString *sparklines =
      [NSMutableString stringWithFormat:format, lowerBound];
  [sparklines appendString:@" "];
  const double largestBucketCount =
      *std::max_element(buckets.begin(), buckets.end());
  for (int i = 0; i < buckets.size(); i++) {
    [sparklines
        appendString:[self sparkWithHeight:buckets[i] / largestBucketCount]];
  }
  [sparklines appendString:@" "];
...
- (NSString *)stringForBucket:(int)bucket
  ...
  NSString *percent = [NSString
      stringWithFormat:@"%0.1f%%", 100.0 * static_cast<double>(count) /
                                       static_cast<double>(total)];
  return [NSString stringWithFormat:@"[%12.0f, %12.0f) %8d (%6s) |%@",
                                    bucketLowerBound, bucketUpperBound, count,
