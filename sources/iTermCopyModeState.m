...
- (BOOL)moveToStartOfLine {
  return
      [self moveInDirection:kPTYTextViewSelectionExtensionDirectionStartOfLine
                       unit:kPTYTextViewSelectionExtensionUnitCharacter];
}
...
- (BOOL)moveToStartOfIndentation {
  return [self
      moveInDirection:kPTYTextViewSelectionExtensionDirectionStartOfIndentation
                 unit:kPTYTextViewSelectionExtensionUnitCharacter];
}
...
- (void)setMode:(iTermSelectionMode)mode {
  ...
  if (_selecting && mode == kiTermSelectionModeLine) {
    ...
    [_textView.selection
        beginSelectionAtAbsCoord:VT100GridAbsCoordFromCoord(_start, overflow)
                            mode:_mode
                          resume:NO
                          append:NO];
    [_textView.selection
        moveSelectionEndpointTo:VT100GridAbsCoordFromCoord(_coord, overflow)];
...
- (PTYTextViewSelectionEndpoint)endpointWithDirection:
    (PTYTextViewSelectionExtensionDirection)direction {
  switch (VT100GridCoordOrder(_start, _coord)) {
  case NSOrderedSame:
    if (direction == kPTYTextViewSelectionExtensionDirectionLeft ||
        direction == kPTYTextViewSelectionExtensionDirectionUp) {
...
- (VT100GridWindowedRange)trivialRange {
  return VT100GridWindowedRangeMake(
      VT100GridCoordRangeMake(_coord.x, _coord.y, 0, 0), 0, 0);
}
...
- (VT100GridCoord)coordFromSelectionEndpoint:
    (PTYTextViewSelectionEndpoint)endpoint {
  iTermSubSelection *sub = _textView.selection.allSubSelections.firstObject;
  const long long overflow = _textView.dataSource.totalScrollbackOverflow;
  if (endpoint == kPTYTextViewSelectionEndpointEnd) {
    return VT100GridCoordFromAbsCoord(sub.absRange.coordRange.end, overflow,
                                      NULL);
  } else {
    return VT100GridCoordFromAbsCoord(sub.absRange.coordRange.start, overflow,
                                      NULL);
  }
}
...
- (BOOL)moveInDirection:(PTYTextViewSelectionExtensionDirection)direction
  ...
  iTermLogicalMovementHelper *helper = [_textView
      logicalMovementHelperForCursorCoordinate:_textView.cursorCoord];
  const VT100GridAbsWindowedRange range =
      [helper absRangeByExtendingRange:absWindowedRange
