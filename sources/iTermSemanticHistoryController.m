...
NSString *const kSemanticHistoryPrefixSubstitutionKey =
    @"semanticHistory.prefix";
NSString *const kSemanticHistorySuffixSubstitutionKey =
    @"semanticHistory.suffix";
NSString *const kSemanticHistoryWorkingDirectorySubstitutionKey =
    @"semanticHistory.workingDirectory";
NSString *const kSemanticHistoryLineNumberKey = @"semanticHistory.lineNumber";
NSString *const kSemanticHistoryColumnNumberKey =
    @"semanticHistory.columnNumber";
...
- (NSString *)cleanedUpPathFromPath:(NSString *)path
  ...
  iTermPathCleaner *cleaner =
      [[iTermPathCleaner alloc] initWithPath:path
                                      suffix:suffix
...
- (NSString *)preferredEditorIdentifier {
  if ([prefs_[kSemanticHistoryActionKey]
          isEqualToString:kSemanticHistoryBestEditorAction]) {
    return [iTermSemanticHistoryPrefsController bestEditor];
  } else if ([prefs_[kSemanticHistoryActionKey]
                 isEqualToString:kSemanticHistoryEditorAction]) {
    return [iTermSemanticHistoryPrefsController
               schemeForEditor:prefs_[kSemanticHistoryEditorKey]]
               ? prefs_[kSemanticHistoryEditorKey]
               : nil;
  } else {
    return nil;
  }
...
- (NSString *)executableInApplicationBundle:(NSBundle *)bundle {
  NSString *executable =
      [bundle.bundlePath stringByAppendingPathComponent:@"Contents/MacOS"];
...
- (NSString *)emacsClientInApplicationBundle:(NSBundle *)bundle {
  ...
  for (
      NSURL *url in [[iTermCachingFileManager cachingFileManager]
    ...
    BOOL isdir = NO;
    [[iTermCachingFileManager cachingFileManager] fileExistsAtPath:url.path
                                                       isDirectory:&isdir];
    if (!isdir) {
  ...
  NSArray<NSString *> *contenders =
      [bindirs filteredArrayUsingBlock:^BOOL(NSString *dir) {
        NSArray<NSString *> *captures =
            [[dir arrayOfCaptureComponentsMatchedByRegex:regex] firstObject];
  ...
  NSString *best = [contenders
      maxWithBlock:^NSComparisonResult(NSString *obj1, NSString *obj2) {
        NSArray<NSString *> *cap1 =
            [obj1 arrayOfCaptureComponentsMatchedByRegex:regex].firstObject;
        NSArray<NSString *> *cap2 =
            [obj2 arrayOfCaptureComponentsMatchedByRegex:regex].firstObject;
...
- (NSString *)intelliJIDEALauncherInApplicationBundle:(NSBundle *)bundle {
  ...
  return [[[[bundle.bundleURL URLByAppendingPathComponent:@"Contents"]
      URLByAppendingPathComponent:@"MacOS"] URLByAppendingPathComponent:@"idea"]
      path];
}
...
- (void)launchVSCodeWithPath:(NSString *)path codium:(BOOL)codium {
  ...
  NSString *bundlePath =
      [self absolutePathForAppBundleWithIdentifier:identifier];
  if (bundlePath) {
    NSString *codeExecutable = [bundlePath
        stringByAppendingPathComponent:@"Contents/Resources/app/bin/code"];
    if ([self.fileManager fileExistsAtPath:codeExecutable]) {
      DLog(@"Launch VSCode %@ %@", codeExecutable, path);
      [self launchTaskWithPath:codeExecutable
                     arguments:@[ path, @"-g" ]
                    completion:nil];
    } else {
      // This isn't as good as opening "code -g" because it always opens a new
      // instance of the app but it's the OS-sanctioned way of running VSCode.
      // We can't use AppleScript because it won't open the file to a particular
      // line number.
...
- (void)launchEmacsWithArguments:(NSArray *)args {
  ...
  if (!emacsClient) {
    ...
    [self
        launchAppWithBundleIdentifier:kEmacsAppIdentifier
                                 args:[@[ @"emacs" ]
  ...
  // That'll use emacsclient if possible and fall back to real emacs if it
  // fails. Normally it will fail unless you've enabled the daemon.
  [self launchTaskWithPath:emacsClient
                 arguments:[@[ @"-n", @"-a", fallback, args ] flattenedArray]
                completion:nil];
}

- (void)launchIntelliJIDEAWithArguments:(NSArray<NSString *> *)args
                                   path:(NSString *)path {
  NSBundle *bundle =
      [self applicationBundleWithIdentifier:kIntelliJIDEAIdentifier];
  if (!bundle) {
  ...
  if (!launcher) {
    ...
    if (path) {
      ...
      [self launchAppWithBundleIdentifier:kIntelliJIDEAIdentifier path:path];
    }
    return;
  }
  [self launchTaskWithPath:launcher arguments:args completion:nil];
}
...
- (void)launchSublimeTextWithBundleIdentifier:(NSString *)bundleId
                                         path:(NSString *)path {
  assert(path);
  if (!path) {
    // I don't expect this to ever happen.
  ...
  if (bundlePath) {
    NSString *sublExecutable = [bundlePath
        stringByAppendingPathComponent:@"Contents/SharedSupport/bin/subl"];
    if ([self.fileManager fileExistsAtPath:sublExecutable]) {
      DLog(@"Launch sublime text %@ %@", sublExecutable, path);
      [self launchTaskWithPath:sublExecutable
                     arguments:@[ path ]
                    completion:nil];
    } else {
      // This isn't as good as opening "subl" because it always opens a new
      // instance of the app but it's the OS-sanctioned way of running
      // Sublimetext.  We can't use AppleScript because it won't open the file
...
+ (NSArray *)bundleIdsThatSupportOpeningToLineNumber {
  return @[
    kAtomIdentifier, kVSCodeIdentifier, kVSCodiumIdentifier,
    kSublimeText2Identifier, kSublimeText3Identifier, kSublimeText4Identifier,
    kMacVimIdentifier, kTextmateIdentifier, kTextmate2Identifier,
    kBBEditIdentifier, kEmacsAppIdentifier, kIntelliJIDEAIdentifier
  ];
}
...
- (void)openFile:(NSString *)path
  ...
  if ([identifier isEqualToString:kEmacsAppIdentifier]) {
    ...
    if (path) {
      ...
      if (lineNumber) {
        if (columnNumber) {
          [args insertObject:[NSString stringWithFormat:@"+%@:%@", lineNumber,
                                                        columnNumber]
                     atIndex:0];
        } else {
          [args insertObject:[NSString stringWithFormat:@"+%@", lineNumber]
  ...
  NSMutableCharacterSet *queryCharset =
  ...
  NSString * (^percentEncoded)(NSString *) = ^NSString *(NSString *string) {
    return [string
        stringByAddingPercentEncodingWithAllowedCharacters:queryCharset];
  };
  NSURLComponents *urlComponents = [[NSURLComponents alloc] init];
  urlComponents.host = @"open";
  urlComponents.path = nil;
  urlComponents.scheme =
      [iTermSemanticHistoryPrefsController schemeForEditor:identifier];
  NSURL *fileURL = [NSURL fileURLWithPath:path];
  urlComponents.percentEncodedQueryItems = @[ [NSURLQueryItem
      queryItemWithName:@"url"
                  value:percentEncoded(fileURL.absoluteString)] ];
  if (lineNumber) {
    NSURLQueryItem *lineItem =
        [NSURLQueryItem queryItemWithName:@"line"
                                    value:percentEncoded(lineNumber)];
    urlComponents.percentEncodedQueryItems =
        [urlComponents.queryItems arrayByAddingObject:lineItem];
  }
...
- (void)openFileInEditor:(NSString *)path
            ...
            columnNumber:(NSString *)columnNumber {
  [self openFile:path
      inEditorWithBundleId:[self preferredEditorIdentifier]
                lineNumber:lineNumber
              columnNumber:columnNumber];
}

- (BOOL)activatesOnAnyString {
  return [prefs_[kSemanticHistoryActionKey]
      isEqualToString:kSemanticHistoryRawCommandAction];
}
...
- (void)launchTaskWithPath:(NSString *)path
  ...
  [self _launchTaskWithPath:path
                  arguments:arguments
                 completion:^(iTermBufferedCommandRunner *runner, int status) {
...
- (void)didFinishCommand:(NSArray *)parts
  ...
  DLog(@"Runner %@ finished with status %@. There are now %@ runners.", runner,
       @(status), @(_commandRunners.count));
  if (!status) {
    return;
  }
  ...
  static const iTermSingleUseWindowOptions options =
      iTermSingleUseWindowOptionsShortLived;
  NSMutableData *inject = [runner.output mutableCopy];
  NSString *truncationWarning =
      [NSString stringWithFormat:@"\n%c[m;[output truncated]\n", 27];
  if (runner.truncated) {
    [inject
        appendData:[truncationWarning dataUsingEncoding:NSUTF8StringEncoding]];
  }
  ...
  NSString *name;
  if ([parts[0] isEqualToString:@"/bin/sh"] &&
      [parts[1] isEqualToString:@"-c"] && parts.count > 2) {
    name = parts[2];
  } else {
    name = parts[0];
  }
  warning.identifier =
      [@"NoSyncSemanticHistoryCommandFailed_" stringByAppendingString:name];
  [warning runModal];
}

- (void)_launchTaskWithPath:(NSString *)path
                  arguments:(NSArray *)arguments
                 completion:(void (^)(iTermBufferedCommandRunner *runner,
                                      int status))completion {
  iTermBufferedCommandRunner *runner = [[iTermBufferedCommandRunner alloc]
      initWithCommand:path
...
- (void)openPath:(NSString *)cleanedUpPath
  ...
  DLog(@"openPath:%@ rawFileName:%@ substitutions:%@ lineNumber:%@ "
       @"columnNumber:%@",
       cleanedUpPath, rawFileName, substitutions, lineNumber, columnNumber);

  NSString *path;
  BOOL isRawAction = [prefs_[kSemanticHistoryActionKey]
      isEqualToString:kSemanticHistoryRawCommandAction];
  if (isRawAction) {
    path = rawFileName;
    lineNumber = @"";
    ...
    path = cleanedUpPath;
    DLog(@"Not a raw action. New path is %@, line number is %@", path,
         lineNumber);
  }

  // Construct a scope with semanticHistory.xxx variables based on the passed-in
  // scope
  iTermVariables *frame = [[iTermVariables alloc]
  ...
  NSString *const escapedPath =
      [path stringWithEscapedShellCharactersIncludingNewlines:YES];
  [scope setValue:escapedPath
      forVariableNamed:kSemanticHistoryPathSubstitutionKey];
  [scope setValue:lineNumber forVariableNamed:kSemanticHistoryLineNumberKey];
  [scope setValue:columnNumber
      forVariableNamed:kSemanticHistoryColumnNumberKey];
  ...
  _expressionEvaluator =
      [[iTermExpressionEvaluator alloc] initWithInterpolatedString:script
                                                             scope:scope];

  if (isRawAction) {
    __weak __typeof(self) weakSelf = self;
    [_expressionEvaluator
        evaluateWithTimeout:self.evaluationTimeout
  ...
  if ([prefs_[kSemanticHistoryActionKey]
          isEqualToString:kSemanticHistoryCommandAction]) {
    __weak __typeof(self) weakSelf = self;
    [_expressionEvaluator
        evaluateWithTimeout:self.evaluationTimeout
                 completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
  ...
  if ([prefs_[kSemanticHistoryActionKey]
          isEqualToString:kSemanticHistoryCoprocessAction]) {
    __weak __typeof(self) weakSelf = self;
    [_expressionEvaluator
        evaluateWithTimeout:self.evaluationTimeout
                 completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                   ...
                   [weakSelf.delegate
                       semanticHistoryLaunchCoprocessWithCommand:evaluator
                                                                     .value];
                   completion(YES);
                 }];
    return;
  ...
  if ([prefs_[kSemanticHistoryActionKey]
          isEqualToString:kSemanticHistoryUrlAction]) {
    NSString *url = prefs_[kSemanticHistoryTextKey];
    // Replace the path with a non-shell-escaped path since we perform escaping
    // later in this code path.
    [scope setValue:path ?: @""
        forVariableNamed:kSemanticHistoryPathSubstitutionKey];

    url = [url stringByPerformingSubstitutions:numericSubstitutions];
    NSString * (^urlEscapeFunction)(NSString *) =
        ^NSString *(NSString *string) {
      return [string stringByAddingPercentEncodingWithAllowedCharacters:
                         [NSCharacterSet URLPathAllowedCharacterSet]];
    };
    iTermParsedExpression *parsedExpression = [iTermExpressionParser
        parsedExpressionWithInterpolatedString:url
                              escapingFunction:urlEscapeFunction
                                         scope:scope
                                        strict:NO];
    _expressionEvaluator = [[iTermExpressionEvaluator alloc]
        initWithParsedExpression:parsedExpression
                      invocation:url
                           scope:scope];
    ...
    [_expressionEvaluator
        evaluateWithTimeout:self.evaluationTimeout
                 completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                   DLog(
                       @"URL format is %@. Error is %@. Evaluated value is %@.",
                       url, evaluator.error, evaluator.value);
                   if (!evaluator.error) {
                     [weakSelf
                         openURL:[NSURL URLWithUserSuppliedString:evaluator
                                                                      .value]];
                   } else {
  ...
  if (lineNumber) {
    ...
    if ([self canOpenFileWithLineNumberUsingEditorWithBundleId:appBundleId]) {
      DLog(@"A line number is present and I know how to open this file to the "
           @"line number using %@. Do so.",
           appBundleId);
      [self openFile:path
          inEditorWithBundleId:appBundleId
                    lineNumber:lineNumber
                  columnNumber:columnNumber];
      completion(YES);
      return;
...
- (BOOL)canOpenFileWithLineNumberUsingEditorWithBundleId:
    (NSString *)appBundleId {
  return [[self.class bundleIdsThatSupportOpeningToLineNumber]
      containsObject:appBundleId];
}
...
- (NSString *)bundleIdForDefaultAppForURL:(NSURL *)fileUrl {
  NSURL *appUrl =
      [[NSWorkspace sharedWorkspace] URLForApplicationToOpenURL:fileUrl];
  if (!appUrl) {
    return nil;
  }
...
- (BOOL)defaultAppForFileIsEditor:(NSString *)file {
  return [iTermSemanticHistoryPrefsController
      bundleIdIsEditor:[self bundleIdForDefaultAppForFile:file]];
}
...
- (NSString *)pathOfExistingFileFoundWithPrefix:(NSString *)beforeStringIn
  ...
  iTermPathFinder *pathfinder =
      [[iTermPathFinder alloc] initWithPrefix:beforeStringIn
...
- (iTermPathFinder *)
    pathOfExistingFileFoundWithPrefix:(NSString *)beforeStringIn
                               suffix:(NSString *)afterStringIn
                     workingDirectory:(NSString *)workingDirectory
                       trimWhitespace:(BOOL)trimWhitespace
                           completion:(void (^)(NSString *path, int prefixChars,
                                                int suffixChars,
  ...
  [pathfinder searchWithCompletion:^{
    ...
    completion(strongPathfinder.path, strongPathfinder.prefixChars,
