...
@interface iTermJobTreeViewController () <NSOutlineViewDelegate,
...
- (instancetype)initWithProcessInfo:(iTermProcessInfo *)processInfo {
  ...
  if (self) {
    ...
    _children = [[processInfo.sortedChildren
        mapWithBlock:^id(iTermProcessInfo *anObject) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p name=%@ pid=%@>", self.class,
                                    self, self.name, @(self.pid)];
}
...
- (instancetype)initWithProcessID:(pid_t)pid {
  self = [super
      initWithNibName:@"iTermJobTreeViewController"
               bundle:[NSBundle
                          bundleForClass:[iTermJobTreeViewController class]]];
  if (self) {
    _pid = pid;
  }
  return self;
...
- (void)update {
  iTermJobProxy *newRoot = [[iTermJobProxy alloc]
      initWithProcessInfo:[[iTermProcessCache sharedInstance]
                              processInfoForPid:_pid]];
  if (!_root) {
    _root = newRoot;
    DLog(@"reloadData");
...
- (void)updateOld:(iTermJobProxy *)old
           andNew:(iTermJobProxy *)new
            index:(NSUInteger)index
           parent:(iTermJobProxy *)parent {
  DLog(@"updateOld:%@ andNew:%@ index:%@ parent:%@", old, new, @(index),
       parent);
  if (old.pid != new.pid) {
    [old moveFrom:new];
  ...
  while (o < oldCount && n < newCount) {
    ...
    if (oldPid == newPid) {
      ...
      [_outlineView
          removeItemsAtIndexes:[NSIndexSet indexSetWithIndex:o + offset]
                      inParent:old
                 withAnimation:YES];
      offset -= 1;
      ...
      [_outlineView
          insertItemsAtIndexes:[NSIndexSet indexSetWithIndex:o + offset]
                      inParent:old
                 withAnimation:YES];
  ...
  while (o < oldCount) {
    DLog(@"Remove index %@ of %@", @(o), old);
    [_outlineView removeItemsAtIndexes:[NSIndexSet indexSetWithIndex:o + offset]
                              inParent:old
                         withAnimation:YES];
    offset -= 1;
  ...
  while (n < newCount) {
    DLog(@"Insert index %@ in %@", @(i), old);
    [_outlineView insertItemsAtIndexes:[NSIndexSet indexSetWithIndex:o + offset]
                              inParent:old
                         withAnimation:YES];
