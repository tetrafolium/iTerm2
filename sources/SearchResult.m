...
+ (SearchResult *)searchResultFromX:(int)x
                                  y:(long long)y
                                toX:(int)endX
                                  y:(long long)endY {
...
- (BOOL)isEqualToSearchResult:(SearchResult *)other {
  return (_startX == other.startX && _endX == other.endX &&
          _absStartY == other.absStartY && _absEndY == other.absEndY);
}

- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p %d,%lld to %d,%lld>", [self class],
                                 self, _startX, _absStartY, _endX, _absEndY];
}
...
- (NSComparisonResult)compare:(SearchResult *)other {
  ...
  return VT100GridAbsCoordOrder(
      VT100GridAbsCoordMake(_startX, _absStartY),
