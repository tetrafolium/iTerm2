...
- (NSArray<NSDictionary *> *)allWindowInfoDictionaries {
  ...
  NSArray<NSDictionary *> *windowInfos =
      (__bridge_transfer NSArray *)CGWindowListCopyWindowInfo(options,
                                                              kCGNullWindowID);
  return windowInfos;
}
...
- (NSArray<NSDictionary *> *)menuBarWindowInfoDictionaries {
  ...
  NSArray<NSDictionary *> *menuBarInfos =
      [windowInfos filteredArrayUsingBlock:^BOOL(NSDictionary *info) {
...
- (CGRect)windowBoundsRectFromWindowInfoDictionary:(NSDictionary *)dictionary {
  ...
  BOOL ok = NO;
  if (dictionary) {
    ok = CGRectMakeWithDictionaryRepresentation(
        (CFDictionaryRef)dictionary[(id)kCGWindowBounds], &rect);
  }
  if (!ok) {
...
- (NSArray<NSValue *> *)actualMenuBarFrames {
  NSArray<NSDictionary *> *menuBarWindowInfos =
      [self menuBarWindowInfoDictionaries];
...
- (CGRect)expectedFrameOfVisibleMenuBarOnScreen:(NSScreen *)screenWithMenuBar {
  ...
  // Menu bar frames are flipped versus screen frames, and are relative to the
  // first screen. A menu bar y coordinate of 0 equals the top of the first
  // screen. A screen y coordinate of 0 equals the bottom of the first screen.
  menuBarFrame.origin.y = firstScreenFrame.size.height -
                          screenWithMenuBar.frame.size.height -
                          screenWithMenuBar.frame.origin.y;
...
- (NSArray<NSScreen *> *)screensWithMenuBars {
  ...
  NSArray<NSScreen *> *screensWithMenuBars =
      [screens filteredArrayUsingBlock:^BOOL(NSScreen *screen) {
        CGRect expectedFrame =
            [self expectedFrameOfVisibleMenuBarOnScreen:screen];
        return [actualFrames anyWithBlock:^BOOL(NSValue *value) {
          const NSRect actualMenuBarFrame = value.rectValue;
          return CGRectContainsPoint(expectedFrame, actualMenuBarFrame.origin);
        }];
      }];
  DLog(@"Screens with menu bars are %@ from all screens %@",
       screensWithMenuBars, screens);
