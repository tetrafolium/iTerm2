...
- (instancetype)initWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
  ...
  if (self) {
    ...
    _selectedIndexSet =
        [textView.selection selectedIndexesIncludingTabFillersInAbsoluteLine:
                                totalScrollbackOverflow + i];
...
- (instancetype)
  ...
  if (self) {
    ...
    _allowOtherMarkStyle =
        [iTermAdvancedSettingsModel showYellowMarkForJobStoppedBySignal];
...
- (iTermMetalPerFrameStateRow *)newRowForLine:(int)line {
  return [[iTermMetalPerFrameStateRow alloc]
        initWithDrawingHelper:_drawingHelper
                     textView:_textView
                       screen:_screen
