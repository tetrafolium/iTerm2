...
+ (NSArray<iTermTouchbarItem *> *)sortedTouchbarItemsInDictionary:
  ...
  keys = [keys sortedArrayUsingComparator:^NSComparisonResult(
                   NSString *_Nonnull key1, NSString *_Nonnull key2) {
...
+ (void)loadGlobalTouchBarMap {
  gTouchBarMappings =
      [[NSUserDefaults standardUserDefaults] objectForKey:@"GlobalTouchBarMap"];
  if (!gTouchBarMappings) {
    NSString *plistFile = [[NSBundle bundleForClass:[self class]]
        pathForResource:@"DefaultGlobalTouchBarMap"
                 ofType:@"plist"];
    gTouchBarMappings =
...
+ (void)setGlobalTouchBarMap:(NSDictionary *)src {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:gTouchBarMappings
                                            forKey:@"GlobalTouchBarMap"];
}
