...
- (BOOL)isApproximatelyEqual:(id)obj epsilon:(double)epsilon {
  ...
  for (NSString *key in self.allKeys) {
    ...
    if (![NSObject object:myValue
            isApproximatelyEqualToObject:otherValue
                                 epsilon:epsilon]) {
      return NO;
    }
  }
...
- (BOOL)isApproximatelyEqual:(id)obj epsilon:(double)epsilon {
  ...
  for (NSInteger i = 0; i < count; i++) {
    if (![NSObject object:self[i]
            isApproximatelyEqualToObject:other[i]
                                 epsilon:epsilon]) {
      return NO;
    }
  }
...
+ (BOOL)object:(__kindof NSObject *)a
    isApproximatelyEqualToObject:(__kindof NSObject *)b
...
- (iTermDelayedPerform *)performBlock:(void (^)(void))block
...
- (void)it_setAssociatedObject:(id)associatedObject forKey:(const void *)key {
  objc_setAssociatedObject(self, key, associatedObject,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)it_setWeakAssociatedObject:(id)associatedObject
...
- (id)it_performAutoreleasedObjectReturningSelector:(SEL)selector
                                         withObject:(id)object {
  IMP imp = [self methodForSelector:selector];
...
- (NSString *)it_invalidPathInPlist:(NSString *)path {
  ...
  if (array) {
    ...
    for (NSObject *obj in array) {
      NSString *oops =
          [obj it_invalidPathInPlist:[NSString stringWithFormat:@"%@array[%@]/",
                                                                path, @(i)]];
      if (oops) {
        return oops;
      }
  ...
  if (dictionary) {
    for (NSObject *key in dictionary) {
      NSString *oops = [key
          it_invalidPathInPlist:[NSString
                                    stringWithFormat:@"%@dict key=%@ class=%@",
                                                     path, key,
                                                     NSStringFromClass(
                                                         [key class])]];
      if (oops) {
      ...
      oops = [value
          it_invalidPathInPlist:[NSString stringWithFormat:@"%@dict[%@]/", path,
                                                           key]];
      if (oops) {
        return oops;
      }
  ...
  return [NSString stringWithFormat:@"%@ has type %@", path,
