...
NSString *const iTermApplicationCharacterPaletteWillOpen =
    @"iTermApplicationCharacterPaletteWillOpen";
NSString *const iTermApplicationCharacterPaletteDidClose =
    @"iTermApplicationCharacterPaletteDidClose";

NSString *const iTermApplicationInputMethodEditorDidOpen =
    @"iTermApplicationInputMethodEditorDidOpen";
NSString *const iTermApplicationInputMethodEditorDidClose =
    @"iTermApplicationInputMethodEditorDidClose";

NSString *const iTermApplicationWillShowModalWindow =
    @"iTermApplicationWillShowModalWindow";
NSString *const iTermApplicationDidCloseModalWindow =
    @"iTermApplicationDidCloseModalWindow";
...
- (void)it_windowDidOrderOnScreen:(NSNotification *)notification {
  ...
  if ([NSStringFromClass(object.class) isEqualToString:@"NSPanelViewBridge"]) {
    _it_imeOpen = YES;
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermApplicationInputMethodEditorDidOpen
                      object:nil];
  }
}
...
- (void)it_windowDidOrderOffScreen:(NSNotification *)notification {
  ...
  if ([NSStringFromClass(object.class) isEqualToString:@"NSPanelViewBridge"]) {
    _it_imeOpen = NO;
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermApplicationInputMethodEditorDidClose
                      object:nil];
  }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"modalWindow"]) {
    ...
    [self it_modalWindowDidChangeFrom:dict[NSKeyValueChangeOldKey]
                                   to:dict[NSKeyValueChangeNewKey]];
    return;
  }
}

- (void)it_modalWindowDidChangeFrom:(NSWindow *)oldValue
                                 to:(NSWindow *)newValue {
  DLog(@"modal window did change from %@ to %@", oldValue, newValue);
  if (oldValue == nil && newValue != nil) {
    _it_modalWindowOpen = YES;
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermApplicationWillShowModalWindow
                      object:nil];
    return;
  }
  if (oldValue != nil && newValue == nil) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermApplicationDidCloseModalWindow
                      object:nil];
  }
}
...
- (NSEvent *)eventByRemappingForSecureInput:(NSEvent *)event {
  if ([[iTermModifierRemapper sharedInstance] isAnyModifierRemapped] &&
      (IsSecureEventInputEnabled() ||
       ![[iTermModifierRemapper sharedInstance] isRemappingModifiers])) {
    // The event tap is not working, but we can still remap modifiers for
    // non-system keys. Only things like cmd-tab will not be remapped in this
    // case. Otherwise, the event tap performs the remapping.
...
- (BOOL)routeEventToShortcutInputView:(NSEvent *)event {
  ...
  if ([firstResponder isKindOfClass:[iTermShortcutInputView class]]) {
    iTermShortcutInputView *shortcutView =
        (iTermShortcutInputView *)firstResponder;
    if (shortcutView) {
      if (event.keyCode == iTermBogusVirtualKeyCode) {
        // You can't register a carbon hotkey for these so just ignore them when
...
- (BOOL)switchToWindowByNumber:(NSEvent *)event {
  ...
  const NSUInteger requirement = [iTermPreferences
      maskForModifierTag:[iTermPreferences
                             intForKey:kPreferenceKeySwitchWindowModifier]];
  if (maskedValue == requirement) {
    ...
    if (digit >= 1 && digit <= 9) {
      PseudoTerminal *termWithNumber =
          [[iTermController sharedInstance] terminalWithNumber:(digit - 1)];
      DLog(@"Switching windows");
      if (termWithNumber) {
        if ([termWithNumber isHotKeyWindow] &&
            [[termWithNumber window] alphaValue] < 1) {
          iTermProfileHotKey *hotKey = [[iTermHotKeyController sharedInstance]
              profileHotKeyForWindowController:termWithNumber];
          [[iTermHotKeyController sharedInstance]
              showWindowForProfileHotKey:hotKey
                                     url:nil];
        } else {
...
- (BOOL)switchToPaneInWindowController:(PseudoTerminal *)currentTerminal
                              byNumber:(NSEvent *)event {
  const int mask = NSEventModifierFlagShift | NSEventModifierFlagControl |
                   NSEventModifierFlagOption | NSEventModifierFlagCommand;
  if (([event it_modifierFlags] & mask) ==
      [iTermPreferences
          maskForModifierTag:[iTermPreferences
                                 intForKey:kPreferenceKeySwitchPaneModifier]]) {
    int digit = [self digitKeyForEvent:event];
    ...
    if (digit == 9 && numSessions > 0) {
      ...
      [currentTerminal.currentTab
          setActiveSession:[orderedSessions lastObject]];
      return YES;
    }
...
- (BOOL)switchToTabInTabView:(PTYTabView *)tabView byNumber:(NSEvent *)event {
  ...
  if (([event it_modifierFlags] & mask) ==
    ...
    if (digit == 9 && [tabView numberOfTabViewItems] > 0) {
      // Command (or selected modifier)+9: Switch to last tab if there are fewer
      // than 9.
...
- (BOOL)remapEvent:(NSEvent *)event
       inResponder:(NSResponder *)responder
    currentSession:(PTYSession *)currentSession {
  BOOL okToRemap = YES;
  ...
  if (okToRemap && [currentSession hasActionableKeyMappingForEvent:event]) {
    if (currentSession.copyMode &&
        [currentSession copyModeConsumesEvent:event]) {
      return NO;
    }
    // Remap key.
...
- (BOOL)handleKeypressInTerminalWindow:(NSEvent *)event {
  if ([[self keyWindow] isTerminalWindow]) {
    ...
    if ([self remapEvent:event
               inResponder:responder
...
- (BOOL)handleKeyDownEvent:(NSEvent *)event {
  DLog(@"Received KeyDown event: %@. Key window is %@. First responder is %@",
       event, [self keyWindow], [[self keyWindow] firstResponder]);
...
- (void)handleScrollWheelEvent:(NSEvent *)event {
  ...
  while (current) {
    if ([current
            respondsToSelector:@selector(it_wantsScrollWheelMomentumEvents)] &&
...
- (void)setIsUIElement:(BOOL)uiElement {
  ...
  TransformProcessType(&psn, uiElement
                                 ? kProcessTransformToUIElementApplication
                                 : kProcessTransformToForegroundApplication);
  if (uiElement) {
    // Gotta wait for a spin of the runloop or else it doesn't activate. That's
    // bad news when toggling the preference because all the windows disappear.
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSApplication sharedApplication] activateIgnoringOtherApps:YES];
    });

    if ([iTermAdvancedSettingsModel statusBarIcon]) {
      NSImage *image = [NSImage it_imageNamed:@"StatusItem"
                                     forClass:self.class];
      self.statusBarItem =
          [[NSStatusBar systemStatusBar] statusItemWithLength:image.size.width];
      _statusBarItem.button.title = @"";
      _statusBarItem.button.image = image;
      _statusBarItem.button.alternateImage =
          [NSImage it_imageNamed:@"StatusItemAlt" forClass:self.class];
      ((NSButtonCell *)_statusBarItem.button.cell).highlightsBy =
...
- (NSArray<NSWindow *> *)orderedWindowsPlusVisibleHotkeyPanels {
  NSArray<NSWindow *> *panels =
      [[iTermHotKeyController sharedInstance] visibleFloatingHotkeyWindows]
          ?: @[];
...
- (NSArray<NSWindow *> *)orderedWindowsPlusAllHotkeyPanels {
  NSArray<NSWindow *> *panels =
      [[iTermHotKeyController sharedInstance] allFloatingHotkeyWindows] ?: @[];
  return [panels arrayByAddingObjectsFromArray:[self orderedWindows]];
}
    ...
    observer = [[NSNotificationCenter defaultCenter]
        addObserverForName:NSApplicationDidBecomeActiveNotification
                ...
                usingBlock:^(NSNotification *_Nonnull note) {
                  DLog(@"Application did become active. Invoke completion "
                       @"block");
                  completion();
                  DLog(@"Application did become active completion block "
                       @"finished. Removing observer.");
                  [[NSNotificationCenter defaultCenter]
                      removeObserver:observer];
                }];
    // It's not clear how this differs from [self
    // activateIgnoringOtherApps:YES], but on 10.13 it does not cause previously
    // ordered-out windows to be ordered over other applications' windows. See
    // issue 6875.
...
- (void)orderFrontCharacterPalette:(id)sender {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermApplicationCharacterPaletteWillOpen
                    object:nil];
  ...
  [iTermWindowHacks
      pollForCharacterPanelToOpenOrCloseWithCompletion:^BOOL(BOOL open) {
        if (open && _it_characterPanelShouldOpenSoon) {
          ...
          [[NSNotificationCenter defaultCenter]
              postNotificationName:iTermApplicationCharacterPaletteDidClose
                            object:nil];
        }
        return open || ([NSDate timeIntervalSinceReferenceDate] <
                        deadlineToOpen); // keep running while open
      }];
}
