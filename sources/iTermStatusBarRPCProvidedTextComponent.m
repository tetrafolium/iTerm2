...
static NSString *const iTermStatusBarRPCRegistrationRequestKey =
...
- (NSDictionary *)statusBarConfiguration {
  return @{
    iTermStatusBarRPCRegistrationRequestKey : self.data,
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    NSData *data = [aDecoder decodeObjectOfClass:[NSData class]
                                          forKey:@"registrationRequest"];
    if (!data) {
      return nil;
    }
    _savedRegistrationRequest =
        [[ITMRPCRegistrationRequest alloc] initWithData:data error:nil];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:_savedRegistrationRequest.data
                forKey:@"registrationRequest"];
}

- (NSString *)componentDescription {
  return _savedRegistrationRequest.latestStatusBarRequest
...
- (NSDictionary *)defaultKnobs {
  ...
  [_savedRegistrationRequest.latestStatusBarRequest.statusBarComponentAttributes
          .knobsArray
      enumerateObjectsUsingBlock:^(
          ITMRPCRegistrationRequest_StatusBarComponentAttributes_Knob
              *_Nonnull obj,
          NSUInteger idx, BOOL *_Nonnull stop) {
        id value =
            [NSJSONSerialization it_objectForJsonString:obj.jsonDefaultValue];
        if (value) {
          knobs[obj.key] = value;
        }
      }];
...
- (id<iTermStatusBarComponent>)
  ...
  return [[iTermStatusBarRPCProvidedTextComponent alloc]
      initWithRegistrationRequest:_savedRegistrationRequest
                                      .latestStatusBarRequest
                            scope:scope
                            knobs:knobs];
}
...
- (instancetype)initWithRegistrationRequest:
  ...
  return [self initWithConfiguration:@{
    iTermStatusBarRPCRegistrationRequestKey : registrationRequest.data,
...
- (instancetype)
  ...
  if (self) {
    _savedRegistrationRequest = registrationRequest;
    iTermVariableRecordingScope *scope =
        [[iTermVariableRecordingScope alloc] initWithScope:self.scope];
    ...
    [_dependencies enumerateObjectsUsingBlock:^(
                       iTermVariableReference *_Nonnull obj, NSUInteger idx,
                       BOOL *_Nonnull stop) {
      obj.onChangeBlock = ^{
        [weakSelf updateWithKnobValues:
                      weakSelf.configuration
                          [iTermStatusBarComponentConfigurationKeyKnobValues]];
      };
    }];
    [self updateWithKnobValues:
              self.configuration
                  [iTermStatusBarComponentConfigurationKeyKnobValues]];
...
- (NSString *)invocation {
  ...
  ITMRPCRegistrationRequest_RPCArgument *knobs =
      [[ITMRPCRegistrationRequest_RPCArgument alloc] init];
  knobs.name = @"knobs";
  knobs.path = @"__knobs";
  return [iTermAPIHelper
      invocationWithFullyQualifiedName:_savedRegistrationRequest
                                           .latestStatusBarRequest
                                           .it_fullyQualifiedName
                              defaults:[defaults arrayByAddingObject:knobs]];
}

- (NSString *)statusBarComponentIdentifier {
  // Old (prerelease) ones did not have a unique identifier so assign one to
  // prevent disaster.
  return _savedRegistrationRequest.statusBarComponentAttributes.uniqueIdentifier
...
- (NSTextField *)newTextField {
  ...
  NSClickGestureRecognizer *recognizer =
      [[NSClickGestureRecognizer alloc] init];
...
- (nullable NSImage *)statusBarComponentIcon {
  if (_savedRegistrationRequest.latestStatusBarRequest
          .statusBarComponentAttributes.iconsArray.count == 0) {
    return nil;
  }
  if (_computedIcon) {
    return _icon;
  ...
  NSArray<NSBitmapImageRep *> *reps =
      [_savedRegistrationRequest.latestStatusBarRequest
              .statusBarComponentAttributes.iconsArray
          mapWithBlock:^id(
              ITMRPCRegistrationRequest_StatusBarComponentAttributes_Icon
                  *proto) {
            NSBitmapImageRep *rep =
                [[NSBitmapImageRep alloc] initWithData:proto.data_p];
            if (!rep) {
              return nil;
            }
            if (proto.scale <= 0) {
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  ...
  knobs =
      [_savedRegistrationRequest.latestStatusBarRequest
              .statusBarComponentAttributes.knobsArray
          mapWithBlock:^id(
              ITMRPCRegistrationRequest_StatusBarComponentAttributes_Knob
                  *descriptor) {
            return [[iTermStatusBarComponentKnob alloc]
                initWithLabelText:descriptor.name
                             type:[self knobTypeFromDescriptorType:descriptor
                                                                       .type]
                      placeholder:descriptor.hasPlaceholder
                                      ? descriptor.placeholder
                                      : nil
                     defaultValue:descriptor.hasJsonDefaultValue
                                      ? [NSJSONSerialization
                                            it_objectForJsonString:
                                                descriptor.jsonDefaultValue]
                                      : nil
                              key:descriptor.key];
          }]
          ?: @[];
...
- (void)updateWithKnobValues:(NSDictionary<NSString *, id> *)knobValues {
  ...
  if (!scope) {
    // This happens in the setup UI because the component is not attached to a
    // real session. To avoid spurious errors, do not actually evaluate the
    // invocation.
    return;
  }
  DLog(@"Update status bar component %@ instance %p for session %@\n%@",
       ...
       self, [scope valueForVariableName:iTermVariableKeySessionID],
       [NSThread callStackSymbols]);
  // Create a temporary frame to shadow __knobs in the scope. This avoids
  // mutating a scope we don't own.
  ...
  NSDictionary *knobsDict =
      weakSelf.configuration[iTermStatusBarComponentConfigurationKeyKnobValues]
          ?: @{};
  ...
  [iTermScriptFunctionCall
      callFunction:self.invocation
        ...
        completion:^(id value, NSError *error,
                     NSSet<NSString *> *missingFunctions) {
          DLog(@"evaluation of %@ completed with value %@ error %@",
               self.invocation, value, error);
          if (error) {
            [weakSelf handleEvaluationError:error
...
- (void)maybeOfferToMoveScriptToAutoLaunch {
  ...
  if ([iTermWarning
          showWarningWithTitle:
              @"This will move the script into the AutoLaunch folder."
                       actions:@[ @"OK", @"Cancel" ]
                     accessory:nil
                    identifier:
                        [NSString stringWithFormat:@"NoSyncAutoLaunchScript_%@",
                                                   _fullPath]
                   silenceable:kiTermWarningTypePermanentlySilenceable
                       heading:@"Always launch this script when iTerm2 starts?"
                        window:self.textField.window] ==
      kiTermWarningSelection0) {
...
- (void)handleSuccessfulEvaluationWithValue:(id)value {
  ...
  if (stringValue) {
    ...
    [[iTermAPIHelper sharedInstance]
        logToConnectionHostingFunctionWithSignature:_savedRegistrationRequest
                                                        .latestStatusBarRequest
...
- (void)handleEvaluationError:(NSError *)error
  ...
  _errorMessage = [NSString
      stringWithFormat:
          @"Status bar component “%@” (%@) failed.\n\nThis function call had "
          @"an error:\n\n%@\n\nThe error was:\n\n%@\n\n%@",
          _savedRegistrationRequest.latestStatusBarRequest
              .statusBarComponentAttributes.shortDescription,
          _savedRegistrationRequest.latestStatusBarRequest
...
- (void)didRegisterStatusBarComponent:(NSNotification *)notification {
  if (![notification.object
          isEqual:_savedRegistrationRequest.statusBarComponentAttributes
                      .uniqueIdentifier]) {
    return;
  }
...
- (void)registeredFunctionsDidChange:(NSNotification *)notification {
  NSArray<NSString *> *registered = [_missingFunctions.allObjects
      filteredArrayUsingBlock:^BOOL(NSString *signature) {
        return [[iTermAPIHelper sharedInstance]
            haveRegisteredFunctionWithSignature:signature];
      }];
  if (!registered.count) {
    return;
  }
...
- (NSTimeInterval)statusBarComponentUpdateCadence {
  if (_savedRegistrationRequest.latestStatusBarRequest
          .statusBarComponentAttributes.hasUpdateCadence) {
    return _savedRegistrationRequest.latestStatusBarRequest
...
- (BOOL)scriptIsRunning:(NSString *)fullPath {
  iTermScriptHistoryEntry *entry =
      [[iTermScriptHistory sharedInstance] runningEntryWithFullPath:fullPath];
  if (!entry) {
    return NO;
  }
...
- (BOOL)scriptIsNotRunningButCouldBeLaunched {
  ...
  return [[[[iTermApplication sharedApplication] delegate]
      scriptsMenuController] couldLaunchScriptWithAbsolutePath:fullPath];
}
...
- (nullable NSString *)fullPathOfScript {
  ...
  return
      [iTermAPIHelper nameOfScriptVendingStatusBarComponentWithUniqueIdentifier:
                          _savedRegistrationRequest.latestStatusBarRequest
                              .statusBarComponentAttributes.uniqueIdentifier];
}

- (void)launchScript {
  if (!_savedRegistrationRequest.statusBarComponentAttributes
           .uniqueIdentifier) {
    return;
  }
  NSString *fullPath = [self fullPathOfScript];
...
- (void)onClick:(id)sender {
  if (_errorMessage) {
    ...
    NSArray *actions = @[ [iTermWarningAction warningActionWithLabel:@"OK"
                                                               block:nil] ];
    if ([self scriptIsNotRunningButCouldBeLaunched]) {
      iTermWarningAction *launch = [iTermWarningAction
          warningActionWithLabel:@"Launch Script"
                           block:^(iTermWarningSelection selection) {
                             [self launchScript];
                           }];
      iTermWarningAction *reveal = [iTermWarningAction
          warningActionWithLabel:@"Reveal in Finder"
                           block:^(iTermWarningSelection selection) {
                             [self revealInFinder];
                           }];
      ...
      warning.title = [NSString
          stringWithFormat:@"%@It looks like the script is not running. "
                           @"Launching it might fix the problem.",
                           _errorMessage];
    }
  ...
  NSString *identifier = [[self.statusBarComponentIdentifier
      ...
      stringByReplacingOccurrencesOfString:@"-"
                                withString:@"_"];
  NSString *func =
      [NSString stringWithFormat:@"__%@__on_click(session_id: \"%@\")",
  ...
  [iTermScriptFunctionCall
      callFunction:func
        ...
        completion:^(id result, NSError *error, NSSet<NSString *> *mutations) {
          if (error) {
            NSString *message =
                [NSString stringWithFormat:@"Error in onclick handler: %@\n%@",
...
- (void)itermWebViewJavascriptError:(NSString *)errorText {
  NSError *error = nil;
  NSString *signature =
      [iTermExpressionParser signatureForFunctionCallInvocation:self.invocation
                                                          error:&error];
  if (!signature && error) {
