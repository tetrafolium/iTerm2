...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    searchField_ = [[iTermSearchField alloc]
        initWithFrame:NSMakeRect(0, 0, frame.size.width, 1)];
    ...
    if (@available(macOS 10.16, *)) {
      ...
      clear_.bordered = NO;
      clear_.image = [NSImage it_imageForSymbolName:@"trash"
    ...
    scrollView_ = [[NSScrollView alloc]
        initWithFrame:NSMakeRect(0, searchField_.frame.size.height + kMargin,
                                 frame.size.width,
    ...
    [tableView_ performSelector:@selector(scrollToEndOfDocument:)
                     withObject:nil
                     afterDelay:0];
  }
  return self;
...
- (void)relayout_bigSur {
  ...
  NSRect searchFieldFrame =
      NSMakeRect(0, 0,
                 frame.size.width - help_.frame.size.width -
                     clear_.frame.size.width - 2 * kMargin,
                 searchField_.frame.size.height);
  ...
  // Clear button
  {
    ...
    clear_.frame = NSMakeRect(
        help_.frame.origin.x - clear_.frame.size.width - kMargin, fudgeFactor,
  ...
  [scrollView_
      setFrame:NSMakeRect(0, searchFieldFrame.size.height + kMargin,
                          frame.size.width, frame.size.height - 2 * kMargin)];
...
- (void)relayout_legacy {
  ...
  [clear_ setFrame:NSMakeRect(0, frame.size.height - kButtonHeight,
                              ...
                              kButtonHeight)];
  help_.frame =
      NSMakeRect(NSMaxX(clear_.frame) + kHelpMargin,
                 NSMinY(clear_.frame) +
                     (NSHeight(clear_.frame) - NSHeight(help_.frame)) / 2.0 + 2,
                 NSWidth(help_.frame), NSHeight(help_.frame));
  scrollView_.frame =
      NSMakeRect(0, searchField_.frame.size.height + kMargin, frame.size.width,
                 frame.size.height - kButtonHeight - 2 * kMargin -
                     searchField_.frame.size.height);
  NSSize contentSize = [self contentSize];
...
- (id)stringOrAttributedStringForColumn:(NSTableColumn *)aTableColumn
  ...
  if ([[aTableColumn identifier] isEqualToString:@"date"]) {
    ...
    return [NSDateFormatter
        compactDateDifferenceStringFromDate:
            [NSDate dateWithTimeIntervalSinceReferenceDate:commandUse.time
                                                               .doubleValue]];
  } else {
    // Contents
    NSString *value =
        [commandUse.command stringByReplacingOccurrencesOfString:@"\n"
                                                      withString:@" "];
    ...
    if (commandUse.mark &&
        [wrapper.delegate.delegate
            toolbeltCurrentSessionHasGuid:commandUse.mark.sessionGuid]) {
      return [[NSAttributedString alloc]
          initWithString:value
              attributes:@{
                NSFontAttributeName : boldFont_,
...
- (id<NSPasteboardWriting>)tableView:(NSTableView *)tableView
  ...
  [pbItem setString:commandUse.command
            forType:(NSString *)kUTTypeUTF8PlainText];
  return pbItem;
}
...
- (void)clear:(id)sender {
  ...
  if ([alert runModal] == NSAlertFirstButtonReturn) {
    [[iTermShellHistoryController sharedInstance] eraseCommandHistory:YES
                                                          directories:NO];
  }
}
...
- (void)computeFilteredEntries {
  ...
  if (searchField_.stringValue.length == 0) {
    ...
    for (iTermCommandHistoryCommandUseMO *entry in entries) {
      if ([entry.command rangeOfString:searchField_.stringValue].location !=
          NSNotFound) {
...
- (void)help:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL
                  URLWithString:@"http://iterm2.com/shell_integration.html"]];
}
