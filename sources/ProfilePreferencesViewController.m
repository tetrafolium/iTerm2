...
static const CGFloat
    kExtraMarginBetweenWindowBottomAndTabViewForEditCurrentSessionMode = 7;
static const CGFloat kSideMarginsWithinInnerTabView = 11;
NSString *const kProfileSessionNameDidEndEditing =
    @"kProfileSessionNameDidEndEditing";
NSString *const kProfileSessionHotkeyDidChange =
    @"kProfileSessionHotkeyDidChange";

@interface ProfilePreferencesViewController () <
    iTermProfilePreferencesBaseViewControllerDelegate, NSTabViewDelegate,
    ProfileListViewDelegate, ProfilesGeneralPreferencesViewControllerDelegate>
@end
...
- (void)setPreferencePanel:
    (NSWindowController<iTermPreferencePanelSizing> *)preferencePanel {
  for (iTermPreferencesBaseViewController *viewController in
...
- (NSArray *)tabViewTuples {
  return @[
    @[ _generalTab, _generalViewController.view ],
...
- (void)awakeFromNib {
  [_profilesListView
      setUnderlyingDatasource:[_delegate profilePreferencesModel]];
  ...
  for (NSArray *tuple in tabViewTuples) {
    ...
    // Maximum allowed height for a tab view item. Taller ones get a scroll
    // view.
    ...
    if (view.frame.size.height > kMaxHeight) {
      ...
      sizeRememberingView.autoresizingMask =
...
- (void)refresh {
  ...
  [self reloadData];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kPreferencePanelDidUpdateProfileFields
                    object:nil
                  userInfo:nil];
}
...
- (void)layoutSubviewsForEditCurrentSessionMode {
  ...
  newFrame.size.height -=
      kExtraMarginBetweenWindowBottomAndTabViewForEditCurrentSessionMode;
  newFrame.origin.y +=
      kExtraMarginBetweenWindowBottomAndTabViewForEditCurrentSessionMode;
...
- (void)reloadProfileInProfileViewControllers {
  [[self tabViewControllers]
      makeObjectsPerformSelector:@selector(reloadProfile)];
}
...
- (void)openToProfileWithGuid:(NSString *)guid
  ...
  if (!self.view.window.attachedSheet) {
    ...
    [_sessionViewController
        configureStatusBarComponentWithIdentifier:identifier];
  }
}
...
- (void)openToProfileWithGuid:(NSString *)guid
  ...
  // Use safeMakeFirstResponder in case the profile name is not visible at the
  // time it is called. Prevents an annoying message threatening a crash that
  // never comes.
  [self.view.window performSelector:@selector(safeMakeFirstResponder:)
                         withObject:_generalViewController
                                        .profileNameFieldForEditCurrentSession
                         afterDelay:0];
}
...
- (void)resizeWindowForCurrentTabAnimated:(BOOL)animated {
  [self resizeWindowForTabViewItem:_tabView.selectedTabViewItem
                          animated:animated];
}
...
- (void)profileTableSelectionDidChange:(id)profileTable {
  [[self tabViewControllers]
      makeObjectsPerformSelector:@selector(willReloadProfile)];
  Profile *profile = [self selectedProfile];
  BOOL hasSelection = (profile != nil);

  _tabView.hidden = !hasSelection;
  _otherActionsPopup.enabled = hasSelection;
  _removeProfileButton.enabled =
      hasSelection && [_profilesListView numberOfRows] > 1;
...
- (void)profileTableTagsVisibilityDidChange:(ProfileListView *)profileListView {
  [_toggleTagsButton
      setTitle:profileListView.tagsVisible ? @"< Tags" : @"Tags >"];
}

#pragma mark - Private

- (BOOL)confirmProfileDeletion:(Profile *)profile {
  NSMutableString *question = [NSMutableString
      stringWithFormat:@"Delete profile %@?", profile[KEY_NAME]];
  if ([iTermWarning
          showWarningWithTitle:question
...
- (NSArray *)tabViewControllers {
  return @[
    _generalViewController, _colorsViewController, _textViewController,
    _windowViewController, _terminalViewController, _sessionViewController,
    _keysViewController, _advancedViewController
  ];
}
...
- (id<iTermSearchableViewController>)viewControllerWithOwnerIdentifier:
  ...
  return [[self tabViewControllers]
      objectPassingTest:^BOOL(id<iTermSearchableViewController> vc,
...
- (void)updateSubviewsForProfile:(Profile *)profile {
  ...
  if ([model numberOfBookmarks] < 2 || !profile) {
    ...
    _removeProfileButton.enabled =
        [[_profilesListView selectedGuids] count] < [model numberOfBookmarks];
  }
...
- (void)resizeWindowForView:(iTermSizeRememberingView *)theView
                   animated:(BOOL)animated {
  // The window's size includes all space around the tab view, plus the tab
  // view. These variables hold the space on each side of the tab view.
  ...
  CGSize tabViewSize;
  CGFloat preferredWidth = kSideMarginsWithinInnerTabView +
                           theView.originalSize.width +
                           kSideMarginsWithinInnerTabView;
  ...
  if (_profilesListView.isHidden) {
    ...
    spaceLeft =
        NSMaxX(_profilesListView.frame) + kSideMarginBetweenWindowAndTabView;
  }
  ...
  // Compute a window frame with the new size that preserves the top left
  // coordinate.
  NSWindow *window = self.view.window;
  NSPoint windowTopLeft =
      NSMakePoint(NSMinX(window.frame), NSMaxY(window.frame));
  NSRect frame = [window
      frameRectForContentRect:NSMakeRect(windowTopLeft.x, 0, contentSize.width,
                                         contentSize.height)];
  frame.origin.y = windowTopLeft.y - frame.size.height;
  frame.size.width = MAX([self.preferencePanel preferencePanelMinimumWidth]
                             ?: iTermPreferencePanelGetWindowMinimumWidth(NO),
                         frame.size.width);
...
- (IBAction)removeProfile:(id)sender {
  ...
  if (![ITAddressBookMgr canRemoveProfile:profile fromModel:model]) {
    ...
    // profileWasDeleted: gets called by notification from within
    // removeProfile:fromModel:.
...
- (void)profileWasDeleted:(NSNotification *)notification {
  ...
  // If a profile was deleted, update the shortcut titles that might refer to
  // it.
...
- (IBAction)addProfile:(id)sender {
  ...
  if (!prototype) {
    ...
    [newDict setValuesForKeysWithDictionary:[[_delegate profilePreferencesModel]
                                                defaultBookmark]];
  }
  ...
  [newDict
      removeObjectForKey:KEY_DEFAULT_BOOKMARK]; // remove deprecated attribute
                                                // with side effects
  newDict[KEY_TAGS] = @[];
...
- (IBAction)copyToProfile:(id)sender {
  ...
  Profile *destination =
      [[ProfileModel sharedInstance] bookmarkWithGuid:profileGuid];
  if (!destination) {
    return;
  }
  ...
  NSString *title = [NSString
      stringWithFormat:
          @"Replace profile “%@” with the current session's settings?",
          [iTermProfilePreferences stringForKey:KEY_NAME
                                      inProfile:destination]];
  if ([iTermWarning
...
- (IBAction)openCopyBookmarks:(id)sender {
  ...
  _bulkCopyController.keysForTerminal =
      [_terminalViewController keysForBulkCopy];
  ...
  _bulkCopyController.keysForAdvanced =
      [_advancedViewController keysForBulkCopy];
...
- (IBAction)duplicateProfile:(id)sender {
  ...
  NSMutableDictionary *newProfile =
      [NSMutableDictionary dictionaryWithDictionary:profile];
  NSString *newName =
      [NSString stringWithFormat:@"Copy of %@", newProfile[KEY_NAME]];
  ...
  newProfile[KEY_TAGS] =
      [newProfile[KEY_TAGS] arrayByRemovingObject:kProfileDynamicTag];
  newProfile[KEY_HAS_HOTKEY] =
      @NO; // Don't create another hotkey window with the same hotkey
...
- (NSString *)jsonForProfile:(Profile *)profile error:(NSError **)error {
  NSData *jsonData =
      [NSJSONSerialization dataWithJSONObject:[profile it_jsonSafeValue]
...
- (NSString *)jsonForAllProfilesWithErrorCount:(int *)errorCount {
  ...
  for (Profile *profile in [model bookmarks]) {
    ...
    if (json) {
      ...
      errors++;
      NSLog(@"Couldn't convert profile %@ to JSON: %@", profile[KEY_NAME],
      ...
      if (![self tryToImportJSONProfileFromURL:url error:&error]) {
        ...
        iTermWarningSelection selection = [iTermWarning
            showWarningWithTitle:
                [NSString stringWithFormat:@"Import from %@ failed: %@",
...
- (BOOL)tryToImportJSONProfileFromURL:(NSURL *)url error:(out NSError **)error {
  ...
  if (!dict) {
    if (error) {
      *error = [[NSError alloc]
          initWithDomain:@"com.iterm2.json-import-profile"
                    code:0
                userInfo:@{
                  NSLocalizedDescriptionKey :
                      @"Invalid JSON: does not have a top-level dictionary."
                }];
    }
    return NO;
  }
  ...
  if (!name || !guid) {
    ...
    if (profiles) {
      return [self tryToImportMultipleProfilesFromJSONObject:profiles
                                                       error:error];
    }
    if (error) {
      *error = [[NSError alloc] initWithDomain:@"com.iterm2.json-import-profile"
                                          code:0
                                      userInfo:@{
                                        NSLocalizedDescriptionKey :
                                            @"Invalid JSON: missing Name or "
                                            @"Guid key in top-level dictionary."
                                      }];
    }
    return NO;
  }
  ...
  if ([[ProfileModel sharedInstance] bookmarkWithGuid:guid]) {
    dict = [dict dictionaryBySettingObject:[[NSUUID UUID] UUIDString]
                                    forKey:KEY_GUID];
  }
...
- (BOOL)tryToImportMultipleProfilesFromJSONObject:(NSArray *)profiles
                                            error:(out NSError **)error {
  NSMutableArray<NSDictionary *> *dicts = [NSMutableArray array];
  __block BOOL ok = YES;
  __block NSError *internalError = nil;
  [profiles enumerateObjectsUsingBlock:^(id _Nonnull object, NSUInteger idx,
                                         BOOL *_Nonnull stop) {
    NSDictionary *dict = [NSDictionary castFrom:object];
    if (!dict) {
      internalError = [[NSError alloc]
          initWithDomain:@"com.iterm2.json-import-profile"
                ...
                userInfo:@{
                  NSLocalizedDescriptionKey :
                      @"Invalid JSON: not an array of dictionaries."
                }];
      ok = NO;
      *stop = YES;
      return;
    ...
    if (!name || !guid) {
      NSString *reason = [NSString
          stringWithFormat:
              @"Invalid JSON: missing Name or Guid key in profile number %@.",
              @(idx + 1)];
      internalError = [[NSError alloc]
          initWithDomain:@"com.iterm2.json-import-profile"
                    code:0
...
- (BOOL)saveString:(NSString *)string toPath:(NSString *)path {
  ...
  if (error) {
    ...
    alert.informativeText =
        [NSString stringWithFormat:@"Couldn't save to %@: %@", path,
                                   [error localizedDescription]];
    [alert runModal];
    return NO;
  }
...
- (IBAction)saveProfileAsJSON:(id)sender {
  ...
  iTermSavePanel *savePanel = [iTermSavePanel
       showWithOptions:0
            identifier:@"SaveProfile"
      initialDirectory:NSHomeDirectory()
       defaultFilename:[self.selectedProfile[KEY_NAME]
  ...
  if (!string) {
    ...
    alert.messageText = @"Error";
    alert.informativeText =
        [NSString stringWithFormat:@"Couldn't convert profile to JSON: %@",
                                   [error localizedDescription]];
    [alert runModal];
    return;
  }
  ...
  if ([self saveString:string toPath:savePanel.path]) {
    [[NSWorkspace sharedWorkspace]
        activateFileViewerSelectingURLs:@[ [NSURL
                                            fileURLWithPath:savePanel.path] ]];
  }
}
...
- (IBAction)saveAllProfilesAsJSON:(id)sender {
  ...
  if (errors) {
    ...
    alert.messageText = @"Error";
    alert.informativeText =
        [NSString stringWithFormat:@"Couldn't convert one or more profiles to "
                                   @"JSON. Check Console.app for errors."];
    [alert runModal];
    return;
  }
...
- (void)profilePreferencesViewController:
  ...
  _needsWarning = YES;
  __weak __typeof(self) weakSelf = self;
  if (viewController.delegate.profilePreferencesCurrentModel ==
...
- (void)showDynamicProfileWarning {
  ...
  NSString *message = [NSString
      stringWithFormat:
          @"The selected profile, “%@”, is a dynamic profile. Changes made "
          @"through preferences will be lost when you restart iTerm2. To "
          @"modify this profile you must edit its source file.",
          profileName];
...
- (void)profilesGeneralPreferencesNameDidEndEditing {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kProfileSessionNameDidEndEditing
                    object:[_profilesListView selectedGuid]];
}

- (void)profilesGeneralPreferencesSessionHotkeyDidChange {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kProfileSessionHotkeyDidChange
                    object:[_profilesListView selectedGuid]];
}
...
- (NSArray<iTermPreferencesSearchDocument *> *)
  ...
  return [[[self tabViewControllers]
      mapWithBlock:^id(id<iTermSearchableViewController> vc) {
