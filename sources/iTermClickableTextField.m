...
- (void)mouseUp:(NSEvent *)event {
  ...
  NSTextContainer *textContainer =
      [[NSTextContainer alloc] initWithContainerSize:self.bounds.size];
  NSLayoutManager *layoutManager = [[NSLayoutManager alloc] init];
  [layoutManager addTextContainer:textContainer];
  [textStorage addLayoutManager:layoutManager];

  NSInteger index = [layoutManager characterIndexForPoint:point
                                          inTextContainer:textContainer
                 fractionOfDistanceBetweenInsertionPoints:nil];
  if (index >= 0 && index < self.attributedStringValue.length) {
    NSDictionary *attributes =
        [self.attributedStringValue attributesAtIndex:index effectiveRange:nil];
