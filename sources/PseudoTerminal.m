...
NSString *const kTerminalWindowControllerWasCreatedNotification =
    @"kTerminalWindowControllerWasCreatedNotification";
NSString *const iTermDidDecodeWindowRestorableStateNotification =
    @"iTermDidDecodeWindowRestorableStateNotification";
NSString *const iTermTabDidChangePositionInWindowNotification =
    @"iTermTabDidChangePositionInWindowNotification";
...
NSString *const iTermDidCreateTerminalWindowNotification =
    @"iTermDidCreateTerminalWindowNotification";
...
static NSString *const TERMINAL_ARRANGEMENT_EDGE_SPANNING_OFF =
    @"Edge Spanning Off"; // Deprecated. Included in window type now.
...
static NSString *const TERMINAL_ARRANGEMENT_SAVED_WINDOW_TYPE =
    @"Saved Window Type"; // Only relevant for fullscreen
static NSString *const TERMINAL_ARRANGEMENT_SELECTED_TAB_INDEX =
    @"Selected Tab Index";
static NSString *const TERMINAL_ARRANGEMENT_SCREEN_INDEX = @"Screen";
static NSString *const TERMINAL_ARRANGEMENT_HIDE_AFTER_OPENING =
    @"Hide After Opening";
static NSString *const TERMINAL_ARRANGEMENT_DESIRED_COLUMNS =
    @"Desired Columns";
static NSString *const TERMINAL_ARRANGEMENT_DESIRED_ROWS = @"Desired Rows";
static NSString *const TERMINAL_ARRANGEMENT_IS_HOTKEY_WINDOW =
    @"Is Hotkey Window";
static NSString *const TERMINAL_ARRANGEMENT_INITIAL_PROFILE =
...
static NSString
    *const TERMINAL_ARRANGEMENT_HIDING_TOOLBELT_SHOULD_RESIZE_WINDOW =
        @"Hiding Toolbelt Should Resize Window";
static NSString *const TERMINAL_ARRANGEMENT_USE_TRANSPARENCY =
    @"Use Transparency";
static NSString *const TERMINAL_ARRANGEMENT_TOOLBELT_PROPORTIONS =
    @"Toolbelt Proportions";
...
static NSRect iTermRectCenteredHorizontallyWithinRect(NSRect frameToCenter,
                                                      NSRect container) {
...
static NSRect iTermRectCenteredVerticallyWithinRect(NSRect frameToCenter,
                                                    NSRect container) {
...
@interface PseudoTerminal () <
    iTermBroadcastInputHelperDelegate, iTermGraphCodable, iTermObject,
    iTermTabBarControlViewDelegate, iTermPasswordManagerDelegate,
    iTermUniquelyIdentifiable, PTYTabDelegate, iTermRootTerminalViewDelegate,
    iTermToolbeltViewDelegate, NSComboBoxDelegate, PSMMinimalTabStyleDelegate>
...
@property(nonatomic, readonly)
    iTermSwiftyString *windowTitleOverrideSwiftyString;
@end
...
@implementation PseudoTerminal {
  ...
  // For top/left/bottom of screen windows, this is the size it really wants to
  // be. Initialized to -1 in -init and then set to the size of the first
  // session forever.
  int desiredRows_, desiredColumns_;

  // If set, then hiding the toolbelt should shrink the window by the toolbelt's
  // width.
  BOOL hidingToolbeltShouldResizeWindow_;

  // If set, prevents hidingToolbeltShouldResizeWindow_ from getting its value
  // inferred based on the window's frame.
  BOOL hidingToolbeltShouldResizeWindowInitialized_;

  iTermTabBarAccessoryViewController *
      _titleBarAccessoryTabBarViewController NS_AVAILABLE_MAC(10_14);

  // Number of tabs since last change.
  NSInteger _previousNumberOfTabs;

  // The window restoration completion block was called but
  // windowDidDecodeRestorableState: has not yet been called.
  ...
  // This is true if the user is dragging the window by the titlebar. It should
  // not be set for programmatic moves or moves because of disconnecting a
  // display.
  BOOL _windowIsMoving;
  NSInteger _screenBeforeMoving;
  BOOL _constrainFrameAfterDeminiaturization;

  // Size of the last grid size shown in the transient window title, or 0,0 for
  // never shown before.
  VT100GridSize _previousGridSize;
  // Have we started showing a transient title? If so, don't stop until time
  // runs out.
  ...
  // Work around a macOS bug. If you set the window's appearance to light when
  // creating a new lion full screen window while a lion full screen window is
  // key then a bogus black window is created on the primary display. Issue
  // 8842.
  ...
  // When restoring an arrangement with lots of tabs, updating object counts is
  // slow because it adds and removes tracking rects for each tab, which its
  // itself slow. This is an optimization to only update object counts once when
  // creating gobs of tabs at once.
...
+ (Profile *)expurgatedInitialProfile:(Profile *)profile {
  // We don't care about almost all the keys in the profile, so don't waste
  // space and privacy storing them.
  return [profile ?: @{} dictionaryKeepingOnlyKeys:@[
    KEY_CUSTOM_WINDOW_TITLE, KEY_USE_CUSTOM_WINDOW_TITLE, KEY_DISABLE_AUTO_FRAME
  ]];
}
...
+ (int)screenNumberForPreferredScreenNumber:(int)screenNumber
  ...
  if ((windowType == WINDOW_TYPE_TRADITIONAL_FULL_SCREEN ||
    ...
    if (n == NSNotFound) {
      DLog(@"Convert default screen to screen number: No screen matches the "
           @"window's screen so using main screen");
      return 0;
    } else {
      DLog(@"Convert default screen to screen number: System chose screen %lu",
           (unsigned long)n);
      return n;
    }
...
- (NSScreen *)anchorToScreenNumber:(int)screenNumber {
  ...
  if (screenNumber == -1 || screenNumber >= [[NSScreen screens] count]) {
    ...
    DLog(@"Screen number %d is out of range [0,%d] so using 0", screenNumber,
...
- (void)
    finishInitializationWithSmartLayout:(BOOL)smartLayout
  ...
  iTermWindowType savedWindowType =
      iTermThemedWindowType(unsafeSavedWindowType);
  DLog(@"-[%p finishInitializationWithSmartLayout:%@ windowType:%d screen:%d "
       @"hotkeyWindowType:%@ ",
       self, smartLayout ? @"YES" : @"NO", windowType, screenNumber,
       @(hotkeyWindowType));

  _variables = [[iTermVariables alloc]
      initWithContext:iTermVariablesSuggestionContextWindow
                owner:self];
  _variables.primaryKey = iTermVariableKeyWindowID;
  _scope = [iTermVariableScope
      newWindowScopeWithVariables:self.variables
                     tabVariables:
                         [[[iTermVariables alloc]
  ...
  screenNumber = [PseudoTerminal
      screenNumberForPreferredScreenNumber:screenNumber
                                windowType:windowType
  ...
  if (windowType == WINDOW_TYPE_NORMAL) {
    // If you create a window with a minimize button and the menu bar is hidden
    // then the minimize button is disabled. Currently the only window type with
    // a miniaturize button is NORMAL.
  ...
  case WINDOW_TYPE_TRADITIONAL_FULL_SCREEN:
    ...
    // By setting this flag, we'll use the window size necessary to hold the
    // current session's rows/columns setting when exiting fullscreen.
  ...
  case WINDOW_TYPE_ACCESSORY:
    ...
    if (_isAnchoredToScreen) {
      ...
      DLog(@"Assigned screen has origin %@, destination screen has origin %@",
           NSStringFromPoint(baseScreen.visibleFrame.origin),
      ...
      double xover = destPoint.x + initialFrame.size.width -
                     (destScreenFrame.origin.x + destScreenFrame.size.width);
      double yover = destPoint.y + initialFrame.size.height -
                     (destScreenFrame.origin.y + destScreenFrame.size.height);
      if (xover > 0) {
        destPoint.x -= xover;
      }
      if (yover > 0) {
      ...
      DLog(@"after adjusting top right, initial origin is %@",
           NSStringFromPoint(destPoint));
  ...
  if (savedWindowType == WINDOW_TYPE_LION_FULL_SCREEN ||
    ...
    PtyLog(@"Downgraded saved window type from fullscreen to %@",
           @(savedWindowType));
  }
  ...
  if (windowType == WINDOW_TYPE_LION_FULL_SCREEN) {
    // We want to set the style mask to the window's non-fullscreen appearance
    // so we're prepared to exit fullscreen with the right style.
  ...
  DLog(@"initWithContentRect:%@ styleMask:%d",
       [NSValue valueWithRect:initialFrame], (int)styleMask);
  // This is necessary to do here because the collection behavior is computed in
  // setWindowWithWindowType, and the window may become full screen thereafter.
  // That's no good for a hotkey window.to
  _hotkeyWindowType = hotkeyWindowType;
  [self setWindowWithWindowType:windowType
                savedWindowType:savedWindowType
         windowTypeForStyleMask:(windowType ==
                                 WINDOW_TYPE_TRADITIONAL_FULL_SCREEN)
                                    ? windowType
                                    : savedWindowType
               hotkeyWindowType:hotkeyWindowType
  ...
  _contentView = [[[iTermRootTerminalView alloc]
  ...
  // hidingToolbeltShouldResizeWindow_ can only be set to the right value after
  // the window's frame has been established. The window is always fiddled with
  // (e.g., adding tabs) after this call returns, so we'll do it on the next
  // spin of the runloop.
  [self performSelector:@selector(finishToolbeltInitialization)
             withObject:nil
             afterDelay:0];
  ...
  useTransparency_ =
      [iTermProfilePreferences boolForKey:KEY_INITIAL_USE_TRANSPARENCY
                                inProfile:profile];
  ...
  if (@available(macOS 10.14, *)) {
    if ([self.window respondsToSelector:@selector
      ...
      _shortcutAccessoryViewController =
          [[iTermWindowShortcutLabelTitlebarAccessoryViewController alloc]
              initWithNibName:@"iTermWindowShortcutAccessoryView"
  ...
  if ([iTermProfilePreferences boolForKey:KEY_USE_CUSTOM_WINDOW_TITLE
                                inProfile:profile]) {
    NSString *override =
        [iTermProfilePreferences stringForKey:KEY_CUSTOM_WINDOW_TITLE
                                    inProfile:profile];
    ;
    [self.scope setValue:override.length ? override : @" "
        forVariableNamed:iTermVariableKeyWindowTitleOverrideFormat];
  }

  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTerminalWindowControllerWasCreatedNotification
                    object:self];

  _windowTitleOverrideSwiftyString = [[iTermSwiftyString alloc]
        initWithScope:self.scope
           sourcePath:iTermVariableKeyWindowTitleOverrideFormat
      destinationPath:iTermVariableKeyWindowTitleOverride];
  _windowTitleOverrideSwiftyString.observer =
...
- (BOOL)isFloatingHotKeyWindow {
  return self.isHotKeyWindow &&
         [[[iTermHotKeyController sharedInstance]
             profileHotKeyForWindowController:self] floats];
}
...
- (NSWindowCollectionBehavior)desiredWindowCollectionBehavior {
  ...
  case iTermHotkeyWindowTypeFloatingWindow: {
    ...
    BOOL excludeFromCycling =
        [iTermAdvancedSettingsModel hotkeyWindowsExcludedFromCycling];
    if (!excludeFromCycling) {
      iTermProfileHotKey *profileHotKey =
          [[iTermHotKeyController sharedInstance]
              profileHotKeyForWindowController:self];
...
- (void)finishToolbeltInitialization {
  // If the right edge of the window is "near" the right edge of the screen,
  // then hiding an initially visible toolbelt should not resize the window, the
  // theory being that the user wanted the window to be near the right edge of
  // the screen. This will probably sow confusion but may cause more good than
  // harm. We'll see how loud they yell.
  if (!hidingToolbeltShouldResizeWindowInitialized_) {
    BOOL rightEdgeOfWindowIsNearRightEdgeOfScreen;
    CGFloat distanceFromRightEdgeOfWindowToRightEdgeOfScreen = fabs(
        NSMaxX(self.window.frame) - NSMaxX(self.window.screen.visibleFrame));
    rightEdgeOfWindowIsNearRightEdgeOfScreen =
        (distanceFromRightEdgeOfWindowToRightEdgeOfScreen < 10);
    hidingToolbeltShouldResizeWindow_ =
        !rightEdgeOfWindowIsNearRightEdgeOfScreen;

    // This isn't really necessary since this method isn't called a second time,
    // but just to be safe we'll set it.
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p tabs=%d window=%@>", [self class],
                                 self, (int)[self numberOfTabs], [self window]];
}

- (BOOL)tabBarVisibleOnTopEvenWhenOnLoan {
  return ([self tabBarShouldBeVisibleEvenWhenOnLoan] &&
          [iTermPreferences intForKey:kPreferenceKeyTabPosition] ==
              PSMTab_TopTab);
}
...
- (BOOL)divisionViewShouldBeVisible {
  // The division is only shown if there is a title bar and no tab bar on top.
  // There are cases in fullscreen (e.g., when entering Lion fullscreen) when
  // the window doesn't have a title bar but also isn't borderless we also check
  ...
  return ([iTermPreferences boolForKey:kPreferenceKeyEnableDivisionView] &&
          preferredStyle != TAB_STYLE_MINIMAL && !togglingFullScreen_ &&
          (self.window.styleMask & NSWindowStyleMaskTitled) &&
          ![self titleBarShouldAppearTransparent] && ![self anyFullScreen] &&
...
- (BOOL)windowIsResizing {
  return togglingFullScreen_ || liveResize_ || togglingLionFullScreen_ ||
         exitingLionFullscreen_ || zooming_;
}
...
- (BOOL)showToolbeltNotFullScreen {
  ...
  if (overage > 0) {
    // Compensate by making the toolbelt a little smaller, unless that would
    // make it too small.
    if (_contentView.toolbeltWidth - overage > 100) {
      ...
      const CGFloat proposedToolbeltWidth =
          _contentView.toolbeltWidth - overage;
      const CGFloat desiredNonToolbeltWidth =
          windowFrame.size.width - proposedToolbeltWidth;
      _contentView.toolbeltWidth =
          MIN([_contentView maximumToolbeltWidthForViewWidth:viewWidth],
              proposedToolbeltWidth);
      windowFrame.size.width =
          desiredNonToolbeltWidth + _contentView.toolbeltWidth;
      overage = 0;
    }
  }
...
- (void)tmuxFontDidChange:(NSNotification *)notification {
  ...
  if ([[self uniqueTmuxControllers] count]) {
    if ([self.tabs anyWithBlock:^BOOL(PTYTab *tab) {
          return [tab.sessions containsObject:session] ||
                 !tab.tmuxController.variableWindowSize;
        }]) {
...
- (NSWindowController<iTermWindowController> *)
    terminalDraggedFromAnotherWindowAtPoint:(NSPoint)point {
  PseudoTerminal *term;
...
- (NSScreen *)screen {
  ...
  if (screens.count == 0) {
    ...
    DLog(@"Not anchored, or anchored screen does not exist. Using current "
         @"screen.");
    return self.window.screen;
  } else if (_screenNumberFromFirstProfile >= 0 &&
             _screenNumberFromFirstProfile < screens.count) {
    DLog(@"Using screen number from first profile %d",
         _screenNumberFromFirstProfile);
    ...
    DLog(@"Using first screen because screen number from first profile is %d",
         _screenNumberFromFirstProfile);
...
- (void)magnifyWithEvent:(NSEvent *)event {
  ...
  if ([[NSDate date]
          timeIntervalSinceDate:
              [NSDate dateWithTimeIntervalSince1970:lastMagChangeTime_]] >
      kMagTimeout) {
    cumulativeMag_ = 0;
  }
...
- (void)tabSessionDidChangeTransparency:(PTYTab *)tab {
  // In case the last pane just becamse opaque, we can drop the visual effect
  // view in the fake window title bar.
...
- (NSColor *)terminalWindowDecorationControlColor {
  ...
  if (self.shouldUseMinimalStyle) {
    ...
    return [color colorDimmedBy:[iTermAdvancedSettingsModel
                                    minimalSplitPaneDividerProminence]
...
- (NSColor *)terminalWindowDecorationTextColorForBackgroundColor:
  ...
  return [[iTermTheme sharedInstance]
      terminalWindowDecorationTextColorForBackgroundColor:backgroundColor
                                      effectiveAppearance:
                                          self.window.effectiveAppearance
                                                 tabStyle:_contentView
                                                              .tabBarControl
                                                              .style
                                            mainAndActive:(self.window
                                                               .isMainWindow &&
                                                           NSApp.isActive)];
}
...
- (BOOL)confirmCloseForSessions:(NSArray *)sessions
  ...
  for (PTYSession *aSession in sessions) {
    if (![aSession exited]) {
      names = [aSession.childJobNameTuples
          mapWithBlock:^id(iTermTuple<NSString *, NSString *> *tuple) {
  ...
  if ([sortedNames count] == 1) {
    message = [NSString stringWithFormat:@"%@ is running %@.", identifier,
                                         [sortedNames objectAtIndex:0]];
  } else if ([sortedNames count] > 1 && [sortedNames count] <= 10) {
    message = [NSString
        stringWithFormat:@"%@ is running the following jobs: %@.", identifier,
                         [sortedNames componentsJoinedWithOxfordComma]];
  } else if ([sortedNames count] > 10) {
    message = [NSString
        stringWithFormat:@"%@ is running the following jobs: %@, plus %ld %@.",
                         identifier,
                         [sortedNames componentsJoinedWithOxfordComma],
  ...
  // The PseudoTerminal might close while the dialog is open so keep it around
  // for now.
...
- (BOOL)confirmCloseTab:(PTYTab *)aTab
    suppressConfirmation:(BOOL)suppressConfirmation {
  if ([_contentView.tabView indexOfTabViewItemWithIdentifier:aTab] ==
      NSNotFound) {
    return NO;
  }
  ...
  if (mustAsk && !suppressConfirmation) {
    ...
    if (numClosing == 1) {
      okToClose = [self
          confirmCloseForSessions:[aTab sessions]
                       identifier:@"This tab"
...
- (void)close {
  if (self.swipeIdentifier) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermSwipeHandlerCancelSwipe
...
- (BOOL)tabIsAttachedTmuxTabWithSessions:(PTYTab *)aTab {
  return ([aTab isTmuxTab] && [[aTab sessions] count] > 0 &&
...
- (void)closeTab:(PTYTab *)aTab soft:(BOOL)soft {
  if (!soft && [self tabIsAttachedTmuxTabWithSessions:aTab]) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            ...
            @"Hidden windows may be restored from the tmux dashboard."
               ...
               actionMapping:@[
                 @(kiTermWarningSelection0), @(kiTermWarningSelection2),
                 @(kiTermWarningSelection1)
               ]
                   accessory:nil
...
- (iTermRestorableSession *)restorableSessionForSession:(PTYSession *)session {
  ...
  DLog(@"Create restorable session with terminal guid %@",
       restorableSession.terminalGuid);
  return restorableSession;
}

- (void)storeWindowStateInRestorableSession:
    (iTermRestorableSession *)restorableSession {
  restorableSession.windowType =
      self.lionFullScreen ? WINDOW_TYPE_LION_FULL_SCREEN : self.windowType;
  ...
  if (![aTab isTmuxTab]) {
    ...
    if (restorableSession.arrangement) {
      [[iTermController sharedInstance]
          pushCurrentRestorableSession:restorableSession];
    }
    for (PTYSession *session in [aTab sessions]) {
      [session terminate];
    }
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermTabDidCloseNotification
                      object:aTab];
  } else {
    for (PTYSession *session in [aTab sessions]) {
      [session terminate];
...
- (IBAction)toggleSelectionRespectsSoftBoundaries:(id)sender {
  ...
  controller.selectionRespectsSoftBoundaries =
      !controller.selectionRespectsSoftBoundaries;
}
...
- (void)activeSpaceDidChange:(NSNotification *)notification {
  DLog(@"Active space did change. active=%@ self.window.isOnActiveSpace=%@",
       @(NSApp.isActive), @(self.window.isOnActiveSpace));
  if ([(iTermApplication *)NSApp isUIElement] && !NSApp.isActive &&
      self.lionFullScreen && self.window.isOnActiveSpace) {
    DLog(@"Activating app because lion full screen window is on active space. "
         @"%@",
         self);
...
- (BOOL)closeTabIfConfirmed:(PTYTab *)tab {
  const BOOL shouldClose =
      [self tabView:_contentView.tabView
...
- (IBAction)closeCurrentSession:(id)sender {
  ...
  if ([[self window] isKeyWindow]) {
    PTYSession *aSession =
        [[[_contentView.tabView selectedTabViewItem] identifier] activeSession];
...
- (BOOL)closeSessionWithConfirmation:(PTYSession *)aSession {
  ...
  if ([aSession exited]) {
    ...
    okToClose = [self
        confirmCloseForSessions:[NSArray arrayWithObject:aSession]
...
- (void)restartSessionWithConfirmation:(PTYSession *)aSession {
  ...
  if (aSession.exited) {
    ...
    iTermWarningAction *cancel =
        [iTermWarningAction warningActionWithLabel:@"Cancel" block:nil];
    iTermWarningAction *ok = [iTermWarningAction
        warningActionWithLabel:@"OK"
...
// For a million years macOS did not have tabs. So I made my own half-assed
// tabs. Then they finally got around to adding tabs and, even though this app
// opts out of their tabs (because they can't do half of what I need), they
// still add garbage disabled menu items to the window menu that conflict with
// longstanding shortcuts. I can't seem to prevent "Show Next/Previous Tab" from
// being added to the menu, and I already have the previously-standard shortcuts
// to switch tabs in the menu (as cmd-shift [ and ]) which I cannot change. So
// rather than have dead menu items I implement these methods to do what the
// default shortcut has always done. Of course this is different than the
// standard for macOS apps but I can't very well remove the existing menu items
// since people are used to their shortcuts; I don't want to remove them from
// the menu because people may have "perform menu item" keyboard shortcuts; and
...
- (void)setWindowTitle {
  if (self.isShowingTransientTitle) {
    ...
    if (self.window.frame.size.width < 250) {
      aTitle =
          [NSString stringWithFormat:@"%d✕%d", session.columns, session.rows];
    } else {
      NSString *detail = [self rootTerminalViewWindowSizeViewDetailString];
      aTitle = [NSString
          stringWithFormat:@"%@ \u2014 %d✕%d%@", [self undecoratedWindowTitle],
...
- (void)setWindowTitle:(NSString *)title {
  ...
  if (_deallocing) {
    // This uses -weakSelf and can be called during dealloc. Doing so is a
    // crash.
    return;
  }
  if (title == nil) {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyShowWindowNumber]) {
    ...
    titleExWindowNumber =
        [NSString stringWithFormat:@"%@%@", titleExWindowNumber, tmuxId];
    [self.contentView windowNumberDidChangeTo:@(number_ + 1)];
  } else {
    [self.contentView windowNumberDidChangeTo:nil];
  }
  if ((self.numberOfTabs == 1) &&
      (self.tabs.firstObject.state & kPTYTabBellState) &&
  ...
  if (liveResize_) {
    // During a live resize this has to be done immediately because the runloop
    // doesn't get around to delayed performs until the live resize is done (bug
    // 2812).
  ...
  // In bug 3957, we see that GNU screen is buggy and sends a crazy number of
  // title changes. We want to coalesce them to avoid the title flickering like
  // mad. Also, setting the title seems to be relatively slow, so we don't want
  // to spend too much time doing that if the terminal goes nuts and sends lots
  // of title-change sequences.
  ...
  if (!hadTimer) {
    if (!_windowWasJustCreated && ![self.ptyWindow titleChangedRecently]) {
      // Unless the window was just created, set the title immediately. Issue
      // 5876.
    ...
    dispatch_after(
        dispatch_time(
            DISPATCH_TIME_NOW,
            (int64_t)(iTermWindowTitleChangeMinimumInterval * NSEC_PER_SEC)),
        dispatch_get_main_queue(), ^{
          if (!(weakSelf.window.title == weakSelf.desiredTitle ||
                [weakSelf.window.title
                    isEqualToString:weakSelf.desiredTitle])) {
...
- (NSArray<PTYSession *> *)broadcastSessions {
  ...
  return [[_broadcastInputHelper currentDomain].allObjects
      mapWithBlock:^id(NSString *guid) {
        PTYSession *session =
            [[iTermController sharedInstance] sessionWithGUID:guid];
...
- (void)sendInputToAllSessions:(NSString *)string
  ...
  for (PTYSession *aSession in [self broadcastSessions]) {
    if (![aSession isTmuxGateway]) {
      [aSession writeTaskNoBroadcast:string
...
- (BOOL)broadcastInputHelper:(iTermBroadcastInputHelper *)helper
    tabWithSessionIsBroadcasting:(NSString *)sessionID {
  PTYSession *session =
      [[iTermController sharedInstance] sessionWithGUID:sessionID];
  if (!session) {
    return NO;
  }
  PTYTab *tab = [self tabForSession:session];
...
+ (iTermWindowType)_windowTypeForArrangement:(NSDictionary *)arrangement {
  ...
  if ([arrangement objectForKey:TERMINAL_ARRANGEMENT_WINDOW_TYPE]) {
    windowType = iTermThemedWindowType(
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_WINDOW_TYPE] intValue]);
  } else {
    if ([arrangement objectForKey:TERMINAL_ARRANGEMENT_FULLSCREEN] &&
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_FULLSCREEN]
            boolValue]) {
      windowType = WINDOW_TYPE_TRADITIONAL_FULL_SCREEN;
    } else if ([[arrangement objectForKey:TERMINAL_ARRANGEMENT_LION_FULLSCREEN]
                   boolValue]) {
      windowType = WINDOW_TYPE_LION_FULL_SCREEN;
    } else {
...
+ (int)_screenIndexForArrangement:(NSDictionary *)arrangement {
  return
      [[arrangement objectForKey:TERMINAL_ARRANGEMENT_SCREEN_INDEX] intValue];
}

+ (void)drawArrangementPreview:(NSDictionary *)terminalArrangement
                  screenFrames:(NSArray *)frames {
  int windowType =
      [PseudoTerminal _windowTypeForArrangement:terminalArrangement];
  int screenIndex =
      [PseudoTerminal _screenIndexForArrangement:terminalArrangement];
  ...
  case WINDOW_TYPE_NORMAL:
    rect.origin.x =
        xOrigin +
        xScale * ([[terminalArrangement
                      objectForKey:TERMINAL_ARRANGEMENT_X_ORIGIN] doubleValue] -
                  screenFrame.origin.x);
    double h = [[terminalArrangement objectForKey:TERMINAL_ARRANGEMENT_HEIGHT]
        doubleValue];
    double y = [[terminalArrangement objectForKey:TERMINAL_ARRANGEMENT_Y_ORIGIN]
                   doubleValue] -
    ...
    rect.size.width =
        xScale * [[terminalArrangement objectForKey:TERMINAL_ARRANGEMENT_WIDTH]
                     doubleValue];
  ...
  case WINDOW_TYPE_TOP:
    ...
    rect.size.height =
        yScale * [[terminalArrangement objectForKey:TERMINAL_ARRANGEMENT_HEIGHT]
                     doubleValue];
    break;
  ...
  int N = [(NSDictionary *)[terminalArrangement
      objectForKey:TERMINAL_ARRANGEMENT_TABS] count];
  [[NSColor windowFrameColor] set];
  NSRect tabsRect =
      NSMakeRect(rect.origin.x + 1, rect.origin.y, rect.size.width - 2, 10);
  ...
  NSRect tabRect = NSMakeRect(tabsRect.origin.x + 1, tabsRect.origin.y + 1,
                              tabSize.width, tabSize.height);
  for (int i = 0; i < N; i++) {
    if (NSMaxY(tabRect) > NSMaxY(rect) || NSMaxX(tabRect) > NSMaxX(rect)) {
      break;
    }
    NSRectFill(tabRect);
  ...
  NSDictionary *tabArrangement =
      terminalArrangement[TERMINAL_ARRANGEMENT_TABS][0];
  NSRect contentRect = NSMakeRect(rect.origin.x + 1, rect.origin.y,
                                  rect.size.width - 2, rect.size.height - 11);
  switch ([iTermPreferences intForKey:kPreferenceKeyTabPosition]) {
  case PSMTab_BottomTab:
    break;
  case PSMTab_TopTab:
  ...
  [PTYTab drawArrangementPreview:tabArrangement frame:contentRect];
...
// Indicates if a newly created window will automatically enter Lion full
// screen.
+ (BOOL)willAutoFullScreenNewWindow {
  PseudoTerminal *keyWindow =
      [[iTermController sharedInstance] keyTerminalWindow];
  return [keyWindow lionFullScreen] ||
         (keyWindow && keyWindow->togglingLionFullScreen_);
}

+ (BOOL)anyWindowIsEnteringLionFullScreen {
  for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
    if (term->togglingLionFullScreen_ ||
        term->_haveDelayedEnterFullScreenMode) {
      return YES;
    }
  }
  return NO;
...
+ (BOOL)arrangementIsLionFullScreen:(NSDictionary *)arrangement {
  return [PseudoTerminal _windowTypeForArrangement:arrangement] ==
         WINDOW_TYPE_LION_FULL_SCREEN;
}

+ (BOOL)shouldRestoreHotKeyWindowWithGUID:(NSString *)guid {
  if (!guid) {
    // Something went wrong and we don't know the GUID. Or you just upgraded and
    // a GUID wasn't saved because it's new.
    if ([[iTermHotKeyMigrationHelper sharedInstance] didMigration] &&
        [[[iTermHotKeyController sharedInstance] profileHotKeys] count] == 1) {
      iTermProfileHotKey *profileHotKey =
          [[[iTermHotKeyController sharedInstance] profileHotKeys] firstObject];
      guid = profileHotKey.profile[KEY_GUID];
      DLog(@"Restoring an arrangement with a hotkey window but its profile's "
           @"GUID is missing. Guessing that it's the profile named %@ because "
           @"there is only one hotkey profile",
           profileHotKey.profile[KEY_NAME]);
    } else {
      DLog(@"Restoring an arrangement with a hotkey window but its profile's "
           @"GUID is missing. There is more than one hotkey profile so I give "
           @"up.");
      return NO;
    }
  }
  BOOL foundHotKey = NO;
  for (iTermProfileHotKey *hotKey in
       [[iTermHotKeyController sharedInstance] profileHotKeys]) {
    if ([[iTermProfilePreferences stringForKey:KEY_GUID
                                     inProfile:hotKey.profile]
            isEqualToString:guid]) {
      foundHotKey = YES;
...
+ (PseudoTerminal *)bareTerminalWithArrangement:(NSDictionary *)arrangement
  ...
  BOOL isHotkeyWindow =
      [arrangement[TERMINAL_ARRANGEMENT_IS_HOTKEY_WINDOW] boolValue];
  ...
  PseudoTerminal *term;
  iTermWindowType windowType = iTermThemedWindowType(
      [PseudoTerminal _windowTypeForArrangement:arrangement]);
  int screenIndex = [PseudoTerminal _screenIndexForArrangement:arrangement];
  iTermProfileHotKey *profileHotKey =
      [[iTermHotKeyController sharedInstance] profileHotKeyForGUID:guid];
  ...
  if (windowType == WINDOW_TYPE_TRADITIONAL_FULL_SCREEN) {
    term = [[[PseudoTerminal alloc]
        initWithSmartLayout:NO
                 windowType:WINDOW_TYPE_TRADITIONAL_FULL_SCREEN
            savedWindowType:[arrangement[TERMINAL_ARRANGEMENT_SAVED_WINDOW_TYPE]
                                intValue]
                     screen:screenIndex
           hotkeyWindowType:hotkeyWindowType
                    profile:arrangement[TERMINAL_ARRANGEMENT_INITIAL_PROFILE]]
        autorelease];

    NSRect rect;
    rect.origin.x = [[arrangement
        objectForKey:TERMINAL_ARRANGEMENT_OLD_X_ORIGIN] doubleValue];
    rect.origin.y = [[arrangement
        objectForKey:TERMINAL_ARRANGEMENT_OLD_Y_ORIGIN] doubleValue];
    rect.size.width =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_OLD_WIDTH] doubleValue];
    rect.size.height = [[arrangement
        objectForKey:TERMINAL_ARRANGEMENT_OLD_HEIGHT] doubleValue];
    term->oldFrame_ = rect;
    term->useTransparency_ = ![iTermPreferences
        boolForKey:kPreferenceKeyDisableFullscreenTransparencyByDefault];
    ...
    NSRect rect;
    rect.origin.x =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_X_ORIGIN] doubleValue];
    rect.origin.y =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_Y_ORIGIN] doubleValue];
    // TODO: for window type top, set width to screen width.
    rect.size.width =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_WIDTH] doubleValue];
    rect.size.height =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_HEIGHT] doubleValue];
    DLog(@"Initialize nonfullscreen window to saved frame %@",
         NSStringFromRect(rect));
    rect = [self sanitizedWindowFrame:rect];
    [[term window] setFrame:rect display:NO];
  }

  if ([[arrangement objectForKey:TERMINAL_ARRANGEMENT_HIDE_AFTER_OPENING]
          boolValue]) {
    [term hideAfterOpening];
  }
  if (isHotkeyWindow) {
    BOOL ok = [[iTermHotKeyController sharedInstance]
        addRevivedHotkeyWindowController:term
...
- (IBAction)editWindowTitle:(id)sender {
  ...
  alert.informativeText =
      @"If this is empty, the window takes the active session’s title. "
      @"Variables and function calls enclosed in \\(…) will be replaced with "
      @"their evaluation.";
  NSTextField *titleTextField = [[[NSTextField alloc]
      initWithFrame:NSMakeRect(0, 0, 400, 24 * 3)] autorelease];
  iTermFunctionCallTextFieldDelegate *delegate;
  delegate = [[[iTermFunctionCallTextFieldDelegate alloc]
      initWithPathSource:
          [iTermVariableHistory
              pathSourceForContext:iTermVariablesSuggestionContextWindow]
             passthrough:nil
  ...
  titleTextField.stringValue =
      [self.scope
          valueForVariableName:iTermVariableKeyWindowTitleOverrideFormat]
          ?: @"";
  ...
  if ([alert runModal] == NSAlertFirstButtonReturn) {
    [self.scope setValue:titleTextField.stringValue.length
                             ? titleTextField.stringValue
                             : nil
        forVariableNamed:iTermVariableKeyWindowTitleOverrideFormat];
  }
...
- (IBAction)findUrls:(id)sender {
  ...
  [findDriver
      closeViewAndDoTemporarySearchForString:regex
...
- (TmuxController *)currentTmuxController {
  ...
  if (!controller) {
    controller =
        [[[iTermController sharedInstance] anyTmuxSession] tmuxController];
...
- (IBAction)newTmuxWindow:(id)sender {
  [[self currentTmuxController]
      newWindowWithAffinity:nil
                       size:
                           [PTYTab
                               sizeForTmuxWindowWithAffinity:nil
                                                  controller:
                                                      self.currentTmuxController]
           initialDirectory:[iTermInitialDirectory
                                initialDirectoryFromProfile:self.currentSession
                                                                .profile
...
- (IBAction)newTmuxTab:(id)sender {
  ...
  [[self currentTmuxController]
      newWindowWithAffinity:affinity
                       size:
                           [PTYTab
                               sizeForTmuxWindowWithAffinity:affinity
                                                  controller:
                                                      self.currentTmuxController]
           initialDirectory:[iTermInitialDirectory
                                initialDirectoryFromProfile:self.currentSession
...
- (void)loadTmuxLayout:(NSMutableDictionary *)parseTree
  ...
  for (PTYSession *aSession in [tab sessions]) {
    [tmuxController registerSession:aSession
...
+ (NSDictionary *)repairedArrangement:(NSDictionary *)arrangement
  ...
  NSMutableDictionary *mutableArrangement =
      [[arrangement mutableCopy] autorelease];
  NSMutableArray *mutableTabs = [NSMutableArray array];

  for (NSDictionary *tabArrangement in
       [arrangement objectForKey:TERMINAL_ARRANGEMENT_TABS]) {
    [mutableTabs
        addObject:[PTYTab repairedArrangement:tabArrangement
...
+ (NSDictionary *)repairedArrangement:(NSDictionary *)arrangement
  ...
  result[TERMINAL_ARRANGEMENT_TABS] =
      [tabs mapWithBlock:^id(NSDictionary *tabArrangement) {
...
- (BOOL)loadArrangement:(NSDictionary *)arrangement
  ...
  if ([arrangement objectForKey:TERMINAL_ARRANGEMENT_DESIRED_ROWS]) {
    desiredRows_ =
        [[arrangement objectForKey:TERMINAL_ARRANGEMENT_DESIRED_ROWS] intValue];
  }
  if ([arrangement objectForKey:TERMINAL_ARRANGEMENT_DESIRED_COLUMNS]) {
    desiredColumns_ = [[arrangement
        objectForKey:TERMINAL_ARRANGEMENT_DESIRED_COLUMNS] intValue];
  }
  iTermWindowType windowType = iTermThemedWindowType(
      [PseudoTerminal _windowTypeForArrangement:arrangement]);
  NSRect rect;
  ...
  // TODO: The anchored screen isn't always respected, e.g., if the screen's
  // origin/size changes then rect might not lie inside it.
  if (windowType == WINDOW_TYPE_LION_FULL_SCREEN) {
    NSArray *screens = [NSScreen screens];
  ...
  _suppressMakeCurrentTerminal =
      (self.hotkeyWindowType != iTermHotkeyWindowTypeNone);
  const BOOL restoreTabsOK =
      [self restoreTabsFromArrangement:arrangement
                                 named:arrangementName
                              sessions:sessions
  ...
  if (arrangement[TERMINAL_ARRANGEMENT_USE_TRANSPARENCY]) {
    useTransparency_ =
        [arrangement[TERMINAL_ARRANGEMENT_USE_TRANSPARENCY] boolValue];
  }
  self.scope.windowTitleOverrideFormat =
      arrangement[TERMINAL_ARRANGEMENT_TITLE_OVERRIDE];

  _contentView.shouldShowToolbelt =
      [arrangement[TERMINAL_ARRANGEMENT_HAS_TOOLBELT] boolValue];
  [_contentView constrainToolbeltWidth];
  [_contentView setToolbeltProportions:
                    arrangement[TERMINAL_ARRANGEMENT_TOOLBELT_PROPORTIONS]];
  [_contentView.toolbelt
      restoreFromState:arrangement[TERMINAL_ARRANGEMENT_TOOLBELT]];

  hidingToolbeltShouldResizeWindow_ =
      [arrangement[TERMINAL_ARRANGEMENT_HIDING_TOOLBELT_SHOULD_RESIZE_WINDOW]
          boolValue];
  hidingToolbeltShouldResizeWindowInitialized_ = YES;

  if (windowType == WINDOW_TYPE_NORMAL || windowType == WINDOW_TYPE_ACCESSORY ||
  ...
  const int tabIndex = [[arrangement
      objectForKey:TERMINAL_ARRANGEMENT_SELECTED_TAB_INDEX] intValue];
  if (tabIndex >= 0 && tabIndex < [_contentView.tabView numberOfTabViewItems]) {
    [_contentView.tabView selectTabViewItemAtIndex:tabIndex];
  }

  Profile *addressbookEntry =
      [[[[[self tabs] objectAtIndex:0] sessions] objectAtIndex:0] profile];
  ...
  if ([arrangement objectForKey:TERMINAL_GUID] &&
    ...
    if ([[iTermController sharedInstance] terminalWithGuid:savedGUID] ||
        ![self stringIsValidTerminalGuid:savedGUID]) {
      // Refuse to create a window with an already-used or invalid guid.
      self.terminalGuid =
          [NSString stringWithFormat:@"pty-%@", [NSString uuid]];
    } else {
  ...
  // Sessions were created at the wrong size, which means they might not have
  // been able to position their cursors where they needed to be. Move the
  // cursors to their rightful places. See the comment where
  // preferredCursorPosition is set for more details.
...
- (BOOL)stringIsValidTerminalGuid:(NSString *)string {
  ...
  return [string rangeOfCharacterFromSet:characterSet.invertedSet].location ==
         NSNotFound;
}
...
- (BOOL)restoreTabsFromArrangement:(NSDictionary *)arrangement
  ...
  for (NSDictionary *tabArrangement in arrangement[TERMINAL_ARRANGEMENT_TABS]) {
    ...
    if (sessions) {
      sessionMap = [PTYTab sessionMapWithArrangement:tabArrangement
                                            sessions:sessions];
    }
...
- (NSDictionary *)arrangementWithTabs:(NSArray<PTYTab *> *)tabs
  ...
  iTermMutableDictionaryEncoderAdapter *adapter =
      [[[iTermMutableDictionaryEncoderAdapter alloc]
          initWithMutableDictionary:result] autorelease];
...
- (BOOL)populateArrangementWithTabs:(NSArray<PTYTab *> *)tabs
  ...
  DLog(@"While creating arrangement for %@ save frame of %@", self,
       NSStringFromRect(rect));
  DLog(@"%@", [NSThread callStackSymbols]);
  result[TERMINAL_ARRANGEMENT_HAS_TOOLBELT] =
      @(_contentView.shouldShowToolbelt);
  ...
  if (self.scope.windowTitleOverrideFormat) {
    result[TERMINAL_ARRANGEMENT_TITLE_OVERRIDE] =
        self.scope.windowTitleOverrideFormat;
  }
  ...
  result[TERMINAL_ARRANGEMENT_WINDOW_TYPE] =
      @([self lionFullScreen] ? WINDOW_TYPE_LION_FULL_SCREEN : self.windowType);
  result[TERMINAL_ARRANGEMENT_SAVED_WINDOW_TYPE] = @(self.savedWindowType);
  result[TERMINAL_ARRANGEMENT_INITIAL_PROFILE] =
      [self expurgatedInitialProfile];
  if (_hotkeyWindowType == iTermHotkeyWindowTypeNone) {
    result[TERMINAL_ARRANGEMENT_SCREEN_INDEX] =
        @([[NSScreen screens] indexOfObjectIdenticalTo:[[self window] screen]]);
  } else {
    result[TERMINAL_ARRANGEMENT_SCREEN_INDEX] =
        @(_screenNumberFromFirstProfile);
  }
  ...
  [result encodeArrayWithKey:TERMINAL_ARRANGEMENT_TABS
                 identifiers:[tabs mapWithBlock:^id(PTYTab *tab) {
  ...
  result[TERMINAL_ARRANGEMENT_SELECTED_TAB_INDEX] = @([_contentView.tabView
      indexOfTabViewItem:[_contentView.tabView selectedTabViewItem]]);
  ...
  NSString *profileGuid = [[[[iTermHotKeyController sharedInstance]
      profileHotKeyForWindowController:self] profile] objectForKey:KEY_GUID];
  if (profileGuid) {
...
- (void)windowDidDeminiaturize:(NSNotification *)aNotification {
  ...
  if (_constrainFrameAfterDeminiaturization) {
    ...
    NSRect frame = [self.window constrainFrameRect:self.window.frame
                                          toScreen:self.window.screen];
    [self.window setFrame:frame display:YES animate:NO];
  }
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermWindowDidDeminiaturize"
                    object:self
                  userInfo:nil];
}
...
- (BOOL)windowShouldClose:(NSNotification *)aNotification {
  ...
  // This counts as an interaction because it is only called when the user
  // initiates the closing of the window (as opposed to a session dying on you).
  iTermApplicationDelegate *appDelegate =
      [iTermApplication.sharedApplication delegate];
  [appDelegate userDidInteractWithASession];
...
- (void)killOrHideTmuxWindow {
  ...
  NSString *title = nil;
  if (n == 1) {
    title = @"Kill window and its jobs, hide window from view, or detach from "
            @"tmux session?\n\n"
            @"Hidden windows may be restored from the tmux dashboard.";
  } else if (n > 1) {
    title = @"Kill all tmux windows and their jobs, hide windows from view, or "
            @"detach from tmux session?\n\n"
            @"Hidden windows may be restored from the tmux dashboard.";
  }
  if (title) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:title
    ...
    if (doTmuxDetach) {
      PTYSession *aSession = [[[_contentView.tabView selectedTabViewItem]
          identifier] activeSession];
...
- (void)windowWillClose:(NSNotification *)aNotification {
  ...
  // _contentView.tabBarControl is holding on to us, so we have to tell it to
  // let go
  [_contentView.tabBarControl setDelegate:nil];

  [self disableBlur];
  // If a fullscreen window is closing, hide the menu bar unless it's only
  // fullscreen because it's mid-toggle in which case it's really the window
  // that's replacing us that is fullscreen.
  ...
  if ([[[iTermController sharedInstance] terminals] count] == 1) {
    if (_instantReplayWindowController) {
      // We don't want the IR window to survive us, nor be saved in the
      // restorable state.
      [self closeInstantReplayWindow];
    }
    if ([iTermPreferences boolForKey:kPreferenceKeySmartWindowPlacement]) {
      [[self window]
          saveFrameUsingName:[NSString stringWithFormat:kWindowNameFormat, 0]];
    } else {
      // Save frame position for window
      [[self window]
          saveFrameUsingName:[NSString stringWithFormat:kWindowNameFormat,
                                                        uniqueNumber_]];
      [[TemporaryNumberAllocator sharedInstance]
          deallocateNumber:uniqueNumber_];
    }
  } else {
    if (![iTermPreferences boolForKey:kPreferenceKeySmartWindowPlacement]) {
      // Save frame position for window
  ...
  if ([[self allSessions] count]) {
    // Save restorable sessions in controllers and make sessions terminate or
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermWindowDidCloseNotification
                    object:nil
                  userInfo:nil];
...
- (void)windowWillMiniaturize:(NSNotification *)aNotification {
  ...
  [self disableBlur];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermWindowWillMiniaturize"
                    object:self
                  userInfo:nil];
}
...
- (void)windowDidBecomeKey:(NSNotification *)aNotification {
  DLog(@"windowDidBecomeKey:%@ window=%@ stack:\n%@", aNotification,
       self.window, [NSThread callStackSymbols]);
  ...
  [[iTermHotKeyController sharedInstance]
      autoHideHotKeyWindowsExcept:[[iTermHotKeyController sharedInstance]
                                      siblingWindowControllersOf:self]];
  ...
  if (_fullScreen) {
    if (![self isHotKeyWindow] ||
        [[[iTermHotKeyController sharedInstance]
            profileHotKeyForWindowController:self] rollingIn] ||
        [[self window] alphaValue] > 0) {
      // One of the following is true:
      // - This is a regular (non-hotkey) fullscreen window
      // - It's a fullscreen hotkey window that's getting rolled in (but its
      // alpha is 0)
      // - It's a fullscreen hotkey window that's already visible (e.g.,
      // switching back from settings dialog)
      [[iTermPresentationController sharedInstance] update];
    }
  }
  ...
  // Some users report that the first responder isn't always set properly. Let's
  // try to fix that. This attempt (4/20/13) is to fix bug 2431.
...
- (void)makeCurrentSessionFirstResponder {
  if ([self currentSession]) {
    PtyLog(@"makeCurrentSessionFirstResponder. New first responder will be %@. "
           @"The current first responder is %@",
           [[self currentSession] textview], [[self window] firstResponder]);
    [[self window] makeFirstResponder:[[self currentSession] textview]];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermSessionBecameKey
...
// Forbid FFM from changing key window when the key window is an auto-hiding
// hotkey window.
- (BOOL)disableFocusFollowsMouse {
  if (!self.isHotKeyWindow) {
    return NO;
  }
  return [[[iTermHotKeyController sharedInstance]
      profileHotKeyForWindowController:self] autoHides];
}
...
- (void)canonicalizeWindowFrame {
  ...
  // It's important that this method respect the current screen if possible
  // because -windowDidChangeScreen calls it.
  ...
  case WINDOW_TYPE_BOTTOM_PARTIAL: {
    ...
    if (desiredWindowFrame.size.width > 0 &&
        desiredWindowFrame.size.height > 0) {
  ...
  case WINDOW_TYPE_TRADITIONAL_FULL_SCREEN: {
    if ([screen frame].size.width > 0) {
      // This is necessary when restoring a traditional fullscreen window while
      // scrollbars are forced on systemwide.
      ...
      [[self window] setFrame:[self canonicalFrameForScreen:screen]
                      display:YES];

      if (changedScrollBars &&
          NSEqualSizes(self.window.frame.size, originalFrame.size)) {
        DLog(@"Fitting tabs to window when canonicalizing fullscreen window "
             @"because of scrollbar change");
        [self fitTabsToWindow];
        if (!tmuxOriginatedResizeInProgress_) {
          // When opening a new tmux tab in a fullscreen window, it'll be
          // initialized with legacy scrollers (if the system is configured to
          // use them) and then it needs to update its size when the scrollers
          // are forced to be inline.
          for (TmuxController *controller in self.uniqueTmuxControllers) {
            if (controller.variableWindowSize) {
              NSArray<NSString *> *windows =
                  [self.tabs mapWithBlock:^id(PTYTab *anObject) {
                    if (!anObject.tmuxTab) {
                      return nil;
                    }
              ...
              if (windows.count > 0) {
                DLog(@"Calling window did resize because canonicalizing a full "
                     @"screen window, scrollbar style changed, and variable "
                     @"size tmux windows is enabled");
                [controller windowDidResize:self];
              }
            } else {
...
- (NSRect)screenFrameForEdgeSpanningWindows:(NSScreen *)screen {
  if ([[[iTermHotKeyController sharedInstance]
          profileHotKeyForWindowController:self] floats]) {
    const BOOL menuBarIsHidden =
        ![[iTermMenuBarObserver sharedInstance] menuBarVisibleOnScreen:screen];
    if (menuBarIsHidden) {
      return screen.frame;
    }
...
- (NSRect)visibleFrameForScreen:(NSScreen *)screen {
  if ([[[iTermHotKeyController sharedInstance]
          profileHotKeyForWindowController:self] floats]) {
    DLog(@"visibleFrameForScreen: floating hotkey window gets "
         @"frameExceptMenuBar");
    const BOOL menuBarIsHidden =
        ![[iTermMenuBarObserver sharedInstance] menuBarVisibleOnScreen:screen];
    if (menuBarIsHidden) {
  ...
  if (self.fullScreen) {
    DLog(@"visibleFrameForScreen: fullScreen gets visibleFrame %@",
  ...
  if (otherScreenHasLionFullscreenTerminalWindow) {
    DLog(@"visibleFrameForScreen: otherScreenHasLionFullscreenTerminalWindow "
         @"gets frameExceptMenuBar");
    return screen.frameExceptMenuBar;
  } else {
    DLog(@"visibleFrameForScreen: !otherScreenHasLionFullscreenTerminalWindow "
         @"gets visibleFrame %@",
...
- (NSRect)canonicalFrameForScreen:(NSScreen *)screen
  ...
  NSRect screenVisibleFrameIgnoringHiddenDock =
      [self screenFrameForEdgeSpanningWindows:screen];

  PtyLog(@"The new screen visible frame is %@",
         [NSValue valueWithRect:screenVisibleFrame]);

  // NOTE: In bug 1347, we see that for some machines, [screen
  // frame].size.width==0 at some point during sleep/wake from sleep. That is
  // why we check that width is positive before setting the window's frame.
  NSSize decorationSize = [self windowDecorationSize];

  // Note: During window state restoration, this may be called before the tabs
  // are created from the arrangement, in which case the line height and char
  // width will be 0.
  ...
  PtyLog(@"Line height is %f, char width is %f",
         (float)[[session textview] lineHeight],
         [[session textview] charWidth]);
  if (session.textview.lineHeight == 0 || session.textview.charWidth == 0) {
    DLog(
        @"Line height or char width is 0. Returning existing frame. session=%@",
        session);
    return self.window.frame;
  }
  ...
  case WINDOW_TYPE_TOP:
    ...
    if (!preserveSize) {
      // If the screen grew and the window was smaller than the desired number
      // of rows, grow it.
      if (desiredRows_ > 0) {
        frame.size.height =
            MIN(screenVisibleFrame.size.height,
                ceil([[session textview] lineHeight] * desiredRows_) +
                    decorationSize.height +
                    2 * [iTermPreferences
                            intForKey:kPreferenceKeyTopBottomMargins]);
      } else {
        frame.size.height =
            MIN(screenVisibleFrame.size.height, frame.size.height);
      }
    }
    if (!edgeSpanning) {
      if (!preserveSize) {
        frame.size.width = MIN(frame.size.width,
                               screenVisibleFrameIgnoringHiddenDock.size.width);
      }
      frame = iTermRectCenteredHorizontallyWithinRect(
          frame, screenVisibleFrameIgnoringHiddenDock);
    } else {
    ...
    frame.origin.y = screenVisibleFrame.origin.y +
                     screenVisibleFrame.size.height - frame.size.height;
    DLog(@"Canonical frame for top of screen window is %@",
  ...
  case WINDOW_TYPE_LEFT:
    ...
    if (!preserveSize) {
      ...
      if (desiredColumns_ > 0) {
        frame.size.width =
            MIN(screenVisibleFrame.size.width,
                [[session textview] charWidth] * desiredColumns_ +
                    2 * [iTermPreferences intForKey:kPreferenceKeySideMargins]);
      } else {
      ...
      if (!preserveSize) {
        frame.size.height =
            MIN(frame.size.height,
                screenVisibleFrameIgnoringHiddenDock.size.height);
      }
      frame = iTermRectCenteredVerticallyWithinRect(
          frame, screenVisibleFrameIgnoringHiddenDock);
    } else {
  ...
  case WINDOW_TYPE_LION_FULL_SCREEN:
  case WINDOW_TYPE_ACCESSORY:
    PtyLog(@"Window type = NORMAL, NO_TITLE_BAR, WINDOW_TYPE_COMPACT, "
           @"WINDOW_TYPE_ACCSSORY, or LION_FULL_SCREEN");
    return frame;
...
- (void)windowDidResignKey:(NSNotification *)aNotification {
  ...
  if (self.ptyWindow.it_openingSheet) {
    ...
    DLog(@"windowDidResignKey not auto-hiding hotkey window because a sheet is "
         @"being opened.");
  }
  if ([iTermApplication sharedApplication].it_characterPanelIsOpen) {
    shouldAutoHideHotkeyWindow = NO;
    DLog(@"windowDidResignKey not auto-hiding hotkey window because the "
         @"character panel is open.");
  }

  NSWindow *newKeyWindow =
      [NSApp keyWindow]
          ?: [[iTermApplication sharedApplication] it_windowBecomingKey];
  DLog(@"Window %@ resiging key. New key window (or key-window-elect) is %@",
       self, newKeyWindow);
  if (shouldAutoHideHotkeyWindow) {
    NSArray<NSWindowController *> *siblings =
        [[iTermHotKeyController sharedInstance]
            siblingWindowControllersOf:self];
    NSWindowController *newKeyWindowController =
        [newKeyWindow windowController];
  ...
  PtyLog(@"%s(%d):-[PseudoTerminal windowDidResignKey:%@]", __FILE__, __LINE__,
         aNotification);
  ...
  // Note that if you have multiple displays you can see a lion fullscreen
  // window when it's not key.
  for (PTYSession *aSession in [self allSessions]) {
...
- (void)windowDidResignMain:(NSNotification *)aNotification {
  ...
  PtyLog(@"%s(%d):-[PseudoTerminal windowDidResignMain:%@]", __FILE__, __LINE__,
         aNotification);
  if (![iTermApplication sharedApplication].it_characterPanelIsOpen) {
    NSArray<NSWindowController *> *siblings =
        [[iTermHotKeyController sharedInstance]
            siblingWindowControllersOf:self];
    NSWindowController *newMainWindowController =
...
- (NSEdgeInsets)tabBarInsetsForCompactWindow NS_AVAILABLE_MAC(10_14) {
  ...
  case PSMTab_TopTab:
    if ([self rootTerminalViewWindowNumberLabelShouldBeVisible]) {
      const CGFloat leftInset =
          (stoplightButtonsWidth +
           iTermRootTerminalViewWindowNumberLabelMargin * 2 +
           iTermRootTerminalViewWindowNumberLabelWidth +
           MAX(0,
               [iTermAdvancedSettingsModel extraSpaceBeforeCompactTopTabBar]));
...
- (NSSize)windowWillResize:(NSWindow *)sender toSize:(NSSize)proposedFrameSize {
  DLog(@"windowWillResize: self=%@, proposedFrameSize=%@ screen=%@", self,
       NSStringFromSize(proposedFrameSize), self.window.screen);
  DLog(@"%@", [NSThread callStackSymbols]);
  if (self.swipeIdentifier) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermSwipeHandlerCancelSwipe
  ...
  // Decide when to snap.  (We snap unless control, and only control, is held
  // down.)
  const NSUInteger theMask =
      (NSEventModifierFlagControl | NSEventModifierFlagOption |
       NSEventModifierFlagCommand | NSEventModifierFlagShift);
  BOOL modifierDown = (([[NSApp currentEvent] it_modifierFlags] & theMask) ==
  ...
  // If resizing a full-width/height X-of-screen window in a direction
  // perpendicular to the screen edge it's attached to, turn off snapping in the
  // direction parallel to the edge.
  if (self.windowType == WINDOW_TYPE_RIGHT ||
  ...
  if (self.windowType == WINDOW_TYPE_TOP ||
  ...
  NSSize contentSize = [NSScrollView
      contentSizeForFrameSize:tabSize
      horizontalScrollerClass:nil
        verticalScrollerClass:(hasScrollbar ? [PTYScroller class]
                                            : nil)borderType:NSNoBorder
  ...
  tabSize = [PTYScrollView
      frameSizeForContentSize:contentSize
      horizontalScrollerClass:nil
  ...
  proposedFrameSize.width =
      tabSize.width + decorationSize.width + internalDecorationSize.width;
  proposedFrameSize.height =
      tabSize.height + decorationSize.height + internalDecorationSize.height;
  ...
  DLog(@"screenFrame=%@ accepted=%@ originalProposal=%@ charSize=%@",
       NSStringFromSize(screenFrame.size), NSStringFromSize(proposedFrameSize),
  ...
  PtyLog(@"Accepted size: %fx%f", proposedFrameSize.width,
         proposedFrameSize.height);
...
- (void)tmuxTabLayoutDidChange:(BOOL)nontrivialChange
  ...
  if (liveResize_) {
    ...
    if (nontrivialChange) {
      postponedTmuxTabLayoutChange_ =
          variableWindowSize
              ? iTermPostponeTmuxTabLayoutChangeStateVariableSizeWindow
              : iTermPostponeTmuxTabLayoutChangeStateFixedSizeWindow;
    }
    return;
  }
  ...
  if (variableWindowSize) {
    if (![iTermAdvancedSettingsModel disableTmuxWindowResizing]) {
      DLog(@"Variable window size and tmux window resizing is not disabled. "
           @"Fit window to tab.");
      if (tab) {
...
- (void)windowDidChangeScreen:(NSNotification *)notification {
  ...
  DLog(@"windowDidChangeScreen frame=%@ forceFrame=%@ time left in "
       @"forceFrameUntil=%@ screenConfig=%@\n%@",
       NSStringFromRect(self.window.frame), NSStringFromRect(_forceFrame),
       @([NSDate it_timeSinceBoot] - _forceFrameUntil),
       _screenConfigurationAtTimeOfForceFrame, [NSThread callStackSymbols]);
  if (!NSEqualRects(self.window.frame, _forceFrame) &&
      !NSEqualRects(NSZeroRect, _forceFrame) &&
      [NSDate it_timeSinceBoot] < _forceFrameUntil &&
      [[self screenConfiguration]
          isEqual:_screenConfigurationAtTimeOfForceFrame]) {
  ...
  BOOL canonicalize =
      ![self miniaturizedWindowShouldPreserveFrameUntilDeminiaturized];
  ...
  DLog(@"windowDidChangeScreen called. This is known to happen when the screen "
       @"didn't really change! screen=%@",
       self.window.screen);
  if (canonicalize && !_inWindowDidChangeScreen) {
    // Nicolas reported a bug where canonicalizeWindowFrame moved the window
    // causing this to be called reentrantly, and eventually the stack
    // overflowed. If we insist the window should be on screen A and the OS
    // insists it should be on screen B, we'll never agree, so just try once.
...
- (void)windowWillMove:(NSNotification *)notification {
  // AFAICT this is only called when you move the window by dragging it or
...
- (void)windowDidMove:(NSNotification *)notification {
  DLog(@"%@: Window %@ moved. Called from %@", self, self.window,
       [NSThread callStackSymbols]);
  if (_inWindowDidMove) {
  ...
  if (_windowIsMoving && _isAnchoredToScreen) {
    NSInteger screenIndex =
        [[NSScreen screens] indexOfObject:self.window.screen];
    if (screenIndex != _screenBeforeMoving) {
      DLog(@"User appears to have dragged the window from screen %@ to screen "
           @"%@. Removing screen anchor.",
           @(_screenBeforeMoving), @(screenIndex));
      _isAnchoredToScreen = NO;
    }
  }
  _windowIsMoving = NO;
...
- (void)windowDidResize:(NSNotification *)aNotification {
  ...
  PtyLog(@"windowDidResize to: %fx%f", [[self window] frame].size.width,
  ...
  [self notifyTmuxOfWindowResize];
  // windowDidMove does not get called if the origin changes because of a
  // resize.
  [self saveTmuxWindowOrigins];
  ...
  // Post a notification
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermWindowDidResize"
                    object:self
                  userInfo:nil];
...
- (void)haveTransparentPaneDidChange {
  ...
  [self updateTabColors]; // Updates the window's background color as a
                          // side-effect
...
// This is a hack to fix the problem of exiting a fullscreen window that as
// never not-fullscreen. We need to have some size to go to. This method
// computes the size based on the current session's profile's rows and columns
// setting plus the window decoration size. It's sort of arbitrary because split
// panes will have to share that space, but there's no perfect solution to this
// issue.
- (NSSize)
    preferredWindowFrameToPerfectlyFitCurrentSessionInInitialConfiguration {
  PTYSession *session = [self currentSession];
  PTYTextView *textView = session.textview;
  NSSize cellSize = NSMakeSize(textView.charWidth, textView.lineHeight);
  NSSize decorationSize = [self windowDecorationSize];
  VT100GridSize sessionSize = VT100GridSizeMake(
      MIN(iTermMaxInitialSessionSize, [session.profile[KEY_COLUMNS] intValue]),
      MIN(iTermMaxInitialSessionSize, [session.profile[KEY_ROWS] intValue]));
  return NSMakeSize(
      [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2 +
          sessionSize.width * cellSize.width + decorationSize.width,
      [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2 +
          sessionSize.height * cellSize.height + decorationSize.height);
}
...
- (void)addShortcutAccessorViewControllerToTitleBarIfNeeded {
  if (@available(macOS 10.14, *)) {
    ...
    if (self.shouldHaveShortcutAccessory &&
        [self.window.titlebarAccessoryViewControllers
            containsObject:_shortcutAccessoryViewController]) {
    ...
    if ([self.window respondsToSelector:@selector
      ...
      // Explicitly load the view before adding. Otherwise, for some reason, on
      // WINDOW_TYPE_MAXIMIZED windows, the NSWindow miscalculates the size, and
      // ends up resizing the iTermRootTerminalView incorrectly.
      [_shortcutAccessoryViewController view];

      [self.window
          addTitlebarAccessoryViewController:_shortcutAccessoryViewController];
...
- (void)toggleFullScreenMode:(id)sender completion:(void (^)(BOOL))completion {
...
- (void)updateForTransparency:(NSWindow<PTYWindow> *)window {
  ...
  if (@available(macOS 10.16, *)) {
    ...
    if ([iTermAdvancedSettingsModel
            disableWindowShadowWhenTransparencyOnMojave]) {
      [self
          updateWindowShadowForNonFullScreenWindowDisablingIfAnySessionHasTransparency:
              window];
  ...
  if (![iTermPreferences boolForKey:kPreferenceKeyPerPaneBackgroundImage]) {
    ...
    const CGFloat transparency =
        1 - self.currentSession.textview.transparencyAlpha;
...
- (void)
  ...
  DLog(@"%@: have transparency = %@ for sessions %@ in tab %@", self,
       @(haveTransparency), self.currentTab.sessions, self.currentTab);
  // Let's try enabling window shadow on 10.16 when there is transparency. Now
  // that I set the background color to
  // NSColor.clearColor.withAlphaComponent(0.01) maybe shadows will magically
  // start working again.
  if (@available(macOS 10.16, *)) {
  } else {
...
- (void)windowDidEndLiveResize:(NSNotification *)notification {
  ...
  // Canonicalize the frame so that centered windows stay centered,
  // edge-attached windows stay edge attached.
  NSRect frame = [self canonicalFrameForScreen:self.window.screen
                                   windowFrame:self.window.frame
                                  preserveSize:YES];
  NSRect screenFrameForEdgeSpanningWindow =
      [self screenFrameForEdgeSpanningWindows:screen];
  ...
  case WINDOW_TYPE_TOP_PARTIAL:
    ...
    // desiredRows/Columns get reset here to fix issue 4073. If you manually
    // resize a window then its desired size becomes irrelevant; we want it to
    // preserve the size you set and forget about the size in its profile. This
    // way it will go back to the old size when toggling out of fullscreen.
    desiredRows_ = -1;
    break;
  ...
  if (!NSEqualRects(frame, self.window.frame) && frame.size.width > 0 &&
      frame.size.height > 0) {
...
- (NSTitlebarAccessoryViewController *)
    titleBarAccessoryTabBarViewController NS_AVAILABLE_MAC(10_14) {
  if (!_titleBarAccessoryTabBarViewController) {
    _titleBarAccessoryTabBarViewController =
        [[iTermTabBarAccessoryViewController alloc]
            initWithView:[_contentView borrowTabBarControl]];
    _titleBarAccessoryTabBarViewController.layoutAttribute =
...
// Returns whether a permanent (i.e., not flashing) tabbar ought to be drawn
// while in full screen. It does not check if you're already in full screen.
...
- (BOOL)tabBarShouldBeAccessory {
  ...
  if (!(self.window.styleMask & NSWindowStyleMaskTitled)) {
    // You get an assertion if you try to add an accessory to an untitled
    // window.
    return NO;
  }
  DLog(@"%@", self);
  if (!exitingLionFullscreen_) {
    const BOOL assumeFullScreen =
        (self.lionFullScreen || togglingLionFullScreen_);
    if (assumeFullScreen) {
      DLog(@"tabBarShouldBeAccessory - assuing full screen, should=%@",
           @([self shouldMoveTabBarToTitlebarAccessoryInLionFullScreen]));
  ...
  switch (
      (PSMTabPosition)[iTermPreferences intForKey:kPreferenceKeyTabPosition]) {
  case PSMTab_LeftTab:
  case PSMTab_BottomTab:
...
- (void)updateTabBarControlIsTitlebarAccessory {
  const NSInteger index = [self.window.it_titlebarAccessoryViewControllers
      indexOfObject:_titleBarAccessoryTabBarViewController];
  if ([self tabBarShouldBeAccessory]) {
    ...
    NSTitlebarAccessoryViewController *viewController =
        [self titleBarAccessoryTabBarViewController];
    const CGFloat tabBarHeight =
        self.shouldShowPermanentFullScreenTabBar ? self.desiredTabBarHeight : 0;
...
- (void)returnTabBarToContentView {
  ...
  [_contentView
      returnTabBarControlView:(iTermTabBarControlView *)
                                  _titleBarAccessoryTabBarViewController
                                      .realView];
  [_titleBarAccessoryTabBarViewController release];
...
- (NSRect)windowWillUseStandardFrame:(NSWindow *)sender
  ...
  if (togglingLionFullScreen_ || [[self ptyWindow] isTogglingLionFullScreen] ||
      [self lionFullScreen]) {
    ...
    maxVerticallyPref =
        [iTermPreferences boolForKey:kPreferenceKeyMaximizeVerticallyOnly];
    if ([[NSApp currentEvent] type] == NSEventTypeKeyDown) {
      verticalOnly = maxVerticallyPref;
    } else if (maxVerticallyPref ^ (([[NSApp currentEvent] it_modifierFlags] &
...
- (void)windowWillShowInitial {
  ...
  // If it's a full or top-of-screen window with a screen number preference,
  // always honor that.
  if (_isAnchoredToScreen) {
  ...
  NSUInteger numberOfTerminalWindows =
      [[[iTermController sharedInstance] terminals] count];
  ...
  if (_isAnchoredToScreen && window.screenNumber != screenNumber) {
    DLog(
        @"Move window to preferred origin because it moved to another screen.");
...
- (void)loadAutoSaveFrame {
  ...
  iTermTerminalWindow *window = [self ptyWindow];
  NSRect frame = [window frame];
  if ([window setFrameUsingName:[NSString stringWithFormat:kWindowNameFormat,
                                                           uniqueNumber_]]) {
...
- (void)menuForEvent:(NSEvent *)theEvent menu:(NSMenu *)theMenu {
  ...
  // Bookmarks
  [theMenu insertItemWithTitle:NSLocalizedStringFromTableInBundle(
                                   @"New Window", @"iTerm",
  ...
  // Create a menu with a submenu to navigate between tabs if there are more
  // than one
  if ([_contentView.tabView numberOfTabViewItems] > 1) {
    [theMenu insertItemWithTitle:NSLocalizedStringFromTableInBundle(
                                     @"Select", @"iTerm",
    ...
    for (i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
      aMenuItem = [[NSMenuItem alloc]
          initWithTitle:[NSString
                            stringWithFormat:@"%@ #%d",
                                             [[_contentView.tabView
                                                 tabViewItemAtIndex:i] label],
                                             i + 1]
                 action:@selector(selectTab:)
          keyEquivalent:@""];
      [aMenuItem setRepresentedObject:[[_contentView.tabView
                                          tabViewItemAtIndex:i] identifier]];
...
- (void)enableBlur:(double)radius {
  ...
  if (nil != window && [window respondsToSelector:@selector(enableBlur:)]) {
...
- (void)disableBlur {
  iTermTerminalWindow *window = [self ptyWindow];
  if (nil != window && [window respondsToSelector:@selector(disableBlur)]) {
    __weak __typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)tabView:(NSTabView *)tabView
  ...
  [[self window]
      makeFirstResponder:[[[tabViewItem identifier] activeSession] textview]];
  if ([tab blur]) {
  ...
  for (PTYSession *session in tab.sessions) {
    if ([[session.colorMap colorForKey:kColorMapBackground]
  ...
  if (lightCount > darkCount) {
    ...
    _contentView.color = [NSColor colorWithSRGBRed:170 / 255.0
                                             green:167 / 255.0
                                              blue:170 / 255.0
                                             alpha:1];
  } else {
  ...
  [self updateUseMetalInAllTabs];
  [self.scope setValue:self.currentTab.variables
      forVariableNamed:iTermVariableKeyWindowCurrentTab];
  [self updateForTransparency:self.ptyWindow];
  [self updateDocumentEdited];
  [[iTermFindPasteboard sharedInstance] updateObservers];
  [self updateBackgroundImage];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermSelectedTabDidChange
                    object:tab];
  DLog(@"Finished");
}
...
- (void)updateProxyIcon {
  ...
  __weak __typeof(self) weakSelf = self;
  PTYSession *session = self.currentSession;
  id<iTermOrderedToken> token =
      [[_proxyIconOrderEnforcer newToken] autorelease];
...
- (void)notifyTmuxOfTabChange {
  if (self.currentTab.isTmuxTab) {
    [self.currentTab.tmuxController
        setCurrentWindow:self.currentTab.tmuxWindow];
  }
}
...
- (void)saveAffinitiesLater:(PTYTab *)theTab {
  // Avoid saving affinities during detach because the windows will be gone by
  // the time it saves them.
  //    if ([theTab isTmuxTab] && !theTab.tmuxController.detaching) {
  if ([theTab isTmuxTab]) {
    PtyLog(@"Queueing call to saveAffinitiesLater from %@",
           [NSThread callStackSymbols]);
...
- (void)tabView:(NSTabView *)tabView
  ...
  [self tabView:tabView
      willInsertTabViewItem:tabViewItem
                    atIndex:[tabView numberOfTabViewItems]];
  [self saveAffinitiesLater:[tabViewItem identifier]];
}
...
- (void)tabView:(NSTabView *)tabView
    willInsertTabViewItem:(NSTabViewItem *)tabViewItem
                  atIndex:(int)anIndex {
  DLog(@"%@: tabView:%@ willInsertTabViewItem:%@ atIndex:%d", self, tabView,
       tabViewItem, anIndex);
  PTYTab *theTab = [tabViewItem identifier];
...
- (BOOL)tabView:(NSTabView *)tabView
  ...
  return [self tabView:tabView
      shouldCloseTabViewItem:tabViewItem
        suppressConfirmation:NO];
}

// This isn't a delegate method, but I need the functionality with the added
// suppressConfirmation flag to avoid showing two warnings in tmux integration
// mode.
- (BOOL)tabView:(NSTabView *)tabView
    shouldCloseTabViewItem:(NSTabViewItem *)tabViewItem
      suppressConfirmation:(BOOL)suppressConfirmation {
...
- (BOOL)tabView:(NSTabView *)aTabView
  ...
  } else if ([[aTabBarControl tabView] indexOfTabViewItem:tabViewItem] !=
             NSNotFound) {
...
- (void)updateTabObjectCounts {
  ...
  [self.tabs enumerateObjectsUsingBlock:^(PTYTab *_Nonnull tab, NSUInteger i,
                                          BOOL *_Nonnull stop) {
    [tab setObjectCount:i + 1];
    [_contentView.tabBarControl setIsProcessing:tab.isProcessing
...
- (void)didDonateTab:(PTYTab *)aTab toWindowController:(PseudoTerminal *)term {
  ...
  // In fullscreen mode reordering the tabs causes the tabview not to be
  // displayed properly. This seems to fix it.
...
- (NSImage *)imageFromSelectedTabView:(NSTabView *)aTabView
  ...
  NSImage *viewImage =
      [[[NSImage alloc] initWithSize:contentFrame.size] autorelease];
...
- (NSImage *)tabView:(NSTabView *)aTabView
  ...
  *styleMask = self.window.styleMask;
  ...
  if (tabViewItem == [aTabView selectedTabViewItem]) {
    viewImage = [self imageFromSelectedTabView:aTabView
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)tabView {
  PtyLog(@"%s(%d):-[PseudoTerminal tabViewDidChangeNumberOfTabViewItems]",
         __FILE__, __LINE__);
  if (self.swipeIdentifier) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermSwipeHandlerCancelSwipe
  ...
  const BOOL willShowTabBar =
      ([iTermPreferences boolForKey:kPreferenceKeyHideTabBar] &&
       ...
       ([_contentView.tabBarControl isHidden] ||
        [self rootTerminalViewShouldLeaveEmptyAreaAtTop]));
  // check window size in case tabs have to be hidden or shown
  if (([_contentView.tabView numberOfTabViewItems] ==
       1) || // just decreased to 1 or increased above 1 and is hidden
      willShowTabBar) {
    ...
    NSTabViewItem *tabViewItem =
        [[_contentView.tabView tabViewItems] objectAtIndex:0];
    PTYTab *firstTab = [tabViewItem identifier];

    NSPoint originalOrigin = self.window.frame.origin;
    if (wasDraggedFromAnotherWindow_) {
      // A tab was just dragged out of another window's tabbar into its own
      // window. When this happens, it loses its size. This is our only chance
      // to resize it. So we put it in a mode where it will resize to its
      // "ideal" size instead of its incorrect current size.
      [firstTab setReportIdealSizeAsCurrent:YES];

      // Update visibility of title bars.
      const BOOL perPaneTitleBarEnabled =
          [iTermPreferences boolForKey:kPreferenceKeyShowPaneTitles];
      const BOOL statusBarsOnTop =
          ([iTermPreferences
               unsignedIntegerForKey:kPreferenceKeyStatusBarPosition] ==
      ...
      for (PTYSession *session in firstTab.sessions) {
        const BOOL sessionHasStatusBar =
            [iTermProfilePreferences boolForKey:KEY_SHOW_STATUS_BAR
                                      inProfile:session.profile];
        const BOOL showTitleBar =
            perPaneTitleBarEnabled &&
            (firstTab.isMaximized || haveMultipleSessions);
        const BOOL showTopStatusBar =
            statusBarsOnTop && perPaneStatusBars && sessionHasStatusBar;
        [[session view] setShowTitle:showTitleBar || showTopStatusBar
                    adjustScrollView:YES];
      }
    }
    ...
    if (_windowNeedsInitialSize ||
      ...
      if (NSEqualRects(frameBefore, frameAfter) && neededInitialSize) {
        // If the initial window frame happened to exactly equal the
        // fitWindowToTabs size the session will remain at its initial size
        // specified by its profile. It's necessary to call fitTabsToWindow once
        // during window creation, and this is where it happens. This is easy to
        // reproduce with the maximized window style in the Minimal theme.
        [self fitTabsToWindow];
      }
    }
    if (@available(macOS 10.14, *)) {
      // May need to enter or exit being a titlebar accessory if its visibility
      // changed.
      [self updateTabBarControlIsTitlebarAccessory];
    }
    ...
    if (wasDraggedFromAnotherWindow_) {
      ...
      // fitWindowToTabs will detect the window changed sizes and do a bogus
      // move of it in this case.
      switch (self.windowType) {
      case WINDOW_TYPE_NORMAL:
      case WINDOW_TYPE_COMPACT:
      case WINDOW_TYPE_NO_TITLE_BAR:
  ...
  if (_contentView.tabView.numberOfTabViewItems == 1 &&
      ...
      [iTermProfilePreferences boolForKey:KEY_OPEN_TOOLBELT
                                inProfile:self.currentSession.profile] &&
      !_contentView.shouldShowToolbelt) {
    // This is the first tab of a new window. Open the toolbelt if that's what
    // the profile wants. You can't open the toolbelt until there is at least
    // one session, so that's why it's done here instead of in
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermNumberOfSessionsDidChange"
                    object:self
                  userInfo:nil];
  [self invalidateRestorableState];
  if (@available(macOS 10.14, *)) {
    if ([iTermPreferences boolForKey:kPreferenceKeyHideTabBar] &&
        (self.lionFullScreen || togglingLionFullScreen_)) {
      // Hiding tabbar in fullscreen on 10.14 is extra work because it's a
      // titlebar accessory.
      [self updateTabBarStyle];
    }
  }
...
- (NSMenu *)tabView:(NSTabView *)tabView
  ...
  if ([_contentView.tabView numberOfTabViewItems] > 1) {
    ...
    for (NSTabViewItem *aTabViewItem in [_contentView.tabView tabViewItems]) {
      NSString *title =
          [NSString stringWithFormat:@"%@ #%ld", [aTabViewItem label],
                                     (unsigned long)count++];
    ...
    [rootMenu addItemWithTitle:@"Select" action:nil keyEquivalent:@""];
  ...
  item =
      [[[NSMenuItem alloc] initWithTitle:@"Save Tab as Window Arrangement"
  ...
  if ([_contentView.tabView numberOfTabViewItems] > 1) {
    item = [[[NSMenuItem alloc]
        initWithTitle:@"Move to New Window"
  ...
  if ([_contentView.tabView numberOfTabViewItems] > 1) {
    NSString *title;
    if ([iTermPreferences intForKey:kPreferenceKeyTabPosition] ==
        PSMTab_LeftTab) {
  ...
  item =
      [[[NSMenuItem alloc] initWithTitle:@"Tab Color"
  ...
  case PSMTab_TopTab:
    ...
    case WINDOW_TYPE_NORMAL: {
      CGFloat contentHeight =
          [term.window contentRectForFrameRect:NSMakeRect(0, 0, 100, 100)]
              .size.height;
...
- (NSTabViewItem *)tabView:(NSTabView *)tabView
  ...
  if (tab.realParentWindow == self && tab.sessions.count == 1) {
    // This is an edge case brought to light in issue 4189. If you have a window
    // with a single tab and a single session and you drag the session (by
    // holding cmd+opt+shift and dragging) onto the tab bar, the window
    // disappears. The bug describes a crash, but as of 2.9.20160107 it closes
    // the window without crashing.
    //
    // The issue is that calling -removeAndClearSession closes the tab. That has
    // a knock-on effect of closing the window. In this case we must very
    // delicately keep the tab alive until the new tab has been added.
    //
    // We can't just do nothing in this case because if there are multiple tabs
    // this is a valid way to reorder tabs.
  ...
  PTYTab *theTab = [[[PTYTab alloc] initWithSession:session
                                       parentWindow:self] autorelease];
  [theTab setActiveSession:session];
  theTab.delegate = self;
  NSTabViewItem *tabViewItem =
      [[[NSTabViewItem alloc] initWithIdentifier:(id)theTab] autorelease];
...
- (void)openEditTabTitleWindow {
  ...
  alert.informativeText =
      @"If this is empty, the tab takes the active session’s title. Variables "
      @"and function calls enclosed in \\(…) will replaced with their "
      @"evaluation.";
  NSTextField *titleTextField = [[[NSTextField alloc]
      initWithFrame:NSMakeRect(0, 0, 400, 24 * 3)] autorelease];
  _currentTabTitleTextFieldDelegate =
      [[iTermFunctionCallTextFieldDelegate alloc]
          initWithPathSource:
              [iTermVariableHistory
                  pathSourceForContext:iTermVariablesSuggestionContextTab]
                 passthrough:nil
               functionsOnly:NO];
  ...
  titleTextField.selectable = YES;
  titleTextField.stringValue =
      self.currentTab.variablesScope.tabTitleOverrideFormat ?: @"";
  alert.accessoryView = titleTextField;
  [alert addButtonWithTitle:@"OK"];
  [alert addButtonWithTitle:@"Cancel"];
  __weak __typeof(self) weakSelf = self;
...
- (void)tabViewDoubleClickTabBar:(NSTabView *)tabView {
  iTermApplicationDelegate *itad =
      [iTermApplication.sharedApplication delegate];
  // Note: this assume that self is the front window (it should be!). It is
  // smart enough to create a tmux tab if the user wants one (or ask if needed).
  [itad newSession:nil];
}
...
- (void)tabView:(NSTabView *)tabView
  ...
  [_contentView.tabBarControl setObjectCount:tab.objectCount
                        forTabWithIdentifier:tab];
}

// This updates the window's background color and title text color as well as
// the tab bar's color.
- (void)updateTabColors {
  for (PTYTab *aTab in [self tabs]) {
    ...
    if ([_contentView.tabView selectedTabViewItem] == tabViewItem) {
      NSColor *newTabColor =
          [_contentView.tabBarControl tabColorForTabViewItem:tabViewItem];
      if ([_contentView.tabView numberOfTabViewItems] == 1 &&
          [iTermPreferences boolForKey:kPreferenceKeyHideTabBar] &&
          newTabColor) {
...
- (void)setBackgroundColor:(nullable NSColor *)backgroundColor {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermWindowAppearanceDidChange
                    object:self.window];
}

- (BOOL)anyPaneIsTransparent {
  if ([iTermPreferences boolForKey:kPreferenceKeyPerPaneBackgroundImage] ||
      self.currentSession.backgroundImage == nil) {
...
- (void)setMojaveBackgroundColor:(nullable NSColor *)backgroundColor
  ...
  case TAB_STYLE_LIGHT_HIGH_CONTRAST:
    [self
        safeSetAppearance:[NSAppearance appearanceNamed:NSAppearanceNameAqua]];
    break;

  case TAB_STYLE_DARK:
  case TAB_STYLE_DARK_HIGH_CONTRAST:
    [self safeSetAppearance:[NSAppearance
                                appearanceNamed:NSAppearanceNameDarkAqua]];
    break;
  }
  self.window.backgroundColor =
      self.anyPaneIsTransparent
          ? [[NSColor clearColor] colorWithAlphaComponent:0.01]
          : [NSColor windowBackgroundColor];
  self.window.titlebarAppearsTransparent = [self
      titleBarShouldAppearTransparent]; // Keep it from showing content from
                                        // other windows behind it. Issue 7108.
}
...
- (BOOL)titleBarShouldAppearTransparent {
  if (@available(macOS 10.14, *)) {
    ...
    return [PseudoTerminal
        titleBarShouldAppearTransparentForWindowType:self.windowType];
  }

  switch (self.windowType) {
  case WINDOW_TYPE_LION_FULL_SCREEN:
    return [PseudoTerminal
        titleBarShouldAppearTransparentForWindowType:self.savedWindowType];
    break;
...
+ (BOOL)titleBarShouldAppearTransparentForWindowType:
    (iTermWindowType)windowType {
  if (@available(macOS 10.14, *)) {
    switch (iTermThemedWindowType(windowType)) {
    case WINDOW_TYPE_NORMAL:
...
- (void)setLegacyBackgroundColor:(nullable NSColor *)backgroundColor {
  ...
  if (@available(macOS 10.13, *)) {
    ...
    // Preserve 10.12 behavior. It can change the window title bar color so the
    // appearance is important to keep the title legible. This adds the weird
    // behavior of making the toolbar look buggy when there's a single tab with
    // no visible tabbar and a dark tab color. It's likely 10.12 support will be
    // dropped before I have time to fix this, alas.
    if (backgroundColor == nil &&
        [iTermAdvancedSettingsModel darkThemeHasBlackTitlebar]) {
      switch ((iTermPreferencesTabStyle)
                  [iTermPreferences intForKey:kPreferenceKeyTabStyle]) {
      case TAB_STYLE_LIGHT:
      case TAB_STYLE_LIGHT_HIGH_CONTRAST:
    ...
    darkAppearance =
        (backgroundColor != nil && backgroundColor.perceivedBrightness < 0.5);
  }
...
- (void)tabsDidReorder {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermTabDidChangePositionInWindowNotification
                    object:nil];
  for (PTYSession *session in self.allSessions) {
...
- (BOOL)shouldTweakMinimalTabOutlineAlpha {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyDimBackgroundWindows] &&
    ...
    // Trickery isn't needed because this kind of dimming looks fine for the
    // minimal tabbar outline.
    return NO;
  }
  if ([iTermPreferences boolForKey:kPreferenceKeyDimOnlyText]) {
    return NO;
...
- (id)tabView:(PSMTabBarControl *)tabView
  ...
  if ([option isEqualToString:
                  PSMTabBarControlOptionColoredSelectedTabOutlineStrength]) {
    ...
    return @(
        [iTermAdvancedSettingsModel minimalTabStyleBackgroundColorDifference]);
  } else if (
      [option isEqualToString:
                  PSMTabBarControlOptionColoredUnselectedTabTextProminence]) {
    ...
    const CGFloat alpha =
        [iTermAdvancedSettingsModel minimalTabStyleOutlineStrength];
    if ([self shouldTweakMinimalTabOutlineAlpha]) {
      const CGFloat a =
          [iTermPreferences floatForKey:kPreferenceKeyDimmingAmount] * 0.375;
    ...
    return @(iTermDimmingAmount(tabView));
      ...
      [option
          isEqualToString:
              PSMTabBarControlOptionMinimalStyleTreatLeftInsetAsPartOfFirstTab]) {
    return @([iTermAdvancedSettingsModel
        minimalTabStyleTreatLeftInsetAsPartOfFirstTab]);
  } else if ([option
                 isEqualToString:PSMTabBarControlOptionMinimumSpaceForLabel]) {
    ...
    return @([iTermAdvancedSettingsModel drawBottomLineForHorizontalTabBar]);
      ...
      [option
          isEqualToString:
              PSMTabBarControlOptionMinimalSelectedTabUnderlineProminence]) {
    return
        @([iTermAdvancedSettingsModel minimalSelectedTabUnderlineProminence]);
    ...
    case TAB_STYLE_AUTOMATIC:
      return @0;
                 ...
                 isEqualToString:PSMTabBarControlOptionAttachedToTitleBar]) {
    if (@available(macOS 10.16, *)) {
      iTermPreferencesTabStyle preferredStyle =
          [iTermPreferences intForKey:kPreferenceKeyTabStyle];
      switch (preferredStyle) {
...
- (BOOL)tabViewShouldDragWindow:(NSTabView *)tabView event:(NSEvent *)event {
  ...
  if (_contentView.tabBarControl.numberOfVisibleTabs > 1) {
    const NSPoint point =
        [_contentView.tabBarControl convertPoint:event.locationInWindow
                                        fromView:nil];
...
- (void)openPasswordManagerToAccountName:(NSString *)name
  ...
  _passwordManagerWindowController =
      [[iTermPasswordManagerWindowController alloc] init];
  _passwordManagerWindowController.delegate = self;

  [self.window beginSheet:[_passwordManagerWindowController window]
        completionHandler:^(NSModalResponse returnCode) {
...
- (void)instantReplayExportFrom:(long long)start to:(long long)end {
  [iTermRecordingCodec exportRecording:self.currentSession.liveSession
                                  from:start
                                    to:end];
}
...
- (void)replaceSyntheticActiveSessionWithLiveSessionIfNeeded {
  ...
  if ([[self currentSession] liveSession]) {
    [self showLiveSession:[[self currentSession] liveSession]
                inPlaceOf:[self currentSession]];
  }
}
...
- (NSPoint)originForAccessoryOfSize:(NSSize)size {
  ...
  if (screenRect.origin.y + size.height < windowRect.origin.y) {
    ...
    p.y = [_contentView.tabView convertRect:NSMakeRect(0, 0, 0, 0) toView:nil]
              .origin.y -
          size.height;
  }
  return p;
...
- (void)showHideInstantReplay {
  ...
  if (hide) {
    ...
    _instantReplayWindowController =
        [[iTermInstantReplayWindowController alloc] init];
    NSPoint origin =
        [self originForAccessoryOfSize:_instantReplayWindowController.window
                                           .frame.size];
...
- (BOOL)closeInstantReplay:(id)sender
        orTerminateSession:(BOOL)orTerminateSession {
  if (!self.currentSession.liveSession.screen.dvr.readOnly) {
    [self closeInstantReplayWindow];
    return YES;
...
- (PTYSession *)syntheticSessionForSession:(PTYSession *)oldSession {
  ...
  Profile *profile =
      [self profileForNewSessionPreferringProfile:oldSession.profile];
  ...
  [[newSession view] setShowTitle:[[oldSession view] showTitle]
                 adjustScrollView:YES];
  [[newSession view] setShowBottomStatusBar:oldSession.view.showBottomStatusBar
                           adjustScrollView:YES];
...
- (IBAction)filter:(id)sender {
  ...
  if (query) {
    [self showLinesMatchingQuery:query fromSession:session];
  }
}
...
- (IBAction)zoomOnSelection:(id)sender {
  ...
  if (sub) {
    VT100GridAbsCoordRangeTryMakeRelative(
        sub.absRange.coordRange, session.screen.totalScrollbackOverflow,
...
- (void)showLinesMatchingQuery:(NSString *)query
                   fromSession:(PTYSession *)oldSession {
  ...
  [syntheticSession
      setSessionSpecificProfileValues:@{KEY_UNLIMITED_SCROLLBACK : @YES}];
  ...
  [[self tabForSession:oldSession]
      replaceActiveSessionWithSyntheticSession:syntheticSession];
}

- (void)showRangeOfLines:(NSRange)rangeOfLines
               inSession:(PTYSession *)oldSession {
  PTYSession *syntheticSession = [self syntheticSessionForSession:oldSession];
  [syntheticSession.screen terminalSetCursorVisible:NO];
...
- (IBAction)runCoprocess:(id)sender {
  ...
  [coprocessIgnoreErrors_
      setState:[Coprocess
                   shouldIgnoreErrorsFromCommand:coprocessCommand_.stringValue]
...
- (IBAction)coprocessPanelEnd:(id)sender {
  if (sender == coprocessOkButton_) {
    if ([[[coprocessCommand_ stringValue]
    ...
    [[self currentSession]
        launchCoprocessWithCommand:[coprocessCommand_ stringValue]];
    [Coprocess setSilentlyIgnoreErrors:[coprocessIgnoreErrors_ state] ==
                                       NSControlStateValueOn
                           fromCommand:[coprocessCommand_ stringValue]];
  }
  [NSApp stopModal];
}
...
- (IBAction)coprocessHelp:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"http://www.iterm2.com/coprocesses.html"]];
}
...
- (IBAction)addTrigger:(id)sender {
  [self.currentSession
      textViewAddTrigger:self.currentSession.selectedText ?: @""];
}
...
- (IBAction)openCommandHistory:(id)sender {
  if (!commandHistoryPopup) {
    commandHistoryPopup =
        [[CommandHistoryPopupWindowController alloc] initForAutoComplete:NO];
  }
  if ([[iTermShellHistoryController sharedInstance]
          commandHistoryHasEverBeenUsed]) {
    [self openPopupWindow:commandHistoryPopup];
    [commandHistoryPopup
          loadCommands:[commandHistoryPopup
                           commandsForHost:[[self currentSession] currentHost]
                            partialCommand:[[self currentSession]
                                               currentCommand]
                                    expand:YES]
...
- (IBAction)openDirectories:(id)sender {
  if (!_directoriesPopupWindowController) {
    _directoriesPopupWindowController =
        [[DirectoriesPopupWindowController alloc] init];
  }
  if ([[iTermShellHistoryController sharedInstance]
          commandHistoryHasEverBeenUsed]) {
    [self openPopupWindow:_directoriesPopupWindowController];
    [_directoriesPopupWindowController
        loadDirectoriesForHost:[[self currentSession] currentHost]];
  } else {
    [iTermShellHistoryController showInformationalMessage];
  }
}
  ...
  if ([session.guid isEqualToString:self.autoCommandHistorySessionGuid]) {
    ...
    [NSObject
        cancelPreviousPerformRequestsWithTarget:self
                                       selector:@selector
                                       (reallyShowAutoCommandHistoryForSession:)
                                         object:session];
  }
}
...
- (void)updateAutoCommandHistoryForPrefix:(NSString *)prefix
  ...
  if ([session.guid isEqualToString:self.autoCommandHistorySessionGuid]) {
    if (!commandHistoryPopup) {
      commandHistoryPopup =
          [[CommandHistoryPopupWindowController alloc] initForAutoComplete:YES];
    }
    NSArray<iTermCommandHistoryCommandUseMO *> *commands =
        [commandHistoryPopup commandsForHost:[session currentHost]
    ...
    [commandHistoryPopup loadCommands:commands partialCommand:prefix];
  }
  if (_autocompleteCandidateListItem && session == self.currentSession) {
    iTermShellHistoryController *history =
        [iTermShellHistoryController sharedInstance];
    NSArray<NSString *> *commands =
        [[history commandHistoryEntriesWithPrefix:prefix
                                           onHost:[session currentHost]]
            mapWithBlock:^id(iTermCommandHistoryEntryMO *anObject) {
...
- (void)showAutoCommandHistoryForSession:(PTYSession *)session {
  if ([iTermPreferences boolForKey:kPreferenceAutoCommandHistory]) {
    // Use a delay so we don't get a flurry of windows appearing when restoring
    // arrangements.
...
- (BOOL)autoCommandHistoryIsOpenForSession:(PTYSession *)session {
  return [[commandHistoryPopup window] isVisible] &&
         [self.autoCommandHistorySessionGuid isEqualToString:session.guid];
}
...
- (IBAction)openAutocomplete:(id)sender {
  ...
  if ([[autocompleteView window] isVisible]) {
    ...
    [autocompleteView
        addCommandEntries:[[self currentSession]
                              autocompleteSuggestionsForCurrentCommand]
...
- (BOOL)canSplitPaneVertically:(BOOL)isVertical
  ...
  NSFont *asciiFont = [ITAddressBookMgr
      fontWithDesc:[theBookmark objectForKey:KEY_NORMAL_FONT]];
  NSFont *nonAsciiFont = [ITAddressBookMgr
      fontWithDesc:[theBookmark objectForKey:KEY_NON_ASCII_FONT]];
  NSSize asciiCharSize = [PTYTextView
        charSizeForFont:asciiFont
      horizontalSpacing:[[theBookmark objectForKey:KEY_HORIZONTAL_SPACING]
                            doubleValue]
        verticalSpacing:[[theBookmark objectForKey:KEY_VERTICAL_SPACING]
                            doubleValue]];
  NSSize nonAsciiCharSize = [PTYTextView
        charSizeForFont:nonAsciiFont
      horizontalSpacing:[[theBookmark objectForKey:KEY_HORIZONTAL_SPACING]
                            doubleValue]
        verticalSpacing:[[theBookmark objectForKey:KEY_VERTICAL_SPACING]
                            doubleValue]];
  NSSize charSize =
      NSMakeSize(MAX(asciiCharSize.width, nonAsciiCharSize.width),
                 MAX(asciiCharSize.height, nonAsciiCharSize.height));
  NSSize newSessionSize = NSMakeSize(
      charSize.width * kVT100ScreenMinColumns +
          [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
      charSize.height * kVT100ScreenMinRows +
          [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2);

  return [[self currentTab] canSplitVertically:isVertical
                                      withSize:newSessionSize];
}
...
- (void)recreateTab:(PTYTab *)tab
  ...
  DLog(@"Re-create tab");
  NSInteger tabIndex =
      [_contentView.tabView indexOfTabViewItemWithIdentifier:tab];
  ...
  NSDictionary<NSString *, PTYSession *> *theMap =
      [PTYTab sessionMapWithArrangement:arrangement sessions:allSessions];

  BOOL ok = (theMap != nil);
  if (ok) {
    DLog(@"Found session map");
    // Make sure the proposed tab has at least all the sessions already in the
    // current tab.
...
- (void)addTabWithArrangement:(NSDictionary *)arrangement
  ...
  DLog(@"construct session map with sessions: %@\nArrangement:\n%@", sessions,
       arrangement);
  NSDictionary<NSString *, PTYSession *> *sessionMap =
      [PTYTab sessionMapWithArrangement:arrangement sessions:sessions];
  if (!sessionMap) {
...
- (void)splitVertically:(BOOL)isVertical
  ...
  if (newSession.tabColor) {
    ...
    // Inherit from tab.
    tabColor = [[[_contentView.tabBarControl
        tabColorForTabViewItem:[[self currentTab] tabViewItem]] retain]
        autorelease];
  }
  ...
  if ([[ProfileModel sessionsInstance]
          bookmarkWithGuid:newSession.profile[KEY_GUID]]) {
    // We know the GUID is unique and in sessions instance and the original guid
    // is already set. This might be possible to do earlier, but I'm afraid of
    // introducing bugs.
    [newSession
        inheritDivorceFrom:targetSession
                    decree:[NSString stringWithFormat:
                                         @"Split vertically with guid %@",
                                         newSession.profile[KEY_GUID]]];
  }
  if (![newSession.tabColor isEqual:tabColor] &&
...
- (void)asyncSplitVertically:(BOOL)isVertical
  ...
  if ([targetSession isTmuxClient]) {
    ...
    [controller
         splitWindowPane:[targetSession tmuxPane]
              vertically:isVertical
                   scope:[[self tabForSession:targetSession] variablesScope]
        initialDirectory:[iTermInitialDirectory
                             initialDirectoryFromProfile:targetSession.profile
              ...
              completion:^(int wp) {
                ...
                [controller whenPaneRegistered:wp
                                          call:^(PTYSession *newSession) {
                                            if (completion) {
  ...
  if (currentSession) {
    [currentSession asyncCurrentLocalWorkingDirectoryOrInitialDirectory:^(
                        NSString *oldCWD) {
  ...
  ITBetaAssert(
      NO, @"This should be impossible! Splitting without a current session.");
  PTYSession *session = [self splitVertically:isVertical
                                       before:before
                                      profile:theBookmark
                                targetSession:targetSession
...
- (PTYSession *)splitVertically:(BOOL)isVertical
  ...
  if ([[ProfileModel sessionsInstance]
          bookmarkWithGuid:theBookmark[KEY_GUID]]) {
    // We were given a profile that belongs to an existing divorced session.
    //
    // Don't want to have two divorced sessions with the same guid. Allocate a
    // new sessions instance bookmark with a unique GUID. The isDivorced flag
    // gets set later, by splitVertically:before:...
  ...
  PTYSession *newSession =
      [[self.sessionFactory newSessionWithProfile:theBookmark
...
- (Profile *)profileForSplittingCurrentSession {
  return [self.currentSession profileForSplit]
             ?: [[ProfileModel sharedInstance] defaultBookmark];
}
...
- (void)fitWindowToTabsExcludingTmuxTabs:(BOOL)excludeTmux
  ...
  [self fitWindowToTabsExcludingTmuxTabs:excludeTmux
                        ...
                        sizeOfLargestTab:
                            [self sizeOfLargestTabExcludingTmux:excludeTmux]];
}
...
- (void)fitWindowToTabsExcludingTmuxTabs:(BOOL)excludeTmux
  ...
  NSNumber *preferredHeight =
      preserveHeight ? @(self.window.frame.size.height) : nil;
  if (![self fitWindowToTabSize:maxTabSize preferredHeight:preferredHeight]) {
    // Sometimes the window doesn't resize but widgets need to be moved. For
    // example, when toggling the scrollbar.
    [self repositionWidgets];
  }
}
...
// NOTE: The preferred height is respected only if it would be larger than the
// height the window would otherwise be set to and is less than the max height
// (self.maxFrame.size.height).
- (BOOL)fitWindowToTabSize:(NSSize)tabSize
           preferredHeight:(NSNumber *)preferredHeight {
  DLog(@"fitWindowToTabSize:%@ preferredHeight:%@", NSStringFromSize(tabSize),
       preferredHeight);
  if ([self anyFullScreen]) {
  ...
  // Set the window size to be large enough to encompass that tab plus its
  // decorations.
  NSSize decorationSize = [self windowDecorationSize];
  ...
  if (preferredHeight && preferredHeight.doubleValue > winSize.height) {
    ...
    DLog(@"Ignoring preferred height %@ with winSize.height %@",
         preferredHeight, @(winSize.height));
  }

  NSRect frame = [[self window] frame];
  DLog(@"Original frame: %@ maxy=%@", NSStringFromRect(frame),
       @(NSMaxY(frame)));
  ...
  NSSize maxFrameSize = [self maxFrame].size;
  PtyLog(@"maxFrameSize=%@, screens=%@", [NSValue valueWithSize:maxFrameSize],
         [NSScreen screens]);
  ...
  DLog(@"Existing height is %@. heightChange is %@",
       @([[self window] frame].size.height), @(heightChange));
  frame.size = winSize;
  ...
  if (workAroundBugFix) {
    ...
    // mask for the _contentView.tabView (which I really don't want
    // autoresized--it needs to be done by hand in fitTabToWindow), and add a
    // silly one pixel view that lives just long enough to be resized in this
    // function. I don't know why it works but it does.
    bugFixView =
        [[[NSView alloc] initWithFrame:NSMakeRect(0, 0, 1, 1)] autorelease];
  ...
  DLog(@"fitWindowToTabSize using screen number %@ with frame %@",
       @([[NSScreen screens] indexOfObject:self.screen]),
       NSStringFromRect(self.screen.frame));

  // Update the frame for the window style, but don't mess with the size we've
  // computed except as needed (e.g., for edge-spanning x-of-screen windows).
  frame = [self canonicalFrameForScreen:self.screen
                            windowFrame:frame
                           preserveSize:YES];
  ...
  if (bugFixView) {
    // Restore _contentView.tabView's autoresizingMask and remove the stupid
    // bugFixView.
...
- (IBAction)selectPaneLeft:(id)sender {
  PTYSession *session =
      [[self currentTab] sessionLeftOf:[[self currentTab] activeSession]];
  if (session) {
...
- (IBAction)selectPaneRight:(id)sender {
  PTYSession *session =
      [[self currentTab] sessionRightOf:[[self currentTab] activeSession]];
  if (session) {
    [[self currentTab] setActiveSession:session];
  }
}
...
- (IBAction)selectPaneUp:(id)sender {
  PTYSession *session =
      [[self currentTab] sessionAbove:[[self currentTab] activeSession]];
  if (session) {
    [[self currentTab] setActiveSession:session];
  }
}
...
- (IBAction)selectPaneDown:(id)sender {
  PTYSession *session =
      [[self currentTab] sessionBelow:[[self currentTab] activeSession]];
  if (session) {
    [[self currentTab] setActiveSession:session];
  }
}
...
- (IBAction)movePaneDividerRight:(id)sender {
  [[self currentTab] moveCurrentSessionDividerBy:1 horizontally:YES];
}
...
- (IBAction)movePaneDividerDown:(id)sender {
  [[self currentTab] moveCurrentSessionDividerBy:1 horizontally:NO];
}
...
- (void)swapPaneLeft {
  PTYSession *session =
      [[self currentTab] sessionLeftOf:[[self currentTab] activeSession]];
  if (session) {
    [[self currentTab] swapSession:[[self currentTab] activeSession]
                       withSession:session];
  }
}
...
- (float)minWidth {
  // Pick 400 as an absolute minimum just to be safe. This is rather arbitrary
  // and hacky.
  float minWidth = 400;
  for (NSTabViewItem *tabViewItem in [_contentView.tabView tabViewItems]) {
    PTYTab *theTab = [tabViewItem identifier];
...
- (PTYTab *)
    openTabWithArrangement:(NSDictionary *)arrangement
                     named:(NSString *)arrangementName
  ...
  [theTab didAddToTerminal:self withArrangement:arrangement];
  return theTab;
}
...
- (void)setBroadcastingSessions:(NSArray<NSArray<PTYSession *> *> *)domains {
  NSSet<NSSet<NSString *> *> *stringDomains = [NSSet
      setWithArray:[domains mapWithBlock:^id(NSArray<PTYSession *> *sessions) {
        return [NSSet
            setWithArray:[sessions mapWithBlock:^id(PTYSession *session) {
              return session.guid;
            }]];
      }]];
...
- (void)setSplitSelectionMode:(BOOL)mode
  ...
  for (PTYSession *aSession in [self allSessions]) {
    if (mode) {
      [aSession setSplitSelectionMode:(aSession != session)
...
- (IBAction)moveTabLeft:(id)sender {
  NSInteger selectedIndex = [_contentView.tabView
      indexOfTabViewItem:[_contentView.tabView selectedTabViewItem]];
...
- (void)moveTabAtIndex:(NSInteger)selectedIndex
  ...
  [_contentView.tabBarControl moveTabAtIndex:selectedIndex
...
- (IBAction)moveTabRight:(id)sender {
  ...
  NSInteger destinationIndex =
      (selectedIndex + 1) % [_contentView.tabView numberOfTabViewItems];
  [self moveTabAtIndex:selectedIndex toIndex:destinationIndex];
}
...
// This adjusts the window size to fit tabs by requesting tabs to compute their
// "ideal" size. The ideal size is the smallest size that fits all panes without
// requiring any to shrink, although some may need to grow. For tmux tabs, their
// existing sizes are preserved exactly and the window grows as needed (probably
// leaving "holes" if there are split panes present).
...
- (void)updateWindowNumberVisibility:(NSNotification *)aNotification {
  ...
  if (@available(macOS 10.14, *)) {
    if (_shortcutAccessoryViewController) {
      _shortcutAccessoryViewController.view.hidden =
          ![iTermPreferences boolForKey:kPreferenceKeyShowWindowNumber];
    } else {
      [self setWindowTitle];
    }
  } else {
...
- (void)refreshTerminal:(NSNotification *)aNotification {
  ...
  // Save the size of the largest tab before adding or removing tabbar
  // accessories. We want to preserve the tab size when we update the window
  // size below. Modifying the title bar accessories synchronously changes the
  // tabs' sizes, causing the window not to resize when the tab bar shows or
  // hides.
  const NSSize tabSizeBeforeUpdatingTitleBarAccessories =
      [self sizeOfLargestTabExcludingTmux:NO];
  ...
  if ([self fullScreen] && ![self lionFullScreen]) {
    if ([[self window] isKeyWindow]) {
      // This is only used when changing broadcast mode; otherwise, the
      // kRefreshTerminalNotification notif is never posted when this window is
      // key.
      [[iTermPresentationController sharedInstance] update];
    }
  ...
  [self fitWindowToTabsExcludingTmuxTabs:NO
                        preservingHeight:NO
                        sizeOfLargestTab:
                            tabSizeBeforeUpdatingTitleBarAccessories];
  ...
  for (int i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
    ...
    // Update dimmed status of inactive sessions in split panes in case the
    // preference changed.
    for (PTYSession *aSession in [aTab sessions]) {
      [self setDimmingForSession:aSession];
      [[aSession view] setBackgroundDimmed:![[self window] isKeyWindow]];
  ...
  [_contentView invalidateAutomaticTabBarBackingHiding];
  // If the theme changed from light to dark make sure split pane dividers
  // redraw.
...
- (NSColor *)windowDecorationColor {
  ...
  if (self.currentSession.tabColor && fakeWindowTitleBar &&
    ...
    return [_contentView.tabBarControl.style
        textColorForCell:_contentView.tabBarControl.cells.firstObject];
  }

  // The window number will be displayed over the tabbar color. For non-key
  // windows, use the non-selected tab text color because that more closely
  // matches the titlebar color.
  const BOOL mainAndActive = (self.window.isMainWindow && NSApp.isActive);
  NSColor *color;
  color = [_contentView.tabBarControl.style
        textColorDefaultSelected:mainAndActive
                 backgroundColor:fakeWindowTitleBar
                                     ? self.currentSession.tabColor
                                     : nil
...
- (NSImage *)rootTerminalViewCurrentTabIcon {
  return self.currentSession.shouldShowTabGraphic
             ? self.currentSession.tabGraphic
             : nil;
}
...
- (BOOL)shouldRevealStandardWindowButtons {
  ...
  const iTermWindowType windowType =
      exitingLionFullscreen_ ? _savedWindowType : _windowType;
  switch (windowType) {
  case WINDOW_TYPE_TOP:
  case WINDOW_TYPE_LEFT:
  case WINDOW_TYPE_RIGHT:
...
- (iTermStatusBarViewController *)
    rootTerminalViewSharedStatusBarViewController {
...
- (BOOL)rootTerminalViewShouldLeaveEmptyAreaAtTop {
  if ([PseudoTerminal windowTypeHasFullSizeContentView:self.windowType]) {
    DLog(@"YES because window type %@ has full size content view",
         @(self.windowType));
    return YES;
  }
  if (!self.anyFullScreen) {
  ...
  if ([PseudoTerminal windowTypeHasFullSizeContentView:self.savedWindowType]) {
    DLog(@"YES because saved window type %@ has full size content view",
         @(self.savedWindowType));
    if (![iTermAdvancedSettingsModel allowTabbarInTitlebarAccessoryBigSur] &&
        !self.lionFullScreen && !togglingLionFullScreen_) {
      if (@available(macOS 10.16, *)) {
  ...
  DLog(@"NO because saved window type %@ does not have full size content view",
       @(self.savedWindowType));
  return NO;
}

// Generally yes, but not when a fake titlebar is shown *and* the window has
// transparency. Fake titlebars need a background because transparent windows
// won't give you one for free.
- (BOOL)rootTerminalViewShouldHideTabBarBackingWhenTabBarIsHidden {
  if (@available(macOS 10.14, *)) {
  } else {
    // Doesn't matter but let's not think about 10.13 and earlier since it won't
    // happen
    return YES;
  }
  if (![PseudoTerminal windowTypeHasFullSizeContentView:self.windowType]) {
    // No full size content view? Then there won't be a fake title bar that
    // needs backing.
    return YES;
  }
  ...
  if ([iTermPreferences intForKey:kPreferenceKeyTabStyle] ==
      TAB_STYLE_MINIMAL) {
    return YES;
  }
  return NO;
...
- (void)updateTabBarStyle {
  id<PSMTabStyle> style = [[iTermTheme sharedInstance]
      tabStyleWithDelegate:self
       effectiveAppearance:self.window.effectiveAppearance];
  [_contentView.tabBarControl setStyle:style];
  [_contentView.tabBarControl
      setTabsHaveCloseButtons:
          [iTermPreferences boolForKey:kPreferenceKeyTabsHaveCloseButton]];
...
- (void)setFrameSize:(NSSize)newSize {
  ...
  NSRect frame = [[self window] frame];
  [[self window] setFrame:NSMakeRect(frame.origin.x, frame.origin.y, size.width,
                                     size.height)
                  display:YES];
}
...
- (BOOL)haveLeftBorder {
  BOOL leftTabBar = ([iTermPreferences intForKey:kPreferenceKeyTabPosition] ==
                     PSMTab_LeftTab);
  if (!self.shouldShowBorder) {
    return NO;
  } else if ([self anyFullScreen] || self.windowType == WINDOW_TYPE_LEFT ||
...
- (BOOL)haveBottomBorder {
  ...
  BOOL bottomTabBar = ([iTermPreferences intForKey:kPreferenceKeyTabPosition] ==
                       PSMTab_BottomTab);
  if (!self.shouldShowBorder) {
    return NO;
  }
  if ([self anyFullScreen] || self.windowType == WINDOW_TYPE_BOTTOM) {
    return NO;
  }
  if (!bottomTabBar) {
  ...
  // Visible bottom tab bar with light style. It's light enough so it doesn't
  // need a border.
  return NO;
}
...
- (BOOL)haveTopBorder {
  ...
  BOOL windowTypeCompatibleWithTopBorder =
      (self.windowType == WINDOW_TYPE_BOTTOM ||
       self.windowType == WINDOW_TYPE_NO_TITLE_BAR ||
       self.windowType == WINDOW_TYPE_BOTTOM_PARTIAL);
  return (!visibleTopTabBar && windowTypeCompatibleWithTopBorder);
}

- (BOOL)haveRightBorderRegardlessOfScrollBar {
  if (!self.shouldShowBorder) {
    return NO;
  } else if ([self anyFullScreen] || self.windowType == WINDOW_TYPE_RIGHT) {
    return NO;
  }
  return YES;
}
...
- (NSSize)windowDecorationSize {
  ...
  if (!_contentView.tabBarControl.flashing &&
    ...
    case PSMTab_LeftTab:
      if (self.tabs.count == 1 && self.tabs.firstObject.reportIdeal) {
        // Initial setup. Do this so we can get the initial session's desired
        // number of columns. Issue 8365.
  ...
  NSSize result =
      [[self window]
          frameRectForContentRect:NSMakeRect(0, 0, decorationSize.width,
                                             decorationSize.height)]
          .size;
  if (self.shouldCompensateForDisappearingTabBarAccessory) {
    // Tab bar is currently an accessory but is about to go away and should not
    // be included. We can't remove it before measuring in this case because
    // then the tabview grows to fill the space, causing -fitWindowToTabs to
    // keep the window the same size instead of shrinking it by the height of
    // the tab bar. This code path should only be taken when the last tab
    // disappears and tabView:didChangeNumberOfTabViewItems: calls
    // -fitWindowToSize.
...
- (void)flagsChanged:(NSEvent *)theEvent {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermFlagsChanged"
                    object:theEvent
                  userInfo:nil];
  ...
  if (!(modifierFlags & NSEventModifierFlagCommand) &&
    ...
    if ([[NSDate date]
            timeIntervalSinceDate:
                [NSDate dateWithTimeIntervalSince1970:findCursorStartTime_]] >
        kFindCursorHoldTime) {
  ...
  _contentView.tabBarControl.cmdPressed =
...
- (float)maxCharWidth:(int *)numChars {
  ...
  for (int i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
    ...
    for (PTYSession *session in [tab sessions]) {
      ...
      PtyLog(@"maxCharWidth - session %d has %dx%d, chars are %fx%f", i,
...
- (float)maxCharHeight:(int *)numChars {
  ...
  for (int i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
    ...
    for (PTYSession *session in [tab sessions]) {
      ...
      PtyLog(@"maxCharHeight - session %d has %dx%d, chars are %fx%f", i,
             [session columns], [session rows], [[session textview] charWidth],
             [[session textview] lineHeight]);
      if (h > max) {
        max = h;
...
- (float)tallestSessionHeight:(int *)numChars {
  ...
  for (int i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
    ...
    for (PTYSession *session in [tab sessions]) {
      float h = [[session textview] lineHeight];
      PtyLog(@"tallestSessionheight - session %d has %dx%d, chars are %fx%f", i,
             [session columns], [session rows], [[session textview] charWidth],
             [[session textview] lineHeight]);
...
// Set the session's profile dictionary and initialize its screen and name. Sets
// the window title to the session's name. If size is not nil then the session
// is initialized to fit a view of that size; otherwise the size is derived from
// the existing window if there is already an open tab, or its bookmark's
// preference if it's the first session in the window.
- (void)setupSession:(PTYSession *)aSession withSize:(NSSize *)size {
  ...
  NSSize charSize = [PTYTextView
        charSizeForFont:[ITAddressBookMgr
                            fontWithDesc:[profile objectForKey:KEY_NORMAL_FONT]]
      horizontalSpacing:[[profile objectForKey:KEY_HORIZONTAL_SPACING]
                            doubleValue]
        verticalSpacing:[[profile objectForKey:KEY_VERTICAL_SPACING]
                            doubleValue]];

  if (size == nil && [_contentView.tabView numberOfTabViewItems] != 0) {
    NSSize contentSize =
        [[[[self currentSession] view] scrollview] documentVisibleRect].size;
    rows = (contentSize.height -
            [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) /
           charSize.height;
    columns = (contentSize.width -
               [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2) /
              charSize.width;
  }
  NSRect sessionRect;
  if (size != nil) {
    ...
    sessionRect = NSMakeRect(
        0, 0,
        columns * charSize.width +
            [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
...
- (void)moveSessionToWindow:(id)sender {
  [[MovePaneController sharedInstance]
      moveSessionToNewWindow:[self currentSession]
                     atPoint:[[self window]
                                 pointToScreenCoords:NSMakePoint(10, -10)]];
}
...
- (void)safelySetSessionSize:(PTYSession *)aSession
  ...
  if (![self anyFullScreen]) {
    ...
    // With split panes it is very difficult to directly compute the maximum
    // size of any given pane. However, any growth in a pane can be taken up by
    // the window as a whole. We compute the maximum amount the window can grow
    // and ensure that the rows and columns won't cause the window to exceed the
    // max size.

    // 1. Figure out how big the tabview can get assuming window decoration
    // remains unchanged.
    ...
    [[aSession.view scrollview]
        setPageScroll:2 * [[aSession textview] lineHeight]];
...
- (void)fitTabToWindow:(PTYTab *)aTab {
  ...
  PtyLog(@"fitTabToWindow calling setSize for content size of %@",
...
- (void)insertTab:(PTYTab *)aTab atIndex:(int)anIndex {
  ...
  if ([_contentView.tabView indexOfTabViewItemWithIdentifier:aTab] ==
    ...
    NSTabViewItem *aTabViewItem =
        [[NSTabViewItem alloc] initWithIdentifier:(id)aTab];
    [aTabViewItem setLabel:@""];
    assert(aTabViewItem);
    [aTab setTabViewItem:aTabViewItem];
    PtyLog(@"insertTab:atIndex - calling [_contentView.tabView "
           @"insertTabViewItem:atIndex]");
    const int safeIndex =
        MAX(0, MIN(_contentView.tabView.tabViewItems.count, anIndex));
    ...
    if (self.windowInitialized && !_restoringWindow) {
      if (self.tabs.count == 1) {
        // It's important to do this before makeKeyAndOrderFront because API
        // clients need to know the window exists before learning that it has
        // focus.
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermDidCreateTerminalWindowNotification
                          object:self];
      }
    }
    if (self.windowInitialized && !_fullScreen && !_restoringWindow) {
      ...
      PtyLog(@"window not initialized, is fullscreen, or is being restored. "
             @"Stack:\n%@",
             [NSThread callStackSymbols]);
    }
...
- (PTYTab *)insertSession:(PTYSession *)aSession atIndex:(int)anIndex {
  ...
  // create a new tab
  PTYTab *aTab = [[PTYTab alloc] initWithSession:aSession parentWindow:self];
  [aSession setIgnoreResizeNotifications:YES];
...
- (NSString *)undecoratedWindowTitle {
  ...
  if (![self tabBarShouldBeVisible] &&
      ![iTermAdvancedSettingsModel showWindowTitleWhenTabBarInvisible]) {
    return self.currentSession.nameController.presentationSessionTitle
               ?: @"Untitled";
  }
...
// Assign a value to the 'uniqueNumber_' member variable which is used for
// storing window frame positions between invocations of iTerm.
...
- (IBAction)saveContents:(id)sender {
  ...
  dateFormatter.dateFormat =
      [NSDateFormatter dateFormatFromTemplate:@"yyyyMMMd"
                                      options:0
                                       locale:[NSLocale currentLocale]];
  NSDateFormatter *timeFormatter = [[[NSDateFormatter alloc] init] autorelease];
  timeFormatter.dateFormat =
      [NSDateFormatter dateFormatFromTemplate:@"hh.mm.ss"
                                      options:0
                                       locale:[NSLocale currentLocale]];

  NSDate *now = [NSDate date];
  NSString *suggestedFilename =
      [NSString stringWithFormat:@"iTerm2 Session %@ at %@.txt",
                                 [dateFormatter stringFromDate:now],
                                 [timeFormatter stringFromDate:now]];
  iTermSavePanel *savePanel =
      [iTermSavePanel showWithOptions:kSavePanelOptionFileFormatAccessory
    ...
    if (url) {
      if ([[url pathExtension] isEqualToString:@"rtf"]) {
        NSAttributedString *attributedString =
            [self.currentSession.textview contentWithAttributes:YES];
        ...
        [[self.currentSession.textview content] writeToFile:url.path
                                                 atomically:NO
...
- (IBAction)startStopLogging:(id)sender {
  if (self.currentSession.logging) {
    ...
    [[self retain] autorelease]; // Prevent self from getting dealloc'ed during
                                 // modal panel.
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
  ...
  if ([item action] == @selector(detachTmux:) ||
    ...
    const BOOL ok =
        (self.currentSession.isTmuxClient &&
         self.currentSession.tmuxController.gateway.pauseModeEnabled);
    if (ok) {
      item.state = self.currentSession.tmuxPaused ? NSControlStateValueOn
                                                  : NSControlStateValueOff;
    }
    return ok;
    ...
    [item setState:_contentView.shouldShowToolbelt ? NSControlStateValueOn
                                                   : NSControlStateValueOff];
    ...
    item.state = (_broadcastInputHelper.broadcastMode == BROADCAST_TO_ALL_TABS)
                     ? NSControlStateValueOn
                     : NSControlStateValueOff;
  } else if (item.action == @selector(enableSendInputToAllPanes:)) {
    item.state = (_broadcastInputHelper.broadcastMode == BROADCAST_TO_ALL_PANES)
                     ? NSControlStateValueOn
                     : NSControlStateValueOff;
  } else if (item.action == @selector(disableBroadcasting:)) {
    item.state = (_broadcastInputHelper.broadcastMode == BROADCAST_OFF)
                     ? NSControlStateValueOn
                     : NSControlStateValueOff;
    ...
    item.state =
        session.logging ? NSControlStateValueOn : NSControlStateValueOff;
    return YES;
  } else if ([item action] == @selector(irPrev:)) {
    result = ![[self currentSession] liveSession] &&
             [[self currentSession] canInstantReplayPrev];
    ...
    PTYSession *session = [self currentSession];
    [item setState:session.highlightCursorLine ? NSControlStateValueOn
                                               : NSControlStateValueOff];
    result = YES;
  } else if ([item action] == @selector
             (toggleSelectionRespectsSoftBoundaries:)) {
    [item setState:[[iTermController sharedInstance]
                       selectionRespectsSoftBoundaries]
                       ? NSControlStateValueOn
                       : NSControlStateValueOff];
    result = YES;
  } else if ([item action] == @selector(toggleAutoCommandHistory:)) {
    result = [[iTermShellHistoryController sharedInstance]
        commandHistoryHasEverBeenUsed];
    if (result) {
      if ([item respondsToSelector:@selector(setState:)]) {
        [item
            setState:[iTermPreferences boolForKey:kPreferenceAutoCommandHistory]
                         ? NSControlStateValueOn
                         : NSControlStateValueOff];
      }
    } else {
    ...
    PTYSession *currentSession = [self currentSession];
    if ([item respondsToSelector:@selector(setState:)]) {
      [item setState:currentSession.alertOnNextMark ? NSControlStateValueOn
                                                    : NSControlStateValueOff];
    }
    result = (currentSession != nil);
  } else if (item.action == @selector(nextMark:) || item.action == @selector
                                                        (previousMark:)) {
    ...
    if (controller) {
      // Any object whose window controller implements this selector is closed
      // by cmd-w: pseudoterminal (closes a pane), preferences, bookmarks
    ...
    result = ![[[self currentSession] screen]
        allCharacterSetPropertiesHaveDefaultValues];
    ...
    return [[iTermShellHistoryController sharedInstance]
        haveCommandsForHost:[[self currentSession] currentHost]];
    ...
    return [[iTermShellHistoryController sharedInstance]
        haveDirectoriesForHost:[[self currentSession] currentHost]];
  } else if ([item action] == @selector(movePaneDividerDown:)) {
    return [[self currentTab] canMoveCurrentSessionDividerBy:1 horizontally:NO];
    ...
    item.state =
        [iTermPreferences boolForKey:kPreferenceKeySizeChangesAffectProfile]
            ? NSControlStateValueOn
            : NSControlStateValueOff;
    return YES;
    ...
    iTermTabColorMenuItem *colorMenuItem =
        [iTermTabColorMenuItem castFrom:item];
    ...
    item.state = (iTermWindowTypeNormalized(self.windowType) == item.tag)
                     ? NSControlStateValueOn
                     : NSControlStateValueOff;
    return YES;
  }
...
- (IBAction)mergeAllWindows:(id)sender {
  for (PseudoTerminal *term in
       [[[[iTermController sharedInstance] terminals] copy] autorelease]) {
    if (term == self) {
      continue;
    }

    while (term.tabs.count) {
      [MovePaneController moveTab:term.tabs.firstObject
                         toWindow:self
                          atIndex:self.tabs.count];
    }
  }
}
...
- (IBAction)toggleAutoCommandHistory:(id)sender {
  [iTermPreferences
      setBool:![iTermPreferences boolForKey:kPreferenceAutoCommandHistory]
...
- (IBAction)enableSendInputToAllPanes:(id)sender {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"iTermWindowBecameKey"
                    object:self
                  userInfo:nil];
  [self setWindowTitle];
}
...
- (void)fitTabsToWindow {
  ...
  for (int i = 0; i < [_contentView.tabView numberOfTabViewItems]; ++i) {
    [self fitTabToWindow:[[_contentView.tabView tabViewItemAtIndex:i]
                             identifier]];
  }
...
- (BOOL)showCloseWindow {
  return ([self
      confirmCloseForSessions:[self allSessions]
                   identifier:@"This window"
                  genericName:[NSString stringWithFormat:@"Window #%d",
                                                         number_ + 1]]);
}
...
- (void)closeTabContextualMenuAction:(id)sender {
  ...
  if ([self tabView:_contentView.tabView
          shouldCloseTabViewItem:tabToClose.tabViewItem]) {
...
- (void)createDuplicateOfTab:(PTYTab *)theTab {
  ...
  PseudoTerminal *destinationTerminal = [[iTermController sharedInstance]
      windowControllerForNewTabWithProfile:self.currentSession.profile
                                 candidate:self
                        respectTabbingMode:NO];
  if (destinationTerminal == nil) {
    ...
    [iTermSessionLauncher
            launchBookmark:self.currentSession.profile
                inTerminal:nil
                   withURL:nil
          hotkeyWindowType:iTermHotkeyWindowTypeNone
                   makeKey:YES
        ...
        respectTabbingMode:NO
                   command:nil
               makeSession:^(Profile *profile, PseudoTerminal *term,
                 ...
                 // This prevents the tab from getting resized to fit the
                 // window.
...
- (PseudoTerminal *)it_moveTabToNewWindow:(PTYTab *)aTab {
  ...
  ITAssertWithMessage([self.tabs containsObject:aTab],
                      @"Called on wrong window");
...
- (IBAction)changeTabColorToMenuAction:(id)sender {
  // If we got here because you right clicked on a tab, use the represented
  // object.
  NSTabViewItem *aTabViewItem = [sender representedObject];
  PTYTab *aTab = [aTabViewItem identifier];
...
- (void)reloadBookmarks {
  ...
  for (PTYSession *session in [self allSessions]) {
    ...
    NSString *oldName =
        [[[oldBookmark objectForKey:KEY_NAME] copy] autorelease];
    ...
    if ([session reloadProfile]) {
      ...
      if ([session isDivorced] &&
          [[[PreferencePanel sessionsInstance] currentProfileGuid]
              isEqualToString:guid] &&
  ...
  if (self.isHotKeyWindow) {
    ...
    if (profile) {
      int screenNumber = [iTermProfilePreferences intForKey:KEY_SCREEN
                                                  inProfile:profile];
      _screenNumberFromFirstProfile = screenNumber;
      screenNumber = [PseudoTerminal
          screenNumberForPreferredScreenNumber:screenNumber
...
- (NSArray *)allSessions {
  NSMutableArray *result = [NSMutableArray
      arrayWithCapacity:[_contentView.tabView numberOfTabViewItems]];
  for (NSTabViewItem *item in [_contentView.tabView tabViewItems]) {
    PTYTab *tab = [item identifier];
...
- (BOOL)eligibleForFullScreenTabBarToFlash {
  return ([self anyFullScreen] && !exitingLionFullscreen_ &&
          ![iTermPreferences boolForKey:kPreferenceKeyShowFullscreenTabBar] &&
          ![[[self currentSession] textview] isFindingCursor]);
}
...
- (void)asyncCreateTabWithProfile:(Profile *)profile
  ...
  __weak __typeof(self) weakSelf = self;
  [currentSession
      asyncCurrentLocalWorkingDirectoryOrInitialDirectory:^(NSString *pwd) {
...
- (PTYSession *)createTabWithProfile:(Profile *)profile
                   ...
                   previousDirectory:(NSString *)previousDirectory
                          ...
                          completion:
                              (void (^)(PTYSession *, BOOL ok))completion {
  ...
  if (command) {
    profile = [[profile
        ...
        dictionaryBySettingObject:command
                           forKey:KEY_COMMAND_LINE];
  }

  // Initialize a new session
  PTYSession *aSession =
      [[self.sessionFactory newSessionWithProfile:profile
...
- (void)asyncRestoreArrangement:(NSDictionary *)arrangement
  ...
  [self
      openPartialAttachmentsForArrangement:arrangement
                                   timeout:timeout
...
// The timeout block is called when we get a response after having timed out and
// called completion. It can be called more than once: for each child we
// discover after the deadline.
- (void)openPartialAttachmentsForArrangement:(NSDictionary *)arrangement
  ...
  for (NSDictionary *tabArrangement in arrangement[TERMINAL_ARRANGEMENT_TABS]) {
    ...
    DLog(@"PseudoTerminal.openPartialAttachmentsForArrangement: request for "
         @"tab");
    [PTYTab openPartialAttachmentsForArrangement:tabArrangement
                                      completion:^(NSDictionary *tabResult) {
                                        DLog(@"PseudoTerminal."
                                             @"openPartialAttachmentsForArrange"
                                             @"ment: got result for tab");
                                        if (haveNotified) {
                                          // Timed out.
  ...
  dispatch_after(
      dispatch_time(
          DISPATCH_TIME_NOW,
          (int64_t)([iTermAdvancedSettingsModel timeoutForDaemonAttachment] *
                    NSEC_PER_SEC)),
      dispatch_get_main_queue(), ^{
        if (haveNotified) {
          DLog(@"PseudoTerminal.openPartialAttachmentsForArrangement: no "
               @"timeout, already notified.");
          return;
        }
        haveNotified = YES;
  ...
  dispatch_group_notify(group, dispatch_get_main_queue(), ^{
    DLog(@"PseudoTerminal.openPartialAttachmentsForArrangement: got results "
         @"for all tabs");
    if (haveNotified) {
      DLog(@"PseudoTerminal.openPartialAttachmentsForArrangement:timeout");
...
- (void)window:(NSWindow *)window didDecodeRestorableState:(NSCoder *)state {
  NSDictionary *arrangement = [state
      decodeObjectForKey:kTerminalWindowStateRestorationWindowArrangementKey];
  if ([iTermAdvancedSettingsModel logRestorableStateSize]) {
    NSString *log = [arrangement sizeInfo];
    [log writeToFile:[NSString
                         stringWithFormat:@"/tmp/statesize.window-%p.txt", self]
          atomically:NO
            encoding:NSUTF8StringEncoding
               error:nil];
  }
...
- (void)setRestorableStateDecodePending:(BOOL)restorableStateDecodePending {
  if (_restorableStateDecodePending != restorableStateDecodePending) {
    ...
    if (!restorableStateDecodePending) {
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermDidDecodeWindowRestorableStateNotification
                        object:self];
    }
  }
}
...
- (BOOL)windowRestorationEnabled {
  if (self.isHotKeyWindow) {
    // Hotkey windows are restored unconditionally. See
    // -application:didDecodeRestorableState:. That's not necessarily a good
    // idea, but I don't want to half-break it.
    return YES;
  }
...
- (BOOL)shouldSaveRestorableStateLegacy:(BOOL)legacy {
  ...
  // Don't save and restore the hotkey window. The OS only restores windows that
  // are in the window order, and hotkey windows may be ordered in or out,
  // depending on whether they were in use. So they get a special path for
  // restoration where the arrangement is saved in user defaults.
...
- (void)window:(NSWindow *)window willEncodeRestorableState:(NSCoder *)state {
  ...
  if (![self shouldSaveRestorableStateLegacy:YES]) {
    ...
    if ([self isHotKeyWindow]) {
      [[[iTermHotKeyController sharedInstance]
          profileHotKeyForWindowController:self] saveHotKeyWindowState];
    }
    return;
  }
  ...
  if (_wellFormed) {
    ...
    lastArrangement_ =
        [[self arrangementExcludingTmuxTabs:YES
...
- (void)addSessionInNewTab:(PTYSession *)session {
  ...
  if ([session screen]) { // screen initialized ok
    ...
    if ([iTermAdvancedSettingsModel addNewTabAtEndOfTabs] ||
        ![self currentTab]) {
      tab = [self insertSession:session
    ...
    if (!tab.tmuxTab &&
        [iTermProfilePreferences boolForKey:KEY_USE_CUSTOM_TAB_TITLE
                                  inProfile:session.profile]) {
      [tab setTitleOverride:[iTermProfilePreferences
                                stringForKey:KEY_CUSTOM_TAB_TITLE
                                   inProfile:session.profile]];
    }
  }
  [[self currentTab] numberOfSessionsDidChange];
...
- (void)sessionDidTerminate:(PTYSession *)session {
  ...
  if ([[PreferencePanel sessionsInstance] isWindowLoaded]) {
    if ([iTermAdvancedSettingsModel pinEditSession] &&
        ![NSObject object:session.profile[KEY_GUID]
            isEqualToObject:[[PreferencePanel sessionsInstance]
                                currentProfileGuid]]) {
...
- (void)moveToPreferredScreen {
  if (_screenNumberFromFirstProfile == -2) {
    ...
    [[NSScreen screens]
        enumerateObjectsUsingBlock:^(NSScreen *_Nonnull screen, NSUInteger i,
                                     BOOL *_Nonnull stop) {
          if (NSPointInRect(cursor, screen.frame)) {
            _isAnchoredToScreen = YES;
            _anchoredScreenNumber = i;
            DLog(@"Move window to screen %d %@", (int)i,
...
- (void)broadcastPassword:(NSString *)password {
  [iTermBroadcastPasswordHelper
      tryToSendPassword:password
             toSessions:self.broadcastSessions
             completion:^NSArray<PTYSession *> *_Nonnull(
                 NSArray<PTYSession *> *_Nonnull okSessions,
                 NSArray<PTYSession *> *_Nonnull problemSessions) {
               if (problemSessions.count == 0) {
                 return okSessions;
               }
               NSArray<NSString *> *names =
                   [problemSessions mapWithBlock:^id(PTYSession *session) {
                     return session.nameController.presentationSessionTitle;
                   }];
               NSString *message;
               if (names.count < 2) {
                 message = [NSString
                     stringWithFormat:@"The session named “%@” does not appear "
                                      @"to be at a password prompt.",
                                      names.firstObject];
               } else {
                 message = [NSString
                     stringWithFormat:
                         @"The following sessions to which input is broadcast "
                         @"do not appear to be at a password prompt: %@",
                         [names componentsJoinedWithOxfordComma]];
               }
               NSArray *actions;
               if (okSessions.count > 0) {
                 actions =
                     @[ @"Cancel", @"Enter Password in Sessions at Prompt" ];
               } else {
                 actions = @[ @"OK" ];
               }
               iTermWarningSelection selection = [iTermWarning
...
- (void)tab:(PTYTab *)tab didChangeProcessingStatus:(BOOL)isProcessing {
  [_contentView.tabBarControl setIsProcessing:isProcessing
                         forTabWithIdentifier:tab];
}
...
- (void)tab:(PTYTab *)tab didChangeObjectCount:(NSInteger)objectCount {
  [_contentView.tabBarControl setObjectCount:objectCount
                        forTabWithIdentifier:tab];
}
...
- (void)updateContentViewExpectsMetal {
  [_contentView setUseMetal:[self.currentTab.sessions
                                allWithBlock:^BOOL(PTYSession *anObject) {
                                  MTKView *metalView = anObject.view.metalView;
                                  return !metalView.isHidden &&
...
- (void)currentSessionWordAtCursorDidBecome:(NSString *)word {
  if (word == _previousTouchBarWord ||
      [word isEqualToString:_previousTouchBarWord]) {
    return;
  }
  [_previousTouchBarWord release];
...
- (void)updateDocumentEdited {
  ...
  self.window.documentEdited =
      [self.currentTab.sessions anyWithBlock:^BOOL(PTYSession *session) {
        if ([[iTermProfilePreferences stringForKey:KEY_CUSTOM_COMMAND
                                         inProfile:session.profile]
  ...
  if (@available(macOS 10.14, *)) {
    ...
    case TAB_STYLE_MINIMAL:
      if (self.minimalTabStyleBackgroundColor.isDark) {
        _contentView.toolbelt.appearance =
...
- (void)toolbeltUpdateMouseCursor {
  [[[self currentSession] textview]
      updateCursor:[[NSApplication sharedApplication] currentEvent]];
}
...
- (void)controlTextDidChange:(NSNotification *)aNotification {
  if ([aNotification object] == coprocessCommand_) {
    [coprocessIgnoreErrors_
        setState:[Coprocess shouldIgnoreErrorsFromCommand:coprocessCommand_
                                                              .stringValue]
                     ? NSControlStateValueOn
                     : NSControlStateValueOff];
  }
...
- (void)tabEditActions:(PTYTab *)tab {
  [[PreferencePanel sharedInstance]
      openToPreferenceWithKey:kPreferenceKeyActions];
}

- (void)tabEditSnippets:(PTYTab *)tab {
  [[PreferencePanel sharedInstance]
      openToPreferenceWithKey:kPreferenceKeySnippets];
}
...
- (void)updateBackgroundImage {
  ...
  if (@available(macOS 10.14, *)) {
    [self
        setSharedBackgroundImage:self.currentSession.backgroundImage
...
- (void)tab:(PTYTab *)tab
  ...
  if (@available(macOS 10.14, *)) {
    [self setSharedBackgroundImage:image
                              mode:imageMode
...
- (void)setSharedBackgroundImage:(NSImage *)image
  ...
  _contentView.backgroundImage.hidden =
      !iTermTextIsMonochrome() || (image == nil);
  for (PTYSession *session in self.allSessions) {
...
- (NSColor *)minimalTabStyleBackgroundColor {
  DLog(@"Getting bg color for session %@, colormap %@", self.currentSession,
       self.currentSession.colorMap);
  return [self.currentSession.colorMap colorForKey:kColorMapBackground];
}
...
- (NSArray<NSString *> *)
    broadcastInputHelperSessionsInCurrentTab:(iTermBroadcastInputHelper *)helper
...
- (NSArray<NSString *> *)
    broadcastInputHelperSessionsInAllTabs:(iTermBroadcastInputHelper *)helper
                            includeExited:(BOOL)includeExited {
...
- (NSString *)broadcastInputHelperCurrentSession:
    (iTermBroadcastInputHelper *)helper {
...
- (BOOL)broadcastInputHelperCurrentTabIsBroadcasting:
    (iTermBroadcastInputHelper *)helper {
  return self.currentTab.isBroadcasting;
}

- (void)broadcastInputHelperSetNoTabBroadcasting:
    (iTermBroadcastInputHelper *)helper {
  for (PTYTab *tab in self.tabs) {
    tab.broadcasting = NO;
  }
...
- (void)broadcastInputHelper:(iTermBroadcastInputHelper *)helper
    setCurrentTabBroadcasting:(BOOL)broadcasting {
  self.currentTab.broadcasting = broadcasting;
}

- (NSWindow *)broadcastInputHelperWindowForWarnings:
    (iTermBroadcastInputHelper *)helper {
  return self.window;
}
...
// Only variable-changed notifications are relevant for windows. Everything else
// is just for sessions. Variable-changed notifs are handled before this is
// called.
...
- (BOOL)presentationControllerManagedWindowControllerIsFullScreen:
    (out BOOL *)lionFullScreen {
  if (![self anyFullScreen]) {
    *lionFullScreen = NO;
    return NO;
  }
...
- (id)swipeHandlerBeginSessionAtOffset:(CGFloat)offset
  ...
  frame.size.width =
      self.tabs.firstObject.realRootView.frame.size.width * self.tabs.count;
  ...
  [self.tabs enumerateObjectsUsingBlock:^(PTYTab *_Nonnull tab, NSUInteger idx,
                                          BOOL *_Nonnull stop) {
...
- (CGFloat)truncatedSwipeOffset:(CGFloat)x {
  ...
  const CGFloat upperBound =
      MAX(0, ((NSInteger)params.count) - 1) * params.width;
...
- (BOOL)encodeGraphWithEncoder:(iTermGraphEncoder *)encoder {
  // NOTE: The well-formedness check is not in -shouldSaveRestorableState
  // because I'm afraid of breaking something in the legacy code. Once it is
  // gone, it can move in.
  if (![self shouldSaveRestorableStateLegacy:NO] || !_wellFormed) {
    return NO;
  }
  NSArray<PTYTab *> *tabs = [self tabsToEncodeExcludingTmux:YES];
  const BOOL includeContents =
      [iTermAdvancedSettingsModel restoreWindowContents];
  iTermGraphEncoderAdapter *adapter = [[[iTermGraphEncoderAdapter alloc]
      initWithGraphEncoder:encoder] autorelease];
  const BOOL commit = [self populateArrangementWithTabs:tabs
                                      includingContents:includeContents
                                                encoder:adapter];
  return commit;
