...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  ...
  return [@[
    textColorKnob, backgroundColorKnob, [super statusBarComponentKnobs],
    [self minMaxWidthKnobs]
  ] flattenedArray];
}
...
- (NSColor *)textColor {
  ...
  return [knobValues[iTermStatusBarSharedTextColorKey] colorValue]
             ?: ([self defaultTextColor]
                     ?: [self.delegate statusBarComponentDefaultTextColor]);
}
...
- (BOOL)shouldUpdateValue:(NSString *)proposed
                  inField:(NSTextField *)textField {
  ...
  if (textFieldHasString || iHaveString) {
    BOOL result =
        !([NSObject object:textField.stringValue isEqualToObject:proposed] &&
          [NSObject object:textField.textColor isEqualToObject:self.textColor]);
    if (result) {
      DLog(@"%@ updating because %@ != %@", self, textField.stringValue,
           proposed);
    }
    return result;
  }
...
- (BOOL)setValueInField:(NSTextField *)textField compressed:(BOOL)compressed {
  ...
  if (textField.alignment == NSTextAlignmentRight && textField.superview) {
    [self statusBarComponentSizeView:textField
                          toFitWidth:textField.superview.bounds.size.width];
  } else {
    [textField sizeToFit];
  }
  return YES;
...
- (void)statusBarComponentSizeView:(NSView *)view toFitWidth:(CGFloat)width {
  ...
  DLog(@"Place text view %@ of width %@ at x=%@ in container of width %@",
       textField, @(width), @(x), @(textField.superview.frame.size.width));
...
- (CGFloat)statusBarComponentMinimumWidth {
  NSArray<iTermTuple<NSString *, NSNumber *> *> *tuples =
      [self widthStringTuples];
  NSNumber *number = [tuples minWithBlock:^NSComparisonResult(
                                 iTermTuple<NSString *, NSNumber *> *obj1,
                                 iTermTuple<NSString *, NSNumber *> *obj2) {
                       return [obj1.secondObject compare:obj2.secondObject];
                     }].secondObject;
...
- (CGFloat)statusBarComponentVerticalOffset {
  ...
  const CGFloat descender =
      _textField.font.descender -
      _textField.font.leading; // negative (distance from bottom of bounding box
                               // to baseline)
