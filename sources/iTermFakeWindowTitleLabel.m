...
+ (NSParagraphStyle *)paragraphStyleWithAlignment:
...
+ (NSDictionary *)attributesWithParagraphStyle:
...
+ (NSAttributedString *)
    attributedStringForWindowTitleLabelWithString:(NSString *)title
...
+ (NSTextAttachment *)
    iconTextAttachmentForWindowTitleLabelWithImage:(NSImage *)icon
                                              font:(NSFont *)font {
  NSTextAttachment *textAttachment = [[NSTextAttachment alloc] init];
  textAttachment.image = icon;
  const CGFloat lineHeight = ceilf(font.capHeight);
  textAttachment.bounds = NSMakeRect(0, -(icon.size.height - lineHeight) / 2.0,
...
+ (NSAttributedString *)
    attributedStringForWindowTitleLabelWithString:(NSString *)title
                                        ...
                                        alignment:
                                            (NSTextAlignment)textAlignment {
  NSParagraphStyle *paragraphStyle =
      [self paragraphStyleWithAlignment:textAlignment];
  NSDictionary *attributes = [self attributesWithParagraphStyle:paragraphStyle
                                                           font:font
                                                      textColor:textColor];
  NSAttributedString *attributedString =
      [self attributedStringForWindowTitleLabelWithString:title
                                               attributes:attributes];
  NSTextAttachment *textAttachment =
      [self iconTextAttachmentForWindowTitleLabelWithImage:icon font:font];
  NSMutableAttributedString *result = [[NSAttributedString
      attributedStringWithAttachment:textAttachment] mutableCopy];
...
- (void)setTitle:(NSString *)title
                 icon:(NSImage *)icon
    alignmentProvider:(NSTextAlignment (^NS_NOESCAPE)(NSTextField *scratch))
                          alignmentProvider {
...
- (void)setTitleWithAlignment:(NSTextAlignment)textAlignment {
  DLog(@"Set title to %@ with alignment=%@", self.windowTitle,
       @(textAlignment));
