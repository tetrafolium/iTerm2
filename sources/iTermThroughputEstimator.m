...
- (instancetype)initWithHistoryOfDuration:(NSTimeInterval)historyDuration
  ...
  if (self) {
    ...
    for (NSInteger i = 0; i < MAX(1, round(historyDuration / secondsPerBucket));
         i++) {
...
- (NSInteger)estimatedThroughput {
  ...
  const double weightForCurrentBucket =
      MIN(10, _secondsPerBucket / timeSpentInCurrentBucket);
  const NSUInteger numberOfBuckets = _buckets.count;

  __block double weightedSum = 0;
  __block double weight = 1;
  [_buckets
      enumerateObjectsUsingBlock:^(NSNumber *_Nonnull number, NSUInteger index,
                                   BOOL *_Nonnull stop) {
        double value = [number doubleValue];
        if (index == numberOfBuckets - 1) {
          //            NSLog(@"Time left in current bucket is %0.2f so increase
          //            value %0.0f by %0.0f to %0.0f",
          //                  1.0 - timeSpentInCurrentBucket, value, 1.0 /
          //                  weightForCurrentBucket, value /
          //                  weightForCurrentBucket);
  ...
  //    NSLog(@"weightedSum=%0.0f averageValuePerBucket=%0.0f weight=%0.0f
  //    result=%0.0f %@",
  //          weightedSum, averageValuePerBucket, weight, estimatedThroughput,
  //          _buckets);
...
- (double)eraseBucketsIfNeeded {
  ...
  const NSUInteger numberOfBuckets = _buckets.count;
  const NSInteger numberOfBucketsToErase =
      MIN(numberOfBuckets, timeIndex - _lastTimeIndex);
