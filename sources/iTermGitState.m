...
static NSString *const iTermGitStateVariableNameGitPushCount =
    @"user.gitPushCount";
static NSString *const iTermGitStateVariableNameGitPullCount =
    @"user.gitPullCount";
...
static NSArray<NSString *> *iTermGitStatePaths(void) {
  return @[
    iTermGitStateVariableNameGitBranch, iTermGitStateVariableNameGitPushCount,
    iTermGitStateVariableNameGitPullCount, iTermGitStateVariableNameGitDirty,
    iTermGitStateVariableNameGitAdds, iTermGitStateVariableNameGitDeletes
  ];
}
...
- (instancetype)initWithScope:(iTermVariableScope *)scope {
  ...
  if (self) {
    ...
    _directory =
        [scope valueForVariableName:iTermVariableKeySessionID] ?: @"(null)";
    _branch = [scope valueForVariableName:iTermGitStateVariableNameGitBranch];
    _pushArrow =
        [scope valueForVariableName:iTermGitStateVariableNameGitPushCount];
    _pullArrow =
        [scope valueForVariableName:iTermGitStateVariableNameGitPullCount];
    _dirty = [[scope valueForVariableName:iTermGitStateVariableNameGitDirty]
        gitDirtyBoolValue];
    _adds = [[scope valueForVariableName:iTermGitStateVariableNameGitAdds]
        integerValue];
    _deletes = [[scope valueForVariableName:iTermGitStateVariableNameGitDeletes]
        integerValue];
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p dir=%@ xcode=%@ push=%@ pull=%@ "
                                    @"branch=%@ dirty=%@ adds=%@ deletes=%@>",
                                    self.class, self, _directory, _xcode,
                                    _pushArrow, _pullArrow, _branch, @(_dirty),
                                    @(_adds), @(_deletes)];
}
  ...
  if (self) {
    ...
    _refs = [paths mapWithBlock:^id(NSString *path) {
      iTermVariableReference *ref =
          [[iTermVariableReference alloc] initWithPath:path vendor:scope];
