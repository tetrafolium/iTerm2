...
@interface iTermPreferencesSearchEngineResultsWindowController () <
...
- (void)updateFrameAndAlpha {
  ...
  CGFloat desiredTableViewHeight =
      (_tableView.rowHeight + _tableView.intercellSpacing.height) * rowsToShow;
  NSScrollView *scrollView = _tableView.enclosingScrollView;
  const CGFloat desiredContentHeight =
      [NSScrollView
          frameSizeForContentSize:NSMakeSize(100, desiredTableViewHeight)
  ...
  CGFloat desiredHeight =
      [NSPanel
          frameRectForContentRect:NSMakeRect(0, 0, 100, desiredContentHeight)
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
  [self.delegate
      preferencesSearchEngineResultsDidSelectDocument:self.selectedDocument];
}
...
- (IBAction)action:(id)sender {
  ...
  if (row >= 0) {
    [self.delegate
        preferencesSearchEngineResultsDidActivateDocument:self.documents[row]];
  }
}
