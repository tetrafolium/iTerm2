...
+ (instancetype)closeFrameWithCode:(uint16_t)code reason:(NSString *)reason {
  ...
  NSMutableData *payload = [NSMutableData dataWithBytes:&networkCode
                                                 length:sizeof(networkCode)];
...
+ (instancetype)textFrameWithString:(NSString *)string {
  return
      [self textFrameWithData:[string dataUsingEncoding:NSUTF8StringEncoding]];
}
...
- (NSString *)description {
  ...
  return
      [NSString stringWithFormat:@"<%@: %p opcode=%@ fin=%@ payloadLength=%@>",
                                 NSStringFromClass([self class]), self, opcode,
                                 _fin ? @"YES" : @"NO", @(self.payload.length)];
}
...
- (NSData *)data {
  ...
  if (!_data) {
    ...
    byte = 0;
    // We're a server so we never mask outgoing data. Mask bit won't get set
    // here (would go in high bit of 'byte').
...
- (uint16_t)closeFrameCode {
  ITAssertWithMessage(self.opcode = iTermWebSocketOpcodeConnectionClose,
                      @"Not a close frame");
...
- (NSString *)closeFrameReason {
  ...
  NSData *data =
      [self.payload subdataWithRange:NSMakeRange(2, self.payload.length - 2)];
  return [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
}

- (NSString *)text {
  ITAssertWithMessage(self.opcode == iTermWebSocketOpcodeText,
                      @"Not a text frame");
  return [[NSString alloc] initWithData:self.payload
...
- (iTermWebSocketFrame *)fragmentFromStartWithPayloadLength:(uint64_t)length {
  ...
  if (self.opcode == iTermWebSocketOpcodeText) {
    first = [iTermWebSocketFrame
        textFrameWithData:[self.payload
                              subdataWithRange:NSMakeRange(0, length)]];
  } else if (self.opcode == iTermWebSocketOpcodeBinary) {
    first = [iTermWebSocketFrame
        binaryFrameWithData:[self.payload
                                subdataWithRange:NSMakeRange(0, length)]];
  } else {
    return nil;
  }
  self.payload = [self.payload
      subdataWithRange:NSMakeRange(length, self.payload.length - length)];
