...
NSString *const iTermSessionBuriedStateChangeTabNotification =
    @"iTermSessionBuriedStateChangeTabNotification";
...
- (void)addBuriedSession:(PTYSession *)sessionToBury {
  ...
  id<iTermWindowController> windowController =
      (PseudoTerminal *)sessionToBury.delegate.parentWindow;
  iTermRestorableSession *restorableSession =
      [windowController restorableSessionForSession:sessionToBury];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermSessionBuriedStateChangeTabNotification
                    object:sessionToBury];
}
...
- (void)restoreSession:(PTYSession *)session {
  ...
  PseudoTerminal *term = [[iTermController sharedInstance]
      terminalWithGuid:restorableSession.terminalGuid];
  if (term) {
    DLog(@"Found it. Look for tab with unique id %@",
         @(restorableSession.tabUniqueId));
    ...
    if (tab) {
      ...
      DLog(
          @"The tab doesn't exist. Create a new tab and add the session to it");
    ...
    DLog(@"Failed to find the terminal by uid. Create a new window and add the "
         @"session to it.");
...
- (void)restoreFromState:(NSArray<NSDictionary *> *)state {
  for (NSDictionary *dict in state) {
    iTermRestorableSession *restorable =
        [[iTermRestorableSession alloc] initWithRestorableState:dict];
    if (restorable) {
...
- (void)updateMenu:(NSMenu *)menu {
  ...
  for (PTYSession *session in
    ...
    NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:session.name
  ...
  for (iTermTmuxWindowCacheWindowInfo *window in
       [[iTermTmuxWindowCache sharedInstance] hiddenWindows]) {
    if (needsSeparator) {
      needsSeparator = NO;
      [menu addItem:[NSMenuItem separatorItem]];
    }

    NSString *clientName = [window.clientName
    ...
    NSString *title =
        [NSString stringWithFormat:@"↣ %@ — %@", clientName, window.name];
    NSMenuItem *item =
  ...
  [[menu.supermenu.itemArray
      objectPassingTest:^BOOL(NSMenuItem *element, NSUInteger index,
                              BOOL *stop) {
...
- (void)disinterTmuxWindow:(NSMenuItem *)menuItem {
  ...
  TmuxController *controller = [[TmuxControllerRegistry sharedInstance]
      controllerForClient:window.clientName];
