...
- (NSDictionary<NSString *, id> *)dictionaryValue {
  return @{
    KEY_HAS_HOTKEY : @(self.hotKeyAssigned),
...
- (void)setAlternateShortcutDictionaries:
    (NSArray<NSDictionary *> *)dictionaries {
  self.alternateShortcuts =
      [dictionaries mapWithBlock:^id(NSDictionary *dictionary) {
...
- (BOOL)hotKeyAssigned {
  BOOL hasAlternate =
      [self.alternateShortcuts anyWithBlock:^BOOL(iTermShortcut *shortcut) {
        return shortcut.charactersIgnoringModifiers.length > 0 ||
               shortcut.characters.length > 0 || shortcut.keyCode != 0;
      }];
  return (_primaryShortcut.isAssigned || self.hasModifierActivation ||
          hasAlternate);
}
