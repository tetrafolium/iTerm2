...
@property(nonatomic) BOOL forceUseOldCWD; // Change custom directory setting to
                                          // make it use the passed-in oLDCWD
@property(nonatomic, nullable, copy)
    NSString *command; // Overrides profile's command if nonnil
...
@property(nonatomic, nullable, copy) void (^completion)
    (PTYSession *_Nullable session, BOOL ok)
        ; // If nonnil, attachorLaunch may be async
            ...
            serverConnection:(iTermGeneralServerConnection)serverConnection
              ...
              forceUseOldCWD:(BOOL)forceUseOldCWD
                     command:(nullable NSString *)command
                      isUTF8:(nullable NSNumber *)
                                 isUTF8Number // Overrides profile's iSUTF8 if
                                              // nonnil
