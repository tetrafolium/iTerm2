...
static NSString *const kImageInfoPreserveAspectRatioKey =
...
@property(nonatomic, retain)
    NSMutableDictionary *embeddedImages; // frame number->downscaled image
@property(nonatomic, assign) unichar code;
@property(nonatomic, retain)
    iTermAnimatedImageInfo *animatedImage; // If animated GIF, this is nonnil
@end
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
  ...
  if (self) {
    ...
    _preserveAspectRatio =
        [dictionary[kImageInfoPreserveAspectRatioKey] boolValue];
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@: %p code=%@ size=%@ uniqueIdentifier=%@ filename=%@ broken=%@>",
          self.class, self, @(self.code), NSStringFromSize(self.size),
          self.uniqueIdentifier, self.filename, @(self.broken)];
}
...
- (void)loadFromDictionaryIfNeeded {
  ...
  dispatch_once(&onceToken, ^{
    ...
    queue = dispatch_queue_create("com.iterm2.LazyImageDecoding",
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      if (_image || _animatedImage) {
        ...
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermImageDidLoad
                          object:self];
      }
    });
  };
...
- (void)saveToFile:(NSString *)filename {
  ...
  NSDictionary *universalTypeToCocoaMap = @{
    (NSString *)kUTTypeBMP : @(NSBitmapImageFileTypeBMP),
...
static NSSize
iTermImageInfoGetSizeForRegionPreservingAspectRatio(const NSSize region,
                                                    NSSize imageSize) {
...
- (NSImage *)imageWithCellSize:(CGSize)cellSize
  ...
  int frame =
      [self.animatedImage frameForTimestamp:timestamp]; // 0 if not animated
  NSImage *embeddedImage = _embeddedImages[@(frame)];

  NSSize region =
      NSMakeSize(cellSize.width * _size.width, cellSize.height * _size.height);
  if (!NSEqualSizes(embeddedImage.size, region)) {
    ...
    if (_preserveAspectRatio) {
      size = iTermImageInfoGetSizeForRegionPreservingAspectRatio(region,
                                                                 theImage.size);
    } else {
      size = region;
    }
    ...
    const NSRect destinationRect =
        NSMakeRect((region.width - size.width) / 2 + inset.left,
...
+ (NSEdgeInsets)
  ...
  const NSSize size =
      iTermImageInfoGetSizeForRegionPreservingAspectRatio(region, imageSize);

  // Given the following equalities, inferred from how the destinationRect is
  // computed when creating the embedded image in -imageWithCellSize:timestamp:
  // above:
  //
  ...
  return NSEdgeInsetsMake(bottom / cellSize.height, left / cellSize.width,
                          top / cellSize.height, right / cellSize.width);
}
...
- (NSString *)nameForNewSavedTempFile {
  ...
  if (_filename.pathExtension.length) {
    ...
    NSString *suffix =
        [@"." stringByAppendingString:_filename.lastPathComponent];
    ...
    name =
        [[NSWorkspace sharedWorkspace] temporaryFileNameWithPrefix:@"iTerm2."
                                                            suffix:extension];
  }
...
- (void)pasteboard:(NSPasteboard *)pasteboard
  ...
  if ([type isEqualToString:(NSString *)kUTTypeFileURL]) {
    ...
    [item setString:[[NSURL fileURLWithPath:self.nameForNewSavedTempFile]
                        absoluteString]
            forType:(NSString *)kUTTypeFileURL];
  } else {
    if ([type isEqualToString:(NSString *)kUTTypeImage] &&
        ![_data uniformTypeIdentifierForImageData]) {
