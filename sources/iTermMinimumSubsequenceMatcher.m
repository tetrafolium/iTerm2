...
@implementation iTermMinimumSubsequenceMatcher {
  ...
  NSDictionary *
      _postingLists; // Maps a character to an array of sorted document offsets.
  NSMutableArray *_indexes; // 1:1 with _queryChars, gives indexes into matching
                            // posting list.
}
...
- (BOOL)advanceIndexes {
  ...
  for (NSInteger i = 0; i < queryLength; i++) {
    ...
    // previousDocOffset. An open-ended binary search would be faster but
    // probably not worth the extra complexity in this application.
...
- (NSRange)range {
  ...
  NSInteger end =
      [_postingLists[_queryChars.lastObject][lastIndex] integerValue];
...
- (NSArray *)bestIndexes {
  ...
  while (1) {
    ...
    if (bestRange.location == NSNotFound ||
        bestRange.length > currentRange.length) {
