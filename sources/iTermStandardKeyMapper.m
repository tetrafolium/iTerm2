...
- (NSString *)preCocoaString {
  ...
  const BOOL onlyControlPressed =
      (modflag & (NSEventModifierFlagControl | NSEventModifierFlagCommand |
...
// Called when control is pressed and command and option are not pressed. Shift
...
- (unichar)controlCodeForEvent:(NSEvent *)event {
  ...
  const unichar character =
      characters.length > 0 ? [characters characterAtIndex:0] : 0;
  const BOOL shiftPressed =
      !!(event.it_modifierFlags & NSEventModifierFlagShift);

  return [iTermStandardKeyMapper
      codeForSpecialControlCharacter:character
...
+ (unichar)codeForSpecialControlCharacter:(unichar)character
  ...
  if (character == '|') {
    // This is necessary to handle Japanese keyboards correctly. Pressing
    // Control+backslash generates characters=@"|" and
    // charactersIgnoringModifiers=@"¥". This code path existed in iTerm 0.1.
    DLog(@"C-backslash");
    return 28; // Control-backslash
  } else if (character == '/' && shiftPressed) {
    // This was in the original iTerm code. It's the normal path for US
    // keyboards for ^-?.
    DLog(@"C-?");
    return 127;
  }

  DLog(@"Checking characterIgnoringModifiers %@",
...
- (NSData *)dataForRegularKeypress {
  ...
  const unichar character =
      _event.characters.length > 0 ? [_event.characters characterAtIndex:0] : 0;
  NSString *const charactersIgnoringModifiers =
      [_event charactersIgnoringModifiers];
  ...
  const BOOL isNumericKeypadKey =
      !!(mutableModifiers & NSEventModifierFlagNumericPad);
  if (isNumericKeypadKey ||
      [self keycodeShouldHaveNumericKeypadFlag:_event.keyCode]) {
    DLog(@"PTYSession keyDown numeric keypad");
    return [_configuration.outputFactory keypadData:character
                                             keystr:characters];
  }
  ...
  // Commit a00a9385b2ed722315ff4d43e2857180baeac2b4 in old-iterm suggests this
  // is necessary for some Japanese input sources, but is vague.
...
- (NSData *)dataWhenOptionPressed {
  ...
  const BOOL controlPressed =
      !!(_event.it_modifierFlags & NSEventModifierFlagControl);
...
- (NSData *)dataForFunctionKeyPress {
  ...
  case NSHomeFunctionKey:
    return [_configuration.outputFactory keyHome:modifiers
                              screenlikeTerminal:_configuration.screenlike];
    break;
  case NSEndFunctionKey:
    return [_configuration.outputFactory keyEnd:modifiers
                             screenlikeTerminal:_configuration.screenlike];
    break;
  ...
  case NSF35FunctionKey:
    return [_configuration.outputFactory
        keyFunction:unicode - NSF1FunctionKey + 1];
  }

  return [characters dataUsingEncoding:_configuration.encoding];
}

// In issue 4039 we see that in some cases the numeric keypad mask isn't set
// properly.
...
- (BOOL)shouldSquelchKeystrokeWithString:(NSString *)keystr
  ...
  const NSEventModifierFlags deviceIndependentModifiers =
      modflag & NSEventModifierFlagDeviceIndependentFlagsMask;
  const BOOL pressingCommand =
      !!(deviceIndependentModifiers & NSEventModifierFlagCommand);
  if (!pressingCommand) {
    return NO;
  }
...
- (BOOL)keyMapperShouldBypassPreCocoaForEvent:(NSEvent *)event {
  ...
  if (willSendOptionModifiedKey) {
    ...
    DLog(@"isNonEmpty=%@ rightAltPressed=%@ leftAltPressed=%@ "
         @"leftOptionModifiesKey=%@ rightOptionModifiesKey=%@ "
         @"willSendOptionModifiedKey=%@ -> bypass pre-cocoa",
