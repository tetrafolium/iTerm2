...
- (NSScriptObjectSpecifier *)objectSpecifier {
  ...
  NSArray<iTermScriptingWindow *> *windows =
      [[iTermApplication sharedApplication] orderedScriptingWindows];
  anIndex = [windows
      indexOfObjectPassingTest:^BOOL(iTermScriptingWindow *_Nonnull obj,
...
- (id)handleSelectCommand:(NSScriptCommand *)command {
  [[iTermController sharedInstance]
      setCurrentTerminal:(PseudoTerminal *)self.ptyDelegate];
  return nil;
}
...
- (id)handleCreateTabWithDefaultProfileCommand:
          ...
          didMakeSession:nil
              completion:^(PTYSession *session, BOOL ok) {
                dispatch_async(dispatch_get_main_queue(), ^{
                  [scriptCommand
                      resumeExecutionWithResult:[self.ptyDelegate
                                                    tabForSession:session]];
                });
              }];
  return nil;
}
  ...
  if (!profile) {
    ...
    [scriptCommand
        setScriptErrorString:
            [NSString
                stringWithFormat:@"No profile exists named '%@'", profileName]];
    return nil;
  }
  [scriptCommand suspendExecution];
  [iTermSessionLauncher
...
- (id)handleRevealHotkeyWindowCommand:(NSScriptCommand *)scriptCommand {
  [[[iTermHotKeyController sharedInstance]
      profileHotKeyForWindowController:(PseudoTerminal *)self.ptyDelegate]
      revealForScripting];
  return nil;
}
...
- (void)replaceInTabs:(PTYTab *)replacementTab atIndex:(unsigned)anIndex {
  ...
  [(PseudoTerminal *)self.ptyDelegate
      closeTab:[(PseudoTerminal *)self.ptyDelegate tabs][anIndex + 1]];
}
