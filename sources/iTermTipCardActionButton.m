...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  [self.color set];
  NSRectFill(NSMakeRect(rect.origin.x, rect.origin.y, rect.size.width, 0.5));
}
...
- (void)drawRect:(NSRect)dirtyRect {
  NSRect rect = self.bounds;
  [self.color set];
  NSRectFill(NSMakeRect(rect.origin.x, rect.origin.y, 0.5, rect.size.height));
}
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    ...
    iTermTipCardActionButtonTopDividerView *divider =
        [[[iTermTipCardActionButtonTopDividerView alloc]
            initWithFrame:NSMakeRect(0, 0, frameRect.size.width, 1)]
            autorelease];
    ...
    _leftDivider = [[[iTermTipCardActionButtonLeftDividerView alloc]
        initWithFrame:NSMakeRect(0, 0, 1, frameRect.size.height)] autorelease];
    ...
    NSBezierPath *path =
        [NSBezierPath bezierPathWithOvalInRect:NSMakeRect(0, 0, 1, 1)];
    _highlightLayer.path = path.iterm_CGPath;
    _highlightLayer.anchorPoint = CGPointMake(0.5, 0.5);
    _highlightLayer.fillColor = [[NSColor colorWithCalibratedWhite:0.90
                                                             alpha:1] CGColor];
...
- (void)updateTitle {
  ...
  // For inscrutable reasons, putting the tab stop all the way at the right edge
  // of the field doesn't work--the tab seems to be ignored.
  NSTextTab *tab =
  ...
  NSString *string =
      [NSString stringWithFormat:@"%@\t%@", self.titleValue ?: @"",
                                 self.shortcutValue ?: @""];
  ...
  rect.origin.y = [self
      retinaRound:(NSHeight(_textField.superview.frame) - NSHeight(rect)) /
                  2.0];
...
- (void)setIcon:(NSImage *)image {
  ...
  _iconLayer.contents =
      (id)[image CGImageForProposedRect:NULL context:nil hints:nil];
  [self setNeedsDisplay:YES];
}

- (void)setIconFlipped:(BOOL)isFlipped {
  _iconLayer.transform =
      CATransform3DMakeRotation(isFlipped ? M_PI : 0, 0, 0, 1);
}
...
// Keep the highlight circle centered on the cursor during a drag, up to our
// bounds.
- (void)mouseDragged:(NSEvent *)theEvent {
  ...
  CGPoint position =
      (CGPoint)[self convertPoint:theEvent.locationInWindow fromView:nil];
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  CATransform3D transform =
      CATransform3DConcat(CATransform3DMakeTranslation(-0.5, -0.5, 0),
                          CATransform3DMakeScale(scale, scale, 1));

  CABasicAnimation *animation =
      [CABasicAnimation animationWithKeyPath:@"transform"];
  animation.fromValue =
      (id)[NSValue valueWithCATransform3D:CATransform3DIdentity];
...
- (void)mouseUp:(NSEvent *)theEvent {
  ...
  if (self.enabled && NSPointInRect([self convertPoint:theEvent.locationInWindow
                                              fromView:nil],
                                    self.bounds)) {
