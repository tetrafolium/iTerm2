...
- (nullable NSString *)attachOrLaunchRequest:
...
@property(nonatomic, weak) id<iTermSessionAttachOrLaunchRequestDelegate>
    delegate;
...
+ (instancetype)
    launchRequestWithSession:(PTYSession *)aSession
         ...
         hasServerConnection:(BOOL)hasServerConnection
            ...
            windowController:(PseudoTerminal *_Nonnull)windowController
                  ...
                  completion:(void (^_Nullable)(PTYSession *_Nullable,
                                                BOOL))completion {
  ...
  if (forceUseOldCWD) {
    request->_profileForComputingCommand = [request->_profile
        dictionaryBySettingObject:kProfilePreferenceInitialDirectoryCustomValue
  ...
  request.customShell =
      customShell
          ?: [ITAddressBookMgr
                 customShellForProfile:request.profileForComputingCommand];
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@: %p session=%@ canPrompt=%@ objectType=%@ serverConnection=%@ "
          @"urlString=%@ allowURLSubs=%@ environment=%@ customShell=%@ "
          @"oldCWD=%@ forceUseOldCWD=%@ command=%@ isUTF8=%@ substitutions=%@ "
          @"windowController=%@>",
          NSStringFromClass(self.class), self, self.session, @(self.canPrompt),
          @(self.objectType),
          self.hasServerConnection ? @(self.xx_serverConnection.type) : @"none",
          self.urlString, @(self.allowURLSubs), self.environment,
          self.customShell, self.oldCWD, @(self.forceUseOldCWD), self.command,
          @(self.isUTF8), self.substitutions, self.windowController];
}
...
- (void)computeWorkingDirectoryWithCompletion:(void (^)(void))completion {
  ...
  iTermInitialDirectory *initialDirectory =
      [iTermInitialDirectory initialDirectoryFromProfile:self.profile
                                              objectType:self.objectType];
  ...
  [initialDirectory
      evaluateWithOldPWD:self.oldCWD
                   scope:self.session.variablesScope
...
- (BOOL)computeSubstitutions {
  ...
  // If the command or name have any $$VARS$$ not accounted for above, prompt
  // the user for substitutions.
  ...
  if (self.completion) {
    ...
    // Ensure the controller has it before removing it, since this might get
    // called by the controller.
    // TODO: Remove cyclic dependency
    dispatch_async(dispatch_get_main_queue(), ^{
      [[[iTermController sharedInstance] terminalWithSession:self.session]
          closeSessionWithoutConfirmation:self.session];
    });
    DLog(@"  Completing");
...
- (void)didComputeSubstitutions {
  ...
  [self.windowController setName:self.name ?: @"" forSession:self.session];
  DLog(@"  done setting name");
}

// Returns nil if the user pressed cancel, otherwise returns a dictionary that's
// a supeset of |substitutions|.
- (nullable NSDictionary *)substitutionsAfterPrompting {
  NSDictionary *baseSubstitutions =
      self.allowURLSubs ? [self substitutionsForURL:self.urlString] : @{};
  baseSubstitutions =
      [baseSubstitutions dictionaryByMergingDictionary:@{@"$$$$" : @"$$"}];
  ...
  for (NSString *var in allVars) {
    if (!baseSubstitutions[var]) {
      NSString *value =
          [self.delegate attachOrLaunchRequest:self
...
- (void)updateVariables {
  ...
  if (hostSub) {
    [self.session.variablesScope setValue:hostSub
                         forVariableNamed:iTermVariableKeySessionHostname];
  }

  NSString *userSub = self.substitutions[@"$$USER$$"];
  if (userSub) {
    [self.session.variablesScope setValue:userSub
...
- (void)setWorkingDirectory:(NSString *)suggestion {
  ...
  _environment =
      [self.environment ?: @{} dictionaryBySettingObject:_workingDirectory
                                                  forKey:@"PWD"];
}
...
- (PTYSession *)newSessionWithProfile:(Profile *)profile
  ...
  [[aSession screen]
      setUnlimitedScrollback:[profile[KEY_UNLIMITED_SCROLLBACK] boolValue]];
  [[aSession screen]
      setMaxScrollbackLines:[profile[KEY_SCROLLBACK_LINES] intValue]];
...
- (void)handleRealizedRequest:(iTermSessionAttachOrLaunchRequest *)request
  ...
  if (request.hasServerConnection) {
    ...
    [request.session attachToServer:request.xx_serverConnection
                         completion:^{
                           if (completion) {
                             completion(YES);
                           }
  ...
  if (request.partialAttachment) {
    const iTermJobManagerAttachResults results =
        [request.session tryToFinishAttachingToMultiserverWithPartialAttachment:
                             request.partialAttachment];
    const BOOL ok = (results & iTermJobManagerAttachResultsAttached) ==
...
- (nullable NSString *)attachOrLaunchRequest:
  ...
  _parameterPanelWindowController = [[iTermParameterPanelWindowController alloc]
      initWithWindowNibName:@"iTermParameterPanelWindowController"];
  [_parameterPanelWindowController window];
  [_parameterPanelWindowController.parameterName
      setStringValue:[NSString stringWithFormat:@"“%@”:", name]];
  [_parameterPanelWindowController.parameterValue setStringValue:@""];

  [window beginSheet:_parameterPanelWindowController.window
