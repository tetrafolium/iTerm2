...
// Must find at least this many divider chars in a row for it to count as a
// divider.
...
+ (instancetype)textExtractorWithDataSource:
    (id<iTermTextDataSource>)dataSource {
...
+ (NSCharacterSet *)wordSeparatorCharacterSet {
  ...
  NSMutableCharacterSet *complement =
      [[[NSMutableCharacterSet alloc] init] autorelease];
  [complement
      formUnionWithCharacterSet:[NSCharacterSet alphanumericCharacterSet]];
  [complement
...
- (BOOL)hasLogicalWindow {
  return !(_logicalWindow.location == 0 &&
           [self xLimit] == [_dataSource width]);
}

- (void)restrictToLogicalWindowIncludingCoord:(VT100GridCoord)coord {
  NSIndexSet *possibleDividers =
      [self possibleColumnDividerIndexesAround:coord];
...
- (NSString *)fastWordAt:(VT100GridCoord)location {
  ...
  VT100GridCoordRange theRange =
      VT100GridCoordRangeMake(location.x, location.y, width, location.y + 1);
  ...
  if (theClass == kTextExtractorClassWord) {
    ...
    [self enumerateCharsInRange:VT100GridWindowedRangeMake(
                                    theRange, _logicalWindow.location,
        ...
        charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
          ...
          iTermTextExtractorClass newClass = [self
                     classForCharacter:theChar
              definitionOfAlphanumeric:iTermAlphaNumericDefinitionUnixCommands];
          if (newClass == kTextExtractorClassWord) {
            foundWord = YES;
            if (theChar.complexChar || theChar.code < ITERM2_PRIVATE_BEGIN ||
  ...
  [self enumerateInReverseCharsInRange:VT100GridWindowedRangeMake(
                                           theRange, _logicalWindow.location,
                                           _logicalWindow.length)
...
- (NSURL *)urlOfHypertextLinkAt:(VT100GridCoord)coord
  ...
  *urlId = [[iTermURLStore sharedInstance] paramWithKey:@"id"
                                                forCode:c.urlCode];
  return [[iTermURLStore sharedInstance] urlForCode:c.urlCode];
}

- (VT100GridWindowedRange)
    rangeOfCoordinatesAround:(VT100GridCoord)origin
             maximumDistance:(int)maximumDistance
                 passingTest:
                     (BOOL (^)(screen_char_t *c, VT100GridCoord coord))block {
  ...
  while (distanceLeft > 0 && !VT100GridCoordEquals(coord, previousCoord) &&
         block(&c, coord)) {
    previousCoord = coord;
    coord = [self predecessorOfCoord:coord];
...
- (int)startOfIndentationOnLine:(int)line {
  ...
  [self
      enumerateCharsInRange:VT100GridWindowedRangeMake(
                                VT100GridCoordRangeMake(
                                    0, line, [_dataSource width], line),
                                _logicalWindow.location, _logicalWindow.length)
      charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
                      VT100GridCoord coord) {
        if (!theChar.complexChar &&
            (theChar.code == ' ' || theChar.code == '\t' || theChar.code == 0 ||
             theChar.code == TAB_FILLER)) {
          result++;
          return NO;
        } else {
          return YES;
...
- (VT100GridAbsWindowedRange)rangeForWordAtAbsCoord:
                                 (VT100GridAbsCoord)absLocation
                                      maximumLength:(NSInteger)maximumLength {
  return VT100GridAbsWindowedRangeFromWindowedRange(
      [self rangeForWordAt:[self coordFromAbsolute:absLocation]
...
- (VT100GridAbsWindowedRange)
    rangeForBigWordAtAbsCoord:(VT100GridAbsCoord)location
                maximumLength:(NSInteger)maximumLength {
  return VT100GridAbsWindowedRangeFromWindowedRange(
      [self rangeForBigWordAt:[self coordFromAbsolute:location]
                maximumLength:maximumLength],
      [_dataSource totalScrollbackOverflow]);
}
...
// The maximum length is a rough guideline. You might get a word up to twice as
// long.
- (VT100GridWindowedRange)rangeForWordAt:(VT100GridCoord)location
                           maximumLength:(NSInteger)maximumLength
                                     big:(BOOL)big {
  ...
  DLog(@"Compute range for word at %@, max length %@",
       VT100GridCoordDescription(location), @(maximumLength));
  ...
  DLog(@"Initial class for '%@' at %@ is %@",
       [self stringForCharacter:[self characterAt:location]],
       VT100GridCoordDescription(location), @(theClass));
  if (theClass == kTextExtractorClassDoubleWidthPlaceholder) {
  ...
  if (theClass == kTextExtractorClassOther) {
    ...
    return [self windowedRangeWithRange:VT100GridCoordRangeMake(
                                            location.x, location.y,
                                            location.x + 1, location.y)];
  }

  // String composed of the characters found to be in the word, excluding
  ...
  VT100GridCoordRange theRange = VT100GridCoordRangeMake(
      location.x, location.y, width, [_dataSource numberOfLines] - 1);
  ...
  [self enumerateCharsInRange:VT100GridWindowedRangeMake(
      ...
      charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
                      VT100GridCoord coord) {
        DLog(@"Character at %@ is '%@'", VT100GridCoordDescription(coord),
             [self stringForCharacter:theChar]);
        ++iterations;
        ...
        BOOL isInWord =
            (newClass == kTextExtractorClassDoubleWidthPlaceholder ||
             newClass == theClass);
        if (isInWord) {
          DLog(@"Is in word");
          if (theChar.complexChar || theChar.code < ITERM2_PRIVATE_BEGIN ||
            ...
            [stringFromLocation
                appendString:(ScreenCharToStr(&theChar) ?: @"")];
  ...
  // We want to iterate backward over the string and concatenate characters in
  // reverse order. Appending to the start of a NSMutableString is very slow,
  // but appending to the start of a NSMutableArray is fast. So we build an
  // array of tiny strings in the reverse order of how they appear and then
  // concatenate them after the enumeration.
        ...
        if (isInWord) {
          ...
          if (theChar.complexChar || theChar.code < ITERM2_PRIVATE_BEGIN ||
            ...
            if (theString) {
              ...
              [coords insertObject:[NSValue valueWithGridCoord:coord]
                           atIndex:0];
              [stringLengthsInPrefix insertObject:@(theString.length)
                                          atIndex:0];
            }
          }
        }
  ...
  if (!coords.count) {
    ...
    return [self
        windowedRangeWithRange:VT100GridCoordRangeMake(location.x, location.y,
                                                       location.x, location.y)];
  }
  ...
  if (theClass != kTextExtractorClassWord || big) {
    ...
    return
        [self windowedRangeWithRange:VT100GridCoordRangeMake(start.x, start.y,
                                                             end.x + 1, end.y)];
  }

  __block VT100GridWindowedRange result;
  [self performBlockWithLineCache:^{
    DLog(@"An alphanumeric character was selected. Begin language-specific "
         @"logic");

    // An alphanumeric character was selected. This is where it gets
    // interesting.

    // We have now retrieved the longest possible string that could have a word.
    // This is because we are more permissive than the OS about what can be in a
    // word (the user can add punctuation, for example, making foo/bar a word if
    // / belongs to the “characters considered part of a word.”) Now we want to
    // shrink the range. For non-English languages, there is an added wrinkle:
    // in issue 4325 we see that 翻真的 consists of two words: 翻 and 真的. The
    // OS (presumably by using ICU's text boundary analysis code) knows how to
    // do the segmentation.

    NSString *string =
        [stringBeforeLocation stringByAppendingString:stringFromLocation];
    ...
    // Set end to an index that is not in the middle of an OS-defined-word. It
    // will be at the start of a word or on a whitelisted character.
    ...
    while (end < coords.count) {
      DLog(@"Consider end=%@ at %@", @(end),
           VT100GridCoordDescription([coords[end] gridCoordValue]));
      if ([self
              isWhitelistedAlphanumericAtCoord:[coords[end] gridCoordValue]]) {
        DLog(@"Is whitelisted");
        ++end;
    ...
    // `provisionalStart` is an initial place to begin looking for the start of
    // the word. This is used to compute the initial value of `start`, later on.
    // If there is a suffix it is the index of the first character of the
    // suffix. Otherwise it is the index of the last character of the prefix.
    NSUInteger provisionalStart = numberOfCellsInPrefix;
    if (coords.count == numberOfCellsInPrefix) {
      // Earlier, we bailed out if `coords.count` was 0. Since `coords.count` >
      // 0 and `coords.count` equals `numberOfCellsInPrefix` and
    ...
    // First, ensure that start is either at the start of a word (as defined by
    // the OS) or on a whitelisted character.
    if ([self isWhitelistedAlphanumericAtCoord:[coords[provisionalStart]
                                                   gridCoordValue]]) {
      ...
      // Not on a whitelisted character. Set start to the index of the cell of
      // the first character of the word enclosing the cell indexed to by
      // `provisionalStart`.
      DLog(@"Not starting on a whitelisted character");
      previousCharacterWasWhitelisted = NO;
      NSUInteger location =
          [attributedString
              doubleClickAtIndex:[indexes[provisionalStart] integerValue]]
              .location;
    ...
    //  Move back until two consecutive OS-defined words are found or we reach
    //  the start of the string.
    while (start > 0) {
      DLog(@"Consider start=%@ at %@", @(start - 1),
           VT100GridCoordDescription([coords[start - 1] gridCoordValue]));
      if ([self isWhitelistedAlphanumericAtCoord:[coords[start - 1]
                                                     gridCoordValue]]) {
        DLog(@"Is whitelisted");
        --start;
        previousCharacterWasWhitelisted = YES;
        ...
        NSUInteger location =
            [attributedString
                doubleClickAtIndex:[indexes[start - 1] integerValue]]
                .location;
        start = [self indexInSortedArray:indexes
              withValueLessThanOrEqualTo:location
                   searchingBackwardFrom:provisionalStart];
    ...
    // Make sure to include the DWC_RIGHT after the last character to be
    // selected.
    if (endCoord.x < [self xLimit] &&
        [self haveDoubleWidthExtensionAt:endCoord]) {
      endCoord.x += 1;
    }
    result = [self windowedRangeWithRange:VT100GridCoordRangeMake(
...
// Make characterAt: much faster when called with the same line number over and
// over again. Assumes the line buffer won't be mutated while it's running.
...
- (NSInteger)indexInSortedArray:(NSArray<NSNumber *> *)indexes
  ...
  NSInteger index = [indexes
        indexOfObject:@(maximumValue)
  ...
  if (index == start + 1) {
    // maximumValue is larger than all values. The last value is the largest one
    // <= maximumValue.
...
// Returns indexes.count if no value can be found greater or equal to
// `minimumValue`. This could be a binary search but it's better to keep it
// simple.
- (NSInteger)indexInSortedArray:(NSArray<NSNumber *> *)indexes
...
- (NSIndexSet *)indexesOnLine:(int)line
...
- (SmartMatch *)smartSelectionAt:(VT100GridCoord)location
  ...
  NSMutableArray *coords =
      [NSMutableArray arrayWithCapacity:numLines * _logicalWindow.length];
  NSString *textWindow =
      [self textAround:location
                    radius:2
  ...
  NSMutableDictionary *matches =
      [NSMutableDictionary dictionaryWithCapacity:13];
  ...
  for (int j = 0; j < numRules; j++) {
    ...
    if (actionRequired &&
        [[SmartSelectionController actionsInRule:rule] count] == 0) {
    ...
    for (int i = 0; i <= targetOffset; i++) {
      NSString *substring = [textWindow
          substringWithRange:NSMakeRange(i, [textWindow length] - i)];
      ...
      if (temp.location != NSNotFound) {
        if (i + temp.location <= targetOffset &&
            i + temp.location + temp.length > targetOffset) {
          ...
          if (!oldMatch || score > oldMatch.score) {
            ...
            if (debug) {
              NSLog(@"Regex matched. Add result %@ at %d,%lld -> %d,%lld with "
                    @"score %lf",
                    result, match.startX, match.absStartY, match.endX,
                    match.absEndY, match.score);
            }
          }
  ...
  if ([matches count]) {
    NSArray *sortedMatches =
        [[matches allValues] sortedArrayUsingSelector:@selector(compare:)];
    ...
    *range = [self windowedRangeWithRange:theRange];
    ...
    if (actionRequired) {
      // There is no match when using word selection and
      // rangeForWordAt:maximumLength: can be slow.
      *range = VT100GridWindowedRangeMake(
          VT100GridCoordRangeMake(-1, -1, -1, -1), -1, -1);
    } else if (location.y >= 0) {
      *range = [self rangeForWordAt:location
                      maximumLength:kReasonableMaximumWordLength];
    } else {
      *range = VT100GridWindowedRangeMake(
          VT100GridCoordRangeMake(-1, -1, -1, -1), _logicalWindow.location,
          _logicalWindow.length);
    }
    return nil;
  }
...
- (iTermTextExtractorClass)classForCharacter:(screen_char_t)theCharacter
  ...
  return [self classForCharacter:theCharacter
        definitionOfAlphanumeric:bigWords
                                     ? iTermAlphaNumericDefinitionBigWords
                                     : iTermAlphaNumericDefinitionUserDefined];
}
...
- (iTermTextExtractorClass)classForCharacter:(screen_char_t)theCharacter
                    definitionOfAlphanumeric:
                        (iTermAlphaNumericDefinition)definition {
  if (!theCharacter.complexChar) {
    if (theCharacter.code == TAB_FILLER) {
      return kTextExtractorClassWhitespace;
    } else if (theCharacter.code == DWC_RIGHT ||
               theCharacter.complexChar == DWC_SKIP) {
  ...
  if ([self characterIsAlphanumeric:asString] ||
      [self characterShouldBeTreatedAsAlphanumeric:asString
                          definitionOfAlphanumeric:definition]) {
...
- (BOOL)characterIsAlphanumeric:(NSString *)characterAsString {
  NSRange range = [characterAsString
      rangeOfCharacterFromSet:[NSCharacterSet alphanumericCharacterSet]];
...
- (BOOL)characterShouldBeTreatedAsAlphanumeric:(NSString *)characterAsString
  ...
  case iTermAlphaNumericDefinitionUserDefined: {
    NSRange range = [[iTermPreferences
...
- (VT100GridWindowedRange)rangeOfParentheticalSubstringAtLocation:
    (VT100GridCoord)location {
  ...
  if (!match) {
    return
        [self windowedRangeWithRange:VT100GridCoordRangeMake(-1, -1, -1, -1)];
  }
  ...
  if (left == 0) {
    ...
    return [self windowedRangeWithRange:VT100GridCoordRangeMake(end.x, end.y,
                                                                location.x + 1,
                                                                location.y)];
  }
}
...
- (VT100GridAbsCoord)coordToAbsolute:(VT100GridCoord)coord {
  return VT100GridAbsCoordMake(coord.x,
                               [_dataSource totalScrollbackOverflow] + coord.y);
}

- (VT100GridAbsCoord)successorOfAbsCoordSkippingContiguousNulls:
...
- (VT100GridCoord)successorOfCoordSkippingContiguousNulls:
    (VT100GridCoord)coord {
  do {
    coord.x++;
    int xLimit = [self xLimit];
...
- (VT100GridAbsCoord)predecessorOfAbsCoordSkippingContiguousNulls:
    (VT100GridAbsCoord)coord {
  return [self coordToAbsolute:[self predecessorOfCoordSkippingContiguousNulls:
                                         [self coordFromAbsolute:coord]]];
}

- (VT100GridCoord)predecessorOfCoordSkippingContiguousNulls:
    (VT100GridCoord)coord {
  int moved = 0;
  VT100GridCoord prev;
  do {
...
// Returns an integer that uniquely identifies a coordinate in a grid with the
// passed-in |width|.
...
- (int)numberOfCoordsInIndexSet:(NSIndexSet *)coords
  ...
  NSUInteger coord1Index = [self indexForCoord:coord1
                                         width:[_dataSource width]];
  NSUInteger coord2Index = [self indexForCoord:coord2
                                         width:[_dataSource width]];
...
- (NSString *)textAround:(VT100GridCoord)coord
  ...
  VT100GridCoordRange theRange =
      VT100GridCoordRangeMake(0, coord.y - radius, coord.x, coord.y);
  NSMutableString *joinedLines =
      [NSMutableString stringWithCapacity:radius * _logicalWindow.length];
  VT100GridWindowedRange windowedRange = VT100GridWindowedRangeMake(
      theRange, _logicalWindow.location, _logicalWindow.length);
  [self enumerateInReverseCharsInRange:windowedRange
      charBlock:^BOOL(screen_char_t theChar, VT100GridCoord charCoord) {
        ...
        if (ignoreContinuations && windowedRange.columnWindow.length > 0 &&
            charCoord.x == windowedRange.columnWindow.location +
                               windowedRange.columnWindow.length - 1 &&
          ...
          // no-op
                   ...
                   theChar.code > ITERM2_PRIVATE_END) {
          NSString *string =
              CharToStr(theChar.code, theChar.complexChar) ?: @"";
          [joinedLines insertString:string atIndex:0];
          for (int i = 0; i < [string length]; i++) {
            [coords insertObject:[NSValue valueWithGridCoord:charCoord]
                         atIndex:0];
          }
        }
        return NO;
      ...
      eolBlock:^BOOL(unichar code, int numPrecedingNulls, int line) {
        return
            [self shouldStopEnumeratingWithCode:code
                                       numNulls:numPrecedingNulls
  ...
  theRange = VT100GridCoordRangeMake(coord.x, coord.y, [_dataSource width],
                                     coord.y + radius);
  windowedRange = VT100GridWindowedRangeMake(theRange, _logicalWindow.location,
                                             _logicalWindow.length);
  ...
  *targetOffset = -1;
  for (int i = 0; i < coords.count; i++) {
    NSComparisonResult order =
        VT100GridCoordOrder(coord, [coords[i] gridCoordValue]);
...
- (VT100GridWindowedRange)rangeForWrappedLineEncompassing:(VT100GridCoord)coord
                                     respectContinuations:
                                         (BOOL)respectContinuations
                                                 maxChars:(int)maxChars {
  int start =
      [self lineNumberWithStartOfWholeLineIncludingLine:coord.y
  ...
  int end = [self lineNumberWithEndOfWholeLineIncludingLine:coord.y
                                       respectContinuations:respectContinuations
                                                   maxChars:maxChars];
  return [self windowedRangeWithRange:VT100GridCoordRangeMake(
                                          _logicalWindow.location, start,
                                          [self xLimit], end)];
}

- (BOOL)haveNonWhitespaceInFirstLineOfRange:
  ...
  [self enumerateCharsInRange:windowedRange
      charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
        ...
        if (theChar.complexChar) {
          ...
          if ([string rangeOfCharacterFromSet:whitespaceCharacterSet]
                  .location != NSNotFound) {
            result = YES;
            return YES;
          } else {
...
- (NSAttributedString *)
    attributedStringForSnippetForRange:(VT100GridAbsCoordRange)range
  ...
  const VT100GridCoordRange relativeRange =
      VT100GridCoordRangeFromAbsCoordRange(
          range, self.dataSource.totalScrollbackOverflow);
  const NSInteger maxMatchLength = 1024;
  NSString *match =
      [self contentInRange:VT100GridWindowedRangeMake(relativeRange,
                                                      _logicalWindow.location,
                                                      _logicalWindow.length)
          ...
          trimTrailingWhitespace:NO
               ...
               continuationChars:nil
                          coords:nil];
  NSAttributedString *matchString =
  ...
  NSString *prefix =
      [[self
           wrappedLocatedStringAt:relativeRange.start
                          forward:NO
  ...
  NSString *suffix = [[self wrappedLocatedStringAt:relativeRange.end
                          ...
                          .string stringByTrimmingTrailingWhitespace];
  if (suffix.length > maximumSuffixLength) {
    suffix = [[[suffix stringByDroppingLastCharacters:1]
        stringByTrimmingOrphanedSurrogates] stringByAppendingString:@"…"];
  }
  NSAttributedString *attributedPrefix = [[[NSAttributedString alloc]
      initWithString:prefix
          attributes:regularAttributes] autorelease];
  NSAttributedString *attributedSuffix = [[[NSAttributedString alloc]
...
- (id)contentInRange:(VT100GridWindowedRange)windowedRange
  ...
  return attributeProvider
             ? ((iTermLocatedAttributedString *)locatedString).attributedString
...
- (id)locatedStringInRange:(VT100GridWindowedRange)windowedRange
  ...
  DLog(@"Find selected text in range %@ pad=%d, includeLastNewline=%d, trim=%d",
       VT100GridWindowedRangeDescription(windowedRange), (int)pad,
       (int)includeLastNewline, (int)trimSelectionTrailingSpaces);
  ...
  if (attributeProvider) {
    locatedAttributedString =
        [[[iTermLocatedAttributedString alloc] init] autorelease];
  ...
  [self enumerateCharsInRange:windowedRange
      charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
        ...
        if (theChar.image) {
          if (attributeProvider && theChar.foregroundColor == 0 &&
              theChar.backgroundColor == 0) {
            ...
            if (image) {
              ...
              [locatedAttributedString
                  appendAttributedString:attributedStringWithAttachment
                                      at:coord];
            }
          }
        ...
        } else if (theChar.code != DWC_RIGHT && theChar.code != DWC_SKIP) {
          ...
          if (continuationChars && windowedRange.columnWindow.length > 0 &&
              coord.x == windowedRange.columnWindow.location +
                             windowedRange.columnWindow.length - 1 &&
              theChar.code == '\\' && !theChar.complexChar) {
            // Is a backslash at the right edge of a window.
        ...
        } else if ([locatedString length] >
                   maxBytes + kMaximumOversizeAmountWhenTruncatingHead) {
          // Truncate from head when significantly oversize.
          //
          // Removing byte from the beginning of the string is slow. The only
          // reason to do it is to save memory. Remove a big chunk periodically.
          // After enumeration is done we'll cut it to the exact size it needs
          // to be.
      ...
      eolBlock:^BOOL(unichar code, int numPrecedingNulls, int line) {
        BOOL ignore =
            (!copiedImage && !lineContainsNonImage && lineContainsImage);
        ...
        if (windowedRange.columnWindow.length) {
          right = windowedRange.columnWindow.location +
...
- (VT100GridAbsCoordRange)rangeByTrimmingWhitespaceFromRange:
...
- (VT100GridAbsCoordRange)
    rangeByTrimmingWhitespaceFromRange:(VT100GridAbsCoordRange)range
                               leading:(BOOL)leading
                              trailing:
                                  (iTermTextExtractorTrimTrailingWhitespace)
                                      trailing {
  ...
  VT100GridCoordRange localRange = VT100GridCoordRangeMake(
      range.start.x, range.start.y - totalScrollbackOverflow, range.end.x,
  ...
  VT100GridWindowedRange windowedRange = VT100GridWindowedRangeMake(
      localRange, _logicalWindow.location, _logicalWindow.length);
  if (leading) {
    [self enumerateCharsInRange:windowedRange
        charBlock:^BOOL(screen_char_t *currentLine, screen_char_t theChar,
                        VT100GridCoord coord) {
          NSString *string = ScreenCharToStr(&theChar);
          if ([string rangeOfCharacterFromSet:nonWhitespace].location !=
              NSNotFound) {
  ...
  if (trailing != iTermTextExtractorTrimTrailingWhitespaceNone) {
    ...
    [self enumerateInReverseCharsInRange:windowedRange
        charBlock:^BOOL(screen_char_t theChar, VT100GridCoord coord) {
          NSString *string = ScreenCharToStr(&theChar);
          BOOL result = NO;
          if ([string rangeOfCharacterFromSet:whitespace].location !=
              NSNotFound) {
...
// maxChars is a rough guideline here. If the line is very long it will go over
// by up to the width of one windowed range.
- (int)lineNumberWithStartOfWholeLineIncludingLine:(int)y
  ...
  int i = y;
  NSInteger count = 0;
  while (count < maxChars + width && i > 0 &&
         [self lineHasSoftEol:i - 1
             respectContinuations:respectContinuations]) {
    count += width;
    i--;
  }
...
- (BOOL)lineHasSoftEol:(int)y respectContinuations:(BOOL)respectContinuations {
  ...
  if ([self hasLogicalWindow]) {
    // If there are soft boundaries, it's impossible to detect soft line wraps
    // so just stop at whitespace.
...
- (BOOL)tabFillerAtIndex:(int)index isOrphanInLine:(screen_char_t *)line {
  // A tab filler orphan is a tab filler that is followed by a tab filler orphan
  // or a non-tab character.
  int xLimit = [self xLimit];
...
- (iTermLocatedString *)wrappedLocatedStringAt:(VT100GridCoord)coord
                           ...
                           respectHardNewlines:(BOOL)respectHardNewlines
                                      maxChars:(int)maxChars
                             continuationChars:
                                 (NSMutableIndexSet *)continuationChars
  ...
  if (respectHardNewlines) {
    range = [self rangeForWrappedLineEncompassing:coord
                             respectContinuations:YES
                                         maxChars:maxChars];
  } else {
    VT100GridCoordRange coordRange = VT100GridCoordRangeMake(
        _logicalWindow.location, MAX(0, coord.y - 10), [self xLimit],
  ...
  if (forward) {
    ...
    if (VT100GridCoordOrder(range.coordRange.start, range.coordRange.end) !=
        NSOrderedAscending) {
      return [[[iTermLocatedString alloc] init] autorelease];
    }
  } else {
    nullPolicy = kiTermTextExtractorNullPolicyFromLastToEnd;
    // This doesn't include the boundary character when returning a prefix
    // because we don't want it twice when getting the prefix and suffix at the
    // same coord.
    range.coordRange.end = coord;
    if (VT100GridCoordOrder(range.coordRange.start, range.coordRange.end) !=
        NSOrderedAscending) {
...
- (void)enumerateCharsInRange:(VT100GridWindowedRange)range
                    charBlock:(BOOL (^)(screen_char_t *currentLine,
                                        screen_char_t theChar,
                                        VT100GridCoord coord))charBlock
                     eolBlock:(BOOL (^)(unichar code, int numPrecedingNulls,
                                        int line))eolBlock {
  int width = [_dataSource width];
  ...
  int endx = range.columnWindow.length
  ...
  BOOL fullWidth = ((range.columnWindow.location == 0 &&
                     range.columnWindow.length == width) ||
                    range.columnWindow.length <= 0);
  int left = range.columnWindow.length ? range.columnWindow.location : 0;
  for (int y = MAX(0, range.coordRange.start.y);
       y <= MIN(bound, range.coordRange.end.y); y++) {
    if (y == range.coordRange.end.y) {
      // Reduce endx for last line.
      const int reducedEndX = range.columnWindow.length
                                  ? VT100GridWindowedRangeEnd(range).x
    ...
    for (int x = endx - 1; x >= range.columnWindow.location; x--) {
      ITAssertWithMessage(x >= 0 && x < width,
                          @"Counting number of nulls. x=%@ range=%@ width=%@",
                          @(x), VT100GridWindowedRangeDescription(range),
                          @(width));
      BOOL isNull;
      // If not full-width then treat terminal spaces as nulls. This makes soft
      // selection find newlines more reliably, but can occasionally insert
      // newlines where they don't belong.
      if (fullWidth) {
    ...
    for (int x = MIN(width - 1, MAX(range.columnWindow.location, startx));
         x < endx - numNulls; x++) {
      ITAssertWithMessage(
          x >= 0 && x < width,
...
- (void)enumerateInReverseCharsInRange:(VT100GridWindowedRange)range
                             charBlock:(BOOL (^)(screen_char_t theChar,
                                                 VT100GridCoord coord))charBlock
                              eolBlock:(BOOL (^)(unichar code,
                                                 int numPrecedingNulls,
                                                 int line))eolBlock {
  int xLimit = range.columnWindow.length == 0
                   ? [_dataSource width]
                   : (range.columnWindow.location + range.columnWindow.length);
...
- (BOOL)coordContainsColumnDivider:(VT100GridCoord)coord {
  int n = 1;
  for (int y = coord.y - 1; y >= 0 && y > coord.y - kNumCharsToSearchForDivider;
       y--) {
  ...
  for (int y = coord.y + 1;
       y < limit && y < coord.y + kNumCharsToSearchForDivider; y++) {
    if ([self characterAtCoordIsColumnDivider:VT100GridCoordMake(coord.x, y)]) {
      n++;
    } else {
      break;
