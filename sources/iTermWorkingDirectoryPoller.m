...
- (instancetype)initWithTmuxGateway:(TmuxGateway *)gateway
  ...
  if (self) {
    ...
    _completions = [NSMutableArray array];
    _tmuxOptionMonitor = [[iTermTmuxOptionMonitor alloc]
        ...
        fallbackVariableName:nil
                ...
                variableName:nil
                       block:^(NSString *_Nonnull directory) {
                         [weakSelf
                             tmuxOptionMonitorDidProduceDirectory:directory];
                       }];
  }
  return self;
}
...
- (void)didReceiveLineFeed {
  ...
  [_pwdPollRateLimit
      performRateLimitedSelector:@selector(maybePollForWorkingDirectory)
                        onTarget:self
                      withObject:nil];
  [self pollIfNeeded];
}
...
- (void)pollForWorkingDirectory {
  ...
  [iTermLSOF asyncWorkingDirectoryOfProcess:pid
...
- (void)didInferWorkingDirectory:(NSString *)pwd valid:(BOOL)valid {
  ...
  NSArray<iTermWorkingDirectoryPollerClosure> *completions =
      [_completions copy];
  [_completions removeAllObjects];
  [completions enumerateObjectsUsingBlock:^(
                   iTermWorkingDirectoryPollerClosure _Nonnull completion,
...
- (void)tmuxOptionMonitorDidProduceDirectory:(NSString *)directory {
  // These can't be invalidated because we don't have local lookups competing
  // with remote. Remote should always win.
