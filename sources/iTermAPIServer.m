...
NSString *const iTermAPIServerDidReceiveMessage =
    @"iTermAPIServerDidReceiveMessage";
NSString *const iTermAPIServerWillSendMessage =
    @"iTermAPIServerWillSendMessage";
NSString *const iTermAPIServerConnectionRejected =
    @"iTermAPIServerConnectionRejected";
NSString *const iTermAPIServerConnectionAccepted =
    @"iTermAPIServerConnectionAccepted";
NSString *const iTermAPIServerConnectionClosed =
    @"iTermAPIServerConnectionClosed";
...
+ (NSString *)folderForUnixSocket {
  return [[[NSFileManager defaultManager] applicationSupportDirectory]
      stringByAppendingPathComponent:@"private"];
}
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _executionQueue =
        dispatch_queue_create("com.iterm2.apiexec", DISPATCH_QUEUE_SERIAL);
...
- (BOOL)listenOnUnixSocket {
  ...
  [[NSFileManager defaultManager]
            createDirectoryAtPath:[iTermAPIServer folderForUnixSocket]
  ...
  BOOL ok = [_unixSocket
      listenWithBacklog:5
                 accept:^(int fd, iTermSocketAddress *clientAddress,
                          NSNumber *euid) {
...
- (void)postAPINotification:(ITMNotification *)notification
  ...
  dispatch_async(_queue, ^{
    iTermWebSocketConnection *webSocketConnection =
        self->_connections[connectionKey];
    if (webSocketConnection) {
      ITMServerOriginatedMessage *response =
          [[ITMServerOriginatedMessage alloc] init];
...
- (void)stop {
  ...
  dispatch_sync(_queue, ^{
    [self->_pendingConnections
        enumerateObjectsUsingBlock:^(iTermHTTPConnection *_Nonnull connection,
...
// _queue
- (void)queueStop {
  [_connections enumerateKeysAndObjectsUsingBlock:^(
                    id _Nonnull key, iTermWebSocketConnection *_Nonnull conn,
                    BOOL *_Nonnull stop) {
...
- (void)didAcceptConnectionOnFileDescriptor:(int)fd
  ...
  dispatch_async(queue, ^{
    iTermHTTPConnection *connection =
        [[iTermHTTPConnection alloc] initWithFileDescriptor:fd
...
- (void)reallyDidAcceptConnection:(iTermHTTPConnection *)connection
  ...
  if (!connection.euid || connection.euid.unsignedIntValue != geteuid()) {
    ...
    dispatch_async(connection.queue, ^{
      NSString *reason = [NSString
          stringWithFormat:@"Peer's euid of %@ not equal to my euid of %@",
  ...
  [self startRequestOnConnection:connection
                      ...
                      completion:^(BOOL ok, NSString *reason) {
                        ...
                        if (!ok) {
                          XLog(@"Reject unix domain socket connection: %@",
                               reason);
                          dispatch_async(connection.queue, ^{
                            [connection unauthorized];
                          });
...
- (void)startRequestOnConnection:(iTermHTTPConnection *)connection
  ...
  dispatch_async(connection.queue, ^{
    ...
    dispatch_async(self->_queue, ^{
      [self reallyStartRequestOnConnection:connection
                                      pids:pids
                                   request:request
...
- (void)reallyStartRequestOnConnection:(iTermHTTPConnection *)connection
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIServerConnectionRejected
                        object:nil
                      userInfo:@{
                        @"reason" : @"Failed to read request from connection",
                        @"pids" : pids
                      }];
    });
  ...
  if (![request.URL.path isEqualToString:@"/"]) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIServerConnectionRejected
                      ...
                      userInfo:@{
                        @"reason" : [NSString
                            stringWithFormat:@"Bad request. URL path was %@, "
                                             @"but should be /",
                                             request.URL.path],
                        @"pids" : pids
                      }];
    ...
    completion(
        NO, [NSString stringWithFormat:@"Path %@ not known", request.URL.path]);
    return;
  }
  ...
  if (webSocketConnection) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      const BOOL disableAuthUI =
          request.allHTTPHeaderFields[@"x-iterm2-disable-auth-ui"] != nil;
      assert(connection.clientAddress.addressFamily == AF_UNIX);
      const BOOL ok = [self.delegate
          apiServerAuthorizeProcesses:pids
      ...
      if (ok) {
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermAPIServerConnectionAccepted
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIServerConnectionRejected
                        object:request.allHTTPHeaderFields[@"x-iterm2-key"]
                      userInfo:@{
                        @"reason" : authReason ?: @"Unknown reason",
                        @"pids" : pids
                      }];
    });
    dispatch_async(connection.queue, ^{
      if ([authReason
              hasPrefix:iTermWebSocketConnectionLibraryVersionTooOldString]) {
...
- (void)sendResponse:(ITMServerOriginatedMessage *)response
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermAPIServerWillSendMessage
                      object:webSocketConnection.key
...
- (void)dispatchRequestWhileNotInTransaction:
            (ITMClientOriginatedMessage *)request
                                  connection:(iTermWebSocketConnection *)
                                                 webSocketConnection {
  ITAssertWithMessage(!self.transaction, @"Already in a transaction");

  __weak __typeof(self) weakSelf = self;
  if (request.submessageOneOfCase ==
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIServerDidReceiveMessage
                        object:webSocketConnection.key
    ...
    // Enter the main queue before sending the transaction response. This
    // guarantees the main thread doesn't do anything after that response is
    // sent.
    dispatch_async(dispatch_get_main_queue(), ^{
      dispatch_async(self->_queue, ^{
        response.transactionResponse = [[ITMTransactionResponse alloc] init];
...
- (void)drainTransaction:(iTermAPITransaction *)transaction {
  while (1) {
    ...
    iTermAPIRequest *transactionRequest =
        [transaction dequeueRequestFromAnyConnection:NO];
    ...
    if (transactionRequest.request.submessageOneOfCase ==
      ...
      ITMServerOriginatedMessage *response =
          [self newResponseForRequest:transactionRequest.request];
      response.transactionResponse = [[ITMTransactionResponse alloc] init];
      response.transactionResponse.status = ITMTransactionResponse_Status_Ok;
      dispatch_async(_queue, ^{
  ...
  dispatch_async(_executionQueue, ^{
    ...
    iTermAPIRequest *apiRequest =
        [transaction dequeueRequestFromAnyConnection:YES];
    while (apiRequest) {
      if (apiRequest.connection) {
        [self enqueueOrDispatchRequest:apiRequest.request
                          onConnection:apiRequest.connection];
      }
...
- (void)finishHandlingRequestWithResponse:(ITMServerOriginatedMessage *)response
                             onConnection:(iTermWebSocketConnection *)
                                              webSocketConnection {
...
- (ITMServerOriginatedMessage *)newResponseForRequest:
    (ITMClientOriginatedMessage *)request {
  ITMServerOriginatedMessage *response =
      [[ITMServerOriginatedMessage alloc] init];
...
- (void)handleTransactionRequest:(ITMClientOriginatedMessage *)request
  ...
  [self finishHandlingRequestWithResponse:response
...
- (void)handleGetPromptRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerGetPrompt:request.getPromptRequest
...
- (void)handleListPromptsRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerListPrompts:request.listPromptsRequest
...
- (void)handleNotificationRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerNotification:request.notificationRequest
...
- (void)handleRegisterToolRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerRegisterTool:request.registerToolRequest
...
- (void)handleSetProfilePropertyRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSetProfileProperty:request.setProfilePropertyRequest
                          handler:^(ITMSetProfilePropertyResponse
                                        *setProfilePropertyResponse) {
                            assert(!handled);
                            handled = YES;
                            response.setProfilePropertyResponse =
                                setProfilePropertyResponse;
                            [weakSelf
                                finishHandlingRequestWithResponse:response
                                                     onConnection:
...
- (void)handleGetProfilePropertyRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerGetProfileProperty:request.getProfilePropertyRequest
                          handler:^(ITMGetProfilePropertyResponse
                                        *getProfilePropertyResponse) {
                            assert(!handled);
                            handled = YES;
                            response.getProfilePropertyResponse =
                                getProfilePropertyResponse;
                            [weakSelf
                                finishHandlingRequestWithResponse:response
                                                     onConnection:
...
- (void)handleListSessionsRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerListSessions:request.listSessionsRequest
...
- (void)handleSendTextRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSendText:request.sendTextRequest
...
- (void)handleCreateTabRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerCreateTab:request.createTabRequest
...
- (void)handleSplitPaneRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSplitPane:request.splitPaneRequest
...
- (void)handleSetPropertyRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSetProperty:request.setPropertyRequest
...
- (void)handleGetPropertyRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerGetProperty:request.getPropertyRequest
...
- (void)handleInjectRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerInject:request.injectRequest
...
- (void)handleActivateRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerActivate:request.activateRequest
...
- (void)handleVariableRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerVariable:request.variableRequest
...
- (void)handleSavedArrangementRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSavedArrangement:request.savedArrangementRequest
                        handler:^(ITMSavedArrangementResponse
                                      *savedArrangementResponse) {
                          assert(!handled);
                          handled = YES;
                          response.savedArrangementResponse =
                              savedArrangementResponse;
                          [weakSelf
                              finishHandlingRequestWithResponse:response
                                                   onConnection:
...
- (void)handleFocusRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerFocus:request.focusRequest
...
- (void)handleListProfilesRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerListProfiles:request.listProfilesRequest
...
- (void)handleServerOriginatedRpcResultRequest:
            (ITMClientOriginatedMessage *)request
                                    connection:(iTermWebSocketConnection *)
                                                   webSocketConnection {
  ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerServerOriginatedRPCResult:request
                                             .serverOriginatedRpcResultRequest
                           connectionKey:webSocketConnection.key
                                 handler:^(ITMServerOriginatedRPCResultResponse
                                               *listProfilesResponse) {
                                   assert(!handled);
                                   handled = YES;
                                   response.serverOriginatedRpcResultResponse =
                                       listProfilesResponse;
                                   [weakSelf
                                       finishHandlingRequestWithResponse:
                                           response
                                                            onConnection:
                                                                webSocketConnection];
                                 }];
...
- (BOOL)tryHandleResponse:(ITMClientOriginatedMessage *)request
  ...
  if (![blockingRPC.rpcID
          isEqualToString:request.serverOriginatedRpcResultRequest.requestId]) {
    return NO;
  }
...
- (void)handleUnhandleableRequest:(ITMClientOriginatedMessage *)request
  ...
  ITMServerOriginatedMessage *response = [self newResponseForRequest:request];
  response.error =
      @"Not ready. This is a bug! Please report it at https://iterm2.com/bugs";
  [self finishHandlingRequestWithResponse:response
                             onConnection:webSocketConnection];
}
...
- (void)handleRestartSessionRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerRestartSession:request.restartSessionRequest
                      handler:^(
                          ITMRestartSessionResponse *restartSessionResponse) {
                        assert(!handled);
                        handled = YES;
                        response.restartSessionResponse =
...
- (void)handleMenuItemRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerMenuItem:request.menuItemRequest
...
- (void)handleGetBroadcastDomainsRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerGetBroadcastDomains:request.getBroadcastDomainsRequest
                           handler:^(
                               ITMGetBroadcastDomainsResponse *theResponse) {
                             assert(!handled);
                             handled = YES;
...
- (void)handleTmuxRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerTmuxRequest:request.tmuxRequest
...
- (void)handleReorderTabsRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerReorderTabsRequest:request.reorderTabsRequest
...
- (void)handlePreferencesRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerPreferencesRequest:request.preferencesRequest
...
- (void)handleColorPresetRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerColorPresetRequest:request.colorPresetRequest
...
- (void)handleSelectionRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSelectionRequest:request.selectionRequest
...
- (void)handleStatusBarComponentRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerStatusBarComponentRequest:request.statusBarComponentRequest
                                 handler:^(ITMStatusBarComponentResponse
                                               *theResponse) {
                                   assert(!handled);
                                   handled = YES;
                                   response.statusBarComponentResponse =
                                       theResponse;
                                   [weakSelf
                                       finishHandlingRequestWithResponse:
                                           response
...
- (void)handleSetBroadcastDomainsRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerSetBroadcastDomainsRequest:request.setBroadcastDomainsRequest
                                  handler:^(ITMSetBroadcastDomainsResponse
                                                *theResponse) {
                                    assert(!handled);
                                    handled = YES;
                                    response.setBroadcastDomainsResponse =
                                        theResponse;
                                    [weakSelf
                                        finishHandlingRequestWithResponse:
                                            response
...
- (void)handleInvokeFunctionRequest:(ITMClientOriginatedMessage *)request
  ...
  __weak __typeof(self) weakSelf = self;
  [_delegate
      apiServerInvokeFunctionRequest:request.invokeFunctionRequest
...
- (void)dispatchRequest:(ITMClientOriginatedMessage *)request
  ...
  case ITMClientOriginatedMessage_Submessage_OneOfCase_SetProfilePropertyRequest:
    [self handleSetProfilePropertyRequest:request
                               connection:webSocketConnection];
    break;

  case ITMClientOriginatedMessage_Submessage_OneOfCase_GetProfilePropertyRequest:
    [self handleGetProfilePropertyRequest:request
                               connection:webSocketConnection];
    break;
  ...
  case ITMClientOriginatedMessage_Submessage_OneOfCase_ServerOriginatedRpcResultRequest:
    [self handleServerOriginatedRpcResultRequest:request
                                      connection:webSocketConnection];
    break;
  ...
  case ITMClientOriginatedMessage_Submessage_OneOfCase_GetBroadcastDomainsRequest:
    [self handleGetBroadcastDomainsRequest:request
                                connection:webSocketConnection];
    break;
  ...
  case ITMClientOriginatedMessage_Submessage_OneOfCase_StatusBarComponentRequest:
    [self handleStatusBarComponentRequest:request
                               connection:webSocketConnection];
    break;

  case ITMClientOriginatedMessage_Submessage_OneOfCase_SetBroadcastDomainsRequest:
    [self handleSetBroadcastDomainsRequest:request
                                connection:webSocketConnection];
    break;
...
- (void)enqueueOrDispatchRequest:(ITMClientOriginatedMessage *)request
  ...
  if ([self tryHandleResponse:request
                toBlockingRPC:self.blockingRPC
                   connection:webSocketConnection]) {
    return;
  }

  [self dispatchRequestWhileNotInTransaction:request
                                  connection:webSocketConnection];
}
...
// _queue
- (void)webSocketConnectionDidTerminate:
    (iTermWebSocketConnection *)webSocketConnection {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [self->_delegate
        apiServerDidCloseConnectionWithKey:webSocketConnection.guid];
    dispatch_async(dispatch_get_main_queue(), ^{
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIServerConnectionClosed
                        object:webSocketConnection.key];
    });
  });
}
...
// _queue
- (void)webSocketConnection:(iTermWebSocketConnection *)webSocketConnection
               didReadFrame:(iTermWebSocketFrame *)frame {
  if (frame.opcode == iTermWebSocketOpcodeBinary) {
    ITMClientOriginatedMessage *request =
        [ITMClientOriginatedMessage parseFromData:frame.payload error:nil];
    DLog(@"Dispatch %@", request);
    if (request) {
      DLog(@"Received request: %@", request);
      __weak __typeof(self) weakSelf = self;
      dispatch_async(_executionQueue, ^{
        [weakSelf enqueueOrDispatchRequest:request
                              onConnection:webSocketConnection];
      });
    }
  }
