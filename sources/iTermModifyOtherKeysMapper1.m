...
- (nullable NSString *)keyMapperStringForPreCocoaEvent:
...
- (BOOL)optionSendsEscPlusForEvent:(NSEvent *)event {
  ...
  [self.delegate modifyOtherKeys:_modifyOther
...
// This is an attempt to port ModifyOtherKeys() from xterm's input.c in the case
// that keyboard->modify_now.other_keys == 1. It's probably wrong because xlib's
// documentation is aggressively vague.
//
...
- (BOOL)shouldModifyOtherKeysForEvent:(NSEvent *)event
  ...
  if ((effectiveModifiers & NSEventModifierFlagControl) != 0) {
    specialCode = [iTermStandardKeyMapper
  ...
  if (effectiveKeyCode == kVK_Delete || effectiveKeyCode == kVK_Escape) {
    if (escPlus) {
  ...
  if (effectiveKeyCode == kVK_Return || effectiveKeyCode == kVK_Tab) {
    return YES;
  }
