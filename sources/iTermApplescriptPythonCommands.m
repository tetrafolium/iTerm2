...
- (id)performDefaultImplementation {
  ...
  if ([scriptName hasPrefix:@"/"]) {
    [[[[iTermApplication sharedApplication] delegate] scriptsMenuController]
        launchScriptWithAbsolutePath:scriptName
                           arguments:args
                  explicitUserAction:NO];
    return nil;
  }
  NSArray<NSString *> *relativeFilenames =
      [[[[iTermApplication sharedApplication] delegate] scriptsMenuController]
          allScripts];
  ...
  for (NSString *relativeFilename in relativeFilenames) {
    if ([relativeFilename.stringByDeletingPathExtension
            isEqualToString:scriptName]) {
      [self launchPythonScript:relativeFilename arguments:args];
      return nil;
    }
  }
  ...
  for (NSString *relativeFilename in relativeFilenames) {
    if ([relativeFilename.lastPathComponent.stringByDeletingPathExtension
            isEqualToString:scriptName]) {
      [self launchPythonScript:relativeFilename arguments:args];
      return nil;
    }
...
- (void)launchPythonScript:(NSString *)script
  ...
  [[[[iTermApplication sharedApplication] delegate] scriptsMenuController]
      launchScriptWithRelativePath:script
                         arguments:arguments
                explicitUserAction:NO];
}
...
- (id)performDefaultImplementation {
  ...
  iTermParsedExpression *parsedExpression =
      [[iTermExpressionParser expressionParser]
          parse:invocation
  ...
  [evaluator
      evaluateWithTimeout:sync ? 0 : 10
               completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                 if (evaluator.error) {
                   [self setScriptErrorNumber:2];
                   [self setScriptErrorString:evaluator.error
                 ...
                 dispatch_async(dispatch_get_main_queue(), ^{
                   [weakSelf
                       resumeExecutionWithResult:
                           [NSString stringWithFormat:@"%@", evaluator.value]];
                 });
               }];
  return nil;
}
