...
- (void)it_drawWithBlock:(void (^)(void))block {
  ...
  [NSAppearance
      it_performBlockWithCurrentAppearanceSetToAppearanceForCurrentTheme:^{
        block();
      }];
...
+ (NSData *)dataWithFourBytesPerPixelFromDataWithOneBytePerPixel:
    (NSData *)input {
...
+ (instancetype)imageWithRawData:(NSData *)data
  ...
  if (samplesPerPixel == 1) {
    return [self
        imageWithRawData:
            [self dataWithFourBytesPerPixelFromDataWithOneBytePerPixel:data]
                    size:size
           bitsPerSample:8
         samplesPerPixel:4
                hasAlpha:YES
  ...
  assert(data.length ==
         size.width * size.height * bitsPerSample * samplesPerPixel / 8);
               ...
               samplesPerPixel:samplesPerPixel
                ...
                colorSpaceName:colorSpaceName
                  ...
                  bitsPerPixel:bitsPerSample *
                               samplesPerPixel]; // 0 means OS infers it
...
+ (NSImage *)it_hamburgerForClass:(Class)theClass {
  if (@available(macOS 10.16, *)) {
    return [self it_imageForSymbolName:@"ellipsis.circle"
...
- (NSImage *)blurredImageWithRadius:(int)radius {
  // Initially, this used a CIFilter but this doesn't work on some machines for
  // mysterious reasons. Instead, this algorithm implements a really simple box
  // blur. It's quite fast--about 5ms on a macbook pro with radius 5 for a 48x48
  // image.
...
+ (CGContextRef)newBitmapContextOfSize:(NSSize)size
  ...
  CGContextRef context = CGBitmapContextCreate(
      (void *)data.bytes, size.width, size.height, 8, bytesPerRow, colorSpace,
...
- (void)saveAsPNGTo:(NSString *)filename {
  [[self dataForFileOfType:NSBitmapImageFileTypePNG] writeToFile:filename
                                                      atomically:NO];
}
...
+ (NSImage *)it_imageWithScaledBitmapFromFile:(NSString *)file
  ...
  const NSSize retinaPixelSize =
      NSMakeSize(pointSize.width * 2, pointSize.height * 2);
...
- (NSImage *)grayscaleImage {
  ...
  CGContextRef context = CGBitmapContextCreate(
      nil, size.width, size.height, 8, 0, colorSpace, kCGImageAlphaNone);
