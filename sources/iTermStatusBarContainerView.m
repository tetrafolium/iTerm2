...
- (nullable instancetype)initWithComponent:
  ...
  self = [super initWithFrame:NSMakeRect(0, 0, preferredWidth,
  ...
  if (self) {
    ...
    _view.frame = NSMakeRect(self.minX, 0, self.preferredWidthForComponentView,
                             self.frame.size.height);
    _timer = [NSTimer
        scheduledWeakTimerWithTimeInterval:_component
                                               .statusBarComponentUpdateCadence
                                    target:self
    ...
    if ([component statusBarComponentHandlesClicks]) {
      NSClickGestureRecognizer *recognizer = [[NSClickGestureRecognizer alloc]
          initWithTarget:self
                  action:@selector(clickRecognized:)];
...
- (void)updateIconIfNeeded {
  ...
  if (hasIcon) {
    ...
    NSColor *tintColor =
        [self.component statusBarTextColor]
            ?: [self.component.delegate statusBarComponentDefaultTextColor];
    [_iconImageView it_setTintColor:tintColor];
    [_iconImageView sizeToFit];
    [self addSubview:_iconImageView];
    NSRect area = NSMakeRect(0, 0, iTermStatusBarViewControllerIconWidth,
                             iTermGetStatusBarHeight());
    NSRect frame;
...
- (CGFloat)minimumWidthIncludingIcon {
  ...
  const CGFloat minExIcon =
      knobValue ? MAX(minPreferred, knobValue.doubleValue) : minPreferred;
  if (self.component.statusBarComponentIcon) {
    return minExIcon + iTermStatusBarViewControllerIconWidth +
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p frame=%@ component=%@>", self.class, self,
                       NSStringFromRect(self.frame), self.component];
}
...
- (void)layoutSubviews {
  ...
  DLog(@"set frame of view %@ for component %@ width to %@", _view,
       self.component, @(viewWidth));
  _view.frame =
      NSMakeRect([self retinaRound:self.minX],
                 [self retinaRound:(myHeight - viewHeight) / 2 +
                                   _component.statusBarComponentVerticalOffset],
  ...
  _unreadCountView.frame = NSMakeRect(
      NSMaxX(self.bounds) - NSWidth(_unreadCountView.frame) - margin,
      [self retinaRound:NSMidY(self.bounds) -
                        NSHeight(_unreadCountView.frame) / 2.0],
...
- (void)mouseUp:(NSEvent *)event {
  if (event.clickCount != 1 ||
      !(event.it_modifierFlags & NSEventModifierFlagControl)) {
    [super mouseUp:event];
    return;
  }
...
- (void)showContextMenuForEvent:(NSEvent *)event {
  ...
  NSDictionary<NSString *, id> *values =
      [self.component statusBarComponentKnobValues];
  __block BOOL haveAddedSeparator = NO;
  [[self.component statusBarComponentKnobs]
      enumerateObjectsUsingBlock:^(iTermStatusBarComponentKnob *_Nonnull knob,
        ...
        if (knob.type == iTermStatusBarComponentKnobTypeCheckbox) {
          ...
          NSMenuItem *item =
              [[NSMenuItem alloc] initWithTitle:knob.labelText
                                         action:@selector(toggleKnob:)
                                  keyEquivalent:@""];
          item.identifier = knob.key;
          item.state = [[NSNumber castFrom:values[knob.key]] boolValue]
...
- (void)toggleKnob:(NSMenuItem *)sender {
  ...
  NSMutableDictionary *knobValues =
      [[self.component statusBarComponentKnobValues] mutableCopy]
