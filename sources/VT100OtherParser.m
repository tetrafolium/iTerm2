    ...
    if (datalen > 0) {
      ...
      if (found) {
        if (result.string.length == 0) {
          ...
          // specifies the default shell name for the window. If the name ends
          // in a ‘:’ screen will add what it believes to be the current command
          // running in the window to the end of the specified name (e.g.
          // name:cmd). Otherwise the current command name supersedes the shell
          // name while it is running.
          //
          // Here's how it works: you must modify your shell prompt to output a
          // null title-escape-sequence (<ESC> k <ESC> \) as a part of your
          // prompt. The last part of your prompt must be the same as the string
          // you specified for the search portion of the title. Once this is set
          ...
          // with ‘!’, ‘%’, or ‘^’, screen will use the first word on the
          // following line (if found) in preference to the just-found name.
          // This helps csh users get more accurate titles when using job
