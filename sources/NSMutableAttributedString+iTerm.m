...
- (void)iterm_appendString:(NSString *)string {
  ...
  if (self.length > 0) {
    ...
    NSLog(@"WARNING: iterm_appendString: to empty object %@ will have no "
          @"attributes",
          self);
  }
...
- (void)trimTrailingWhitespace {
  ...
  NSRange rangeOfLastWantedCharacter =
      [self.string rangeOfCharacterFromSet:nonWhitespaceSet
...
- (void)replaceAttributesInRange:(NSRange)range
                  withAttributes:(NSDictionary *)newAttributes {
...
+ (instancetype)attributedStringWithLinkToURL:(NSString *)urlString
...
+ (instancetype)attributedStringWithAttributedStrings:
...
- (NSArray *)attributedComponentsSeparatedByString:(NSString *)separator {
  ...
  [indices enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
    [result addObject:[self attributedSubstringFromRange:NSMakeRange(
                                                             startAt,
                                                             idx - startAt)]];
    startAt = idx + separator.length;
  }];
  [result
      addObject:[self attributedSubstringFromRange:NSMakeRange(startAt,
                                                               string.length -
                                                                   startAt)]];
                      ...
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id>
                        ...
                        if (attrs[NSAttachmentAttributeName]) {
                          NSTextAttachment *attachment =
                              attrs[NSAttachmentAttributeName];
                          NSTextAttachment *replacement =
