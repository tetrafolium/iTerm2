...
@interface iTermMiniSearchField
    : iTermFocusReportingSearchField <iTermHotboxSuppressing>
@end
...
- (void)sizeToFitSize:(NSSize)size {
  NSSize searchFieldSize = [_searchField
      sizeThatFits:NSMakeSize(size.width, self.view.frame.size.height)];
...
- (void)updateSubviews {
  ...
  if (self.canClose) {
    ...
    _closeButton.frame =
        NSMakeRect(size.width - _closeButton.frame.size.width, verticalOffset,
                   _closeButton.frame.size.width, searchFieldSize.height);
    closeWidth = _closeButton.frame.size.width;
  } else {
    _closeButton.hidden = YES;
  }
  _arrowsControl.frame = NSMakeRect(
      size.width - _arrowsControl.frame.size.width - closeWidth, verticalOffset,
      _arrowsControl.frame.size.width, _arrowsControl.frame.size.height);
  const CGFloat margin = 3;
  const CGFloat leftMargin = 2;
  const CGFloat used =
      leftMargin + _arrowsControl.frame.size.width + closeWidth + margin;
  _searchField.frame = NSMakeRect(
      leftMargin, 0, self.view.frame.size.width - used, searchFieldSize.height);
}
...
- (BOOL)validateUserInterfaceItem:(NSMenuItem *)item {
  item.state = (item.tag == self.driver.mode) ? NSControlStateValueOn
...
- (NSArray *)control:(NSControl *)control
  ...
  *index = -1;
  return [self.driver completionsForText:[textView string] range:charRange];
}
...
- (void)controlTextDidEndEditing:(NSNotification *)aNotification {
  ...
  int move =
      [[[aNotification userInfo] objectForKey:@"NSTextMovement"] intValue];
  switch (move) {
  ...
  case NSReturnTextMovement: {
    ...
    const BOOL shiftPressed =
        !!([[NSApp currentEvent] it_modifierFlags] & NSEventModifierFlagShift);
