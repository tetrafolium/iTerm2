...
- (void)tmuxNewWindowCommandInSessionNumber:(nullable NSNumber *)sessionNumber
  ...
  if (sessionNumber) {
    NSString *targetSessionArg =
        [NSString stringWithFormat:@"\"$%d:+\"", sessionNumber.intValue];
    NSArray *insertionArguments = @[ @"-a", @"-t", targetSessionArg ];
...
- (void)tmuxSplitWindowCommand:(int)wp
            ...
            recyclingSupported:(BOOL)recyclingSupported
                    ...
                    completion:(void (^)(NSString *))completion {
  NSArray *args = @[
    @"split-window", splitVertically ? @"-h" : @"-v", @"-t",
...
- (void)
    tmuxCustomDirectoryParameterRecyclingSupported:(BOOL)recyclingSupported
                                             scope:(iTermVariableScope *)scope
                                        completion:
                                            (void (^)(NSString *))completion {
  switch (self.mode) {
...
- (void)tmuxCommandByAddingCustomDirectoryWithArgs:(NSArray *)defaultArgs
                                recyclingSupported:(BOOL)recyclingSupported
                                        ...
                                        completion:
                                            (void (^)(NSString *))completion {
  [self
      tmuxCustomDirectoryParameterRecyclingSupported:recyclingSupported
                                               scope:scope
                                          completion:^(NSString *result) {
                                            NSArray *args = defaultArgs;
                                            NSString *customDirectory = result;
                                            if (customDirectory) {
                                              NSString *escapedCustomDirectory = [customDirectory
                                                  stringByReplacingOccurrencesOfString:
                                                      @"'"
                                                                            withString:
                                                                                @"\\'"];
                                              NSString *customDirectoryArgument =
                                                  [NSString
                                                      stringWithFormat:
                                                          @"-c '%@'",
                                                          escapedCustomDirectory];
                                              args = [args
                                                  arrayByAddingObject:
                                                      customDirectoryArgument];
                                            }
                                            completion([args
