...
- (CGRect)retinaRoundRect:(CGRect)rect {
  NSRect result = NSMakeRect(
      [self retinaRound:NSMinX(rect)], [self retinaRound:NSMinY(rect)],
...
- (BOOL)containsDescendant:(NSView *)possibleDescendant {
  for (NSView *subview in self.subviews) {
    if (subview == possibleDescendant ||
        [subview containsDescendant:possibleDescendant]) {
      return YES;
    }
  }
  return NO;
