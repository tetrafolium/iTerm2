...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    NSSize tableViewSize = [NSScrollView
    ...
    NSRect tableViewFrame =
        NSMakeRect(0, 0, tableViewSize.width, tableViewSize.height);
...
- (NSString *)stringForIndentLevel:(int)level {
  ...
  NSString *space =
      [NSString stringWithCharacters:chars
                              length:sizeof(chars) / sizeof(*chars)];
...
- (NSArray *)sortedIndentedTags {
  if (!_cache) {
    ...
    NSArray *tags = [[[ProfileModel sharedInstance] allTags]
    ...
    for (int i = 0; i < tags.count; i++) {
      ...
      int numPartsMatched = [self numberOfPartsMatchedBetween:previousParts
                                                          and:currentParts];
      while (numPartsMatched < currentParts.count) {
        ...
        NSString *value = [[currentParts
            subarrayWithRange:NSMakeRange(0, numPartsMatched + 1)]
