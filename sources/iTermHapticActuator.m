...
//
// Derived from
// https://raw.githubusercontent.com/niw/HapticKey/master/HapticKey/Classes/HTKMultitouchActuator.m
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _MTActuatorCreateFromDeviceID =
        iTermGetMTActuatorCreateFromDeviceIDFunction();
    ...
    if (!_MTActuatorCreateFromDeviceID || !_MTActuatorOpen ||
        !_MTActuatorClose || !_MTActuatorActuate || !_MTActuatorIsOpen) {
      return nil;
    }
  }
  return self;
...
- (BOOL)actuateActuationID:(SInt32)actuationID
  ...
  BOOL result = [self _htk_main_actuateActuationID:actuationID
                                          unknown1:unknown1
                                          unknown2:unknown2
                                          unknown3:unknown3];

  // In case we failed to actuate with existing actuator, reopen it and try
  // again.
  if (!result) {
    [self _htk_main_closeActuator];
    [self _htk_main_openActuator];
    result = [self _htk_main_actuateActuationID:actuationID
                                       unknown1:unknown1
                                       unknown2:unknown2
                                       unknown3:unknown3];
  }
...
- (void)_htk_main_openActuator {
  ...
  if (self.lastKnownMultitouchDeviceMultitouchID) {
    const CFTypeRef actuatorRef = _MTActuatorCreateFromDeviceID(
        self.lastKnownMultitouchDeviceMultitouchID);
    if (!actuatorRef) {
      DLog(@"Fail to MTActuatorCreateFromDeviceID: 0x%llx",
           self.lastKnownMultitouchDeviceMultitouchID);
      return;
    }
    ...
    const size_t count =
        sizeof(kKnownAppleMultitouchDeviceMultitouchIDs) / sizeof(UInt64);
    for (size_t index = 0; index < count; index++) {
      const UInt64 multitouchDeviceMultitouchID =
          kKnownAppleMultitouchDeviceMultitouchIDs[index];
      const CFTypeRef actuatorRef =
          _MTActuatorCreateFromDeviceID(multitouchDeviceMultitouchID);
      if (actuatorRef) {
        DLog(@"Use MTActuatorCreateFromDeviceID: 0x%llx",
             multitouchDeviceMultitouchID);
        _actuatorRef = actuatorRef;
        self.lastKnownMultitouchDeviceMultitouchID =
            multitouchDeviceMultitouchID;
        break;
      }
      DLog(@"Fail to test MTActuatorCreateFromDeviceID: 0x%llx",
           multitouchDeviceMultitouchID);
    }
    if (!_actuatorRef) {
...
- (BOOL)_htk_main_actuateActuationID:(SInt32)actuationID
                            unknown1:(UInt32)unknown1
                            unknown2:(Float32)unknown2
                            unknown3:(Float32)unknown3 {
  if (!_actuatorRef) {
  ...
  const IOReturn error = _MTActuatorActuate(_actuatorRef, actuationID, unknown1,
                                            unknown2, unknown3);
  if (error != kIOReturnSuccess) {
    DLog(@"Fail to MTActuatorActuate: %p, %d, %d, %f, %f error: 0x%x",
         _actuatorRef, actuationID, unknown1, unknown2, unknown3, error);
    return NO;
  } else {
    return YES;
  }
...
- (SInt32)actuationIDForType:(iTermHapticFeedbackType)type {
  ...
  // $ otool -s __TEXT __tpad_act_plist
  // /System/Library/PrivateFrameworks/MultitouchSupport.framework/Versions/Current/MultitouchSupport|tail
  // -n +3|awk -F'\t' '{print $2}'|xxd -r -p This show a embedded property list
  // file in `MultitouchSupport.framework`. There are default 1, 2, 3, 4, 5, 6,
  // 15, and 16 actuation IDs now.
