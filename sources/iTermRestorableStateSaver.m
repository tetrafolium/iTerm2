...
- (BOOL)saveSynchronously:(BOOL)synchronously
  ...
  if ([iTermRestorableStateController stateRestorationEnabled]) {
    ...
    for (NSWindow *window in [self.delegate restorableStateWindows]) {
      iTermRestorableStateRecord *record =
          [_records objectPassingTest:^BOOL(iTermRestorableStateRecord *element,
                                            NSUInteger index, BOOL *stop) {
            return element.windowNumber == window.windowNumber;
          }];
      const BOOL needsRestoration =
          [self.delegate restorableStateWindowNeedsRestoration:window];
      ...
      if (record) {
        ...
        record = [[iTermRestorableStateRecord alloc]
            initWithWindowNumber:window.windowNumber
  ...
  _records = [recordsNeedingNewContent
      arrayByAddingObjectsFromArray:recordsToKeepUnchanged];
...
// queue
- (void)writeUnchanged:
            (NSArray<iTermRestorableStateRecord *> *)recordsToKeepUnchanged
                update:(NSArray<iTermRestorableStateRecord *> *)
                           recordsNeedingNewContent
            completion:(void (^)(void))completion {
  NSArray<NSURL *> *fileURLsToKeep = [[recordsToKeepUnchanged
      arrayByAddingObjectsFromArray:recordsNeedingNewContent]
...
- (void)eraseFilesExcept:(NSArray<NSURL *> *)fileURLsToKeep {
  ...
  NSDirectoryEnumerator *enumerator = [[NSFileManager defaultManager]
                 enumeratorAtURL:[NSURL fileURLWithPath:savedState]
...
// Runs on _queue
- (void)encryptAndSaveUnchangedRecords:
            (NSArray<iTermRestorableStateRecord *> *)recordsToKeepUnchanged
                        updatedRecords:(NSArray<iTermRestorableStateRecord *> *)
                                           recordsNeedingNewContent {
  NSMutableArray *plist = [NSMutableArray new];
  [recordsNeedingNewContent
      enumerateObjectsUsingBlock:^(iTermRestorableStateRecord *_Nonnull record,
  ...
  DLog(@"Save index:\n%@", plist);
  // This is broken because it won't work with things like NSValue. But it
  // doesn't matter because this code is all but dead.
