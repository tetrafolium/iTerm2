...
static NSString *const iTermOnboardingWindowControllerHasBeenShown =
...
+ (BOOL)hasBeenShown {
  return [[NSUserDefaults standardUserDefaults]
      boolForKey:iTermOnboardingWindowControllerHasBeenShown];
}

+ (void)suppressFutureShowings {
  [[NSUserDefaults standardUserDefaults]
      setBool:YES
       forKey:iTermOnboardingWindowControllerHasBeenShown];
}
...
+ (BOOL)previousLaunchVersionImpliesShouldBeShown {
  ...
  NSString *twoPartVersion = [[parts subarrayWithRange:NSMakeRange(0, 2)]
...
- (void)awakeFromNib {
  ...
  // This is the place to tweak the contents of text fields for conditionally
  // available features or to add hyperlinks. None needed in 3.4, but maybe the
  // next version will want them. Also look at the commit history to see how it
  // was done for 3.3.

  _views = @[
    [self wrap:_view1], [self wrap:_view2], [self wrap:_view3],
    [self wrap:_view4]
  ];
  _pageIndicators =
      @[ _pageIndicator1, _pageIndicator2, _pageIndicator3, _pageIndicator4 ];
...
- (NSView *)wrap:(NSView *)view {
  ...
  NSView *wrapper = [[iTermOnboardingView alloc]
      initWithFrame:NSMakeRect(0, 0, view.frame.size.width + margin * 2,
                               view.frame.size.height)];
  [wrapper addSubview:view];
  view.frame =
      NSMakeRect(margin, 0, view.frame.size.width, view.frame.size.height);
  return wrapper;
}
...
- (void)updateButtons {
  ...
  for (NSInteger i = 0; i < _pageIndicators.count; i++) {
    _pageIndicators[i].animator.alphaValue =
        i == _pageController.selectedIndex ? maxAlpha : 0.25;
  }
}
