...
NSString *const iTermStatusBarSwiftyStringComponentExpressionKey =
    @"expression";
...
- (NSString *)statusBarComponentDetailedDescription {
  return @"Shows the evaluation of a string with inline expressions which may "
         @"include session "
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  iTermStatusBarComponentKnob *expressionKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"String Value:"
...
+ (NSDictionary *)statusBarComponentDefaultKnobs {
  ...
  return [fromSuper dictionaryByMergingDictionary:@{
    iTermStatusBarSwiftyStringComponentExpressionKey : @""
  }];
}
...
- (void)onClick:(id)sender {
  if (_errorReason) {
    [iTermWarning
        showWarningWithTitle:_errorReason
                     actions:@[ @"OK" ]
                   accessory:nil
...
- (void)updateWithKnobValues:(NSDictionary<NSString *, id> *)knobValues {
  NSString *expression =
      knobValues[iTermStatusBarSwiftyStringComponentExpressionKey] ?: @"";
  __weak __typeof(self) weakSelf = self;
  if ([self.delegate statusBarComponentIsInSetupUI:self]) {
    _swiftyString =
        [[iTermSwiftyStringPlaceholder alloc] initWithString:expression];
    ...
    _swiftyString = [[iTermSwiftyString alloc]
        initWithString:expression
              ...
              observer:^NSString *(NSString *_Nonnull newValue,
                                   NSError *error) {
                return [weakSelf didEvaluateExpression:expression
                                            withResult:newValue
                                                 error:error];
              }];
...
- (NSString *)didEvaluateExpression:(NSString *)expression
  ...
  if (error != nil) {
    NSString *message =
        [NSString stringWithFormat:@"Error while evaluating “%@”:\n%@",
