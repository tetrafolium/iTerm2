...
//
//
// DCS [private] [parameters] [intermediate] initial-passthrough
// [remainder-of-passthrough] private: One character from: < = > ? parameters:
// Semicolon-delimited optional numbers (e.g., 1;2;3 or 1;;3) intermediate: One
// or more character from: ! " # $ % & ' ( ) * + , - . / initial-passthrough:
// One character between @ and ~ inclusive remainder-of-passthrough: One or more
// characters between SP and ~ inclusive.
//
// We implement, approximately, the DCS-related parts of the state machine
// described here: http://www.vt100.net/emu/dec_ansi_parser The main departure
// is that tmux mode is terminated by %exit ST, and ST (or other escape
// sequences) may occur prior to its termination and should be interpreted
// literally.
...
// Caps the amount of data to accumulate in _data before returning to the ground
// state. Prevents a random ESC P from eating output forever by leaving us in
// the passthrough state until we get an ST. Note that there's an exception for
// file downloads wrapped in DCS tmux; … ST
...
// Key in the state machine's userInfo dictionary that gives reference to the
// current VT100Token being populated.
static NSString *const kVT100DCSUserInfoToken = @"kVT100DCSUserInfoToken";

static NSRange MakeCharacterRange(unsigned char first,
                                  unsigned char lastInclusive) {
  return NSMakeRange(first, lastInclusive - first + 1);
}

@implementation VT100DCSParser {
  BOOL _malformed; // The current parse has failed but we're waiting for a
                   // terminator.

  // The current token has already been executed. If -execute gets called again,
  // don't do anything. This can happen if you get ESC + letter (for example)
  // after the initial-passthrough character.
  BOOL _executed;

  // The hook has terminated. The |_hook| member will remain non-nil because
  // that causes the client to continue to pass all input to us. We expect to
  // get an ST, and will then nil out
  // |_hook|. This is here only for the tmux workaround (and the tmux parser is
  // the only hook).
  BOOL _hookFinished;

  // Holds the current state and the fine state machine. Up to one of these will
  // be nonnull.
  VT100StateMachine *_stateMachineWithout8BitControlCharacters;
  VT100StateMachine *_stateMachineWith8BitControlCharacters;

  // Concatenation of passthrough characters. Not added to when a hook is
  // present.
  ...
  // If set, and _hookFinished is false, then all input is passed to the hook
  // instead of the state machine. This is a departure from how it's really
  // "supposed" to work but our only hook is for the buggy tmux protocol, which
  // is not terminated by ST alone (only %exit followed by ST).
  id<VT100DCSParserHook> _hook;

  BOOL _support8BitControlCharacters;

  // We periodically check if _data looks like binary garbage. To avoid
  // quadratic runtime we remember the number of UTF-16 codepoints that have
...
+ (NSDictionary *)termcapTerminfoNameDictionary {
  return @{
    @"TN" : @(kDcsTermcapTerminfoRequestTerminalName),
...
- (VT100StateMachine *)stateMachine {
  if (_support8BitControlCharacters) {
    if (!_stateMachineWith8BitControlCharacters) {
      _stateMachineWith8BitControlCharacters =
          [self newStateMachineWith8BitControlCharacters:YES];
  ...
  if (!_stateMachineWithout8BitControlCharacters) {
    _stateMachineWithout8BitControlCharacters =
        [self newStateMachineWith8BitControlCharacters:YES];
...
- (VT100StateMachine *)newStateMachineWith8BitControlCharacters:
  ...
  VT100State *dcsIntermediateState =
      [VT100State stateWithName:@"dcs intermediate"
  ...
  VT100State *dcsIgnoreState =
      [VT100State stateWithName:@"dcs ignore"
                     identifier:@(kVT100DCSStateIgnore)];

  // An ESC was received which might lead to termination or something else.
  VT100State *dcsEscapeState =
      [VT100State stateWithName:@"dcs escape"
                     identifier:@(kVT100DCSStateDCSEscape)];

  // Either accumulating characters in |_data| or sending them to the hook.
  // Because of the tmux hook hackery (see comments for |_hookFinished|) this
  // actually only accumulates passthrough data. Some day if a well-behaved hook
  // is added, this might change to redirect data to the hook.
  VT100State *dcsPassthroughState =
      [VT100State stateWithName:@"dcs passthrough"
  ...
  for (VT100State *state in stateMachine.states) {
    ...
    if (support8BitControlCharacters) {
      [state
          addStateTransitionForCharacterRange:MakeCharacterRange(VT100CC_C1_IND,
                                                                 VT100CC_C1_APC)
                                           to:groundState
                                   withAction:^(unsigned char c) {
                                     [weakSelf execute];
  ...
  [dcsEntryState
      addStateTransitionForCharacterRange:MakeCharacterRange('@', '~')
  ...
  [dcsEntryState
      addStateTransitionForCharacterRange:MakeCharacterRange(' ', '/')
                               ...
                               withAction:^(unsigned char c) {
                                 __strong __typeof(self) strongSelf = weakSelf;
                                 if (strongSelf) {
                                   [strongSelf->_intermediateString
                                       appendCharacter:c];
                                 }
                               }];
  // Transition to parameter string.
  [dcsEntryState
      addStateTransitionForCharacterRange:MakeCharacterRange('0', '9')
                                       to:dcsParamState
                               withAction:^(unsigned char c) {
                                 __strong __typeof(self) strongSelf = weakSelf;
                                 if (strongSelf) {
                                   [strongSelf->_parameterString
                                       appendCharacter:c];
                                 }
                               }];
  ...
  // Illegal characters; swallow them up til we get a terminator in the ignore
  // state.
  [dcsIntermediateState
      addStateTransitionForCharacterRange:MakeCharacterRange('0', '?')
                                       to:dcsIgnoreState
                               withAction:nil];
  ...
  [dcsParamState
      addStateTransitionForCharacter:';'
                                  to:dcsParamState
                          withAction:^(unsigned char c) {
                            __strong __typeof(self) strongSelf = weakSelf;
                            if (strongSelf) {
  ...
  // Escape + anything other than backslash returns to passthrough state. This
  // is necessary to handle doubled-up escapes in DCS tmux;
  // nested-escape-sequence ST, where nested-escape-sequence is a control
  // sequence to pass through to the terminal with every ESC replaced with ESC
  // ESC. Passthrough's entry action will append it to _data.
  ...
  // On entry to passthrough, save the initial character. Note that there could
  // be ESC characters that cause us to re-enter passthrough after having
  // already been in it so don't do anything destructive here.
  ...
  [dcsPassthroughState
      addStateTransitionForCharacterRange:MakeCharacterRange(VT100CC_NULL,
                                                             VT100CC_ETB)
                                       to:dcsPassthroughState
                               withAction:^(unsigned char c) {
                                 [weakSelf put:c];
  ...
  [dcsPassthroughState
      addStateTransitionForCharacterRange:MakeCharacterRange(VT100CC_FS,
                                                             VT100CC_US)
                                       to:dcsPassthroughState
                               withAction:^(unsigned char c) {
                                 [weakSelf put:c];
...
// Save a passthrough character. If the tmux hack weren't here, it would direct
// to the hook if one were present.
...
- (BOOL)dataLooksLikeBinaryGarbage {
  ...
  dispatch_once(&onceToken, ^{
    // This causes us to stop parsing DCS at a newline, which I hope is mostly
    // harmless. But if it's not, add 10 and 13 to this character set.
    NSMutableCharacterSet *characterSet =
        [NSMutableCharacterSet characterSetWithRange:NSMakeRange(32, 127 - 32)];
    [characterSet addCharactersInRange:NSMakeRange(VT100CC_ESC, 1)];
    // Keep going at BEL because the tmux OSC wrapper DCS tmux; OSC with
    // double-escapes ST could end in a BEL.
...
- (void)decodeFromContext:(iTermParserContext *)context
  ...
  _support8BitControlCharacters = (encoding == NSASCIIStringEncoding ||
  ...
  while (result->type == VT100_WAIT && iTermParserCanAdvance(context)) {
    if (_hook && !_hookFinished) {
      ...
      if ([self.stateMachine.currentState.identifier
              isEqual:@(kVT100DCSStatePassthrough)] &&
...
// Only add to this if you're doing something fancy. You can usually add your
// case to -execute instead.
- (void)hook {
...
- (void)unhook {
  ...
  _uniqueID = nil;
  [_parameterString
      deleteCharactersInRange:NSMakeRange(0, _parameterString.length)];
