...
void SetRunningServer(void) { gRunningServer = 1; }

void iTermFileDescriptorServerLog(char *format, ...) {
  va_list args;
  va_start(args, format);
  char temp[1000];
  snprintf(temp, sizeof(temp) - 1, "%s(%d) %s",
           gRunningServer ? "Server" : "ParentServer", getpid(), format);
...
static ssize_t iTermFileDescriptorWriteImpl(int fd, void *buffer,
                                            size_t bufferSize, int *errorOut) {
  const ssize_t rc = iTermFileDescriptorServerWrite(fd, buffer, bufferSize);
  if (rc == -1) {
    if (*errorOut) {
      *errorOut = errno;
    }
    FDLog(LOG_DEBUG, "iTermFileDescriptorServerWrite failed with %s",
          strerror(errno));
  } else {
    if (errorOut) {
      *errorOut = 0;
...
ssize_t iTermFileDescriptorServerWriteLengthAndBuffer(int fd, void *buffer,
                                                      size_t bufferSize,
                                                      int *errorOut) {
  unsigned char temp[sizeof(bufferSize)];
  memmove(temp, &bufferSize, sizeof(bufferSize));
  const ssize_t rc =
      iTermFileDescriptorWriteImpl(fd, temp, sizeof(temp), errorOut);
  if (rc != sizeof(temp)) {
    return rc;
  }
...
ssize_t iTermFileDescriptorServerWriteLengthAndBufferAndFileDescriptor(
  ...
  memmove(temp, &bufferSize, sizeof(bufferSize));
  ssize_t rc =
      iTermFileDescriptorWriteImpl(connectionFd, temp, sizeof(temp), errorOut);
  if (rc != sizeof(temp)) {
    return rc;
  }
  ...
  rc = iTermFileDescriptorServerSendMessageAndFileDescriptor(
      connectionFd, buffer, bufferSize, fdToSend);
  if (rc == -1) {
    if (errorOut) {
      *errorOut = errno;
    }
...
ssize_t iTermFileDescriptorServerWrite(int fd, void *buffer,
                                       size_t bufferSize) {
  assert(bufferSize > 0);
...
ssize_t iTermFileDescriptorClientWrite(int fd, const void *buffer,
  ...
  while (totalWritten < bufferSize) {
    ...
    do {
      ...
      savedErrno = errno;
      FDLog(LOG_DEBUG, "write of %d bytes returned %d, errno=%d",
            (int)bufferSize, (int)rc, (int)savedErrno);
    } while (rc == -1 && savedErrno == EINTR);
    if (rc <= 0) {
      if (savedErrno == EAGAIN && totalWritten > 0) {
        FDLog(LOG_DEBUG, "write: EAGAIN with totalWritten=%d",
              (int)totalWritten);
        return totalWritten;
      }
      errno = savedErrno;
...
ssize_t iTermFileDescriptorServerSendMessageAndFileDescriptor(int connectionFd,
                                                              ...
                                                              size_t bufferSize,
                                                              int fdToSend) {
  // sendmsg wants to send the whole buffer atomically so it has a small upper
  // bound on message size. The client-server protocol allows a message to be
  // fragmented as long as the first one has the file descriptor.
  //
  // Despite what the man page says, I have seen sendmsg fail with EMSGSIZE with
  // a buffer size of IOV_MAX. My theory is that the control block can count
  // against that limit in some circumstances.
  //
  // Furthermore, if you try to send an empty message, that will also fail with
  // EMSGSIZE. So we set the limit to one byte.
  const int maxBufferSize = 1;

  if (bufferSize > maxBufferSize) {
    const ssize_t firstResult =
        iTermFileDescriptorServerSendMessageAndFileDescriptor(
            connectionFd, buffer, maxBufferSize, fdToSend);
    if (firstResult <= 0) {
      return firstResult;
    }
    ...
    // Now write the remainder. Because this doesn't use sendmsg there's no size
    // limit problem.
    int error = 0;
    const ssize_t secondResult = iTermFileDescriptorWriteImpl(
        connectionFd, buffer + maxBufferSize, remainingSize, &error);
    if (secondResult <= 0) {
      return secondResult;
    }
    return bufferSize;
  ...
  *((int *)CMSG_DATA(messageHeader)) = fdToSend;
  ...
  FDLog(LOG_DEBUG,
        "Send message of length %d, iovlen=%d along with file descriptor %d",
  ...
  if (rc >= 0 && rc < bufferSize) {
    // I don't know if this is possible, but you don't want to send the file
    // descriptor more than once or it will create multiple file descriptors in
    // the recipient.
    char *temp = buffer;
    return iTermFileDescriptorServerWrite(connectionFd, temp + rc,
                                          bufferSize - rc);
  }
  return rc;
}
...
int iTermSelect(int *fds, int count, int *results, int wantErrors) {
  ...
  do {
    ...
    result =
        select(max + 1, &readset, NULL, wantErrors ? &errorset : NULL, NULL);
    theError = errno;
    FDLog(LOG_DEBUG, "select returned %d, error = %s", result,
          strerror(theError));
  ...
  for (int i = 0; i < count; i++) {
    results[i] = FD_ISSET(fds[i], &readset) ||
                 (wantErrors && FD_ISSET(fds[i], &errorset));
    if (results[i]) {
      n++;
    }
  }
...
int iTermSelectForWriting(int *fds, int count, int *results, int wantErrors) {
  ...
  do {
    ...
    FDLog(LOG_DEBUG, "Calling select...");
    result =
        select(max + 1, NULL, &writeset, wantErrors ? &errorset : NULL, NULL);
    theError = errno;
    FDLog(LOG_DEBUG, "select returned %d, error = %s", result,
          strerror(theError));
  } while (result == -1 && theError == EINTR);
  ...
  int n = 0;
  for (int i = 0; i < count; i++) {
    results[i] = FD_ISSET(fds[i], &writeset) ||
                 (wantErrors && FD_ISSET(fds[i], &errorset));
    if (results[i]) {
      n++;
    }
...
int iTermFileDescriptorServerSocketBindListen(const char *path) {
  ...
  // Mask off all permissions for group and other. Only user can use this
  // socket.
  ...
  socklen_t len = SizeTToSockLenT(strlen(local.sun_path)) +
                  SizeTToSockLenT(sizeof(local.sun_family)) + 1;
