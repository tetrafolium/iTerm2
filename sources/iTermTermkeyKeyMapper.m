...
- (NSString *)preCocoaString {
  ...
  if (_event.keyCode == kVK_Space &&
      (_event.modifierFlags & allEventModifierFlags) ==
          NSEventModifierFlagShift) {
    // Shift+Space is special. No other unicode character + shift reports a CSI
    // u.
...
- (NSData *)postCocoaData {
  return [[self termkeySequenceForEvent]
      dataUsingEncoding:_configuration.encoding];
}
...
- (NSString *)csiUForCode:(NSString *)code
...
- (NSString *)cursorModeStringWithCode:(NSString *)code
                        eventModifiers:(NSEventModifierFlags)eventModifiers {
...
- (NSString *)sequenceForApplicationModeCodePoint:(int)unicode
  ...
  if (_configuration.applicationCursorMode) {
    NSString *result =
        [self sequenceForApplicationCursorCodePoint:unicode
                                            keyCode:keyCode
  ...
  if (_configuration.applicationKeypadMode) {
    NSString *result =
        [self sequenceForApplicationKeypadCodePoint:unicode
                                            keyCode:keyCode
                                     eventModifiers:eventModifiers];
    if (result) {
      return result;
...
- (NSString *)termkeySequenceForSpecialKey:(int)unicode
                            ...
                            eventModifiers:
                                (NSEventModifierFlags)eventModifiers {
  NSString *applicationModeResult =
      [self sequenceForApplicationModeCodePoint:unicode
                                        keyCode:keyCode
                                 eventModifiers:eventModifiers];
  ...
  // Really special keys
  case NSUpArrowFunctionKey:
    return [self reallySpecialSequenceWithCode:@"A"
                                eventModifiers:eventModifiers];
  case NSDownArrowFunctionKey:
    return [self reallySpecialSequenceWithCode:@"B"
                                eventModifiers:eventModifiers];
  case NSRightArrowFunctionKey:
    return [self reallySpecialSequenceWithCode:@"C"
                                eventModifiers:eventModifiers];
  case NSLeftArrowFunctionKey:
    return [self reallySpecialSequenceWithCode:@"D"
                                eventModifiers:eventModifiers];
  case NSHomeFunctionKey:
    return [self reallySpecialSequenceWithCode:@"H"
                                eventModifiers:eventModifiers];
  case NSEndFunctionKey:
    return [self reallySpecialSequenceWithCode:@"F"
                                eventModifiers:eventModifiers];
  case NSF1FunctionKey:
    return [self reallySpecialSequenceWithCode:@"P"
                                eventModifiers:eventModifiers];
  case NSF2FunctionKey:
    return [self reallySpecialSequenceWithCode:@"Q"
                                eventModifiers:eventModifiers];
  case NSF3FunctionKey:
    return [self reallySpecialSequenceWithCode:@"R"
                                eventModifiers:eventModifiers];
  case NSF4FunctionKey:
    return [self reallySpecialSequenceWithCode:@"S"
                                eventModifiers:eventModifiers];
  }
...
- (NSString *)termkeySequenceForModifiedC0Control:(int)keyCode
  ...
  if (keyCode == kVK_Space &&
      (eventModifiers & allEventModifierFlags) == NSEventModifierFlagControl) {
    // Control-space -> '\0'
    const unichar c = 0;
  ...
  case kVK_Tab:
    if (eventModifiers & NSEventModifierFlagShift) {
      // A really careful reading of the spec shows you ignore the shift
      // modifier here. The shiftness is communicated by being CSI code 'Z'
      // rather than 'u'.
      return [self
          csiZWithEventModifiers:(eventModifiers & ~NSEventModifierFlagShift)];
    } else {
...
- (NSData *)dataWhenOptionPressed {
  return [_event.charactersIgnoringModifiers
      dataUsingEncoding:_configuration.encoding];
}
...
- (NSData *)dataBySettingMetaFlagOnFirstByte:(NSData *)data {
  // I'm pretty sure this is a no-win situation when it comes to any encoding
  // other than ASCII, but see here for some ideas about this mess:
...
- (NSString *)termkeySequenceForCodePoint:(unichar)codePoint
  ...
  NSString *sequence =
      [self termkeySequenceForModifiedC0Control:keyCode
                                 eventModifiers:eventModifiers];
  if (sequence) {
    return sequence;
  }
  ...
  const NSEventModifierFlags allEventModifierFlags =
      (NSEventModifierFlagControl | NSEventModifierFlagOption |
       NSEventModifierFlagShift | maybeFunction);

  // Special and very special keys
  // Function keys, arrows, and keypad in application keypad mode.
  NSString *possibleResult = [self termkeySequenceForSpecialKey:codePoint
                                                        keyCode:keyCode
                                                 eventModifiers:eventModifiers];
  if (possibleResult) {
  ...
  if ((eventModifiers & allEventModifierFlags) == NSEventModifierFlagControl) {
    NSString *string =
        [self modifiedUnicodeStringForControlCharacter:codePoint];
    if (string) {
      return string;
    }
  }
  ...
  if ((eventModifiers & allEventModifierFlagsExShift) ==
      NSEventModifierFlagOption) {
    // Legacy code path: option-letter, for the "simplest form of these keys."
    // Not sure what he meant exactly, but anything that's not a function key
    // seems simple to me. ¯\_(ツ)_/¯ Update: based on his example of m-a -> esc
    // a and m-A -> esc A, I interpret this to mean the shift key should not be
    // considered.
...
- (NSString *)termkeySequenceForEvent {
  ...
  const unichar codePoint =
      [_event.charactersIgnoringModifiers characterAtIndex:0];
...
- (BOOL)keyMapperShouldBypassPreCocoaForEvent:(NSEvent *)event {
  ...
  const BOOL isSpecialKey = !!(modifiers & (NSEventModifierFlagNumericPad |
  ...
  const BOOL leftAltPressed =
      ...
      !rightAltPressed;
  const BOOL leftOptionModifiesKey =
      (leftAltPressed && _configuration.leftOptionKey != OPT_NORMAL);
  const BOOL rightOptionModifiesKey =
      (rightAltPressed && _configuration.rightOptionKey != OPT_NORMAL);
