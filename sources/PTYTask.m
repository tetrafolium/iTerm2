...
static void HandleSigChld(int n) {
  ...
  // as allowed in a signal handler. Calling a method is *NOT* safe since
  // something might be fiddling with the runtime. I saw a lot of crashes where
  // CoreData got interrupted by a sigchild while doing class_addMethod and that
  // caused a crash because of a method call.
  UnblockTaskNotifier();
}
...
@implementation PTYTask {
  ...
  NSString *command_;    // Command that was run if launchWithPath:arguments:etc
                         // was called
...
- (instancetype)init {
  ...
  if (self) {
    const char *label =
        [iTermThread uniqueQueueLabelWithName:@"com.iterm2.job-manager"]
            .UTF8String;
    _jobManagerQueue = dispatch_queue_create(label, DISPATCH_QUEUE_SERIAL);
    _lastSize =
        (PTYTaskSize){.cellSize = iTermTTYCellSizeMake(INFINITY, INFINITY),
    ...
    if ([iTermAdvancedSettingsModel runJobsInServers]) {
      if ([iTermMultiServerJobManager available]) {
        self.jobManager =
            [[iTermMultiServerJobManager alloc] initWithQueue:_jobManagerQueue];
      } else {
        self.jobManager =
            [[iTermMonoServerJobManager alloc] initWithQueue:_jobManagerQueue];
      }
    } else {
      self.jobManager =
          [[iTermLegacyJobManager alloc] initWithQueue:_jobManagerQueue];
    }
    self.fd = -1;
  }
  return self;
...
- (void)dealloc {
  ...
  if (_tmuxClientProcessID) {
    [[iTermProcessCache sharedInstance]
        unregisterTrackedPID:_tmuxClientProcessID.intValue];
  }
...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@: %p jobManager=%@ pid=%@ fd=%@ tmuxClientProcessID=%@>",
          NSStringFromClass([self class]), self, self.jobManager, @(self.pid),
          @(self.fd), _tmuxClientProcessID];
}
...
- (void)getWorkingDirectoryWithCompletion:(void (^)(NSString *pwd))completion {
  ...
  [iTermLSOF asyncWorkingDirectoryOfProcess:self.pid
...
- (BOOL)passwordInput {
  ...
  if ([iTermAdvancedSettingsModel detectPasswordInput] && self.fd > 0 &&
      isatty(self.fd) && tcgetattr(self.fd, &termAttributes) == 0) {
    return !(termAttributes.c_lflag & ECHO) &&
...
- (void)launchWithPath:(NSString *)progpath
  ...
  DLog(@"launchWithPath:%@ args:%@ env:%@ grisSize:%@ isUTF8:%@", progpath,
       args, env, VT100GridSizeDescription(gridSize), @(isUTF8));
  ...
  if ([iTermAdvancedSettingsModel runJobsInServers] &&
    ...
    NSArray *updatedArgs =
        [@[ @"--server", progpath ] arrayByAddingObjectsFromArray:args];
    if (![iTermAdvancedSettingsModel bootstrapDaemon]) {
      env = [env dictionaryBySettingObject:@"1"
                                    forKey:@"ITERM2_DISABLE_BOOTSTRAP"];
    }
...
- (void)setTmuxClientProcessID:(NSNumber *)tmuxClientProcessID {
  if ([NSObject object:tmuxClientProcessID
          isEqualToObject:_tmuxClientProcessID]) {
    return;
  }
  ...
  if (tmuxClientProcessID) {
    [[iTermProcessCache sharedInstance]
        registerTrackedPID:tmuxClientProcessID.intValue];
  }
  _tmuxClientProcessID = tmuxClientProcessID;
}

- (void)setReadOnlyFileDescriptor:(int)readOnlyFileDescriptor {
  iTermTmuxJobManager *jobManager =
      [[iTermTmuxJobManager alloc] initWithQueue:self->_jobManagerQueue];
...
- (void)fetchProcessInfoForCurrentJobWithCompletion:
  ...
  const pid_t pid =
      self.tmuxClientProcessID ? self.tmuxClientProcessID.intValue : self.pid;
  iTermProcessInfo *info =
      [[iTermProcessCache sharedInstance] deepestForegroundJobForPid:pid];
  ...
  [[iTermProcessCache sharedInstance]
      requestImmediateUpdateWithCompletionBlock:^{
        completion([[iTermProcessCache sharedInstance]
            deepestForegroundJobForPid:pid]);
      }];
}
...
// NOTE: maybeScaleFactor will be 0 if the session is not attached to a window.
// For example, if a different space is active.
- (void)setSize:(VT100GridSize)size
  ...
  DLog(@"Set terminal size to %@; %@; %f for %@",
       VT100GridSizeDescription(size), NSStringFromSize(viewSize), scaleFactor,
       self.delegate);
  if (self.fd == -1) {
    return;
  }
...
- (void)stopCoprocess {
  ...
  [[TaskNotifier sharedInstance]
      performSelectorOnMainThread:@selector(notifyCoprocessChange)
                       withObject:nil
                    waitUntilDone:NO];
}
...
- (void)setJobManagerType:(iTermGeneralServerConnectionType)type {
  ...
  case iTermGeneralServerConnectionTypeMono:
    ...
    self.jobManager = [[iTermMonoServerJobManager alloc]
        initWithQueue:self->_jobManagerQueue];
    return;
  ...
  case iTermGeneralServerConnectionTypeMulti:
    ...
    self.jobManager = [[iTermMultiServerJobManager alloc]
        initWithQueue:self->_jobManagerQueue];
    return;
  }
...
// This works for any kind of connection. It finishes the process of attaching a
// PTYTask to a child that we know is in a server, either newly launched or an
// orphan.
...
- (iTermJobManagerAttachResults)attachToServer:
  ...
  if (serverConnection.type == iTermGeneralServerConnectionTypeMulti) {
    DLog(@"PTYTask: attach to multiserver %@",
...
// Monoserver only. Used when restoring a non-ophan session. May block while
// connecting to the server. Deletes the socket after connecting.
- (BOOL)tryToAttachToServerWithProcessId:(pid_t)thePid tty:(NSString *)tty {
  if (![self canAttach]) {
    return NO;
  }

  DLog(@"tryToAttachToServerWithProcessId: Attempt to connect to server for "
       @"pid %d, tty %@",
       (int)thePid, tty);
  iTermFileDescriptorServerConnection serverConnection =
      iTermFileDescriptorClientRun(thePid);
...
// Multiserver only. Used when restoring a non-orphan session. May block while
...
- (iTermJobManagerAttachResults)
  ...
  if (![iTermMultiServerJobManager
               getGeneralConnection:&generalConnection
          fromRestorationIdentifier:restorationIdentifier]) {
    return 0;
  }

  DLog(@"tryToAttachToMultiserverWithRestorationIdentifier:%@",
...
- (NSArray<NSString *> *)environWithOverrides:(NSDictionary *)env {
  ...
  for (NSString *k in environmentDict) {
    NSString *temp =
        [NSString stringWithFormat:@"%@=%@", k, environmentDict[k]];
    [environment addObject:temp];
  }
  return environment;
}

- (NSDictionary *)environmentBySettingShell:
    (NSDictionary *)originalEnvironment {
...
- (void)reallyLaunchWithPath:(NSString *)progpath
                 ...
                 customShell:(NSString *)customShell
                    ...
                    viewSize:(NSSize)viewSize
                      isUTF8:(BOOL)isUTF8
                  completion:(void (^)(void))completion {
  DLog(@"reallyLaunchWithPath:%@ args:%@ env:%@ gridSize:%@ viewSize:%@ "
       @"isUTF8:%@",
       progpath, args, env, VT100GridSizeDescription(gridSize),
       NSStringFromSize(viewSize), @(isUTF8));

  __block iTermTTYState ttyState;
  iTermTTYStateInit(
      &ttyState, iTermTTYCellSizeMake(gridSize.width, gridSize.height),
      iTermTTYPixelSizeMake(viewSize.width, viewSize.height), isUTF8);
  ...
  DLog(@"Preparing to launch a job. Command is %@ and args are %@",
       commandToExec, args);
  DLog(@"Environment is\n%@", env);
  NSArray<NSString *> *newEnviron = [self environWithOverrides:env];

  // Note: stringByStandardizingPath will automatically call
...
- (void)showFailedToCreateTempSocketError {
  ...
  alert.informativeText =
      [NSString stringWithFormat:
                    @"An error was encountered while creating a temporary file "
                    @"with mkstemps. Verify that %@ exists and is writable.",
...
- (void)readTask:(char *)buffer length:(int)length {
  ...
  // The delegate is responsible for parsing VT100 tokens here and sending them
  // off to the main thread for execution. If its queues get too large, it can
  // block.
  [self.delegate threadedReadTask:buffer length:length];

  @synchronized(self) {
    if (coprocess_) {
      [coprocess_.outputBuffer appendData:[NSData dataWithBytes:buffer
                                                         length:length]];
    }
  }
}
...
- (void)closeFileDescriptorAndDeregisterIfPossible {
  ...
  if ([self.jobManager closeFileDescriptor]) {
    DLog(@"Deregister file descriptor %d for process %@ after closing it", fd,
         @(self.pid));
    [[TaskNotifier sharedInstance] deregisterTask:self];
  }
}
...
- (void)rateLimitedSetSizeToDesiredSize {
  ...
  if ([NSDate timeIntervalSinceReferenceDate] - _timeOfLastSizeChange <
      kDelayBetweenSizeChanges) {
    // Avoid problems with signal coalescing of SIGWINCH preventing redraw for
    // the second size change. For example, issue 5096 and 4494.
    _rateLimitedSetSizeToDesiredSizePending = YES;
    DLog(@" ** Rate limiting **");
    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW,
                      (int64_t)(kDelayBetweenSizeChanges * NSEC_PER_SEC)),
...
- (void)setTerminalSizeToDesiredSize {
  DLog(@"Set size of %@ from (%@x%@ cells, %@x%@px) to (%@x%@ cells, %@x%@ px)",
       self.delegate, @(_lastSize.cellSize.width), @(_lastSize.cellSize.height),
       @(_lastSize.pixelSize.width), @(_lastSize.pixelSize.height),

       @(_desiredSize.cellSize.width), @(_desiredSize.cellSize.height),
       @(_desiredSize.pixelSize.width), @(_desiredSize.pixelSize.height));
...
// NOTE: This can be called before the task is launched. It is not used when
// logging plain text.
