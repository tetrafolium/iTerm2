...
- (instancetype)initWithScope:(iTermVariableScope *)scope
  ...
  if (self) {
    _swiftyString =
        [[NSString castFrom:[scope valueForVariableName:sourcePath]] copy]
            ?: @"";
    _scope = scope;
    _refs = [NSMutableArray array];
    _missingFunctions = [NSMutableSet set];
    _destinationPath = [destinationPath copy];
    _sourceRef = [[iTermVariableReference alloc] initWithPath:sourcePath
                                                       vendor:scope];
...
- (void)sourceDidChange {
  DLog(@"%@->%@ sourceDidChange to %@", _sourceRef.path, _destinationPath,
       _sourceRef.value);
...
- (void)setEvaluatedString:(NSString *)evaluatedString error:(NSError *)error {
  if (error == nil && [NSObject object:evaluatedString
                          isEqualToObject:self.evaluatedString]) {
    return;
  }
...
- (void)evaluateSynchronously:(BOOL)synchronously {
  ...
  DLog(@"%p: %@->%@ evaluate %@", self, _sourceRef.path, _destinationPath,
       _swiftyString);
                   ...
                   completion:^(NSString *result, NSError *error) {
                     ...
                     if (strongSelf) {
                       if (strongSelf.appliedCount > count) {
                         // A later async evaluation has already completed.
                         // Don't overwrite it.
                         DLog(@"obsoleted");
                         return;
                       }
                       ...
                       if (error == nil &&
                           [NSObject object:strongSelf.evaluatedString
                               isEqualToObject:result]) {
                         DLog(@"unchanged");
                         return;
                       }
...
- (void)evaluateSynchronously:(BOOL)synchronously
  ...
  __weak __typeof(self) weakSelf = self;
  [self
      evaluateSynchronously:synchronously
                  withScope:scope
                 completion:^(NSString *result, NSError *error,
                   ...
                   if (error) {
                     NSString *message = [NSString
                         stringWithFormat:
                             @"Invocation of “%@” failed with error:\n%@\n",
                     ...
                     iTermScriptHistoryEntry *entry =
                         [[iTermScriptHistory sharedInstance]
                             entryWithIdentifier:connectionKey];
                     if (!entry) {
...
- (void)evaluateSynchronously:(BOOL)synchronously
  ...
  iTermExpressionEvaluator *evaluator =
      [[iTermExpressionEvaluator alloc] initWithInterpolatedString:_swiftyString
                                                             scope:scope];
  [evaluator
      evaluateWithTimeout:synchronously ? 0 : 30
               completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                 completion(evaluator.value, evaluator.error,
                            evaluator.missingValues);
               }];
}
