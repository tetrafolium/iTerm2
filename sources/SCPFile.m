...
static NSString *const kSecureCopyConnectionFailedWarning =
    @"NoSyncSecureCopyConnectionFailedWarning";
...
- (NSString *)authSock {
  ...
  NSString *shell = [ITAddressBookMgr customShellForProfile:profile]
  ...
  if (request) {
    ...
    [[iTermSlowOperationGateway sharedInstance]
        exfiltrateEnvironmentVariableNamed:@"SSH_AUTH_SOCK"
                                     shell:shell
...
- (NSString *)subheading {
  return [NSString stringWithFormat:@"%@@%@:%@", self.path.username,
                                    self.path.hostname, self.path.path];
}
...
- (NSString *)hostname {
  NSArray *hostComponents =
      [self.path.hostname componentsSeparatedByString:@":"];
  ...
  if (components == 2) {
    ...
    hostComponents =
        [hostComponents subarrayWithRange:NSMakeRange(0, components - 1)];
    NSString *bracketedHostname =
        [hostComponents componentsJoinedByString:@":"];
    return [bracketedHostname
        substringWithRange:NSMakeRange(1, bracketedHostname.length - 2)];
  }
...
- (NSArray *)configs {
  ...
  NSArray *paths = @[
    ...
    @"/etc/ssh/ssh_config", @"/etc/ssh_config"
  ];
...
- (NSString *)filenameByExpandingMetasyntacticVariables:(NSString *)filename {
  ...
  for (NSString *metavar in substitutions) {
    filename =
        [filename stringByReplacingOccurrencesOfString:metavar
...
- (void)performTransfer:(BOOL)isDownload agentAllowed:(BOOL)agentAllowed {
  ...
  if (!baseName) {
    self.error =
        [NSString stringWithFormat:@"Invalid path: %@", self.path.path];
  ...
  if (self.session) {
    ...
    if (self.stopped) {
      ...
      dispatch_sync(dispatch_get_main_queue(), ^() {
        [[FileTransferManager sharedInstance]
            transferrableFileDidStopTransfer:self];
      });
      return;
    }
  }
  ...
  if (!self.session.isConnected) {
    ...
    if (!theError) {
      // If connection fails, there is no rawSession in NMSSHSession, so it
      // can't return an error. Should that ever change, this clause will not
      // execute.
      theError = [NSError
          errorWithDomain:@"com.googlecode.iterm2"
                     code:-1
    ...
    dispatch_sync(dispatch_get_main_queue(), ^() {
      ...
      iTermWarningSelection selection = [iTermWarning
          showWarningWithTitle:
              [NSString
                  stringWithFormat:@"Failed to connect to %@:%d. Double-check "
                                   @"that the host name is correct.",
  ...
  if (!self.session.isAuthorized) {
    ...
    for (NSString *authType in authTypes) {
      ...
      if ([authType isEqualToString:@"password"]) {
        ...
        [self.session authenticateByKeyboardInteractiveUsingBlock:^NSString *(
                          NSString *request) {
        ...
        for (NSString *iteratedKeyPath in keyPaths) {
          NSString *keyPath =
              [self filenameByExpandingMetasyntacticVariables:iteratedKeyPath];
          ...
          if ([self privateKeyIsEncrypted:keyPath]) {
            NSString *prompt = [NSString
                stringWithFormat:@"passphrase for private key “%@”:", keyPath];
          ...
          if (![[NSFileManager defaultManager]
                  fileExistsAtPath:publicKeyPath]) {
            XLog(@"Warning: no public key at %@. Trying to authenticate with "
                 @"only a private key.",
                 publicKeyPath);
            publicKeyPath = nil;
          }
  ...
  if (!self.session.session && didConnectToAgent) {
    // Try again without agent. I got into a state where using the agent
    // prevented connections from going through.
    [self.session disconnect];
    self.session = nil;
  ...
  if (!self.session.isAuthorized) {
    ...
    dispatch_sync(dispatch_get_main_queue(), ^() {
      if (!error) {
        error = [NSError
            errorWithDomain:@"com.googlecode.iterm2.SCPFile"
                       code:0
                   userInfo:@{
  ...
  if (isDownload) {
    NSString *downloadDirectory =
        [[NSFileManager defaultManager] downloadsDirectory];
    NSString *tempfile = nil;
    NSString *tempFileName = [self tempFileName];
    if (downloadDirectory) {
      tempfile =
          [downloadDirectory stringByAppendingPathComponent:tempFileName];
    }
    if (!tempfile) {
      self.error =
          [NSString stringWithFormat:@"Downloads folder not writable. Tried %@",
                                     downloadDirectory];
      dispatch_sync(dispatch_get_main_queue(), ^() {
        [[FileTransferManager sharedInstance]
                         transferrableFile:self
            didFinishTransmissionWithError:
                SCPFileError(@"Downloads folder not writable")];
      });
      return;
    }
        ...
        downloadFile:self.path.path
            ...
            progress:^BOOL(NSUInteger bytes, NSUInteger fileSize) {
              ...
              dispatch_sync(dispatch_get_main_queue(), ^() {
                if (!self.stopped) {
                  [[FileTransferManager sharedInstance]
                      transferrableFileProgressDidChange:self];
                }
              });
              if (self.stopped) {
    ...
    if (ok) {
      error = nil;
      // We determine the filename and perform the move in the main thread to
      // avoid two threads trying to determine the final destination at the same
      // time.
      dispatch_sync(dispatch_get_main_queue(), ^() {
      ...
      const BOOL ok = [[NSFileManager defaultManager] removeItemAtPath:tempfile
                                                                 error:&error];
...
- (void)upload {
  ...
  self.status = kTransferrableFileStatusStarting;
  self.fileSize =
      [[[NSFileManager defaultManager] attributesOfItemAtPath:self.localPath
                                                        error:nil] fileSize];
  [[[FileTransferManager sharedInstance] files] addObject:self];
  [[FileTransferManager sharedInstance] transferrableFileDidStartTransfer:self];
...
- (BOOL)session:(NMSSHSession *)session
    shouldConnectToHostWithFingerprint:(NSString *)fingerprint {
  ...
  dispatch_sync(dispatch_get_main_queue(), ^(void) {
    ...
    case NMSSHKnownHostStatusFailure:
      title =
          [NSString stringWithFormat:@"Problem connecting to %@", session.host];
      message = [NSString
          stringWithFormat:@"Could not read the known_hosts file.\n"
                           @"As a result, the authenticity of host '%@' can't "
                           @"be established."
    ...
    case NMSSHKnownHostStatusNotFound:
      title = [NSString
          stringWithFormat:@"First time connecting to %@", session.host];
      message = [NSString
          stringWithFormat:
...
- (NSString *)session:(NMSSHSession *)session
    keyboardInteractiveRequest:(NSString *)request {
