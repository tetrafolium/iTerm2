...
// Prefs-level keys
#define KEY_DEFAULT_GUID                                                       \
  @"Default Bookmark Guid" // use this instead (not in a bookmark)
...
#define KEY_INITIAL_TEXT                                                       \
  @"Initial Text" // String. Evaluated as a swifty string.
#define KEY_CUSTOM_DIRECTORY                                                   \
  @"Custom Directory" // values are Yes, No, Recycle, Advanced
...
#define KEY_ORIGINAL_GUID                                                      \
  @"Original Guid" // GUID before divorce. Not saved to preferences plist.
...
#define KEY_TITLE_FUNC                                                         \
  @"Title Function" // Value is iTermTuple.plistValue of (display name, unique
                    // identifier); e.g. ("Hello world",
                    // "com.iterm2.example.title-provider")
...
#define KEY_PREVENT_APS                                                        \
  @"Prevent Automatic Profile Switching" // Not in regular prefs, only for
                                         // divorced prefs.
...
#define KEY_USE_BOLD_COLOR                                                     \
  @"Use Bright Bold" // Pre-3.3.7: Means "use the specified bold color, and also
                     // use the bright version of dark ansi colors". Post-3.3.7:
                     // Use the specified bold color
...
#define KEY_BACKGROUND_IMAGE_TILED_DEPRECATED                                  \
  @"Background Image Is Tiled" // DEPRECATED
#define KEY_ASCII_LIGATURES @"ASCII Ligatures"
#define KEY_NON_ASCII_LIGATURES @"Non-ASCII Ligatures"
#define KEY_BACKGROUND_IMAGE_MODE                                              \
  @"Background Image Mode" // iTermBackgroundImageMode enum
...
#define KEY_TRANSPARENCY_AFFECTS_ONLY_DEFAULT_BACKGROUND_COLOR                 \
...
#define KEY_TREAT_NON_ASCII_AS_DOUBLE_WIDTH                                    \
  @"Non Ascii Double Width" // DEPRECATED
...
#define KEY_XTERM_MOUSE_REPORTING_ALLOW_MOUSE_WHEEL                            \
  @"Mouse Reporting allow mouse wheel"
#define KEY_XTERM_MOUSE_REPORTING_ALLOW_CLICKS_AND_DRAGS                       \
...
#define KEY_PROMPT_CLOSE_DEPRECATED                                            \
  @"Prompt Before Closing" // Deprecated due to bad migration in 8/28 build
...
#define KEY_HAS_HOTKEY                                                         \
  @"Has Hotkey" // This determines whether the "has a hotkey" box is checked.
                // See also KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS.
#define KEY_HOTKEY_KEY_CODE @"HotKey Key Code"
#define KEY_HOTKEY_CHARACTERS @"HotKey Characters"
#define KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS                               \
  @"HotKey Characters Ignoring Modifiers" // If this is non-empty then a hotkey
                                          // is assigned, but see also
                                          // KEY_HAS_HOTKEY.
...
#define KEY_TRIGGERS_USE_INTERPOLATED_STRINGS                                  \
...
// Minimum time between sending anti-idle codes. "1" otherwise results in a
// flood.
...
// I chose 1250 because on a 6k display each cell would be less than 5 points
// wide, which won't be legible. It needs an upper bound because of issue 8592.
...
typedef NS_OPTIONS(NSUInteger, iTermTitleComponents) {
  ...
  iTermTitleComponentsCustom =
      1 << 4, // Mutually exclusive with all other options.
...
// This is deprecated in favor of -[NSString fontValue] and -[NSFont
// stringValue].
...
// Like computeCommandForProfile:objectType:scope:completion: but does not
// evaluate it.
...
// Removes the profile from the model, removes key mappings that reference this
// profile, and posts a kProfileWasDeletedNotification notification, then
// flushes the model to backing store.
