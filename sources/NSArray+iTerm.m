...
- (NSAttributedString *)attributedComponentsJoinedByAttributedString:
    (NSAttributedString *)joiner {
...
- (NSArray *)mapEnumeratedWithBlock:(id (^NS_NOESCAPE)(NSUInteger, id anObject,
                                                       BOOL *stop))block {
...
- (NSArray *)filteredArrayUsingBlock:(BOOL (^NS_NOESCAPE)(id))block {
  NSIndexSet *indexes =
      [self indexesOfObjectsPassingTest:^BOOL(id _Nonnull obj, NSUInteger idx,
                                              BOOL *_Nonnull stop) {
        return block(obj);
      }];
...
- (id)objectPassingTest:(BOOL (^)(id element, NSUInteger index,
                                  BOOL *stop))block {
...
- (id)objectOfClass:(Class)theClass
  ...
  NSUInteger index =
      [self indexOfObjectPassingTest:^BOOL(id _Nonnull obj, NSUInteger idx,
                                           BOOL *_Nonnull stop) {
...
- (NSArray *)minimumsWithComparator:
    (NSComparisonResult (^NS_NOESCAPE)(id, id))comparator {
  id min = nil;
  for (id object in self) {
...
- (NSArray *)maximumsWithComparator:
    (NSComparisonResult (^NS_NOESCAPE)(id, id))comparator {
...
- (NSString *)componentsJoinedWithOxfordCommaAndConjunction:
  ...
  if (self.count == 0) {
    ...
    return
        [self componentsJoinedByString:[NSString stringWithFormat:@" %@ ",
                                                                  conjunction]];
  } else {
    NSArray *allButLastArray =
        [self subarrayWithRange:NSMakeRange(0, self.count - 1)];
    NSString *allButLastString =
        [allButLastArray componentsJoinedByString:@", "];
    NSString *result =
        [NSString stringWithFormat:@"%@, %@ %@", allButLastString, conjunction,
                                   self.lastObject];
    return result;
  }
}
...
- (NSArray *)arrayByRemovingObject:(id)objectToRemove {
  ...
  if (index == NSNotFound) {
    ...
    return [[self subarrayToIndex:index]
        arrayByAddingObjectsFromArray:[self subarrayFromIndex:index + 1]];
  }
}
...
- (id)reduceWithFirstValue:(id)firstValue
                     block:(id (^)(id first, id second))block {
...
- (NSURL *)lowestCommonAncestorOfURLs {
  ...
  NSArray<NSArray<NSString *> *> *componentsArrays =
      [self mapWithBlock:^id(NSURL *url) {
        return [url.path pathComponents];
      }];
  NSArray<NSNumber *> *counts =
      [componentsArrays mapWithBlock:^id(NSArray<NSString *> *anObject) {
        return @(anObject.count);
      }];
  NSInteger shortestCount =
      [[counts reduceWithBlock:^NSNumber *(NSNumber *first, NSNumber *second) {
        if (second) {
  ...
  for (i = 0; i < shortestCount; i++) {
    ...
    const BOOL allShareAncestor =
        [componentsArrays allWithBlock:^BOOL(NSArray<NSString *> *anObject) {
  ...
  NSString *path = [[componentsArrays.firstObject
      subarrayWithRange:NSMakeRange(0, i)] componentsJoinedByString:@"/"];
  return [NSURL fileURLWithPath:path];
}

- (void)enumerateCoalescedObjectsWithComparator:(BOOL (^)(id obj1,
                                                          id obj2))comparator
                                          block:(void (^)(id object,
                                                          NSUInteger count))
                                                    block {
...
- (NSUInteger)indexOfMaxWithBlock:(NSComparisonResult (^)(id, id))block {
  ...
  __block id max = nil;
  [self enumerateObjectsUsingBlock:^(id _Nonnull object, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
    if (max) {
...
- (NSArray *)countedInstancesStrings {
  ...
  NSArray *sortedKeys = [classified.allKeys
  ...
  return [sortedKeys mapWithBlock:^id(NSString *key) {
    ...
    if (count > 1) {
      return
          [NSString stringWithFormat:@"%@ instances of \"%@\"", @(count), key];
    } else {
...
- (instancetype)it_arrayByRemovingObjectsPassingTest:
    (BOOL (^)(id anObject))block {
...
- (const char **)nullTerminatedCStringArray {
  ...
  [self enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
