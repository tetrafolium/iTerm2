...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconClock"
                                forClass:[self class]];
}
  ...
  iTermStatusBarComponentKnob *formatKnob = [[iTermStatusBarComponentKnob alloc]
      initWithLabelText:@"Date Format:"
           ...
           defaultValue:self.class.statusBarComponentDefaultKnobs
                            [iTermStatusBarClockComponentFormatKey]
                    key:iTermStatusBarClockComponentFormatKey];
  formatKnob.helpURL = [NSURL
      URLWithString:@"https://iterm2.com/clock-status-bar-component-help"];
  iTermStatusBarComponentKnob *dateFormatIsTemplate =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Localize Date Format"
                       type:iTermStatusBarComponentKnobTypeCheckbox
                placeholder:nil
               defaultValue:@YES
                        key:iTermStatusBarClockComponentLocalizeKey];
  return [@[ formatKnob, dateFormatIsTemplate, [super statusBarComponentKnobs] ]
      flattenedArray];
}

+ (NSDictionary *)statusBarComponentDefaultKnobs {
  NSDictionary *fromSuper = [super statusBarComponentDefaultKnobs];
  return [fromSuper dictionaryByMergingDictionary:@{
    iTermStatusBarClockComponentFormatKey : @"M/dd h:mm"
  }];
}
...
- (NSDateFormatter *)dateFormatter {
  if (!_dateFormatter) {
    ...
    NSString *template =
        knobValues[iTermStatusBarClockComponentFormatKey] ?: @"M/dd h:mm";
    if ([knobValues[iTermStatusBarClockComponentLocalizeKey]
                ?: @YES boolValue]) {
