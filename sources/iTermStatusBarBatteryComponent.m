...
static NSString *const iTermBatteryComponentKnobKeyShowPercentage =
    @"ShowPercentage";
    ...
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(powerManagerStateDidChange:)
             ...
             object:nil];
    _chargingImage = [NSImage it_imageNamed:@"StatusBarIconCharging"
                                   forClass:self.class];
    [[iTermPowerManager sharedInstance]
        addPowerStateSubscriber:self
                          block:^(iTermPowerState *state) {
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  iTermStatusBarComponentKnob *showPercentageKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Show Percentage"
  ...
  iTermStatusBarComponentKnob *showEstimatedTimeKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Show Estimated Time"
                       type:iTermStatusBarComponentKnobTypeCheckbox
                placeholder:nil
               defaultValue:@NO
                        key:iTermBatteryComponentKnobKeyShowTime];
  return [@[ showPercentageKnob, showEstimatedTimeKnob ]
...
- (NSImage *)statusBarComponentIcon {
  ...
  dispatch_once(&onceToken, ^{
    if ([self.class machineHasBattery]) {
      image = [NSImage it_cacheableImageNamed:@"StatusBarIconBattery"
                                     forClass:[self class]];
    } else {
      image = [NSImage it_cacheableImageNamed:@"StatusBarIconNoBattery"
                                     forClass:[self class]];
    }
  });
  return image;
...
- (iTermStatusBarSparklinesModel *)sparklinesModel {
  NSArray<NSNumber *> *values = [[[iTermPowerManager sharedInstance]
      percentageSamples] mapWithBlock:^id(NSNumber *anObject) {
    return @(anObject.doubleValue / 100.0);
  }];
  iTermStatusBarTimeSeries *timeSeries =
      [[iTermStatusBarTimeSeries alloc] initWithValues:values];
  iTermStatusBarTimeSeriesRendition *rendition =
      [[iTermStatusBarTimeSeriesRendition alloc]
          initWithTimeSeries:timeSeries
                       color:[self statusBarTextColor]];
  return [[iTermStatusBarSparklinesModel alloc]
      initWithDictionary:@{@"main" : rendition}];
}

- (int)currentEstimate {
  const double x = [[[[iTermPowerManager sharedInstance] currentState]
...
- (BOOL)isCharging {
  return [self.class machineHasBattery] &&
         [[iTermPowerManager sharedInstance] connectedToPower];
}
...
- (BOOL)showTime {
  ...
  iTermPowerState *currentState =
      [[iTermPowerManager sharedInstance] currentState];
...
- (BOOL)showTimeOnRight {
  return (self.showTime && self.showPercentage);
}
...
- (NSString *)leftText {
  ...
  if (self.showTime) {
    return [NSDateFormatter
        durationString:[[[[iTermPowerManager sharedInstance] currentState] time]
                           doubleValue]];
  }
  return @"";
}
