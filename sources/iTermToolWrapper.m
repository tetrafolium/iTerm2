...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    _title = [[NSTextField alloc]
        initWithFrame:NSMakeRect(kCloseButtonLeftMargin + kButtonSize, 0,
    ...
    NSImage *closeImage = [NSImage it_imageNamed:@"closebutton"
                                        forClass:self.class];
    _closeButton =
        [[NSButton alloc] initWithFrame:NSMakeRect(kCloseButtonLeftMargin, 10,
    ...
    _container = [[[NSView alloc]
        initWithFrame:NSMakeRect(kLeftMargin, kTitleHeight + kMargin,
                                 frame.size.width - kLeftMargin - kRightMargin,
                                 frame.size.height - kTitleHeight - kMargin -
                                     kBottomMargin)] autorelease];
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@:%p frame=%@>", [self class], self,
...
- (void)relayout {
  ...
  _title.frame =
      NSMakeRect(sideMargin, kTopMargin,
                 frame.size.width - kButtonSize - kRightMargin - sideMargin,
                 kTitleHeight - kTopMargin);
  _closeButton.frame =
      NSMakeRect(kCloseButtonLeftMargin, kTopMargin, kButtonSize, kButtonSize);
  _container.frame = NSMakeRect(
      kLeftMargin, kTitleHeight + kMargin,
...
- (void)setName:(NSString *)theName {
  [_name autorelease];
  _name = [theName copy];
  [self performSelector:@selector(setTitleEditable)
             withObject:nil
             afterDelay:0];
}
