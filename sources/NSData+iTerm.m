...
+ (NSData *)dataWithBase64EncodedString:(NSString *)string {
  const char *buffer =
      [[string stringByReplacingOccurrencesOfRegex:@"[\x0a\x0d]"
...
- (NSString *)stringWithBase64EncodingWithLineBreak:(NSString *)lineBreak {
  // Subtract because the result includes the trailing null. Take MAX in case it
  ...
  if (buffer) {
    apr_base64_encode_binary(buffer.mutableBytes, self.bytes, self.length);
  }
...
+ (NSData *)dataWithTGZContainingFiles:(NSArray<NSString *> *)files
  ...
  NSArray<NSString *> *args = @[
    ...
    @"-f",
    @"-", // write to stdout
    [NSString stringWithFormat:@"-C%@", basePath]
  ];                                                 // Base path
  ...
  if (errorMessage.length) {
    NSString *errorString =
        [errorMessage stringWithEncoding:NSUTF8StringEncoding];
    if (errorString) {
      if (error) {
        *error = [NSError errorWithDomain:@"com.googlecode.iterm2"
                                     code:1
                                 userInfo:@{@"errorMessage" : errorString}];
      }
    } else {
...
- (NSString *)uniformTypeIdentifierForImageData {
  ...
  for (int i = 0; i < sizeof(identifiers) / sizeof(*identifiers); i++) {
    if (self.length >= identifiers[i].length &&
        !memcmp(self.bytes, identifiers[i].fingerprint,
                identifiers[i].length)) {
...
- (id)it_unarchivedObjectOfClasses:(NSArray<Class> *)allowedClasses {
  ...
  NSKeyedUnarchiver *unarchiver =
      [[[NSKeyedUnarchiver alloc] initForReadingFromData:self
                                                   error:&error] autorelease];
  if (error) {
    return nil;
  }
  unarchiver.requiresSecureCoding = NO;
  return [unarchiver decodeObjectOfClasses:[NSSet setWithArray:allowedClasses]
                                    forKey:@"object"];
}
...
- (id)it_unarchivedObjectOfBasicClassesWithError:(NSError **)error {
  NSArray<Class> *classes = @[
    [NSDictionary class], [NSNumber class], [NSString class], [NSDate class],
    [NSData class], [NSArray class], [NSNull class], [NSValue class]
  ];
  NSError *innerError = nil;
  id obj =
      [NSKeyedUnarchiver unarchivedObjectOfClasses:[NSSet setWithArray:classes]
                                          fromData:self
                                             error:&innerError];
  if (innerError) {
...
- (NSData *)it_compressedData {
  ...
  z_stream stream = {.next_in = (Bytef *)self.bytes,
  ...
  NSMutableData *compressedData =
      [NSMutableData dataWithLength:ceil(self.length * 1.01) + 12];
...
- (NSData *)aesCBCEncryptedDataWithPCKS7PaddingAndKey:(NSData *)key
  ...
  const CCCryptorStatus result =
      CCCrypt(kCCEncrypt, kCCAlgorithmAES, kCCOptionPKCS7Padding, key.bytes,
              key.length, iv.bytes, self.bytes, self.length,
              ciphertext.mutableBytes, ciphertext.length, &length);
...
- (NSData *)decryptedAESCBCDataWithPCKS7PaddingAndKey:(NSData *)key
  ...
  size_t length;
  const CCCryptorStatus result =
      CCCrypt(kCCDecrypt, kCCAlgorithmAES, kCCOptionPKCS7Padding, key.bytes,
              key.length, iv.bytes, self.bytes, self.length,
              plaintext.mutableBytes, plaintext.length, &length);
...
- (void)writeReadOnlyToURL:(NSURL *)url {
  // Use POSIX APIs to ensure that permissions are set before writing to the
  // file.

  // Unlink file if it already exists.
  unlink(url.path.UTF8String);

  // Create it exclusively. If another instance of iTerm2 is trying to create
  // it, back off.
  int fd = -1;
  do {
    fd = open(url.path.UTF8String,
              O_CREAT | O_WRONLY | O_TRUNC | O_CLOEXEC | O_EXCL, 0600);
