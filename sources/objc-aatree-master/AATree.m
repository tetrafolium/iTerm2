...
/*!
 * @abstract				Deletes the node bound to the specified
 *key.
...
/*!
 * @abstract				Insert the specified data in the AA
 *tree.
 * @discussion				The data is inserted by looking up the
 *correct leaf node, setting the node as the left or right child of the leaf.
 *The function is recursive, so the skew and split operations are performed on
 *all parents of the added node automatically. If a node with the same key as
 *the new node is found, the data of the node is replaced with the new data.
 *
 ...
 * @discussion				See the header file for more information
 *on the current implementation of thread safety.
 */
...
/*!
 * @abstract				Retrieves the node bound to the specified
 *key.
 * @discussion				This function uses the key comparator, as
 *specified on initialization of the tree.
 *
...
/*!
 * @abstract				Retrieves the node which comes closest to
 *the specified key.
 * @discussion				This function uses the key comparator, as
 *specified on initialization of the tree. The returned node's key won't surpass
 *the specified key. If no node with a key lower than the specified key can be
 *found, nil is returned. This function performs its operation recursive.
 *
 * @param aKey				The key to look for.
 ...
 * @discussion				This function makes sure that every
 *violation of the first balance rule, which states that no left horizontal
 *logical links are allowed, are fixed. It does this by rotating right at the
 *							parent of the left horizontal
 *logical link.
 *
 * @param aRoot				The root node to check.
 * @result					The possibly new root after the skew, or
 *the same if nothing has been skewed.
 */
 ...
 * @discussion				This function makes sure that every
 *violation of the second balance rule, which states that no two consecutive
 *right horizontal logical links are allowed, are fixed. It does this by
 *rotating left and increasing the level of the parent.
 *
 * @param aRoot				The root node to check.
 * @result					The possibly new root after the split,
 *or the same if nothing has been split.
 */
...
- (id)init {
  ...
  @throw [NSException exceptionWithName:@"MethodNotAllowedException"
                                 reason:@"Initialize an AATree with the "
                                        @"'initWithKeyComparator:' method."
                               userInfo:nil];
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p root=%@>", self.class, self, self.root];
}
...
- (void)setObject:(id)anObject forKey:(id)aKey {
  ...
  ITAssertWithMessage(
      [aKey conformsToProtocol:@protocol(NSCopying)],
      @"The supplied key does not conform to the NSCopying protocol.");

  [self __lockForWriting];
  changedNodes = [[NSMutableSet alloc] init];
  AATreeNode *newNode =
      [[[AATreeNode alloc] initWithData:anObject
                             boundToKey:[[aKey copy] autorelease]] autorelease];
  AATreeNode *prevRoot = self.root;
...
- (AATreeNode *)__deleteNodeAtKey:(id)aKey atRoot:(AATreeNode *)aRoot {
  ...
  if (aRoot) {
    ...
    if (compareResult == NSOrderedSame) {
      ...
      // Check whether we are at an easy to remove node (zero to one children)
      // or a more difficult node.
    ...
    if (aRoot.left.level < aRoot.level - 1 ||
        aRoot.right.level < aRoot.level - 1) {

      // Decrease the level by one.
      aRoot.level--;

      // Decrease the right child's level also, when it is higher than its
      // parent.
...
- (AATreeNode *)__insertNode:(AATreeNode *)aNode atRoot:(AATreeNode *)aRoot {
  ...
  if (aRoot) {
    ...
    // If the key of the new node is equal to the current root, just replace the
    // data.
    if (compareResult == NSOrderedSame) {
      ...
      // new node will get skewed and split, all the way up to the root of the
      // tree.
      AATreeNode *prevRoot = aRoot;
      aRoot = [self __skew:aRoot];
      if (aRoot != prevRoot) {
