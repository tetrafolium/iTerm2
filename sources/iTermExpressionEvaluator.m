...
- (instancetype)initWithStrictInterpolatedString:(NSString *)interpolatedString
  ...
  iTermParsedExpression *parsedExpression = [iTermExpressionParser
      parsedExpressionWithInterpolatedString:interpolatedString
...
static NSMutableArray *iTermExpressionEvaluatorGlobalStore(void) {
  ITBetaAssert(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL) ==
  ...
  [self evaluateParsedExpression:_parsedExpression
                     ...
                     withTimeout:timeout
                      completion:^(id result, NSError *error,
                        ...
                        if (debug) {
                          NSLog(@"%@ result=%@, error=%@, missing=%@", descr,
                                result, error, missing);
                        }
                        [weakSelf didCompleteWithResult:result
                                                  error:error
                                                missing:missing
...
- (void)evaluateSwiftyString:(NSString *)string
                 withTimeout:(NSTimeInterval)timeout
                  completion:
                      (void (^)(id, NSError *, NSSet<NSString *> *))completion {
  NSMutableArray *parts = [NSMutableArray array];
  __block NSError *firstError = nil;
  dispatch_group_t group = dispatch_group_create();
  NSMutableSet<NSString *> *missingFunctionSignatures = [NSMutableSet set];
  [string enumerateSwiftySubstrings:^(NSUInteger index, NSString *substring,
                                      BOOL isLiteral, BOOL *stop) {
    if (isLiteral) {
      [parts
          addObject:[substring it_stringByExpandingBackslashEscapedCharacters]];
    } else {
      ...
      [innerEvaluator
          evaluateWithTimeout:timeout
                   completion:^(iTermExpressionEvaluator *evaluator) {
                     [missingFunctionSignatures
                         unionSet:evaluator.missingValues];
                     if (evaluator.error) {
                       firstError = evaluator.error;
                     } else {
                       parts[index] =
                           [self stringFromJSONObject:evaluator.value];
                     }
  ...
  if (timeout == 0) {
    completion([parts componentsJoinedByString:@""], firstError,
               missingFunctionSignatures);
  } else {
...
- (void)evaluateParsedExpression:(iTermParsedExpression *)parsedExpression
  ...
  case iTermParsedExpressionTypeFunctionCall: {
    ...
    [parsedExpression.functionCall
        performFunctionCallFromInvocation:invocation
                                 receiver:nil
                                    scope:_scope
                                  timeout:timeout
                               completion:completion];
  ...
  case iTermParsedExpressionTypeInterpolatedString: {
    [self
        evaluateInterpolatedStringParts:parsedExpression.interpolatedStringParts
  ...
  NSString *reason =
      [NSString stringWithFormat:@"Invalid parsed expression type %@",
                                 @(parsedExpression.expressionType)];
  NSError *error =
      [NSError errorWithDomain:@"com.iterm2.expression-evaluator"
                          code:2
                      userInfo:@{NSLocalizedDescriptionKey : reason}];
  completion(nil, error, nil);
}

- (void)evaluateInterpolatedStringParts:
            (NSArray<iTermParsedExpression *> *)interpolatedStringParts
                             invocation:(NSString *)invocation
                            withTimeout:(NSTimeInterval)timeout
                             completion:
                                 (void (^)(id, NSError *,
  ...
  [interpolatedStringParts enumerateObjectsUsingBlock:^(
                               iTermParsedExpression *_Nonnull parsedExpression,
                               NSUInteger idx, BOOL *_Nonnull stop) {
    if (parsedExpression.expressionType == iTermParsedExpressionTypeString &&
        parsedExpression.string) {
      // Shortcut. String literals get appended without messing with dispatch
      // groups or inner evaluators. They are also not subject to escaping,
      // since they were under the control of the caller before getting here.
      [parts addObject:parsedExpression.string];
      return;
    }

    [parts addObject:@""];
    iTermExpressionEvaluator *innerEvaluator = [[iTermExpressionEvaluator alloc]
        initWithParsedExpression:parsedExpression
                      invocation:invocation
                           scope:self->_scope];
    [self->_innerEvaluators addObject:innerEvaluator];
  ...
  if (!group) {
    ...
    dispatch_notify(group, dispatch_get_main_queue(), ^{
      [weakSelf
          didFinishEvaluatingInterpolatedStringWithParts:parts
                                                   error:firstError
                                                 missing:
                                                     missingFunctionSignatures
                                              completion:completion];
...
- (void)didFinishEvaluatingInterpolatedStringWithParts:(NSArray *)parts
  ...
  completion(firstError ? nil : [parts componentsJoinedByString:@""],
             firstError, missingFunctionSignatures);
}
...
- (void)evaluateArray:(NSArray *)array
  ...
  [array enumerateObjectsUsingBlock:^(
             iTermParsedExpression *_Nonnull parsedExpression, NSUInteger idx,
             BOOL *_Nonnull stop) {
    iTermExpressionEvaluator *innerEvaluator = [[iTermExpressionEvaluator alloc]
        initWithParsedExpression:parsedExpression
                      invocation:invocation
...
- (NSString *)stringFromJSONObject:(id)jsonObject {
  ...
  if (array) {
    return [NSString
        stringWithFormat:@"[%@]", [[array mapWithBlock:^id(id anObject) {
