...
- (iTermStatusBarContainerView *)containerViewWithLargestMinimumWidthFromViews:
    (NSArray<iTermStatusBarContainerView *> *)views {
  return [views
      maxWithBlock:^NSComparisonResult(iTermStatusBarContainerView *obj1,
                                       iTermStatusBarContainerView *obj2) {
        return [@(obj1.minimumWidthIncludingIcon)
            compare:@(obj2.minimumWidthIncludingIcon)];
      }];
}
...
- (CGFloat)minimumWidthOfContainerViews:
    (NSArray<iTermStatusBarContainerView *> *)views {
  iTermStatusBarContainerView *viewWithLargestMinimumWidth =
      [self containerViewWithLargestMinimumWidthFromViews:views];
  const CGFloat largestMinimumSize =
      viewWithLargestMinimumWidth.minimumWidthIncludingIcon;
  NSArray<iTermStatusBarContainerView *> *viewsExFixedSpacers =
      [self viewsExcludingFixedSpacers:views];
  const CGFloat widthOfAllFixedSpacers =
      [self widthOfFixedSpacersAmongViews:views];
  return largestMinimumSize * viewsExFixedSpacers.count +
         widthOfAllFixedSpacers;
}

- (NSArray<iTermStatusBarContainerView *> *)
    visibleContainerViewsAllowingEqualSpacingFromViews:
        (NSArray<iTermStatusBarContainerView *> *)visibleContainerViews {
  NSArray<iTermStatusBarContainerView *> *sortedViews =
      [self containerViewsSortedByPriority:visibleContainerViews];
  return [self
      visibleContainerViewsAllowingEqualSpacingFromSortedViews:sortedViews
                                                  orderedViews:
                                                      visibleContainerViews];
}

- (NSArray<iTermStatusBarContainerView *> *)
    visibleContainerViewsAllowingEqualSpacingFromSortedViews:
        (NSArray<iTermStatusBarContainerView *> *)sortedViews
                                                orderedViews:
                                                    (NSArray<
  ...
  if (self.mandatoryView) {
    removalCandidates =
        [removalCandidates arrayByRemovingObject:self.mandatoryView];
  }
  if (removalCandidates.count == 0) {
    return orderedViews;
  }
  iTermStatusBarContainerView *viewWithLargestMinimumWidth =
      [self bestViewToRemoveFrom:removalCandidates];
  iTermStatusBarContainerView *adjacentViewToRemove =
      [self viewToRemoveAdjacentToViewBeingRemoved:viewWithLargestMinimumWidth
  ...
  orderedViews =
      [orderedViews arrayByRemovingObject:viewWithLargestMinimumWidth];
  return [self
      visibleContainerViewsAllowingEqualSpacingFromSortedViews:sortedViews
                                                  orderedViews:orderedViews];
}

// views are sorted ascending by priority. First remove spacers regardless of
// priority, then remove views from lowest to highest priority.
- (iTermStatusBarContainerView *)bestViewToRemoveFrom:
    (NSArray<iTermStatusBarContainerView *> *)views {
  if (views.count == 0) {
    return nil;
  }
  NSInteger (^score)(iTermStatusBarContainerView *) = ^NSInteger(
  ...
  return [views
      maxWithComparator:^NSComparisonResult(iTermStatusBarContainerView *a,
                                            iTermStatusBarContainerView *b) {
        ...
        if (aScore == 0 && bScore == 0) {
          ...
          NSComparisonResult result = [@(bPrioirty)
              compare:@(aPriority)]; // Backwards so we score lower priority
                                     // higher for removal
...
- (NSArray<iTermStatusBarContainerView *> *)
    visibleContainerViewsAllowingEqualSpacing {
  if (_statusBarWidth <= 0) {
    return @[];
  }
  return [self visibleContainerViewsAllowingEqualSpacingFromViews:
                   [self allPossibleCandidateViews]];
}

- (iTermStatusBarContainerView *)
    viewToRemoveAdjacentToViewBeingRemoved:
        (iTermStatusBarContainerView *)viewBeingRemoved
                                 fromViews:
                                     (NSArray<iTermStatusBarContainerView *> *)
                                         views {
  ...
  if (![self views:views
          haveSpacersOnBothSidesOfIndex:[views indexOfObject:viewBeingRemoved]
                                   left:&left
                                  right:&right]) {
    return nil;
  }
  if (left.statusBarComponentSpringConstant >
      right.statusBarComponentSpringConstant) {
    return views[index - 1];
  } else if (left.statusBarComponentSpringConstant <
             right.statusBarComponentSpringConstant) {
    return views[index + 1];
...
- (NSArray<iTermStatusBarContainerView *> *)viewsExcludingFixedSpacers:
    (NSArray<iTermStatusBarContainerView *> *)views {
  return
...
- (NSArray<iTermStatusBarContainerView *> *)viewsExcludingPreallocatedViews:
    (NSArray<iTermStatusBarContainerView *> *)views {
  return
      [views filteredArrayUsingBlock:^BOOL(iTermStatusBarContainerView *view) {
...
- (CGFloat)widthOfFixedSpacersAmongViews:
    (NSArray<iTermStatusBarContainerView *> *)views {
  return [[views
      reduceWithFirstValue:@0
                     block:^id(NSNumber *partialSum,
                               iTermStatusBarContainerView *view) {
                       if (![view.component
                               isKindOfClass:[iTermStatusBarFixedSpacerComponent
                                                 class]]) {
                         return partialSum;
                       }
                       return @(partialSum.doubleValue +
  ...
  return [[views
      reduceWithFirstValue:@0
                     block:^id(NSNumber *partialSum,
                               iTermStatusBarContainerView *view) {
                       return
                           @(partialSum.doubleValue +
                             view.component.statusBarComponentSpringConstant);
                     }] doubleValue];
}

- (CGFloat)preallocatedWidthInViews:
               (NSArray<iTermStatusBarContainerView *> *)views
                          fromWidth:(CGFloat)totalWidth {
  NSArray<iTermStatusBarContainerView *> *preallocatedViews =
      [views filteredArrayUsingBlock:^BOOL(iTermStatusBarContainerView *view) {
        return view.component.statusBarComponentPriority == INFINITY;
      }];
  ...
  const CGFloat sumOfPreferredSizesOfPreallocatedViews =
      round([[views mapWithBlock:^id(iTermStatusBarContainerView *view) {
        return @(MAX(MIN([self maximumWidthForComponent:view.component],
                         singleUnitWidth),
                     view.minimumWidthIncludingIcon));
      }] sumOfNumbers]);
  if (sumOfPreferredSizesOfPreallocatedViews <= totalWidth) {
    // Can use larger of min width or the standard 1-unit size for all
    // preallocated views.
    ...
    [preallocatedViews
        enumerateObjectsUsingBlock:^(iTermStatusBarContainerView *_Nonnull view,
          ...
          const CGFloat width =
              MAX(singleUnitWidth, view.minimumWidthIncludingIcon);
          view.desiredWidth = width;
          preallocation += width;
        }];
    return preallocation;
  } else {
    // Not enough space. Divide all available space among preallocated views,
    // leaving nothing for others. Start by initializing all desired widths to
    // 0.
    [preallocatedViews
        enumerateObjectsUsingBlock:^(iTermStatusBarContainerView *_Nonnull view,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    while (round(available) > 0 && available >= numberOfGrowableViews) {
      ...
      [preallocatedViews enumerateObjectsUsingBlock:^(
                             iTermStatusBarContainerView *_Nonnull view,
                             NSUInteger idx, BOOL *_Nonnull stop) {
        const CGFloat maxWidth =
            MAX([self minimumWidthForComponent:view.component],
...
- (void)updateDesiredWidthsForViews:
  ...
  const CGFloat availableWidthBeforePreallocation =
      _statusBarWidth - totalMarginWidth - widthOfAllFixedSpacers;
  const CGFloat preallocatedWidth =
      [self preallocatedWidthInViews:viewsExFixedSpacers
                           fromWidth:availableWidthBeforePreallocation];
  CGFloat availableWidth =
      availableWidthBeforePreallocation - preallocatedWidth;
  NSArray<iTermStatusBarContainerView *> *viewsExPreallocatedViews =
      [self viewsExcludingPreallocatedViews:views];
  ...
  while (round(availableWidth) > 0 && viewsExPreallocatedViews.count > 0) {
    availableWidth =
        [self distributeNonPreallocatedAvailableWidth:availableWidth
                                           amongViews:viewsExPreallocatedViews];
    viewsExPreallocatedViews = [viewsExPreallocatedViews
        filteredArrayUsingBlock:^BOOL(iTermStatusBarContainerView *view) {
          return view.desiredWidth <
                 [self maximumWidthForComponent:view.component];
        }];
  }
}
...
- (CGFloat)
    distributeNonPreallocatedAvailableWidth:(CGFloat)availableWidth
                                 amongViews:
                                     (NSArray<iTermStatusBarContainerView *> *)
                                         views {
  NSArray<iTermStatusBarContainerView *> *viewsExFixedSpacers =
  ...
  const double sumOfSpringConstants =
      [self sumOfSpringConstantsInViews:viewsExFixedSpacers];
  const CGFloat apportionment = availableWidth / sumOfSpringConstants;
  __block CGFloat remainingWidth = availableWidth;
  DLog(@"updateDesiredWidthsForViews available=%@ apportionment=%@",
       @(availableWidth), @(apportionment));
...
- (NSArray<iTermStatusBarContainerView *> *)visibleContainerViews {
  NSArray<iTermStatusBarContainerView *> *visibleContainerViews =
      [self visibleContainerViewsAllowingEqualSpacing];
