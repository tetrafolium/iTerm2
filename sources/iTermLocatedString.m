...
- (void)trimTrailingWhitespace {
  ...
  [_coords
      removeObjectsInRange:NSMakeRange(_string.length,
  ...
  [_string reverseEnumerateSubstringsEqualTo:string
                                       block:^(NSRange range) {
                                         [_string replaceCharactersInRange:range
                                                                withString:@""];
                                         [_coords replaceObjectsInRange:range
                                                   withObjectsFromArray:empty];
                                       }];
}
...
- (void)trimTrailingWhitespace {
  ...
  [_coords removeObjectsInRange:NSMakeRange(_attributedString.length,
                                            lengthBeforeTrimming -
                                                _attributedString.length)];
}

- (void)removeOcurrencesOfString:(NSString *)string {
  NSArray *empty = @[];
  [_attributedString.string
      reverseEnumerateSubstringsEqualTo:string
                                  block:^(NSRange range) {
                                    [_attributedString
