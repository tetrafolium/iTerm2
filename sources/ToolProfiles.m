...
static NSString *const iTermToolProfilesProfileListViewState =
    @"iTermToolProfilesProfileListViewState";
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    listView_ = [[ProfileListView alloc]
        initWithFrame:NSMakeRect(kMargin, 0, frame.size.width - kMargin * 2,
    ...
    if (@available(macOS 10.16, *)) {
      ...
      _openButton.image = [NSImage it_imageForSymbolName:@"play"
                                accessibilityDescription:@"Open Profile"];
    ...
    [_openButton bind:@"enabled"
             toObject:listView_
          withKeyPath:@"hasSelection"
              options:nil];

    popup_ = [[NSPopUpButton alloc]
        initWithFrame:NSMakeRect(0, frame.size.height - kPopupHeight,
                                 frame.size.width -
...
- (void)relayout {
  ...
  listView_.frame =
      NSMakeRect(kMargin, 0, frame.size.width - kMargin * 2,
                 frame.size.height - kPopupHeight - kVerticalMargin);
  if (@available(macOS 10.16, *)) {
    const CGFloat margin = 0;
    popup_.frame = NSMakeRect(
        0, frame.size.height - kPopupHeight,
        frame.size.width - NSWidth(_openButton.frame) - margin, kPopupHeight);
    ...
    popup_.frame = NSMakeRect(0, frame.size.height - kPopupHeight,
                              frame.size.width - _openButton.frame.size.width -
                                  kInnerMargin,
                              kPopupHeight);
    _openButton.frame =
        NSMakeRect(frame.size.width - _openButton.frame.size.width,
                   frame.size.height - kPopupHeight,
                   _openButton.frame.size.width, _openButton.frame.size.height);
  }
}
