...
NSString *const iTermEventTapEventTappedNotification =
    @"iTermEventTapEventTappedNotification";

@interface iTermEventTap ()
@property(nonatomic, strong)
    NSMutableArray<iTermWeakReference<id<iTermEventTapObserver>> *> *observers;
@end
...
- (void)secureInputDidChange:(NSNotification *)notification {
  // Avoid trying to enable it if the process is not trusted because this gets
  // called when the app is activated/deactivated
...
- (void)setRemappingDelegate:
    (id<iTermEventTapRemappingDelegate>)remappingDelegate {
  _remappingDelegate = remappingDelegate;
  [self setEnabled:[self shouldBeEnabled]];
}

- (NSEvent *)runEventTapHandler:(NSEvent *)event {
  CGEventRef newEvent = iTermEventTapCallback(
      nil, kCGEventKeyDown, [event CGEvent], (__bridge void *)self);
  if (newEvent) {
...
- (BOOL)shouldBeEnabled {
  ...
  DLog(@"%@ remappingDelegate=%@ observers=%@", self, self.remappingDelegate,
       self.observers);
 ...
 *
 * The CGEventRef passed into the callback is retained by the calling code, and
 * is released after the callback returns and the data is passed back to the
 * event system.  If a different event is returned by the callback function,
 * then that event will be released by the calling code along with the original
 * event, after the event data has been passed back to the event system.
 */
static CGEventRef iTermEventTapCallback(CGEventTapProxy proxy, CGEventType type,
...
- (CGEventRef)eventTapCallbackWithProxy:(CGEventTapProxy)proxy
  ...
  if (![self userIsActive]) {
    ...
    DLog(@"** not doing any remapping for event %@",
...
- (CGEventRef)handleEvent:(CGEventRef)originalEvent ofType:(CGEventType)type {
  CGEventRef event = [self.remappingDelegate
      remappedEventFromEventTappedWithType:type
                                     event:originalEvent];
...
- (void)pruneReleasedObservers {
  [_observers removeObjectsPassingTest:^BOOL(
...
// Indicates if the user at the keyboard is the same user that owns this
// process. Used to avoid remapping keys when the user has switched users with
// fast user switching.
- (BOOL)userIsActive {
  ...
  if (sessionInfoDict) {
    NSNumber *userIsActiveNumber =
        CFDictionaryGetValue(sessionInfoDict, kCGSessionOnConsoleKey);
...
- (void)stopEventTap {
  ...
  CFRunLoopRemoveSource(CFRunLoopGetCurrent(), _eventSource,
...
- (BOOL)startEventTap {
  ...
  _machPort = CGEventTapCreate(
      kCGHIDEventTap, kCGTailAppendEventTap, kCGEventTapOptionDefault, _types,
      (CGEventTapCallBack)iTermEventTapCallback, (__bridge void *)self);
  if (!_machPort) {
  ...
  if (_eventSource == NULL) {
    ...
    AppendPinnedDebugLogMessage(
        @"EventTap", @"CFMachPortCreateRunLoopSource for key down failed");
    goto error;
  }

  DLog(@"Adding run loop source.");
  AppendPinnedDebugLogMessage(@"EventTap", @"Adding run loop source");
  // Get the CFRunLoop primitive for the Carbon Main Event Loop, and add the new
  // event source
  CFRunLoopAddSource(CFRunLoopGetCurrent(), _eventSource,
                     kCFRunLoopCommonModes);
...
+ (instancetype)sharedInstanceCreatingIfNeeded:(BOOL)createIfNeeded {
  ITAssertWithMessage(
      ...
      @"Don't call this off the main thread because it's not thread-safe");
...
- (instancetype)initPrivate {
  ...
  if (self) {
    ...
    [iTermFlagsChangedNotification
        subscribe:self
            block:^(iTermFlagsChangedNotification *_Nonnull notification) {
...
- (void)flagsDidChange:(iTermFlagsChangedNotification *)notification {
  if (@available(macOS 10.13, *)) {
    if (_count == 0) {
      DLog(@"Injecting flagsChanged event %@ because count is 0",
           notification.event);
