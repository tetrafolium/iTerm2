...
+ (void)openPartialAttachmentsForArrangement:(NSDictionary *)arrangement
  ...
  if (!restorationIdentifier) {
    DLog(@"PTYSession.openPartialAttachmentsForArrangement: NO, lacks server "
         @"dict\n%@",
  ...
  if (![iTermMultiServerJobManager
    ...
    DLog(@"PTYSession.openPartialAttachmentsForArrangement: NO, not "
         @"multiserver");
    completion(@{});
    return;
  }
  ...
  const char *label =
      [iTermThread uniqueQueueLabelWithName:@"com.iterm2.job-manager"]
          .UTF8String;
  dispatch_queue_t jobManagerQueue =
  ...
  DLog(@"PTYSession.openPartialAttachmentsForArrangement: request partial "
       @"attach");
  [jobManager
      asyncPartialAttachToServer:generalConnection
                   withProcessID:@(generalConnection.multi.pid)
                      completion:^(
                          id<iTermJobManagerPartialResult> partialResult) {
                        DLog(@"PTYSession.openPartialAttachmentsForArrangement:"
                             @" finished");
                        if (!partialResult) {
                          assert(NO);
                          return;
                        ...
                        iTermPartialAttachment *attachment =
                            [[iTermPartialAttachment alloc] init];
...
- (BOOL)tryToFinishAttachingToMultiserverWithPartialAttachment:
    (id<iTermPartialAttachment>)partialAttachment {
  if (!partialAttachment) {
    return NO;
  }
  return
      [self.shell finishAttachingToMultiserver:partialAttachment.partialResult
...
- (void)fetchAutoLogFilenameWithCompletion:
    (void (^)(NSString *filename))completion {
  ...
  iTermExpressionEvaluator *evaluator = [[iTermExpressionEvaluator alloc]
      initWithInterpolatedString:format
               ...
               completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                 if (evaluator.error) {
                   NSString *message = [NSString
                       stringWithFormat:@"Cannot start logging to session with "
                                        @"profile “%@”: %@",
                 ...
                 NSString *name = [evaluator.value
                     stringByReplacingOccurrencesOfString:@"/"
                                               withString:@"__"];
                 NSString *folder =
                     [iTermProfilePreferences stringForKey:KEY_LOGDIR
                                                 inProfile:self.profile];
                 NSString *filename =
                     [folder stringByAppendingPathComponent:name];
                 DLog(@"Using autolog filename %@ from format %@", filename,
                      format);
                 completion(filename);
               }];
}
...
- (void)setTermIDIfPossible {
  if (self.delegate.tabNumberForItermSessionId >= 0) {
    [self.variablesScope setValue:[self.sessionId
                                      stringByReplacingOccurrencesOfString:@":"
                                                                withString:@"."]
...
- (void)watchForPasteBracketingOopsieWithPrefix:(NSString *)prefix {
  ...
  self.pasteBracketingOopsieExpectation = [self.expect
      expectRegularExpression:[NSString
                                  stringWithFormat:@"(%@)?%@", redflag,
                                                   prefix.it_escapedForRegex]
  ...
  [self.expect setTimeout:0.5
           forExpectation:self.pasteBracketingOopsieExpectation];
}
