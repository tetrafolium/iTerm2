...
- (instancetype)initWithPath:(nullable NSString *)path
  ...
  if (path) {
    ...
    NSDictionary *knobValues =
        [mergedConfiguration[iTermStatusBarComponentConfigurationKeyKnobValues]
                ?: @{} mutableCopy];
...
- (NSArray<NSString *> *)variantsOf:(NSString *)string {
  ...
  if (parts.count > 1) {
    ...
    if ([localname length] > 1) {
      for (NSInteger numberToOmit = 2;
           numberToOmit < parts.firstObject.numberOfComposedCharacters - 2;
           numberToOmit++) {
        [result
            addObject:[parts.firstObject
                          byTruncatingComposedCharactersInCenter:numberToOmit]];
      }
...
static NSString *const iTermStatusBarHostnameComponentAbbreviateLocalhost =
...
- (instancetype)
    initWithConfiguration:
        (NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)
            configuration
                    scope:(nullable iTermVariableScope *)scope {
  return [super initWithPath:@"hostname"
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  iTermStatusBarComponentKnob *abbreviateLocalhostKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"localhost replacement"
  ...
  return [@[ abbreviateLocalhostKnob ]
      arrayByAddingObjectsFromArray:[super statusBarComponentKnobs]];
}

- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconHost"
                                forClass:[self class]];
}
...
- (nullable NSArray<NSString *> *)stringVariants {
  ...
  NSString *abbreviation = [NSString
      castFrom:knobValues[iTermStatusBarHostnameComponentAbbreviateLocalhost]];
...
- (nullable NSString *)stringByCompressingString:(NSString *)source {
  NSMutableArray<NSString *> *parts =
      [[source componentsSeparatedByString:@"."] mutableCopy];
  __block NSString *replacement = nil;
  NSUInteger index = [parts
      indexOfObjectWithOptions:NSEnumerationReverse
                   passingTest:^BOOL(NSString *_Nonnull part, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
...
- (instancetype)
    initWithConfiguration:
        (NSDictionary<iTermStatusBarComponentConfigurationKey, id> *)
            configuration
                    scope:(nullable iTermVariableScope *)scope {
  return [super initWithPath:@"username"
               configuration:configuration
                       scope:scope];
}

- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconUser"
                                forClass:[self class]];
}
...
- (NSString *)transformedValue:(NSString *)value {
  if (_home && [value hasPrefix:_home]) {
    return
        [@"~" stringByAppendingString:[value substringFromIndex:_home.length]];
  } else {
    return value;
  }
}
...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconFolder"
                                forClass:[self class]];
}
...
+ (NSDictionary *)statusBarComponentDefaultKnobs {
  ...
  return [fromSuper
      dictionaryByMergingDictionary:self.defaultMinMaxWidthKnobValues];
}
...
- (nullable NSString *)stringByCompressingString:(NSString *)source {
  ...
  __block NSString *replacement = nil;
  NSUInteger index = [parts
      indexOfObjectPassingTest:^BOOL(NSString *_Nonnull part, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
        replacement = [part firstComposedCharacter:nil];
...
- (void)openMenuWithView:(NSView *)view {
  ...
  for (iTermRecentDirectoryMO *directory in
       [[[iTermShellHistoryController sharedInstance]
           directoriesSortedByScoreOnHost:remoteHost]
           it_arrayByKeepingFirstN:10]) {
    NSString *title;
...
- (VT100RemoteHost *)remoteHost {
  ...
  result.hostname =
      [self.scope valueForVariableName:iTermVariableKeySessionHostname];
  result.username =
      [self.scope valueForVariableName:iTermVariableKeySessionUsername];
  return result;
}
