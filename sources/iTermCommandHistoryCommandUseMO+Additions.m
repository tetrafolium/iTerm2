    ...
    commandHistoryCommandUseFromDeprecatedSerialization:(id)serializedValue
                                              inContext:
                                                  (NSManagedObjectContext *)
                                                      context {
  iTermCommandHistoryCommandUseMO *managedObject =
      [self commandHistoryCommandUseInContext:context];
  if ([serializedValue isKindOfClass:[NSArray class]]) {
    managedObject.time = serializedValue[0];
    if ([serializedValue count] > 1 &&
        ![serializedValue[1] isKindOfClass:[NSNull class]]) {
      managedObject.directory = serializedValue[1];
    }
    if ([serializedValue count] > 2 &&
        ![serializedValue[2] isKindOfClass:[NSNull class]]) {
    ...
    if ([serializedValue count] > 4 &&
        ![serializedValue[4] isKindOfClass:[NSNull class]]) {
