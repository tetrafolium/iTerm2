...
#ifndef __cplusplus
int decode_utf8_char(const unsigned char *restrict datap, int datalen,
                     int *restrict result);
#endif
...
// May use single quotes by user preference. Only safe to use with user's
// default shell.
...
// Same as componentsInShellCommand but \r, \n, \t, and \a map to the letters r,
// n, t, and a, not to controls.
...
// This handles a few kinds of URLs, after trimming whitespace from the
// beginning and end:
...
//    "(http://example.com/foo?query#fragment.)" ->
//    http://example.com/foo?query#fragment
// 5. Strings wrapped by parens, square brackets, double quotes, or single
// quotes.
...
// Fonts are encoded as strings when stored in a profile. This returns the font
// for such a string.
- (NSFont *)fontValue;
...
// This is better than -precomposedStringWithCanonicalMapping because it
// preserves compatibility equivalence. It's most relevant when two canonically
// equivalent characters have different widths (one is half-width while the
// other is ambiguous width). The difference is in the following ranges:
// 2000-2FFF, F900-FAFF, 2F800-2FAFF. See issue 2872.
...
// Given a bitmask of modifiers like NSEventModifierFlagOption, return a string
// indicating those modifiers.
...
// Returns whether |self| is matched by |glob|, which is a shell-like glob
// pattern (e.g., *x or x*y). Only * is supported as a wildcard.
- (BOOL)stringMatchesGlobPattern:(NSString *)glob
                   caseSensitive:(BOOL)caseSensitive;

// Call |block| for each composed character in the string. If it is a single
// base character or a high surrogate, then |simple| will be valid and |complex|
// will be nil. Otherwise, |complex| will be non-nil.
- (void)enumerateComposedCharacters:(void (^)(NSRange range, unichar simple,
...
// Returns an array of numbers giving code points for each character in the
// string. Surrogate pairs get combined. Combining marks do not.
...
// These methods work on 10.13 with strings that include newlines, and are
// consistent with each other. The built in NSString API ignores everything from
// the first newline on for computing bounds.
...
// Invoke block for each literal and expression. In the above example there
// would be three calls: lll                                      YES
...
// If this is a 2+ part version number, return a 2 part version number.
// Otherwise, nil.
...
// Important! It assumes you put the value in double quotes. Amusingly, the man
// page for env trolls you by explaining that single-quoted values only need to
// escape ' and \ but neglects to mention that other characters simple won't
// work at all, escaped or otherwise.
...
- (NSString *)stringByReplacingCharactersAtIndices:(NSIndexSet *)indexSet
                               withStringFromBlock:
                                   (NSString * (^NS_NOESCAPE)(void))replacement;
