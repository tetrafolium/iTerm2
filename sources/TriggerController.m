...
static NSString *const kParameterColumnIdentifier =
    @"kParameterColumnIdentifier";
NSString *const kTextColorWellIdentifier = @"kTextColorWellIdentifier";
NSString *const kBackgroundColorWellIdentifier =
    @"kBackgroundColorWellIdentifier";
...
- (void)awakeFromNib {
  [_tableView
      registerForDraggedTypes:@[ kiTermTriggerControllerPasteboardType ]];
...
- (Profile *)bookmark {
  Profile *bookmark =
      [[ProfileModel sharedInstance] bookmarkWithGuid:self.guid];
  if (!bookmark) {
...
- (void)setTriggerDictionary:(NSDictionary *)triggerDictionary
  ...
  NSMutableArray *triggerDictionaries =
      [[self triggerDictionariesForCurrentProfile] mutableCopy];
  if (rowIndex < 0) {
    assert(triggerDictionary);
    [triggerDictionaries addObject:triggerDictionary];
  } else {
    if (triggerDictionary) {
      [triggerDictionaries replaceObjectAtIndex:rowIndex
                                     withObject:triggerDictionary];
    } else {
...
- (void)setGuid:(NSString *)guid {
  _guid = [guid copy];
  [_tableView reloadData];
  _interpolatedStringParameters.state =
...
- (BOOL)tableView:(NSTableView *)tableView
  ...
  NSData *data = [NSKeyedArchiver archivedDataWithRootObject:indexes
                                       requiringSecureCoding:NO
                                                       error:nil];
  [pasteboard declareTypes:@[ kiTermTriggerControllerPasteboardType ]
                     owner:self];
...
- (BOOL)tableView:(NSTableView *)aTableView
  ...
  NSData *rowData =
      [pasteboard dataForType:kiTermTriggerControllerPasteboardType];
  NSError *error = nil;
...
- (BOOL)tableView:(NSTableView *)aTableView
  ...
  if (aTableColumn == _parametersColumn) {
    NSDictionary *triggerDictionary =
        [self triggerDictionariesForCurrentProfile][rowIndex];
...
+ (NSView *)viewForParameterForTrigger:(Trigger *)trigger
                                 ...
                                 value:(id)value
                              receiver:
                                  (id<iTermTriggerParameterController>)receiver
                   interpolatedStrings:(BOOL)interpolatedStrings
                           wellFactory:(CPKColorWell * (^NS_NOESCAPE)(
                                           NSRect, NSColor *))wellFactory {
  ...
  if ([trigger paramIsTwoColorWells]) {
    ...
    CGFloat x = 4;
    NSTextField *label = [self labelWithString:@"Text:"
                                        origin:NSMakePoint(x, 0)];
  ...
  iTermFocusReportingTextField *textField =
      [[iTermFocusReportingTextField alloc]
          initWithFrame:NSMakeRect(0, 0, size.width, size.height)];
  textField.font = [NSFont systemFontOfSize:[NSFont systemFontSize]];
  ...
  if ([textField respondsToSelector:@selector(setPlaceholderString:)]) {
    textField.placeholderString =
        [trigger triggerOptionalParameterPlaceholderWithInterpolation:
                     interpolatedStrings];
  }
...
- (NSView *)tableView:(NSTableView *)tableView
    viewForTableColumn:(NSTableColumn *)tableColumn
                   row:(NSInteger)row {
  NSDictionary *triggerDictionary =
      [self triggerDictionariesForCurrentProfile][row];
  if (tableColumn == _actionColumn) {
    ...
    checkbox.title = @"";
    checkbox.state = [triggerDictionary[kTriggerPartialLineKey] boolValue]
                         ? NSControlStateValueOn
                         : NSControlStateValueOff;
    ...
    Trigger *trigger =
        [self triggerWithAction:triggerDicts[row][kTriggerActionKey]];
    trigger.param = triggerDicts[row][kTriggerParameterKey];
    NSView *result =
        [self.class viewForParameterForTrigger:trigger
                                          size:NSMakeSize(tableColumn.width,
                                                          _tableView.rowHeight)
                                         value:triggerDictionary
                                                   [kTriggerParameterKey]
                                      receiver:self
                           interpolatedStrings:_interpolatedStringParameters
                                                   .state ==
                                               NSControlStateValueOn
                                   wellFactory:^iTermColorWell *(
                                       NSRect frame, NSColor *color) {
                                     iTermColorWell *well =
                                         [[iTermColorWell alloc]
                                             initWithFrame:frame];
...
- (void)windowWillClose:(NSNotification *)notification {
  ...
  if ([[[NSColorPanel sharedColorPanel] accessoryView]
          isKindOfClass:[iTermNoColorAccessoryButton class]]) {
...
- (IBAction)addTrigger:(id)sender {
  [self setTriggerDictionary:[self defaultTriggerDictionary]
                      forRow:-1
                  reloadData:YES];
  [_tableView
          selectRowIndexes:[NSIndexSet
                               indexSetWithIndex:_tableView.numberOfRows - 1]
      byExtendingSelection:NO];
}

- (IBAction)removeTrigger:(id)sender {
  if (_tableView.selectedRow < 0) {
    XLog(@"This shouldn't happen: you pressed the button to remove a trigger "
         @"but no row is selected");
    return;
  }
  [self setTriggerDictionary:nil
                      forRow:[_tableView selectedRow]
                  reloadData:YES];
  self.hasSelection = [_tableView numberOfSelectedRows] > 0;
  _removeTriggerButton.enabled = self.hasSelection;
}
...
- (IBAction)toggleUseInterpolatedStrings:(id)sender {
  _interpolatedStringParameters.state =
      (![iTermProfilePreferences
          boolForKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS
           inProfile:[self bookmark]])
          ? NSControlStateValueOn
          : NSControlStateValueOff;
  [self.delegate
      triggerSetUseInterpolatedStrings:_interpolatedStringParameters.state ==
                                       NSControlStateValueOn];
  [_tableView reloadData];
}
...
- (void)doubleClick:(id)sender {
  ...
  NSPoint windowLocation =
      [self.window convertRectFromScreen:NSMakeRect(screenLocation.x,
                                                    screenLocation.y, 0, 0)]
          .origin;
...
- (IBAction)help:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"https://www.iterm2.com/triggers.html"]];
}
...
- (void)colorWellDidChange:(CPKColorWell *)colorWell {
  ...
  // Don't reload data. If this was called because another color picker was
  // opening, reloading the table will cause the presenting view to disappear.
  // That prevents the new popover from appearing correctly.
...
- (void)instantDidChange:(NSButton *)checkbox {
  ...
  // If a text field is editing, make it save its contents before we get the
  // trigger dictionary.
  [_tableView reloadData];
...
- (void)actionDidChange:(NSPopUpButton *)sender {
  ...
  // If a text field is being edited, end it and update the trigger dictionary
  // before we fetch it.
  [_tableView reloadData];
  ...
  [triggerDictionary removeObjectForKey:kTriggerParameterKey];
  Trigger *triggerObj =
      [self triggerWithAction:triggerDictionary[kTriggerActionKey]];
  if ([triggerObj paramIsPopupButton]) {
    triggerDictionary[kTriggerParameterKey] =
        [triggerObj defaultPopupParameterObject];
  } else if ([triggerObj triggerOptionalDefaultParameterValueWithInterpolation:
                             _interpolatedStringParameters.state ==
                             NSControlStateValueOn]) {
    triggerDictionary[kTriggerParameterKey] = [triggerObj
        triggerOptionalDefaultParameterValueWithInterpolation:
            _interpolatedStringParameters.state == NSControlStateValueOn];
  }
...
- (void)controlTextDidBeginEditing:(NSNotification *)obj {
  // We have to save this here because when -reloadData gets called then the
  // text field is no longer in the table and -rowForView: will return -1.
...
- (void)controlTextDidEndEditing:(NSNotification *)obj {
  ...
  NSMutableDictionary *triggerDictionary =
      [[self triggerDictionariesForCurrentProfile][_textEditingRow]
          mutableCopy];
  if ([textField.identifier isEqual:kRegexColumnIdentifier]) {
    triggerDictionary[kTriggerRegexKey] = [textField stringValue];
    [self setTriggerDictionary:triggerDictionary
                        forRow:_textEditingRow
                    reloadData:YES];
