...
+ (NSDictionary *)dictionaryWithGridWindowedRange:
    (VT100GridWindowedRange)range {
  return @{
    kGridCoordRange :
        [NSDictionary dictionaryWithGridCoordRange:range.coordRange],
...
+ (NSDictionary *)dictionaryWithGridAbsWindowedRange:
  ...
  return @{
    kGridCoordAbsRange :
        [NSDictionary dictionaryWithGridAbsCoordRange:absRange.coordRange],
...
+ (NSDictionary *)dictionaryWithGridRange:(VT100GridRange)range {
  return @{
    kGridRangeLocation : @(range.location),
...
- (VT100GridSize)gridSize {
  return VT100GridSizeMake([self[kGridSizeWidth] intValue],
                           [self[kGridSizeHeight] intValue]);
}
...
- (NSColor *)colorValueForKey:(NSString *)key {
  ...
  if ([colorSpace isEqualToString:kEncodedColorDictionarySRGBColorSpace]) {
    NSColor *srgb = [NSColor
        colorWithSRGBRed:[[self
                             objectForKey:kEncodedColorDictionaryRedComponent]
                             floatValue]
                   green:[[self
                             objectForKey:kEncodedColorDictionaryGreenComponent]
                             floatValue]
                    blue:[[self
                             objectForKey:kEncodedColorDictionaryBlueComponent]
                             floatValue]
                   alpha:alpha];
    return srgb;
  } else {
    return [NSColor
        colorWithCalibratedRed:
            [[self objectForKey:kEncodedColorDictionaryRedComponent] floatValue]
                         green:[[self objectForKey:
                                          kEncodedColorDictionaryGreenComponent]
                                   floatValue]
                          blue:[[self objectForKey:
...
- (NSData *)propertyListData {
  NSString *filename = [[NSWorkspace sharedWorkspace]
      temporaryFileNameWithPrefix:@"DictionaryPropertyList"
                           suffix:@"iTerm2"];
...
- (NSString *)sizeInfo {
  ...
  [sizes enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
...
- (NSDictionary *)filteredWithBlock:(BOOL (^NS_NOESCAPE)(id key,
                                                         id value))block {
...
- (NSDictionary<id, NSDictionary *> *)classifyWithBlock:
    (id (^NS_NOESCAPE)(id key, id object))block {
  NSMutableDictionary<id, NSMutableDictionary *> *result =
      [NSMutableDictionary dictionary];
  [self enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                            BOOL *_Nonnull stop) {
...
- (BOOL)it_writeToXMLPropertyListAt:(NSString *)filename {
  NSOutputStream *outputStream =
      [NSOutputStream outputStreamToFileAtPath:filename append:NO];
...
- (NSEventModifierFlags)hotKeyModifiers {
  return
      [self[kHotKeyModifiers] unsignedIntegerValue] & iTermHotkeyModifierMask;
}
...
- (NSDictionary *)dictionaryByMergingDictionary:(NSDictionary *)other {
  NSMutableDictionary *temp = [self mutableCopy];
  [other enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                             BOOL *_Nonnull stop) {
    temp[key] = obj;
  }];
  return temp;
