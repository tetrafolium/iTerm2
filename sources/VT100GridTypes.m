...
const VT100GridCoord VT100GridCoordInvalid = {.x = INT_MIN, .y = INT_MIN};
...
NSString *VT100GridCoordRangeDescription(VT100GridCoordRange range) {
  return [NSString stringWithFormat:@"((%d, %d) to (%d, %d))", range.start.x,
                                    range.start.y, range.end.x, range.end.y];
}

NSString *VT100GridWindowedRangeDescription(VT100GridWindowedRange range) {
  return [NSString
      stringWithFormat:@"<%@ restricted to cols [%d, %d]>",
...
NSString *VT100GridAbsCoordRangeDescription(VT100GridAbsCoordRange range) {
  return
      [NSString stringWithFormat:@"<(%d, %lld) to (%d, %lld)>", range.start.x,
                                 range.start.y, range.end.x, range.end.y];
}
...
NSString *
VT100GridAbsWindowedRangeDescription(VT100GridAbsWindowedRange range) {
  return [NSString
      stringWithFormat:@"<%@ restricted to cols %@>",
...
+ (NSValue *)valueWithGridCoord:(VT100GridCoord)coord {
  return [[[NSValue alloc] initWithBytes:&coord
                                objCType:@encode(VT100GridCoord)] autorelease];
}

+ (NSValue *)valueWithGridAbsCoord:(VT100GridAbsCoord)absCoord {
  return
      [[[NSValue alloc] initWithBytes:&absCoord
                             objCType:@encode(VT100GridAbsCoord)] autorelease];
}

+ (NSValue *)valueWithGridSize:(VT100GridSize)size {
  return [[[NSValue alloc] initWithBytes:&size
                                objCType:@encode(VT100GridSize)] autorelease];
}

+ (NSValue *)valueWithGridRange:(VT100GridRange)range {
  return [[[NSValue alloc] initWithBytes:&range
                                objCType:@encode(VT100GridRange)] autorelease];
}

+ (NSValue *)valueWithGridRect:(VT100GridRect)rect {
  return [[[NSValue alloc] initWithBytes:&rect
                                objCType:@encode(VT100GridRect)] autorelease];
}

+ (NSValue *)valueWithGridRun:(VT100GridRun)run {
  return [[[NSValue alloc] initWithBytes:&run
                                objCType:@encode(VT100GridRun)] autorelease];
}

+ (NSValue *)valueWithGridCoordRange:(VT100GridCoordRange)coordRange {
  return [[[NSValue alloc] initWithBytes:&coordRange
                                objCType:@encode(VT100GridCoordRange)]
      autorelease];
}

+ (NSValue *)valueWithGridAbsCoordRange:(VT100GridAbsCoordRange)absCoordRange {
  return [[[NSValue alloc] initWithBytes:&absCoordRange
                                objCType:@encode(VT100GridAbsCoordRange)]
      autorelease];
}
...
- (NSComparisonResult)compareGridAbsCoordRangeStart:(NSValue *)other {
  return VT100GridAbsCoordOrder([self gridAbsCoordRangeValue].start,
                                [other gridAbsCoordRangeValue].start);
}
