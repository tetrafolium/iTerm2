...
@interface ProfilesTextPreferencesViewController () <BFPCompositeViewDelegate,
                                                     BFPSizePickerViewDelegate>
...
- (void)awakeFromNib {
  ...
  _heightWithoutNonAsciiControls = _heightWithNonAsciiControls -
                                   _nonAsciiFontView.frame.size.height -
                                   _nonAsciiFontView.frame.origin.y;
  ...
  [self defineControl:_cursorType
      ...
      update:^BOOL {
        ...
        [strongSelf->_cursorType
            selectCellWithTag:[self intForKey:KEY_CURSOR_TYPE]];
        return YES;
      }];
  ...
  PreferenceInfo *info = [self defineControl:_asciiLigatures
  ...
  info.customSettingChangedHandler = ^(id sender) {
    ...
    if (isOn) {
      static NSString *const kWarnAboutAmbiguousWidth =
          @"NoSyncWarnAboutAmbiguousWidth";
      // This is a feature of dubious value inherited from iTerm 0.1. Some users
      // who work in mixed Asian/non-asian environments find it useful but
      // almost nobody should turn it on unless they really know what they're
      // doing.
      iTermWarningSelection selection = [iTermWarning
          showWarningWithTitle:@"You probably don't want to turn this on. "
  ...
  [self defineControl:_unicodeVersion9
      ...
      settingChanged:^(id sender) {
        ...
        const NSInteger version =
            (strongSelf->_unicodeVersion9.state == NSControlStateValueOn) ? 9
                                                                          : 8;
      ...
      update:^BOOL {
        ...
        strongSelf->_unicodeVersion9.state =
            [strongSelf integerForKey:KEY_UNICODE_VERSION] == 9
  ...
  _horizontalSpacingView =
      [_asciiFontPicker addHorizontalSpacingAccessoryWithInitialValue:
                            [self doubleForKey:KEY_HORIZONTAL_SPACING] * 100];
  _horizontalSpacingView.delegate = self;
  [_horizontalSpacingView clampWithMin:50 max:200];
  _verticalSpacingView =
      [_asciiFontPicker addVerticalSpacingAccessoryWithInitialValue:
                            [self doubleForKey:KEY_VERTICAL_SPACING] * 100];
  ...
  [self defineControl:_asciiFontPicker.horizontalSpacing.textField
      ...
      update:^BOOL {
        ...
        strongSelf->_asciiFontPicker.horizontalSpacing.size =
            [self doubleForKey:KEY_HORIZONTAL_SPACING] * 100;
        return YES;
      }
      searchable:YES];
  ...
  [self defineControl:_asciiFontPicker.verticalSpacing.textField
      ...
      update:^BOOL {
        ...
        strongSelf->_asciiFontPicker.verticalSpacing.size =
            [self doubleForKey:KEY_VERTICAL_SPACING] * 100;
        return YES;
      }
      searchable:YES];
  ...
  info = [self defineControl:_useNonAsciiFont
                 ...
                 relatedView:nil
                        type:kPreferenceInfoTypeCheckbox];
  info.observer = ^{
    [weakSelf updateNonAsciiFontViewVisibility];
  };
...
- (void)updateLigatureWarning {
  const BOOL enabled = ([self boolForKey:KEY_ASCII_LIGATURES] ||
                        [self boolForKey:KEY_NON_ASCII_LIGATURES]);
...
- (void)updateNonAsciiFontViewVisibility {
  ...
  ((iTermSizeRememberingView *)self.view).originalSize =
      self.myPreferredContentSize;
  [self.delegate
      profilePreferencesContentViewSizeDidChange:(iTermSizeRememberingView *)
                                                     self.view];
}
...
- (NSSize)myPreferredContentSize {
  if ([self boolForKey:KEY_USE_NONASCII_FONT]) {
    return NSMakeSize(
        ((iTermSizeRememberingView *)self.view).originalSize.width,
        _heightWithNonAsciiControls);
  } else {
    return NSMakeSize(
        ((iTermSizeRememberingView *)self.view).originalSize.width,
        _heightWithoutNonAsciiControls);
  }
}
...
- (void)updateFontsDescriptionsIncludingSpacing:(BOOL)includingSpacing {
  ...
  const double horizontalSpacing =
      round([self doubleForKey:KEY_HORIZONTAL_SPACING] * 100);
  _asciiFontPicker.horizontalSpacing.size = horizontalSpacing;
  const double verticalSpacing =
      round([self doubleForKey:KEY_VERTICAL_SPACING] * 100);
  ...
  if (self.normalFont.it_defaultLigatures) {
    ...
    _asciiLigatures.state = [self boolForKey:KEY_ASCII_LIGATURES]
                                ? NSControlStateValueOn
                                : NSControlStateValueOff;
  ...
  if (self.nonAsciiFont.it_defaultLigatures) {
    ...
    _nonAsciiLigatures.state = [self boolForKey:KEY_NON_ASCII_LIGATURES]
                                   ? NSControlStateValueOn
                                   : NSControlStateValueOff;
...
- (void)updateWarnings {
  [_normalFontWantsAntialiasing
      setHidden:!self.normalFont.futureShouldAntialias];
  [_nonasciiFontWantsAntialiasing
      setHidden:!self.nonAsciiFont.futureShouldAntialias];
}
...
- (IBAction)didToggleSubpixelAntiAliasing:(id)sender {
  ...
  alert.informativeText = @"This change will affect all profiles. You must "
                          @"restart iTerm2 for this change to take effect.";
  [alert runModal];
  [self updateWarnings];
}
...
- (void)fontPickerCompositeView:(BFPCompositeView *)view
                  didSelectFont:(NSFont *)font {
  NSString *key;
  ...
  [self setString:view.font.stringValue forKey:key];
  [self updateFontsDescriptionsIncludingSpacing:YES];
}

#pragma mark - BFPSizePickerViewDelegate

- (void)sizePickerView:(BFPSizePickerView *)sizePickerView
       didChangeSizeTo:(double)size {
...
- (void)saveChangesFromFontPicker {
  ...
  DLog(@"Save changes from font picker. asciiFontPicker=%@ "
       @"asciiFontPicker.font=%@ asciiFontPicker.font.stringValue=%@",
       _asciiFontPicker, _asciiFontPicker.font,
       _asciiFontPicker.font.stringValue);
  DLog(@"Save changes from font picker. nonASCIIFontPicker=%@ "
       @"asciiFontPicker.font=%@ asciiFontPicker.font.stringValue=%@",
       _nonASCIIFontPicker, _nonASCIIFontPicker.font,
       _nonASCIIFontPicker.font.stringValue);
  NSDictionary *dictionaryWithNulls = @{
