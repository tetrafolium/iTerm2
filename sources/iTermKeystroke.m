...
- (instancetype)initWithString:(NSString *)string {
  {
    ...
    if (sscanf(string.UTF8String, "%x-%llx-%x", &character, &flags,
               &virtualKeyCode) == 3) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p char=%@ (%C) flags=0x%llx>",
                                    ...
                                    @(self.character), (unichar)self.character,
...
- (NSString *)legacySerialized {
  return [NSString
      stringWithFormat:@"0x%x-0x%x", self.character, (int)self.modifierFlags];
}
- (NSString *)serialized {
  return [NSString stringWithFormat:@"0x%x-0x%llx-0x%x", self.character,
                                    (unsigned long long)self.modifierFlags,
                                    self.virtualKeyCode];
}

- (NSString *)keyInBindingDictionary:
...
- (NSDictionary *)valueInBindingDictionary:
    (NSDictionary<NSString *, NSDictionary *> *)dict {
...
- (BOOL)isEqual:(id)object {
  ...
  return self.character == other.character &&
         self.modifierFlags == other.modifierFlags;
}
...
- (NSDictionary<iTermKeystroke *, id> *)it_withDeserializedKeystrokeKeys {
  return [self
      mapKeysWithBlock:^iTermKeystroke *(NSString *serialized, id object) {
...
- (NSDictionary *)it_dictionaryByMergingSerializedKeystrokeKeyedDictionary:
    (NSDictionary *)other {
  NSMutableDictionary *temp = [self mutableCopy];

  [other enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull serialized,
                                             id _Nonnull obj,
                                             BOOL *_Nonnull stop) {
...
- (NSComparisonResult)compareSerializedKeystroke:(NSString *)other {
  iTermKeystroke *lhsKeystroke =
      [[iTermKeystroke alloc] initWithSerialized:self];
  iTermKeystroke *rhsKeystroke =
      [[iTermKeystroke alloc] initWithSerialized:other];
  iTermTouchbarItem *lhsTouchbar =
      [[iTermTouchbarItem alloc] initWithIdentifier:[NSString castFrom:self]];
  iTermTouchbarItem *rhsTouchbar =
      [[iTermTouchbarItem alloc] initWithIdentifier:[NSString castFrom:other]];
