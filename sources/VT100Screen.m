...
NSString *const kScreenStateLastCommandOutputRangeKey =
    @"Last Command Output Range";
NSString *const kScreenStateShellIntegrationInstalledKey =
...
@interface VT100Screen () <iTermTemporaryDoubleBufferedGridControllerDelegate,
                           iTermLineBufferDelegate, iTermMarkDelegate,
...
@implementation VT100Screen {
  ...
  // BOOLs indicating, for each of the characters sets, which ones are in
  // line-drawing mode.
  ...
  VT100Grid
      *currentGrid_; // Weak reference. Points to either primaryGrid or altGrid.
  VT100Grid *realCurrentGrid_; // When a saved grid is swapped in, this is the
                               // live current grid.
  ...
  // How many scrollback lines have been lost due to overflow. Periodically
  // reset with -resetScrollbackOverflow.
  int scrollbackOverflow_;

  // A rarely reset count of the number of lines lost to scrollback overflow.
  // Adding this to a line number gives a unique line number that won't be
  // reused when the linebuffer overflows.
  long long cumulativeScrollbackOverflow_;

  // When set, strings, newlines, and linefeeds are appended to printBuffer_.
  // When ANSICSI_PRINT with code 4 is received, it's sent for printing.
  ...
  // Holds notes on alt/primary grid (the one we're not in). The origin is the
  ...
  // to an id<IntervalTreeObject>, which is either PTYNoteViewController or
  // VT100ScreenMark.
  IntervalTree *intervalTree_;

  NSMutableDictionary
      *markCache_; // Maps an absolute line number to a VT100ScreenMark.
  VT100GridCoordRange markCacheRange_;

  // Location of the start of the current command, or -1 for none. Y is
  // absolute.
  int commandStartX_;
  ...
  BOOL _cursorVisible;
  // Line numbers containing animated GIFs that need to be redrawn for the next
  // frame.
...
@synthesize saveToScrollbackInAlternateScreen =
    saveToScrollbackInAlternateScreen_;
@synthesize dvr = dvr_;
...
- (instancetype)initWithTerminal:(VT100Terminal *)terminal {
  ...
  if (self) {
    ...
    primaryGrid_ =
        [[VT100Grid alloc] initWithSize:VT100GridSizeMake(kDefaultScreenColumns,
                                                          kDefaultScreenRows)
                               delegate:self];
    currentGrid_ = primaryGrid_;
    _temporaryDoubleBuffer =
        [[iTermTemporaryDoubleBufferedGridController alloc] init];
    ...
    dvr_ = [DVR alloc];
    [dvr_ initWithBufferCapacity:
              [iTermPreferences
                  intForKey:kPreferenceKeyInstantReplayMemoryMegabytes] *
              1024 * 1024];
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p grid:%@>", [self class], self, currentGrid_];
}
...
// This is used for a very specific case. It's used when you have some history,
// optionally followed by lines pulled from the primary grid, followed by the
// alternate grid, all stuffed into a line buffer. Given a pair of positions, it
// converts them to a range. If a position is between originalLastPos and
// newLastPos, it's invalid. Likewise, if a position is in the first
// |linesMovedUp| lines of the screen, it's invalid.
// NOTE: This assumes that linebuffer_ contains the history plus lines from the
// primary grid. Returns YES if the range is valid, NO if it could not be
// converted (e.g., because it was entirely in the area of dropped lines).
/*
 * 0 History      } } 1 History      } These lines were in history before
 * resizing began                   } 2 History      }                    <-
 * originalLimit                                 } equal to linebuffer_ 3 Line
 * from primary grid            <- limit (pushed into history due to resize) }
 * 4 Line to be lost from alt grid     <- linesMovedUp = 1 because this one line
 * will be lost 5 Line from alt grid                } 6 Line from alt grid }
 * These lines will be restored to the alt grid later
 */
- (BOOL)computeRangeFromOriginalLimit:(LineBufferPosition *)originalLimit
                        ...
                        startPosition:(LineBufferPosition *)startPos
                          endPosition:(LineBufferPosition *)endPos
                           ...
                           lineBuffer:(LineBuffer *)
                                          lineBuffer // NOTE: May be append-only
                                range:(VT100GridCoordRange *)resultRangePtr
                         linesMovedUp:(int)linesMovedUp {
  BOOL result = YES;
  // Compute selection positions relative to the end of the line buffer, which
  ...
  if (growth > 0) {
    ...
    if (startPos.absolutePosition >= originalLimit.absolutePosition) {
      // Start position was on alt screen originally. Move it down by the number
      // of lines pulled in from the primary screen.
      startPos.absolutePosition += growth;
    }
    if (endPos.absolutePosition >= originalLimit.absolutePosition) {
      // End position was on alt screen originally. Move it down by the number
      // of lines pulled in from the primary screen.
    ...
    /*
        ...
        |                    | | into primary from  | |- growth = -(number of
    ...
    if (startPos.absolutePosition >= limit.absolutePosition &&
      ...
      // Started in history in the region pulled into primary screen. Advance
      // start to new beginning of alt screen
      startPos = limit;
    } else if (startPos.absolutePosition >= originalLimit.absolutePosition) {
      // Starts after deleted region. Move start position up by number of
      // deleted lines so it refers to the same cell.
      startPos.absolutePosition += growth;
    }
    if (endPos.absolutePosition >= predecessorOfLimit.absolutePosition &&
        endPos.absolutePosition < originalLimit.absolutePosition) {
      // Ended in deleted region. Move end point to just before current alt
      // screen.
  ...
  // |linesMovedUp| wrapped lines will not be restored into the alt grid later
  // on starting at |limit|
  if (resultRangePtr->start.y >= numScrollbackLines) {
    if (resultRangePtr->start.y < numScrollbackLines + linesMovedUp) {
      ...
      // The selection starts on screen, so move it up by the number of lines by
      // which the alt screen shifted up.
  ...
  if (resultRangePtr->end.y >= numScrollbackLines) {
    if (resultRangePtr->end.y < numScrollbackLines + linesMovedUp) {
      // The selection ends in one of the lines that was lost. The whole
      // selection is gone.
      result = NO;
    } else {
      // The selection ends on screen, so move it up by the number of lines by
      // which the alt screen shifted up.
...
- (NSSize)viewSize {
  ...
  return NSMakeSize(cellSize.width * gridSize.width,
...
- (BOOL)shouldSetSizeTo:(VT100GridSize)size {
  ...
  if (currentGrid_.size.width == 0 || currentGrid_.size.height == 0 ||
...
- (NSArray *)subSelectionTuplesWithUsedHeight:(int)usedHeight
  ...
  LineBuffer *lineBufferWithAltScreen =
      [[linebuffer_ newAppendOnlyCopy] autorelease];
  ...
  for (iTermSubSelection *sub in selection.allSubSelections) {
    VT100GridAbsCoordRangeTryMakeRelative(
        sub.absRange.coordRange, self.totalScrollbackOverflow,
...
- (NSArray *)intervalTreeObjectsWithUsedHeight:(int)usedHeight
  ...
  // Add notes that were on the alt grid to altScreenNotes, leaving notes in
  // history alone.
  VT100GridCoordRange screenCoordRange =
      VT100GridCoordRangeMake(0, [self numberOfScrollbackLines], 0,
                              [self numberOfScrollbackLines] + self.height);
  NSArray *notesAtLeastPartiallyOnScreen = [intervalTree_
      objectsInInterval:[self intervalForGridCoordRange:screenCoordRange]];

  LineBuffer *appendOnlyLineBuffer =
      [[realLineBuffer newAppendOnlyCopy] autorelease];
  ...
  for (id<IntervalTreeObject> note in notesAtLeastPartiallyOnScreen) {
    VT100GridCoordRange range =
        [self coordRangeForInterval:note.entry.interval];
    [[note retain] autorelease];
    [intervalTree_ removeObject:note];
    LineBufferPositionRange *positionRange = [self
        positionRangeForCoordRange:range
    ...
    if (positionRange) {
      DLog(@"Add note on alt screen at %@ (position %@ to %@) to triples",
           VT100GridCoordRangeDescription(range), positionRange.start,
           positionRange.end);
      [triples addObject:@[ note, positionRange.start, positionRange.end ]];
    } else {
      DLog(@"Failed to get position range while in alt screen for note %@ with "
           @"range %@",
...
- (NSArray *)subSelectionsWithConvertedRangesFromSelection:
  ...
  for (iTermSubSelection *sub in selection.allSubSelections) {
    ...
    VT100GridAbsCoordRangeTryMakeRelative(
        sub.absRange.coordRange, overflow, ^(VT100GridCoordRange range) {
          ...
          if (ok) {
            ...
            const VT100GridWindowedRange relativeRange =
                VT100GridWindowedRangeMake(newSelection, 0, 0);
            const VT100GridAbsWindowedRange absRange =
                VT100GridAbsWindowedRangeFromWindowedRange(relativeRange,
                                                           overflow);
...
- (IntervalTree *)replacementIntervalTreeForNewWidth:(int)newWidth {
  // Convert ranges of notes to their new coordinates and replace the interval
  // tree.
    ...
    if (noteRange.end.x < 0 && noteRange.start.y == 0 && noteRange.end.y < 0) {
      ...
      if ([self convertRange:noteRange
        ...
        Interval *newInterval =
            [self intervalForGridCoordRange:newRange
                                      width:newWidth
  ...
  for (int i = 0; i < altScreenSubSelectionTuples.count; i++) {
    ...
    if (ok) {
      const VT100GridAbsWindowedRange theRange = VT100GridAbsWindowedRangeMake(
          VT100GridAbsCoordRangeFromCoordRange(newSelection,
...
- (void)restorePrimaryGridWithLineBuffer:(LineBuffer *)realLineBuffer
  ...
  [primaryGrid_
      setCharsFrom:VT100GridCoordMake(0, 0)
  ...
  // at bottom instead). Note there's a little hack here: we use
  // saved_primary_buffer as the default line because it was just initialized
  // with default lines.
  //
  // If the height decreased or stayed the same:
  // Shrinking (avoid pulling in stuff from scrollback, pull in no more
  // than might have been pushed, even if more is available). Note there's a
  // little hack here: we use saved_primary_buffer as the default line because
  // it was just initialized with default lines.
  [primaryGrid_
      restoreScreenFromLineBuffer:realLineBuffer
                  withDefaultChar:[primaryGrid_ defaultChar]
                maxLinesToRestore:MIN(oldSize.height, newSize.height)];
}

- (NSArray *)
    subSelectionsAfterRestoringPrimaryGridWithCopyOfAltGrid:
        (VT100Grid *)copyOfAltGrid
                                               linesMovedUp:(int)linesMovedUp
                                               toLineBuffer:
                                                   (LineBuffer *)realLineBuffer
                                         subSelectionTuples:
                                             (NSArray *)
                                                 altScreenSubSelectionTuples
                                       originalLastPosition:
                                           (LineBufferPosition *)originalLastPos
                                                    oldSize:
                                                        (VT100GridSize)oldSize
                                                    newSize:
                                                        (VT100GridSize)newSize
                                                 usedHeight:(int)usedHeight
                                        intervalTreeObjects:
                                            (NSArray *)altScreenNotes {
  ...
  NSArray *newSubSelections =
      [self subSelectionsForNewSize:newSize
                         lineBuffer:realLineBuffer
                               grid:copyOfAltGrid
...
- (void)updateAlternateScreenIntervalTreeForNewSize:(VT100GridSize)newSize {
  ...
  [self appendScreen:altGrid_
      ...
      withUsedHeight:[altGrid_ numberOfLinesUsed]
           newHeight:newSize.height];
  int numLinesThatWillBeRestored = MIN(
      [altScreenLineBuffer numLinesWithWidth:newSize.width], newSize.height);
  int numLinesDroppedFromTop =
      [altScreenLineBuffer numLinesWithWidth:newSize.width] -
      numLinesThatWillBeRestored;

  // Convert note ranges to new coords, dropping or truncating as needed
  currentGrid_ = altGrid_; // Swap to alt grid temporarily for
                           // convertRange:toWidth:to:inLineBuffer:
  ...
  for (PTYNoteViewController *note in [savedIntervalTree_ allObjects]) {
    ...
    if ([self convertRange:noteRange
      ...
      assert(noteRange.start.y >= 0);
      assert(noteRange.end.y >= 0);
      // Anticipate the lines that will be dropped when the alt grid is
      // restored.
      newRange.start.y +=
          [self totalScrollbackOverflow] - numLinesDroppedFromTop;
      newRange.end.y += [self totalScrollbackOverflow] - numLinesDroppedFromTop;
      if (newRange.start.y < 0) {
        newRange.start.y = 0;
        newRange.start.x = 0;
      }
      DLog(@"  Its new range is %@ including %d lines dropped from top",
           VT100GridCoordRangeDescription(noteRange), numLinesDroppedFromTop);
  ...
  [altGrid_ restoreScreenFromLineBuffer:altScreenLineBuffer
                      ...
                      maxLinesToRestore:[altScreenLineBuffer
                                            numLinesWithWidth:currentGrid_.size
                                                                  .width]];
...
- (void)didResizeToSize:(VT100GridSize)newSize
     ...
     couldHaveSelection:(BOOL)couldHaveSelection
          subSelections:(NSArray *)newSubSelections
                 newTop:(int)newTop {
  [terminal_ clampSavedCursorToScreenSize:VT100GridSizeMake(newSize.width,
                                                            newSize.height)];
  ...
  int lines __attribute__((unused)) =
      [linebuffer_ numLinesWithWidth:currentGrid_.size.width];
...
- (LineBuffer *)
    prepareToResizeInAlternateScreenMode:
        (NSArray **)altScreenSubSelectionTuplesPtr
                     intervalTreeObjects:(NSArray **)altScreenNotesPtr
                            hasSelection:(BOOL)couldHaveSelection
                              ...
                              usedHeight:(int)usedHeight
                                 newSize:(VT100GridSize)newSize {
  if (couldHaveSelection) {
    *altScreenSubSelectionTuplesPtr =
        [self subSelectionTuplesWithUsedHeight:usedHeight
                                     newHeight:newSize.height
                                     selection:selection];
  }
  ...
  if ([intervalTree_ count]) {
    *altScreenNotesPtr =
        [self intervalTreeObjectsWithUsedHeight:usedHeight
                                      newHeight:newSize.height
                                           grid:altGrid_
                                     lineBuffer:realLineBuffer];
  }
  ...
  currentGrid_ = primaryGrid_;
  // Move savedIntervalTree_ into intervalTree_. This should leave
  // savedIntervalTree_ empty.
  [self swapNotes];
  currentGrid_ = altGrid_;
...
- (void)fixUpPrimaryGridIntervalTreeForNewSize:(VT100GridSize)newSize
  ...
  if ([intervalTree_ count]) {
    ...
    if (wasShowingAltScreen) {
      // Temporarily swap in primary grid so convertRange: will do the right
      // thing.
      currentGrid_ = primaryGrid_;
    }
    ...
    intervalTree_ =
        [[self replacementIntervalTreeForNewWidth:newSize.width] retain];
  ...
  if (gDebugLogging) {
    ...
    for (PTYNoteViewController *note in intervalTree_.allObjects) {
      ...
      DLog(@"Note has coord range %@",
           VT100GridCoordRangeDescription(
               [self coordRangeForInterval:note.entry.interval]));
    }
    DLog(@"------------ end -----------");
  }
...
- (void)reallySetSize:(VT100GridSize)newSize {
  ...
  const VT100GridRange previouslyVisibleLineRange =
      [self.delegate screenRangeOfVisibleLines];
  const VT100GridCoordRange previouslyVisibleLines =
      VT100GridCoordRangeMake(0, previouslyVisibleLineRange.location, 0,
  ...
  const BOOL couldHaveSelection =
      [delegate_ screenHasView] && selection.hasSelection;
  ...
  if (wasShowingAltScreen) {
    altScreenLineBuffer =
        [self prepareToResizeInAlternateScreenMode:&altScreenSubSelectionTuples
  ...
  const BOOL rangeOfVisibleLinesConvertedCorrectly =
      [self convertRange:previouslyVisibleLines
                 toWidth:newSize.width
  ...
  if (!wasShowingAltScreen && couldHaveSelection) {
    newSubSelections =
        [self subSelectionsWithConvertedRangesFromSelection:selection
                                                   newWidth:newSize.width];
  }
  ...
  [currentGrid_
      restoreScreenFromLineBuffer:wasShowingAltScreen ? altScreenLineBuffer
                                                      : linebuffer_
                  withDefaultChar:[currentGrid_ defaultChar]
                maxLinesToRestore:[wasShowingAltScreen ? altScreenLineBuffer
                                                       : linebuffer_
                                      numLinesWithWidth:currentGrid_.size
                                                            .width]];
  ...
  if (wasShowingAltScreen) {
    ...
    // linebuffer.
    // In alternate screen mode, the screen contents move up when the screen
    // gets smaller. For example, if your alt screen looks like this before:
    //   abcd
    //   ef..
    // And then gets shrunk to 3 wide, it becomes
    //   d..
    //   ef.
    // The "abc" line was lost, so "linesMovedUp" is 1. That's the number of
    // lines at the top of the alt screen that were lost.
  ...
  const int newTop = rangeOfVisibleLinesConvertedCorrectly
                         ? convertedRangeOfVisibleLines.start.y
                         : -1;
...
- (void)sanityCheckIntervalsFrom:(VT100GridSize)oldSize note:(NSString *)note {
  ...
  for (id<IntervalTreeObject> obj in [intervalTree_ allObjects]) {
    ...
    ITBetaAssert(interval.limit >= 0,
                 @"Bogus interval %@ after resizing from %@ to %@. Note: %@",
                 interval, VT100GridSizeDescription(oldSize),
                 VT100GridSizeDescription(currentGrid_.size), note);
  }
#endif
}
...
- (void)reloadMarkCache {
  ...
  for (id<IntervalTreeObject> obj in [intervalTree_ allObjects]) {
    if ([obj isKindOfClass:[VT100ScreenMark class]]) {
      VT100GridCoordRange range =
          [self coordRangeForInterval:obj.entry.interval];
...
- (void)clearBuffer {
  ...
  if (commandStartX_ >= 0) {
    ...
    if (!VT100GridAbsCoordEquals(_currentPromptRange.start,
                                 _currentPromptRange.end)) {
      numberOfPromptLines =
          MAX(1, _currentPromptRange.end.y - _currentPromptRange.start.y + 1);
    }
    newCommandStart =
        VT100GridCoordMake(commandStartX_, numberOfPromptLines - 1);
  ...
  if (newCommandStart.x >= 0) {
    // Create a new mark and inform the delegate that there's new command start
    // coord.
...
// This clears the screen, leaving the cursor's line at the top and preserves
// the cursor's x coordinate. Scroll regions and the saved cursor position are
// reset.
- (void)clearAndResetScreenSavingLines:(int)linesToSave {
  [delegate_ screenTriggerableChangeDidOccur];
  // This clears the screen.
  int x = currentGrid_.cursorX;
  [self incrementOverflowBy:[currentGrid_
                                  resetWithLineBuffer:linebuffer_
...
- (int)numberOfLinesToPreserveWhenClearingScreen {
  ...
  VT100GridCoordRange lastCommandMarkRange =
      [self coordRangeForInterval:lastCommandMark.entry.interval];
...
- (void)appendLinesMatchingQuery:(NSString *)query
  ...
  const int numPushed =
      [source.currentGrid appendLines:[source.currentGrid numberOfLinesUsed]
  ...
  while (context.status == Searching || context.status == Matched) {
    ...
    case Matched: {
      NSArray *positions = [source->linebuffer_ convertPositions:context.results
...
- (void)appendScreenChars:(screen_char_t *)line
  ...
  [self appendScreenCharArrayAtCursor:line length:length shouldFree:NO];
...
- (void)appendAsciiDataAtCursor:(AsciiData *)asciiData {
  ...
  DLog(@"appendAsciiDataAtCursor: %ld chars starting with %c at x=%d, y=%d, "
       @"line=%d",
       (unsigned long)len, firstChar, currentGrid_.cursorX,
  ...
  [self appendScreenCharArrayAtCursor:buffer length:len shouldFree:NO];
...
- (void)appendStringAtCursor:(NSString *)string {
  ...
  DLog(@"appendStringAtCursor: %ld chars starting with %c at x=%d, y=%d, "
       @"line=%d",
       (unsigned long)len, firstChar, currentGrid_.cursorX,
       currentGrid_.cursorY,
       currentGrid_.cursorY +
  ...
  if (3 * len >= kStaticBufferElements) {
    buffer = dynamicBuffer =
        (screen_char_t *)iTermCalloc(3 * len, sizeof(screen_char_t));
    assert(buffer);
    if (!buffer) {
  ...
  VT100GridCoord pred =
      [currentGrid_ coordinateBefore:currentGrid_.cursor
            movedBackOverDoubleWidth:&predecessorIsDoubleWidth];
  NSString *augmentedString = string;
  NSString *predecessorString =
      pred.x >= 0 ? [currentGrid_ stringForCharacterAt:pred] : nil;
  const BOOL augmented = predecessorString != nil;
  if (augmented) {
    augmentedString = [predecessorString stringByAppendingString:string];
  } else {
    // Prepend a space so we can detect if the first character is a combining
    // mark.
    augmentedString = [@" " stringByAppendingString:string];
  }

  assert(terminal_);
  // Add DWC_RIGHT after each double-byte character, build complex characters
  // out of surrogates and combining marks, replace private codes with
  // replacement characters, swallow zero- width spaces, and set fg/bg colors
  // and attributes.
  BOOL dwc = NO;
  StringToScreenChars(augmentedString, buffer, [terminal_ foregroundColorCode],
                      [terminal_ backgroundColorCode], &len,
                      [delegate_ screenShouldTreatAmbiguousCharsAsDoubleWidth],
                      NULL, &dwc, _normalization,
  ...
  if (augmented && len > 0) {
    ...
    // We infer that the first character in |string| was not a combining mark.
    // If it were, it would have combined with the space we added to the start
    // of |augmentedString|. Skip past the space.
    bufferOffset++;
  }
...
- (void)appendScreenCharArrayAtCursor:(screen_char_t *)buffer
  ...
  if (len >= 1) {
    ...
    if (lastCharacter.code == DWC_RIGHT && !lastCharacter.complexChar) {
      // Last character is the right half of a double-width character. Use the
      // penultimate character instead.
      if (len >= 2) {
    ...
    if (currentGrid_ != altGrid_ || saveToScrollbackInAlternateScreen_) {
      // Not in alt screen or it's ok to scroll into line buffer while in alt
      // screen.k
      lineBuffer = linebuffer_;
    }
    [self incrementOverflowBy:
              [currentGrid_ appendCharsAtCursor:buffer
                                         length:len
...
- (void)linefeed {
  ...
  const BOOL noScrollback =
      (currentGrid_ == altGrid_ && !saveToScrollbackInAlternateScreen_);
  if (noScrollback) {
    // In alt grid but saving to scrollback in alt-screen is off, so pass in a
    // nil linebuffer.
    lineBufferToUse = nil;
  }
  [self
      incrementOverflowBy:
          [currentGrid_
              moveCursorDownOneLineScrollingIntoLineBuffer:lineBufferToUse
                                       unlimitedScrollback:unlimitedScrollback_
                                   useScrollbackWithRegion:
                                       _appendToScrollbackWithStatusBar
                                                willScroll:^{
                                                  if (noScrollback) {
                                                    // This is a temporary hack.
                                                    // In this case, keeping the
                                                    // selection in the right
                                                    // place requires more
                                                    // cooperation between
                                                    // VT100Screen and
                                                    // PTYTextView than is
                                                    ...
                                                    // and regardless, will need
                                                    // to move up a line in
                                                    // terms of absolute Y
                                                    // position (normally when
                                                    // the screen scrolls the
                                                    // absolute Y position of
                                                    // the selection stays the
                                                    // same and the viewport
                                                    // moves down, or else there
                                                    // is some scrollback
                                                    // overflow and PTYTextView
                                                    // -refresh bumps the
                                                    // selection's Y position,
                                                    // but because in this case
                                                    // we don't append to the
                                                    // line buffer, scrollback
                                                    // overflow will not
                                                    // increment).
                                                    [delegate_
                                                        screenRemoveSelection];
                                                  }
                                                }]];
}
...
- (void)activateBell {
  if ([delegate_ screenShouldIgnoreBellWhichIsAudible:audibleBell_
                                              visible:flashBell_]) {
    return;
  }
...
- (void)setHistory:(NSArray *)history {
  // This is way more complicated than it should be to work around something
  // dumb in tmux. It pads lines in its history with trailing spaces, which we'd
  // like to trim. More importantly, we need to trim empty lines at the end of
  // the history because that breaks how we move the screen contents around on
  // resize. So we take the history from tmux, append it to a temporary line
  // buffer, grab each wrapped line and trim spaces from it, and then append
  // those modified line (excluding empty ones at the end) to the real line
  // buffer.
  [self clearBuffer];
  ...
  [currentGrid_
      restoreScreenFromLineBuffer:linebuffer_
                  withDefaultChar:[currentGrid_ defaultChar]
                maxLinesToRestore:MIN([linebuffer_
                                          numLinesWithWidth:currentGrid_.size
                                                                .width],
...
- (void)setAltScreen:(NSArray *)lines {
  ...
  [altGrid_ setCharsFrom:VT100GridCoordMake(0, 0)
                      to:VT100GridCoordMake(altGrid_.size.width - 1,
                                            altGrid_.size.height - 1)
                  toChar:[altGrid_ defaultChar]];
  // Copy the lines back over it
  int o = 0;
  for (int i = 0;
       o < altGrid_.size.height && i < MIN(lines.count, altGrid_.size.height);
       i++) {
    ...
    do {
      // Add up to altGrid_.size.width characters at a time until they're all
      // used.
      screen_char_t *dest = [altGrid_ screenCharsAtLineNumber:o];
      memcpy(dest, line,
             MIN(altGrid_.size.width, length) * sizeof(screen_char_t));
      const BOOL isPartial = (length > altGrid_.size.width);
      dest[altGrid_.size.width] =
          dest[altGrid_.size.width - 1]; // TODO: This is probably wrong?
...
- (void)setTmuxState:(NSDictionary *)state {
  BOOL inAltScreen = [[self
      objectInDictionary:state
        withFirstKeyFrom:[NSArray arrayWithObjects:kStateDictSavedGrid,
                                                   kStateDictSavedGrid, nil]]
      intValue];
  if (inAltScreen) {
  ...
  if (altSavedX && altSavedY && inAltScreen) {
    primaryGrid_.cursor =
        VT100GridCoordMake([altSavedX intValue], [altSavedY intValue]);
...
- (void)highlightTextInRange:(NSRange)range
  ...
  VT100GridRun gridRun = [currentGrid_ gridRunFromRange:range
                                          relativeToRow:lineNumber];
  if (gridRun.length > 0) {
    NSColor *foreground = colors[kHighlightForegroundColor];
    NSColor *background = colors[kHighlightBackgroundColor];
    [self highlightRun:gridRun
...
- (void)restoreSavedPositionToFindContext:(FindContext *)context {
  ...
  [linebuffer_ storeLocationOfAbsPos:savedFindContextAbsPos_ inContext:context];
...
// This is a wee hack until PTYTextView breaks its direct dependence on
// PTYSession
- (PTYSession *)session {
...
// theIndex = 0 for first line in history; for sufficiently large values, it
// pulls from the current grid.
- (screen_char_t *)getLineAtIndex:(int)theIndex
  ...
  if (theIndex >= numLinesInLineBuffer) {
    ...
    return [currentGrid_
        screenCharsAtLineNumber:(theIndex - numLinesInLineBuffer)];
  } else {
    ...
    int cont = [linebuffer_ copyLineToBuffer:buffer
                                     ...
                                     lineNum:theIndex
                                continuation:&continuation];
    if (cont == EOL_SOFT && theIndex == numLinesInLineBuffer - 1 &&
      ...
      // if the first char on the screen is double-width and the buffer is
      // soft-wrapped without a last char.
      cont = EOL_DWC;
    }
    if (cont == EOL_DWC) {
...
- (NSArray<ScreenCharArray *> *)gridLinesInRange:(const NSRange)range {
  ...
  for (NSInteger i = range.location; i < NSMaxRange(range); i++) {
    screen_char_t *line =
        [currentGrid_ screenCharsAtLineNumber:i - numLinesInLineBuffer];
...
- (NSArray<ScreenCharArray *> *)historyLinesInRange:(const NSRange)range {
  return [linebuffer_ wrappedLinesFromIndex:range.location
                                      width:currentGrid_.size.width
                                      count:range.length];
}

- (NSArray<ScreenCharArray *> *)linesInRange:(NSRange)range {
  const int numLinesInLineBuffer =
      [linebuffer_ numLinesWithWidth:currentGrid_.size.width];
  const NSRange gridRange =
      NSMakeRange(numLinesInLineBuffer, currentGrid_.size.height);
...
- (long long)absoluteLineNumberOfCursor {
  return [self totalScrollbackOverflow] + [self numberOfLines] - [self height] +
         currentGrid_.cursorY;
}
...
- (BOOL)continueFindAllResults:(NSMutableArray<SearchResult *> *)results
  ...
  BOOL keepSearching;
  do {
    keepSearching = [self continueFindResultsInContext:context toArray:results];
  } while (keepSearching &&
...
- (void)setFindString:(NSString *)aString
  ...
  // Append the screen contents to the scrollback buffer so they are included in
  // the search.
  ...
  if (!startPos) {
    // x,y wasn't a real position in the line buffer, probably a null after the
    // end.
    if (direction) {
    ...
    BOOL ok;
    VT100GridCoord startPosCoord =
        [linebuffer_ coordinateForPosition:startPos
                                     width:currentGrid_.size.width
                              extendsRight:YES
                                        ok:&ok];
    LineBufferPosition *lastValidPosition =
        [[linebuffer_ lastPosition] predecessor];
    if (!ok) {
      startPos = lastValidPosition;
    } else {
      VT100GridCoord lastPositionCoord =
          [linebuffer_ coordinateForPosition:lastValidPosition
                                       width:currentGrid_.size.width
                                extendsRight:YES
                                          ok:&ok];
      assert(ok);
  ...
  [linebuffer_ prepareToSearchFor:aString
                       startingAt:startPos
                          options:opts
                             mode:mode
                      withContext:context];
...
- (NSString *)compactLineDumpWithHistory {
  NSMutableString *string = [NSMutableString
      stringWithString:[linebuffer_ compactLineDumpWithWidth:[self width]
...
- (NSString *)compactLineDumpWithHistoryAndContinuationMarksAndLineNumbers {
  ...
  for (int i = 0; i < lines.count; i++) {
    lines[i] = [NSString
        stringWithFormat:@"%8lld:        %@", absoluteLineNumber++, lines[i]];
  }
  ...
  for (int i = 0; i < gridLines.count; i++) {
    [lines addObject:[NSString stringWithFormat:@"%8lld (%04d): %@",
...
- (void)setCharDirtyAtCursorX:(int)x Y:(int)y {
  ...
  int xToMark = x;
  int yToMark = y;
  if (xToMark == currentGrid_.size.width &&
      yToMark < currentGrid_.size.height - 1) {
    xToMark = 0;
    yToMark++;
  }
...
- (void)saveToDvr:(NSIndexSet *)cleanLines {
  ...
  [dvr_ appendFrame:[currentGrid_ orderedLines]
             length:sizeof(screen_char_t) * (currentGrid_.size.width + 1) *
                    (currentGrid_.size.height)cleanLines:cleanLines
               info:&info];
}
...
- (NSDate *)timestampForLine:(int)y {
  ...
  if (y >= numLinesInLineBuffer) {
    ...
    interval = [linebuffer_ timestampForLineNumber:y
                                             width:currentGrid_.size.width];
  }
...
- (NSInteger)generationForLine:(int)y {
  ...
  if (y >= numLinesInLineBuffer) {
    ...
    return [linebuffer_ generationForLineNumber:y
                                          width:currentGrid_.size.width];
  }
}
...
- (void)setWorkingDirectory:(NSString *)workingDirectory
  ...
  [self setWorkingDirectory:workingDirectory
                     onLine:line
                     pushed:pushed
                      token:[[_setWorkingDirectoryOrderEnforcer newToken]
                                autorelease]];
}
...
//
// pushed means it's a higher confidence update. The directory must be pushed to
// be remote, but that alone is not sufficient evidence that it is remote.
// Pushed directories will update the recently used directories and will change
// the current remote host to the remote host on `line`.
- (void)setWorkingDirectory:(NSString *)workingDirectory
                     onLine:(int)line
                     pushed:(BOOL)pushed
                      token:(id<iTermOrderedToken>)token {
  // If not timely, record the update but don't consider it the latest update.
  // Peek now so we can log but don't commit because we might recurse
  // asynchronously.
  const BOOL timely = !token || [token peek];
  DLog(@"%p: setWorkingDirectory:%@ onLine:%d token:%@ (timely=%@)", self,
       workingDirectory, line, token, @(timely));
  VT100WorkingDirectory *workingDirectoryObj =
      [[[VT100WorkingDirectory alloc] init] autorelease];
  if (token && !workingDirectory) {
    __weak __typeof(self) weakSelf = self;
    DLog(@"%p: Performing async working directory fetch for token %@", self,
         token);
    [delegate_ screenGetWorkingDirectoryWithCompletion:^(NSString *path) {
      DLog(@"%p: Async update got %@ for token %@", self, path, token);
      if (path) {
        [weakSelf setWorkingDirectory:path
                               onLine:line
                               pushed:pushed
                                token:token];
      }
  ...
  if (workingDirectory.length) {
    ...
    VT100WorkingDirectory *previousWorkingDirectory =
    ...
    if ([previousWorkingDirectory.workingDirectory
      ...
      // Extend the previous working directory. We used to add a new
      // VT100WorkingDirectory every time but if the window title gets changed a
      // lot then they can pile up really quickly and you spend all your time
      // searching through VT001WorkingDirectory marks just to find
      // VT100RemoteHost or VT100ScreenMark objects.
      //
      // It's a little weird that a VT100WorkingDirectory can now represent the
      // same path on two different hosts (e.g., you ssh from /Users/georgen to
      // another host and you're in /Users/georgen over there, but you can share
      // the same VT100WorkingDirectory between the two hosts because the path
      // is the same). I can't see the harm in it besides being odd.
      //
      // Intervals aren't removed while part of them is on screen, so this works
      // fine.
      VT100GridCoordRange range =
          [self coordRangeForInterval:previousWorkingDirectory.entry.interval];
      [intervalTree_ removeObject:previousWorkingDirectory];
      range.end = VT100GridCoordMake(self.width, line);
      DLog(@"Extending the previous directory to %@",
           VT100GridCoordRangeDescription(range));
      ...
      VT100GridCoordRange range;
      range =
          VT100GridCoordRangeMake(currentGrid_.cursorX, line, self.width, line);
      DLog(@"Set range of %@ to %@", workingDirectory,
           VT100GridCoordRangeDescription(range));
...
- (VT100RemoteHost *)setRemoteHost:(NSString *)host
                              user:(NSString *)user
                            onLine:(int)line {
  ...
  VT100GridCoordRange range =
      VT100GridCoordRangeMake(0, line, self.width, line);
...
- (id)objectOnOrBeforeLine:(int)line ofClass:(Class)cls {
  long long pos = [self intervalForGridCoordRange:VT100GridCoordRangeMake(
                                                      0, line + 1, 0, line + 1)]
                      .location;
  if (pos < 0) {
    return nil;
...
- (VT100RemoteHost *)remoteHostOnLine:(int)line {
  return (VT100RemoteHost *)[self objectOnOrBeforeLine:line
                                               ofClass:[VT100RemoteHost class]];
}
...
- (void)addNote:(PTYNoteViewController *)note
  ...
  [intervalTree_ addObject:note
              withInterval:[self intervalForGridCoordRange:range]];
  [currentGrid_ markAllCharsDirty:YES];
  note.delegate = self;
  [delegate_ screenDidAddNote:note];
  [self.intervalTreeObserver
      intervalTreeDidAddObjectOfType:iTermIntervalTreeObjectTypeAnnotation
                              onLine:range.start.y +
                                     self.totalScrollbackOverflow];
}

- (iTermIntervalTreeObjectType)intervalTreeObserverTypeForObject:
    (id<IntervalTreeObject>)object {
...
- (void)removeInaccessibleNotes {
  long long lastDeadLocation =
      [self totalScrollbackOverflow] * (self.width + 1);
  long long totalScrollbackOverflow = [self totalScrollbackOverflow];
  if (lastDeadLocation > 0) {
    Interval *deadInterval =
        [Interval intervalWithLocation:0 length:lastDeadLocation + 1];
    for (id<IntervalTreeObject> obj in
         [intervalTree_ objectsInInterval:deadInterval]) {
      if ([obj.entry.interval limit] <= lastDeadLocation) {
        ...
        iTermIntervalTreeObjectType type =
            [self intervalTreeObserverTypeForObject:obj];
        if (type != iTermIntervalTreeObjectTypeUnknown) {
          VT100GridCoordRange range =
              [self coordRangeForInterval:obj.entry.interval];
          [_intervalTreeObserver
              intervalTreeDidRemoveObjectOfType:type
                                         onLine:range.start.y +
                                                self.totalScrollbackOverflow];
        }
      }
...
- (id<iTermMark>)addMarkStartingAtAbsoluteLine:(long long)line
  ...
  VT100GridCoordRange range;
  if (oneLine) {
    range = VT100GridCoordRangeMake(0, nonAbsoluteLine, self.width,
                                    nonAbsoluteLine);
  } else {
    // Interval is whole screen
    int limit = nonAbsoluteLine + self.height - 1;
    if (limit >=
        [self numberOfScrollbackLines] + [currentGrid_ numberOfLinesUsed]) {
      limit =
          [self numberOfScrollbackLines] + [currentGrid_ numberOfLinesUsed] - 1;
    }
    range = VT100GridCoordRangeMake(0, nonAbsoluteLine, self.width, limit);
  }
  if ([mark isKindOfClass:[VT100ScreenMark class]]) {
    markCache_[@([self totalScrollbackOverflow] + range.end.y)] = mark;
  }
  [intervalTree_ addObject:mark
              withInterval:[self intervalForGridCoordRange:range]];
  [_intervalTreeObserver
      intervalTreeDidAddObjectOfType:[self
                                         intervalTreeObserverTypeForObject:mark]
                              onLine:range.start.y +
                                     self.totalScrollbackOverflow];
  [delegate_ screenNeedsRedraw];
  return mark;
...
- (NSArray *)charactersWithNotesOnLine:(int)line {
  ...
  Interval *interval = [self
      intervalForGridCoordRange:VT100GridCoordRangeMake(0, line, 0, line + 1)];
...
- (void)enumerateObservableMarks:
    (void (^NS_NOESCAPE)(iTermIntervalTreeObjectType, NSInteger))block {
  ...
  for (NSArray *objects in intervalTree_.forwardLimitEnumerator) {
    for (id<IntervalTreeObject> obj in objects) {
      const iTermIntervalTreeObjectType type =
          [self intervalTreeObserverTypeForObject:obj];
      if (type == iTermIntervalTreeObjectTypeUnknown) {
        continue;
      }
      NSInteger line =
          [self coordRangeForInterval:obj.entry.interval].start.y + overflow;
      block(type, line);
    }
  }
}
...
- (NSArray *)lastMarksOrNotes {
  NSEnumerator *enumerator = [intervalTree_ reverseLimitEnumerator];
  return [self firstMarkBelongingToAnyClassIn:@[
    [PTYNoteViewController class], [VT100ScreenMark class]
  ]
...
- (long long)lineNumberOfMarkBeforeAbsLine:(long long)absLine {
  ...
  Interval *interval = [self
      intervalForGridCoordRange:VT100GridCoordRangeMake(0, line, 0, line)];
  NSEnumerator *enumerator =
      [intervalTree_ reverseLimitEnumeratorAt:interval.limit];
  NSArray *objects = [enumerator nextObject];
  while (objects) {
    for (id object in objects) {
      if ([object isKindOfClass:[VT100ScreenMark class]]) {
        ...
        return overflow +
               [self coordRangeForInterval:mark.entry.interval].start.y;
      }
    }
    objects = [enumerator nextObject];
  }
...
- (long long)lineNumberOfMarkAfterAbsLine:(long long)absLine {
  ...
  Interval *interval =
      [self intervalForGridCoordRange:VT100GridCoordRangeMake(0, line + 1, 0,
                                                              line + 1)];
  NSEnumerator *enumerator =
      [intervalTree_ forwardLimitEnumeratorAt:interval.limit];
  NSArray *objects = [enumerator nextObject];
  while (objects) {
    for (id object in objects) {
      if ([object isKindOfClass:[VT100ScreenMark class]]) {
        VT100ScreenMark *mark = object;
        return overflow +
               [self coordRangeForInterval:mark.entry.interval].end.y;
      }
    }
    objects = [enumerator nextObject];
  }
...
- (NSArray *)marksOfAnyClassIn:(NSArray<Class> *)allowedClasses
  ...
  NSEnumerator *enumerator =
      [intervalTree_ reverseLimitEnumeratorAt:location.limit];
...
- (NSArray *)marksOfAnyClassIn:(NSArray<Class> *)allowedClasses
  ...
  NSEnumerator *enumerator =
      [intervalTree_ forwardLimitEnumeratorAt:location.limit];
  return [self firstObjectsFoundWithEnumerator:enumerator
                                  ofAnyClassIn:allowedClasses];
}
...
- (VT100GridCoordRange)textViewRangeOfOutputForCommandMark:
  ...
  NSEnumerator *enumerator =
      [intervalTree_ forwardLimitEnumeratorAt:mark.entry.interval.limit];
  NSArray *objects;
  do {
    objects = [enumerator nextObject];
  ...
  range.end.x = 0;
  range.end.y =
      self.numberOfLines - self.height + [currentGrid_ numberOfLinesUsed];
  return range;
}

- (PTYTextViewSynchronousUpdateState *)setUseSavedGridIfAvailable:
    (BOOL)useSavedGrid {
  if (useSavedGrid && !realCurrentGrid_ &&
      self.temporaryDoubleBuffer.savedState) {
...
- (iTermStringLine *)
    stringLineAsStringAtAbsoluteLineNumber:(long long)absoluteLineNumber
  ...
  *startAbsLineNumber = self.totalScrollbackOverflow;
  ...
  BOOL done = NO;
  for (i = lineNumber;
       !done && i < self.numberOfLines && i < lineNumber + kMaxRadius; i++) {
    screen_char_t *line = [self getLineAtIndex:i];
    int length = self.width;
    done = line[length].code == EOL_HARD;
    if (done) {
      // Remove trailing newlines
      while (length > 0 && line[length - 1].code == 0 &&
             !line[length - 1].complexChar) {
        --length;
      }
    }
  ...
  return [[[iTermStringLine alloc]
      initWithScreenChars:data.mutableBytes
...
- (void)terminalAppendString:(NSString *)string {
  ...
  [delegate_ screenDidAppendStringToCurrentLine:string isPlainText:YES];
}
...
- (void)doBackspace {
  ...
  if (cursorX >= self.width && terminal_.reverseWraparoundMode &&
  ...
  } else if (cursorX > leftMargin || // Cursor can move back without hitting the
                                     // left margin: normal case
             (cursorX < leftMargin &&
              cursorX > 0)) { // Cursor left of left margin, right of left edge.
  ...
  // It is OK to land on the right half of a double-width character (issue
  // 3475).
}

// Reverse wrap is allowed when the cursor is on the left margin or left edge,
// wraparoundMode is set, the cursor is not at the top margin/edge, and:
// 1. reverseWraparoundMode is set (xterm's rule), or
// 2. there's no left-right margin and the preceding line has EOL_SOFT
// (Terminal.app's rule)
- (BOOL)shouldReverseWrap {
  ...
  // If reverseWraparoundMode is reset, then allow only if there's a soft
  // newline on previous line
...
- (void)terminalBackspace {
  ...
  if (commandStartX_ != -1 &&
      (currentGrid_.cursorX != cursorX || currentGrid_.cursorY != cursorY)) {
    [delegate_ screenCommandDidChangeWithRange:[self commandRange]];
  }
}
...
- (void)softWrapCursorToNextLineScrollingIfNeeded {
  ...
  if (currentGrid_.cursorY == currentGrid_.bottomMargin) {
    [self incrementOverflowBy:
              [currentGrid_
                   scrollUpIntoLineBuffer:linebuffer_
                      unlimitedScrollback:unlimitedScrollback_
                  useScrollbackWithRegion:_appendToScrollbackWithStatusBar
                                softBreak:YES]];
  }
...
- (void)terminalAppendTabAtCursor:(BOOL)setBackgroundColors {
  ...
  if (terminal_.moreFix && self.cursorX > self.width &&
      terminal_.wraparoundMode) {
  ...
  int nextTabStop =
      MIN(rightMargin, [self tabStopAfterColumn:currentGrid_.cursorX]);
  ...
  screen_char_t *aLine =
      [currentGrid_ screenCharsAtLineNumber:currentGrid_.cursorY];
  BOOL allNulls = YES;
  ...
  if (allNulls) {
    ...
    InitializeScreenChar(&filler, [terminal_ foregroundColorCode],
...
- (void)terminalLineFeed {
  if (currentGrid_.cursor.y ==
          VT100GridRangeMax(currentGrid_.scrollRegionRows) &&
      [self cursorOutsideLeftRightMargin]) {
    DLog(@"Ignore linefeed/formfeed/index because cursor outside left-right "
         @"margin.");
    return;
  }
...
- (void)terminalSetScrollRegionTop:(int)top bottom:(int)bottom {
  if (top >= 0 && top < currentGrid_.size.height && bottom >= 0 &&
      bottom < currentGrid_.size.height && bottom > top) {
    currentGrid_.scrollRegionRows = VT100GridRangeMake(top, bottom - top + 1);

    if ([terminal_ originMode]) {
      currentGrid_.cursor =
          VT100GridCoordMake(currentGrid_.leftMargin, currentGrid_.topMargin);
    } else {
      currentGrid_.cursor = VT100GridCoordMake(0, 0);
    }
  }
...
- (void)scrollScreenIntoHistory {
  // Scroll the top lines of the screen into history, up to and including the
  // last non- empty line.
  LineBuffer *lineBuffer;
  ...
  const int n =
      [currentGrid_ numberOfNonEmptyLinesIncludingWhitespaceAsEmpty:YES];
  for (int i = 0; i < n; i++) {
    [self incrementOverflowBy:
              [currentGrid_
                  scrollWholeScreenUpIntoLineBuffer:lineBuffer
                                unlimitedScrollback:unlimitedScrollback_]];
  }
}

// Remove soft eol on previous line, provided the cursor is on the first column.
// This is useful because zsh likes to ED 0 after wrapping around before drawing
// the prompt. See issue 8938. For consistency, EL uses it, too.
- (void)removeSoftEOLBeforeCursor {
  ...
  if (currentGrid_.cursor.y > 0) {
    [currentGrid_ setContinuationMarkOnLine:currentGrid_.cursor.y - 1
                                         to:EOL_HARD];
  } else {
...
- (void)terminalEraseInDisplayBeforeCursor:(BOOL)before
                               afterCursor:(BOOL)after {
  int x1, yStart, x2, y2;
  ...
  if (before && after) {
    ...
    y2 = currentGrid_.size.height - 1;
    if (x1 == 0 && yStart == 0 &&
        [iTermAdvancedSettingsModel saveScrollBufferWhenClearing] &&
        self.terminal.softAlternateScreenMode) {
      // Save the whole screen. This helps the "screen" terminal, where CSI H
      // CSI J is used to clear the screen. Only do it in alternate screen mode
      // to avoid doing this for zsh (issue 8822)
      [delegate_ screenRemoveSelection];
      [self scrollScreenIntoHistory];
    }
  } else {
  ...
  [currentGrid_ setCharsInRun:theRun toChar:0];
  [delegate_ screenTriggerableChangeDidOccur];
}
...
- (void)terminalEraseLineBeforeCursor:(BOOL)before afterCursor:(BOOL)after {
  ...
  VT100GridRun theRun = VT100GridRunFromCoords(
      VT100GridCoordMake(x1, currentGrid_.cursor.y),
      VT100GridCoordMake(x2, currentGrid_.cursor.y), currentGrid_.size.width);
  [currentGrid_ setCharsInRun:theRun toChar:0];
  [delegate_ screenTriggerableChangeDidOccur];
}
...
- (void)terminalSetLeftMargin:(int)scrollLeft rightMargin:(int)scrollRight {
  if (currentGrid_.useScrollRegionCols) {
    currentGrid_.scrollRegionCols =
        VT100GridRangeMake(scrollLeft, scrollRight - scrollLeft + 1);
    // set cursor to the home position
    [self cursorToX:1 Y:1];
  }
}

- (void)terminalSetCharset:(int)charset
         toLineDrawingMode:(BOOL)lineDrawingMode {
...
- (void)terminalSetWidth:(int)width {
  if ([delegate_ screenShouldInitiateWindowResize] &&
    ...
    // set the column
    [delegate_ screenResizeToWidth:width height:currentGrid_.size.height];
    [self terminalEraseInDisplayBeforeCursor:YES
                                 afterCursor:YES]; // erase the screen
    currentGrid_.cursorX = 0;
...
- (void)terminalBackTab:(int)n {
  for (int i = 0; i < n; i++) {
    ...
    if (currentGrid_.cursorX > 0) {
      ...
      while (![self haveTabStopAt:currentGrid_.cursorX] &&
             currentGrid_.cursorX > 0) {
        currentGrid_.cursorX = currentGrid_.cursorX - 1;
      }
      [delegate_ screenTriggerableChangeDidOccur];
...
- (void)terminalEraseCharactersAfterCursor:(int)j {
  if (currentGrid_.cursorX < currentGrid_.size.width) {
    ...
    [currentGrid_
        setCharsFrom:VT100GridCoordMake(currentGrid_.cursorX,
                                        currentGrid_.cursorY)
                  to:VT100GridCoordMake(limit - 1, currentGrid_.cursorY)
              toChar:[currentGrid_ defaultChar]];
    // TODO: This used to always set the continuation mark to hard, but I think
    // it should only do that if the last char in the line is erased.
    [delegate_ screenTriggerableChangeDidOccur];
  }
}
...
- (void)terminalSetWindowTitle:(NSString *)title {
  ...
  // If you know to use RemoteHost then assume you also use CurrentDirectory.
  // Innocent window title changes shouldn't override CurrentDirectory.
  if (![self remoteHostOnLine:[self numberOfScrollbackLines] + self.height]) {
    ...
    // TODO: There's a bug here where remote host can scroll off the end of
    // history, causing the working directory to come from PTYTask (which is
    // what happens when nil is passed here).
    //
    ...
    DLog(@"Already have a remote host so not updating working directory "
         @"because of title change");
  }
}
...
- (void)terminalInsertBlankLinesAfterCursor:(int)n {
  ...
  if (scrollRegionRect.origin.x + scrollRegionRect.size.width ==
      currentGrid_.size.width) {
    // Cursor can be in right margin and still be considered in the scroll
    // region if the scroll region abuts the right margin.
    scrollRegionRect.size.width++;
  }
  BOOL cursorInScrollRegion =
      VT100GridCoordInRect(currentGrid_.cursor, scrollRegionRect);
...
- (void)terminalDeleteLinesAtCursor:(int)n {
  ...
  if (cursorInScrollRegion) {
    [currentGrid_
        scrollRect:VT100GridRectMake(
                       currentGrid_.leftMargin, currentGrid_.cursorY,
...
- (BOOL)terminalPostUserNotification:(NSString *)message {
  if (postUserNotifications_ &&
      [delegate_ screenShouldPostTerminalGeneratedAlert]) {
    [delegate_ screenIncrementBadge];
    NSString *description = [NSString
        stringWithFormat:@"Session %@ #%d: %@", [delegate_ screenName],
                         [delegate_ screenNumber], message];
...
- (void)moveNotesOnScreenFrom:(IntervalTree *)source
  ...
  VT100GridCoordRange screenRange = VT100GridCoordRangeMake(
      0, screenOrigin, [self width], screenOrigin + self.height);
  DLog(@"  moveNotes: looking in range %@",
       VT100GridCoordRangeDescription(screenRange));
  Interval *sourceInterval = [self intervalForGridCoordRange:screenRange];
  self.lastCommandMark = nil;
  for (id<IntervalTreeObject> obj in
       [source objectsInInterval:sourceInterval]) {
...
- (void)swapNotes {
  ...
  Interval *origin =
      [self intervalForGridCoordRange:VT100GridCoordRangeMake(0, historyLines,
                                                              1, historyLines)];
...
- (void)hideOnScreenNotesAndTruncateSpanners {
  ...
  for (id<IntervalTreeObject> note in
       [intervalTree_ objectsInInterval:screenInterval]) {
    if (note.entry.interval.location < screenInterval.location) {
      // Truncate note so that it ends just before screen.
      note.entry.interval.length =
          screenInterval.location - note.entry.interval.location;
    }
    if ([note isKindOfClass:[PTYNoteViewController class]]) {
...
- (void)terminalSetRemoteHost:(NSString *)remoteHost {
  ...
  // Search backwards because Windows UPN format includes an @ in the user name.
  // I don't think hostnames would ever have an @ sign.
...
- (void)setHost:(NSString *)host user:(NSString *)user {
  ...
  if (!host || !user) {
    // A trigger can set the host and user alone. If remoteHost looks like
    // example.com or user@, then preserve the previous host/user. Also ensure
    // neither value is nil; the empty string will stand in for a real value if
    // necessary.
  ...
  VT100RemoteHost *remoteHostObj = [self setRemoteHost:host
                                                  user:user
                                                onLine:cursorLine];
...
- (void)terminalSetWorkingDirectoryURL:(NSString *)URLString {
  ...
  NSURLComponents *components =
      [[[NSURLComponents alloc] initWithURL:URL
                    resolvingAgainstBaseURL:NO] autorelease];
...
- (void)terminalClearScreen {
  ...
  // This behavior changed in the Great VT100Grid Refactoring of 2013. Before,
  // clearScreen used to move the cursor's wrapped line to the top of the
  // screen. It's only used from DECSET 1049, and neither xterm nor terminal
  // have this behavior, and I'm not sure why it would be desirable anyway. Like
  // xterm (and unlike Terminal) we leave the cursor put.
  [delegate_ screenRemoveSelection];
  [currentGrid_ setCharsFrom:VT100GridCoordMake(0, 0)
                          to:VT100GridCoordMake(currentGrid_.size.width - 1,
                                                currentGrid_.size.height - 1)
...
- (void)terminalSaveScrollPositionWithArgument:(NSString *)argument {
  // The difference between an argument of saveScrollPosition and saveCursorLine
  // (the default) is subtle. When saving the scroll position, the entire region
  // of visible lines is recorded and will be restored exactly. When saving only
  // the line the cursor is on, when restored, that line will be made visible
  // but no other aspect of the scroll position must be restored. This is often
  // preferable because when setting a mark as part of the prompt, we wouldn't
  // want the prompt to be the last line on the screen (such lines are scrolled
  // to the center of the screen).
  if ([argument isEqualToString:@"saveScrollPosition"]) {
    [delegate_ screenSaveScrollPosition];
  } else { // implicitly "saveCursorLine"
    [delegate_
        screenAddMarkOnLine:[self numberOfScrollbackLines] + self.cursorY - 1];
  }
}
...
- (void)terminalCurrentDirectoryDidChangeTo:(NSString *)dir {
  ...
  [delegate_
      screenSetPreferredProxyIcon:nil]; // Clear current proxy icon if exists.
  ...
  id<iTermOrderedToken> token =
      [[_currentDirectoryDidChangeOrderEnforcer newToken] autorelease];
...
- (void)currentDirectoryReallyDidChangeTo:(NSString *)dir
  ...
  BOOL willChange =
      ![dir isEqualToString:[self workingDirectoryOnLine:cursorLine]];
...
- (void)terminalAddNote:(NSString *)value show:(BOOL)show {
  ...
  if (length > 0 && message.length > 0 && endVal <= maxVal) {
    ...
    [note sizeToFit];
    [self addNote:note
          inRange:VT100GridCoordRangeMake(
                      location.x, location.y + [self numberOfScrollbackLines],
                      end.x, end.y + [self numberOfScrollbackLines])];
    if (!show) {
...
- (BOOL)terminalWillReceiveFileNamed:(NSString *)name ofSize:(NSInteger)size {
  ...
  [delegate_ screenWillReceiveFileNamed:name
                                 ofSize:size
...
- (BOOL)terminalWillReceiveInlineFileNamed:(NSString *)name
  ...
  [_inlineImageHelper release];
  _inlineImageHelper = [[VT100InlineImageHelper alloc]
             initWithName:name
                    width:width
...
- (void)appendNativeImageAtCursorWithName:(NSString *)name width:(int)width {
  VT100InlineImageHelper *helper = [[[VT100InlineImageHelper alloc]
      initWithNativeImageNamed:name
                 spanningWidth:width
                   scaleFactor:[delegate_ screenBackingScaleFactor]]
      autorelease];
...
- (void)addURLMarkAtLineAfterCursorWithCode:(unsigned short)code {
  long long absLine =
      (self.totalScrollbackOverflow + [self numberOfScrollbackLines] +
...
- (void)terminalDidChangeSendModifiers {
  // CSI u is too different from xterm's modifyOtherKeys to allow the terminal
  // to change it with xterm's control sequences. Lots of strange problems
  // appear with vim. For example, mailing list thread with subject "Control
  // Keys Failing After System Bell".
  // TODO: terminal_.sendModifiers[i] holds the settings. See xterm's
  // modifyOtherKeys and friends.
...
- (BOOL)confirmBigDownloadWithBeforeSize:(NSInteger)sizeBefore
                               afterSize:(NSInteger)afterSize
                                    name:(NSString *)name {
  if (sizeBefore < VT100ScreenBigFileDownloadThreshold &&
      afterSize > VT100ScreenBigFileDownloadThreshold) {
    if (![self.delegate
            screenConfirmDownloadNamed:name
...
- (void)terminalPromptDidStart {
  [self
      promptDidStartAt:VT100GridAbsCoordMake(currentGrid_.cursor.x,
...
- (void)promptDidStartAt:(VT100GridAbsCoord)coord {
  ...
  // FinalTerm uses this to define the start of a collapsible region. That would
  // be a nightmare to add to iTerm, and our answer to this is marks, which
...
- (void)terminalCommandDidStart {
  ...
  _currentPromptRange.end = VT100GridAbsCoordMake(
      currentGrid_.cursor.x, currentGrid_.cursor.y +
                                 self.numberOfScrollbackLines +
                                 self.totalScrollbackOverflow);
  [self commandDidStartAtScreenCoord:currentGrid_.cursor];
  [delegate_ screenPromptDidEndAtLine:[self numberOfScrollbackLines] +
                                      self.cursorY - 1];
}

- (void)commandDidStartAtScreenCoord:(VT100GridCoord)coord {
  [self
      commandDidStartAt:VT100GridAbsCoordMake(
                            coord.x, coord.y + [self numberOfScrollbackLines] +
                                         [self totalScrollbackOverflow])];
}
...
- (void)terminalCommandDidEnd {
  ...
  _currentPromptRange.start = _currentPromptRange.end =
      VT100GridAbsCoordMake(0, 0);

  [self commandDidEndAtAbsCoord:VT100GridAbsCoordMake(
                                    currentGrid_.cursor.x,
                                    currentGrid_.cursor.y +
                                        [self numberOfScrollbackLines] +
                                        [self totalScrollbackOverflow])];
}
...
- (void)commandWasAborted {
  ...
  if (screenMark) {
    ...
    [_intervalTreeObserver
        intervalTreeDidRemoveObjectOfType:
            [self intervalTreeObserverTypeForObject:screenMark]
                                   onLine:[self coordRangeForInterval:
                                                    screenMark.entry.interval]
                                              .start.y +
                                          self.totalScrollbackOverflow];
  ...
  commandStartX_ = commandStartY_ = -1;
  [delegate_
      screenCommandDidEndWithRange:VT100GridCoordRangeMake(-1, -1, -1, -1)];
}
      ...
      if ([obj isKindOfClass:[VT100ScreenMark class]]) {
        ...
        if (mark.command) {
          DLog(@"Found mark %@ in line number range %@", mark,
               VT100GridRangeDescription(
                   [self lineNumberRangeOfInterval:obj.entry.interval]));
...
- (void)terminalReturnCodeOfLastCommandWas:(int)returnCode {
  ...
  if (mark) {
    ...
    const NSInteger line =
        [self coordRangeForInterval:mark.entry.interval].start.y +
        self.totalScrollbackOverflow;
    [_intervalTreeObserver intervalTreeDidRemoveObjectOfType:
                               [self intervalTreeObserverTypeForObject:mark]
                                                      onLine:line];
    mark.code = returnCode;
    [_intervalTreeObserver intervalTreeDidAddObjectOfType:
                               [self intervalTreeObserverTypeForObject:mark]
                                                   onLine:line];
    VT100RemoteHost *remoteHost = [self remoteHostOnLine:[self numberOfLines]];
    [[iTermShellHistoryController sharedInstance]
        setStatusOfCommandAtMark:mark
                          onHost:remoteHost
                              to:returnCode];
    [delegate_ screenNeedsRedraw];
  } else {
...
// The only defined key is "shell", and the value will be tcsh, bash, zsh, or
// fish.
- (void)terminalSetShellIntegrationVersion:(NSString *)version {
  ...
  NSDictionary<NSString *, NSNumber *> *lastVersionByShell =
      @{@"tcsh" : @2, @"bash" : @5, @"zsh" : @5, @"fish" : @5};
  NSInteger latestKnownVersion =
      [lastVersionByShell[shell ?: @""] integerValue];
  if (shell) {
...
- (int)terminalChecksumInRectangle:(VT100GridRect)rect {
  ...
  for (int y = rect.origin.y; y < rect.origin.y + rect.size.height; y++) {
    ...
    for (int x = rect.origin.x;
         x < rect.origin.x + rect.size.width && x < self.width; x++) {
...
- (void)terminalRepeatPreviousCharacter:(int)times {
  ...
  if (_lastCharacter.code) {
    ...
    for (int i = 0; i < times; i++) {
      ...
      [delegate_
          screenDidAppendStringToCurrentLine:string
...
- (VT100GridCoordRange)commandRange {
  ...
  if (commandStartX_ < 0) {
    ...
    return VT100GridCoordRangeMake(
        commandStartX_, MAX(0, commandStartY_ - offset), currentGrid_.cursorX,
...
- (void)linkRun:(VT100GridRun)run withURLCode:(unsigned short)code {
...
// Set the color of prototypechar to all chars between startPoint and endPoint
// on the screen.
- (void)highlightRun:(VT100GridRun)run
  ...
  NSColor *genericFgColor =
      [fgColor colorUsingColorSpace:[NSColorSpace genericRGBColorSpace]];
  NSColor *genericBgColor =
      [bgColor colorUsingColorSpace:[NSColorSpace genericRGBColorSpace]];
...
// This assumes the window's height is going to change to newHeight but
// currentGrid_.size.height is still the "old" height. Returns the number of
// lines appended.
- (int)appendScreen:(VT100Grid *)grid
  ...
  if (grid.size.height - newHeight >= usedHeight) {
    // Height is decreasing but pushing HEIGHT lines into the buffer would
    // scroll all the used lines off the top, leaving the cursor floating
    // without any text. Keep all used lines that fit onscreen.
    ...
    if (newHeight < grid.size.height) {
      ...
      // Screen is not shrinking in height. New content may be brought in on
      // top.
      n = grid.size.height;
    }
  }
...
// It's kind of wrong to use VT100GridRun here, but I think it's harmless
// enough.
- (VT100GridRun)runByTrimmingNullsFromRun:(VT100GridRun)run {
  ...
  x = end.x;
  y = end.y;
  ITBetaAssert(y >= 0, @"Negative y to from max of run %@",
               VT100GridRunDescription(run));
...
- (BOOL)trimSelectionFromStart:(VT100GridCoord)start
  ...
  VT100GridRun run = VT100GridRunFromCoords(VT100GridCoordMake(startX, startY),
                                            ...
                                            currentGrid_.size.width);
  if (run.length <= 0) {
    DLog(@"Run has length %@ given start and end of %@ and %@", @(run.length),
         VT100GridCoordDescription(start), VT100GridCoordDescription(end));
    return NO;
  }
  run = [self runByTrimmingNullsFromRun:run];
  if (run.length == 0) {
    DLog(@"After trimming, run has length 0 given start and end of %@ and %@",
         VT100GridCoordDescription(start), VT100GridCoordDescription(end));
    return NO;
  }
...
- (LineBufferPositionRange *)positionRangeForCoordRange:
                                 (VT100GridCoordRange)range
  ...
  LineBufferPositionRange *positionRange =
      [[[LineBufferPositionRange alloc] init] autorelease];

  BOOL endExtends = NO;
  // Use the predecessor of endx,endy so it will have a legal position in the
  // line buffer.
  if (range.end.x == [self width]) {
    ...
    if (line[range.end.x - 1].code == 0 && line[range.end.x].code == EOL_HARD) {
      // The selection goes all the way to the end of the line and there is a
      // null at the end of the line, so it extends to the end of the line. The
      // linebuffer can't recover this from its position because the trailing
      // null in the line wouldn't be in the linebuffer.
      endExtends = YES;
    }
  }
  range.end.x--;
  ...
  if (range.start.x < 0 || range.start.y < 0 || range.end.x < 0 ||
      range.end.y < 0) {
    return nil;
  }
  ...
  BOOL ok = [self
      trimSelectionFromStart:VT100GridCoordMake(range.start.x, range.start.y)
  ...
  positionRange.start =
      [lineBuffer positionForCoordinate:trimmedStart
                                  width:currentGrid_.size.width
                                 offset:0];
...
- (BOOL)convertRange:(VT100GridCoordRange)range
  ...
  // Temporarily swap in the passed-in linebuffer so the call below can access
  // lines in the right line buffer.
  LineBuffer *savedLineBuffer = linebuffer_;
  linebuffer_ = lineBuffer;
  selectionRange = [self positionRangeForCoordRange:range
  ...
  if (!selectionRange) {
    // One case where this happens is when the start and end of the range are
    // past the last character in the line buffer (e.g., all nulls). It could
    // occur when a note exists on a null line.
    return NO;
  }
  ...
  if (ok) {
    ...
    // I'm not sure how to get here. It would happen if the endpoint of the
    // selection could be converted into a LineBufferPosition with the original
    // width but that LineBufferPosition could not be converted back into a
    // VT100GridCoord with the new width.
    resultPtr->end.x = currentGrid_.size.width;
    resultPtr->end.y =
        [lineBuffer numLinesWithWidth:newWidth] + currentGrid_.size.height - 1;
  }
...
- (void)setMaxScrollbackLines:(unsigned int)lines {
  ...
  if (!unlimitedScrollback_) {
    [self incrementOverflowBy:[linebuffer_
                                  dropExcessLinesWithWidth:currentGrid_.size
                                                               .width]];
  }
...
- (void)setUseColumnScrollRegion:(BOOL)mode {
  ...
  if (!mode) {
    currentGrid_.scrollRegionCols =
        VT100GridRangeMake(0, currentGrid_.size.width);
  }
}
...
- (void)popScrollbackLines:(int)linesPushed {
  ...
  int i;
  screen_char_t *dummy =
      iTermCalloc(currentGrid_.size.width, sizeof(screen_char_t));
...
- (BOOL)continueFindResultsInContext:(FindContext *)context
  ...
  do {
    ...
    case Matched: {
      ...
      NSArray *allPositions =
          [linebuffer_ convertPositions:context.results
    ...
    case NotFound:
      ...
      if (context.hasWrapped) {
        ...
        [linebuffer_
            prepareToSearchFor:findContext_.substring
                    startingAt:(findContext_.dir > 0
                                    ? [linebuffer_ firstPosition]
                                    : [[linebuffer_ lastPosition] predecessor])
  ...
  // NSLog(@"Did %d iterations in %dms. Average time per block was %dms",
  // iterations, ms_diff, ms_diff/iterations);
...
- (void)noteDidRequestRemoval:(PTYNoteViewController *)note {
  if ([intervalTree_ containsObject:note]) {
    ...
    [_intervalTreeObserver
        intervalTreeDidRemoveObjectOfType:
            [self intervalTreeObserverTypeForObject:note]
                                   onLine:[self coordRangeForInterval:
                                                    note.entry.interval]
                                              .start.y +
                                          self.totalScrollbackOverflow];
...
- (void)gridCursorDidChangeLine {
  if (_trackCursorLineMovement) {
    [delegate_ screenCursorDidMoveToLine:currentGrid_.cursorY +
                                         [self numberOfScrollbackLines]];
  }
}
...
- (void)markDidBecomeCommandMark:(id<iTermMark>)mark {
  if (mark.entry.interval.location >
      self.lastCommandMark.entry.interval.location) {
    self.lastCommandMark = mark;
  }
}

- (int)
    numberOfLinewDroppedWhenEncodingContentsIncludingGrid:(BOOL)includeGrid
                                                  encoder:
                                                      (id<iTermEncoderAdapter>)
                                                          encoder
                                           intervalOffset:
                                               (long long *)intervalOffsetPtr {
  // We want 10k lines of history at 80 cols, and fewer for small widths, to
  ...
  // Make a copy of the last blocks of the line buffer; enough to contain at
  // least |maxLines|.
  LineBuffer *temp =
      [linebuffer_ appendOnlyCopyWithMinimumLines:maxLines
  ...
  long long intervalOffset =
      -(linesDroppedForBrevity + [self totalScrollbackOverflow]) *
      (self.width + 1);
...
// Deprecated
- (int)numberOfLinesDroppedWhenEncodingLegacyFormatWithEncoder:
           (id<iTermEncoderAdapter>)encoder
                                                intervalOffset:
                                                    (long long *)
                                                        intervalOffsetPtr {
  if (gDebugLogging) {
  ...
  return [self
      numberOfLinewDroppedWhenEncodingContentsIncludingGrid:YES
                                                    encoder:encoder
                                             intervalOffset:intervalOffsetPtr];
}

- (int)numberOfLinesDroppedWhenEncodingModernFormatWithEncoder:
           (id<iTermEncoderAdapter>)encoder
                                                intervalOffset:
                                                    (long long *)
                                                        intervalOffsetPtr {
  ...
  [encoder
      encodeDictionaryWithKey:@"LineBuffer"
                   generation:iTermGenerationAlwaysEncode
                        block:^BOOL(
                            id<iTermEncoderAdapter> _Nonnull subencoder) {
                          linesDropped = [self
                              numberOfLinesDroppedWhenEncodingLegacyFormatWithEncoder:
                                  subencoder
                                                                       intervalOffset:
                                                                           intervalOffsetPtr];
                          return YES;
  ...
  if (altGrid_) {
    [encoder
        encodeDictionaryWithKey:@"AltGrid"
                     generation:iTermGenerationAlwaysEncode
                          block:^BOOL(
                              id<iTermEncoderAdapter> _Nonnull subencoder) {
...
- (BOOL)encodeContents:(id<iTermEncoderAdapter>)encoder
  ...
  if ([iTermAdvancedSettingsModel useNewContentFormat]) {
    ...
    const int linesDroppedForBrevity = [self
        numberOfLinesDroppedWhenEncodingModernFormatWithEncoder:encoder
                                                 intervalOffset:&
                                                                intervalOffset];
    extra = @{
      kScreenStateIntervalTreeKey :
              [intervalTree_ dictionaryValueWithOffset:intervalOffset] ?: @{},
    };
    if (linesDroppedOut) {
    ...
    long long intervalOffset = 0;
    const int linesDroppedForBrevity = [self
        numberOfLinesDroppedWhenEncodingLegacyFormatWithEncoder:encoder
                                                 intervalOffset:&
                                                                intervalOffset];
    extra = @{
      kScreenStateIntervalTreeKey :
                        ...
                        block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                          ...
                          NSDictionary *dict = @{
                            kScreenStateTabStopsKey : [tabStops_ allObjects]
                                ?: @[],
                            kScreenStateTerminalKey :
                                    [terminal_ stateDictionary] ?: @{},
                            kScreenStateLineDrawingModeKey : @[
                              @(charsetUsesLineDrawingMode_[0]),
                              @(charsetUsesLineDrawingMode_[1]),
                              @(charsetUsesLineDrawingMode_[2]),
                              @(charsetUsesLineDrawingMode_[3])
                            ],
                            kScreenStateNonCurrentGridKey :
                                    [self contentsOfNonCurrentGrid] ?: @{},
                            kScreenStateCurrentGridIsPrimaryKey :
                                @(primaryGrid_ == currentGrid_),
                            kScreenStateSavedIntervalTreeKey :
                                    [savedIntervalTree_
                                        dictionaryValueWithOffset:0]
                                ?: [NSNull null],
                            ...
                            kScreenStateNextCommandOutputStartKey :
                                [NSDictionary dictionaryWithGridAbsCoord:
                                                  _startOfRunningCommandOutput],
                            kScreenStateCursorVisibleKey : @(_cursorVisible),
                            kScreenStateTrackCursorLineMovementKey :
                                @(_trackCursorLineMovement),
                            kScreenStateLastCommandOutputRangeKey :
                                [NSDictionary dictionaryWithGridAbsCoordRange:
                                                  _lastCommandOutputRange],
                            kScreenStateShellIntegrationInstalledKey :
                                @(_shellIntegrationInstalled),
                            kScreenStateLastCommandMarkKey :
                                    _lastCommandMark.guid
                                ?: [NSNull null],
                            kScreenStatePrimaryGridStateKey :
                                    primaryGrid_.dictionaryValue
                                ?: @{},
                            kScreenStateAlternateGridStateKey :
                                    altGrid_.dictionaryValue
                                ?: [NSNull null],
                          };
...
- (NSDictionary *)contentsOfNonCurrentGrid {
  ...
  iTermMutableDictionaryEncoderAdapter *encoder =
      [[[iTermMutableDictionaryEncoderAdapter alloc] init] autorelease];
...
- (void)appendSessionRestoredBanner {
  ...
  NSString *message =
      [NSString stringWithFormat:@"Session Contents Restored on %@",
                                 [dateFormatter stringFromDate:[NSDate date]]];
  [self appendStringAtCursor:message];

  // Restore the graphic rendition, add a newline, and calculate how far down
  // the cursor moved.
  ...
  if (currentGrid_.preferredCursorPosition.y >= 0 &&
      currentGrid_.preferredCursorPosition.y + 1 < currentGrid_.size.height) {
...
- (void)restoreFromDictionary:(NSDictionary *)dictionary
  ...
  if (!newFormat) {
    ...
    DLog(@"appendFromDictionary: Grid size is %dx%d", currentGrid_.size.width,
         currentGrid_.size.height);
    DLog(@"Restored %d wrapped lines from dictionary",
         [self numberOfScrollbackLines] + linesRestored);
    DLog(@"setCursorPosition=%@", @(setCursorPosition));
    if (!setCursorPosition) {
      VT100GridCoord coord;
      if (VT100GridCoordFromDictionary(screenState[kScreenStateCursorCoord],
                                       &coord)) {
        // The initial size of this session might be smaller than its eventual
        // size. Save the coord because after the window is set to its correct
        // size it might be possible to place the cursor in this position.
        currentGrid_.preferredCursorPosition = coord;
        DLog(@"Save preferred cursor position %@",
             VT100GridCoordDescription(coord));
        if (coord.x >= 0 && coord.y >= 0 && coord.x <= self.width &&
            coord.y < self.height) {
    ...
    // Reduce line buffer's max size to not include the grid height. This is its
    // final state.
    ...
    primaryGrid_ =
        [[VT100Grid alloc] initWithDictionary:dictionary[@"PrimaryGrid"]
                                     delegate:self];
    ...
    if (!altGrid_) {
      altGrid_ = [[VT100Grid alloc] initWithSize:primaryGrid_.size
                                        delegate:self];
    }
    ...
    LineBuffer *lineBuffer =
        [[LineBuffer alloc] initWithDictionary:dictionary[@"LineBuffer"]];
    [lineBuffer setMaxLines:maxScrollbackLines_ + self.height];
    if (!unlimitedScrollback_) {
      [lineBuffer dropExcessLinesWithWidth:self.width];
    }
  ...
  if (includeRestorationBanner &&
      [iTermAdvancedSettingsModel showSessionRestoredBanner]) {
  ...
  if (screenState) {
    ...
    for (int i = 0; i < sizeof(charsetUsesLineDrawingMode_) /
                            sizeof(charsetUsesLineDrawingMode_[0]) &&
                    i < array.count;
         i++) {
    ...
    if (!newFormat) {
      ...
      VT100Grid *otherGrid =
          (currentGrid_ == primaryGrid_) ? altGrid_ : primaryGrid_;
      LineBuffer *otherLineBuffer = [[[LineBuffer alloc]
          initWithDictionary:screenState[kScreenStateNonCurrentGridKey]]
          autorelease];
      ...
      [primaryGrid_
          setStateFromDictionary:screenState[kScreenStatePrimaryGridStateKey]];
      if (addedBanner && currentGrid_.preferredCursorPosition.x < 0 &&
          currentGrid_.preferredCursorPosition.y < 0) {
        primaryGrid_.cursor = savedCursor;
      }
      [altGrid_ setStateFromDictionary:screenState
                                           [kScreenStateAlternateGridStateKey]];
    }

    NSString *guidOfLastCommandMark =
        screenState[kScreenStateLastCommandMarkKey];
    if (reattached) {
      ...
      _startOfRunningCommandOutput =
          [screenState[kScreenStateNextCommandOutputStartKey] gridAbsCoord];
    }
    _cursorVisible = [screenState[kScreenStateCursorVisibleKey] boolValue];
    _trackCursorLineMovement =
        [screenState[kScreenStateTrackCursorLineMovementKey] boolValue];
    _lastCommandOutputRange =
        [screenState[kScreenStateLastCommandOutputRangeKey] gridAbsCoordRange];
    _shellIntegrationInstalled =
        [screenState[kScreenStateShellIntegrationInstalledKey] boolValue];
    ...
    if (!newFormat) {
      ...
      VT100GridSize savedSize = [VT100Grid
          sizeInStateDictionary:screenState[kScreenStatePrimaryGridStateKey]];
      [self setSize:savedSize];
    }
    [intervalTree_ release];
    intervalTree_ = [[IntervalTree alloc]
        initWithDictionary:screenState[kScreenStateIntervalTreeKey]];
    ...
    [savedIntervalTree_ release];
    savedIntervalTree_ = [[IntervalTree alloc]
        initWithDictionary:screenState[kScreenStateSavedIntervalTreeKey]];
...
// Link references to marks in CapturedOutput (for the lines where output was
// captured) to the deserialized mark. Link marks for commands to CommandUse
// objects in command history. Notify delegate of PTYNoteViewControllers so they
// get added as subviews, and set the delegate of not view controllers to self.
- (void)fixUpDeserializedIntervalTree:(IntervalTree *)intervalTree
  ...
  NSMutableDictionary *markGuidToCapturedOutput =
  ...
  for (NSArray *objects in [intervalTree forwardLimitEnumerator]) {
    for (id<IntervalTreeObject> object in objects) {
      if ([object isKindOfClass:[VT100RemoteHost class]]) {
        ...
        screenMark.delegate = self;
        // If |capturedOutput| is not empty then this mark is a command, some of
        // whose output was captured. The iTermCapturedOutputMarks will come
        // later so save the GUIDs we need in markGuidToCapturedOutput and
        // they'll get backfilled when found.
        ...
        if (screenMark.command) {
          ...
          iTermCommandHistoryCommandUseMO *commandUse =
              [[iTermShellHistoryController sharedInstance]
                  commandUseWithMarkGuid:screenMark.guid
        ...
        // This mark represents a line whose output was captured. Find the
        // preceding command mark that has a CapturedOutput corresponding to
        // this mark and fill it in.
        iTermCapturedOutputMark *capturedOutputMark =
            (iTermCapturedOutputMark *)object;
        CapturedOutput *capturedOutput =
            markGuidToCapturedOutput[capturedOutputMark.guid];
        ...
        ScreenCharClearProvisionalFlagForImageWithCode(
            imageMark.imageCode.intValue);
      }
    }
  }
}
...
- (iTermTemporaryDoubleBufferedGridController *)temporaryDoubleBuffer {
  if ([delegate_ screenShouldReduceFlicker] ||
      _temporaryDoubleBuffer.explicit) {
...
- (PTYTextViewSynchronousUpdateState *)temporaryDoubleBufferedGridSavedState {
  PTYTextViewSynchronousUpdateState *state =
  ...
  state.cursorVisible =
      self.temporaryDoubleBuffer.explicit ? _cursorVisible : YES;
...
- (void)temporaryDoubleBufferedGridDidExpire {
  ...
  // Force the screen to redraw right away. Some users reported lag and this
  // seems to fix it. I think the update timer was hitting a worst case scenario
  // which made the lag visible. See issue 3537.
...
- (void)inlineImageSetMarkOnScreenLine:(NSInteger)line code:(unichar)code {
  long long absLine =
      (self.totalScrollbackOverflow + [self numberOfScrollbackLines] + line);
  iTermImageMark *mark =
      [self addMarkStartingAtAbsoluteLine:absLine
                                  oneLine:YES
