...
+ (NSURL *)urlForNewZipFileInFolder:(NSURL *)destinationFolder
                               name:(NSString *)name
  ...
  do {
    count++;
    if (count == 1) {
      zipURL = [destinationFolder
          URLByAppendingPathComponent:
              [name stringByAppendingPathExtension:extension]];
    } else {
      NSString *nameWithCount =
          [NSString stringWithFormat:@"%@ (%@)", name, @(count)];
      zipURL = [destinationFolder
          URLByAppendingPathComponent:
              [nameWithCount stringByAppendingPathExtension:extension]];
    }
...
+ (void)exportScriptAtURL:(NSURL *)fullURL
          signingIdentity:(SIGIdentity *)sigIdentity
               completion:
                   (void (^)(NSString *errorMessage, NSURL *zipURL))completion {
  ...
  if (!fullEnvironment) {
    ...
    NSString *temp = [[[NSFileManager defaultManager] temporaryDirectory]
    ...
    [self writeMetadataTo:[NSURL fileURLWithPath:temp] sourceURL:fullURL];
  ...
  [self writeMetadataTo:fullURL sourceURL:fullURL];
...
+ (void)writeMetadataTo:(NSURL *)destinationURL sourceURL:(NSURL *)sourceURL {
  NSString *autoLaunchPath =
      [[NSFileManager defaultManager] autolaunchScriptPath];
  NSDictionary *metadata = @{
    @"AutoLaunch" : @([sourceURL.path.stringByResolvingSymlinksInPath
        hasPrefix:autoLaunchPath.stringByResolvingSymlinksInPath])
  };
  [[NSJSONSerialization it_jsonStringForObject:metadata]
      writeToURL:[destinationURL URLByAppendingPathComponent:@"metadata.json"]
      atomically:NO
...
+ (void)exportFullEnvironmentScriptAtURL:(NSURL *)fullURL
                         ...
                         signingIdentity:(SIGIdentity *)signingIdentity
                              completion:(void (^)(NSString *errorMessage,
                                                   NSURL *zipURL))completion {
  NSArray<NSURL *> *sourceURLs;
  NSURL *destinationFolder =
      [NSURL fileURLWithPath:[[NSFileManager defaultManager] desktopDirectory]];

  NSString *absSetupPath =
      [fullURL URLByAppendingPathComponent:@"setup.cfg"].path;
  iTermSetupCfgParser *setupParser =
      [[iTermSetupCfgParser alloc] initWithPath:absSetupPath];
  ...
  sourceURLs = @[
    [relativeURL URLByAppendingPathComponent:@"setup.cfg"],
  ...
  NSURL *zipURL = [self urlForNewZipFileInFolder:destinationFolder
                                            name:name
                                       extension:extension];
  [iTermCommandRunner
         zipURLs:sourceURLs
      ...
      completion:^(BOOL ok) {
        ...
        if (signingIdentity) {
          [self signInPlace:zipURL
               withIdentity:signingIdentity
                 completion:^(NSError *signingError) {
                   if (signingError) {
...
+ (void)signInPlace:(NSURL *)url
  ...
  NSError *error = nil;
  NSURL *payloadURL =
      [url URLByAppendingPathExtension:[[NSUUID UUID] UUIDString]];
  ...
  SIGArchiveBuilder *builder =
      [[SIGArchiveBuilder alloc] initWithPayloadFileURL:payloadURL
                                               identity:identity];
...
+ (void)copySimpleScriptAtURL:(NSURL *)simpleScriptSourceURL
  ...
  [iTermSetupCfgParser
      writeSetupCfgToFile:[destination
                              stringByAppendingPathComponent:
                                  [NSString stringWithFormat:@"setup.cfg"]]
                     name:name
             dependencies:@[]
  ...
  [fileManager createDirectoryAtPath:sourceFolder
         withIntermediateDirectories:NO
                          attributes:nil
                               error:NULL];
  NSURL *destinationPy = [NSURL
      fileURLWithPath:[sourceFolder
                          stringByAppendingPathComponent:
                              [simpleScriptSourceURL lastPathComponent]]];
...
+ (BOOL)urlContainsScript:(NSURL *)url
          fullEnvironment:(out nullable BOOL *)fullEnvironment {
  BOOL isDirectory = NO;
  NSFileManager *fileManager = [NSFileManager defaultManager];
  ...
  if (isDirectory) {
    // Legal scripts must have a setup.cfg, iterm2env, and appropriately named
    // source folder and file.
    NSString *setupCfg = [url.path stringByAppendingPathComponent:@"setup.cfg"];
    NSString *iterm2env =
        [url.path stringByAppendingPathComponent:@"iterm2env"];
    NSString *name = url.path.lastPathComponent;
    NSString *folder = [url.path stringByAppendingPathComponent:name];
    NSString *mainPy = [folder stringByAppendingPathComponent:
