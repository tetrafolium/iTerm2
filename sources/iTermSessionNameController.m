...
static NSString *const iTermSessionNameControllerStateKeyWindowTitleStack =
    @"window title stack";
static NSString *const iTermSessionNameControllerStateKeyIconTitleStack =
    @"icon title stack";
NSString *const iTermSessionNameControllerSystemTitleUniqueIdentifier =
    @"com.iterm2.system-title";
...
- (NSDictionary *)stateDictionary {
  return @{
    iTermSessionNameControllerStateKeyWindowTitleStack :
            [_windowTitleStack it_arrayByReplacingOccurrencesOf:[NSNull null]
                                                           with:@0]
        ?: @[],
    iTermSessionNameControllerStateKeyIconTitleStack :
            [_iconTitleStack it_arrayByReplacingOccurrencesOf:[NSNull null]
                                                         with:@0]
        ?: @[]
  };
}
...
- (void)restoreNameFromStateDictionary:(NSDictionary *)state {
  _windowTitleStack =
      [[state[iTermSessionNameControllerStateKeyWindowTitleStack]
          it_arrayByReplacingOccurrencesOf:@0
                                      with:[NSNull null]] mutableCopy];
...
- (NSString *)invocationForTitleProviderID:(NSString *)uniqueIdentifier {
  if ([uniqueIdentifier
          isEqualToString:
              iTermSessionNameControllerSystemTitleUniqueIdentifier]) {
    return @"iterm2.private.session_title(session: session.id)";
  }
  return [[iTermAPIHelper sessionTitleFunctions]
             objectPassingTest:^BOOL(iTermSessionTitleProvider *provider,
                                     NSUInteger index, BOOL *stop) {
               return
                   [provider.uniqueIdentifier isEqualToString:uniqueIdentifier];
             }]
      .invocation;
}
...
- (BOOL)usingBuiltInTitleProvider {
  NSString *uniqueIdentifier =
      self.delegate.sessionNameControllerUniqueIdentifier;
  return [uniqueIdentifier
      isEqualToString:iTermSessionNameControllerSystemTitleUniqueIdentifier];
}

// Synchronous evaluation updates _dependencies with all paths occurring in the
// title format.
- (void)evaluateInvocationSynchronously:(BOOL)sync
                             completion:(void (^)(NSString *presentationName))
                                            completion {
  ...
  if ([uniqueIdentifier
          isEqualToString:
              iTermSessionNameControllerSystemTitleUniqueIdentifier]) {
    // Do it again to get the window title. We know this completes
    // synchronously.
    __block NSString *windowTitle = nil;
    [iTermScriptFunctionCall
        callFunction:@"iterm2.private.window_title(session: session.id)"
             timeout:0
               scope:scope
          retainSelf:YES
          completion:^(NSString *possiblyEmptyResult, NSError *error,
  ...
  if (recordingScope) {
    ...
    for (NSString *tmuxVariableName in @[
           iTermVariableKeySessionTmuxClientName,
           iTermVariableKeySessionTmuxRole, iTermVariableKeySessionTmuxPaneTitle
         ]) {
...
- (void)logError:(NSError *)error forInvocation:(NSString *)invocation {
  NSString *message = [NSString
      stringWithFormat:
          @"Invoked “%@” to compute name for session. Failed with error:\n%@\n",
  ...
  if ([self errorIsUnregisteredFunctionCall:error]) {
    [self logMessage:[NSString
                         stringWithFormat:
                             @"Could not make a function call into a script. "
                             @"Either its unique identifier changed (in which "
                             @"case you should update your settings in Prefs > "
                             @"Profiles > General > Title) or the script is "
                             @"not running. The failed invocation was:\n%@",
                             invocation]
          invocation:invocation];
  } else {
    [self logMessage:error.localizedDescription invocation:invocation];
  }
}
...
- (NSString *)valueForInvocation:(NSString *)invocation
                      withResult:(NSString *)possiblyEmptyResult
                           error:(NSError *)error {
  if (!error) {
    NSString *result = [possiblyEmptyResult
...
- (void)logMessage:(NSString *)message invocation:(NSString *)invocation {
  ...
  NSString *signature = [iTermExpressionParser
      signatureForFunctionCallInvocation:invocation
  ...
  if (signature) {
    ...
    [[iTermAPIHelper sharedInstance]
        logToConnectionHostingFunctionWithSignature:nil
                                             format:@"Malformed invocation in "
...
- (void)didEvaluateInvocationWithResult:(NSString *)result {
  ...
  [_delegate sessionNameControllerPresentationNameDidChangeTo:
                 self.presentationSessionTitle];
}
...
- (NSString *)presentationWindowTitle {
  ...
  if ([self usingBuiltInTitleProvider] && _cachedBuiltInWindowTitleEvaluation) {
    // We're using a standard window title (not provided by a script). Use the
    // window version of the session title. This combines the OSC 0/2 window
    // title (if any) with other title components.
    return [self formattedName:_cachedBuiltInWindowTitleEvaluation];
  } else if (self.windowNameFromVariable) {
    // Using a custom title provider and there's an OSC 0/OSC 2 window title
    // set. That overrides the title provider's value. Some day I should add
    // support for window title providers, but for now they are just *session*
    // title providers.
    return [self formattedName:self.windowNameFromVariable];
  } else {
    // Either we're using a builtin title provider but haven't cached it for
    // some reason (this should not happen), or there's a custom title provider
    // not overridden by OSC 0/OSC 2. Finally, if lots of impossible things
    // happen return Unnamed so we can correlate bug reports with this comment.
    // Hi, future me.
...
- (NSString *)formattedName:(NSString *)base {
  iTermSessionFormattingDescriptor *descriptor =
      [self.delegate sessionNameControllerFormattingDescriptor];
  if (descriptor.isTmuxGateway) {
    return [NSString
        stringWithFormat:@"[↣ %@ %@]", base, descriptor.tmuxClientName];
  }
  if (descriptor.haveTmuxController) {
    // There won't be a valid job name, and the profile name is always tmux, so
    // just show the window name. This is confusing: this refers to the name of
    // a tmux window, which is equivalent to an iTerm2 tab. It is reported to us
    // by tmux. We ignore the base name because the real name comes from the
    // server and that's all we care about.
...
- (void)evaluateInvocationSynchronously:(BOOL)synchronous {
  ...
  [self
      evaluateInvocationSynchronously:synchronous
                           completion:^(NSString *presentationName) {
                             __strong __typeof(self) strongSelf = weakSelf;
                             if (strongSelf) {
                               if (strongSelf->_appliedCount > count) {
                                 // A later async evaluation has already
                                 // completed. Don't overwrite it.
                                 return;
                               }
                               ...
                               if ([NSObject object:strongSelf
                                                        ->_cachedEvaluation
                                       isEqualToObject:presentationName]) {
                                 return;
                               }
                               strongSelf->_cachedEvaluation =
                                   [presentationName copy];
                               if (!synchronous) {
                                 [strongSelf.delegate
                                     sessionNameControllerPresentationNameDidChangeTo:
                                         presentationName];
                               }
                             }
                           }];
