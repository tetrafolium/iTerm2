// PTYTab abstracts the concept of a tab. This is attached to the tabview's
// identifier and is the owner of PTYSession.
...
// This implements NSSplitViewDelegate but it was an informal protocol in 10.5.
// If 10.5 support is eventually dropped, change this to make it official.
@interface PTYTab
    : NSObject <NSCopying, NSSplitViewDelegate, iTermUniquelyIdentifiable,
                PTYSessionDelegate, PTYSplitViewDelegate,
...
// uniqueId lazily auto-assigns a unique id unless you assign it a value first.
// It is never 0.
...
// While activeSession is not retained, it should only ever refer to a session
// that belongs to this tab, and is thus retained through the view-to-session
// map.
...
// A string that overrides the default behavior of using the active session's
// title. Set to nil to use the default behavior. This is a swifty string.
@property(nonatomic, copy) NSString *titleOverride;

@property(nonatomic, readonly)
    iTermVariableScope<iTermTabScope> *variablesScope;
@property(nonatomic, readonly)
    iTermMetalUnavailableReason metalUnavailableReason;
...
// This is useful for a tmux tab where scrollviews sizes are not tightly coupled
// to the SessionView size because autoresizing is turned off. When something
// changes, such as toggling the pane title bars, it's necessary to grow or
// shrink session views for a tight fit. This should be followed by fitting the
// window to tabs.
- (void)recompact;
...
//   more than one child and a vertical orientation: add a new subview and
//   return it. more than one child and a horizontal orientation: add a new
//   split subview with vertical orientation and add a sessionview subview to it
//   and return that sessionview.
...
// Update the tab's title from the active session's name. Needed for
// initializing the tab's title after setting up tmux tabs.
