...
- (void)drawBackgroundImageInView:(NSView *)view
  ...
  const BOOL debug = NO;
  const iTermBackgroundDraws draws =
      [self drawsForBackgroundImageInView:view
                                dirtyRect:dirtyRect
  ...
  const float alpha =
      [self.delegate backgroundDrawingHelperUseTransparency]
          ? (1.0 - [self.delegate backgroundDrawingHelperTransparency])
          : 1.0;
  if (!draws.image) {
    [[[self.delegate backgroundDrawingHelperDefaultBackgroundColor]
        colorWithAlphaComponent:alpha] set];
    NSRectFillUsingOperation(draws.solidBackgroundColorRect,
  ...
  NSRect (^flip)(NSRect) = ^NSRect(NSRect r) {
    return NSMakeRect(r.origin.x,
                      draws.image.size.height - r.origin.y - r.size.height,
  ...
  if (blendDefaultBackground) {
    ...
    [[defaultBackgroundColor
        colorWithAlphaComponent:1 -
                                [self.delegate backgroundDrawingHelperBlending]]
        set];
  ...
  if (debug) {
    ...
    NSString *s = [NSString stringWithFormat:@"rect=%@ local=%@ src=%@ dst=%@",
                                             ...
                                             NSStringFromRect(NSIntegralRect(
                                                 draws.imageSourceRect)),
                                             NSStringFromRect(NSIntegralRect(
                                                 draws.imageDestinationRect))];

    [[NSColor whiteColor] set];
    NSRectFill(NSMakeRect(100, dirtyRect.origin.y + 20, 600, 24));

    [s drawAtPoint:NSMakePoint(100, dirtyRect.origin.y + 20)
        withAttributes:@{
...
- (iTermBackgroundDraws)
    drawsForBackgroundImageInView:(NSView *)view
                        dirtyRect:(NSRect)dirtyRect
  ...
  if (backgroundImage) {
    const NSRect dirtyRectInContainerCoords =
        [containerView convertRect:dirtyRect fromView:view];
    NSRect dirtyRectInAdjustedContainerCoords = dirtyRectInContainerCoords;
    dirtyRectInAdjustedContainerCoords.origin.x -=
        windowVisibleAreaRect.origin.x;
    dirtyRectInAdjustedContainerCoords.origin.y -=
        windowVisibleAreaRect.origin.y;
    NSImage *image;
    NSRect sourceRect;
    ...
    case iTermBackgroundImageModeStretch:
      ...
      sourceRect =
          [self sourceRectForImageSize:image.size
    ...
    case iTermBackgroundImageModeScaleAspectFill:
      image = backgroundImage;
      sourceRect =
          [self scaleAspectFillSourceRectForImageSize:image.size
                                          contentRect:windowVisibleAreaRect
                                      destinationRect:
                                          dirtyRectInAdjustedContainerCoords];
      break;

    case iTermBackgroundImageModeScaleAspectFit:
      image = backgroundImage;
      // TODO: The analyze complained about this dead store, which suggests I'm
      // assing the wrong argument for dirtyRect below.
      // dirtyRectInAdjustedContainerCoords =
      // NSIntersectionRect(dirtyRectInAdjustedContainerCoords,
      // containerView.bounds);
      sourceRect = [iTermBackgroundDrawingHelper
          scaleAspectFitSourceRectForForImageSize:image.size
      ...
      for (int i = 0; i < sizeof(result.boxes) / sizeof(*result.boxes); i++) {
        result.boxes[i] = [view convertRect:result.boxes[i]
...
- (NSImage *)patternedImageForViewOfSize:(NSSize)size {
  ...
  if (!_patternedImage || !NSEqualSizes(_patternedImage.size, size)) {
    ...
    NSColor *pattern = [NSColor
        colorWithPatternImage:[self.delegate backgroundDrawingHelperImage]];
    [pattern drawSwatchInRect:NSMakeRect(0, 0, _patternedImage.size.width,
...
- (NSRect)sourceRectForImageSize:(NSSize)imageSize
  ...
  NSRect sourceRect = NSMakeRect(
      destinationRect.origin.x * dx, destinationRect.origin.y * dy,
      destinationRect.size.width * dx, destinationRect.size.height * dy);
  return sourceRect;
}
...
- (NSRect)scaleAspectFillSourceRectForImageSize:(NSSize)imageSize
  ...
  // Compute the normalized offsets/sizes of the destination rect relative to
  // the view. The map directly onto the imageSpaceRect. In other words, if the
  // destination rect's origin is 25% of the way across the view, then it's also
  // 25% of the way across the imageSpaceRect.
...
+ (NSRect)scaleAspectFitRectWithImageOfSize:(NSSize)imageSize
  ...
  if (imageAspectRatio > viewAspectRatio) {
    ...
    rectWithImage.origin.y =
        destinationRect.origin.y +
        (destinationRect.size.height - rectWithImage.size.height) / 2.0;
    *scale = imageSize.width / destinationRect.size.width;
    ...
    rectWithImage.origin.x =
        destinationRect.origin.x +
        (destinationRect.size.width - rectWithImage.size.width) / 2.0;
...
+ (void)getPillarBoxesForImageRect:(NSRect)rectWithImage
  ...
  // Left pillarbox
  const CGFloat pillarboxWidth =
      (destinationRect.size.width - rectWithImage.size.width) / 2;
  NSRect leftPillarboxInViewCoords =
      NSMakeRect(destinationRect.origin.x, destinationRect.origin.y,
                 pillarboxWidth, destinationRect.size.height);
  if (boxRect1) {
    *boxRect1 = NSIntersectionRect(
        NSIntersectionRect(leftPillarboxInViewCoords, destinationRect),
        dirtyRect);
  }

  // Right pillarbox
  NSRect rightPillarboxInViewCoords = NSMakeRect(
      destinationRect.origin.x + destinationRect.size.width - pillarboxWidth,
      destinationRect.origin.y, pillarboxWidth, destinationRect.size.height);
  if (boxRect2) {
    *boxRect2 = NSIntersectionRect(
        NSIntersectionRect(rightPillarboxInViewCoords, destinationRect),
        dirtyRect);
  }
}
...
+ (void)getLetterBoxesForImageRect:(NSRect)rectWithImage
                   destinationRect:(NSRect)destinationRect
                         dirtyRect:(NSRect)dirtyRect
                          ...
                          boxRect2:(out NSRect *)boxRect2 {
  // Top letterbox
  CGFloat letterboxHeight =
      (destinationRect.size.height - rectWithImage.size.height) / 2;
  NSRect topLetterboxInViewCoords =
      NSMakeRect(destinationRect.origin.x, destinationRect.origin.y,
                 destinationRect.size.width, letterboxHeight);
  if (boxRect1) {
    *boxRect1 = NSIntersectionRect(
        NSIntersectionRect(topLetterboxInViewCoords, destinationRect),
        dirtyRect);
  }

  // Bottom letterbox
  NSRect bottomLetterboxInViewCoords = NSMakeRect(
      destinationRect.origin.x,
      destinationRect.origin.y + destinationRect.size.height - letterboxHeight,
      destinationRect.size.width, letterboxHeight);
  if (boxRect2) {
    *boxRect2 = NSIntersectionRect(
        NSIntersectionRect(bottomLetterboxInViewCoords, destinationRect),
        dirtyRect);
  }
}
...
+ (void)getClippedScaleAspectFitSourceRect:(inout NSRect *)sourceRectInOut
  ...
  const NSRect clippedDestinationRect =
      NSIntersectionRect(clipRect, originalDestinationRect);
  ...
  *destinationRectInOut = clippedDestinationRect;
  ...
  const CGFloat leftLost =
      (NSMinX(clippedDestinationRect) - NSMinX(originalDestinationRect)) /
      NSWidth(originalDestinationRect);
  const CGFloat topLost =
      (NSMinY(clippedDestinationRect) - NSMinY(originalDestinationRect)) /
      NSHeight(originalDestinationRect);
  const CGFloat rightLost =
      (NSMaxX(originalDestinationRect) - NSMaxX(clippedDestinationRect)) /
      NSWidth(originalDestinationRect);
  const CGFloat bottomLost =
      (NSMaxY(originalDestinationRect) - NSMaxY(clippedDestinationRect)) /
      NSHeight(originalDestinationRect);

  const NSRect sourceRect = NSMakeRect(
      NSMinX(originalSourceRect) + leftLost * NSWidth(originalSourceRect),
      NSMinY(originalSourceRect) + topLost * NSHeight(originalSourceRect),
      NSWidth(originalSourceRect) -
          (leftLost + rightLost) * NSWidth(originalSourceRect),
      NSHeight(originalSourceRect) -
          (bottomLost + topLost) * NSHeight(originalSourceRect));
...
+ (NSRect)scaleAspectFitSourceRectForForImageSize:(NSSize)imageSize
                                  destinationRect:(NSRect)destinationRect
                                        dirtyRect:(NSRect)dirtyRect
                                         ...
                                         boxRect2:(out NSRect *)boxRect2 {
  CGFloat imageAspectRatio = imageSize.width / imageSize.height;
  CGFloat viewAspectRatio =
      destinationRect.size.width / destinationRect.size.height;

  // Compute the viewRect which is the part of the view that has an image (and
  // not a letterbox/pillarbox)
  CGFloat scale = 0;
  const NSRect rectWithImage =
      [self scaleAspectFitRectWithImageOfSize:imageSize
                            inDestinationRect:destinationRect
                                        scale:&scale];
  *drawRect = rectWithImage;

  NSRect destinationRectRelativeToViewRect =
      NSMakeRect(destinationRect.origin.x - rectWithImage.origin.x,
                 destinationRect.origin.y - rectWithImage.origin.y,
                 destinationRect.size.width, destinationRect.size.height);
  NSRect sourceRect =
      NSMakeRect(destinationRectRelativeToViewRect.origin.x * scale,
  ...
  // Ensure the bounds of the source rect are legit. The out-of-bounds parts are
  // covered by letter/pillar boxes.
  NSRect safeSourceRect = NSIntersectionRect(
