...
// On macOS 10.15, this doesn't seem to be a problem and it works up to 64
// (issue 9229).
...
- (void)awakeFromNib {
  ...
  info.observer = ^() {
    ...
    strongSelf->_transparencyAffectsOnlyDefaultBackgroundColor.enabled =
        haveTransparency;
  ...
  info.observer = ^() {
    ...
    strongSelf->_blurRadius.enabled =
        (strongSelf->_useBlur.state == NSControlStateValueOn);
  ...
  info.observer = ^() {
    ...
    switch ((iTermBackgroundImageMode)
                strongSelf->_backgroundImageMode.selectedTag) {
    ...
    case iTermBackgroundImageModeStretch:
      self->_backgroundImagePreview.imageScaling =
    ...
    case iTermBackgroundImageModeScaleAspectFill: {
      ...
      if (filename) {
        ...
        strongSelf->_backgroundImagePreview.image = [anImage
            it_imageFillingSize:strongSelf->_backgroundImagePreview.frame.size];
        self->_backgroundImagePreview.imageScaling = NSImageScaleNone;
  ...
  // Custom window title
  {
    ...
    _customWindowTitleDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:
  ...
  // Custom tab title
  {
    ...
    _customTabTitleDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:
            [iTermVariableHistory
...
- (void)reloadProfile {
  [super reloadProfile];
  [self loadBackgroundImageWithFilename:
            [self stringForKey:KEY_BACKGROUND_IMAGE_LOCATION]];
  [self updateCustomWindowTitleEnabled];
  [self updateCustomTabTitleEnabled];
}
...
- (void)openFilePicker {
  ...
  void (^completion)(NSInteger) = ^(NSInteger result) {
    if (result == NSModalResponseOK) {
      ...
      [self setString:self.backgroundImageFilename
               forKey:KEY_BACKGROUND_IMAGE_LOCATION];
    } else {
...
- (void)loadBackgroundImageWithFilename:(NSString *)filename {
  NSImage *anImage = filename.length > 0
...
// Refreshes the entries in the list of screens and tries to preserve the
// current selection.
- (void)repopulateScreen {
  ...
  for (i = 0; i < numScreens; i++) {
    if (i == 0) {
      ...
      [_screen
          addItemWithTitle:[NSString stringWithFormat:@"Screen %d", i + 1]];
    }
...
- (void)maybeWarnAboutSpaces {
  [iTermWarning showWarningWithTitle:
                    @"To have a new window open in a specific space, "
