...
static NSString *const iTermImageDisplayColumnAttribute =
    @"iTermImageDisplayColumnAttribute";
static NSString *const iTermIsBoxDrawingAttribute =
    @"iTermIsBoxDrawingAttribute";
static NSString *const iTermUnderlineLengthAttribute =
    @"iTermUnderlineLengthAttribute";
...
static CGFloat iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(
    BOOL hasBackgroundImage, BOOL enableBlending, BOOL reverseVideo,
    CGFloat transparencyAlpha, CGFloat blend);
@implementation iTermTextDrawingHelper {
  ...
  // Used by drawCursor: to remember the last time the cursor moved to avoid
  // drawing a blinked-out cursor while it's moving.
  ...
  NSMutableDictionary<iTermAttributedStringProxy *, id>
      *_replacementLineRefCache;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    iTermPreciseTimerStatsInit(&_stats[TIMER_TOTAL_DRAW_RECT],
                               "Total drawRect");
    iTermPreciseTimerStatsInit(&_stats[TIMER_CONSTRUCT_BACKGROUND_RUNS],
                               "Construct BG runs");
    iTermPreciseTimerStatsInit(&_stats[TIMER_DRAW_BACKGROUND], "Draw BG");

    iTermPreciseTimerStatsInit(&_stats[TIMER_STAT_CONSTRUCTION],
                               "Construction");
    iTermPreciseTimerStatsInit(
        &_stats[TIMER_STAT_BUILD_MUTABLE_ATTRIBUTED_STRING],
        "Build attr strings");
    ...
    iTermPreciseTimerStatsInit(&_stats[TIMER_COMBINE_ATTRIBUTES],
                               "Combine Attrs");
    iTermPreciseTimerStatsInit(&_stats[TIMER_ADVANCES], "Advances");
    iTermPreciseTimerStatsInit(&_stats[TIMER_BETWEEN_CALLS_TO_DRAW_RECT],
                               "Between calls");
...
- (void)setUnderlinedRange:(VT100GridAbsWindowedRange)underlinedRange {
  ...
  DLog(@"Update underlined range of %@ to %@", self.delegate,
       VT100GridAbsWindowedRangeDescription(underlinedRange));
...
- (void)drawTextViewContentInRect:(NSRect)rect
  ...
  DLog(@"begin drawRect:%@ in view %@", [NSValue valueWithRect:rect],
       _delegate);
  iTermPreciseTimerSetEnabled(YES);
  ...
  // If there are two or more rects that need display, the OS will pass in
  // |rect| as the smallest bounding rect that contains them all. Luckily, we
  // can get the list of the "real" dirty rects and they're guaranteed to be
  // disjoint. So draw each of them individually.
  [self startTiming];
  ...
  // Start at 0 because ligatures can draw incorrectly otherwise. When a font
  // has a ligature for
  // -> and >-, then a line like ->->-> needs to start at the beginning since
  // drawing only a suffix of it could draw a >- ligature at the start of the
  // range being drawn. Issue 5030.
  boundingCoordRange.start.x = 0;
  boundingCoordRange.start.y =
      MAX(MAX(0, boundingCoordRange.start.y - 1), visibleLines.location);
  boundingCoordRange.end.x =
      MIN(_gridSize.width, boundingCoordRange.end.x + haloWidth);
  boundingCoordRange.end.y = MIN(yLimit, boundingCoordRange.end.y + 1);

  int numRowsInRect =
      MAX(0, boundingCoordRange.end.y - boundingCoordRange.start.y);
  if (numRowsInRect == 0) {
    return;
  }
  NSMutableData *store =
      [NSMutableData dataWithLength:numRowsInRect * sizeof(NSRange)];
  NSRange *ranges = (NSRange *)store.mutableBytes;
  for (int i = 0; i < rectCount; i++) {
    VT100GridCoordRange coordRange = [self coordRangeForRect:rectArray[i]];
    //        NSLog(@"Have to draw rect %@ (%@)",
    //        NSStringFromRect(rectArray[i]),
    //        VT100GridCoordRangeDescription(coordRange));
    ...
    for (int j = 0; j < numRowsInRect; j++) {
      ...
      if (gridRange.location == 0 && gridRange.length == 0) {
        ...
        const int max =
            MAX(gridRange.location + gridRange.length, coordRangeMaxX);
        ranges[j].location = min;
        ranges[j].length = max - min;
      }
      //            NSLog(@"Set range on line %d to %@", j +
      //            boundingCoordRange.start.y, NSStringFromRange(ranges[j]));
    }
  }

  // Make sure to draw above and below the region that we plan to draw text in
  // to avoid artifacts as seen in issue 5665. This will cause double-draws on
  // the top and bottom margin, but that's small enough that it probably won't
  // matter.
  {
    NSColor *color;
    if (_hasBackgroundImage) {
      ...
      color = [self.delegate drawingHelperColorForCode:ALTSEM_DEFAULT
                                          ...
                                          isBackground:YES];
      const CGFloat alpha =
          iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(
              _hasBackgroundImage, !iTermTextIsMonochrome(), _reverseVideo,
              _transparencyAlpha, _blend);
    ...
    if (height > 0) {
      NSRectFillUsingOperation(
          NSMakeRect(NSMinX(rect), NSMinY(rect), NSWidth(rect), height),
    ...
    if (height > 0) {
      NSRectFillUsingOperation(
          NSMakeRect(NSMinX(rect), maxDrawnY, NSWidth(rect), height),
          NSCompositingOperationSourceOver);
    }
  }
  ...
  iTermPreciseTimerPeriodicLog(
      @"drawRect", _stats, sizeof(_stats) / sizeof(*_stats), 5,
      [iTermAdvancedSettingsModel logDrawingPerformance], nil);
  ...
  // Release cached CTLineRefs from the last set of drawings and update them
  // with the new ones. This keeps us from having too many lines cached at once.
  [_lineRefCache release];
...
- (NSInteger)
    numberOfEquivalentBackgroundColorLinesInRunArrays:
...
- (void)drawRanges:(NSRange *)ranges
  ...
  for (NSInteger i = 0; i < numRanges; i++) {
    ...
    // We work hard to paint all the backgrounds first and then all the
    // foregrounds. The reason this is necessary is because sometimes a glyph is
    // larger than its cell. Some fonts draw narrow- width characters as
    // full-width, some combining marks (e.g., combining enclosing circle) are
    // necessarily larger than a cell, etc. For example, see issue 3446.
    //
    // By drawing characters after backgrounds and also drawing an extra "ring"
    // of characters just outside the clipping region, we allow oversize
    // characters to draw outside their bounds without getting painted-over by a
    // background color. Of course if a glyph extends more than one full cell
    // outside its bounds, it will still get overwritten by a background
    // sometimes.

    // First, find all the background runs. The outer array
    // (backgroundRunArrays) will have one element per line. That element (a
    // PTYTextViewBackgroundRunArray) contains the line number, y origin, and an
    // array of PTYTextViewBackgroundRunBox objects.
    ...
    NSIndexSet *selectedIndexes =
        [_selection selectedIndexesIncludingTabFillersInAbsoluteLine:
                        line + _totalScrollbackOverflow];
    ...
    iTermPreciseTimerStatsMeasureAndAccumulate(
        &_stats[TIMER_CONSTRUCT_BACKGROUND_RUNS]);
  }
  ...
  for (NSInteger i = 0; i < backgroundRunArrays.count;) {
    NSInteger rows = [self
        numberOfEquivalentBackgroundColorLinesInRunArrays:backgroundRunArrays
                                                fromIndex:i];
  ...
  // Draw default background color over the line under the last drawn line so
  // the tops of characters aren't visible there. If there is an IME, that could
  // be many lines tall.
  VT100GridCoordRange drawableCoordRange =
      [self drawableCoordRangeForRect:_visibleRect];
  ...
  const BOOL drawCursorBeforeText =
      (_cursorType == CURSOR_UNDERLINE || _cursorType == CURSOR_VERTICAL);
  ...
  if ([self textAppearanceDependsOnBackgroundColor]) {
    [self drawForegroundForBackgroundRunArrays:backgroundRunArrays ctx:ctx];
  } else {
...
- (BOOL)textAppearanceDependsOnBackgroundColor {
  ...
  if (self.thinStrokes == iTermThinStrokesSettingRetinaDarkBackgroundsOnly &&
      _isRetina) {
    return YES;
  }
  return NO;
...
- (void)drawBackgroundForLine:(int)line
  ...
  for (iTermBoxedBackgroundColorRun *box in runs) {
    ...
    //        NSLog(@"Paint background row %d range %@", line,
    //        NSStringFromRange(run->range));

    NSRect rect = NSMakeRect(
        floor([iTermPreferences intForKey:kPreferenceKeySideMargins] +
              run->range.location * _cellSize.width),
        yOrigin, ceil(run->range.length * _cellSize.width),
        _cellSize.height * rows);
    BOOL enableBlending = YES;
    if (@available(macOS 10.14, *)) {
      // If subpixel AA is enabled, then we want to draw the default background
      // color directly. Otherwise, we'll disable blending and make it clear.
      // Then the background color view can do the job.
      enableBlending = !iTermTextIsMonochrome();
    }
    NSColor *color = [self unprocessedColorForBackgroundRun:run
                                             enableBlending:enableBlending];
    // The unprocessed color is needed for minimum contrast computation for text
    // color.
...
- (NSColor *)unprocessedColorForBackgroundRun:(iTermBackgroundColorRun *)run
  ...
  if (run->selected) {
    ...
    if (defaultBackground) {
      alpha = iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(
          _hasBackgroundImage, enableBlending, _reverseVideo, alpha, _blend);
    }
  }
...
static CGFloat iTermTextDrawingHelperAlphaValueForDefaultBackgroundColor(
    BOOL hasBackgroundImage, BOOL enableBlending, BOOL reverseVideo,
    CGFloat transparencyAlpha, CGFloat blend) {
  // We can draw the default background color with a solid-color view under some
  // circumstances.
...
- (void)drawTopMargin {
  ...
  topMarginRect.origin.y -=
      MAX(0, [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] -
                 NSMinY(_delegate.enclosingScrollView.documentVisibleRect));

  topMarginRect.size.height =
...
- (void)drawMarginsAndMarkForLine:(int)line y:(CGFloat)y {
  NSRect leftMargin = NSMakeRect(
      0, y, MAX(0, [iTermPreferences intForKey:kPreferenceKeySideMargins]),
      _cellSize.height);
  NSRect rightMargin;
  NSRect visibleRect = _visibleRect;
  rightMargin.origin.x = _cellSize.width * _gridSize.width +
...
- (void)drawStripesInRect:(NSRect)rect {
  if (!_backgroundStripesImage) {
    _backgroundStripesImage = [[NSImage it_imageNamed:@"BackgroundStripes"
...
- (void)drawAccessoriesInRect:(NSRect)bgRect {
  ...
  const BOOL drawCursorGuide =
      (self.highlightCursorLine && cursorLine >= coordRange.start.y &&
...
- (void)drawCursorGuideForColumns:(NSRange)range y:(CGFloat)yOrigin {
  ...
  NSPoint textOrigin =
      NSMakePoint([iTermPreferences intForKey:kPreferenceKeySideMargins] +
                      range.location * _cellSize.width,
                  yOrigin);
  NSRect rect = NSMakeRect(textOrigin.x, textOrigin.y,
                           range.length * _cellSize.width, _cellSize.height);
  NSRectFillUsingOperation(rect, NSCompositingOperationSourceOver);
...
+ (NSRect)frameForMarkContainedInRect:(NSRect)container
  ...
  const CGFloat verticalSpacing =
      MAX(0, scale * round((cellSize.height / scale -
                            cellSizeWithoutSpacing.height / scale) /
                           2.0));
  CGRect rect =
      NSMakeRect(container.origin.x, container.origin.y + verticalSpacing,
                 container.size.width, cellSizeWithoutSpacing.height);
  const CGFloat kMaxHeight = 15 * scale;
  const CGFloat kMinMargin = 3 * scale;
  const CGFloat kMargin =
      MAX(kMinMargin, (cellSizeWithoutSpacing.height - kMaxHeight) / 2.0);
  ...
  rect.size.height -=
      MAX(3 * scale, (cellSizeWithoutSpacing.height - 15 * scale) / 2.0);

  return rect;
}

+ (NSColor *)successMarkColor {
  return [NSColor colorWithSRGBRed:0.53846 green:0.757301 blue:1 alpha:1];
}
...
- (void)drawMarkIfNeededOnLine:(int)line leftMarginRect:(NSRect)leftMargin {
  ...
  if (mark.isVisible && self.drawMarkIndicators) {
    ...
    NSRect rect = [iTermTextDrawingHelper
        frameForMarkContainedInRect:insetLeftMargin
                           cellSize:_cellSize
    ...
    } else if ([iTermAdvancedSettingsModel
                   showYellowMarkForJobStoppedBySignal] &&
...
- (void)drawNoteRangesOnLine:(int)line {
  NSArray *noteRanges =
      [self.delegate drawingHelperCharactersWithNotesOnLine:line];
  if (noteRanges.count) {
    for (NSValue *value in noteRanges) {
      VT100GridRange range = [value gridRangeValue];
      CGFloat x = range.location * _cellSize.width +
                  [iTermPreferences intForKey:kPreferenceKeySideMargins];
      CGFloat y = line * _cellSize.height;
      [[NSColor yellowColor] set];

      CGFloat maxX =
          MIN(_frame.size.width -
                  [iTermPreferences intForKey:kPreferenceKeySideMargins],
...
- (void)createTimestampDrawingHelper {
  ...
  _timestampDrawHelper = [[iTermTimestampDrawHelper alloc]
      initWithBackgroundColor:[self defaultBackgroundColor]
...
- (void)drawTimestamps {
  ...
  // Note: for the foreground color, we don't use the dimmed version because it
  // looks bad on nonretina displays. That's why I go to the colormap instead of
  // using -defaultForegroundColor.
  for (int y = _scrollViewDocumentVisibleRect.origin.y / _cellSize.height;
       y < NSMaxY(_scrollViewDocumentVisibleRect) / _cellSize.height &&
       y < _numberOfLines;
       y++) {
    [_timestampDrawHelper setDate:[_delegate drawingHelperTimestampForLine:y]
                          forLine:y];
  }
  ...
  NSRect destination =
      NSMakeRect(textViewSize.width - imageSize.width - margins.right,
                 textViewSize.height - visibleSize.height +
                     kiTermIndicatorStandardHeight + margins.top,
...
- (NSSize)drawBadgeInRect:(NSRect)rect margins:(NSEdgeInsets)margins {
  ...
  NSRect intersection = [iTermTextDrawingHelper
      rectForBadgeImageOfSize:_badgeImage.size
         ...
         destinationFrameSize:_frame.size
                ...
                sourceRectPtr:&source
                      margins:NSEdgeInsetsMake(self.badgeTopMargin, 0, 0,
                                               self.badgeRightMargin)];
...
- (void)drawDropTargets {
  ...
  [self enumerateDropTargets:^(NSString *label, NSRange range) {
    NSRect rect = NSMakeRect(0, range.location * _cellSize.height,
    ...
    [label drawInRect:rect
        withAttributes:[label
                           attributesUsingFont:[NSFont boldSystemFontOfSize:8]
...
- (void)enumerateDropTargets:(void (^)(NSString *, NSRange))block {
  ...
  for (int line = coordRange.start.y; line <= coordRange.end.y;
       line++, y += _cellSize.height) {
    ...
    [lineRanges
        addObject:[NSValue valueWithRange:NSMakeRange(firstLine,
                                                      line - firstLine + 1)]];
  }
...
// Draw assuming no foreground color processing. Keeps glyphs together in a
// single background color run across different background colors.
- (void)drawUnprocessedForegroundForBackgroundRunArrays:
            (NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
  ...
  NSArray<iTermBackgroundColorRunsInLine *> *fakeRunArrays =
      [backgroundRunArrays mapWithBlock:^id(
                               iTermBackgroundColorRunsInLine *runs) {
        ...
        for (iTermBoxedBackgroundColorRun *run in runs.array) {
          if (!havePreviousRun) {
            ...
            previousRun.range =
                NSUnionRange(previousRun.range, run.valuePointer->range);
          } else {
            [combinedRuns
                addObject:[iTermBoxedBackgroundColorRun
                              boxedBackgroundColorRunWithValue:previousRun]];
            previousRun = *run.valuePointer;
          }
        }
        if (havePreviousRun) {
        ...
        iTermBackgroundColorRunsInLine *fakeRuns =
  ...
  [self drawForegroundForBackgroundRunArrays:fakeRunArrays ctx:ctx];
}

// Draws
- (void)drawForegroundForBackgroundRunArrays:
            (NSArray<iTermBackgroundColorRunsInLine *> *)backgroundRunArrays
                                         ctx:(CGContextRef)ctx {
...
- (void)drawForegroundForLineNumber:(int)line
                                  y:(CGFloat)y
                     backgroundRuns:(NSArray<iTermBoxedBackgroundColorRun *> *)
                                        backgroundRuns
...
- (void)drawCharactersForLine:(int)line
                          atY:(CGFloat)y
               backgroundRuns:
                   (NSArray<iTermBoxedBackgroundColorRun *> *)backgroundRuns
                      context:(CGContextRef)ctx {
  screen_char_t *theLine = [self.delegate drawingHelperLineAtIndex:line];
  NSData *matches = [_delegate drawingHelperMatchesOnLine:line];
...
- (void)constructAndDrawRunsForLine:(screen_char_t *)theLine
  ...
  //    NSLog(@"Draw text on line %d range %@", row,
  //    NSStringFromRange(indexRange));

  iTermPreciseTimerStatsStartTimer(&_stats[TIMER_STAT_CONSTRUCTION]);
  NSArray<id<iTermAttributedString>> *attributedStrings = [self
      attributedStringsForLine:theLine
               ...
               backgroundColor:bgColor
                forceTextColor:forceTextColor
                   ...
                   findMatches:matches
               underlinedRange:
                   [self underlinedRangeOnLine:row + _totalScrollbackOverflow]
                     positions:&positions];
  ...
  [self
      drawMultipartAttributedString:attributedStrings
...
- (void)drawMultipartAttributedString:
  ...
  NSInteger start = 0;
  for (id<iTermAttributedString> singlePartAttributedString in
           attributedStrings) {
    ...
    if ([singlePartAttributedString isKindOfClass:[NSAttributedString class]]) {
      numCellsDrawn =
          [self drawSinglePartAttributedString:(NSAttributedString *)
                                                   singlePartAttributedString
                                       atPoint:point
                                        origin:origin
      ...
      offsetPoint.y -=
          round((_cellSize.height - _cellSizeWithoutSpacing.height) / 2.0);
      numCellsDrawn = [self drawFastPathString:(iTermCheapAttributedString *)
                                                   singlePartAttributedString
                                       atPoint:offsetPoint
    ...
    //                         alpha:1] set];
    //        NSFrameRect(NSMakeRect(point.x + subpositions[0], point.y,
    //        numCellsDrawn * _cellSize.width, _cellSize.height));
...
- (void)drawBoxDrawingCharacter:(unichar)theCharacter
  ...
  [iTermBoxDrawingBezierCurveFactory
      drawCodeInCurrentContext:theCharacter
                      cellSize:_cellSize
                         scale:1
                        offset:CGPointZero
                         color:color
...
- (int)setSmoothingWithContext:(CGContextRef)ctx
  ...
  return iTermSetSmoothing(ctx, savedFontSmoothingStyle, useThinStrokes,
                           antialiased);
}

// Just like drawTextOnlyAttributedString but 2-3x faster. Uses
// CGContextShowGlyphsAtPositions instead of CTFontDrawGlyphs.
// NOTE: point is adjusted when vertical spacing is not 100% to vertically
// center the text box. unadjustedPoint gives the bottom of the line.
- (int)drawFastPathString:(iTermCheapAttributedString *)cheapString
  ...
  if ([cheapString.attributes[iTermIsBoxDrawingAttribute] boolValue]) {
    ...
    for (NSUInteger i = 0; i < cheapString.length; i++) {
      unichar c = chars[i];
      NSPoint p =
          NSMakePoint(unadjustedPoint.x + positions[i], unadjustedPoint.y);
  ...
  int result =
      [self drawFastPathStringWithoutUnderlineOrStrikethrough:cheapString
                                                      atPoint:point
                                                       origin:origin
                                                    positions:positions
                                                    inContext:ctx
...
- (int)drawFastPathStringWithoutUnderlineOrStrikethrough:
  ...
  if (attributes[iTermImageCodeAttribute]) {
    ...
    VT100GridCoord originInImage =
        VT100GridCoordMake([attributes[iTermImageColumnAttribute] intValue],
  ...
  BOOL ok = CTFontGetGlyphsForCharacters(
      (CTFontRef)font, cheapString.characters, glyphs, cheapString.length);
  if (!ok) {
    NSString *string = [NSString stringWithCharacters:cheapString.characters
                                               length:cheapString.length];
    [self drawTextOnlyAttributedStringWithoutUnderlineOrStrikethrough:
              [NSAttributedString
  ...
  CGColorRef const color =
      (CGColorRef)
          cheapString.attributes[(NSString *)kCTForegroundColorAttributeName];
  const BOOL fakeItalic =
      [cheapString.attributes[iTermFakeItalicAttribute] boolValue];
  const BOOL fakeBold =
      [cheapString.attributes[iTermFakeBoldAttribute] boolValue];
  const BOOL antiAlias =
      !smear && [cheapString.attributes[iTermAntiAliasAttribute] boolValue];

  CGContextSetShouldAntialias(ctx, antiAlias);

  const CGFloat *components = CGColorGetComponents(color);
  const BOOL useThinStrokes =
      [self useThinStrokesAgainstBackgroundColor:backgroundColor
  ...
  if (smear) {
    ...
    if (fakeBold) {
      // If anti-aliased, drawing twice at the same position makes the strokes
      // thicker. If not anti-alised, draw one pixel to the right.
      CGContextSetTextMatrix(
          ctx,
          CGAffineTransformMake(1.0, 0.0, m21, -1.0,
                                x + (antiAlias ? _antiAliasedShift : 1), y));
...
- (void)drawUnderlineOrStrikethroughForFastPathString:
            (iTermCheapAttributedString *)cheapString
  ...
  NSNumber *value =
      attributes[wantUnderline ? NSUnderlineStyleAttributeName
  ...
  const NSSize size = NSMakeSize(
      [attributes[iTermUnderlineLengthAttribute] intValue] * _cellSize.width,
      _cellSize.height * 2);
                                    ...
                                    underlineColor:underlineColor
                                             ...
                                             block:^(CGContextRef ctx) {
                                               ...
                                               NSMutableDictionary *attrs =
                                                   [[cheapString.attributes
                                                           mutableCopy]
                                                       autorelease];
                                               CGFloat components[4] = {0, 0, 0,
                                                                        1};
                                               CGColorSpaceRef colorSpace =
                                                   CGColorSpaceCreateDeviceRGB();
                                               CGColorRef black = CGColorCreate(
                                               ...
                                               iTermCheapAttributedString
                                                   *blackCopy = [[cheapString
                                                       copyWithAttributes:attrs]
                                                       autorelease];
                                               [self
                                                   drawFastPathStringWithoutUnderlineOrStrikethrough:
                                                       blackCopy
                                                                                             atPoint:
                                                                                                 NSMakePoint(
                                                                                                     -stringPositions
...
- (int)drawSinglePartAttributedString:(NSAttributedString *)attributedString
  ...
  if (attributes[iTermImageCodeAttribute]) {
    ...
    [attributedString.string
        enumerateComposedCharacters:^(NSRange range, unichar simple,
                                      NSString *complexString, BOOL *stop) {
          NSPoint p = NSMakePoint(point.x + positions[range.location], point.y);
          [self drawBoxDrawingCharacter:simple
                         withAttributes:[attributedString
...
- (void)
    drawTextOnlyAttributedStringWithoutUnderlineOrStrikethrough:
  ...
  CTLineRef lineRef;
  iTermAttributedStringProxy *proxy =
      [iTermAttributedStringProxy withAttributedString:attributedString];
  ...
  CGAffineTransform textMatrix = CGAffineTransformMake(
      1.0, 0.0, c, -1.0, origin.x + stringPositions[0], ty);
  ...
  for (CFIndex j = 0; j < CFArrayGetCount(runs); j++) {
    ...
    if (!buffer) {
      NSMutableData *tempBuffer = [[[NSMutableData alloc]
          initWithLength:sizeof(CGGlyph) * length] autorelease];
      CTRunGetGlyphs(run, CFRangeMake(0, length),
                     (CGGlyph *)tempBuffer.mutableBytes);
      buffer = tempBuffer.mutableBytes;
    }

    NSMutableData *positionsBuffer = [[[NSMutableData alloc]
        initWithLength:sizeof(CGPoint) * length] autorelease];
    CTRunGetPositions(run, CFRangeMake(0, length),
                      (CGPoint *)positionsBuffer.mutableBytes);
    ...
    if (!glyphIndexToCharacterIndex) {
      NSMutableData *tempBuffer = [[[NSMutableData alloc]
          initWithLength:sizeof(CFIndex) * length] autorelease];
      CTRunGetStringIndices(run, CFRangeMake(0, length),
                            (CFIndex *)tempBuffer.mutableBytes);
    ...
    for (size_t glyphIndex = 0; glyphIndex < length; glyphIndex++) {
      ...
      CGFloat characterPosition =
          stringPositions[characterIndex] - stringPositions[0];
      if (characterIndex != previousCharacterIndex &&
          characterPosition != cellOrigin) {
    ...
    if (!smear) {
      ...
      if (fakeBold && _boldAllowed) {
        ...
        CTFontDrawGlyphs(runFont, buffer, (NSPoint *)positions, length,
                         cgContext);
        CGContextTranslateCTM(cgContext, antiAlias ? -_antiAliasedShift : -1,
                              0);
      }
    } else {
      const int radius = 1;
...
- (void)drawUnderlineAndStrikethroughForAttributedString:
            (NSAttributedString *)attributedString
                                                 atPoint:(NSPoint)origin
                                               positions:
                                                   (CGFloat *)stringPositions
                                           wantUnderline:(BOOL)wantUnderline {
  iTermUnderlineContext storage = {
      .maskGraphicsContext = nil,
      .alphaMask = nil,
  ...
  [attributedString
      enumerateAttribute:wantUnderline ? NSUnderlineStyleAttributeName
                                       : NSStrikethroughStyleAttributeName
                 inRange:NSMakeRange(0, attributedString.length)
                 options:0
              usingBlock:^(NSNumber *_Nullable value, NSRange range,
                           BOOL *_Nonnull stop) {
                NSUnderlineStyle underlineStyle = value.integerValue;
                if (underlineStyle == NSUnderlineStyleNone) {
                  return;
                ...
                NSDictionary *const attributes =
                    [attributedString attributesAtIndex:range.location
                                         effectiveRange:nil];
                const NSSize size = NSMakeSize(
                    [attributes[iTermUnderlineLengthAttribute] intValue] *
                        _cellSize.width,
                    _cellSize.height * 2);
                const CGFloat xOrigin =
                    origin.x + stringPositions[range.location];
                const NSRect rect =
                    NSMakeRect(xOrigin, origin.y, size.width, size.height);
                NSColor *underline =
                    [self.colorMap colorForKey:kColorMapUnderline];
                NSColor *underlineColor;
                if (underline) {
                  underlineColor = underline;
                } else {
                  CGColorRef cgColor = (CGColorRef)
                      attributes[(NSString *)kCTForegroundColorAttributeName];
                  underlineColor = [NSColor colorWithCGColor:cgColor];
                }
                [self
                    drawUnderlinedOrStruckthroughTextWithContext:
                        underlineContext
                                                   wantUnderline:wantUnderline
                                                          inRect:rect
                                                  underlineColor:underlineColor
...
- (void)
    drawUnderlinedOrStruckthroughTextWithContext:
        (iTermUnderlineContext *)underlineContext
                                   wantUnderline:(BOOL)wantUnderline
                                          inRect:(NSRect)rect
...
- (void)drawAttributedStringForMask:(NSAttributedString *)attributedString
  ...
  CGColorRef black = [[NSColor colorWithSRGBRed:0 green:0 blue:0
                                          alpha:1] CGColor];
  NSAttributedString *modifiedAttributedString =
      [self attributedString:attributedString bySettingForegroundColorTo:black];
...
- (void)initializeUnderlineContext:(iTermUnderlineContext *)underlineContext
  ...
  [NSGraphicsContext
      setCurrentContext:
          [NSGraphicsContext
              graphicsContextWithCGContext:underlineContext->maskGraphicsContext
                                   flipped:NO]];
  ...
  underlineContext->alphaMask =
      CGBitmapContextCreateImage(underlineContext->maskGraphicsContext);
}
...
- (void)drawInContext:(CGContextRef)cgContext
  ...
  CGContextClipToMask(cgContext, rect, alphaMask);
...
- (CGContextRef)newGrayscaleContextOfSize:(NSSize)size {
  ...
  CGContextRef maskContext =
      CGBitmapContextCreate(NULL, size.width * scale, size.height * scale,
...
NSColor *iTermTextDrawingHelperGetTextColor(iTermTextDrawingHelper *self,
                                            screen_char_t *c,
                                            BOOL inUnderlinedRange, int index,
  ...
  const BOOL needsProcessing =
      ...
      (context->minimumContrast > 0.001 || context->dimmingAmount > 0.001 ||
       context->mutingAmount > 0.001 ||
       c->faint); // faint implies alpha<1 and is faster than getting the alpha
                  // component
  ...
  if (context->findMatches && !context->hasSelectedText) {
    ...
    if (theIndex < [context->findMatches length] &&
        (matchBytes[theIndex] & mask)) {
      isMatch = YES;
    }
  }
  ...
  if (isMatch) {
    ...
    context->havePreviousCharacterAttributes = NO;
             ...
             ((c->foregroundColor == ALTSEM_DEFAULT &&
              ...
              (c->foregroundColor == ALTSEM_CURSOR &&
               c->foregroundColorMode == ColorModeAlternate))) {
    // Reverse video is on. Either is cursor or has default foreground color.
    // Use background color.
    rawColor = [context->colorMap colorForKey:kColorMapBackground];
    context->havePreviousCharacterAttributes = NO;
  } else if (!context->havePreviousCharacterAttributes ||
             c->foregroundColor !=
                 context->previousCharacterAttributes.foregroundColor ||
             ...
             c->foregroundColorMode !=
                 context->previousCharacterAttributes.foregroundColorMode ||
    ...
    // "Normal" case for uncached text color. Recompute the unprocessed color
    // from the character.
    context->previousCharacterAttributes = *c;
    context->havePreviousCharacterAttributes = YES;
    rawColor =
        [context->delegate drawingHelperColorForCode:c->foregroundColor
                                               green:c->fgGreen
                                                blue:c->fgBlue
    ...
    // Foreground attributes are just like the last character. There is a cached
    // foreground color.
    if (needsProcessing &&
        context->backgroundColor != context->previousBackgroundColor) {
      // Process the text color for the current background color, which has
      // changed since the last cell.
      rawColor = context->lastUnprocessedColor;
    } else {
      // Text color is unchanged. Either it's independent of the background
      // color or the background color has not changed.
  ...
  NSColor *result = nil;
  if (needsProcessing) {
    result = [context->colorMap
        processedTextColorForTextColor:rawColor
...
static BOOL iTermTextDrawingHelperShouldAntiAlias(
    screen_char_t *c, BOOL useNonAsciiFont, BOOL asciiAntiAlias,
    BOOL nonAsciiAntiAlias, BOOL isRetina, BOOL forceAntialiasingOnRetina) {
...
- (BOOL)shouldSegmentWithAttributes:(iTermCharacterAttributes *)newAttributes
  ...
  if (likely(!imageAttributes && !previousImageAttributes)) {
    // Not an image cell. Try to quickly check if the attributes are the same,
    // which is the normal case.
    if (likely(!memcmp(previousAttributes, newAttributes,
                       sizeof(*previousAttributes)))) {
      // Identical, byte-for-byte
      *combinedAttributesChanged = NO;
    } else {
      // Properly compare object fields
      *combinedAttributesChanged =
          (newAttributes->shouldAntiAlias !=
               previousAttributes->shouldAntiAlias ||
           ![newAttributes->foregroundColor
               isEqual:previousAttributes->foregroundColor] ||
    ...
    *combinedAttributesChanged = YES;
    ...
    // Going from image cell to image cell. Segment unless it's an adjacent
    // image cell.
    *combinedAttributesChanged = YES; // In theory an image cell should never
                                      // repeat, so shortcut comparison.
    return ![self imageAttributes:imageAttributes
            followImageAttributes:previousImageAttributes];
  }
}

- (BOOL)imageAttributes:(NSDictionary *)imageAttributes
    followImageAttributes:(NSDictionary *)previousImageAttributes {
  if (![previousImageAttributes[iTermImageCodeAttribute]
          isEqual:imageAttributes[iTermImageCodeAttribute]]) {
    return NO;
  }
  if (![previousImageAttributes[iTermImageLineAttribute]
          isEqual:imageAttributes[iTermImageLineAttribute]]) {
    return NO;
  }
  if ((([previousImageAttributes[iTermImageColumnAttribute] integerValue] + 1) &
       0xff) !=
      ([imageAttributes[iTermImageColumnAttribute] integerValue] & 0xff)) {
    return NO;
  }
...
- (void)getAttributesForCharacter:(screen_char_t *)c
  ...
  attributes->shouldAntiAlias = iTermTextDrawingHelperShouldAntiAlias(
      c, _useNonAsciiFont, _asciiAntiAlias, _nonAsciiAntiAlias, _isRetina,
      _forceAntialiasingOnRetina);
  const BOOL isComplex = c->complexChar;
  const unichar code = c->code;

  attributes->boxDrawing =
      !isComplex && [[iTermBoxDrawingBezierCurveFactory
                        boxDrawingCharactersWithBezierPathsIncludingPowerline:
                            _useNativePowerlineGlyphs] characterIsMember:code];

  if (forceTextColor) {
    attributes->foregroundColor = forceTextColor;
  } else {
    attributes->foregroundColor = iTermTextDrawingHelperGetTextColor(
        self, c, inUnderlinedRange, i, textColorContext, colorRun,
        attributes->boxDrawing);
  }
  ...
  PTYFontInfo *fontInfo =
      [_delegate drawingHelperFontForChar:code
                                isComplex:isComplex
...
- (NSDictionary *)dictionaryForCharacterAttributes:
    (iTermCharacterAttributes *)attributes {
  ...
  case iTermCharacterAttributesUnderlineCurly:
    underlineStyle =
        NSUnderlineStyleThick; // Curly isn't an option, so repurpose this.
    break;
  }
  ...
  return @{
    ...
    (NSString *)
    kCTForegroundColorAttributeName : (id)[attributes->foregroundColor CGColor],
...
- (NSDictionary *)imageAttributesForCharacter:(screen_char_t *)c
  ...
  if (c->image) {
    return @{
      iTermImageCodeAttribute : @(c->code),
...
- (BOOL)character:(screen_char_t *)c
  ...
  if (!c->complexChar) {
    ...
    if (iTermCharacterSupportsFastPath(c->code, _asciiLigaturesAvailable) !=
        iTermCharacterSupportsFastPath(pc->code, _asciiLigaturesAvailable)) {
      return NO;
    }
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      boxSet = [[iTermBoxDrawingBezierCurveFactory
          boxDrawingCharactersWithBezierPathsIncludingPowerline:
              _useNativePowerlineGlyphs] retain];
    });
...
- (NSArray<id<iTermAttributedString>> *)
    attributedStringsForLine:(screen_char_t *)line
             ...
             underlinedRange:(NSRange)underlinedRange
                   positions:(CTVector(CGFloat) *)positions {
  NSMutableArray<id<iTermAttributedString>> *attributedStrings =
      [NSMutableArray array];
  ...
  iTermMutableAttributedStringBuilder *builder =
      [[[iTermMutableAttributedStringBuilder alloc] init] autorelease];
  ...
  NSCharacterSet *emojiWithDefaultTextPresentation =
      [NSCharacterSet emojiWithDefaultTextPresentation];
  NSCharacterSet *emojiWithDefaultEmojiPresentationCharacterSet =
  ...
  for (int i = indexRange.location; i < NSMaxRange(indexRange); i++) {
    ...
    if (isComplex) {
      ...
      if (i > indexRange.location && builder.length > 0 &&
          !(!predecessor.complexChar && predecessor.code < 128) &&
          ComplexCharCodeIsSpacingCombiningMark(code)) {
        // Spacing combining marks get their own cell but get drawn together
        // with their base character which is assumed to be in the preceding
        // cell so they combine properly. This does not apply to ASCII
        // characters, since they can never combine with a spacing combining
        // mark. That's done for performance in the GPU renderer to avoid
        // complicating its ASCII fastpath.
        [builder appendString:charAsString];
        const CGFloat lastValue =
            CTVectorGet(positions, CTVectorCount(positions) - 1);
      ...
      if (lastCharacterImpartsEmojiPresentation &&
          [emojiWithDefaultTextPresentation longCharacterIsMember:base] &&
          ![charAsString containsString:@"\ufe0f"]) {
        // Prevent previous character's emoji presentation from making this one
        // have an emoji presentation as well. Leave
        // lastCharacterImpartsEmojiPresentation set to YES intentionally.
        charAsString = [charAsString stringByAppendingString:@"\ufe0e"];
      } else {
        lastCharacterImpartsEmojiPresentation =
            [emojiWithDefaultEmojiPresentationCharacterSet
                longCharacterIsMember:base];
      }
    } else {
      charAsString = nil;
      if (lastCharacterImpartsEmojiPresentation &&
          [emojiWithDefaultTextPresentation characterIsMember:code]) {
        unichar chars[2] = {code, 0xfe0e};
        // Prevent previous character's emoji presentation from making this one
        // have an emoji presentation as well. See issue 9185
        charAsString = [NSString stringWithCharacters:chars length:2];
      } else if (code != DWC_RIGHT &&
                 code >=
                     iTermMinimumDefaultEmojiPresentationCodePoint) { // filter
                                                                      // out
                                                                      // small
                                                                      // values
                                                                      // for
        ...
        lastCharacterImpartsEmojiPresentation =
            [emojiWithDefaultEmojiPresentationCharacterSet
                characterIsMember:code];
      }
    }

    const BOOL drawable = iTermTextDrawingHelperIsCharacterDrawable(
        &c, i > indexRange.location ? &predecessor : NULL, charAsString != nil,
        _blinkingItemsVisible, _blinkAllowed,
        _preferSpeedToFullLigatureSupport);
    predecessor = c;
    if (!drawable) {
      if ((characterAttributes.drawable && c.code == DWC_RIGHT &&
           !c.complexChar) ||
    ...
    if (likely(underlinedRange.length == 0) &&
      ...
      if (drawable ||
          ((characterAttributes.underlineType ||
            characterAttributes.strikethrough || characterAttributes.isURL) &&
           segmentLength == 1)) {
    ...
    // I tried segmenting when fastpath eligibility changes so we can use the
    // fast path as much as possible. In the vimdiff benchmark it was neutral,
    // and in the spam.cc benchmark it was hugely negative (66->210 ms). The
    // failed change was to segment when this is true: builder.canUseFastPath !=
    // (!c.complexChar && iTermCharacterSupportsFastPath(code,
    ...
    if ([self shouldSegmentWithAttributes:&characterAttributes
      ...
      iTermPreciseTimerStatsStartTimer(
          &_stats[TIMER_STAT_BUILD_MUTABLE_ATTRIBUTED_STRING]);
      ...
      builder =
          [[[iTermMutableAttributedStringBuilder alloc] init] autorelease];
      builder.zippy = self.zippy;
      builder.asciiLigaturesAvailable =
          _asciiLigaturesAvailable && _asciiLigatures;
    }
    ++segmentLength;
    memcpy(&previousCharacterAttributes, &characterAttributes,
           sizeof(previousCharacterAttributes));
    ...
    if (combinedAttributesChanged) {
      NSDictionary *combinedAttributes =
          [self dictionaryForCharacterAttributes:&characterAttributes];
      if (imageAttributes) {
        combinedAttributes =
            [combinedAttributes dictionaryByMergingDictionary:imageAttributes];
      }
      [builder setAttributes:combinedAttributes];
    }
    iTermPreciseTimerStatsMeasureAndAccumulate(
        &_stats[TIMER_COMBINE_ATTRIBUTES]);

    if (drawable ||
        ((characterAttributes.underlineType ||
          characterAttributes.strikethrough || characterAttributes.isURL) &&
         segmentLength == 1)) {
      // Use " " when not drawable to prevent 0-length attributed strings when
      // an underline/strikethrough is present. If we get here's because there's
      // an underline/strikethrough (which isn't quite obvious from the if
...
- (BOOL)useThinStrokesAgainstBackgroundColor:(NSColor *)backgroundColor
  ...
  return [backgroundColor brightnessComponent] <
         PerceivedBrightness(components[0], components[1], components[2]);
}
...
- (CGFloat)yOriginForUnderlineForFont:(NSFont *)font
  ...
  CGFloat preferredOffset =
      [self retinaRound:yOffset + _cellSize.height + underlineOffset] -
      1.0 / (2 * scaleFactor);
  ...
  const CGFloat maximumOffset =
      [self retinaFloor:yOffset + cellHeight - thickness];
...
- (void)drawUnderlineOrStrikethroughOfColor:(NSColor *)color
  ...
  const CGFloat y =
      (wantUnderline ? [self yOriginForUnderlineForFont:font
  ...
  const CGFloat lineWidth = wantUnderline
                                ? [self underlineThicknessForFont:font]
                                : [self strikethroughThicknessForFont:font];
  ...
  case NSUnderlineStyleDouble: { // Single underline with dash beneath
    ...
    [path lineToPoint:NSMakePoint(origin.x + rect.size.width,
  ...
  case NSUnderlineStyleThick: { // We use this for curly. Cocoa doesn't have
                                // curly underlines, so we reprupose thick.
    const CGFloat offset = 0;
    origin.y = rect.origin.y + _cellSize.height - 1.5;
    CGContextRef cgContext = [[NSGraphicsContext currentContext] CGContext];
    CGContextSaveGState(cgContext);
    CGContextClipToRect(cgContext,
                        NSMakeRect(origin.x, origin.y - 1, rect.size.width, 3));
    ...
    for (CGFloat x = origin.x - fmod(origin.x, width * 2); x < NSMaxX(rect);
         x += width * 2) {
...
- (void)drawImageWithCode:(unichar)code
  ...
  // DLog(@"Drawing image at %@ with code %@",
  // VT100GridCoordDescription(origin), @(code));
  ...
  if (!image) {
    ...
    NSRectFill(NSMakeRect(point.x, point.y, _cellSize.width * length,
                          _cellSize.height));
    return;
  }
  ...
  if (imageInfo.animated) {
    [_delegate drawingHelperDidFindRunOfAnimatedCellsStartingAt:origin
                                                       ofLength:length];
    _animated = YES;
  }
  const NSRect destRect =
      NSMakeRect(0, 0, _cellSize.width * length, _cellSize.height);
  const NSRect sourceRect = NSMakeRect(
      chunkSize.width * originInImage.x,
      image.size.height - _cellSize.height - chunkSize.height * originInImage.y,
      chunkSize.width * length, chunkSize.height);
  DLog(@"Draw %@ -> %@ with source image of size %@",
       NSStringFromRect(sourceRect), NSStringFromRect(destRect),
...
- (BOOL)drawInputMethodEditorTextAt:(int)xStart
  ...
  if ([self hasMarkedText]) {
    ...
    StringToScreenChars(str, buf, fg, bg, &len, _ambiguousIsDoubleWidth,
                        &cursorIndex, NULL, _normalization,
                        self.unicodeVersion);
    int cursorX = 0;
    int baseX = floor(xStart * _cellSize.width +
                      [iTermPreferences intForKey:kPreferenceKeySideMargins]);
    int i;
    ...
    for (i = 0; i < len;) {
      ...
      int skipped = 0;
      if (charsInLine + i < len && buf[charsInLine + i].code == DWC_RIGHT) {
      ...
      // Draw the background.
      NSRect r =
          NSMakeRect(x, y, charsInLine * _cellSize.width, _cellSize.height);
      ...
      PTYFontInfo *fontInfo =
          [_delegate drawingHelperFontForChar:128
                                    isComplex:NO
      ...
      x = floor(xStart * _cellSize.width +
                [iTermPreferences intForKey:kPreferenceKeySideMargins]);
    ...
    if (!foundCursor && i == cursorIndex) {
      if (justWrapped) {
        cursorX = [iTermPreferences intForKey:kPreferenceKeySideMargins] +
    ...
    double rightMargin =
        [iTermPreferences intForKey:kPreferenceKeySideMargins] +
        _gridSize.width * _cellSize.width;
    ...
    [[colorMap processedBackgroundColorForBackgroundColor:
...
- (NSRect)frameForCursorAt:(VT100GridCoord)cursorCoord {
  ...
  if ([iTermAdvancedSettingsModel fullHeightCursor]) {
    const CGFloat height =
        MAX(_cellSize.height, _cellSizeWithoutSpacing.height);
    return NSMakeRect(
        floor(cursorCoord.x * _cellSize.width +
              [iTermPreferences intForKey:kPreferenceKeySideMargins]),
        rowNumber * _cellSize.height,
        MIN(_cellSize.width, _cellSizeWithoutSpacing.width), height);
  } else {
    const CGFloat height =
        MIN(_cellSize.height, _cellSizeWithoutSpacing.height);
    return NSMakeRect(
        floor(cursorCoord.x * _cellSize.width +
              [iTermPreferences intForKey:kPreferenceKeySideMargins]),
        rowNumber * _cellSize.height +
            MAX(0, round((_cellSize.height - _cellSizeWithoutSpacing.height) /
                         2.0)),
        MIN(_cellSize.width, _cellSizeWithoutSpacing.width), height);
  }
}
...
- (void)drawCopyModeCursor {
  iTermCursor *cursor =
      [iTermCursor itermCopyModeCursorInSelectionState:self.copyModeSelecting];
  cursor.delegate = self;

  [self reallyDrawCursor:cursor
                      at:VT100GridCoordMake(_copyModeCursorCoord.x,
                                            _copyModeCursorCoord.y -
                                                _numberOfScrollbackLines)
                 outline:NO];
}
...
- (void)drawCursor:(BOOL)outline {
  ...
  if ([self shouldDrawCursor]) {
    ...
    if (_showSearchingCursor) {
      NSImage *image = [NSImage it_imageNamed:@"SearchCursor"
                                     forClass:self.class];
      if (image) {
...
- (NSColor *)blockCursorFillColorRespectingSmartSelection {
  if (_useSmartCursorColor) {
    ...
    if (_cursorCoord.y >= 0) {
      ...
      theLine = [self.delegate
          drawingHelperLineAtIndex:_cursorCoord.y + _numberOfScrollbackLines];
    }
    BOOL isDoubleWidth;
    screen_char_t screenChar = [self charForCursorAtColumn:_cursorCoord.x
                                                    inLine:theLine
                                               doubleWidth:&isDoubleWidth];
    iTermSmartCursorColor *smartCursorColor =
        [[[iTermSmartCursorColor alloc] init] autorelease];
...
- (NSRect)reallyDrawCursor:(iTermCursor *)cursor
  ...
  if (cursorCoord.y >= 0) {
    ...
    theLine = [self.delegate
        drawingHelperLineAtIndex:cursorCoord.y + _numberOfScrollbackLines];
  }
  BOOL isDoubleWidth;
  ...
  if (_passwordInput) {
    ...
    [keyImage drawInRect:NSMakeRect(point.x, point.y, _cellSize.width,
                                    _cellSize.height)
                fromRect:NSZeroRect
               operation:NSCompositingOperationSourceOver
                fraction:1
  ...
  [cursor drawWithRect:rect
       ...
       foregroundColor:cursorTextColor
               ...
               focused:((_isInKeyWindow && _textViewIsActiveSession) ||
...
- (NSRange)underlinedRangeOnLine:(long long)row {
  ...
  if (row == _underlinedRange.coordRange.start.y &&
      row == _underlinedRange.coordRange.end.y) {
    ...
    const int start = VT100GridAbsWindowedRangeStart(_underlinedRange).x;
    const int end = _underlinedRange.columnWindow.length > 0
                        ? VT100GridRangeMax(_underlinedRange.columnWindow) + 1
                        : _gridSize.width;
    return NSMakeRange(start, end - start);
  } else if (row == _underlinedRange.coordRange.end.y) {
    // Underline spans multiple lines, ending at this one.
    const int start = _underlinedRange.columnWindow.length > 0
                          ? _underlinedRange.columnWindow.location
                          : 0;
    const int end = VT100GridAbsWindowedRangeEnd(_underlinedRange).x;
    return NSMakeRange(start, end - start);
  } else if (row > _underlinedRange.coordRange.start.y &&
             row < _underlinedRange.coordRange.end.y) {
    // Underline spans multiple lines. This is not the first or last line, so
    // all chars in it are underlined.
    const int start = _underlinedRange.columnWindow.length > 0
                          ? _underlinedRange.columnWindow.location
                          : 0;
    const int end = _underlinedRange.columnWindow.length > 0
...
- (NSColor *)backgroundColorForCursor {
  NSColor *color;
  if (_reverseVideo) {
    color = [[_colorMap colorForKey:kColorMapCursorText]
        colorWithAlphaComponent:1.0];
  } else {
    color =
        [[_colorMap colorForKey:kColorMapCursor] colorWithAlphaComponent:1.0];
  }
  return [_colorMap colorByDimmingTextColor:color];
}

- (BOOL)shouldShowCursor {
  if (_cursorBlinking && self.isInKeyWindow && _textViewIsActiveSession &&
      [NSDate timeIntervalSinceReferenceDate] - _lastTimeCursorMoved > 0.5) {
    // Allow the cursor to blink if it is configured, the window is key, this
    // session is active in the tab, and the cursor has not moved for half a
    // second.
...
- (screen_char_t)charForCursorAtColumn:(int)column
  ...
  if (screenChar.code) {
    if (screenChar.code == DWC_RIGHT) {
      ...
      *doubleWidth =
          (column < width - 1) && (theLine[column + 1].code == DWC_RIGHT);
    }
  } else {
    *doubleWidth = NO;
  }
...
- (BOOL)shouldDrawCursor {
  ...
  if (!NSLocationInRange(cursorRow, [self rangeOfVisibleRows])) {
    // Don't draw a cursor that isn't in one of the rows that's being drawn
    // (e.g., if it's on a row that's just below the last visible row, don't
    // draw it, or else the top of the cursor will be visible at the bottom of
    // the window).
    return NO;
  }
  // Draw the regular cursor only if there's not an IME open as it draws its
  // own cursor. Also, it must be not blinked-out, and it must be within the
  // expected bounds of the screen (which is just a sanity check, really).
  BOOL result = (![self hasMarkedText] && _cursorVisible && shouldShowCursor &&
                 column <= width && column >= 0 && row >= 0 && row < height);
  DLog(@"shouldDrawCursor: hasMarkedText=%d, cursorVisible=%d, showCursor=%d, "
       @"column=%d, row=%d"
       @"width=%d, height=%d. Result=%@",
       (int)[self hasMarkedText], (int)_cursorVisible, (int)shouldShowCursor,
       column, row, width, height, @(result));
  return result;
}
...
- (NSRange)rangeOfVisibleRows {
  int visibleRows =
      floor((_scrollViewContentSize.height -
...
- (NSRect)rectForCoordRange:(VT100GridCoordRange)coordRange {
  return NSMakeRect(coordRange.start.x * _cellSize.width +
                        [iTermPreferences intForKey:kPreferenceKeySideMargins],
...
- (NSRect)rectByGrowingRect:(NSRect)innerRect {
  ...
  NSPoint minPoint =
      NSMakePoint(MAX(0, innerRect.origin.x - extraWidth * _cellSize.width),
                  MAX(0, innerRect.origin.y - extraHeight * _cellSize.height));
  NSPoint maxPoint = NSMakePoint(
      MIN(frameSize.width, NSMaxX(innerRect) + extraWidth * _cellSize.width),
      MIN(frameSize.height,
          NSMaxY(innerRect) + extraHeight * _cellSize.height));
  NSRect outerRect = NSMakeRect(minPoint.x, minPoint.y, maxPoint.x - minPoint.x,
...
- (NSRange)rangeOfColumnsFrom:(CGFloat)x ofWidth:(CGFloat)width {
  NSRange charRange;
  charRange.location =
      MAX(0, (x - [iTermPreferences intForKey:kPreferenceKeySideMargins]) /
                 _cellSize.width);
  charRange.length =
      ceil(
          (x + width - [iTermPreferences intForKey:kPreferenceKeySideMargins]) /
          _cellSize.width) -
      charRange.location;
...
// Not inclusive of end.x or end.y. Range of coords clipped to addressable
// lines.
- (VT100GridCoordRange)drawableCoordRangeForRect:(NSRect)rect {
  VT100GridCoordRange range;
  NSRange charRange = [self rangeOfColumnsFrom:rect.origin.x
                                       ofWidth:rect.size.width];
...
- (NSColor *)selectionColorForCurrentFocus {
  if (_isFrontTextView) {
    return [_colorMap processedBackgroundColorForBackgroundColor:
                          [_colorMap colorForKey:kColorMapSelection]];
  } else {
...
- (void)updateCachedMetrics {
  ...
  _scrollViewDocumentVisibleRect =
      _delegate.enclosingScrollView.documentVisibleRect;
  _preferSpeedToFullLigatureSupport =
  ...
  _asciiLigaturesAvailable =
      (fontInfo.ligatureLevel > 0 || fontInfo.hasDefaultLigatures) &&
      _asciiLigatures;
}

- (void)startTiming {
  iTermPreciseTimerStatsMeasureAndRecordTimer(
      &_stats[TIMER_BETWEEN_CALLS_TO_DRAW_RECT]);
...
- (void)stopTiming {
  ...
  iTermPreciseTimerStatsRecordTimer(
      &_stats[TIMER_STAT_BUILD_MUTABLE_ATTRIBUTED_STRING]);
...
- (iTermCursorNeighbors)cursorNeighbors {
  return [iTermSmartCursorColor
      neighborsForCursorAtCoord:_cursorCoord
                       gridSize:_gridSize
...
- (void)cursorDrawCharacterAt:(VT100GridCoord)coord
  ...
  VT100GridCoordRange coordRange =
      VT100GridCoordRangeMake(coord.x, row, coord.x + width, row + 1);
  ...
  [self
      constructAndDrawRunsForLine:line
                              row:row
                          inRange:NSMakeRange(0, _gridSize.width)
                  startingAtPoint:NSMakePoint(
                                      [iTermPreferences
...
- (NSColor *)cursorColorForCharacter:(screen_char_t)screenChar
                      wantBackground:(BOOL)wantBackgroundColor
                               muted:(BOOL)muted {
  BOOL isBackground = [iTermTextDrawingHelper
      cursorUsesBackgroundColorForScreenChar:screenChar
