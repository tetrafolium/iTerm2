...
NSString *const kTerminalFileShouldStopNotification =
    @"kTerminalFileShouldStopNotification";
  ...
  if (self) {
    if (!name) {
      ...
      if (path) {
        ...
        [NSSavePanel setDirectoryURL:url
                           onceForID:@"TerminalFile"
                           savePanel:panel];
      }
      ...
      _localPath =
          [[self finalDestinationForPath:_filename
...
- (void)stop {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTerminalFileShouldStopNotification
                    object:self];
  [TransferrableFile unlockFileName:_localPath];
}
...
- (BOOL)appendData:(NSString *)data {
  ...
  if (approximateSize > self.fileSize + 5) {
    DLog(@"Have %@ bytes of base64 which encodes as much as %@ but the file's "
         @"declared size is %@",
...
- (void)handleEndOfData {
  ...
  if (destLength < 1) {
    [[FileTransferManager sharedInstance]
        ...
        didFinishTransmissionWithError:
            [self errorWithDescription:@"No data received."]];
    return;
  }
  ...
  if (resultLength < 0) {
    [[FileTransferManager sharedInstance]
                     transferrableFile:self
        didFinishTransmissionWithError:
            [self errorWithDescription:@"File corrupted (not valid base64)."]];
    return;
  }
  ...
  if (![data writeToFile:self.localPath atomically:NO]) {
    [[FileTransferManager sharedInstance]
                     transferrableFile:self
        didFinishTransmissionWithError:
            [self errorWithDescription:@"Failed to write file to disk."]];
    return;
  }
  if (![self quarantine:self.localPath sourceURL:nil]) {
    [[FileTransferManager sharedInstance]
                     transferrableFile:self
        didFinishTransmissionWithError:
            [self errorWithDescription:@"Failed to set quarantine."]];
    NSError *error = nil;
    const BOOL ok =
        [[NSFileManager defaultManager] removeItemAtPath:self.localPath
                                                   error:&error];
    if (!ok || error) {
