...
- (instancetype)init {
  ...
  if (self) {
    ...
    NSDictionary *tempDict =
        [userDefaults objectForKey:kUrlHandlersUserDefaultsKey];
    if (!tempDict) {
      // Iterate over old style url handlers (which stored bookmark by index)
      // and add guid->urlkey to urlHandlersByGuid.
      tempDict =
          [userDefaults objectForKey:kOldStyleUrlHandlersUserDefaultsKey];

      for (id key in tempDict) {
        int theIndex = [[tempDict objectForKey:key] intValue];
        if (theIndex >= 0 && theIndex < [profileModel numberOfBookmarks]) {
          NSString *guid =
              [[profileModel profileAtIndex:theIndex] objectForKey:KEY_GUID];
...
- (void)connectBookmarkWithGuid:(NSString *)guid toScheme:(NSString *)scheme {
  ...
  appURL = (NSURL *)LSCopyDefaultApplicationURLForURL(
  ...
  if (appURL == nil) {
    ...
    alert.messageText = [NSString
        stringWithFormat:@"iTerm is not the default handler for %@. "
  ...
  } else if (![[[NSFileManager defaultManager] displayNameAtPath:[appURL path]]
                 isEqualToString:@"iTerm 2"]) {
...
- (Profile *)profileForScheme:(NSString *)url {
  NSString *handlerId =
      (NSString *)LSCopyDefaultHandlerForURLScheme((CFStringRef)url);
  Profile *profile = nil;
  if ([handlerId isEqualToString:@"com.googlecode.iterm2"] ||
    ...
    profile = [[ProfileModel sharedInstance]
        bookmarkWithGuid:[self guidForScheme:url]];
  }
  if (handlerId) {
    CFRelease(handlerId);
  }
...
- (BOOL)pickApplicationToOpenFile:(NSString *)fullPath {
  ...
  if ([panel runModal] == NSModalResponseOK) {
    ...
    if (bundleId) {
      NSString *uti;
      NSError *error;
      if ([_currentFileUrlForOpenPanel getResourceValue:&uti
                                                 forKey:NSURLTypeIdentifierKey
                                                  error:&error]) {
        DLog(@"UTI is %@. Make it the default viewer.", uti);
        LSSetDefaultRoleHandlerForContentType((CFStringRef)uti, kLSRolesViewer,
...
- (BOOL)offerToPickApplicationToOpenFile:(NSString *)fullPath {
  NSAlert *alert = [[[NSAlert alloc] init] autorelease];
  alert.messageText = [NSString
      stringWithFormat:@"There is no application set to open the document “%@”",
...
- (BOOL)openFile:(NSString *)fullPath fragment:(NSString *)fragment {
  ...
  if (fragment) {
    NSURLComponents *components =
        [NSURLComponents componentsWithURL:[NSURL fileURLWithPath:fullPath]
...
- (BOOL)iTermIsDefaultTerminal {
  CFStringRef unixExecutableContentType =
      (CFStringRef) @"public.unix-executable";
  CFStringRef unixHandler = LSCopyDefaultRoleHandlerForContentType(
      unixExecutableContentType, kLSRolesShell);
  NSString *iTermBundleId = [[NSBundle mainBundle] bundleIdentifier];
...
- (void)setDefaultTerminal:(NSString *)bundleId {
  ...
  LSSetDefaultRoleHandlerForContentType(unixExecutableContentType,
                                        kLSRolesShell, (CFStringRef)bundleId);
}
...
- (BOOL)panel:(id)sender shouldEnableURL:(NSURL *)url {
  ...
  LSCanURLAcceptURL((CFURLRef)_currentFileUrlForOpenPanel, (CFURLRef)url,
                    kLSRolesViewer, kLSAcceptDefault, &acceptsItem);
