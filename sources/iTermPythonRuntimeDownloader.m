...
@implementation iTermPythonRuntimeDownloader {
  ...
  iTermPythonRuntimeDownloaderStatus
      _status;             // Set when _downloadGroup notified.
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _queue = dispatch_queue_create("com.iterm2.python-runtime",
                                   DISPATCH_QUEUE_SERIAL);
  }
  return self;
}

// Picks the largest 3-part version given a 2-part version. E.g., if you give
// it 3.7 and 3.7.0 and 3.7.1 exist in `versionsPath` it will return 3.7.1.
...
- (NSString *)executableNamed:(NSString *)name
  ...
  if (pythonVersion) {
    if (pythonVersion.it_twoPartVersionNumber) {
      bestVersion =
          [self threePartVersionForTwoPartVersion:pythonVersion
                                                      .it_twoPartVersionNumber
                                               at:path];
    } else {
  ...
  if ([[NSFileManager defaultManager]
          fileExistsAtPath:[path stringByAppendingPathComponent:bestVersion]]) {
...
- (NSString *)pip3At:(NSString *)root pythonVersion:(NSString *)pythonVersion {
  return [self executableNamed:@"pip3"
                   atPyenvRoot:root
...
- (NSString *)pyenvAt:(NSString *)root pythonVersion:(NSString *)pythonVersion {
  return [self executableNamed:@"python3"
                   atPyenvRoot:root
                 pythonVersion:pythonVersion
                    searchPath:root];
}

- (NSString *)pathToStandardPyenvPythonWithPythonVersion:
...
- (NSURL *)urlOfStandardEnvironmentContainerCreatingSymlinkForVersion:
    (NSString *)pythonVersion {
  NSString *path = [self pathToStandardPyenvWithVersion:pythonVersion];
...
- (BOOL)shouldDownloadEnvironmentForPythonVersion:(NSString *)pythonVersion
                        minimumEnvironmentVersion:
                            (NSInteger)minimumEnvironmentVersion {
...
// Returns 0 if no version is installed, otherwise returns the installed version
// of the python runtime.
- (int)installedVersionWithPythonVersion:(NSString *)pythonVersion {
  return [self versionInMetadataAtURL:
                   [self pathToMetadataWithPythonVersion:pythonVersion]];
}
...
- (void)performPeriodicUpgradeCheck {
  if (!_checkForUpdateRateLimit) {
    _checkForUpdateRateLimit = [[iTermPersistentRateLimitedUpdate alloc]
        initWithName:@"CheckForUpdatedPythonRuntime"];
...
- (void)
    downloadOptionalComponentsIfNeededWithConfirmation:(BOOL)confirm
                                         pythonVersion:(NSString *)pythonVersion
                             minimumEnvironmentVersion:
                                 (NSInteger)minimumEnvironmentVersion
  ...
  const int installedVersion =
      [self installedVersionWithPythonVersion:pythonVersion];
  [self checkForNewerVersionThan:MAX(minimumEnvironmentVersion - 1,
                                     installedVersion)
                        silently:YES
                         confirm:confirm
...
#if BETA
  url = [NSURL
      URLWithString:[iTermAdvancedSettingsModel pythonRuntimeBetaDownloadURL]];
#else
  if ([iTermPreferences boolForKey:kPreferenceKeyCheckForTestReleases]) {
    url = [NSURL URLWithString:[iTermAdvancedSettingsModel
                                   pythonRuntimeBetaDownloadURL]];
  } else {
    url = [NSURL
        URLWithString:[iTermAdvancedSettingsModel pythonRuntimeDownloadURL]];
  }
#endif
  __weak __typeof(self) weakSelf = self;
  __block BOOL stillNeedsConfirmation = confirm;
  iTermManifestDownloadPhase *manifestPhase = [[iTermManifestDownloadPhase
      ...
      requestedPythonVersion:pythonVersion
            nextPhaseFactory:^iTermOptionalComponentDownloadPhase *(
              ...
              iTermManifestDownloadPhase *mphase =
                  [iTermManifestDownloadPhase castFrom:currentPhase];
              ...
              if (stillNeedsConfirmation) {
                ...
                if (requiredToContinue) {
                  alert.informativeText = [NSString
                      stringWithFormat:
                          @"The Python Runtime is used by Python scripts that "
                          @"work with iTerm2. It must be downloaded to "
                          @"complete the requested action. The download is "
                          @"about %@. OK to download it now?",
                          [NSString it_formatBytes:info.size]];
                } else {
                  alert.informativeText = [NSString
                      stringWithFormat:
                          @"The Python Runtime is used by Python scripts that "
                          @"work with iTerm2. The download is about %@. OK to "
                          @"download it now?",
                          [NSString it_formatBytes:info.size]];
                }
              ...
              if (silent) {
                [strongSelf->_downloadController.window
              ...
              return [[iTermPayloadDownloadPhase alloc]
                  ...
                  requestedPythonVersion:mphase.requestedPythonVersion
                        ...
                        nextPhaseFactory:^iTermOptionalComponentDownloadPhase *(
                            iTermOptionalComponentDownloadPhase
                                *completedPhase) {
                          const BOOL shouldContinue = [weakSelf
                              payloadDownloadPhaseDidComplete:
                                  (iTermPayloadDownloadPhase *)completedPhase
                                             sitePackagesOnly:
                                                 info.isSitePackagesOnly
                                          latestFullComponent:
                                              latestFullComponent];
                          if (!shouldContinue) {
                            return nil;
                          }
                          return [[iTermInstallingPhase alloc]
                                   initWithURL:nil
                                         title:@"Download Finished"
                              nextPhaseFactory:nil];
                        }];
            }];
  _downloadController.completion = ^(
      iTermOptionalComponentDownloadPhase *lastPhase) {
    if (lastPhase.error) {
    ...
    if (lastPhase == manifestPhase) {
      iTermPythonRuntimeDownloader *strongSelf = weakSelf;
      [strongSelf
          didStopCheckAfterReceivingManifestBecauseDeclined:declined
...
- (void)didStopCheckAfterReceivingManifestBecauseDeclined:(BOOL)declined
  ...
  if (_status == iTermPythonRuntimeDownloaderStatusWorking) {
    // You can get here when the manifest download completes and it decides not
    // to keep going, for example if the requested version was not available.
...
- (BOOL)showDownloadFailedAlertWithError:(NSError *)error
  ...
  if (pythonVersion) {
    alert.informativeText = [NSString
        stringWithFormat:@"An iTerm2 Python Runtime with Python version %@ "
                         @"must be downloaded to proceed.%@",
                         pythonVersion, reason];
  } else {
    alert.informativeText = [NSString
        stringWithFormat:
            @"An iTerm2 Python Runtime must be downloaded to proceed.%@",
            reason];
  }
  [alert runModal];
  return NO;
...
- (BOOL)payloadDownloadPhaseDidComplete:
  ...
  NSString *pubkey = [NSString
      stringWithContentsOfURL:[[NSBundle mainBundle] URLForResource:@"rsa_pub"
                                                      withExtension:@"pem"]
                     encoding:NSUTF8StringEncoding
                        error:nil];
  NSError *verifyError =
      [iTermSignatureVerifier validateFileURL:tempURL
  ...
  if (verifyError) {
    ...
    alert.informativeText =
        [NSString stringWithFormat:
                      @"The Python runtime's signature failed validation: %@",
    ...
    void (^completion)(BOOL) = ^(BOOL ok) {
      if (ok) {
        ...
        [self->_downloadController.window close];
        self->_downloadController = nil;
        self->_status = ok ? iTermPythonRuntimeDownloaderStatusDownloaded
      ...
      alert.informativeText = @"An error occurred while unzipping the "
                              @"downloaded python environment";
      [alert runModal];
...
+ (NSString *)bestPythonVersionAt:(NSString *)path {
  // TODO: This is convenient but I'm not sure it's technically correct for all
  // possible Python versions. But it'll do for three dotted numbers, which is
  // the norm.
  SUStandardVersionComparator *comparator =
      [[SUStandardVersionComparator alloc] init];
...
- (NSSet<NSString *> *)twoPartPythonVersionsInRuntimeVersion:
    (NSInteger)pythonVersion {
  NSString *const path =
      [self pathToStandardPyenvWithVersion:[@(pythonVersion) stringValue]];
  NSString *versionsPath = [path stringByAppendingPathComponent:@"versions"];
  NSArray<NSString *> *versions =
      [iTermPythonRuntimeDownloader pythonVersionsAt:versionsPath];
  NSArray<NSString *> *twoPartVersions =
      iTermConvertThreePartVersionNumbersToTwoPart(versions);
...
- (void)installSitePackagesFromZip:(NSString *)zip
               ...
               latestFullComponent:(NSInteger)latestFullComponent
                        completion:(void (^)(BOOL))completion {
  NSURL *const finalDestination = [NSURL
      fileURLWithPath:[self pathToStandardPyenvWithVersion:[@(runtimeVersion)
                                                               stringValue]]];
  NSURL *const tempDestination = [NSURL
      fileURLWithPath:[[[NSFileManager defaultManager]
                          applicationSupportDirectory]
                          stringByAppendingPathComponent:[[NSUUID UUID]
                                                             UUIDString]]];

  NSURL *const sourceURL = [NSURL
      fileURLWithPath:[self
                          pathToStandardPyenvWithVersion:[@(latestFullComponent)
                                                             stringValue]]];
  [[NSFileManager defaultManager] removeItemAtPath:finalDestination.path
                                             error:nil];
  NSURL *overwritableURL =
      [tempDestination URLByAppendingPathComponent:@"iterm2env"];
  NSError *error = nil;
  ...
  [self
      copyFullEnvironmentFrom:sourceURL
                           to:overwritableURL
                   completion:^(BOOL ok) {
                     if (!ok) {
                       completion(NO);
                     ...
                     NSString *searchFor1 =
                         [NSString stringWithFormat:@"/iterm2env-%@/",
                                                    @(latestFullComponent)];
                     NSString *replaceWith1 = [NSString
                         stringWithFormat:@"/iterm2env-%@/", @(runtimeVersion)];
                     NSString *searchFor2 =
                         [NSString stringWithFormat:@"/iterm2env-%@\"",
                                                    @(latestFullComponent)];
...
- (void)installPythonEnvironmentFromZip:(NSString *)zip
                         ...
                         pythonVersions:(NSArray<NSString *> *)pythonVersions
                             completion:(void (^)(BOOL))completion {
  NSURL *finalDestination = [NSURL
      fileURLWithPath:[self pathToStandardPyenvWithVersion:[@(runtimeVersion)
                                                               stringValue]]];
  NSURL *tempDestination = [NSURL
      fileURLWithPath:[[[NSFileManager defaultManager]
                          applicationSupportDirectory]
                          stringByAppendingPathComponent:[[NSUUID UUID]
                                                             UUIDString]]];
  ...
  [self unzip:[NSURL fileURLWithPath:zip]
              to:tempDestination
      completion:^(BOOL unzipOk) {
        if (unzipOk) {
          NSString *backslashEscaped = [finalDestination.path
          ...
          NSDictionary<NSString *, NSDictionary *> *subs = @{
            ...
            @"" : @{
              ...
              @"#!__ITERM2_ENV__" : [NSString
                  stringWithFormat:@"#!/usr/bin/env -S \"%@\"",
                                   finalDestination.path.it_escapedForEnv],
              @"__ITERM2_ENV__" : finalDestination.path,
              @"__ITERM2_PYENV__" : [finalDestination.path
                  stringByAppendingPathComponent:@"pyenv"]
            }
          };
...
static NSArray<NSString *> *iTermConvertThreePartVersionNumbersToTwoPart(
    NSArray<NSString *> *pythonVersions) {
  return [pythonVersions mapWithBlock:^id(NSString *possibleThreePart) {
    NSArray<NSString *> *parts =
        [possibleThreePart componentsSeparatedByString:@"."];
    if (parts.count == 3) {
...
- (void)finishInstallingRuntimeVersion:(int)runtimeVersion
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray<NSString *> *twoPartVersions =
            iTermConvertThreePartVersionNumbersToTwoPart(pythonVersions);
        NSArray<NSString *> *extendedPythonVersions =
            [NSSet
                setWithArray:[pythonVersions
                                 arrayByAddingObjectsFromArray:twoPartVersions]]
                .allObjects;
        [self createDeepLinksTo:[tempDestination.path
                                    stringByAppendingPathComponent:@"iterm2env"]
                 runtimeVersion:runtimeVersion
                    forVersions:extendedPythonVersions];
        [self createDeepLinkTo:[tempDestination.path
                                   stringByAppendingPathComponent:@"iterm2env"]
                 pythonVersion:nil
                runtimeVersion:runtimeVersion];
        [[NSFileManager defaultManager]
            moveItemAtURL:[tempDestination
                              URLByAppendingPathComponent:@"iterm2env"]
                    toURL:finalDestination
                    error:nil];
        // Delete older versions that have the same Python versions.
        NSSet<NSString *> *versionsToRemove =
            [NSSet setWithArray:twoPartVersions];
        for (int i = 1; i < runtimeVersion; i++) {
          NSSet<NSString *> *pythonVersionsInOldRuntime =
              [self twoPartPythonVersionsInRuntimeVersion:i];
          if (![pythonVersionsInOldRuntime isSubsetOfSet:versionsToRemove]) {
            // There’s at least one python version in the old runtime that isn’t
            // in the new one so keep it around.
            continue;
          }
          [[NSFileManager defaultManager]
              removeItemAtPath:
                  [self pathToStandardPyenvWithVersion:[@(i) stringValue]]
                         error:nil];
        }
        [[NSFileManager defaultManager] removeItemAtURL:tempDestination
                                                  error:nil];
        dispatch_async(dispatch_get_main_queue(), ^{
          completion(YES);
        });
...
- (void)createDeepLinkTo:(NSString *)container
  ...
  if (runtimeVersion > existingVersion) {
    NSString *pathToVersionedEnvironment =
        [self pathToStandardPyenvWithVersion:pythonVersion];
...
- (void)createDeepLinksTo:(NSString *)container
  ...
  for (NSString *pythonVersion in pythonVersions) {
    [self createDeepLinkTo:container
             pythonVersion:pythonVersion
            runtimeVersion:runtimeVersion];
  }
}
...
- (void)installPythonEnvironmentTo:(NSURL *)container
                ...
                environmentVersion:(NSInteger)environmentVersion
                    ...
                    createSetupCfg:(BOOL)createSetupCfg
                        completion:
                            (void (^)(iTermInstallPythonStatus))completion {
  NSString *source = [self pathToStandardPyenvWithVersion:pythonVersion];
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError *error = nil;
        NSString *destination =
            [container URLByAppendingPathComponent:@"iterm2env"].path;
        BOOL ok;
        ok =
            [[NSFileManager defaultManager] createDirectoryAtPath:container.path
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
        if (!ok) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          ...
          // pip3 must use the python in this environment so it will install new
          // dependencies to the right place.
          NSString *const pathToEnvironment =
              [container.path stringByAppendingPathComponent:@"iterm2env"];
          NSString *const pip3 = [self pip3At:pathToEnvironment
                                pythonVersion:pythonVersion];
          NSString *const pathToPython = [self
              executableNamed:@"python3"
                  atPyenvRoot:[eventualLocation.path
                                  stringByAppendingPathComponent:@"iterm2env"]
                pythonVersion:pythonVersion
                   searchPath:source];

          // Replace the shebang in pip3 to point at the right version of
          // python.
          ...
          // because it does a search-and-replace on the iterm2env-XX part.
          [self
              replaceShebangInScriptAtPath:pip3
                                      with:[NSString
                                               stringWithFormat:
                                                   @"#!/usr/bin/env -S \"%@\"",
                                                   envEscapedPathToPython]];
                    ...
                    pythonVersion:pythonVersion
                       completion:^(NSArray<NSString *> *failures,
                         ...
                         if (failures.count) {
                           ...
                           NSString *failureList = [[failures
                               sortedArrayUsingSelector:@selector(compare:)]
                               componentsJoinedByString:@", "];
                           alert.informativeText = [NSString
                               stringWithFormat:@"The following dependencies "
                                                @"failed to install: %@",
                                                failureList];
                           ...
                           for (NSInteger i = 0; i < failures.count; i++) {
                             ...
                             NSString *stringOutput = [[NSString alloc]
                                 initWithData:output
                                     encoding:NSUTF8StringEncoding];
                             if (!stringOutput) {
                               stringOutput = [[NSString alloc]
                                   initWithData:output
                           ...
                           iTermDisclosableView *accessory =
                               [[iTermDisclosableView alloc]
                                   initWithFrame:NSZeroRect
                                          prompt:@"Output"
                                         message:[messages
                                                     componentsJoinedByString:
                                                         @"\n\n"]];
                           accessory.frame = NSMakeRect(
                               0, 0, accessory.intrinsicContentSize.width,
                           ...
                           accessory.requestLayout = ^{
                             ...
                             if (@available(macOS 10.16, *)) {
                               // FB8897296:
                               // Prior to Big Sur, you could call [NSAlert
                               // layout] on an already-visible NSAlert to have
                               // it change its size to accommodate an accessory
                               // view controller whose frame changed.
                               //
                               // On Big Sur, it no longer works. Instead, you
                               // must call NSAlert.layout *twice*.
                               [alert layout];
                             }
                           };
                         ...
                         NSString *pythonVersionToUse =
                             pythonVersion ?: [self.class latestPythonVersion];
...
- (void)replaceShebangInScriptAtPath:(NSString *)scriptPath
                                with:(NSString *)newShebang {
  NSError *error = nil;
  NSString *script = [NSString stringWithContentsOfFile:scriptPath
                                               encoding:NSUTF8StringEncoding
                                                  error:&error];
  if (!script) {
    DLog(@"Failed to replace shebang in %@ because I couldn't read the file "
         @"contents: %@",
         scriptPath, error);
    return;
  }
  NSMutableArray<NSString *> *lines =
      [[script componentsSeparatedByString:@"\n"] mutableCopy];
  if (lines.count == 0) {
    DLog(@"Empty script at %@", scriptPath);
    return;
  }
  if (![lines.firstObject hasPrefix:@"#!/"]) {
    DLog(@"First line of %@ is not a shebang: %@", scriptPath,
         lines.firstObject);
    return;
  }
  const BOOL unlinkedOk =
      [[NSFileManager defaultManager] removeItemAtPath:scriptPath error:&error];
  if (!unlinkedOk) {
  ...
  const BOOL ok = [fixedScript writeToFile:scriptPath
                                atomically:NO
                                  encoding:NSUTF8StringEncoding
                                     error:&error];
  if (!ok) {
  ...
  const BOOL chmodOk = [[NSFileManager defaultManager]
      setAttributes:@{NSFilePosixPermissions : @(0755)}
      ...
      runPip3InContainer:container
           ...
           withArguments:@[ @"install", dependencies.firstObject ]
              completion:^(BOOL thisOK, NSData *output) {
                if (!thisOK) {
                  completion(@[ dependencies.firstObject ],
                             @[ output ?: [NSData data] ]);
                  return;
                }
                [self
                    installDependencies:[dependencies subarrayFromIndex:1]
                                     to:container
                          pythonVersion:pythonVersion
                             completion:^(NSArray<NSString *> *failures,
                                          NSArray<NSData *> *outputs) {
                               if (!thisOK) {
                                 completion(
                                     [failures
                                         arrayByAddingObject:dependencies
                                                                 .firstObject],
...
- (void)runPip3InContainer:(NSURL *)container
  ...
  NSString *pip3 =
      [self pip3At:[container.path stringByAppendingPathComponent:@"iterm2env"]
          pythonVersion:pythonVersion];
  if (!pip3) {
    completion(
        NO, [[NSString
                stringWithFormat:@"pip3 not found for python version %@ in %@",
                                 pythonVersion, container.path]
                dataUsingEncoding:NSUTF8StringEncoding]);
    return;
  }
  NSMutableData *output = [NSMutableData data];
  iTermCommandRunner *runner =
      [[iTermCommandRunner alloc] initWithCommand:pip3
                                    withArguments:arguments
                                             path:container.path];
  NSString *identifier = [runner description];
  runner.outputHandler = ^(NSData *data) {
    DLog(@"Runner %@ recvd: %@", identifier,
...
- (BOOL)writeInputStream:(NSInputStream *)inputStream
                  toFile:(NSString *)destination {
  NSOutputStream *outputStream =
      [NSOutputStream outputStreamToFileAtPath:destination append:NO];
  [outputStream open];
  NSMutableData *buffer = [NSMutableData dataWithLength:4096];
  BOOL ok = NO;
  NSInteger total = 0;
  while (YES) {
    NSInteger n = [inputStream read:buffer.mutableBytes
...
// Keys in `subs` can be like *.ext, Filename.txt, or "" (empty string). If
// neither the extension nor exact file name match, the empty string dict is
// used as a fallback.
- (void)performSubstitutions:
            (NSDictionary<NSString *, NSDictionary<NSString *, NSString *> *> *)
                subs
          inFilesUnderFolder:(NSURL *)folderURL {
  NSDictionary<NSString *, NSDictionary<NSData *, NSData *> *> *dataSubs = [subs
      mapValuesWithBlock:^id(NSString *key,
                             NSDictionary<NSString *, NSString *> *object) {
        return [object mapWithBlock:^iTermTuple *(NSString *key,
                                                  NSString *object) {
          return [iTermTuple
              tupleWithObject:[key dataUsingEncoding:NSUTF8StringEncoding]
                    andObject:[object dataUsingEncoding:NSUTF8StringEncoding]];
        }];
      }];
  NSDirectoryEnumerator *directoryEnumerator =
      [[NSFileManager defaultManager] enumeratorAtPath:folderURL.path];
...
- (void)performSubstitutions:(NSDictionary *)subs inFile:(NSString *)path {
  ...
  [subs enumerateKeysAndObjectsUsingBlock:^(
            NSData *_Nonnull key, NSData *_Nonnull obj, BOOL *_Nonnull stop) {
