...
- (instancetype)
  ...
  if (self) {
    ...
    _childPidRef = [[iTermVariableReference alloc]
        initWithPath:iTermVariableKeySessionChildPid
              vendor:scope];
...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconJobs"
                                forClass:[self class]];
}
...
- (NSString *)statusBarComponentDetailedDescription {
  return @"Shows the currently running job. If space permits, parent process "
...
- (void)updateTextFieldIfNeeded {
  int pid =
      [NSNumber
          castFrom:[self.scope
                       valueForVariableName:iTermVariableKeySessionJobPid]]
          .intValue;
...
- (NSArray<NSString *> *)newAncestryChainForPid:(int)pid {
  iTermProcessInfo *deepestForegroundJob =
      [[iTermProcessCache sharedInstance] processInfoForPid:pid];
  int sessionTaskPid =
      [NSNumber
          castFrom:[self.scope
                       valueForVariableName:iTermVariableKeySessionChildPid]]
          .intValue;

  iTermProcessInfo *current = deepestForegroundJob;
  NSMutableArray<NSString *> *chain = [NSMutableArray array];
  while (current) {
    if (current.processID == sessionTaskPid &&
        [current.name isEqualToString:@"login"]) {
      // Don't include login.
      break;
    }
...
- (NSArray<NSString *> *)variantsOfChain:(NSArray<NSString *> *)chain {
  ...
  for (NSInteger i = 0; i < chain.count; i++) {
    NSArray<NSString *> *subarray =
        [chain subarrayWithRange:NSMakeRange(0, i + 1)];
...
- (void)statusBarComponentDidClickWithView:(NSView *)view {
  ...
  pid_t pid = [[self.scope valueForVariableName:iTermVariableKeySessionChildPid]
      integerValue];
