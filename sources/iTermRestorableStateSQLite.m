...
@interface iTermRestorableStateSQLiteRecord
...
- (void)didFinishRestoring {
  // Does nothing because the point of this thing is to not redo unnecessary
  // work.
}
...
- (nonnull id<iTermRestorableStateRecord>)recordWithPayload:
    (nonnull id)payload {
  return self;
}
...
@interface iTermRestorableStateSQLiteIndex
    : NSObject <iTermRestorableStateIndex>
@end
...
- (id<iTermRestorableStateRecord>)restorableStateRecordAtIndex:(NSUInteger)i {
  ...
  NSError *error = nil;
  NSInteger windowNumber = [_windows[i] integerWithKey:@"__windowNumber"
                                                 error:&error];
  if (error || !identifier) {
    DLog(@"identifier=%@, error=%@", identifier, error);
    return nil;
  }
  return [[iTermRestorableStateSQLiteRecord alloc]
      initWithIndex:i
...
- (void)initializeDb {
  ...
  iTermGraphDatabase *db = [[iTermGraphDatabase alloc]
      initWithDatabase:[[iTermSqliteDatabaseImpl alloc] initWithURL:_url]];
  _db = db;
}
...
- (void)loadRestorableStateIndexWithCompletion:
  ...
  [_db whenReady:^{
    ...
    completion([[iTermRestorableStateSQLiteIndex alloc]
        initWithGraphRecord:self->_db.record]);
  }];
}
...
- (void)restoreWindowWithRecord:(id<iTermRestorableStateRecord>)record
  ...
  [self.delegate
      restorableStateRestoreWithRecord:windowRecord
                            identifier:identifier
...
- (BOOL)saveSynchronously:(BOOL)sync withCompletion:(void (^)(void))completion {
  ...
  NSArray<NSString *> *identifiers = [windows mapWithBlock:^id(
                                                  NSWindow *window) {
    if (![window.delegate conformsToProtocol:@protocol(iTermGraphCodable)]) {
      return nil;
    }
    if (![window.delegate
            conformsToProtocol:@protocol(iTermUniquelyIdentifiable)]) {
      return nil;
    }
    id<iTermUniquelyIdentifiable> identifiable =
        (id<iTermUniquelyIdentifiable>)window.delegate;
    return identifiable.stringUniqueIdentifier;
  }];
  void (^update)(iTermGraphEncoder *_Nonnull) = ^(
      iTermGraphEncoder *_Nonnull encoder) {
    // Encode state for each window.
    [encoder
        encodeArrayWithKey:@"windows"
               ...
               identifiers:identifiers
                   options:0
                     block:^BOOL(NSString *_Nonnull identifier, NSInteger index,
                       ...
                       [subencoder encodeString:identifier
                                         forKey:@"__identifier"];
                       [subencoder encodeNumber:@(window.windowNumber)
                                         forKey:@"__windowNumber"];
                       id<iTermGraphCodable> codable =
                           (id<iTermGraphCodable>)window.delegate;
  ...
  iTermCallback *callback = [iTermCallback
      onThread:[iTermThread main]
         block:^(iTermMainThreadState *state, id _Nullable result) {
           completion();
         }];
