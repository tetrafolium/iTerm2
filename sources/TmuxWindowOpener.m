...
NSString *const kTmuxWindowOpenerWindowOptionStyleValueFullScreen =
    @"FullScreen";
...
- (BOOL)openWindows:(BOOL)initial {
  ...
  if (!self.layout) {
    [gateway_
        abortWithErrorMessage:
            [NSString stringWithFormat:@"Can't open window: missing layout"]];
    return NO;
  }
  self.parseTree =
      [[TmuxLayoutParser sharedInstance] parsedLayoutFromString:self.layout];
  if (!self.parseTree) {
    [gateway_
        abortWithErrorMessage:[NSString
                                  stringWithFormat:@"Error parsing layout %@",
                                                   self.layout]];
    return NO;
  }
  ...
  [[TmuxLayoutParser sharedInstance]
      depthFirstSearchParseTree:self.parseTree
                callingSelector:@selector(appendRequestsForNode:toArray:)
                       onTarget:self
                     withObject:cmdList];
  if (self.zoomed.boolValue) {
    // Unzoom the window because there's no way to tell which window pane is
    // zoomed in tmux 2.1. I submitted a patch to tmux (commit
    // 531869bd92f0daff3cc3c3cc0ab273846f411dc8) to correct this. Once a fixed
    // version of tmux is ubiquitous, I can improve this by respecting the tmux
    // server's initial setting of the zoomed flag. This is a race condition
    // because another client could change the window's zoom status at the same
    // time, causing a mess.
...
- (BOOL)updateLayoutInTab:(PTYTab *)tab {
  ...
  [tab setTmuxLayout:self.parseTree tmuxController:controller_ zoomed:_zoomed];
  return YES;
}
...
- (NSDictionary *)dictToUnpauseWindowPane:(NSNumber *)wp {
  ...
  // If we are unpausing the pane at the user's behest, this is obviously
  // important. The tricky case: if we got unpaused before accepting
  // notifications, this unpauses the pane.
  NSString *command =
      [NSString stringWithFormat:@"refresh-client -A '%%%@:continue'", wp];
  return
...
- (NSDictionary *)dictToToggleZoomForWindow {
  ...
  NSString *command =
      [NSString stringWithFormat:@"resize-pane -Z -t @%d", self.windowIndex];
  return
      [gateway_ dictionaryForCommand:command
                      responseTarget:self
                    responseSelector:@selector(requestDidComplete)
...
- (NSDictionary *)dictForGetPendingOutputForWindowPane:(NSNumber *)wp {
  ...
  NSString *command = [NSString
      stringWithFormat:@"capture-pane -p -P -C -t \"%%%d\"", [wp intValue]];
  return
      [gateway_ dictionaryForCommand:command
                      responseTarget:self
...
- (NSDictionary *)dictForGetUserVars:(NSNumber *)wp {
  ...
  DLog(@"Increment pending requests to %d", pendingRequests_);
  NSString *command = [NSString
      stringWithFormat:@"show-options -v -p -t %%%d @uservars", [wp intValue]];
  return
      [gateway_ dictionaryForCommand:command
                      responseTarget:self
...
- (NSDictionary *)dictForDumpStateForWindowPane:(NSNumber *)wp {
  ...
  DLog(@"Increment pending requests to %d", pendingRequests_);
  NSString *command =
      [NSString stringWithFormat:@"list-panes -t \"%%%d\" -F \"%@\"",
                                 [wp intValue], [TmuxStateParser format]];
  return
      [gateway_ dictionaryForCommand:command
                      responseTarget:self
...
- (NSDictionary *)dictForRequestHistoryForWindowPane:(NSNumber *)wp
  ...
  if (self.minimumServerVersion &&
      [self.minimumServerVersion
          compare:[NSDecimalNumber decimalNumberWithString:@"3.1"]] !=
          NSOrderedAscending) {
    maybeN = @"N";
  }
  NSString *command =
      [NSString stringWithFormat:@"capture-pane -peqJ%@ %@-t \"%%%d\" -S -%d",
                                 maybeN, (alternate ? @"-a " : @""),
                                 [wp intValue], self.maxHistory];
  return [gateway_
      dictionaryForCommand:command
            responseTarget:self
          responseSelector:@selector(dumpHistoryResponse:paneAndAlternate:)
            responseObject:[NSArray
                               arrayWithObjects:wp,
                                                [NSNumber
                                                    numberWithBool:alternate],
                                                nil]
                     flags:kTmuxGatewayCommandShouldTolerateErrors];
}
...
- (void)dumpHistoryResponse:(NSString *)response
  ...
  NSArray *history = [[TmuxHistoryParser sharedInstance]
      parseDumpHistoryResponse:response
...
- (void)getPendingOutputResponse:(NSData *)response pane:(NSNumber *)wp {
  ...
  for (int i = 0; i < response.length; i++) {
    ...
    if (c == '\\' && response.length >= i + 4) {
      // tmux has a bug where control characters get escaped but backslashes do
      // not. Only accept octal values that are control codes to minimize the
      // chance of problems.
  ...
  NSMutableDictionary *state =
      [[[states_ objectForKey:wp] mutableCopy] autorelease];
...
- (void)dumpStateResponse:(NSString *)response pane:(NSNumber *)wp {
  ...
  NSDictionary *state =
      [[TmuxStateParser sharedInstance] parsedStateFromString:response
...
- (void)requestDidComplete {
  ...
  const BOOL useOriginalWindow =
      ...
      kOpenTmuxWindowsAsNativeTabsInExistingWindow;
  ...
  if (tabToUpdate_) {
    ...
    if (![self.controller window:windowIndex_]) {
      ...
      NSArray *panes =
          [[TmuxLayoutParser sharedInstance] windowPanesInParseTree:parseTree];
      windowPos = [[[self.controller positionForWindowWithPanes:panes
                                                       windowID:windowIndex_]
          retain] autorelease];
      ...
      // screen).
      DLog(@"Calling windowDidResize: in case the window was smaller than we'd "
           @"hoped");
      ...
      BOOL wantFullScreen =
          [style isEqual:kTmuxWindowOpenerWindowOptionStyleValueFullScreen];
      ...
      if (wantFullScreen && !isFullScreen) {
        ...
        if ([iTermAdvancedSettingsModel
                serializeOpeningMultipleFullScreenWindows]) {
  ...
  if (windowPos) {
    if (!useOriginalWindow || initialTabs < 2 || !self.initial) {
      // Do this after calling the completion selector because it may affect the
      // window's frame (e.g., when burying a session and that causes the number
      // of tabs to change).
      [[term window] setFrameOrigin:[windowPos pointValue]];
    }
  }
  if (isNewWindow) {
    [[iTermController sharedInstance]
        didFinishCreatingTmuxWindow:(PseudoTerminal *)term];
  }
  if (self.completion) {
    self.completion(windowIndex_);
  }
...
- (void)decorateParseTree:(NSMutableDictionary *)parseTree {
  [[TmuxLayoutParser sharedInstance]
      depthFirstSearchParseTree:parseTree
