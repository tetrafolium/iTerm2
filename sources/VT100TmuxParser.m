...
- (BOOL)handleInput:(iTermParserContext *)context
  ...
  if (bytesTilNewline == -1) {
    ...
    // Append bytes up to the newline, stripping out linefeeds. Consume the
    // newline.
...
- (BOOL)processLineIntoToken:(VT100Token *)result {
  ...
  NSString *command =
      [[[NSString alloc] initWithData:_line
                             encoding:NSUTF8StringEncoding] autorelease];

  if (!command) {
    // The command was not UTF-8. Unfortunately, this can happen. If tmux has a
    // non-UTF-8 character in a pane, it will just output it in capture-pane.
    command =
        [[[NSString alloc] initWithUTF8DataIgnoringErrors:_line] autorelease];
  }

  if (_recoveryMode) {
    // In recovery mode, we always ignore the first line unless it is a %begin
    // or %exit. That's because we expect we came into an existing connection,
    // but not one that is responding to a command. We could start reading in
    // the middle of a notification, such as half the line of an %output.
    // Notifications always fit on a single line so ignoring the first line is
    // safe. If we came in to a silent connection then the first thing we'll get
    // back from the server that we do expect to see is a response, so we don't
    // want to ignore that.
    _recoveryMode = NO;
  ...
  if (_inResponseBlock) {
    if ([command hasPrefix:@"%exit"]) {
      ...
      // TODO: test tmux 1.9 and make sure this code can be removed, then remove
      // it.
      ...
      if (parts.count >= 3 && [_currentCommandId isEqual:parts[1]] &&
