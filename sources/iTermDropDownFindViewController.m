...
- (void)open {
  ...
  [[NSAnimationContext currentContext] setDuration:kAnimationDuration];
  DLog(@"Animate find view %@ to full size frame: %@", self.view,
       NSStringFromRect([self fullSizeFrame]));
...
- (NSArray *)control:(NSControl *)control
  ...
  DLog(@"completions:forPartialWordRange: existing string is %@, range is "
       @"%@\n%@",
       textView.string, NSStringFromRange(charRange),
...
- (BOOL)control:(NSControl *)control
  ...
  if (control != findBarTextField_) {
    DLog(@"Wrong control. I'm %@ but was sent by %@", findBarTextField_,
         control);
    return NO;
  }
...
- (void)controlTextDidEndEditing:(NSNotification *)aNotification {
  ...
  if (postingObject != findBarTextField_) {
    DLog(@"Wrong object. I'm %@, but posting object is %@", self,
         postingObject);
    return;
  }
...
- (NSRect)fullSizeFrame {
  return NSMakeRect([[self view] frame].origin.x,
                    fullFrame_.origin.y + [self superframe].size.height,
                    [[self view] frame].size.width, fullFrame_.size.height);
}
