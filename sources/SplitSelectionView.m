...
- (void)_showMessage:(NSString *)message inRect:(NSRect)frame {
  ...
  do {
    ...
    NSDictionary *attrs = [NSDictionary
        dictionaryWithObjectsAndKeys:[NSFont systemFontOfSize:fontSize],
                                     NSFontAttributeName, [NSColor whiteColor],
                                     NSForegroundColorAttributeName, pStyle,
    ...
    rect = NSMakeRect(frame.origin.x,
                      ...
                      frame.size.width, attributedString.size.height);
    if (fontSize < 8) {
      break;
    }
  ...
  [theShadow set];
  [attributedString drawWithRect:rect options:0];
}
...
- (void)drawTargetWithMessage:(NSString *)theMessage {
  ...
  case kSouthHalf:
    NSDivideRect([self frame], &highlightRect, &clearRect, rect.size.height / 2,
                 NSMinYEdge);
    break;
  ...
  case kWestHalf:
    NSDivideRect([self frame], &highlightRect, &clearRect, rect.size.width / 2,
                 NSMinXEdge);
    break;
...
- (void)updateAtPoint:(NSPoint)point {
  ...
  if (point.x < self.frame.size.width / 2) {
    ...
    scores[numPossibilities] =
        (self.frame.size.width - point.x) / self.frame.size.width;
  ...
  if (point.y < self.frame.size.height / 2) {
    ...
    scores[numPossibilities] =
        (self.frame.size.height - point.y) / self.frame.size.height;
