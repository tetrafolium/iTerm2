...
static NSString *const iTermToolSnippetsPasteboardType =
    @"iTermToolSnippetsPasteboardType";

@interface iTermToolSnippets () <NSDraggingDestination, NSTableViewDataSource,
                                 NSTableViewDelegate>
@end
...
static NSButton *iTermToolSnippetsNewButton(NSString *imageName,
                                            NSString *title, id target,
                                            SEL selector, NSRect frame) {
  ...
  if (imageName) {
    if (@available(macOS 10.16, *)) {
      button.image = [NSImage it_imageForSymbolName:imageName
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    if (@available(macOS 10.16, *)) {
      _applyButton = iTermToolSnippetsNewButton(@"play", @"Send", self,
                                                @selector(apply:), frame);
      _addButton = iTermToolSnippetsNewButton(@"plus", @"Add", self,
                                              @selector(add:), frame);
      _removeButton = iTermToolSnippetsNewButton(@"minus", @"Remove", self,
                                                 @selector(remove:), frame);
      _editButton = iTermToolSnippetsNewButton(@"switch.2", @"Edit", self,
                                               @selector(edit:), frame);
    } else {
      _applyButton = iTermToolSnippetsNewButton(nil, @"Send", self,
                                                @selector(apply:), frame);
      _addButton = iTermToolSnippetsNewButton(NSImageNameAddTemplate, nil, self,
                                              @selector(add:), frame);
      _removeButton = iTermToolSnippetsNewButton(
          NSImageNameRemoveTemplate, nil, self, @selector(remove:), frame);
      _editButton =
          iTermToolSnippetsNewButton(nil, @"✐", self, @selector(edit:), frame);
    }
    ...
    _scrollView = [[NSScrollView alloc]
        initWithFrame:NSMakeRect(0, 0, frame.size.width,
                                 frame.size.height - kButtonHeight - kMargin)];
    ...
    NSTableColumn *valueColumn =
        [[NSTableColumn alloc] initWithIdentifier:@"value"];
    [valueColumn setEditable:NO];
    [_tableView addTableColumn:valueColumn];

    _tableView.columnAutoresizingStyle =
        NSTableViewUniformColumnAutoresizingStyle;
    _tableView.headerView = nil;
    _tableView.dataSource = self;
    _tableView.delegate = self;
    _tableView.intercellSpacing =
        NSMakeSize(_tableView.intercellSpacing.width, 0);
    ...
    [_tableView performSelector:@selector(scrollToEndOfDocument:)
                     withObject:nil
                     afterDelay:0];
    ...
    [iTermSnippetsDidChangeNotification
        subscribe:self
...
- (void)relayout {
  ...
  [_applyButton
      setFrame:NSMakeRect(0, frame.size.height - kButtonHeight,
                          _applyButton.frame.size.width, kButtonHeight)];
  ...
  for (NSButton *button in @[ _addButton, _removeButton, _editButton ]) {
    ...
    x -= width + margin;
    button.frame =
        NSMakeRect(x, frame.size.height - kButtonHeight, width, kButtonHeight);
  }

  [_scrollView
      setFrame:NSMakeRect(0, 0, frame.size.width,
                          frame.size.height - kButtonHeight - kMargin)];
...
- (void)add:(id)sender {
  ...
  [self.window beginSheet:_windowController.window
    ...
    _windowController = [[iTermEditSnippetWindowController alloc]
        initWithSnippet:snippet
             completion:^(iTermSnippet *_Nullable updatedSnippet) {
               ...
               [[iTermSnippetsModel sharedInstance]
                   replaceSnippet:snippet
                      withSnippet:updatedSnippet];
             }];
    [self.window beginSheet:_windowController.window
          completionHandler:^(NSModalResponse returnCode){
          }];
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
  if (_tableView.window.firstResponder == _tableView &&
...
- (void)snippetsDidChange:(iTermSnippetsDidChangeNotification *)notif {
  ...
  case iTermSnippetsDidChangeMutationTypeEdit: {
    [_tableView it_performUpdateBlock:^{
      [_tableView
          reloadDataForRowIndexes:[NSIndexSet indexSetWithIndex:notif.index]
...
- (void)applySelectedSnippets {
  for (iTermSnippet *snippet in [self selectedSnippets]) {
    ...
    iTermAction *action = [[iTermAction alloc] initWithTitle:@"Send Snippet"
                                                      action:KEY_ACTION_TEXT
                                                   parameter:snippet.value];
    [wrapper.delegate.delegate toolbeltApplyActionToCurrentSession:action];
  }
}

- (NSArray<iTermSnippet *> *)selectedSnippets {
  NSArray<iTermSnippet *> *snippets =
      [[iTermSnippetsModel sharedInstance] snippets];
  return [[_tableView.selectedRowIndexes it_array]
      mapWithBlock:^id(NSNumber *indexNumber) {
...
- (NSSize)contentSize {
  ...
  size.height +=
      ...
      ([_tableView rowHeight] + [_tableView intercellSpacing].height);
  return size;
}
...
- (NSString *)combinedStringForRow:(NSInteger)rowIndex {
  ...
  return
      [NSString stringWithFormat:@"%@ — %@", title, [snippet trimmedValue:40]];
}
...
- (BOOL)tableView:(NSTableView *)tableView
  ...
  [pboard
      declareTypes:@[ iTermToolSnippetsPasteboardType, NSPasteboardTypeString ]
             owner:self];

  NSArray<NSNumber *> *plist =
      [rowIndexes.it_array mapWithBlock:^id(NSNumber *anObject) {
        return @(_snippets[anObject.integerValue].identifier);
      }];
  [pboard setPropertyList:plist forType:iTermToolSnippetsPasteboardType];
...
- (BOOL)tableView:(NSTableView *)aTableView
  ...
  NSArray<NSNumber *> *identifiers =
      [pboard propertyListForType:iTermToolSnippetsPasteboardType];
...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender {
  BOOL pasteOK = !![[sender draggingPasteboard]
...
- (NSString *)titleFromString:(NSString *)string {
  NSArray<NSString *> *parts =
      [string componentsSeparatedByCharactersInSet:[NSCharacterSet
                                                       newlineCharacterSet]];
  return [parts objectPassingTest:^BOOL(NSString *element, NSUInteger index,
                                        BOOL *stop) {
    return
        [[element stringByTrimmingCharactersInSet:[NSCharacterSet
