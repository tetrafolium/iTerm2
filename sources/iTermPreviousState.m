...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p itermWasActive=%@ other app pid=%@>",
                       self.class, self, @(_itermWasActiveWhenHotkeyOpened),
                       self.previouslyActiveAppPID];
}
...
- (BOOL)restorePreviouslyActiveApp {
  ...
  NSRunningApplication *app = [NSRunningApplication
      runningApplicationWithProcessIdentifier:[_previouslyActiveAppPID
                                                  intValue]];
...
- (BOOL)restoreAllowingAppSwitch:(BOOL)allowAppSwitch {
  DLog(@"Restore %p with previously active app %@", self,
       _previouslyActiveAppPID);
  BOOL result = allowAppSwitch && [self restorePreviouslyActiveApp];
  if (self.itermWasActiveWhenHotkeyOpened) {
    PseudoTerminal *currentTerm =
        [[iTermController sharedInstance] currentTerminal];
    if (currentTerm && ![currentTerm isHotKeyWindow] &&
        [currentTerm fullScreen]) {
...
- (NSInteger)indexOfFrontNonHotKeyTerminal {
  ...
  [[[iTermController sharedInstance] terminals]
      enumerateObjectsUsingBlock:^(PseudoTerminal *_Nonnull term,
