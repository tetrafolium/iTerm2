...
+ (NSBezierPath *)smoothPathAroundBottomOfFrame:(NSRect)frame {
  ...
  nx = x + radius;
  ny = y - radius -
       0.5; // Subtract 0.5 to return to the "true" origin of the frame
  [path curveToPoint:NSMakePoint(nx, ny)
...
- (CGPathRef)iterm_cgPathOpen:(BOOL)open {
  ...
  for (NSInteger i = 0; i < self.elementCount; i++) {
    ...
    case NSMoveToBezierPathElement:
      CGPathMoveToPoint(path, NULL, associatedPoints[0].x,
                        associatedPoints[0].y);
      break;

    case NSLineToBezierPathElement:
      closed = NO;
      CGPathAddLineToPoint(path, NULL, associatedPoints[0].x,
                           associatedPoints[0].y);
      break;

    case NSCurveToBezierPathElement:
      closed = NO;
      CGPathAddCurveToPoint(path, NULL, associatedPoints[0].x,
                            associatedPoints[0].y, associatedPoints[1].x,
                            associatedPoints[1].y, associatedPoints[2].x,
                            associatedPoints[2].y);
      break;
