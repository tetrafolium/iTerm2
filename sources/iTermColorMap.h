...
// This value plus 0...255 are accepted. The ANSI colors below followed by their
// bright variants make the first 16 entries of the 256-color space.
...
// This class holds the collection of colors used by a single session. Some
// colors are index-mapped (foreground, background, etc.). An 8-bit gamut
// (kColorMap8bitBase to kColorMap8bitBase+255) exists, as does a 24-bit gamut
// (kColorMap24bitBase to kColorMap24bitBase+16777215). Additionally, two
// transformations on colors are performed by this class. Dimming moves colors
// by self.dimmingAmount towards a neutral gray, and is used to indicate a
// session's inactivity. Muting moves colors towards the background color and is
// used by the "cursor boost" feature to make the cursor stand out more.
...
// 1. Modify textColor to have at least self.minimumContrast against
// backgroundColor
// 2. Average textColor proportionally with the background color by
// self.mutingAmount
// 3. Dim textColor proportionally with either gray or background color
// (depending on
...
// Returns non-nil profile key name for valid logical colors, ANSI colors, and
// bright ANSI colors.
