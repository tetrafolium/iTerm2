...
@interface iTermStatusBarSetupDestinationCollectionViewController () <
    iTermStatusBarSetupElementDelegate, NSCollectionViewDataSource,
...
- (void)awakeFromNib {
  ...
  [self.collectionView
      registerForDraggedTypes:@[ iTermStatusBarElementPasteboardType ]];
...
- (void)viewDidLoad {
  ...
  [self.collectionView
              registerClass:[iTermStatusBarSetupCollectionViewItem class]
...
- (void)deleteItemAtIndex:(NSInteger)index {
  NSRect rectInWindowCoords = [self.collectionView
      convertRect:[self.collectionView frameForItemAtIndex:index]
           toView:nil];
  NSRect rect =
      [self.collectionView.window convertRectToScreen:rectInWindowCoords];
  NSShowAnimationEffect(NSAnimationEffectPoof,
                        NSMakePoint(NSMidX(rect), NSMidY(rect)), NSZeroSize,
                        nil, nil, NULL);
  ...
  [self.collectionView.animator
      performBatchUpdates:^{
        ...
        NSIndexPath *indexPath = [NSIndexPath indexPathForItem:index
                                                     inSection:0];
        [self.collectionView
            deleteItemsAtIndexPaths:[NSSet setWithObject:indexPath]];
      }
...
- (void)updateAutoRainbowColors {
  ...
  iTermStatusBarAutoRainbowController *controller =
      [[iTermStatusBarAutoRainbowController alloc]
          initWithStyle:_advancedConfiguration.autoRainbowStyle];
  controller.darkBackground = self.darkBackground;
  _updatingAutoRainbowColors = YES;
  [controller
      enumerateColorsWithCount:_elements.count
                         block:^(NSInteger i, NSColor *color) {
                           id<iTermStatusBarComponent> component =
                               _elements[i].component;
...
- (void)setAdvancedConfiguration:
    (iTermStatusBarAdvancedConfiguration *)advancedConfiguration {
  _advancedConfiguration = advancedConfiguration;
  iTermStatusBarLayout *temporaryLayout =
      [[iTermStatusBarLayout alloc] initWithDictionary:[self layoutDictionary]
                                                 scope:nil];
...
- (void)loadElementsFromLayout:(iTermStatusBarLayout *)layout {
  ...
  [layout.components enumerateObjectsUsingBlock:^(
                         id<iTermStatusBarComponent> _Nonnull component,
                         NSUInteger idx, BOOL *_Nonnull stop) {
    iTermStatusBarSetupElement *element =
        [[iTermStatusBarSetupElement alloc] initWithComponent:component];
...
- (NSDictionary *)layoutDictionary {
  ...
  iTermStatusBarLayout *layout =
      [[iTermStatusBarLayout alloc] initWithComponents:components
...
- (void)configureStatusBarComponentWithIdentifier:(NSString *)identifier {
  NSInteger index = [_elements indexOfObjectPassingTest:^BOOL(
                                   iTermStatusBarSetupElement *_Nonnull element,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
    return [element.component.statusBarComponentIdentifier
  ...
  [self.collectionView
      selectItemsAtIndexPaths:indexPaths
  ...
  if (_configureButton.enabled) {
    [_configureButton.target
        it_performNonObjectReturningSelector:_configureButton.action
...
- (iTermStatusBarSetupCollectionViewItem *)newItemWithIndexPath:
    (NSIndexPath *)indexPath {
  iTermStatusBarSetupCollectionViewItem *item =
      [self.collectionView makeItemWithIdentifier:@"element"
...
- (void)initializeItem:(iTermStatusBarSetupCollectionViewItem *)item
  ...
  item.textField.attributedStringValue = [_elements[index]
      exemplarWithBackgroundColor:_advancedConfiguration.backgroundColor
  ...
  item.backgroundColor = _elements[index].component.statusBarBackgroundColor
                             ?: _advancedConfiguration.backgroundColor
...
- (NSSize)collectionView:(NSCollectionView *)collectionView
  ...
  iTermStatusBarSetupCollectionViewItem *item =
      [[iTermStatusBarSetupCollectionViewItem alloc]
          initWithNibName:@"iTermStatusBarSetupCollectionViewItem"
...
- (void)collectionView:(NSCollectionView *)collectionView
  ...
  NSPoint windowPoint =
      [collectionView.window
          ...
          .origin;
  NSPoint collectionViewPoint = [collectionView convertPoint:windowPoint
                                                    fromView:nil];
  if (operation == NSDragOperationNone &&
      !NSPointInRect(collectionViewPoint, collectionView.bounds)) {
...
- (BOOL)collectionView:(NSCollectionView *)collectionView
  ...
  NSArray *objects =
      [indexPaths.allObjects mapWithBlock:^id(NSIndexPath *indexPath) {
...
- (NSImage *)collectionView:(NSCollectionView *)collectionView
  ...
  iTermStatusBarSetupCollectionViewItem *item =
      [iTermStatusBarSetupCollectionViewItem
          castFrom:[collectionView itemAtIndexPath:indexPaths.anyObject]];
  assert(item);
  NSPoint locationInItem = [item.view convertPoint:locationInWindow
                                          fromView:nil];
  NSPoint center = NSMakePoint(item.view.frame.size.width / 2,
                               item.view.frame.size.height / 2);
  *dragImageOffset =
      NSMakePoint(center.x - locationInItem.x, center.y - locationInItem.y);
...
- (BOOL)collectionView:(NSCollectionView *)collectionView
  ...
  NSData *data = [draggingInfo.draggingPasteboard
      dataForType:iTermStatusBarElementPasteboardType];
  NSError *error = nil;
  ...
  [collectionView.animator
      performBatchUpdates:^{
        if (draggingInfo.draggingSource == collectionView) {
          const NSInteger fromIndex =
              [self->_draggingIndexPath indexAtPosition:1];
          ...
          if (fromIndex < toIndex) {
            [self->_elements removeObjectAtIndex:fromIndex];
            [collectionView
                moveItemAtIndexPath:self->_draggingIndexPath
                        toIndexPath:[NSIndexPath indexPathForItem:toIndex - 1
                                                        inSection:0]];
          }
          NSLog(@"Collection move item from %@ to %@", self->_draggingIndexPath,
                indexPath);
        } else {
          [self->_elements insertObject:element
                                atIndex:[indexPath indexAtPosition:1]];
          [collectionView
              insertItemsAtIndexPaths:[NSSet setWithObject:indexPath]];
        }
      }
        completionHandler:^(BOOL finished){
        }];
...
- (void)collectionView:(NSCollectionView *)collectionView
    didChangeItemsAtIndexPaths:(NSSet<NSIndexPath *> *)indexPaths
              toHighlightState:
                  (NSCollectionViewItemHighlightState)highlightState {
  [indexPaths enumerateObjectsUsingBlock:^(NSIndexPath *_Nonnull indexPath,
                                           BOOL *_Nonnull stop) {
...
- (iTermStatusBarSetupKnobsViewController *)viewControllerToConfigureComponent:
    (id<iTermStatusBarComponent>)component {
  return [[iTermStatusBarSetupKnobsViewController alloc]
...
- (void)disableAutoRainbow {
  iTermStatusBarAdvancedConfiguration *config =
      [self.advancedConfiguration copy];
  config.autoRainbowStyle = iTermStatusBarAutoRainbowStyleDisabled;
  self.advancedConfiguration = config;
}

- (void)itermStatusBarSetupElementDidChange:
            (iTermStatusBarSetupElement *)element
  ...
  [self.collectionView.animator
      performBatchUpdates:^{
        NSIndexPath *indexPath = [NSIndexPath indexPathForItem:index
                                                     inSection:0];
        [self.collectionView
            reloadItemsAtIndexPaths:[NSSet setWithObject:indexPath]];
      }
        completionHandler:^(BOOL finished){
        }];
  [self didChange:YES];
...
- (IBAction)configureComponent:(id)sender {
  ...
  iTermStatusBarSetupKnobsViewController *viewController =
      [self viewControllerToConfigureComponent:component];
  viewController.view.frame =
      NSMakeRect(0, 0, viewController.preferredContentSize.width,
  ...
  iTermStatusBarSetupConfigureComponentWindowController *windowController =
      [[iTermStatusBarSetupConfigureComponentWindowController alloc]
          initWithWindowNibName:
              @"iTermStatusBarSetupConfigureComponentWindowController"];
