...
static NSString *kUseBackgroundPatternIndicatorKey =
    @"Use background pattern indicator";
NSString *kUseBackgroundPatternIndicatorChangedNotification =
    @"kUseBackgroundPatternIndicatorChangedNotification";
NSString *const kSavedArrangementDidChangeNotification =
    @"kSavedArrangementDidChangeNotification";
NSString *const kNonTerminalWindowBecameKeyNotification =
    @"kNonTerminalWindowBecameKeyNotification";
...
NSString *const kShowFullscreenTabsSettingDidChange =
    @"kShowFullscreenTabsSettingDidChange";

static NSString *const kScreenCharRestorableStateKey =
    @"kScreenCharRestorableStateKey";
static NSString *const kURLStoreRestorableStateKey =
    @"kURLStoreRestorableStateKey";
static NSString *const kHotkeyWindowRestorableState =
    @"kHotkeyWindowRestorableState"; // deprecated
static NSString *const kHotkeyWindowsRestorableStates =
    @"kHotkeyWindowsRestorableState"; // deprecated
...
NSString *const iTermApplicationWillTerminate =
    @"iTermApplicationWillTerminate";

static BOOL gStartupActivitiesPerformed = NO;
// Prior to 8/7/11, there was only one window arrangement, always called
// Default.
static NSString *LEGACY_DEFAULT_ARRANGEMENT_NAME = @"Default";
static BOOL hasBecomeActive = NO;

@interface iTermApplicationDelegate () <
    iTermGraphCodable, iTermOrphanServerAdopterDelegate,
    iTermPasswordManagerDelegate, iTermRestorableStateControllerDelegate,
...
@implementation iTermApplicationDelegate {
  ...
  IBOutlet NSMenuItem *
      _showTipOfTheDay; // Here because we must remove it for older OS versions.
  ...
  // Cross app request forgery prevention token. Get this with applescript and
  // then include in a URI request.
  NSString *token_;
  ...
  // If the advanced pref to turn off app nap is enabled, then we hold a
  // reference to this NSProcessInfo-provided object to make the system think
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _untitledWindowStateMachine =
        [[iTermUntitledWindowStateMachine alloc] init];
    ...
    if ([iTermAdvancedSettingsModel useRestorableStateController] &&
      ...
      _restorableStateController =
          [[iTermRestorableStateController alloc] init];
    ...
    _focusFollowsMouseController =
        [[iTermFocusFollowsMouseController alloc] init];
  }
...
- (void)awakeFromNib {
  ...
  [self addMenuItemView:labelTrackView
                 toMenu:viewMenu
                  title:@"Current Tab Color"];
  ...
  if ([iTermAdvancedSettingsModel showHintsInSplitPaneMenuItems]) {
    _splitHorizontally.title =
        [@"─⃞ " stringByAppendingString:_splitHorizontally.title];
    _splitHorizontallyWithCurrentProfile.title = [@"─⃞ "
        stringByAppendingString:_splitHorizontallyWithCurrentProfile.title];
    _splitVertically.title =
        [@"│⃞ " stringByAppendingString:_splitVertically.title];
    _splitVerticallyWithCurrentProfile.title = [@"│⃞ "
        stringByAppendingString:_splitVerticallyWithCurrentProfile.title];
  }
  ...
  [[iTermBuriedSessions sharedInstance]
      setMenus:[NSArray arrayWithObjects:_buriedSessions,
                                         _statusIconBuriedSessions, nil]];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
  if ([menuItem action] == @selector(toggleUseBackgroundPatternIndicator:)) {
    ...
    [menuItem setState:[[self markAlertAction]
                           isEqualToString:kMarkAlertActionModalAlert]
                           ? NSControlStateValueOn
                           : NSControlStateValueOff];
    return YES;
  } else if ([menuItem action] == @selector
             (enableMarkAlertPostsNotification:)) {
    [menuItem setState:[[self markAlertAction]
                           isEqualToString:kMarkAlertActionPostNotification]
                           ? NSControlStateValueOn
                           : NSControlStateValueOff];
    return YES;
    ...
    if ([[[iTermController sharedInstance] currentTerminal] inInstantReplay]) {
      ...
      return NO;
    } else if ([[[[[iTermController sharedInstance] currentTerminal] currentTab]
                   activeSession] isTmuxClient]) {
      return YES;
    } else if ([[[[iTermController sharedInstance] currentTerminal] currentTab]
                   hasMaximizedPane]) {
      return YES;
    ...
    [menuItem setState:[iTermPreferences
                           boolForKey:kPreferenceKeyShowFullscreenTabBar]
                           ? NSControlStateValueOn
                           : NSControlStateValueOff];
    return YES;
    ...
    if (menuItem.tag == 0) {
      menuItem.state =
          ![iTermAdvancedSettingsModel noSyncDoNotWarnBeforeMultilinePaste]
              ? NSControlStateValueOn
              : NSControlStateValueOff;
    } else if (menuItem.tag == 1) {
      menuItem.state =
          ![iTermAdvancedSettingsModel promptForPasteWhenNotAtPrompt]
              ? NSControlStateValueOn
              : NSControlStateValueOff;
    ...
    iTermSecureKeyboardEntryController *controller =
    ...
    if (controller.isEnabled) {
      ...
      menuItem.state =
          controller.isDesired ? NSControlStateValueOn : NSControlStateValueOff;
    }
    return YES;
  } else if ([menuItem action] == @selector(togglePinHotkeyWindow:)) {
    iTermProfileHotKey *profileHotkey = self.currentProfileHotkey;
    menuItem.state = profileHotkey.autoHides ? NSControlStateValueOff
                                             : NSControlStateValueOn;
    ...
    menuItem.state =
        gDebugLogging ? NSControlStateValueOn : NSControlStateValueOff;
    return YES;
  } else if (menuItem.action == @selector(arrangeSplitPanesEvenly:)) {
    PTYTab *tab =
        [[[iTermController sharedInstance] currentTerminal] currentTab];
    return (tab.sessions.count > 0 && !tab.isMaximized);
  } else if (menuItem.action == @selector
             (promptToConvertTabsToSpacesWhenPasting:)) {
    menuItem.state = [iTermPasteHelper promptToConvertTabsToSpacesWhenPasting]
                         ? NSControlStateValueOn
                         : NSControlStateValueOff;
    return YES;
...
- (BOOL)isAppleScriptTestApp {
  return
      [[[NSBundle mainBundle] bundleIdentifier] containsString:@"applescript"];
}

- (BOOL)isRunningOnTravis {
  NSString *travis =
      [[[NSProcessInfo processInfo] environment] objectForKey:@"TRAVIS"];
...
- (void)updateMaximizePaneMenuItem {
  [maximizePane setState:[[[[iTermController sharedInstance] currentTerminal]
                             currentTab] hasMaximizedPane]
                             ? NSControlStateValueOn
                             : NSControlStateValueOff];
}

- (void)updateUseTransparencyMenuItem {
  [useTransparency setState:[[[iTermController sharedInstance] currentTerminal]
                                useTransparency]
                                ? NSControlStateValueOn
                                : NSControlStateValueOff];
}
...
- (NSString *)markAlertAction {
  NSString *action =
      [[NSUserDefaults standardUserDefaults] objectForKey:kMarkAlertAction];
  if (!action) {
...
- (BOOL)useBackgroundPatternIndicator {
  return [[NSUserDefaults standardUserDefaults]
      boolForKey:kUseBackgroundPatternIndicatorKey];
}
...
- (NSMenu *)downloadsMenu {
  if (!downloadsMenu_) {
    ...
    [mainMenu insertItem:downloadsMenu_ atIndex:mainMenu.itemArray.count - 1];
    [downloadsMenu_
        setSubmenu:[[[NSMenu alloc] initWithTitle:@"Downloads"] autorelease]];

    NSMenuItem *clearAll =
        [[[NSMenuItem alloc] initWithTitle:@"Clear All"
...
- (NSMenu *)uploadsMenu {
  if (!uploadsMenu_) {
    ...
    NSMenu *mainMenu = [[NSApplication sharedApplication] mainMenu];
    [mainMenu insertItem:uploadsMenu_ atIndex:mainMenu.itemArray.count - 1];
    [uploadsMenu_
        setSubmenu:[[[NSMenu alloc] initWithTitle:@"Uploads"] autorelease]];

    NSMenuItem *clearAll =
        [[[NSMenuItem alloc] initWithTitle:@"Clear All"
                                    action:@selector(clearAllUploads:)
                             keyEquivalent:@""] autorelease];
...
- (BOOL)application:(NSApplication *)theApplication
  ...
  if ([[filename pathExtension] isEqualToString:@"its"]) {
    [iTermScriptImporter
        importScriptFromURL:[NSURL fileURLWithPath:filename]
              userInitiated:NO
            offerAutoLaunch:NO
                 completion:^(NSString *_Nullable errorMessage, BOOL quiet,
                              NSURL *location) {
                   if (quiet) {
                     return;
                   }
                   [self->_scriptsMenuController
                       importDidFinishWithErrorMessage:errorMessage
                                              location:location
                                           originalURL:[NSURL fileURLWithPath:
                                                                  filename]];
                 }];
    return YES;
  }
  if ([filename hasSuffix:@".itermcolors"]) {
    ...
    if ([iTermColorPresets importColorPresetFromFile:filename]) {
      ...
      alert.informativeText =
          @"The color scheme was imported and added to presets. You can find "
          @"it under Preferences>Profiles>Colors>Load Presets….";
      [alert runModal];
    }
    return YES;
  }
  ...
  NSLog(@"Quiet launch");
  quiet_ = YES;
  if ([filename isEqualToString:[[NSFileManager defaultManager]
                                    versionNumberFilename]]) {
    return YES;
  }
  if (filename) {
    ...
    NSMutableDictionary *bookmark =
        [[[controller defaultBookmark] mutableCopy] autorelease];

    if (isDir) {
      bookmark[KEY_WORKING_DIRECTORY] = filename;
      bookmark[KEY_CUSTOM_DIRECTORY] =
      ...
      if (filename) {
        ...
        if (initialText &&
            ![iTermAdvancedSettingsModel openFileOverridesSendText]) {
          initialText =
              [initialText stringByAppendingFormat:@"\n%@; exit", filename];
        } else {
          initialText = [NSString stringWithFormat:@"%@; exit", filename];
        }

        const iTermWarningSelection selection = [iTermWarning
            showWarningWithTitle:[NSString stringWithFormat:@"OK to run “%@”?",
                                                            filename]
                         actions:@[ @"OK", @"Cancel" ]
        ...
        bookmark[KEY_INITIAL_TEXT] =
            [initialText stringByReplacingOccurrencesOfString:@"\\"
                                                   withString:@"\\\\"];
      }
    }
    ...
    DLog(@"application:openFile: launching new session in window %@",
         windowController);
                ...
                completion:^(PTYSession *session) {
                  ...
                  iTermProfileHotKey *profileHotkey =
                      [[iTermHotKeyController sharedInstance]
                          profileHotKeyForWindowController:term];
                  if (!profileHotkey) {
                    return;
                  }
                  DLog(@"application:openFile: revealing hotkey window");
                  [[iTermHotKeyController sharedInstance]
                      showWindowForProfileHotKey:profileHotkey
                                             url:nil];
                }];
  }
  return YES;
...
- (BOOL)applicationShouldTerminateAfterLastWindowClosed:(NSApplication *)app {
  ...
  if (terminals.count > 0) {
    // The last window wasn't really closed, it was just the hotkey window
    // getting ordered out or a window entering fullscreen.
    DLog(@"Not quitting automatically. Terminals are %@", terminals);
    return NO;
  }
  if (!userHasInteractedWithAnySession_) {
    DLog(@"applicationShouldTerminateAfterLastWindowClosed - user has not "
         @"interacted with any session");
    if ([[NSDate date] timeIntervalSinceDate:launchTime_] <
        [iTermAdvancedSettingsModel minRunningTime]) {
      DLog(@"Returning NO");
      NSLog(@"Not quitting iTerm2 because it ran very briefly and had no user "
            @"interaction. Set the MinRunningTime float preference to 0 to "
...
- (void)applicationDidChangeScreenParameters:(NSNotification *)aNotification {
  // The screens' -visibleFrame is not updated when this is called. Doing a
  // delayed perform with a delay of 0 is usually, but not always enough. Not
  // that 1 second is always enough either, I suppose, but I don't want to die
  // on this hill.
...
- (void)didToggleTraditionalFullScreenMode {
  // LOL
  // When you have only one window, and you do windowController.window =
  // something new then it thinks you closed the only window and asks if you
  // want to terminate the app. We run into this problem with compact windows,
  // as other window types are able to simply change the window style without
  // actually replacing the window. This awful hack catches that case. It takes
  // two spins of the runloop because everything is terrible.
...
- (NSApplicationTerminateReply)applicationShouldTerminate:
  ...
  if (!quittingBecauseLastWindowClosed_ && // cmd-q
      [iTermPreferences
          boolForKey:kPreferenceKeyPromptOnQuit]) { // preference is to prompt
                                                    // on quit cmd
    if (terminals.count > 0) {
      [reason addReason:[iTermPromptOnCloseReason
                            alwaysConfirmQuitPreferenceEnabled]];
    } else if ([iTermPreferences
                   boolForKey:
                       kPreferenceKeyPromptOnQuitEvenIfThereAreNoWindows]) {
      [reason
          addReason:
              [iTermPromptOnCloseReason
                  alwaysConfirmQuitPreferenceEvenIfThereAreNoWindowsEnabled]];
    }
  }
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyConfirmClosingMultipleTabs] &&
    ...
    [reason addReason:[iTermPromptOnCloseReason
                          closingMultipleSessionsPreferenceEnabled]];
  }
  if ([iTermAdvancedSettingsModel runJobsInServers] && self.sparkleRestarting &&
  ...
  if (reason.hasReason) {
    ...
    if ([[iTermController sharedInstance] shouldLeaveSessionsRunningOnQuit]) {
      message =
          @"Sessions will be restored automatically when iTerm2 is relaunched.";
    } else {
    ...
    iTermDisclosableView *accessory = [[iTermDisclosableView alloc]
        initWithFrame:NSZeroRect
              ...
              message:[NSString stringWithFormat:
                                    @"You are being prompted because:\n\n%@",
                                    reason.message]];
    ...
    accessory.requestLayout = ^{
      ...
      if (@available(macOS 10.16, *)) {
        // FB8897296:
        // Prior to Big Sur, you could call [NSAlert layout] on an
        // already-visible NSAlert to have it change its size to accommodate an
        // accessory view controller whose frame changed.
        //
        // On Big Sur, it no longer works. Instead, you must call NSAlert.layout
        // *twice*.
        [alert layout];
      }
    };
  ...
  // Prevent sessions from making their termination undoable since we're
  // quitting.
  [[iTermController sharedInstance] setApplicationIsQuitting:YES];

  if ([iTermAdvancedSettingsModel runJobsInServers]) {
    // Restorable sessions must be killed or they'll auto-restore as orphans on
    // the next start. If jobs aren't run in servers, they'll just die normally.
    [[iTermController sharedInstance] killRestorableSessions];
  }

  // Last chance before windows get closed.
  DLog(@"Post applikcationWillTerminate which triggers saving restorable "
       @"state.");
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermApplicationWillTerminate
                    object:nil];
...
- (void)applicationWillTerminate:(NSNotification *)aNotification {
  ...
  [iTermUserDefaults
      setIgnoreSystemWindowRestoration:[iTermAdvancedSettingsModel
                                           useRestorableStateController]];
}
...
- (BOOL)applicationOpenUntitledFile:(NSApplication *)theApplication {
  ...
  if ([PseudoTerminalRestorer shouldIgnoreOpenUntitledFile] &&
    ...
    DLog(@"Already restored one of our own windows so not opening an untitled "
         @"file during window state restoration.");
    return NO;
  }
  ...
  DLog(@"finishedLaunching=%@ openArrangementAtStartup=%@ "
...
- (void)application:(NSApplication *)app
  ...
  NSArray *hotkeyWindowsStates =
      [[iTermHotKeyController sharedInstance] restorableStates];
  ...
  if ([[[iTermBuriedSessions sharedInstance] buriedSessions] count]) {
    [coder encodeObject:[[iTermBuriedSessions sharedInstance] restorableState]
                 forKey:iTermBuriedSessionState];
  }
...
- (void)application:(NSApplication *)app
  ...
  NSDictionary *screenCharState =
      [coder decodeObjectForKey:kScreenCharRestorableStateKey];
  if (screenCharState) {
  ...
  NSDictionary *urlStoreState =
      [coder decodeObjectForKey:kURLStoreRestorableStateKey];
  if (urlStoreState) {
  ...
  hotkeyWindowsStates =
      [coder decodeObjectForKey:kHotkeyWindowsRestorableStates];
  if (hotkeyWindowsStates) {
    // We have to create the hotkey window now because we need to attach to
    // servers before launch finishes; otherwise any running hotkey window jobs
    // will be treated as orphans.
    const NSInteger count = [[iTermHotKeyController sharedInstance]
        createHiddenWindowsFromRestorableStates:hotkeyWindowsStates];
    if (count > 0) {
      [_untitledWindowStateMachine didRestoreSomeWindows];
    }
  }

  _buriedSessionsState =
      [[coder decodeObjectForKey:iTermBuriedSessionState] retain];
  if (finishedLaunching_) {
    [self restoreBuriedSessionsState];
  }
  if ([iTermAdvancedSettingsModel logRestorableStateSize]) {
    NSDictionary *dict = @{
      kScreenCharRestorableStateKey : screenCharState ?: @{},
    ...
    [log writeToFile:[NSString
                         stringWithFormat:@"/tmp/statesize.app-%p.txt", self]
          atomically:NO
...
- (void)sorry1013 {
  [iTermWarning showWarningWithTitle:
                    @"I’ve decided that iTerm2 version 3.4 will only support "
                    @"macOS 10.14 and later.\nApple made significant changes "
                    @"in macOS 10.14 that makes supporting both code paths "
                    @"very difficult. Version 3.3.x will continue to receive "
                    @"bug fixes and security updates until Big Sur is released."
                             actions:@[ @"😢" ]
                           accessory:nil
                          identifier:@"RIP1013"
                         silenceable:kiTermWarningTypePersistent
                             heading:@"This is the last nightly build that "
                                     @"will run on macOS 10.13."
                              window:nil];
  _exit(0);
}
...
- (void)applicationWillFinishLaunching:(NSNotification *)aNotification {
  ...
  // Bookmarks must be loaded for this to work since it needs to know if the
  // hotkey's profile exists.
  ...
  [iTermPreferences addObserverForKey:kPreferenceKeyCheckForTestReleases
                                block:^(id before, id after) {
                                  [[iTermController sharedInstance]
                                      refreshSoftwareUpdateUserDefaults];
                                }];
  [iTermLoggingHelper
      observeNotificationsWithHandler:^(NSString *_Nonnull guid) {
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  ...
  if ([iTermAdvancedSettingsModel disableAppNap]) {
    ...
    [[NSProcessInfo processInfo]
        disableAutomaticTermination:@"User Preference"];
    _appNapStoppingActivity = [[[NSProcessInfo processInfo]
  ...
  CFPreferencesSetAppValue(CFSTR("NSQuotedKeystrokeBinding"), CFSTR(""),
                           kCFPreferencesCurrentApplication);
  // This is off by default, but would wreak havoc if set globally.
  CFPreferencesSetAppValue(CFSTR("NSRepeatCountBinding"), CFSTR(""),
                           kCFPreferencesCurrentApplication);
  ...
  if ([[iTermModifierRemapper sharedInstance] isAnyModifierRemapped]) {
    // Use a brief delay so windows have a chance to open before the dialog is
    // shown.
    dispatch_after(
  ...
  [self updateRestoreWindowArrangementsMenu:windowArrangementsAsTabs_
                                     asTabs:YES];
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [NSApp
        setServicesProvider:[[[iTermServiceProvider alloc] init] autorelease]];
  });

  // Sometimes, open untitled doc isn't called in Lion. We need to give
  // application:openFile: a chance to run because a "special" filename cancels
  // performStartupActivities.
  ...
  if ([iTermAPIHelper isEnabled]) {
    [iTermAPIHelper sharedInstance]; // starts the server. Won't ask the user
                                     // since it's enabled.
  }
...
- (void)sparkleWillRestartApp:(NSNotification *)notification {
  ...
  [[NSApp windows]
      makeObjectsPerformSelector:@selector(invalidateRestorableState)];
...
- (void)itermDidDecodeWindowRestorableState:(NSNotification *)notification {
  if (!_orphansAdopted &&
      [[iTermController sharedInstance] numberOfDecodesPending] == 0) {
    _orphansAdopted = YES;
    [[iTermOrphanServerAdopter sharedInstance]
        openWindowWithOrphansWithCompletion:nil];
  }
...
- (void)toolDidToggle:(NSNotification *)notification {
  ...
  NSInteger newState = ([menuItem state] == NSControlStateValueOn)
                           ? NSControlStateValueOff
                           : NSControlStateValueOn;
...
- (void)getUrl:(NSAppleEventDescriptor *)event
  ...
  if (profile) {
    ...
    BOOL launch = NO;
    if (profileHotkey) {
      const BOOL newWindowCreated = [[iTermHotKeyController sharedInstance]
          showWindowForProfileHotKey:profileHotkey
                                 url:url];
      if (!newWindowCreated) {
        launch = YES;
...
- (void)updateAddressBookMenu:(NSNotification *)aNotification {
  ...
  iTermProfileModelJournalParams *params =
      [[[iTermProfileModelJournalParams alloc] init] autorelease];
  if ([iTermAdvancedSettingsModel openProfilesInNewWindow]) {
    ...
    [bookmarkMenu itemAtIndex:2].title = [[bookmarkMenu itemAtIndex:2].title
        stringByReplacingOccurrencesOfString:@"Window"
                                  withString:@"Tab"];
    [bookmarkMenu itemAtIndex:3].title = [[bookmarkMenu itemAtIndex:3].title
        stringByReplacingOccurrencesOfString:@"Window"
                                  withString:@"Tab"];
  } else {
    ...
    [bookmarkMenu itemAtIndex:2].title = [[bookmarkMenu itemAtIndex:2].title
        stringByReplacingOccurrencesOfString:@"Tab"
                                  withString:@"Window"];
    [bookmarkMenu itemAtIndex:3].title = [[bookmarkMenu itemAtIndex:3].title
        stringByReplacingOccurrencesOfString:@"Tab"
                                  withString:@"Window"];
  ...
  if (age > 30 * 24 * 60 * 60) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            @"This nightly build is over 30 days old. Consider updating soon: "
            @"you may be suffering from awful bugs in blissful ignorance."
...
- (void)performStartupActivities {
  ...
  // Check if we have an autolaunch script to execute. Do it only once, i.e. at
  // application launch.
  BOOL ranAutoLaunchScripts = NO;
  if (![self isAppleScriptTestApp] &&
      ![[NSApplication sharedApplication] isRunningUnitTests]) {
    ranAutoLaunchScripts =
        [self.scriptsMenuController runAutoLaunchScriptsIfNeeded];
  }
  DLog(@"ranAutoLaunchScripts=%@", @(ranAutoLaunchScripts));

  if ([WindowArrangements defaultArrangementName] == nil &&
      [WindowArrangements
          arrangementWithName:LEGACY_DEFAULT_ARRANGEMENT_NAME] != nil) {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyOpenArrangementAtStartup]) {
    ...
    [[iTermController sharedInstance]
        loadWindowArrangementWithName:[WindowArrangements
                                          defaultArrangementName]];
  } else if (!ranAutoLaunchScripts &&
             [iTermAdvancedSettingsModel openNewWindowAtStartup] &&
             ![iTermPreferences
                 boolForKey:kPreferenceKeyOpenNoWindowsAtStartup] &&
             [[[iTermController sharedInstance] terminals] count] == 0 &&
             ![self isAppleScriptTestApp] &&
             [[[iTermHotKeyController sharedInstance] profileHotKeys] count] ==
                 0 &&
             [[[iTermBuriedSessions sharedInstance] buriedSessions] count] ==
                 0 &&
             ![[NSApplication sharedApplication] isRunningUnitTests]) {
...
- (void)createVersionFile {
  ...
  [versionString
      writeToFile:[[NSFileManager defaultManager] versionNumberFilename]
       atomically:NO
         encoding:NSUTF8StringEncoding
            error:nil];
}
...
- (BOOL)quietFileExists {
  return [[NSFileManager defaultManager]
      fileExistsAtPath:[[NSFileManager defaultManager] quietFilePath]];
}
...
- (void)checkForQuietMode {
  if ([self quietFileExists]) {
    ...
    [[NSFileManager defaultManager]
        removeItemAtPath:[[NSFileManager defaultManager] quietFilePath]
                   error:&error];
    if (error) {
      NSLog(@"Failed to remove %@: %@; not launching in quiet mode",
            [[NSFileManager defaultManager] quietFilePath], error);
    } else {
      NSLog(@"%@ exists, launching in quiet mode",
            [[NSFileManager defaultManager] quietFilePath]);
      quiet_ = YES;
    }
  }
}
...
- (BOOL)version:(NSString *)version newerThan:(NSString *)otherVersion {
  id<SUVersionComparison> comparator =
...
- (void)updateRestoreWindowArrangementsMenu:(NSMenuItem *)menuItem
                                     asTabs:(BOOL)asTabs {
  [WindowArrangements
      refreshRestoreArrangementsMenu:menuItem
                        withSelector:asTabs ? @selector
                                         (restoreWindowArrangementAsTabs:)
                                            : @selector
                                         (restoreWindowArrangement:)
                     defaultShortcut:kRestoreDefaultWindowArrangementShortcut
                          identifier:asTabs
                                         ? @"Restore Window Arrangement as Tabs"
...
- (void)addMenuItemView:(NSView *)view
                 toMenu:(NSMenu *)menu
                  title:(NSString *)title {
  NSMenuItem *newItem;
  newItem = [[[iTermTabColorMenuItem alloc]
      initWithTitle:title
  ...
  for (NSTabViewItem *aTabViewItem in tabViewItemArray) {
    ...
    if ([aTab activeSession]) {
      aMenuItem = [[NSMenuItem alloc]
          initWithTitle:[[aTab activeSession] name]
...
- (IBAction)findGlobally:(id)sender {
  if (!_globalSearchWindowController) {
    _globalSearchWindowController =
        [[iTermGlobalSearchWindowController alloc] init];
  }
...
- (void)restoreWindowArrangement:(id)sender {
  [[iTermController sharedInstance]
      loadWindowArrangementWithName:[sender title]];
}

- (void)restoreWindowArrangementAsTabs:(id)sender {
  [[iTermController sharedInstance]
      loadWindowArrangementWithName:[sender title]
                   asTabsInTerminal:[[iTermController sharedInstance]
                                        currentTerminal]];
}
...
- (IBAction)toggleToolbeltTool:(NSMenuItem *)menuItem {
  if ([iTermToolbeltView numberOfVisibleTools] == 1 &&
      [menuItem state] == NSControlStateValueOn) {
    return;
  }
...
- (IBAction)toggleFullScreenTabBar:(id)sender {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kShowFullscreenTabsSettingDidChange
                    object:nil
                  userInfo:nil];
}
...
- (IBAction)undo:(id)sender {
  ...
  if (undoResponder) {
    ...
    iTermRestorableSession *restorableSession =
        [controller popRestorableSession];
    if (restorableSession) {
      ...
      case kiTermRestorableSessionGroupWindow:
        ...
        term = [PseudoTerminal
             terminalWithArrangement:restorableSession.arrangement
                               named:nil
                            sessions:restorableSession.sessions
...
- (IBAction)toggleMultiLinePasteWarning:(NSButton *)sender {
  if (sender.tag == 0) {
    [iTermAdvancedSettingsModel
        setNoSyncDoNotWarnBeforeMultilinePaste:
            ![iTermAdvancedSettingsModel noSyncDoNotWarnBeforeMultilinePaste]];
  } else if (sender.tag == 1) {
    [iTermAdvancedSettingsModel
        setPromptForPasteWhenNotAtPrompt:![iTermAdvancedSettingsModel
                                             promptForPasteWhenNotAtPrompt]];
  } else if (sender.tag == 2) {
    [iTermAdvancedSettingsModel
        setNoSyncDoNotWarnBeforePastingOneLineEndingInNewlineAtShellPrompt:
            ![iTermAdvancedSettingsModel
                noSyncDoNotWarnBeforePastingOneLineEndingInNewlineAtShellPrompt]];
  }
}
...
- (IBAction)showHelp:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL
                  URLWithString:@"https://www.iterm2.com/documentation.html"]];
}

- (iTermScriptsMenuController *)scriptsMenuController {
  if (!_scriptsMenuController) {
    _scriptsMenuController =
        [[iTermScriptsMenuController alloc] initWithMenu:_scriptsMenu];
...
- (IBAction)installPythonRuntime:(id)sender { // Explicit request from menu item
  [[iTermPythonRuntimeDownloader sharedInstance]
      downloadOptionalComponentsIfNeededWithConfirmation:NO
                                           pythonVersion:nil
                               minimumEnvironmentVersion:0
                                      requiredToContinue:NO
                                          withCompletion:^(
                                              iTermPythonRuntimeDownloaderStatus
                                                  status) {
                                            if (status ==
                               ...
                               minimumEnvironmentVersion:0
                                      requiredToContinue:YES
                                          withCompletion:^(
                                            ...
                                            if (![iTermAPIHelper
                                                    sharedInstanceFromExplicitUserAction]) {
                                              return;
                                            }
                                            NSString *apython = [[[[[iTermPythonRuntimeDownloader
                                                sharedInstance]
                                                pathToStandardPyenvPythonWithPythonVersion:
                                                    nil]
                                                stringByDeletingLastPathComponent]
                                                stringByAppendingPathComponent:
                                                    @"apython"]
                                                stringWithBackslashEscapedShellCharactersIncludingNewlines:
                                                    YES];
                                            NSURL *bannerURL = [[NSBundle
                                                mainBundle]
                                                URLForResource:@"repl_banner"
                                                 withExtension:@"txt"];
                                            NSString *bannerText = [NSString
                                                stringWithContentsOfURL:
                                                    bannerURL
                                                               encoding:
                                                                  ...
                                                                  error:nil];
                                            NSString *cookie =
                                                [[iTermWebSocketCookieJar
                                                    sharedInstance]
                                                    randomStringForCookie];
                                            NSString *key =
                                                [[NSUUID UUID] UUIDString];
                                            NSString *identifier =
                                                [[iTermAPIConnectionIdentifierController
                                                    sharedInstance]
                                                    identifierForKey:key];
                                            iTermScriptHistoryEntry *entry =
                                                [[[iTermScriptHistoryEntry
                                                    alloc]
                                                    initWithName:@"REPL"
                                                        fullPath:nil
                                                      identifier:identifier
                                                        relaunch:nil]
                                                    autorelease];
                                            [[iTermScriptHistory sharedInstance]
                                                addHistoryEntry:entry];
                                            NSDictionary *environment = @{
                                              @"ITERM2_COOKIE" : cookie,
                                              @"ITERM2_KEY" : key
                                            };

                                            [[iTermController sharedInstance]
                                                openSingleUseWindowWithCommand:
                                                    apython
                                                                     arguments:@[
                                                                       @"--banner=\\\"\\\""
                                                                     ]
...
// TODO(georgen): Disable "Edit Current Session..." when there are no current
// sessions.
...
- (IBAction)toggleUseBackgroundPatternIndicator:(id)sender {
  ...
  [[NSUserDefaults standardUserDefaults]
      setBool:value
       forKey:kUseBackgroundPatternIndicatorKey];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kUseBackgroundPatternIndicatorChangedNotification
                    object:nil];
}

- (IBAction)enableMarkAlertShowsModalAlert:(id)sender {
  [[NSUserDefaults standardUserDefaults] setObject:kMarkAlertActionModalAlert
                                            forKey:kMarkAlertAction];
}

- (IBAction)enableMarkAlertPostsNotification:(id)sender {
  [[NSUserDefaults standardUserDefaults]
      setObject:kMarkAlertActionPostNotification
         forKey:kMarkAlertAction];
}
...
- (NSString *)gpuUnavailableStringForReason:
    (iTermMetalUnavailableReason)reason {
...
- (IBAction)gpuRendererAvailability:(id)sender {
  ...
  NSString *reason =
      [self gpuUnavailableStringForReason:tab.metalUnavailableReason];
  if (reason) {
    alert.informativeText = [NSString
        stringWithFormat:
            @"GPU rendering is off in the current session because %@", reason];
  } else {
    alert.informativeText =
        @"GPU rendering is enabled for the current session.";
  }
  [alert runModal];
}

- (IBAction)openSourceLicenses:(id)sender {
  NSURL *url = [[NSBundle bundleForClass:self.class] URLForResource:@"Licenses"
                                                      withExtension:@"txt"];
...
- (void)updateProcessType {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyUIElementRequiresHotkeys]) {
    const BOOL onlyHotKeyWindowsOpen =
        [self.terminals allWithBlock:^BOOL(PseudoTerminal *term) {
          return term.isHotKeyWindow;
        }];
...
- (BOOL)possiblyTmuxValueForWindow:(BOOL)isWindow cancel:(BOOL *)cancel {
  *cancel = NO;
  ...
  if ([[[[iTermController sharedInstance] currentTerminal] currentSession]
          isTmuxClient]) {
    NSString *heading =
    ...
    NSString *title =
        [NSString stringWithFormat:@"The current session is a tmux session. "
                                   @"Would you like to create a new tmux %@ or "
    ...
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:title
                 ...
                 silenceable:kiTermWarningTypePermanentlySilenceable
                     heading:heading
                      window:[[[iTermController sharedInstance] currentTerminal]
                                 window]];
...
- (void)changePasteSpeedBy:(double)factor
  ...
  [ToastWindowController
      showToastWithMessage:[NSString
                               stringWithFormat:@"Pasting at up to %@/sec",
                                                [NSString
                                                    it_formatBytes:rate]]];
}
  ...
  if ([iTermAdvancedSettingsModel hideStuckTooltips]) {
    for (NSWindow *window in [NSApp windows]) {
      if ([NSStringFromClass([window class])
              isEqualToString:@"NSToolTipPanel"]) {
        [window close];
      }
    }
  }
...
- (void)restoreBuriedSessionsState {
  if (_buriedSessionsState) {
    [[iTermBuriedSessions sharedInstance]
        restoreFromState:_buriedSessionsState];
...
- (void)iTermPasswordManagerEnterUserName:(NSString *)username
                                broadcast:(BOOL)broadcast {
  [[[[iTermController sharedInstance] currentTerminal] currentSession]
      writeTask:[username stringByAppendingString:@"\n"]];
}
...
- (void)currentSessionDidChange {
  ...
  iTermQuickLookController *quickLookController =
      currentWindow.currentSession.quickLookController;
...
- (void)orphanServerAdopterOpenSessionForConnection:
            (iTermGeneralServerConnection)generalConnection
                                         ...
                                         completion:
                                             (void (^)(PTYSession *session))
                                                 completion {
  assert([iTermAdvancedSettingsModel runJobsInServers]);

  void (^makeSession)(NSDictionary *_Nonnull, PseudoTerminal *_Nonnull,
...
- (void)makeSessionWithConnection:
            (iTermGeneralServerConnection)generalConnection
  ...
  PTYSession *session =
      [[term.sessionFactory newSessionWithProfile:defaultProfile
  ...
  iTermSessionAttachOrLaunchRequest *launchRequest =
      [iTermSessionAttachOrLaunchRequest
          launchRequestWithSession:session
                         canPrompt:NO
               ...
               hasServerConnection:YES
                  serverConnection:temp
                      ...
                      allowURLSubs:NO
                       environment:@{}
                       customShell:[ITAddressBookMgr
                                       customShellForProfile:defaultProfile]
                            oldCWD:nil
                    forceUseOldCWD:NO
                           command:nil
                            isUTF8:nil
...
- (void)makeSessionWithPartialAttachment:
            (id<iTermPartialAttachment>)partialAttachment
                        windowController:(PseudoTerminal *)term
                              completion:
                                  (void (^)(PTYSession *))didMakeSession {
  Profile *defaultProfile = [[ProfileModel sharedInstance] defaultBookmark];
  PTYSession *session =
      [[term.sessionFactory newSessionWithProfile:defaultProfile
                                           parent:nil] autorelease];
...
- (NSArray<NSWindow *> *)restorableStateWindows {
  return [[[iTermController sharedInstance] terminals]
      mapWithBlock:^id(PseudoTerminal *term) {
...
- (void)restorableStateRestoreWithCoder:(NSCoder *)coder
                             ...
                             completion:
                                 (void (^)(NSWindow *_Nonnull,
                                           NSError *_Nonnull))completion {
  DLog(@"Enqueue(1) restoration for window with identifier %@", identifier);
  [_untitledWindowStateMachine didRestoreSomeWindows];
  [PseudoTerminalRestorer
      restoreWindowWithIdentifier:identifier
              pseudoTerminalState:[[[PseudoTerminalState alloc]
                                      initWithCoder:coder] autorelease]
                           system:NO
...
- (void)
  ...
  if (!dict) {
    NSError *error = [[[NSError alloc] initWithDomain:@"com.iterm2.app-delegate"
                                                 code:1
  ...
  PseudoTerminalState *state =
      [[PseudoTerminalState alloc] initWithDictionary:dict];
      ...
      restoreWindowWithIdentifier:identifier
              pseudoTerminalState:state
                ...
                completionHandler:^(NSWindow *window, NSError *error) {
                  ...
                  PseudoTerminal *term =
                      [PseudoTerminal castFrom:window.delegate];
                  if (!term) {
                    completion(window, error);
                    return;
                  }
                  ...
                  [term asyncRestoreState:state
                      timeout:^(NSArray *partialAttachments) {
                        [[iTermOrphanServerAdopter sharedInstance]
                            adoptPartialAttachments:partialAttachments];
                      }
                      completion:^{
                        DLog(@"Async restore finished for identifier %@",
                             identifier);
                        completion(window, error);
                      }];
                }];
      ...
      encodeChildWithKey:@"app"
              ...
              generation:generation
                   block:^BOOL(iTermGraphEncoder *_Nonnull encoder) {
                     ...
                     [encoder
                         encodeChildWithKey:kScreenCharRestorableStateKey
                                 identifier:@""
                                 generation:ScreenCharGeneration()
                                      block:^BOOL(iTermGraphEncoder
                                                      *_Nonnull subencoder) {
                                        [subencoder
                                            mergeDictionary:
                                                ScreenCharEncodedRestorableState()];
                                        return YES;
                                      }];

                     [encoder
                         encodeChildWithKey:kURLStoreRestorableStateKey
                                 identifier:@""
                                 generation:[[iTermURLStore sharedInstance]
                                                generation]
                                      block:^BOOL(iTermGraphEncoder
                                                      *_Nonnull subencoder) {
                                        [subencoder
                                            mergeDictionary:
                                                [[iTermURLStore sharedInstance]
                                                    dictionaryValue]];
                                        return YES;
                                      }];

                     [encoder
                         encodeChildWithKey:kHotkeyWindowsRestorableStates
                                 identifier:@""
                                 generation:iTermGenerationAlwaysEncode
                                      block:^BOOL(iTermGraphEncoder
                                                      *_Nonnull subencoder) {
                                        return [[iTermHotKeyController
                                            sharedInstance]
                                            encodeGraphWithEncoder:subencoder];
                                      }];
                     ...
                     if ([[[iTermBuriedSessions sharedInstance] buriedSessions]
                       ...
                       [encoder
                           encodeObject:[[iTermBuriedSessions sharedInstance]
                                            restorableState]
...
- (void)restorableStateRestoreApplicationStateWithRecord:
  ...
  iTermEncoderGraphRecord *app = [record childRecordWithKey:@"app"
                                                 identifier:@""];
  if (!app) {
    DLog(@"No app record");
    return;
  ...
  NSDictionary *screenCharState =
      [app objectWithKey:kScreenCharRestorableStateKey
                   class:[NSDictionary class]];
  [NSDictionary castFrom:[[app childRecordWithKey:kScreenCharRestorableStateKey
                                       identifier:@""] propertyListValue]];
  if (screenCharState) {
    ScreenCharDecodeRestorableState(screenCharState);
  }
  [PseudoTerminalRestorer setPostRestorationCompletionBlock:^{
  ...
  NSDictionary *urlStoreState = [NSDictionary
      castFrom:[[app childRecordWithKey:kURLStoreRestorableStateKey
                             identifier:@""] propertyListValue]];
  if (urlStoreState) {
    [[iTermURLStore sharedInstance] loadFromDictionary:urlStoreState];
  }

  iTermEncoderGraphRecord *hotkeyWindowsStates =
      [app childRecordWithKey:kHotkeyWindowsRestorableStates identifier:@""];
  if (hotkeyWindowsStates) {
    // We have to create the hotkey window now because we need to attach to
    // servers before launch finishes; otherwise any running hotkey window jobs
    // will be treated as orphans.
    const BOOL createdAny = [[iTermHotKeyController sharedInstance]
        createHiddenWindowsByDecoding:hotkeyWindowsStates];
    if (createdAny) {
      [_untitledWindowStateMachine didRestoreSomeWindows];
    }
  }
  ...
  _buriedSessionsState =
      [[NSArray fromGraphRecord:app withKey:iTermBuriedSessionState] retain];
...
- (void)untitledWindowStateMachineCreateNewWindow:
    (iTermUntitledWindowStateMachine *)sender {
