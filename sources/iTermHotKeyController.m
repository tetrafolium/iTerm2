...
- (BOOL)showWindowForProfileHotKey:(iTermProfileHotKey *)profileHotKey
                               url:(NSURL *)url {
...
- (iTermProfileHotKey *)profileHotKeyForWindowController:
...
- (BOOL)createHiddenWindowsByDecoding:(iTermEncoderGraphRecord *)record {
  iTermOrderedDictionary<NSString *, iTermProfileHotKey *> *index =
      [self profileHotkeyWindowsIndex];
  NSMutableArray<NSDictionary *> *restorableStates = [NSMutableArray array];

  [record enumerateArrayWithKey:@"windows"
                          block:^(NSString *_Nonnull guid, NSInteger i, id obj,
                                  BOOL *_Nonnull stop) {
...
- (iTermOrderedDictionary<NSString *, iTermProfileHotKey *> *)
    profileHotkeyWindowsIndex {
  return [iTermOrderedDictionary
      byMapping:_hotKeys
          block:^id _Nullable(NSUInteger index,
                              __kindof iTermBaseHotKey *hotkey) {
            iTermProfileHotKey *profileHotKey =
                [iTermProfileHotKey castFrom:hotkey];
            if (!profileHotKey) {
              return nil;
            }
                  ...
                  identifiers:index.keys
                      options:0
                        block:^BOOL(NSString *_Nonnull identifier, NSInteger i,
                          ...
                          return
                              [profileHotKey encodeGraphWithEncoder:subencoder];
                        }];
  return YES;
}

- (BOOL)anyProfileHotkeyWindowHasInvalidState {
  return [[[self profileHotkeyWindowsIndex]
              values] filteredArrayUsingBlock:^BOOL(iTermProfileHotKey *obj) {
           const BOOL result =
               obj.windowController.ptyWindow.it_restorableStateInvalid;
...
- (void)nonHotKeyWindowDidBecomeKey {
  BOOL anyWindowRollingOut =
      [[self profileHotKeys] anyWithBlock:^BOOL(iTermProfileHotKey *anObject) {
        return [anObject rollingOut];
      }];
  BOOL anyWindowRollingIn =
      [[self profileHotKeys] anyWithBlock:^BOOL(iTermProfileHotKey *anObject) {
        return [anObject rollingIn];
      }];
  if (!anyWindowRollingOut && !anyWindowRollingIn) {
    // If there is previous state:
    // Going from hotkey window to non-hotkey window. Forget the previous state
    // because there's no need to ever return to it now. This happens when
    // navigating explicitly from a hotkey window to a non-hotkey terminal.
    DLog(@"A non-hotkey window became key. Removing previous state %p",
         self.previousState);
    self.previousState = nil;
  }

  // Clear the `wasAutoHidden` flag if the auto-hide was not due to the
  // application resigning active state. We can detect this because the only
  // other way a window would auto-hide is if another window becomes key.
  for (iTermProfileHotKey *profileHotKey in self.profileHotKeys) {
    profileHotKey.wasAutoHidden = NO;
  }
}

- (void)autoHideHotKeyWindowsExcept:
    (NSArray<NSWindowController *> *)exceptions {
  NSMutableArray *controllers =
      [[[self hotKeyWindowControllers] mutableCopy] autorelease];
  [controllers removeObjectsInArray:exceptions];
  [self autoHideHotKeyWindows:controllers];
}

- (void)autoHideHotKeyWindows:
    (NSArray<NSWindowController *> *)windowControllersToConsiderHiding {
  ...
  // If any window is rolling in, then we don't want this window to restore the
  // previously active app when it finishes rolling out.
  BOOL anyHotkeyWindowIsRollingIn =
      [self.profileHotKeys anyWithBlock:^BOOL(iTermProfileHotKey *anObject) {
        return anObject.rollingIn;
      }];
  ...
  for (PseudoTerminal
           *hotKeyWindowController in windowControllersToConsiderHiding) {
    ...
    iTermProfileHotKey *profileHotKey = [[iTermHotKeyController sharedInstance]
        profileHotKeyForWindowController:hotKeyWindowController];
    if (!profileHotKey.autoHides) {
      DLog(@"Autohide disabled for %@ so not auto-hiding it",
           hotKeyWindowController);
      continue;
    }

    if (profileHotKey.rollingIn) {
      DLog(@"Currently rolling in %@ so not auto-hiding it",
           hotKeyWindowController);
      continue;
    }
    if (hotKeyWindowController.isReplacingWindow) {
      DLog(@"Currently replacing window for %@ so not auto-hiding it",
           hotKeyWindowController);
      continue;
    }
    ...
    BOOL suppressHide = [[[NSApp keyWindow] windowController]
        isKindOfClass:[PseudoTerminal class]];
...
- (NSArray<iTermHotKeyDescriptor *> *)descriptorsForProfileHotKeysExcept:
    (Profile *)profile {
  return [self.profileHotKeys flatMapWithBlock:^id(
                                  iTermProfileHotKey *profileHotKey) {
    if (![profileHotKey.profile[KEY_GUID] isEqualToString:profile[KEY_GUID]]) {
      NSMutableArray *result = [NSMutableArray array];
      if ([iTermProfilePreferences stringForKey:KEY_HAS_HOTKEY
                                      inProfile:profileHotKey.profile]) {
        NSUInteger keyCode = [iTermProfilePreferences
            unsignedIntegerForKey:KEY_HOTKEY_KEY_CODE
                        inProfile:profileHotKey.profile];
        NSEventModifierFlags modifiers = [iTermProfilePreferences
            unsignedIntegerForKey:KEY_HOTKEY_MODIFIER_FLAGS
                        inProfile:profileHotKey.profile];
        [result
            addObject:[iTermHotKeyDescriptor descriptorWithKeyCode:keyCode
                                                         modifiers:modifiers]];
      }
      ...
      if ([iTermProfilePreferences boolForKey:KEY_HOTKEY_ACTIVATE_WITH_MODIFIER
        ...
        iTermHotKeyModifierActivation modifierActivation =
            [iTermProfilePreferences
                unsignedIntegerForKey:KEY_HOTKEY_MODIFIER_ACTIVATION
                            inProfile:profileHotKey.profile];
        [result
            addObject:[iTermHotKeyDescriptor
                          descriptorWithModifierActivation:modifierActivation]];
      }
      return result;
    } else {
      return nil;
...
- (NSArray<PseudoTerminal *> *)siblingWindowControllersOf:
    (PseudoTerminal *)windowController {
  NSArray<iTermHotKeyDescriptor *> *referenceHotKeyDescriptors = [[self
      profileHotKeyForWindowController:windowController] hotKeyDescriptors];
  iTermHotKeyDescriptor *referenceModifierActivationDescriptor =
      [[self profileHotKeyForWindowController:windowController]
          modifierActivationDescriptor];
  return [self.profileHotKeys mapWithBlock:^id(
                                  iTermProfileHotKey *profileHotKey) {
    if ([profileHotKey.hotKeyDescriptors isEqual:referenceHotKeyDescriptors] ||
        [profileHotKey.modifierActivationDescriptor
            isEqual:referenceModifierActivationDescriptor]) {
      NSWindowController *windowController =
          profileHotKey.windowController.weaklyReferencedObject;
      if (windowController) {
...
- (BOOL)dockIconClicked {
  ...
  if (self.visibleWindowControllers.count > 0) {
    ...
    [self.profileHotKeys
        enumerateObjectsUsingBlock:^(iTermProfileHotKey *_Nonnull profileHotKey,
    ...
    NSUInteger numberOfTerminalWindowsOpen =
    ...
    [self.profileHotKeys
        enumerateObjectsUsingBlock:^(iTermProfileHotKey *_Nonnull profileHotKey,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
          switch ([iTermProfilePreferences
              unsignedIntegerForKey:KEY_HOTKEY_DOCK_CLICK_ACTION
                          inProfile:profileHotKey.profile]) {
...
- (iTermProfileHotKey *)didCreateWindowController:
                            (PseudoTerminal *)windowController
                                      withProfile:(Profile *)profile {
  iTermProfileHotKey *profileHotKey = [_hotKeys
      objectOfClass:[iTermProfileHotKey class]
        passingTest:^BOOL(id element, NSUInteger index, BOOL *stop) {
          return
              [[element profile][KEY_GUID] isEqualToString:profile[KEY_GUID]];
        }];
  if (!profileHotKey) {
    return nil;
  }
...
- (NSArray<iTermPanel *> *)visibleFloatingHotkeyWindows {
  return [[self allFloatingHotkeyWindows]
      filteredArrayUsingBlock:^BOOL(iTermPanel *anObject) {
...
- (iTermProfileHotKey *)profileHotKeyForGUID:(NSString *)guid {
  NSArray<iTermProfileHotKey *> *profileHotKeys =
      [_hotKeys objectsOfClasses:@[ [iTermProfileHotKey class] ]];
  iTermProfileHotKey *profileHotKey =
      [profileHotKeys objectPassingTest:^BOOL(iTermProfileHotKey *element,
...
- (BOOL)shouldAutoHide {
  ...
  if ([keyWindowElect respondsToSelector:@selector(autoHidesHotKeyWindow)] &&
    ...
    DLog(@"The key window-elect does not auto-hide the hotkey window: %@",
         keyWindowElect);
    return NO;
  }
  ...
  if ([keyWindowController
          respondsToSelector:@selector(autoHidesHotKeyWindow)] &&
      ![keyWindowController autoHidesHotKeyWindow]) {
    DLog(
        @"The key window's controller does not auto-hide the hotkey window: %@",
        keyWindow);
    return NO;
  }
  if (keyWindowController != nil &&
      [[[NSBundle bundleForClass:keyWindowController.class] bundlePath]
          isEqualToString:[[NSBundle bundleForClass:[SUUpdater class]]
                              bundlePath]]) {
    DLog(@"The key window's controller appears to belong to Sparkle (it is "
         @"%@), so don't auto-hide.",
         keyWindowController);
    return NO;
  }
  ...
  NSArray<PseudoTerminal *> *hotKeyWindowControllers =
      self.hotKeyWindowControllers;
  BOOL nonHotkeyTerminalIsKey = [keyTerminalWindows
      containsObjectBesidesObjectsInArray:hotKeyWindowControllers];
  BOOL haveMain = [[iTermController sharedInstance] anyWindowIsMain];
  DLog(@"main window is %@, key terminals are %@, hotkey terminals are %@",
       [NSApp mainWindow], keyTerminalWindows, hotKeyWindowControllers);
  if (haveMain && !nonHotkeyTerminalIsKey) {
    // Issue 1251: we can take this path when a "clipboard manager" window is
    // open.
...
- (void)storePreviouslyActiveApp:(iTermProfileHotKey *)profileHotKey {
  if (self.previousState && [NSApp isActive]) {
    DLog(@"Not updating previous state because we are active and already have "
         @"previous state");
    return;
  }
  if (self.previousState.owner.rollingIn) {
    // Is the previous state for a hotkey window that's rolling in? If so, keep
    // it, because it will know about the previously active app.
    DLog(@"Not updating previous state because the existing previous state's "
         @"owner is rolling in");
    return;
  }
...
- (BOOL)otherHotKeyWindowWillBecomeKeyAfterOrderOut:
    (iTermProfileHotKey *)profileHotKey {
...
- (BOOL)willFinishRollingOutProfileHotKey:(iTermProfileHotKey *)profileHotKey {
  ...
  DLog(@"Finished rolling out %p. key window is %@.",
       profileHotKey.windowController, [[NSApp keyWindow] windowController]);
  if (![self otherHotKeyWindowWillBecomeKeyAfterOrderOut:profileHotKey]) {
    ...
    BOOL result = [previousState
        restoreAllowingAppSwitch:!profileHotKey
                                      .closedByOtherHotkeyWindowOpening];
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
  for (iTermProfileHotKey *profileHotKey in self.profileHotKeys) {
    if (profileHotKey.windowController.weaklyReferencedObject &&
        ...
        [iTermProfilePreferences boolForKey:KEY_HOTKEY_REOPEN_ON_ACTIVATION
                                  inProfile:profileHotKey.profile]) {
      [profileHotKey showHotKeyWindow];
    }
  }
