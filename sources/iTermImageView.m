...
- (void)loadTiledImage {
  ...
  const CGPatternCallbacks callbacks = {.version = 0,
                                        .drawPattern = iTermImageViewDrawImage,
                                        .releaseInfo =
                                            iTermImageViewReleaseImage};
  CGPatternRef pattern = CGPatternCreate(
      cgImage, CGRectMake(0, 0, width, height),
      CGAffineTransformMake(1, 0, 0, 1, 0, 0), width, height,
      kCGPatternTilingConstantSpacing, YES /* isColored */, &callbacks);
  const CGColorSpaceRef colorSpace = CGColorSpaceCreatePattern(NULL);
  const CGFloat components[1] = {1.0};
  const CGColorRef color =
