...
NSString *const kItermIndicatorBroadcastInput =
...
NSString *const kiTermIndicatorAllOutputSuppressed =
...
- (CGFloat)alpha {
  NSTimeInterval elapsed =
      [NSDate timeIntervalSinceReferenceDate] - _flashStartTime;
...
@implementation iTermIndicatorsHelper {
  ...
  // Rate limits calls to setNeedsDisplay: to not be faster than drawRect can be
  // called.
...
+ (NSDictionary *)indicatorImages {
  ...
  dispatch_once(&onceToken, ^{
    gIndicatorImages = @{
      ...
      kiTermIndicatorWrapToTop : [NSImage it_imageNamed:@"wrap_to_top"
                                               forClass:self.class],
      kiTermIndicatorWrapToBottom : [NSImage it_imageNamed:@"wrap_to_bottom"
                                                  forClass:self.class],
      kItermIndicatorBroadcastInput : [NSImage it_imageNamed:@"BroadcastInput"
                                                    forClass:self.class],
      kiTermIndicatorMaximized : [NSImage it_imageNamed:@"Maximized"
                                               forClass:self.class],
      kiTermIndicatorCoprocess : [NSImage it_imageNamed:@"Coprocess"
                                               forClass:self.class],
      kiTermIndicatorAlert : [NSImage it_imageNamed:@"Alert"
                                           forClass:self.class],
      kiTermIndicatorAllOutputSuppressed :
          [NSImage it_imageNamed:@"SuppressAllOutput" forClass:self.class],
      kiTermIndicatorZoomedIn : [NSImage it_imageNamed:@"Zoomed"
                                              forClass:self.class],
      kiTermIndicatorCopyMode : [NSImage it_imageNamed:@"CopyMode"
                                              forClass:self.class]
    };
...
+ (NSArray *)flashingIndicatorIdentifiers {
  return @[
    kiTermIndicatorBell, kiTermIndicatorWrapToTop, kiTermIndicatorWrapToBottom
  ];
}

+ (NSArray *)sequentialIndicatorIdentifiers {
  return @[
    kiTermIndicatorMaximized, kItermIndicatorBroadcastInput,
    kiTermIndicatorCoprocess, kiTermIndicatorAlert,
    kiTermIndicatorAllOutputSuppressed, kiTermIndicatorZoomedIn,
...
- (void)enumerateTopRightIndicatorsInFrame:(NSRect)frame
                                   andDraw:(BOOL)shouldDraw
                                     block:(void (^)(NSString *, NSImage *,
                                                     NSRect))block {
  NSArray *sequentialIdentifiers =
      [iTermIndicatorsHelper sequentialIndicatorIdentifiers];
  const CGFloat vmargin =
  ...
  for (NSString *identifier in sequentialIdentifiers) {
    ...
    if (indicator) {
      ...
      block(identifier, image,
            NSMakeRect(point.x, point.y, image.size.width, image.size.height));
      if (shouldDraw) {
        [image drawInRect:NSMakeRect(point.x, point.y, image.size.width,
                                     image.size.height)
...
- (void)enumerateCenterIndicatorsInFrame:(NSRect)frame
                                   block:(void (^)(NSString *, NSImage *,
                                                   NSRect, CGFloat))block {
  NSArray *centeredIdentifiers =
      [iTermIndicatorsHelper flashingIndicatorIdentifiers];
...
- (void)drawInFrame:(NSRect)frame {
  ...
  if (_fullScreenAlpha > 0) {
    ...
    [[[_delegate indicatorFullScreenFlashColor]
        colorWithAlphaComponent:_fullScreenAlpha] set];
...
- (void)didDraw {
  NSTimeInterval elapsed =
      [NSDate timeIntervalSinceReferenceDate] - _fullScreenFlashStartTime;
...
- (void)checkForFlashUpdate {
  DLog(@"Check for flash update. full screen flash start time is %@, "
       @"haveSetNeedsDisplay=%@",
       @(_fullScreenFlashStartTime), @(_haveSetNeedsDisplay));
  NSTimeInterval elapsed =
      [NSDate timeIntervalSinceReferenceDate] - _fullScreenFlashStartTime;
  if (_fullScreenFlashStartTime > 0 || [self haveFlashingIndicator]) {
    const CGFloat kMaxFullScreenFlashAlpha =
        [iTermAdvancedSettingsModel indicatorFlashInitialAlpha];
    _fullScreenAlpha = MAX(0, 1.0 - elapsed / kFullScreenFlashDuration) *
                       kMaxFullScreenFlashAlpha;
  ...
  if (_fullScreenFlashStartTime > 0 || [self haveFlashingIndicator]) {
    ...
    [self performSelector:@selector(checkForFlashUpdate)
               withObject:nil
               afterDelay:1 / 60.0];
  }
}
...
- (void)beginFlashingIndicator:(NSString *)identifier {
  assert([[iTermIndicatorsHelper flashingIndicatorIdentifiers]
