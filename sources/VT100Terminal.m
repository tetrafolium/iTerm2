...
NSString *const kTerminalStateCanonicalEncodingKey =
...
NSString *const kTerminalStateReverseWraparoundModeKey =
...
NSString *const kTerminalStateSoftAlternateScreenModeKey =
    @"Soft Alternate Screen Mode";
...
@property(nonatomic, strong, readwrite)
    NSMutableArray<NSNumber *> *sendModifiers;
@end
...
// Color constants
// Color codes for 8-color mode. Black and white are the limits; other codes can
...
// Color codes for 16-color mode. Black and white are the limits; other codes
...
- (void)setTermType:(NSString *)termtype {
  ...
  self.allowKeypadMode =
      [_termType rangeOfString:@"xterm"].location != NSNotFound;

  int r;

  // NOTE: This seems to cause a memory leak. The setter for termTypeIsValid
  // (below) has the side effect of copying various curses strings, and it
  // depends on this. When I redo output, fix this disaster.
...
- (void)executeDecSetReset:(VT100Token *)token {
  assert(token->type == VT100CSI_DECSET || token->type == VT100CSI_DECRST);
  ...
  for (int i = 0; i < token.csi->count; i++) {
    ...
    case 9:
      ...
      break;
    case 20:
      // This used to be the setter for "line mode", but it wasn't used and it's
      // not supported by xterm. Seemed to have something to do with CR vs LF.
      break;
    case 25:
    ...
    case 2004:
      ...
      [self setBracketedPasteMode:mode && self.allowPasteBracketing
                  withSideEffects:YES];
      break;
    }
  }
  ...
  if (token.csi->count == 0) {
    ...
    for (i = 0; i < token.csi->count; ++i) {
      ...
      case VT100CHARATTR_UNDERLINE: {
        ...
        const int numberOfSubparameters =
            iTermParserGetAllCSISubparametersForParameter(token.csi, i, subs);
      ...
      case VT100CHARATTR_FG_256: {
        ...
        // You can download it for free! If you prefer to spend money, ISO/IEC
        // 8613-6 is supposedly the same thing.
        //
        // Here's a sad story about CSI 38:2, which is used to do 24-bit color.
        //
        // Lots of terminal emulators, iTerm2 included, misunderstood the spec.
        // That's easy to understand if you read it, which I can't recommend
        // doing unless you're looking for inspiration for your next
        // Bulwer-Lytton Fiction Contest entry.
        //
        // See issue 6377 for more context.
        //
        // Ignoring color types we don't support like CMYK, the spec says to do
        // this: CSI 38:2:[color
        // space]:[red]:[green]:[blue]:[unused]:[tolerance]:[tolerance
        // colorspace]
        //
        // Everything after [blue] is optional. Values are decimal numbers in
        // 0...255.
        //
        // Unfortunately, what was implemented for a long time was this:
        // CSI 38:2:[red]:[green]:[blue]:[unused]:[tolerance]:[tolerance
        // colorspace]
        //
        ...
        // The New Order
        // -------------
        // Tolerance never did anything, so we'll accept this non-standards
        // compliant code, which people use: CSI 38:2:[red]:[green]:[blue]
        //
        // As well as the following forms:
        // CSI 38:2:[colorspace]:[red]:[green]:[blue]
        // CSI 38:2:[colorspace]:[red]:[green]:[blue]:<one or more additional
        // colon-delimited arguments, all ignored> CSI 38;2;[red];[green];[blue]
        // // Notice semicolons in place of colons here
        ...
        if (numberOfSubparameters > 0) {
          ...
          // screen_char_t in the comments for fgColorCode.
      ...
      default:
        ...
        if (n >= VT100CHARATTR_FG_BLACK && n <= VT100CHARATTR_FG_WHITE) {
          graphicRendition_.fgColorCode =
              n - VT100CHARATTR_FG_BASE - COLORCODE_BLACK;
          graphicRendition_.fgGreen = 0;
          graphicRendition_.fgBlue = 0;
          graphicRendition_.fgColorMode = ColorModeNormal;
        } else if (n >= VT100CHARATTR_BG_BLACK && n <= VT100CHARATTR_BG_WHITE) {
          graphicRendition_.bgColorCode =
              n - VT100CHARATTR_BG_BASE - COLORCODE_BLACK;
          graphicRendition_.bgGreen = 0;
          graphicRendition_.bgBlue = 0;
          graphicRendition_.bgColorMode = ColorModeNormal;
        }
        ...
        if (n >= VT100CHARATTR_FG_HI_BLACK && n <= VT100CHARATTR_FG_HI_WHITE) {
          graphicRendition_.fgColorCode =
              n - VT100CHARATTR_FG_HI_BASE - COLORCODE_BLACK + 8;
          graphicRendition_.fgGreen = 0;
          graphicRendition_.fgBlue = 0;
          graphicRendition_.fgColorMode = ColorModeNormal;
          ...
          graphicRendition_.bgColorCode =
              n - VT100CHARATTR_BG_HI_BASE - COLORCODE_BLACK + 8;
          graphicRendition_.bgGreen = 0;
          graphicRendition_.bgBlue = 0;
          graphicRendition_.bgColorMode = ColorModeNormal;
        }
...
- (NSColor *)colorForXtermCCSetPaletteString:(NSString *)argument
                              colorNumberPtr:(int *)numberPtr {
  if ([argument length] == 7) {
    int n, r, g, b;
    int count = 0;
    count += sscanf(
        [[argument substringWithRange:NSMakeRange(0, 1)] UTF8String], "%x", &n);
    if (count == 0) {
    ...
    count += sscanf(
        [[argument substringWithRange:NSMakeRange(1, 2)] UTF8String], "%x", &r);
    count += sscanf(
        [[argument substringWithRange:NSMakeRange(3, 2)] UTF8String], "%x", &g);
    count += sscanf(
        [[argument substringWithRange:NSMakeRange(5, 2)] UTF8String], "%x", &b);
    if (count == 4 && n >= 0 && n <= 22 && r >= 0 && r <= 255 && g >= 0 &&
        g <= 255 && b >= 0 && b <= 255) {
      NSColor *srgb = [NSColor colorWithSRGBRed:((double)r) / 255.0
                                          ...
                                          alpha:1];
      NSColor *theColor =
          [srgb colorUsingColorSpace:[NSColorSpace genericRGBColorSpace]];
      *numberPtr = n;
      return theColor;
    }
  }
...
- (void)handleDeviceStatusReportWithToken:(VT100Token *)token
                             withQuestion:(BOOL)withQuestion {
  if ([delegate_ terminalShouldSendReport]) {
    ...
    case 6: // Command from host -- Please report active position
      if (self.originMode) {
        // This is compatible with Terminal but not old xterm :(. it always did
        // what we do in the else clause. This behavior of xterm is fixed by
        // Patch #297.
        [delegate_
            terminalSendReport:
                [self.output
                    reportActivePositionWithX:[delegate_
                                                  terminalRelativeCursorX]
                                            Y:[delegate_
                                                  terminalRelativeCursorY]
                                 withQuestion:withQuestion]];
      } else {
        [delegate_ terminalSendReport:
                       [self.output
                           reportActivePositionWithX:[delegate_ terminalCursorX]
...
- (VT100GridRect)rectangleInToken:(VT100Token *)token
  ...
  // First, construct a coord range from the passed-in parameters. They may be
  ...
  VT100GridCoordRange coordRange =
      VT100GridCoordRangeMake(left, top, right, bottom);
  ...
  // Convert the coordRange to a 0-based rect (all coords are 1-based so far)
  // and return it.
...
- (void)sendChecksumReportWithId:(int)identifier rectangle:(VT100GridRect)rect {
  ...
  if (![self rectangleIsValid:rect]) {
    [delegate_ terminalSendReport:[self.output reportChecksum:0
  ...
  // DCS Pid ! ~ D..D ST
  [delegate_ terminalSendReport:[self.output reportChecksum:checksum
                                             withIdentifier:identifier]];
}
...
// The main and alternate screens have different saved cursors. This returns the
// current one. In tmux mode, only one is used to more closely approximate
// tmux's behavior.
...
- (void)saveCursor {
  ...
  for (int i = 0; i < NUM_CHARSETS; i++) {
    savedCursor->lineDrawing[i] =
        [delegate_ terminalLineDrawingFlagForCharset:i];
  }
...
- (void)setBracketedPasteMode:(BOOL)bracketedPasteMode
              withSideEffects:(BOOL)sideEffects {
  if (sideEffects) {
...
- (void)clampSavedCursorToScreenSize:(VT100GridSize)newSize {
  mainSavedCursor_.position =
      VT100GridCoordMake(MIN(newSize.width - 1, mainSavedCursor_.position.x),
                         MIN(newSize.height - 1, mainSavedCursor_.position.y));
  altSavedCursor_.position =
      VT100GridCoordMake(MIN(newSize.width - 1, altSavedCursor_.position.x),
...
- (void)restoreCursor {
  ...
  for (int i = 0; i < NUM_CHARSETS; i++) {
    [delegate_ terminalSetCharset:i
                toLineDrawingMode:savedCursor->lineDrawing[i]];
  }
...
- (void)softReset {
  // The steps here are derived from xterm's implementation. The order is
...
- (void)executeToken:(VT100Token *)token {
  // Handle tmux stuff, which completely bypasses all other normal execution
  // steps.
  ...
  if ([iTermAdvancedSettingsModel translateScreenToXterm] &&
      [self.termType containsString:@"screen"]) {
  ...
  if (receivingFile_) {
    ...
    if (token->type == XTERMCC_MULTITOKEN_BODY) {
      ...
      [delegate_
          terminalDidReceiveBase64PasteboardString:[token stringForAsciiData]];
      return;
    } else if (token->type == XTERMCC_MULTITOKEN_END) {
  ...
  if (token->savingData && token->type != VT100_SKIP &&
      [delegate_
          terminalIsAppendingToPasteboard]) { // This is the old code that
                                              // echoes to the screen. Its use
                                              // is discouraged.
    // We are probably copying text to the clipboard until esc]1337;EndCopy^G is
    // received.
    if (token->type != XTERMCC_SET_KVP ||
        ![token.string hasPrefix:@"CopyToClipboard"]) {
      // Append text to clipboard except for initial command that turns on
  ...
  case VT100CC_ENQ:
    [delegate_
        terminalSendReport:[_answerBackString dataUsingEncoding:self.encoding]];
    break;
  case VT100CC_BEL:
  ...
  case VT100CSI_DA2:
    if ([delegate_ terminalShouldSendReport]) {
      [delegate_
          terminalSendReport:[self.output reportSecondaryDeviceAttribute]];
    }
    break;
  case VT100CSI_XDA:
    if ([delegate_ terminalShouldSendReport]) {
      if (token.csi->p[0] == 0 || token.csi->p[0] == -1) {
        [delegate_
            terminalSendReport:[self.output reportExtendedDeviceAttribute]];
      }
    }
    break;
  ...
  case ANSICSI_SCP:
  // ANSI SC is just like DECSC, but it's only available when left-right mode is
  // off. There's code before the big switch statement that changes the token
  // type for this case, so if we get here it's definitely the same as DECSC.
  // Fall through.
  case VT100CSI_DECSC:
    [self saveCursor];
    break;
  ...
  case VT100CSI_DECSTBM: {
    ...
    [delegate_ terminalSetScrollRegionTop:top bottom:bottom];
  ...
  case VT100CSI_DECRQCRA: {
    if ([delegate_ terminalIsTrusted]) {
      ...
      VT100GridRect defaultRectangle = VT100GridRectMake(
          0, 0, [delegate_ terminalWidth], [delegate_ terminalHeight]);
      // xterm incorrectly uses the second parameter for the Pid. Since I use
      // this mostly to test xterm compatibility, it's handy to be
      // bugwards-compatible.
  ...
  case VT100CSI_HVP:
    ...
    break;
  case VT100CSI_NEL:
    // We do the linefeed first because it's a no-op if the cursor is outside
    // the left- right margin. Carriage return will move it to the left margin.
    [delegate_ terminalLineFeed];
    [delegate_ terminalCarriageReturn];
    break;
  case VT100CSI_IND:
  ...
  case VT100CSI_XTREPORTSGR: {
    if ([delegate_ terminalIsTrusted]) {
      VT100GridRect defaultRectangle = VT100GridRectMake(
          0, 0, [delegate_ terminalWidth], [delegate_ terminalHeight]);
      [self
          sendSGRReportWithRectangle:[self rectangleInToken:token
                                            startingAtIndex:0
                                           defaultRectangle:defaultRectangle]];
    }
    break;
   ...
   * 0 to 3 inclusive. It is an index into Screen's charsetUsesLineDrawingMode_
   * array. In iTerm2, it is an array of booleans where 0 means normal behavior
   * and 1 means line-drawing. There should be a bunch of other values too (like
  ...
  // The parser sets its own encoding property when these codes are parsed
  // because it must change synchronously, since it also does decoding in its
  // own thread (possibly long before this happens in the main thread).
  ...
  case XTERMCC_ICON_TITLE:
    [delegate_
        terminalSetIconTitle:
            [token.string stringByReplacingControlCharactersWithCaretLetter]];
    break;
  case VT100CSI_ICH:
  ...
  case XTERMCC_WINDOWSIZE_PIXEL:
    [delegate_ terminalSetPixelWidth:token.csi->p[2] height:token.csi->p[1]];

    break;
  case XTERMCC_WINDOWPOS:
    [delegate_ terminalMoveWindowTopLeftPointTo:NSMakePoint(token.csi->p[1],
                                                            token.csi->p[2])];
    break;
  ...
  case XTERMCC_REPORT_WIN_POS: {
    ...
    NSString *s = [NSString
        stringWithFormat:@"\033[3;%d;%dt", (int)topLeft.x, (int)topLeft.y];
    [delegate_ terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
    break;
  }
  case XTERMCC_REPORT_WIN_PIX_SIZE: {
    ...
    NSString *s =
        [NSString stringWithFormat:@"\033[4;%d;%dt",
  ...
  case XTERMCC_REPORT_WIN_SIZE: {
    NSString *s =
        [NSString stringWithFormat:@"\033[8;%d;%dt", [delegate_ terminalHeight],
  ...
  case XTERMCC_REPORT_SCREEN_SIZE: {
    NSString *s =
        [NSString stringWithFormat:@"\033[9;%d;%dt",
  ...
  case XTERMCC_REPORT_ICON_TITLE: {
    NSString *s = [NSString
        stringWithFormat:@"\033]L%@\033\\", [delegate_ terminalIconTitle]];
    [delegate_ terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
    break;
  }
  case XTERMCC_REPORT_WIN_TITLE: {
    // NOTE: In versions prior to 2.9.20150415, we used "L" as the leader here,
    // not "l". That was wrong and may cause bug reports due to breaking bugward
    // compatibility. (see xterm docs)
    NSString *s = [NSString
        stringWithFormat:@"\033]l%@\033\\", [delegate_ terminalWindowTitle]];
    [delegate_ terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
    break;
  }
  ...
  case DCS_REQUEST_TERMCAP_TERMINFO: {
    ...
    NSDictionary *inverseMap =
        [VT100DCSParser termcapTerminfoInverseNameDictionary];
    for (int i = 0; i < token.csi->count; i++) {
      ...
      case kDcsTermcapTerminfoRequestTerminfoName:
        [parts
            addObject:[NSString stringWithFormat:kFormat, hexEncodedKey,
      ...
      case kDcsTermcapTerminfoRequestiTerm2ProfileName:
        [parts addObject:[NSString
                             stringWithFormat:kFormat, hexEncodedKey,
                                              [[delegate_ terminalProfileName]
                                                  hexEncodedString]]];
        ok = YES;
        break;
    ...
    NSString *s =
        [NSString stringWithFormat:@"\033P%d+r%@\033\\", ok ? 1 : 0,
  ...
  case DCS_DECRQSS:
    [delegate_ terminalSendReport:[[self decrqss:token.string]
...
- (NSString *)decrqss:(NSString *)pt {
  ...
  if (payload) {
    return [NSString
        stringWithFormat:@"%cP1$r%@%c\\", VT100CC_ESC, payload, VT100CC_ESC];
  }
  return
      [NSString stringWithFormat:@"%cP0$r%@%c\\", VT100CC_ESC, pt, VT100CC_ESC];
}
  ...
  * |     ⇒  DECSNLS
  ...
  if ([pt isEqualToString:@"m"]) {
    NSArray<NSString *> *codes =
        [[self sgrCodesForGraphicRendition:graphicRendition_].allObjects
            sortedArrayUsingSelector:@selector(compare:)];
    return [NSString
        stringWithFormat:@"%@m", [codes componentsJoinedByString:@";"]];
  }
...
- (NSSet<NSString *> *)sgrCodesForCharacter:(screen_char_t)c {
  VT100GraphicRendition g = {.bold = c.bold,
                             .blink = c.blink,
...
- (NSSet<NSString *> *)sgrCodesForGraphicRendition:
    (VT100GraphicRendition)graphicRendition {
  ...
  case ColorModeNormal:
    if (graphicRendition.fgColorCode < 8) {
      [result
          addObject:[NSString
                        stringWithFormat:@"%@",
                                         @(graphicRendition.fgColorCode + 30)]];
    } else if (graphicRendition.fgColorCode < 16) {
      [result
          addObject:[NSString
                        stringWithFormat:@"%@",
                                         @(graphicRendition.fgColorCode + 90)]];
    } else {
      [result addObject:[NSString
                            stringWithFormat:@"38:5:%@",
  ...
  case ColorModeAlternate:
    ...
    case ALTSEM_DEFAULT:
      break;
    case ALTSEM_REVERSED_DEFAULT: // Not sure quite how to handle this, going
  ...
  case ColorMode24bit:
    [result
        addObject:[NSString stringWithFormat:@"38:2:1:%@:%@:%@",
                                             @(graphicRendition.fgColorCode),
  ...
  case ColorModeNormal:
    if (graphicRendition.bgColorCode < 8) {
      [result
          addObject:[NSString
                        stringWithFormat:@"%@",
                                         @(graphicRendition.bgColorCode + 40)]];
    } else if (graphicRendition.bgColorCode < 16) {
      [result
          addObject:[NSString
                        stringWithFormat:@"%@", @(graphicRendition.bgColorCode +
                                                  100)]];
    } else {
      [result addObject:[NSString
                            stringWithFormat:@"48:5:%@",
  ...
  case ColorMode24bit:
    [result
        addObject:[NSString stringWithFormat:@"48:2:1:%@:%@:%@",
                                             @(graphicRendition.bgColorCode),
...
- (NSArray<NSNumber *> *)xtermParseColorArgument:(NSString *)part {
  if ([part hasPrefix:@"rgb:"]) {
    ...
    // "<index>;rgb:<redhex>/<greenhex>/<bluehex>", e.g. "105;rgb:00/cc/ff"
  ...
  for (int i = 0; i < parts.count; i++) {
    ...
    if ((i % 2) == 0) {
      ...
      if (components) {
        ...
        NSColor *theColor =
            [srgb colorUsingColorSpace:[NSColorSpace genericRGBColorSpace]];
        [delegate_ terminalSetColorTableEntryAtIndex:theIndex color:theColor];
      } else if ([part isEqualToString:@"?"]) {
        NSColor *theColor = [delegate_ terminalColorForIndex:theIndex];
        [delegate_ terminalSendReport:[self.output reportColor:theColor
                                                       atIndex:theIndex
                                                        prefix:@"4;"]];
      }
    }
...
- (void)executeFileCommandWithValue:(NSString *)value {
  ...
  NSString *name = [dict[@"name"]
      stringByBase64DecodingStringWithEncoding:NSISOLatin1StringEncoding];
  if (!name) {
    name = @"Unnamed file";
  }
  if ([dict[@"inline"] boolValue]) {
    NSEdgeInsets inset = {.top = insetTop,
                          .left = insetLeft,
                          .bottom = insetBottom,
                          .right = insetRight};
    const BOOL ok = [delegate_
        terminalWillReceiveInlineFileNamed:name
                                    ofSize:[dict[@"size"] integerValue]
                                     width:width
                                     units:widthUnits
                                    height:height
                                     units:heightUnits
                       preserveAspectRatio:[dict[@"preserveAspectRatio"]
                                               boolValue]
                                     inset:inset];
    if (!ok) {
      return;
    ...
    if (![delegate_
            terminalWillReceiveFileNamed:name
                                  ofSize:[dict[@"size"] integerValue]]) {
      return;
    }
  }
...
- (void)executeXtermTextColorForeground:(BOOL)foreground arg:(NSString *)arg {
  ...
  //   rgb:ffff/ffff/ffff
  //   ?
  const VT100TerminalColorIndex ptyIndex =
      foreground ? VT100TerminalColorIndexText
                 : VT100TerminalColorIndexBackground;
  ...
  if ([arg isEqualToString:@"?"]) {
    ...
    if (components) {
      ...
      NSColor *theColor =
          [srgb colorUsingColorSpace:[NSColorSpace genericRGBColorSpace]];
      [delegate_ terminalSetColorTableEntryAtIndex:ptyIndex color:theColor];
    }
  }
}
...
- (void)executeLink:(VT100Token *)token {
  ...
  self.url =
      urlString.length ? [NSURL URLWithUserSuppliedString:urlString] : nil;
  if (self.url == nil) {
    if (_currentURLCode) {
    ...
    unsigned short code = [[iTermURLStore sharedInstance] codeForURL:self.url
                                                          withParams:params];
    if (code) {
      if (_currentURLCode) {
        [delegate_ terminalWillEndLinkWithCode:_currentURLCode];
...
- (void)executeXtermSetKvp:(VT100Token *)token {
  ...
  if ([key isEqualToString:@"CursorShape"]) {
    ...
    ITermCursorType shapeMap[] = {CURSOR_BOX, CURSOR_VERTICAL,
                                  CURSOR_UNDERLINE};
    ...
    [delegate_
        terminalSetHighlightCursorLine:value.length ? [value boolValue] : YES];
    ...
    if ([delegate_ terminalIsTrusted]) {
      ...
      receivingFile_ = YES;
      [delegate_
          terminalAppendString:[NSString stringWithLongCharacter:0x1F6AB]];
    }
    ...
    XLog(@"Deprecated and unsupported code BeginFile received. Use File "
         @"instead.");
  } else if ([key isEqualToString:@"EndFile"]) {
    XLog(
        @"Deprecated and unsupported code EndFile received. Use File instead.");
    ...
    if ([delegate_ terminalShouldSendReport]) {
      ...
      NSString *width = [[NSString stringWithFormat:@"%0.2f", size.width]
          stringByCompactingFloatingPointString];
      NSString *height = [[NSString stringWithFormat:@"%0.2f", size.height]
          stringByCompactingFloatingPointString];
      NSString *scale = [[NSString stringWithFormat:@"%0.2f", floatScale]
          stringByCompactingFloatingPointString];
      NSString *s =
          [NSString stringWithFormat:@"\033]1337;ReportCellSize=%@;%@;%@\033\\",
                                     height, width, scale];
      [delegate_ terminalSendReport:[s dataUsingEncoding:NSUTF8StringEncoding]];
    }
    ...
    if ([delegate_ terminalIsTrusted]) {
      ...
      NSData *decodedData =
          [[[NSData alloc] initWithBase64EncodedData:valueAsData
      ...
      if (name && [delegate_ terminalShouldSendReportForVariable:name]) {
        ...
        if (name) {
          NSString *variableValue =
              [delegate_ terminalValueOfVariableNamed:name];
          encodedValue = [[variableValue dataUsingEncoding:self.encoding]
              base64EncodedStringWithOptions:0];
        }
        NSString *report = [NSString
            stringWithFormat:@"%c]1337;ReportVariable=%@%c", VT100CC_ESC,
                             encodedValue ?: @"", VT100CC_BEL];
    ...
    if ([delegate_ terminalIsTrusted]) {
      ...
      if (colon != NSNotFound) {
        NSArray<NSString *> *parts =
            [[value substringToIndex:colon] componentsSeparatedByString:@";"];
...
- (void)executeXtermProprietaryEtermExtension:(VT100Token *)token {
  NSString *argument = token.string;
  if (![argument startsWithDigit]) { // Support for proxy icon, if argument is
                                     // empty clears current proxy icon
    if ([delegate_ terminalIsTrusted]) {
  ...
  if (func) {
    if ([func isEqualToString:@"1"]) {
      // Adjusts a color modifier. This attempts to roughly follow the pattern
      // that Eterm defines.
      //
      ...
      //     class: determines which image class will have its color modifier
      //     altered:
...
- (void)executeFinalTermToken:(VT100Token *)token {
  ...
  case 'A':
    ...
    self.softAlternateScreenMode =
        NO; // We can reasonably assume alternate screen mode has ended if
            // there's a prompt. Could be ssh dying, etc.
  ...
  case 'C':
    // Sequence marking the end of the command read from the command prompt
    // (FTCS_COMMAND_END)
    if (inCommand_) {
...
- (NSData *)decrpmForMode:(int)mode
  ...
  if (ansi) {
    string =
        [NSString stringWithFormat:@"%c[%d;%d$y", VT100CC_ESC, mode, setting];
  } else {
    string =
        [NSString stringWithFormat:@"%c[?%d;%d$y", VT100CC_ESC, mode, setting];
  }
...
- (void)executeANSIRequestMode:(int)mode {
  ...
  [self.delegate terminalSendReport:[self decrpmForMode:mode
                                                setting:setting
                                                   ansi:YES]];
}
...
- (iTermDECRPMSetting)settingForDECRequestMode:(int)mode {
  ...
  case 20:
    ...
    break;
  case 25:
    return VT100TerminalDECRPMSettingFromBoolean(
        [self.delegate terminalCursorVisible]);
  case 40:
  ...
  case 69:
    return VT100TerminalDECRPMSettingFromBoolean(
        [delegate_ terminalUseColumnScrollRegion]);
  ...
  case 1004:
    return VT100TerminalDECRPMSettingFromBoolean(
        self.reportFocus && [delegate_ terminalFocusReportingAllowed]);

  case 1005:
    return VT100TerminalDECRPMSettingFromBoolean(self.mouseFormat ==
...
- (void)pushUnicodeVersion:(NSString *)label {
  ...
  [_unicodeVersionStack
      addObject:@[ label ?: @"", @([delegate_ terminalUnicodeVersion]) ]];
}
...
- (void)popUnicodeVersion:(NSString *)label {
  ...
  while (_unicodeVersionStack.count > 0) {
    ...
    if ([entry isKindOfClass:[NSNumber class]]) {
      // A restored value might have just a number. New values are always an
      // array.
      value = entry;
    } else {
...
- (NSDictionary *)dictionaryForGraphicRendition:
    (VT100GraphicRendition)graphicRendition {
  return @{
...
- (VT100GraphicRendition)graphicRenditionFromDictionary:(NSDictionary *)dict {
  ...
  graphicRendition.strikethrough =
      [dict[kGraphicRenditionStrikethroughKey] boolValue];
  graphicRendition.underlineStyle =
      [dict[kGraphicRenditionUnderlineStyle] unsignedIntegerValue];
  ...
  graphicRendition.fgColorCode =
      [dict[kGraphicRenditionForegroundColorCodeKey] intValue];
  graphicRendition.fgGreen =
      [dict[kGraphicRenditionForegroundGreenKey] intValue];
  graphicRendition.fgBlue = [dict[kGraphicRenditionForegroundBlueKey] intValue];
  graphicRendition.fgColorMode =
      [dict[kGraphicRenditionForegroundModeKey] intValue];

  graphicRendition.bgColorCode =
      [dict[kGraphicRenditionBackgroundColorCodeKey] intValue];
  graphicRendition.bgGreen =
      [dict[kGraphicRenditionBackgroundGreenKey] intValue];
  graphicRendition.bgBlue = [dict[kGraphicRenditionBackgroundBlueKey] intValue];
  graphicRendition.bgColorMode =
      [dict[kGraphicRenditionBackgroundModeKey] intValue];
...
- (VT100SavedCursor)savedCursorFromDictionary:(NSDictionary *)dict {
  ...
  for (int i = 0;
       i < NUM_CHARSETS && i < [dict[kSavedCursorLineDrawingArrayKey] count];
       i++) {
    NSNumber *n = [dict[kSavedCursorLineDrawingArrayKey] objectAtIndex:i];
    savedCursor.lineDrawing[i] = [n boolValue];
  }
  savedCursor.graphicRendition = [self
      graphicRenditionFromDictionary:dict[kSavedCursorGraphicRenditionKey]];
...
- (NSDictionary *)stateDictionary {
  NSDictionary *dict = @{
    ...
    kTerminalStateGraphicRenditionKey :
        [self dictionaryForGraphicRendition:graphicRendition_],
    kTerminalStateMainSavedCursorKey :
        [self dictionaryForSavedCursor:mainSavedCursor_],
    kTerminalStateAltSavedCursorKey :
        [self dictionaryForSavedCursor:altSavedCursor_],
...
- (void)setStateFromDictionary:(NSDictionary *)dict {
  ...
  self.canonicalEncoding =
      [dict[kTerminalStateCanonicalEncodingKey] unsignedIntegerValue];
  ...
  self.reverseWraparoundMode =
      [dict[kTerminalStateReverseWraparoundModeKey] boolValue];
  ...
  _previousMouseMode = [dict[kTerminalStatePreviousMouseModeKey]
                            ?: @(MOUSE_REPORTING_NORMAL) intValue];
  ...
  if ([dict[kTerminalStateSendModifiers] isKindOfClass:[NSArray class]]) {
    self.sendModifiers =
        [[dict[kTerminalStateSendModifiers] mutableCopy] autorelease];
  }
  if (!_sendModifiers) {
    self.sendModifiers =
        [[@[ @-1, @-1, @-1, @-1, @-1 ] mutableCopy] autorelease];
  } else {
  ...
  self.allowPasteBracketing =
      [dict[kTerminalStateAllowPasteBracketing] boolValue];
  self.url = [dict[kTerminalStateURL] nilIfNull];
  self.urlParams = [dict[kTerminalStateURLParams] nilIfNull];

  self.bracketedPasteMode =
      [dict[kTerminalStateBracketedPasteModeKey] boolValue];
  ansiMode_ = [dict[kTerminalStateAnsiModeKey] boolValue];
  numLock_ = [dict[kTerminalStateNumLockKey] boolValue];
  graphicRendition_ = [self
      graphicRenditionFromDictionary:dict[kTerminalStateGraphicRenditionKey]];
  mainSavedCursor_ =
      [self savedCursorFromDictionary:dict[kTerminalStateMainSavedCursorKey]];
  altSavedCursor_ =
      [self savedCursorFromDictionary:dict[kTerminalStateAltSavedCursorKey]];
  ...
  self.disableSmcupRmcup =
      [dict[kTerminalStateDisableSMCUPAndRMCUPKey] boolValue];
  _softAlternateScreenMode =
      [dict[kTerminalStateSoftAlternateScreenModeKey] boolValue];
  ...
  if (dict[kTerminalStateUnicodeVersionStack]) {
    [_unicodeVersionStack
        addObjectsFromArray:dict[kTerminalStateUnicodeVersionStack]];
  }
}
...
- (NSString *)sanitizedTitle:(NSString *)unsafeTitle {
  ...
  NSCharacterSet *newlinesCharacterSet =
      [NSCharacterSet characterSetWithCharactersInString:@"\r\n"];
  NSRange newlineRange =
      [unsafeTitle rangeOfCharacterFromSet:newlinesCharacterSet];
