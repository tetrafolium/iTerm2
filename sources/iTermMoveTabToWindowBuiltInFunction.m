...
+ (void)registerBuiltInFunction {
  iTermBuiltInFunction *func = [[iTermBuiltInFunction alloc]
           initWithName:@"move_tab_to_window"
              arguments:@{}
...
+ (void)moveTabWithID:(NSString *)tabID
  ...
  if (!tabID) {
    completion(nil, [NSError errorWithDomain:@"com.iterm2.move-tab-to-window"
                                        code:1
                                    userInfo:@{
                                      NSLocalizedDescriptionKey : @"No such tab"
                                    }]);
    return;
  }
  ...
  if (!term) {
    completion(
        nil, [NSError errorWithDomain:@"com.iterm2.move-tab-to-window"
                                 code:2
                             userInfo:@{
                               NSLocalizedDescriptionKey : @"Tab has no window"
                             }]);
    return;
  }
