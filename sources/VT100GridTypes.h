...
// Use for sorting array of VT100GridCoorRange's in NSValue*s by the start
// coord.
...
NS_INLINE VT100GridAbsWindowedRange VT100GridAbsWindowedRangeClampedToWidth(
    const VT100GridAbsWindowedRange range, const int width) {
  if (width <= 0) {
    return range;
  }
...
NS_INLINE BOOL VT100GridRunEquals(VT100GridRun a, VT100GridRun b) {
  return (a.length == b.length && VT100GridCoordEquals(a.origin, b.origin));
}

NS_INLINE BOOL VT100GridAbsCoordEquals(VT100GridAbsCoord a,
                                       VT100GridAbsCoord b) {
  return a.x == b.x && a.y == b.y;
}
...
NS_INLINE VT100GridWindowedRange VT100GridWindowedRangeMake(
    VT100GridCoordRange range, int windowStart, int windowWidth) {
...
NS_INLINE VT100GridAbsWindowedRange VT100GridAbsWindowedRangeMake(
    VT100GridAbsCoordRange range, int windowStart, int windowWidth) {
...
NS_INLINE VT100GridCoord
VT100GridWindowedRangeStart(VT100GridWindowedRange range) {
...
NS_INLINE VT100GridCoord
VT100GridWindowedRangeEnd(VT100GridWindowedRange range) {
...
NS_INLINE BOOL VT100GridCoordRangeEqualsCoordRange(VT100GridCoordRange a,
                                                   VT100GridCoordRange b) {
  return (VT100GridCoordEquals(a.start, b.start) &&
          VT100GridCoordEquals(a.end, b.end));
}

NS_INLINE BOOL VT100GridWindowsRangeEqualsWindowedRange(
    VT100GridWindowedRange a, VT100GridWindowedRange b) {
  return (VT100GridRangeEqualsRange(a.columnWindow, b.columnWindow) &&
          VT100GridCoordRangeEqualsCoordRange(a.coordRange, b.coordRange));
}

NS_INLINE BOOL VT100GridAbsCoordRangeEquals(VT100GridAbsCoordRange a,
                                            VT100GridAbsCoordRange b) {
  return (VT100GridAbsCoordEquals(a.start, b.start) &&
          VT100GridAbsCoordEquals(a.end, b.end));
}

NS_INLINE BOOL VT100GridAbsWindowedRangeEquals(VT100GridAbsWindowedRange a,
                                               VT100GridAbsWindowedRange b) {
  return (VT100GridAbsCoordRangeEquals(a.coordRange, b.coordRange) &&
          VT100GridRangeEqualsRange(a.columnWindow, b.columnWindow));
}

NS_INLINE BOOL VT100GridAbsWindowsRangeEqualsAbsWindowedRange(
    VT100GridAbsWindowedRange a, VT100GridAbsWindowedRange b) {
  return (VT100GridRangeEqualsRange(a.columnWindow, b.columnWindow) &&
          VT100GridAbsCoordRangeEquals(a.coordRange, b.coordRange));
}

NSString *VT100GridAbsWindowedRangeDescription(VT100GridAbsWindowedRange range);

NS_INLINE VT100GridAbsCoord
VT100GridAbsWindowedRangeStart(VT100GridAbsWindowedRange range) {
...
NS_INLINE VT100GridAbsCoord
VT100GridAbsWindowedRangeEnd(VT100GridAbsWindowedRange range) {
...
// Descending: a > b
// Same: a == b
NS_INLINE NSComparisonResult VT100GridCoordOrder(VT100GridCoord a,
                                                 VT100GridCoord b) {
  if (a.y < b.y) {
...
NS_INLINE NSComparisonResult VT100GridAbsCoordOrder(VT100GridAbsCoord a,
                                                    VT100GridAbsCoord b) {
  if (a.y < b.y) {
    return NSOrderedAscending;
  }
...
NS_INLINE VT100GridCoordRange VT100GridCoordRangeMake(int startX, int startY,
                                                      int endX, int endY) {
...
NS_INLINE VT100GridWindowedRange VT100GridWindowedRangeFromAbsWindowedRange(
    VT100GridAbsWindowedRange absrange, long long offset) {
  return VT100GridWindowedRangeMake(
      ...
      absrange.columnWindow.location, absrange.columnWindow.length);
}

NS_INLINE VT100GridAbsCoordRange VT100GridAbsCoordRangeFromCoordRange(
    VT100GridCoordRange range, long long offset) {
  return VT100GridAbsCoordRangeMake(range.start.x, range.start.y + offset,
                                    range.end.x, range.end.y + offset);
}

NS_INLINE VT100GridAbsWindowedRange VT100GridAbsWindowedRangeFromRelative(
    VT100GridWindowedRange range, long long scrollbackOffset) {
  VT100GridAbsWindowedRange windowedRange;
  windowedRange.coordRange = VT100GridAbsCoordRangeMake(
      range.coordRange.start.x, range.coordRange.start.y + scrollbackOffset,
      range.coordRange.end.x, range.coordRange.end.y + scrollbackOffset);
  windowedRange.columnWindow = range.columnWindow;
  return windowedRange;
}

NS_INLINE VT100GridAbsWindowedRange VT100GridAbsWindowedRangeFromWindowedRange(
    VT100GridWindowedRange range, long long offset) {
  return VT100GridAbsWindowedRangeFromRelative(range, offset);
}

NS_INLINE VT100GridWindowedRange VT100GridWindowedRangeClampedToWidth(
    const VT100GridWindowedRange range, const int width) {
  const VT100GridAbsWindowedRange input =
      VT100GridAbsWindowedRangeFromWindowedRange(range, 0);
  const VT100GridAbsWindowedRange output =
      VT100GridAbsWindowedRangeClampedToWidth(input, width);
  return VT100GridWindowedRangeFromAbsWindowedRange(output, 0);
}

NS_INLINE VT100GridWindowedRange
VT100GridWindowedRangeFromVT100GridAbsWindowedRange(
    VT100GridAbsWindowedRange source, long long totalScrollbackOverflow) {
  const long long minY = source.coordRange.start.y;
  const long long maxY = source.coordRange.end.y;
  VT100GridWindowedRange result = VT100GridWindowedRangeMake(
      VT100GridCoordRangeMake(source.coordRange.start.x,
                              MAX(0, (int)(minY - totalScrollbackOverflow)),
                              source.coordRange.end.x,
                              MAX(0, (int)(maxY - totalScrollbackOverflow))),
      source.columnWindow.location, source.columnWindow.length);
  return result;
}

NS_INLINE VT100GridCoordRange VT100GridCoordRangeFromAbsCoordRange(
    VT100GridAbsCoordRange absRange, long long totalOverflow) {
  ...
  return VT100GridCoordRangeMake(absRange.start.x, (int)startY, absRange.end.x,
                                 (int)endY);
}

NS_INLINE VT100GridAbsCoord VT100GridAbsCoordFromCoord(VT100GridCoord coord,
                                                       long long overflow) {
...
NS_INLINE BOOL VT100GridAbsCoordRangeTryMakeRelative(
    VT100GridAbsCoordRange range, long long overflow,
...
NS_INLINE NSString *VT100GridRunDescription(VT100GridRun run) {
  return [NSString stringWithFormat:@"[origin=%@ length=%@]",
                                    VT100GridCoordDescription(run.origin),
                                    @(run.length)];
}

NS_INLINE NSString *VT100GridRangeDescription(VT100GridRange r) {
  return [NSString
      stringWithFormat:@"[%d, %d)", r.location, r.location + r.length];
}
...
NS_INLINE VT100GridAbsCoord
VT100GridAbsCoordRangeMin(VT100GridAbsCoordRange range) {
...
NS_INLINE VT100GridAbsCoord
VT100GridAbsCoordRangeMax(VT100GridAbsCoordRange range) {
  if (VT100GridAbsCoordOrder(range.start, range.end) == NSOrderedAscending) {
    return range.end;
  } else {
    return range.start;
...
NS_INLINE BOOL VT100GridCoordRangeContainsCoord(VT100GridCoordRange range,
                                                VT100GridCoord coord) {
  NSComparisonResult order =
      VT100GridCoordOrder(VT100GridCoordRangeMin(range), coord);
...
NS_INLINE long long VT100GridCoordDistance(VT100GridCoord a, VT100GridCoord b,
                                           int gridWidth) {
  long long aPos = a.y;
  aPos *= gridWidth;
  aPos += a.x;
...
NS_INLINE long long VT100GridAbsCoordDistance(VT100GridAbsCoord a,
                                              VT100GridAbsCoord b,
                                              int gridWidth) {
  long long aPos = a.y;
  aPos *= gridWidth;
...
NS_INLINE long long
VT100GridAbsWindowedRangeLength(VT100GridAbsWindowedRange range,
                                int gridWidth) {
  if (range.coordRange.start.y == range.coordRange.end.y) {
    return VT100GridAbsWindowedRangeEnd(range).x -
           VT100GridAbsWindowedRangeStart(range).x;
  } else {
    ...
    const long long numFullLines =
        MAX(0, (range.coordRange.end.y - range.coordRange.start.y - 1));
    return (
        (right -
         VT100GridAbsWindowedRangeStart(range).x) +      // Chars on first line
        (VT100GridAbsWindowedRangeEnd(range).x - left) + // Chars on second line
        (right - left) * numFullLines);                  // Chars between
  }
}

NS_INLINE long long VT100GridWindowedRangeLength(VT100GridWindowedRange range,
                                                 int gridWidth) {
  return VT100GridAbsWindowedRangeLength(
      VT100GridAbsWindowedRangeFromWindowedRange(range, 0), gridWidth);
}

NS_INLINE long long VT100GridCoordRangeLength(VT100GridCoordRange range,
                                              int gridWidth) {
  return VT100GridCoordDistance(range.start, range.end, gridWidth);
}

NS_INLINE long long VT100GridAbsCoordRangeLength(VT100GridAbsCoordRange range,
                                                 int gridWidth) {
...
// Returns the coord of the bottom-right cell that is in the rect. The rect must
// not be 0-dimensioned.
...
NS_INLINE BOOL VT100GridCoordInRect(VT100GridCoord coord, VT100GridRect rect) {
  return (coord.x >= rect.origin.x && coord.y >= rect.origin.y &&
...
VT100GridRun VT100GridRunFromCoords(VT100GridCoord start, VT100GridCoord end,
                                    int width);
...
NS_INLINE BOOL VT100GridCoordFromDictionary(NSDictionary *dict,
                                            VT100GridCoord *coord) {
  if (!dict) {
    return NO;
  }
