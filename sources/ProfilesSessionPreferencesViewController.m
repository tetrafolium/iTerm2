...
@interface ProfilesSessionPreferencesViewController () <
    NSTableViewDelegate, NSTableViewDataSource, PSMMinimalTabStyleDelegate>
@end
...
- (void)awakeFromNib {
  ...
  [self defineControl:_alwaysWarn
      ...
      update:^BOOL {
        [self updatePromptBeforeClosing];
        return YES;
      }
      searchable:NO];
  ...
  [self addViewToSearchIndex:_warnContainer
                 displayName:@"Prompt before closing profile"
                     phrases:@[
                       @"Always prompt before closing profile",
  ...
  info.customSettingChangedHandler = ^(id sender) {
    ...
    if (isOn) {
      static NSString *const kWarnAboutSendCodeWhenIdle =
          @"NoSyncWarnAboutSendCodeWhenIdle";
      // This stupid feature was inherited from iTerm 0.1. It doesn't work
      // because people set a code of 0, thinking it will keep their ssh
      // sessions alive. While it does, it will also fill your prompt with ^@
      // characters, if you're lucky. If you're not at your prompt it could do
      // basically anything. It's useful for people working with awful outdated
      // networking equipment who know what they're doing so I'm not killing it.
      // If you came here because you want to keep your ssh sessions alive, look
      // into enabling KeepAlive on your ssh client. Put this in your
      // ~/.ssh/config: Host *
...
- (void)onEndSettingDidChange {
  [self setUnsignedInteger:_onEndAction.selectedTag
...
- (void)layoutSubviewsForEditCurrentSessionMode {
  NSArray *viewsToDisable = @[ _autoLog, _logDir, _changeLogDir ];
...
- (BOOL)allowRainbow {
  // I was going to make this an easter egg but it was revealed by the Whats New
  // screenshot.
  return YES;
}
...
- (NSColor *)sessionBackgroundColor {
  NSDictionary *dict =
      [NSDictionary castFrom:[self objectForKey:KEY_BACKGROUND_COLOR]];
  if (!dict) {
...
- (NSColor *)tabColor {
  ...
  NSDictionary *dict =
      [NSDictionary castFrom:[self objectForKey:KEY_TAB_COLOR]];
  if (!dict) {
    return nil;
  }
  return [dict colorValue];
...
- (IBAction)configureStatusBar:(id)sender {
  NSDictionary *layoutDictionary =
      [NSDictionary castFrom:[self objectForKey:KEY_STATUS_BAR_LAYOUT]] ?: @{};
  NSDictionary *colorDict =
      [NSDictionary castFrom:[self objectForKey:KEY_BACKGROUND_COLOR]];
  const BOOL dark = [[colorDict colorValue] perceivedBrightness] < 0.5;
  _statusBarSetupViewController = [[iTermStatusBarSetupViewController alloc]
      initWithLayoutDictionary:layoutDictionary
                darkBackground:
                    [NSAppearance
                        it_decorationsAreDarkWithTerminalBackgroundColorIsDark:
                            dark]
                  allowRainbow:[self allowRainbow]];
  _statusBarSetupViewController.defaultTextColor = [[iTermTheme sharedInstance]
      statusBarTextColorForEffectiveAppearance:[self appearanceForCurrentTheme]
                                      colorMap:[self colorMap]
                                      tabStyle:[self tabStyle]
                                 mainAndActive:YES];
  _statusBarSetupViewController
      .defaultBackgroundColor = [[iTermTheme sharedInstance]
      statusBarContainerBackgroundColorForTabColor:[self tabColor]
                               effectiveAppearance:
                                   [self appearanceForCurrentTheme]
                                          tabStyle:[self tabStyle]
  ...
  _statusBarSetupWindow = [[iTermStatusBarSetupPanel alloc]
      initWithContentRect:_statusBarSetupViewController.view.frame
                styleMask:NSWindowStyleMaskResizable
                  backing:NSBackingStoreBuffered
                    defer:NO];
  _statusBarSetupWindow.minSize = _statusBarSetupViewController.view.frame.size;
  NSDictionary *savedLayoutDictionary =
      [_statusBarSetupViewController.layoutDictionary copy];
  _statusBarSetupWindow.contentView = _statusBarSetupViewController.view;
  [self.view.window beginSheet:_statusBarSetupWindow
             completionHandler:^(NSModalResponse returnCode) {
               __strong __typeof(weakSelf) strongSelf = weakSelf;
               if (strongSelf) {
                 if (!strongSelf->_statusBarSetupViewController.ok) {
                   [strongSelf setObject:savedLayoutDictionary
...
- (IBAction)addJob:(id)sender {
  ...
  [_jobsTable reloadData];
  [_jobsTable
          selectRowIndexes:[NSIndexSet indexSetWithIndex:[augmented count] - 1]
...
- (id)tableView:(NSTableView *)aTableView
  ...
  if (rowIndex >= jobs.count) {
    // Can happen during teardown when the ProfilePreferencesViewController's
    // delegate is nilled.
    return @"";
  }
...
- (void)updateLogDirWarning {
  ...
  if ([self logDirIsWritable]) {
    _logDirWarning.image = [NSImage it_imageNamed:@"CheckMark"
                                         forClass:self.class];
  } else {
    _logDirWarning.image = [NSImage it_imageNamed:@"WarningSign"
                                         forClass:self.class];
  }
}
...
- (BOOL)logDirIsWritable {
  return [[NSFileManager defaultManager]
      directoryIsWritable:[_logDir stringValue]];
}
