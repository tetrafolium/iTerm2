...
static NSString *const kSavedArrangementWillChangeNotification =
    @"kSavedArrangementWillChangeNotification";
...
- (void)updateActionsEnabled {
  [deleteButton_ setEnabled:([WindowArrangements count] > 0) &&
                            ([tableView_ numberOfSelectedRows] > 0)];
  [defaultButton_ setEnabled:([WindowArrangements count] > 0) &&
                             ([tableView_ numberOfSelectedRows] == 1)];
}
...
+ (BOOL)hasWindowArrangement:(NSString *)name {
  NSDictionary *arrangements =
      [[NSUserDefaults standardUserDefaults] objectForKey:WINDOW_ARRANGEMENTS];
...
+ (void)postChangeNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kSavedArrangementDidChangeNotification
                    object:nil
                  userInfo:nil];
}
...
+ (void)setArrangement:(NSArray *)arrangement withName:(NSString *)name {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kSavedArrangementWillChangeNotification
                    object:nil
                  userInfo:nil];

  NSMutableDictionary *arrangements = [NSMutableDictionary
      dictionaryWithDictionary:[WindowArrangements arrangements]];
  [arrangements setObject:arrangement forKey:name];
  @try {
    [[NSUserDefaults standardUserDefaults] setObject:arrangements
                                              forKey:WINDOW_ARRANGEMENTS];
  } @catch (NSException *e) {
    NSString *oops = [arrangements it_invalidPathInPlist];
    ITCriticalError(
        NO, @"Exception %@ while saving arrangement. Invalid path is %@\n%@", e,
...
- (void)updatePreviewView {
  ...
  if (selection.count == 1) {
    ...
    [previewView_
        setArrangement:[WindowArrangements
                           arrangementWithName:[self nameAtIndex:rowIndex]]];
...
+ (void)makeDefaultArrangement:(NSString *)name {
  [[NSUserDefaults standardUserDefaults] setObject:name
                                            forKey:DEFAULT_ARRANGEMENT_KEY];
  [WindowArrangements postChangeNotification];
}

+ (NSString *)defaultArrangementName {
  return [[NSUserDefaults standardUserDefaults]
      objectForKey:DEFAULT_ARRANGEMENT_KEY];
}

- (NSArray *)defaultArrangement {
  return [[WindowArrangements arrangements]
      objectForKey:[WindowArrangements defaultArrangementName]];
}
...
+ (void)refreshRestoreArrangementsMenu:(NSMenuItem *)menuItem
  ...
  for (NSString *theName in [self allNames]) {
    ...
    NSMenuItem *individualItem =
        [[menuItem submenu] addItemWithTitle:theName
                                      action:selector
                               keyEquivalent:theShortcut];
    individualItem.identifier =
        [NSString stringWithFormat:@"%@:%@", theName, identifier];
  }
}

+ (NSString *)showAlertWithText:(NSString *)prompt
...
+ (NSString *)nameForNewArrangement {
  NSString *name = [self
      showAlertWithText:@"Name for saved window arrangement:"
           defaultInput:[NSString
                            stringWithFormat:@"Arrangement %d",
                                             1 + [WindowArrangements count]]];
  if (!name) {
    return nil;
  }
  if ([WindowArrangements hasWindowArrangement:name]) {
    ...
    alert.informativeText =
        @"There is an existing saved window arrangement with this name. Would "
        @"you like to replace it with the current arrangement?";
...
- (void)controlTextDidEndEditing:(NSNotification *)obj {
  ...
  const BOOL wasDefault =
      [oldName isEqualToString:[WindowArrangements defaultArrangementName]];
  NSMutableDictionary *dict = [[[NSUserDefaults standardUserDefaults]
      objectForKey:WINDOW_ARRANGEMENTS] mutableCopy];
  NSDictionary *value = [dict[oldName] copy];
  if (dict[newName]) {
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Replace Arrangement?";
    alert.informativeText =
        [NSString stringWithFormat:@"An arrangement named “%@” already exists. "
                                   @"Would you like to replace it?",
                                   newName];
    [alert addButtonWithTitle:@"OK"];
    [alert addButtonWithTitle:@"Cancel"];
    if ([alert runSheetModalForWindow:self.view.window] ==
  ...
  [[NSUserDefaults standardUserDefaults] setObject:dict
                                            forKey:WINDOW_ARRANGEMENTS];
  if (wasDefault) {
    [[NSUserDefaults standardUserDefaults] setObject:newName
                                              forKey:DEFAULT_ARRANGEMENT_KEY];
  }
  [tableView_ reloadData];
  [WindowArrangements postChangeNotification];
...
- (void)setDefaultIfNeededAtRow:(int)rowid {
  if ([WindowArrangements count] > 0) {
    if ([WindowArrangements
            arrangementWithName:[WindowArrangements defaultArrangementName]] ==
        nil) {
      int newDefault = rowid;
      if (newDefault >= [WindowArrangements count]) {
        newDefault = [WindowArrangements count] - 1;
      }
      [WindowArrangements makeDefaultArrangement:[[WindowArrangements allNames]
                                                     objectAtIndex:newDefault]];
    }
  }
}
...
- (void)setArrangements:(NSDictionary *)saved {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:arrangements
                                            forKey:WINDOW_ARRANGEMENTS];
  [[NSUserDefaults standardUserDefaults] setObject:defaultArrangementName
                                            forKey:DEFAULT_ARRANGEMENT_KEY];
  [WindowArrangements postChangeNotification];
}
  ...
  [[self undoManager]
      registerUndoWithTarget:self
                    selector:@selector(setArrangements:)
                      object:@{
                        @"default" : [WindowArrangements defaultArrangementName]
                            ?: [NSNull null],
                        @"arrangements" : [[NSUserDefaults standardUserDefaults]
                            objectForKey:WINDOW_ARRANGEMENTS]
                            ?: @{}
                      }];
}
