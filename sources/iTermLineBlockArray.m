...
- (id)copyWithZone:(NSZone *)zone {
  iTermLineBlockCacheCollection *theCopy =
      [[iTermLineBlockCacheCollection alloc] init];
  theCopy.capacity = self.capacity;
  for (iTermTuple<NSNumber *, iTermCumulativeSumCache *> *tuple in _caches) {
    [theCopy->_caches
        addObject:[iTermTuple tupleWithObject:tuple.firstObject
                                    andObject:tuple.secondObject.copy]];
  }
  return theCopy;
}
...
- (iTermCumulativeSumCache *)numLinesCacheForWidth:(int)width {
  NSInteger index = [_caches
      indexOfObjectPassingTest:^BOOL(
          iTermTuple<NSNumber *, iTermCumulativeSumCache *> *_Nonnull tuple,
...
- (void)setNumLinesCache:(iTermCumulativeSumCache *)numLinesCache
  ...
  assert(!existing);
  [_caches insertObject:[iTermTuple tupleWithObject:@(width)
                                          andObject:numLinesCache]
                atIndex:0];
  [self evictIfNeeded];
}
...
- (void)buildNumLinesCacheForWidth:(int)width {
  assert(width > 0);
  iTermCumulativeSumCache *numLinesCache =
      [_numLinesCaches numLinesCacheForWidth:width];
  if (numLinesCache) {
    return;
  }
...
- (NSInteger)indexOfBlockContainingLineNumber:(int)lineNumber
                                        width:(int)width
  ...
  __block int r = 0;
  const NSInteger result =
      [self internalIndexOfBlockContainingLineNumber:lineNumber
                                               width:width
                                           remainder:&r];
  if (remainderPtr) {
    *remainderPtr = r;
...
- (void)enumerateLinesInRange:(NSRange)range
                        width:(int)width
                        block:(void (^)(screen_char_t *_Nonnull, int, int,
                                        screen_char_t,
                                        BOOL *_Nonnull))callback {
  int remainder;
  NSInteger startIndex = [self indexOfBlockContainingLineNumber:range.location
                                                          width:width
                                                      remainder:&remainder];
  ITAssertWithMessage(startIndex != NSNotFound, @"Line %@ not found",
                      @(range.location));

  int numberLeft = range.length;
  ITAssertWithMessage(numberLeft >= 0, @"Invalid length in range %@",
  ...
  for (NSInteger i = startIndex; i < _blocks.count; i++) {
    ...
    // Grab lines from this block until we're done or reach the end of the
    // block.
    BOOL stop = NO;
    int line = remainder;
    do {
  ...
  ITAssertWithMessage(
      numberLeft == 0,
      @"not all lines available in range %@. Have %@ remaining.",
      NSStringFromRange(range), @(numberLeft));
}
...
- (LineBlock *)blockContainingPosition:(long long)position
  ...
  if (width > 0 && _rawSpaceCache) {
    int r = 0, y = 0, i = 0;
    LineBlock *result =
        [self fast_blockContainingPosition:position
                                     width:width
                                 remainder:&r
                               blockOffset:yoffsetPtr ? &y : NULL
                                     index:&i];
    if (remainderPtr) {
      *remainderPtr = r;
    }
...
- (LineBlock *)fast_blockContainingPosition:(long long)position
  ...
  if (remainderPtr) {
    *remainderPtr =
        position - [_rawSpaceCache sumOfValuesInRange:NSMakeRange(0, index)];
  }
  if (yoffsetPtr) {
    *yoffsetPtr = [[_numLinesCaches numLinesCacheForWidth:width]
        sumOfValuesInRange:NSMakeRange(0, index)];
  }
  if (indexPtr) {
    *indexPtr = index;
  }
...
- (void)addBlock:(LineBlock *)block {
  ...
  if (_rawSpaceCache) {
    ...
    // The block might not be empty. Treat it like a bunch of lines just got
    // appended.
...
- (void)updateCacheForBlock:(LineBlock *)block {
  ...
  if (block == _blocks.firstObject) {
    ...
    ITAssertWithMessage(block == _blocks.firstObject ||
                            block == _blocks.lastObject,
                        @"Block with index %@/%@ changed",
                        @([_blocks indexOfObject:block]), @(_blocks.count));
  }
}
