...
- (BOOL)shouldPrintWithProfile:(Profile *)profile inWindow:(NSWindow *)window {
  ...
  const BOOL okByProfile =
      ![[profile objectForKey:KEY_DISABLE_PRINTING] boolValue];
  if (!okByProfile) {
    return NO;
  }
  if ([self haveTriedToPrintRecently]) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            @"There's a lot of printing going on. Want to keep allowing it?"
                     actions:@[
                       @"Allow", @"Disable Temporarily", @"Disable Permanently"
                     ]
                   accessory:nil
    ...
    case kiTermWarningSelection1: {
      ...
      dispatch_after(
          dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * 60 * NSEC_PER_SEC)),
    ...
    case kiTermWarningSelection2: {
      ...
      [iTermProfilePreferences setBool:YES
                                forKey:KEY_DISABLE_PRINTING
                             inProfile:profile
