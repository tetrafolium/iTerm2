...
static NSString *const iTermSelectedActiveForegroundColor =
    @"iTermSelectedActiveForegroundColor";
static NSString *const iTermRegularForegroundColor =
    @"iTermRegularForegroundColor";
...
// This is a text field that updates its text colors depending on the current
// background style.
...
- (void)setAttributedTextColorsForKey:(NSString *)key {
  NSMutableAttributedString *m =
      [self.attributedStringValue.mutableCopy autorelease];
  [self.attributedStringValue
      enumerateAttributesInRange:NSMakeRange(
                                     0,
                                     self.attributedStringValue.string.length)
                         options:0
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id>
                                       *_Nonnull attrs,
                                   NSRange range, BOOL *_Nonnull stop) {
                        NSMutableDictionary *newAttrs =
                            [[attrs mutableCopy] autorelease];
...
NSString *const kProfileWasDeletedNotification =
    @"kProfileWasDeletedNotification";
...
- (instancetype)initWithFrame:(NSRect)frameRect
  ...
  if (self) {
    ...
    [self _addTags:[[dataSource_ underlyingModel] allTags]
        toSearchField:searchField_];
    ...
    splitView_ =
        [[[NSSplitView alloc] initWithFrame:splitViewFrame] autorelease];
    ...
    tableViewFrame.size = [NSScrollView
        contentSizeForFrameSize:scrollViewFrame.size
    ...
    [tableView_
        registerForDraggedTypes:[NSArray
                                    arrayWithObject:kProfileTableViewDataType]];
    [tableView_
    ...
    // Tags view
    // -------------------------------------------------------------------------------
    NSRect tagsViewFrame =
        NSMakeRect(0, 0, kTagsViewWidth, splitViewFrame.size.height);
    lastTagsWidth_ = kDefaultTagsWidth;
    tagsViewIsCollapsed_ = (tagsView_.frame.size.width == 0);
    tagsView_ =
        [[[ProfileTagsView alloc] initWithFrame:tagsViewFrame] autorelease];
...
- (BOOL)tableView:(NSTableView *)tv
  ...
  NSData *data = [NSKeyedArchiver archivedDataWithRootObject:guids
                                       requiringSecureCoding:NO
                                                       error:nil];
  [pboard declareTypes:[NSArray arrayWithObject:kProfileTableViewDataType]
                 owner:self];
...
- (BOOL)tableView:(NSTableView *)aTableView
  ...
  NSSet<NSString *> *guids =
      [unarchiver decodeObjectOfClass:[NSSet class]
  ...
  for (NSString *guid in guids) {
    [map setObject:guid
            forKey:[NSNumber numberWithInt:[dataSource_
                                               indexOfProfileWithGuid:guid]]];
  }
...
- (void)selectGuids:(NSArray *)guids {
  NSMutableIndexSet *newIndexes =
      [[[NSMutableIndexSet alloc] init] autorelease];
  for (NSString *guid in guids) {
...
- (void)_addTag:(id)sender {
  ...
  NSString *trimmedSearchString = [[searchField_ stringValue]
      stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
  NSString *searchStringPlusTag =
      [NSString stringWithFormat:@"%@ tag:%@", trimmedSearchString, tag];
  [searchField_
      setStringValue:[searchStringPlusTag
...
- (void)_addTags:(NSArray *)tags toSearchField:(NSSearchField *)searchField {
  NSMenu *cellMenu =
      [[[NSMenu alloc] initWithTitle:@"Search Menu"] autorelease];
  NSMenuItem *item;
  ...
  [cellMenu insertItem:[NSMenuItem separatorItem]
               atIndex:cellMenu.numberOfItems];
  [cellMenu addItemWithTitle:@"Search Syntax Help"
                      action:@selector(openHowToSearchHelp:)
               keyEquivalent:@""];
...
- (void)openHowToSearchHelp:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"https://iterm2.com/search_syntax.html"]];
}
...
- (void)selectLockedSelection {
  NSInteger theIndex =
      [dataSource_ indexOfProfileWithGuid:dataSource_.lockedGuid];
  if (theIndex < 0) {
    return;
  }
  [tableView_ selectRowIndexes:[NSIndexSet indexSetWithIndex:theIndex]
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent {
  if ([self.delegate respondsToSelector:@selector(profileTable:
                                                  menuForEvent:)]) {
...
- (void)tableView:(NSTableView *)aTableView
  ...
  if ([sortDescriptors count] > 0) {
    ...
    [aTableView
        setIndicatorImage:([primarySortDesc ascending]
                               ? [NSImage
                                     imageNamed:@"NSAscendingSortIndicator"]
                               : [NSImage
                                     imageNamed:@"NSDescendingSortIndicator"])
            inTableColumn:[aTableView
                              tableColumnWithIdentifier:[primarySortDesc key]]];
  }
...
- (CGFloat)heightOfRowWithTags:(BOOL)hasTags {
  if (!_haveHeights) {
    _heightWithTags = [[self attributedStringForName:@"Mj"
                      ...
                      [self extraHeightWithTags:YES];
    _heightWithoutTags =
        [[self attributedStringForName:@"Mj"
                                  tags:nil
                              selected:NO
                             isDefault:YES
                                filter:nil] heightForWidth:100] +
        [self extraHeightWithTags:NO];
    _haveHeights = YES;
  }
...
- (NSAttributedString *)attributedStringForName:(NSString *)name
  ...
  NSColor *highlightedBackgroundColor = [NSColor colorWithCalibratedRed:1
                                                                  green:1
                                                                   blue:0
                                                                  alpha:0.4];
  ...
  NSDictionary *smallAttributes = @{
    iTermSelectedActiveForegroundColor : selectedActiveTagColor,
  ...
  NSMutableAttributedString *theAttributedString =
      [[[ProfileModel attributedStringForName:name
                 highlightingMatchesForFilter:filter
                        ...
                        highlightedAttributes:highlightedNameAttributes]
          mutableCopy] autorelease];

  if (isDefault) {
    NSAttributedString *star = [[[NSAttributedString alloc]
        initWithString:@"★ "
  ...
  if (tags.count) {
    ...
    NSArray *attributedTags =
        [ProfileModel attributedTagsForTags:tags
               highlightingMatchesForFilter:filter
                          defaultAttributes:smallAttributes
                      highlightedAttributes:highlightedSmallAttributes];
    NSAttributedString *comma = [[[NSAttributedString alloc]
        initWithString:@", "
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  NSTableCellView *result = [tableView makeViewWithIdentifier:identifier
                                                        owner:self];
  if (result == nil) {
    ...
    result = [[[NSTableCellView alloc] initWithFrame:NSMakeRect(0, 0, 100, 100)]
        autorelease];
    result.textField = [iTermProfileListViewTextField
        it_textFieldForTableViewWithIdentifier:identifier];
    result.textField.frame = result.bounds;
    result.textField.autoresizingMask =
        (NSViewWidthSizable | NSViewHeightSizable);
    result.autoresizesSubviews = YES;
    [result addSubview:result.textField];
  }
  BOOL multiline = NO;
  id value = [self stringOrAttributedStringForColumn:tableColumn
                                                 row:row
                                           multiline:&multiline];
...
- (id)stringOrAttributedStringForColumn:(NSTableColumn *)aTableColumn
  ...
  *multilinePtr = 0;
  if (aTableColumn == tableColumn_) {
    DLog(@"Getting name of profile at row %d. The dictionary's address is %p. "
         @"Its name is %@",
    ...
    *multilinePtr = [bookmark[KEY_TAGS] count] > 0;
    return [self
        attributedStringForName:bookmark[KEY_NAME] ?: @""
                           tags:bookmark[KEY_TAGS]
                       selected:[[tableView_ selectedRowIndexes]
                                    containsIndex:rowIndex]
                      isDefault:[bookmark[KEY_GUID]
                                    isEqualToString:defaultProfile[KEY_GUID]]
    ...
    if ([customCommand
      ...
      theString = [bookmark objectForKey:KEY_COMMAND_LINE];
                   ...
                   isEqualToString:
                       kProfilePreferenceCommandTypeLoginShellValue]) {
    ...
    if ([key length]) {
      NSString *theString = [NSString
          stringWithFormat:@"^⌘%@", [bookmark objectForKey:KEY_SHORTCUT]];
      return [self attributedStringForString:theString
                                    selected:[[tableView_ selectedRowIndexes]
                                                 containsIndex:rowIndex]];
    } else {
...
- (void)tableView:(NSTableView *)aTableView
  ...
  NSMutableArray *newSortDescriptors =
      [NSMutableArray arrayWithArray:[tableView_ sortDescriptors]];
  BOOL done = NO;
  BOOL ascending = YES;
  ...
  for (int i = 0; i < [newSortDescriptors count]; ++i) {
    ...
    if ([[desc key] isEqualToString:[aTableColumn identifier]]) {
      ...
      [newSortDescriptors insertObject:[[[NSSortDescriptor alloc]
...
- (BOOL)selectionShouldChangeInTableView:(NSTableView *)aTableView {
  if (self.delegate && [self.delegate respondsToSelector:@selector
                                      (profileTableSelectionWillChange:)]) {
...
- (void)tableViewSelectionIsChanging:(NSNotification *)aNotification {
  ...
  if (self.delegate && [self.delegate respondsToSelector:@selector
                                      (profileTableSelectionDidChange:)]) {
...
- (void)reloadData {
  ...
  NSSet *newSelectedGuids = [NSSet
      setWithArray:[selectedGuids_.allObjects filteredArrayUsingBlock:^BOOL(
                                                  id guid) {
  ...
  if (self.delegate && [selectedGuids_ isEqualToSet:newSelectedGuids]) {
    ...
    if (self.delegate && ![selectedGuids_ isEqualToSet:newSelectedGuids]) {
      ...
      [selectedGuids_ retain];
      if ([self.delegate
              respondsToSelector:@selector(profileTableSelectionDidChange:)]) {
        [self.delegate profileTableSelectionDidChange:self];
      }
    }
...
- (void)updateResultsForSearch {
  ...
  if ([self.delegate
          respondsToSelector:@selector(profileTableFilterDidChange:)]) {
...
- (void)dataChangeNotification:(id)sender {
  ...
  // Use a delayed perform so the underlying model has a chance to parse its
  // journal.
...
- (void)turnOnDebug {
  NSLog(@"Debugging object at %p. Current count is %d", (void *)self,
...
- (void)restoreFromState:(NSDictionary *)state {
  ...
  const CGFloat fraction =
      [NSNumber castFrom:state[iTermProfileListViewRestorableStateTagsFraction]]
          .doubleValue;
  if ([state[iTermProfileListViewRestorableStateTagsVisible] boolValue] &&
...
- (void)splitViewDidResizeSubviews:(NSNotification *)notification {
  if ((tagsView_.frame.size.width == 0) != tagsViewIsCollapsed_ &&
      [self.delegate
          respondsToSelector:@selector(profileTableTagsVisibilityDidChange:)]) {
