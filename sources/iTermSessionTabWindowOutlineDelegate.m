...
@interface iTermSessionTabWindowOutlineDelegate () <NSOutlineViewDataSource,
...
- (NSString *)displayName {
  return
      [NSString stringWithFormat:@"＞ %@ (%@)", _session.name, _session.guid];
}
...
- (NSString *)displayName {
  return [NSString stringWithFormat:@"🗂 %@ (%@)", _tab.tabViewItem.label,
                                    @(_tab.uniqueId)];
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _children = [[[iTermBuriedSessions sharedInstance] buriedSessions]
        mapWithBlock:^id(PTYSession *session) {
          return [[iTermOutlineSessionProxy alloc] initWithSession:session];
        }];
  }
  return self;
...
- (NSString *)displayName {
  return
      [NSString stringWithFormat:@"🔲 %@ (%@)", _windowController.window.title,
                                 _windowController.terminalGuid];
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _children = [[[iTermController sharedInstance] terminals]
        mapWithBlock:^id(PseudoTerminal *windowController) {
          return [[iTermOutlineWindowProxy alloc]
              initWithWindowController:windowController];
        }];
    NSArray<PTYSession *> *sessions =
        [[iTermBuriedSessions sharedInstance] buriedSessions];
    if (sessions.count) {
      _children = [_children
          arrayByAddingObject:[[iTermOutlineBuriedSessionsProxy alloc] init]];
    }
  }
  return self;
}
...
// View Based OutlineView: See the delegate method
// -tableView:viewForTableColumn:row: in NSTableView.
...
- (void)showVariablesForProxy:(id<iTermOutlineProxy>)proxy {
  ...
  if (scope) {
    NSString *path = [_variablesOutlineDelegate
        selectedPathForOutlineView:_variablesOutlineView];
    _variablesOutlineDelegate =
        [[iTermVariablesOutlineDelegate alloc] initWithScope:scope];
    ...
    if (path) {
      [_variablesOutlineDelegate selectPath:path
                              inOutlineView:_variablesOutlineView];
    }
  }
}
...
- (void)selectObjectEquivalentTo:(id<iTermOutlineProxy>)selectedObject {
  ...
  _variablesOutlineDelegate =
      [[iTermVariablesOutlineDelegate alloc] initWithScope:nil];
  _variablesOutlineView.delegate = _variablesOutlineDelegate;
  _variablesOutlineView.dataSource = _variablesOutlineDelegate;
  [_variablesOutlineView reloadData];
}
...
- (void)reload {
  id<iTermOutlineProxy> selectedObject = [_sessionTabWindowOutlineView
      itemAtRow:[_sessionTabWindowOutlineView selectedRow]];
  _root = [[iTermOutlineRoot alloc] init];
...
- (IBAction)reveal:(id)sender {
  id<iTermOutlineProxy> proxy = [_sessionTabWindowOutlineView
      itemAtRow:_sessionTabWindowOutlineView.clickedRow];
  [proxy reveal];
}
...
- (IBAction)pickSession:(id)sender {
  ...
  if (session) {
    iTermOutlineSessionProxy *proxy =
        [[iTermOutlineSessionProxy alloc] initWithSession:session];
    [self selectObjectEquivalentTo:proxy];
  }
  NSWindow *window = _sessionTabWindowOutlineView.window;
  // Doesn't work unless you dispatch_async, I guess because modal sessions are
  // a hack.
