...
static NSString *const iTermMetalBufferPoolContextStackKey =
    @"iTermMetalBufferPoolContextStackKey";
...
- (void)addBuffer:(id<MTLBuffer>)buffer pool:(id<iTermMetalBufferPool>)pool {
  ...
  iTermMetalBufferPoolContextEntry *entry =
      [[iTermMetalBufferPoolContextEntry alloc] init];
  entry.buffer = buffer;
  entry.pool = pool;
...
- (void)relinquishOwnershipOfBuffer:(id<MTLBuffer>)buffer {
  [_entries
      removeObjectsPassingTest:^BOOL(iTermMetalBufferPoolContextEntry *entry) {
...
- (id<MTLBuffer>)requestBufferFromContext:(iTermMetalBufferPoolContext *)context
  ...
  @synchronized(self) {
    id<MTLBuffer> buffer;
    id<MTLBuffer> bestMatch =
        [[[_buffers filteredArrayUsingBlock:^BOOL(id<MTLBuffer> buffer) {
          return buffer.length > size;
        }] minimumsWithComparator:^NSComparisonResult(id<MTLBuffer> a,
                                                      id<MTLBuffer> b) {
    ...
    if (bestMatch != nil) {
      ...
      buffer = [_device newBufferWithLength:size
...
- (id<MTLBuffer>)requestBufferFromContext:(iTermMetalBufferPoolContext *)context
  ...
  @synchronized(self) {
    ...
    if (index < _buffers.count) {
      ...
      DLog(@"%@ allocating a new buffer of size %d (%d outstanding)", _name,
           (int)size, (int)_numberOutstanding);
      buffer = [_device newBufferWithBytes:bytes
                                    length:size
                                   options:MTLResourceStorageModeShared];
    }
    [context addBuffer:buffer pool:self];
...
- (id<MTLBuffer>)requestBufferFromContext:
  ...
  @synchronized(self) {
    ...
    if (_buffers.count) {
      ...
      buffer = [_device newBufferWithLength:_bufferSize
                                    options:MTLResourceStorageModeShared];
    }
    [context addBuffer:buffer pool:self];
    return buffer;
...
- (id<MTLBuffer>)requestBufferFromContext:(iTermMetalBufferPoolContext *)context
  ...
  @synchronized(self) {
    ...
    [context addBuffer:buffer pool:self];
    if (!buffer) {
      DLog(@"Failed to allocate buffer of size %@", @(_bufferSize));
    }
    return buffer;
  }
}
