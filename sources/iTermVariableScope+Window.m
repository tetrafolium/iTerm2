...
- (iTermVariableScope<iTermTabScope> *)currentTab {
  return [iTermVariableScope
      newTabScopeWithVariables:
          [self valueForVariableName:iTermVariableKeyWindowCurrentTab]];
}
...
- (NSRect)frame {
  NSArray<NSNumber *> *array = [NSArray
      castFrom:[self valueForVariableName:iTermVariableKeyWindowFrame]];
  if (array.count != 4) {
    return NSZeroRect;
  }
  return NSMakeRect([array[0] doubleValue] ?: 0, [array[1] doubleValue] ?: 0,
                    [array[2] doubleValue] ?: 0, [array[3] doubleValue] ?: 0);
}
