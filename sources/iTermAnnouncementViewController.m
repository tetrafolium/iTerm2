...
- (void)loadView {
  ...
  self.view =
      [iTermAnnouncementView announcementViewWithTitle:self.title
                                                 style:_style
...
- (BOOL)reallyHandleKeyDown:(NSEvent *)event {
  ...
  const NSUInteger i = [self.actions
      indexOfObjectPassingTest:^BOOL(NSString *action, NSUInteger i,
                                     BOOL *_Nonnull stop) {
        ...
        NSString *shortcut =
            [[action substringWithRange:NSMakeRange(NSMaxRange(range), 1)]
                uppercaseString];
        if (![[[event charactersIgnoringModifiers] uppercaseString]
                isEqualToString:shortcut]) {
          return NO;
        }
        return YES;
      }];
...
- (void)setDismissOnKeyDown:(BOOL)dismissOnKeyDown {
  if (!dismissOnKeyDown || _dismissOnKeyDown) {
    // Because of limitations in the view's implementation this can never be
    // unset.
    return;
  }
