...
// A temporary divider used at the bottom of the card while it's growing in
// height.
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  [self.color set];
  NSRectFill(
      NSMakeRect(rect.origin.x, rect.origin.y + 0.5, rect.size.width, 0.5));
}
...
- (void)awakeFromNib {
  ...
  self.layer.borderColor = [[NSColor colorWithCalibratedWhite:0.65
                                                        alpha:1] CGColor];
...
@implementation iTermTipCardViewController {
  ...
  // A view that sits above the buttons and below the body content to hide
  // staged buttons.
...
- (void)viewDidLoad {
  ...
  [dropShadow
      setShadowColor:[[NSColor blackColor] colorWithAlphaComponent:0.3]];
  ...
  _fakeBottomDivider =
      [[iTermTipCardFakeDividerView alloc] initWithFrame:NSZeroRect];
...
- (void)setBodyText:(NSString *)body {
  ...
  NSMutableParagraphStyle *bigTextParagraphStyle =
  ...
  NSDictionary *bigTextAttributes = @{
    ...
    NSForegroundColorAttributeName : [NSColor colorWithCalibratedWhite:0.2
                                                                 alpha:1],
  ...
  NSDictionary *signatureAttributes = @{
    NSFontAttributeName : [NSFont fontWithName:@"Helvetica Neue Light Italic"
                                          size:12]
        ?: [NSFont systemFontOfSize:12],
    NSForegroundColorAttributeName : [NSColor colorWithCalibratedWhite:0.3
                                                                 alpha:1],
...
- (iTermTipCardActionButton *)addActionWithTitle:(NSString *)title
  ...
  iTermTipCardActionButton *button = [[[iTermTipCardActionButton alloc]
      initWithFrame:NSMakeRect(
                        kButtonSideInset, 0,
                        _container.bounds.size.width - kButtonSideInset * 2, 0)]
      autorelease];
  ...
  // Place later buttons under earlier buttons and all under body so they can
  // animate in and out.
  [_container addSubview:button
              positioned:NSWindowBelow
              relativeTo:viewToPlaceButtonBelow];

  return button;
}

- (void)combineActionWithTitle:(NSString *)leftTitle
                      andTitle:(NSString *)rightTitle {
  ...
  frame.size.width =
      _container.frame.size.width - frame.size.width - kContainerSideInset * 2;
  right.frame = frame;
  right.indexInRow = 1;
  ...
  [_actionButtons insertObject:left
                       atIndex:[_actionButtons indexOfObject:right]];
}
...
- (NSRect)performLayoutForWidth:(CGFloat)width
  ...
  bodyFrame.size =
      [_body sizeThatFits:NSMakeSize(containerWidth - kBodySideMargin * 2,
                                     CGFLOAT_MAX)];
  ...
  frame.size.height =
      (kCardTopMargin + kContainerTopBorder + _titleBox.frame.size.height +
       kMarginBetweenTitleAndBody + bodyFrame.size.height +
       kMarginBetweenTextAndButtons + totalButtonHeight +
       kContainerBottomBorder + kCardBottomMargin);
  frame.origin = newOrigin;
  frame = NSIntegralRect(frame);

  // Calculate the container's frame.
  CGFloat containerHeight =
      (1 + _titleBox.frame.size.height + kMarginBetweenTitleAndBody +
       bodyFrame.size.height + kMarginBetweenTextAndButtons +
       totalButtonHeight + 1);
  NSRect containerFrame = NSMakeRect(kContainerSideInset, kCardTopMargin,
  ...
  NSRect coverViewFrame = NSMakeRect(
      1, NSMaxY(titleFrame), containerWidth - 2,
  ...
  CGFloat stageY = NSMaxY(bodyFrame) + totalButtonHeight +
                   kMarginBetweenTextAndButtons - stagedButtonHeight;
  ...
  for (iTermTipCardActionButton *actionButton in _actionButtons) {
    ...
    if (!dry) {
      ...
      CGFloat buttonWidth =
          (containerFrame.size.width - 2 * kContainerSideInset) /
          actionButton.numberOfButtonsInRow;
      buttonFrame.origin.x =
          kContainerSideInset + buttonWidth * actionButton.indexInRow;
  ...
  if (foundAnimatingOut || foundAnimatingIn) {
    ...
    if (foundAnimatingOut) {
      _fakeBottomDivider.frame =
          NSMakeRect(1, finalYBottom, _container.frame.size.width - 2, 1);
    } else if (foundAnimatingIn) {
      _fakeBottomDivider.frame =
          NSMakeRect(1, liveY, _container.frame.size.width - 2, 1);
    }
...
- (void)animateCardWithDuration:(CGFloat)duration
  ...
  CABasicAnimation *containerAnimation =
      [CABasicAnimation animationWithKeyPath:@"bounds"];
  ...
  for (iTermTipCardActionButton *button in self.actionButtons) {
    if (button.animationState != kTipCardButtonNotAnimating) {
      ...
      animation.toValue = [NSValue
          valueWithPoint:NSMakePoint(position.x, position.y + heightChange)];
      animation.duration = duration;
  ...
  // Now that animations are in flight, update the card's layout. It won't be
  // visible until the animations are done.
  [self layoutWithWidth:superviewWidth origin:NSZeroPoint];

  // Change button frames. Also won't be visible until animations are done. A
  // change to hidden apparently has to be doen after the transaction is
  // committed so this goes here.
