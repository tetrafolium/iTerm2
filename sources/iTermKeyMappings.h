...
// Given a keycode and modifier mask, return the action and fill in the optional
// text string. if keyMappings is provided, that is searched first, and if
// nothing is found (or keyMappings is nil) then the global mappings are
// searched.
+ (nullable iTermKeyBindingAction *)
    actionForKeystroke:(iTermKeystroke *)keystroke
...
+ (iTermKeyBindingAction *_Nullable)
    localActionForKeystroke:(iTermKeystroke *)keystroke
...
+ (iTermKeystroke *_Nullable)keystrokeAtIndex:(int)rowIndex
                                    inprofile:(Profile *)profile;
...
// must be in bounds.
// If newMapping is YES, either replace the existing mapping for the given
// keystroke or add a new one if there is no existing mapping.
//
