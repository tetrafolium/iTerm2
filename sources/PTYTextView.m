...
- (instancetype)initWithAbsoluteLineNumber:(long long)row
...
@interface PTYTextView (MouseHandler) <iTermSecureInputRequesting,
                                       PTYMouseHandlerDelegate>
@end
...
@implementation PTYTextView {
  ...
  // Position of cursor last time we looked. Since the cursor might move around
  // a lot between calls to -updateDirtyRects without making any changes, we
  // only redraw the old and new cursor positions.
...
// and hiding it while using Metal seems to fix the problem (at least on my
// iMac).
//
...
- (instancetype)initWithFrame:(NSRect)aRect colorMap:(iTermColorMap *)colorMap {
  ...
  if (self) {
    ...
    _drawingHelper.underlinedRange = VT100GridAbsWindowedRangeMake(
        VT100GridAbsCoordRangeMake(-1, -1, -1, -1), 0, 0);
    ...
    _urlActionHelper = [[iTermURLActionHelper alloc]
        initWithSemanticHistoryController:_semanticHistoryController];
    _urlActionHelper.delegate = self;
    _contextMenuHelper = [[iTermTextViewContextMenuHelper alloc]
        initWithURLActionHelper:_urlActionHelper];
    ...
    threeFingerTapGestureRecognizer_ = [[ThreeFingerTapGestureRecognizer alloc]
        initWithTarget:self
    ...
    _mouseHandler = [[PTYMouseHandler alloc]
                    initWithSelectionScrollHelper:_selectionScrollHelper
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<PTYTextView: %p frame=%@ visibleRect=%@ "
                                    @"dataSource=%@ delegate=%@ window=%@>",
                                    self, [NSValue valueWithRect:self.frame],
                                    [NSValue valueWithRect:[self visibleRect]],
                                    _dataSource, _delegate, self.window];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
  if ([item action] == @selector(paste:)) {
    ...
    return [[[NSPasteboard generalPasteboard] pasteboardItems]
        anyWithBlock:^BOOL(NSPasteboardItem *item) {
  ...
  if ([item action] == @selector(showHideNotes:)) {
    item.state = [self anyAnnotationsAreVisible] ? NSControlStateValueOn
                                                 : NSControlStateValueOff;
    return YES;
  }
  if ([item action] == @selector(toggleShowTimestamps:)) {
    item.state = _drawingHelper.showTimestamps ? NSControlStateValueOn
                                               : NSControlStateValueOff;
    return YES;
  }
  ...
  if (item.action == @selector(terminalStateToggleAlternateScreen:) ||
    ...
    item.state = [self.delegate textViewTerminalStateForMenuItem:item]
...
- (void)scrollEnd {
  ...
  lastLine.origin.y =
      (([_dataSource numberOfLines] - 1) * _lineHeight + [self excess] +
...
- (void)flagsChanged:(NSEvent *)theEvent {
  ...
  NSString *string =
      [_keyboardHandler.keyMapper keyMapperStringForPreCocoaEvent:theEvent];
  if (string) {
...
// Addendum: control page up/down seem to be supported because I did not
// understand macOS very well back in issue 1112. I guess I won't break it.
- (BOOL)performKeyEquivalent:(NSEvent *)theEvent {
  ...
  unichar unmodunicode =
      [unmodkeystr length] > 0 ? [unmodkeystr characterAtIndex:0] : 0;

  if ([_keyboardHandler performKeyEquivalent:theEvent
  ...
  if ((modifiers & NSEventModifierFlagControl) &&
    ...
    case NSPageUpFunctionKey:
      [(PTYScroller *)(
          [[self enclosingScrollView] verticalScroller]) setUserScroll:YES];
...
- (int)inputMethodEditorLength {
  ...
  int len;
  StringToScreenChars(str, buf, fg, bg, &len,
                      [_delegate textViewAmbiguousWidthCharsAreDoubleWidth],
                      NULL, NULL, [_delegate textViewUnicodeNormalizationForm],
  ...
  if (width == 0 && len > 0) {
    // Width should only be zero in weirdo edge cases, but the modulo below
    // caused crashes.
    return len;
  }
  int extra = 0;
...
- (CGFloat)pageScrollHeight {
  ...
  return scrollRect.size.height -
         [[self enclosingScrollView] verticalPageScroll];
}
...
- (void)scrollToAbsoluteOffset:(long long)absOff height:(int)height {
  ...
  aFrame.origin.x = 0;
  aFrame.origin.y =
      (absOff - [_dataSource totalScrollbackOverflow]) * _lineHeight -
      [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins];
...
- (BOOL)withRelativeWindowedRange:(VT100GridAbsWindowedRange)range
                            block:(void (^NS_NOESCAPE)(VT100GridWindowedRange))
                                      block {
  const long long overflow = _dataSource.totalScrollbackOverflow;
  if (range.coordRange.start.y < overflow ||
      range.coordRange.start.y - overflow > INT_MAX) {
    return NO;
  }
  if (range.coordRange.end.y < overflow ||
      range.coordRange.end.y - overflow > INT_MAX) {
    return NO;
  }
  VT100GridWindowedRange relative =
      VT100GridWindowedRangeFromAbsWindowedRange(range, overflow);
  block(relative);
  return YES;
}
...
- (void)scrollToSelection {
  ...
  [self
      withRelativeCoordRange:[_selection spanningAbsRange]
                       block:^(VT100GridCoordRange range) {
                         ...
                         aFrame.origin.y =
                             ...
                             [iTermPreferences
                                 intForKey:
                                     kPreferenceKeyTopBottomMargins]; // allow
                                                                      // for top
                                                                      // margin
                         aFrame.size.width = [self frame].size.width;
                         aFrame.size.height =
                             (range.end.y - range.start.y + 1) * _lineHeight;
                         [self scrollRectToVisible:aFrame];
                         [(PTYScroller *)([[self enclosingScrollView]
...
- (void)scrollLineNumberRangeIntoView:(VT100GridRange)range {
  ...
  if (range.location >= firstVisibleLine &&
      range.location + range.length <= lastVisibleLine) {
    // Already visible
    return;
  }
  if (range.length < [_dataSource height]) {
    ...
    const int currentBottomLine =
        rangeOfVisibleLines.location + rangeOfVisibleLines.length;
...
- (NSRect)adjustScroll:(NSRect)proposedVisibleRect {
  proposedVisibleRect.origin.y =
      (int)(proposedVisibleRect.origin.y / _lineHeight + 0.5) * _lineHeight;
...
- (void)setNeedsDisplayOnLine:(int)line {
  [self setNeedsDisplayOnLine:line
                      inRange:VT100GridRangeMake(0, _dataSource.width)];
}

- (void)setNeedsDisplayOnLine:(int)y inRange:(VT100GridRange)range {
  NSRect dirtyRect;
  const BOOL allowPartialLineRedraw =
      [iTermAdvancedSettingsModel preferSpeedToFullLigatureSupport];
  const int x = allowPartialLineRedraw ? range.location : 0;
  const int maxX = range.location + range.length;

  dirtyRect.origin.x =
      [iTermPreferences intForKey:kPreferenceKeySideMargins] + x * _charWidth;
  dirtyRect.origin.y = y * _lineHeight;
  dirtyRect.size.width =
      allowPartialLineRedraw ? (maxX - x) * _charWidth : _dataSource.width;
  ...
  // Expand the rect in case we're drawing a changed cell with an oversize
  // glyph.
  dirtyRect = [self rectWithHalo:dirtyRect];

  DLog(@"Line %d is dirty in range [%d, %d), set rect %@ dirty", y, x, maxX,
       [NSValue valueWithRect:dirtyRect]);
...
- (void)invalidateInputMethodEditorRect {
  ...
  int imeLines =
      ([_dataSource cursorX] - 1 + [self inputMethodEditorLength] + 1) /
          [_dataSource width] +
      1;

  NSRect imeRect =
      NSMakeRect([iTermPreferences intForKey:kPreferenceKeySideMargins],
                 ([_dataSource cursorY] - 1 + [_dataSource numberOfLines] -
                  [_dataSource height]) *
                     _lineHeight,
                 [_dataSource width] * _charWidth, imeLines * _lineHeight);
...
- (void)mouseEntered:(NSEvent *)event {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyFocusFollowsMouse] &&
      [[self window] alphaValue] > 0 && ![NSApp modalWindow]) {
    // Some windows automatically close when they lose key status and are
    // incompatible with FFM. Check if the key window or its controller
    // implements disableFocusFollowsMouse and if it returns YES do nothing.
    id obj = nil;
    if ([[NSApp keyWindow]
            respondsToSelector:@selector(disableFocusFollowsMouse)]) {
      obj = [NSApp keyWindow];
    } else if ([[[NSApp keyWindow] windowController]
                   respondsToSelector:@selector(disableFocusFollowsMouse)]) {
      obj = [[NSApp keyWindow] windowController];
    }
    if (!NSEqualPoints(_mouseLocationToRefuseFirstResponderAt,
                       [NSEvent mouseLocation])) {
      DLog(@"%p Mouse location is %@, refusal point is %@", self,
           NSStringFromPoint([NSEvent mouseLocation]),
           NSStringFromPoint(_mouseLocationToRefuseFirstResponderAt));
      if ([iTermAdvancedSettingsModel stealKeyFocus]) {
...
- (void)sendFakeThreeFingerClickDown:(BOOL)isDown
                        basedOnEvent:(NSEvent *)event {
  NSEvent *fakeEvent = isDown ? [event mouseDownEventFromGesture]
...
- (void)drawRect:(NSRect)rect {
  ...
  if (_dataSource.width <= 0) {
    ITCriticalError(_dataSource.width < 0, @"Negative datasource width of %@",
                    @(_dataSource.width));
    return;
  }
  DLog(@"drawing document visible rect %@",
       NSStringFromRect(self.enclosingScrollView.documentVisibleRect));
  ...
  [self performBlockWithFlickerFixerGrid:^{
    ...
    [_drawingHelper drawTextViewContentInRect:rect
                                     rectsPtr:rectArray
                                    rectCount:rectCount];
    ...
    // Not sure why this is needed, but for some reason this view draws over its
    // subviews.
    ...
    if (_drawingHelper.blinkingFound && _blinkAllowed) {
      // The user might have used the scroll wheel to cause blinking text to
      // become visible. Make sure the timer is running if anything onscreen is
      // blinking.
...
- (void)performBlockWithFlickerFixerGrid:(void(NS_NOESCAPE ^)(void))block {
  ...
  PTYTextViewSynchronousUpdateState *savedState =
...
- (iTermTextDrawingHelper *)drawingHelper {
  ...
  _drawingHelper.ambiguousIsDoubleWidth =
      [_delegate textViewAmbiguousWidthCharsAreDoubleWidth];
  ...
  _drawingHelper.gridSize =
      VT100GridSizeMake(_dataSource.width, _dataSource.height);
  _drawingHelper.numberOfLines = _dataSource.numberOfLines;
  _drawingHelper.cursorCoord =
      VT100GridCoordMake(_dataSource.cursorX - 1, _dataSource.cursorY - 1);
  _drawingHelper.totalScrollbackOverflow =
      [_dataSource totalScrollbackOverflow];
  _drawingHelper.numberOfScrollbackLines =
      [_dataSource numberOfScrollbackLines];
  _drawingHelper.reverseVideo = [[_dataSource terminal] reverseVideo];
  _drawingHelper.textViewIsActiveSession =
      [self.delegate textViewIsActiveSession];
  _drawingHelper.isInKeyWindow = [self isInKeyWindow];
  // Draw the cursor filled in when we're inactive if there's a popup open or
  // key focus was stolen.
  _drawingHelper.shouldDrawFilledInCursor = (
      [self.delegate textViewShouldDrawFilledInCursor] || _keyFocusStolenCount);
  _drawingHelper.isFrontTextView =
      (self == [[iTermController sharedInstance] frontTextView]);
  _drawingHelper.transparencyAlpha = [self transparencyAlpha];
  _drawingHelper.now = [NSDate timeIntervalSinceReferenceDate];
  _drawingHelper.drawMarkIndicators =
      [_delegate textViewShouldShowMarkIndicators];
  ...
  _drawingHelper.asciiLigatures =
      _primaryFont.hasDefaultLigatures || _asciiLigatures;
  _drawingHelper.nonAsciiLigatures =
      _secondaryFont.hasDefaultLigatures || _nonAsciiLigatures;
  ...
  _drawingHelper.passwordInput =
      ([self isInKeyWindow] && [_delegate textViewIsActiveSession] &&
  ...
  _drawingHelper.forceAntialiasingOnRetina =
      [iTermAdvancedSettingsModel forceAntialiasingOnRetina];
  ...
  _drawingHelper.indicatorFrame =
      [self configureIndicatorsHelperWithRightMargin:rightMargin];
...
- (NSColor *)defaultBackgroundColor {
  ...
  return [[_colorMap processedBackgroundColorForBackgroundColor:
                         [_colorMap colorForKey:kColorMapBackground]]
      colorWithAlphaComponent:alpha];
}

- (NSColor *)defaultTextColor {
  return [_colorMap
      processedTextColorForTextColor:[_colorMap colorForKey:kColorMapForeground]
                 overBackgroundColor:[self defaultBackgroundColor]
              disableMinimumContrast:NO];
}

- (void)updateMarkedTextAttributes {
  // During initialization, this may be called before the non-ascii font is set
  // so we use a system font as a placeholder.
...
- (void)updateScrollerForBackgroundColor {
  ...
  // The knob style is used only for overlay scrollers. In the minimal theme,
  // the window decorations' colors are based on the terminal background color.
  // That means the appearance must be changed to get legacy scrollbars to
  // change color.
  if (@available(macOS 10.14, *)) {
    if ([self.delegate
                textViewTerminalBackgroundColorDeterminesWindowDecorationColor]) {
      scroller.appearance =
          isDark ? [NSAppearance appearanceNamed:NSAppearanceNameDarkAqua]
...
// Number of extra lines below the last line of text that are always the
// background color. This is 2 except for just after the frame has changed and
// things are resizing.
- (double)excess {
  NSRect visible = [self scrollViewContentSize];
  visible.size.height -=
      [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] *
      2; // Height without top and bottom margins.
  int rows = visible.size.height / _lineHeight;
  double usablePixels = rows * _lineHeight;
  return MAX(
      visible.size.height - usablePixels +
          [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins],
      [iTermPreferences
          intForKey:kPreferenceKeyTopBottomMargins]); // Never have less than
                                                      // VMARGIN excess, but it
                                                      // can be more (if another
                                                      // tab has a bigger font)
}
...
- (void)maybeInvalidateWindowShadow {
  ...
  if (@available(macOS 10.14, *)) {
    const double invalidateFPS =
        [iTermAdvancedSettingsModel invalidateShadowTimesPerSecond];
    if (invalidateFPS > 0) {
...
- (NSRect)configureIndicatorsHelperWithRightMargin:(CGFloat)rightMargin {
  ...
  [_indicatorsHelper
      setIndicator:kItermIndicatorBroadcastInput
...
- (CGFloat)desiredHeight {
  ...
  return ([_dataSource numberOfLines] * _lineHeight + [self excess] +
          _drawingHelper.numberOfIMELines * _lineHeight);
}
...
- (NSPoint)locationInTextViewFromEvent:(NSEvent *)event {
  ...
  // Clamp the y position to be within the view. Sometimes we get events we
  // probably shouldn't.
  locationInTextView.y =
      MIN(self.frame.size.height - 1, MAX(0, locationInTextView.y));
...
- (VT100GridRange)rangeOfVisibleLines {
  ...
  int start =
      [self coordForPoint:visibleRect.origin allowRightMarginOverflow:NO].y;
  int end = [self coordForPoint:NSMakePoint(0, NSMaxY(visibleRect) - 1)
...
- (NSRect)gridRect {
  ...
  return NSMakeRect(visibleRect.origin.x, lineStart * _lineHeight,
...
- (void)setLineHeight:(double)aLineHeight {
  ...
  _drawingHelper.cellSizeWithoutSpacing =
      NSMakeSize(_charWidthWithoutSpacing, _charHeightWithoutSpacing);
}
...
// This exists to work around an apparent OS bug described in issue 2690. Under
// some circumstances (which I cannot reproduce) the key window will be an
// NSToolbarFullScreenWindow and the iTermTerminalWindow will be one of the main
// windows. NSToolbarFullScreenWindow doesn't appear to handle keystrokes, so
// they fall through to the main window. We'd like the cursor to blink and have
// other key- window behaviors in this case.
- (BOOL)isInKeyWindow {
  ...
  if (!strcmp("NSToolbarFullScreenWindow", object_getClassName(theKeyWindow))) {
    DLog(@"key window is a NSToolbarFullScreenWindow, using my main window "
         @"status of %d as key status",
...
// Uses an undocumented/deprecated API to receive key presses even when
// inactive.
- (BOOL)stealKeyFocus {
  // Make sure everything needed for focus stealing exists in this version of
  // Mac OS.
  CPSGetCurrentProcessFunction *getCurrentProcess =
      GetCPSGetCurrentProcessFunction();
  ...
  if (getCurrentProcess(&psn) == noErr) {
    ...
    // CPSStealKeyFocus appears to succeed even when it returns an error. See
    // issue 4113.
    return YES;
  }
...
- (BOOL)_isTextBlinking {
  ...
  int lineEnd = ceil(([self visibleRect].origin.y +
                      [self visibleRect].size.height - [self excess]) /
                     _lineHeight);
  if (lineStart < 0) {
    lineStart = 0;
  }
...
- (BOOL)updateDirtyRects:(BOOL *)foundDirtyPtr {
  ...
  anythingIsBlinking = [self _markChangedSelectionAndBlinkDirty:redrawBlink
                                                          width:WIDTH];
  ...
  if (_previousCursorCoord.x != cursorPosition.x ||
    ...
    DLog(@"Mark previous cursor position %d,%lld dirty", _previousCursorCoord.x,
         _previousCursorCoord.y - totalScrollbackOverflow);
    int maxX = [_dataSource width] - 1;
    if (_drawingHelper.highlightCursorLine) {
      [_dataSource
          setLineDirtyAtY:_previousCursorCoord.y - totalScrollbackOverflow];
      ...
      [_dataSource setCharDirtyAtCursorX:MIN(maxX, _previousCursorCoord.x)
                                       Y:_previousCursorCoord.y -
                                         totalScrollbackOverflow];
      DLog(@"Mark current cursor position %d,%lld dirty",
           _previousCursorCoord.x,
           _previousCursorCoord.y - totalScrollbackOverflow);
      [_dataSource setCharDirtyAtCursorX:MIN(maxX, cursorPosition.x)
                                       Y:cursorPosition.y];
    }
    // Set _previousCursorCoord to new cursor position
    _previousCursorCoord = VT100GridAbsCoordMake(
        cursorPosition.x, cursorPosition.y + totalScrollbackOverflow);
  }

  // Remove results from dirty lines and mark parts of the view as needing
  // display.
  NSMutableIndexSet *cleanLines = [NSMutableIndexSet indexSet];
  if (allDirty) {
    foundDirty = YES;
    [_findOnPageHelper
        removeHighlightsInRange:NSMakeRange(lineStart + totalScrollbackOverflow,
                                            lineEnd - lineStart)];
    ...
    for (int y = lineStart; y < lineEnd; y++) {
      ...
      if (range.length > 0) {
        foundDirty = YES;
        [_findOnPageHelper
            removeHighlightsInRange:NSMakeRange(y + totalScrollbackOverflow,
                                                1)];
        [_findOnPageHelper
            removeSearchResultsInRange:NSMakeRange(y + totalScrollbackOverflow,
                                                   1)];
  ...
  // If you're viewing the scrollback area and it contains an animated gif it
  // will need to be redrawn periodically. The set of animated lines is added to
  // while drawing and then reset here.
...
- (void)handleScrollbackOverflow:(int)scrollbackOverflow
                      userScroll:(BOOL)userScroll {
...
- (void)refreshAccessibility {
  ...
  long long absCursorY =
      ([_dataSource cursorY] + [_dataSource numberOfLines] +
  ...
  if ([_dataSource cursorX] != _lastAccessibilityCursorX ||
    ...
    NSAccessibilityPostNotification(
        self, NSAccessibilitySelectedRowsChangedNotification);
    NSAccessibilityPostNotification(
        self, NSAccessibilitySelectedColumnsChangedNotification);
    ...
    if (UAZoomEnabled()) {
      CGRect selectionRect = NSRectToCGRect(
          [self.window convertRectToScreen:[self convertRect:[self cursorFrame]
                                                      toView:nil]]);
      selectionRect = [self accessibilityConvertScreenRect:selectionRect];
      UAZoomChangeFocus(&selectionRect, &selectionRect,
                        kUAZoomFocusTypeInsertionPoint);
    }
  }
}

// This is called periodically. It updates the frame size, scrolls if needed,
// ensures selections and subviews are positioned correctly in case things
// scrolled
//
...
- (BOOL)refresh {
  ...
  BOOL userScroll = [(PTYScroller *)(
      [[self enclosingScrollView] verticalScroller]) userScroll];
  ...
  if (!userScroll) {
    iTermMetalClipView *clipView =
        [iTermMetalClipView castFrom:self.enclosingScrollView.contentView];
  ...
  if ([[self subviews] count]) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:PTYNoteViewControllerShouldUpdatePosition
                      object:nil];
    // Not sure why this is needed, but for some reason this view draws over its
    // subviews.
    for (NSView *subview in [self subviews]) {
  ...
  BOOL foundDirty = NO;
  const BOOL foundBlink =
      [self updateDirtyRects:&foundDirty] || [self isCursorBlinking];
...
- (BOOL)_markChangedSelectionAndBlinkDirty:(BOOL)redrawBlink width:(int)width {
  ...
  if (_blinkAllowed) {
    lines =
        [_dataSource linesInRange:NSMakeRange(lineStart, lineEnd - lineStart)];
  }
  ...
  for (int y = lineStart, i = 0; y < lineEnd; y++, i++) {
    if (_blinkAllowed && i < numLines) {
      ...
      for (int x = 0; x < lines[i].length; x++) {
        ...
        if (blinked) {
          ...
          DLog(@"Redraw rect for line y=%d i=%d blink: %@", y, i,
    ...
    const long long overflow = _dataSource.totalScrollbackOverflow;
    NSIndexSet *areSelected =
        [_selection selectedIndexesOnAbsoluteLine:y + overflow];
    NSIndexSet *wereSelected =
        [_oldSelection selectedIndexesOnAbsoluteLine:y + overflow];
...
- (SmartMatch *)smartSelectAtX:(int)x
  ...
  const long long overflow = _dataSource.totalScrollbackOverflow;
  SmartMatch *match = [_urlActionHelper
      smartSelectAtAbsoluteCoord:VT100GridAbsCoordMake(x, y + overflow)
                              to:&absRange
...
- (IBAction)selectAll:(id)sender {
  ...
  [_selection beginSelectionAtAbsCoord:VT100GridAbsCoordMake(0, overflow)
                                ...
                                append:NO];
  [_selection
      moveSelectionEndpointTo:VT100GridAbsCoordMake(
                                  [_dataSource width],
...
- (void)selectRange:(VT100GridAbsCoordRange)range {
  ...
  DLog(@"Total scrollback overflow is %lld",
       [_dataSource totalScrollbackOverflow]);
...
// Returns an NSAttributedString* if style is iTermCopyTextStyleAttributed, or
// an NSString* if not.
- (id)selectedTextWithStyle:(iTermCopyTextStyle)style
  ...
  BOOL copyLastNewline =
      [iTermPreferences boolForKey:kPreferenceKeyCopyLastNewline];
  ...
  [_selection enumerateSelectedAbsoluteRanges:^(
                  VT100GridAbsWindowedRange absRange, BOOL *stop, BOOL eol) {
    [self
        withRelativeWindowedRange:absRange
                            block:^(VT100GridWindowedRange range) {
                              ...
                              iTermTextExtractor *extractor =
                                  [iTermTextExtractor
                                      textExtractorWithDataSource:_dataSource];
                              ...
                              if (attributeProvider != nil) {
                                [theSelectedText
                                    appendAttributedString:content];
                              } else {
                                [theSelectedText appendString:content];
                              }
                              NSString *contentString =
                                  (attributeProvider != nil) ? [content string]
                                                             : content;
  ...
  if (style == iTermCopyTextStyleWithControlSequences) {
    ...
    [attributedString
        enumerateAttribute:sgrAttribute
                   inRange:NSMakeRange(0, attributedString.length)
                   options:0
                usingBlock:^(NSSet *_Nullable value, NSRange range,
                             BOOL *_Nonnull stop) {
                  NSString *code = [NSString
                      stringWithFormat:@"%c[%@m", VT100CC_ESC,
                                       [value.allObjects
...
- (iTermLogicalMovementHelper *)logicalMovementHelperForCursorCoordinate:
    (VT100GridCoord)relativeCursorCoord {
  const long long overflow = _dataSource.totalScrollbackOverflow;
  VT100GridAbsCoord cursorCoord = VT100GridAbsCoordMake(
      relativeCursorCoord.x, relativeCursorCoord.y + overflow);
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:_dataSource];
  iTermLogicalMovementHelper *helper = [[iTermLogicalMovementHelper alloc]
        initWithTextExtractor:extractor
...
- (void)moveSelectionEndpoint:(PTYTextViewSelectionEndpoint)endpoint
  ...
  iTermLogicalMovementHelper *helper =
      [self logicalMovementHelperForCursorCoordinate:cursorCoord];
  VT100GridAbsCoordRange newRange = [helper moveSelectionEndpoint:endpoint
                                                      inDirection:direction
                                                               by:unit];

  [self
      withRelativeCoordRange:newRange
                       block:^(VT100GridCoordRange range) {
                         [self scrollLineNumberRangeIntoView:VT100GridRangeMake(
                                                                 range.start.y,
                                                                 range.end.y -
                                                                     range.start
                                                                         .y)];
...
- (BOOL)moveSelectionEndpointToX:(int)x
  ...
  DLog(@"Move selection endpoint to %d,%d, coord=%@", x, y,
       [NSValue valueWithPoint:locationInTextView]);
  int width = [_dataSource width];
  if (locationInTextView.y == 0) {
    x = y = 0;
  } else if (locationInTextView.x < [iTermPreferences
  ...
  const BOOL hasColumnWindow =
      (_selection.liveRange.columnWindow.location > 0 ||
  ...
  const BOOL result = [_selection
      moveSelectionEndpointTo:VT100GridAbsCoordMake(
                                  x, y + _dataSource.totalScrollbackOverflow)];
...
- (NSString *)getWordForX:(int)x
  ...
  VT100GridWindowedRange range =
      [extractor rangeForWordAt:coord maximumLength:kLongMaximumWordLength];
  if (rangePtr) {
    *rangePtr = range;
  }
...
- (void)copy:(id)sender {
  // TODO: iTermSelection should use absolute coordinates everywhere. Until that
  // is done, we must call refresh here to take care of any scrollback overflow
  // that would cause the selected range to not match reality.
  [self refresh];
...
- (IBAction)copyWithStyles:(id)sender {
  ...
  NSAttributedString *copyAttributedString =
      [self selectedAttributedTextWithPad:NO];
  if ([iTermAdvancedSettingsModel disallowCopyEmptyString] &&
  ...
  DLog(@"Have selected text of length %d. selection=%@",
       (int)[copyAttributedString length], _selection);
  ...
  // I used to do
  //   [pboard setString:[copyAttributedString string]
...
- (IBAction)copyWithControlSequences:(id)sender {
  ...
  NSString *copyString =
      [self selectedTextWithStyle:iTermCopyTextStyleWithControlSequences
                     cappedAtSize:-1
                minimumLineNumber:0];
...
- (BOOL)pasteValuesOnPasteboard:(NSPasteboard *)pasteboard
                  cdToDirectory:(BOOL)cdToDirectory {
  ...
  if ([types containsObject:NSPasteboardTypeFileURL]) {
    ...
    NSArray *filenames =
        [pasteboard filenamesOnPasteboardWithShellEscaping:YES];
    if (filenames.count) {
      BOOL pasteNewline = NO;

      NSMutableString *stringToPaste = [NSMutableString string];
      if (cdToDirectory) {
        // cmd-drag: "cd" to dragged directory (well, we assume it's a
        // directory). If multiple files are dragged, balk.
...
- (id)contentWithAttributes:(BOOL)attributes {
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:_dataSource];
  VT100GridCoordRange theRange = VT100GridCoordRangeMake(
      0, 0, [_dataSource width], [_dataSource numberOfLines] - 1);
...
- (void)saveDocumentAs:(id)sender {
  ...
  dateFormatter.dateFormat =
      [NSDateFormatter dateFormatFromTemplate:@"yyyy-MM-dd hh-mm-ss"
                                      options:0
                                       locale:nil];
  NSString *formattedDate = [dateFormatter stringFromDate:[NSDate date]];
  // Stupid mac os can't have colons in filenames
  formattedDate = [formattedDate stringByReplacingOccurrencesOfString:@":"
                                                           withString:@"-"];
  NSString *nowStr =
      [NSString stringWithFormat:@"Log at %@.txt", formattedDate];

  // Show the save panel. The first time it's done set the path, and from then
  // on the save panel will remember the last path you used.tmp
  [NSSavePanel setDirectoryURL:[NSURL fileURLWithPath:path]
                     onceForID:@"saveDocumentAs:"
                     savePanel:aSavePanel];
...
- (void)highlightMarkOnLine:(int)line hasErrorCode:(BOOL)hasErrorCode {
  ...
  NSView *highlightingView = [[[NSView alloc]
      initWithFrame:NSMakeRect(0, y, self.frame.size.width, _lineHeight)]
      autorelease];
  [highlightingView setWantsLayer:YES];
  [self addSubview:highlightingView];

  // Set up layer's initial state
  highlightingView.layer.backgroundColor = hasErrorCode
  ...
  iTermHighlightedRow *entry = [[iTermHighlightedRow alloc]
      initWithAbsoluteLineNumber:_dataSource.totalScrollbackOverflow + line
...
- (void)addNote {
  ...
  [self
      withRelativeCoordRange:_selection.lastAbsRange.coordRange
                       block:^(VT100GridCoordRange range) {
  ...
  for (NSView *view in [self subviews]) {
    if ([view isKindOfClass:[PTYNoteView class]]) {
      ...
      if (coordRange.end.y >= 0) {
        [note
            setAnchor:NSMakePoint(coordRange.end.x * _charWidth +
                                      [iTermPreferences
                                          intForKey:kPreferenceKeySideMargins],
...
- (VT100GridCoord)moveCursorHorizontallyTo:(VT100GridCoord)target
                                      from:(VT100GridCoord)cursor {
  ...
  // Note that we could overshoot the destination because of double-width
  // characters if the target is a DWC_RIGHT.
  while (![extractor coord:cursor isEqualToCoord:target]) {
    switch (initialOrder) {
    case NSOrderedAscending:
      [_delegate writeStringWithLatin1Encoding:
                     [[terminal.output keyArrowRight:0]
    ...
    case NSOrderedDescending:
      [_delegate writeStringWithLatin1Encoding:
                     [[terminal.output keyArrowLeft:0]
                         stringWithEncoding:NSISOLatin1StringEncoding]];
...
- (void)placeCursorOnCurrentLineWithEvent:(NSEvent *)event
  ...
  VT100GridCoord cursor = VT100GridCoordMake(
      [_dataSource cursorX] - 1, [_dataSource absoluteLineNumberOfCursor] -
                                     [_dataSource totalScrollbackOverflow]);
  if (!verticalOk) {
  ...
  if (cursor.x > target.x) {
    ...
    cursor =
        [self moveCursorHorizontallyTo:VT100GridCoordMake(target.x, cursor.y)
                                  from:cursor];
  }

  // Move cursor vertically.
  DLog(@"Move cursor vertically from %@ to y=%d",
       VT100GridCoordDescription(cursor), target.y);
  while (cursor.y != target.y) {
    if (cursor.y > target.y) {
      [_delegate writeStringWithLatin1Encoding:
                     [[terminal.output keyArrowUp:0]
                         stringWithEncoding:NSISOLatin1StringEncoding]];
      cursor.y--;
    } else {
      [_delegate writeStringWithLatin1Encoding:
                     [[terminal.output keyArrowDown:0]
                         stringWithEncoding:NSISOLatin1StringEncoding]];
      cursor.y++;
    }
  }
...
// Called for a right click that isn't control+click (e.g., two fingers on
// trackpad).
- (void)openContextMenuWithEvent:(NSEvent *)event {
  NSPoint clickPoint = [self clickPoint:event allowRightMarginOverflow:NO];
  [_contextMenuHelper
      openContextMenuAt:VT100GridCoordMake(clickPoint.x, clickPoint.y)
                  event:event];
}
...
- (void)extendSelectionWithEvent:(NSEvent *)event {
  ...
  const long long overflow = _dataSource.totalScrollbackOverflow;
  [_selection
      beginExtendingSelectionAt:VT100GridAbsCoordMake(clickPoint.x,
                                                      clickPoint.y + overflow)];
  [_selection endLiveSelection];
  if ([iTermPreferences boolForKey:kPreferenceKeySelectionCopiesText]) {
    [self copySelectionAccordingToUserPreferences];
  }
...
- (void)newVerticalSplitWithProfile:(NSString *)guid
...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender {
  // NOTE: draggingUpdated: calls this method because they need the same
  // implementation.
  int numValid = -1;
  if ([NSEvent modifierFlags] &
      NSEventModifierFlagOption) { // Option-drag to copy
    _drawingHelper.showDropTargets = YES;
    [self.delegate textViewDidUpdateDropTargetVisibility];
  }
  NSDragOperation operation = [self dragOperationForSender:sender
                                        numberOfValidItems:&numValid];
...
- (BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender {
  ...
  // If parent class does not know how to deal with this drag type, check if we
  // do.
  if (result != YES &&
...
- (NSDragOperation)dragOperationForSender:(id<NSDraggingInfo>)sender
  ...
  // It's ok to upload if a file is being dragged in and the drop location has a
  // remote host path.
  BOOL uploadOK =
      ([types containsObject:NSPasteboardTypeFileURL] && dropScpPath);

  // It's ok to paste if the the drag object is either a file or a string.
  BOOL pasteOK = !![[sender draggingPasteboard] availableTypeFromArray:@[
    NSPasteboardTypeFileURL, NSPasteboardTypeString
  ]];

  const BOOL optionPressed =
      ([NSEvent modifierFlags] & NSEventModifierFlagOption) != 0;
  NSDragOperation sourceMask = [sender draggingSourceOperationMask];
  DLog(@"source mask=%@, optionPressed=%@, pasteOk=%@", @(sourceMask),
       @(optionPressed), @(pasteOK));
  if (!optionPressed && pasteOK &&
      (sourceMask & (NSDragOperationGeneric | NSDragOperationCopy)) != 0) {
    DLog(@"Allowing a filename drag");
    // No modifier key was pressed and pasting is OK, so select the paste
    // operation.
    ...
    if (sourceMask & NSDragOperationGeneric) {
      // This is preferred since it doesn't have the green plus indicating a
      // copy
  ...
  } else if (optionPressed && uploadOK &&
             (sourceMask & NSDragOperationCopy) != 0) {
    ...
    if (numberOfValidItemsPtr) {
      *numberOfValidItemsPtr =
          [[pb filenamesOnPasteboardWithShellEscaping:NO] count];
    }
...
- (void)_dragImage:(iTermImageInfo *)imageInfo forEvent:(NSEvent *)theEvent {
  NSImage *icon =
      [imageInfo imageWithCellSize:NSMakeSize(_charWidth, _lineHeight)];
  ...
  NSPoint dragPoint = [self convertPoint:[theEvent locationInWindow]
                                fromView:nil];

  VT100GridCoord coord = VT100GridCoordMake(
      (dragPoint.x - [iTermPreferences intForKey:kPreferenceKeySideMargins]) /
          _charWidth,
  ...
  if (theLine && coord.x < [_dataSource width] && theLine[coord.x].image &&
    ...
    VT100GridCoord imageCellOrigin =
        VT100GridCoordMake(coord.x - pos.x, coord.y - pos.y);

    // Compute the pixel coordinate of the image's top left point
    NSPoint imageTopLeftPoint =
        NSMakePoint(imageCellOrigin.x * _charWidth +
  ...
  [dragItem setDraggingFrame:NSMakeRect(dragPoint.x, dragPoint.y,
...
- (void)_dragText:(NSString *)aString forEvent:(NSEvent *)theEvent {
  ...
  [tmpString drawInRect:NSMakeRect(0, 0, _charWidth * length, _lineHeight)
         withAttributes:nil];
  [anImage unlockFocus];
...
- (BOOL)confirmUploadOfFiles:(NSArray *)files toPath:(SCPPath *)path {
  ...
  if (files.count == 1) {
    ...
    text = [NSString
        stringWithFormat:@"Ok to scp the following files:\n%@\n\nto\n%@@%@:%@?",
                         [files componentsJoinedByString:@", "], path.username,
...
- (BOOL)uploadFilenamesOnPasteboard:(NSPasteboard *)pasteboard
                           location:(NSPoint)windowDropPoint {
  // Upload a file.
  NSArray *types = [pasteboard types];
  NSPoint dropPoint = [self convertPoint:windowDropPoint fromView:nil];
  int dropLine = dropPoint.y / _lineHeight;
  ...
  if ([types containsObject:NSPasteboardTypeFileURL] && filenames.count &&
      dropScpPath) {
    // This is all so the mouse cursor will change to a plain arrow instead of
    // the drop target cursor.
    if (![[self window] isKindOfClass:[NSPanel class]]) {
      // Can't do this to a floating panel or we switch away from the lion
      // fullscreen app we're over.
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  if (dragOperation & (NSDragOperationCopy | NSDragOperationGeneric)) {
    ...
    if ([NSEvent modifierFlags] & NSEventModifierFlagOption) {
      ...
      NSPoint windowDropPoint = [sender draggingLocation];
      return [self uploadFilenamesOnPasteboard:draggingPasteboard
                                      location:windowDropPoint];
    } else {
      DLog(@"No option so pasting filename");
      return [self
          pasteValuesOnPasteboard:draggingPasteboard
...
- (void)print:(id)sender {
  ...
  case 0: // visible range
    ...
    VT100GridCoordRange coordRange = VT100GridCoordRangeMake(
        0, lineOffset, [_dataSource width], lineOffset + numLines - 1);
    [self
        printContent:
            [extractor contentInRange:VT100GridWindowedRangeMake(coordRange, 0,
                                                                 0)
                     attributeProvider:^NSDictionary *(screen_char_t theChar) {
                       return [self charAttributes:theChar];
                     }
...
- (void)printContent:(id)content {
  ...
  if ([content isKindOfClass:[NSAttributedString class]]) {
    ...
    accessory = [[[iTermPrintAccessoryViewController alloc]
        initWithNibName:@"iTermPrintAccessoryViewController"
    ...
    accessory.userDidChangeSetting = ^() {
      ...
      if (accessory.blackAndWhite) {
        theAttributedString =
            [attributedString attributedStringByRemovingColor];
      } else {
  ...
  NSPrintOperation *operation =
      [NSPrintOperation printOperationWithView:tempView printInfo:printInfo];
  if (accessory) {
    operation.printPanel.options =
        (NSPrintPanelShowsCopies | NSPrintPanelShowsPaperSize |
...
// Legacy NSTextInput method, probably not used by the system but used
// internally.
- (void)insertText:(id)aString {
  // TODO: The replacement range is wrong
  [self insertText:aString
      replacementRange:NSMakeRange(0, [_drawingHelper.markedText length])];
}
...
- (void)setMarkedText:(id)aString
  ...
  DLog(@"setMarkedText%@ selectedRange%@ replacementRange:%@", aString,
       NSStringFromRange(selRange), NSStringFromRange(replacementRange));
  if ([aString isKindOfClass:[NSAttributedString class]]) {
    _drawingHelper.markedText = [[[NSAttributedString alloc]
        initWithString:[aString string]
  ...
  _drawingHelper.inputMethodMarkedRange =
  ...
  do {
    dirtStart = ([_dataSource cursorY] - 1 - _drawingHelper.numberOfIMELines) *
                    [_dataSource width] +
...
- (NSRange)markedRange {
  ...
  if (_drawingHelper.inputMethodMarkedRange.length > 0) {
    range = NSMakeRange([_dataSource cursorX] - 1,
                        _drawingHelper.inputMethodMarkedRange.length);
  } else {
...
- (NSArray *)validAttributesForMarkedText {
  return @[
    ...
    NSUnderlineStyleAttributeName, NSFontAttributeName
  ];
}
...
- (NSAttributedString *)
    attributedSubstringForProposedRange:(NSRange)proposedRange
                            actualRange:(NSRangePointer)actualRange {
  DLog(@"attributedSubstringForProposedRange:%@",
       NSStringFromRange(proposedRange));
  NSRange aRange = NSIntersectionRange(
      proposedRange, NSMakeRange(0, _drawingHelper.markedText.length));
  ...
  return [_drawingHelper.markedText
      attributedSubstringFromRange:NSMakeRange(0, aRange.length)];
}
...
- (NSRect)firstRectForCharacterRange:(NSRange)theRange
  ...
  NSRect rect = NSMakeRect(
      x * _charWidth + [iTermPreferences intForKey:kPreferenceKeySideMargins],
      (y + [_dataSource numberOfLines] - [_dataSource height] + 1) *
          _lineHeight,
      _charWidth * theRange.length, _lineHeight);
  rect.origin = [[self window]
      pointToScreenCoords:[self convertPoint:rect.origin toView:nil]];
  if (actualRange) {
...
- (IBAction)performFindPanelAction:(id)sender {
  ...
  case NSFindPanelActionReplaceAllInSelection:
    // For now we use a nonstandard way of doing these things for no good
    // reason. This will be fixed over time.
    return;
...
- (BOOL)continueFind:(double *)progress {
  return [_findOnPageHelper continueFind:progress
                                 context:[_dataSource findContext]
                                   width:[_dataSource width]
                           numberOfLines:[_dataSource numberOfLines]
                      overflowAdjustment:[_dataSource totalScrollbackOverflow] -
...
- (void)selectCoordRange:(VT100GridCoordRange)range {
  [_selection clearSelection];
  VT100GridAbsCoordRange absRange = VT100GridAbsCoordRangeFromCoordRange(
      range, _dataSource.totalScrollbackOverflow);
  iTermSubSelection *sub = [iTermSubSelection
      subSelectionWithAbsRange:VT100GridAbsWindowedRangeMake(absRange, 0, 0)
                          mode:kiTermSelectionModeCharacter
                         width:_dataSource.width];
...
- (NSRect)frameForCoord:(VT100GridCoord)coord {
  return NSMakeRect(
      MAX(0, floor(coord.x * _charWidth +
                   [iTermPreferences intForKey:kPreferenceKeySideMargins])),
...
- (long long)findOnPageOverflowAdjustment {
  return
      [_dataSource totalScrollbackOverflow] - [_dataSource scrollbackOverflow];
}
...
- (NSRange)findOnPageRangeOfVisibleLines {
  return NSMakeRange(_dataSource.totalScrollbackOverflow,
...
- (id)validRequestorForSendType:(NSString *)sendType
  ...
  NSSet *acceptedReturnTypes = [NSSet setWithArray:@[
    (NSString *)kUTTypeUTF8PlainText, NSPasteboardTypeString
  ]];
  NSSet *acceptedSendTypes = nil;
  if ([_selection hasSelection] &&
      [_selection length] <= [_dataSource width] * 10000) {
...
- (BOOL)writeSelectionToPasteboard:(NSPasteboard *)pboard
                             types:(NSArray *)types {
  // It is agonizingly slow to copy hundreds of thousands of lines just because
  // the context menu is opening. Services use this to get access to the
  // clipboard contents but it's lousy to hang for a few minutes for a feature
  // that won't be used very much, esp. for such large selections. In OS 10.9
  // this is called when opening the context menu, even though it is deprecated
  // by 10.9.
  NSString *copyString =
      [self selectedTextCappedAtSize:[iTermAdvancedSettingsModel
                                         maximumBytesToProvideToServices]];
...
- (NSRect)cursorFrame {
  ...
  return NSMakeRect([iTermPreferences intForKey:kPreferenceKeySideMargins] +
                        cursorX * _charWidth,
                    (lineStart + cursorY) * _lineHeight, _charWidth,
                    _lineHeight);
}
...
- (NSPoint)cursorCenterInScreenCoords {
  ...
  NSPoint cursorCenterInWindowCoords = [self convertPoint:cursorCenter
                                                   toView:nil];
  return [[self window] pointToScreenCoords:cursorCenterInWindowCoords];
}

// Returns the location of the cursor relative to the origin of
// self.findCursorWindow.
...
// Returns the proper frame for self.findCursorWindow, including every screen
// that the "hole" will be in.
...
- (void)createFindCursorWindowWithFireworks:(BOOL)forceFireworks {
  ...
  if (forceFireworks) {
    self.findCursorView = [iTermFindCursorView
        newFireworksViewWithFrame:NSMakeRect(0, 0, screenFrame.size.width,
...
- (void)selectionDidChange:(iTermSelection *)selection {
  ...
  if (!_selection.live && selection.hasSelection) {
    ...
    selectionString = [self selectedTextCappedAtSize:MAX_SELECTION_SIZE
                                   minimumLineNumber:0];
    [[iTermController sharedInstance]
        setLastSelection:selectionString.length < MAX_SELECTION_SIZE
                             ? selectionString
                             : nil];
  }
  [_delegate textViewSelectionDidChangeToTruncatedString:selectionString];
  DLog(@"Selection did change: selection=%@. stack=%@", selection,
       [NSThread callStackSymbols]);
}
...
- (VT100GridRange)selectionRangeOfTerminalNullsOnAbsoluteLine:
  ...
  const long long lineNumber =
      absLineNumber - _dataSource.totalScrollbackOverflow;
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForParentheticalAt:
  ...
  BOOL ok;
  const VT100GridCoord coord = VT100GridCoordFromAbsCoord(
      absCoord, _dataSource.totalScrollbackOverflow, &ok);
  if (!ok) {
    const long long totalScrollbackOverflow =
        _dataSource.totalScrollbackOverflow;
    return VT100GridAbsWindowedRangeMake(
        VT100GridAbsCoordRangeMake(0, totalScrollbackOverflow, 0,
                                   totalScrollbackOverflow),
        -1, -1);
  }
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:_dataSource];
  [extractor restrictToLogicalWindowIncludingCoord:coord];
  const VT100GridWindowedRange relative =
      [extractor rangeOfParentheticalSubstringAtLocation:coord];
  return VT100GridAbsWindowedRangeFromWindowedRange(
      relative, _dataSource.totalScrollbackOverflow);
}
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForWordAt:
  ...
  return VT100GridAbsWindowedRangeFromWindowedRange(range,
                                                    totalScrollbackOverflow);
}
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForSmartSelectionAt:
  ...
  VT100GridAbsWindowedRange absRange;
  [_urlActionHelper
      smartSelectAtAbsoluteCoord:absCoord
                              to:&absRange
                ignoringNewlines:NO
                  actionRequired:NO
                 respectDividers:[self liveSelectionRespectsSoftBoundaries]];
...
- (VT100GridAbsWindowedRange)selectionAbsRangeForWrappedLineAt:
  ...
  const BOOL ok = [self
      withRelativeCoord:absCoord
                  block:^(VT100GridCoord coord) {
                    iTermTextExtractor *extractor = [iTermTextExtractor
                        textExtractorWithDataSource:_dataSource];
  ...
  return VT100GridAbsWindowedRangeFromWindowedRange(relativeRange,
                                                    totalScrollbackOverflow);
}
  ...
  const BOOL ok = [self
      withRelativeCoord:absCoord
                  block:^(VT100GridCoord coord) {
                    if (![self liveSelectionRespectsSoftBoundaries]) {
                      result = VT100GridAbsWindowedRangeMake(
                          VT100GridAbsCoordRangeMake(
                              0, absCoord.y, [_dataSource width], absCoord.y),
                          0, 0);
                      return;
                    }
                    iTermTextExtractor *extractor = [iTermTextExtractor
                    ...
                    result = VT100GridAbsWindowedRangeMake(
                        VT100GridAbsCoordRangeMake(
                            extractor.logicalWindow.location, absCoord.y,
...
- (VT100GridAbsCoord)selectionPredecessorOfAbsCoord:
    (VT100GridAbsCoord)absCoord {
  __block VT100GridAbsCoord result = {0};
  const long long totalScrollbackOverflow = _dataSource.totalScrollbackOverflow;
  const BOOL ok =
      [self withRelativeCoord:absCoord
                        block:^(VT100GridCoord relativeCoord) {
                          ...
                          result = VT100GridAbsCoordFromCoord(
                              coord, totalScrollbackOverflow);
                        }];
  if (!ok) {
...
- (NSIndexSet *)selectionIndexesOnAbsoluteLine:(long long)absLine
  ...
  const long long totalScrollbackOverflow = _dataSource.totalScrollbackOverflow;
  if (absLine < totalScrollbackOverflow ||
      absLine - totalScrollbackOverflow > INT_MAX) {
    return nil;
  }
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:_dataSource];
...
- (void)colorMap:(iTermColorMap *)colorMap
  ...
  if (theKey == kColorMapBackground) {
    ...
    [[self enclosingScrollView]
        setBackgroundColor:[colorMap colorForKey:theKey]];
    [self recomputeBadgeLabel];
    [_delegate textViewBackgroundColorDidChange];
    ...
    _drawingHelper.unfocusedSelectionColor =
        [[_colorMap colorForKey:theKey] colorDimmedBy:2.0 / 3.0
...
- (void)selectionScrollWillStart {
  PTYScroller *scroller =
      (PTYScroller *)self.enclosingScrollView.verticalScroller;
...
- (iTermColorMapKey)colorMapKeyForCode:(int)theIndex
  ...
  case ColorModeNormal:
    ...
    // preference.
    if (isBold && self.brightenBold && (theIndex < 8) &&
...
- (iTermTextExtractor *)drawingHelperTextExtractor {
  return
      [[[iTermTextExtractor alloc] initWithDataSource:_dataSource] autorelease];
}
...
- (NSData *)drawingHelperMatchesOnLine:(int)line {
  return _findOnPageHelper
      .highlightMap[@(line + _dataSource.totalScrollbackOverflow)];
}

- (void)drawingHelperDidFindRunOfAnimatedCellsStartingAt:(VT100GridCoord)coord
                                                ofLength:(int)length {
  [_dataSource setRangeOfCharsAnimated:NSMakeRange(coord.x, length)
                                onLine:coord.y];
}
...
- (NSString *)drawingHelperLabelForDropTargetOnLine:(int)line {
  ...
  return [NSString stringWithFormat:@"%@@%@:%@", scpFile.username,
                                    scpFile.hostname, scpFile.path];
}
...
- (NSRange)accessibilityRangeForPosition:(NSPoint)point {
  ...
  DLog(@"Range for position %@ is %@", NSStringFromPoint(point),
...
- (NSRect)accessibilityFrameForRange:(NSRange)range {
  ...
  DLog(@"Frame for range %@ is %@", NSStringFromRange(range),
...
- (NSAttributedString *)accessibilityAttributedStringForRange:(NSRange)range {
  NSAttributedString *string =
      [_accessibilityHelper attributedStringForRange:range];
  DLog(@"Attributed string for range %@ is %@", NSStringFromRange(range),
       string);
  return string;
}
...
- (NSString *)accessibilityDocument {
  NSString *const doc =
      [[_accessibilityHelper currentDocumentURL] absoluteString];
...
- (int)accessibilityHelperAccessibilityLineNumberForLineNumber:
    (int)actualLineNumber {
  int numberOfLines = [_dataSource numberOfLines];
  int offset =
      MAX(0, numberOfLines -
                 [iTermAdvancedSettingsModel numberOfLinesForAccessibility]);
  return actualLineNumber - offset;
}

- (int)accessibilityHelperLineNumberForAccessibilityLineNumber:
    (int)accessibilityLineNumber {
  int numberOfLines = [_dataSource numberOfLines];
  int offset =
      MAX(0, numberOfLines -
                 [iTermAdvancedSettingsModel numberOfLinesForAccessibility]);
...
- (VT100GridCoord)accessibilityHelperCoordForPoint:(NSPoint)screenPosition {
  NSRect screenRect = NSMakeRect(screenPosition.x, screenPosition.y, 0, 0);
  NSRect windowRect = [self.window convertRectFromScreen:screenRect];
  NSPoint locationInTextView = [self convertPoint:windowRect.origin
                                         fromView:nil];
  NSRect visibleRect = [[self enclosingScrollView] documentVisibleRect];
  int x = (locationInTextView.x -
           [iTermPreferences intForKey:kPreferenceKeySideMargins] -
           visibleRect.origin.x) /
          _charWidth;
  int y = locationInTextView.y / _lineHeight;
  return VT100GridCoordMake(
      x, [self accessibilityHelperAccessibilityLineNumberForLineNumber:y]);
}

- (NSRect)accessibilityHelperFrameForCoordRange:
    (VT100GridCoordRange)coordRange {
  coordRange.start.y = [self
      accessibilityHelperLineNumberForAccessibilityLineNumber:coordRange.start
                                                                  .y];
  coordRange.end.y = [self
      accessibilityHelperLineNumberForAccessibilityLineNumber:coordRange.end.y];
  NSRect result = NSMakeRect(
      MAX(0, floor(coordRange.start.x * _charWidth +
                   [iTermPreferences intForKey:kPreferenceKeySideMargins])),
...
- (VT100GridCoord)accessibilityHelperCursorCoord {
  int y = [_dataSource numberOfLines] - [_dataSource height] +
          [_dataSource cursorY] - 1;
  return VT100GridCoordMake(
      [_dataSource cursorX] - 1,
...
- (void)accessibilityHelperSetSelectedRange:(VT100GridCoordRange)coordRange {
  ...
  [_selection clearSelection];
  const long long overflow = _dataSource.totalScrollbackOverflow;
  [_selection beginSelectionAtAbsCoord:VT100GridAbsCoordFromCoord(
                                           coordRange.start, overflow)
                                  mode:kiTermSelectionModeCharacter
                                resume:NO
                                append:NO];
  [_selection moveSelectionEndpointTo:VT100GridAbsCoordFromCoord(coordRange.end,
                                                                 overflow)];
  [_selection endLiveSelection];
}
...
- (VT100GridCoordRange)accessibilityHelperSelectedRange {
  ...
  [self withRelativeCoordRange:sub.absRange.coordRange
                         block:^(VT100GridCoordRange initialCoordRange) {
                           coordRange = initialCoordRange;
                           int minY =
                               _dataSource.numberOfLines - _dataSource.height;
...
- (NSString *)accessibilityHelperSelectedText {
  return [self
      selectedTextWithStyle:iTermCopyTextStylePlainText
               cappedAtSize:0
          minimumLineNumber:
              [self accessibilityHelperLineNumberForAccessibilityLineNumber:0]];
}
...
- (screen_char_t *)accessibilityHelperLineAtIndex:(int)accessibilityIndex {
  return [_dataSource
      getLineAtIndex:
          [self accessibilityHelperLineNumberForAccessibilityLineNumber:
                    accessibilityIndex]];
}
...
- (BOOL)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler
  ...
  if (!_selection.live && [iTermAdvancedSettingsModel typingClearsSelection]) {
    // Remove selection when you type, unless the selection is live because it's
    // handy to be able to scroll up, click, hit a key, and then drag to select
    // to (near) the end. See issue 3340.
    [self deselect];
  }
...
- (void)keyboardHandler:(iTermKeyboardHandler *)keyboardhandler
  ...
  context->hasActionableKeyMapping =
      [_delegate hasActionableKeyMappingForEvent:event];
...
- (NSRange)keyboardHandlerMarkedTextRange:
    (iTermKeyboardHandler *)keyboardhandler {
...
- (VT100GridCoord)mouseHandler:(PTYMouseHandler *)handler
  ...
  const NSPoint temp = [self clickPoint:event
...
- (void)mouseHandlerLockScrolling:(PTYMouseHandler *)handler {
  // Lock auto scrolling while the user is selecting text, but not for a
  // first-mouse event because drags are ignored for those.
  [(PTYScroller *)(
      [[self enclosingScrollView] verticalScroller]) setUserScroll:YES];
}
...
- (BOOL)mouseHandlerIsScrolledToBottom:(PTYMouseHandler *)handler {
  return (([self visibleRect].origin.y + [self visibleRect].size.height -
           [self excess]) /
              _lineHeight ==
          [_dataSource numberOfLines]);
}
...
- (void)mouseHandlerUnlockScrolling:(PTYMouseHandler *)handler {
  [(PTYScroller *)(
      [[self enclosingScrollView] verticalScroller]) setUserScroll:NO];
}
...
- (VT100GridCoord)mouseHandlerCoordForPointInView:(NSPoint)point {
  ...
  VT100GridCoord coord =
      VT100GridCoordMake((point.x - liveRect.origin.x) / _charWidth,
...
- (BOOL)mouseHandler:(PTYMouseHandler *)handler
    viewCoordIsReportable:(NSPoint)point {
...
- (void)mouseHandlerSetFindOnPageCursorCoord:(VT100GridCoord)clickPoint {
  VT100GridAbsCoord absCoord = VT100GridAbsCoordMake(
      clickPoint.x, [_dataSource totalScrollbackOverflow] + clickPoint.y);
...
- (BOOL)mouseHandler:(PTYMouseHandler *)handler
  ...
  const VT100GridAbsCoord findOnPageCursor =
      [_findOnPageHelper findCursorAbsCoord];
...
- (BOOL)mouseHandler:(PTYMouseHandler *)mouseHandler
  ...
  return [self moveSelectionEndpointToX:x
                                      Y:y
...
- (NSString *)mouseHandler:(PTYMouseHandler *)mouseHandler
  ...
  if ([iTermAdvancedSettingsModel alternateMouseScroll]) {
    ...
    *forceLatin1 = NO;
    NSString *string =
        down ? [iTermAdvancedSettingsModel alternateMouseScrollStringForDown]
...
- (CGFloat)mouseHandlerAccumulatedDeltaY:(PTYMouseHandler *)sender
                                forEvent:(NSEvent *)event {
  return [_scrollAccumulator
      deltaYForEvent:event
          lineHeight:self.enclosingScrollView.verticalLineScroll];
}

- (long long)mouseHandlerTotalScrollbackOverflow:
    (nonnull PTYMouseHandler *)sender {
  return _dataSource.totalScrollbackOverflow;
}
