...
@implementation AutocompleteView {
  ...
  // Is there whitespace before the cursor? If so, strip whitespace from before
  // candidates.
...
- (instancetype)init {
  ...
  self = [super
      initWithWindowNibName:@"Autocomplete"
                   tablePtr:nil
...
- (void)appendContextAtX:(int)x
                       y:(int)y
                    into:(NSMutableArray *)context
  ...
  for (int i = 0; i < kMaxIterations && [context count] < maxWords; ++i) {
    ...
    VT100GridWindowedRange range =
        [textExtractor rangeForWordAt:VT100GridCoordMake(x, y)
                        maximumLength:kReasonableMaximumWordLength];
    NSString *s = [textExtractor
    ...
    if ([s rangeOfCharacterFromSet:nonWhitespace].location != NSNotFound) {
      ...
      AcLog(@"Add to context (%d/%d): %@", (int)[context count], (int)maxWords,
            s);
      [context addObject:s];
    }
...
- (void)onOpen {
  ...
  wordSeparatorCharacterSet_ =
      [[iTermTextExtractor wordSeparatorCharacterSet] retain];
  ...
  if (x < 0) {
    ...
    range = [extractor rangeForWordAt:VT100GridCoordMake(x, y)
                        maximumLength:kReasonableMaximumWordLength];
    NSString *s =
        [extractor contentInRange:range
                 attributeProvider:nil
                        nullPolicy:kiTermTextExtractorNullPolicyFromStartToFirst
                               pad:NO
                includeLastNewline:NO
...
- (double)contextSimilarityBetweenQuery:(NSArray *)queryContext
                              andResult:(NSArray *)resultContext {
...
- (double)scoreResultNumber:(int)resultNumber
        ...
        joiningPrefixLength:(int)joiningPrefixLength
                       word:(NSString *)word {
  AcLog(@"Score result #%d with queryContext:%@ and resultContext:%@",
        resultNumber, [self formatContext:queryContext],
        [self formatContext:resultContext]);
  double similarity = [self contextSimilarityBetweenQuery:queryContext
                                                andResult:resultContext] *
                      2;
  ...
  // Strongly penalize very short candidates, because they're not worth
  // completing, unless the context similarity is very strong.
...
- (void)_processPasteboardEntry:(PasteboardEntry *)entry {
  ...
  NSRange range = [value rangeOfString:prefix_
                               options:(NSCaseInsensitiveSearch)];
  if (range.location != NSNotFound) {
    if (range.location > 0) {
      if (![wordSeparatorCharacterSet_
    ...
    // TODO: This is kind of sketchy. We need to look at text before the prefix
    // and lower the score if not much of the prefix was matched.
    double score = [self
          scoreResultNumber:[self _timestampToResultNumber:[entry timestamp]]
...
- (void)_pushStackObject {
  NSArray *value = [NSArray
      arrayWithObjects:[[moreText_ ? moreText_ : @"" copy] autorelease],
...
- (void)_popStackObject {
  ...
  moreText_ =
      [[NSMutableString stringWithString:[value objectAtIndex:0]] retain];
...
- (void)_doPopulateMore {
  ...
  do {
    ...
    AcLog(@"This iteration found %d results in %lf sec",
          (int)[findResults_ count], [[NSDate date] timeIntervalSinceDate:cs]);
    NSDate *ps = [NSDate date];
    int n = 0;
    while ([findResults_ count] > 0 &&
           [[NSDate date] timeIntervalSinceDate:cs] < 0.15) {
      ++n;
      ...
      range = [extractor rangeForWordAt:VT100GridCoordMake(startX, startY)
                          maximumLength:kReasonableMaximumWordLength];
      NSString *immutableWord = [extractor
                  contentInRange:range
               attributeProvider:nil
                      nullPolicy:kiTermTextExtractorNullPolicyFromStartToFirst
                             pad:NO
          ...
          trimTrailingWhitespace:NO
               ...
               continuationChars:nil
                          coords:nil];
      NSMutableString *firstWord =
          [NSMutableString stringWithString:immutableWord];
      while ([firstWord length] < [prefix_ length]) {
        range = [extractor rangeForWordAt:range.coordRange.end
                            maximumLength:kReasonableMaximumWordLength];
        NSString *part = [extractor
                    contentInRange:range
                 attributeProvider:nil
                        nullPolicy:kiTermTextExtractorNullPolicyFromStartToFirst
                               pad:NO
      ...
      NSRange wordRange =
          [word rangeOfString:prefix_
                      options:(NSCaseInsensitiveSearch | NSAnchoredSearch)];
      if (wordRange.location == 0) {
        // Result has prefix_ as prefix.
        // Set fullMatch to true if the word we found is equal to prefix, or
        // false if word just has prefix as its prefix.
        BOOL fullMatch = (wordRange.length == [word length]);

        // Grab the context before the match.
        NSMutableArray *resultContext =
            [NSMutableArray arrayWithCapacity:kMaxResultContextWords];
        AcLog(@"Word before what we want is in x=[%d to %d]", startX, endX);
        [self appendContextAtX:startX
                             y:(int)startY
                          into:resultContext
                      maxWords:kMaxResultContextWords];

        if (fullMatch) {
          // Grab the word after the match (presumably containing non-word
          // characters)
          ++endX;
          ...
          word =
              [extractor contentInRange:range
                  ...
                  trimTrailingWhitespace:NO
                       ...
                       continuationChars:nil
                                  coords:nil];
          AcLog(@"First candidate is at %@",
                VT100GridWindowedRangeDescription(range));
          if ([word rangeOfCharacterFromSet:nonWhitespace].location ==
              NSNotFound) {
          ...
          // Get suffix of word after match. If there's whitespace before the
          // cursor then only full matches are interesting.
          word = [word
              substringWithRange:NSMakeRange(wordRange.length,
                                             [word length] - wordRange.length)];
        } else {
        ...
        if ([word rangeOfCharacterFromSet:nonWhitespace].location !=
          ...
          PopupEntry *e = [PopupEntry
              entryWithString:word
        ...
        AcLog(@"Search found %@ which doesn't start the same as our search "
              @"term %@",
              word, prefix_);
        x_ = startX;
        y_ = startY + [screen scrollbackOverflow];
    ...
    AcLog(@"This iteration processed %d results in %lf sec", n,
          [[NSDate date] timeIntervalSinceDate:ps]);
...
- (void)addCommandEntries:(NSArray<iTermCommandHistoryEntryMO *> *)entries
  ...
  for (iTermCommandHistoryEntryMO *entry in entries) {
    ...
    NSArray *parts = [command
        componentsSeparatedByCharactersInSet:[iTermTextExtractor
                                                 wordSeparatorCharacterSet]];
    if ([parts count] == 0) {
      continue;
    }
    command = parts[0];
    ...
    double score =
        [self scoreResultNumber:i++
                   queryContext:context_
                  resultContext:context_ // Maximize similarity because the
                                         // whole prompt is in our favor
            joiningPrefixLength:[prefix_ length]
                           word:command];
    ...
    score = MIN(10, score); // Limit score of commands so really relevant
...
- (iTermTextExtractor *)textExtractor {
  // TODO: It would be nice to support windowed ranges, but it will hurt
  // performance and not provide a huge benefit.
  return [iTermTextExtractor
      textExtractorWithDataSource:[[self delegate] popupVT100Screen]];
}
