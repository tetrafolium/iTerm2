...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p path=%@ variables=%@>",
...
@implementation iTermVariableScope {
  ...
  // References to paths without an owner. This normally only happens when a
  // session is being shut down (e.g, tab.currentSession is assigned to nil)
...
- (void)addVariables:(iTermVariables *)variables
        toScopeNamed:(nullable NSString *)scopeName {
  [_frames insertObject:[iTermTuple tupleWithObject:scopeName
                                          andObject:variables]
                atIndex:0];
...
- (void)removeFrameWithName:(NSString *)name {
  [_frames removeObjectsPassingTest:^BOOL(
               iTermTuple<NSString *, iTermVariables *> *obj) {
    return [obj.firstObject isEqual:name];
  }];
}

- (NSArray<iTermVariables *> *)variablesInScopeNamed:
    (nullable NSString *)scopeName {
  return [_frames
      mapWithBlock:^id(iTermTuple<NSString *, iTermVariables *> *anObject) {
...
- (void)enumerateVariables:(void (^)(NSString *_Nonnull,
                                     iTermVariables *_Nonnull))block {
  [_frames enumerateObjectsUsingBlock:^(
               iTermTuple<NSString *, iTermVariables *> *_Nonnull obj,
...
- (NSDictionary<NSString *, NSString *> *)dictionaryWithStringValues {
  ...
  [self enumerateVariables:^(NSString *_Nonnull scopeName,
                             iTermVariables *_Nonnull variables) {
...
- (id)valueForPath:(NSString *)firstName, ... {
  ...
  for (NSString *name = firstName; name != nil;
       name = va_arg(args, NSString *)) {
...
- (id)valueForVariableName:(NSString *)name {
  ...
  iTermVariables *owner = [self ownerForKey:name
                                 forWriting:NO
                                   stripped:&stripped];
...
- (nullable iTermVariables *)ownerForKey:(NSString *)key
  ...
  if (parts.count == 1) {
    *stripped = key;
    ...
    return
        [_frames objectPassingTest:^BOOL(
                     iTermTuple<NSString *, iTermVariables *> *element,
  ...
  iTermVariables *owner =
      [_frames objectPassingTest:^BOOL(
                   iTermTuple<NSString *, iTermVariables *> *element,
                   NSUInteger index, BOOL *stop) {
        if (element.firstObject == nil &&
            [element.secondObject valueForVariableName:parts[0]]) {
          strippedOut = key;
          return YES;
        } else {
          strippedOut =
              [[parts subarrayFromIndex:1] componentsJoinedByString:@"."];
...
- (nullable iTermVariables *)ownerOfTerminalForPath:(NSString *)path
  ...
  NSString *stripped;
  iTermVariables *owner = [self ownerForKey:path
  ...
  while (parts.count > 1) {
    ...
    ITAssertWithMessage([value isKindOfClass:[iTermVariables class]],
                        @"Value is not iTermVariables. It is %@. The path is "
...
- (BOOL)variableNamed:(NSString *)name
       isReferencedBy:(iTermVariableReference *)reference {
  NSString *tail;
  iTermVariables *variables = [self ownerForKey:name
                                     forWriting:NO
                                       stripped:&tail];
  if (!variables) {
    return NO;
...
- (void)setValuesFromDictionary:(NSDictionary<NSString *, id> *)dict {
  ...
  NSMutableDictionary<NSValue *, NSMutableDictionary<NSString *, id> *>
      *valuesByOwner = [NSMutableDictionary dictionary];
  for (NSString *key in dict) {
    id object = dict[key];
    NSString *stripped = nil;
    iTermVariables *owner = [self ownerForKey:key
                                   forWriting:YES
                                     stripped:&stripped];
...
- (BOOL)setValue:(nullable id)value forPath:(NSString *)firstName, ... {
  ...
  return [self setValue:value
       forVariableNamed:[parts componentsJoinedByString:@"."]];
}
...
- (BOOL)setValue:(nullable id)value
    forVariableNamed:(NSString *)name
                weak:(BOOL)weak {
  assert(![value isKindOfClass:[iTermVariableScope
                                   class]]); // You meant to use iTermVariables,
                                             // not iTermVariableScope.
  ...
  const BOOL result = [owner setValue:value
                     forVariableNamed:stripped
                                 weak:weak];
...
- (void)addLinksToReference:(id<iTermVariableReference>)reference {
  NSString *tail;
  iTermVariables *variables = [self ownerForKey:reference.path
                                     forWriting:YES
                                       stripped:&tail];
  if (!variables) {
...
- (iTermVariableRecordingScope *)recordingCopy {
  iTermVariableRecordingScope *theCopy =
      [[iTermVariableRecordingScope alloc] initWithScope:self];
  [_frames enumerateObjectsUsingBlock:^(
...
- (iTermVariableDesignator *)designatorForPath:(NSString *)path {
  ...
  iTermVariables *owner = [self ownerOfTerminalForPath:path
                                            forWriting:NO
                                              terminal:&terminal];
  return [[iTermVariableDesignator alloc] initWithVariables:owner
                                                       path:terminal];
}
...
- (NSArray<iTermVariableReference *> *)recordedReferences {
  return [_names.allObjects mapWithBlock:^id(NSString *path) {
    return [[iTermVariableReference alloc] initWithPath:path
                                                 vendor:self->_scope];
  }];
}
@end
