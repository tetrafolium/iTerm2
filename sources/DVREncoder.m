...
- (NSString *)stringForFrameLines:(NSArray *)lines
                            width:(int)width
                           height:(int)height {
...
- (void)appendFrame:(NSArray *)frameLines
  ...
  if (cleanLines.count > info->height * 0.8 && lastFrame_ &&
      length == [lastFrame_ length] && info->width == lastInfo_.width &&
  ...
  if (!eligibleForDiff || count_++ % kKeyFrameFrequency == 0) {
    ...
    [self _appendDiffFrame:frameLines
                    length:length
...
- (void)_appendKeyFrame:(NSArray *)frameLines
  ...
  [self _appendFrameImpl:scratch
                  length:length
                    type:DVRFrameTypeKeyFrame
                    info:info];
...
- (void)_appendDiffFrame:(NSArray *)frameLines
  ...
  [self _appendFrameImpl:scratch
                  length:diffBytes
                    type:DVRFrameTypeDiffFrame
                    info:info];
  bytesSinceLastKeyFrame_ += diffBytes;
}

// Save a frame into DVRBuffer.
- (void)_appendFrameImpl:(char *)dest
                  length:(int)length
                    type:(DVRFrameType)type
                    info:(DVRFrameInfo *)info {
  assert(haveReservation_);
  haveReservation_ = NO;

#ifdef DVRDEBUG
  NSLog(@"Encoder: Append frame of type %d starting at offset %d length %d at "
        @"index %lld",
        (int)type, (int)[buffer_ offsetOfPointer:dest], length,
        [buffer_ lastKey] + 1);
#endif
  ...
  DLog(@"Append frame with key %lld, size %dx%d", key, info->width,
       info->height);
}

// Calculate the diff between buffer,length and the previous frame. Saves
// results into scratch. Won't use more than maxSize bytes in scratch. Returns
// number of bytes used or -1 if the diff was larger than maxSize.
...
#ifdef DVRDEBUG
      NSLog(@"Append samesequence at offset %@, address %p. No data is "
            @"appended for samesequence.",
            @(o), scratch + o);
#endif
...
#ifdef DVRDEBUG
      NSLog(@"Append diffsequence at offset %@ of length %@", @(o),
            @(numChars));
#endif
