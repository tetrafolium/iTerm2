...
+ (id)quotedRecogniserWithStartQuote:(NSString *)startQuote
                            endQuote:(NSString *)endQuote
                                name:(NSString *)name {
  return [self quotedRecogniserWithStartQuote:startQuote
                                     endQuote:endQuote
                               escapeSequence:nil
                                         name:name];
}
...
- (CPToken *)recogniseTokenInString:(NSString *)tokenString
  ...
  CFRange searchRange =
      CFRangeMake(*tokenPosition,
                  MIN(inputLength - *tokenPosition,
                      startQuoteLength + endQuoteLength + self.maximumLength));
  CFRange range;
  BOOL matched = CFStringFindWithOptions(
      (CFStringRef)tokenString, (CFStringRef)self.startQuote, searchRange,
      kCFCompareAnchored, &range);
  ...
  if (matched) {
    ...
    CFRange endRange;
    CFRange escapeRange;
    BOOL matchedEndSequence = CFStringFindWithOptions(
        (CFStringRef)tokenString, (CFStringRef)self.endQuote, searchRange, 0L,
        &endRange);
    BOOL matchedEscapeSequence =
        nil == self.escapeSequence
            ? NO
    ...
    while (matchedEndSequence && searchRange.location < inputLength) {
      if (!matchedEscapeSequence || endRange.location < escapeRange.location) {
        *tokenPosition = endRange.location + endRange.length;
        ...
        CFRelease(substr);
        CPQuotedToken *t =
            [CPQuotedToken content:(__bridge NSString *)outputString
        ...
        if (endRange.location < searchRange.location) {
          matchedEndSequence = CFStringFindWithOptions(
              (CFStringRef)tokenString, (CFStringRef)self.endQuote, searchRange,
              0L, &endRange);
        }
        if (escapeRange.location < searchRange.location) {
          matchedEscapeSequence = CFStringFindWithOptions(
              (CFStringRef)tokenString, (CFStringRef)self.escapeSequence,
              searchRange, 0L, &escapeRange);
        }
      }
    }
    ...
    *tokenPosition = searchRange.location + searchRange.length;
    CFStringRef substr = CFStringCreateWithSubstring(
        kCFAllocatorDefault, (CFStringRef)tokenString, searchRange);
    CFStringAppend(outputString, substr);
    CFRelease(substr);
