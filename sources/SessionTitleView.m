...
// Prevent that from happening. Bug 1924. This is just an experiment to see if
// it works (4/16/13)
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    closeButton_ = [[NoFirstResponderButton alloc]
        initWithFrame:NSMakeRect(x, (frame.size.height - kButtonSize) / 2,
    ...
    // Popup buttons want to have huge margins on the sides. This one look best
    // right up against the right margin, though. So I'll make it as small as it
    // can be and then push it right so some of it is clipped.
    ...
    menuButton_.frame =
        NSMakeRect(frame.size.width - menuButton_.image.size.width - 6,
    ...
    label_ = [[NSTextField alloc]
        initWithFrame:NSMakeRect(x, 0, menuButton_.frame.origin.x - x - kMargin,
                                 frame.size.height)];
    ...
    lframe.origin.y +=
        (frame.size.height - lframe.size.height) / 2 + kBottomMargin;
...
- (void)setStatusBarViewController:
...
- (void)layoutStatusBar {
  if (_statusBarViewController) {
    ...
    _statusBarViewController.view.frame =
        NSMakeRect(minX, 1, NSMinX(menuButton_.frame) - margin - minX,
...
- (void)openMenu:(id)sender {
  [NSMenu popUpContextMenu:delegate_.menu
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  if (!self.window.ptyWindow.it_terminalWindowUseMinimalStyle) {
    ...
    NSRectFillUsingOperation(
        NSMakeRect(dirtyRect.origin.x, 0, dirtyRect.size.width, 1),
...
- (void)updateTextColor {
  ...
  if (self.window.ptyWindow.it_terminalWindowUseMinimalStyle) {
    label_.textColor = [self.window.ptyWindow
        it_terminalWindowDecorationTextColorForBackgroundColor:
            [delegate_ sessionTitleViewBackgroundColor]];
    [self setNeedsDisplay:YES];
    return;
  }
