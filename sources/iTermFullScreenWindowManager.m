...
// Only one window can enter full screen mode at a time. This ensures it is done
// safely when opening multiple windows.
...
- (void)windowWillTransition:(NSNotification *)notification {
  ...
  DLog(@"Window will enter/exit full screen. Count is now %ld",
       _numberOfTransitions);
}
...
- (void)windowDidTransition:(NSNotification *)notification {
  DLog(@"%@", notification);
  --_numberOfTransitions;
  DLog(@"Window did enter/exit full screen. Count is now %ld",
       _numberOfTransitions);
...
- (void)transitionNextWindowInQueue {
  ...
  while (_queue.count) {
    iTermFullScreenTransition *transition =
        [[_queue.firstObject retain] autorelease];
...
// Returns YES if the window is already in the queue. Removes it if its
// `enterFullScreen` equals `ifEntering`.
- (BOOL)haveTransitionWithWindow:(NSWindow *)window
                removeIfEntering:(BOOL)ifEntering {
  NSInteger index = [_queue
      indexOfObjectPassingTest:^BOOL(iTermFullScreenTransition *_Nonnull obj,
...
- (void)enqueueWindow:(NSWindow<iTermWeakReference> *)window enter:(BOOL)enter {
  iTermFullScreenTransition *transition =
      [[[iTermFullScreenTransition alloc] init] autorelease];
