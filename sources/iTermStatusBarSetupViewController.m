...
@interface iTermStatusBarSetupViewController () <
...
@implementation iTermStatusBarSetupViewController {
  ...
  IBOutlet iTermStatusBarSetupDestinationCollectionViewController
...
- (nullable instancetype)initWithLayoutDictionary:
                             (NSDictionary *)layoutDictionary
                                   darkBackground:(BOOL)darkBackground
                                     allowRainbow:(BOOL)allowRainbow {
  self = [super initWithNibName:@"iTermStatusBarSetupViewController"
...
- (iTermStatusBarSetupElement *)newElementForProviderRegistrationRequest:
    (ITMRPCRegistrationRequest *)request {
  iTermStatusBarRPCComponentFactory *factory =
      [[iTermStatusBarRPCComponentFactory alloc]
          initWithRegistrationRequest:request];
  return [[iTermStatusBarSetupElement alloc]
      initWithComponentFactory:factory
...
- (void)loadElements {
  ...
  _elements = [classes mapWithBlock:^id(Class theClass) {
    ...
    return [[iTermStatusBarSetupElement alloc]
        initWithComponentFactory:factory
                 layoutAlgorithm:self->_layout.advancedConfiguration
                                     .layoutAlgorithm
                           knobs:factory.defaultKnobs];
  }];
}
...
- (void)awakeFromNib {
  _destinationViewController.defaultBackgroundColor =
  ...
  [self loadElements];
  for (ITMRPCRegistrationRequest *request in iTermAPIHelper
           .statusBarComponentProviderRegistrationRequests) {
    iTermStatusBarSetupElement *element =
        [self newElementForProviderRegistrationRequest:request];
    if (element) {
  ...
  [_collectionView setDraggingSourceOperationMask:NSDragOperationCopy
                                         forLocal:YES];
  _collectionView.selectable = YES;

  [_autoRainbow
      selectItemWithTag:_layout.advancedConfiguration.autoRainbowStyle];

  _destinationViewController.darkBackground = _darkBackground;
  _destinationViewController.advancedConfiguration =
      _layout.advancedConfiguration;
  ...
  [self setFont:_layout.advancedConfiguration.font
...
- (void)initializeUI {
  ...
  _removeEmptyComponents.state =
      _layout.advancedConfiguration.removeEmptyComponents
...
- (NSView *)fontPanelAccessory {
  ...
  button.autoresizingMask = (NSViewMinXMargin | NSViewMaxXMargin |
...
- (IBAction)openFontPanel:(id)sender {
  ...
  NSFont *theFont = _layout.advancedConfiguration.font
                        ?: [iTermStatusBarAdvancedConfiguration defaultFont];
...
- (void)changeFont:(nullable id)sender {
  NSFont *font = [sender
      convertFont:_layout.advancedConfiguration.font
                      ?: [iTermStatusBarAdvancedConfiguration defaultFont]];
  [self setFont:font];
}
...
- (void)setFont:(NSFont *)font {
  ...
  _fontLabel.stringValue =
      [NSString stringWithFormat:@"%@pt %@", @(font.pointSize), font.fontName];
}
...
- (void)apply {
  ...
  const NSInteger tag =
      _destinationViewController.advancedConfiguration.autoRainbowStyle;
...
- (IBAction)advanced:(id)sender {
  __weak __typeof(self) weakSelf = self;
  [self.view.window beginSheet:_advancedPanel
...
- (IBAction)autoRainbow:(id)sender {
  ...
  iTermStatusBarAdvancedConfiguration *config =
      [_destinationViewController.advancedConfiguration copy];
...
- (IBAction)help:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"https://iterm2.com/status-bar-layout"]];
}
...
- (void)advancedPanelDidClose {
  ...
  _layout.advancedConfiguration.layoutAlgorithm =
      (_tightPacking.state == NSControlStateValueOn)
          ? iTermStatusBarLayoutAlgorithmSettingTightlyPacked
          : iTermStatusBarLayoutAlgorithmSettingStable;
  _layout.advancedConfiguration.removeEmptyComponents =
      (_removeEmptyComponents.state == NSControlStateValueOn);
  _layout.delegate = nil;

  _layout =
      [[iTermStatusBarLayout alloc] initWithDictionary:_layout.dictionaryValue
                                                 scope:nil];
...
- (iTermStatusBarSetupCollectionViewItem *)newItemWithIndexPath:
  ...
  iTermStatusBarSetupCollectionViewItem *item =
      [_collectionView makeItemWithIdentifier:@"element"
...
- (void)initializeItem:(iTermStatusBarSetupCollectionViewItem *)item
  ...
  item.textField.attributedStringValue = [_elements[index]
      exemplarWithBackgroundColor:_layout.advancedConfiguration.backgroundColor
...
- (NSImage *)collectionView:(NSCollectionView *)collectionView
  ...
  iTermStatusBarSetupCollectionViewItem *item =
      [self newItemWithIndexPath:indexPaths.anyObject];
  item.hideDetail = YES;
  [item sizeToFit];
  assert(item);

  iTermStatusBarSetupCollectionViewItem *originalItem =
      [iTermStatusBarSetupCollectionViewItem
          castFrom:[collectionView itemAtIndexPath:indexPaths.anyObject]];

  NSPoint locationInItem = [originalItem.view convertPoint:locationInWindow
                                                  fromView:nil];
  NSPoint center = NSMakePoint(originalItem.view.frame.size.width / 2,
                               originalItem.view.frame.size.height / 2);
  const CGFloat heightDifference =
      originalItem.view.frame.size.height - item.view.frame.size.height;
  *dragImageOffset =
      NSMakePoint(center.x - locationInItem.x,
                  center.y - locationInItem.y + heightDifference / 2);

  NSVisualEffectView *vev =
      [[NSVisualEffectView alloc] initWithFrame:item.view.bounds];
