...
@interface iTermHotkeyPreferencesWindowController () <
    iTermShortcutInputViewDelegate, NSTableViewDelegate, NSTableViewDataSource>
...
@implementation iTermHotkeyPreferencesWindowController {
  ...
  NSMutableArray<iTermShortcut *>
      *_mutableShortcuts; // Model for _tableView. Only nonnil while additional
                          // shortcuts sheet is open.
}
...
- (void)updateViewsEnabled {
  NSArray<NSView *> *buttons = @[
    _pinned, _showAutoHiddenWindowOnAppActivation, _animate, _floats,
    _doNotShowOnDockClick, _alwaysShowOnDockClick,
    _showIfNoWindowsOpenOnDockClick
  ];
  for (NSButton *button in buttons) {
    button.enabled = self.model.hotKeyAssigned;
  }
  _duplicateWarning.hidden = ![self.descriptorsInUseByOtherProfiles
      containsObject:self.model.primaryShortcut.descriptor];
  _duplicateWarningForModifierActivation.hidden =
      ![self.descriptorsInUseByOtherProfiles
          containsObject:self.modifierActivationDescriptor];
  _showAutoHiddenWindowOnAppActivation.enabled =
      (self.model.hotKeyAssigned && _pinned.state == NSControlStateValueOff);
  _modifierActivation.enabled =
      (_activateWithModifier.state == NSControlStateValueOn);
...
- (iTermHotKeyDescriptor *)modifierActivationDescriptor {
  if (self.model.hasModifierActivation) {
    return [iTermHotKeyDescriptor
        descriptorWithModifierActivation:self.model.modifierActivation];
  } else {
    return nil;
  }
}
...
- (void)modelDidChange {
  _activateWithModifier.state = _model.hasModifierActivation
  ...
  _pinned.state =
      _model.autoHide ? NSControlStateValueOff : NSControlStateValueOn;
  _showAutoHiddenWindowOnAppActivation.enabled = _model.autoHide;
  _showAutoHiddenWindowOnAppActivation.state =
      _model.showAutoHiddenWindowOnAppActivation ? NSControlStateValueOn
                                                 : NSControlStateValueOff;
  _animate.state =
      _model.animate ? NSControlStateValueOn : NSControlStateValueOff;
  _floats.state =
      _model.floats ? NSControlStateValueOn : NSControlStateValueOff;
...
- (IBAction)settingChanged:(id)sender {
  _model.hasModifierActivation =
      _activateWithModifier.state == NSControlStateValueOn;
  _model.modifierActivation = [_modifierActivation selectedTag];

  _model.autoHide = _pinned.state == NSControlStateValueOff;
  _model.showAutoHiddenWindowOnAppActivation =
      _showAutoHiddenWindowOnAppActivation.state == NSControlStateValueOn;
...
- (IBAction)editAdditionalHotKeys:(id)sender {
  ...
  [self.window beginSheet:_editAdditionalWindow
        completionHandler:^(NSModalResponse returnCode) {
          __strong __typeof(self) strongSelf = weakSelf;
          if (!strongSelf) {
            return;
          }
          self.model.alternateShortcuts = [strongSelf->_mutableShortcuts
              filteredArrayUsingBlock:^BOOL(iTermShortcut *shortcut) {
...
- (void)shortcutInputView:(iTermShortcutInputView *)view
  ...
  if (!event && _model.alternateShortcuts.count) {
    ...
    _model.alternateShortcuts =
        [_model.alternateShortcuts arrayByRemovingFirstObject];
    [self modelDidChange];
  } else {
    _model.primaryShortcut =
        event ? [iTermShortcut shortcutWithEvent:event] : nil;
    [self modelDidChange];
  }
  [self updateViewsEnabled];
}
...
- (id)tableView:(NSTableView *)tableView
    objectValueForTableColumn:(NSTableColumn *)tableColumn
                          row:(NSInteger)row {
  return [iTermAdditionalHotKeyObjectValue
      objectValueWithShortcut:_mutableShortcuts[row]
