...
- (BOOL)encodeChildWithKey:(NSString *)key
  ...
  iTermGraphEncoder *encoder =
      [[iTermGraphEncoder alloc] initWithKey:key
...
- (void)encodeArrayWithKey:(NSString *)key
  ...
  if (identifiers.count != [NSSet setWithArray:identifiers].count) {
    ITBetaAssert(NO, @"Identifiers for %@ contains a duplicate: %@", key,
                 identifiers);
  }
  [self encodeChildWithKey:@"__array"
                ...
                generation:generation
                     block:^BOOL(iTermGraphEncoder *_Nonnull subencoder) {
                       ...
                       [identifiers enumerateObjectsUsingBlock:^(
                       ...
                       orderedIdentifiers =
                           [orderedIdentifiers arrayByRemovingDuplicatesStably];
                       [subencoder
                           encodeString:[orderedIdentifiers
...
- (void)transaction:(BOOL (^)(void))block {
  ...
  NSMutableArray<iTermEncoderGraphRecord *> *savedChildren =
