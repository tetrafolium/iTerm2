...
NSString *const kTmuxControllerSessionsWillChange =
    @"kTmuxControllerSessionsWillChange";
NSString *const kTmuxControllerSessionsDidChange =
    @"kTmuxControllerSessionsDidChange";
...
NSString *const kTmuxControllerWindowWasRenamed =
    @"kTmuxControllerWindowWasRenamed";
...
NSString *const kTmuxControllerWindowDidClose =
    @"kTmuxControllerWindowDidClose";
NSString *const kTmuxControllerSessionWasRenamed =
    @"kTmuxControllerSessionWasRenamed";
NSString *const kTmuxControllerDidFetchSetTitlesStringOption =
    @"kTmuxControllerDidFetchSetTitlesStringOption";
NSString *const iTermTmuxControllerWillKillWindow =
    @"iTermTmuxControllerWillKillWindow";
NSString *const kTmuxControllerDidChangeHiddenWindows =
    @"kTmuxControllerDidChangeHiddenWindows";
...
static NSString *const iTermTmuxControllerSplitStateInitialPanes =
    @"initial panes";
...
@implementation TmuxController {
  ...
  NSMutableDictionary<NSNumber *, iTermTmuxWindowState *>
      *_windowStates; // Key is window number
  ...
  NSTimer *listSessionsTimer_; // Used to do a cancelable delayed perform of
                               // listSessions.
  ...
  // Maps a window id string to a dictionary of window flags defined by
  // TmuxWindowOpener (see the top of its header file)
  NSMutableDictionary *_windowOpenerOptions;
  BOOL _manualOpenRequested;
  BOOL _haveOpenedInitialWindows;
  ProfileModel *_profileModel;
  // Maps the window ID of an about to be opened window to a completion block to
  // invoke when it opens.
  NSMutableDictionary<NSNumber *, void (^)(int)> *_pendingWindows;
  BOOL _hasStatusBar;
  int _currentWindowID; // -1 if undefined
  // Pane -> (Key -> Value)
  NSMutableDictionary<NSNumber *, NSMutableDictionary<NSString *, NSString *> *>
      *_userVars;
  NSMutableDictionary<
      NSNumber *, void (^)(PTYSession<iTermTmuxControllerSession> *)> *_when;
  NSMutableArray *_listWindowsQueue;
  // If nonnegative, make this pane active after it comes in to being. If
  // negative, invalid.
  int _paneToActivateWhenCreated;
  iTermTmuxBufferSizeMonitor *_tmuxBufferMonitor;
  NSMutableDictionary<NSNumber *, NSValue *>
      *_windowSizes; // window -> NSValue cell size
  BOOL _versionDetected;
  // terminal guid -> [(tmux window id, tab index), ...]
  NSMutableDictionary<NSString *,
                      NSMutableArray<iTermTuple<NSNumber *, NSNumber *> *> *>
      *_buriedWindows;
...
static NSDictionary *
iTermTmuxControllerDefaultFontOverridesFromProfile(Profile *profile) {
  return @{
    KEY_NORMAL_FONT : [iTermProfilePreferences stringForKey:KEY_NORMAL_FONT
                                                  inProfile:profile],
    KEY_NON_ASCII_FONT :
        [iTermProfilePreferences stringForKey:KEY_NON_ASCII_FONT
                                    inProfile:profile],
    KEY_HORIZONTAL_SPACING :
        [iTermProfilePreferences objectForKey:KEY_HORIZONTAL_SPACING
                                    inProfile:profile],
    KEY_VERTICAL_SPACING :
        [iTermProfilePreferences objectForKey:KEY_VERTICAL_SPACING
                                    inProfile:profile]
  };
}
...
- (instancetype)initWithGateway:(TmuxGateway *)gateway
  ...
  if (self) {
    ...
    _sharedFontOverrides =
        [iTermTmuxControllerDefaultFontOverridesFromProfile(profile) retain];
    ...
    self.clientName = [[TmuxControllerRegistry sharedInstance]
        uniqueClientNameBasedOn:clientName];
    ...
    [iTermPreferenceDidChangeNotification
        subscribe:self
            block:^(
                iTermPreferenceDidChangeNotification *_Nonnull notification) {
              if ([notification.key
                      isEqualToString:kPreferenceKeyTmuxPauseModeAgeLimit]) {
...
- (NSDictionary *)sharedProfile {
  Profile *profile = [_profileModel bookmarkWithGuid:_sharedProfile[KEY_GUID]]
                         ?: _sharedProfile;
...
// Called when listing window finishes. Happens for all new windows/tabs,
// whether initiated by iTerm2 or not.
- (void)openWindowWithIndex:(int)windowIndex
  ...
  windowOpener.zoomed =
      windowFlags ? @([windowFlags containsString:@"Z"]) : nil;
...
// When we attach we get affinities with terminal GUIDs that may no longer
// exist. The GUIDs get rewritten after creating the window for the first tab.
// For restored sessions it just works because 2nd through Nth tabs can find
// their comrades through their affinity with its window ID. For buried
// sessions, we must rewrite the terminal GUID since it has no affinity with
// other tabs by window ID.
...
- (void)adjustWindowSizeIfNeededForTabs:(NSArray<PTYTab *> *)tabs {
  ...
  const BOOL outstandingResize = [tabs anyWithBlock:^BOOL(PTYTab *tab) {
    return [[[tab realParentWindow] uniqueTmuxControllers]
        anyWithBlock:^BOOL(TmuxController *controller) {
          return [controller hasOutstandingWindowResize];
        }];
  }];
  if (outstandingResize) {
    DLog(@"One of the tabs has a tmux controller with an outstanding window "
         @"resize. Don't update layouts.");
    return;
  }
  // If there are no outstanding window resizes then
  // setTmuxLayout:tmuxController: has called fitWindowToTabs:, and it's still
  // too big, so shrink the layout.
...
- (void)sessionChangedTo:(NSString *)newSessionName sessionId:(int)sessionid {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerAttachedSessionDidChange
                    object:nil];
}
...
- (void)sessionsChanged {
  ...
  // Wait a few seconds. We always get a sessions-changed notification when the
  // last window in a session closes. To avoid spamming the command line with
  // list-sessions, we wait a bit to see if there is an exit notification coming
  // down the pipe.
  const CGFloat kListSessionsDelay = 1.5;
  [listSessionsTimer_ invalidate];
  listSessionsTimer_ =
      [NSTimer scheduledTimerWithTimeInterval:kListSessionsDelay
                                       target:self
...
- (void)session:(int)sessionId renamedTo:(NSString *)newName {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerSessionWasRenamed
                    object:@[ @(sessionId), newName ?: @"", self ]];
}

- (void)windowWasRenamedWithId:(int)wid to:(NSString *)newName {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerWindowWasRenamed
                    object:@[ @(wid), newName ?: @"", self ]];
}

- (void)windowsChanged {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerWindowsChangeNotification
                    object:self];
}

- (NSArray *)listWindowFields {
  return [NSArray arrayWithObjects:@"session_name", @"window_id",
                                   @"window_name", @"window_width",
                                   @"window_height", @"window_layout",
                                   @"window_flags", @"window_active", nil];
}
...
- (void)initialListWindowsResponse:(NSString *)response {
  ...
  if (!doc) {
    ...
    [gateway_
        abortWithErrorMessage:[NSString
                                  stringWithFormat:@"Bad response for initial "
  ...
  const iTermOpenTmuxWindowsMode openWindowsMode =
      [iTermPreferences intForKey:kPreferenceKeyOpenTmuxWindowsIn];
  const BOOL newWindowsInTabs =
      openWindowsMode == kOpenTmuxWindowsAsNativeTabsInNewWindow;
  DLog(@"Iterating records...");
  for (NSArray *record in doc.records) {
    DLog(@"Consider record %@", record);
    const int wid = [self windowIdFromString:[doc valueInRecord:record
                                                       forField:@"window_id"]];
    if (hiddenWindows_ &&
    ...
    if (![affinities_ valuesEqualTo:[n stringValue]] && newWindowsInTabs) {
      ...
      if (!newWindowAffinity) {
        ...
        DLog(@"Add window id %@ to existing affinity class %@", n,
             [newWindowAffinity stringValue]);
  ...
  if (windowsToOpen.count >
    ...
    tooMany = YES;
    // Save that these windows are hidden so the UI will be consistent next time
    // you attach.
    NSArray<NSNumber *> *wids =
        [windowsToOpen mapWithBlock:^NSNumber *(NSArray *record) {
          const int wid =
              [self windowIdFromString:[doc valueInRecord:record
                                                 forField:@"window_id"]];
          return @(wid);
        }];
  ...
  [[TmuxDashboardController sharedInstance]
      didAttachWithHiddenWindows:haveHidden
                  tooManyWindows:tooMany];
  if (tooMany) {
    [[iTermNotificationController sharedInstance]
                 notify:@"Too many tmux windows!"
        withDescription:@"Use the tmux dashboard to select which to open."];
  } else if (haveHidden) {
    [[iTermNotificationController sharedInstance]
                 notify:@"Some tmux windows were hidden."
        withDescription:@"Use the tmux dashboard to select which to open."];
  }
  for (NSArray *record in windowsToOpen) {
    DLog(@"Open window %@", record);
    int wid = [self windowIdFromString:[doc valueInRecord:record
                                                 forField:@"window_id"]];
    [self
        openWindowWithIndex:wid
                       name:[[doc valueInRecord:record forField:@"window_name"]
                                it_unescapedTmuxWindowName]
                       size:NSMakeSize(
                                [[doc valueInRecord:record
                                           forField:@"window_width"] intValue],
                                [[doc valueInRecord:record
                                           forField:@"window_height"] intValue])
                     layout:[doc valueInRecord:record forField:@"window_layout"]
                 affinities:[self
                                savedAffinitiesForWindow:[NSString
                                                             stringWithInt:wid]]
  ...
  if (windowsToOpen.count == 0) {
    DLog(@"Did not open any windows so turn on accept notifications in tmux "
         @"gateway");
...
- (void)openWindowsInitial {
  NSString *command =
      [NSString stringWithFormat:@"show -v -q -t $%d @iterm2_size", sessionId_];
...
- (void)handleShowSize:(NSString *)response {
  ...
  int height = 0;
  BOOL ok =
      ([scanner scanInt:&width] && [scanner scanString:@"," intoString:nil] &&
...
- (void)openWindowsOfSize:(VT100GridSize)size {
  ...
  // There's a (hopefully) minor race condition here. When we initially connect
  // to a session we get its @iterm2_id. If one doesn't exist, it is assigned.
  // This lets us know if a single instance of iTerm2 is trying to attach to the
  // same session twice. A really evil user could attach twice to the same
  // session simultaneously, and we'd get the value twice, see it's empty twice,
  // and set it twice, causing chaos. Or two separate instances of iTerm2
  // attaching simultaneously could also hit this race. The consequence of this
  // race condition is easily recovered from by reattaching.
  [_windowSizes removeAllObjects];
  NSString *getSessionGuidCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @iterm2_id", sessionId_];
  ...
  NSString *setSizeCommand = [NSString
      stringWithFormat:@"refresh-client -C %d,%d", size.width, size.height];
  NSString *listWindowsCommand =
      [NSString stringWithFormat:@"list-windows -F %@", kListWindowsFormat];
  NSString *listSessionsCommand =
      @"list-sessions -F \"#{session_id} #{session_name}\"";
  NSString *getAffinitiesCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @affinities", sessionId_];
  NSString *getBuriedIndexesCommand = [NSString
      stringWithFormat:@"show -v -q -t $%d @buried_indexes", sessionId_];
  NSString *getOriginsCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @origins", sessionId_];
  NSString *getHotkeysCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @hotkeys", sessionId_];
  NSString *getTabColorsCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @tab_colors", sessionId_];
  NSString *getHiddenWindowsCommand =
      [NSString stringWithFormat:@"show -v -q -t $%d @hidden", sessionId_];
  NSArray *commands = @[
  ...
  [gateway_
      sendCommandList:[commands filteredArrayUsingBlock:^BOOL(id anObject) {
...
// Sets the current controller's session guid and updates the global set of
// attached session GUIDs. If guid is nil then it is removed from the global
// set.
...
- (void)getSessionGuidResponse:(NSString *)sessionGuid {
  if (!sessionGuid.length) {
    ...
    NSString *command = [NSString
        stringWithFormat:@"set -t $%d @iterm2_id \"%@\"", sessionId_, guid];
...
- (void)whenPaneRegistered:(int)wp
                      call:(void (^)(PTYSession<iTermTmuxControllerSession> *))
                               block {
  PTYSession<iTermTmuxControllerSession> *already =
      [self sessionForWindowPane:wp];
  if (already) {
    dispatch_async(dispatch_get_main_queue(), ^{
      block(already);
    });
...
- (void)detach {
  ...
  [gateway_ release];
  gateway_ = nil;
  [_when enumerateKeysAndObjectsUsingBlock:^(
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerDetachedNotification
                    object:self];
...
- (void)windowDidResize:(NSWindowController<iTermWindowController> *)term {
  ...
  DLog(@"The last known size of tmux windows is %@",
       NSStringFromSize(lastSize_));
  if (NSEqualSizes(size, lastSize_)) {
    return;
  }

  DLog(@"Looks like the window resize is legit. Change client size to %@",
       NSStringFromSize(size));
  [self setClientSize:size];
}

- (void)variableSizeWindowDidResize:
    (NSWindowController<iTermWindowController> *)term {
  DLog(@"Window %@ did resize. Updating tmux tabs", term);
  [self
      setWindowSizes:[term.tabs
                         mapWithBlock:^iTermTuple<NSString *, NSValue *> *(
                             PTYTab *tab) {
                           if (!tab.tmuxTab || tab.tmuxController != self) {
                             return nil;
                           }
                           return [iTermTuple
                               tupleWithObject:[NSString
                                                   stringWithInt:tab.tmuxWindow]
                                     andObject:[NSValue
                                                   valueWithSize:tab.tmuxSize]];
                         }]];
}
...
- (void)fitLayoutToWindows {
  ...
  if (minSize.width <= 0 || minSize.height <= 0) {
    // After the last session closes a size of 0 is reported. Apparently
    // unplugging a monitor leads to a negative value here. That's inferred from
    // crash report 1468853197.309853926.txt (at the time of that crash, this
    // tested only for zero values so it passed through and asserted anyway).
    return;
  }
  if (NSEqualSizes(minSize, lastSize_)) {
    return;
  }
  DLog(@"fitLayoutToWindows setting client size to %@",
       NSStringFromSize(minSize));
  [self setClientSize:minSize];
}

- (void)fitLayoutToVariableSizeWindows {
  [self setWindowSizes:[_windowStates.allKeys mapWithBlock:^iTermTuple<
                                                  NSString *, NSValue *> *(
                                                  NSNumber *windowNumber) {
          iTermTmuxWindowState *state = _windowStates[windowNumber];
          PTYTab *tab = state.tab;
          return [iTermTuple
              tupleWithObject:[NSString stringWithInt:windowNumber.intValue]
...
- (NSArray<NSDictionary *> *)commandListToSetSize:(NSSize)size
                                         ofWindow:(int)window {
  NSSet *siblings = [affinities_ valuesEqualTo:[@(window) stringValue]];
  if (!siblings.count) {
    return [self commandListToSetSize:size
                            ofWindows:@[ [NSString stringWithInt:window] ]];
  } else {
...
- (void)setWindowSizes:
    (NSArray<iTermTuple<NSString *, NSValue *> *> *)windowSizes {
  [gateway_ sendCommandList:[self commandListToSetWindowSizes:windowSizes]];
}

- (NSArray<NSDictionary *> *)commandListToSetWindowSizes:
    (NSArray<iTermTuple<NSString *, NSValue *> *> *)windowSizes {
  return [windowSizes
      mapWithBlock:^NSDictionary *(iTermTuple<NSString *, NSValue *> *tuple) {
        ...
        if (size.width < 1 || size.height < 1 || size.width >= 10000 ||
            size.height >= 10000) {
          return nil;
        }
        ...
        NSString *command =
            [NSString stringWithFormat:@"resize-window -x %@ -y %@ -t @%d",
                                       @((int)size.width), @((int)size.height),
                                       window.intValue];
        NSDictionary *dict = [gateway_
            dictionaryForCommand:command
                  responseTarget:self
...
- (NSArray<NSDictionary *> *)commandListToSetSize:(NSSize)size
  ...
  return
      [self commandListToSetWindowSizes:[windows mapWithBlock:^iTermTuple<
                                                     NSString *, NSValue *> *(
                                                     NSString *window) {
              return [iTermTuple tupleWithObject:window
                                       andObject:[NSValue valueWithSize:size]];
            }]];
}
...
- (int)adjustHeightForStatusBar:(int)height {
  ...
  NSArray *buggyVersions = @[
    [NSDecimalNumber decimalNumberWithString:@"2.9"],
    [NSDecimalNumber decimalNumberWithString:@"2.91"]
  ];
  if (_hasStatusBar &&
      [buggyVersions containsObject:gateway_.minimumServerVersion]) {
    return height + 1;
  }
  return height;
}
...
- (void)setClientSize:(NSSize)size {
  ...
  NSString *listStr = [NSString
      stringWithFormat:
          @"list-windows -F \"#{window_id} #{window_layout} #{window_flags}\""];
  NSString *setSizeCommand =
      [NSString stringWithFormat:@"set -t $%d @iterm2_size %d,%d", sessionId_,
                                 (int)size.width, (int)size.height];
  const int height = [self adjustHeightForStatusBar:(int)size.height];
  ITBetaAssert(height > 0, @"Invalid size");
  [_windowSizes removeAllObjects];
  NSArray *commands = [NSArray
      arrayWithObjects:
          [gateway_ dictionaryForCommand:setSizeCommand
                          responseTarget:nil
                        responseSelector:nil
                          responseObject:nil
                                   flags:0],
          [gateway_
              dictionaryForCommand:
                  [NSString stringWithFormat:@"refresh-client -C %d,%d",
...
- (void)enablePauseModeIfPossible {
  DLog(@"enablePauseModeIfPossible min=%@ max=%@",
       gateway_.minimumServerVersion, gateway_.maximumServerVersion);
  if (gateway_.minimumServerVersion &&
      [gateway_.minimumServerVersion
          compare:[NSDecimalNumber decimalNumberWithString:@"3.2"]] ==
          NSOrderedAscending) {
    DLog(@"min < 3.2");
    return;
  }
  ...
  NSUInteger catchUpTime = [iTermPreferences
      unsignedIntegerForKey:kPreferenceKeyTmuxPauseModeAgeLimit];
  ...
  [gateway_ sendCommand:[NSString
                            stringWithFormat:@"refresh-client -fpause-after=%@",
                                             @(age)]
         responseTarget:nil
       responseSelector:nil];
  [_tmuxBufferMonitor release];
  _tmuxBufferMonitor =
      [[iTermTmuxBufferSizeMonitor alloc] initWithController:self pauseAge:age];
...
- (void)panesDidUnpause:(TmuxWindowOpener *)opener {
  for (NSNumber *wp in opener.unpausingWindowPanes) {
    PTYSession<iTermTmuxControllerSession> *session =
        [self sessionForWindowPane:wp.intValue];
    [session setTmuxHistory:[opener historyLinesForWindowPane:wp.intValue
                                              alternateScreen:NO]
                 altHistory:[opener historyLinesForWindowPane:wp.intValue
                                              alternateScreen:YES]
...
- (void)pausePanes:(NSArray<NSNumber *> *)wps {
  ...
  NSString *command =
      [NSString stringWithFormat:@"refresh-client -A '%@'", adjustments];
...
- (void)validateOptions {
  for (NSString *option in [self unsupportedGlobalOptions]) {
    [gateway_ sendCommand:[NSString
                              stringWithFormat:@"show-window-options -g %@",
                                               option]
           responseTarget:self
...
- (void)clearHistoryForWindowPane:(int)windowPane {
  [gateway_ sendCommand:[NSString stringWithFormat:@"clear-history -t \"%%%d\"",
                                                   windowPane]
         responseTarget:nil
       responseSelector:nil];
}
...
- (void)loadTitleFormat {
  ...
  [gateway_
      sendCommandList:@[ [gateway_
                          dictionaryForCommand:@"show-options -v -g set-titles"
                                responseTarget:self
...
- (void)handleShowSetTitles:(NSString *)result {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerDidFetchSetTitlesStringOption
                    object:self];
}

- (void)guessVersion {
  // Run commands that will fail in successively older versions.
  // show-window-options pane-border-format will succeed in 2.3 and later
  // (presumably. 2.3 isn't out yet) the socket_path format was added in 2.2.
    ...
    [gateway_ dictionaryForCommand:@"list-windows -F \"#{pid}\""
                  ...
                  responseSelector:@selector(guessVersion21Response:)
                    responseObject:nil
                             flags:kTmuxGatewayCommandShouldTolerateErrors],
    [gateway_
        dictionaryForCommand:@"show-options -g message-style" // message-style
                                                              // added in 1.9
              responseTarget:self
...
- (void)checkForUTF8Response:(NSString *)response {
  if ([response containsString:@"_"]) {
    [gateway_
        abortWithErrorMessage:@"tmux is not in UTF-8 mode. Please pass the -u "
                              @"command line argument to tmux or change your "
                              @"LANG environment variable to end with “.UTF-8”."
...
- (void)handleDisplayMessageVersion:(NSString *)response {
  ...
  if ([response hasPrefix:nextPrefix]) {
    [self
        handleDisplayMessageVersion:[response
                                        substringFromIndex:nextPrefix.length]];
    return;
  }
  if ([response hasSuffix:@"-rc"]) {
    response = [response stringByDroppingLastCharacters:3];
  }
  // In case we get back something that's not a number, or a totally
  // unreasonable number, just ignore this.
  DLog(@"response=%@", response);
  NSDecimalNumber *number = [NSDecimalNumber decimalNumberWithString:response];
  DLog(@"number=%@", number);
  if (number.doubleValue != number.doubleValue || number.doubleValue < 2.4 ||
      number.doubleValue > 10) {
    DLog(@"nan or out of bounds, do nothing.");
    return;
  }

  // Sadly tmux version numbers look like 2.9 or 2.9a instead of a proper
  // decimal number.
  NSRange range = [response
...
- (void)guessVersion22Response:(NSString *)response {
  ...
  const NSInteger index =
      [response rangeOfCharacterFromSet:[[NSCharacterSet
...
- (void)guessVersion18Response:(NSString *)response {
  ...
  // This is the oldest version supported. By the time you get here you know the
  // version.
  [self didGuessVersion];
}
...
// Show an error and terminate the connection because tmux has an unsupported
// option turned on.
...
- (void)windowPane:(int)wp
  ...
  NSString *resizeStr = [NSString
      stringWithFormat:@"resize-pane -%@ -t \"%%%d\" %d", dir, wp, abs(amount)];
  NSString *listStr = [NSString
      stringWithFormat:
          @"list-windows -F \"#{window_id} #{window_layout} #{window_flags}\""];
  NSArray *commands =
      [NSArray arrayWithObjects:[gateway_ dictionaryForCommand:resizeStr
                                                responseTarget:nil
                                              responseSelector:nil
                                                responseObject:nil
                                                         flags:0],
...
- (void)splitWindowPane:(int)wp
  ...
  // No need for a callback. We should get a layout-changed message and act on
  // it.
  [initialDirectory
      tmuxSplitWindowCommand:wp
          ...
          recyclingSupported:self.recyclingSupported
                  ...
                  completion:^(NSString *command) {
                    ...
                    // Get the list of panes, then split, then get the list of
                    // panes again. This seems to be the only way to get the
                    // pane ID of the new pane.
                    NSString *listPanesCommand = [NSString
                        stringWithFormat:@"list-panes -t %%%d -F '#{pane_id}'",
                                         wp];
                    NSMutableDictionary *state =
                        [NSMutableDictionary dictionary];
                    state[iTermTmuxControllerSplitStateCompletion] =
                        [[completion copy] autorelease];
                    NSDictionary *initialListPanes = [gateway_
                        dictionaryForCommand:listPanesCommand
                              responseTarget:self
                            responseSelector:@selector(recordPanes:state:)
                              responseObject:state
                                       flags:0];
                    NSDictionary *split = [gateway_
                        dictionaryForCommand:command
                              responseTarget:nil
                            responseSelector:nil
                              responseObject:nil
                                       flags:
                                           kTmuxGatewayCommandShouldTolerateErrors];
                    NSDictionary *followupListPanes = [gateway_
                        dictionaryForCommand:listPanesCommand
                              responseTarget:self
                            responseSelector:@selector(didSplit:state:)
                              responseObject:state
                                       flags:
                                           kTmuxGatewayCommandShouldTolerateErrors];
                    [gateway_ sendCommandList:@[
                      initialListPanes, split, followupListPanes
                    ]];
                  }];
}
...
- (void)recordPanes:(NSString *)list state:(NSMutableDictionary *)state {
  state[iTermTmuxControllerSplitStateInitialPanes] =
      [NSSet setWithArray:[list componentsSeparatedByString:@"\n"]];
}
...
- (void)didSplit:(NSString *)list state:(NSMutableDictionary *)state {
  NSSet<NSString *> *after =
      [NSSet setWithArray:[list componentsSeparatedByString:@"\n"]];
  NSSet<NSString *> *before =
      state[iTermTmuxControllerSplitStateInitialPanes] ?: [NSSet set];
...
- (void)selectPane:(int)windowPane {
  NSDecimalNumber *version2_9 =
      [NSDecimalNumber decimalNumberWithString:@"2.9"];

  if ([gateway_.minimumServerVersion isEqual:version2_9]) {
    // I presume this will be fixed in whatever verson follows 2.9, so use an
    // isEqual:. I need to remember to revisit this after the bug is fixed!
    return;
  }

  NSString *command =
      [NSString stringWithFormat:@"select-pane -t \"%%%d\"", windowPane];
  [gateway_ sendCommand:command responseTarget:nil responseSelector:nil];
}
...
- (void)newWindowInSessionNumber:(NSNumber *)sessionNumber
                ...
                initialDirectory:(iTermInitialDirectory *)initialDirectory {
  [initialDirectory
      tmuxNewWindowCommandInSessionNumber:sessionNumber
                       recyclingSupported:self.recyclingSupported
                                    scope:scope
                               completion:^(NSString *command) {
                                 ...
                                 if (_variableWindowSize) {
                                   ...
                                   ITBetaAssert((int)size.width > 0,
                                                @"Invalid size");
                                   const int height = [self
                                       adjustHeightForStatusBar:size.height];
                                   ITBetaAssert(height > 0, @"Invalid size");
                                   [_windowSizes removeAllObjects];
                                   NSString *setSizeCommand =
                                       [NSString stringWithFormat:
...
- (void)newWindowWithAffinity:(NSString *)windowIdString
  ...
  [initialDirectory
      tmuxNewWindowCommandRecyclingSupported:self.recyclingSupported
                                       scope:scope
                                  completion:^(NSString *command) {
                                    NSMutableArray *commands =
                                        [NSMutableArray array];
...
- (void)movePane:(int)srcPane
  ...
  [gateway_ sendCommand:[NSString stringWithFormat:
                                      @"move-pane -s \"%%%d\" -t \"%%%d\" %@%@",
                                      srcPane, destPane,
                                      splitVertical ? @"-h" : @"-v",
                                      addBefore ? @" -b" : @""]
...
- (void)killWindowPane:(int)windowPane {
  [gateway_ sendCommand:[NSString stringWithFormat:@"kill-pane -t \"%%%d\"",
                                                   windowPane]
         responseTarget:nil
       responseSelector:nil
         responseObject:nil
...
- (void)renameWindowWithId:(int)windowId
  ...
  if (sessionNumber) {
    theCommand = [NSString
        stringWithFormat:
            @"rename-window -t \"$%d:@%d\" \"%@\"", sessionNumber.intValue,
            windowId,
            [self stringByEscapingBackslashesAndRemovingNewlines:newName]];
  } else {
    theCommand = [NSString
        stringWithFormat:
            @"rename-window -t @%d \"%@\"", windowId,
            [self stringByEscapingBackslashesAndRemovingNewlines:newName]];
  }
...
- (BOOL)canRenamePane {
  NSDecimalNumber *version2_6 =
      [NSDecimalNumber decimalNumberWithString:@"2.6"];
  if ([gateway_.minimumServerVersion compare:version2_6] ==
      NSOrderedAscending) {
    return NO;
  }
  return YES;
...
- (void)renamePane:(int)windowPane toTitle:(NSString *)newTitle {
  ...
  NSString *theCommand = [NSString
      stringWithFormat:
          @"select-pane -t %%%d -T \"%@\"", windowPane,
...
- (void)setHotkeyForWindowPane:(int)windowPane to:(NSDictionary *)dict {
  ...
  // First get a list of existing panes so we can avoid setting hotkeys for any
  // nonexistent panes. Keeps the string from getting too long.
  NSString *getPaneIDsCommand = [NSString
      stringWithFormat:@"list-panes -s -t $%d -F \"#{pane_id}\"", sessionId_];
...
- (void)setTabColorString:(NSString *)colorString
            forWindowPane:(int)windowPane {
  if ([_tabColors[@(windowPane)] isEqualToString:colorString]) {
    return;
  }
  _tabColors[@(windowPane)] = colorString;

  // First get a list of existing panes so we can avoid setting tab colors for
  // any nonexistent panes. Keeps the string from getting too long.
  NSString *getPaneIDsCommand = [NSString
      stringWithFormat:@"list-panes -s -t $%d -F \"#{pane_id}\"", sessionId_];
  [gateway_ sendCommand:getPaneIDsCommand
         responseTarget:self
...
- (NSString *)encodedString:(NSString *)string prefix:(NSString *)prefix {
  return [prefix
      stringByAppendingString:[[string dataUsingEncoding:NSUTF8StringEncoding]
                                  it_hexEncoded]];
}

- (NSString *)decodedString:(NSString *)string
             optionalPrefix:(NSString *)prefix {
  if (![string hasPrefix:prefix]) {
    return string;
  }
  return [[[NSString alloc]
      initWithData:[[string substringFromIndex:prefix.length] dataFromHexValues]
          encoding:NSUTF8StringEncoding] autorelease];
}

- (void)sendCommandToSetHotkeys {
  NSString *hexEncoded =
      [self encodedString:[self.hotkeysString stringByEscapingQuotes]
                   prefix:iTermTmuxControllerEncodingPrefixHotkeys];
  NSString *command = [NSString
      stringWithFormat:@"set -t $%d @hotkeys \"%@\"", sessionId_, hexEncoded];
  [gateway_ sendCommand:command
         responseTarget:nil
       responseSelector:nil
         responseObject:nil
...
- (void)killWindow:(int)window {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermTmuxControllerWillKillWindow
                    object:@(window)];
  [gateway_
           sendCommand:[NSString stringWithFormat:@"kill-window -t @%d", window]
        responseTarget:nil
      responseSelector:nil
        responseObject:nil
                 flags:kTmuxGatewayCommandOfferToDetachIfLaggyDuplicate |
...
- (NSString *)breakPaneWindowPaneFlag {
  NSDecimalNumber *version2_1 =
      [NSDecimalNumber decimalNumberWithString:@"2.1"];

  if ([gateway_.maximumServerVersion compare:version2_1] ==
      NSOrderedAscending) {
    // 2.0 and earlier versions take -t for the window pane
    return @"-t";
  }
  if ([gateway_.minimumServerVersion compare:version2_1] !=
      NSOrderedAscending) {
...
- (void)breakOutWindowPane:(int)windowPane toPoint:(NSPoint)screenPoint {
  ...
  [gateway_ sendCommand:[NSString
                            stringWithFormat:@"break-pane %@ \"%%%d\"",
                                             [self breakPaneWindowPaneFlag],
                                             windowPane]
         responseTarget:nil
       responseSelector:nil];
}
...
- (NSString *)terminalGUIDForWindowID:(int)wid {
  for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
    for (PTYTab *tab in term.tabs) {
      if (tab.isTmuxTab && tab.tmuxController == self &&
          tab.tmuxWindow == wid) {
...
- (void)setWindowID:(int)wid
    buriedFromTerminalGUID:(NSString *)terminalGUID
                  tabIndex:(int)tabIndex {
  DLog(@"set %@ = %@", @(wid), terminalGUID);
  NSMutableArray<iTermTuple<NSNumber *, NSNumber *> *> *wids =
      _buriedWindows[terminalGUID];
  if (!wids) {
    wids = [NSMutableArray array];
    _buriedWindows[terminalGUID] = wids;
  }
  if (![wids
          objectPassingTest:^BOOL(iTermTuple<NSNumber *, NSNumber *> *element,
...
- (void)hideWindows:(NSArray<NSNumber *> *)windowIDs
       andCloseTabs:(BOOL)closeTabs {
  DLog(@"hideWindow: Add these window IDs to hidden: %@", windowIDs);
  if (closeTabs) {
    DLog(@"burying window IDs %@",
         [[windowIDs mapWithBlock:^id(NSNumber *anObject) {
           return [anObject description];
         }] componentsJoinedByString:@", "]);
    // Update _buriedWindows
    [windowIDs
        enumerateObjectsUsingBlock:^(NSNumber *_Nonnull wid, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerDidChangeHiddenWindows
                    object:self];
}
...
- (void)openWindowWithId:(int)windowId
  ...
  [_buriedWindows
      enumerateKeysAndObjectsUsingBlock:^(
          ...
          NSMutableArray<iTermTuple<NSNumber *, NSNumber *> *> *_Nonnull tuples,
          BOOL *_Nonnull stop) {
        const NSInteger i =
            [tuples indexOfObjectPassingTest:^BOOL(
                        iTermTuple<NSNumber *, NSNumber *> *_Nonnull obj,
        ...
        if (i != NSNotFound) {
          ...
          DLog(@"Add affinities for terminal %@: %@", terminalGUID,
               [[tuples mapWithBlock:^id(iTermTuple *anObject) {
                 return anObject.description;
               }] componentsJoinedByString:@", "]);
          [affinities_ setValue:[@(windowId) stringValue]
                   equalToValue:terminalGUID];
        }
      }];
  ...
  [gateway_ sendCommand:[NSString stringWithFormat:@"display -p -F %@ -t @%d",
         ...
         responseTarget:self
       responseSelector:@selector(listedWindowsToOpenOne:
                                forWindowIdAndAffinities:)
...
- (void)moveWindowId:(int)windowId
  ...
  [gateway_ sendCommand:[NSString
                            stringWithFormat:
                                @"move-window -s \"$%d:@%d\" -t \"$%d:+\"",
                                sessionNumber, windowId, targetSessionNumber]
         responseTarget:nil
       responseSelector:nil];
}

// Find a position for any key in panes and remove all entries with keys in
// panes. windowPositions_ is used for setting the origin of a pane after moving
// it into a window, which is rarely done. This falls back to the recorded
// window origin if one is present.
...
- (void)renameSessionNumber:(int)sessionNumber to:(NSString *)newName {
  NSString *renameCommand = [NSString
      stringWithFormat:@"rename-session -t \"$%d\" \"%@\"", sessionNumber,
...
- (void)killSessionNumber:(int)sessionNumber {
  NSString *killCommand =
      [NSString stringWithFormat:@"kill-session -t \"$%d\"", sessionNumber];
...
- (void)addSessionWithName:(NSString *)sessionName {
  NSString *attachCommand =
      [NSString stringWithFormat:@"new-session -s \"%@\"",
...
- (void)attachToSessionWithNumber:(int)sessionNumber {
  NSString *attachCommand =
      [NSString stringWithFormat:@"attach-session -t \"$%d\"", sessionNumber];
  [gateway_ sendCommand:attachCommand responseTarget:nil responseSelector:nil];
}
...
- (void)listWindowsInSessionNumber:(int)sessionNumber
  ...
  NSString *listWindowsCommand =
      [NSString stringWithFormat:@"list-windows -F %@ -t \"$%d\"",
                                 kListWindowsFormat, sessionNumber];
  NSArray *userInfo =
      @[ listWindowsCommand, object, target, NSStringFromSelector(selector) ];
  if ([_listWindowsQueue containsObject:userInfo]) {
    // Already have this queued up.
    return;
  }
  // Wait a few seconds. We always get a windows-close notification when the
  // last window in a window closes. To avoid spamming the command line with
  // list-windows, we wait a bit to see if there is an exit notification coming
  // down the pipe.
  ...
  [gateway_
      ...
      responseSelector:@selector(didListWindows:userData:)
        responseObject:@[ object, selector, target ]
                 flags:kTmuxGatewayCommandShouldTolerateErrors]; // Tolerates
                                                                 // errors
                                                                 // because the
                                                                 // session may
                                                                 // have been
...
- (void)saveHiddenWindows {
  NSString *hidden =
      [[hiddenWindows_ allObjects] componentsJoinedByString:@","];
  DLog(@"Save hidden windows: %@", hidden);
  NSString *command = [NSString
      stringWithFormat:
          @"set -t $%d @hidden \"%@\"", sessionId_,
...
- (void)saveWindowOrigins {
  ...
  DLog(@"Save window origins to %@ called from %@", enc,
       [NSThread callStackSymbols]);
  NSString *command = [NSString
      stringWithFormat:
          @"set -t $%d @origins \"%@\"", sessionId_,
...
- (NSString *)windowOptionsForTerminal:(PseudoTerminal *)term {
  if (term.anyFullScreen) {
    return [NSString
        stringWithFormat:@"%@=%@", kTmuxWindowOpenerWindowOptionStyle,
...
- (void)saveAffinities {
  ...
  for (PseudoTerminal *term in terminals) {
    ...
    for (iTermTuple<NSNumber *, NSNumber *>
             *tuple in _buriedWindows[term.terminalGuid]) {
    ...
    if (siblings.count > 0) {
      NSString *value = [NSString
          stringWithFormat:@"%@;%@", [siblings componentsJoinedByString:@","],
...
- (void)saveBuriedIndexes {
  NSString *arg =
      [[_buriedWindows.allKeys mapWithBlock:^id(NSString *terminalGUID) {
        NSString *rhs = [[_buriedWindows[terminalGUID]
            mapWithBlock:^id(iTermTuple<NSNumber *, NSNumber *> *tuple) {
              return [NSString stringWithFormat:@"%@=%@", tuple.firstObject,
                                                tuple.secondObject];
            }] componentsJoinedByString:@","];
  ...
  NSString *command = [NSString
      stringWithFormat:
          @"set -t $%d @buried_indexes \"%@\"", sessionId_,
          [self encodedString:[arg stringByEscapingQuotes]
...
- (PseudoTerminal *)windowWithAffinityForWindowId:(int)wid {
  for (NSString *n in
       [self savedAffinitiesForWindow:[NSString stringWithInt:wid]]) {
    if ([n hasPrefix:@"pty-"]) {
...
- (void)swapPane:(int)pane1 withPane:(int)pane2 {
  NSString *swapPaneCommand = [NSString
      stringWithFormat:@"swap-pane -s \"%%%d\" -t \"%%%d\"", pane1, pane2];
...
- (void)toggleZoomForPane:(int)pane {
  NSArray *commands = @[
    [gateway_
        dictionaryForCommand:[NSString
                                 stringWithFormat:@"resize-pane -Z -t \"%%%d\"",
                                                  pane]
              responseTarget:nil
            responseSelector:NULL
              responseObject:nil
...
- (void)setLayoutInWindow:(int)window toLayout:(NSString *)layout {
  NSArray *commands = @[
    [gateway_
        dictionaryForCommand:[NSString
                                 stringWithFormat:@"select-layout -t @%@ %@",
                                                  @(window), layout]
              responseTarget:self
...
- (void)getOriginsResponse:(NSString *)encodedResult {
  NSString *result =
      [self decodedString:encodedResult
...
- (NSString *)shortStringForHotkeyDictionary:(NSDictionary *)dict
                                      paneID:(int)wp {
  return
      [NSString stringWithFormat:@"%d=%@", wp,
                                 [iTermShortcut shortStringForDictionary:dict]];
}

- (NSString *)hotkeysString {
  NSMutableArray *parts = [NSMutableArray array];
  [_hotkeys enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
                                                NSDictionary *_Nonnull obj,
                                                BOOL *_Nonnull stop) {
    if ([_paneIDs containsObject:key]) {
      [parts addObject:[self shortStringForHotkeyDictionary:obj
                                                     paneID:key.intValue]];
    }
  }];
...
- (NSString *)tabColorsString {
  NSMutableArray *parts = [NSMutableArray array];
  [_tabColors enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
...
- (void)getHotkeysResponse:(NSString *)encodedResult {
  NSString *result =
      [self decodedString:encodedResult
           optionalPrefix:iTermTmuxControllerEncodingPrefixHotkeys];
  ...
  if (result.length > 0) {
    ...
    for (NSString *part in parts) {
      ...
      if (equals != NSNotFound && equals + 1 < part.length) {
        ...
        NSDictionary *dict =
            [iTermShortcut dictionaryForShortString:shortString];
        if (dict) {
...
- (void)getHiddenWindowsResponse:(NSString *)encodedResponse {
  NSString *response =
      [self decodedString:encodedResponse
  ...
  if ([response length] > 0) {
    ...
    DLog(@"getHiddenWindowsResponse: Add these window IDs to hidden: %@",
         windowIds);
    for (NSString *wid in windowIds) {
...
- (void)getBuriedIndexesResponse:(NSString *)result {
  ...
  NSString *decoded =
      [self decodedString:result
           optionalPrefix:iTermTmuxControllerEncodingPrefixBuriedIndexes];
  if (!decoded.length) {
    return;
  }
  ...
  [parts enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx,
                                      BOOL *_Nonnull stop) {
    ...
    NSArray<NSString *> *pairStrings =
        [encodedPairs componentsSeparatedByString:@","];
    NSArray<iTermTuple<NSNumber *, NSNumber *> *> *tuples =
        [pairStrings mapWithBlock:^id(NSString *string) {
          iTermTuple<NSString *, NSString *> *sstuple = [string keyValuePair];
          if (!sstuple) {
            return nil;
          }
          if (!sstuple.firstObject.isNumeric ||
              !sstuple.secondObject.isNumeric) {
            return nil;
          }
          return
              [iTermTuple tupleWithObject:@([sstuple.firstObject intValue])
...
- (NSArray *)componentsOfAffinities:(NSString *)affinities {
  ...
  if (semicolonRange.location != NSNotFound) {
    ...
    NSString *windowOptions =
        [affinities substringFromIndex:NSMaxRange(semicolonRange)];
...
- (void)setAffinitiesFromString:(NSString *)result {
  ...
  for (NSString *theString in affinities) {
    ...
    if (siblings.count == 1) {
      ...
      // then create an equivalence class containing only (wid, wid+"_ph").
      // ph=placeholder The equivalence class's existence signals not to apply
      // the default mode for unrecognized windows.
    ...
    for (NSString *widString in siblings) {
      ...
      if (widString.isNumeric &&
          [hiddenWindows_ containsObject:@(widString.intValue)]) {
        NSString *terminalGUID =
            [[siblings filteredArrayUsingBlock:^BOOL(NSString *candidate) {
              return !candidate.isNumeric && ![candidate hasSuffix:@"_ph"];
            }] firstObject];
        if (terminalGUID) {
          [self setWindowID:widString.intValue
              buriedFromTerminalGUID:terminalGUID
                            tabIndex:-1];
        }
      }
...
- (void)listSessionsResponse:(NSString *)result {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerSessionsWillChange
                    object:nil];
  self.sessionObjects = [[result componentsSeparatedByRegex:@"\n"]
      mapWithBlock:^iTermTmuxSessionObject *(NSString *line) {
        ...
        NSScanner *scanner =
            [NSScanner scannerWithString:[sessionID substringFromIndex:1]];
        int sessionNumber = -1;
        if (![scanner scanInt:&sessionNumber] || sessionNumber < 0) {
          return nil;
        }
        iTermTmuxSessionObject *obj =
            [[[iTermTmuxSessionObject alloc] init] autorelease];
        obj.name = sessionName;
        obj.number = sessionNumber;
        return obj;
      }];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kTmuxControllerSessionsDidChange
                    object:self];
}
...
- (void)listedWindowsToOpenOne:(NSString *)response
  ...
  if (response == nil) {
    DLog(@"Listing windows failed. Maybe the window died before we could get "
         @"to it?");
    return;
  }
  ...
  if (!doc) {
    [gateway_
        abortWithErrorMessage:
            [NSString
                stringWithFormat:@"Bad response for list windows request: %@",
                                 response]];
    return;
  }
  for (NSArray *record in doc.records) {
    ...
    if ([self windowIdFromString:recordWindowId] == [windowId intValue]) {
      [self openWindowWithIndex:
                [self windowIdFromString:[doc valueInRecord:record
                                                   forField:@"window_id"]]
                           name:[[doc valueInRecord:record
                                           forField:@"window_name"]
                                    it_unescapedTmuxWindowName]
                           size:NSMakeSize([[doc valueInRecord:record
                                                      forField:@"window_width"]
                                               intValue],
                                           [[doc valueInRecord:record
                                                      forField:@"window_height"]
                                               intValue])
                         layout:[doc valueInRecord:record
                                          forField:@"window_layout"]
                     affinities:affinities
                    windowFlags:[doc valueInRecord:record
                                          forField:@"window_flags"]
                        profile:profile
                        initial:NO
                       tabIndex:tabIndex];
    }
...
- (void)parseListWindowsResponseAndUpdateLayouts:(NSString *)response {
  ...
  for (NSString *layoutString in layoutStrings) {
    NSArray *components = [layoutString
        captureComponentsMatchedByRegex:@"^@([0-9]+) ([^ ]+)(?: ([^ ]+))?"];
    if ([components count] < 3) {
      ...
      if (tab) {
        [tabs addObject:tab];
        NSNumber *zoomed =
            components.count > 3 ? @([components[3] containsString:@"Z"]) : nil;
        const BOOL adjust =
...
- (void)retainWindow:(int)window withTab:(PTYTab *)tab {
  ...
  if (notify) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kTmuxControllerWindowDidOpen
                      object:@[ k, self ]];
  }
}
...
- (void)releaseWindow:(int)window {
  ...
  if (!state.refcount) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kTmuxControllerWindowDidClose
                      object:@[ k, self ]];
...
- (void)
    newWindowWithAffinityCreated:(NSString *)responseStr
     affinityWindowAndCompletion:
         (iTermTuple *)
             tuple { // Value passed in to -newWindowWithAffinity:, may be nil
  if ([responseStr hasPrefix:@"@"]) {
    ...
    DLog(@"Response to new-window doesn't look like a window id: \"%@\"",
         responseStr);
  }
}
...
- (void)windowDidOpen:(TmuxWindowOpener *)windowOpener {
  ...
  DLog(@"TmuxController windowDidOpen for index %@ with error count %@",
       windowIndex, @(windowOpener.errorCount));
...
- (void)responseForListWindows:(NSString *)response
             toSetPartialOrder:(NSArray *)partialOrder {
  ...
  // We have two lists, desiredOrder and currentOrder, that contain the same
  // objects but in (possibly) a different order. For each out-of-place value,
  // swap it with a later value, placing the later value in its correct
  // location.
  ...
  for (int i = 0; i < currentOrder.count; i++) {
    if ([currentOrder[i] intValue] != [desiredOrder[i] intValue]) {
      ...
      NSString *command =
          [NSString stringWithFormat:@"swap-window -s @%@ -t @%@",
...
- (void)setCurrentWindow:(int)windowId {
  ...
  NSString *command =
      [NSString stringWithFormat:@"select-window -t @%d", windowId];
  [gateway_ sendCommand:command responseTarget:nil responseSelector:nil];
}
...
- (void)setEncodedUserVars:(NSString *)encodedUserVars forPane:(int)paneID {
  NSString *decoded =
      [self decodedString:encodedUserVars
  ...
  NSMutableDictionary<NSString *, NSString *> *dict =
      [self mutableUserVarsForPane:paneID];
...
- (NSMutableDictionary<NSString *, NSString *> *)mutableUserVarsForPane:
    (int)paneID {
