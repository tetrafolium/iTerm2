...
+ (NSArray *)signalNames {
  return @[
    [NSNull null], @"HUP",  @"INT",  @"QUIT", @"ILL",  @"TRAP",   @"ABRT",
...
- (void)setIntValue:(int)i {
  ...
  [self
      setToolTip:[NSString stringWithFormat:@"SIG%@ (%d)", signalNames[i], i]];
}
...
- (void)textDidEndEditing:(NSNotification *)aNotification {
  ...
  id<NSComboBoxDelegate> comboBoxDelegate =
      (id<NSComboBoxDelegate>)[self delegate];
  [comboBoxDelegate
      comboBoxSelectionIsChanging:
          [NSNotification
              notificationWithName:NSComboBoxSelectionIsChangingNotification
                            object:nil]];
}
...
- (NSUInteger)comboBox:(NSComboBox *)aComboBox
  ...
  if ([aString intValue] > 0) {
    return NSNotFound; // Without this, "1", "2", and "3" get replaced
                       // immediately!
  }
...
- (NSString *)comboBox:(NSComboBox *)aComboBox
  ...
  uncompletedString = [[uncompletedString
      stringByTrimmingCharactersInSet:[NSCharacterSet
                                          whitespaceAndNewlineCharacterSet]]
      uppercaseString];
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    signal_ = [[SignalPicker alloc]
        initWithFrame:NSMakeRect(0, frame.size.height - kButtonHeight + 1,
                                 frame.size.width - kill_.frame.size.width -
                                     2 * kMargin,
                                 kButtonHeight)];
    ...
    kill_ = [[[NSButton alloc]
        initWithFrame:NSMakeRect(0, frame.size.height - kButtonHeight,
                                 frame.size.width, kButtonHeight)] autorelease];
    if (@available(macOS 10.16, *)) {
      kill_.bezelStyle = NSBezelStyleRegularSquare;
      kill_.bordered = NO;
      kill_.image = [NSImage it_imageForSymbolName:@"play"
      ...
      [kill_ sizeToFit];
      kill_.frame = NSMakeRect(signal_.frame.size.width + kMargin, 0,
                               kill_.frame.size.width, kill_.frame.size.height);
    }
    ...
    scrollView_ = [[NSScrollView alloc]
    ...
    tableView_.intercellSpacing =
        NSMakeSize(tableView_.intercellSpacing.width, 0);
...
- (void)relayout {
  NSRect frame = self.frame;
  signal_.frame = NSMakeRect(0, frame.size.height - kButtonHeight + 1,
                             signal_.frame.size.width, kButtonHeight);
  [signal_ sizeToFit];

  if (@available(macOS 10.16, *)) {
    NSRect rect = kill_.frame;
    rect.origin.x = NSMaxX(signal_.frame);
    rect.origin.y = frame.size.height - kButtonHeight +
                    (kButtonHeight - rect.size.height) / 2;
    kill_.frame = rect;
  } else {
    kill_.frame = NSMakeRect(NSMaxX(signal_.frame) + kMargin,
                             frame.size.height - kButtonHeight,
                             frame.size.width, kButtonHeight);
    [kill_ sizeToFit];
  }
  scrollView_.frame = NSMakeRect(0, 0, frame.size.width,
                                 frame.size.height - kButtonHeight - kMargin);
}
...
- (void)updateTimer:(id)sender {
  ...
  NSArray<iTermProcessInfo *> *infos = [[[iTermProcessCache sharedInstance]
      processInfoForPid:rootPid] flattenedTree];
  NSSet<NSNumber *> *oldPids = [NSSet
      setWithArray:[_processInfos mapWithBlock:^id(iTermProcessInfo *info) {
        return @(info.processID);
      }]];
  NSSet<NSNumber *> *newPids =
      [NSSet setWithArray:[infos mapWithBlock:^id(iTermProcessInfo *info) {
               return @(info.processID);
             }]];
  if (![oldPids isEqual:newPids]) {
    ...
    NSArray<iTermProcessInfo *> *sortedInfos = [infos
        sortedArrayUsingComparator:^NSComparisonResult(
            iTermProcessInfo *_Nonnull obj1, iTermProcessInfo *_Nonnull obj2) {
          return [@(obj1.processID) compare:@(obj2.processID)];
        }];
    [sortedInfos
        enumerateObjectsUsingBlock:^(iTermProcessInfo *_Nonnull info,
    ...
    if (previouslySelectedPID) {
      NSInteger indexToSelect = [_processInfos
          indexOfObjectPassingTest:^BOOL(iTermProcessInfo *_Nonnull obj,
                                         NSUInteger idx, BOOL *_Nonnull stop) {
            return obj.processID == previouslySelectedPID.integerValue;
          }];
      if (indexToSelect != NSNotFound) {
        [tableView_
                selectRowIndexes:[NSIndexSet indexSetWithIndex:indexToSelect]
