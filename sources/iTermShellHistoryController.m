...
NSString *const kCommandHistoryDidChangeNotificationName =
    @"kCommandHistoryDidChangeNotificationName";
NSString *const kDirectoriesDidChangeNotificationName =
    @"kDirectoriesDidChangeNotificationName";
NSString *const kCommandHistoryHasEverBeenUsed =
...
@implementation iTermShellHistoryController {
  ...
  NSMutableDictionary<NSString *,
                      NSMutableArray<iTermCommandHistoryCommandUseMO *> *>
      *_expandedCache;
  NSManagedObjectContext *_managedObjectContext;
  iTermDirectoryTree *_tree;

  // Prevents notifications from being posted during initialization since that
  // causes deadlock in -sharedInstance.
  BOOL _initializing;
...
- (NSString *)pathToDatabase {
  ...
  if (!self.shouldSaveToDisk) {
    // For migration to work the in-memory path must be different than the
    // on-disk path.
...
// Note: setting vacuum to YES forces it to use the on-disk sqlite database.
// This allows you to vacuum a database after changing the setting to in-memory.
// It doesn't make sense to vacuum RAM, after all.
- (BOOL)initializeCoreDataWithRetry:(BOOL)retry vacuum:(BOOL)vacuum {
  NSURL *modelURL =
      [[NSBundle bundleForClass:self.class] URLForResource:@"Model"
                                             withExtension:@"momd"];
  assert(modelURL);
  ...
  // Apple wants you to do this on a background thread, but you can't do a fetch
  // until it's done. Since -init does a fetch, there's no point doing this on a
  // background thread, since it would need to wait for it to finish before
  // -loadCommandHistory could be called.
  NSError *error = nil;
  NSString *storeType;
  ...
  // An exception will be thrown here during the unit test that checks database
  // corruption. This is expected, just hit continue.
  [persistentStoreCoordinator addPersistentStoreWithType:storeType
                                           configuration:nil
                                                 ...
                                                 options:options
                                                   error:&error];
  if (error) {
    NSLog(@"Got an exception when opening the command history database: %@",
          error);
    if (![self shouldSaveToDisk]) {
...
- (void)makeDatabaseReadableOnlyByUser {
  ...
  NSDirectoryEnumerator<NSURL *> *enumerator = [[NSFileManager defaultManager]
                 enumeratorAtURL:[NSURL fileURLWithPath:basePath]
  ...
  for (NSURL *fileURL in enumerator) {
    ...
    if (![filename hasPrefix:self.databaseFilenamePrefix]) {
      DLog(@"Skip setting permissions on file %@ that isn't part of the shell "
           @"history db",
           filename);
      continue;
    }
    ...
    if (!permissionsNumber) {
      XLog(@"Couldn't get permissions of file %@. Attributes are: %@", fullPath,
           attributes);
      continue;
    }

    short posixPermissions = [permissionsNumber shortValue];
    short fixedPosixPermissions =
        (posixPermissions & 0700); // Remove other and group permissions.
    if (fixedPosixPermissions != posixPermissions) {
      NSDictionary *updatedAttributes =
          [attributes dictionaryBySettingObject:@(fixedPosixPermissions)
                                         forKey:NSFilePosixPermissions];
      error = nil;
      [[NSFileManager defaultManager] setAttributes:updatedAttributes
                                       ofItemAtPath:fullPath
                                              error:&error];
      if (error) {
        XLog(@"Failed to set attributes of %@ to %@: %@", fullPath,
             updatedAttributes, error);
      }
    }
  }
}
...
- (void)deleteObjectsWithEntityName:(NSString *)entityName {
  ...
  NSArray *objects = [_managedObjectContext executeFetchRequest:fetchRequest
                                                          error:nil];
...
- (NSArray *)managedObjects {
  NSFetchRequest *fetchRequest = [NSFetchRequest
      fetchRequestWithEntityName:[iTermHostRecordMO entityName]];
  NSError *error = nil;
...
+ (void)showInformationalMessage {
  ...
  case NSAlertFirstButtonReturn:
    [[NSWorkspace sharedWorkspace]
        openURL:
            [NSURL URLWithString:@"https://iterm2.com/shell_integration.html"]];
    break;
...
- (void)backingStoreTypeDidChange {
  NSPersistentStore *store = _managedObjectContext.persistentStoreCoordinator
                                 .persistentStores.firstObject;
  NSString *storeType =
      self.shouldSaveToDisk ? NSSQLiteStoreType : NSInMemoryStoreType;
...
- (void)eraseCommandHistory:(BOOL)commandHistory directories:(BOOL)directories {
  ...
  if (commandHistory) {
    [[NSFileManager defaultManager]
        removeItemAtPath:self.pathToDeprecatedCommandHistoryPlist
                   error:NULL];
  }
  if (directories) {
    [[NSFileManager defaultManager]
        removeItemAtPath:self.pathToDeprecatedDirectoriesPlist
                   error:NULL];
  }

  if (commandHistory) {
    [self deleteObjectsWithEntityName:[iTermCommandHistoryEntryMO entityName]];
    [self deleteObjectsWithEntityName:[iTermCommandHistoryCommandUseMO
                                          entityName]];
  }
  if (directories) {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kDirectoriesDidChangeNotificationName
                    object:nil];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kCommandHistoryDidChangeNotificationName
                    object:nil];
}
...
- (void)addCommand:(NSString *)command
  ...
  [[NSUserDefaults standardUserDefaults]
      setBool:YES
       forKey:kCommandHistoryHasEverBeenUsed];
  ...
  if (!theEntry) {
    theEntry = [iTermCommandHistoryEntryMO
        commandHistoryEntryInContext:_managedObjectContext];
  ...
  iTermCommandHistoryCommandUseMO *commandUse = [iTermCommandHistoryCommandUseMO
      commandHistoryCommandUseInContext:_managedObjectContext];
...
- (void)setStatusOfCommandAtMark:(VT100ScreenMark *)mark
  ...
  // If the status is 0 and commandUse doesn't have a code set, do nothing. This
  // saves some time in the common case.
...
- (BOOL)commandHistoryHasEverBeenUsed {
  return (_records.count > 0 || [[NSUserDefaults standardUserDefaults]
                                    boolForKey:kCommandHistoryHasEverBeenUsed]);
}

- (NSArray<iTermCommandHistoryEntryMO *> *)
    commandHistoryEntriesWithPrefix:(NSString *)partialCommand
                             onHost:(VT100RemoteHost *)host {
  ...
  for (iTermCommandHistoryEntryMO *entry in hostRecord.entries) {
    if (emptyPartialCommand ||
        [entry.command caseInsensitiveHasPrefix:partialCommand]) {
      // The FinalTerm algorithm doesn't require |partialCommand| to be a prefix
      // of the history entry, but based on how our autocomplete works, it makes
      // sense to only accept prefixes. Their scoring algorithm is implemented
      // in case this should change.
  ...
  return [sortedEntries
      subarrayWithRange:NSMakeRange(0, MIN(kMaxResults, sortedEntries.count))];
}

- (NSArray<iTermCommandHistoryCommandUseMO *> *)
    autocompleteSuggestionsWithPartialCommand:(NSString *)partialCommand
                                       onHost:(VT100RemoteHost *)host {
...
- (iTermCommandHistoryCommandUseMO *)commandUseWithMarkGuid:(NSString *)markGuid
                                                     onHost:(VT100RemoteHost *)
                                                                host {
  if (!markGuid) {
    return nil;
  }
  iTermHostRecordMO *hostRecord = [self recordForHost:host];
  // TODO: Create an index of markGuid's in command uses if this becomes a
  // performance problem during restore.
  for (iTermCommandHistoryEntryMO *entry in hostRecord.entries) {
...
- (NSArray<iTermCommandHistoryCommandUseMO *> *)commandUsesForHost:
...
- (iTermRecentDirectoryMO *)recordUseOfPath:(NSString *)path
  ...
  if (!directory) {
    ...
    directory = [NSEntityDescription
        insertNewObjectForEntityForName:[iTermRecentDirectoryMO entityName]
...
- (NSIndexSet *)abbreviationSafeIndexesInRecentDirectory:
    (iTermRecentDirectoryMO *)entry {
  return [_tree abbreviationSafeIndexesInPath:entry.path];
}

- (NSArray *)directoriesSortedByScoreOnHost:(VT100RemoteHost *)host {
  return [[self directoriesForHost:host]
...
- (BOOL)removeOldData {
  ...
  [fetchRequest
      setEntity:[NSEntityDescription
                             entityForName:[iTermCommandHistoryCommandUseMO
                                               entityName]
                    inManagedObjectContext:_managedObjectContext]];
  NSPredicate *predicate =
      [NSPredicate predicateWithFormat:@"time < %f",
                                       [self now] - kMaxTimeToRememberCommands];
  ...
  [fetchRequest
      setEntity:[NSEntityDescription
                             entityForName:[iTermRecentDirectoryMO entityName]
                    inManagedObjectContext:_managedObjectContext]];
  predicate = [NSPredicate
      predicateWithFormat:@"lastUse < %f and starred == 0",
  ...
  NSSortDescriptor *lastUseDescriptor =
      [[[NSSortDescriptor alloc] initWithKey:@"lastUse"
                                   ascending:NO
  ...
  [fetchRequest setPredicate:predicate];
  error = nil;
  directories = [_managedObjectContext executeFetchRequest:fetchRequest
                                                     error:&error];
  if (directories.count > iTermMaxDirectoriesToSave) {
    for (iTermRecentDirectoryMO *directory in
         [directories subarrayFromIndex:iTermMaxDirectoriesToSave]) {
      iTermHostRecordMO *hostRecord = directory.remoteHost;
      [hostRecord removeDirectoriesObject:directory];
      [_managedObjectContext deleteObject:directory];
    }
...
- (NSMutableArray<iTermCommandHistoryCommandUseMO *> *)
  ...
  [result sortWithOptions:0
          usingComparator:^NSComparisonResult(
              iTermCommandHistoryCommandUseMO *obj1,
...
- (NSArray<iTermRecentDirectoryMO *> *)directoriesForHost:
    (VT100RemoteHost *)host {
  ...
  for (iTermRecentDirectoryMO *directory in
       [[self recordForHost:host] directories]) {
