...
static NSViewController<iTermStatusBarKnobViewController> *
iTermNewViewControllerForKnob(iTermStatusBarComponentKnob *knob) {
  switch (knob.type) {
  ...
  case iTermStatusBarComponentKnobTypeInvocation:
    return [[iTermStatusBarKnobTextViewController alloc]
        initWithInvocationSuggester];
...
@implementation iTermStatusBarSetupKnobsViewController {
  NSArray<NSViewController<iTermStatusBarKnobViewController> *>
      *_viewControllers;
  CGSize _size;
...
- (instancetype)initWithComponent:(id<iTermStatusBarComponent>)component {
  ...
  if (self) {
    ...
    _knobs =
        [component statusBarComponentKnobs].reverseObjectEnumerator.allObjects;
    ...
    _viewControllers = [_knobs mapWithBlock:^id(
                                   iTermStatusBarComponentKnob *knob) {
      NSViewController<iTermStatusBarKnobViewController> *vc =
          iTermNewViewControllerForKnob(knob);
    ...
    _size.width = MAX(150, _maxControlOffset + maxControlWidth +
                               iTermStatusBarSetupPopoverMargin * 2);
...
- (void)loadView {
  self.view = [[NSView alloc]
      initWithFrame:NSMakeRect(0, 0, _size.width, _size.height)];
}
...
- (void)viewDidLoad {
  [super viewDidLoad];
  for (NSViewController<iTermStatusBarKnobViewController>
           *vc in _viewControllers) {
...
- (void)layoutSubviews {
  ...
  for (NSViewController<iTermStatusBarKnobViewController>
           *vc in _viewControllers) {
    const CGFloat x =
        _maxControlOffset - vc.controlOffset + iTermStatusBarSetupPopoverMargin;
    vc.view.frame =
        NSMakeRect(x, y, vc.view.frame.size.width, vc.view.frame.size.height);
...
- (NSDictionary *)knobValues {
  NSArray *keys =
      [_knobs mapWithBlock:^id(iTermStatusBarComponentKnob *anObject) {
