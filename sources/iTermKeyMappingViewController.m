...
- (instancetype)init {
  self = [super initWithNibName:@"iTermKeyMapping"
...
- (void)awakeFromNib {
  self.hapticFeedbackForEscEnabled =
      [iTermPreferences boolForKey:kPreferenceKeyEnableHapticFeedbackForEsc];
  self.soundForEscEnabled =
      [iTermPreferences boolForKey:kPreferenceKeyEnableSoundForEsc];
  self.visualIndicatorForEscEnabled =
      [iTermPreferences boolForKey:kPreferenceKeyVisualIndicatorForEsc];
}

- (void)setHapticFeedbackForEscEnabled:(BOOL)hapticFeedbackForEscEnabled {
  _hapticFeedbackForEsc.state = hapticFeedbackForEscEnabled
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  NSTextField *result = [tableView makeViewWithIdentifier:identifier
                                                    owner:self];
  if (result == nil) {
...
- (NSString *)descriptionForKeystroke:(iTermKeystroke *)keystroke
  ...
  iTermKeyBindingAction *action = [iTermKeyBindingAction
      withDictionary:[keystroke valueInBindingDictionary:dict]];
...
- (NSString *)stringValueForKeyMappingOnRow:(NSInteger)rowIndex
                          ...
                          bindingDictionary:(NSDictionary *)dict {
  NSArray<iTermKeystroke *> *sortedKeystrokes =
      [_delegate keyMappingSortedKeystrokes:self];
...
- (NSString *)stringValueForTouchBarMappingOnRow:(NSInteger)rowIndex
  ...
  NSArray<iTermTouchbarItem *> *sortedKeys =
      [_delegate keyMappingSortedTouchbarItems:self];
...
- (void)presentEditActionSheet:
  ...
  [self.view.window
             beginSheet:editActionWindowController.window
      completionHandler:^(NSModalResponse returnCode) {
        [self editActionWindowCompletionHandler:editActionWindowController];
      }];
}

- (void)editActionWindowCompletionHandler:
    (iTermEditKeyActionWindowController *)editActionWindowController {
  if (editActionWindowController.ok) {
    [_delegate
        ...
        didChangeItem:editActionWindowController.keystrokeOrTouchbarItem
             ...
             toAction:[iTermKeyBindingAction
                          withAction:editActionWindowController.action
...
- (IBAction)addTouchBarItem:(id)sender {
  ...
  editActionWindowController = [[iTermEditKeyActionWindowController alloc]
      initWithContext:iTermVariablesSuggestionContextSession |
                      iTermVariablesSuggestionContextApp
                 mode:iTermEditKeyActionWindowControllerModeTouchBarItem];
  editActionWindowController.isNewMapping = YES;
  editActionWindowController.touchBarItemID =
      [iTermTouchBarIDPrefix stringByAppendingString:[NSString uuid]];
...
- (IBAction)removeMapping:(id)sender {
  ...
  NSArray<iTermKeystroke *> *sortedRegularKeystrokes =
      [_delegate keyMappingSortedKeystrokes:self];
  NSArray<iTermTouchbarItem *> *sortedTouchbarItems =
      [_delegate keyMappingSortedTouchbarItems:self];
  ...
  [indexes enumerateIndexesUsingBlock:^(NSUInteger row, BOOL *_Nonnull stop) {
    if (row < sortedRegularKeystrokes.count) {
      ...
      [touchbarItems
          addObject:sortedTouchbarItems[row - sortedRegularKeystrokes.count]];
    }
  }];
  ...
  [_tableView removeRowsAtIndexes:_tableView.selectedRowIndexes
                    withAnimation:YES];
...
- (void)doubleClick:(id)sender {
  ...
  NSArray<iTermKeystrokeOrTouchbarItem *> *sortedItems =
      [[_delegate keyMappingSortedKeystrokes:self]
          mapWithBlock:^id(iTermKeystroke *anObject) {
  ...
  if (rowIndex < sortedItems.count) {
    ...
    sortedItems = [[_delegate keyMappingSortedTouchbarItems:self]
        mapWithBlock:^id(iTermTouchbarItem *anObject) {
  ...
  [selectedItem
      ...
      second:^(iTermTouchbarItem *_Nonnull touchbarItem) {
        iTermKeyBindingAction *action = [iTermKeyBindingAction
            withDictionary:dict[touchbarItem.identifier]];
...
- (IBAction)visualIndicatorForEscToggled:(id)sender {
  [iTermPreferences
      setBool:_visualIndicatorForEsc.state == NSControlStateValueOn
...
- (void)importMenuItem:(id)sender {
  ...
  [_openPanel beginSheetModalForWindow:self.view.window
...
- (void)importFromOpenPanel {
  ...
  id decoded = [NSJSONSerialization it_objectForJsonString:content
                                                     error:&error];
  if (!decoded) {
    XLog(@"Beep: %@", error);
    NSBeep();
  ...
  NSDictionary *keymappings =
      [NSDictionary castFrom:dict[INTERCHANGE_KEY_MAPPING_DICT]];
  ...
  if (![self.delegate keyMapping:self
          shouldImportKeystrokes:keystrokesThatWillChange]) {
    return;
  }

  for (id serialized in keymappings) {
    iTermKeystroke *keystroke =
        [[iTermKeystroke alloc] initWithSerialized:serialized];
    if (!keystroke.isValid) {
      continue;
    }

    NSDictionary *entry = [NSDictionary
        castFrom:[keystroke valueInBindingDictionary:keymappings]];
    iTermKeyBindingAction *action =
        [iTermKeyBindingAction withDictionary:entry];
    if (!action) {
      continue;
    }
    [self.delegate
  ...
  NSDictionary *touchbarItems =
      [NSDictionary castFrom:dict[INTERCHANGE_TOUCH_BAR_ITEMS]];
  for (id identifier in touchbarItems) {
    iTermTouchbarItem *touchbarItem =
        [[iTermTouchbarItem alloc] initWithIdentifier:identifier];
    if (!touchbarItem) {
      continue;
    }
    NSDictionary *entry =
        [NSDictionary castFrom:touchbarItems[touchbarItem.identifier]];
    if (!entry) {
      continue;
    }
...
- (NSNumber *)removeBeforeLoading:(NSString *)thing {
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString
              stringWithFormat:@"Remove all key mappings before %@?", thing]
...
- (void)exportMenuItem:(id)sender {
  ...
  __weak __typeof(self) weakSelf = self;
  [_savePanel beginSheetModalForWindow:self.view.window
                     completionHandler:^(NSModalResponse result) {
                       if (result == NSModalResponseOK) {
...
- (void)exportFromSavePanel {
  ...
  NSDictionary *const dict = @{
    INTERCHANGE_KEY_MAPPING_DICT : keymappings ?: @{},
