...
static iTermAnimationDirection
iTermAnimationDirectionOpposite(iTermAnimationDirection direction) {
  switch (direction) {
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p shortcuts=%@ hasModAct=%@ modAct=%@ "
                       @"profile.name=%@ profile.guid=%@ open=%@>",
                       [self class], self, self.shortcuts,
                       @(self.hasModifierActivation),
                       @(self.modifierActivation), self.profile[KEY_NAME],
                       self.profile[KEY_GUID], @(self.isHotKeyWindowOpen)];
}
...
- (void)createWindowWithCompletion:(void (^)(void))completion {
  [self createWindowWithURL:nil completion:completion];
}
...
- (void)createWindowWithURL:(NSURL *)url completion:(void (^)(void))completion {
  ...
  [self getWindowControllerFromProfile:[self profile]
                                   url:url
                            completion:^(PseudoTerminal *windowController) {
                              self.windowController =
                                  [windowController weakSelf];
                              if (completion) {
                                completion();
                              }
                            }];
...
- (void)setWindowController:
    (PseudoTerminal<iTermWeakReference> *)windowController {
  ...
  if (_windowController.windowType != WINDOW_TYPE_TRADITIONAL_FULL_SCREEN) {
    [_windowController.window
        setCollectionBehavior:self.windowController.window.collectionBehavior &
...
- (void)activeSpaceDidChange:(NSNotification *)notification {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (self.windowController.window.isKeyWindow) {
        DLog(@"Hotkey window still key a spin after changing spaces. "
             @"Inexplicable.");
        return;
      }
      DLog(@"One spin after changing spaces with open non-autohiding "
           @"joins-all-spaces key hotkey window that lost key status. Make "
           @"key.");
...
- (NSWindowLevel)floatingLevel {
  ...
  if (app.it_characterPanelIsOpen || app.it_modalWindowOpen || app.it_imeOpen) {
    DLog(@"Use floating window level. characterPanelIsOpen=%@, "
         @"modalWindowOpen=%@ imeOpen=%@",
  ...
  // NSStatusWindowLevel overlaps the menu bar and the dock. This is obviously
  // desirable because you don't want these things blocking your view. But if
  // you've configured your menu bar to automatically hide (system prefs >
  // general > automatically hide and show menu bar) then the menu bar gets
  // overlapped when you show it and that is lame (issue 7924).
  //
  // NSTornOffMenuWindowLevel does not overlap the dock, so it is no good. You
  // can't go having your dock overlapping your fullscreen hotkey window, as
  // that is lame (issue 7963).
  //
  // NSMainMenuWindowLevel seems to do what you'd want, but I think it just
  // works by accident.
  //
  // It seems the sweet spot is between the dock and main menu levels, of which
  // there are three (21…23). They are unnamed.
  //
  // It is ok to be leveled below the main menu because the window is always
  // positioned under the menu bar *except* when the menu bar is auto-hidden.
  //
  // However, there is an exception for floating panels. iTerm2 does not get
  // activated when you open a floating panel. That means it does not have the
  // ability to hide the menu bar. We *want* floating panels to be overlapped by
  // an auto-hiding menu barm, but not by a fixed menu bar. So use the status
  // window level for them when the menu bar is set to auto-hide. See issue 7984
  // for why we want a floating panel hotkey window to overlap the menu bar.
  //
  // Mind you, this is all irrelevant if iTerm2's "auto-hide menu bar in
  // non-native full screen" is turned off. Then the window is just shifted down
  // and the menu bar hangs around.
...
- (NSRect)preferredFrameForWindowController:
    (PseudoTerminal<iTermWeakReference> *)windowController {
  // This can be the anchored screen (typically the screen the window was
  // created with, but can be changed by -moveToPreferredScreen). If unanchored,
  // then it is the current screen.
  ...
  case WINDOW_TYPE_TRADITIONAL_FULL_SCREEN: // Framerate drops too much to roll
                                            // this (2014 5k iMac)
...
- (NSRect)frameByMovingFrame:(NSRect)sourceFrame
                  fromScreen:(NSScreen *)sourceScreen
                    toScreen:(NSScreen *)destinationScreen {
  NSSize originOffset =
      NSMakeSize(sourceFrame.origin.x - sourceScreen.visibleFrame.origin.x,
...
- (NSPoint)hiddenOriginForScreen:(NSScreen *)screen {
  ...
  DLog(@"Basing hidden origin on screen frame (IHD) %@",
  ...
  case WINDOW_TYPE_LEFT:
  case WINDOW_TYPE_LEFT_PARTIAL:
    return NSMakePoint(NSMinX(screen.visibleFrameIgnoringHiddenDock),
                       rect.origin.y);

  case WINDOW_TYPE_RIGHT:
  case WINDOW_TYPE_RIGHT_PARTIAL:
    return NSMakePoint(NSMaxX(screen.visibleFrameIgnoringHiddenDock),
                       rect.origin.y);

  case WINDOW_TYPE_BOTTOM:
  case WINDOW_TYPE_BOTTOM_PARTIAL:
    return NSMakePoint(rect.origin.x,
                       NSMinY(screen.visibleFrameIgnoringHiddenDock) -
                           NSHeight(rect));
  ...
  case WINDOW_TYPE_COMPACT:
  case WINDOW_TYPE_ACCESSORY:
    return [self frameByMovingFrame:rect
                         fromScreen:self.windowController.window.screen
                           toScreen:screen]
        .origin;
...
- (void)rollInAnimatingInDirection:(iTermAnimationDirection)direction {
  ...
  NSRect destination =
      [self preferredFrameForWindowController:self.windowController];
  NSRect proposedHiddenRect = self.windowController.window.frame;
  proposedHiddenRect.origin =
      [self hiddenOriginForScreen:self.windowController.screen];
  if ([self rect:proposedHiddenRect
          intersectsAnyScreenExcept:self.windowController.window.screen]) {
  ...
  [NSAnimationContext
      runAnimationGroup:^(NSAnimationContext *_Nonnull context) {
        [context setDuration:[iTermAdvancedSettingsModel
...
- (void)rollOutAnimatingInDirection:(iTermAnimationDirection)direction {
  ...
  destination.origin =
      [self hiddenOriginForScreen:self.windowController.window.screen];

  if ([self rect:destination
          intersectsAnyScreenExcept:self.windowController.window.screen]) {
    destination = source;
  }
...
- (void)moveToPreferredScreen {
  ...
  NSRect destination =
      [self preferredFrameForWindowController:self.windowController];
  DLog(@"iTermProfileHotKey: move to preferred screen setting frame to %@",
       NSStringFromRect(destination));
...
- (void)fadeIn {
  ...
  [[NSAnimationContext currentContext]
      setDuration:[iTermAdvancedSettingsModel hotkeyTermAnimationDuration]];
...
#if BETA
  SetPinnedDebugLogMessage(
      [NSString stringWithFormat:@"Fade out hotkey window %p", self],
      [[NSThread callStackSymbols] componentsJoinedByString:@"\n"]);
#endif
  [NSAnimationContext endGrouping];
}

- (iTermAnimationDirection)animateInDirectionForWindowType:
...
- (BOOL)floats {
  return [iTermProfilePreferences boolForKey:KEY_HOTKEY_FLOAT
...
- (void)rollInAnimated:(BOOL)animated {
  ...
  if (self.windowController.windowType == WINDOW_TYPE_TRADITIONAL_FULL_SCREEN) {
    // This has to be done before making it key or the dock will be hidden based
    // on the display it was last on, not the display it will be on. I think it
    // might be safe to do this for all window types, but I don't want to risk
    // introducing bugs here.
    [self moveToPreferredScreen];
  }
  if (self.hotkeyWindowType != iTermHotkeyWindowTypeFloatingPanel) {
...
- (void)reallyRollInAnimated:(BOOL)animated {
  ...
  if (animated) {
    ...
    case WINDOW_TYPE_BOTTOM_PARTIAL:
      [self
          rollInAnimatingInDirection:[self
                                         animateInDirectionForWindowType:
                                             self.windowController.windowType]];
      break;
    ...
    case WINDOW_TYPE_MAXIMIZED:
    case WINDOW_TYPE_COMPACT_MAXIMIZED:
    case WINDOW_TYPE_TRADITIONAL_FULL_SCREEN: // Framerate drops too much to
                                              // roll this (2014 5k iMac)
    case WINDOW_TYPE_ACCESSORY:
      [self moveToPreferredScreen];
      [self fadeIn];
      break;
...
- (void)rollOut {
  ...
  if ([iTermProfilePreferences boolForKey:KEY_HOTKEY_ANIMATE
                                inProfile:self.profile]) {
    switch (self.windowController.windowType) {
    case WINDOW_TYPE_TOP:
    case WINDOW_TYPE_TOP_PARTIAL:
    ...
    case WINDOW_TYPE_BOTTOM:
    case WINDOW_TYPE_BOTTOM_PARTIAL: {
      iTermAnimationDirection inDirection = [self
          animateInDirectionForWindowType:self.windowController.windowType];
      iTermAnimationDirection outDirection =
          iTermAnimationDirectionOpposite(inDirection);
...
- (void)saveHotKeyWindowState {
  if (self.windowController.weaklyReferencedObject && self.profileGuid) {
    ...
    NSDictionary *arrangement =
        [self.windowController arrangementExcludingTmuxTabs:YES
                                          includingContents:includeContents];
    if (arrangement) {
      self.restorableState =
          @{kGUID : self.profileGuid, kArrangement : arrangement};
    } else {
...
- (BOOL)encodeGraphWithEncoder:(iTermGraphEncoder *)encoder {
  ...
  if (![self.windowController
          conformsToProtocol:@protocol(iTermGraphCodable)]) {
    XLog(@"Window controller %@ does not conform to iTermGraphCodable",
         self.windowController);
    return NO;
  }
...
- (BOOL)isRevealed {
  return self.windowController.window.alphaValue == 1 &&
...
- (void)cancelRollOut {
  ...
  if (_rollOutCancelable && _rollingOut) {
    ...
    DLog(@"Cannot cancel. cancelable=%@ rollingOut=%@", @(_rollOutCancelable),
         @(_rollingOut));
  }
}
...
- (NSArray<iTermBaseHotKey *> *)hotKeyPressedWithSiblings:
    (NSArray<iTermBaseHotKey *> *)genericSiblings {
  DLog(@"hotKeypressedWithSiblings called on %@ with siblings %@", self,
       genericSiblings);

  genericSiblings = [genericSiblings arrayByAddingObject:self];

  NSArray<iTermProfileHotKey *> *siblings =
      [genericSiblings mapWithBlock:^id(iTermBaseHotKey *anObject) {
  ...
  // If any sibling is rolling out but we can cancel the rollout, do so. This is
  // after the window has finished animating out but we're in the delay period
  // before activating the app the user was in before pressing the hotkey to
  ...
  // If any sibling is rolling in or rolling out, do nothing. This keeps us from
  // ending up in a broken state where some siblings are in and others are out.
  BOOL anyTransitioning =
      [siblings anyWithBlock:^BOOL(iTermProfileHotKey *other) {
        BOOL result = other.rollingIn || other.rollingOut;
        if (result) {
          DLog(@"Found a transitioning sibling: %@ rollingIn=%@ rollingOut=%@",
               other, @(other.rollingIn), @(other.rollingOut));
        }
        return result;
      }];
  if (anyTransitioning) {
    DLog(@"One or more siblings is transitioning so I'm returning without "
         @"doing anything.");
    return siblings;
  }
  DLog(@"toggle window %@. siblings=%@", self, siblings);
  BOOL allSiblingsOpen =
      [siblings allWithBlock:^BOOL(iTermProfileHotKey *sibling) {
  ...
  for (iTermProfileHotKey *sibling in
       [NSSet setWithArray:[siblings arrayByAddingObject:self]]) {
    DLog(@"Invoking handleHotkeyPressWithAllOpen:%@ anyIsKey:%@ on %@",
         @(allSiblingsOpen), @(anyIsKey), sibling);
    [sibling handleHotkeyPressWithAllOpen:allSiblingsOpen anyIsKey:anyIsKey];
  }
  return siblings;
}

- (void)handleHotkeyPressWithAllOpen:(BOOL)allSiblingsOpen
                            anyIsKey:(BOOL)anyIsKey {
  if (self.windowController.weaklyReferencedObject) {
    DLog(@"already have a hotkey window created");
    if (self.windowController.window.alphaValue == 1) {
      if (self.windowController.spaceSetting ==
              iTermProfileOpenInCurrentSpace &&
          !self.windowController.window.isOnActiveSpace) {
        DLog(@"Move already-open hotkey window to current space");
        NSWindow *window = self.windowController.window;
        // I tested this on 10.12 and it's sufficient to move the window. Maybe
        // not in older OS versions?
      ...
      if (anyIsKey || ![self switchToVisibleHotKeyWindowIfPossible]) {
        ...
        [self hideHotKeyWindowAnimated:YES
                       suppressHideApp:NO
                      otherIsRollingIn:NO];
      }
    } else {
...
- (PseudoTerminal *)windowControllerFromRestorableState {
  NSDictionary *arrangement =
      [[self.restorableState[kArrangement] copy] autorelease];
  self.restorableState = nil;
  if (!arrangement) {
    return nil;
  }
...
- (iTermHotkeyWindowType)hotkeyWindowType {
  if (self.floats) {
    if ([iTermProfilePreferences intForKey:KEY_SPACE inProfile:self.profile] ==
        iTermProfileJoinsAllSpaces) {
...
- (void)getWindowControllerFromProfile:(Profile *)hotkeyProfile
  ...
  if ([[hotkeyProfile objectForKey:KEY_WINDOW_TYPE] intValue] ==
    ...
    NSMutableDictionary *replacement =
        [[hotkeyProfile mutableCopy] autorelease];
  ...
  [iTermSessionLauncher launchBookmark:hotkeyProfile
                    ...
                    respectTabbingMode:NO
                        ...
                        didMakeSession:^(PTYSession *_Nonnull session) {
                          ...
                          if (session) {
                            result = [[iTermController sharedInstance]
...
- (void)rollInFinished {
  ...
  if (self.windowController.window) {
    [[iTermApplication sharedApplication]
        it_makeWindowKey:self.windowController.window];
  }
  [self.windowController.window
      makeFirstResponder:self.windowController.currentSession.textview];
  [[self.windowController currentTab] recheckBlur];
  self.windowController.window.collectionBehavior =
      self.windowController.desiredWindowCollectionBehavior;
...
- (void)didFinishRollingOut {
  ...
  BOOL activatingOtherApp =
      [self.delegate willFinishRollingOutProfileHotKey:self];
...
- (void)orderOut {
  ...
  // This must be done after orderOut: so autoHideHotKeyWindowsExcept: will know
  // to throw out the previous state.
  _rollingOut = NO;
}

- (BOOL)autoHides {
  return [iTermProfilePreferences boolForKey:KEY_HOTKEY_AUTOHIDE
                                   inProfile:self.profile];
}
...
// If there's a visible hotkey window that is either not key or is on another
// space, switch to it. Save the previously active app unless switching spaces
// (why the exception? Not sure.) Return YES if it was switched to, or NO if the
// window isn't suitable for switching to.
- (BOOL)switchToVisibleHotKeyWindowIfPossible {
  DLog(@"switchToVisibleHotKeyWindowIfPossible");
  const BOOL activateStickyHotkeyWindow =
      (!self.autoHides && !self.windowController.window.isKeyWindow);
  ...
  const BOOL hotkeyWindowOnOtherSpace =
      ![self.windowController.window isOnActiveSpace];
  if (hotkeyWindowOnOtherSpace || activateStickyHotkeyWindow) {
    DLog(@"Hotkey window is active on another space, or else it doesn't "
         @"autohide but isn't key. Switch to it.");
    if (self.hotkeyWindowType != iTermHotkeyWindowTypeFloatingPanel) {
      [NSApp activateIgnoringOtherApps:YES];
    }
    [self.windowController.window makeKeyAndOrderFront:nil];
...
- (BOOL)showHotKeyWindowCreatingWithURLIfNeeded:(NSURL *)url {
  ...
  if (self.windowController.window.alphaValue == 1 &&
      self.windowController.window.isVisible) {
    // This path is taken when you use a session hotkey to navigate to an
    // already-open hotkey window.
  ...
  if (!self.windowController.weaklyReferencedObject) {
    ...
    [self createWindowWithURL:url
                   completion:^{
                     [self rollInAnimated:[iTermProfilePreferences
                                              boolForKey:KEY_HOTKEY_ANIMATE
                                               inProfile:self.profile]];
                   }];
    result = YES;
  } else {
    [self rollInAnimated:[iTermProfilePreferences boolForKey:KEY_HOTKEY_ANIMATE
                                                   inProfile:self.profile]];
  }
  return result;
}
...
- (void)hideHotKeyWindowAnimated:(BOOL)animated
  ...
  DLog(@"Hide hotkey window. animated=%@ suppressHideApp=%@", @(animated),
       @(suppressHideApp));
