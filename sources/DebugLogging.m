...
static void AppendWindowDescription(NSWindow *window,
                                    NSMutableString *windows) {
...
static void WriteDebugLogHeader() {
  ...
  NSString *header = [NSString
      stringWithFormat:@"iTerm2 version: %@\n"
                       ...
                       [[NSApplication sharedApplication] keyWindow], windows,
                       [(iTermApplication *)
                               NSApp orderedWindowsPlusAllHotkeyPanels],
                       pinnedMessages];
...
static void FlushDebugLog() {
  ...
  if ([iTermAdvancedSettingsModel appendToExistingDebugLog] &&
    ...
    NSFileHandle *fileHandle =
        [NSFileHandle fileHandleForWritingAtPath:kDebugLogFilename];
    [fileHandle seekToEndOfFile];
    [fileHandle writeData:[log dataUsingEncoding:NSUTF8StringEncoding]];
    [fileHandle closeFile];
  } else {
    [log writeToFile:kDebugLogFilename
          atomically:NO
...
void AppendPinnedDebugLogMessage(NSString *key, NSString *value, ...) {
  ...
  va_list args;
  va_start(args, value);
  NSString *s = [[[NSString alloc] initWithFormat:value
                                        arguments:args] autorelease];
  va_end(args);

  NSString *log = [NSString stringWithFormat:@"%lld.%06lld [%@]: %@\n",
                                             (long long)tv.tv_sec,
                                             (long long)tv.tv_usec, key, s];
...
int CDebugLogImpl(const char *file, int line, const char *function,
                  const char *format, ...) {
  va_list args;
  ...
  NSString *value = [NSString stringWithUTF8String:stringValue]
                        ?: @"utf8 encoding problem in C string";
  free(stringValue);
  va_end(args);
...
int DebugLogImpl(const char *file, int line, const char *function,
  ...
  if (gDebugLogging) {
    ...
    [gDebugLogStr
        appendFormat:@"%lld.%06lld %s:%d (%s): ", (long long)tv.tv_sec,
...
void LogForNextCrash(const char *file, int line, const char *function,
                     NSString *value, BOOL force) {
  ...
  @synchronized(object) {
    ...
    if (force || (numLines % 100 == 0)) {
      ...
      dateFormatter.dateFormat =
          [NSDateFormatter dateFormatFromTemplate:@"yyyy-MM-dd HH:mm:ss.SSS ZZZ"
                                          options:0
                                           locale:nil];
      NSDate *date =
          [NSDate dateWithTimeIntervalSinceNow:-clock() / CLOCKS_PER_SEC];
      NSString *string =
  ...
  NSString *string = [NSString
      stringWithFormat:@"%lld.%06lld %s:%d (%s): %@\n", (long long)tv.tv_sec,
  ...
  if (!gDebugLogging) {
    if (![iTermAdvancedSettingsModel appendToExistingDebugLog]) {
      [[NSFileManager defaultManager]
          removeItemAtURL:[NSURL fileURLWithPath:kDebugLogFilename]
                    error:nil];
    }
...
BOOL TurnOffDebugLoggingSilently(void) { return StopDebugLogging(); }

void ToggleDebugLogging(void) {
  if (!gDebugLogging) {
    NSAlert *alert = [[[NSAlert alloc] init] autorelease];
    alert.messageText = @"Debug Logging Enabled";
    alert.informativeText = @"Please reproduce the bug. Then toggle debug "
                            @"logging again to save the log.";
...
- (NSString *)it_compressedDescription {
  NSString *uncompressed =
      [NSString stringWithFormat:@"%@:\n%@\n%@", self.name, self.reason,
                                 [self.it_originalCallStackSymbols
                                     componentsJoinedByString:@"\n"]];
  return [[[uncompressed dataUsingEncoding:NSUTF8StringEncoding]
      it_compressedData] stringWithBase64EncodingWithLineBreak:@""];
}
