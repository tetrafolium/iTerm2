...
- (PTYSession *)pickSession {
  NSWindow *window = [[[iTermApplication sharedApplication]
      orderedWindowsPlusVisibleHotkeyPanels] firstObject];
  if (!window) {
    return nil;
  }
  ...
  while (!_done) {
    ...
    NSTimer *timer =
        [NSTimer timerWithTimeInterval:1 / 60.0
                                target:self
  ...
  [_sessionView setSplitSelectionMode:kSplitSelectionModeOff
                                 move:NO
                              session:nil];
...
- (NSStatusItem *)addStatusBarItem {
  ...
  NSImage *image = [NSImage it_imageNamed:@"StopStatusIcon"
                                 forClass:self.class];
  item.button.title = @"";
  ...
  iTermHelpMessageViewController *viewController =
      [[iTermHelpMessageViewController alloc]
          initWithNibName:@"iTermHelpMessageViewController"
                   bundle:[NSBundle bundleForClass:self.class]];
  [viewController
      setMessage:
          @"Click the stop icon exit picker mode without making a selection."];
...
- (void)chooseSessionUnderCursor:(NSTimer *)timer {
  ...
  if (view) {
    ...
    [_sessionView setSplitSelectionMode:kSplitSelectionModeInspect
                                   move:NO
                                session:nil];
  }
}
...
- (NSView *)viewAtMouseRect:(NSRect)mouseRect {
  NSArray<NSWindow *> *frontToBackWindows =
      [[iTermApplication sharedApplication]
          orderedWindowsPlusVisibleHotkeyPanels];
