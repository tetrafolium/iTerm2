...
+ (instancetype)itermCopyModeCursorInSelectionState:(BOOL)selecting {
  iTermCopyModeCursor *cursor =
      [[[iTermCopyModeCursor alloc] init] autorelease];
...
- (void)drawOutlineOfRect:(NSRect)cursorRect withColor:(NSColor *)color {
  ...
  NSFrameRectWithWidthUsingOperation(rect, 0.5,
  ...
  NSRect cursorRect =
      NSMakeRect(rect.origin.x,
                 rect.origin.y + rect.size.height - height -
                     [iTermAdvancedSettingsModel underlineCursorOffset],
                 ceil(rect.size.width), height);
  if (outline) {
...
- (void)drawWithRect:(NSRect)rect
  ...
  NSRect cursorRect = NSMakeRect(
      ...
      [iTermAdvancedSettingsModel verticalBarCursorWidth], rect.size.height);
  if (outline) {
    [self drawOutlineOfRect:cursorRect withColor:backgroundColor];
  } else {
    [backgroundColor set];
...
- (void)drawWithRect:(NSRect)rect
  ...
  NSRect cursorRect =
      NSMakeRect(rect.origin.x - rect.size.width, rect.origin.y,
                 rect.size.width * 2, rect.size.height * heightFraction);
  ...
  if (self.selecting) {
    [[NSColor colorWithRed:0xc1 / 255.0
                     green:0xde / 255.0
                      blue:0xff / 255.0
                     alpha:1] set];
  } else {
...
- (void)drawWithRect:(NSRect)rect
  ...
  if (smart) {
    ...
    backgroundColor =
        [_smartCursorColor backgroundColorForCharacter:screenChar];
  }
  [backgroundColor set];
...
- (void)drawSmartCursorCharacter:(screen_char_t)screenChar
  ...
  NSColor *overrideColor =
      [_smartCursorColor textColorForCharacter:screenChar
