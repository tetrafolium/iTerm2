...
+ (void)registerBuiltInFunction {
  NSDictionary<NSString *, NSString *> *defaults = @{
    ...
    iTermSessionTitleArgTmuxWindowName :
        [NSString stringWithFormat:@"%@.%@", iTermVariableKeySessionTab,
                                   iTermVariableKeyTabTmuxWindowName],
    iTermSessionTitleArgTmuxWindowTitle :
        [NSString stringWithFormat:@"%@.%@", iTermVariableKeySessionTab,
                                   iTermVariableKeyTabTmuxWindowTitle],
  };
  // This would be a cyclic reference since the session.name is the result of
  // this function.
  assert(![defaults.allValues containsObject:iTermVariableKeySessionName]);
  NSSet *optionalArguments = [NSSet setWithArray:@[
    iTermSessionTitleArgTmuxPane, iTermSessionTitleArgTmuxRole,
    iTermSessionTitleArgTmuxClientName, iTermSessionTitleArgTmuxWindowName,
    iTermSessionTitleArgTmuxWindowTitle
  ]];
  {
            ...
            defaultValues:defaults
                  context:iTermVariablesSuggestionContextSession
                    block:^(
                      ...
                      NSString *result = [self titleForParameters:parameters
                                                         isWindow:NO];
            ...
            defaultValues:defaults
                  context:iTermVariablesSuggestionContextSession
                    block:^(
                      ...
                      NSString *result = [self titleForParameters:parameters
                                                         isWindow:YES];
                      // NOTE: iTermSessionNameController assumes that the
                      // built-in window_title function completes synchronously.
                      completion(result, nil);
                    }];
    [[iTermBuiltInFunctions sharedInstance] registerFunction:func
                                                   namespace:@"iterm2.private"];
...
+ (NSString *)titleForParameters:(NSDictionary *)parameters
  ...
  NSString *tmuxWindowName =
      trim(parameters[iTermSessionTitleArgTmuxWindowName]);
  NSString *tmuxWindowTitle =
      trim(parameters[iTermSessionTitleArgTmuxWindowTitle]);
...
// Historical note: 3.2 and earlier had three flags that controlled behavior:
// job, profile, and sticky. SessionName is the name inherited from the profile
// or set by icon title, manual edit, or trigger. Job Profile Sticky      Name
// unchanged    Name changed no  no      no          "Shell" SessionName
...
//
// no  yes     yes         ProfileName       ProfileName: IconTitle -or-
// SessionName yes yes     yes         ProfileName (job) ProfileName: IconTitle
// -or- SessionName (job)
+ (NSString *)titleForSessionName:(NSString *)sessionName
  ...
  DLog(@"sessionName=%@ profileName=%@ job=%@ commandLine=%@ pwd=%@ tty=%@ "
       @"user=%@ host=%@ tmuxPane=%@ iconName=%@ windowName=%@ "
       @"tmuxWindowName=%@ tmuxWindowTitle=%@ isWindowTitle=%@",
       sessionName, profileName, jobVariable, commandLineVariable, pwdVariable,
       ttyVariable, userVariable, hostVariable, tmuxPaneVariable, iconName,
       windowName, tmuxWindowName, tmuxWindowTitle, @(isWindowTitle));
  ...
  if (tmuxPaneVariable) {
    if (isWindowTitle) {
      effectiveSessionName =
          tmuxWindowTitle ?: tmuxWindowName ?: windowName ?: tmuxPaneVariable;
    } else {
  ...
  if (titleComponents & iTermTitleComponentsSessionName) {
    ...
    if (effectiveSessionName && profileName) {
      if ([effectiveSessionName isEqualToString:profileName]) {
        ...
        name = [NSString
            stringWithFormat:@"%@: %@", profileName, effectiveSessionName];
      }
    } else {
  ...
  const BOOL showUser =
      userVariable.length && (titleComponents & iTermTitleComponentsUser);
  const BOOL showHost =
      hostVariable.length && (titleComponents & iTermTitleComponentsHost);
  const BOOL showPWD = pwdVariable.length &&
                       (titleComponents & iTermTitleComponentsWorkingDirectory);
