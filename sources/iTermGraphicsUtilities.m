...
int iTermSetSmoothing(CGContextRef ctx, int *savedFontSmoothingStyle,
  ...
  if (style >= 0) {
    // This seems to be available at least on 10.8 and later. The only reference
    // to it is in WebKit. This causes text to render just a little lighter,
    // which looks nicer. It does not work in Mojave without subpixel AA.
