...
- (instancetype)initWithPhrase:(NSString *)phrase {
  ...
  if (self) {
    ...
    // First, parse an operator like tag: or name:, which must be at the
    // beginning.
    NSArray *operators =
        @[ kTagRestrictionOperator, kTitleRestrictionOperator ];
    ...
    if (phrase.length >= 2 && [phrase hasPrefix:@"\""] &&
        [phrase hasSuffix:@"\""]) {
      string = [phrase substringWithRange:NSMakeRange(1, phrase.length - 2)];
    }

    // If the first letter of the string is ^, use an anchored search. If it is
    // * do nothing because that used to be the operator for non-anchored
    // search.
    if ([string hasPrefix:@"^"]) {
...
- (BOOL)matchesAnyWordInWords:(NSArray *)words {
  ...
  for (int i = 0; i + _strings.count <= words.count; i++) {
    NSString *combinedWords =
        [[words subarrayWithRange:NSMakeRange(i, _strings.count)]
