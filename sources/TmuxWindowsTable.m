...
- (void)updateEnabledStateOfButtons {
  [addWindowButton_ setEnabled:[delegate_ haveSelectedSession] &&
                               [self filteredModel].count > 0];
  [removeWindowButton_ setEnabled:[delegate_ haveSelectedSession] &&
                                  [tableView_ numberOfSelectedRows] > 0];
  [openInTabsButton_ setEnabled:[delegate_ currentSessionSelected] &&
                                [tableView_ numberOfSelectedRows] > 1 &&
                                ![self anySelectedWindowIsOpen]];
  [openInWindowsButton_ setEnabled:[delegate_ currentSessionSelected] &&
                                   [tableView_ numberOfSelectedRows] > 0 &&
                                   ![self anySelectedWindowIsOpen]];
  if ([openInWindowsButton_ isEnabled] &&
      [tableView_ numberOfSelectedRows] == 1) {
    [openInWindowsButton_ setTitle:@"Open in Window"];
  } else {
    [openInWindowsButton_ setTitle:@"Open in Windows"];
  }
  [hideWindowButton_ setEnabled:[tableView_ numberOfSelectedRows] > 0 &&
                                [self allSelectedWindowsAreOpen]];
}
...
- (void)tableView:(NSTableView *)tableView
    willDisplayCell:(id)cell
     forTableColumn:(NSTableColumn *)tableColumn
                row:(NSInteger)row {
  if ([delegate_ haveOpenWindowWithId:[[[[self filteredModel] objectAtIndex:row]
...
- (id)tableView:(NSTableView *)aTableView
  ...
  NSString *name =
      [[[self filteredModel] objectAtIndex:rowIndex] objectAtIndex:0];
...
- (void)tableView:(NSTableView *)aTableView
    ...
    forTableColumn:(NSTableColumn *)aTableColumn
               row:(NSInteger)rowIndex {
  const int windowID = [[[[self filteredModel] objectAtIndex:rowIndex]
      objectAtIndex:1] intValue];
  [delegate_ renameWindowWithId:windowID toName:anObject];
}
...
- (BOOL)tableView:(NSTableView *)tv
  ...
  [pboard declareTypes:[NSArray arrayWithObject:kWindowPasteboardType]
                 owner:self];
  [pboard
      setPropertyList:[NSArray
                          arrayWithObjects:[delegate_ selectedSessionNumber],
                                           selectedItems, nil]
...
- (BOOL)nameMatchesFilter:(NSString *)name {
  ...
  return (!needle.length || [name rangeOfString:needle
