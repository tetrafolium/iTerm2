...
// Last time any window was resized TODO(georgen):it would be better to track
// per window.
static NSDate *lastResizeDate_;

NSString *const SessionViewWasSelectedForInspectionNotification =
    @"SessionViewWasSelectedForInspectionNotification";
...
@interface SessionView () <
    iTermAnnouncementDelegate, iTermFindDriverDelegate,
    iTermGenericStatusBarContainer, iTermSearchResultsMinimapViewDelegate,
    NSDraggingSource, PTYScrollerDelegate, SplitSelectionViewDelegate>
...
@implementation SessionView {
  ...
  // For macOS 10.14+ when subpixel AA is turned on and the scroller style is
  // legacy, this draws some blended default background color under the vertical
  // scroller. In all other conditions its frame is 0x0.
  iTermBackgroundColorView *
      _legacyScrollerBackgroundView NS_AVAILABLE_MAC(10_14);
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    [self registerForDraggedTypes:@[
      iTermMovePaneDragType, @"com.iterm2.psm.controlitem"
    ]];
    lastResizeDate_ = [NSDate date];
    ...
    if (@available(macOS 10.14, *)) {
      ...
      _backgroundColorView.frame =
          NSMakeRect(0, 0, frame.size.width, frame.size.height);
      _backgroundColorView.layer.actions =
      ...
      _legacyScrollerBackgroundView.layer.actions =
          @{@"backgroundColor" : [NSNull null]};
    ...
    _dropDownFindDriver = [[iTermFindDriver alloc]
        initWithViewController:_dropDownFindViewController];
    ...
    _scrollview =
        [[PTYScrollView alloc] initWithFrame:NSMakeRect(0, 0, aRect.size.width,
                                                        aRect.size.height)
    ...
    if (@available(macOS 10.11, *)) {
      _metalClipView = [[iTermMetalClipView alloc]
      ...
      if ([iTermAdvancedSettingsModel showLocationsInScrollbar]) {
        ...
        iTermTuple<NSColor *, NSColor *> * (^tuple)(NSColor *) =
          ...
          return [iTermTuple tupleWithObject:saturated
                                   andObject:[saturated colorDimmedBy:0.2
                                                     towardsGrayLevel:1]];
        };
        ...
        NSArray<iTermTuple<NSColor *, NSColor *> *> *colors = @[
          ...
          [iTermTuple tupleWithObject:[iTermTextDrawingHelper otherMarkColor]
                            andObject:[[iTermTextDrawingHelper otherMarkColor]
                                             colorDimmedBy:0.2
                                          towardsGrayLevel:1]],
        ...
        _marksMinimap =
            [[iTermIncrementalMinimapView alloc] initWithColors:colors];
      ...
      [[NSNotificationCenter defaultCenter]
          addObserver:self
             selector:@selector(preferredMetalDeviceDidChange:)
                 name:
                     iTermMetalDeviceProviderPreferredDeviceDidChangeNotification
               object:nil];
    }
#endif
    if (@available(macOS 10.14, *)) {
...
- (void)setTransparencyAlpha:(CGFloat)transparencyAlpha blend:(CGFloat)blend {
  if (@available(macOS 10.14, *)) {
    ...
    if (![iTermPreferences boolForKey:kPreferenceKeyPerPaneBackgroundImage]) {
      // This is unfortunate but because I can't use an imageview behind
      // everything when subpixel AA is enabled, I have to draw *something*
      // behind the legacy scrollers. NSImageView is not equipped to do the job.
...
- (NSRect)frameForScroller NS_AVAILABLE_MAC(10_14) {
  ...
  NSRect frame =
      NSMakeRect(mySize.width - size.width, 0, size.width, mySize.height);
  return frame;
}
...
- (iTermDropDownFindViewController *)newDropDownFindView {
  ...
  iTermDropDownFindViewController *dropDownViewController =
      [[iTermDropDownFindViewController alloc]
  ...
  [dropDownViewController
      setFrameOrigin:NSMakePoint(aRect.size.width - size.width - 30,
                                 aRect.size.height - size.height)];
  return dropDownViewController;
}

- (BOOL)isDropDownSearchVisible {
  return _findDriverType == iTermSessionViewFindDriverDropDown &&
         _dropDownFindDriver.isVisible;
}
...
- (BOOL)findViewIsHidden {
  ...
  case iTermSessionViewFindDriverPermanentStatusBar:
    return NO;
  case iTermSessionViewFindDriverTemporaryStatusBar:
    return self.delegate.sessionViewStatusBarViewController
               .temporaryLeftComponent == nil;
  }
  assert(false);
  return YES;
}
...
- (void)loadTemporaryStatusBarFindDriverWithStatusBarViewController:
  ...
  NSDictionary *knobs = @{
    iTermStatusBarPriorityKey : @(INFINITY),
...
- (void)showFindUI {
  ...
  if (_findDriverType == iTermSessionViewFindDriverPermanentStatusBar) {
    ...
    _temporaryStatusBarFindDriver = [[iTermFindDriver alloc]
        initWithViewController:statusBarViewController.temporaryLeftComponent
...
- (void)setUseMetal:(BOOL)useMetal
  ...
  if (useMetal != _useMetal) {
    ...
    iTermMetalClipView *metalClipView =
        (iTermMetalClipView *)_scrollview.contentView;
...
- (id<MTLDevice>)metalDevice {
  ...
  dispatch_once(&onceToken, ^{
    preferIntegrated =
        [iTermPreferences boolForKey:kPreferenceKeyPreferIntegratedGPU];
    if (preferIntegrated) {
      ...
      // I'm intentionally leaking devices and gpu because I'm seeing crazy
      // crashes where metal occasionally thinks something is over-released.
      // There's no reason to do that dangerous dance here.
      chosenDevice = gpu;
    } else {
...
- (void)installMetalViewWithDataSource:
    (id<iTermMetalDriverDataSource>)dataSource NS_AVAILABLE_MAC(10_11) {
  if (_metalView) {
    [self removeMetalView];
  }
  ...
  // Tell the clip view about it so it can ask the metalview to draw itself on
  // scroll.
  _metalClipView.metalView = _metalView;
  ...
  // Configure and hide the metal view. It will be shown by PTYSession after it
  // has rendered its first frame. Until then it's just a solid gray rectangle.
  _metalView.paused = YES;
  _metalView.enableSetNeedsDisplay = NO;
  _metalView.hidden = NO;
  _metalView.alphaValue = 0;

  // Start the metal driver going. It will receive delegate calls from MTKView
  // that kick off frame rendering.
...
- (void)setMetalViewNeedsDisplayInTextViewRect:(NSRect)textViewRect
    NS_AVAILABLE_MAC(10_11) {
  if (_useMetal) {
    // TODO: Would be nice to draw only the rect, but I don't see a way to do
    // that with MTKView that doesn't involve doing something nutty like saving
    // a copy of the drawable.
...
- (void)addSubviewBelowFindView:(NSView *)aView {
  if ([aView isKindOfClass:[PTYScrollView class]]) {
    NSInteger i = [self.subviews
        indexOfObjectPassingTest:^BOOL(__kindof NSView *_Nonnull obj,
  ...
  if (_dropDownFindViewController.view &&
      [self.subviews containsObject:_dropDownFindViewController.view]) {
    [self addSubview:aView
          positioned:NSWindowBelow
          relativeTo:[_dropDownFindViewController view]];
  } else {
...
- (NSRect)frameForLegacyScroller {
  ...
  return [_scrollview.verticalScroller
      convertRect:_scrollview.verticalScroller.bounds
           toView:self];
}
...
- (void)updateLayout {
  ...
  if ([_delegate sessionViewShouldUpdateSubviewsFramesAutomatically]) {
    ...
    if (self.showTitle) {
      ...
      [_title setFrame:NSMakeRect(0, maxY, _title.frame.size.width,
    ...
    if (_showBottomStatusBar) {
      _genericStatusBarContainer.frame =
          NSMakeRect(0, 0, self.frame.size.width,
...
- (NSRect)frameByInsettingForMetal:(NSRect)frame {
  if (@available(macOS 10.14, *)) {
    ...
    return NSInsetRect(
        frame, 1, [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins]);
  }
}
...
- (double)adjustedDimmingAmount {
  ...
  // Cap amount within reasonable bounds. Before 1.1, dimming amount was only
  // changed by twiddling the prefs file so it could have all kinds of crazy
  // values.
...
- (void)updateDim {
  ...
  iTermStatusBarViewController *statusBar =
      self.delegate.sessionViewStatusBarViewController;
...
// It's very expensive for PTYTextView to own its own tracking events because
// its frame changes constantly, plus it can miss mouse exit events and spurious
// mouse enter events (issue 3345). I believe it also caused hangs (issue 3974).
...
- (void)rightMouseDown:(NSEvent *)event {
  if (!_splitSelectionView) {
    ...
    if (inme) {
      ...
      // few steps through the OS) bring you back here. It happens when randomly
      // touching a bunch of fingers on the trackpad.
      return;
    }
    ++inme;
...
- (void)mouseDown:(NSEvent *)event {
  ...
  if (inme) {
    ...
    // consistently when dragging the pane title bar, but it happens
    // inconsistently with clicks in the title bar too.
    return;
  }
  ++inme;
  ...
  NSRect windowRect =
      [self.window convertRectFromScreen:NSMakeRect(p.x, p.y, 0, 0)];
  pointInSessionView = [self convertRect:windowRect fromView:nil].origin;
  DLog(@"Point in screen coords=%@, point in window coords=%@, point in "
       @"session view=%@",
       NSStringFromPoint(p), NSStringFromPoint(windowRect.origin),
...
- (void)setFrameSize:(NSSize)frameSize {
  ...
  if (frameSize.width < 340) {
    ...
    [_dropDownFindViewController
        setFrameOrigin:NSMakePoint(
                           frameSize.width - [findView frame].size.width - 30,
...
// This is called as part of the live resizing protocol when you let up the
// mouse button.
...
- (void)createSplitSelectionViewWithMode:(SplitSelectionViewMode)mode
                                 session:(id)session {
  ...
  _splitSelectionView = [[SplitSelectionView alloc]
      initWithMode:mode
         withFrame:NSMakeRect(0, 0, [self frame].size.width,
                              [self frame].size.height)
           session:session
          delegate:delegate];
  _splitSelectionView.wantsLayer =
      [iTermPreferences boolForKey:kPreferenceKeyUseMetal];
...
- (void)setSplitSelectionMode:(SplitSelectionMode)mode
  ...
  case kSplitSelectionModeOn:
    ...
    if (move) {
      [self createSplitSelectionViewWithMode:SplitSelectionViewModeTargetMove
                                     session:session];
    } else {
      [self createSplitSelectionViewWithMode:SplitSelectionViewModeTargetSwap
                                     session:session];
    }
    break;
  ...
  case kSplitSelectionModeCancel:
    if (move) {
      [self createSplitSelectionViewWithMode:SplitSelectionViewModeSourceMove
                                     session:session];
    } else {
      [self createSplitSelectionViewWithMode:SplitSelectionViewModeSourceSwap
                                     session:session];
    }
    break;

  case kSplitSelectionModeInspect:
    [self createSplitSelectionViewWithMode:SplitSelectionViewModeInspect
                                   session:session];
    break;
  }
}
  ...
  return [[iTermTheme sharedInstance]
      backgroundColorForDecorativeSubviewsInSessionWithTabColor:self.tabColor
                                         ...
                                         sessionBackgroundColor:
                                             [_delegate
                                                 sessionViewBackgroundColor]
                                               isFirstResponder:
                                                   [_delegate
                                                       sessionViewTerminalIsFirstResponder]
                                                    dimOnlyText:
                                                        [_delegate
                                                            sessionViewShouldDimOnlyText]
                                          adjustedDimmingAmount:
                                              [self adjustedDimmingAmount]];
}
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  if (_useMetal && _metalView.alphaValue == 1) {
    ...
    if (@available(macOS 10.14, *)) {
      // work around issue 7101. Draw a window background colored area under the
      // legacy scroller.
      if (_scrollview.isLegacyScroller &&
  ...
  if (_useMetal && _scrollview.isLegacyScroller &&
      ![iTermPreferences boolForKey:kPreferenceKeyHideScrollbar] &&
      [_scrollview.effectiveAppearance.name
          isEqualToString:NSAppearanceNameVibrantDark]) {
    [[NSColor colorWithWhite:20.0 / 255.0 alpha:1] set];
    NSRectFill(NSMakeRect(self.frame.size.width - 15, 0, self.frame.size.height,
                          self.frame.size.height));
  }
}
...
- (void)drawAroundFrame:(NSRect)svFrame dirtyRect:(NSRect)dirtyRect {
  ...
  if (svFrame.origin.x > 0) {
    [self drawBackgroundInRect:NSMakeRect(0, 0, svFrame.origin.x,
                                          self.frame.size.height)];
  }
  ...
  if (svFrame.size.width < self.frame.size.width) {
    ...
    [self drawBackgroundInRect:NSMakeRect(self.frame.size.width - widthDiff, 0,
                                          widthDiff, self.frame.size.height)];
  }
  // bottom
  if (svFrame.origin.y != 0) {
    [self drawBackgroundInRect:NSMakeRect(0, 0, self.frame.size.width,
                                          svFrame.origin.y)];
  }

  // top
  if (NSMaxY(svFrame) < self.frame.size.height) {
    [self drawBackgroundInRect:NSMakeRect(dirtyRect.origin.x, NSMaxY(svFrame),
...
- (NSRect)insetRect:(NSRect)rect
    ...
    includeBottomStatusBar:(BOOL)includeBottomStatusBar {
...
- (BOOL)setHoverURL:(NSString *)url {
  ...
  if (_hoverURLView == nil) {
    _hoverURLView = [[iTermHoverContainerView alloc]
        initWithFrame:NSMakeRect(0, 0, 100, 100)];
    _hoverURLView.url = url;
    _hoverURLTextField =
        [[NSTextField alloc] initWithFrame:_hoverURLView.bounds];
...
- (void)draggingSession:(NSDraggingSession *)session
  ...
  if (![[MovePaneController sharedInstance] dragFailed]) {
    [[MovePaneController sharedInstance] dropInSession:nil
                                                  half:kNoHalf
                                               atPoint:aPoint];
  }
}
...
- (BOOL)setShowBottomStatusBar:(BOOL)value
              adjustScrollView:(BOOL)adjustScrollView {
  ...
  if (_showBottomStatusBar) {
    ...
    [self invalidateStatusBar];
    if (adjustScrollView) {
      [_genericStatusBarContainer
          setAutoresizingMask:NSViewWidthSizable | NSViewMaxYMargin];
    }
...
- (void)invalidateStatusBar {
  ...
  switch ((iTermStatusBarPosition)[iTermPreferences
...
- (void)updateFindDriver {
  ...
  if (statusBarViewController.searchViewController &&
      statusBarViewController.temporaryLeftComponent == nil) {
    _findDriverType = iTermSessionViewFindDriverPermanentStatusBar;
    _permanentStatusBarFindDriver = [[iTermFindDriver alloc]
        initWithViewController:statusBarViewController.searchViewController];
...
- (NSSize)maximumPossibleScrollViewContentSize {
  ...
  DLog(@"maximumPossibleScrollViewContentSize. size=%@",
       [NSValue valueWithSize:size]);
  if (_showTitle) {
    size.height -= iTermGetSessionViewTitleHeight();
    DLog(@"maximumPossibleScrollViewContentSize: sub title height. size=%@",
         [NSValue valueWithSize:size]);
  }
  if (_showBottomStatusBar) {
    size.height -= iTermGetStatusBarHeight();
    DLog(@"maximumPossibleScrollViewContentSize: sub bottom status bar height. "
         @"size=%@",
  ...
  NSSize contentSize = [NSScrollView
      ...
      horizontalScrollerClass:nil
        verticalScrollerClass:verticalScrollerClass
                  ...
                  controlSize:NSControlSizeRegular
                scrollerStyle:[[[self scrollview] verticalScroller]
                                  scrollerStyle]];
  return contentSize;
}
...
- (void)updateTitleFrame {
  ...
  if (_showTitle) {
    [_title
        setFrame:NSMakeRect(
                     0, aRect.size.height - iTermGetSessionViewTitleHeight(),
                     aRect.size.width, iTermGetSessionViewTitleHeight())];
...
- (void)updateBottomStatusBarFrame {
  ...
  if (_showBottomStatusBar) {
    _genericStatusBarContainer.frame =
        NSMakeRect(0, 0, aRect.size.width, iTermGetStatusBarHeight());

    [_genericStatusBarContainer.statusBarViewController.view
        setNeedsLayout:YES];
  }
  [self updateScrollViewFrame];
  [self updateFindViewFrame];
...
- (void)updateFindViewFrame {
  ...
  [_dropDownFindViewController
      setFrameOrigin:NSMakePoint(
                         aRect.size.width - [findView frame].size.width - 30,
...
- (void)updateScrollViewFrame {
  ...
  CGFloat bottomStatusBarHeight =
      _showBottomStatusBar ? iTermGetStatusBarHeight() : 0;
  NSSize proposedSize =
  ...
  NSRect rect =
      NSMakeRect(0, bottomStatusBarHeight + proposedSize.height - size.height,
                 size.width, size.height);
  DLog(@"titleHeight=%@ bottomStatusBarHeight=%@ proposedSize=%@ size=%@ "
       @"rect=%@",
       @(titleHeight), @(bottomStatusBarHeight), NSStringFromSize(proposedSize),
       NSStringFromSize(size), NSStringFromRect(rect));
...
- (void)updateMinimapFrameAnimated:(BOOL)animated {
  ...
  if (@available(macOS 10.14, *)) {
    ...
    PTYScroller *scroller =
        [PTYScroller castFrom:self.scrollview.verticalScroller];
    ...
    if (animated) {
      [NSView
          animateWithDuration:5.0 / 60.0
                   animations:^{
                     [[NSAnimationContext currentContext]
                         setTimingFunction:[CAMediaTimingFunction
                                               functionWithName:@"easeOut"]];
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@:%p frame:%@ size:%@>", [self class],
                                    self, [NSValue valueWithRect:[self frame]],
                                    VT100GridSizeDescription(
                                        [_delegate sessionViewGridSize])];
}
...
- (NSColor *)sessionTitleViewBackgroundColor {
  if (!_showBottomStatusBar && _title.statusBarViewController) {
    NSColor *color = _title.statusBarViewController.layout.advancedConfiguration
                         .backgroundColor;
    if (color) {
      return color;
    }
...
- (void)showNextAnnouncement {
  ...
  if (_announcements.count) {
    iTermAnnouncementViewController *possibleAnnouncement =
        [self nextAnnouncement];
    ...
    // Animate in
    NSRect finalRect = NSMakeRect(
        0, self.frame.size.height - _currentAnnouncement.view.frame.size.height,
    ...
    _currentAnnouncement.view.autoresizingMask =
...
- (void)ptyScrollerDidTransitionToState:(PTYScrollerState)state {
  ...
  const CGFloat maxAlpha =
      _scrollview.verticalScroller.effectiveAppearance.it_isDark ? 0.5 : 0.75;
  switch (state) {
...
- (void)showUnobtrusiveMessage:(NSString *)message {
  ...
  if (@available(macOS 10.14, *)) {
    _unobtrusiveMessage =
        [[iTermUnobtrusiveMessage alloc] initWithMessage:message];
...
- (void)didSelectDestinationSession:(PTYSession *)session
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:SessionViewWasSelectedForInspectionNotification
                    object:self];
}
...
- (NSIndexSet *)searchResultsMinimapViewLocations:
  ...
  return [self.searchResultsMinimapViewDelegate
      searchResultsMinimapViewLocations:view];
}

- (NSRange)searchResultsMinimapViewRangeOfVisibleLines:
    (iTermSearchResultsMinimapView *)view NS_AVAILABLE_MAC(10_14) {
  return [self.searchResultsMinimapViewDelegate
      searchResultsMinimapViewRangeOfVisibleLines:view];
}
