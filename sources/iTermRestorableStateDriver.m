...
static NSString *const iTermRestorableStateControllerUserDefaultsKeyCount =
...
- (void)saveSynchronously:(BOOL)sync {
  ...
  const BOOL saved = [_saver saveSynchronously:sync
                                withCompletion:^{
                                  [weakSelf didSave];
                                }];
  // Do this after saveSynchronously:withCompletion:. It guarantees not to run
  // its completion block synchronously. It could fail if it was already busy
  // saving, in which case we don't want to reset _needsSave. Considering it is
  // busy, the other guy will eventually finish and cause didSave to be called,
...
- (void)restoreWithReady:(void (^)(void))ready
  ...
  __weak __typeof(self) weakSelf = self;
  [self.restorer loadRestorableStateIndexWithCompletion:^(
                     id<iTermRestorableStateIndex> index) {
...
- (void)restoreWithIndex:(id<iTermRestorableStateIndex>)index
  ...
  const NSInteger count = [[NSUserDefaults standardUserDefaults]
      integerForKey:iTermRestorableStateControllerUserDefaultsKeyCount];
  if (count > 1) {
    const iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:@"Some windows had trouble restoring last time "
                             @"iTerm2 launched. Try again?"
                     actions:@[ @"OK", @"Cancel" ]
                   accessory:nil
  ...
  _restoring = YES;
  [self reallyRestoreWindows:index
              withCompletion:^{
...
- (void)reallyRestoreWindows:(id<iTermRestorableStateIndex>)index
  ...
  // When all windows have finished being restored, mark the restoration as a
  // success.
  ...
  for (NSInteger i = 0; i < count; i++) {
    ...
    [self.restorer
        restoreWindowWithRecord:[index restorableStateRecordAtIndex:i]
                     completion:^{
