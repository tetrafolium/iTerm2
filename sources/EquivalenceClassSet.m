...
@implementation EquivalenceClassSet {
  ...
  NSMutableDictionary<NSNumber *, NSMutableSet<NSObject<NSCopying> *> *>
      *classes_;
}
...
- (void)log {
  [classes_ enumerateKeysAndObjectsUsingBlock:^(
                ...
                NSMutableSet<NSObject<NSCopying> *> *_Nonnull obj,
                BOOL *_Nonnull stop) {
    DLog(@"Class %@ contains: %@", key,
         [[obj.allObjects mapWithBlock:^id(NSObject<NSCopying> *anObject) {
...
- (void)setValue:(NSObject<NSCopying> *)n1
    equalToValue:(NSObject<NSCopying> *)n2 {
  DLog(@"Set %@ = %@", n1, n2);
  ...
  if (n1Class) {
    if (n2Class) {
      ...
      // n2 does not belong to an existing equivalence class yet so add it to
      // n1's class
    ...
    if (n2Class) {
      ...
      // Neither n1 nor n2 has an existing relation so create a new equivalence
      // class
