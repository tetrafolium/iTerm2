...
static dispatch_queue_t iTermPathCleanerQueue(void) {
  ...
  dispatch_once(&onceToken, ^{
    queue =
        dispatch_queue_create("com.iterm2.path-cleaner", DISPATCH_QUEUE_SERIAL);
  });
  return queue;
}
...
- (instancetype)initWithPath:(NSString *)path
  ...
  if (self) {
    ...
    _pathsToIgnore = [[iTermAdvancedSettingsModel pathsToIgnore]
...
- (NSString *)reallyCleanSynchronously:(NSString *)path {
  ...
  NSString *pathWithoutNearbyGunk =
      [self pathByStrippingEnclosingPunctuationFromPath:path
                                     lineAndColumnMatch:
                                         &stringToSearchForLineAndColumn];
  ...
  NSString *fullPath = [self getFullPath:pathWithoutNearbyGunk
                        workingDirectory:_workingDirectory];
  if (fullPath) {
    return fullPath;
  }

  // If path doesn't exist and it starts with "a/" or "b/" (from `diff`), try
  // again without the [ab]/ prefix.
  ...
  return [self getFullPath:pathWithoutNearbyGunk
          workingDirectory:_workingDirectory];
}

- (NSString *)getFullPath:(NSString *)pathExLineNumberAndColumn
         workingDirectory:(NSString *)workingDirectory {
  DLog(@"Check if %@ is a valid path in %@", pathExLineNumberAndColumn,
       workingDirectory);
  ...
  if ([path length] == 0) {
    // Everything was stripped out, meaning we'd try to open the working
    // directory.
    return nil;
  }
  if (![path hasPrefix:@"/"]) {
  ...
  // NOTE: The path used to be standardized first. While that would allow us to
  // catch network paths, it also caused filesystem access that would hit
  // network paths. That was also true for fileURLWithPath:.
...
- (void)extractFromSuffix:(NSString *)suffix {
  NSArray<NSString *> *regexes =
      [[self lineAndColumnNumberRegexes] mapWithBlock:^id(NSString *anObject) {
  ...
  for (NSString *regex in regexes) {
    ...
    NSArray<NSArray<NSString *> *> *matches =
        [suffix arrayOfCaptureComponentsMatchedByRegex:regex];
...
- (NSString *)pathByStrippingEnclosingPunctuationFromPath:(NSString *)path
  ...
  // Strip various trailing characters that are unlikely to be part of the file
  // name.
