...
NSString *const kPasteSpecialViewControllerUnicodeDashesRegularExpression =
    @"[" DASHES @"]";
NSString
    *const kPasteSpecialViewControllerUnicodeDoubleQuotesRegularExpression =
        @"[" DOUBLE_QUOTES @"]";
NSString
    *const kPasteSpecialViewControllerUnicodeSingleQuotesRegularExpression =
        @"[" SINGLE_QUOTES @"]";
...
static NSString *const kShouldConvertUnicodePunctuation =
...
@implementation iTermPasteSpecialViewController {
  ...
  IBOutlet NSTextField *_icuRegexHelpLabel; // Warning: this gets removed from
                                            // superview in awakeFromNib.
}

- (instancetype)init {
  self = [super initWithNibName:@"iTermPasteSpecialViewController"
...
- (void)awakeFromNib {
  ...
  [_icuRegexHelpLabel
      replaceWithHyperlinkTo:[NSURL URLWithString:@"https://iterm2.com/regex"]];
...
- (NSString *)descriptionForByteSize:(double)chunkSize {
  ...
  NSString *description = [NSString
      stringWithFormat:@"%@ %@B", [formatter stringFromNumber:@(chunkSize)],
                       units[exponent]];
  return description;
}
...
- (IBAction)delayBetweenChunksDidChange:(id)sender {
  ...
  _delayBetweenChunksLabel.stringValue =
      [self descriptionForDuration:_delayBetweenChunks];
  [_delegate pasteSpecialViewSpeedDidChange];
}

- (BOOL)controlIsTransform:(NSControl *)control {
  return (control != _bracketedPasteMode && control != _waitForPrompts &&
          control != _chunkSizeSlider && control != _delayBetweenChunksSlider);
}

- (IBAction)settingChanged:(id)sender {
  _spacesPerTab.enabled =
      (_tabTransform.enabled &&
...
- (void)controlTextDidChange:(NSNotification *)notification {
  if ([notification object] == _regex ||
      [notification object] == _substitution) {
    [_delegate pasteSpecialTransformDidChange];
  } else {
...
- (void)setEnableConvertUnicodePunctuation:
    (BOOL)enableConvertUnicodePunctuation {
...
- (void)setShouldConvertUnicodePunctuation:
    (BOOL)shouldConvertUnicodePunctuation {
  _convertUnicodePunctuation.state = shouldConvertUnicodePunctuation
...
- (void)setShouldConvertNewlines:(BOOL)shouldConvertNewlines {
  _convertNewlines.state =
      shouldConvertNewlines ? NSControlStateValueOn : NSControlStateValueOff;
}
...
- (void)setShouldRemoveNewlines:(BOOL)shouldRemoveNewlines {
  _removeNewlines.state =
      shouldRemoveNewlines ? NSControlStateValueOn : NSControlStateValueOff;
}
...
- (void)setEnableEscapeShellCharsWithBackslash:
    (BOOL)enableEscapeShellCharsWithBackslash {
...
- (void)setShouldEscapeShellCharsWithBackslash:
    (BOOL)shouldEscapeShellCharsWithBackslash {
  _escapeShellCharsWithBackslash.state = shouldEscapeShellCharsWithBackslash
                                             ? NSControlStateValueOn
                                             : NSControlStateValueOff;
}
...
- (void)setShouldRemoveControlCodes:(BOOL)shouldRemoveControlCodes {
  _removeControlCodes.state =
      shouldRemoveControlCodes ? NSControlStateValueOn : NSControlStateValueOff;
}
...
- (void)setShouldUseBracketedPasteMode:(BOOL)shouldUseBracketedPasteMode {
  _bracketedPasteMode.state = shouldUseBracketedPasteMode
                                  ? NSControlStateValueOn
                                  : NSControlStateValueOff;
}
...
- (void)setShouldBase64Encode:(BOOL)shouldBase64Encode {
  _base64Encode.state =
      shouldBase64Encode ? NSControlStateValueOn : NSControlStateValueOff;
}
...
- (void)setShouldUseRegexSubstitution:(BOOL)shouldUseRegexSubstitution {
  _useRegexSubstitution.state = shouldUseRegexSubstitution
                                    ? NSControlStateValueOn
                                    : NSControlStateValueOff;
...
- (void)setShouldWaitForPrompt:(BOOL)shouldWaitForPrompt {
  _waitForPrompts.state =
      shouldWaitForPrompt ? NSControlStateValueOn : NSControlStateValueOff;
}
...
+ (NSString *)descriptionForCodedSettings:(NSString *)jsonString {
  ...
  iTermTabTransformTags tabTransform =
      [dict[kSelectedTabTransform] integerValue];
  ...
  if (tabTransform == kTabTransformConvertToSpaces) {
    [components
        addObject:[NSString stringWithFormat:@"Tabs->%@ spcs",
                                             dict[kNumberOfSpacesPerTab]]];
  ...
  if ([dict[kShouldUseRegexSubstitution] boolValue]) {
    [components
        addObject:[NSString stringWithFormat:@"s/%@/%@/g",
...
+ (PasteEvent *)pasteEventForConfig:(NSString *)jsonConfig
  ...
  iTermTabTransformTags selectedTabTransform =
      [dict[kSelectedTabTransform] integerValue];
  ...
  PasteEvent *pasteEvent =
      [PasteEvent pasteEventWithString:string
                                 flags:flags
