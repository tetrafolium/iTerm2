...
- (instancetype)initWithURLActionHelper:
    (iTermURLActionHelper *)urlActionHelper {
  self = [super init];
  if (self) {
...
- (NSDictionary<NSNumber *, NSString *> *)
    smartSelectionActionSelectorDictionary {
  // The selector's name must begin with contextMenuAction to
  // pass validateMenuItem.
  return @{
    @(kOpenFileContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionOpenFile:)),
    @(kOpenUrlContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionOpenURL:)),
    @(kRunCommandContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionRunCommand:)),
    @(kRunCoprocessContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionRunCoprocess:)),
    @(kSendTextContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionSendText:)),
    @(kRunCommandInWindowContextMenuAction) :
        NSStringFromSelector(@selector(contextMenuActionRunCommandInWindow:))
  };
}

// This method is called by control-click or by clicking the hamburger icon in
// the session title bar. Two-finger tap (or presumably right click with a
// mouse) would go through mouseUp->
// PointerController->openContextMenuWithEvent.
- (NSMenu *)menuForEvent:(NSEvent *)theEvent {
  if (theEvent) {
    // Control-click
    if ([iTermPreferences
            boolForKey:kPreferenceKeyControlLeftClickBypassesContextMenu]) {
      return nil;
    }
    NSPoint clickPoint = [self.delegate contextMenu:self
                                         clickPoint:theEvent
  ...
  _savedSelectedText =
      [self.delegate contextMenuSelectedText:self capped:0].copy;
  menu.delegate = self;
  return menu;
}
...
- (NSMenu *)contextMenuWithEvent:(NSEvent *)event {
  ...
  DLog(@"contextMenuWithEvent:%@ x=%d, mark=%@, mark command=%@", event, x,
       mark, [mark command]);
  if (mark && mark.command.length) {
    ...
    if (locationInWindow.x < [iTermPreferences
  ...
  iTermImageInfo *imageInfo = [self.delegate contextMenu:self
                                        imageInfoAtCoord:coord];

  const long long overflow =
      [self.delegate contextMenuTotalScrollbackOverflow:self];
  iTermSelection *selection = [self.delegate contextMenuSelection:self];
  const BOOL clickedInExistingSelection =
      [selection containsAbsCoord:VT100GridAbsCoordMake(x, y + overflow)];
  if (!imageInfo && !clickedInExistingSelection) {
    // Didn't click on selection.
    // Save the selection and do a smart selection. If we don't like the result,
    // restore it.
    ...
    NSString *text = [[self.delegate contextMenuSelectedText:self
                                                      capped:0] copy];
    if (!text || !text.length ||
        [text rangeOfCharacterFromSet:nonWhiteSpaceSet].location ==
            NSNotFound) {
      // If all we selected was white space, undo it.
      [self.delegate contextMenu:self setSelection:savedSelection];
      _savedSelectedText = [[self.delegate contextMenuSelectedText:self
                                                            capped:0] copy];
    } else {
      _savedSelectedText = text;
    }
  } else if (clickedInExistingSelection &&
             [self.delegate contextMenuSelectionIsShort:self]) {
    _savedSelectedText = [[self.delegate contextMenuSelectedText:self
                                                          capped:0] copy];
  }
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
  if ([item action] == @selector(downloadWithSCP:)) {
    ...
    iTermSelection *selection = [self.delegate contextMenuSelection:self];
    const BOOL valid = [self.delegate
              contextMenu:self
        withRelativeCoord:selection.lastAbsRange.coordRange.start
                    block:^(VT100GridCoord coord) {
                      const BOOL haveShortSelection =
                          [self.delegate contextMenuSelectionIsShort:self];
                      NSString *selectedText =
                          [self.delegate contextMenuSelectedText:self capped:0];
                      result =
                          (haveShortSelection && [selection hasSelection] &&
                           [self.delegate contextMenu:self
                                       scpPathForFile:selectedText
                                               onLine:coord.y] != nil);
                    }];
    return (valid && result);
  }
  ...
  if ([item action] == @selector(showNotes:)) {
    ...
    const VT100GridCoordRange range = VT100GridCoordRangeMake(
        _validationClickPoint.x, _validationClickPoint.y,
        _validationClickPoint.x + 1, _validationClickPoint.y);
    return [self.delegate contextMenu:self hasOpenAnnotationInRange:range];
  }

  if ([self.smartSelectionActionSelectorDictionary.allValues
          containsObject:NSStringFromSelector(item.action)]) {
    return YES;
  }
...
- (NSMenu *)menuAtCoord:(VT100GridCoord)coord {
  ...
  if (imageInfo) {
    ...
    if (imageInfo.animated || imageInfo.paused) {
      NSString *selector =
          NSStringFromSelector(@selector(togglePauseAnimatingImage:));
      if (imageInfo.paused) {
        entryDicts = [entryDicts arrayByAddingObject:@{
          @"title" : @"Resume Animating",
          @"selector" : selector
        }];
      } else {
        entryDicts = [entryDicts arrayByAddingObject:@{
          @"title" : @"Stop Animating",
          @"selector" : selector
        }];
      }
    }
  ...
  if (selection.length == 1) {
    ...
    [self.delegate contextMenu:self
             withRelativeCoord:sub.absRange.coordRange.start
                         block:^(VT100GridCoord coord) {
                           iTermTextExtractor *extractor =
                               [self.delegate contextMenuTextExtractor:self];
                           const screen_char_t c =
                               [extractor characterAt:coord];
    ...
    [self.delegate contextMenu:self
             withRelativeCoord:selection.lastAbsRange.coordRange.start
                         block:^(VT100GridCoord coord) {
                           SCPPath *scpPath =
                               [self.delegate contextMenu:self
                                           scpPathForFile:shortSelectedText
                                                   onLine:coord.y];
                           if (scpPath) {
                             scpTitle = [NSString
                                 stringWithFormat:@"Download with scp from %@",
                                                  scpPath.hostname];
                           }
                         }];
  }
  ...
  void (^add)(NSString *, SEL) = ^(NSString *title, SEL selector) {
    [theMenu addItemWithTitle:title action:selector keyEquivalent:@""];
  ...
  if ([selection hasSelection] &&
    ...
    NSString *selectedText = [self.delegate contextMenuSelectedText:self
                                                             capped:1024];
    if ([self addCustomActionsToMenu:theMenu
                        matchingText:selectedText
                                line:coord.y]) {
      [theMenu addItem:[NSMenuItem separatorItem]];
    }
  }
  ...
  // Copy,  paste, and save
  [theMenu addItemWithTitle:NSLocalizedStringFromTableInBundle(
                                @"Copy", @"iTerm",
  ...
  // Terminal State
  [theMenu addItem:[NSMenuItem separatorItem]];
  NSMenuItem *terminalState =
      [[NSMenuItem alloc] initWithTitle:@"Terminal State"
                                 action:nil
                          keyEquivalent:@""];
  ...
  NSInteger j = 1;
  for (size_t i = 0;
       i < sizeof(terminalStateDecls) / sizeof(*terminalStateDecls); i++) {
    if (!terminalStateDecls[i].title) {
      [terminalState.submenu addItem:[NSMenuItem separatorItem]];
      continue;
    }
    NSMenuItem *item =
        [terminalState.submenu addItemWithTitle:terminalStateDecls[i].title
...
- (SEL)selectorForSmartSelectionAction:(NSDictionary *)action {
  NSDictionary<NSNumber *, NSString *> *dictionary =
      [self smartSelectionActionSelectorDictionary];
  ContextMenuActions contextMenuAction =
      [ContextMenuActionPrefsController actionForActionDict:action];
  return NSSelectorFromString(dictionary[@(contextMenuAction)]);
}

- (BOOL)addCustomActionsToMenu:(NSMenu *)theMenu
                  matchingText:(NSString *)textWindow
                          line:(int)line {
  BOOL didAdd = NO;
  NSArray *rulesArray = [self.delegate contextMenuSmartSelectionRules:self]
    ...
    for (int i = 0; i <= textWindow.length; i++) {
      ...
      if (components.count) {
        ...
        for (NSDictionary *action in actions) {
          ...
          NSString *workingDirectory = [self.delegate contextMenu:self
                                           workingDirectoryOnLine:line];
          VT100RemoteHost *remoteHost = [self.delegate contextMenu:self
                                                  remoteHostOnLine:line];
          NSString *theTitle = [ContextMenuActionPrefsController
                 titleForActionDict:action
          ...
          NSMenuItem *theItem = [[NSMenuItem alloc] initWithTitle:theTitle
                                                           action:mySelector
                                                    keyEquivalent:@""];
          NSString *parameter = [ContextMenuActionPrefsController
              parameterForActionDict:action
               withCaptureComponents:components
                    workingDirectory:workingDirectory
                          remoteHost:remoteHost];
...
- (BOOL)addAPIProvidedMenuItems:(NSMenu *)theMenu {
  NSArray<ITMRPCRegistrationRequest *> *reqs =
      [iTermAPIHelper contextMenuProviderRegistrationRequests];
  if (reqs.count == 0) {
    return NO;
  }
  [theMenu addItem:[NSMenuItem separatorItem]];
  for (ITMRPCRegistrationRequest *req in reqs) {
    NSMenuItem *theItem =
        [[NSMenuItem alloc] initWithTitle:req.contextMenuAttributes.displayName
...
- (NSMenu *)menuForMark:(VT100ScreenMark *)mark
...
- (void)contextMenuActionOpenFile:(id)sender {
  ...
  [[NSWorkspace sharedWorkspace]
      openFile:[[sender representedObject] stringByExpandingTildeInPath]];
}
  ...
  [self.delegate contextMenu:self
      withRelativeCoordRange:selection.lastAbsRange.coordRange
                       block:^(VT100GridCoordRange coordRange) {
                         SCPPath *scpPath =
                             [self.delegate contextMenu:self
                                         scpPathForFile:parts[0]
                                                 onLine:coordRange.start.y];
                         [_urlActionHelper
                             downloadFileAtSecureCopyPath:scpPath
                                              displayName:selectedText
                                           locationInView:coordRange];
                       }];
}

- (void)browse:(id)sender {
  [_urlActionHelper findUrlInString:[self.delegate contextMenuSelectedText:self
                                                                    capped:0]
...
- (void)searchInBrowser:(id)sender {
  NSURL *url = [NSURL
      urlByReplacingFormatSpecifier:@"%@"
                           inString:[iTermAdvancedSettingsModel searchCommand]
                          withValue:[self.delegate contextMenuSelectedText:self
                                                                    capped:0]];
...
- (void)addTrigger:(id)sender {
  [self.delegate contextMenu:self
                  addTrigger:[self.delegate contextMenuSelectedText:self
                                                             capped:0]];
}
...
- (void)reRunCommand:(id)sender {
  NSString *command = [sender representedObject];
  [self.delegate contextMenu:self
                  insertText:[command stringByAppendingString:@"\n"]];
}

- (void)selectCommandOutput:(id)sender {
  VT100ScreenMark *mark = [sender representedObject];
  VT100GridCoordRange range = [self.delegate contextMenu:self
                             rangeOfOutputForCommandMark:mark];
  if (range.start.x == -1) {
  ...
  VT100GridAbsCoordRange absRange =
      VT100GridAbsCoordRangeFromCoordRange(range, overflow);
  iTermSelection *selection = [self.delegate contextMenuSelection:self];
...
- (void)togglePauseAnimatingImage:(NSMenuItem *)item {
  [self.delegate contextMenu:self
      toggleAnimationOfImage:item.representedObject];
}
...
- (void)apiMenuItem:(NSMenuItem *)item {
  ...
  NSArray<ITMRPCRegistrationRequest *> *reqs =
      [iTermAPIHelper contextMenuProviderRegistrationRequests];
  for (ITMRPCRegistrationRequest *req in reqs) {
    if (![req.contextMenuAttributes.uniqueIdentifier
    ...
    [iTermScriptFunctionCall
        callFunction:invocation
          ...
          completion:^(id value, NSError *error,
                       NSSet<NSString *> *missingFunctions) {
            if (error) {
              [self.delegate contextMenu:self
                              invocation:invocation
                         failedWithError:error
                             forMenuItem:item.title];
            }
          }];
    return;
  }
...
- (NSString *)invocationForAPIRegistrationRequest:
    (ITMRPCRegistrationRequest *)req {
...
- (NSString *)hexOrDecimalConversionHelp {
  ...
  if (is32bit) {
    ...
    NSString *formattedDecimalValue =
        [numberFormatter stringFromNumber:@(intValue)];
    if (decToHex) {
      if (intValue < 0) {
        humanReadableSize = @"";
      }
      return [NSString stringWithFormat:@"%@ = 0x%x%@", formattedDecimalValue,
                                        intValue, humanReadableSize];
    } else if (intValue >= 0) {
      return
          [NSString stringWithFormat:@"0x%x = %@%@", intValue,
      ...
      return [NSString stringWithFormat:@"0x%x = %@ or %@%@", intValue,
                                        formattedDecimalValue,
                                        formattedUnsignedDecimalValue,
                                        humanReadableSize];
    }
  } else {
    // 64-bit value
    ...
    NSString *formattedDecimalValue =
        [numberFormatter stringFromNumber:decimalNumber];
    if (decToHex) {
      if (!mustBePositive && signedValue < 0) {
        humanReadableSize = @"";
      }
      return [NSString stringWithFormat:@"%@ = 0x%llx%@", formattedDecimalValue,
                                        value, humanReadableSize];
    } else if (signedValue >= 0) {
      return
          [NSString stringWithFormat:@"0x%llx = %@%@", value,
                                     formattedDecimalValue, humanReadableSize];
    } else {
      ...
      return [NSString
          stringWithFormat:@"0x%llx = %@ or %@%@", value, formattedDecimalValue,
                           formattedUnsignedDecimalValue, humanReadableSize];
    }
  }
}
...
- (NSString *)timestampConversionHelp {
  ...
  if (date) {
    ...
    [fmt setDateFormat:[NSDateFormatter
...
+ (instancetype)stringWithHumanReadableSize:(unsigned long long)value {
  ...
  // Show 2 fraction digits, always rounding downwards. Printf rounds floats to
  // the nearest representable value, so do the calculation with integers until
  // we get 100-fold the desired value, and then switch to float.
  if (100 * num % 1024 != 0) {
    exact = NO;
  }
  num = 100 * num / 1024;
  NSArray *iecPrefixes = @[ @"Ki", @"Mi", @"Gi", @"Ti", @"Pi", @"Ei" ];
  return [NSString stringWithFormat:@"%@%.2f %@", exact ? @"" : @ "≈",
                                    (double)num / 100, iecPrefixes[pow]];
}
...
- (NSString *)utf8Help {
  ...
  CFRange graphemeClusterRange =
      CFStringGetRangeOfComposedCharactersAtIndex((CFStringRef)self, 0);
  ...
  for (size_t i = 0; utf8[i]; i++) {
    [byteStrings
        addObject:[NSString stringWithFormat:@"0x%02x", utf8[i] & 0xff]];
  }
  ...
  for (NSUInteger i = 0; i < numCharacters; i++) {
    ...
    [ucs4Strings
        addObject:[NSString stringWithFormat:@"U+%04x", characters[i]]];
  }
  NSString *ucs4String = [ucs4Strings componentsJoinedByString:@" "];

  return [NSString
      stringWithFormat:@"“%@” = %@ = %@ (UTF-8)", self, ucs4String, utf8String];
}
...
- (NSDate *)dateValueFromUnix {
  ...
  Format formats[] = {{.regex = @"^1[0-9]{9}$", .divisor = 1},
                      {.regex = @"^1[0-9]{12}$", .divisor = 1000},
                      {.regex = @"^1[0-9]{15}$", .divisor = 1000000},
                      {.regex = @"^1[0-9]{9}\\.[0-9]+$", .divisor = 1}};
...
- (NSDate *)dateValueFromUTC {
  NSArray<NSString *> *formats = @[
    @"E, d MMM yyyy HH:mm:ss zzz", @"yyyy-MM-dd'T'HH:mm:ss.SSS'Z'",
    @"yyyy-MM-dd't'HH:mm:ss.SSS'z'", @"yyyy-MM-dd'T'HH:mm:ss'Z'",
    @"yyyy-MM-dd't'HH:mm:ss'z'", @"yyyy-MM-dd'T'HH:mm'Z'",
