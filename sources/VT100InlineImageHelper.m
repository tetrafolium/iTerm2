...
- (void)broke {
  ...
  _image =
      [iTermImage imageWithNativeImage:[NSImage it_imageNamed:@"broken_image"
                                                     forClass:self.class]];
  assert(_image);
}
...
- (void)appendBase64EncodedData:(NSString *)data {
  ...
  if (!_preconfirmed) {
    [self.delegate
        inlineImageConfirmBigDownloadWithBeforeSize:lengthBefore
...
- (void)writeToGrid:(VT100Grid *)grid {
  ...
  // Add a mark after the image. When the mark gets freed, it will release the
  // image's memory.
...
- (void)getRequestedHeightInPoints:(CGFloat *)requestedHeightInPointsPtr
  ...
  case kVT100TerminalUnitsPercentage: {
    ...
    *heightPtr =
        ceil((double)grid.sizeRespectingRegionConditionally.height * fraction);
    *requestedHeightInPointsPtr =
        grid.sizeRespectingRegionConditionally.height * cellSize.height *
        fraction;
    break;
  }
  case kVT100TerminalUnitsCells:
...
- (void)getRequestedWidthInPoints:(CGFloat *)requestedWidthInPointsPtr
  ...
  *heightPtr = MAX(1, *heightPtr);
  ...
  const CGFloat maxWidth =
      grid.sizeRespectingRegionConditionally.width - grid.cursorX;
  ...
  // Height is capped at 255 because only 8 bits are used to represent the line
  // number of a cell within the image.
...
- (NSEdgeInsets)fractionalInsetForInset:(NSEdgeInsets)inset
  ...
  return [iTermImageInfo
      fractionalInsetsForPreservedAspectRatioWithDesiredSize:desiredSize
                                                forImageSize:decodedImage.image
                                                                 .size
                                                    cellSize:cellSize
                                               numberOfCells:NSMakeSize(
                                                                 width,
                                                                 height)];
}
...
- (screen_char_t)
    screenCharacterForWidthInPoints:(CGFloat)requestedWidthInPoints
                       widthInCells:(int)width
                     heightInPoints:(CGFloat)requestedHeightInPoints
                      heightInCells:(int)height
                           cellSize:(NSSize)cellSize
                       decodedImage:(VT100DecodedImage *)decodedImage {
  const NSEdgeInsets inset =
      [self insetsForWidthInPoints:requestedWidthInPoints
                      widthInCells:width
  ...
  const NSEdgeInsets fractionalInset =
      [self fractionalInsetForInset:inset
                       ...
                       decodedImage:decodedImage
                              width:width
                             height:height];
  return ImageCharForNewImage(_name, width, height, _preserveAspectRatio,
                              fractionalInset);
}
...
- (void)writeBaseCharacter:(screen_char_t)screenChar
  ...
  DLog(@"Append %d rows of image characters with %d columns. The value of "
       @"c.image is %@",
       height, width, @(screenChar.image));
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = xOffset; x < xOffset + width && x < screenWidth; x++) {
      ...
      // DLog(@"Set character at %@,%@: %@", @(x), @(currentGrid_.cursorY),
      // DebugStringForScreenChar(c));
...
- (void)getScreenCharacter:(screen_char_t *)cPtr
  ...
  screen_char_t c =
      [self screenCharacterForWidthInPoints:requestedWidthInPoints
                               widthInCells:width
                             heightInPoints:requestedHeightInPoints
                              heightInCells:height
                                   cellSize:cellSize
