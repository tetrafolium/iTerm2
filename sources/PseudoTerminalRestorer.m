...
+ (void)setPostRestorationCompletionBlock:
  ...
  if (gRanQueuedBlocks && gExternalRestorationDidComplete) {
    ...
    if (gPostRestorationCompletionBlock) {
      void (^oldBlock)(void) =
          [[gPostRestorationCompletionBlock retain] autorelease];
      gPostRestorationCompletionBlock = [^{
      ...
      DLog(@"replace postretoration block %p with new one %p", oldBlock,
           gPostRestorationCompletionBlock);
    } else {
...
+ (void)restoreWindowWithIdentifier:(NSString *)identifier
  ...
  [self restoreWindowWithIdentifier:identifier
                pseudoTerminalState:[[[PseudoTerminalState alloc]
                                        initWithCoder:state] autorelease]
                             system:YES
...
+ (void)restoreWindowWithIdentifier:(NSString *)identifier
  ...
  if (system && [iTermUserDefaults ignoreSystemWindowRestoration]) {
    DLog(@"Ignore system window restoration because we're using our own "
         @"restorable state controller.");
  ...
  if (arrangement) {
    ...
    VoidBlock theBlock = ^{
      ...
      DLog(@"Creating term");
      PseudoTerminal *term =
          [PseudoTerminal bareTerminalWithArrangement:arrangement
      ...
      if (!term.togglingLionFullScreen) {
        ...
        // Keep any more blocks from running until this window finishes entering
        // fullscreen.
        ...
        term.didEnterLionFullscreen = ^(PseudoTerminal *theTerm) {
          ...
          DLog(@"%@ finished entering fullscreen, running completion handler",
               theTerm);
          term.restoringWindow = YES;
