...
- (instancetype)initWithPreviousRevision:
    (iTermEncoderGraphRecord *_Nullable)previousRevision {
...
- (instancetype)initWithKey:(NSString *)key
  ...
  if (self) {
    if (previousRevision && !previousRevision.rowid) {
      ITBetaAssert(NO, @"Previous revision lacks a rowID: %@",
                   previousRevision);
    }
...
- (BOOL)encodeChildWithKey:(NSString *)key
  ...
  iTermEncoderGraphRecord *record =
      [_previousRevision childRecordWithKey:key identifier:identifier];
  if (!record) {
  ...
  if (record.generation == generation) {
    ...
    DLog(@"Record %@[%@] at generation %@ didn't change", key, identifier,
         @(generation));
  ...
  iTermGraphEncoder *encoder =
      [[iTermGraphDeltaEncoder alloc] initWithKey:key
                                       identifier:identifier
...
- (BOOL)enumerateRecords:(void (^)(iTermEncoderGraphRecord *_Nullable before,
  ...
  return [self enumerateBefore:_previousRevision
                         after:self.record
                        parent:self.record.rowid
                          path:@"root"
                         block:block];
}
...
- (BOOL)enumerateBefore:(iTermEncoderGraphRecord *)preRecord
                 ...
                 parent:(NSNumber *)parent
                  ...
                  block:(void (^)(iTermEncoderGraphRecord *_Nullable before,
                                  ...
                                  NSNumber *parent, NSString *path,
                                  BOOL *stop))block {
  iTermOrderedDictionary<NSDictionary *, iTermEncoderGraphRecord *>
      *beforeDict = [iTermOrderedDictionary
          byMapping:preRecord.graphRecords
              block:^id _Nonnull(NSUInteger index,
                                 iTermEncoderGraphRecord *_Nonnull record) {
                return
                    @{@"key" : record.key, @"identifier" : record.identifier};
              }];
  iTermOrderedDictionary<NSDictionary *, iTermEncoderGraphRecord *> *afterDict =
      [iTermOrderedDictionary
          byMapping:postRecord.graphRecords
              block:^id _Nonnull(NSUInteger index,
                                 iTermEncoderGraphRecord *_Nonnull record) {
                return
                    @{@"key" : record.key, @"identifier" : record.identifier};
  ...
  __block BOOL ok = YES;
  void (^handle)(NSDictionary *, iTermEncoderGraphRecord *, NSString *,
                 BOOL *) = ^(NSDictionary *key, iTermEncoderGraphRecord *record,
  ...
  [beforeDict.keys
      enumerateObjectsUsingBlock:^(NSDictionary *_Nonnull key, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
