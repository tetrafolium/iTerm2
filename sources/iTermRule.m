...
+ (instancetype)ruleWithString:(NSString *)string {
  // Any rule may begin with ! to indicate it is sticky (it will be reverted to
  // in the future if no APS rule matches). hostname username@ username@hostname
  ...
  if (atSign != NSNotFound) {
    ...
    if (colon != NSNotFound && colon < atSign) {
      ...
      // user@host:path
      hostname = [string
          substringWithRange:NSMakeRange(atSign + 1, colon - atSign - 1)];
    } else if (colon == NSNotFound) {
      // user@host
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p hostname=%@ username=%@ path=%@ job=%@>",
                       [self class], self, self.hostname, self.username,
                       self.path, self.job];
}

// This is a monotonically increasing function whose range is [0, 1) for the
// domain of nonnegative values. It grows very slowly so that for any value of x
// that could be a Unix path name, squash(x + 1) - squash(x) > machine_epsilon.
...
- (double)scoreForHostname:(NSString *)hostname
  ...
  if (self.hostname != nil) {
    ...
    if (wildcardPos.location == NSNotFound &&
        [hostname isEqualToString:self.hostname]) {
      score += kHostExactMatchScore;
    } else if ([self.hostname isEqualToString:@"*"]) {
      if (![self haveAnyComponentBesidesHostname]) {
        // This is for backward compatibility. Previously, a hostname of * would
        // be treated the same as not having a host name at all. That made sense
        // from a scoring POV because you shouldn't get a higher score than a
        // profile that doesn't specify a hostname at all, as they are exactly
        // equivalent. However, if you specify only a hostname of * that should
        // outrank a profile without any APS rules at all. I decided to give the
        // lowest possible score because it's a catch-all, so any other rule
        // should outrank it.
        score += kCatchallRuleScore;
      }
    } else if ([hostname stringMatchesGlobPattern:self.hostname
                                    caseSensitive:NO]) {
      score +=
          kHostPartialMatchScore * (1.0 + [self squash:self.hostname.length]);
  ...
  if (self.path != nil) {
    // Make sure path ends in a / so a path glob pattern "/foo/bar/*" will match
    // a path of "/foo/bar".
    ...
    if (wildcardPos.location == NSNotFound &&
        [path isEqualToString:self.path]) {
      score += kPathExactMatchScore;
    } else if ([pathForGlob stringMatchesGlobPattern:self.path
                                       caseSensitive:YES]) {
...
- (BOOL)haveAnyComponentBesidesHostname {
  return (self.username != nil || self.path != nil || self.job != nil);
}
