...
- (BOOL)performActionWithCapturedStrings:(NSString *const *)capturedStrings
  ...
  output.mark =
      [aSession markAddedAtCursorOfClass:[iTermCapturedOutputMark class]];
  [aSession addCapturedOutput:output];
  return NO;
}

- (void)showCapturedOutputToolNotVisibleAnnouncementInSession:
    (PTYSession *)aSession {
  if ([aSession hasAnnouncementWithIdentifier:
                    kSuppressCaptureOutputToolNotVisibleWarning]) {
    return;
  }
  NSString *theTitle = @"A Capture Output trigger fired, but the Captured "
                       @"Output tool is not visible.";
  [aSession retain];
  ...
  iTermAnnouncementViewController *announcement =
      [iTermAnnouncementViewController
          announcementWithTitle:theTitle
...
- (void)showShellIntegrationRequiredAnnouncementInSession:
    (PTYSession *)aSession {
  NSString *theTitle = @"A Capture Output trigger fired, but Shell Integration "
                       @"is not installed.";
  [aSession retain];
  void (^completion)(int selection) = ^(int selection) {
    switch (selection) {
...
- (void)activateOnOutput:(CapturedOutput *)capturedOutput
  ...
  if (!session.hasCoprocess) {
    [self
        paramWithBackreferencesReplacedWithValues:capturedOutput.values
                                            scope:session.variablesScope
                                 useInterpolation:
                                     session
                                         .triggerParametersUseInterpolatedStrings
                                       completion:^(NSString *command) {
                                         if (command) {
                                           [session launchCoprocessWithCommand:
                                                        command];
                                         }
                                       }];
  } else {
    iTermAnnouncementViewController *announcement =
        [iTermAnnouncementViewController
            announcementWithTitle:@"Can't run two coprocesses at once."
                            style:kiTermAnnouncementViewStyleWarning
                      withActions:@[]
