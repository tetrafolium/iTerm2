...
- (instancetype)initWithPODs:(NSDictionary<NSString *, id> *)pods
  ...
  if (self) {
    ...
    [graphRecords
        enumerateObjectsUsingBlock:^(iTermEncoderGraphRecord *_Nonnull child,
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<iTermEncoderGraphRecord: rowid=%@ key=%@ gen=%@ "
                       @"id=%@ pod=%@ graphs=%@>",
                       self.rowid, self.key, @(self.generation),
                       self.identifier, self.pod,
                       [[self.graphRecords mapWithBlock:^id(
                                               iTermEncoderGraphRecord
                                                   *anObject) {
                         return [NSString
                             stringWithFormat:@"<graph rowid=%@ key=%@ id=%@>",
...
- (void)setRowid:(NSNumber *)rowid {
  if (_rowid != nil) {
    @throw [NSException
        exceptionWithName:@"DuplicateRowID"
                   reason:[NSString stringWithFormat:@"_rowid=%@ setRowid:%@",
                                                     _rowid, rowid]
                 userInfo:nil];
  }
  assert(_rowid == nil);
...
- (BOOL)isEqual:(id)object {
  ...
  if (![[other.graphRecords
          sortedArrayUsingSelector:@selector(compareGraphRecord:)]
          isEqual:[self.graphRecords sortedArrayUsingSelector:@selector
                                     (compareGraphRecord:)]]) {
    return NO;
  }
...
- (iTermEncoderGraphRecord *_Nullable)childRecordWithKey:(NSString *)key
                                              identifier:
                                                  (NSString *)identifier {
  return
      [_graphRecords objectPassingTest:^BOOL(iTermEncoderGraphRecord *element,
...
- (void)enumerateArrayWithKey:(NSString *)key
  ...
  NSArray<NSString *> *order = [[NSString castFrom:record.pod[@"__order"]]
                                   componentsSeparatedByString:@"\t"]
                                   ?: @[];
  order = [order arrayByRemovingDuplicatesStably];
  NSDictionary *items = [[record.graphRecords
      classifyWithBlock:^id(iTermEncoderGraphRecord *itemRecord) {
        return itemRecord.identifier;
      }] mapValuesWithBlock:^id(id key,
                                NSArray<iTermEncoderGraphRecord *> *object) {
    return object.firstObject.propertyListValue;
  }];
  [order enumerateObjectsUsingBlock:^(NSString *_Nonnull key, NSUInteger idx,
                                      BOOL *_Nonnull stop) {
...
- (NSArray *)arrayWithKey:(NSString *)key {
  ...
  [self enumerateArrayWithKey:key
                        block:^(NSString *_Nonnull identifier, NSInteger index,
...
- (NSArray<iTermEncoderGraphRecord *> *_Nullable)recordArrayWithKey:
    (NSString *)key {
  iTermEncoderGraphRecord *record = [self childArrayWithKey:key];
  if (!record) {
    return nil;
...
- (id)objectWithKey:(NSString *)key
  ...
  if (!instance) {
    if (error) {
      *error = [[NSError alloc]
          initWithDomain:@"com.iterm2.graph-record"
                    code:1
                userInfo:@{
                  NSLocalizedDescriptionKey : @"No such record or wrong type"
                }];
    }
    return nil;
  }
...
- (NSInteger)integerWithKey:(NSString *)key
                      error:(out NSError *__autoreleasing _Nullable *_Nullable)
                                error {
  NSNumber *number = [self objectWithKey:key
...
- (id)objectWithKey:(NSString *)key class:(Class)theClass {
  ...
  iTermEncoderGraphRecord *record = [self childRecordWithKey:key
                                                  identifier:@""];
...
- (NSArray *)arrayValue {
  ...
  NSArray<NSString *> *order = [[NSString castFrom:self.pod[@"__order"]]
                                   componentsSeparatedByString:@"\t"]
                                   ?: @[];
  order = [order arrayByRemovingDuplicatesStably];
  NSDictionary *items = [[self.graphRecords
      classifyWithBlock:^id(iTermEncoderGraphRecord *itemRecord) {
        return itemRecord.identifier;
      }] mapValuesWithBlock:^id(id key,
                                NSArray<iTermEncoderGraphRecord *> *object) {
  ...
  NSArray *array = [order mapWithBlock:^id(NSString *key) {
    // Arrays encoded using `-[iTermGraphEncoder encodeObject:key:]` use
    // __arrayValue for their key in the POD.
...
- (NSDictionary *)dictionaryValue {
  ...
  for (iTermEncoderGraphRecord *graph in self.graphRecords) {
    if ([graph.key isEqualToString:@"__array"] ||
        [graph.key isEqualToString:@"__dict"]) {
...
// Was not originally encoded as a dictionary, but we can make one from it
// nonetheless. This is meant as a fallback and may lose information because it
// ignores identifiers.
...
- (NSData *)data {
  ...
  NSError *error = nil;
  NSData *data = [NSData it_dataWithSecurelyArchivedObject:self.pod
                                                     error:&error];
  if (error) {
...
+ (nullable instancetype)fromGraphRecord:(iTermEncoderGraphRecord *)record
