...
- (void)viewDidLayout {
  if (@available(macOS 10.16, *)) {
    // Big sur likes to change the height of this accessory view when the tab
    // bar is added or removed from being an accessory view. Luckily there's
    // enough wiggle room to keep it aligned.
...
- (void)updateLabel {
  ...
  BOOL deemphasized;
  NSString *string = [self.class stringForOrdinal:_ordinal
...
- (void)updateTextColor {
  if (@available(macOS 10.14, *)) {
    NSString *closest =
        [_label.effectiveAppearance bestMatchFromAppearancesWithNames:@[
  ...
  _label.textColor = _isMain ? [NSColor windowFrameTextColor]
                             : [NSColor colorWithWhite:0.67 alpha:1];
}
