...
static NSString *const kScreenCharSpacingCombiningMarksKey =
    @"Spacing Combining Marks";
static NSString *const kScreenCharInverseComplexCharMapKey =
    @"Inverse Complex Char Map";
...
+ (iTermStringLine *)stringLineWithString:(NSString *)string {
  ...
  return [[[self alloc] initWithScreenChars:screenChars
                                     length:string.length] autorelease];
}
...
- (instancetype)initWithScreenChars:(screen_char_t *)screenChars
  ...
  if (self) {
    _length = length;
    _stringValue = [ScreenCharArrayToString(screenChars, 0, length,
                                            &_backingStore, &_deltas) retain];
  }
  return self;
}
...
- (NSRange)rangeOfScreenCharsForRangeInString:(NSRange)rangeInString {
  ...
  NSInteger indexInScreenCharsOfFirstCharInRange =
      location + _deltas[MIN(_length - 1, location)];
  if (length == 0) {
  ...
  const NSInteger indexInScreenCharsOfLastCharInRange =
      indexInStringOfLastCharInRange +
      _deltas[MIN(_length - 1, indexInStringOfLastCharInRange)];
  const NSInteger numberOfScreenChars = indexInScreenCharsOfLastCharInRange -
                                        indexInScreenCharsOfFirstCharInRange +
                                        1;
...
- (BOOL)isEqualToScreenCharArray:(ScreenCharArray *)other {
  ...
  return (
      _line == other->_line && _length == other->_length &&
      _eol == other->_eol &&
...
static void CreateComplexCharMapIfNeeded() {
  if (!complexCharMap) {
    ...
    spacingCombiningMarkCodeNumbers =
        [[NSMutableSet alloc] initWithCapacity:1000];
...
screen_char_t ImageCharForNewImage(NSString *name, int width, int height,
  ...
  iTermImageInfo *imageInfo =
      [[[iTermImageInfo alloc] initWithCode:c.code] autorelease];
...
iTermImageInfo *GetImageInfo(unichar code) { return gImages[@(code)]; }

VT100GridCoord GetPositionOfImageInChar(screen_char_t c) {
  return VT100GridCoordMake(c.foregroundColor, c.backgroundColor);
}

int GetOrSetComplexChar(NSString *str, iTermTriState isSpacingCombiningMark) {
  CreateComplexCharMapIfNeeded();
  ...
  case iTermTriStateOther: {
    NSCharacterSet *scmSet =
        [NSCharacterSet spacingCombiningMarksForUnicodeVersion:12];
...
int AppendToComplexChar(int key, unichar codePoint) {
  ...
  if ([str length] == kMaxParts) {
    NSLog(@"Warning: char <<%@>> with key %d reached max length %d", str, key,
          kMaxParts);
    return key;
  }
  assert(str);
...
NSString *StringByNormalizingString(NSString *theString,
...
void SetComplexCharInScreenChar(screen_char_t *screenChar, NSString *theString,
                                iTermUnicodeNormalization normalization,
                                BOOL isSpacingCombiningMark) {
  NSString *normalizedString =
      StringByNormalizingString(theString, normalization);
  ...
  if (normalizedString.length == 1 && !isSpacingCombiningMark) {
    ...
    screenChar->code = GetOrSetComplexChar(
        theString, iTermTriStateFromBool(isSpacingCombiningMark));
...
BOOL StringContainsCombiningMark(NSString *s) {
  ...
  [s enumerateComposedCharacters:^(NSRange range, unichar simple,
...
NSString *ScreenCharArrayToString(screen_char_t *screenChars, int start,
                                  int end, unichar **backingStorePtr,
                                  int **deltasPtr) {
  const int lineLength = end - start;
  unichar *charHaystack =
      iTermMalloc(sizeof(unichar) * lineLength * kMaxParts + 1);
  ...
  *deltasPtr = deltas;
  ...
  //     r = i + deltas[i]
  //
  // Array of screen_char_t with some double-width characters, where DWC_RIGHT
  // is shown as '-', and d & f have combining marks/surrogate pairs (not shown
  // here): 0123456789 ab-c-de-fg
  //
  ...
  for (int i = start; i < end; ++i) {
    ...
    if (c >= ITERM2_PRIVATE_BEGIN && c <= ITERM2_PRIVATE_END) {
      // Skip private-use characters which signify things like double-width
      // characters and tab fillers.
      ++delta;
    } else {
...
NSString *DebugStringForScreenChar(screen_char_t c) {
  ...
  return [NSString
      stringWithFormat:
          @"<screen_char_t: code=%@ complex=%@ image=%@ url=%@ "
          @"foregroundColor=%@ fgGreen=%@ fgBlue=%@ backgroundColor=%@ "
          @"bgGreen=%@ bgBlue=%@ fgMode=%@ bgMode=%@ bold=%@ faint=%@ "
          @"italic=%@ blink=%@ underline=%@ strikethrough=%@ underlinestyle=%@ "
          @"unused=%@>",
          @(c.code), @(c.complexChar), @(c.image), @(c.urlCode),
          @(c.foregroundColor), @(c.fgGreen), @(c.fgBlue), @(c.backgroundColor),
          @(c.bgGreen), @(c.bgBlue), modes[c.foregroundColorMode],
          modes[c.backgroundColorMode], @(c.bold), @(c.faint), @(c.italic),
          @(c.blink), @(c.underline), @(c.strikethrough), @(c.underlineStyle),
          @(c.unused)];
}
...
void StringToScreenChars(NSString *s, screen_char_t *buf, screen_char_t fg,
                         screen_char_t bg, int *len,
                         BOOL ambiguousIsDoubleWidth, int *cursorIndex,
                         BOOL *foundDwc,
                         iTermUnicodeNormalization normalization,
                         NSInteger unicodeVersion) {
  __block NSInteger j = 0;
  __block BOOL foundCursor = NO;
  NSCharacterSet *zeroWidthSpaces = [NSCharacterSet
      zeroWidthSpaceCharacterSetForUnicodeVersion:unicodeVersion];
  NSCharacterSet *spacingCombiningMarks =
      [NSCharacterSet spacingCombiningMarksForUnicodeVersion:12];

  [s enumerateComposedCharacters:^(NSRange range, unichar baseBmpChar,
                                   NSString *composedOrNonBmpChar, BOOL *stop) {
    ...
    // Set the code and the complex flag. Also return early if no cell should be
    // used by this grapheme cluster. Set the isDoubleWidth flag.
    if (!composedOrNonBmpChar) {
      if ([zeroWidthSpaces characterIsMember:baseBmpChar]) {
      ...
      } else if (baseBmpChar >= ITERM2_PRIVATE_BEGIN &&
                 baseBmpChar <= ITERM2_PRIVATE_END) {
        ...
        baseBmpChar = UNICODE_REPLACEMENT_CHAR;
      } else if (IsHighSurrogate(baseBmpChar) &&
                 NSMaxRange(range) != s.length) {
    ...
    if (composedOrNonBmpChar) {
      ...
      if (composedLength > kMaxParts) {
        composedOrNonBmpChar =
            [composedOrNonBmpChar substringToIndex:kMaxParts];

        // Ensure a high surrogate isn't left dangling at the end.
        if (CFStringIsSurrogateHighCharacter(
                [composedOrNonBmpChar characterAtIndex:kMaxParts - 1])) {
          composedOrNonBmpChar =
              [composedOrNonBmpChar substringToIndex:kMaxParts - 1];
        }
      }
      ...
      if (IsHighSurrogate(baseChar) && composedLength > 1) {
        baseChar = DecodeSurrogatePair(
            baseChar, [composedOrNonBmpChar characterAtIndex:1]);
        next += 1;
      }
      ...
      if (!isDoubleWidth && composedLength > next) {
        ...
        if (peek == 0xfe0f) {
          // VS16
          if ([[NSCharacterSet emojiAcceptingVS16]
                  characterIsMember:baseChar] &&
...
void InitializeScreenChar(screen_char_t *s, screen_char_t fg,
                          screen_char_t bg) {
  s->code = 0;
  s->complexChar = NO;
...
NSInteger ScreenCharGeneration(void) { return gScreenCharGeneration; }

NSDictionary *ScreenCharEncodedRestorableState(void) {
  return @{
    kScreenCharComplexCharMapKey : complexCharMap ?: @{},
...
void ScreenCharGarbageCollectImages(void) {
  NSArray<NSNumber *> *provisionalKeys =
      [gImages.allKeys filteredArrayUsingBlock:^BOOL(NSNumber *key) {
...
void ScreenCharDecodeRestorableState(NSDictionary *state) {
  ...
  NSArray<NSString *> *spacingCombiningMarksArray =
      state[kScreenCharSpacingCombiningMarksKey];
  ...
  for (id key in imageMap) {
    ...
    iTermImageInfo *info =
        [[[iTermImageInfo alloc] initWithDictionary:imageMap[key]] autorelease];
    if (info) {
...
static NSString *ScreenCharColorDescription(unsigned int red,
                                            unsigned int green,
                                            unsigned int blue, ColorMode mode) {
  switch (mode) {
  ...
  case ColorModeNormal:
    ...
    if (red < 232) {
      ...
      return
          [NSString stringWithFormat:@"8bit(%@/5,%@/5,%@/5)", @(r), @(g), @(b)];
    }
    const int gray = red - 232;
    return [NSString stringWithFormat:@"gray%@/22", @(gray)];
  case ColorMode24bit:
    return [NSString
        stringWithFormat:@"24bit(%@,%@,%@)", @(red), @(green), @(blue)];
...
NSString *ScreenCharDescription(screen_char_t c) {
  ...
  return [NSString
      stringWithFormat:@"fg=%@ bg=%@%@",
                       ScreenCharColorDescription(c.foregroundColor, c.fgGreen,
                                                  c.fgBlue,
                                                  c.foregroundColorMode),
                       ScreenCharColorDescription(c.backgroundColor, c.bgGreen,
                                                  c.bgBlue,
