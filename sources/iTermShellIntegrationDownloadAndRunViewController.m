...
- (NSString *)urlString {
  if (self.installUtilities) {
    return @"https://iterm2.com/shell_integration/"
...
- (void)setInstallUtilities:(BOOL)installUtilities {
  ...
  NSString *prefix = self.busy ? @"Waiting for this command to finish:"
                               : @"Press “Continue” to run this command:";
...
- (void)showShellUnsupportedError {
  self.textField.stringValue =
      @"😞 Your shell is not supported, or perhaps your $SHELL environment "
      @"variable is not set correctly. Press “Continue” to try again.";
}
