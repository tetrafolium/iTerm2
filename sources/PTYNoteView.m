...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    NSImage *closeImage = [NSImage it_imageNamed:@"closebutton"
                                        forClass:self.class];
    killButton_ = [[NSButton alloc]
        initWithFrame:NSMakeRect(0, 0, kButtonSize, kButtonSize)];
...
static NSRect FlipRect(NSRect rect, CGFloat height) {
  return NSMakeRect(rect.origin.x, height - NSMaxY(rect), rect.size.width,
                    rect.size.height);
}
...
- (NSBezierPath *)roundedRectangleWithPointerInRect:(NSRect)frame
  ...
  NSPoint p = [self trailingCornerOfRoundedRect:bubbleFrame
                                         radius:radius
                                             dx:-1
                                             dy:0];
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Get the location of the point just before the rounded rect at the end of
    // this edge
    p = [self leadingCornerOfRoundedRect:bubbleFrame radius:radius dx:dx dy:dy];

    if (edge == tipEdge) {
      // This edge has the arrow. Compute the points where the arrow's base
      // intersects the edge.
      ...
      for (int j = 0; j < 2; j++) {
        if (edge == kPTYNoteViewTipEdgeTop ||
            edge == kPTYNoteViewTipEdgeBottom) {
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  struct {
    ...
    BOOL horizontal;
    BOOL bottom;
  } regions[] = {{rightDragRegion, YES, NO},
...
- (NSRect)rightDragRect {
  ...
  return FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kDragAreaSize,
                             NSMinY(bubbleFrame) + kDragAreaSize, kDragAreaSize,
...
- (NSRect)bottomRightDragRect {
  NSRect bubbleFrame = [self bubbleFrameInRect:self.bounds
                                 ...
                                 pointerLength:kPointerLength
                                     tipOnEdge:self.tipEdge];
  return FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kDragAreaSize,
                             NSMaxY(bubbleFrame) - kDragAreaSize, kDragAreaSize,
                             kDragAreaSize),
                  self.frame.size.height);
}
...
- (NSRect)bottomDragRect {
  NSRect bubbleFrame = [self bubbleFrameInRect:self.bounds
                                 ...
                                 pointerLength:kPointerLength
                                     tipOnEdge:self.tipEdge];
  return FlipRect(
      NSMakeRect(NSMinX(bubbleFrame), NSMaxY(bubbleFrame) - kDragAreaSize,
...
- (void)layoutSubviews {
  ...
  self.contentView.frame =
      FlipRect(NSMakeRect(NSMinX(bubbleFrame) + kLeftMargin,
               ...
               self.frame.size.height);
  killButton_.frame =
      FlipRect(NSMakeRect(NSMaxX(bubbleFrame) - kButtonSize,
...
- (NSSize)sizeThatFitsContentView {
  ...
  case kPTYNoteViewTipEdgeBottom:
  case kPTYNoteViewTipEdgeTop:
    return NSMakeSize(contentView_.frame.size.width + kLeftMargin +
                          kRightMargin + kInset,
                      contentView_.frame.size.height + kBottomMargin +
                          kTopMargin + kPointerLength + kInset);

  case kPTYNoteViewTipEdgeLeft:
  case kPTYNoteViewTipEdgeRight:
    return NSMakeSize(contentView_.frame.size.width + kLeftMargin +
                          kRightMargin + kPointerLength + kInset,
                      contentView_.frame.size.height + kBottomMargin +
                          kTopMargin + kInset);
  }
  assert(false);
