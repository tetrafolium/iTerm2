...
static NSString
    *const iTermSemanticHistoryPrefsControllerCaveatTextFieldDidClickOnLink =
...
+ (BOOL)applicationExists:(NSString *)bundleId {
  CFArrayRef appURLs = LSCopyApplicationURLsForBundleIdentifier(
      (__bridge CFStringRef)bundleId, nil);
...
+ (NSArray *)editorsInPreferenceOrder {
  ...
  return @[
    ...
    kBBEditIdentifier, kAtomIdentifier, kVSCodeIdentifier, kVSCodiumIdentifier,
    kVSCodeInsidersIdentifier, kEmacsAppIdentifier, kIntelliJIDEAIdentifier
  ];
}
...
- (void)awakeFromNib {
      ...
      addObserver:self
         selector:@selector(showPopover)
             name:
                 iTermSemanticHistoryPrefsControllerCaveatTextFieldDidClickOnLink
           object:nil];
  NSSet<NSString *> * (^fallbackSource)(NSString *) = [iTermVariableHistory
  ...
  _textFieldDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
      initWithPathSource:^NSSet<NSString *> *(NSString *prefix) {
        NSArray *filtered =
            [mine filteredArrayUsingBlock:^BOOL(NSString *path) {
  ...
  for (NSString *identifier in [[self class] editorsInPreferenceOrder]) {
    ...
    if (!item) {
      item = [[NSMenuItem alloc] initWithTitle:names[identifier]
                                        action:nil
                                 keyEquivalent:@""];
  ...
  NSArray *sortedNames = [items.allKeys
...
- (NSAttributedString *)attributedStringWithLearnMoreLinkAfterText:
    (NSString *)text {
  NSDictionary *attributes = @{
    NSFontAttributeName : caveat_.font
        ?: [NSFont systemFontOfSize:[NSFont smallSystemFontSize]]
  };
  NSAttributedString *legacy =
      [NSAttributedString attributedStringWithString:text
                                          attributes:attributes];
  NSAttributedString *learnMore =
      [NSAttributedString attributedStringWithLinkToURL:
                              @"iterm2-private://semantic-history-learn-more/"
                                                 string:@"Learn more"];
...
- (NSString *)detailTextForCurrentMode {
  NSString *subs =
      ...
      @"  \\(semanticHistory.lineNumber) will be replaced with the line "
      @"number.\n"
      @"  \\(semanticHistory.columnNumber) will be replaced with the column "
      @"number.\n"
      @"  \\(semanticHistory.prefix) will be replaced with the text before the "
      @"click.\n"
      @"  \\(semanticHistory.suffix) will be replaced with the text after the "
      @"click.\n"
      @"  \\(semanticHistory.workingDirectory) will be replaced with the "
      @"working directory.\n";
  ...
  case 5:
    return [@"In this mode semantic history will be activated on any click, "
            @"even if you click on something that is not an existing file.\n"
        stringByAppendingString:subs];

  case 2:
  case 4:
  case 6:
    return [@"In this mode semantic history will only be activated when you "
            @"click on an existing file name.\n" stringByAppendingString:subs];
  }
  return @"";
}
...
- (IBAction)actionChanged:(id)sender {
  ...
  switch ([[action_ selectedItem] tag]) {
  case 1:
    [caveat_ setStringValue:@"When you activate Semantic History on a "
                            @"filename, the associated app loads the file."];
    hideCaveat = NO;
    break;

  case 2: {
    [[text_ cell] setPlaceholderString:@"Enter URL."];
    NSString *text = @"When you activate Semantic History on a filename, the "
                     @"browser opens a URL.\n"
                     @"Use \\1 for the filename you clicked on and \\2 for the "
                     @"line number. ";
    caveat_.attributedStringValue =
        [self attributedStringWithLearnMoreLinkAfterText:text];
    hideCaveat = NO;
    hideText = NO;
    break;
  }
  ...
  case 3:
    hideEditors = NO;
    [caveat_
        setStringValue:@"When you activate Semantic History on a text file, "
                       @"the specified editor opens it.\nOther kinds of files "
                       @"will be opened with their default apps."];
    hideCaveat = NO;
    break;
  ...
  case 4: {
    ...
    NSString *text =
        ...
        @"Use \\1 for filename, \\2 for line number, \\3 for text before "
        @"click, \\4 for text after click, \\5 for pwd. "
  ...
  case 5: {
    ...
    NSString *text = @"Command runs when you activate Semantic History on any "
                     @"text, even if it's not a valid filename. "
                     @"Use \\1 for filename, \\2 for line number, \\3 for text "
                     @"before click, \\4 for text after click, \\5 for pwd. ";
...
- (void)setGuid:(NSString *)guid {
  ...
  // Check selected item
  [[[action_ menu] itemWithTag:[action_ selectedTag]]
