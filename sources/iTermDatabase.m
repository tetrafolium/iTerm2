...
- (void)unlink {
  ...
  NSError *error = nil;
  NSArray<NSURL *> *urls =
      [iTermSqliteDatabaseImpl allURLsForDatabaseAt:self.url];
...
- (NSString *)formatSQL:(NSString *)sql vaList:(va_list)args {
  NSString *fmt = [sql stringByReplacingOccurrencesOfString:@"?"
                                                 withString:@"“%@”"];
...
- (BOOL)passesIntegrityCheck {
  ...
  NSString *checkType =
      arc4random_uniform(100) == 0 ? @"integrity_check" : @"quick_check";
  FMResultSet *results =
      [_db executeQuery:[NSString stringWithFormat:@"pragma %@", checkType]];
  if (![results next]) {
...
- (NSString *)advisoryLockPath {
  NSURL *url = [_db.databaseURL.URLByDeletingLastPathComponent
      URLByAppendingPathComponent:@"lock"];
  return url.path;
}
...
- (BOOL)lock {
  ...
  do {
    ...
    fd = open(self.advisoryLockPath.UTF8String,
              O_CREAT | O_TRUNC | O_EXLOCK | O_NONBLOCK, 0600);
  } while (fd < 0 && errno == EINTR);
  if (fd >= 0) {
    DLog(@"Lock acquired");
    _advisoryLock = [[NSFileHandle alloc] initWithFileDescriptor:fd
                                                  closeOnDealloc:YES];
    return YES;
  } else {
...
- (BOOL)transaction:(BOOL (^NS_NOESCAPE)(void))block {
  ...
  {
    FMResultSet *rs =
        [_db executeQuery:@"select count(*) as c from Node where parent=0"];
    if ([rs next]) {
