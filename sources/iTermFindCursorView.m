...
- (void)drawRect:(CGRect)dirtyRect {
  ...
  NSPoint relativeCursorPosition =
      NSMakePoint(2 * (self.cursorPosition.x / self.frame.size.width - 0.5),
  ...
  NSBezierPath *circle = [NSBezierPath
      bezierPathWithOvalInRect:NSMakeRect(x - focusRadius, y - focusRadius,
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    ...
    _arrowLayer.contents =
        (id)[image CGImageForProposedRect:nil context:nil hints:nil];
    [self.layer addSublayer:_arrowLayer];

    [CATransaction begin];
    CAKeyframeAnimation *spinAnim =
        [CAKeyframeAnimation animationWithKeyPath:@"transform"];
    spinAnim.duration = 0.75;

    // Animations take the shortest path.
    // Specifying 0-2*pi doesn't work (that's no animation).
    // Specifying a keyframe of 0 -> pi -> 2*pi doesn't work (it goes to 180
    // degrees and then back the way it came) It works to do 0 -> pi -> 2*pi +
    // epsilon, although it shouldn't. I could do it with three keyframe steps
    // but it's strange to work with 2pi/3, so I use four instead.
    spinAnim.values = @[
      [NSValue valueWithCATransform3D:CATransform3DMakeRotation(0, 0, 0, 1)],
      [NSValue
          valueWithCATransform3D:CATransform3DMakeRotation(M_PI_2, 0, 0, 1)],
      [NSValue valueWithCATransform3D:CATransform3DMakeRotation(M_PI, 0, 0, 1)],
      [NSValue valueWithCATransform3D:CATransform3DMakeRotation(3.0 * M_PI_2, 0,
                                                                0, 1)],
      [NSValue valueWithCATransform3D:CATransform3DMakeRotation(0, 0, 0, 1)]
    ];
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    ...
    _emitterLayer.emitterPosition = CGPointMake(
        self.bounds.size.width / 2, self.bounds.size.height * (.75));
    _emitterLayer.renderMode = kCAEmitterLayerAdditive;
    _emitterLayer.emitterShape = kCAEmitterLayerPoint;

    // If the emitter layer has multiple emitterCells then it shows white boxes
    // on 10.10.2. So instead we create an invisible cell and give it multiple
    // emitterCells.
...
- (void)setCursorPosition:(NSPoint)cursorPosition {
  ...
  NSBezierPath *path = [NSBezierPath
      bezierPathWithOvalInRect:NSMakeRect(cursorPosition.x - 20,
                                          cursorPosition.y - 20, 40, 40)];
...
- (CAEmitterCell *)rootEmitterCell {
  ...
  float v = 1000;
  float b = 100;
  supercell.emitterCells = @[
    [self subcellWithImageNumber:1 birthRate:b / 5 velocity:v delay:0],
...
- (CAEmitterCell *)subcellWithImageNumber:(int)imageNumber
  ...
  if (image) {
    [cell setContents:(id)[image CGImageForProposedRect:nil
                                                context:nil
                                                  hints:nil]];
  }
  float lifetime = 1;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
  if ([NSDate isAprilFools] &&
      [[NSView class] instancesRespondToSelector:@selector(allowsVibrancy)]) {
...
- (void)startTearDownTimer {
  ...
  _findCursorTeardownTimer = [NSTimer
      scheduledTimerWithTimeInterval:kFindCursorHoldTime
                              target:self
