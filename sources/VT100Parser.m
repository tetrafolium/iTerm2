  ...
  if (datalen == 0) {
    ...
    if (isAsciiString(datap) && !_dcsHooked) {
      ...
      position = datap;
               ...
               (support8BitControlCharacters && isc1(datap[0]))) {
  ...
  if (token->type != VT100_WAIT && token->type != VT100CC_NULL) {
    ...
    if (gDebugLogging) {
      ...
      while (i < length) {
        ...
        if (i == length - 1) {
          DLog(@"%@Bytes %d-%d of %d: %@ (%@)", prefix, start, i, (int)length,
               loginfo, ascii);
        }
        i++;
      }
    ...
    // Don't append the outer wrapper to the output. Earlier, it was unwrapped
    // and the inner tokens were already added.
