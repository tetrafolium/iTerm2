...
- (NSArray<iTermBaseHotKey *> *)hotKeyPressedWithSiblings:
  ...
  if ([NSApp isActive]) {
    ...
    if (prefsWindow != keyWindow ||
        prefsWindowController.window.firstResponder !=
            prefsWindowController.hotkeyField) {
      if (_previousState &&
          (keyWindow.styleMask & NSWindowStyleMaskFullScreen)) {
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
  ...
  if (_previousApp) {
    _previousState =
        [[iTermPreviousState alloc] initWithRunningApp:_previousApp];
  }
}
