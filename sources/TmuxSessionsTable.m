...
- (void)awakeFromNib {
  ...
  [tableView_
      registerForDraggedTypes:[NSArray
                                  arrayWithObjects:kWindowPasteboardType, nil]];
  [tableView_ setDraggingDestinationFeedbackStyle:
...
- (void)setSessionObjects:(NSArray<iTermTmuxSessionObject *> *)sessions {
  ...
  // Reload in case a cell is being edited. Otherwise NSTableView asks for its
  // row.
...
- (void)selectSessionNumber:(int)number {
  ...
  NSInteger i = [_model
      indexOfObjectPassingTest:^BOOL(iTermTmuxSessionObject *_Nonnull obj,
