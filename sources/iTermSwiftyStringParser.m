...
- (NSInteger)enumerateSwiftySubstringsWithBlock:
    ...
    case SWIFTY_STATE_LITERAL:
      if (c == '\\') {
        ...
        if (block) {
          BOOL stop = YES;
          block(index,
                [_string substringWithRange:NSMakeRange(start, i - 1 - start)],
                YES, &stop);
        }
        return i;
      }
          ...
          if (parensStack.count == 0) {
            ...
            if (i - start >= 0) {
              if (block) {
                ...
                block(
                    index,
                    [_string substringWithRange:NSMakeRange(start, i - start)],
                    NO, &stop);
                if (stop) {
                  return i;
                }
            ...
            start = i + 1;
            _state = SWIFTY_STATE_LITERAL;
            break; // do not output this paren. The opening \( was also not
                   // output.
          } else {
  ...
  if (_string.length > start) {
    if (block) {
      ...
      block(index,
            [_string
                substringWithRange:NSMakeRange(start, _string.length - start)],
            YES, &ignore);
    }
  }
  if (self.stopAtUnescapedQuote) {
    if (self.tolerateTruncation) {
      _wasTruncated = YES;
      _wasTruncatedInLiteral = (_state == SWIFTY_STATE_LITERAL ||
                                _state == SWIFTY_STATE_LITERAL_ESC);
      return index;
    } else {
      return NSNotFound;
  ...
  if (_state == SWIFTY_STATE_EXPR || _state == SWIFTY_STATE_EXPR_STR ||
      _state == SWIFTY_STATE_EXPR_STR_ESC) {
    if (self.tolerateTruncation) {
      _wasTruncated = YES;
