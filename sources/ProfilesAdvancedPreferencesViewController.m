...
@interface ProfilesAdvancedPreferencesViewController () <
    NSTableViewDataSource, NSTableViewDelegate, SmartSelectionDelegate,
    TriggerDelegate, iTermSemanticHistoryPrefsControllerDelegate>
@end
  ...
  [self addViewToSearchIndex:_semanticHistoryAction
                 displayName:@"Semantic history"
                     phrases:@[ @"cmd click", @"open file", @"open url" ]
                         key:nil];
  _enableAPSLogging.state =
      iTermUserDefaults.enableAutomaticProfileSwitchingLogging
...
- (NSArray *)keysForBulkCopy {
  NSArray *keys = @[
    KEY_TRIGGERS, KEY_TRIGGERS_USE_INTERPOLATED_STRINGS,
...
- (void)layoutSubviewsForEditCurrentSessionMode {
  NSArray *viewsToDisable = @[
    _boundHostTitle, _boundHostLabel, _boundHostsTableView, _addBoundHost,
    _removeBoundHost, _boundHostShellIntegrationWarning, _boundHostHelp
  ];
...
- (void)reloadProfile {
  ...
  NSString *selectedGuid =
      [self.delegate profilePreferencesCurrentProfile][KEY_GUID];
...
- (IBAction)closeTriggersSheet:(id)sender {
  [[_triggerWindowController.window undoManager]
...
- (void)triggerChanged:(TriggerController *)triggerController
              newValue:(NSArray *)value {
  [[triggerController.window undoManager]
...
- (void)triggerSetUseInterpolatedStrings:(BOOL)useInterpolatedStrings {
  [self setBool:useInterpolatedStrings
         forKey:KEY_TRIGGERS_USE_INTERPOLATED_STRINGS];
}
...
- (IBAction)editSmartSelection:(id)sender {
  ...
  __weak __typeof(self) weakSelf = self;
  [self.view.window beginSheet:_smartSelectionWindowController.window
             completionHandler:^(NSModalResponse returnCode) {
               __strong typeof(weakSelf) strongSelf = weakSelf;
               if (strongSelf) {
...
- (IBAction)addBoundHost:(id)sender {
  ...
  [_boundHostsTableView
          selectRowIndexes:[NSIndexSet indexSetWithIndex:_boundHostsTableView
                                                             .numberOfRows -
                                                         1]
...
- (void)tableView:(NSTableView *)aTableView
  ...
  if (dupProfile) {
    NSString *theTitle;
    theTitle = [NSString
        stringWithFormat:@"The profile “%@” is already bound to hostname “%@”.",
                         dupProfile[KEY_NAME], anObject];
    NSString *removeFromOtherAction =
        [NSString stringWithFormat:@"Remove from “%@”", dupProfile[KEY_NAME]];
    switch ([iTermWarning
        showWarningWithTitle:theTitle
                     actions:@[
                       removeFromOtherAction, @"Remove from This Profile"
                     ]
                  identifier:nil
...
- (IBAction)didToggleAutomaticProfileSwitchingDebugLogging:(id)sender {
  iTermUserDefaults.enableAutomaticProfileSwitchingLogging =
      (_enableAPSLogging.state == NSControlStateValueOn);
}
