...
@implementation iTermGitPollWorker {
  ...
  NSMutableDictionary<NSString *, NSMutableArray<iTermGitCallback> *>
      *_outstanding;
  int _bucket;
}
...
+ (iTermCommandRunnerPool *)commandRunnerPool {
  ...
  dispatch_once(&onceToken, ^{
    ...
    NSString *sandboxConfig = [[[NSString
        stringWithContentsOfFile:[bundle pathForResource:@"git" ofType:@"sb"]
                        encoding:NSUTF8StringEncoding
                           error:nil] componentsSeparatedByString:@"\n"]
        componentsJoinedByString:@" "];
    pool = [[iTermCommandRunnerPool alloc]
        initWithCapacity:gNumberOfGitPollWorkerBuckets
...
+ (instancetype)instanceForPath:(NSString *)path {
  // If one of the gets hung because of a network file system then it won't
  // affect most of the others.
...
- (void)invalidateCacheForPath:(NSString *)path {
  DLog(@"git poll worker for bucket %d: remove cache entry for path %@",
       _bucket, path);
...
- (NSDictionary<NSString *, NSString *> *)environment {
  ...
  if (searchPath.length) {
    ...
    NSString *existingPath =
        environment[key] ?: @"/usr/bin:/bin:/usr/sbin:/sbin";
    environment[key] =
        [NSString stringWithFormat:@"%@:%@", searchPath, existingPath];
    return environment;
  } else {
...
- (void)requestPath:(NSString *)path completion:(iTermGitCallback)completion {
  DLog(@"git poll worker for bucket %d: got request for path %@", _bucket,
       path);
  // This age limits the polling rate. If an older cache entry is around it will
  // still be used until expiry when the poll command fails.
  iTermGitState *cached =
      [_cache stateForPath:path
                maximumAge:[iTermAdvancedSettingsModel gitTimeout]];
  if (cached) {
    DLog(@"git poll worker for bucket %d: return cached value %@ for path %@",
         _bucket, cached, path);
    completion(cached);
    return;
  }
  ...
  if (callbacks) {
    ...
    DLog(@"git poll worker for bucket %d: Attach request for %@ to existing "
         @"callback (totalling %@ callbacks)",
         _bucket, path, @(callbacks.count));
    return;
  }
  DLog(@"git poll worker for bucket %d: This is a new path. Enqueue request "
       @"for %@",
       _bucket, path);
  ...
  if (!_commandRunner) {
    ...
    DLog(@"git poll worker for bucket %d: create a new command runner.",
         bucket);
    __weak __typeof(self) weakSelf = self;
    iTermCommandRunnerPool *pool = [iTermGitPollWorker commandRunnerPool];
    pool.environment = [self environment];
    _commandRunner = [pool requestCommandRunnerWithTerminationBlock:^(
                               iTermCommandRunner *decedent, int status) {
      DLog(@"git poll worker for bucket %d: command runner terminated with "
           @"status %@. Resetting it.",
           bucket, @(status));
      [weakSelf commandRunnerDied:decedent];
    }];
    if (!_commandRunner) {
      DLog(@"git poll worker for bucket %d: failed to allocate command runner",
           bucket);
      [self reset];
      return;
    }
  ...
  DLog(@"git poll worker for bucket %d: Using command runner %@ for path %@",
       _bucket, _commandRunner, path);
  __block BOOL finished = NO;
  const int bucket = _bucket;
  void (^callbackWrapper)(iTermGitState *) = ^(iTermGitState *state) {
    DLog(@"git poll worker for bucket %d: Callback wrapper invoked with state "
         @"%@",
         bucket, state);
    if (state) {
      // This gives the maximum age to show a stale entry. It does not limit the
      // polling rate.
      [self->_cache setState:state forPath:path ttl:10];
    }
    if (!finished) {
      DLog(@"git poll worker for bucket %d: Command runner %@ finished", bucket,
           self->_commandRunner);
      finished = YES;
      completion(state);
    } else {
      DLog(@"git poll worker for bucket %d: [already killed] - Command runner "
           @"%@ finished",
  ...
  [_commandRunner
           write:[[path stringByAppendingString:@"\n"]
                     dataUsingEncoding:NSUTF8StringEncoding]
      completion:^(size_t written, int error) {
        DLog(@"git component for bucket %d: wrote %d bytes, got error code %d",
             bucket, (int)written, error);
      }];
  ...
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{
        DLog(@"git poll worker for bucket %d: Timer for %@ fired. finished=%@",
             bucket, path, @(finished));
        if (finished) {
          DLog(@"git poll worker for bucket %d: was already finished", bucket);
          return;
        }
        finished = YES;
        DLog(@"git poll worker for bucket %d: Terminating %@", bucket,
             weakCommandRunner);
        [[iTermGitPollWorker commandRunnerPool]
            terminateCommandRunner:weakCommandRunner];
      });
}

- (void)didRead:(NSData *)data {
  DLog(@"git poll worker for bucket %d: Read %@ bytes from git poller script",
       _bucket, @(data.length));
  ...
  if (_readData.length + data.length > maxBytes) {
    DLog(@"git poll worker for bucket %d: wtf, have queued up more than 100k "
         @"of output from the git poller script",
         _bucket);
    [self killScript];
    return;
  }
...
- (BOOL)handleBlock {
  char *endMarker = "\n--END--\n";
  NSData *endMarkerData = [NSData dataWithBytesNoCopy:endMarker
                                               length:strlen(endMarker)
                                         freeWhenDone:NO];
  NSRange range = [_readData rangeOfData:endMarkerData
                                 options:0
                                   range:NSMakeRange(0, _readData.length)];
  ...
  NSString *string = [[NSString alloc] initWithData:thisData
                                           encoding:NSUTF8StringEncoding];
  DLog(@"git poll worker for bucket %d: Read this string from git poller "
       @"script:\n%@",
       _bucket, string);
  NSArray<NSString *> *lines = [string componentsSeparatedByString:@"\n"];
  NSDictionary<NSString *, NSString *> *dict =
      [lines keyValuePairsWithBlock:^iTermTuple *(NSString *line) {
  ...
  if (path) {
    DLog(@"git poll worker for bucket %d: Invoking callbacks for path %@ with "
         @"state %@",
         _bucket, path, state);
...
- (void)killScript {
  ...
  DLog(@"git poll worker for bucket %d: KILL command runner %@", _bucket,
       _commandRunner);
  [[iTermGitPollWorker commandRunnerPool]
      terminateCommandRunner:_commandRunner];
  [self reset];
}

- (void)reset {
  DLog(@"git poll worker for bucket %d: RESET - erasing read data, nilling "
       @"command runner, and invoking all callbacks with nil",
       _bucket);
  ...
  // Report failure.
  NSDictionary<NSString *, NSMutableArray<iTermGitCallback> *> *outstanding =
      _outstanding.copy;
  [_outstanding removeAllObjects];
  [outstanding enumerateKeysAndObjectsUsingBlock:^(
                   NSString *_Nonnull path,
                   NSArray<iTermGitCallback> *_Nonnull callbacks,
                   BOOL *_Nonnull stop) {
    iTermGitState *cached = [self->_cache stateForPath:path
                                            maximumAge:INFINITY];
    for (iTermGitCallback callback in callbacks) {
      DLog(
          @"git poll worker for bucket %d: RESET - run callback with %@ for %@",
          self->_bucket, cached, path);
      callback(cached);
    }
  }];
}
...
- (void)commandRunnerDied:(iTermCommandRunner *)commandRunner {
  DLog(@"git poll worker for bucket %d: * command runner died: %@ *", _bucket,
       commandRunner);
  if (commandRunner != _commandRunner) {
    DLog(@"git poll worker for bucket %d: The child is not my son (mine is %@)",
         _bucket, _commandRunner);
    return;
  }
  if (!commandRunner) {
