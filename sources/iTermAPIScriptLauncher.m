...
static NSString *const
    iTermAPIScriptLauncherScriptDidFailUserNotificationCallbackNotification =
        @"iTermAPIScriptLauncherScriptDidFailUserNotificationCallbackNotificati"
        @"on";
...
+ (NSString *)pythonVersionForScript:(NSString *)path {
  ...
  iTermSetupCfgParser *parser =
      [[iTermSetupCfgParser alloc] initWithPath:setupCfgPath];
  if (parser) {
...
+ (int)environmentVersionAt:(NSString *)iterm2env {
  NSString *manifest =
      [iterm2env stringByAppendingPathComponent:@"iterm2env-metadata.json"];
  return [[iTermPythonRuntimeDownloader sharedInstance]
      versionInMetadataAtURL:[NSURL fileURLWithPath:manifest]];
}
...
+ (void)upgradeFullEnvironmentScriptAt:(NSString *)fullPath
  ...
  NSString *message = [NSString
      stringWithFormat:
          @"The Python API script “%@” needs a newer version of the runtime "
          @"environment for security reasons. You must upgrade it before this "
          @"version of iTerm2 can launch the script.",
...
+ (void)downloadIfNeededAndUpgradeFullEnvironmentScriptAt:(NSString *)fullPath
  ...
  if ([downloader
          installedVersionWithPythonVersion:configParser.pythonVersion] >=
...
+ (void)reallyUpgradeFullEnvironmentScriptAt:(NSString *)fullPath
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:existingEnv.path]) {
    if ([[NSFileManager defaultManager] fileExistsAtPath:savedEnv.path]) {
      ...
      NSError *error = nil;
      [[NSFileManager defaultManager] moveItemAtURL:existingEnv
                                              toURL:savedEnv
                                              error:&error];
      DLog(@"saving - move '%@' to '%@': %@", existingEnv.path, savedEnv.path,
           error);
    }
  }
  [[iTermPythonRuntimeDownloader sharedInstance]
                      ...
                      completion:^(BOOL ok) {
                        if (ok) {
                          ...
                          [[NSFileManager defaultManager]
                              removeItemAtURL:savedEnv
                                        error:&error];
                          DLog(@"remove saved - %@: %@", savedEnv.path, error);
                          NSString *venv = [self environmentForScript:fullPath
                                                         checkForMain:YES
                                                        checkForSaved:NO];
                          completion(venv);
                          return;
                        ...
                        NSError *error = nil;
                        [[NSFileManager defaultManager]
                            removeItemAtURL:existingEnv
                                      error:&error];
                        DLog(@"remove failed install - %@: %@",
                             existingEnv.path, error);

                        error = nil;
                        [[NSFileManager defaultManager]
                            moveItemAtURL:savedEnv
                                    toURL:existingEnv
                                    error:&error];
                        DLog(@"restore saved - move '%@' to '%@': %@",
                             savedEnv.path, existingEnv.path, error);
                      }];
}

+ (void)
    upgradeIfNeededFullEnvironmentScriptAt:(NSString *)fullPath
                              configParser:(iTermSetupCfgParser *)configParser
  ...
  NSString *saved =
      [fullPath stringByAppendingPathComponent:@"saved-iterm2env"];
  if ([[NSFileManager defaultManager] fileExistsAtPath:saved]) {
    // If there's a saved folder, then something went wrong while upgrading.
    // Restore it.
    NSError *error = nil;
    [[NSFileManager defaultManager] removeItemAtPath:iterm2env error:&error];
    ...
    virtualenv = [self environmentForScript:fullPath
                               checkForMain:YES
                              checkForSaved:NO];
  }
...
+ (void)launchScript:(NSString *)filename
    ...
    explicitUserAction:(BOOL)explicitUserAction {
  if (virtualenv != nil) {
    // This is a full-environment script. Check if its environment version is
    // supported and offer to upgrade.
    iTermSetupCfgParser *parser =
        [[iTermSetupCfgParser alloc] initWithPath:setupCfgPath];
    [self
        upgradeIfNeededFullEnvironmentScriptAt:fullPath
                                  configParser:parser
...
+ (void)reallyLaunchScript:(NSString *)filename
  ...
  NSString *identifier =
      [[iTermAPIConnectionIdentifierController sharedInstance]
  ...
  if (virtualenv) {
    ...
    name = [[[filename stringByDeletingLastPathComponent] pathComponents]
        lastObject];
  }
  iTermScriptHistoryEntry *entry = [[iTermScriptHistoryEntry alloc]
      initWithName:name
          fullPath:fullPath
  ...
  @try {
    ...
    [[iTermScriptHistory sharedInstance] addHistoryEntry:entry];
    [entry addOutput:[NSString stringWithFormat:@"ERROR: Failed to launch: %@",
                                                e.reason]];
...
+ (void)tryLaunchScript:(NSString *)filename
  ...
  // I've tested these shells and they all work when run as: $SHELL -c command
  // arg arg
  NSArray<NSString *> *const knownShells =
      @[ @"bash", @"tcsh", @"zsh", @"fish" ];
  ...
  NSString *standardEnv = [[iTermPythonRuntimeDownloader sharedInstance]
      pathToStandardPyenvPythonWithPythonVersion:pythonVersion];
  NSString *searchPath = [iTermPythonRuntimeDownloader.sharedInstance
      pathToStandardPyenvWithVersion:pythonVersion];
  NSString *path = [searchPath stringByAppendingPathComponent:@"versions"];
  NSString *standardPythonVersion = [[iTermPythonRuntimeDownloader
      bestPythonVersionAt:path] it_twoPartVersionNumber];
  task.environment =
      [self environmentFromEnvironment:task.environment
...
+ (NSDictionary *)environmentFromEnvironment:(NSDictionary *)initialEnvironment
  ...
  // OpenSSL bakes in the directory where you compiled it so it can find root
  // certs. That works great if you happen to be me, but it seems that most
  // people aren't. Luckily it lets you set some environment variables to find
  // cert stores.
  NSString *version = [[virtualenv stringByDeletingLastPathComponent]
      stringByDeletingLastPathComponent];
  environment[@"SSL_CERT_FILE"] = [version stringByAppendingPathComponents:@[
    @"lib", [NSString stringWithFormat:@"python%@", pythonVersion],
    @"site-packages", @"pip", @"_vendor", @"certifi", @"cacert.pem"
  ]];
  environment[@"SSL_CERT_DIR"] = [version
      stringByAppendingPathComponents:@[ @"openssl", @"ssl", @"certs" ]];
  return environment;
}
...
+ (NSArray *)argumentsToRunScript:(NSString *)filename
                   ...
                   withVirtualEnv:(NSString *)providedVirtualEnv
                    pythonVersion:(NSString *)pythonVersion {
  NSString *wrapper =
      [[NSBundle bundleForClass:self.class] pathForResource:@"it2_api_wrapper"
                                                     ofType:@"sh"];
  NSString *pyenv = [[iTermPythonRuntimeDownloader sharedInstance]
      pathToStandardPyenvPythonWithPythonVersion:pythonVersion];
  ...
  if (arguments.count > 0) {
    NSArray<NSString *> *escapedArguments =
        [arguments mapWithBlock:^id(NSString *anObject) {
          return
              [anObject stringWithEscapedShellCharactersIncludingNewlines:YES];
        }];
    NSString *joinedArguments =
...
+ (void)waitForTask:(NSTask *)task
  ...
  dispatch_queue_t q =
      dispatch_queue_create("com.iterm2.script-launcher", NULL);
  @synchronized(queues) {
    [queues addObject:q];
  }
  dispatch_async(q, ^{
    ...
    while (inData.length) {
      dispatch_async(dispatch_get_main_queue(), ^{
        [entry addOutput:[[NSString alloc] initWithData:inData
                                               encoding:NSUTF8StringEncoding]];
      });
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      if (!task.isRunning &&
        ...
        if (!entry.terminatedByUser) {
          NSString *message = [NSString
              stringWithFormat:@"“%@” ended unexpectedly.", entry.name];
...
+ (void)revealFailedScriptInConsole:(NSNotification *)notification {
  ...
  iTermScriptHistoryEntry *entry =
      [[iTermScriptHistory sharedInstance] entryWithIdentifier:identifier];
  if (entry) {
    [[iTermScriptConsole sharedInstance] revealTailOfHistoryEntry:entry];
  }
}

+ (void)didFailToLaunchScript:(NSString *)filename
                withException:(NSException *)e {
  ELog(@"Exception occurred %@", e);
  NSAlert *alert = [[NSAlert alloc] init];
  alert.messageText = @"Error running script";
  alert.informativeText = [NSString
      stringWithFormat:@"Script at \"%@\" failed.\n\n%@", filename, e.reason];
  [alert runModal];
}
...
+ (NSString *)prospectivePythonPathForPyenvScriptNamed:(NSString *)name {
  ...
  NSString *pythonVersion =
      [iTermPythonRuntimeDownloader bestPythonVersionAt:path]
          ?: @"_NO_PYTHON_VERSION_FOUND_";
...
+ (NSString *)environmentForScript:(NSString *)path
  ...
  if (checkForMain) {
    ...
    NSString *expectedPath = [[path stringByAppendingPathComponent:name]
        stringByAppendingPathComponent:
            [name stringByAppendingPathExtension:@"py"]];
  ...
  iTermPythonRuntimeDownloader *downloader =
      [iTermPythonRuntimeDownloader sharedInstance];
  {
    NSString *pyenvPython = [downloader
              pyenvAt:[path stringByAppendingPathComponent:@"iterm2env"]
        pythonVersion:[iTermAPIScriptLauncher pythonVersionForScript:path]];
    if ([[NSFileManager defaultManager] fileExistsAtPath:pyenvPython
                                             isDirectory:nil]) {
      return pyenvPython;
    }
  }
  ...
  {
    NSString *pyenvPython = [downloader
              pyenvAt:[path stringByAppendingPathComponent:@"saved-iterm2env"]
        pythonVersion:[iTermAPIScriptLauncher pythonVersionForScript:path]];
    if ([[NSFileManager defaultManager] fileExistsAtPath:pyenvPython
                                             isDirectory:nil]) {
      return pyenvPython;
