...
static dispatch_queue_t iTermPathFinderQueue(void) {
  ...
  dispatch_once(&onceToken, ^{
    queue =
        dispatch_queue_create("com.iterm2.path-finder", DISPATCH_QUEUE_SERIAL);
  });
  return queue;
}
...
- (void)searchSynchronously {
  ...
  if (!workingDirectoryIsOk) {
    DLog(@"Working directory %@ is a network share or doesn't exist. Not using "
         @"it for context.",
         _workingDirectory);
  }
  ...
  int iterationsBeforeQuitting =
      100; // Bail after 100 iterations if nothing is still found.
  ...
  for (int i = [beforeChunks count]; i >= 0; i--) {
    ...
    // Do not search more than 10 chunks forward to avoid starving leftward
    // search.
    for (int j = 0; j < MAX(1, afterChunks.count) && j < 10; j++) {
      ...
      if (_trimWhitespace) {
        trimmedPath =
            [trimmedPath stringByTrimmingCharactersInSet:whitespaceCharset];
      }
      ...
      trimmedPath =
          [trimmedPath stringByReplacingOccurrencesOfRegex:removeEscapingSlashes
                                                withString:@"$1"];

      // Some programs will thoughtlessly print a filename followed by some
      // silly suffix. We'll try versions with and without a questionable
      // suffix. The version with the suffix is always preferred if it exists.
      ...
      for (NSString *modifiedPossiblePath in
           [self pathsFromPath:trimmedPath
        ...
        if (workingDirectoryIsOk || [modifiedPossiblePath hasPrefix:@"/"]) {
          iTermPathCleaner *cleaner =
              [[iTermPathCleaner alloc] initWithPath:modifiedPossiblePath
                                              suffix:nil
        ...
        if (exists) {
          ...
          if (j + 1 < afterChunks.count) {
            extra = [self
                columnAndLineNumberFromChunks:[afterChunks
                                                  subarrayFromIndex:j + 1]];
          }
          NSString *extendedPath =
              [modifiedPossiblePath stringByAppendingString:extra];
          [right appendString:extra];

          if (_trimWhitespace &&
              [[right stringByTrimmingTrailingCharactersFromCharacterSet:
                          whitespaceCharset] length] == 0) {
            // trimmedPath is trim(left + right). If trim(right) is empty
            // then we don't want to count trailing whitespace from left in the
            // chars taken from prefix.
            _prefixChars =
                [[left stringByTrimmingTrailingCharactersFromCharacterSet:
                           whitespaceCharset] length];
          } else {
            _prefixChars = left.length;
          }
          NSInteger lengthOfBadSuffix =
              extra.length ? 0
                           : trimmedPath.length - modifiedPossiblePath.length;
          int n;
          if (_trimWhitespace) {
            n = [[right stringByTrimmingTrailingCharactersFromCharacterSet:
                            whitespaceCharset] length] -
                lengthOfBadSuffix;
          } else {
...
- (BOOL)fileExistsAtPathLocally:(NSString *)path {
  _workingDirectoryIsLocal =
      [self.fileManager fileIsLocal:path
             additionalNetworkPaths:[[iTermAdvancedSettingsModel pathsToIgnore]
...
- (NSArray *)pathsFromPath:(NSString *)source
     byRemovingBadSuffixes:(NSArray *)badSuffixes {
  NSMutableArray *result = [NSMutableArray array];
  [result addObject:source];
  for (NSString *badSuffix in badSuffixes) {
    if ([source hasSuffix:badSuffix]) {
      NSString *stripped =
          [source substringToIndex:source.length - badSuffix.length];
      if (stripped.length) {
