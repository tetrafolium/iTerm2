...
static NSString *const iTermProfilePreferencesUpdateSessionName =
    @"iTermProfilePreferencesUpdateSessionName";

@interface ProfilesGeneralPreferencesViewController () <
    iTermImageWellDelegate, iTermShortcutInputViewDelegate, NSMenuDelegate,
    NSTabViewDelegate, ProfileListViewDelegate>
@end
...
@implementation ProfilesGeneralPreferencesViewController {
  ...
  IBOutlet NSButton
      *_editAdvancedConfigButton; // Advanced initial directory button
  IBOutlet AdvancedWorkingDirectoryWindowController
      *_advancedWorkingDirWindowController;
  ...
  iTermFunctionCallTextFieldDelegate
...
- (void)awakeFromNib {
  ...
  info.customSettingChangedHandler = ^(id sender) {
    ...
    assert(weakInfo);
    [strongSelf setString:strongSelf->_profileNameField.stringValue
                   forKey:weakInfo.key];
      ...
      [[iTermFunctionCallTextFieldDelegate alloc]
          initWithPathSource:
              [iTermVariableHistory
               ...
               functionsOnly:NO];
  _profileNameFieldForEditCurrentSession.delegate =
      _profileNameFieldForEditCurrentSessionDelegate;

  if (@available(macOS 10.13, *)) {
  } else {
    // 10.12 code path — I can't get NSTextAttachment to behave itself. For some
    // reason it doesn't draw the tab graphic at all. I'm ragequitting.
  ...
  _commandDelegate = [[iTermFunctionCallTextFieldDelegate alloc]
      initWithPathSource:
          [iTermVariableHistory
           ...
           functionsOnly:NO];
  if ([[self objectForKey:KEY_CUSTOM_COMMAND]
          isEqual:kProfilePreferenceCommandTypeCustomValue]) {
  ...
  info.shouldBeEnabled = ^BOOL {
    ...
    return strongSelf->_commandType.selectedTag !=
           iTermGeneralProfilePreferenceCustomCommandTagLoginShell;
  };
  ...
  NSSet<NSString *> *exclusions = [NSSet setWithArray:@[
    iTermVariableKeySessionTTY, iTermVariableKeySessionUsername,
    iTermVariableKeySessionHostname, iTermVariableKeySessionName,
    iTermVariableKeySessionJob, iTermVariableKeySessionProcessTitle,
  ...
  [self addViewToSearchIndex:_editBadgeButton
                 displayName:@"Edit badge appearance"
                     phrases:@[
                       @"Badge font", @"Badge minum and maximum width",
      ...
      [[iTermFunctionCallTextFieldDelegate alloc]
          initWithPathSource:
              [iTermVariableHistory
               ...
               functionsOnly:NO];
  _badgeTextForEditCurrentSession.delegate =
      _badgeTextForEditCurrentSessionFieldDelegate;
  ...
  [_profiles selectRowByGuid:[self.delegate profilePreferencesCurrentProfile]
                                 [KEY_ORIGINAL_GUID]];
...
- (void)sessionNameDidEndEditing {
  if ([self stringForKey:KEY_TMUX_PANE_TITLE]) {
    [self setString:_profileNameFieldForEditCurrentSession.stringValue
             forKey:KEY_TMUX_PANE_TITLE];
  } else {
    [self setString:_profileNameFieldForEditCurrentSession.stringValue
             forKey:KEY_NAME];
  }
...
- (void)updateImageWell {
  ...
  _imageWell.image =
      iconPath ? [[NSImage alloc] initWithContentsOfFile:iconPath] : nil;
}
...
- (void)updateImageWellHidden {
  _imageWell.hidden =
      ([self unsignedIntegerForKey:KEY_ICON] != iTermProfileIconCustom ||
...
- (void)windowWillClose {
  ...
  if ([_tagsTokenField textFieldIsFirstResponder]) {
    // The token field's editor is the first responder. Force the token field to
    // end editing so the last token entered will be tokenized and prefs saved
    // with it.
    [self.view.window makeFirstResponder:self.view];
  } else if ([_profileNameFieldForEditCurrentSession
                 textFieldIsFirstResponder]) {
    [_profileDelegate profilesGeneralPreferencesNameDidEndEditing];
  }
}

- (NSArray *)keysForBulkCopy {
  return [[super keysForBulkCopy]
      arrayByAddingObjectsFromArray:[_advancedWorkingDirWindowController
                                        allKeys]];
}
...
- (void)toggleLock {
  [self setBool:![self boolForKey:KEY_ALLOW_TITLE_SETTING]
         forKey:KEY_ALLOW_TITLE_SETTING];
}
...
- (void)updateTmuxTabTitle {
  ...
  NSString *tmuxTabTitle =
      [scope valueForVariableName:iTermVariableKeyTabTmuxWindowName];
  if (tmuxTabTitle) {
...
- (void)reloadProfile {
  [super reloadProfile];
  [self populateBookmarkUrlSchemesFromProfile:
            [self.delegate profilePreferencesCurrentProfile]];
  [_profiles selectRowByGuid:[self.delegate profilePreferencesCurrentProfile]
                                 [KEY_ORIGINAL_GUID]];
  _sessionHotkeyInputView.shortcut = [iTermShortcut
      shortcutWithDictionary:(NSDictionary *)[self
                                 objectForKey:KEY_SESSION_HOTKEY]];
  id<iTermSessionScope> scope = self.scope;
  if (scope) {
    _tabTitle.stringValue = scope.tab.tabTitleOverrideFormat ?: @"";
    _windowTitle.stringValue =
        scope.tab.window.windowTitleOverrideFormat ?: @"";
...
- (void)ensureSessionNameVisible {
  // Unless a custom title function is in use, ensure session name is visible in
  // the title.
...
- (void)updateTitleSettingsMenuForView:(NSPopUpButton *)titleSettings {
  ...
  NSIndexSet *indexSet = [titleSettings.menu.itemArray
      indexesOfObjectsPassingTest:^BOOL(NSMenuItem *_Nonnull obj,
  ...
  NSArray<iTermSessionTitleProvider *> *funcs =
      [iTermAPIHelper sessionTitleFunctions];
  if (funcs.count) {
  ...
  for (iTermSessionTitleProvider *provider in funcs) {
    ...
    if (uniqueIdentifier &&
        [uniqueIdentifier isEqualToString:provider.uniqueIdentifier]) {
  ...
  if (uniqueIdentifier) {
    // Did not find the currently selected func. Maybe the script hasn't
    // started, crashed, etc.
    NSMenuItem *item = [[NSMenuItem alloc] init];
    item.title = funcName;
...
- (void)updateEnabledState {
  ...
  if ([[self stringForKey:KEY_CUSTOM_COMMAND]
          isEqualToString:kProfilePreferenceCommandTypeCustomValue] ||
      [[self stringForKey:KEY_CUSTOM_COMMAND]
          isEqualToString:kProfilePreferenceCommandTypeCustomShellValue]) {
  ...
  _customDirectory.enabled = ([[self stringForKey:KEY_CUSTOM_DIRECTORY]
      isEqualToString:kProfilePreferenceInitialDirectoryCustomValue]);
}
...
- (IBAction)configureBadge:(id)sender {
  ...
  [self.view.window beginSheet:badgeConfigurationWindowController.window
             completionHandler:^(NSModalResponse returnCode) {
               ...
               NSDictionary *mutations =
                   badgeConfigurationWindowController.profileMutations;
               [mutations enumerateKeysAndObjectsUsingBlock:^(
                              id _Nonnull key, id _Nonnull value,
                              BOOL *_Nonnull stop) {
...
// Replace a Profile in the sessions profile with a new dictionary that
// preserves the original name and guid, takes all other fields from |bookmark|,
// and has KEY_ORIGINAL_GUID point at the guid of the profile from which all
// that data came.n
...
- (BOOL)profileHandlesScheme:(NSString *)scheme {
  ...
  return (
      handler && [[handler objectForKey:KEY_GUID] isEqualToString:guid] &&
      [[iTermLaunchServices sharedInstance] iTermIsDefaultForScheme:scheme]);
}

- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
  if (menuItem.menu == _urlSchemes.menu) {
    menuItem.state = [self profileHandlesScheme:menuItem.title]
...
- (IBAction)urlSchemeHandlerDidChange:(id)sender {
  ...
  [self populateBookmarkUrlSchemesFromProfile:[[ProfileModel sharedInstance]
                                                  bookmarkWithGuid:guid]];
}
...
- (void)updateEditAdvancedConfigButton {
  ...
  BOOL isAdvanced = [directoryType
      isEqualToString:kProfilePreferenceInitialDirectoryAdvancedValue];
...
- (IBAction)showAdvancedWorkingDirConfigPanel:(id)sender {
  ...
  _advancedWorkingDirWindowController.profile =
      [self.delegate profilePreferencesCurrentProfile];
  __weak typeof(self) weakSelf = self;
  [self.view.window beginSheet:_advancedWorkingDirWindowController.window
             completionHandler:^(NSModalResponse returnCode) {
               ...
               for (NSString *key in
                    [strongSelf->_advancedWorkingDirWindowController allKeys]) {
                 [strongSelf
                     setString:strongSelf->_advancedWorkingDirWindowController
                                   .profile[key]
                        forKey:key];
               }
...
- (void)updateCommandType {
  ...
  if ([value isEqualToString:kProfilePreferenceCommandTypeCustomValue]) {
    [_commandType
        selectItemWithTag:iTermGeneralProfilePreferenceCustomCommandTagCustom];
    _customCommand.placeholderString =
        @"Enter command to run when a new session is created";
  } else if ([value isEqualToString:
                        kProfilePreferenceCommandTypeCustomShellValue]) {
    [_commandType selectItemWithTag:
                      iTermGeneralProfilePreferenceCustomCommandTagCustomShell];
...
- (void)setShortcutValueToSelectedItem {
  NSString *shortcut =
      [self shortcutKeyForTag:[[_profileShortcut selectedItem] tag]];
  if (shortcut) {
    ...
    if (guidsOfProfilesToModify.count) {
      for (NSString *guid in guidsOfProfilesToModify) {
        Profile *profile =
            [[ProfileModel sharedInstance] bookmarkWithGuid:guid];
        [[ProfileModel sharedInstance] setObject:nil
                                          forKey:KEY_SHORTCUT
                                      inBookmark:profile];
      }
  ...
  for (Profile *profile in [profileModel bookmarks]) {
    ...
    if (tag != -1) {
      ...
      NSString *newTitle = [NSString
          stringWithFormat:@"⌃⌘%@ (%@)", existingShortcut, profile[KEY_NAME]];
...
- (void)toggleSelectedTitleComponent {
  ...
  [self
      toggleSelectedTitleComponentForView:_titleSettingsForEditCurrentSession];
}
...
- (void)toggleSelectedTitleComponentForView:(NSPopUpButton *)titleSettings {
  ...
  if (menuItem.tag == -1) {
    ...
    iTermTuple<NSString *, NSString *> *tuple =
        [iTermTuple tupleWithObject:displayName andObject:uniqueIdentifier];
    [self setTuple:tuple forKey:KEY_TITLE_FUNC];

    [self setUnsignedInteger:iTermTitleComponentsCustom
                      forKey:KEY_TITLE_COMPONENTS];
    [self updateSelectedTitleComponents];
    return;
  }

  const iTermTitleComponents originalValue =
      [self unsignedIntegerForKey:KEY_TITLE_COMPONENTS];
  const iTermTitleComponents selectedTag =
      (iTermTitleComponents)titleSettings.selectedItem.tag;
  ...
  NSUInteger nameTagsMask =
      (iTermTitleComponentsProfileName | iTermTitleComponentsSessionName |
       iTermTitleComponentsProfileAndSessionName);
  if (selectedTag & nameTagsMask) {
    // Selected a name tag. Deselect all other name tags. Toggle the selected
    // one.
...
- (void)setTuple:(iTermTuple<NSString *, NSString *> *)tuple
          forKey:(NSString *)key {
...
- (void)updateSelectedTitleComponents {
  ...
  [self
      updateSelectedTitleComponentsForView:_titleSettingsForEditCurrentSession];
}

- (void)updateSelectedTitleComponentsForView:(NSPopUpButton *)titleSettings {
  const iTermTitleComponents value =
      [self unsignedIntegerForKey:KEY_TITLE_COMPONENTS];
  ...
  for (NSMenuItem *item in titleSettings.menu.itemArray) {
    ...
    if (value & iTermTitleComponentsCustom) {
      selected = [NSObject object:item.identifier
                  isEqualToObject:currentUniqueIdentifier];
  ...
  const CGFloat maxWidth =
      NSMinX(_customTitleHelp.frame) - NSMinX(titleSettings.frame) - 5;
...
- (void)removeWhitespaceFromCustomCommand {
  while (1) {
    const NSRange range = [_customCommand.stringValue
        rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
    if (range.location == NSNotFound) {
      return;
    }
    _customCommand.stringValue =
        [_customCommand.stringValue stringByReplacingCharactersInRange:range
                                                            withString:@""];
  }
}
...
- (void)sessionNameDidChange {
  ...
  [_rateLimit
      performRateLimitedSelector:@selector(postUpdateSessionNameNotification)
                        onTarget:self
                      withObject:nil];
}
...
- (void)tabTitleDidChange {
  ...
  // Do this rather than updating the variable directly because tmux needs
  // special handling.
  iTermCallMethodByIdentifier(self.scope.tab.tabID, @"iterm2.set_title",
                              @{@"title" : value}, nil);
}
...
- (void)postUpdateSessionNameNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermProfilePreferencesUpdateSessionName
                    object:nil];
}
