...
static void ExecImageDecoder(char *executable, char *type, char *sandbox,
                             int jsonFD, int compressedDataFD, int dtablesize) {
  ...
  char *args[] = {"sandbox-exec", "-p", sandbox, executable, type, NULL};
...
- (NSString *)executable {
  return [[NSBundle bundleForClass:self.class]
      pathForAuxiliaryExecutable:@"image_decoder"];
}

- (NSString *)sandbox {
  NSString *sandboxFileName =
      [[NSBundle bundleForClass:self.class] pathForResource:@"image_decoder"
                                                     ofType:@"sb"];
  NSString *sandboxContents =
      [NSString stringWithContentsOfFile:sandboxFileName
  ...
  NSDictionary *subs = @{
    @"@PATH_TO_EXECUTABLE@" : [[executable stringByDeletingLastPathComponent]
        stringByEscapingForSandboxLiteral],
  ...
  for (NSString *key in subs) {
    sandboxContents =
        [sandboxContents stringByReplacingOccurrencesOfString:key
                                                   withString:subs[key]];
  }
...
- (NSData *)jsonForCompressedImageData:(NSData *)compressedData
  ...
  case 0:
    ...
    ExecImageDecoder(utf8Executable, typeCString, sandbox, jsonFDs[1],
                     compressedImageFDs[0], dtablesize);
    exit(1);
    return nil;
