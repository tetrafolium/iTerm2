...
@implementation THE_CLASS {
  double blurRadius_;
  // Hack for a 10.16 issue. Once you set blur from >0 to 0 then it is broken
  ...
  // Cached value of the percentage of this window that is occluded by other
  // nonpanel windows in this app.
  ...
  self = [super initWithContentRect:contentRect
                          styleMask:aStyle
                            backing:bufferingType
                             ...
                             screen:screen];
  if (self) {
    DLog(@"Invalidate cached occlusion: %@ %p", NSStringFromSelector(_cmd),
         self);
    [[iTermWindowOcclusionChangeMonitor sharedInstance]
        invalidateCachedOcclusion];
  }
  return self;
}
...
- (void)iterm_dealloc {
  ...
  // Not safe to call this from dealloc because can very indirectly try to
  // retain this object.
...
- (void)setDocumentEdited:(BOOL)documentEdited {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermWindowDocumentedEditedDidChange
                    object:self];
}

- (BOOL)titleChangedRecently {
  NSTimeInterval now = [NSDate timeIntervalSinceReferenceDate];
  return (now > _timeOfLastWindowTitleChange &&
          now - _timeOfLastWindowTitleChange <
              iTermWindowTitleChangeMinimumInterval);
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)item {
  if (item.action == @selector(performMiniaturize:)) {
    if (@available(macOS 10.13, *)) {
      ...
      // This was originally for #4402. Not sure how it worked, but I don't want
      // to mess with 10.12.
...
#endif
  return [NSString
      stringWithFormat:@"<%@: %p frame=%@ title=%@ alpha=%f isMain=%d isKey=%d "
                       @"isVisible=%d delegate=%p%@>",
                       [self class], self, [NSValue valueWithRect:self.frame],
                       self.title, self.alphaValue, (int)self.isMainWindow,
                       (int)self.isKeyWindow, (int)self.isVisible,
                       self.delegate, extra];
}
...
- (void)encodeRestorableStateWithCoder:(NSCoder *)coder
                       backgroundQueue:(NSOperationQueue *)queue {
...
- (void)encodeRestorableStateWithCoder:(NSCoder *)coder {
  ...
  [coder encodeObject:restoreState_
               forKey:kTerminalWindowStateRestorationWindowArrangementKey];
}
...
- (void)toggleFullScreen:(nullable id)sender {
  if (![[self ptyDelegate] lionFullScreen] &&
    ...
    [(id<PTYWindowDelegateProtocol>)[self delegate]
        toggleTraditionalFullScreenMode];
  } else {
...
- (int)screenNumber {
  return [[[[self screen] deviceDescription] objectForKey:@"NSScreenNumber"]
      intValue];
}

- (CGFloat)sumOfIntersectingAreaOfRect:(NSRect)rect
...
- (void)smartLayout {
  ...
  NSArray<NSWindow *> *windows =
      [[(iTermApplication *)NSApp orderedWindowsPlusVisibleHotkeyPanels]
          filteredArrayUsingBlock:^BOOL(id window) {
            return (window != self && [window isVisible] &&
  ...
  const NSPoint screenCenter =
      NSMakePoint(NSMidX(screenRect), NSMidY(screenRect));
  const CGFloat maxDistance =
      sqrt(pow(screenRect.size.width, 2) + pow(screenRect.size.height, 2));
  for (CGFloat xOffset = 0; xOffset < widthToScan; xOffset += stride) {
    for (CGFloat yOffset = 0; yOffset < heightToScan; yOffset += stride) {
      NSRect proposedRect = NSMakeRect(
          screenRect.origin.x + xOffset, screenRect.origin.y + yOffset,
          self.frame.size.width, self.frame.size.height);
      // Ensure distance from screen center is less than 1 so any amount of
      // overlap dominates even the greatest cost from additional distance
      const NSPoint proposedCenter =
          NSMakePoint(NSMidX(proposedRect), NSMidY(proposedRect));
      CGFloat distanceFromScreenCenter =
          sqrt(pow(proposedCenter.x - screenCenter.x, 2) +
               pow(proposedCenter.y - screenCenter.y, 2)) /
          maxDistance;
      const CGFloat cost = [self sumOfIntersectingAreaOfRect:proposedRect
                                                   withRects:frames] +
  ...
  DLog(@"Using smart layout place window at %@ given frames %@",
       NSStringFromRect(bestFrame), frames);
...
- (void)makeKeyAndOrderFront:(nullable id)sender {
  DLog(@"%@ makeKeyAndOrderFront: layoutDone=%@ %@",
       NSStringFromClass([self class]), @(_layoutDone),
  ...
  DLog(@"%@ - calling makeKeyAndOrderFont, which triggers a window resize",
       NSStringFromClass([self class]));
  DLog(@"The current window frame is %fx%f", [self frame].size.width,
...
- (double)approximateFractionOccluded {
  if (_totalOcclusionCacheTime >
      [[iTermWindowOcclusionChangeMonitor sharedInstance]
          timeOfLastOcclusionChange]) {
    // -orderedWindows is expensive, so avoid doing anything here if nothing has
    // changed.
  ...
  if (myArea >= 1) {
    for (NSScreen *screen in [NSScreen screens]) {
      ...
      const CGFloat onscreenArea =
          (onscreenFrame.size.width * onscreenFrame.size.height);
  ...
  if (myIndex != 0 && myIndex != NSNotFound) {
    ...
    NSSize partSize =
        NSMakeSize(myFrame.size.width / kCols, myFrame.size.height / kRows);
    ...
    for (NSUInteger i = 0; i < myIndex; i++) {
      ...
      if ([other isMiniaturized] || other.alphaValue < 0.1) {
        // The other window is almost transparent or miniaturized, so short
        // circuit.
        continue;
      }
      ...
      for (int y = 0; y < kRows; y++) {
        for (int x = 0; x < kCols; x++) {
          if (parts[y][x].occlusion != 1) {
            NSRect intersection =
                NSIntersectionRect(parts[y][x].rect, otherFrame);
            CGFloat pixelsOfOcclusion =
                intersection.size.width * intersection.size.height;
            parts[y][x].occlusion =
                MAX(parts[y][x].occlusion, pixelsOfOcclusion / pixelsInPart);
          }
...
- (NSRect)constrainFrameRect:(NSRect)frameRect
                    toScreen:(nullable NSScreen *)screen {
...
- (void)setFrame:(NSRect)frameRect display:(BOOL)flag {
  DLog(@"setFrame:%@ display:%@ maxy=%@ from\n%@", NSStringFromRect(frameRect),
       @(flag), @(NSMaxY(frameRect)), [NSThread callStackSymbols]);
...
+ (Class)frameViewClassForStyleMask:(NSUInteger)windowStyle {
  return [iTermThemeFrame class]
             ?: [super frameViewClassForStyleMask:windowStyle];
}
...
//
// In particular, this fixes issue 8478 (traffic light buttons vertically
// off-center in native full screen).
...
- (NSColor *)it_terminalWindowDecorationTextColorForBackgroundColor:
  ...
  return [self.ptyDelegate
      terminalWindowDecorationTextColorForBackgroundColor:(NSColor *)
                                                              backgroundColor];
}
