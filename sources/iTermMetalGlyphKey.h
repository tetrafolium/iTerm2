...
typedef NS_OPTIONS(int, iTermMetalGlyphKeyTypeface) {
  ...
  iTermMetalGlyphKeyTypefaceBoldItalic = (iTermMetalGlyphKeyTypefaceBold |
...
// Features of a cell that do not affect which texture is selected as source
// material.
typedef struct {
...
NS_INLINE NSString *
iTermMetalGlyphKeyDescription(const iTermMetalGlyphKey *key) {
  if (!key->drawable) {
    return @"not drawable";
  }
  id formattedCode;
  if (!key->isComplex) {
    formattedCode =
        [NSString stringWithFormat:@"0x%x (%C)", key->code, key->code];
  } else {
  ...
  if (key->combiningSuccessor) {
    formattedCombiningSuccessor =
        [NSString stringWithFormat:@"0x%x (%C)", key->combiningSuccessor,
                                   key->combiningSuccessor];
  } else {
  ...
  return [NSString stringWithFormat:@"code=%@ combiningSuccessor=%@ complex=%@ "
                                    @"boxDrawing=%@ thinStrokes=%@ typeface=%@",
                                    formattedCode, formattedCombiningSuccessor,
...
NS_INLINE NSString *iTermStringFromColorVectorFloat4(vector_float4 v) {
  return [NSString
      stringWithFormat:@"(%0.2f, %0.2f, %0.2f, %0.2f)", v.x, v.y, v.z, v.w];
}

NS_INLINE NSString *
iTermMetalGlyphAttributesDescription(iTermMetalGlyphAttributes *attrs) {
  NSString *underline;
