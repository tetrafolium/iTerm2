...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    [self setTabsHaveCloseButtons:
              [iTermPreferences boolForKey:kPreferenceKeyTabsHaveCloseButton]];
    self.minimumTabDragDistance =
        [iTermAdvancedSettingsModel minimumTabDragDistance];
    // This used to depend on job but it's too difficult to do now that
    // different sessions might have different title formats.
    ...
    self.selectsTabsOnMouseDown =
        [iTermAdvancedSettingsModel selectsTabsOnMouseDown];
  }
  return self;
}
...
- (void)cancelFadeOut {
  // Cancel fade out so a new timer can be started below, in case we were
  // already holding or fading in.
...
- (void)setAlphaValue:(CGFloat)alphaValue animated:(BOOL)animated {
  if ([self.superview
          conformsToProtocol:@protocol(iTermTabBarControlViewContainer)]) {
    if (animated) {
...
- (void)setHidden:(BOOL)hidden {
  if (!hidden || [self.itermTabBarDelegate iTermTabBarShouldHideBacking]) {
    if ([self.superview
            conformsToProtocol:@protocol(iTermTabBarControlViewContainer)]) {
      id<iTermTabBarControlViewContainer> container =
          (id<iTermTabBarControlViewContainer>)self.superview;
...
- (void)scheduleFadeOutAfterDelay {
  ...
  _flashDelayedPerform = [NSView
      animateWithDuration:[iTermAdvancedSettingsModel tabFlashAnimationDuration]
...
- (void)fadeOut {
  DLog(@"fade out");
  // If there is a delayed perform to fade out, cancel that so we don't try to
  // fade out twice.
  _flashDelayedPerform.canceled = YES;
  _flashDelayedPerform = nil;
...
- (void)setFlashState:(iTermTabBarFlashState)flashState {
  ...
  DLog(@"%@ -> %@ from\n%@", names[self.flashState], names[flashState],
...
- (BOOL)mouseDownCanMoveWindow {
  return [self.itermTabBarDelegate iTermTabBarCanDragWindow]
             ? NO
...
- (NSRect)_opaqueRectForWindowMoveWhenInTitlebar {
  return [self.itermTabBarDelegate iTermTabBarCanDragWindow]
             ? self.bounds
             : [super _opaqueRectForWindowMoveWhenInTitlebar];
}
...
- (void)mouseDown:(NSEvent *)event {
  ...
  const BOOL handleDrag =
      ([self.itermTabBarDelegate iTermTabBarCanDragWindow] &&
       ![self wantsMouseDownAtPoint:pointInView] && hitView == self &&
