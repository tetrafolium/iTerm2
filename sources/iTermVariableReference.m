...
+ (instancetype)linkToVariables:(iTermVariables *)variables
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p path=%@ numlinks=%@>",
                                    NSStringFromClass(self.class), self, _path,
                                    @(_links.count)];
}
...
- (void)addLinkToVariables:(iTermVariables *)variables
  ...
  [_links addObject:[iTermVariableReferenceLink linkToVariables:variables
                                                      localPath:path]];
}
