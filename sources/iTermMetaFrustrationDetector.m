...
- (void)didSendKeyEvent:(NSEvent *)event {
  ...
  case iTermMetaFrustrationDetectorReady:
    if ([self eventIsCandidate:event]) {
      ...
      _lastWasLeft = (event.it_modifierFlags & NSLeftAlternateKeyMask) ==
  ...
  case iTermMetaFrustrationDetectorWaitingForEsc:
    if ([self eventIsEsc:event]) {
      ...
      if ([NSDate timeIntervalSinceReferenceDate] - _lastTime < threshold) {
        if (_lastWasLeft) {
          [self.delegate
                  metaFrustrationDetectorDidDetectFrustrationForLeftOption];
        } else {
          [self.delegate
                  metaFrustrationDetectorDidDetectFrustrationForRightOption];
        }
...
- (void)setState:(iTermMetaFrustrationDetectorState)newState {
  NSString * (^name)(iTermMetaFrustrationDetectorState) =
      ^NSString *(iTermMetaFrustrationDetectorState state) {
    switch (state) {
    case iTermMetaFrustrationDetectorWaitingForEsc:
...
- (BOOL)eventIsCandidate:(NSEvent *)event {
  ...
  dispatch_once(&onceToken, ^{
    alphabeticKeyCodes = [NSSet setWithArray:@[
...
- (BOOL)eventIsBackspace:(NSEvent *)event {
  ...
  if ((event.it_modifierFlags & mask) == NSEventModifierFlagControl &&
