...
- (NSString *)keyForColor:(NSColor *)color {
  // Quantizing at 4 bits per component should be enough to produce good logos,
  // while hopefully getting us wins here and there.
...
- (NSImage *)generatedImage {
  ...
  NSImage *shadow = [NSImage it_imageNamed:@"LogoShadow.png"
                                  forClass:self.class];

  [_backgroundColor set];
  NSBezierPath *path = [NSBezierPath bezierPath];
  [path appendBezierPathWithRoundedRect:NSMakeRect(3, 6, 42, 36)
                                xRadius:2
                                yRadius:2];
  [path fill];
  ...
  if (self.tabColor) {
    ...
    NSRectFillUsingOperation(
        NSMakeRect(0, height - tabHeight, width, tabHeight),
        NSCompositingOperationSourceIn);
  }

  [shadow drawInRect:NSMakeRect(0, 0, width, height)];
  NSString *prompt = @"$";
  [prompt drawAtPoint:NSMakePoint(7, 25)
       withAttributes:@{
