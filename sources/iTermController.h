...
typedef NS_ENUM(NSUInteger, iTermHotkeyWindowType) {
  ...
  iTermHotkeyWindowTypeFloatingPanel, // joins all spaces and has a higher level
                                      // than a regular window. Is an NSPanel.
  iTermHotkeyWindowTypeFloatingWindow // has a higher level than a regular
                                      // window.
};
...
// Does not enter fullscreen automatically; that is left to the caller, since
// tmux has special logic around this. Call -didFinishCreatingTmuxWindow: after
// it is doing being set up.
- (PseudoTerminal *)openTmuxIntegrationWindowUsingProfile:(Profile *)profile;

// This is called when the window created by
// -openTmuxIntegrationWindowUsingProfile is done being initialized.
...
// Indicates a rough guess as to whether a terminal window is substantially
// visible. Being on another space will count as being obscured. If the window
// is completely covered by another app's window, it's obscured. If other iTerm
// windows cover more than ~40% of |terminal| then it's obscured.
...
// Does a serialized fullscreening of the term's window. Slated for production
// in 3.1.
...
typedef NS_OPTIONS(NSUInteger, iTermSingleUseWindowOptions) {
  ...
  // Treat the session as short-lived: it will not post a notification when it
  // ends and it can be closed while buried.
  iTermSingleUseWindowOptionsShortLived = (1 << 0),
  // Override the default profile's close on termination  setting to always
