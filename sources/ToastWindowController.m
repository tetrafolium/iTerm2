...
+ (void)showToastWithMessage:(NSString *)message duration:(NSInteger)duration {
  ToastWindowController *toast =
      [[[ToastWindowController alloc] init] autorelease];
  ...
  [roundedRect
      setFrame:NSMakeRect(0, 0, textField.frame.size.width + hPadding * 2,
  ...
  [panel setOpaque:NO];
  [panel setFrame:NSMakeRect((screen.visibleFrame.size.width -
...
- (void)hideAfterDelay:(NSTimeInterval)delay {
  ...
  hideTimer_ = [NSTimer scheduledTimerWithTimeInterval:delay
                                                target:self
