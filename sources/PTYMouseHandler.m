...
@implementation PTYMouseHandler {
  ...
  // Saves the monotonically increasing event number of a first-mouse click,
  // which disallows selection.
  ...
  // Works around an apparent OS bug where we get drag events without a
  // mousedown.
  BOOL dragOk_;

  BOOL _committedToDrag;

  // Detects when the user is trying to scroll in alt screen with the scroll
  // wheel.
...
- (instancetype)initWithSelectionScrollHelper:
  ...
  if (self) {
    ...
    _altScreenMouseScrollInferrer =
        [[iTermAltScreenMouseScrollInferrer alloc] init];
    ...
    _mouseReportingFrustrationDetector =
        [[iTermMouseReportingFrustrationDetector alloc] init];
    _mouseReportingFrustrationDetector.delegate =
        mouseReportingFrustrationDetectorDelegate;
...
- (void)mouseDown:(NSEvent *)event superCaller:(void (^)(void))superCaller {
  ...
  DLog(@"Mouse Down on %@ with event %@, num touches=%d", self, event,
       self.numTouches);
...
- (BOOL)mouseDownImpl:(NSEvent *)event {
  ...
  _mouseDownWasFirstMouse =
      ([event eventNumber] == _firstMouseEventNumber) || ![NSApp keyWindow];
  _lastMouseDownOnSelectedText = NO; // This may get updated to YES later.
  const BOOL altPressed =
      ([event it_modifierFlags] & NSEventModifierFlagOption) != 0;
  BOOL cmdPressed =
      ([event it_modifierFlags] & NSEventModifierFlagCommand) != 0;
  const BOOL shiftPressed =
      ([event it_modifierFlags] & NSEventModifierFlagShift) != 0;
  const BOOL ctrlPressed =
      ([event it_modifierFlags] & NSEventModifierFlagControl) != 0;
  if (gDebugLogging && altPressed && cmdPressed && shiftPressed &&
      ctrlPressed) {
    // Dump view hierarchy
  ...
  if (_mouseDownWasFirstMouse && !cmdPressed &&
    ...
    // A click in an inactive window without cmd pressed by default just brings
    // the window to the fore and takes no additional action. If you enable
    // alwaysAcceptFirstMouse then it is treated like a normal click (issue
    // 3236). Returning here prevents mouseDown=YES which keeps -mouseUp from
    // doing anything such as changing first responder.
  ...
  if (_numTouches == 3) {
    ...
    //   Options... > Enable dragging > three finger drag
    // Then a three-finger drag gets translated into
    // mouseDown:…mouseDragged:…mouseUp:. Prior to commit 96323ddf8 we didn't
    // track touch-up/touch-down unless necessary, so that feature worked unless
    // you had a mouse gesture that caused touch tracking to be enabled. In
    // issue 8321 it was revealed that touch tracking breaks three-finger drag.
    // The solution is to not return early if we detect a three-finger down but
    // don't have a pointer action for it. Then it proceeds to behave like
    // before commit 96323ddf8. Otherwise, it'll just watch for the actions that
    // three-finger touches can cause.
    ...
    if ([iTermPreferences boolForKey:kPreferenceKeyThreeFingerEmulatesMiddle]) {
      ...
      DLog(@"Set mouseDown=YES because of 3 finger mouseDown (not emulating "
           @"middle)");
      _mouseDown = YES;
    }
  ...
  if (cmdPressed) {
    if (![self.mouseDelegate mouseHandlerViewHasFocus:self]) {
      ...
      // A cmd-click in an active session in a non-key window acts like a click
      // without cmd. This can be changed in advanced settings so cmd-click will
      // still invoke semantic history even for non-key windows.
      DLog(@"Cmd-click in active session in non-key window");
      cmdPressed = [iTermAdvancedSettingsModel
          cmdClickWhenInactiveInvokesSemanticHistory];
    }
  }
  if (([event it_modifierFlags] & kDragPaneModifiers) == kDragPaneModifiers) {
  ...
  if ([self.mouseDelegate mouseHandler:self
                        coordIsMutable:VT100GridCoordMake(x, y)] &&
  ...
  iTermImageInfo *const imageBeingClickedOn =
      [self.mouseDelegate mouseHandler:self imageAt:VT100GridCoordMake(x, y)];
  const long long overflow =
      [_mouseDelegate mouseHandlerTotalScrollbackOverflow:self];
  ...
  if (!_mouseDownWasFirstMouse) {
    ...
    if (event.clickCount == 1 && !cmdPressed && !shiftPressed &&
        !imageBeingClickedOn && !mouseDownOnSelection) {
      [self.selection clearSelection];
    }
  }
  ...
  DLog(@"clickCount=%d altPressed=%d cmdPressed=%d", clickCount,
       (int)altPressed, (int)cmdPressed);
  ...
  if (isExtension && [self.selection hasSelection]) {
    if (!self.selection.live) {
      [self.selection
          beginExtendingSelectionAt:VT100GridAbsCoordMake(x, y + overflow)];
    }
  } else if (clickCount < 2) {
    // single click
    iTermSelectionMode mode;
    if ((event.it_modifierFlags & kRectangularSelectionModifierMask) ==
    ...
    if (imageBeingClickedOn) {
      ...
      [self.selection
          beginSelectionAtAbsCoord:VT100GridAbsCoordMake(x, y + overflow)
                              mode:mode
                            resume:NO
    ...
    BOOL wholeLines = [iTermPreferences
        boolForKey:kPreferenceKeyTripleClickSelectsFullWrappedLines];
    iTermSelectionMode mode =
...
- (void)mouseUp:(NSEvent *)event {
  DLog(@"Mouse Up on %@ with event %@, numTouches=%d", self, event,
       _numTouches);
  ...
  _numTouches = 0;
  _firstMouseEventNumber =
      -1; // Synergy seems to interfere with event numbers, so reset it here.
  if (_mouseDownIsThreeFingerClick) {
    ...
    _mouseDown = NO;
    DLog(@"Returning from mouseUp because there were 3 touches. Set "
         @"mouseDown=NO");
    // We don't want a three finger tap followed by a scrollwheel to make a
    // selection.
    [self.selection endLiveSelection];
    return;
  }
  ...
  const BOOL cmdActuallyPressed =
      (([event it_modifierFlags] & NSEventModifierFlagCommand) != 0);
  // Make an exception to the first-mouse rule when cmd-click is set to always
  // invoke semantic history.
  const BOOL cmdPressed =
      cmdActuallyPressed &&
      (!_mouseDownWasFirstMouse ||
  ...
  BOOL isUnshiftedSingleClick =
      ([event clickCount] < 2 && !mouseDragged &&
       !([event it_modifierFlags] & NSEventModifierFlagShift));
  BOOL isShiftedSingleClick =
      ([event clickCount] == 1 && !mouseDragged &&
       ([event it_modifierFlags] & NSEventModifierFlagShift));
  BOOL willFollowLink =
      (isUnshiftedSingleClick && cmdPressed &&
       [iTermPreferences boolForKey:kPreferenceKeyCmdClickOpensURLs]);

  // Reset _mouseDragged; it won't be needed again and we don't want it to get
  // stuck like in issue 3766.
  _mouseDragged = NO;
  ...
  if ([self reportMouseEvent:event]) {
    if (willFollowLink) {
      // This is a special case. Cmd-click is treated like alt-click at the
      // protocol level (because we use alt to disable mouse reporting,
      // unfortunately). Few apps interpret alt-clicks specially, and we really
      // want to handle cmd-click on links even when mouse reporting is on. Link
      // following has to be done on mouse up to allow the user to drag links
      // and to cancel accidental clicks (by doing mouseUp far away from
      // mouseDown). So we report the cmd-click as an alt-click and then open
      // the link. Note that cmd-alt-click isn't handled here because you won't
      // get here if alt is pressed. Note that openTargetWithEvent: may not do
      // anything if the pointer isn't over a clickable string.
  ...
  if (!cmdActuallyPressed) {
    // Make ourselves the first responder except on cmd-click. A cmd-click on a
    // non-key window gets treated as a click that doesn't raise the window. A
    // cmd-click in an inactive pane in the key window shouldn't make it first
    // responder, but still gets treated as cmd-click.
    //
    // We use cmdActuallyPressed instead of cmdPressed because on first-mouse
    // cmdPressed gets unset so this function generally behaves like it got a
  ...
  if (isUnshiftedSingleClick) {
    ...
    if (altPressed &&
        ...
        !_mouseDownWasFirstMouse) {
      // This moves the cursor, but not if mouse reporting is on for button
      // clicks. It's also off for first mouse because of issue 2943 (alt-click
      // to activate an app is used to order-back all of the previously active
      // app's windows).
      ...
      default: {
        // Not reporting mouse clicks, so we'll move the cursor since the remote
        // app can't.
        VT100GridCoord coord = [self.mouseDelegate
            mouseHandlerCoordForPointInWindow:[event locationInWindow]];
        if (!cmdPressed) {
    ...
    if (willFollowLink) {
      ...
      const VT100GridCoord clickPoint = [self.mouseDelegate mouseHandler:self
                                                              clickPoint:event
                                                           allowOverflow:NO];
      [self.mouseDelegate mouseHandlerSetFindOnPageCursorCoord:clickPoint];
    }
    if ([self.mouseDelegate mouseHandlerAtPasswordPrompt:self] && !altPressed &&
        !cmdPressed) {
      const VT100GridCoord clickCoord = [self.mouseDelegate mouseHandler:self
                                                              clickPoint:event
                                                           allowOverflow:NO];
      ...
      if (VT100GridCoordEquals(clickCoord, cursorCoord)) {
        [self.mouseDelegate mouseHandlerOpenPasswordManager:self];
             ...
             [self.mouseDelegate mouseHandler:self
                          getFindOnPageCursor:&findOnPageCursor] &&
             ![self.selection hasSelection]) {
    const long long overflow =
        [_mouseDelegate mouseHandlerTotalScrollbackOverflow:self];
    [self.selection beginSelectionAtAbsCoord:VT100GridAbsCoordFromCoord(
                                                 findOnPageCursor, overflow)
                                        mode:kiTermSelectionModeCharacter
                                      resume:NO
                                      append:NO];

    const VT100GridCoord newEndPoint = [self.mouseDelegate mouseHandler:self
                                                             clickPoint:event
                                                          allowOverflow:YES];
    [self.selection moveSelectionEndpointTo:VT100GridAbsCoordFromCoord(
                                                newEndPoint, overflow)];
    [self.selection endLiveSelection];

    [self.mouseDelegate mouseHandlerResetFindOnPageCursor:self];
  }

  DLog(@"Has selection=%@, delegate=%@ wasSelecting=%@",
       @([self.selection hasSelection]), self.mouseDelegate, @(wasSelecting));
  if ([self.selection hasSelection] && event.clickCount == 1 && !wasSelecting &&
      _lastMouseDownOnSelectedText) {
    // Click on selection. When the mouse-down was on the selection we delay
    // clearing it until mouse-up so you have the chance to drag it.
    [self.selection clearSelection];
  }
  if ([self.selection hasSelection] &&
      [self.mouseDelegate mouseHandlerIsValid:self] && wasSelecting) {
    // if we want to copy our selection, do so
    DLog(@"selection copies text=%@",
         @([iTermPreferences boolForKey:kPreferenceKeySelectionCopiesText]));
...
- (void)mouseDragged:(NSEvent *)event {
  ...
  const NSPoint locationInTextView = [self.mouseDelegate mouseHandler:self
  ...
  if (EuclideanDistance(mouseDownLocation, locationInWindow) >=
      kDragThreshold) {
  ...
  const BOOL okToDrag =
      (![iTermAdvancedSettingsModel requireCmdForDraggingText] ||
  ...
  if (okToDrag) {
    if (_mouseDownOnImage && dragThresholdMet) {
      ...
      NSString *theSelectedText =
          [self.mouseDelegate mouseHandlerSelectedText:self];
      if ([theSelectedText length] > 0) {
        _committedToDrag = YES;
        [self.mouseDelegate mouseHandler:self
                                dragText:theSelectedText
                                forEvent:event];
  ...
  if (pressingCmdOnly && !dragThresholdMet) {
    // If you're holding cmd (but not opt) then you're either trying to click on
    // a link and accidentally dragged a little bit, or you're trying to drag a
    // selection. Do nothing until the threshold is met.
  ...
  if (_mouseDownOnSelection == YES &&
      ([event it_modifierFlags] &
       (NSEventModifierFlagOption | NSEventModifierFlagCommand)) ==
          (NSEventModifierFlagOption | NSEventModifierFlagCommand) &&
      !dragThresholdMet) {
    // Would be a drag of a rect region but mouse hasn't moved far enough yet.
    // Prevent the selection from changing.
    DLog(@"too-short drag of rect region");
    return;
  }

  if (![self.selection hasSelection] && pressingCmdOnly &&
      _semanticHistoryDragged == NO) {
  ...
  [_selectionScrollHelper mouseDraggedTo:locationInTextView
                                   coord:VT100GridCoordMake(x, y)];

  if (!wasMakingThreeFingerSelection && _makingThreeFingerSelection) {
    DLog(@"Just started a three finger selection in mouseDragged (because of "
         @"macOS bugs)");
    const BOOL shiftPressed =
        ([event it_modifierFlags] & NSEventModifierFlagShift) != 0;
    const BOOL isExtension = ([self.selection hasSelection] && shiftPressed);
    ...
    if ([self.mouseDelegate mouseHandler:self
                moveSelectionToGridCoord:VT100GridCoordMake(x, y)
...
- (void)rightMouseDown:(NSEvent *)event
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyFocusOnRightOrMiddleClick]) {
    [_mouseDelegate
        mouseHandlerMakeKeyAndOrderFrontAndMakeFirstResponderAndActivateApp:
            self];
  }
...
- (BOOL)scrollWheel:(NSEvent *)event pointInView:(NSPoint)point {
  ...
  if (!_haveSeenScrollWheelEvent) {
    // Work around a weird bug. Commit 9e4b97b18fac24bea6147c296b65687f0523ad83
    // caused it. When you restore a window and have an inline scroller (but not
    // a legacy scroller!) then it will be at the top, even though we're
    // scrolled to the bottom. You can either jiggle it after a delay to fix it,
    // or after thousands of dispatch_async calls, or here. None of these make
    // any sense, nor does the bug itself. I get the feeling that a giant rats'
    // nest of insanity underlies scroll wheels on macOS.
  ...
  if ([self scrollWheelShouldSendDataForEvent:event at:point]) {
    ...
    const CGFloat deltaY = [self.mouseDelegate mouseHandler:self
                          accumulateVerticalScrollFromEvent:event];
    BOOL forceLatin1 = NO;
  ...
  if ([self handleMouseEvent:event
                    testOnly:NO
                   deltaYOut:&deltaY
               reportableOut:&reportable]) {
    return NO;
  }
  if (self.selection.live) {
    // Scroll via wheel while selecting. This is even possible with the trackpad
    // by using three fingers as described in issue 8538.
    if ([self.mouseDelegate mouseHandler:self
             moveSelectionToPointInEvent:event]) {
      _committedToDrag = YES;
    }
    return YES;
  }
...
- (void)didDragSemanticHistory {
  ...
  // Valid drag, so we reset the flag because mouseUp doesn't get called when a
  // drag is done
  _semanticHistoryDragged = NO;
}
...
// such as dragging.
// The NSEvent method for creating an event can't be used because it doesn't let
// you set the buttonNumber field.
...
// Returns YES if the mouse event would be handled, ignoring trivialities like a
// drag that hasn't changed coordinate since the last drag.
...
- (BOOL)handleMouseEvent:(NSEvent *)event
  ...
  const NSPoint point = [self.mouseDelegate mouseHandler:self
                                       viewCoordForEvent:event
                                                 clipped:NO];
  ...
  VT100GridCoord coord =
      [self.mouseDelegate mouseHandlerCoordForPointInView:point];
  const CGFloat deltaY =
      [self.mouseDelegate mouseHandlerAccumulatedDeltaY:self forEvent:event];

  if (deltaYOut) {
    *deltaYOut = deltaY;
  }

  return [self.mouseDelegate
                  mouseHandler:self
...
// If mouse reports are sent to the delegate, will it use them? Use with
// -xtermMouseReporting, which understands Option to turn off reporting.
...
- (BOOL)shouldReportMouseEvent:(NSEvent *)event at:(NSPoint)point {
  ...
  if (event.type == NSEventTypeLeftMouseDown ||
    ...
    // Three-finger click mousedown should not be reported normally to avoid
    // accidental reports because what reporting a 3-finger click when not
    // emulating the middle button is nonsense. It's certainly not something
    // mouse reporting supports. These are buttonNumber=0, _numTouches=3. Issue
    // 8481.
    //
    // However, three-finger mousedown with *three-finger drag enabled* must be
    // reported so 3-finger drags get reported.
    //
    // Three-finger tap emulating middle click has button 2. Middle clicks are
    // reportable.
    //
    // A 2-finger mouse-down is reportable because it's necessary for drag
    // reporting to work with two touches. Issue 8481.
    if (event.buttonNumber == 0 && _numTouches == 3 &&
        ![self trackpadThreeFingerDragEnabled]) {
      DLog(@"Three touches on left button up/down, but three-finger drag is "
           @"disabled, so don't report it.");
      return NO;
    }
  }
  if ((event.type == NSEventTypeLeftMouseDown ||
...
- (BOOL)scrollWheelShouldSendDataForEvent:(NSEvent *)event at:(NSPoint)point {
  ...
  if ([self shouldReportMouseEvent:event at:point] &&
    ...
    // Prefer to report the scroll than to send arrow keys in this mouse
    // reporting mode.
  ...
  NSString *downString =
      [iTermAdvancedSettingsModel alternateMouseScrollStringForDown];
...
- (BOOL)shouldSelectWordWithClicks:(int)clickCount {
  if ([iTermPreferences
          boolForKey:kPreferenceKeyDoubleClickPerformsSmartSelection]) {
    return clickCount == 4;
  } else {
    return clickCount == 2;
  }
...
- (iTermSwipeState *)swipeTrackerWillBeginNewSwipe:
    (iTermSwipeTracker *)tracker {
  id<iTermSwipeHandler> handler =
      [self.mouseDelegate mouseHandlerSwipeHandler:self];
