...
NSString *const PTYNoteViewControllerShouldUpdatePosition =
    @"PTYNoteViewControllerShouldUpdatePosition";
NSString *const iTermAnnotationVisibilityDidChange =
    @"iTermAnnotationVisibilityDidChange";
...
- (void)loadView {
  ...
  self.noteView = [[[PTYNoteView alloc]
      initWithFrame:NSMakeRect(0, 0, kWidth, kHeight)] autorelease];
  ...
  NSRect frame = NSMakeRect(0, 3, kWidth, kHeight);
  ...
  self.textView = [[[NSTextView alloc]
      initWithFrame:NSMakeRect(0, 0, scrollView_.contentSize.width,
                               scrollView_.contentSize.height)] autorelease];
  ...
  textView_.textContainer.containerSize =
      NSMakeSize(scrollView_.frame.size.width, FLT_MAX);
...
- (void)setAnchor:(NSPoint)anchor {
  ...
  if (anchor_.x + noteView_.frame.size.width > superViewFrame.size.width) {
    xOffset =
        anchor_.x + noteView_.frame.size.width - superViewFrame.size.width;
  }
  ...
  noteView_.frame =
      NSMakeRect(anchor_.x - xOffset, anchor_.y, size.width, size.height);
  ...
  if (NSMaxY(noteView_.frame) > superViewMaxY) {
    ...
    noteView_.frame =
        NSMakeRect(anchor_.x - xOffset, anchor_.y - noteView_.frame.size.height,
...
- (void)finalizeToggleOfHide:(BOOL)newValue {
  ...
  if (newValue) {
    ...
    if (gVisibleNotes == 0) {
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAnnotationVisibilityDidChange
                        object:nil];
    }
  }
}
...
- (void)sizeToFit {
  ...
  NSSize scrollViewSize = [NSScrollView
      frameSizeForContentSize:usedRect.size
  ...
  NSRect theFrame =
      NSMakeRect(NSMinX(scrollView_.frame), NSMinY(scrollView_.frame),
...
- (void)updateBackgroundColor {
  ...
  NSTimeInterval elapsed =
      [NSDate timeIntervalSinceReferenceDate] - highlightStartTime_;
  ...
  self.noteView.backgroundColor = [NSColor colorWithCalibratedRed:components[0]
                                                            green:components[1]
                                                             blue:components[2]
                                                            alpha:1];
  [self.noteView setNeedsDisplay:YES];
  if (alpha < 1) {
    [self performSelector:@selector(updateBackgroundColor)
               withObject:nil
               afterDelay:1 / 30.0];
  }
}
