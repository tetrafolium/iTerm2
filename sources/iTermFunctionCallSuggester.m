...
- (instancetype)
  ...
  if (self) {
    ...
    // NOTE:
    // CPSLRParser is a slightly faster parser but is not capable of dealing
    // with the grammar rules for functions in expressions. You need lookahead
    // to distinguish function calls from paths. "foo.bar(" is a valid beginning
    // for a function call. "foo.bar.baz(" is not because nested namespaces are
    // not allowed. Without lookahead, the SLR parser has a shift-reduce
    // conflict because it can't distinguish "funcname ::= Identifier '.'
    // Identifier" from "path ::= Identifier ['.' path]". The reason a funcname
    // is not a path is that it can only have two parts.
    //
    // See the comments in the headers for the two parsers for more details
    // about their costs.
    _parser = [CPLALR1Parser parserWithBNF:_grammarProcessor.backusNaurForm
                                     start:self.grammarStart];
    assert(_parser);
...
- (CPQuotedRecogniser *)stringRecognizer {
  return [iTermExpressionParser
      stringRecognizerWithClass:[iTermTruncatedQuotedRecognizer class]];
}
...
- (void)loadRulesAndTransforms {
  ...
  [_grammarProcessor addProductionRule:@"call ::= <funcname> <arglist>"
                         treeTransform:^id(CPSyntaxTree *syntaxTree) {
                           return
                               [weakSelf callWithName:syntaxTree.children[0]
  ...
  [_grammarProcessor
      addProductionRule:@"funcname ::= 'Identifier' '.' 'Identifier'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [NSString
                stringWithFormat:@"%@.%@", [syntaxTree.children[0] identifier],
  ...
  [_grammarProcessor
      addProductionRule:@"arglist ::= '(' <args> 'EOF'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return
                @{@"partial-arglist" : @YES,
  ...
  [_grammarProcessor addProductionRule:@"arglist ::= '(' 'EOF'"
                         treeTransform:^id(CPSyntaxTree *syntaxTree) {
                           return @{@"partial-arglist" : @YES, @"args" : @[]};
                         }];
  ...
  [_grammarProcessor
      addProductionRule:@"args ::= <arg> ',' <args>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [@[ syntaxTree.children[0], @"," ]
                arrayByAddingObjectsFromArray:syntaxTree.children[2]];
          }];
  [_grammarProcessor
      addProductionRule:@"arg ::= 'Identifier' ':' <expression>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return @{
              @"identifier" : [syntaxTree.children[0] identifier],
              @"colon" : @YES,
              @"expression" : syntaxTree.children[2]
            };
          }];
  [_grammarProcessor
      addProductionRule:@"arg ::= 'Identifier' ':' 'EOF'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return @{
              @"identifier" : [syntaxTree.children[0] identifier],
              @"colon" : @YES
  ...
  [_grammarProcessor
      addProductionRule:@"expression ::= <path> '?'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return @{@"path" : syntaxTree.children[0], @"terminated" : @YES};
          }];
  ...
  [_grammarProcessor
      addProductionRule:@"expression ::= 'SwiftyString'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            iTermSwiftyStringToken *token =
                [iTermSwiftyStringToken castFrom:syntaxTree.children[0]];
  ...
  [_grammarProcessor
      addProductionRule:
          @"expression ::= '[' <comma_delimited_expressions> 'EOF'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [syntaxTree.children[1]
                dictionaryBySettingObject:@YES
                                   forKey:@"inside-truncated-array-literal"];
          }];
  [_grammarProcessor
      addProductionRule:@"expression ::= '[' <comma_delimited_expressions> ']'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return @{@"literal" : @YES};
          }];
  [_grammarProcessor
      addProductionRule:@"comma_delimited_expressions ::= <expression>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return syntaxTree.children[0];
          }];
  [_grammarProcessor
      addProductionRule:@"comma_delimited_expressions ::= <expression> ',' "
                        @"<comma_delimited_expressions>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
  ...
  [_grammarProcessor
      addProductionRule:@"path ::= 'Identifier' '.' 'EOF'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [NSString
                stringWithFormat:@"%@.", [syntaxTree.children[0] identifier]];
          }];
  ...
  [_grammarProcessor
      addProductionRule:@"composed_call ::= <funcname> <composed_arglist>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
  ...
  [_grammarProcessor
      addProductionRule:@"composed_arglist ::= '(' <args> 'EOF'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return
                @{@"partial-arglist" : @YES,
                  @"args" : syntaxTree.children[1]};
...
- (NSArray<NSString *> *)suggestionsForString:(NSString *)prefix {
  ...
  if (prefix.length == 0) {
    // Zero-prefix suggest. This is a special case to avoid an annoying
    // shift-reduce conflict and because I expect it will need fancier ranking.
    return [self
        pathsAndFunctionSuggestionsWithPrefix:@""
                                   legalPaths:[_pathSource(@"") allObjects]];
  }
  _prefix = prefix;
...
- (NSArray<NSString *> *)usedParameterNamesInPartialArgList:
    (NSArray *)partialArgList {
...
- (NSArray<NSString *> *)suggestionsForFunctionName:(NSString *)prefix {
  ...
  return
      [[registeredFunctions filteredArrayUsingBlock:^BOOL(NSString *anObject) {
        return [anObject hasPrefix:prefix] || prefix.length == 0;
      }] mapWithBlock:^id(NSString *s) {
        NSString *firstArgName = [[self argumentNamesForFunction:s].firstObject
...
- (NSArray<NSString *> *)
    suggestedNextArgumentForExistingArgs:(NSArray *)partialArgList
  ...
  NSArray<NSString *> *usedParameterNames =
      [self usedParameterNamesInPartialArgList:partialArgList];
  ...
  if (![lastArg isEqual:@","]) {
    ...
    if (argDict[@"colon"]) {
      NSString *nextArgumentName = [[self argumentNamesForFunction:function]
          objectPassingTest:^BOOL(NSString *element, NSUInteger index,
                                  BOOL *stop) {
            return ![usedParameterNames
                containsObject:[element stringByAppendingString:@":"]];
          }];
...
- (NSArray<NSString *> *)suggestedExpressions:(NSDictionary *)expression
  ...
  if (expression == nil || expression[@"literal"] ||
    ...
    iTermFunctionCallSuggester *inner = [[iTermFunctionCallSuggester alloc]
        initWithFunctionSignatures:_functionSignatures
                        pathSource:_pathSource];
    return [inner suggestionsForString:expression[@"truncated_interpolation"]];
  } else {
    NSArray<NSString *> *legalPaths =
        [_pathSource(expression[@"path"]) allObjects];
    const BOOL insideTruncatedArrayLiteral =
        [expression[@"inside-truncated-array-literal"] boolValue];
...
- (NSArray<NSString *> *)
    pathsAndFunctionSuggestionsWithPrefix:(NSString *)prefix
  ...
  functionNames = [functionNames mapWithBlock:^id(NSString *anObject) {
    NSString *firstArgName =
        [self argumentNamesForFunction:anObject].firstObject ?: @")";
  ...
  NSArray<NSString *> *options =
      [legalPaths arrayByAddingObjectsFromArray:functionNames];
...
- (NSArray<NSString *> *)
    suggestedParameterNamesForFunction:(NSString *)function
                             excluding:(NSArray<NSString *> *)exclusions
                                prefix:(NSString *)prefix {
  NSArray<NSString *> *legalNames = [[self argumentNamesForFunction:function]
  ...
  NSArray *suggestions =
      [[legalNames filteredArrayUsingBlock:^BOOL(NSString *anObject) {
        return ![exclusions containsObject:anObject] &&
               (prefix.length == 0 || [anObject hasPrefix:prefix]);
      }] mapWithBlock:^id(NSString *s) {
        return [s substringFromIndex:prefix.length];
      }];
  return suggestions;
...
- (CPRecoveryAction *)parser:(CPParser *)parser
  ...
  if (inputStream.peekToken == nil &&
      [acceptableTokens containsObject:@"EOF"]) {
    return
        [CPRecoveryAction recoveryActionWithAdditionalToken:[CPEOFToken eof]];
  }
...
- (NSArray<NSString *> *)suggestionsForString:(NSString *)prefix {
  ...
  iTermSwiftyStringParser *parser =
      [[iTermSwiftyStringParser alloc] initWithString:prefix];
  parser.tolerateTruncation = YES;
  NSInteger index = [parser enumerateSwiftySubstringsWithBlock:nil];
  if (index > prefix.length || index == NSNotFound || !parser.wasTruncated ||
  ...
  NSArray<NSString *> *allSuggestions =
      [undecoratedSuggestions mapWithBlock:^id(NSString *tail) {
        return [prefix stringByAppendingString:tail];
      }];
  NSArray<NSString *> *suggestionsUpToFirstPeriod =
      [allSuggestions mapWithBlock:^id(NSString *string) {
        ...
        NSInteger index =
            [string rangeOfString:@"."
                          options:0
                            range:NSMakeRange(prefix.length, remaining)]
                .location;
        if (index == NSNotFound) {
          return string;
        } else {
  ...
  return
      [[suggestionsUpToFirstPeriod sortedArrayUsingSelector:@selector(compare:)]
          reduceWithFirstValue:@[]
                         block:^id(NSArray *uniqueValues, NSString *string) {
                           if (uniqueValues.count < 50 &&
                               ![NSObject object:uniqueValues.lastObject
                                   isEqualToObject:string]) {
...
- (NSArray<NSString *> *)parsedResult:(id)result forString:(NSString *)prefix {
  NSArray<NSString *> *suggestions =
      [self suggestedExpressions:[NSDictionary castFrom:result]
