...
- (instancetype)initWithMessage:(NSString *)message {
  ...
  if (self) {
    ...
    self.frame = NSMakeRect(0, 0, _textField.frame.size.width + margin * 2,
                            _textField.frame.size.height + margin * 2);
    [self addSubview:_vev];
    _vev.frame = self.bounds;
    [_vev addSubview:_textField];
    _textField.frame = NSMakeRect(margin, margin, _textField.frame.size.width,
