...
+ (iTermReflectionMethodArgument *)argumentForTypeString:(NSString *)typeString
  ...
  if (type) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:type.unsignedIntegerValue
  ...
  if ([typeString hasPrefix:@"^"]) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:iTermReflectionMethodArgumentTypePointer
        argumentName:argumentName];
  }
  if ([typeString hasPrefix:@"{"]) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:iTermReflectionMethodArgumentTypeStruct
        argumentName:argumentName];
  }
  if ([typeString hasPrefix:@"("]) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:iTermReflectionMethodArgumentTypeUnion
        argumentName:argumentName];
  }
  if ([typeString hasPrefix:@"b"]) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:iTermReflectionMethodArgumentTypeBitField
        argumentName:argumentName];
  }
  if ([typeString hasPrefix:@"["]) {
    return
        [self argumentForArrayTypeString:
                  [typeString
                      substringWithRange:NSMakeRange(1, typeString.length - 2)]
                            argumentName:argumentName];
  }
  return [[iTermReflectionMethodArgument alloc]
      initWithType:iTermReflectionMethodArgumentTypeUnknown
      argumentName:argumentName];
}

+ (iTermReflectionMethodArgument *)
    argumentForObjectTypeString:(NSString *)typeString
                   argumentName:(NSString *)argumentName {
  if ([typeString hasPrefix:@"?"]) {
    return [[iTermReflectionMethodArgument alloc]
        initWithType:iTermReflectionMethodArgumentTypeBlock
        argumentName:argumentName];
  }
  if (![typeString hasPrefix:@"\""]) {
    // Really, this is an id, but I don't care about non-NSObject objects.
    return [[iTermReflectionMethodArgument alloc]
        initWithObjectHavingClassName:@"NSObject"
                         argumentName:argumentName];
  }
  NSRange closeQuote =
      [typeString rangeOfString:@"\""
                        options:0
                          range:NSMakeRange(1, typeString.length - 1)];
  assert(closeQuote.location != NSNotFound);
  NSString *className =
      [typeString substringWithRange:NSMakeRange(1, closeQuote.location - 2)];
  return [[iTermReflectionMethodArgument alloc]
      initWithObjectHavingClassName:className
                       argumentName:argumentName];
}

+ (iTermReflectionMethodArgument *)
    argumentForArrayTypeString:(NSString *)typeString
                  argumentName:(NSString *)argumentName {
  // iTermReflectionMethodArgument *inner = [self
  // argumentForTypeString:typeString];
  // TODO
  return [[iTermReflectionMethodArgument alloc]
      initWithType:iTermReflectionMethodArgumentTypeArray
      argumentName:argumentName];
}
...
- (NSArray<iTermReflectionMethodArgument *> *)reflectedArguments {
  ...
  for (NSInteger i = 2; i < numberOfArguments; i++) {
    ...
    iTermReflectionMethodArgument *arg =
        [iTermReflectionMethodArgument argumentForTypeString:@(buffer)
