...
+ (NSString *)
  ...
  iTermVariableRecordingScope *permissiveScope =
      [[iTermVariableRecordingScope alloc]
          initWithScope:[[iTermVariableScope alloc] init]];
  permissiveScope.neverReturnNil = YES;
  iTermParsedExpression *expression =
      [[iTermExpressionParser callParser] parse:invocation
      ...
      *error = [NSError errorWithDomain:@"com.iterm2.call"
                               ...
                               userInfo:@{
                                 NSLocalizedDescriptionKey :
                                     @"Expected function call, not a literal"
                               }];
    }
    return nil;
  ...
  case iTermParsedExpressionTypeNil:
    if (error) {
      *error = [NSError errorWithDomain:@"com.iterm2.call"
                               ...
                               userInfo:@{
                                 NSLocalizedDescriptionKey :
                                     @"Expected function call, not nil"
                               }];
    }
    return nil;
  case iTermParsedExpressionTypeInterpolatedString:
    if (error) {
      *error = [NSError
          errorWithDomain:@"com.iterm2.call"
                     code:3
                 userInfo:@{
                   NSLocalizedDescriptionKey :
                       @"Expected function call, not an interpolated string"
                 }];
    }
    return nil;
  }
...
+ (instancetype)expressionParser {
  ...
  dispatch_once(&onceToken, ^{
    sCachedInstance =
        [[iTermExpressionParser alloc] initWithStart:@"expression"];
  });
...
+ (void)setEscapeReplacerInStringRecognizer:(id)stringRecogniser {
  [stringRecogniser
      setEscapeReplacer:^NSString *(NSString *str, NSUInteger *loc) {
        if (str.length > *loc) {
...
+ (id<CPTokenRecogniser>)stringRecognizerWithClass:(Class)theClass {
  CPQuotedRecogniser *stringRecogniser =
      [theClass quotedRecogniserWithStartQuote:@"\""
                                      endQuote:@"\""
                                escapeSequence:@"\\"
                                          name:@"String"];
...
- (id)initWithStart:(NSString *)start {
  ...
  if (self) {
    ...
    _parser = [CPSLRParser parserWithBNF:_grammarProcessor.backusNaurForm
                                   start:start];
    assert(_parser);
...
- (iTermFunctionArgument *)newFunctionArgumentWithName:(NSString *)name
                                            expression:(iTermParsedExpression *)
                                                           expression {
...
- (iTermParsedExpression *)parsedExpressionWithValue:(id)value
  ...
  if (errorReason) {
    return [[iTermParsedExpression alloc] initWithErrorCode:3
                                                     reason:errorReason];
  }
  ...
  if (optional) {
    ...
    fallbackError = [NSString
        stringWithFormat:@"Reference to undefined variable “%@”. Change it to "
                         @"“%@?” to treat the undefined value as null.",
                         path, path];
...
+ (iTermParsedExpression *)parsedExpressionWithInterpolatedStringParts:
  ...
  NSArray<iTermParsedExpression *> *coalesced = [interpolatedParts
      reduceWithFirstValue:@[]
                     block:^id(NSArray<iTermParsedExpression *> *arraySoFar,
                               iTermParsedExpression *expression) {
                       if (expression.expressionType ==
                               iTermParsedExpressionTypeString &&
                           arraySoFar.lastObject &&
                           arraySoFar.lastObject.expressionType ==
                               iTermParsedExpressionTypeString) {
                         NSString *concatenated = [arraySoFar.lastObject.string
                             stringByAppendingString:expression.string];
                         iTermParsedExpression *combined =
                             [[iTermParsedExpression alloc]
                                 initWithString:concatenated];
                         return
                             [[arraySoFar subarrayToIndex:arraySoFar.count - 1]
                                 arrayByAddingObject:combined];
                       }
                       return [arraySoFar arrayByAddingObject:expression];
                     }];
  return
      [[iTermParsedExpression alloc] initWithInterpolatedStringParts:coalesced];
}

- (iTermParsedExpression *)parsedExpressionWithInterpolatedString:
    (NSString *)swifty {
  return [self.class parsedExpressionWithInterpolatedString:swifty
                                                      scope:_scope];
}
...
+ (iTermParsedExpression *)
  ...
  [swifty enumerateSwiftySubstrings:^(NSUInteger index, NSString *substring,
                                      BOOL isLiteral, BOOL *stop) {
    if (isLiteral) {
      NSString *escapedString =
          [substring it_stringByExpandingBackslashEscapedCharacters];
      [interpolatedParts addObject:[[iTermParsedExpression alloc]
                                       initWithString:escapedString]];
      return;
    }
    allLiterals = NO;

    iTermExpressionParser *parser =
        [[iTermExpressionParser alloc] initWithStart:@"expression"];
    ...
    if (!strict &&
      ...
      // If the expression was a variable reference, replace it with empty
      // string. This works around the annoyance of remembering to add question
      // marks in interpolated strings, where you know the result you want is
      // always an empty string.
      iTermParsedExpression *expressionWithPlaceholders =
          [parser parse:substring
                  scope:[[iTermVariablePlaceholderScope alloc] init]];
      if ([expressionWithPlaceholders.object
              conformsToProtocol:@protocol(iTermExpressionParserPlaceholder)]) {
  ...
  if (allLiterals) {
    return [[iTermParsedExpression alloc]
        initWithString:[swifty it_stringByExpandingBackslashEscapedCharacters]];
  }
  return [self parsedExpressionWithInterpolatedStringParts:interpolatedParts];
}

- (iTermTriple<id, NSString *, NSString *> *)
    pathOrDereferencedArrayFromPath:(NSString *)path
  ...
  if (_scope.usePlaceholders) {
    id placeholder;
    if (indexNumber) {
      placeholder = [[iTermExpressionParserArrayDereferencePlaceholder alloc]
          initWithPath:path
                 index:indexNumber.integerValue];
    } else {
      placeholder = [[iTermExpressionParserVariableReferencePlaceholder alloc]
          initWithPath:path];
    }
  ...
  if (!indexNumber) {
    return [iTermTriple tripleWithObject:untypedValue
                               andObject:nil
                                  object:path];
  }
  ...
  if (!array) {
    NSString *reason = [NSString
        stringWithFormat:@"Variable “%@” is of type %@, not array", path,
                         NSStringFromClass([untypedValue class])];
  ...
  if (index < 0 || index >= array.count) {
    NSString *reason = [NSString
        stringWithFormat:@"Index %@ out of range of “%@”, which %@ values",
                         @(index), path, @(array.count)];
    return [iTermTriple tripleWithObject:nil andObject:reason object:path];
  }
...
- (void)loadRulesAndTransforms {
  ...
  [_grammarProcessor
      addProductionRule:@"call ::= <path> <arglist>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            NSError *error = nil;
            iTermParsedExpression *result = [weakSelf
                parsedExpressionForFunctionCallWithFullyQualifiedName:
                    (NSString *)syntaxTree.children[0]
                                                              arglist:
                                                                  syntaxTree
                                                                      .children
                                                                          [1]
  ...
  [_grammarProcessor
      addProductionRule:@"arg ::= 'Identifier' ':' <expression>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [weakSelf
                newFunctionArgumentWithName:[(CPIdentifierToken *)syntaxTree
                                                    .children[0] identifier]
                                 expression:syntaxTree.children[2]];
          }];
  [_grammarProcessor
      addProductionRule:@"expression ::= <path_or_dereferenced_array>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            iTermTriple *triple = syntaxTree.children[0];
            // Explicit nulls must be optional to signal the caller intended
            // them to be null.
            return [weakSelf
                parsedExpressionWithValue:triple.firstObject
                              errorReason:triple.secondObject
                                     path:triple.thirdObject
                                 optional:[triple.thirdObject
  ...
  [_grammarProcessor
      addProductionRule:@"expression ::= 'Number'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [[iTermParsedExpression alloc]
                initWithNumber:[(CPNumberToken *)syntaxTree.children[0]
                                   number]];
          }];
  [_grammarProcessor addProductionRule:@"expression ::= '[' ']'"
                         treeTransform:^id(CPSyntaxTree *syntaxTree) {
                           return [[iTermParsedExpression alloc]
                               initWithArrayOfExpressions:@[]];
                         }];
  [_grammarProcessor
      addProductionRule:@"expression ::= '[' <comma_delimited_expressions> ']'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [[iTermParsedExpression alloc]
                initWithArrayOfExpressions:syntaxTree.children[1]];
          }];
  [_grammarProcessor
  ...
  [_grammarProcessor
      addProductionRule:@"expression ::= 'SwiftyString'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            NSString *swifty =
                [(CPQuotedToken *)syntaxTree.children[0] content];
  ...
  [_grammarProcessor
      addProductionRule:
          @"path_or_dereferenced_array ::= <path>" // -> (value, error string,
                                                   // path); value of NSNull
                                                   // means undefined.
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return
                [weakSelf pathOrDereferencedArrayFromPath:syntaxTree.children[0]
                                                    index:nil];
          }];
  [_grammarProcessor
      addProductionRule:@"path_or_dereferenced_array ::= <path> '[' 'Number' "
                        @"']'" // -> (value, error string, path); value of
                               // NSNull means undefined.
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
  ...
  [_grammarProcessor
      addProductionRule:@"path ::= 'Identifier'"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
  ...
  [_grammarProcessor
      addProductionRule:@"path ::= 'Identifier' '.' <path>"
          treeTransform:^id(CPSyntaxTree *syntaxTree) {
            return [NSString
                stringWithFormat:@"%@.%@",
                                 [(CPIdentifierToken *)syntaxTree.children[0]
                                     identifier],
...
- (iTermParsedExpression *)parse:(NSString *)invocation
  ...
  NSError *error =
      [NSError errorWithDomain:@"com.iterm2.parser"
                          code:2
...
- (CPRecoveryAction *)parser:(CPParser *)parser
  ...
  NSArray *quotedExpected =
      [acceptableTokens.allObjects mapWithBlock:^id(id anObject) {
        return [NSString stringWithFormat:@"“%@”", anObject];
      }];
  NSString *expectedString = [quotedExpected componentsJoinedByString:@", "];
  NSString *reason = [NSString
      stringWithFormat:@"Syntax error at index %@ of “%@”. Expected%@: %@",
