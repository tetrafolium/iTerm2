...
- (void)handleActivationRegularly {
  ...
  if (![view
          conformsToProtocol:@protocol(iTermFocusFollowsMouseFocusReceiver)]) {
    return;
  }
...
- (void)handleActivationAggressively:(NSPoint)savedMouseLocation {
  ...
  const NSPoint mouseLocation = [NSEvent mouseLocation];
  // If focus follows mouse is on, find the window under the cursor and make it
  // key. If a id<iTermFocusFollowsMouseFocusReceiver> is under the cursor make
  // it first responder.
