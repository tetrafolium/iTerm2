...
@implementation ThreeFingerTapGestureRecognizer {
  int numTouches_;
  NSTimeInterval
      firstTouchTime_; // Time since ref date of transition from 0 to >0 touches
  NSTimeInterval
      threeTouchTime_; // Time since ref date of transition from <3 to 3 touches
  NSView *target_;     // weak reference
  SEL selector_;
  BOOL fired_; // True if we just faked a three-finger click and future mouse
...
- (void)touchesBeganWithEvent:(NSEvent *)ev {
  ...
  if (touches > 3) {
    ...
    // Not possible to be a three finger tap if more than three fingers were
    // down at any point.
    [self cancel];
  }
...
- (void)touchesEndedWithEvent:(NSEvent *)ev {
  ...
  if (numTouches_ == 0 && firstTouchTime_ && threeTouchTime_ && !_moved &&
      [NSDate timeIntervalSinceReferenceDate] - firstTouchTime_ <
          maxTimeForSimulatedThreeFingerTap) {
  ...
  DLog(@"%@ End touch. numTouches_ -> %d (first=%d, three=%d, dt=%d)", self,
       numTouches_, (int)firstTouchTime_, (int)threeTouchTime_,
       (int)([NSDate timeIntervalSinceReferenceDate] - firstTouchTime_));
}
...
- (BOOL)getAndResetFired {
  // This is called for [right]mouseUp events. It returns and resets the value
  // of fired_ so it doesn't get stuck.
  BOOL value = fired_;
