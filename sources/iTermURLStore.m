...
@implementation iTermURLStore {
  ...
  // Internally, the code is stored as a 64-bit integer so we don't have to
  // think about overflow. The value that's exported is truncated to 16 bits and
...
- (unsigned short)codeForURL:(NSURL *)url withParams:(NSString *)params {
  if (!url.absoluteString || !params) {
    DLog(@"codeForURL:%@ withParams:%@ returning 0 because of nil value",
...
- (NSURL *)urlForCode:(unsigned short)code {
  if (code == 0) {
    // Safety valve in case something goes awry. There should never be an entry
    // at 0.
    return nil;
  }
...
- (NSDictionary *)dictionaryValue {
  ...
  return @{@"store" : _store, @"refcounts" : coder.encodedData};
}
...
- (void)loadFromDictionary:(NSDictionary *)dictionary {
  ...
  [store enumerateKeysAndObjectsUsingBlock:^(NSDictionary *_Nonnull key,
                                             NSNumber *_Nonnull obj,
                                             BOOL *_Nonnull stop) {
    if (![key isKindOfClass:[NSDictionary class]] ||
        ![obj isKindOfClass:[NSNumber class]]) {
      ELog(@"Unexpected types when loading dictionary: %@ -> %@", key.class,
           obj.class);
      return;
    }
    ...
    unsigned short truncated =
        [iTermURLStore truncatedCodeForCode:obj.integerValue];
    self->_reverseStore[@(truncated)] =
        @{@"url" : url, @"params" : key[@"params"] ?: @""};
    self->_nextCode = MAX(self->_nextCode, obj.integerValue + 1);
  }];

  NSError *error = nil;
  NSKeyedUnarchiver *decoder =
      [[NSKeyedUnarchiver alloc] initForReadingFromData:refcounts error:&error];
  if (error) {
