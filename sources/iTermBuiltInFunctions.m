...
NSString *iTermFunctionSignatureFromNamespaceAndNameAndArguments(
    NSString *namespace, NSString *name, NSArray<NSString *> *argumentNames) {
  NSString *combinedArguments =
      [[argumentNames sortedArrayUsingSelector:@selector(compare:)]
          componentsJoinedByString:@","];
  NSString *tail =
      [NSString stringWithFormat:@"%@(%@)", name, combinedArguments];
  if (!namespace) {
    return tail;
  }
...
- (nullable NSError *)typeCheckParameters:
...
- (instancetype)
  ...
  if (self) {
    ...
    [defaultValues enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
...
- (NSError *)typeMismatchError:(NSString *)argument
                        wanted:(Class)wanted
                           got:(nullable id)object {
  NSString *reason =
      [NSString stringWithFormat:
                    @"Type mismatch for argument %@. Expected %@ but got %@.",
                    argument, NSStringFromClass(wanted),
...
- (NSError *)invalidArgumentError:(NSString *)argument {
  NSString *reason =
      [NSString stringWithFormat:@"Invalid argument %@ to %@", argument, _name];
  return [NSError errorWithDomain:@"com.iterm2.bif"
                             code:4
                         userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
@implementation iTermBuiltInFunctions {
  // NOTE: The keys include the namespace, but the iTermBuiltInFunction object
  // is ignorant of it.
...
+ (void)registerStandardFunctions {
  ...
  [iTermSetStatusBarComponentUnreadCountBuiltInFunction
...
- (void)registerFunction:(iTermBuiltInFunction *)function
  ...
  NSString *signature = iTermFunctionSignatureFromNamespaceAndNameAndArguments(
      namespace, function.name, function.argumentsAndTypes.allKeys);
...
- (NSString *)signatureOfAnyRegisteredFunctionWithName:(NSString *)name {
  return [_functions.allKeys
      objectPassingTest:^BOOL(NSString *signature, NSUInteger index,
                              BOOL *stop) {
...
- (NSDictionary<NSString *, NSArray<NSString *> *> *)
  ...
  NSDictionary *signatureToArgs = [_functions
      mapValuesWithBlock:^id(NSString *signature, iTermBuiltInFunction *bif) {
        return bif.argumentsAndTypes.allKeys;
      }];
  NSDictionary *nameToArgs =
      [signatureToArgs mapKeysWithBlock:^id(NSString *signature, id object) {
        return iTermFunctionNameFromSignature(signature);
      }];
  NSDictionary *publicNameToArgs =
      [nameToArgs filteredWithBlock:^BOOL(NSString *name, id object) {
...
- (void)callFunctionWithName:(NSString *)name
                  ...
                  completion:
                      (nonnull iTermBuiltInFunctionCompletionBlock)completion {
  ...
  NSMutableDictionary<NSString *, id> *amendedParameters =
  ...
  function.block([amendedParameters dictionaryByRemovingNullValues],
                 completion);
}

- (NSError *)undeclaredIdentifierError:(NSString *)identifier {
  NSString *reason =
      [NSString stringWithFormat:@"Undeclared identifier %@", identifier];
  return [NSError errorWithDomain:@"com.iterm2.bif"
                             code:1
                         userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
- (NSError *)invalidReferenceError:(NSString *)reference name:(NSString *)name {
  NSString *reason = [NSString
      stringWithFormat:@"Invalid reference “%@” to %@", reference, name];
  return [NSError errorWithDomain:@"com.iterm2.bif"
                             code:3
                         userInfo:@{NSLocalizedFailureReasonErrorKey : reason}];
}
...
- (iTermBuiltInMethod *)methodWithSignature:(NSString *)signature {
  iTermBuiltInMethod *result =
      [iTermBuiltInMethod castFrom:_functions[signature]];
  if (result) {
    return result;
  }
  for (NSString *methodSignature in _functions) {
    iTermBuiltInMethod *method =
        [iTermBuiltInMethod castFrom:_functions[methodSignature]];
    assert(method);
...
+ (void)countOfObject:(id)value
           completion:(iTermBuiltInFunctionCompletionBlock)completion {
  if (!value) {
    NSError *error = [NSError
        errorWithDomain:@"com.iterm2.array-count"
                   code:1
               userInfo:@{
                 NSLocalizedDescriptionKey : @"Array argument must be non-null"
               }];
    completion(nil, error);
    return;
  }
        ...
        errorWithDomain:@"com.iterm2.array-count"
               ...
               userInfo:@{
                 NSLocalizedDescriptionKey : [NSString
                     stringWithFormat:@"Argument must be an array (was %@)",
                                      [value class]]
               }];
    completion(nil, error);
    return;
...
+ (NSArray<iTermReflectionMethodArgument *> *)argumentsFromTarget:
                                                  (id<iTermObject>)target
                                                           action:(SEL)action {
  iTermReflection *reflection =
      [[iTermReflection alloc] initWithClass:[(NSObject *)target class]
                                    selector:action];
  return reflection.arguments;
}

+ (NSDictionary<NSString *, Class> *)argumentDictionaryFromArray:
    (NSArray<iTermReflectionMethodArgument *> *)arguments {
  __block BOOL foundCompletion = NO;
  NSMutableDictionary<NSString *, Class> *result =
  ...
  for (iTermReflectionMethodArgument *obj in arguments) {
    ...
    case iTermReflectionMethodArgumentTypeSelector:
      // Not legal for built-in methods. They must take only objects and one
      // block named completion.
      return nil;
    }
  }
  if (!foundCompletion) {
...
- (instancetype)initWithName:(NSString *)name
               defaultValues:(NSDictionary<NSString *, NSString *> *)
                                 defaultValues // arg name -> variable name
                       types:(NSDictionary<NSString *, Class> *)types
           optionalArguments:(NSSet<NSString *> *)optionalArguments
                     context:(iTermVariablesSuggestionContext)context
                      target:(id<iTermObject>)target
                      action:(SEL)action {
  NSArray<iTermReflectionMethodArgument *> *args =
      [iTermBuiltInMethod argumentsFromTarget:target action:action];
  if (!args) {
    return nil;
  }
  NSDictionary<NSString *, Class> *argDict =
      [iTermBuiltInMethod argumentDictionaryFromArray:args];
  if (!argDict) {
    return nil;
  }
  self = [super
...
- (BOOL)matchedBySignature:(NSString *)signature
               inNamespace:(NSString *)namespace {
  NSString *name = iTermFunctionNameFromSignature(signature);
  NSString *myFullyQualifiedName =
      namespace ? [NSString stringWithFormat:@"%@.%@", namespace, self.name]
                : self.name;
...
- (void)callWithArguments:(NSDictionary<NSString *, id> *)parameters
  ...
  NSMethodSignature *signature =
      [[(NSObject *)_target class] instanceMethodSignatureForSelector:_action];
  ...
  for (NSInteger i = 0; i < _args.count; i++) {
    ...
    if (requiredClass) {
      const BOOL isNull =
          [parameters[arg.argumentName] isKindOfClass:[NSNull class]];
