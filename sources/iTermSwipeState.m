...
@implementation iTermSwipeState {
  // Set up at initialization time. If you drag-release-drag-again then the
  // currentIndex gets updated in place.
  iTermSwipeHandlerParameters _parameters;

  // Usually negative. How far to scroll the big container view. Can be
  // extremely huge. Use -squashedOffset, which is a not-too-huge value computed
  // from this.
  CGFloat _rawOffset;

  // The offset where scrolling began. If you touch-up and touch-down then this
  // can get updated.
  CGFloat _initialOffset;

  // Used when in the ground state to animate toward the destination frame.
  // Gives the speed in points per 1/60 sec.
  CGFloat _momentum;

  // Used to decipher NSEvents into meaningful things.
  iTermScrollWheelStateMachineState _state;

  // Which tab we are animating towards after drag while in ground state. Nil if
  // no drag started.
  ...
  // Used to relate cancel notifications with the swipe state that's being
  // canceled.
  NSString *_identifier;

  // While in the Scroll state while _dragStarted is NO, these grow for each
  // scroll event until we have enough info to decide if this is a vertical or
  // horizontal scroll.
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p width=%@ offset=%@ (%@) momentum=%@ "
                                 @"state=%@ momentumStage=%@ targetIndex=%@>",
                                 NSStringFromClass(self.class), self,
                                 @(_parameters.width), @(self.squashedOffset),
                                 @(_rawOffset), @(_momentum), @(_state),
                                 @(self.momentumStage), _targetIndexNumber];
}
...
- (void)retire {
  ...
  _isRetired = YES;
  [self.swipeHandler
      swipeHandlerEndSession:self.userInfo
                     atIndex:_targetIndexNumber
                                 ? _targetIndexNumber.integerValue
                                 : NSNotFound];
...
- (void)update:(NSTimeInterval)elapsed {
  ...
  DLog(@"Apply force %d times given elapsed time of %0.3f", iterations,
       elapsed);
...
- (void)reallyUpdate {
  ...
  case iTermSwipeStateMomentumStageNegative:
    if (self.squashedOffset == [self offsetOfTargetIndex]) {
      // Super unlikely. This should only happen if the offset was just right
      // when the touch up occurred.
      [self retire];
      return;
    }
    if (fabs(force) > 0.1) {
...
- (void)applyForce:(CGFloat)force {
  ...
  if (([self offsetIsAfterTargetIndex:offsetBefore] &&
       ![self offsetIsAfterTargetIndex:self.squashedOffset]) ||
      (![self offsetIsAfterTargetIndex:offsetBefore] &&
       [self offsetIsAfterTargetIndex:self.squashedOffset])) {
    [self didCrossZero];
    return;
  }

  [self.swipeHandler swipeHandlerSetOffset:self.squashedOffset
                                forSession:self.userInfo];
...
- (void)didCrossZero {
  DLog(@"Zero crossing");
  [self.swipeHandler swipeHandlerSetOffset:[self offsetOfTargetIndex]
                                forSession:self.userInfo];
  [self retire];
  self.momentumStage = iTermSwipeStateMomentumStageNone;
}

- (BOOL)shouldDrag:(NSEvent *)event
             state:(iTermScrollWheelStateMachineState)state {
  if (_dragStarted) {
  ...
  // If you've moved at least 10 in the any direction and the accumulated slope
  // is under this value then we infer you meant to scroll vertically.
  const CGFloat slopeThreshold = 0.4;

  if ((_preScrollAccumulatedDeltaY + _preScrollAccumulatedDeltaX) >= 10 &&
      _preScrollAccumulatedDeltaY >
          slopeThreshold * _preScrollAccumulatedDeltaX) {
    [self retire];
  ...
  if (_preScrollAccumulatedDeltaX > 10) {
    ...
    DLog(@"Not ready to drag yet: Accumulated pre-scroll deltas are dx=%0.1f "
         @"dy=%0.1f",
         _preScrollAccumulatedDeltaX, _preScrollAccumulatedDeltaY);
  }
  return _dragStarted;
...
- (void)startDrag {
  ...
  if (!_userInfo) {
    ...
    _userInfo =
        [self.swipeHandler swipeHandlerBeginSessionAtOffset:_initialOffset
                                                 identifier:_identifier];
    if (fabs(_initialOffset - _rawOffset) >= 1) {
      [self.swipeHandler swipeHandlerSetOffset:self.squashedOffset
                                    forSession:_userInfo];
    }
    DLog(@"Set offset=%@ initialOffset=%@", @(self.squashedOffset),
         @(_initialOffset));
  } else {
    DLog(@"Resume existing session. Update raw & initial offsets");
    _parameters.currentIndex = [self indexForOffset:self.squashedOffset
                                              round:0];
...
- (void)badStateTransition:
            (iTermScrollWheelStateMachineStateTransition)transition
                     event:(NSEvent *)event {
  ITBetaAssert(NO, @"Unexpected state transition %@ -> %@: %@",
               @(transition.before), @(transition.after), event);
  [self retire];
}

- (BOOL)handleEvent:(NSEvent *)event
         transition:(iTermScrollWheelStateMachineStateTransition)transition {
  DLog(@"handleEvent:%@ before=%@ after=%@ dy=%0.1f",
       iTermShortEventPhasesString(event), @(transition.before),
...
// To move by distance d in time T, we will move by m on each frame. Then m will
// be decreased by gSwipeFriction. This function calculates the initial value of
// m. More formally: v(t) = v(t - 1) * 0.9  (for t > 0) Solve for v(0), which is
...
- (CGFloat)momentumForDistance:(CGFloat)distance {
  ...
  dispatch_once(&onceToken, ^{
    // This is only approximate because animation stops whens velocity drops
    // under 1. The actual duration could end up being quite a bit shorter than
    // the target, especially if the distance is small compared to the duration.
...
- (void)setMomentumStage:(iTermSwipeStateMomentumStage)stage {
  ...
  // Clamp it
  _targetIndexNumber =
      @(MAX(MIN(_targetIndexNumber.integerValue, _parameters.currentIndex + 1),
  ...
  _momentum =
      [self momentumForDistance:[self offsetOfTargetIndex] - _rawOffset];
  DLog(@"Set target index to %@ given offset %@, width %@, stage %@. Set "
       @"momentum to %@.",
       _targetIndexNumber, @(self.squashedOffset), @(_parameters.width),
       @(stage), @(_momentum));
}
...
- (CGFloat)squashedOffset {
  ...
  const CGFloat squashedShifted =
      iTermSquash(shifted, softMovementLimit * 2, wiggle);
  ...
  DLog(@"Squash %0.0f (initial=%0.0f) -> %0.0f (width=%0.0f)", _rawOffset,
       _initialOffset, squashed, _parameters.width);
  return squashed;
}
