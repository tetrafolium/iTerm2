...
static NSString
    *const iTermBackgroundCommandRunnerDidSelectNotificationNotificationName =
...
- (NSString *)description {
  return
      [NSString stringWithFormat:
                    @"<%@: %p command=%@ shell=%@ title=%@ path=%@ running=%@>",
                    NSStringFromClass(self.class), self, self.command,
                    self.shell, self.title, self.path, @(_running)];
}
...
- (void)reallyRun {
  ...
  assert(!_running);
  _running = YES;
  iTermCommandRunner *commandRunner = [[iTermCommandRunner alloc]
      initWithCommand:@"/bin/sh"
        withArguments:@[ @"-c", self.command ]
                 path:[[NSFileManager defaultManager] currentDirectoryPath]];
  if (self.path.length > 0) {
    NSDictionary *environment = [[NSProcessInfo processInfo] environment];
    environment = [environment dictionaryBySettingObject:self.path
                                                  forKey:@"PATH"];
...
- (void)didCompleteWithStatus:(int)status
                        entry:(iTermScriptHistoryEntry *)entry {
  DLog(@"%@", self);
  ...
  if (self.notificationTitle && status) {
    ...
    [self.class maybeNotify:^(NSInteger deferCount) {
      ...
      if (deferCount > 1) {
        detail = [detail
            stringByAppendingFormat:@", plus %@ other error%@ silenced.",
                                    @(deferCount - 1),
                                    deferCount > 2 ? @"s" : @""];
      }
