...
static NSDictionary *GIFProperties(CGImageSourceRef source, size_t i) {
  ...
  if (properties) {
    NSDictionary *gifProperties = (NSDictionary *)CFDictionaryGetValue(
...
- (instancetype)initWithJson:(NSData *)json {
  ...
  if (self) {
    ...
    if (delays.count != 0 && delays.count != imageData.count) {
      DLog(@"delays.count=%@, imageData.count=%@", @(delays.count),
           @(imageData.count));
      return nil;
    }

    _size = NSMakeSize([size[0] doubleValue], [size[1] doubleValue]);
    if (_size.width <= 0 || _size.width >= kMaxDimension || _size.height <= 0 ||
        _size.height >= kMaxDimension) {
    ...
    for (NSString *imageString in imageData) {
      ...
      if (data.length < _size.width * _size.height * 4) {
        DLog(@"data too small %@ < %@", @(data.length),
             @(_size.width * _size.height * 4));
        return nil;
      }
