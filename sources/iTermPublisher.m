...
- (instancetype)initWithWeakReferenceToObject:(id)object
                                        block:(void (^)(id))block;
@end
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p object=%@>", self.class, self, _object];
}
...
@implementation iTermPublisher {
  ...
  //
  // Quoting from the closest thing to documentation that Apple can be bothered
  // to write:
  //
  ...
  //
  // "weak-to-strong NSMapTables are not currently recommended, as the strong
  // values for weak keys which get zero'd out do not get cleared away (and
  // released) until/unless the map table resizes itself"
  //
  // While this doesn't seem to be the case in 10.13, the count is still wrong.
  // As this appears to be a dumpster fire I'll go my own way.
...
- (void)addSubscriber:(id)object block:(void (^)(id))block {
  iTermSubscriberAttachment *attachment =
      [[iTermSubscriberAttachment alloc] init];
  __weak __typeof(self) weakSelf = self;
  iTermSubscriber *subscriber =
      [[iTermSubscriber alloc] initWithWeakReferenceToObject:object
                                                       block:block];
...
- (void)removeSubscriber:(id)object {
  iTermSubscriber *subscriber =
      [_subscribers objectPassingTest:^BOOL(iTermSubscriber *element,
