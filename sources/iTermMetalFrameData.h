...
extern void
iTermMetalFrameDataStatsBundleInitialize(iTermPreciseTimerStats *bundle);
extern void iTermMetalFrameDataStatsBundleAdd(iTermPreciseTimerStats *dest,
                                              iTermPreciseTimerStats *source);
...
@property(atomic, strong) id<iTermMetalDriverDataSourcePerFrameState>
    perFrameState;
...
@property(nonatomic, strong)
    MTLRenderPassDescriptor *debugRealRenderPassDescriptor;
...
@property(nonatomic, strong)
    dispatch_group_t group; // nonnil implies synchronous
...
// When drawing to an intermediate texture there may be two passes (i.e., two
// render encoders)
...
// When using subpixel AA, all draw stages prior to Text write to this
// descriptor.
@property(nonatomic, strong)
    MTLRenderPassDescriptor *intermediateRenderPassDescriptor;

// When using subpixel AA, the intermediate rpd's texture is copied to the
// temporary rpd's texture and then text is rendered to this rpd while sampling
// from the intermediate rpd's texture. Eventually this gets copied to the
// drawable.
@property(nonatomic, strong)
    MTLRenderPassDescriptor *temporaryRenderPassDescriptor;
