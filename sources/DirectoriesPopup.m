...
- (void)loadDirectoriesForHost:(VT100RemoteHost *)host {
  ...
  for (iTermRecentDirectoryMO *entry in
       [[iTermShellHistoryController sharedInstance]
           directoriesSortedByScoreOnHost:host]) {
    DirectoriesPopupEntry *popupEntry =
        [[[DirectoriesPopupEntry alloc] init] autorelease];
...
- (id)tableView:(NSTableView *)aTableView
  ...
  DirectoriesPopupEntry *entry =
      [[self model] objectAtIndex:[self convertIndex:rowIndex]];
  if ([[aTableColumn identifier] isEqualToString:@"date"]) {
    // Date
    return [NSDateFormatter
        dateDifferenceStringFromDate:[NSDate
                                         dateWithTimeIntervalSinceReferenceDate:
                                             entry.entry.lastUse.doubleValue]];
  } else {
    // Contents
...
- (void)rowSelected:(id)sender {
  if ([_tableView selectedRow] >= 0) {
    DirectoriesPopupEntry *entry = [[self model]
...
- (NSAttributedString *)
  ...
  NSIndexSet *indexes = [[iTermShellHistoryController sharedInstance]
      abbreviationSafeIndexesInRecentDirectory:entry.entry];
