...
static NSString *const iTermURLActionFactoryCancelPathfinders =
    @"iTermURLActionFactoryCancelPathfinders";
...
- (void)tryExistingFileForceRespectingHardNewlines:(BOOL)forceRespect {
  ...
  iTermLocatedString *locatedPrefix = [self.extractor
      wrappedLocatedStringAt:self.coord
                     forward:NO
         respectHardNewlines:forceRespect || self.respectHardNewlines
                    maxChars:[iTermAdvancedSettingsModel
  ...
  iTermLocatedString *locatedSuffix = [self.extractor
      wrappedLocatedStringAt:self.coord
                     forward:YES
         respectHardNewlines:forceRespect || self.respectHardNewlines
                    maxChars:[iTermAdvancedSettingsModel
...
- (void)trySecureCopy {
  // TODO: We usually don't get here because "foo.txt" looks enough like a URL
  // that we do a DNS lookup and fail. It'd be nice to fallback to an SCP file
  // path. See if we can conjure up a secure copy path.
...
- (URLAction *)urlActionForHypertextLink {
  ...
  if (url != nil) {
    ...
    action.hover = YES;
    // file: URLs with a fragment go through semantic history and therefore need
    // a workingDirectory.
    action.workingDirectory = self.workingDirectory;
    action.range = [extractor
        rangeOfCoordinatesAround:self.coord
                 maximumDistance:1000
                     passingTest:^BOOL(screen_char_t *c, VT100GridCoord coord) {
                       ...
                       NSString *thisId;
                       NSURL *thisURL =
                           [extractor urlOfHypertextLinkAt:coord urlId:&thisId];
...
- (void)urlActionForExistingFileWithPrefix:(iTermLocatedString *)locatedPrefix
                                ...
                                completion:
                                    (void (^)(URLAction *,
                                              BOOL workingDirectoryIsLocal))
                                        completion {
  NSString *possibleFilePart1 = [locatedPrefix.string
      substringIncludingOffset:[locatedPrefix.string length] - 1
  ...
  NSMutableCharacterSet *suffixCharacterSet =
      [[NSCharacterSet filenameCharacterSet] mutableCopy];
  [suffixCharacterSet
  ...
  // Because path finders cache their results, this is not a disaster. Since the
  // inputs tend to be the same, whatever work was already done can be exploited
  // this time around.
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermURLActionFactoryCancelPathfinders
                    object:nil];

  _pathfinder = [self.semanticHistoryController
      pathOfExistingFileFoundWithPrefix:possibleFilePart1
                                 suffix:possibleFilePart2
                       workingDirectory:self.workingDirectory
                         trimWhitespace:NO
                             completion:^(NSString *filename, int prefixChars,
                                          int suffixChars,
                                          BOOL workingDirectoryIsLocal) {
                               DLog(@"Semantic history controller returned "
                                    @"filename %@ with %@ prefix and %@ suffix "
                                    @"chars",
                                    filename, @(prefixChars), @(suffixChars));
                               URLAction *action =
                                   [self urlActionForFilename:filename
...
- (URLAction *)urlActionForFilename:(NSString *)filename
  ...
  // Don't consider / to be a valid filename because it's useless and
  // single/double slashes are pretty common.
  ...
  if (locatedPrefix.coords.count > 0 && prefixChars > 0) {
    ...
    // Everything is coming from the suffix (e.g., when mouse is on first char
    // of filename)
    range.coordRange.start = [locatedSuffix.coords[0] gridCoordValue];
  }
  VT100GridCoord lastCoord;
  // Ensure we don't run off the end of suffixCoords if something unexpected
  // happens. Subtract 1 because the 0th index into suffixCoords corresponds to
  // 1 suffix char being used, etc.
  ...
  action.fullPath = [self.semanticHistoryController
      cleanedUpPathFromPath:filename
                     suffix:[locatedSuffix.string
                                substringFromIndex:suffixChars]
           workingDirectory:self.workingDirectory
...
- (URLAction *)urlActionForSmartSelection {
  ...
  SmartMatch *smartMatch =
      [self.extractor smartSelectionAt:self.coord
  ...
  if (actions.count) {
    ...
    URLAction *action =
        [URLAction urlActionToPerformSmartSelectionRule:smartMatch.rule
                                               onString:content];
    action.range = smartRange;
    ContextMenuActions value =
        [ContextMenuActionPrefsController actionForActionDict:actions[0]];
    action.selector = NSSelectorFromString(self.selectors[@(value)]);
    action.representedObject = [ContextMenuActionPrefsController
        parameterForActionDict:actions[0]
...
- (URLAction *)urlActionForURLLike {
  NSString *joined = [self.locatedPrefix.string
      stringByAppendingString:self.locatedSuffix.string];
  DLog(@"Smart selection found nothing. Look for URL-like things in %@ around "
       @"offset %d",
       joined, (int)[self.locatedPrefix.string length]);
  int prefixChars = 0;
  NSString *possibleUrl =
      [joined substringIncludingOffset:[self.locatedPrefix.string length]
                      fromCharacterSet:[NSCharacterSet urlCharacterSet]
                  charsTakenFromPrefix:&prefixChars];
  DLog(@"String of just permissible chars is <<%@>> with prefix length %d",
       possibleUrl, prefixChars);
  ...
  NSString *stringWithoutNearbyPunctuation =
      [possibleUrl substringWithRange:rangeWithoutNearbyPunctuation];
  DLog(@"String without nearby punctuation: %@",
       stringWithoutNearbyPunctuation);
  ...
  if ([iTermAdvancedSettingsModel conservativeURLGuessing]) {
    ...
    NSString *hostnameRegex =
        @"(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\\-]*[a-zA-Z0-9])\\.)+([A-Za-z0-9]"
        @"|[A-Za-z0-9][A-Za-z0-9\\-]*[A-Za-z0-9])";
    NSString *portRegex = @"(:[1-9][0-9]{0,4})?";
    NSString *pathRegex = @"/";
    NSString *urlRegex =
        [NSString stringWithFormat:@"%@%@%@%@", schemeRegex, hostnameRegex,
                                   portRegex, pathRegex];
    if ([stringWithoutNearbyPunctuation rangeOfRegex:urlRegex].location !=
        NSNotFound) {
      DLog(@"LGTM, using %@ with range %@ and prefix %d",
           stringWithoutNearbyPunctuation,
           NSStringFromRange(rangeWithoutNearbyPunctuation), prefixChars);
  ...
  const BOOL hasColon =
      ([stringWithoutNearbyPunctuation rangeOfString:@":"].location !=
       NSNotFound);
  BOOL looksLikeURL;
  if (hasColon) {
    DLog(@"Has a colon, looks like a URL to me");
    // The test later on for whether an app exists to open the URL is
    // sufficient.
    DLog(@"Contains a colon so it looks like a URL to me");
    looksLikeURL = YES;
  } else {
    // Only try to use HTTP if the string has something especially HTTP URL-like
    // about it, such as containing a slash. This helps reduce the number of
    // random strings that are misinterpreted as URLs.
    looksLikeURL = [self
        stringLooksLikeURL:
            [possibleUrl substringWithRange:rangeWithoutNearbyPunctuation]];

    if (looksLikeURL) {
      if (!self.workingDirectoryIsLocal &&
          ![stringWithoutNearbyPunctuation containsString:@"/"]) {
        DLog(@"The working directory is not local and there's no slash in the "
             @"filename or colon for a scheme, so this might be a file on a "
             @"remote filesystem. Don't treat it as a URL.");
        return nil;
      }
      DLog(@"There's no colon but it seems like it could be an HTTP URL. Let's "
           @"give that a try.");
      NSString *defaultScheme;
      if ([self stringIsSingleDomainWord:
                    [self hostnameInSchemelessPossibleURL:
                              stringWithoutNearbyPunctuation]]) {
        DLog(@"Use http because it's a single word");
        defaultScheme = @"http:";
      } else {
        defaultScheme = [[iTermAdvancedSettingsModel defaultURLScheme]
            stringByAppendingString:@":"];
        DLog(@"Use default scheme of %@", defaultScheme);
      }
      stringWithoutNearbyPunctuation = [defaultScheme
          stringByAppendingString:stringWithoutNearbyPunctuation];
    } else {
  ...
  if (looksLikeURL) {
    DLog(@"Looks like a URL. Return %@ with range %@ and prefix %d",
         stringWithoutNearbyPunctuation,
         NSStringFromRange(rangeWithoutNearbyPunctuation), prefixChars);
    // If the string contains non-ascii characters, percent escape them. URLs
...
- (BOOL)stringIsSingleDomainWord:(NSString *)string {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    NSCharacterSet *lowercase =
        [NSCharacterSet characterSetWithRange:NSMakeRange('a', 26)];
    NSCharacterSet *uppercase =
        [NSCharacterSet characterSetWithRange:NSMakeRange('A', 26)];
    NSCharacterSet *digits =
        [NSCharacterSet characterSetWithRange:NSMakeRange('0', 10)];
...
- (URLAction *)urlActionForString:(NSString *)string
                      ...
                      prefixChars:(int)prefixChars {
  DLog(@"urlActionForString:%@ range:%@ prefixChars:%@", string,
       NSStringFromRange(stringRange), @(prefixChars));
  ...
  BOOL openable =
      (url &&
       ...
       prefixChars >= 0 && prefixChars <= self.locatedPrefix.string.length);

  if (openable) {
    DLog(@"%@ is openable", url);
    VT100GridWindowedRange range;
    NSInteger j = self.locatedPrefix.string.length - prefixChars;
    DLog(@"j=%@-%@=%@", @(self.locatedPrefix.string.length), @(prefixChars),
         @(j));
    if (j < self.locatedPrefix.coords.count) {
      DLog(@"j=%@ < self.locatedPrefix.coords.count=%@", @(j),
           @(self.locatedPrefix.coords.count));
      range.coordRange.start = [self.locatedPrefix.coords[j] gridCoordValue];
      DLog(@"range.coordRange.start=%@",
      ...
      range.coordRange.start = [self.extractor
          successorOfCoord:[self.locatedPrefix.coords[j - 1] gridCoordValue]];
      DLog(@"range.coordRange.start=%@ which is successor of last prefix coord "
           @"%@",
           VT100GridCoordDescription(range.coordRange.start),
           VT100GridCoordDescription(
               [self.locatedPrefix.coords[j - 1] gridCoordValue]));
    } else {
      DLog(@"prefixCoordscount=%@ j=%@", @(self.locatedPrefix.coords.count),
           @(j));
      return nil;
    }
    ...
    if (i < self.locatedSuffix.coords.count) {
      ...
      DLog(@"range.coordRange.end=%@",
           VT100GridCoordDescription(
               [self.locatedSuffix.coords[i] gridCoordValue]));
    } else if (i > 0 && i == self.locatedSuffix.coords.count) {
      DLog(@"i == suffixCoords.count");
      range.coordRange.end = [self.extractor
          successorOfCoord:[self.locatedSuffix.coords[i - 1] gridCoordValue]];
      DLog(@"range.coordRange.end=%@, successor of %@",
           VT100GridCoordDescription(range.coordRange.end),
           VT100GridCoordDescription(
               [self.locatedSuffix.coords[i - 1] gridCoordValue]));
    } else {
      DLog(@"i=%@ suffixcoords.count=%@", @(i),
           @(self.locatedSuffix.coords.count));
      return nil;
    }
    range.columnWindow = self.extractor.logicalWindow;
    ...
    DLog(@"%@ is not openable (couldn't convert it to a URL [%@] or no scheme "
         @"handler",
         string, url);
  }
  return nil;
...
- (URLAction *)urlActionWithSecureCopy {
  ...
  if (smartMatch) {
    ...
    SCPPath *scpPath =
        self.pathFactory([smartMatch.components firstObject], self.coord.y);
    if (scpPath) {
...
- (BOOL)stringLooksLikeURL:(NSString *)s {
  ...
  // [NSURL URLWithString] is supposed to do this, but it doesn't accept
  // IDN-encoded domains like http://例子.测试 Just about any word can be a URL
  // in the local search path. The code that calls this prefers false positives,
  // so just make sure it's not empty and doesn't have illegal characters.
  if ([s rangeOfCharacterFromSet:[[NSCharacterSet urlCharacterSet] invertedSet]]
          .location != NSNotFound) {
    return NO;
  }
  if ([s length] == 0) {
  ...
  NSString *ipRegex = @"^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\.){"
