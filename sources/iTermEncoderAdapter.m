...
- (BOOL)encodeDictionaryWithKey:(NSString *)key
                     generation:(NSInteger)generation
                          block:(BOOL (^NS_NOESCAPE)(
                                    id<iTermEncoderAdapter> encoder))block {
  return [_encoder
      encodeChildWithKey:key
              identifier:@""
              generation:generation
                   block:^BOOL(iTermGraphEncoder *_Nonnull subencoder) {
                     return block([[iTermGraphEncoderAdapter alloc]
                         initWithGraphEncoder:subencoder]);
                   }];
}
...
+ (instancetype)encoder {
  return
      [[self alloc] initWithMutableDictionary:[NSMutableDictionary dictionary]];
}

- (instancetype)initWithMutableDictionary:
...
- (BOOL)encodeDictionaryWithKey:(NSString *)key
  ...
  if (!block([[iTermMutableDictionaryEncoderAdapter alloc]
...
- (void)
    encodeArrayWithKey:(NSString *)key
           identifiers:(NSArray<NSString *> *)identifiers
            generation:(NSInteger)generation
                 block:(BOOL (^NS_NOESCAPE)(id<iTermEncoderAdapter> _Nonnull,
                                            NSInteger index, NSString *_Nonnull,
                                            BOOL *stop))block {
  [self encodeArrayWithKey:key
               identifiers:identifiers
                generation:generation
                ...
                generation:(NSInteger)generation
                   options:(iTermGraphEncoderArrayOptions)options
                     block:(BOOL (^NS_NOESCAPE)(id<iTermEncoderAdapter> encoder,
                                                ...
                                                NSString *identifier,
                                                BOOL *stop))block {
  NSArray *array = [identifiers
      mapEnumeratedWithBlock:^id(NSUInteger index, NSString *identifier,
                                 BOOL *stop) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        if (!block([[iTermMutableDictionaryEncoderAdapter alloc]
                       initWithMutableDictionary:dict],
