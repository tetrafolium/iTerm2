...
// It's nice for each of these to be unique so in degenerate cases (e.g., empty
// query) the detail uses the same feature for all items.
...
- (id<iTermOpenQuicklyCommand>)commandForQuery:(NSString *)queryString {
  ...
  id<iTermOpenQuicklyCommand> theCommand =
      [[iTermOpenQuicklyNoCommand alloc] init];
  ...
  for (Class commandClass in self.commands) {
    ...
    item.detail = [_delegate
        openQuicklyModelDisplayStringForFeatureNamed:nil
                                  ...
                                  highlightedIndexes:nil];
    item.identifier =
        [NSString stringWithFormat:@"/%@ ", [commandClass command]];
...
- (NSString * (^)(PTYSession *))detailFunctionForSessions:
  ...
  NSArray<NSString * (^)(PTYSession *)> *functions =
      @[ pwd, command, hostname, badge ];
  NSMutableArray<NSMutableArray *> *functionValues = [NSMutableArray array];
  [functions
      enumerateObjectsUsingBlock:^(NSString * (^_Nonnull obj)(PTYSession *),
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        [functionValues addObject:[NSMutableArray array]];
      }];
  [sessions enumerateObjectsUsingBlock:^(PTYSession *_Nonnull session,
                                         NSUInteger sessionIndex,
                                         BOOL *_Nonnull stop) {
    [functions enumerateObjectsUsingBlock:^(
                   NSString * (^_Nonnull f)(PTYSession *),
  ...
  [functionValues
      enumerateObjectsUsingBlock:^(NSMutableArray *_Nonnull values,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
  ...
  NSMutableArray<NSString * (^)(PTYSession *)> *filteredFunctions =
      [NSMutableArray array];
  NSMutableArray<NSMutableArray *> *filteredFunctionValues =
      [NSMutableArray array];
  [indexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
    [filteredFunctions addObject:functions[idx]];
    [filteredFunctionValues addObject:functionValues[idx]];
  }];
  double (^variance)(NSArray<NSString *> *) =
      ^double(NSArray<NSString *> *strings) {
        NSSet<NSString *> *set = [NSSet setWithArray:strings];
        return set.count;
      };
  const NSUInteger best = [filteredFunctionValues
      indexOfMaxWithBlock:^NSComparisonResult(
          NSMutableArray<NSString *> *obj1, NSMutableArray<NSString *> *obj2) {
...
- (void)addSessionLocationToItems:
            (NSMutableArray<iTermOpenQuicklyItem *> *)items
                      withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
  NSString * (^detailFunction)(PTYSession *) =
      [self detailFunctionForSessions:self.sessions];

  for (PTYSession *session in self.sessions) {
    NSMutableArray *features = [NSMutableArray array];
    iTermOpenQuicklySessionItem *item =
        [[iTermOpenQuicklySessionItem alloc] init];
    ...
    if (item.score > 0) {
      ...
      if (detailFunction) {
        item.detail = [self.delegate
            openQuicklyModelAttributedStringForDetail:detailFunction(session)];
      }
      [items addObject:item];
    }
  }
...
- (void)addCreateNewTabToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
  ...
  for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
    iTermOpenQuicklyProfileItem *newSessionWithProfileItem =
        [[iTermOpenQuicklyProfileItem alloc] init];
    NSMutableAttributedString *attributedName =
        [[NSMutableAttributedString alloc] init];
    newSessionWithProfileItem.score = [self scoreForProfile:profile
                                                    matcher:matcher
                                             attributedName:attributedName];
    if (newSessionWithProfileItem.score > 0) {
      ...
      newSessionWithProfileItem.detail =
...
- (void)addChangeColorPresetToItems:
            (NSMutableArray<iTermOpenQuicklyItem *> *)items
                        withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
  ...
  for (NSString *name in allPresets) {
    ...
    iTermOpenQuicklyColorPresetItem *item =
        [[iTermOpenQuicklyColorPresetItem alloc] init];
    ...
    item.logoGenerator.textColor = [preset[KEY_FOREGROUND_COLOR] colorValue]
                                       ?: [NSColor colorWithRed:0.75
                                                          green:0.75
                                                           blue:0.75
                                                          alpha:1];
    item.logoGenerator.backgroundColor =
        [preset[KEY_BACKGROUND_COLOR] colorValue]
            ?: [NSColor colorWithRed:0.25 green:0.25 blue:0.25 alpha:1];
    item.logoGenerator.tabColor =
        [preset[KEY_TAB_COLOR] colorValue] ?: defaultColor;
    item.logoGenerator.cursorColor =
        [preset[KEY_CURSOR_COLOR] colorValue] ?: defaultColor;

    NSMutableAttributedString *attributedName =
        [[NSMutableAttributedString alloc] init];
    item.score = [self scoreForColorPreset:name
                                   matcher:matcher
                            attributedName:attributedName];
    if (item.score > 0) {
      NSString *value =
          [NSString stringWithFormat:@"Load color preset ”%@“", name];
      item.detail =
          [_delegate openQuicklyModelDisplayStringForFeatureNamed:nil
                                                            value:value
                                               highlightedIndexes:nil];
...
- (void)addChangeProfileToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
                    withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
  for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
    iTermOpenQuicklyChangeProfileItem *changeProfileItem =
        [[iTermOpenQuicklyChangeProfileItem alloc] init];
    NSMutableAttributedString *attributedName =
        [[NSMutableAttributedString alloc] init];
    changeProfileItem.score = [self scoreForProfile:profile
                                            matcher:matcher
                                     attributedName:attributedName];
    if (changeProfileItem.score > 0) {
      changeProfileItem.detail = [_delegate
          openQuicklyModelDisplayStringForFeatureNamed:nil
...
- (void)addActionsToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
  ...
  [[[iTermActionsModel sharedInstance] actions]
      enumerateObjectsUsingBlock:^(iTermAction *_Nonnull action, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
        iTermOpenQuicklyActionItem *actionItem =
            [self actionItemForAction:action matcher:matcher];
        if (actionItem) {
...
- (iTermOpenQuicklyActionItem *)
    actionItemForAction:(iTermAction *)action
                matcher:(iTermMinimumSubsequenceMatcher *)matcher {
  iTermOpenQuicklyActionItem *actionItem =
      [[iTermOpenQuicklyActionItem alloc] init];
  actionItem.action = action;
  NSMutableAttributedString *attributedName =
      [[NSMutableAttributedString alloc] init];
  actionItem.score = [self scoreForAction:action
                                  matcher:matcher
                           attributedName:attributedName];
  if (actionItem.score <= 0) {
    return nil;
  }
  actionItem.detail = [_delegate
      openQuicklyModelDisplayStringForFeatureNamed:nil
...
- (void)addSnippetsToItems:(NSMutableArray<iTermOpenQuicklyItem *> *)items
  ...
  [[[iTermSnippetsModel sharedInstance] snippets]
      enumerateObjectsUsingBlock:^(iTermSnippet *_Nonnull snippet,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        iTermOpenQuicklySnippetItem *snippetItem =
            [self snippetItemForSnippet:snippet matcher:matcher];
        if (snippetItem) {
...
- (iTermOpenQuicklySnippetItem *)
    snippetItemForSnippet:(iTermSnippet *)snippet
                  matcher:(iTermMinimumSubsequenceMatcher *)matcher {
  iTermOpenQuicklySnippetItem *snippetItem =
      [[iTermOpenQuicklySnippetItem alloc] init];
  snippetItem.snippet = snippet;
  NSMutableAttributedString *attributedName =
      [[NSMutableAttributedString alloc] init];
  snippetItem.score = [self scoreForSnippet:snippet
                                    matcher:matcher
                             attributedName:attributedName];
...
- (iTermOpenQuicklyArrangementItem *)
    arrangementItemWithName:(NSString *)arrangementName
                    matcher:(iTermMinimumSubsequenceMatcher *)matcher
                     inTabs:(BOOL)inTabs {
  iTermOpenQuicklyArrangementItem *item =
      [[iTermOpenQuicklyArrangementItem alloc] init];
  NSMutableAttributedString *attributedName =
      [[NSMutableAttributedString alloc] init];
...
- (iTermOpenQuicklyScriptItem *)
    scriptItemWithName:(NSString *)scriptName
               matcher:(iTermMinimumSubsequenceMatcher *)matcher {
...
- (void)addOpenArrangementToItems:
            (NSMutableArray<iTermOpenQuicklyItem *> *)items
                      withMatcher:(iTermMinimumSubsequenceMatcher *)matcher {
  for (NSString *arrangementName in [WindowArrangements allNames]) {
    iTermOpenQuicklyArrangementItem *item;
    item = [self arrangementItemWithName:arrangementName
                                 matcher:matcher
                                  inTabs:NO];
    if (item) {
      [items addObject:item];
...
- (void)updateWithQuery:(NSString *)queryString {
  id<iTermOpenQuicklyCommand> command =
      [self commandForQuery:[queryString lowercaseString]];
  ...
  BOOL haveCurrentWindow =
      [[iTermController sharedInstance] currentTerminal] != nil;
  if ([command supportsCreateNewTab]) {
    [self addCreateNewTabToItems:items
                     withMatcher:matcher
               haveCurrentWindow:haveCurrentWindow];
  }
  ...
  if (items.count > kMaxItems) {
    [items
        removeObjectsInRange:NSMakeRange(kMaxItems, items.count - kMaxItems)];
  }
...
- (double)scoreForArrangementWithName:(NSString *)arrangementName
  ...
  double score =
      [self scoreUsingMatcher:matcher
  ...
  if (score > 0 && [[WindowArrangements defaultArrangementName]
                       isEqualToString:arrangementName]) {
    // Make the default arrangement always be the highest-scored arrangement if
...
- (double)scoreForSnippet:(iTermSnippet *)snippet
  ...
  double score =
      [self scoreUsingMatcher:matcher
                    documents:@[
                      snippet.title ?: @"", [snippet trimmedValue:80] ?: @""
                    ]
...
- (double)scoreForProfile:(Profile *)profile
  ...
  if (score > 0 && [[[ProfileModel sharedInstance] defaultBookmark][KEY_GUID]
                       isEqualToString:profile[KEY_GUID]]) {
    // Make the default profile always be the highest-scored profile if it
    // matches the query.
    score += 0.2;
  }
  if (nameFeature.count) {
...
- (double)scoreForSession:(PTYSession *)session
  ...
  NSDictionary<NSString *, NSString *> *userVariablesDict = [[session.variables
      discouragedValueForVariableName:@"user"] stringValuedDictionary];
...
- (NSAttributedString *)detailForSession:(PTYSession *)session
                                features:(NSArray *)features {
  NSArray *sorted = [features sortedArrayUsingComparator:^NSComparisonResult(
                                  NSArray *tuple1, NSArray *tuple2) {
...
- (double)scoreUsingMatcher:(iTermMinimumSubsequenceMatcher *)matcher
  ...
  if (matcher.query.length == 0) {
    ...
    for (NSString *document in documents) {
      if (features) {
        id displayString = [_delegate
            openQuicklyModelDisplayStringForFeatureNamed:name
                                                   value:document
  ...
  if (bestFeature && features) {
    id displayString =
        [_delegate openQuicklyModelDisplayStringForFeatureNamed:name
                                                          value:bestFeature
...
//   0 < score < 0.5 if query is a subsequence of a document. Each gap of
...
- (double)qualityOfMatchWithMatcher:(iTermMinimumSubsequenceMatcher *)matcher
  ...
  if (!indexSet.count) {
    ...
    score = 0;
             ...
             indexSet.lastIndex == documentString.length - 1) {
    // Exact equality
    score = 1;
  } else if (indexSet.firstIndex == 0) {
    // Is a prefix
