...
- (void)awakeFromNib {
  ...
  _textFieldInsets.top =
      (_box.frame.size.height - self.textField.frame.size.height) / 2 - 1;
...
- (void)sizeToFit {
  ...
  const CGFloat descriptionWidth =
      self.hideDetail ? 0 : _description.frame.size.width;
  const NSSize textFieldSize =
      NSMakeSize(MAX(textFieldFrame.size.width, descriptionWidth),
                 textFieldFrame.size.height);

  const CGFloat totalWidth =
      textFieldSize.width + _textFieldInsets.left + _textFieldInsets.right;

  const CGFloat boxMinY = self.hideDetail ? 0 : _boxMinY;
  self.view.frame = [self.view
      retinaRoundRect:NSMakeRect(self.view.frame.origin.x,
                                 self.view.frame.origin.y, totalWidth,
                                 boxMinY + _box.frame.size.height)];

  const CGFloat boxWidth = textFieldFrame.size.width + _textFieldInsets.left +
                           _textFieldInsets.right;
  _box.frame = [self.view
      retinaRoundRect:NSMakeRect(round((totalWidth - boxWidth) / 2), boxMinY,
                                 boxWidth,
                                 textFieldSize.height + _textFieldInsets.top +
                                     _textFieldInsets.bottom)];
  self.textField.frame = [self.view
      retinaRoundRect:NSMakeRect(_textFieldInsets.left, _textFieldInsets.bottom,
                                 textFieldSize.width, textFieldSize.height)];
  if (!self.hideDetail) {
    _description.frame = [self.view
        retinaRoundRect:NSMakeRect(_textFieldInsets.left, _descriptionMinY,
                                   textFieldSize.width,
