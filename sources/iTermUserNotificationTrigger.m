...
// I foolishly renamed GrowlTrigger to iTermUserNotificationTrigger in 3.2.1,
// which broke everyone's triggers. It got renamed back in 3.2.2. If someone
// created a new trigger in 3.2.1 it would have the bogus name. Then the 3.3.0
// betas didn't cherrypick the fix, so more problems. I'm done with this so
// let's just keep both around forever.
...
- (void)postNotificationWithText:(NSString *)notificationText
  ...
  iTermNotificationController *notificationController =
      [iTermNotificationController sharedInstance];
  [notificationController
               notify:notificationText
      withDescription:
          [NSString
              stringWithFormat:@"A trigger fired in session \"%@\" in tab #%d.",
                               [aSession name], aSession.delegate.tabNumber]
