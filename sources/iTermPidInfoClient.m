...
- (void)localGetPidInfoForProcessID:(int)pid
                         ...
                         completion:
                             (void (^)(int rc, NSData *buffer))completion {
  ...
  dispatch_async(_localQueue, ^{
    const int rc =
        proc_pidinfo(pid, flavor, arg,
                     (bufferSize > 0) ? result.mutableBytes : NULL, bufferSize);
...
- (void)asyncGetInfoForProcess:(int)pid
  ...
  if (!self.ready) {
    DLog(@"Not ready");
    [self localGetPidInfoForProcessID:pid
                               flavor:flavor
                                  arg:arg
...
- (void)getMaximumNumberOfFileDescriptorsForProcess:(pid_t)pid
  ...
  [self asyncGetInfoForProcess:pid
                    ...
                    completion:^(int rc, NSData *_Nonnull buffer) {
                      DLog(@"get task info for pid %@ finished with rc %@",
                           @(pid), @(rc));
                      ...
                      memmove(&tai, buffer.bytes,
                              MIN(buffer.length, sizeof(tai)));
...
- (void)getFileDescriptorsForProcess:(pid_t)pid
                          ...
                          completion:
                              (void (^)(int count,
                                        struct proc_fdinfo *fds))completion {
  [self
      getMaximumNumberOfFileDescriptorsForProcess:pid
                                       completion:^(size_t count) {
                                         DLog(@"getMaximumNumberOfFileDescripto"
                                              @"rsForProcess finished for %@ "
                                              @"with %@",
                                              @(pid), @(count));
                                         if (count == 0) {
                                           dispatch_async(queue, ^{
                                         ...
                                         [self
                                             getFileDescriptorsForProcess:pid
                                                                    count:count
                                                                    queue:queue
                                                               completion:
                                                                   completion];
...
- (void)getNumberOfFilePortsInProcess:(pid_t)pid
  ...
  [self asyncGetInfoForProcess:pid
                    ...
                    buffersize:0
                         reqid:[self nextReqid]
                    completion:^(int rc, NSData *_Nonnull buffer) {
                      DLog(@"Get number of file ports for pid %@ finished with "
                           @"rc %@",
                           @(pid), @(rc));
                      if (rc <= 0) {
                        dispatch_async(queue, ^{
...
- (void)getPortsInProcess:(pid_t)pid
  ...
  [self
      getNumberOfFilePortsInProcess:pid
                              queue:queue
                         completion:^(int count) {
                           DLog(@"getNumberOfFilePortsInProcess:%@ finished "
                                @"with count=%@",
                                @(pid), @(count));
                           const int size =
                               count * sizeof(struct proc_fileportinfo);
                           if (size <= 0) {
                             dispatch_async(queue, ^{
                               completion(0, NULL);
                             });
                               ...
                               asyncGetInfoForProcess:pid
                                           ...
                                           completion:^(
                                             ...
                                             struct proc_fileportinfo
                                                 *filePortInfoArray =
                                                     iTermMalloc(size);
                                             memmove(filePortInfoArray,
                                                     buffer.bytes,
                                                     MIN(size, buffer.length));
                                             dispatch_async(queue, ^{
                                               completion(count,
                                                          filePortInfoArray);
                                             });
...
- (void)getWorkingDirectoryOfProcessWithID:(pid_t)pid
  ...
  [self asyncGetInfoForProcess:pid
                    ...
                    completion:^(int ret, NSData *_Nonnull buffer) {
                      DLog(@"Get path info for pid %@ finished with rc %@",
                           @(pid), @(ret));
                      if (ret <= 0) {
                        // An error occurred
                      ...
                      memmove(&vpi, buffer.bytes,
                              MIN(sizeof(vpi), buffer.length));
                      // All is good
                      NSString *rawDir =
                          [NSString stringWithUTF8String:vpi.pvi_cdir.vip_path];
                      dispatch_async(queue, ^{
                        completion(rawDir);
                      });
                    }];
