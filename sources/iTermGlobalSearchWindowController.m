...
@interface iTermGlobalSearchWindowController () <
    iTermFocusReportingSearchFieldDelegate, NSOutlineViewDelegate,
    NSOutlineViewDataSource, NSWindowDelegate>
@end

@implementation iTermGlobalSearchWindowController {
  iTermGlobalSearchEngine *_engine;
  NSMutableDictionary<NSString *, NSMutableArray<iTermGlobalSearchResult *> *>
      *_results;
...
- (NSArray<PTYSession *> *)sessions {
  return [[[iTermController sharedInstance] terminals]
      flatMapWithBlock:^NSArray *(PseudoTerminal *terminal) {
...
- (void)setFraction:(double)progress {
  iTermSearchFieldCell *cell =
      [iTermSearchFieldCell castFrom:_searchField.cell];
...
- (void)addResults:(NSArray<iTermGlobalSearchResult *> *)results
  ...
  NSMutableArray<iTermGlobalSearchResult *> *sessionResults =
      _results[session.guid];
  ...
  NSIndexSet *indexes =
      [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(sessionResults.count,
                                                         results.count)];
  [sessionResults addObjectsFromArray:results];
  if (isNew) {
    const NSUInteger i =
        [self.sortedNonEmptyResultSessionGUIDs indexOfObject:session.guid];
    if (i != NSNotFound) {
...
- (NSRect)rectForCoordRange:(VT100GridCoordRange)coordRange
  ...
  NSRect (^screenRect)(VT100GridCoord) = ^NSRect(VT100GridCoord coord) {
    ...
    const NSRect windowRect = [session.textview convertRect:viewRect
                                                     toView:nil];
...
- (void)movePanelAwayFromResultInRange:(VT100GridCoordRange)coordRange
                               session:(PTYSession *)session {
  ...
  // Move it above, below, left, or right. Pick the option where the least
  // amount of the window falls off the screen.
  NSRect candidates[] = {
      ...
      NSMakeRect(NSMinX(windowRect), // up
                 NSMinY(hull) - NSHeight(windowRect), NSWidth(windowRect),
                 NSHeight(windowRect)),
  };
  ...
  for (int i = 0; i < sizeof(candidates) / sizeof(*candidates); i++) {
    ...
    const CGFloat areaOffScreen =
        totalArea - (onScreenRect.size.width * onScreenRect.size.height);
    const CGFloat distance =
        EuclideanDistance(_panel.frame.origin, candidates[i].origin);
...
- (NSInteger)focusReportingSearchFieldNumberOfResults:
  ...
  [_results enumerateKeysAndObjectsUsingBlock:^(
                NSString *_Nonnull key,
                NSMutableArray<iTermGlobalSearchResult *> *_Nonnull obj,
                BOOL *_Nonnull stop) {
    count += obj.count;
  }];
  return count;
...
- (nullable NSView *)outlineView:(NSOutlineView *)outlineView
  ...
  if (guid) {
    ...
    view.textField.stringValue =
        [[[iTermController sharedInstance] sessionWithGUID:guid] name]
            ?: @"Session";
    return view;
  }
...
- (NSDictionary<NSString *, NSMutableArray<iTermGlobalSearchResult *> *> *)
    nonEmptyResults {
  return [_results
      filteredWithBlock:^BOOL(
          NSString *key, NSMutableArray<iTermGlobalSearchResult *> *value) {
...
- (NSArray<NSString *> *)sortedNonEmptyResultSessionGUIDs {
  return [self.nonEmptyResults.allKeys
      sortedArrayUsingSelector:@selector(compare:)];
}
// An item is either nil (the root), a string (a session), or a
// iTermGlobalSearchResult object.
