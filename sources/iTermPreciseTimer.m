...
void iTermPreciseTimerLog(NSString *identifier, iTermPreciseTimerStats stats[],
                          size_t count, BOOL logToConsole,
                          NSArray *histograms) {
  ...
  NSString * (^formatMillis)(double) = ^NSString *(double ms) {
    ...
    return [[@" " stringRepeatedTimes:millisWidth - numeric.length]
        stringByAppendingString:numeric];
  };
  @synchronized([iTermPreciseTimersLock class]) {
    NSMutableString *log =
    ...
    [log
        appendFormat:@"%-20s%@    %@µ      N  %@p50  %@p75  %@p95 [min  "
                     @"distribution  max]\n",
                     "Statistic",
                     [@"    "
                         stringRepeatedTimes:maxlevel], // corresponds to dead
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (histograms) {
        ...
        [log appendFormat:@"%@%@ %-20s%@ %@  %5d  %@  %@  %@ [%@]\n",
                          ...
                          formatMillis(mean), (int)[histograms[i] count],
                          formatMillis([histograms[i] valueAtNTile:0.5]),
                          formatMillis(p75),
                          formatMillis([histograms[i] valueAtNTile:0.95]),
                          [histograms[i] sparklineGraphWithPrecision:2
                                                          multiplier:1
                                                               units:@"ms"]];
      } else {
        NSTimeInterval stddev =
            iTermPreciseTimerStatsGetStddev(&stats[i]) * 1000.0;
        [log
            appendFormat:@"%@ %20s: µ=%0.3fms σ=%.03fms (95%% CI ≅ "
                         @"%0.3fms–%0.3fms) 𝚺=%.2fms N=%@ avg. events=%01.f\n",
                         iTermEmojiForDuration(mean), stats[i].name, mean,
                         stddev, MAX(0, mean - stddev), mean + stddev,
                         stats[i].n * mean, @(stats[i].n),
...
void iTermPreciseTimerLogOneEvent(NSString *identifier,
  ...
  @synchronized([iTermPreciseTimersLock class]) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      [name appendString:[[[NSString alloc] initWithBytes:cname
                                                   length:length
      ...
      if (stats[i].n > 1) {
        ...
        if (histograms) {
          other = [NSString
              stringWithFormat:@"N=%@ µ=%0.1fms p50=%@ p95=%@ | %@", @(count),
                               mean * 1000, @([histograms[i] valueAtNTile:0.5]),
                               @([histograms[i] valueAtNTile:0.95]),
                               [histograms[i] sparklines]];
        } else {
          other = [NSString
              stringWithFormat:@"N=%@ µ=%0.1fms [%0.1fms…%0.1fms]", @(count),
                               mean * 1000, stats[i].min * 1000,
                               stats[i].max * 1000];
        }
      }
...
NSString *iTermPreciseTimerGetSavedLogs(void) {
  @synchronized([iTermPreciseTimersLock class]) {
    ...
    [sLogs enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                               BOOL *_Nonnull stop) {
