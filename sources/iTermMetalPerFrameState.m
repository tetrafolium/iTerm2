...
typedef struct {
  ...
  unsigned int
      inUnderlinedRange : 1; // This is the underline for semantic history
...
static vector_float4 VectorForColor(NSColor *color) {
  return (vector_float4){color.redComponent, color.greenComponent,
...
@interface iTermMetalPerFrameState () {
  ...
  NSDictionary<NSNumber *, NSIndexSet *>
      *_rowToAnnotationRanges; // Row on screen to characters with annotation
                               // underline on that row.
...
- (void)loadAllWithTextView:(PTYTextView *)textView
  ...
  [_configuration loadSettingsWithDrawingHelper:drawingHelper
                                       textView:textView
                                           glue:glue];
  [self loadSettingsWithDrawingHelper:drawingHelper textView:textView];
  [self loadMetricsWithDrawingHelper:drawingHelper
                            textView:textView
                              screen:screen];
  [self loadLinesWithDrawingHelper:drawingHelper
                          textView:textView
                            screen:screen];
...
- (void)loadMetricsWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
  ...
  _visibleRange.end.y =
      _visibleRange.start.y + _configuration->_gridSize.height;
  const long long totalScrollbackOverflow = [screen totalScrollbackOverflow];
  _firstVisibleAbsoluteLineNumber =
      _visibleRange.start.y + totalScrollbackOverflow;
  _lastVisibleAbsoluteLineNumber =
      _visibleRange.end.y + totalScrollbackOverflow;
...
- (void)loadLinesWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
  ...
  iTermMetalPerFrameStateRowFactory *factory =
      [[iTermMetalPerFrameStateRowFactory alloc]
          initWithDrawingHelper:drawingHelper
                       textView:textView
                         screen:screen
    ...
    _badgeDestinationRect = [iTermTextDrawingHelper
        rectForBadgeImageOfSize:_badgeImage.size
           ...
           destinationFrameSize:textView.frame.size
                    visibleSize:textView.enclosingScrollView.documentVisibleRect
                                    .size
                  sourceRectPtr:&_badgeSourceRect
                        margins:NSEdgeInsetsMake(
                                    drawingHelper.badgeTopMargin, 0, 0,
                                    drawingHelper.badgeRightMargin)];
  }
}

- (void)loadBlinkingCursorWithTextView:(PTYTextView *)textView
                                  glue:(id<iTermMetalPerFrameStateDelegate>)
                                           glue {
  VT100GridCoord cursorScreenCoord = VT100GridCoordMake(
      textView.dataSource.cursorX - 1, textView.dataSource.cursorY - 1);
...
- (void)loadCursorInfoWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
  ...
  _cursorInfo.copyModeCursorCoord = VT100GridCoordMake(
      drawingHelper.copyModeCursorCoord.x,
  ...
  _cursorInfo.coord =
      VT100GridCoordMake(textView.dataSource.cursorX - 1,
                         textView.dataSource.cursorY - 1 - offset);
  NSInteger lineWithCursor =
      textView.dataSource.cursorY - 1 + _numberOfScrollbackLines;
  if ([self shouldDrawCursor] && _cursorVisible &&
    ...
    _cursorInfo.cursorColor = [self backgroundColorForCursor];
    {
      ...
      const screen_char_t *const line =
          (const screen_char_t *const)lineData.bytes;
      ...
      if (screenChar.code) {
        if (screenChar.code == DWC_RIGHT) {
          ...
          _cursorInfo.doubleWidth =
              (column < _configuration->_gridSize.width - 1) &&
              (line[column + 1].code == DWC_RIGHT);
        }
      } else {
        _cursorInfo.doubleWidth = NO;
      }
      ...
      if (_cursorInfo.type == CURSOR_BOX) {
        ...
        const BOOL focused = ((_configuration->_isInKeyWindow &&
...
- (void)loadMarkedTextWithDrawingHelper:
    (iTermTextDrawingHelper *)drawingHelper {
  if ([self hasMarkedText]) {
    VT100GridCoord startCoord = drawingHelper.cursorCoord;
    startCoord.y +=
        drawingHelper.numberOfScrollbackLines - _visibleRange.start.y;
...
- (iTermCharacterSourceDescriptor *)
    characterSourceDescriptorForASCIIWithGlyphSize:(CGSize)glyphSize
                                       asciiOffset:(CGSize)asciiOffset {
  return [iTermCharacterSourceDescriptor
      characterSourceDescriptorWithAsciiFont:_configuration->_asciiFont
                                nonAsciiFont:_configuration->_nonAsciiFont
                                 asciiOffset:asciiOffset
                                   glyphSize:glyphSize
                                    cellSize:_configuration->_cellSize
                      cellSizeWithoutSpacing:_configuration
                                                 ->_cellSizeWithoutSpacing
                         ...
                         nonAsciiAntiAliased:_configuration->
                                             _nonasciiAntialias];
}
...
- (void)loadAnnotationRangesFromTextView:(PTYTextView *)textView {
  NSRange rangeOfRows = NSMakeRange(
      _visibleRange.start.y, _visibleRange.end.y - _visibleRange.start.y + 1);
  NSArray<NSNumber *> *rows = [NSArray sequenceWithRange:rangeOfRows];
  _rowToAnnotationRanges = [rows
      reduceWithFirstValue:[NSMutableDictionary dictionary]
                     block:^id(NSMutableDictionary *dict, NSNumber *second) {
                       NSArray<NSValue *> *ranges = [textView.dataSource
                           charactersWithNotesOnLine:second.intValue];
                       if (ranges.count) {
                         NSMutableIndexSet *indexes =
                             [NSMutableIndexSet indexSet];
                         [ranges enumerateObjectsUsingBlock:^(
                                     NSValue *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
                           VT100GridRange gridRange = [obj gridRangeValue];
                           [indexes
                               addIndexesInRange:NSMakeRange(gridRange.location,
                                                             gridRange.length)];
                         }];
                         dict[@(second.intValue -
                                self->_visibleRange.start.y)] = indexes;
                       }
                       return dict;
                     }];
}
...
- (void)loadIndicatorsFromTextView:(PTYTextView *)textView {
  ...
  NSRect frame = NSMakeRect(0, vmargin, textView.visibleRect.size.width,
                            textView.visibleRect.size.height);
  [textView.indicatorsHelper
      enumerateTopRightIndicatorsInFrame:frame
                                 andDraw:NO
                                   block:^(NSString *identifier, NSImage *image,
                                           NSRect rect) {
                                     rect.origin.y =
                                         frame.size.height - NSMaxY(rect);
                                     iTermIndicatorDescriptor *indicator =
                                         [[iTermIndicatorDescriptor alloc]
                                             init];
  ...
  [textView.indicatorsHelper
      enumerateCenterIndicatorsInFrame:frame
                                 block:^(NSString *identifier, NSImage *image,
                                         NSRect rect, CGFloat alpha) {
                                   rect.origin.y =
                                       frame.size.height - NSMaxY(rect);
                                   iTermIndicatorDescriptor *indicator =
...
- (void)copyMarkedText:(NSString *)str
  ...
  StringToScreenChars(str, buf, fg, bg, &len, ambiguousIsDoubleWidth,
                      &cursorIndex, NULL, normalization, unicodeVersion);
  ...
  // If the screen contents are getting moved up to make room for a multi-row
  // IME line, clear out the lines at the bottom it adds.
  ...
  for (int i = 0; i < len; i++) {
    if (coord.y >= 0 && coord.y < _rows.count) {
      ...
      if (i + 1 < len && coord.x == gridWidth - 1 &&
          buf[i + 1].code == DWC_RIGHT && !buf[i + 1].complexChar) {
...
- (void)enumerateIndicatorsInFrame:(NSRect)frame
                             block:
                                 (void (^)(iTermIndicatorDescriptor *_Nonnull))
                                     block {
  [_indicators
      enumerateObjectsUsingBlock:^(iTermIndicatorDescriptor *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        block(obj);
      }];
}

- (void)metalEnumerateHighlightedRows:(void (^)(vector_float3, NSTimeInterval,
                                                int))block {
  for (iTermHighlightedRow *row in _highlightedRows) {
    long long line = row.absoluteLineNumber;
    if (line >= _firstVisibleAbsoluteLineNumber &&
        line <= _lastVisibleAbsoluteLineNumber) {
      vector_float3 color;
      if (row.success) {
      ...
      block(color, _startTime - row.creationDate,
            line - _firstVisibleAbsoluteLineNumber);
    }
  }
}
...
- (BOOL)cursorGuideEnabled {
  return _configuration->_cursorGuideColor &&
         _configuration->_cursorGuideEnabled;
}
...
- (vector_float4)processedDefaultBackgroundColor {
  ...
  if (iTermTextIsMonochrome()) {
    if (_backgroundImage) {
      ...
      alpha =
          iTermAlphaValueForTopView(1 - _configuration->_transparencyAlpha, 0);
    }
  } else {
    // Can assume transparencyAlpha is 1
    if (@available(macOS 10.14, *)) {
      alpha =
          iTermAlphaValueForTopView(0, _configuration->_backgroundImageBlend);
    } else {
      alpha = _backgroundImage ? 1 - _configuration->_backgroundColorAlpha : 1;
    }
  }
  return simd_make_float4(
      (float)_configuration->_processedDefaultBackgroundColor.redComponent,
...
// Private queue
- (NSImage *)metalBackgroundImageGetMode:
    (nullable iTermBackgroundImageMode *)mode {
  if (mode) {
...
- (void)metalGetGlyphKeys:(iTermMetalGlyphKey *)glyphKeys
  ...
  NSCharacterSet *boxCharacterSet = [iTermBoxDrawingBezierCurveFactory
      boxDrawingCharactersWithBezierPathsIncludingPowerline:
          _configuration->_useNativePowerlineGlyphs];
  ...
  const BOOL underlineHyperlinks =
      [iTermAdvancedSettingsModel underlineHyperlinks];
  ...
  *markStylePtr = [_rows[row]->_markStyle intValue];
  ...
  for (int x = 0; x < width; x++) {
    ...
    if (lastSelected && line[x].code == DWC_RIGHT && !line[x].complexChar) {
      // If the left half of a DWC was selected, extend the selection to the
      // right half.
      lastSelected = selected;
      selected = YES;
    } else if (!lastSelected && selected && line[x].code == DWC_RIGHT &&
               !line[x].complexChar) {
      // If the right half of a DWC is selected but the left half is not,
      // un-select the right half.
      lastSelected = YES;
      selected = NO;
    } else {
      // Normal code path
    ...
    const BOOL inUnderlinedRange =
        NSLocationInRange(x, underlinedRange) || annotated;

    // Background colors
    iTermBackgroundColorKey backgroundKey = {.bgColor = line[x].backgroundColor,
                                             .bgGreen = line[x].bgGreen,
                                             .bgBlue = line[x].bgBlue,
                                             .bgColorMode =
                                                 line[x].backgroundColorMode,
                                             .selected = selected,
                                             .isMatch = findMatch,
                                             .image = line[x].image};
    ...
    if (x > 0 && backgroundKey.bgColor == lastBackgroundKey.bgColor &&
      ...
      unprocessedBackgroundColor =
          [self unprocessedColorForBackgroundColorKey:&backgroundKey];
      lastUnprocessedBackgroundColor = unprocessedBackgroundColor;
      // The unprocessed color is needed for minimum contrast computation for
      // text color.
      backgroundColor = [_configuration->_colorMap
          fastProcessedBackgroundColorForBackgroundColor:
              unprocessedBackgroundColor];
    ...
    const BOOL characterIsDrawable = iTermTextDrawingHelperIsCharacterDrawable(
        &line[x], x > 0 ? &line[x - 1] : NULL,
        line[x].complexChar && (ScreenCharToStr(&line[x]) != nil),
        _configuration->_blinkingItemsVisible, _configuration->_blinkAllowed,
        NO /* preferSpeedToFullLigatureSupport */);
    const BOOL isBoxDrawingCharacter =
        (characterIsDrawable && !line[x].complexChar && line[x].code > 127 &&
    ...
    if (x > 0 && currentColorKey->isMatch == previousColorKey->isMatch &&
        currentColorKey->inUnderlinedRange ==
            previousColorKey->inUnderlinedRange &&
      ...
      vector_float4 textColor =
          [self textColorForCharacter:&line[x]
                                 line:row
    ...
    if (annotated) {
      ...
      const BOOL curly = line[x].underline &&
                         line[x].underlineStyle == VT100UnderlineStyleCurly;
      if (line[x].urlCode) {
    ...
    if (line[x].strikethrough) {
      // This right here is why strikethrough and underline is mutually
      // exclusive
      attributes[x].underlineStyle |=
          iTermMetalGlyphAttributesUnderlineStrikethroughFlag;
    }
    ...
    if (line[x].image) {
    ...
    } else if (attributes[x].underlineStyle !=
      ...
      if (x < width && line[x + 1].complexChar &&
          !(!line[x].complexChar && line[x].code < 128) &&
          ComplexCharCodeIsSpacingCombiningMark(line[x + 1].code)) {
        // Next character is a combining spacing mark that will join with this
        // non-ascii character.
  ...
  *drawableGlyphsPtr = lastDrawableGlyph + 1;
  ...
  if (row == _cursorInfo.coord.y && _cursorInfo.type == CURSOR_BOX &&
      _cursorInfo.cursorVisible && !_cursorInfo.frameOnly) {
    ...
    if (_cursorInfo.shouldDrawText) {
      ...
      cursorTextColor = VectorForColor(
          [_configuration->_colorMap colorForKey:kColorMapBackground]);
    } else {
...
- (BOOL)useThinStrokesWithAttributes:(iTermMetalGlyphAttributes *)attributes {
  ...
  const float backgroundBrightness =
      SIMDPerceivedBrightness(attributes->backgroundColor);
  const float foregroundBrightness =
      SIMDPerceivedBrightness(attributes->foregroundColor);
...
- (vector_float4)selectionColorForCurrentFocus {
  if (_configuration->_isFrontTextView) {
    return VectorForColor([_configuration->_colorMap
        processedBackgroundColorForBackgroundColor:
            [_configuration->_colorMap colorForKey:kColorMapSelection]]);
  } else {
    return _configuration->_unfocusedSelectionColor;
  }
}

- (vector_float4)unprocessedColorForBackgroundColorKey:
    (iTermBackgroundColorKey *)colorKey {
  ...
  if (colorKey->selected) {
    ...
    iTermBackgroundColorKey temp = {.bgColor = ALTSEM_DEFAULT,
                                    .bgGreen = 0,
                                    .bgBlue = 0,
    ...
    const BOOL defaultBackground =
        (colorKey->bgColor == ALTSEM_DEFAULT &&
...
- (vector_float4)colorForCode:(int)theIndex
  ...
  if (isBackground) {
    ...
    vector_float4 color =
        VectorForColor([_configuration->_colorMap colorForKey:key]);
    if (isFaint) {
      color.w = 0.5;
    }
    return color;
...
- (iTermColorMapKey)colorMapKeyForCode:(int)theIndex
  ...
  case ColorModeAlternate:
    ...
    case ALTSEM_SYSTEM_MESSAGE:
      return [_configuration->_colorMap
          keyForSystemMessageForBackground:isBackground];
...
- (id)metalASCIICreationIdentifierWithOffset:(CGSize)asciiOffset {
  return @{
    @"font" : _configuration->_asciiFont.font ?: [NSNull null],
    @"boldFont" : _configuration->_asciiFont.boldVersion ?: [NSNull null],
    @"boldItalicFont" : _configuration->_asciiFont.boldItalicVersion
        ?: [NSNull null],
...
- (nullable NSDictionary<NSNumber *, iTermCharacterBitmap *> *)
    metalImagesForGlyphKey:(iTermMetalGlyphKey *)glyphKey
               asciiOffset:(CGSize)asciiOffset
                      size:(CGSize)size
                     scale:(CGFloat)scale
  ...
  iTermCharacterSourceDescriptor *descriptor = [iTermCharacterSourceDescriptor
      characterSourceDescriptorWithAsciiFont:_configuration->_asciiFont
                                nonAsciiFont:_configuration->_nonAsciiFont
                                 asciiOffset:asciiOffset
                                   glyphSize:size
                      ...
                      cellSizeWithoutSpacing:_configuration
                         ...
                         nonAsciiAntiAliased:_configuration->
                                             _nonasciiAntialias];
  iTermCharacterSourceAttributes *attributes = [iTermCharacterSourceAttributes
      characterSourceAttributesWithThinStrokes:glyphKey->thinStrokes
                                          bold:bold
                                        italic:italic];
  ...
  if (glyphKey->combiningSuccessor) {
    if (ComplexCharCodeIsSpacingCombiningMark(glyphKey->combiningSuccessor) &&
        !(glyphKey->isComplex &&
          ComplexCharCodeIsSpacingCombiningMark(glyphKey->code))) {
      // Append the successor cell's spacing combining mark, provided it has a
      // predecessor.
      NSString *successorString = CharToStr(glyphKey->combiningSuccessor, YES);
      string = [string stringByAppendingString:successorString];
    }
  }
  iTermCharacterSource *characterSource = [[iTermCharacterSource alloc]
             initWithCharacter:string
  ...
  [characterSource.parts
      enumerateObjectsUsingBlock:^(NSNumber *_Nonnull partNumber,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)
    metalGetUnderlineDescriptorsForASCII:
        (out iTermMetalUnderlineDescriptor *)ascii
                                nonASCII:(out iTermMetalUnderlineDescriptor *)
                                             nonAscii
                           strikethrough:(out iTermMetalUnderlineDescriptor *)
                                             strikethrough {
...
- (void)setTextColor:(vector_float4)textColor
  ...
  if (coord.x < 0 || coord.y < 0 || coord.x >= gridSize.width ||
...
- (void)setDebugString:(NSString *)debugString {
  ...
  for (int i = 0, o = MAX(0, _configuration->_gridSize.width -
...
- (vector_float4)textColorForCharacter:(const screen_char_t *const)c
  ...
  const BOOL needsProcessing =
      (colorMap.minimumContrast > 0.001 || colorMap.dimmingAmount > 0.001 ||
  ...
  if (findMatch) {
    ...
    // Blue link text.
    rawColor =
        VectorForColor([_configuration->_colorMap colorForKey:kColorMapLink]);
    caches->havePreviousCharacterAttributes = NO;
  } else if (selected) {
    // Selected text.
    ...
    caches->havePreviousCharacterAttributes = NO;
             ...
             c->foregroundColor !=
                 caches->previousCharacterAttributes.foregroundColor ||
             ...
             c->foregroundColorMode !=
                 caches->previousCharacterAttributes.foregroundColorMode ||
  ...
  if (needsProcessing) {
    result = VectorForColor([_configuration->_colorMap
        processedTextColorForTextColor:ColorForVector(rawColor)
                   overBackgroundColor:ColorForVector(
                                           unprocessedBackgroundColor)
...
- (NSColor *)backgroundColorForCursor {
  NSColor *color;
  if (_configuration->_reverseVideo) {
    color = [[_configuration->_colorMap colorForKey:kColorMapCursorText]
        colorWithAlphaComponent:1.0];
  } else {
    color = [[_configuration->_colorMap colorForKey:kColorMapCursor]
        colorWithAlphaComponent:1.0];
  }
  ...
  return [iTermSmartCursorColor
      neighborsForCursorAtCoord:_cursorInfo.coord
                     ...
                     lineSource:^const screen_char_t *(int y) {
                       const int i = y + self->_numberOfScrollbackLines -
                                     self->_visibleRange.start.y;
                       if (i >= 0 && i < self->_rows.count) {
                         return (const screen_char_t *)self->_rows[i]
                             ->_screenCharLine.bytes;
                       } else {
                         return nil;
                       }
                     }];
...
- (NSColor *)cursorColorForCharacter:(screen_char_t)screenChar
                      wantBackground:(BOOL)wantBackgroundColor
                               muted:(BOOL)muted {
  vector_float4 v = [self fastCursorColorForCharacter:screenChar
                                       wantBackground:wantBackgroundColor
                                                muted:muted];
  return [NSColor colorWithRed:v.x green:v.y blue:v.z alpha:v.w];
}
...
- (BOOL)shouldDrawCursor {
  ...
  BOOL result =
      (![self hasMarkedText] && _cursorVisible && !hideCursorBecauseBlinking);
  DLog(@"shouldDrawCursor: hasMarkedText=%d, cursorVisible=%d, "
       @"hideCursorBecauseBlinking=%d, result=%@",
       (int)[self hasMarkedText], (int)_cursorVisible,
       (int)hideCursorBecauseBlinking, @(result));
  return result;
}

- (BOOL)hideCursorBecauseBlinking {
  if (_cursorBlinking && _configuration->_isInKeyWindow &&
      _configuration->_textViewIsActiveSession && _timeSinceCursorMoved > 0.5) {
