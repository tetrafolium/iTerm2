...
- (void)scheduleSelectionScroll {
  ...
  _scrolling = YES;
  [self performSelector:@selector(updateSelectionScroll)
             withObject:nil
             afterDelay:_prevScrollDelay];
}

// Scroll the screen up or down a line for a selection drag scroll.
- (void)updateSelectionScroll {
  double actualDelay =
      [[NSDate date] timeIntervalSince1970] - _lastSelectionScroll;
  ...
  case kiTermSelectionScrollDirectionUp:
    ...
    // Allow the origin to go as far as y=-VMARGIN so the top border is shown
    // when the first line is on screen.
    if (visibleRect.origin.y >=
  ...
  case kiTermSelectionScrollDirectionDown:
    ...
    if (visibleRect.origin.y + visibleRect.size.height >
        _delegate.frame.size.height) {
      visibleRect.origin.y =
          _delegate.frame.size.height - visibleRect.size.height;
    }
    [_delegate scrollRectToVisible:visibleRect];
    y = (visibleRect.origin.y + visibleRect.size.height - [_delegate excess]) /
        lineHeight;
    break;
  }
...
- (void)mouseDraggedTo:(NSPoint)locationInTextView coord:(VT100GridCoord)coord {
  ...
  if (_selectionScrollDirection &&
      previousDirection == kiTermSelectionScrollDirectionStopped) {
