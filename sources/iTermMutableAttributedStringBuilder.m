...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p %@>", NSStringFromClass([self class]), self,
                       [[[NSString alloc] initWithCharacters:[self characters]
                                                      length:[self length]]
                           autorelease]];
}
...
- (void)addAttribute:(NSString *)name value:(id)value {
  self.attributes = [self.attributes dictionaryBySettingObject:value
                                                        forKey:name];
}
...
- (void)build {
  if (_canUseFastPath && !_attributedString &&
      _characterData.length / sizeof(unichar) && !_string.length) {
    // Create a cheap attributed string from character array
    iTermCheapAttributedString *cheap =
        [[iTermCheapAttributedString alloc] init];
    cheap.characterData = _characterData;
    cheap.attributes = _attributes;
    _attributedString = cheap;
    return;
  } else if ([_attributedString
                 isKindOfClass:[iTermCheapAttributedString class]]) {
    ...
    [self appendRealAttributedStringWithText:
              [NSString stringWithCharacters:cheap.characters
                                      length:cheap.length]
...
- (void)appendRealAttributedStringWithText:(NSString *)string
...
- (void)appendString:(NSString *)string {
  // Require a string length of 1 to avoid using zippy for combining marks,
  // which core graphics renders poorly. Zippy still has value for using core
  // graphics for nonascii uncombined characters.
  BOOL tryZippy = _zippy;
  if (tryZippy && ![iTermAdvancedSettingsModel lowFiCombiningMarks] &&
      string.length > 1) {
    tryZippy = NO;
  }
  if (tryZippy) {
...
- (void)flushCharacters {
  ...
  [_string
      appendString:[NSString stringWithCharacters:_characterData.mutableBytes
                                           length:_characterData.length /
                                                  sizeof(unichar)]];
  _canUseFastPath = NO;
  [_characterData release];
  _characterData = nil;
...
- (void)appendCharacter:(unichar)code {
  if (!_zippy) {
    _canUseFastPath &=
        iTermCharacterSupportsFastPath(code, _asciiLigaturesAvailable);
  }
  if (!_characterData) {
...
- (NSInteger)length {
  return _string.length + _attributedString.length +
         _characterData.length / sizeof(unichar);
}
