...
- (NSComparisonResult)compare:(iTermRecentDirectoryMO *)other {
  ...
  if ((int)log2(self.useCount.integerValue) >
      (int)log2(other.useCount.integerValue)) {
    return NSOrderedAscending;
  } else if ((int)log2(self.useCount.integerValue) <
             (int)log2(other.useCount.integerValue)) {
    return NSOrderedDescending;
  }
  ...
  // Fall back on small differences of use count. Ordinarily lastUse will always
  // differ so we won't get here, but this makes unit tests easier to write
  // since the order will be predictable.
  return [other.useCount compare:self.useCount];
}

- (NSAttributedString *)
    attributedStringForTableColumn:(NSTableColumn *)aTableColumn
  ...
  NSMutableArray *components =
      [iTermDirectoryTree attributedComponentsInPath:attributedString];
  ...
  NSMutableAttributedString *abbreviatedName =
      [[[NSMutableAttributedString alloc] init] autorelease];
  ...
  // Abbreviate each allowed component until it fits. The last component can't
  // be abbreviated.
  CGFloat maxWidth = aTableColumn.width;
  for (int i = 0;
       i + 1 < components.count && [abbreviatedName size].width > maxWidth;
       i++) {
    if ([abbreviationSafeIndexes containsIndex:i]) {
      components[i] =
          [components[i] attributedSubstringFromRange:NSMakeRange(0, 1)];
    }
    [abbreviatedName
        deleteCharactersInRange:NSMakeRange(0, abbreviatedName.length)];
    [abbreviatedName iterm_appendString:prefix withAttributes:attributes];
    attributedPath = [components
        attributedComponentsJoinedByAttributedString:attributedSlash];
    [abbreviatedName appendAttributedString:attributedPath];
  }
...
- (NSAttributedString *)
  ...
  NSAttributedString *theString = [[[NSAttributedString alloc]
      initWithString:self.path ?: @""] autorelease];
