...
- (void)optionalComponentDownloadPhaseDidComplete:
    (iTermOptionalComponentDownloadPhase *)sender;
- (void)optionalComponentDownloadPhase:
            (iTermOptionalComponentDownloadPhase *)sender
...
@property(nonatomic, weak) id<iTermOptionalComponentDownloadPhaseDelegate>
    delegate;
...
- (instancetype)initWithURL:(NSURL *)url
           ...
           nextPhaseFactory:
               (iTermOptionalComponentDownloadPhase * (^)(
                   iTermOptionalComponentDownloadPhase *))nextPhaseFactory {
  self = [super init];
  if (self) {
    _url = [url copy];
    _title = [title copy];
...
- (void)URLSession:(NSURLSession *)session
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self.downloading) {
      // Was not canceled
      [self.delegate
          optionalComponentDownloadPhase:self
                      didProgressToBytes:totalBytesWritten
                                 ofTotal:downloadTask
...
- (void)URLSession:(NSURLSession *)session
  ...
  if ([error.domain isEqualToString:NSURLErrorDomain] &&
    ...
    // There seems to be a bug in Big Sur. There's a spurious "The network
    // connection was lost" error. I get it every once in a while, and a user
    // reported it in issue 9236. This attempts to work around it. I made a
    // wireshark trace and I can see that the client sends a FIN long before the
    // download is complete. Turning on CFNETWORK_DIAGNOSTICS doesn't help; at
    // level 1 we get "failed strict content length check" which suggests the
    // connection was closed before the whole content was recevied. At level 2
    // and higher it is very slow and the bug doesn't reproduce. This is an
    // attempt to route around the damage. I was able to reproduce the problem
...
- (instancetype)initWithURL:(NSURL *)url
  ...
  self = [super initWithURL:url
           ...
           nextPhaseFactory:nextPhaseFactory];
  if (self) {
...
- (iTermDownloadableComponentInfo *)infoGivenExistingFullComponent:
    (NSNumber *)existingFullComponent {
...
- (BOOL)iTermVersionAtLeast:(NSString *)minVersion
  ...
  if ([version isEqual:@"unknown"] || [version containsString:@".git."]) {
    // Assume it's the top of master because there's no ordering on git commit
    // numbers
    return YES;
  }
...
- (NSDictionary *)parsedManifestFromInputStream:(NSInputStream *)stream {
  id obj = [NSJSONSerialization JSONObjectWithStream:stream
                                             options:0
                                               error:nil];
  ...
  const NSUInteger requestedPartCount =
      [[self.requestedPythonVersion componentsSeparatedByString:@"."] count];
  int bestVersion = -1;
  ...
  for (id element in array) {
    ...
    if (self.requestedPythonVersion) {
      ...
      if (requestedPartCount == 3) {
        ...
        NSArray<NSString *> *twoPartPythonVersions = [pythonVersions
            mapWithBlock:^id(NSString *possiblyThreePartVersion) {
              NSArray<NSString *> *parts =
                  [possiblyThreePartVersion componentsSeparatedByString:@"."];
              if (parts.count != 3) {
                return nil;
              }
              return [[parts subarrayToIndex:2] componentsJoinedByString:@"."];
            }];
        if (![twoPartPythonVersions
                containsObject:self.requestedPythonVersion]) {
          continue;
        }
      }
    }
    if (dict[@"url"] && dict[@"signature"] && dict[@"version"]) {
      ...
      if (version > bestVersion) {
        ...
        if ([self iTermVersionAtLeast:minimumTermVersion
                               atMost:maximumTermVersion]) {
          bestVersion = version;
          bestDict = dict;
        }
      }
  ...
  if (!error) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (version < iTermMinimumPythonEnvironmentVersion) {
        innerError = [NSError errorWithDomain:@"com.iterm2"
                                         code:3
                                     userInfo:@{
                                       NSLocalizedDescriptionKey :
                                           @"☹️ No usable version found."
                                     }];
      } else if (dict) {
        self->_fullComponent = [[iTermDownloadableComponentInfo alloc]
                   initWithURL:[NSURL URLWithString:dict[@"url"]]
                          size:dict[@"size"] ? [dict[@"size"] integerValue]
                                             : 30 * 1000 * 1000
                     signature:dict[@"signature"]
            isSitePackagesOnly:NO];
        self->_sitePackagesComponent = [[iTermDownloadableComponentInfo alloc]
                   initWithURL:[NSURL URLWithString:dict[@"site-packages-url"]]
                          size:[dict[@"site-packages-size"] integerValue]
                     signature:dict[@"site-packages-signature"]
            isSitePackagesOnly:YES];
        const NSInteger fullMin =
            [dict[@"site-packages-full-min"] integerValue];
        const NSInteger fullMax =
            [dict[@"site-packages-full-max"] integerValue];
        if (fullMin && fullMax && fullMax >= fullMin) {
          self->_sitePackagesDependencies =
              NSMakeRange(fullMin, fullMax - fullMin + 1);
        } else {
        ...
        innerError = [NSError
            errorWithDomain:@"com.iterm2"
                       code:2
                   userInfo:@{
                     NSLocalizedDescriptionKey : @"☹️ Malformed manifest."
                   }];
      }
     ...
     requestedPythonVersion:(NSString *)requestedPythonVersion
           ...
           nextPhaseFactory:
               (iTermOptionalComponentDownloadPhase * (^)(
                   iTermOptionalComponentDownloadPhase *))nextPhaseFactory {
  self = [super initWithURL:url
                      title:@"Downloading Python runtime…"
           nextPhaseFactory:nextPhaseFactory];
  if (self) {
    _version = version;
...
@interface iTermOptionalComponentDownloadWindowController () <
    iTermOptionalComponentDownloadPhaseDelegate>
...
- (void)optionalComponentDownloadPhaseDidComplete:
  ...
  iTermOptionalComponentDownloadPhase *nextPhase =
      sender.nextPhaseFactory ? sender.nextPhaseFactory(_currentPhase) : nil;
  if (nextPhase) {
...
- (void)optionalComponentDownloadPhase:
  ...
  self->_progressLabel.stringValue = [NSString
      stringWithFormat:@"%@ of %@", [NSString it_formatBytes:bytesWritten],
