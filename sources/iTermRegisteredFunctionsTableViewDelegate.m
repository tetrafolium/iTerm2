...
- (void)loadRows {
  ...
  _rows = [subs.allKeys mapWithBlock:^id(NSString *signature) {
    ...
    iTermScriptHistoryEntry *entry = [[iTermAPIHelper sharedInstance]
        scriptHistoryEntryForConnectionKey:connectionKey];
    NSString *script = entry.name ?: @"Unknown";
    return [[iTermRegisteredFunctionProxy alloc] initWithSignature:signature
                                                              role:role
                                                            script:script];
  }];
}
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  NSString *identifier =
      [NSString stringWithFormat:@"%@_%@", NSStringFromClass(proxy.class),
  ...
  view.textField.stringValue = [self stringForProxy:proxy
