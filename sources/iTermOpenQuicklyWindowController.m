...
@interface iTermOpenQuicklyWindowController () <
    iTermOpenQuicklyTextFieldDelegate, iTermOpenQuicklyModelDelegate,
...
- (void)awakeFromNib {
  ...
  if (@available(macOS 10.16, *)) {
    ...
    // Possibly a 10.16 beta bug? Using intercell spacing clips the selection
    // rect.
  ...
  contentView.layer.borderColor = [[NSColor colorWithCalibratedRed:0.66
                                                             green:0.66
                                                              blue:0.66
                                                             alpha:1] CGColor];
  contentView.layer.borderWidth = 0.5;

  _divider.color = [NSColor colorWithCalibratedRed:0.66
                                             green:0.66
                                              blue:0.66
                                             alpha:1];
...
- (void)presentWindow {
  ...
  // After the window is rendered, call update which will animate to the new
  // frame.
...
- (void)update {
  ...
  _scrollView.frame = NSMakeRect(
      _scrollView.frame.origin.x, _scrollView.frame.origin.y,
      contentViewFrame.size.width, contentViewFrame.size.height - 41);
  // Select the first item.
  if (self.model.items.count) {
    [_table selectRowIndexes:[NSIndexSet indexSetWithIndex:0]
...
- (NSRect)frame {
  ...
  CGFloat nonTableSpace =
      kMarginAboveField + _textField.frame.size.height + kMarginBelowField;
  int numberOfVisibleRowsDesired =
      MIN(self.model.items.count,
          (maxHeight - nonTableSpace) /
              (_table.rowHeight + _table.intercellSpacing.height));
  NSRect frame = self.window.frame;
  NSSize contentSize = frame.size;

  contentSize.height = nonTableSpace;
  if (numberOfVisibleRowsDesired > 0) {
    // Use the bottom of the last visible cell's frame for the height of the
    // table view portion of the window. This is the most reliable way of
    // getting its max-Y position.
    NSRect frameOfLastVisibleCell =
        [_table frameOfCellAtColumn:0 row:numberOfVisibleRowsDesired - 1];
  ...
  frame.origin.x = NSMinX(screen.frame) +
                   floor((screen.frame.size.width - frame.size.width) / 2);
  frame.origin.y =
      NSMaxY(screen.frame) - kMarginAboveWindow - frame.size.height;
  return frame;
}
    ...
    if ([object isKindOfClass:[PTYSession class]]) {
      ...
      if (!profileHotkey ||
        ...
        [iTermSessionLauncher launchBookmark:profile
                                  inTerminal:[[iTermController sharedInstance]
                                                 currentTerminal]
                                     withURL:nil
      ...
      iTermOpenQuicklyArrangementItem *item =
          (iTermOpenQuicklyArrangementItem *)object;
      [[iTermController sharedInstance]
          loadWindowArrangementWithName:item.identifier
                       asTabsInTerminal:item.inTabs
                                            ? [[iTermController sharedInstance]
                                                  currentTerminal]
                                            : nil];
    } else if ([object
                   isKindOfClass:[iTermOpenQuicklyChangeProfileItem class]]) {
      // Change profile
      ...
      iTermOpenQuicklyScriptItem *item =
          [iTermOpenQuicklyScriptItem castFrom:object];
      [[[[iTermApplication sharedApplication] delegate] scriptsMenuController]
          launchScriptWithRelativePath:item.identifier
                             arguments:@[]
...
- (NSView *)tableView:(NSTableView *)tableView
    viewForTableColumn:(NSTableColumn *)tableColumn
                   row:(NSInteger)row {
  iTermOpenQuicklyTableCellView *result =
  ...
  for (int i = 0; i < _model.items.count; i++) {
    ...
    if (@available(macOS 10.14, *)) {
      if (i == _table.selectedRow) {
        [[_table viewAtColumn:0 row:i makeIfNecessary:NO]
            setBackgroundStyle:NSBackgroundStyleEmphasized];
      } else {
        [[_table viewAtColumn:0 row:i
              makeIfNecessary:NO] setBackgroundStyle:NSBackgroundStyleNormal];
      }
    } else {
      if (i == row) {
        color = [NSColor whiteColor];
...
- (BOOL)control:(NSControl *)control
  ...
  if (commandSelector == @selector(moveUp:) || commandSelector == @selector
                                                   (moveDown:)) {
    NSInteger row = [_table selectedRow];
    if (row < 0) {
      row = 0;
      ...
      row--;
               ...
               row + 1 < _table.numberOfRows) {
      row++;
    }
    [_table selectRowIndexes:[NSIndexSet indexSetWithIndex:row]
        byExtendingSelection:NO];
    result = YES;
  }
  return result;
...
- (id)openQuicklyModelDisplayStringForFeatureNamed:(NSString *)name
  ...
  NSMutableAttributedString *theString = [[[NSMutableAttributedString alloc]
      initWithString:prefix
          attributes:[self attributes]] autorelease];
  [theString
      appendAttributedString:[self attributedStringFromString:value
...
- (NSAttributedString *)openQuicklyModelAttributedStringForDetail:
    (NSString *)detail {
  return [self attributedStringFromString:detail byHighlightingIndices:nil];
}
...
- (NSAttributedString *)attributedStringFromString:(NSString *)source
  ...
  NSMutableAttributedString *attributedString =
      [[[NSMutableAttributedString alloc] initWithString:source
