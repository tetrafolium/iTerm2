...
// Finished downloads will be automatically removed from the downloads menu
...
- (void)cleanUpMenus {
  ...
  for (TransferrableFileMenuItemViewController
           *controller in _viewControllers) {
    if ([controller timeSinceLastStatusChange] >
            kMaximumTimeToKeepFinishedDownload &&
        controller.transferrableFile.status !=
            kTransferrableFileStatusStarting &&
        controller.transferrableFile.status !=
            kTransferrableFileStatusTransferring &&
        controller.transferrableFile.status !=
            kTransferrableFileStatusCancelling) {
      [controller.view.enclosingMenuItem.menu
          removeItem:controller.view.enclosingMenuItem];
      [controllersToRemove addObject:controller];
    }
  }

  for (TransferrableFileMenuItemViewController
           *controller in controllersToRemove) {
...
- (void)animateImage:(NSImage *)image
  ...
  CFTypeRef temp = NULL;
  AXUIElementCopyAttributeValue(menuElement, kAXPositionAttribute,
                                (CFTypeRef *)&temp);
  if (!temp) {
    return;
  }
  ...
  NSWindow *window = [[NSWindow alloc]
      initWithContentRect:NSMakeRect(point.x, point.y, image.size.width,
                                     image.size.height)
  ...
  [window.animator
      setFrame:NSMakeRect(position.x, position.y, window.frame.size.width,
...
- (AXUIElementRef)menuElementNamed:(NSString *)menuName {
  ...
  // Despite what the name would suggest, the children array and its contents
  // don't seen to need too be released by us.
  error = AXUIElementCopyAttributeValues(menuBar, kAXChildrenAttribute, 0,
                                         count, (CFArrayRef *)&children);
  if (error) {
    CFRelease(menuBar);
    return NULL;
  }
  ...
  for (id child in children) {
    ...
    id title;
    error = AXUIElementCopyAttributeValue(element, kAXTitleAttribute,
                                          (CFTypeRef *)&title);
    if (error) {
      continue;
    }
...
- (void)removeAllDownloads {
  ...
  NSArray *downloads =
      [_viewControllers filteredArrayUsingBlock:^BOOL(
                            TransferrableFileMenuItemViewController *anObject) {
        return anObject.transferrableFile.isDownloading &&
               [removableStatuses
                   containsObject:@(anObject.transferrableFile.status)];
      }];
...
- (TransferrableFileMenuItemViewController *)viewControllerForTransferrableFile:
...
- (NSMenuItem *)menuItemForTransferrableFile:
    (TransferrableFile *)transferrableFile {
  NSMenuItem *item = [[NSMenuItem alloc] init];
  TransferrableFileMenuItemViewController *controller =
      [[[TransferrableFileMenuItemViewController alloc]
          initWithTransferrableFile:transferrableFile] autorelease];
...
- (void)transferrableFileDidStartTransfer:
  ...
  [[self menuForFile:transferrableFile]
      addItem:[self menuItemForTransferrableFile:transferrableFile]];
  TransferrableFileMenuItemViewController *controller =
      [self viewControllerForTransferrableFile:transferrableFile];
  [controller update];
}
...
- (void)transferrableFile:(TransferrableFile *)transferrableFile
  ...
  transferrableFile.status = error
                                 ? kTransferrableFileStatusFinishedWithError
                                 : kTransferrableFileStatusFinishedSuccessfully;
...
- (void)transferrableFileDidStopTransfer:
    (TransferrableFile *)transferrableFile {
...
- (void)transferrableFile:(TransferrableFile *)transferrableFile
  ...
  NSString *text = [NSString
      stringWithFormat:@"Authenticate %@", transferrableFile.authRequestor];
  NSAlert *alert = [[[NSAlert alloc] init] autorelease];
  alert.messageText = text;
  alert.informativeText =
      [NSString stringWithFormat:@"Please enter the %@ for %@ to begin %@.",
...
- (void)asynchronouslySelectPasswordFromPasswordManager:
