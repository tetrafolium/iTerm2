...
@property(nonatomic, weak) IBOutlet id<iTermShellIntegrationInstallerDelegate>
    shellInstallerDelegate;
...
static NSString *const iTermShellIntegrationInstallUtilitiesUserDefaultsKey =
    @"NoSyncInstallUtilities";
...
- (NSAttributedString *)attributedStringWithLinkToURL:(NSURL *)url
...
- (void)appendLearnMoreToAttributedString:
            (NSMutableAttributedString *)attributedString
                                      url:(NSURL *)url {
  [attributedString
      appendAttributedString:[self
                                 attributedStringWithLinkToURL:url
                                                         title:@"Learn more."]];
}

- (void)viewDidLoad {
  NSNumber *number = [[NSUserDefaults standardUserDefaults]
      objectForKey:iTermShellIntegrationInstallUtilitiesUserDefaultsKey];
  BOOL installUtilities = number ? number.boolValue : YES;
  self.utilities.state =
      installUtilities ? NSControlStateValueOn : NSControlStateValueOff;

  NSMutableAttributedString *attributedString;
  attributedString = [[self
      attributedStringWithFont:_descriptionLabel.font
  ...
  attributedString =
      [[self attributedStringWithFont:_utilitiesLabel.font
...
- (IBAction)toggleInstallUtilities:(id)sender {
  [[NSUserDefaults standardUserDefaults]
      setBool:self.utilities.state == NSControlStateValueOn
       forKey:iTermShellIntegrationInstallUtilitiesUserDefaultsKey];
}

- (IBAction)next:(id)sender {
  [self.shellInstallerDelegate
      shellIntegrationInstallerSetInstallUtilities:self.utilities.state ==
