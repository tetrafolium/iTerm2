...
+ (NSString *)titleForActionDict:(NSDictionary *)dict
  ...
  title = [title stringByReplacingEscapedChar:'d'
                                   withString:workingDirectory ?: @"."];
  title = [title stringByReplacingEscapedChar:'h'
                                   withString:remoteHost.hostname];
  title = [title stringByReplacingEscapedChar:'u'
                                   withString:remoteHost.username];
...
+ (NSString *)parameterValue:(NSString *)parameter
  ...
  case kRunCoprocessContextMenuAction:
    return [parameter
        stringWithBackslashEscapedShellCharactersIncludingNewlines:NO];
...
+ (NSString *)parameterForActionDict:(NSDictionary *)dict
  ...
  ContextMenuActions action =
      (ContextMenuActions)[[dict objectForKey:kActionKey] intValue];
  for (int i = 0; i < 9; i++) {
    NSString *repl = @"";
    if (i < components.count) {
    ...
    parameter = [parameter stringByReplacingBackreference:i
                                               withString:repl ?: @""];
  }

  parameter = [parameter stringByReplacingEscapedChar:'d'
                                           withString:workingDirectory ?: @"."];
  parameter = [parameter stringByReplacingEscapedChar:'h'
                                           withString:remoteHost.hostname];
  parameter = [parameter stringByReplacingEscapedChar:'u'
                                           withString:remoteHost.username];
...
- (IBAction)add:(id)sender {
  NSDictionary *defaultAction = [NSDictionary
      dictionaryWithObjectsAndKeys:
          @"", kTitleKey, [NSNumber numberWithInt:kOpenFileContextMenuAction],
          kActionKey, nil];
...
- (NSCell *)tableView:(NSTableView *)tableView
  ...
  NSArray *actionNames = @[
    @"Open File…", @"Open URL…", @"Run Command…", @"Run Coprocess…",
    @"Send text…", @"Run Command in Window…"
  ];
  NSArray *paramPlaceholders = @[
    @"Enter file name", @"Enter URL", @"Enter command",
    @"Enter coprocess command", @"Enter text", @"Enter command"
  ];
  ...
  if (tableColumn == _titleColumn) {
    ...
    NSPopUpButtonCell *cell =
        [[NSPopUpButtonCell alloc] initTextCell:[actionNames objectAtIndex:0]
                                      pullsDown:NO];
