...
// This has a tuple of (iTermVariables, terminal path name). It compares equal
// to other designators that refer to the same variable. The variable must be
// set to be designated.
@interface iTermVariableDesignator : NSObject <NSCopying>
@end

// Provides access to  the variables that are visible from a particular
// callsite. Each set of variables except one (that of the most local scope)
// must have a name. Variables are searched for one matching the name. You could
// get and set variables through this object. If you want to get called back
// when a value changes, use iTermVariableReference.
...
@property(nonatomic, readonly)
    NSArray<iTermTuple<NSString *, iTermVariables *> *> *frames;
...
- (BOOL)setValue:(nullable id)value
         forPath:(NSString *)firstName, ... NS_REQUIRES_NIL_TERMINATION;
...
- (void)setValuesForKeysWithDictionary:
    (NSDictionary<NSString *, id> *)keyedValues NS_UNAVAILABLE;
...
// It does not attempt to resolve dangling references and should not be
// long-lived.
...
// A scope that causes parsed expressions to record variable references rather
// than expand them to values.
