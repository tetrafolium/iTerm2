...
- (instancetype)initWithUninitializedLength:(NSUInteger)length {
  return [self initWithBytesNoCopy:iTermMalloc(length)
                            length:length
                      freeWhenDone:YES];
}

- (void)appendBytes:(unsigned char *)bytes
                length:(int)length
    excludingCharacter:(char)exclude {
  int i;
  int lastIndex = 0;
  for (i = 0; i < length; i++) {
    if (bytes[i] == exclude) {
...
- (NSInteger)it_replaceOccurrencesOfData:(NSData *)target
                                withData:(NSData *)replacement {
  NSInteger count = 0;
  ...
  while (range.length > 0) {
    ...
    [self replaceBytesInRange:replacementRange
