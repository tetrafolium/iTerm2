...
static cc_t iTermTTYMakeControlKey(char c) { return c - 'A' + 1; }
...
void iTermTTYStateInit(iTermTTYState *ttyState, iTermTTYCellSize cellSize,
                       iTermTTYPixelSize pixelSize, int isUTF8) {
  ...
  term->c_iflag =
      ICRNL | IXON | IXANY | IMAXBEL | BRKINT | (isUTF8 ? IUTF8 : 0);
...
static void iTermForceSetTerminalSize(int fd, PTYTaskSize taskSize) {
  struct winsize winsize = {.ws_col = taskSize.cellSize.width,
  ...
  FDLog(LOG_DEBUG, "Set window size to cells=(%d x %d) pixels=(%d x %d)",
        winsize.ws_col, winsize.ws_row, winsize.ws_xpixel, winsize.ws_ypixel);
