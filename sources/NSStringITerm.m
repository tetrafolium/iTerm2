...
+ (BOOL)isDoubleWidthCharacter:(int)unicode
  ...
  if ([[NSCharacterSet fullWidthCharacterSetForUnicodeVersion:version]
          longCharacterIsMember:unicode]) {
    return YES;
  }
  if (ambiguousIsDoubleWidth &&
      [[NSCharacterSet ambiguousWidthCharacterSetForUnicodeVersion:version]
          longCharacterIsMember:unicode]) {
    return YES;
  }
  return NO;
...
+ (NSString *)stringFromPasteboard {
  ...
  if (gDebugLogging) {
    ...
    for (NSPasteboardItem *item in
         [[NSPasteboard generalPasteboard] pasteboardItems]) {
      DLog(@"Item=%@", item);
      for (NSPasteboardType type in item.types) {
        DLog(@"  For type %@: plist=%@ data=%@ string=%@", type,
             [item propertyListForType:type], [item dataForType:type],
...
- (NSString *)stringWithBackslashEscapedShellCharactersIncludingNewlines:
    (BOOL)includingNewlines {
  NSMutableString *aMutableString =
      [[NSMutableString alloc] initWithString:self];
  [aMutableString
      escapeShellCharactersWithBackslashIncludingNewlines:includingNewlines];
...
- (NSString *)stringWithEscapedShellCharactersIncludingNewlines:
    (BOOL)includingNewlines {
  NSMutableString *aMutableString =
      [[NSMutableString alloc] initWithString:self];
...
- (NSString *)it_stringByExpandingBackslashEscapedCharacters {
  ...
  for (NSInteger i = 0; i < self.length; i++) {
    ...
    if (escape) {
      ...
      if (replacement) {
        [result
            appendString:[self substringWithRange:NSMakeRange(start,
                                                              i - start - 1)]];
  ...
  if (self.length > start) {
    [result
        appendString:[self substringWithRange:NSMakeRange(start, self.length -
                                                                     start)]];
  }
  return result;
}

- (NSArray *)componentsBySplittingStringWithQuotesAndBackslashEscaping:
    (NSDictionary *)escapes {
...
- (NSString *)stringByReplacingEscapedHexValuesWithChars {
  ...
  while (range.location != NSNotFound) {
    int numSlashes = 0;
    for (int i = range.location - 1;
         i >= 0 && [self characterAtIndex:i] == '\\'; i--) {
      ++numSlashes;
    }
...
- (NSString *)stringByReplacingEscapedChar:(unichar)echar
                                withString:(NSString *)maybeString {
...
// Returns the number of valid bytes in a sequence from a row in table 3-7 of
// the Unicode 6.2 spec. Returns 0 if no bytes are valid (a true maximal subpart
// is never less than 1).
static int maximal_subpart_of_row(
    const unsigned char *datap, int datalen, int bytesInRow,
...
// This function finds the longest initial sequence of bytes that look like a
// valid UTF-8 sequence. It's used to gobble them up and replace them with a <?>
// replacement mark in an invalid sequence.
static int minimal_subpart(const unsigned char *datap, int datalen) {
  // This comes from table 3-7 in
  ...
  for (int row = 0; wellFormedSequencesTable[row].numBytes > 0; row++) {
    longest = MAX(longest,
                  maximal_subpart_of_row(datap, datalen,
...
int decode_utf8_char(const unsigned char *datap, int datalen,
                     int *restrict result) {
  ...
  // considered an error).
  unsigned int smallest[7] = {0,         0,          0x80UL,     0x800UL,
                              0x10000UL, 0x200000UL, 0x4000000UL};
  ...
  if (theChar < smallest[utf8Length]) {
    // A too-long sequence was used to encode a value. For example, a 4-byte
    // sequence must encode a value of at least 0x10000 (it is F0 90 80 80). A
    // sequence like F0 8F BF BF is invalid because there is a 3-byte sequence
    // to encode U+FFFF (the sequence is EF BF BF).
...
- (NSString *)initWithUTF8DataIgnoringErrors:(NSData *)data {
  ...
  return [self initWithData:utf16Data
...
- (NSString *)stringByBase64DecodingStringWithEncoding:
    (NSStringEncoding)encoding {
  return
      [[NSString alloc] initWithData:[NSData dataWithBase64EncodedString:self]
                            encoding:encoding];
}

- (NSString *)stringByTrimmingTrailingWhitespace {
  return [self stringByTrimmingTrailingCharactersFromCharacterSet:
                   [NSCharacterSet whitespaceCharacterSet]];
}

- (NSString *)stringByTrimmingTrailingCharactersFromCharacterSet:
    (NSCharacterSet *)charset {
  NSCharacterSet *invertedCharset = [charset invertedSet];
  NSRange rangeOfLastWantedCharacter =
      [self rangeOfCharacterFromSet:invertedCharset options:NSBackwardsSearch];
  ...
  } else if (rangeOfLastWantedCharacter.location +
                 rangeOfLastWantedCharacter.length <
             self.length) {
    NSUInteger i =
        rangeOfLastWantedCharacter.location + rangeOfLastWantedCharacter.length;
...
- (NSString *)substringIncludingOffset:(int)offset
  ...
  NSRange firstBadCharRange =
      [self rangeOfCharacterFromSet:[charSet invertedSet]
                            options:NSBackwardsSearch
                              range:NSMakeRange(0, offset)];
  NSRange lastBadCharRange = [self
      rangeOfCharacterFromSet:[charSet invertedSet]
                      options:0
...
- (NSRange)rangeOfURLInString {
  ...
  if (range.location != NSNotFound) {
    ...
    NSMutableCharacterSet *schemeCharacterSet =
        [NSMutableCharacterSet alphanumericCharacterSet];
    [schemeCharacterSet addCharactersInString:@"-"]; // for chrome-devtools: and
                                                     // x-man-page:, issue 5298
    for (NSInteger i = ((NSInteger)range.location) - 1; i >= 0; i--) {
      if (![schemeCharacterSet
              characterIsMember:[trimmedURLString characterAtIndex:i]]) {
        ...
        trimmedURLString =
            [trimmedURLString stringByRemovingEnclosingPunctuationMarks];
    ...
    // If the string begins with an opening brace or quote, return just the
    // bracketed string.
    trimmedURLString =
        [trimmedURLString stringByRemovingEnclosingPunctuationMarks];
  }
  ...
  for (NSArray *pair in pairs) {
    if ([trimmed hasPrefix:pair[0]] && [trimmed hasSuffix:pair[1]]) {
      return [[self substringWithRange:NSMakeRange(1, self.length - 2)]
          stringByRemovingEnclosingBrackets];
    }
  }
  return self;
}
...
- (NSString *)stringByRemovingTerminatingPunctuation {
  NSCharacterSet *punctuationCharacterSet = [NSCharacterSet
      characterSetWithCharactersInString:[iTermAdvancedSettingsModel
                                             trailingPunctuationMarks]];
  NSRange range =
      [self rangeOfCharacterFromSet:punctuationCharacterSet
...
- (NSFont *)fontValue {
  ...
  // The sscanf man page is unclear whether it will always null terminate when
  // the length hits the maximum field width, so ensure it is null terminated.
  utf8FontName[127] = '\0';
  ...
  if (aFont == nil) {
    DLog(@"Failed to look up font named %@. Falling back to to user font",
         fontName);
    return [NSFont userFixedPitchFontOfSize:0.0]
               ?: [NSFont systemFontOfSize:[NSFont systemFontSize]];
  }
...
+ (NSString *)stringWithHexEncodedString:(NSString *)hexEncodedString {
  ...
  for (int i = 0; i + 1 < hexEncodedString.length; i += 2) {
    char buffer[3] = {[hexEncodedString characterAtIndex:i],
                      [hexEncodedString characterAtIndex:i + 1], 0};
    int value;
...
static TECObjectRef
CreateTECConverterForUTF8Variants(TextEncodingVariant variant) {
  TextEncoding utf16Encoding =
      CreateTextEncoding(kTextEncodingUnicodeDefault,
                         kTextEncodingDefaultVariant, kUnicodeUTF16Format);
  TextEncoding hfsPlusEncoding = CreateTextEncoding(
      kTextEncodingUnicodeDefault, variant, kUnicodeUTF16Format);
...
- (NSString *)_convertBetweenUTF8AndHFSPlusAsPrecomposition:(BOOL)precompose {
  ...
  dispatch_once(&onceToken, ^{
    gHFSPlusComposed =
        CreateTECConverterForUTF8Variants(kUnicodeHFSPlusCompVariant);
    gHFSPlusDecomposed =
        CreateTECConverterForUTF8Variants(kUnicodeHFSPlusDecompVariant);
  });
  ...
  if (TECConvertText(precompose ? gHFSPlusComposed : gHFSPlusDecomposed,
                     (TextPtr)in, in_len, &in_len, (TextPtr)out, out_len,
                     &out_len) != noErr) {
    ret = self;
  } else {
...
- (NSString *)stringByExpandingVimSpecialCharacters {
  enum {
    kSpecialCharacterThreeDigitOctal, // \...    three-digit octal number (e.g.,
                                      // "\316")
    kSpecialCharacterTwoDigitOctal,   // \..     two-digit octal number (must be
                                      // followed by non-digit)
    kSpecialCharacterOneDigitOctal,   // \.      one-digit octal number (must be
                                      // followed by non-digit)
    kSpecialCharacterTwoDigitHex, // \x..    byte specified with two hex numbers
                                  // (e.g., "\x1f")
    kSpecialCharacterOneDigitHex, // \x.     byte specified with one hex number
                                  // (must be followed by non-hex char)
    kSpecialCharacterFourDigitUnicode, // \u....  character specified with up to
                                       // 4 hex numbers
  ...
  while (index != NSNotFound && index < self.length) {
    [result
        appendString:[self substringWithRange:NSMakeRange(
                                                  haveAppendedUpToIndex,
                                                  index -
                                                      haveAppendedUpToIndex)]];
...
- (iTermTuple<NSString *, NSString *> *)it_stringBySplittingOnFirstSubstring:
    (NSString *)substring {
  ...
  if (range.location == NSNotFound) {
    ...
    return [iTermTuple
        tupleWithObject:[self substringToIndex:range.location]
...
- (void)enumerateSwiftySubstrings:(void (^)(NSUInteger index,
                                            NSString *substring, BOOL isLiteral,
                                            BOOL *stop))block {
  iTermSwiftyStringParser *parser =
      [[iTermSwiftyStringParser alloc] initWithString:self];
...
- (NSString *)ellipsizedDescriptionNoLongerThan:(int)maxLength {
  NSString *noNewlineSelf =
      [self stringByReplacingOccurrencesOfRegex:@"[\\r\\n]" withString:@"↩"];
  ...
  NSRange firstNonWhitespaceRange =
      [noNewlineSelf rangeOfCharacterFromSet:[whitespace invertedSet]];
  ...
  if (length < maxLength) {
    prefix =
        [noNewlineSelf substringFromIndex:firstNonWhitespaceRange.location];
  } else {
    prefix = [noNewlineSelf
        substringWithRange:NSMakeRange(firstNonWhitespaceRange.location,
                                       maxLength - 1)];
    truncated = YES;
  }
  prefix = [prefix
  ...
  return [self
      stringByReplacingOccurrencesOfRegex:@"[\x00-\x1f\x7f]"
                               usingBlock:^NSString *(
                                 ...
                                 for (NSInteger i = 0;
                                      i < capturedRanges[0].length; i++) {
                                   unichar c = [[self
                                       substringWithRange:NSMakeRange(index + i,
                                                                      1)]
                                       characterAtIndex:0];
                                   if (c == 0x7f) {
...
- (NSSet *)doubleDollarVariables {
  ...
  while (rangeToSearch.length > 0) {
    ...
    if (start < 0) {
      ...
      if (string.length >=
          4) { // length of 4 implies $$$$, which should be interpreted as $$
        [set addObject:string];
      }
      start = -1;
    }
    rangeToSearch = NSMakeRange(
        NSMaxRange(range),
        MAX(0, (NSInteger)self.length - (NSInteger)NSMaxRange(range)));
  }

  return set;
}

- (BOOL)stringMatchesGlobPattern:(NSString *)glob
...
- (void)enumerateComposedCharacters:(void (^)(NSRange, unichar, NSString *,
                                              BOOL *))block {
  if (self.length == 0) {
    return;
  }
  ...
  dispatch_once(&onceToken, ^{
    ...
    //
    // This also came up in issue 6048 for FF9E and FF9F (HALFWIDTH KATAKANA
    // VOICED SOUND MARK)
    if ([iTermAdvancedSettingsModel aggressiveBaseCharacterDetection]) {
      exceptions = [NSCharacterSet codePointsWithOwnCell];
    } else {
      exceptions =
          [NSCharacterSet characterSetWithCharactersInString:@"\uff9e\uff9f"];
    }
  });
  CFIndex index = 0;
  ...
  do {
    CFRange tempRange =
        CFStringGetRangeOfComposedCharactersAtIndex((CFStringRef)self, index);
    ...
    if (range.length > 0) {
      ...
      if (rangeOfFirstException.location != NSNotFound) {
        const BOOL precededByZWJ =
            (rangeOfFirstException.location > 0 &&
             [self characterAtIndex:rangeOfFirstException.location - 1] ==
                 0x200d);
        if (!precededByZWJ) {
      ...
      unichar simple =
          range.length == 1 ? [self characterAtIndex:range.location] : 0;
      NSString *complexString =
          range.length == 1 ? nil : [self substringWithRange:range];
      BOOL stop = NO;
...
- (NSString *)firstComposedCharacter:(NSString **)rest {
  ...
  [self enumerateComposedCharacters:^(NSRange range, unichar simple,
...
- (NSString *)byTruncatingComposedCharactersInCenter:(NSInteger)numberToOmit {
  ...
  NSString *prefix =
      [[characters subarrayWithRange:NSMakeRange(0, prefixLength)]
          componentsJoinedByString:@""];
  const NSInteger suffixLength = length - numberToOmit - prefixLength;
  NSString *suffix = [[characters
      subarrayWithRange:NSMakeRange(characters.count - suffixLength,
                                    suffixLength)]
      componentsJoinedByString:@""];
...
- (void)reverseEnumerateSubstringsEqualTo:(NSString *)query
                                    block:(void (^NS_NOESCAPE)(NSRange range))
                                              block {
  NSRange range = [self rangeOfString:query options:NSBackwardsSearch];
  while (range.location != NSNotFound) {
    block(range);
    range = [self rangeOfString:query
                        options:NSBackwardsSearch
...
- (NSDictionary *)attributesUsingFont:(NSFont *)font
  ...
  while (points != prevPoints) {
    attributes[NSFontAttributeName] = [NSFont fontWithName:font.fontName
                                                      size:points];
    NSRect boundingRect =
        [self boundingRectWithSize:NSMakeSize(CGFLOAT_MAX, CGFLOAT_MAX)
...
- (NSString *)stringByCompactingFloatingPointString {
  ...
  NSString *compact =
      [self stringByTrimmingTrailingCharactersFromCharacterSet:
...
- (BOOL)startsWithQuotationMark {
  return [self hasPrefix:[[NSLocale currentLocale]
                             objectForKey:NSLocaleQuotationBeginDelimiterKey]];
}

- (BOOL)endsWithQuotationMark {
  return [self hasSuffix:[[NSLocale currentLocale]
                             objectForKey:NSLocaleQuotationEndDelimiterKey]];
}
...
- (NSString *)stringByInsertingTerminalPunctuation:(NSString *)punctuation {
  if ([[NSLocale currentLocale] commasAndPeriodsGoInsideQuotationMarks] &&
      [self endsWithQuotationMark]) {
    NSString *endQuote = [[NSLocale currentLocale]
        objectForKey:NSLocaleQuotationEndDelimiterKey];
    NSInteger quotationLength = [endQuote length];
    NSString *stringWithoutEndQuote =
        [self substringToIndex:self.length - quotationLength];
    return [[stringWithoutEndQuote stringByAppendingString:punctuation]
        stringByAppendingString:endQuote];
  } else {
  ...
  *truncated = NO;
  for (NSString *part in [self componentsSeparatedByString:@"\n"]) {
    NSAttributedString *string =
        [[NSAttributedString alloc] initWithString:part attributes:attributes];

    CTFramesetterRef framesetter = CTFramesetterCreateWithAttributedString(
        (__bridge CFMutableAttributedStringRef)string);
    CFRange fitRange;

    CFRange textRange = CFRangeMake(0, part.length);
    CGSize frameSize = CTFramesetterSuggestFrameSizeWithConstraints(
        framesetter, textRange, NULL, bounds, &fitRange);
...
- (void)it_drawInRect:(CGRect)rect attributes:(NSDictionary *)attributes {
  ...
  for (NSString *part in [self componentsSeparatedByString:@"\n"]) {
    ...
    CFRange textRange = CFRangeMake(0, part.length);
    CGSize frameSize = CTFramesetterSuggestFrameSizeWithConstraints(
        framesetter, textRange, NULL, rect.size, &fitRange);
...
- (BOOL)startsWithEmoji {
  ...
  dispatch_once(&onceToken, ^{
    ...
    void (^addRange)(NSUInteger, NSUInteger) =
...
+ (NSString *)it_formatBytes:(double)bytes {
  ...
  for (int i = 0; i < sizeof(units) / sizeof(*units); i++) {
    if (bytes < units[i].limit) {
      return
          [NSString stringWithFormat:units[i].format, bytes / units[i].divisor];
    }
  }
  return @"∞";
}
...
- (NSInteger)it_numberOfLines {
  ...
  NSMutableData *data =
      [NSMutableData dataWithLength:self.length * sizeof(unichar)];
...
- (NSString *)stringByTrimmingOrphanedSurrogates {
  ...
  return [self
      substringWithRange:NSMakeRange(startIndex, endIndex - startIndex + 1)];
}

- (NSString *)stringByAppendingPathComponents:
...
- (NSString *)it_escapedForEnv {
  ...
  return [self
      stringByPerformingOrderedSubstitutions:[iTermOrderedDictionary
                                                 withTuples:substitutions]];
}

- (NSString *)stringByPerformingOrderedSubstitutions:
    (iTermOrderedDictionary<NSString *, NSString *> *)substitutions {
  return [substitutions.keys
      reduceWithFirstValue:self
...
- (NSString *)stringByReplacingCharactersAtIndices:(NSIndexSet *)indexSet
                               withStringFromBlock:(NSString * (^NS_NOESCAPE)(
                                                       void))replacement {
...
- (BOOL)caseInsensitiveHasPrefix:(NSString *)prefix {
  const NSRange prefixRange =
      [self rangeOfString:prefix
...
- (void)trimTrailingWhitespace {
  ...
  NSRange rangeOfLastWantedCharacter =
      [self rangeOfCharacterFromSet:nonWhitespaceSet options:NSBackwardsSearch];
  if (rangeOfLastWantedCharacter.location == NSNotFound) {
    [self deleteCharactersInRange:NSMakeRange(0, self.length)];
  } else if (NSMaxRange(rangeOfLastWantedCharacter) < self.length) {
    [self deleteCharactersInRange:NSMakeRange(
                                      NSMaxRange(rangeOfLastWantedCharacter),
                                      self.length -
                                          NSMaxRange(
                                              rangeOfLastWantedCharacter))];
  }
}
...
- (void)escapeShellCharactersIncludingNewlines:(BOOL)includingNewlines {
  if ([iTermAdvancedSettingsModel escapeWithQuotes]) {
    [self escapeShellCharactersWithSingleQuotesIncludingNewlines:
              includingNewlines];
  } else {
    [self
        escapeShellCharactersWithBackslashIncludingNewlines:includingNewlines];
...
- (void)escapeShellCharactersWithSingleQuotesIncludingNewlines:
    (BOOL)includingNewlines {
  return [self
      escapeShellCharactersWithSingleQuotesIncludingNewlines:includingNewlines
                                                     forBash:NO];
}
...
- (void)escapeShellCharactersWithSingleQuotesIncludingNewlines:
  ...
  NSMutableCharacterSet *charsToSearch = [NSMutableCharacterSet
      characterSetWithCharactersInString:[NSString shellEscapableCharacters]];
...
- (void)escapeShellCharactersWithBackslashIncludingNewlines:
    (BOOL)includingNewlines {
  NSString *charsToEscape = [NSString shellEscapableCharacters];
  if (includingNewlines) {
