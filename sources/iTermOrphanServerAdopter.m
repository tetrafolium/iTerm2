...
static void iTermOrphanServerAdopterFindMonoServers(
    void (^completion)(NSArray<NSString *> *)) {
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSMutableArray *array = [NSMutableArray array];
        NSString *dir =
            [NSString stringWithUTF8String:iTermFileDescriptorDirectory()];
        for (NSString *filename in
             [[NSFileManager defaultManager] contentsOfDirectoryAtPath:dir
                                                                 error:nil]) {
          NSString *prefix = [NSString
              stringWithUTF8String:iTermFileDescriptorSocketNamePrefix];
...
static void iTermOrphanServerAdopterFindMultiServers(
    void (^completion)(NSArray<NSString *> *)) {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    NSDirectoryEnumerator *enumerator = [[NSFileManager defaultManager]
                   enumeratorAtURL:[NSURL fileURLWithPath:appSupportPath]
        includingPropertiesForKeys:nil
                           options:
                               NSDirectoryEnumerationSkipsSubdirectoryDescendants
                      errorHandler:nil];
    for (NSURL *url in enumerator) {
      if (![url.path.lastPathComponent
              stringMatchesGlobPattern:@"iterm2-daemon-*.socket"
                         caseSensitive:YES]) {
        continue;
      }
...
- (void)removePath:(NSString *)path {
  _pathsOfOrphanedMonoServers =
      [_pathsOfOrphanedMonoServers arrayByRemovingObject:path];
...
- (void)adoptMonoServerOrphanWithPath:(NSString *)filename
  ...
  iTermFileDescriptorServerConnection serverConnection =
      iTermFileDescriptorClientRun(pid);
...
- (void)enqueueAdoptionsOfMultiServerOrphansWithPath:(NSString *)filename
                                          completion:
  ...
  DLog(@"iTermOrphanServerAdopter: get connection for multiserver socket %@",
       @(number));
                                      ...
                                      .main newCallbackWithBlock:^(
                                                ...
                                                iTermResult<
                                                    iTermMultiServerConnection
                                                        *> *result) {
                                [result
                                    handleObject:^(iTermMultiServerConnection
                                                       *_Nonnull connection) {
                                      [self
                                          didEstablishMultiserverConnection:
                                              connection
                                                               socketNumber:
                                                                   number
                                                                      state:
                                    ...
                                    error:^(NSError *_Nonnull error) {
                                      XLog(@"Orphan server adopter: Failed to "
                                           @"connect to %@",
                                           filename);
                                      completion();
                                    }];
...
- (void)didEstablishMultiserverConnection:
  ...
  NSArray<iTermFileDescriptorMultiClientChild *> *children =
      [connection.unattachedChildren copy];
  for (iTermFileDescriptorMultiClientChild *child in children) {
    ...
    dispatch_group_enter(group);
    DLog(@"Orphan server adopter wants to open session for pid %@ on socket %@",
         @(child.pid), @(number));
    [self.delegate
        orphanServerAdopterOpenSessionForConnection:generalConnection
...
- (void)adoptPartialAttachments:
  ...
  [partialAttachments enumerateObjectsUsingBlock:^(id _Nonnull obj,
                                                   NSUInteger idx,
                                                   BOOL *_Nonnull stop) {
    [self.delegate
        orphanServerAdopterOpenSessionForPartialAttachment:obj
                                                  inWindow:self->_window
                                                completion:^(
                                                    PTYSession *session) {
                                                  if (!self->_window) {
