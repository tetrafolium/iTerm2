...
// h-split ::=
// parent-width,parent-height-parent-x-offset,parent-y-offset{layout,layout-array}
// v-split ::=
// parent-width,parent-height-parent-x-offset,parent-y-offset[layout,layout-array]
...
- (NSMutableDictionary *)coalescedTree:(NSMutableDictionary *)node {
  ...
  if (nodeType == kVSplitLayoutNode || nodeType == kHSplitLayoutNode) {
    ...
    for (NSMutableDictionary *child in children) {
      // Recursively apply this algorithm to the child (does nothing if it's a
      // leaf node)
      NSMutableDictionary *coalescedChild = [self coalescedTree:child];

      if ([[coalescedChild objectForKey:kLayoutDictNodeType] intValue] ==
          nodeType) {
        // The child is a split of the same orientation as this node.
        // Iterate over its children (this node's grandchildren) and hoist each
        // up to be a child of this node.
        [coalescedChildren
            addObjectsFromArray:[coalescedChild
                                    objectForKey:kLayoutDictChildrenKey]];
      } else {
...
- (NSMutableDictionary *)parsedLayoutFromString:(NSString *)layout {
  ...
  if ([self parseLayout:layout
                  range:NSMakeRange(5, layout.length - 5)
               intoTree:temp]) {
    ...
    NSArray *components =
        [layout captureComponentsMatchedByRegex:
                    @"^[0-9a-fA-F]{4},([0-9]+)x([0-9]+),[0-9]+,[0-9]+[\\[{]"];
    if (components.count == 3) {
      ...
      DLog(@"%%layout-change reported window size of %@ x %@",
           tree[kLayoutDictWidthKey], tree[kLayoutDictHeightKey]);
    }
...
- (id)depthFirstSearchParseTree:(NSMutableDictionary *)parseTree
  ...
  if ([[parseTree objectForKey:kLayoutDictNodeType] intValue] ==
    ...
    return [target performSelector:selector
                        withObject:parseTree
                        withObject:obj];
  } else {
    for (NSMutableDictionary *child in
         [parseTree objectForKey:kLayoutDictChildrenKey]) {
...
- (NSMutableDictionary *)windowPane:(int)windowPane
  ...
  return [self depthFirstSearchParseTree:parseTree
                         callingSelector:@selector(searchParseTree:
                                                     forWindowPane:)
                                onTarget:self
...
- (NSArray *)windowPanesInParseTree:(NSDictionary *)parseTree {
  ...
  if ([[parseTree objectForKey:kLayoutDictNodeType] intValue] ==
    ...
    [result
        addObject:[NSNumber
                      numberWithInt:[[parseTree
                                        objectForKey:kLayoutDictWindowPaneKey]
                                        intValue]]];
  } else {
    for (NSDictionary *child in
         [parseTree objectForKey:kLayoutDictChildrenKey]) {
...
- (NSMutableDictionary *)dictForLeafNodeInLayout:(NSString *)layout
                                           range:(NSRange)range {
  NSArray *components =
      [layout captureComponentsMatchedByRegex:
  ...
  if (components.count != 6 && components.count != 5) {
    NSLog(@"Matched wrong number of components in singleton layout \"%@\"",
          [layout substringWithRange:range]);
    return nil;
  }

  NSMutableDictionary *result = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:[components objectAtIndex:1],
                                   kLayoutDictWidthKey,
                                   [components objectAtIndex:2],
                                   kLayoutDictHeightKey,
                                   [components objectAtIndex:3],
                                   kLayoutDictXOffsetKey,
                                   [components objectAtIndex:4],
                                   kLayoutDictYOffsetKey,
                                   [NSNumber numberWithInt:kLeafLayoutNode],
                                   kLayoutDictNodeType, nil];
  if (components.count == 6) {
    [result setObject:[NSNumber
                          numberWithInt:[[components objectAtIndex:5] intValue]]
...
- (NSRange)rangeOfChildrenInLayout:(NSString *)layout
  ...
  NSLog(@"Unbalanced braces in children in layout: %@",
        [layout substringWithRange:range]);
...
- (NSMutableDictionary *)splitDictWithType:(LayoutNodeType)nodeType {
  return [NSMutableDictionary
      dictionaryWithObject:[NSNumber numberWithInt:nodeType]
                    forKey:kLayoutDictNodeType];
}

- (NSString *)splitOffFirstLayoutInLayoutArray:(NSString *)layouts
                                          rest:(NSMutableString *)rest {
  if (!layouts.length) {
    return nil;
  }
  ...
  NSArray *components =
      [layouts captureComponentsMatchedByRegex:
                   @"^(?:,?)([0-9]+x[0-9]+,[0-9]+,[0-9]+(?:(?:,[0-9]+)?))(.*)"];
  if (components.count != 3) {
  ...
  if ([suffix length] == 0) {
    ...
    if (c == ',') {
      ...
      [rest setString:[suffix substringWithRange:NSMakeRange(1, suffix.length -
                                                                    1)]];
      return [components objectAtIndex:1];
    } else if (c == '[' || c == '{') {
      // Find matching close bracket/brace
      NSRange childrenRange =
          [self rangeOfChildrenInLayout:suffix
                                  range:NSMakeRange(0, suffix.length)
                                   open:c == '[' ? @"[" : @"{"
                                  close:c == '[' ? @"]" : @"}"];
      int nextItemOffset = childrenRange.location + childrenRange.length + 1;
      [rest setString:[suffix
                          substringWithRange:NSMakeRange(nextItemOffset,
                                                         suffix.length -
                                                             nextItemOffset)]];
      return [NSString
          stringWithFormat:@"%@%@", [components objectAtIndex:1],
                           [suffix
                               substringWithRange:NSMakeRange(
                                                      childrenRange.location -
                                                          1,
                                                      childrenRange.length +
                                                          2)]];
    } else {
...
- (BOOL)parseLayoutArray:(NSString *)layout
                   range:(NSRange)range
                intoTree:(NSMutableArray *)tree {
  // layout,layout,...
  NSMutableString *rest = [NSMutableString string];
  NSString *first =
      [self splitOffFirstLayoutInLayoutArray:[layout substringWithRange:range]
                                        rest:rest];
  while (first) {
...
- (BOOL)parseLayout:(NSString *)layout
              range:(NSRange)range
           intoTree:(NSMutableArray *)tree {
  NSString *openChar = @"[";
  NSString *closeChar = @"]";
...
- (NSMutableDictionary *)searchParseTree:(NSMutableDictionary *)parseTree
  ...
  if ([[parseTree objectForKey:kLayoutDictWindowPaneKey] intValue] ==
      [wp intValue]) {
    return parseTree;
  } else {
    return nil;
