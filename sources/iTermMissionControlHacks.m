...
+ (CGEventRef)newEventToSwitchToSpace:(int)spaceNum // 1 indexed
  ...
  NSUserDefaults *userDefaults =
      [[NSUserDefaults alloc] initWithSuiteName:@"com.apple.symbolichotkeys"];
  const NSInteger action = 118 + spaceNum - 1;
  NSDictionary *dict = [NSDictionary
      castFrom:[userDefaults objectForKey:@"AppleSymbolicHotKeys"]];
  if (!dict) {
    return nil;
  }
  dict = [NSDictionary castFrom:dict[@(action)]]
             ?: [NSDictionary castFrom:dict[@(action).stringValue]];
  if (!dict) {
    return nil;
  }
  ...
  CGEventRef event = CGEventCreateKeyboardEvent(
      NULL, (CGKeyCode)keycode.unsignedShortValue, down);
...
+ (void)complainThatCantSwitchToSpace:(int)spaceNum fix:(NSString *)fix {
  ...
  dispatch_once(&onceToken, ^{
    [[iTermNotificationController sharedInstance]
                 notify:[NSString
                            stringWithFormat:@"Can’t switch to desktop %d",
                                             spaceNum]
        withDescription:fix];
  });
}
...
+ (void)switchToSpace:(int)spaceNum {
  if (!AXIsProcessTrustedWithOptions(
    ...
    [self complainThatCantSwitchToSpace:spaceNum
                                    fix:@"You must grant iTerm2 accessibility "
  ...
  if (!keyDownEvent || !keyUpEvent) {
    [self complainThatCantSwitchToSpace:spaceNum
                                    fix:@"You must enable shortcuts to switch "
                                        @"desktops in System Preferences > "
                                        @"Keyboard."];
    if (keyDownEvent) {
  ...
  // Give the space-switching animation time to get started; otherwise a window
  // opened subsequent to this will appear in the previous space. This is short
  // enough of a delay that it's not annoying when you're already there.
