...
- (void)update {
  ...
  if (stage < 0) {
    ...
    if (self.shell == iTermShellIntegrationShellUnknown) {
      prefix = @"🛑 Your shell is not supported.\n\nOnly bash, fish, tcsh, "
               @"and zsh work with shell integration";
    } else {
      prefix = @"✅ Discovered";
    }
  }
  ...
  if (stage > 0) {
    if (self.shell != iTermShellIntegrationShellUnknown) {
      step = [step
          stringByAppendingFormat:@": you use “%@”.",
  ...
  const BOOL unavailable =
      (stage == 1 && self.shell == iTermShellIntegrationShellUnknown);
  ...
  if (unavailable) {
    ...
    if (_busy && stage == 1) {
      ...
      step = [NSString
          stringWithFormat:@"%@ the shell integration script.", prefix];
    }
    [lines addObject:step];
    ...
    if (self.installUtilities) {
      ...
      if (_busy && stage == 2) {
        ...
        step =
            [NSString stringWithFormat:@"%@ iTerm2 utility scripts.", prefix];
      }
      [lines addObject:step];
    }
  ...
  [lines enumerateObjectsUsingBlock:^(NSString *_Nonnull string, NSUInteger idx,
                                      BOOL *_Nonnull stop) {
  ...
  NSString *preview =
      [self.shellInstallerDelegate
              shellIntegrationInstallerNextCommandForSendShellCommands];
...
- (NSArray<NSButton *> *)previewCommandButtons {
  return @[
    self.previewCommandButton1, self.previewCommandButton2,
    self.previewCommandButton3, self.previewCommandButton4
  ];
}
...
- (IBAction)previewCommand:(id)sender {
  if (_busy) {
    ...
    [self.shellInstallerDelegate
        shellIntegrationInstallerSendShellCommands:_stage];
    return;
  }
