...
static const CGEventFlags kCGEventHotKeyModifierMask =
...
// Override this to do a thing that needs to be done. `siblings` are other
// hotkeys (besides the one this call was made for) that have the same keypress.
- (NSArray<iTermBaseHotKey *> *)hotKeyPressedWithSiblings:
    (NSArray<iTermBaseHotKey *> *)siblings;
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p shortcuts=%@ modifierActivation=%@,%@>",
                       NSStringFromClass([self class]), self, _shortcuts,
                       @(self.hasModifierActivation),
                       @(self.modifierActivation)];
}
...
- (iTermHotKeyDescriptor *)modifierActivationDescriptor {
  if (self.hasModifierActivation) {
    return [iTermHotKeyDescriptor
        descriptorWithModifierActivation:self.modifierActivation];
  } else {
    return nil;
  }
}
...
- (void)registerShortcut:(iTermShortcut *)shortcut {
  if (shortcut.isAssigned) {
    NSUInteger index = [_carbonHotKeys
        indexOfObjectPassingTest:^BOOL(iTermHotKey *_Nonnull obj,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
          // Just test if they would map to the same keycode+modifier, ignore
          // niceties like how it displays.
    ...
    iTermHotKey *newHotkey = [[[iTermCarbonHotKeyController sharedInstance]
...
- (void)setShortcuts:(NSArray<iTermShortcut *> *)shortcuts
  ...
  NSSet<iTermShortcut *> *newShortcuts =
      [[[NSSet alloc] initWithArray:shortcuts] autorelease];
  NSSet<iTermShortcut *> *oldShortcuts =
      [[[NSSet alloc] initWithArray:self.shortcuts] autorelease];

  // NOTE:
  // It's important to detect changes to charactersIgnoringModifiers because if
  // it's not up-to-date in the carbon hotkey then keypresses while a shortcut
  // input field is first responder will send the wrong 'characters' field to
  // it.
...
- (NSArray<iTermBaseHotKey *> *)hotKeyPressedWithSiblings:
  ...
  [NSException raise:NSInternalInconsistencyException
              format:@"Not implemented. Use a subclass."];
  return nil;
}
...
- (BOOL)activationModifierPressIsDoubleTap {
  ...
  const NSTimeInterval kMaxTimeBetweenTaps =
      [iTermAdvancedSettingsModel hotKeyDoubleTapMaxDelay];
  const NSTimeInterval kMinTimeBetweenTabs =
      [iTermAdvancedSettingsModel hotKeyDoubleTapMinDelay];
  const NSTimeInterval elapsedTime = time - _lastModifierTapTime;
  BOOL result =
      (kMinTimeBetweenTabs <= elapsedTime && elapsedTime < kMaxTimeBetweenTaps);
...
- (void)handleActivationModifierPress {
  ...
  if ([self activationModifierPressIsDoubleTap]) {
    DLog(@"is double tap");
    NSArray *siblings = [[[iTermFlagsChangedEventTap sharedInstance] observers]
        mapWithBlock:^id(
            iTermWeakReference<id<iTermEventTapObserver>> *anObject) {
          if (![anObject.weaklyReferencedObject isKindOfClass:[self class]]) {
            return nil;
          }
          iTermBaseHotKey *other = (iTermBaseHotKey *)anObject;
          if (other.hasModifierActivation &&
              other.modifierActivation == self.modifierActivation) {
...
- (NSArray<iTermBaseHotKey *> *)carbonHotkeyPressed:(NSDictionary *)userInfo
                                           siblings:(NSArray<iTermHotKey *> *)
                                                        siblings {
  if (![[[iTermApplication sharedApplication] delegate]
          workspaceSessionActive]) {
    return nil;
  }
  if ([NSApp modalWindow]) {
    DLog(@"Ignore hotkey because there is a modal window %@",
         [NSApp modalWindow]);
    return nil;
  }

  // Only elements of siblings whose targets are iTermBaseHotKey
  NSArray<iTermHotKey *> *siblingHotkeys =
      [siblings filteredArrayUsingBlock:^BOOL(iTermHotKey *anObject) {
  ...
  NSArray<iTermBaseHotKey *> *siblingBaseHotKeys =
      [siblingHotkeys mapWithBlock:^id(iTermHotKey *anObject) {
        return anObject.target;
      }];

  NSArray<iTermBaseHotKey *> *handledBaseHotkeys =
...
- (void)eventTappedWithType:(CGEventType)type event:(CGEventRef)event {
  DLog(@"eventTappedWithType:%@ event:%@", @(type),
  ...
  if (type == kCGEventFlagsChanged) {
    ...
    DLog(@"_modifierWasPressed=%@ modifierIsPressed=%@", @(_modifierWasPressed),
         @(modifierIsPressed));
