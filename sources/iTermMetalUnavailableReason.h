...
iTermMetalUnavailableReasonDescription(iTermMetalUnavailableReason reason) {
  switch (reason) {
  ...
  case iTermMetalUnavailableReasonLigatures:
    return @"ligatures are enabled. You can disable them in Preferences > "
           @"Profiles > Text > Use ligatures.";
  ...
  case iTermMetalUnavailableReasonTransparency:
    return @"transparent windows are not supported. They can be disabled in "
           @"Preferences > Profiles > Window > Transparency.";
  case iTermMetalUnavailableReasonVerticalSpacing:
    return @"the font's vertical spacing set to less than 100%. You can change "
           @"it in Preferences > Profiles > Text > Change Font.";
  case iTermMetalUnavailableReasonMarginSize:
    return @"terminal window margins are too small. You can edit them in "
           @"Preferences > Advanced.";
  case iTermMetalUnavailableReasonAnnotations:
    return @"annotations are open. Find the session with visible annotations "
           @"and close them with View > Show Annotations.";
  ...
  case iTermMetalUnavailableReasonDisconnectedFromPower:
    return @"the computer is not connected to power. You can enable GPU "
           @"rendering while disconnected from "
           @"power in Preferences > General > Advanced GPU Settings.";
  case iTermMetalUnavailableReasonIdle:
    return @"the session is idle. You can enable Metal while idle in "
           @"Preferences > Advanced.";
