...
- (void)setup {
  ...
  // Nuke its prefs
  NSString *defaultsDelete =
      [NSString stringWithFormat:@"defaults delete %@", kTestBundleId];
...
- (void)killTestApp {
  ...
  if (thePid) {
    ...
    // For some reason the scripting bridge test produces an app that doesn't
    // show up in runningApplications.
    system("killall -9 iTerm2ForAppleScriptTesting");
  }
}

- (NSString *)scriptWithCommands:(NSArray *)commands
...
- (NSAppleEventDescriptor *)runScript:(NSString *)script {
  NSAppleScript *appleScript =
      [[[NSAppleScript alloc] initWithSource:script] autorelease];
...
- (void)testScriptingBridge {
  iTerm2Application *iterm =
      [SBApplication applicationWithBundleIdentifier:kTestBundleId];
  [iterm activate];
  [iterm createWindowWithDefaultProfileCommand:nil];
  iTerm2Window *terminal = [iterm currentWindow];
  [terminal.currentSession writeContentsOfFile:nil
                                          text:@"echo Testing123"
                                       newline:NO];
  for (int i = 0; i < 10; i++) {
...
- (void)testCreateWindowWithDefaultProfile {
  NSArray *commands = @[
    @"set oldWindowCount to (count of windows)",
  ...
  assert([[eventDescriptor descriptorAtIndex:2] int32Value] ==
         [[eventDescriptor descriptorAtIndex:1] int32Value] + 1);
}
...
- (void)testCreateWindowWithDefaultProfileAndCommand {
  NSArray *commands = @[
    @"create window with default profile command \"touch /tmp/rancommand\""
  ];
...
- (void)testSelectWindow {
  ...
  NSArray *commands = @[
    ...
    @"  write text \"echo NUMBER ONE\"", @"end tell",
    @"create window with default profile",
    @"tell current session of current window",
  ...
  NSString *script = [self scriptWithCommands:commands outputs:outputs];
  NSAppleEventDescriptor *eventDescriptor = [self runScript:script];
  NSString *firstWindowContents =
      [[eventDescriptor descriptorAtIndex:1] stringValue];
  NSString *secondWindowContents =
      [[eventDescriptor descriptorAtIndex:2] stringValue];
...
- (void)testSelectTab {
  NSArray *commands = @[
    @"create window with default profile",
    @"tell current session of current window",
    @"  write text \"echo NUMBER ONE\"", @"end tell", @"tell current window",
    @"  create tab with default profile", @"end tell",
    @"tell current session of current window",
    @"  write text \"echo NUMBER TWO\"", @"end tell",
    @"delay 0.2", // Give write text time to echo result back
    @"set secondTabContents to (text of current session of current window)",
    @"tell first tab of current window", @"  select", @"end tell",
    @"set firstTabContents to (text of current session of current window)"
  ];
  NSArray *outputs = @[ @"firstTabContents", @"secondTabContents" ];
  NSString *script = [self scriptWithCommands:commands outputs:outputs];
  NSAppleEventDescriptor *eventDescriptor = [self runScript:script];
  NSString *firstTabContents =
      [[eventDescriptor descriptorAtIndex:1] stringValue];
  NSString *secondTabContents =
      [[eventDescriptor descriptorAtIndex:2] stringValue];
...
- (void)testSelectSession {
  NSArray *commands = @[
    ...
    @"  split horizontally with default profile", @"end tell",
    @"tell current session of current window",
    @"  write text \"echo NUMBER TWO\"", @"end tell",
    @"delay 0.2", // Give write text time to echo result back
    @"set secondSessionContents to (text of current session of current window)",
    @"tell first session of current tab of current window", @"  select",
    @"end tell",
  ...
  NSString *script = [self scriptWithCommands:commands outputs:outputs];
  NSAppleEventDescriptor *eventDescriptor = [self runScript:script];
  NSString *firstSessionContents =
      [[eventDescriptor descriptorAtIndex:1] stringValue];
  NSString *secondSessionContents =
      [[eventDescriptor descriptorAtIndex:2] stringValue];
...
- (void)testSplitVerticallyWithDefaultProfile {
  NSArray *commands = @[
    ...
    @"set oldSessionCount to (count of sessions in first tab in first window)",
    @"tell current session of current window",
    @"  split vertically with default profile", @"end tell",
    @"set newSessionCount to (count of sessions in first tab in first window)"
  ];
  NSArray *outputs = @[ @"oldSessionCount", @"newSessionCount" ];
  NSString *script = [self scriptWithCommands:commands outputs:outputs];
...
- (void)testCreateTabWithNamedProfile {
  NSArray *commands = @[
    @"create window with default profile",
    @"set oldTabCount to (count of tabs in first window)",
    @"tell current window", @"  create tab with profile \"Default\"",
    @"end tell", @"set newTabCount to (count of tabs in first window)"
  ];
  NSArray *outputs = @[ @"oldTabCount", @"newTabCount" ];
  NSString *script = [self scriptWithCommands:commands outputs:outputs];
  NSAppleEventDescriptor *eventDescriptor = [self runScript:script];
...
- (void)testResizeSession {
  NSArray *commands = @[
    ...
    @"tell current session of current window", @"  set rows to 20",
    @"  set columns to 30", @"end tell",
...
- (void)testWriteContentsOfFile {
  ...
  NSArray *commands = @[
    ...
    @"  write contents of file \"/tmp/testFile\"", @"end tell",
    @"delay 0.2", // Give write text time to echo result back
...
- (void)testSetGetColors {
  ...
  for (NSString *color in colors) {
    NSString *name = [color stringByReplacingOccurrencesOfString:@" "
                                                      withString:@""];
    [commands
        addObject:[NSString stringWithFormat:@"set old%@ to %@", name, color]];
    [commands
        addObject:[NSString
                      stringWithFormat:@"set %@ to {65535, 0, 0, 0}", color]];
    [commands
        addObject:[NSString stringWithFormat:@"set new%@ to %@", name, color]];
  ...
  for (NSString *name in outputs) {
    NSString *value =
        [NSString stringWithFormat:@"{%d, %d, %d, %d}",
                                   [[[eventDescriptor descriptorAtIndex:i]
                                       descriptorAtIndex:1] int32Value],
                                   [[[eventDescriptor descriptorAtIndex:i]
                                       descriptorAtIndex:2] int32Value],
                                   [[[eventDescriptor descriptorAtIndex:i]
                                       descriptorAtIndex:3] int32Value],
                                   [[[eventDescriptor descriptorAtIndex:i]
                                       descriptorAtIndex:4] int32Value]];
...
- (void)testSetGetName {
  NSArray *commands = @[
    ...
    @"tell current session of current window", @"  set name to \"Testing\"",
    @"end tell", @"set newName to name of current session of current window"
  ];
...
- (void)testIsAtShellPrompt {
  NSArray *commands = @[
    @"create window with default profile", @"delay 0.5",
    @"tell current session of current window",
    @"  set beforeSleep to (is at shell prompt)", @"  write text \"cat\"",
    @"  delay 0.2", @"  set afterSleep to (is at shell prompt)", @"end tell"
  ];
