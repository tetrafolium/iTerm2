...
+ (iTermKeyBindingAction *)globalActionAtIndex:(NSInteger)rowIndex {
  ...
  NSArray *allKeys = [[km allKeys]
      sortedArrayUsingSelector:@selector(compareSerializedKeystroke:)];
...
+ (iTermKeyBindingAction *)localActionForKeystroke:(iTermKeystroke *)keystroke
  ...
  NSDictionary *theKeyMapping =
      [keystroke valueInBindingDictionary:keyMappings];
...
+ (NSSet<iTermKeystroke *> *)keystrokesInKeyMappingsInProfile:
...
+ (NSSet<iTermKeystroke *> *)keystrokesInGlobalMapping {
  return [NSSet setWithArray:[[[self globalKeyMap]
                                 allKeys] mapWithBlock:^id(id anObject) {
                  return [[iTermKeystroke alloc] initWithSerialized:anObject];
                }]];
}
...
+ (BOOL)haveKeyMappingForKeystroke:(iTermKeystroke *)keystroke
...
+ (NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *)
    tuplesOfActionsInProfile:(Profile *)profile {
  NSMutableArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *>
      *result = [NSMutableArray array];
  NSDictionary<id, iTermKeyBindingAction *> *keyboardMap =
      profile[KEY_KEYBOARD_MAP];
  [keyboardMap
      enumerateKeysAndObjectsUsingBlock:^(
          id _Nonnull serialized, iTermKeyBindingAction *_Nonnull action,
          BOOL *_Nonnull stop) {
        iTermKeystroke *key =
...
+ (iTermKeystroke *)keystrokeAtIndex:(int)rowIndex
                           inprofile:(Profile *)profile {
  NSDictionary *km = profile[KEY_KEYBOARD_MAP];
  NSArray *allKeys = [[km allKeys]
      sortedArrayUsingSelector:@selector(compareSerializedKeystroke:)];
...
+ (NSArray<iTermKeystroke *> *)sortedKeystrokesForKeyMappingsInProfile:
    (Profile *)profile {
  NSDictionary *km = profile[KEY_KEYBOARD_MAP];
  NSArray *serialized = [[km allKeys]
      sortedArrayUsingSelector:@selector(compareSerializedKeystroke:)];
...
+ (void)setMappingAtIndex:(int)rowIndex
  ...
  NSArray *allKeys = [[mutableKeyMapping allKeys]
      sortedArrayUsingSelector:@selector(compareSerializedKeystroke:)];
  if (!newMapping) {
    if (rowIndex >= 0 && rowIndex < [allKeys count]) {
      originalKeystroke =
          [[iTermKeystroke alloc] initWithSerialized:allKeys[rowIndex]];
    } else {
      return;
    }
  ...
  if (originalKeystroke) {
    id keyToRemove =
        [originalKeystroke keyInBindingDictionary:mutableKeyMapping];
    if (keyToRemove) {
...
+ (NSMutableDictionary *)removeMappingAtIndex:(int)rowIndex
...
+ (void)removeMappingAtIndex:(int)rowIndex
                 fromProfile:(MutableProfile *)profile {
  profile[KEY_KEYBOARD_MAP] =
      [self removeMappingAtIndex:rowIndex
...
+ (void)loadGlobalKeyMap {
  gGlobalKeyMapping =
      [[NSUserDefaults standardUserDefaults] objectForKey:@"GlobalKeyMap"];
  if (!gGlobalKeyMapping) {
...
+ (NSDictionary *)defaultGlobalKeyMap {
  NSString *plistFile = [[NSBundle bundleForClass:[self class]]
      pathForResource:@"DefaultGlobalKeyMap"
               ofType:@"plist"];
...
+ (void)setGlobalKeyMap:(NSDictionary *)src {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:gGlobalKeyMapping
                                            forKey:@"GlobalKeyMap"];
}
...
+ (iTermKeystroke *)
    keystrokeForMappingReferencingProfileWithGuid:(NSString *)guid
                                        inProfile:(Profile *)profile {
  ...
  [keyboardMap enumerateKeysAndObjectsUsingBlock:^(
                   id _Nonnull key, id _Nonnull keyMap, BOOL *_Nonnull stop) {
    iTermKeyBindingAction *action =
        [iTermKeyBindingAction withDictionary:keyMap];
...
+ (Profile *)removeKeyMappingsReferencingGuid:(NSString *)guid
  ...
  if (profile) {
    ...
    while (YES) {
      iTermKeystroke *keystrokeToRemove = [iTermKeyMappings
          keystrokeForMappingReferencingProfileWithGuid:guid
                                              inProfile:mutableProfile
                                                            ?: profile];
      if (!keystrokeToRemove) {
        break;
      }
      NSArray *sortedKeystrokes = [iTermKeyMappings
          sortedKeystrokesForKeyMappingsInProfile:mutableProfile ?: profile];
      ...
      if (i != NSNotFound) {
        ...
        XLog(@"Profile with guid %@ has key mapping referencing guid %@ with "
             @"keystroke %@ but I can't find it in sorted keys",
  ...
  BOOL change;
  do {
    NSMutableDictionary *mutableGlobalKeyMap =
        [[self globalKeyMap] mutableCopy];
    change = NO;
