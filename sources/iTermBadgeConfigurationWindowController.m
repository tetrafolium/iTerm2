...
@property(nonatomic, weak) id<iTermBadgeConfigurationBadgeViewDelegate>
    delegate;
@end
...
- (void)setDelegate:(id<iTermBadgeConfigurationBadgeViewDelegate>)delegate {
  ...
  if (delegate) {
    _badge.fillColor =
        [[[iTermProfilePreferences objectForKey:KEY_BADGE_COLOR
                                      inProfile:[delegate badgeViewProfile]]
            colorValue] colorWithAlphaComponent:1];
    _badge.backgroundColor = [NSColor clearColor];
    _loremIpsum.image = [_badge image];
    NSColor *backgroundColor = [[iTermProfilePreferences
        objectForKey:KEY_BACKGROUND_COLOR
           inProfile:[delegate badgeViewProfile]] colorValue];
...
- (iTermBadgeViewEdge)edge:(int)i {
  ...
  iTermBadgeViewEdge edges[4] = {
      {0, radius * 2, 0, height, @selector(adjustLeft:),
       [NSCursor resizeLeftRightCursor]},
      {width - radius * 2, width, 0, height, @selector(adjustRight:),
       [NSCursor resizeLeftRightCursor]},
      {0, width, height - radius * 2, height, @selector(adjustTop:),
       [NSCursor resizeUpDownCursor]},
      {0, width, 0, radius * 2, @selector(adjustBottom:),
       [NSCursor resizeUpDownCursor]}};
  return edges[i];
}
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  NSRect superviewsFrameInMyCoords =
      [self.superview convertRect:self.superview.bounds toView:self];
...
- (iTermBadgeViewEdge)edgeAtPoint:(NSPoint)locationInWindow {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (point.x >= edge.minX && point.x < edge.maxX && point.y >= edge.minY &&
        point.y < edge.maxY) {
      return edge;
    }
  }
...
- (void)mouseDragged:(NSEvent *)event {
  ...
  _point = point;
  [self it_performNonObjectReturningSelector:_selector
                                  withObject:[NSValue valueWithSize:delta]];
  [[NSCursor closedHandCursor] set];
}
...
- (void)adjustLeft:(NSValue *)value {
  ...
  self.frame = frame;
  _point = [self.superview convertPoint:NSMakePoint(NSMinX(frame), _point.y)
                                 toView:nil];
}
...
- (void)adjustRight:(NSValue *)value {
  ...
  const CGFloat upperBound =
      NSWidth(self.superview.bounds) + iTermBadgeConfigurationBadgeViewInset;
  ...
  self.frame = frame;
  _point = [self.superview convertPoint:NSMakePoint(NSMaxX(frame), _point.y)
                                 toView:nil];
}
...
- (void)adjustTop:(NSValue *)value {
  ...
  const CGFloat upperBound =
      NSHeight(self.superview.bounds) + iTermBadgeConfigurationBadgeViewInset;
  ...
  self.frame = frame;
  _point = [self.superview convertPoint:NSMakePoint(_point.x, NSMaxY(frame))
                                 toView:nil];
}
...
- (void)adjustBottom:(NSValue *)value {
  ...
  self.frame = frame;
  _point = [self.superview convertPoint:NSMakePoint(_point.x, NSMinY(frame))
                                 toView:nil];
}
...
- (void)updateImageViewFrame {
  ...
  CGFloat imageAspectRatio =
      _loremIpsum.image.size.width / _loremIpsum.image.size.height;
  if (imageAspectRatio != imageAspectRatio ||
      _loremIpsum.image.size.height < 1) {
  ...
  if (imageAspectRatio > myAspectRatio) {
    ...
    // image is taller
    _loremIpsum.frame = NSMakeRect(
        myFrame.size.width - (myFrame.size.height * imageAspectRatio), 0,
...
- (NSSize)badgeLabelSizeFraction {
  const CGFloat width =
      MIN(0.95, self.bounds.size.width / self.superview.bounds.size.width);
  const CGFloat height =
      MIN(0.95, self.bounds.size.height / self.superview.bounds.size.height);
  return NSMakeSize(width, height);
}

@end

@interface iTermBadgeConfigurationWindowController () <
    iTermBadgeConfigurationBadgeViewDelegate, NSTextFieldDelegate,
...
@property(nonatomic, strong)
    IBOutlet iTermBadgeConfigurationBadgeView *badgeView;
@end
...
- (instancetype)initWithProfile:(Profile *)profile {
  ...
  if (self) {
    ...
    _fontName = [iTermProfilePreferences stringForKey:KEY_BADGE_FONT
                                            inProfile:profile];
...
- (void)windowDidLoad {
  [super windowDidLoad];
  const CGFloat rightMargin =
      [iTermProfilePreferences doubleForKey:KEY_BADGE_RIGHT_MARGIN
                                  inProfile:_profile];
  const CGFloat topMargin =
      [iTermProfilePreferences doubleForKey:KEY_BADGE_TOP_MARGIN
                                  inProfile:_profile];
  const CGFloat maxWidth =
      [iTermProfilePreferences doubleForKey:KEY_BADGE_MAX_WIDTH
                                  inProfile:_profile];
  const CGFloat maxHeight =
      [iTermProfilePreferences doubleForKey:KEY_BADGE_MAX_HEIGHT
                                  inProfile:_profile];
  ...
  _fakeSessionView.fillColor =
      [[iTermProfilePreferences objectForKey:KEY_BACKGROUND_COLOR
...
- (void)setBadgeFrameFromTextFields {
  ...
  const CGFloat right =
      round(containerSize.width - _rightMarginTextField.integerValue);
  const CGFloat top =
      round(containerSize.height - _topMarginTextField.integerValue);
  const CGFloat height = round(
      MIN(95, _maxHeightTextField.integerValue / 100.0) * containerSize.height);
  const CGFloat width = round(MIN(95, _maxWidthTextField.integerValue / 100.0) *
                              containerSize.width);
  NSRect insetFrame = NSMakeRect(right - width, top - height, width, height);
  _badgeView.frame =
      NSInsetRect(insetFrame, -iTermBadgeConfigurationBadgeViewInset,
                  -iTermBadgeConfigurationBadgeViewInset);
}
...
- (void)badgeViewFrameDidChange:(NSRect)frameIncludingInsets {
  ...
  NSRect frame =
      NSInsetRect(frameIncludingInsets, iTermBadgeConfigurationBadgeViewInset,
                  iTermBadgeConfigurationBadgeViewInset);
  const NSSize containerSize = self.badgeContainerSize;
  _maxWidthTextField.integerValue =
      MIN(95, 100.0 * NSWidth(frame) / containerSize.width);
  _maxHeightTextField.integerValue =
      MIN(95, 100.0 * NSHeight(frame) / containerSize.height);
  _topMarginTextField.integerValue =
      MAX(0, containerSize.height - NSMaxY(frame));
  _rightMarginTextField.integerValue =
      MAX(0, containerSize.width - NSMaxX(frame));
}
...
- (void)fontPickerCompositeView:(BFPCompositeView *_Nonnull)view
                  didSelectFont:(NSFont *_Nonnull)selectedFont {
