...
@interface iTermQuickLookController () <QLPreviewPanelDataSource,
...
- (void)showWithSourceRect:(NSRect)sourceRect controller:(id)controller {
  ...
  // This undocumented API makes the quicklook window appear beneath the cursor.
  // It doesn't seem to matter what the item is. NSURL implements QLPreviewItem
  // so it's as good as anything.
  if ([panel respondsToSelector:@selector(setPositionNearPreviewItem:)]) {
    [panel
        setPositionNearPreviewItem:[NSURL
