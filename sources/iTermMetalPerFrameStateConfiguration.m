...
- (void)loadSettingsWithDrawingHelper:(iTermTextDrawingHelper *)drawingHelper
                             textView:(PTYTextView *)textView
                                 glue:
                                     (id<iTermMetalPerFrameStateDelegate>)glue {
  ...
  _gridSize =
      VT100GridSizeMake(textView.dataSource.width, textView.dataSource.height);
  ...
  _shouldDrawFilledInCursor =
      ([textView.delegate textViewShouldDrawFilledInCursor] ||
       textView.keyFocusStolenCount);
  _blinkAllowed = textView.blinkAllowed;
  _blinkingItemsVisible = drawingHelper.blinkingItemsVisible;
  const BOOL forceAA =
      (drawingHelper.forceAntialiasingOnRetina && drawingHelper.isRetina);
  _asciiAntialias = drawingHelper.asciiAntiAlias || forceAA;
  _nonasciiAntialias =
      (_useNonAsciiFont ? drawingHelper.nonAsciiAntiAlias : _asciiAntialias) ||
      forceAA;
  ...
  _isFrontTextView =
      (textView == [[iTermController sharedInstance] frontTextView]);
  _unfocusedSelectionColor = VectorForColor(
      [[_colorMap colorForKey:kColorMapSelection] colorDimmedBy:2.0 / 3.0
                                               towardsGrayLevel:0.5]);
  _transparencyAlpha = textView.transparencyAlpha;
  _transparencyAffectsOnlyDefaultBackgroundColor =
      drawingHelper.transparencyAffectsOnlyDefaultBackgroundColor;
  ...
  _asciiUnderlineDescriptor.color =
      VectorForColor([_colorMap colorForKey:kColorMapUnderline]);
  _asciiUnderlineDescriptor.offset =
      [drawingHelper yOriginForUnderlineForFont:_asciiFont.font
                                        yOffset:0
                                     cellHeight:_cellSize.height];
  _asciiUnderlineDescriptor.thickness =
      [drawingHelper underlineThicknessForFont:_asciiFont.font];

  if (_useNonAsciiFont) {
    _nonAsciiUnderlineDescriptor.color = _asciiUnderlineDescriptor.color;
    _nonAsciiUnderlineDescriptor.offset =
        [drawingHelper yOriginForUnderlineForFont:_nonAsciiFont.font
                                          yOffset:0
                                       cellHeight:_cellSize.height];
    _nonAsciiUnderlineDescriptor.thickness =
        [drawingHelper underlineThicknessForFont:_nonAsciiFont.font];
  } else {
  ...
  _strikethroughUnderlineDescriptor.offset =
      [drawingHelper yOriginForStrikethroughForFont:_asciiFont.font
                                            yOffset:0
                                         cellHeight:_cellSize.height];
  _strikethroughUnderlineDescriptor.thickness =
      [drawingHelper strikethroughThicknessForFont:_asciiFont.font];

  // Indicators
  NSColor *color = [[textView indicatorFullScreenFlashColor]
      colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
  _fullScreenFlashColor = simd_make_float4(
