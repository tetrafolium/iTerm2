...
static NSString *const iTermTouchBarIdentifierAddMark =
    @"iTermTouchBarIdentifierAddMark";
static NSString *const iTermTouchBarIdentifierNextMark =
    @"iTermTouchBarIdentifierNextMark";
static NSString *const iTermTouchBarIdentifierPreviousMark =
    @"iTermTouchBarIdentifierPreviousMark";
static NSString *const iTermTouchBarIdentifierManPage =
    @"iTermTouchBarIdentifierManPage";
static NSString *const iTermTouchBarIdentifierColorPreset =
    @"iTermTouchBarIdentifierColorPreset";
static NSString *const iTermTouchBarIdentifierFunctionKeys =
    @"iTermTouchBarIdentifierFunctionKeys";
static NSString *const iTermTouchBarIdentifierColorPresetScrollview =
    @"iTermTouchBarIdentifierColorPresetScrollview";
static NSString *const iTermTouchBarIdentifierAutocomplete =
    @"iTermTouchBarIdentifierAutocomplete";
static NSString *const iTermTouchBarFunctionKeysScrollView =
    @"iTermTouchBarFunctionKeysScrollView";
static NSString *const iTermTouchBarIdentifierStatus =
...
- (void)updateTouchBarFunctionKeyLabels {
  ...
  NSTouchBarItem *item =
      [self.touchBar itemForIdentifier:iTermTouchBarFunctionKeysScrollView];
  ...
  NSPopoverTouchBarItem *popoverItem =
      [self.touchBar itemForIdentifier:iTermTouchBarIdentifierFunctionKeys];
  NSTouchBar *popoverTouchBar = popoverItem.popoverTouchBar;
  item =
      [popoverTouchBar itemForIdentifier:iTermTouchBarFunctionKeysScrollView];
...
- (void)updateTouchBarWithWordAtCursor:(NSString *)word {
  if (IsTouchBarAvailable() && [self respondsToSelector:@selector(touchBar)]) {
    NSTouchBarItem *item =
        [self.touchBar itemForIdentifier:iTermTouchBarIdentifierManPage];
    if (item) {
...
- (void)updateStatus {
  NSTouchBarItem *item =
      [self.touchBar itemForIdentifier:iTermTouchBarIdentifierStatus];
  if (item) {
    iTermTouchBarButton *button = (iTermTouchBarButton *)item.view;
...
- (NSTouchBar *)makeGenericTouchBar {
  ...
  touchBar.defaultItemIdentifiers = @[
    iTermTouchBarIdentifierManPage, iTermTouchBarIdentifierColorPreset,
    iTermTouchBarIdentifierFunctionKeys, NSTouchBarItemIdentifierFlexibleSpace,
    NSTouchBarItemIdentifierOtherItemsProxy, iTermTouchBarIdentifierAddMark,
    iTermTouchBarIdentifierPreviousMark, iTermTouchBarIdentifierNextMark
  ];
  return touchBar;
}
...
- (void)updateTouchBarIfNeeded:(BOOL)force {
  ...
  if (IsTouchBarAvailable()) {
    ...
    if (force || ![replacement.customizationIdentifier
                     isEqualToString:self.touchBar.customizationIdentifier]) {
    ...
    NSArray *ids = @[
      iTermTouchBarIdentifierManPage, iTermTouchBarIdentifierColorPreset,
      iTermTouchBarIdentifierFunctionKeys, iTermTouchBarFunctionKeysScrollView,
      NSTouchBarItemIdentifierFlexibleSpace, iTermTouchBarIdentifierAddMark,
      iTermTouchBarIdentifierNextMark, iTermTouchBarIdentifierPreviousMark,
      iTermTouchBarIdentifierAutocomplete, iTermTouchBarIdentifierStatus
    ];
    NSArray<iTermTouchbarItem *> *items = [iTermTouchbarMappings
        sortedTouchbarItemsInDictionary:[iTermTouchbarMappings
                                            globalTouchBarMap]];
    NSArray<NSString *> *customIDs =
        [items mapWithBlock:^id(iTermTouchbarItem *anObject) {
...
- (NSNumber *)versionFor:(NSString *)identifier
               withValue:(NSDictionary *)binding {
  assert(binding);
...
- (NSArray<NSString *> *)idsByAddingVersions:
    (NSArray<NSString *> *)unversioned {
  ...
  return [unversioned mapWithBlock:^id(NSString *string) {
    ...
    return [string stringByAppendingFormat:@"/v%@", [self versionFor:string
                                                           withValue:binding]];
  }];
}
...
- (NSTouchBar *)amendTouchBar:(NSTouchBar *)touchBar {
  ...
  if (self.anyFullScreen) {
    NSMutableArray *temp =
        [[touchBar.defaultItemIdentifiers mutableCopy] autorelease];
    NSInteger index =
        [temp indexOfObject:NSTouchBarItemIdentifierOtherItemsProxy];
    if (index != NSNotFound) {
    ...
    touchBar.customizationAllowedItemIdentifiers =
        [touchBar.customizationAllowedItemIdentifiers
            arrayByAddingObjectsFromArray:@[ iTermTabBarTouchBarIdentifier ]];
  }
  return touchBar;
}
...
- (void)updateManPageButton:(iTermTouchBarButton *)button
  ...
  if (word) {
    if (![button.title isEqualToString:word]) {
      ...
      button.enabled = YES;
      NSString *manCommand = [NSString
          stringWithFormat:
              [iTermAdvancedSettingsModel viewManPageCommand],
...
- (NSTouchBarItem *)functionKeysTouchBarItem {
  ...
  NSCustomTouchBarItem *item = [[[NSCustomTouchBarItem alloc]
      initWithIdentifier:iTermTouchBarFunctionKeysScrollView] autorelease];
  ...
  for (NSInteger n = 1; n <= iTermTouchBarNumberOfFunctionKeys; n++) {
    NSString *label = [NSString stringWithFormat:@"F%@", @(n)];
    iTermTouchBarButton *button = [iTermTouchBarButton
        buttonWithTitle:label
                 target:self
                 action:@selector(functionKeyTouchBarItemSelected:)];
    button.tag = n;
    [button sizeToFit];
    [documentView addSubview:button];
    button.translatesAutoresizingMaskIntoConstraints = NO;
    [self addConstraintsToButton:button
                       superView:documentView
                        previous:previous];
    previous = button;
  }
  if (previous) {
...
- (void)addConstraintsToButton:(iTermTouchBarButton *)button
                     superView:(NSView *)documentView
                      previous:(NSButton *)previous {
  if (previous == nil) {
    // Constrain the first item's left to the document view's left
    [documentView addConstraint:[NSLayoutConstraint
...
- (void)constrainButton:(NSButton *)previous
     toRightOfSuperview:(NSView *)documentView {
  // Constrain button's right to document view's right
  [documentView addConstraint:[NSLayoutConstraint
                                  constraintWithItem:previous
                                           attribute:NSLayoutAttributeRight
                                           relatedBy:NSLayoutRelationEqual
                                              toItem:documentView
                                           attribute:NSLayoutAttributeRight
...
- (void)addButtonsToColorPresetsDocumentView:(NSView *)documentView {
  ...
  for (NSDictionary *dict in @[
         [iTermColorPresets builtInColorPresets] ?: @{},
    ...
    for (NSString *name in dict) {
      ...
      textColor = [ITAddressBookMgr
          decodeColor:[dict objectForKey:name][KEY_FOREGROUND_COLOR]];
      if (!textColor) {
        continue;
      }
      backgroundColor = [ITAddressBookMgr
          decodeColor:[dict objectForKey:name][KEY_BACKGROUND_COLOR]];
      NSDictionary *attributes = @{NSForegroundColorAttributeName : textColor};
      NSAttributedString *title =
          [[[NSAttributedString alloc] initWithString:name
                                           attributes:attributes] autorelease];
      button = [iTermTouchBarButton
          buttonWithTitle:@""
                   target:self
...
- (NSTouchBarItem *)colorPresetsScrollViewTouchBarItem {
  ...
  NSScrollView *scrollView = [[[NSScrollView alloc] init] autorelease];
  NSCustomTouchBarItem *item = [[[NSCustomTouchBarItem alloc]
      initWithIdentifier:iTermTouchBarIdentifierColorPresetScrollview]
      autorelease];
  item.view = scrollView;
  NSView *documentView = [[[NSView alloc] init] autorelease];
  documentView.translatesAutoresizingMaskIntoConstraints = NO;
...
- (nullable NSTouchBarItem *)touchBar:(NSTouchBar *)touchBar
  ...
  if ([identifier isEqualToString:iTermTabBarTouchBarIdentifier]) {
    ...
    if (!self.tabsTouchBarItem) {
      self.tabsTouchBarItem = [[[NSCustomTouchBarItem alloc]
      ...
      [scrubber registerClass:[NSScrubberTextItemView class]
            forItemIdentifier:iTermTabBarItemTouchBarIdentifier];

      // Use the flow layout.
      NSScrubberLayout *scrubberLayout =
          [[[NSScrubberFlowLayout alloc] init] autorelease];
      scrubber.scrubberLayout = scrubberLayout;

      scrubber.mode = NSScrubberModeFree;

      NSScrubberSelectionStyle *outlineStyle =
    ...
    // Reload the scrubber after a spin of the runloop because it gets laid out
    // tighter after the rest of the toolbar is created. If we reloadData now
    // then it jumps the first time we change tabs.
    dispatch_async(dispatch_get_main_queue(), ^{
      [scrubber reloadData];
      [scrubber setSelectedIndex:[self.tabs indexOfObject:self.currentTab]];
    });
    ...
    self.autocompleteCandidateListItem = [[[NSCandidateListTouchBarItem alloc]
        initWithIdentifier:identifier] autorelease];
    self.autocompleteCandidateListItem.delegate = self;
    self.autocompleteCandidateListItem.customizationLabel =
        @"Autocomplete Suggestions";
    NSAttributedString * (^commandUseToAttributedString)(NSString *commandUse,
                                                         NSInteger index) =
        ^(NSString *command, NSInteger index) {
          return [[[NSAttributedString alloc] initWithString:command ?: @""]
              autorelease];
        };
    self.autocompleteCandidateListItem.attributedStringForCandidate =
        commandUseToAttributedString;
  ...
  if ([identifier isEqualToString:iTermTouchBarIdentifierManPage]) {
    ...
    image = [[NSImage it_imageNamed:@"Add Mark Touch Bar Icon"
                           forClass:self.class]
        imageWithColor:[NSColor labelColor]];
    ...
    image = [NSImage it_imageNamed:@"Touch Bar Function Keys"
                          forClass:self.class];
    NSPopoverTouchBarItem *item = [[[NSPopoverTouchBarItem alloc]
        initWithIdentifier:identifier] autorelease];
    ...
    functionKeys.defaultItemIdentifiers =
        @[ iTermTouchBarFunctionKeysScrollView ];
  ...
  if (image || label) {
    iTermTouchBarButton *button;
    if (image) {
      button = [iTermTouchBarButton buttonWithImage:image
                                             target:self
                                             action:selector];
    } else {
      button = [iTermTouchBarButton buttonWithTitle:label
                                             target:self
                                             action:selector];
    }
    NSCustomTouchBarItem *item = [[[NSCustomTouchBarItem alloc]
    ...
    if ([identifier isEqualToString:iTermTouchBarIdentifierManPage]) {
      button.title = @"";
      button.image = [NSImage it_imageNamed:@"Man Page Touch Bar Icon"
                                   forClass:self.class];
      button.imagePosition = NSImageOnly;
      button.enabled = NO;
    }
  ...
  iTermKeyBindingAction *action =
      [iTermKeyBindingAction withDictionary:binding];
  if (!action) {
    return nil;
  }
  NSCustomTouchBarItem *item = [[[NSCustomTouchBarItem alloc]
      initWithIdentifier:identifier] autorelease];
  iTermTouchBarButton *button =
      [iTermTouchBarButton buttonWithTitle:action.label
                                    target:self
...
- (void)touchBarItemSelected:(iTermTouchBarButton *)sender {
  NSDictionary *map = [iTermTouchbarMappings globalTouchBarMap];
  NSString *unversionedIdentifier =
      [self idByRemovingVersion:sender.identifier];
  NSDictionary *binding =
      unversionedIdentifier ? map[unversionedIdentifier] : nil;
  if (!binding) {
  ...
  [self.currentSession performKeyBindingAction:action
...
- (void)colorPresetTouchBarItemSelected:(iTermTouchBarButton *)sender {
  [self.currentSession
      setColorsFromPresetNamed:sender.keyBindingAction[@"presetName"]];
}
...
- (void)sendFunctionKeyToCurrentSession:(NSInteger)number {
  ...
  unsigned short keyCodes[iTermTouchBarNumberOfFunctionKeys] = {
      ...
      // There is no virtual keycode for these function keys, but it doesn't
  ...
  NSString *chars = [NSString
      stringWithFormat:@"%C", (unichar)(NSF1FunctionKey + number - 1)];
  NSPoint screenPoint = [NSEvent mouseLocation];
  NSEvent *event = [NSEvent
                 keyEventWithType:NSEventTypeKeyDown
                         location:[self.window
                                      convertRectFromScreen:NSMakeRect(
                                                                screenPoint.x,
                                                                screenPoint.y,
                                                                0, 0)]
                                      .origin
  ...
  if ([app routeEventToShortcutInputView:event]) {
    return;
             ...
             self.window.firstResponder == self.currentSession.textview) {
...
- (void)candidateListTouchBarItem:(NSCandidateListTouchBarItem *)anItem
  ...
  if (index != NSNotFound) {
    ...
    if (prefix.length == 0 || [command hasPrefix:prefix]) {
      [self.currentSession
          insertText:[command substringFromIndex:prefix.length]];
    }
  }
}
...
- (__kindof NSScrubberItemView *)scrubber:(NSScrubber *)scrubber
  ...
  NSScrubberTextItemView *itemView =
      [scrubber makeItemWithIdentifier:iTermTabBarItemTouchBarIdentifier
                                 owner:nil];
  itemView.textField.stringValue =
      [self scrubber:scrubber labelAtIndex:index] ?: @"";
  return itemView;
}
...
- (NSSize)scrubber:(NSScrubber *)scrubber
  ...
  NSRect textRect =
      [title boundingRectWithSize:size
                          options:NSStringDrawingUsesLineFragmentOrigin |
                                  NSStringDrawingUsesFontLeading
                       attributes:@{
                         NSFontAttributeName : [NSFont systemFontOfSize:0]
                       }];
  // Apple says: "+10: NSTextField horizontal padding, no good way to retrieve
  // this though. +6 for spacing." 8 is because the items become smaller the
  // first time you change tabs for some mysterious reason and that leaves
  // enough room for them. :( The 30 is also Apple's magic number.
