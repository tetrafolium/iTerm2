...
+ (void)addCommandToMostRecentlyUsed:(NSString *)command {
  ...
  NSArray *oldMru = [[NSUserDefaults standardUserDefaults]
      stringArrayForKey:kCoprocessMruKey];
  ...
  [[NSUserDefaults standardUserDefaults] setObject:newMru
                                            forKey:kCoprocessMruKey];
}
...
+ (void)setSilentlyIgnoreErrors:(BOOL)shouldIgnore
                    fromCommand:(NSString *)command {
  NSArray *array =
      [[NSUserDefaults standardUserDefaults]
          objectForKey:iTermCoprocessCommandsToIgnoreErrorOutputPrefsKey]
          ?: @[];
  if (shouldIgnore) {
  ...
  [[NSUserDefaults standardUserDefaults]
      setObject:array
         forKey:iTermCoprocessCommandsToIgnoreErrorOutputPrefsKey];
}
...
- (void)monitorErrorsOnFileDescriptor:(int)errorFd {
  dispatch_queue_t queue =
      dispatch_queue_create("com.iterm2.coprocess-errors", 0);
...
- (BOOL)wantToWrite {
  return self.pid >= 0 && !eof_ && !writePipeClosed_ &&
