...
// Relinquished buffers will not automatically be returned to the pool after the
// frame is done.
...
// Keeps up to `capacity` buffers in the pool. If it exceeds that size the
// smallest ones are released. This might use more texture memory than necessary
// but it will stabilize quickly and stop creating new buffers, which is
// terribly slow.
NS_CLASS_AVAILABLE(10_11, NA)
