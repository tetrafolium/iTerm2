...
// Respect System Prefs "dock" setting to open tabs instead of new windows?
// Defaults to NO.
...
@property(nullable, nonatomic, copy) void (^makeSession)
    (Profile *profile, PseudoTerminal *windowController,
     void (^completion)(PTYSession *_Nullable));

// Early notification of session creation, possibly called before the job is
// launched.
@property(nullable, nonatomic, copy) void (^didCreateSession)
    (PTYSession *_Nullable session);

// Called on completion. ok gives whether it succeeded.
@property(nullable, nonatomic, copy) void (^completion)
    (PTYSession *session, BOOL ok);
...
// These class methods are a migration path for legacy code that predates the
// session launcher.

// Super-flexible way to create a new window or tab. If |block| is given then it
// is used to add a new session/tab to the window; otherwise the bookmark is
// used in conjunction with the optional URL.
+ (void)launchBookmark:(nullable NSDictionary *)bookmarkData
    ...
    respectTabbingMode:(BOOL)respectTabbingMode
           ...
           makeSession:
               (void (^_Nullable)(Profile *profile,
