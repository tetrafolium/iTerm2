...
// Used in preferenceDidChangeFromOtherPanel:'s notification's user info
// dictionary.
extern NSString *const kPreferenceDidChangeFromOtherPanelKeyUserInfoKey;

// View controllers for tabs in the Preferences dialog inherit from this class.
// Consider it abstract. The pattern is to call -defineControl:key:type: in
// -awakeFromNib for each control. In IB, assign all controls the
// -settingChanged: selector, and for text fields, make your view controller the
// delegate.
@interface iTermPreferencesBaseViewController
    : NSViewController <iTermSearchableViewController, NSTabViewDelegate>
...
// Define a control with a custom settingChanged and update handler. If they're
// both not null then the default value is not type checked.
- (PreferenceInfo *)defineControl:(NSControl *)control
                              key:(NSString *)key
                      relatedView:(NSView *)relatedView
                             type:(PreferenceInfoType)type
...
// Standard selector invoked by controls when their values changed, bound in XIB
// file.
...
// Just a convenience method in the base class, but subclasses can use this to
// be "more atomic".
- (void)setObjectsFromDictionary:(NSDictionary *)dictionary;

// By default, this class uses iTermPreferences class methods to change
// settings. Override these methods to use a different model.
...
// If this returns YES, then changes to this panel will post a notification
// causing other panels to update their values for the affected preference.
...
// The prefs panel this view controller belongs to will close. This
