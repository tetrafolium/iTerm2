...
- (BOOL)it_fontIsOnLigatureBlacklist {
  ...
  dispatch_once(&onceToken, ^{
    blacklist = [[NSSet setWithArray:@[
      @"AndaleMono", @"Courier", @"LetterGothicStd", @"Menlo", @"Monaco",
      @"OCRAStd", @"OratorStd", @"Osaka", @"PTMono", @"SFMono"
    ]] retain];
  });
...
- (BOOL)it_defaultLigatures {
  // Some fonts have great ligatures but unlike FiraCode you need to ask for
  // them. FiraCode gives you ligatures whether you like it or not.
...
- (CGFloat)computedBaselineOffset {
  if ([iTermAdvancedSettingsModel useExperimentalFontMetrics]) {
    ...
    NSLayoutManager *layoutManager =
        [FontSizeEstimator layoutManagerForFont:font_
                                  textContainer:textContainer];
    CGFloat lineHeight =
        [layoutManager usedRectForTextContainer:textContainer].size.height;
    CGFloat baselineOffsetFromTop =
        [layoutManager defaultBaselineOffsetForFont:font_];
...
// From
// https://github.com/DrawKit/DrawKit/blob/master/framework/Code/NSBezierPath%2BText.m#L648
- (CGFloat)computedUnderlineOffset {
  ...
  NSTextStorage *textStorage = [[[NSTextStorage alloc]
      initWithAttributedString:attributedString] autorelease];
...
// Issue 4294 reveals that merely upconverting the weight of a font once is not
// sufficient because it might go from Regular to Medium. You need to keep
// trying until you find a font that is relatively bold. This is a nice way to
// do it because the user could, e.g., pick a "thin" font and get the "medium"
// version for "bold" text. We convertWeight: until the weight is at least 4
// higher than the original font. See the table in the docs for
// convertWeight:ofFont: for what this means.
- (NSFont *)boldVersionOfFont:(NSFont *)font {
  NSFontManager *fontManager = [NSFontManager sharedFontManager];
  NSInteger weight = [fontManager weightOfFont:font];
  NSInteger minimumAcceptableWeight =
      weight + [iTermAdvancedSettingsModel minimumWeightDifferenceForBoldFont];
  DLog(@"Looking for a bold version of %@, whose weight is %@", font,
       @(weight));
  ...
  const NSFontTraitMask kImmutableTraits =
      (NSItalicFontMask | NSNarrowFontMask | NSExpandedFontMask |
       NSCondensedFontMask | NSSmallCapsFontMask | NSPosterFontMask |
       NSCompressedFontMask | NSFixedPitchFontMask | NSUnitalicFontMask);
  NSFontTraitMask requiredTraits =
      ([fontManager traitsOfFont:font] & kImmutableTraits);
  ...
  while (lastFont) {
    ...
    DLog(@"  next bolder font is %@ with a weight of %@", heavierFont,
         @(weight));
    NSFontTraitMask maskedTraits =
        ([fontManager traitsOfFont:heavierFont] & kImmutableTraits);
...
- (PTYFontInfo *)computedItalicVersion {
  NSFontManager *fontManager = [NSFontManager sharedFontManager];
  NSFont *italicFont = [fontManager convertFont:font_
