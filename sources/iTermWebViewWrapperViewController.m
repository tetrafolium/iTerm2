...
- (void)loadView {
  self.view =
      [[iTermFlippedView alloc] initWithFrame:NSMakeRect(0, 0, 800, 600)];
  ...
  if (_backupURL != nil) {
    ...
    [button
        setTitle:[NSString stringWithFormat:@"Open in %@", [self browserName]]];
  ...
  const NSRect frame = NSMakeRect(0, y, self.view.frame.size.width,
...
- (NSString *)browserName {
  ...
  NSURL *appUrl = (__bridge_transfer NSURL *)LSCopyDefaultApplicationURLForURL(
      (__bridge CFURLRef)URL, kLSRolesAll, &error);
  if (appUrl) {
    NSString *name = nil;
...
- (WKWebView *)webViewWithDelegate:(id<iTermWebViewDelegate>)delegate {
  ...
  Class WKWebViewConfigurationClass =
      NSClassFromString(@"WKWebViewConfiguration");
  WKWebViewConfiguration *configuration =
      [[WKWebViewConfigurationClass alloc] init];
  ...
  configuration.processPool =
      [[NSClassFromString(@"WKProcessPool") alloc] init];

  [self registerUserScriptInConfiguration:configuration delegate:delegate];
  configuration.websiteDataStore =
      [NSClassFromString(@"WKWebsiteDataStore") defaultDataStore];
  WKWebView *webView =
      [[WKWebViewClass alloc] initWithFrame:NSMakeRect(0, 0, 800, 600)
                              configuration:configuration];
  [configuration.userContentController
      it_setWeakAssociatedObject:webView
...
- (void)registerUserScriptInConfiguration:
            (WKWebViewConfiguration *)configuration
                                 delegate:(id<iTermWebViewDelegate>)delegate {
  WKUserContentController *userController =
      [[WKUserContentController alloc] init];
  [userController
      it_setWeakAssociatedObject:delegate
                          forKey:&iTermWebViewFactoryUserControllerDelegateKey];
  [userController addScriptMessageHandler:self name:@"iterm2Invoke"];

  NSURL *url =
      [[NSBundle bundleForClass:[self class]] URLForResource:@"iterm2Invoke"
                                               withExtension:@"js"];
  ...
  // Specify when and where and what user script needs to be injected into the
  // web document
...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  if ([message.name isEqualToString:@"iterm2Invoke"]) {
    __weak id<iTermWebViewDelegate> delegate = [userContentController
        it_associatedObjectForKey:&
                                  iTermWebViewFactoryUserControllerDelegateKey];
    ...
    WKWebView *webview = [userContentController
        it_associatedObjectForKey:&iTermWebViewFactoryUserControllerWebviewKey];
    if (!webview) {
      assert(NO);
      return;
    }
    ...
    if (!invocation) {
      [self sendReturnValue:nil
                 forMessage:dict
                  toWebview:webview
                 completion:nil];
      [delegate
          itermWebViewScriptInvocation:nil
                      didFailWithError:
                          [NSError
                              errorWithDomain:iTermWebViewErrorDomain
                                         code:
                                             iTermWebViewErrorCodeMissingInvocation
                                     userInfo:nil]];
      return;
    }
    iTermVariableScope *scope = [delegate
        itermWebViewScriptScopeForUserContentController:userContentController];
    if (!scope) {
      NSError *error =
          [NSError errorWithDomain:iTermWebViewErrorDomain
                              code:iTermWebViewErrorCodeReceiverDealloced
                          userInfo:nil];
      [self sendReturnValue:error
                 forMessage:dict
                  toWebview:webview
                 completion:nil];
      return;
...
- (void)sendReturnValue:(id)value
  ...
  if (error) {
    script = innerScript = [NSString
        stringWithFormat:@"throw %@",
                         [NSJSONSerialization
                             it_jsonStringForObject:error
                                                        .localizedDescription]];
  } else {
    innerScript = [NSString
        stringWithFormat:@"%@(%@)", callback,
                         [NSJSONSerialization it_jsonStringForObject:value]];
    script = [NSString
        stringWithFormat:@"try { %@; } catch(err) { alert('Caught error while "
                         @"sending return value to javascript: ' + err.message "
                         @"+ '\\n' + err.stack); }",
                         innerScript];
  }
  __weak id<iTermWebViewDelegate> delegate = [webview.configuration
       ...
       completionHandler:^(id _Nullable value, NSError *_Nullable error) {
         ...
         if (error.userInfo[messageKey]) {
           description = [NSString
               stringWithFormat:@"Error evaluating '%@' at line %@: %@",
                                innerScript, error.userInfo[lineKey],
                                error.userInfo[messageKey]];
         }
         completion([NSError errorWithDomain:iTermWebViewErrorDomain
