...
- (NSString *)description {
  ...
  if (_asciiData.length) {
    [params appendFormat:@" asciiData=“%.*s”", _asciiData.length,
                         _asciiData.buffer];
  }
  return [NSString stringWithFormat:@"<%@: %p type=%@%@>", self.class, self,
                                    [self codeName], params];
}
...
// There is a lot more that can be done to perform this translation but
// remapping italic->inverse is particularly visible.
