...
+ (BOOL)removeLegacyAppSupportFolderIfPossible {
  ...
  if (foundVersionTxt) {
    NSError *error = nil;
    [fileManager
        removeItemAtPath:[legacy stringByAppendingPathComponent:@"version.txt"]
                   error:&error];
    if (error) {
      return NO;
    }
...
+ (void)migrateApplicationSupportDirectoryIfNeeded {
  ...
  if ([fileManager itemIsDirectory:modern] &&
      [fileManager itemIsDirectory:legacy]) {
    ...
    if (legacyEmpty) {
      ...
      [fileManager createSymbolicLinkAtPath:legacy
                        withDestinationPath:modern
                                      error:nil];
      return;
    }
    ...
    alert.informativeText =
        ...
        @"Previously, both these directories were "
        @"supported:\n~/Library/Application "
        @"Support/iTerm\n~/Library/Application Support/iTerm2.\n\n"
        @"Now, only the iTerm2 version is supported. But you have files in "
        @"both so please move everything from iTerm to iTerm2.";
    ...
    iTermDisclosableView *accessory =
        [[iTermDisclosableView alloc] initWithFrame:NSZeroRect
...
+ (void)copyProfileToBookmark:(NSMutableDictionary *)dict {
  NSString *plistFile =
      [[NSBundle bundleForClass:[self class]] pathForResource:@"MigrationMap"
                                                       ofType:@"plist"];
  ...
  for (int i = 0; i < [xforms count]; ++i) {
    ...
    if ([prefix isEqualToString:@"Terminal"]) {
      parent = [terminalProfiles
          objectForKey:[dict objectForKey:KEY_TERMINAL_PROFILE]];
    } else if ([prefix isEqualToString:@"Displays"]) {
      parent = [displayProfiles
          objectForKey:[dict objectForKey:KEY_DISPLAY_PROFILE]];
    } else if ([prefix isEqualToString:@"KeyBindings"]) {
      parent = [keybindingProfiles
          objectForKey:[dict objectForKey:KEY_KEYBOARD_PROFILE]];
    } else {
...
+ (void)recursiveMigrateBookmarks:(NSDictionary *)node path:(NSArray *)path {
  ...
  if ([data objectForKey:KEY_COMMAND_LINE]) {
    ...
    [temp setObject:kProfilePreferenceCommandTypeCustomValue
