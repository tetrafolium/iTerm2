...
- (void)drawInContext:(NSGraphicsContext *)context frame:(NSRect)frame {
  [_rows enumerateObjectsUsingBlock:^(iTermTimestampRow *_Nonnull row,
...
- (void)drawBackgroundInFrame:(NSRect)frame
  ...
  NSGradient *gradient = [[NSGradient alloc]
      initWithStartingColor:[bgColor colorWithAlphaComponent:0]
  ...
  [gradient drawInRect:gradientFrame angle:0];
...
- (void)drawString:(NSString *)s row:(int)index frame:(NSRect)frame {
  NSDictionary *attributes =
      [self attributesForTextColor:_fgColor
...
- (NSRect)backgroundFrameForTextFrame:(NSRect)frame {
  return NSMakeRect(NSMinX(frame) - iTermTimestampGradientWidth, NSMinY(frame),
...
- (NSRect)frameForString:(NSString *)s line:(int)line maxX:(CGFloat)maxX {
  NSString *widest =
      [s stringByReplacingOccurrencesOfRegex:@"[\\d\\p{Alphabetic}]"
                                  withString:@"M"];
  NSSize size = [widest sizeWithAttributes:@{
    NSFontAttributeName : [NSFont
        systemFontOfSize:[iTermAdvancedSettingsModel pointSizeOfTimeStamp]]
  }];
...
- (NSString *)stringForTimestamp:(NSDate *)timestamp
                             now:(NSTimeInterval)now
              useTestingTimezone:(BOOL)useTestingTimezone {
  const NSTimeInterval timeDelta =
      timestamp.timeIntervalSinceReferenceDate - now;
...
- (NSDictionary *)attributesForTextColor:(NSColor *)fgColor
  ...
  if (retina) {
    attributes = @{
      NSFontAttributeName :
          [NSFont userFixedPitchFontOfSize:[iTermAdvancedSettingsModel
                                               pointSizeOfTimeStamp]],
      NSForegroundColorAttributeName : fgColor,
      NSShadowAttributeName : shadow
    };
  } else {
    NSFont *font = [NSFont userFixedPitchFontOfSize:[iTermAdvancedSettingsModel
                                                        pointSizeOfTimeStamp]];
    font = [[NSFontManager sharedFontManager] fontWithFamily:font.familyName
                                                      ...
                                                      weight:0
                                                        size:font.pointSize];
    attributes =
        @{NSFontAttributeName : font, NSForegroundColorAttributeName : fgColor};
  }
