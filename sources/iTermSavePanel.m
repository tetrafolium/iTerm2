...
+ (NSString *)keyForIdentifier:(NSString *)identifier {
  return [NSString
      stringWithFormat:@"NoSyncSavePanelSavedSettings_%@", identifier];
}
...
+ (NSString *)nameForFileType:(NSString *)extension {
  ...
  NSString *lowercaseDescription =
      (__bridge_transfer NSString *)UTTypeCopyDescription(fileUTI);

  CFRelease(fileUTI);

  lowercaseDescription = [lowercaseDescription stringByCapitalizingFirstLetter];
  NSRange range = [lowercaseDescription
      rangeOfString:[NSString stringWithFormat:@"(%@)", extension]];
...
+ (iTermSavePanelFileFormatAccessory *)
    newFileFormatAccessoryViewControllerFileWithFileTypes:
        (NSArray<NSString *> *)fileTypes {
  iTermSavePanelFileFormatAccessory *accessory =
      [[iTermSavePanelFileFormatAccessory alloc]
          initWithNibName:@"iTermSavePanelFileFormatAccessory"
...
+ (iTermSavePanel *)showWithOptions:(NSInteger)options
  ...
  if (options & kSavePanelOptionFileFormatAccessory) {
    accessoryViewController = [self
        newFileFormatAccessoryViewControllerFileWithFileTypes:allowedFileTypes];
  ...
  do {
    ...
    if (delegate.forcedExtension) {
      URL = [[URL URLByDeletingPathExtension]
          URLByAppendingPathExtension:delegate.forcedExtension];
    }
    retrying = NO;
    if (options & kSavePanelOptionAppendOrReplace) {
      ...
      // The path contains random crap in the last path component. Use what we
      // saved instead.
      NSString *directory = [URL.path stringByDeletingLastPathComponent];
      delegate.path =
          [directory stringByAppendingPathComponent:delegate.filename];
      if (allowedFileTypes.count &&
          ![allowedFileTypes containsObject:delegate.path.pathExtension]) {
        delegate.path = [delegate.path
            stringByAppendingPathExtension:allowedFileTypes.firstObject];
      }
...
// Returns YES if the save panel should be shown again because the user
// canceled.
...
- (NSString *)panel:(id)sender
  ...
  if (_options & kSavePanelOptionAppendOrReplace) {
    ...
    // Save the file the user entered. He could've typed slashes, which the
    // system would turn into colons normally.
    self.filename = [filename stringByReplacingOccurrencesOfString:@"/"
                                                        withString:@":"];

    // Return a filename that definitely doesn't exist to avoid the Replace
    // panel.
...
- (BOOL)panel:(id)sender validateURL:(NSURL *)url error:(NSError **)outError {
  ...
  iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:@"You can choose to use both, so that "
                                     @"your file name ends in “.%@.%@”.",
                                     proposedExtension, _requiredExtension]
                   actions:@[
                     [NSString stringWithFormat:@"Use .%@", _requiredExtension],
                     @"Cancel", @"Use both"
                   ]
                 accessory:nil
                identifier:nil
               silenceable:kiTermWarningTypePersistent
                   heading:[NSString
                               stringWithFormat:
                                   @"You cannot save this document with "
  ...
  case kiTermWarningSelection2:
    self.forcedExtension = [NSString
        stringWithFormat:@"%@.%@", proposedExtension, _requiredExtension];
    break;
