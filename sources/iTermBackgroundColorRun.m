...
static void iTermMakeBackgroundColorRun(iTermBackgroundColorRun *run,
  ...
  if (matches) {
    ...
    run->isMatch =
        theIndex < [matches length] && (matchBytes[theIndex] & bitMask);
  } else {
    run->isMatch = NO;
  }
...
+ (void)addBackgroundRun:(iTermBackgroundColorRun *)run
                ...
                endingAt:(int)end { // end is the location after the last
                                    // location in the run
  ...
  // Add it to the array.
  iTermBoxedBackgroundColorRun *box =
      [[[iTermBoxedBackgroundColorRun alloc] init] autorelease];
...
+ (instancetype)backgroundRunsInLine:(screen_char_t *)theLine
  ...
  for (j = charRange.location; j < charRange.location + charRange.length; j++) {
    ...
    if (theLine[j].code == DWC_RIGHT) {
      x = j - 1;
      if (x < 0) {
        // AFAIK this only happens in tests, but it's a nice safety in case
        // things go sideways.
        continue;
      }
    }
    iTermMakeBackgroundColorRun(&current, theLine, VT100GridCoordMake(x, row),
                                extractor, selectedIndexes, matches, width);
    if (theLine[x].blink) {
      *anyBlinkPtr = YES;
    }
    if (first) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p line=%@ numberEquiv=%@ runs:%@>",
                                    self.class, self, @(self.line),
                                    @(self.numberOfEquivalentRows), self.array];
}
...
+ (instancetype)boxedBackgroundColorRunWithValue:
    (iTermBackgroundColorRun)value {
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p selected=%@ range=%@ backgroundColor=%@>",
                       self.class, self, @(_value.selected),
                       NSStringFromRange(_value.range), self.backgroundColor];
}
