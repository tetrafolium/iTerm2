...
//
// Scroll wheels on macOS are a total shitshow. If you want to understand them
// read anything but the documentation, which is worthless. Here are some useful
// things which kind of hint at what might be going on:
//
...
- (CGFloat)deltaYForEvent:(NSEvent *)event lineHeight:(CGFloat)lineHeight {
  ...
  case NSEventTypeScrollWheel: {
    ...
    DLog(@"deltaYForEvent:%@ lineHeight:%@ -> %@. deltaY=%@ scrollingDeltaY=%@ "
         @"Accumulator <- %@",
         event, @(lineHeight), @(result), @(event.deltaY),
         @(event.scrollingDeltaY), @(_accumulatedDeltaY));
    return result;
  }
...
// Get a delta Y out of the event with the most precision available and a
...
- (CGFloat)accumulatedDeltaYForTrackpadEvent:(NSEvent *)event {
  ...
  // If it is large (>1), return its integer part. This enables quick scroll,
  // which feels like natural trackpad. If `delta * _accumulatedDeltaY < 0`, it
  // means turnaround. Round delta to turn around quickly (fabs 0.5 is enough to
  // move). If it is not large enough, return 0 and keep accumulating.
...
- (CGFloat)legacyDeltaYForEvent:(NSEvent *)theEvent
