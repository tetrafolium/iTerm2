...
// This value plus 0...2^24-1 are accepted as read-only keys. These must be the
// highest-valued keys.
...
@implementation iTermColorMap {
  ...
  // Only 3 components are used here, but I'm paranoid screwing up and
  // overflowing.
...
+ (iTermColorMapKey)keyFor8bitRed:(int)red green:(int)green blue:(int)blue {
  return kColorMap24bitBase + ((red & 0xff) << 16) + ((green & 0xff) << 8) +
         (blue & 0xff);
}
...
- (void)setColor:(NSColor *)theColor forKey:(iTermColorMapKey)theKey {
  ...
  [theColor getComponents:components];
  vector_float4 value = {(float)components[0], (float)components[1],
...
// There is an issue where where the passed-in color can be in a different color
// space than the default background color. It doesn't make sense to combine RGB
// values from different color spaces. The effects are generally subtle.
...
- (vector_float4)fastAverageComponents:(vector_float4)rgb1
                                  with:(vector_float4)rgb2
                                 alpha:(float)alpha {
  vector_float4 result = {rgb1.x * (1 - alpha) + rgb2.x * alpha,
...
- (NSColor *)processedTextColorForTextColor:(NSColor *)textColor
  ...
  CGFloat dimmedRgb[4];
  CGFloat grayRgb[] = {_backgroundBrightness, _backgroundBrightness,
  ...
  if (_lastTextColor &&
      !memcmp(_lastTextComponents, dimmedRgb, sizeof(CGFloat) * 3)) {
...
- (NSColor *)colorByMutingColor:(NSColor *)color {
  ...
  vector_float4 colorVector = simd_make_float4(components[0], components[1],
...
+ (NSColor *)dimmedTextColor:(NSColor *)color
  ...
  CGFloat dimmedRgb[4];
  CGFloat grayRgb[] = {backgroundBrightness, backgroundBrightness,
...
- (vector_float4)fastProcessedBackgroundColorForBackgroundColor:
    (vector_float4)backgroundColor {
  vector_float4 defaultBackgroundComponents =
      [self fastColorForKey:kColorMapBackground];
  const vector_float4 mutedRgb =
      [self fastAverageComponents:backgroundColor
                             with:defaultBackgroundComponents
                            alpha:_mutingAmount];
  vector_float4 grayRgb = {0.5, 0.5, 0.5, 1};

  BOOL shouldDim = !_dimOnlyText && _dimmingAmount > 0;
  // If dimOnlyText is set then text and non-default background colors get
  // dimmed toward black.
  if (_dimOnlyText) {
    ...
    if (!isDefaultBackgroundColor) {
      grayRgb = (vector_float4){(float)_backgroundBrightness,
  ...
  if (shouldDim) {
    dimmedRgb = [self fastAverageComponents:mutedRgb
                                       with:grayRgb
                                      alpha:_dimmingAmount];
  } else {
    dimmedRgb = mutedRgb;
...
// space than the default background color. It doesn't make sense to combine RGB
// values from different color spaces. The effects are generally subtle.
- (NSColor *)processedBackgroundColorForBackgroundColor:
  ...
  if (!memcmp(_lastBackgroundComponents, dimmedRgb, sizeof(CGFloat) * 4)) {
    ...
    _lastBackgroundColor =
        [[NSColor colorWithColorSpace:backgroundColor.colorSpace
                           components:dimmedRgb
                                count:4] retain];
    return _lastBackgroundColor;
  }
...
- (id)copyWithZone:(NSZone *)zone {
  ...
  memmove(other->_lastTextComponents, _lastTextComponents,
          sizeof(_lastTextComponents));
  other->_lastTextColor = [_lastTextColor retain];

  memmove(other->_lastBackgroundComponents, _lastBackgroundComponents,
          sizeof(_lastBackgroundComponents));
