...
+ (iTermCursorNeighbors)
    neighborsForCursorAtCoord:(VT100GridCoord)cursorCoord
  ...
  for (int y = 0; y < 3; y++) {
    for (int x = 0; x < 3; x++) {
      if (neighbors.valid[y][x]) {
        [constraints
            addObject:@([self
                          brightnessOfCharBackground:neighbors.chars[y][x]])];
      }
    }
  }
...
- (NSColor *)textColorForCharacter:(screen_char_t)screenChar
  ...
  proposedForeground = [proposedForeground
...
// Return the value between 0 and 1 that is farthest from any value in
// 'constraints'.
- (CGFloat)farthestValueFromAnyValueIn:(NSArray *)constraints {
  ...
  NSArray *sortedConstraints =
      [constraints sortedArrayUsingSelector:@selector(compare:)];
...
- (NSColor *)
    overrideColorForSmartCursorWithForegroundColor:(NSColor *)proposedForeground
