...
- (instancetype)initWithChunkSize:(NSInteger)chunkSize
  ...
  if (self) {
    ...
    if ([iTermAdvancedSettingsModel includePasteHistoryInAdvancedPaste]) {
      NSArray<PasteboardEntry *> *historyEntries =
          [[PasteboardHistory sharedInstance] entries];
      if (historyEntries.count > 1) {
        for (PasteboardEntry *entry in historyEntries) {
          if (values.count && [values.lastObject isEqual:entry.mainValue]) {
            // Remove consecutive duplicates, which happen if you copy and then
            // paste.
            continue;
          }
          if (entry == historyEntries.lastObject) {
            NSString *pasteboardString = [NSString stringFromPasteboard];
            if (pasteboardString && entry.mainValue &&
                [pasteboardString isEqualToString:entry.mainValue]) {
              // Include the last entry only if it differs from the current
              // pasteboard contents.
              continue;
            }
          }
          ...
          if (title.length > kMaxLength) {
            title = [[title substringToIndex:kMaxLength]
    ...
    _pasteSpecialViewController =
        [[iTermPasteSpecialViewController alloc] init];
...
- (void)awakeFromNib {
  ...
  if ([iTermAdvancedSettingsModel includePasteHistoryInAdvancedPaste]) {
    [_labels enumerateObjectsUsingBlock:^(id _Nonnull label, NSUInteger idx,
                                          BOOL *_Nonnull stop) {
...
- (void)getLabels:(NSMutableArray *)labels andValues:(NSMutableArray *)values {
  ...
  for (NSPasteboardItem *item in pasteboard.pasteboardItems) {
    ...
    if (string && ![item stringForType:(NSString *)kUTTypeFileURL]) {
      ...
      NSString *label = [NSString
          stringWithFormat:@"%@: “%@”",
                           [((NSString *)description ?: @"Unknown Type")
  ...
  if (filenames.count == 1) {
    ...
    for (NSString *filename in filenames) {
      [modifiedFilenames
          addObject:[NSString stringWithFormat:@"\"%@\"", filename]];
    }
  }
  ...
  for (NSString *filename in filenames) {
    ...
    if ([fileManager fileExistsAtPath:filename isDirectory:&isDirectory] &&
      ...
      [values addObject:[[[iTermFileReference alloc] initWithName:filename]
                            autorelease]];
      [labels
          addObject:[NSString stringWithFormat:@"Contents of %@", filename]];
    }
  }
}
...
- (void)selectValueAtIndex:(NSInteger)index {
  ...
  if (isData) {
    ...
    // If the data happens to be valid UTF-8 data then don't insist on base64
    // encoding it.
  ...
  NSInteger tabTransformTag =
      [iTermPreferences intForKey:kPreferenceKeyPasteSpecialTabTransform];
  ...
  BOOL containsUnicodePunctuation =
      ([string
           rangeOfRegex:
               kPasteSpecialViewControllerUnicodePunctuationRegularExpression]
           .location != NSNotFound);
  BOOL convertValue = [iTermPreferences
      boolForKey:kPreferenceKeyPasteSpecialConvertDosNewlines];
  BOOL shouldEscape =
      _forceEscapeSymbols ||
      [iTermPreferences
          boolForKey:kPreferenceKeyPasteSpecialEscapeShellCharsWithBackslash];
  BOOL convertUnicodePunctuation = [iTermPreferences
      boolForKey:kPreferenceKeyPasteSpecialConvertUnicodePunctuation];
  ...
  BOOL removeValue = [iTermPreferences
      boolForKey:kPreferenceKeyPasteSpecialRemoveControlCodes];
  BOOL shouldBracket = [iTermPreferences
      boolForKey:kPreferenceKeyPasteSpecialBracketedPasteMode];
  ...
  _pasteSpecialViewController.enableEscapeShellCharsWithBackslash =
      containsShellCharacters;
  _pasteSpecialViewController.shouldEscapeShellCharsWithBackslash =
      (containsShellCharacters && shouldEscape);
  ...
  _pasteSpecialViewController.shouldRemoveControlCodes =
      (containsControlCodes && removeValue);
  _pasteSpecialViewController.enableBracketedPaste = _bracketingEnabled;
  _pasteSpecialViewController.shouldUseBracketedPasteMode =
      (_bracketingEnabled && shouldBracket);
  _pasteSpecialViewController.enableBase64 = !_base64only;
  _pasteSpecialViewController.shouldBase64Encode = _base64only;
  _pasteSpecialViewController.enableUseRegexSubstitution =
      !_base64only; // Binary data can't be regexed
  _pasteSpecialViewController.shouldUseRegexSubstitution =
      !_base64only &&
      [iTermPreferences
          boolForKey:kPreferencesKeyPasteSpecialUseRegexSubstitution];
  _pasteSpecialViewController.regexString =
      [iTermPreferences stringForKey:kPreferencesKeyPasteSpecialRegex];
  ...
  _pasteSpecialViewController.shouldWaitForPrompt =
      _isAtShellPrompt && _canWaitForPrompt;
...
- (void)updatePreview {
  ...
  NSNumberFormatter *linesFormatter =
      [[[NSNumberFormatter alloc] init] autorelease];
  ...
  _statsLabel.stringValue = [NSString
      stringWithFormat:@"%@ byte%@ in %@ line%@.",
                       [[bytesFormatter stringFromNumber:@(numBytes)]
...
- (void)updateDuration {
  ...
  // This is very high (pasting locally can be quite fast), and it just here to
  // prevent absurdly low time estimates.
  ...
  if (duration < 0.01) {
    ...
    _estimatedDuration.stringValue =
        [_pasteSpecialViewController descriptionForDuration:duration];
  }
}
...
+ (void)showAsPanelInWindow:(NSWindow *)presentingWindow
  ...
  iTermPasteSpecialWindowController *controller =
      [[[iTermPasteSpecialWindowController alloc]
           initWithChunkSize:chunkSize
...
- (void)saveUserDefaults {
  ...
  if (_pasteSpecialViewController.selectedTabTransform ==
      kTabTransformConvertToSpaces) {
  ...
  if (_pasteSpecialViewController.isEscapeShellCharsWithBackslashEnabled) {
    [iTermPreferences
        setBool:_pasteSpecialViewController.shouldEscapeShellCharsWithBackslash
...
- (PasteEvent *)pasteEventWithString:(NSString *)string
                          forPreview:(BOOL)forPreview {
  iTermPasteFlags flags = _pasteSpecialViewController.flags;
  if (_base64only) {
    // We already base64 encoded the data, so don't set the flag or else it gets
    // double encoded.
    flags &= ~kPasteFlagsBase64Encode;
  }

  iTermTabTransformTags tabTransform =
      _pasteSpecialViewController.selectedTabTransform;
  if (forPreview) {
