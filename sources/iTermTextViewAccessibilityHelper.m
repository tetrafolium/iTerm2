...
- (NSRange)rangeOfLine:(NSUInteger)lineNumber {
  ...
  if (lineNumber == 0) {
    ...
    range.location = [[_lineBreakIndexOffsets objectAtIndex:lineNumber - 1]
        unsignedLongValue];
  }
  if (lineNumber >= [_lineBreakIndexOffsets count]) {
    ...
    range.length =
        [[_lineBreakIndexOffsets objectAtIndex:lineNumber] unsignedLongValue] -
        range.location;
  }
  return range;
}
...
- (NSUInteger)offsetOfLine:(NSUInteger)accessibilityLineNumber {
  ...
  return
      [_lineBreakIndexOffsets[accessibilityLineNumber - 1] unsignedLongValue];
}

// Onscreen X-position of a location (respecting compositing chars) in _allText.
- (NSUInteger)columnOfChar:(NSUInteger)location
                    inLine:(NSUInteger)accessibilityLineNumber {
  NSUInteger lineStart = [self offsetOfLine:accessibilityLineNumber];
  screen_char_t *theLine =
      [_delegate accessibilityHelperLineAtIndex:accessibilityLineNumber];
...
- (NSRange)rangeOfIndex:(NSUInteger)theIndex {
  ...
  screen_char_t *theLine =
      [_delegate accessibilityHelperLineAtIndex:accessibilityLineNumber];
  NSUInteger startingIndexOfLine =
      [self startingIndexOfLineNumber:accessibilityLineNumber];
...
// Range, respecting compositing chars, of a character at an x,y position where
// 0,0 is the first char of the first line in the scrollback buffer.
- (NSRange)rangeOfCharAtX:(int)x y:(int)accessibilityY {
  screen_char_t *theLine =
      [_delegate accessibilityHelperLineAtIndex:accessibilityY];
...
- (VT100GridCoordRange)coordRangeForAccessibilityRange:(NSRange)range {
  ...
  coordRange.start.x = [self columnOfChar:range.location
                                   inLine:coordRange.start.y];
  if (range.length == 0) {
    coordRange.end = coordRange.start;
  } else {
    range.length--;
    coordRange.end.y = [self lineNumberOfIndex:NSMaxRange(range)];
    coordRange.end.x = [self columnOfChar:NSMaxRange(range)
                                   inLine:coordRange.end.y];
    ++coordRange.end.x;
...
- (NSRange)accessibilityRangeForCoordRange:(VT100GridCoordRange)coordRange {
  NSUInteger location1 =
      [self rangeOfCharAtX:coordRange.start.x y:coordRange.start.y].location;
  NSUInteger location2 =
      [self rangeOfCharAtX:coordRange.end.x y:coordRange.end.y].location;
...
- (NSRange)rangeForPosition:(NSPoint)screenPosition {
  VT100GridCoord point =
      [_delegate accessibilityHelperCoordForPoint:screenPosition];
  if (point.y < 0) {
...
- (NSRect)boundsForRange:(NSRange)range {
  ...
  NSRect result = [_delegate
      accessibilityHelperFrameForCoordRange:VT100GridCoordRangeMake(
                                                xMin, yMin, xMax, yMax)];
  return result;
}
...
- (NSString *)allText {
  ...
  for (int i = 0; i < [_delegate accessibilityHelperNumberOfLines]; i++) {
    ...
    if (line[width].code == EOL_HARD) {
      // Add a newline and update offsets arrays that track line break
      // locations.
  ...
  [_allText replaceCharactersInRange:NSMakeRange(i + 1, newlines)
                          withString:@""];
...
- (NSRange)selectedTextRange {
  ...
  // attributes that (re)generate _allText and especially
  // lineBreak{Char,Index}Offsets_ which are needed for rangeOfCharAtX:y:
  [self allText];
