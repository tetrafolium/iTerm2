...
- (NSImage *)image {
  if (_fillColor && _stringValue && !NSEqualSizes(_viewSize, NSZeroSize) &&
      !_image) {
...
// Compute the best point size and return a new image of the badge. Returns nil
// if the badge is empty or zero pixels.r
- (NSImage *)freshlyComputedImage {
  DLog(@"Recompute badge self=%p, label=“%@”, color=%@, view size=%@. Called "
       @"from:\n%@",
       self, _stringValue, _fillColor, NSStringFromSize(_viewSize),
...
// Returns an image from the current text with the given |attributes|, or nil if
// the image would have 0 pixels.
- (NSImage *)imageWithPointSize:(CGFloat)pointSize {
  ...
  temp[NSStrokeColorAttributeName] =
      [_backgroundColor colorWithAlphaComponent:1];
  BOOL truncated;
  ...
  [image lockFocus];
  [_stringValue
      it_drawInRect:NSMakeRect(0, 0, sizeWithFont.width, sizeWithFont.height)
         attributes:temp];
  [image unlockFocus];

  NSImage *reducedAlphaImage =
      [[[NSImage alloc] initWithSize:sizeWithFont] autorelease];
...
// Size of the image resulting from drawing an attributed string with
// |attributes|.
...
- (CGFloat)idealPointSize {
  ...
  while (points != prevPoints) {
    BOOL truncated;
    sizeWithFont =
        [self sizeWithAttributes:[self attributesWithPointSize:points]
                       truncated:&truncated];
    DLog(@"Point size of %@ gives label size of %@", @(points),
         NSStringFromSize(sizeWithFont));
    if (truncated || sizeWithFont.width > maxSize.width ||
