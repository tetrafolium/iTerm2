...
@implementation GeneralPreferencesViewController {
  ...
  IBOutlet NSImageView
      *_prefsDirWarning; // Image shown when path is not writable
...
- (void)awakeFromNib {
  ...
  [self defineControl:_openWindowsAtStartup
      ...
      settingChanged:^(id sender) {
        ...
        case kOpenDefaultWindowArrangementTag:
          [strongSelf setBool:YES
                       forKey:kPreferenceKeyOpenArrangementAtStartup];
          [strongSelf setBool:NO forKey:kPreferenceKeyOpenNoWindowsAtStartup];
          break;
      ...
      update:^BOOL {
        ...
        if ([strongSelf boolForKey:kPreferenceKeyOpenNoWindowsAtStartup]) {
          [strongSelf->_openWindowsAtStartup
              selectItemWithTag:kDontOpenAnyWindowsTag];
        } else if ([WindowArrangements count] &&
                   [self boolForKey:kPreferenceKeyOpenArrangementAtStartup]) {
          [strongSelf->_openWindowsAtStartup
              selectItemWithTag:kOpenDefaultWindowArrangementTag];
        } else {
          [strongSelf->_openWindowsAtStartup
              selectItemWithTag:kUseSystemWindowRestorationSettingTag];
        }
        [strongSelf updateEnabledState];
        return YES;
      }];
  ...
  [iTermPreferenceDidChangeNotification
      subscribe:self
          block:^(iTermPreferenceDidChangeNotification *_Nonnull notification) {
            if ([notification.key
                    isEqualToString:kPreferenceKeyEnableAPIServer]) {
  ...
  _advancedGPUWindowController =
      [[iTermAdvancedGPUSettingsWindowController alloc]
          initWithWindowNibName:@"iTermAdvancedGPUSettingsWindowController"];
  [_advancedGPUWindowController window];
  _advancedGPUWindowController.viewController.disableWhenDisconnected.target =
      self;
  _advancedGPUWindowController.viewController.disableWhenDisconnected.action =
      @selector(settingChanged:);
  info = [self
      defineUnsearchableControl:_advancedGPUWindowController.viewController
                                    .disableWhenDisconnected
                            key:kPreferenceKeyDisableMetalWhenUnplugged
                           type:kPreferenceInfoTypeCheckbox];
  info.observer = ^{
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermMetalSettingsDidChangeNotification
                      object:nil];
  };

  _advancedGPUWindowController.viewController.preferIntegratedGPU.target = self;
  _advancedGPUWindowController.viewController.preferIntegratedGPU.action =
      @selector(settingChanged:);
  info = [self defineUnsearchableControl:_advancedGPUWindowController
                                             .viewController.preferIntegratedGPU
  ...
  _advancedGPUWindowController.viewController.maximizeThroughput.action =
      @selector(settingChanged:);

  info = [self defineUnsearchableControl:_advancedGPUWindowController
                                             .viewController.maximizeThroughput
  ...
  [self addViewToSearchIndex:_advancedGPUPrefsButton
                 displayName:@"Advanced GPU settings"
                     phrases:@[
                       _advancedGPUWindowController.viewController
                           .disableWhenDisconnected.title,
                       _advancedGPUWindowController.viewController
                           .preferIntegratedGPU.title,
                       _advancedGPUWindowController.viewController
                           .maximizeThroughput.title
                     ]
                         key:nil];
  ...
  info =
      [self defineControl:_autoSaveOnQuit
  ...
  info.customSettingChangedHandler = ^(id sender) {
    ...
    [[NSUserDefaults standardUserDefaults]
        setBool:YES
         forKey:@"NoSyncNeverRemindPrefsChangesLostForFile"];
    NSNumber *value;
  ...
  // Called on programmatic change (e.g., selecting a different profile. Returns
  // YES to avoid normal code path.
  info.onUpdate = ^BOOL() {
    __strong __typeof(weakSelf) strongSelf = weakSelf;
    if (!strongSelf) {
      return NO;
  ...
  info.observer = ^{
    ...
    strongSelf->_wordChars.enabled = ![strongSelf
        boolForKey:kPreferenceKeyDoubleClickPerformsSmartSelection];
    strongSelf->_wordCharsLabel.labelEnabled = ![strongSelf
        boolForKey:kPreferenceKeyDoubleClickPerformsSmartSelection];
  };
  ...
  info = [self defineControl:_openTmuxWindows
                 ...
                 relatedView:_openTmuxWindowsLabel
                        type:kPreferenceInfoTypePopup];
  // This is how it was done before the great refactoring, but I don't see why
  // it's needed.
  info.onChange = ^() {
  ...
  [self defineControl:_tmuxPauseModeAgeLimit
          ...
          displayName:@"Pause a tmux pane if it would take more than this many "
                      @"seconds to catch up."
...
- (void)updateAPIEnabledState {
  ...
  [_apiPermission
      selectItemWithTag:[iTermAPIHelper requireApplescriptAuth] ? 0 : 1];
  [self updateEnabledState];
}
...
- (void)updateEnabledState {
  ...
  [_apiPermission
      selectItemWithTag:[iTermAPIHelper requireApplescriptAuth] ? 0 : 1];
  _evenIfThereAreNoWindows.enabled =
      [self boolForKey:kPreferenceKeyPromptOnQuit];
  const BOOL useSystemWindowRestoration =
      (![self boolForKey:kPreferenceKeyOpenArrangementAtStartup] &&
       ![self boolForKey:kPreferenceKeyOpenNoWindowsAtStartup]);
  const BOOL systemRestorationEnabled = [[NSUserDefaults standardUserDefaults]
      boolForKey:@"NSQuitAlwaysKeepsWindows"];
  _warningButton.hidden =
      (!useSystemWindowRestoration || systemRestorationEnabled);
}
...
- (BOOL)reallyEnableAPISettingDidChange {
  ...
  if (enabled) {
    // Prompt the user. If they agree, or have permanently agreed, set the user
    // default to YES.
    if ([iTermAPIHelper
            confirmShouldStartServerAndUpdateUserDefaultsForced:YES]) {
...
- (IBAction)warning:(id)sender {
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          @"System window restoration has been disabled, which prevents iTerm2 "
          @"from respecting this setting. Disable System Preferences > General "
          @"> Close windows when quitting an app to enable window restoration."
  ...
  if (selection == kiTermWarningSelection0) {
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL fileURLWithPath:@"/System/Library/PreferencePanes/"
                                       @"Appearance.prefPane"]];
  }
}
...
- (IBAction)pythonAPIAuthHelp:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"https://iterm2.com/python-api-auth.html"]];
}
...
- (void)updateRemotePrefsViews {
  ...
  BOOL remoteLocationIsValid =
      [[iTermRemotePreferences sharedInstance] remoteLocationIsValid];
  _prefsDirWarning.image =
      remoteLocationIsValid
          ? [NSImage it_imageNamed:@"CheckMark" forClass:self.class]
          : [NSImage it_imageNamed:@"WarningSign" forClass:self.class];
  BOOL isValidFile =
      (shouldLoadRemotePrefs && remoteLocationIsValid &&
...
- (void)loadPrefsFromCustomFolderDidChange {
  ...
  if (shouldLoadRemotePrefs) {
    // Just turned it on.
    if ([[_prefsCustomFolder stringValue] length] == 0 &&
        ![self pasteboardHasGitlabURL]) {
      // Field was initially empty so browse for a dir.
      if ([self choosePrefsCustomFolder]) {
        // User didn't hit cancel; if he chose a writable directory, ask if he
        ...
        if ([[iTermRemotePreferences sharedInstance] remoteLocationIsValid]) {
          ...
          if ([alert runModal] == NSAlertFirstButtonReturn) {
            [[iTermRemotePreferences sharedInstance]
                saveLocalUserDefaultsToRemotePrefs];
          }
        }
      }
    }
...
- (BOOL)pasteboardHasGitlabURL {
  ...
  return [pasteboardString
      isMatchedByRegex:@"^https://gitlab.com/gnachman/iterm2/uploads/[a-f0-9]*/"
