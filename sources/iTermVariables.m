...
typedef iTermTriple<NSNumber *, iTermVariables *, NSString *>
    iTermVariablesDepthOwnerNamesTriple;
...
NSString *const iTermVariableKeyTabTmuxWindowTitle =
    @"tmuxWindowTitle"; // tmux window title if set-title is on, comes from
                        // evaluating set-titles-strings
...
NSString *const iTermVariableKeySessionCreationTimeString =
    @"creationTimeString";
...
NSString *const iTermVariableKeySessionMouseReportingMode =
    @"mouseReportingMode";
...
NSString *const iTermVariableKeyWindowTitleOverrideFormat =
    @"titleOverrideFormat";
...
+ (instancetype)globalInstance {
  ...
  dispatch_once(&onceToken, ^{
    instance = [[iTermVariables alloc]
        initWithContext:iTermVariablesSuggestionContextApp
                  owner:NSApp];
  });
  return instance;
}
...
- (NSString *)debugInfo {
  NSString *resolvedString =
      [[_resolvedLinks.allKeys mapWithBlock:^id(NSString *key) {
        NSString *refs = [[self->_resolvedLinks[key].allObjects
            mapWithBlock:^id(id<iTermVariableReference> ref) {
              return [@"    " stringByAppendingString:[ref description]];
            }] componentsJoinedByString:@"\n"];
        return [NSString stringWithFormat:@"%@ ->\n%@", key, refs];
      }] componentsJoinedByString:@"\n"];
  NSString *unresolvedString =
      [[_unresolvedLinks.allKeys mapWithBlock:^id(NSString *key) {
        NSString *refs = [[self->_unresolvedLinks[key].allObjects
            mapWithBlock:^id(id<iTermVariableReference> ref) {
              return [@"    " stringByAppendingString:[ref description]];
            }] componentsJoinedByString:@"\n"];
        ;
  ...
  return [NSString
      stringWithFormat:@"Resolved links:\n%@\n\nUnresolved links:\n%@",
                       resolvedString, unresolvedString];
}

- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p owner=%@>", self.class, self, self.owner];
}
...
- (BOOL)setValue:(nullable id)value
    forVariableNamed:(NSString *)name
                weak:(BOOL)weak {
  iTermVariables *owner = [self setValue:value
                        forVariableNamed:name
                         withSideEffects:YES
                                    weak:weak];
  return owner != nil;
}
...
- (BOOL)setValuesFromDictionary:(NSDictionary<NSString *, id> *)dict {
  ...
  for (NSString *name in dict) {
    ...
    owner = [self setValue:value
          forVariableNamed:name
           withSideEffects:NO
                      weak:NO];
    if (owner) {
      NSInteger depth = [[name componentsSeparatedByString:@"."] count];
      [mutations addObject:[iTermTriple tripleWithObject:@(depth)
                                               andObject:owner
                                                  object:name]];
    }
  }
...
- (nullable id)valueForVariableName:(NSString *)name {
  ...
  iTermVariables *child =
      [iTermVariables castFrom:[self valueByUnwrappingWeakVariables:value]];
  if (!child) {
    return nil;
  }
  return [child valueForVariableName:[[parts subarrayFromIndex:1]
                                         componentsJoinedByString:@"."]];
}
...
- (void)addLinkToReference:(id<iTermVariableReference>)reference
  ...
  id value = _values[parts.firstObject];
  if (value) {
    [self addWeakReferenceToLinkTable:_resolvedLinks
                             toObject:reference
    ...
    if (sub && parts.count > 1) {
      [sub addLinkToReference:reference
                         path:[[parts subarrayFromIndex:1]
                                  componentsJoinedByString:@"."]];
    }
  } else {
    [self addWeakReferenceToLinkTable:_unresolvedLinks
                             toObject:reference
                               forKey:parts.firstObject];
  }
}
...
- (BOOL)hasLinkToReference:(id<iTermVariableReference>)reference
  ...
  iTermVariables *sub =
      [iTermVariables castFrom:[self valueByUnwrappingWeakVariables:value]];
  if (sub && parts.count > 1) {
    return [sub hasLinkToReference:reference
                              path:[[parts subarrayFromIndex:1]
                                       componentsJoinedByString:@"."]];
  }
...
- (void)removeLinkToReference:(id<iTermVariableReference>)reference
                         path:(NSString *)path {
  [self removeWeakReferenceFromLinkTable:_resolvedLinks
                                toObject:reference
                                  forKey:path];
  [self removeWeakReferenceFromLinkTable:_unresolvedLinks
                                toObject:reference
                                  forKey:path];
}
...
- (void)setPrimaryKey:(nullable NSString *)primaryKey {
  ...
  _primaryKeyReference =
      [[iTermVariableReference alloc] initWithPath:primaryKey
                                            vendor:self.it_weakProxy];
...
- (void)valueOfPrimaryKeyDidChange {
  ...
  [[iTermVariablesIndex sharedInstance] setVariables:self
                                              forKey:_indexedValueOfPrimaryKey];
}

- (void)didChangeNonterminalValueWithPath:(NSString *)name {
  NSArray<id<iTermVariableReference>> *refs =
      [self strongArrayFromWeakArray:_resolvedLinks[name]];
...
- (NSString *)linksDescription {
  return [[_resolvedLinks.allKeys mapWithBlock:^NSString *(NSString *key) {
    NSArray<id<iTermVariableReference>> *refs =
        [self strongArrayFromWeakArray:self->_resolvedLinks[key]];
    NSString *refsString =
        [[refs mapWithBlock:^id(id<iTermVariableReference> ref) {
          return [NSString stringWithFormat:@"%@=%p", ref.path,
                                            (__bridge void *)ref.onChangeBlock];
        }] componentsJoinedByString:@", "];
...
- (void)addWeakReferenceToLinkTable:
            (NSMutableDictionary<NSString *, NSPointerArray *> *)linkTable
...
- (void)removeWeakReferenceFromLinkTable:
            (NSMutableDictionary<NSString *, NSPointerArray *> *)linkTable
                                toObject:(id<iTermVariableReference>)reference
                                  forKey:(NSString *)localPath {
  NSPointerArray *array = linkTable[localPath];
...
- (nullable iTermVariables *)setValue:(id)value
                     forVariableNamed:(NSString *)name
  ...
  NSArray<NSString *> *parts = [name componentsSeparatedByString:@"."];
  if (parts.count > 1) {
    iTermVariables *child = [iTermVariables
        castFrom:[self
                     valueByUnwrappingWeakVariables:_values[parts
                                                                .firstObject]]];
    if (!child) {
      return nil;
    }
    return [child setValue:value
          forVariableNamed:[[parts subarrayFromIndex:1]
                               componentsJoinedByString:@"."]
  ...
  const BOOL changed =
      ![NSObject object:value
          isEqualToObject:[self valueByUnwrappingWeakVariables:_values[name]]];
  if (!changed) {
    return self;
  }
  ...
  if (value && ![NSNull castFrom:value]) {
    if ([value isKindOfClass:[iTermVariables class]]) {
      ...
      const BOOL wasVariables =
          [[self valueByUnwrappingWeakVariables:_values[name]]
  ...
  if ([value isKindOfClass:[iTermVariables class]] ||
      [value isKindOfClass:[iTermWeakVariables class]]) {
    // Don't record the use of nonterminals.
    return self;
  }
  [self didReferenceVariables:@[ [iTermTriple tripleWithObject:@1
                                                     andObject:self
                                                        object:name] ]];
  return self;
}

// TODO: This is way more complex than it needs to be. It used to batch notify
// owners of changes but I removed the delegate interface. I don't think the
// batching logic is needed any more, but this commit is already too large.
- (void)didReferenceVariables:
    (NSArray<iTermVariablesDepthOwnerNamesTriple *> *)mutations {
  [self enumerateMutationsByOwnersByDepth:mutations
                                    block:^(iTermVariables *owner,
                                            NSSet<NSString *> *nameSet) {
                                      [owner recordUseOfVariables:nameSet];
                                    }];
}

- (void)enumerateMutationsByOwnersByDepth:
            (NSArray<iTermVariablesDepthOwnerNamesTriple *> *)mutations
                                    block:(void (^)(iTermVariables *owner,
                                                    NSSet<NSString *> *names))
                                              block {
  [self
      enumerateMutationsByDepth:mutations
                          block:^(NSArray<iTermVariablesDepthOwnerNamesTriple *>
                                      *isodepthTriples) {
                            [self
                                enumerateOwners:isodepthTriples
                                          block:^(iTermVariables *owner,
                                                  NSArray<NSString *> *names) {
                                            NSSet *uniqueNames = [NSSet
                                                setWithArray:
                                                    [names mapWithBlock:^id(
                                                               NSString
                                                                   *anObject) {
                                                      return [[anObject
...
- (void)
    enumerateMutationsByDepth:
        (NSArray<iTermVariablesDepthOwnerNamesTriple *> *)mutations
                        block:(void (^)(
                                  NSArray<iTermVariablesDepthOwnerNamesTriple *>
                                      *isodepthTriple))block {
  NSDictionary<NSNumber *, NSArray<iTermVariablesDepthOwnerNamesTriple *> *>
      *byDepth;
  byDepth = [mutations
      classifyWithBlock:^id(iTermVariablesDepthOwnerNamesTriple *triple) {
        return triple.firstObject;
      }];

  // Iterate from deepest to shallowest
  NSArray<NSNumber *> *sortedDepths =
      [byDepth.allKeys sortedArrayUsingSelector:@selector(compare:)];
  for (NSNumber *number in [sortedDepths reverseObjectEnumerator]) {
    block(byDepth[number]);
  }
}

- (void)enumerateOwners:
            (NSArray<iTermVariablesDepthOwnerNamesTriple *> *)mutations
                  block:(void (^)(iTermVariables *owner,
                                  NSArray<NSString *> *names))block {
  NSDictionary<NSValue *, NSArray<iTermVariablesDepthOwnerNamesTriple *> *>
      *byOwner;
  byOwner = [mutations
      classifyWithBlock:^id(iTermVariablesDepthOwnerNamesTriple *triple) {
        return [NSValue valueWithNonretainedObject:triple.secondObject];
      }];
  [byOwner enumerateKeysAndObjectsUsingBlock:^(
               NSValue *_Nonnull ownerValue,
               NSArray<iTermVariablesDepthOwnerNamesTriple *> *_Nonnull triples,
               BOOL *_Nonnull stop) {
    NSArray<NSString *> *names = [triples
        mapWithBlock:^id(iTermVariablesDepthOwnerNamesTriple *anObject) {
...
+ (NSSet<NSString *> *)namesToRecordFromSet:(NSSet<NSString *> *)names
                                  inContext:
...
- (void)recordUseOfVariables:(NSSet<NSString *> *)allNames {
  if (_context != iTermVariablesSuggestionContextNone) {
    NSSet<NSString *> *names = [iTermVariables namesToRecordFromSet:allNames
                                                          inContext:_context];
    if (names.count == 0) {
      return;
    }
    [names enumerateObjectsUsingBlock:^(NSString *_Nonnull name,
                                        BOOL *_Nonnull stop) {
      if (![[self valueByUnwrappingWeakVariables:self->_values[name]]
              isKindOfClass:[iTermVariables class]]) {
        [iTermVariableHistory recordUseOfVariableNamed:name
...
- (NSSet<NSString *> *)namesByPrependingParentName:(NSSet<NSString *> *)names {
  ...
  return [NSSet
      setWithArray:[names.allObjects mapWithBlock:^id(NSString *anObject) {
...
- (NSDictionary<NSString *, NSString *> *)stringValuedDictionaryInScope:
    (nullable NSString *)scopeName {
  ...
  for (NSString *name in _values) {
    ...
    iTermVariables *child = [iTermVariables castFrom:value];
    if (child) {
      [result it_mergeFrom:[child.stringValuedDictionary mapKeysWithBlock:^id(
                                                             NSString *key,
                                                             NSString *object) {
                if (scopeName) {
                  return [NSString
                      stringWithFormat:@"%@.%@.%@", scopeName, name, key];
                } else {
...
- (NSDictionary<NSString *, id> *)dictionaryInScope:
    (nullable NSString *)scopeName {
  NSMutableDictionary<NSString *, NSString *> *result =
      [NSMutableDictionary dictionary];
  for (NSString *name in _values) {
    id value = _values[name];
    assert(value != self);
    // Weak variables are intentionally not unwrapped here to avoid getting
    // stuck in a cycle.
    iTermVariables *child = [iTermVariables castFrom:value];
    if (child) {
      [result it_mergeFrom:[[child dictionaryInScope:nil]
                               mapKeysWithBlock:^id(NSString *key, id object) {
                                 if (scopeName) {
                                   return [NSString
                                       stringWithFormat:@"%@.%@.%@", scopeName,
                                                        name, key];
                                 } else {
                                   return [NSString
                                       stringWithFormat:@"%@.%@", name, key];
                                 }
                               }]];
    } else {
      NSString *scopedName = name;
