...
+ (nullable NSURL *)urlByReplacingFormatSpecifier:(NSString *)formatSpecifier
  ...
  NSString *placeholder =
      [[[NSUUID UUID] UUIDString] stringByReplacingOccurrencesOfString:@"-"
                                                            withString:@""];
  NSString *urlString =
      [string stringByReplacingOccurrencesOfString:formatSpecifier
  ...
  // Query item value?
  {
    ...
    const NSUInteger i = [queryItems
        indexOfObjectPassingTest:^BOOL(NSURLQueryItem *_Nonnull obj,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
          return [obj.value isEqualToString:placeholder];
        }];
    if (queryItems && i != NSNotFound) {
      queryItems[i] = [NSURLQueryItem queryItemWithName:queryItems[i].name
                                                  value:value];
  ...
  // Query item name?
  {
    ...
    if (queryItems && i != NSNotFound) {
      queryItems[i] = [NSURLQueryItem queryItemWithName:value
                                                  value:queryItems[i].value];
      components.queryItems = queryItems;
      return components.URL;
    }
  }
  ...
  // Fragment?
  {
    ...
    if (components.fragment && range.location != NSNotFound) {
      components.fragment =
          [components.fragment stringByReplacingCharactersInRange:range
                                                       withString:value];
      return components.URL;
    }
  }
  ...
  // Path?
  {
    ...
    if (components.path && range.location != NSNotFound) {
      components.path =
          [components.path stringByReplacingCharactersInRange:range
                                                   withString:value];
      return components.URL;
    }
  }
  ...
  // Hostname?
  {
    ...
    if (components.host && range.location != NSNotFound) {
      components.host =
          [components.host stringByReplacingCharactersInRange:range
                                                   withString:value];
      return components.URL;
    }
  }
  ...
  // Scheme?
  {
    ...
    if (components.scheme && range.location != NSNotFound) {
      components.scheme =
          [components.scheme stringByReplacingCharactersInRange:range
                                                     withString:value];
      return components.URL;
    }
  }
  ...
  // User name?
  {
    ...
    if (components.user && range.location != NSNotFound) {
      components.user =
          [components.user stringByReplacingCharactersInRange:range
                                                   withString:value];
      return components.URL;
    }
  }
  ...
  // Password?
  {
    ...
    if (components.password && range.location != NSNotFound) {
      components.password =
          [components.password stringByReplacingCharactersInRange:range
                                                       withString:value];
      return components.URL;
    }
  }
...
- (NSURL *)URLByRemovingFragment {
  if (self.fragment) {
    ...
    if (range.location != NSNotFound) {
      NSString *stringWithoutFragment =
...
- (NSURL *)URLByAppendingQueryParameter:(NSString *)queryParameter {
  ...
  NSArray *components = @[
    urlWithoutFragment.absoluteString, separator, queryParameter, fragment
  ];
...
// This takes a string that has a combination of well-encoded characters and
// totally illegal-in-a-URL characters, and turns it into a proper URL.
//
// Nobody wants to write code to parse a URL. So we don't!
//
// This algorithm replaces any sequence of characters that aren't part of the
// structure of a URL and replaces them with a nice safe ASCII string, mostly
...
//
// If what the user gave is us is roughly URL shaped this can be turned in to
// NSURLComponents. It appears to have a competent URL parser, but it does want
// reasonably well-formed input.
//
// Then we replace each component with its original value. So x12 -> https, 11
// -> user, etc.
//
// Note that NSURLComponents likes its values to be unencoded, so it can have
// all the crazy stuff the user gave us and it can (mostly) turn it in to a URL.
//
// The reason there was a mostly is because it doesn't know about IDN. So we
// IDN-encode the hostname before generating a URL.
+ (NSURL *)URLWithUserSuppliedString:(NSString *)string {
  ...
  NSCharacterSet *reservedSymbols =
      [NSCharacterSet characterSetWithCharactersInString:@":/@:.#?&="];
  NSIndexSet *nonReservedSymbolIndices =
      [string indicesOfCharactersInSet:[reservedSymbols invertedSet]];
  ...
  [nonReservedSymbolIndices
      enumerateRangesWithOptions:NSEnumerationReverse
                      usingBlock:^(NSRange range, BOOL *_Nonnull stop) {
                        NSString *placeholder =
                            [NSString stringWithFormat:@"%08d", count++];
                        if (range.location == 0) {
                          // Schemes can't start with a number. In case the
                          // first thing is a scheme, start it with a letter.
                          // This is safe because ports are the only thing that
                          // must be a number but they can't come first!
                          placeholder =
                              [@"x" stringByAppendingString:placeholder];
                        }
                        [stringWithPlaceholders
                            replaceCharactersInRange:range
                                          withString:placeholder];
                        // We have to remove percent encoding here or it gets
                        // double-encoded. If there is a percent that is not
                        // part of a percent encoding scheme, then
                        // -stringByRemovingPercentEncoding will return nil and
                        // then we just use the raw string. A mix of
                        // percent-encoded and non-percent-encoded will not
                        // work, nor will non-percent-encoded that happens to
                        // look like percent-encoded.
  ...
  NSURLComponents *components =
      [NSURLComponents componentsWithString:stringWithPlaceholders];
  DLog(@"components=%@", components);

  NSString * (^glue)(NSString *) = ^NSString *(NSString *encoded) {
    NSMutableString *result = [encoded mutableCopy];
    [map enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
  ...
  if (components.queryItems.count) {
    components.queryItems =
        [components.queryItems mapWithBlock:^id(NSURLQueryItem *item) {
...
- (nullable NSData *)zippedContents {
  ...
  NSURL *destination = [[NSURL fileURLWithPath:NSTemporaryDirectory()]
      URLByAppendingPathComponent:uuid];
...
- (BOOL)saveContentsOfPathToZip:(NSURL *)destination {
  ...
  if (isDir) {
    ...
    NSString *uuid = [[NSUUID UUID] UUIDString];
    sourceDir = [[NSURL fileURLWithPath:NSTemporaryDirectory()]
        URLByAppendingPathComponent:uuid];
    ...
    NSURL *tempURL = [sourceDir
        URLByAppendingPathComponent:self.lastPathComponent ?: @"file"];
