...
- (void)loadCardExpanded:(BOOL)expanded {
  iTermTipCardViewController *card = [[[iTermTipCardViewController alloc]
      initWithNibName:@"iTermTipCardViewController"
...
- (void)addButtonsToCard:(iTermTipCardViewController *)card
                expanded:(BOOL)expanded {
  if (_tip.url) {
    [card addActionWithTitle:kLearnMoreTitle
                        icon:[NSImage it_imageNamed:@"Navigate"
                                           forClass:self.class]
  ...
  [card addActionWithTitle:kDismissTipTitle
                  shortcut:@"⎋"
                      icon:[NSImage it_imageNamed:@"Dismiss"
                                         forClass:self.class]
                     block:^(id sendingCard) {
                       [self dismiss];
                     }];
  ...
  iTermTipCardActionButton *button =
      [card addActionWithTitle:toggleTitle
                          icon:[NSImage it_imageNamed:@"ChevronDown"
                                             forClass:self.class]
                         block:^(id sendingCard) {
  ...
  button = [card addActionWithTitle:kShowThisLaterTitle
                               icon:[NSImage it_imageNamed:@"Later"
                                                  forClass:self.class]
                              block:^(id sendingCard) {
                                [self showThisLater];
                              }];
  ...
  NSRectFillUsingOperation(
      NSMakeRect(0, 0, shareImage.size.width, shareImage.size.height),
      NSCompositingOperationSourceAtop);
  [shareImage unlockFocus];
  shareImage.template = NO;

  button = [card addActionWithTitle:kShareTitle
                               icon:shareImage
                              block:^(id card) {
                                [self shareThis:card];
                              }];
  ...
  button = [card
      addActionWithTitle:frequencyTitle
                    icon:[NSImage it_imageNamed:@"TipCalendar"
                                       forClass:self.class]
                   block:^(id sendingCard) {
                     [_delegate toggleTipFrequency];
                     iTermTipCardActionButton *theButton =
                         [card actionWithTitle:kShowTipsWeeklyTitle];
                     if (theButton) {
  ...
  button = [card addActionWithTitle:enableOrDisableTitle
                               icon:[NSImage it_imageNamed:@"DisableTips"
                                                  forClass:self.class]
                              block:^(id sendingCard) {
                                if (![_delegate tipWindowTipsAreDisabled]) {
                                  [self disableTips];
                                  ...
                                  iTermTipCardActionButton *theButton =
                                      [card actionWithTitle:kEnableTipsTitle];
                                  [self enableTips];
  ...
  if ([_delegate tipWindowTipAfterTipWithIdentifier:self.tip.identifier]) {
    button = [card addActionWithTitle:kShowNextTipTitle
                                 icon:[NSImage it_imageNamed:@"NextTip"
                                                    forClass:self.class]
                                block:^(id sendingCard) {
                                  [self showNextTip];
                                }];
  ...
  if ([_delegate tipWindowTipBeforeTipWithIdentifier:self.tip.identifier]) {
    button = [card addActionWithTitle:kShowPreviousTipTitle
                                 icon:[NSImage it_imageNamed:@"NextTip"
                                                    forClass:self.class]
                                block:^(id sendingCard) {
    ...
    iTermTipCardActionButton *nextButton =
        [card actionWithTitle:kShowNextTipTitle];
    if (nextButton) {
      [card combineActionWithTitle:kShowPreviousTipTitle
                          andTitle:kShowNextTipTitle];
    }
  }
...
// Action button titles that are collapsible. These must appear adjacently and
// last.
- (NSArray *)collapsingTitles {
  return @[
    kShowThisLaterTitle, kShareTitle, kDisableTipsTitle, kEnableTipsTitle,
    kShowTipsDailyTitle, kShowTipsWeeklyTitle, kShowNextTipTitle,
...
- (NSArray<iTermHotKey *> *)showTipWindow {
  ...
  self.intermediateView = [[[iTermFlippedView alloc]
  ...
  if (!_hotKey) {
    ...
    iTermShortcut *shortcut =
        [[[iTermShortcut alloc] initWithKeyCode:kVK_Escape
                                      modifiers:0
                                     characters:characters
                    charactersIgnoringModifiers:characters] autorelease];
    _hotKey = [[[iTermCarbonHotKeyController sharedInstance]
...
- (void)layoutCard:(iTermTipCardViewController *)card animated:(BOOL)animated {
  ...
  NSRect windowFrame =
      NSMakeRect(NSMinX(screenFrame) + kWindowLeftMargin,
                 NSMaxY(screenFrame) - NSHeight(frame) -
                     kWindowTopMargin, // In case menu bar is hidden and later
                                       // becomes visible
  ...
  if (animated) {
    ...
    CGFloat heightChange =
        card.postAnimationFrame.size.height - card.view.frame.size.height;
    NSRect finalWindowFrame = NSMakeRect(
        NSMinX(screenFrame) + kWindowLeftMargin,
        NSMaxY(screenFrame) - NSHeight(postAnimationFrame) - kWindowTopMargin,
        postAnimationFrame.size.width, postAnimationFrame.size.height);
...
- (void)animateOut {
  ...
  [[NSAnimationContext currentContext] setCompletionHandler:^{
    ...
    for (iTermTipCardActionButton *button in _cardViewController
             .actionButtons) {
      button.block = nil;
    }
    [self release];
...
// 2) It does not work (when you tell it to performWithItems it just gets slow
// but does nothing).
...
- (void)shareThis:(iTermTipCardViewController *)card {
  ...
  NSArray<NSSharingService *> *services =
      [NSSharingService sharingServicesForItems:@[ item ]];

  NSMenu *menu =
      [[[NSMenu alloc] initWithTitle:@"Sharing Services"] autorelease];
  for (NSSharingService *service in services) {
    NSMenuItem *item =
        [[[NSMenuItem alloc] initWithTitle:service.title
...
- (void)toggleOptionsInCard:(iTermTipCardViewController *)card {
  ...
  if (action) {
    ...
    [[card actionWithTitle:kShowThisLaterTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kShareTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kDisableTipsTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kEnableTipsTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kShowTipsWeeklyTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kShowTipsDailyTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kShowNextTipTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
    [[card actionWithTitle:kShowPreviousTipTitle]
        setAnimationState:kTipCardButtonAnimatingIn];
  } else {
    // Collapsing
    ...
    [[card actionWithTitle:kShowThisLaterTitle]
        setAnimationState:kTipCardButtonAnimatingOut];
    [[card actionWithTitle:kShareTitle]
        setAnimationState:kTipCardButtonAnimatingOut];
    [[card actionWithTitle:kDisableTipsTitle]
        setAnimationState:kTipCardButtonAnimatingOut];
...
- (void)showNextTip {
  iTermTip *nextTip =
      [_delegate tipWindowTipAfterTipWithIdentifier:_tip.identifier];
  if (nextTip) {
...
- (void)showPreviousTip {
  iTermTip *previousTip =
      [_delegate tipWindowTipBeforeTipWithIdentifier:_tip.identifier];
  if (previousTip) {
...
- (void)rollInTip:(iTermTip *)nextTip fromRight:(BOOL)fromRight {
  ...
  self.tip = nextTip;
  BOOL expanded =
      ([_cardViewController actionWithTitle:kMoreOptionsTitle] == nil);
  // Card moved to exitingCardViewControllers while buttons are disabled so
  // buttons will never be enabled in this card again.
