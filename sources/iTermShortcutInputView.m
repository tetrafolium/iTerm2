...
- (void)setBackgroundStyle:(NSBackgroundStyle)backgroundStyle {
  if (backgroundStyle == NSBackgroundStyleLight) {
    [_clearButton setImage:[NSImage it_imageNamed:@"Erase"
                                         forClass:self.class]];
  } else {
    [_clearButton setImage:[NSImage it_imageNamed:@"EraseDarkBackground"
                                         forClass:self.class]];
  }
...
- (void)addClearButton {
  ...
  NSSize buttonSize = [[NSImage it_imageNamed:@"Erase"
                                     forClass:self.class] size];

  _clearButton = [[NSButton alloc]
      initWithFrame:NSMakeRect(size.width - buttonSize.width - 2,
...
- (void)handleShortcutEvent:(NSEvent *)event {
  if (event.type == NSEventTypeKeyDown) {
    ...
    self.hotkeyBeingRecorded =
        [NSString stringForModifiersWithMask:event.it_modifierFlags];
  }
  [self setNeedsDisplay:YES];
}
...
- (NSString *)identifierForCode:(NSUInteger)code
  ...
  if (code || character) {
    return [NSString
        stringWithFormat:@"0x%x-0x%x", (int)character, (int)modifiers];
  } else {
    return nil;
  }
}
