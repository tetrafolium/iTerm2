...
static NSString *const kCommandUses =
    @"use times"; // The name is a historical artifact
...
+ (instancetype)commandHistoryEntryFromDeprecatedDictionary:(NSDictionary *)dict
                                                  inContext:
                                                      (NSManagedObjectContext *)
                                                          context {
  iTermCommandHistoryEntryMO *managedObject = [NSEntityDescription
      insertNewObjectForEntityForName:@"CommandHistoryEntry"
               inManagedObjectContext:context];
  ...
  for (id serializedCommandUse in dict[kCommandUses]) {
    iTermCommandHistoryCommandUseMO *useManagedObject =
        [iTermCommandHistoryCommandUseMO
            commandHistoryCommandUseFromDeprecatedSerialization:
                serializedCommandUse
                                                      inContext:context];
...
// Core data is a piece of garbage. The generated code throws an exception when
// there's a one-to-many relationship. These methods route around the damage.
...
- (void)removeUsesObject:(iTermCommandHistoryCommandUseMO *)value {
  ...
  NSIndexSet *indexSet =
      [NSIndexSet indexSetWithIndex:[self.uses indexOfObject:value]];
...
// Used to sort from highest to lowest score. So Ascending means self's score is
// higher than other's.
