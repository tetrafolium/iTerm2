...
- (NSString *)triggerOptionalDefaultParameterValueWithInterpolation:
    (BOOL)interpolation {
  return nil;
}
...
- (id<iTermFocusReportingTextFieldDelegate>)newParameterDelegateWithPassthrough:
    (id<NSTextFieldDelegate>)passthrough {
  return nil;
}
...
- (BOOL)tryString:(iTermStringLine *)stringLine
    ...
    useInterpolation:(BOOL)useInterpolation {
  if (_partialLine && !self.instantTriggerCanFireMultipleTimesPerLine &&
  ...
  [s enumerateStringsMatchedByRegex:regex_
                         usingBlock:^(NSInteger captureCount,
                           ...
                           if (![self
                                   performActionWithCapturedStrings:
                                       capturedStrings
                                                     capturedRanges:
                                                         capturedRanges
                                               ...
                                               atAbsoluteLineNumber:lineNumber
                                                   useInterpolation:
                                                       useInterpolation
                                                               stop:
                                                                   &
                                                                   stopFutureTriggersFromRunningOnThisLine]) {
                             *stopEnumerating = YES;
                           }
                         }];
  if (!partialLine) {
...
- (void)paramWithBackreferencesReplacedWithValues:(NSString *const *)strings
  ...
  if (useInterpolation) {
    [self evaluateSwiftyStringParameter:self.param
                         backreferences:[[NSArray alloc] initWithObjects:strings
                                                                   count:count]
                                  scope:scope
                             completion:completion];
    return;
...
- (iTermVariableScope *)variableScope:(iTermVariableScope *)scope
  ...
  iTermVariables *matchesFrame = [[iTermVariables alloc]
...
- (void)evaluateSwiftyStringParameter:(NSString *)expression
  ...
  iTermVariableScope *myScope = [self variableScope:scope
                             byAddingBackreferences:backreferences];
  ;
  if (![_cachedSwiftyString.swiftyString isEqualToString:expression]) {
    _cachedSwiftyString = [[iTermSwiftyString alloc] initWithString:expression
                                                              scope:myScope
                                                           observer:nil];
  }

  [_cachedSwiftyString
      evaluateSynchronously:NO
                  withScope:myScope
                 completion:^(NSString *_Nonnull value, NSError *_Nonnull error,
                              NSSet<NSString *> *_Nonnull missing) {
                   if (error) {
                     [iTermWarning
                         showWarningWithTitle:
                             [NSString
                                 stringWithFormat:
                                     @"The following parameter for a “%@” "
                                     @"trigger could not be "
                                     @"evaluated:\n\n%@\n\nThe error "
                                     @"was:\n\n%@",
                                     [[self class] title],
                                     self->_cachedSwiftyString.swiftyString,
                                     error.localizedDescription]
                                      actions:@[ @"OK" ]
                                    accessory:nil
                                   identifier:@"NoSyncErrorInTriggerParameter"
                                  silenceable:
                                      kiTermWarningTypeTemporarilySilenceable
...
- (NSArray *)objectsSortedByValueInDict:(NSDictionary *)dict {
  return [dict keysSortedByValueUsingSelector:@selector
...
- (NSData *)digest {
  ...
  for (NSString *key in [[triggerDictionary allKeys]
