...
- (NSColor *)dividerColor {
  NSColor *color =
      self.window.ptyWindow.it_terminalWindowDecorationControlColor;
  return color;
}

- (NSString *)description {
  NSMutableString *d = [NSMutableString stringWithString:@"<PTYSplitView "];
  [d appendFormat:@"<%@:%p frame:%@ splitter:%@ [", [self class], self,
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  [[self delegate] splitView:self
      draggingWillBeginOfSplit:clickedOnSplitterIndex];
...
- (void)willRemoveSubview:(NSView *)subview {
  if (_dead) {
    // Was called from within superclass's -dealloc, and trying to construct a
    // weak reference will crash.
    return;
  }
...
- (void)setFrame:(NSRect)frame {
  DLog(@"%@: setFrame:%@\n%@", self, NSStringFromRect(frame),
       [NSThread callStackSymbols]);
  [super setFrame:frame];
}

- (void)setFrameSize:(NSSize)newSize {
  DLog(@"%@: setFrameSize:%@\n%@", self, NSStringFromSize(newSize),
       [NSThread callStackSymbols]);
  [super setFrameSize:newSize];
}

- (void)setFrameOrigin:(NSPoint)newOrigin {
  DLog(@"%@: setFrameOrigin:%@\n%@", self, NSStringFromPoint(newOrigin),
       [NSThread callStackSymbols]);
  [super setFrameOrigin:newOrigin];
}

- (NSArray<PTYSplitViewDividerInfo *> *)transitiveDividerLocationsVertical:
    (BOOL)vertical {
  return [self transitiveDividerLocationsVertical:vertical root:self];
}

- (NSArray<PTYSplitViewDividerInfo *> *)
    transitiveDividerLocationsVertical:(BOOL)vertical
                                  root:(PTYSplitView *)root {
  const NSPoint originPoint =
      [root convertRect:self.bounds fromView:self].origin;
  ...
  for (NSInteger i = 0; i < count; i++) {
    ...
    if (i + 1 < count && self.isVertical == vertical) {
      NSRect dividerFrame;
      if (vertical) {
        dividerFrame = NSMakeRect(offset, originPoint.y, dividerThickness,
                                  self.bounds.size.height);
      } else {
        dividerFrame = NSMakeRect(originPoint.x, offset, self.bounds.size.width,
                                  dividerThickness);
      }
      PTYSplitViewDividerInfo *info =
          [[PTYSplitViewDividerInfo alloc] initWithFrame:dividerFrame
                                                vertical:vertical];
    ...
    if ([subview isKindOfClass:[PTYSplitView class]]) {
      ...
      [result
          addObjectsFromArray:[childSplit
                                  transitiveDividerLocationsVertical:vertical
                                                                root:root]];
    }
