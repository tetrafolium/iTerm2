...
+ (iTermInitialDirectoryMode)modeForString:(NSString *)modeString
  ...
  if ([modeString
    ...
    if (customDirectory) {
      *customDirectory =
          [iTermProfilePreferences stringForKey:KEY_WORKING_DIRECTORY
                                      inProfile:profile];
    }
    ...
    switch (objectType) {
    case iTermTabObject:
      key = [iTermProfilePreferences stringForKey:KEY_AWDS_TAB_OPTION
                                        inProfile:profile];
      if (customDirectory) {
        *customDirectory =
            [iTermProfilePreferences stringForKey:KEY_AWDS_TAB_DIRECTORY
                                        inProfile:profile];
      }
      break;
    case iTermPaneObject:
      key = [iTermProfilePreferences stringForKey:KEY_AWDS_PANE_OPTION
                                        inProfile:profile];
      if (customDirectory) {
        *customDirectory =
            [iTermProfilePreferences stringForKey:KEY_AWDS_PANE_DIRECTORY
                                        inProfile:profile];
      }
      break;
    case iTermWindowObject:
      key = [iTermProfilePreferences stringForKey:KEY_AWDS_WIN_OPTION
                                        inProfile:profile];
      if (customDirectory) {
        *customDirectory =
            [iTermProfilePreferences stringForKey:KEY_AWDS_WIN_DIRECTORY
                                        inProfile:profile];
      }
      break;
    }
...
+ (instancetype)initialDirectoryFromProfile:(Profile *)profile
  ...
  iTermInitialDirectory *initialDirectory =
      [[iTermInitialDirectory alloc] init];
  NSString *customDirectorySetting =
      [iTermProfilePreferences stringForKey:KEY_CUSTOM_DIRECTORY
                                  inProfile:profile];
...
- (void)evaluateWithOldPWD:(NSString *)oldPWD
  ...
  iTermExpressionEvaluator *evaluator = [[iTermExpressionEvaluator alloc]
      initWithInterpolatedString:self.customDirectoryFormat
                           scope:scope];
  [evaluator
