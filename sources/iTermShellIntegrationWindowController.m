...
@interface iTermShellIntegrationWindowController () <
    iTermShellIntegrationInstallerDelegate>
...
@property(nonatomic, strong) IBOutlet
    iTermShellIntegrationFirstPageViewController *firstPageViewController;
@property(nonatomic, strong)
    IBOutlet NSViewController<iTermShellIntegrationInstallerViewController>
        *secondPageViewController;
@property(nonatomic, strong)
    IBOutlet iTermShellIntegrationDownloadAndRunViewController<
        iTermShellIntegrationInstallerViewController>
        *downloadAndRunViewController;
@property(nonatomic, strong)
    IBOutlet iTermShellIntegrationPasteShellCommandsViewController
        *sendShellCommandsViewController;
@property(nonatomic, strong)
    IBOutlet NSViewController<iTermShellIntegrationInstallerViewController>
...
- (NSViewController<iTermShellIntegrationInstallerViewController> *)
    viewControllerForState:(iTermShellIntegrationInstallationState)state {
  switch (state) {
  case iTermShellIntegrationInstallationStateFirstPage:
  ...
  iTermExpectation *newExpectation =
      [self expectRegularExpression:regex
                         ...
                         willExpect:^{
                           if (text) {
                             [weakSelf.delegate
                                 shellIntegrationWindowControllerSendText:text];
                           }
                         }
...
- (iTermExpectation *)expectRegularExpression:(NSString *)regex
                                   completion:
                                       (void (^)(NSArray<NSString *> *_Nonnull))
                                           completion {
...
- (iTermExpectation *)expectRegularExpression:(NSString *)regex
  ...
  __block iTermExpectation *expectation =
      [[self.delegate shellIntegrationExpect]
          expectRegularExpression:regex
                            after:precedecessor
                       willExpect:willExpect
                       completion:^(
                           NSArray<NSString *> *_Nonnull captureGroups) {
                         if (expectation) {
                           removed = YES;
...
- (void)cancelAllExpectations {
  [_pendingExpectations
      enumerateObjectsUsingBlock:^(iTermExpectation *_Nonnull obj,
...
- (NSString *)shellIntegrationPath {
  return [NSString
      stringWithFormat:@"%@/.iterm2_shell_integration.%@", self.dotdir,
...
- (NSString *)scriptForCurrentShell {
  ...
  // Add aliases
  url =
      [[NSBundle bundleForClass:self.class] URLForResource:@"utilities-manifest"
                                             withExtension:@"txt"];
  NSString *names = [NSString stringWithContentsOfURL:url
                                             encoding:NSUTF8StringEncoding
                                                error:nil];
  names = [names
  ...
  NSArray<NSString *> *aliasCommands =
      [commands mapWithBlock:^id(NSString *command) {
        switch (self.shell) {
        case iTermShellIntegrationShellZsh:
        case iTermShellIntegrationShellBash:
        case iTermShellIntegrationShellFish:
          return [NSString stringWithFormat:@"alias %@=%@/.iterm2/%@", command,
                                            self.dotdir, command];
        case iTermShellIntegrationShellTcsh:
          return [NSString stringWithFormat:@"alias %@ %@/.iterm2/%@", command,
                                            self.dotdir, command];
...
- (NSString *)curlPipeBashCommand {
  return
      [self.downloadAndRunViewController.command stringByAppendingString:@"\n"];
}
...
- (NSString *)dotfileCommandWithBashDotfile:(NSString *)bashDotFile {
  ...
  NSString *relative_filename = [NSString
      stringWithFormat:@"%@/.iterm2_shell_integration.%@", self.dotdir,
                       iTermShellIntegrationShellString(self.shell)];

  return
      [NSString stringWithFormat:@"test -e %@%@%@ %@ source %@%@%@ %@ true\n",
                                 quote, relative_filename, quote, shell_and,
                                 quote, relative_filename, quote, shell_or];
}
...
- (NSString *)discoverShell:(BOOL)reallySend
                 completion:
                     (void (^)(NSString *shell, NSString *dotdir))completion {
  iTermExpectation *expectation = nil;
  NSString *result = [self sendText:@"echo My shell is $SHELL\n"
                         reallySend:reallySend];
  if (!reallySend) {
    result = [result
        stringByAppendingString:@"# If the result is zsh, send:\n"
...
- (NSString *)amendDotFileWithExpectation:(inout iTermExpectation **)expectation
  ...
  NSArray<NSString *> *parts = [[[self launchBashString]
  ...
  case iTermShellIntegrationShellBash: {
    NSString *assignment =
        @"IT2_INSTALLER_DOTFILE=$(test -f ~/.bash_profile && echo -n "
        @"~/.bash_profile || echo -n ~/.profile)\n";
  ...
  [strings
      addObject:[self sendText:[NSString
                                   stringWithFormat:@"    cat <<-EOF >> %@\n",
                                                    script]
                     reallySend:reallySend
                     afterRegex:@"^> "
                    expectation:expectation]];
  [strings addObject:[self sendText:[self dotfileCommandWithBashDotfile:
                                              @"$IT2_INSTALLER_DOTFILE"]
                          reallySend:reallySend
                          afterRegex:@"^> "
                         expectation:expectation]];
  ...
  joined =
      [joined stringByAppendingString:[strings componentsJoinedByString:@""]];
  return joined;
}
...
- (NSString *)catString:(NSString *)string
  ...
  [result
      appendString:[self sendText:[NSString
                                      stringWithFormat:@"cat <<'EOF' > %@\n",
                                                       path]
                        reallySend:reallySend
                        afterRegex:@"."
...
- (NSString *)catToUtilities:(BOOL)reallySend
  ...
  [result
      appendString:[self sendText:[NSString
                                      stringWithFormat:@"mkdir %@; echo ok\n",
                                                       folder]
                        reallySend:reallySend
                        afterRegex:@"."
                       expectation:expectation]];
  [result appendString:[self sendText:[NSString
                                          stringWithFormat:@"cd %@; echo ok\n",
                                                           folder]
                            reallySend:reallySend
                            afterRegex:@"^ok$"
                           expectation:expectation]];
  [result appendString:[self sendText:@"base64 --decode <<'EOF'| tar xfz -\n"
                            ...
                            afterRegex:@"^ok$"
                           expectation:expectation]];
  NSURL *url = [[NSBundle bundleForClass:self.class] URLForResource:@"utilities"
                                                      withExtension:@"tgz"];
  if (!url) {
    completion();
    return nil;
...
- (NSString *)addScriptFiles:(BOOL)reallySend {
  ...
  NSString *result = [self catToScript:reallySend
                            completion:^{
                              if (reallySend) {
                                weakSelf.sendShellCommandsViewController.stage =
                                    2;
                              }
                            }];
  return result;
...
- (NSString *)installUtilityScripts:(BOOL)reallySend {
  ...
  [result
      appendString:[self
                       catToUtilities:reallySend
                          expectation:&expectation
                           completion:^{
                             if (reallySend) {
                               [weakSelf.sendShellCommandsViewController
                                   setBusy:NO];
                               weakSelf.sendShellCommandsViewController.stage =
                                   3;
                             }
...
- (void)shellIntegrationInstallerReallyDownloadAndRun {
  [self.delegate
      shellIntegrationWindowControllerSendText:[self curlPipeBashCommand]];
  __weak __typeof(self) weakSelf = self;

  [self expectRegularExpression:
            @"(^Done.$)|(^Your shell, .*, is not supported yet)"
                     completion:^(NSArray<NSString *> *_Nonnull captureGroups) {
                       if ([captureGroups[0] hasPrefix:@"Your shell"]) {
                         [self.downloadAndRunViewController
                                 showShellUnsupportedError];
                       } else {
                         [weakSelf next:nil];
                       }
                     }];
