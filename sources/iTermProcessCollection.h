...
// This is the "real" name, not the hacked one with a leading hyphen, etc. See
// also: argv0.
@property(nonatomic, readonly, strong) NSString *name;

// This is one that is often changed by the app by modifying argv[0]. For
// example, your login shell will insert a - at the first position. Python
// changes it with setproctitle (issue 4214). NOTE: it may be nil. Fall back to
// `name` in that case.
@property(nonatomic, readonly, strong) NSString *argv0;

@property(nonatomic, readonly, strong) NSString
    *commandLine; // only set for foreground jobs and the child of login
...
// Pre-order traversal starting with self. Set *stop=YES to abort. Returns
