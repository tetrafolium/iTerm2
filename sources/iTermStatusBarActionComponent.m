...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  iTermStatusBarComponentKnob *actionKnob = [[iTermStatusBarComponentKnob alloc]
...
- (NSString *)statusBarComponentDetailedDescription {
  return @"Adds a button that performs a user-configurable action, similar to "
         @"a key binding.";
}
...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconAction"
                                forClass:[self class]];
}
...
- (NSString *)statusBarComponentDetailedDescription {
  return @"When clicked, opens a menu of actions. Actions are like custom key "
         @"bindings, but without a keystroke attached.";
}
...
- (void)openMenuWithView:(NSView *)view {
  ...
  for (iTermAction *action in [[iTermActionsModel sharedInstance] actions]) {
    NSMenuItem *item =
        [[NSMenuItem alloc] initWithTitle:action.title
...
- (void)performAction:(id)sender {
  ...
  iTermAction *action = [[iTermActionsModel sharedInstance]
      actionWithIdentifier:[menuItem.identifier integerValue]];
  if (!action) {
    return;
  }
