...
+ (BOOL)windowType:(iTermWindowType)windowType
    shouldBeCompactWithSavedWindowType:(iTermWindowType)savedWindowType {
  switch (iTermThemedWindowType(windowType)) {
  case WINDOW_TYPE_TOP:
  case WINDOW_TYPE_BOTTOM:
  case WINDOW_TYPE_LEFT:
...
- (NSWindow *)setWindowWithWindowType:(iTermWindowType)windowType
  ...
  // For reasons that defy comprehension, you have to do this when switching to
  // full-size content view style mask. Otherwise, you are left with an unusable
  // title bar.
  ...
  const BOOL compact = [PseudoTerminal windowType:windowType
               shouldBeCompactWithSavedWindowType:savedWindowType];
  Class windowClass;
  if (panel) {
    if (compact) {
  ...
  NSWindowStyleMask styleMask =
      [PseudoTerminal styleMaskForWindowType:windowTypeForStyleMask
  ...
  NSWindow<PTYWindow> *myWindow =
      [[windowClass alloc] initWithContentRect:initialFrame
                                     styleMask:styleMask
  ...
  if (windowType != WINDOW_TYPE_LION_FULL_SCREEN) {
    ...
    // However, setting the frame at this point while restoring a Lion
    // fullscreen window causes it to appear with a title bar. TODO: Test if
    // lion fullscreen windows restore on the right monitor.
  ...
  if (@available(macOS 10.16, *)) {
    ...
    NSView *view =
        [myWindow it_titlebarViewOfClassWithName:@"_NSTitlebarDecorationView"];
    [view setHidden:YES];
  }
...
- (void)changeToWindowType:(iTermWindowType)newWindowType {
  ...
  // Normal
  // No title bar
  if (![self windowTypeIsFullScreen:newWindowType] &&
      [self windowTypeIsFullScreen:self.windowType]) {
  ...
  // Because this is not a transition out of full screen, we must update the
  // saved window type before calculating the style mask. That is because the
  // style mask calculation decides whether there is a full screen content view
  // based on the *saved* window type, since it wants to preserve the value when
...
- (BOOL)replaceWindowWithWindowOfType:(iTermWindowType)newWindowType {
  ...
  iTermRootTerminalView *originalCntentView NS_VALID_UNTIL_END_OF_SCOPE =
      self.contentView;
  [self setWindowWithWindowType:newWindowType
         ...
         windowTypeForStyleMask:newWindowType
               hotkeyWindowType:self.hotkeyWindowType
                   initialFrame:[self traditionalFullScreenFrameForScreen:
                                          self.window.screen]];
...
- (void)updateWindowType {
  ...
  // -viewDidChangeEffectiveAppearance to be called, which eventually calls back
  // into this method. Then cocoa 💩s when you try to change the content view
  // from within setContentView:.
...
- (void)willEnterTraditionalFullScreenMode {
  ...
  if (self.ptyWindow.isCompact) {
    ...
    [self.window
        setFrame:[self traditionalFullScreenFrameForScreen:self.window.screen]
         display:YES];
  }
...
- (void)willExitTraditionalFullScreenMode {
  BOOL shouldForce = NO;
  if ([PseudoTerminal windowType:self.savedWindowType
          shouldBeCompactWithSavedWindowType:self.savedWindowType]) {
  ...
  // This will be close but probably not quite right because tweaking to the
  // decoration size happens later.
...
- (void)toggleTraditionalFullScreenModeImpl {
  ...
  [self didToggleTraditionalFullScreenModeWithSavedToolbeltWidth:
            savedToolbeltWidth];
...
- (void)didToggleTraditionalFullScreenModeWithSavedToolbeltWidth:
    (CGFloat)savedToolbeltWidth {
  ...
  if (_fullScreen) {
    DLog(@"toggleFullScreenMode - call "
         @"adjustFullScreenWindowForBottomBarChange");
...
- (NSRect)traditionalFullScreenFrameForScreen:(NSScreen *)screen {
  ...
  frameMinusMenuBar.size.height -=
      [[[NSApplication sharedApplication] mainMenu] menuBarHeight];
  BOOL menuBarIsVisible = NO;

  if ([self fullScreenWindowFrameShouldBeShiftedDownBelowMenuBarOnScreen:
                screen]) {
...
- (BOOL)fullScreenWindowFrameShouldBeShiftedDownBelowMenuBarOnScreen:
    (NSScreen *)screen {
  const BOOL wantToHideMenuBar =
      [iTermPreferences boolForKey:kPreferenceKeyHideMenuBarInFullscreen];
  const BOOL canHideMenuBar =
      ![[iTermApplication sharedApplication] isUIElement];
  ...
  DLog(@"Checking if the fullscreen window frame should be shifted down below "
       @"the menu bar. "
       @"wantToHideMenuBar=%@, canHideMenuBar=%@, menuIsHidden=%@, "
       @"canOverlapMenuBar=%@",
       @(wantToHideMenuBar), @(canHideMenuBar), @(menuBarIsHidden),
       @(canOverlapMenuBar));
...
- (void)enterTraditionalFullScreenMode {
  if (!togglingFullScreen_ && !togglingLionFullScreen_ &&
      ![self anyFullScreen]) {
...
- (BOOL)toggleFullScreenShouldUseLionFullScreen {
  ...
  if (self.isHotKeyWindow) {
    // NSWindowCollectionBehaviorFullScreenAuxiliary window can't enter Lion
...
- (void)delayedEnterFullscreenImpl {
  if (self.windowType == WINDOW_TYPE_LION_FULL_SCREEN &&
    ...
    if (![[[iTermController sharedInstance] keyTerminalWindow]
            lionFullScreen]) {
...
- (void)didFinishFullScreenTransitionSuccessfully:(BOOL)success {
  ...
  NSArray<void (^)(BOOL)> *blocks =
      [_toggleFullScreenModeCompletionBlocks copy];
...
- (void)windowDidFailToEnterFullScreenImpl:(NSWindow *)window {
  ...
  if (_fullScreenRetryCount < 3) {
    ...
    DLog(
        @"Increment retry count to %@ and schedule an attempt after a delay %@",
        @(_fullScreenRetryCount), self);
    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)),
        dispatch_get_main_queue(), ^{
          DLog(@"About to retry entering full screen with count %@: %@",
               @(self->_fullScreenRetryCount), self);
...
- (void)windowDidExitFullScreenImpl:(NSNotification *)notification {
  ...
  DLog(@"Window did exit fullscreen. Set window type to %d",
       self.savedWindowType);
  [self safelySetStyleMask:[PseudoTerminal
                               styleMaskForWindowType:self.savedWindowType
                                      savedWindowType:self.savedWindowType
                                     hotkeyWindowType:self.hotkeyWindowType]];
  ...
  // Windows forget their collection behavior when exiting full screen when the
  // app is a LSUIElement. Issue 8048.
...
+ (NSWindowStyleMask)styleMaskForWindowType:(iTermWindowType)windowType
  ...
  case WINDOW_TYPE_RIGHT_PARTIAL:
  case WINDOW_TYPE_NO_TITLE_BAR:
    return (mask | NSWindowStyleMaskFullSizeContentView |
  ...
  case WINDOW_TYPE_COMPACT_MAXIMIZED:
    return (mask | NSWindowStyleMaskFullSizeContentView |
            ...
            NSWindowStyleMaskTexturedBackground | NSWindowStyleMaskResizable);

  case WINDOW_TYPE_MAXIMIZED:
    return (mask | NSWindowStyleMaskTitled | NSWindowStyleMaskClosable |
            NSWindowStyleMaskMiniaturizable |
            NSWindowStyleMaskTexturedBackground | NSWindowStyleMaskResizable);
  ...
  case WINDOW_TYPE_ACCESSORY:
  case WINDOW_TYPE_NORMAL:
    if ([self windowTypeHasFullSizeContentView:iTermThemedWindowType(
                                                   savedWindowType)]) {
...
- (NSWindowStyleMask)styleMask {
  NSWindowStyleMask styleMask =
      [PseudoTerminal styleMaskForWindowType:self.windowType
                             savedWindowType:self.savedWindowType
                            hotkeyWindowType:self.hotkeyWindowType];
...
- (void)forceFrame:(NSRect)frame {
  ...
  if (NSEqualRects(frame, NSIntersectionRect(
                              frame, NSScreen.screens.firstObject.frame))) {
