...
+ (NSLayoutManager *)layoutManagerForFont:(NSFont *)aFont
  ...
  NSTextStorage *textStorage =
      [[[NSTextStorage alloc] initWithString:myString] autorelease];
...
+ (BOOL)useGenerousRoundingForFont:(NSFont *)aFont {
  for (NSString *family in
       [[iTermAdvancedSettingsModel fontsForGenerousRounding]
           componentsSeparatedByString:@","]) {
    if ([[[aFont familyName]
            lowercaseStringWithLocale:[NSLocale
...
+ (id)fontSizeEstimatorForFont:(NSFont *)aFont {
  ...
  if (fse) {
    ...
    DLog(@"Initial guess at a size for %@ is %@", aFont,
    ...
    if (CGFontGetGlyphAdvances(cgfont, glyphs, 1, advances)) {
      ...
      DLog(@"Improving my guess for width using formula round(%d * %f / %d) "
           @"giving %f",
           advances[0], [aFont pointSize], CGFontGetUnitsPerEm(cgfont),
           size.width);
    }
    ...
    if ([iTermAdvancedSettingsModel useExperimentalFontMetrics]) {
      ...
      NSLayoutManager *layoutManager =
          [self layoutManagerForFont:aFont textContainer:textContainer];
