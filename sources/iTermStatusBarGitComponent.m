...
static NSString *const iTermStatusBarGitComponentPollingIntervalKey =
    @"iTermStatusBarGitComponentPollingIntervalKey";
...
- (instancetype)
  ...
  if (self) {
    const NSTimeInterval cadence = [self
        cadenceInDictionary:
            configuration[iTermStatusBarComponentConfigurationKeyKnobValues]];
    __weak __typeof(self) weakSelf = self;
    iTermGitPoller *gitPoller =
        [[iTermGitPoller alloc] initWithCadence:cadence
                                         update:^{
    ...
    _pwdRef =
        [[iTermVariableReference alloc] initWithPath:iTermVariableKeySessionPath
                                              vendor:scope];
    _pwdRef.onChangeBlock = ^{
      DLog(@"PWD changed, update git poller directory");
      gitPoller.currentDirectory =
          [scope valueForVariableName:iTermVariableKeySessionPath];
    };
    _hostRef = [[iTermVariableReference alloc]
        initWithPath:iTermVariableKeySessionHostname
              vendor:scope];
    _hostRef.onChangeBlock = ^{
      DLog(@"Hostname changed, update git poller enabled");
      [weakSelf updatePollerEnabled];
    };
    _lastCommandRef = [[iTermVariableReference alloc]
        initWithPath:iTermVariableKeySessionLastCommand
              vendor:scope];
    _lastCommandRef.onChangeBlock = ^{
      [weakSelf bumpIfLastCommandWasGit];
    };
    _remoteObserver = [[iTermRemoteGitStateObserver alloc]
        initWithScope:scope
                block:^{
    ...
    DLog(@"Initializing git component %@ for scope of session with ID %@. "
         @"poller is %@",
         self, scope.ID, gitPoller);
  };
  return self;
}
...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconGitBranch"
                                forClass:[self class]];
}
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  ...
  iTermStatusBarComponentKnob *formatKnob = [[iTermStatusBarComponentKnob alloc]
      initWithLabelText:@"Polling Interval (seconds):"
...
+ (NSDictionary *)statusBarComponentDefaultKnobs {
  ...
  knobs = [knobs dictionaryByMergingDictionary:@{
    iTermStatusBarGitComponentPollingIntervalKey :
        @(iTermStatusBarGitComponentDefaultCadence)
  }];
  knobs =
      [knobs dictionaryByMergingDictionary:self.defaultMinMaxWidthKnobValues];
  return knobs;
}
...
- (NSArray<NSAttributedString *> *)attributedStringVariants {
  NSArray<NSAttributedString *> *result =
      [[[self variantsOfCurrentStateBranch] mapWithBlock:^id(NSString *branch) {
        return [self attributedStringValueForBranch:branch];
      }]
          sortedArrayUsingComparator:^NSComparisonResult(
              NSAttributedString *_Nonnull obj1,
...
- (NSAttributedString *)attributedStringWithString:(NSString *)string {
  NSDictionary *attributes = @{
    NSFontAttributeName : self.advancedConfiguration.font
...
- (CGFloat)statusBarComponentMinimumWidth {
  return [self
      widthForAttributedString:[self attributedStringValueForBranch:@"M"]];
}

- (BOOL)onLocalhost {
  NSString *localhostName = [NSHost fullyQualifiedDomainName];
  NSString *currentHostname = self.scope.hostname;
  DLog(@"git poller current hostname is %@, localhost is %@", currentHostname,
       localhostName);
...
- (NSArray<NSString *> *)variantsOfBranch:(NSString *)branch {
  NSIndexSet *dividers = [branch
      indicesOfCharactersInSet:
          [NSCharacterSet characterSetWithCharactersInString:@"-_./+,"]];
  NSMutableArray<NSString *> *result = [NSMutableArray array];
  [dividers enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
    const NSRange composedRange =
        [branch rangeOfComposedCharacterSequenceAtIndex:idx];
...
- (nullable NSAttributedString *)attributedStringValueForBranch:
  ...
  NSAttributedString *branch =
      branchString ? [self attributedStringWithString:branchString] : nil;
  if (!branch) {
    return nil;
  }

  NSAttributedString *upCount =
      self.currentState.pushArrow.integerValue > 0
          ? [self attributedStringWithString:self.currentState.pushArrow]
          : nil;
  NSAttributedString *downCount =
      self.currentState.pullArrow.integerValue > 0
...
- (NSTimeInterval)cadenceInDictionary:(NSDictionary *)knobValues {
  return [knobValues[iTermStatusBarGitComponentPollingIntervalKey] doubleValue]
             ?: 1;
}
...
- (void)bumpIfLastCommandWasGit {
  ...
  NSInteger lastSlash =
      [command rangeOfString:@"/" options:NSBackwardsSearch].location;
...
- (void)killSession:(id)sender {
  [[[iTermController sharedInstance] terminalWithSession:_session]
      closeSessionWithoutConfirmation:_session];
}
...
- (void)openMenuWithView:(NSView *)view {
  ...
  if (_session) {
    ...
    NSString *actionName = [_status stringByReplacingOccurrencesOfString:@"…"
                                                              withString:@""];
    NSMenuItem *item = [[NSMenuItem alloc]
        initWithTitle:[NSString stringWithFormat:@"Cancel %@", actionName]
               action:@selector(killSession:)
        keyEquivalent:@""];
    item.target = self;
    [menu addItem:item];

    item = [[NSMenuItem alloc] initWithTitle:@"Reveal"
  ...
  if (self.currentState.xcode.length > 0) {
    [iTermWarning
        showWarningWithTitle:[self.currentState.xcode
  ...
  __weak __typeof(self) weakSelf = self;
  [self
      fetchRecentBranchesWithTimeout:0.5
                          completion:^(NSArray<NSString *> *branches) {
                            NSMenu *menu = [[NSMenu alloc] init];
                            iTermGitMenuItemContext * (^addItem)(NSString *,
                                                                 SEL, BOOL) =
                                ^iTermGitMenuItemContext *(NSString *title,
                                                           SEL selector,
                                                           BOOL enabled) {
                              NSMenuItem *item = [[NSMenuItem alloc]
                                  initWithTitle:title
                                         action:enabled ? selector
                                                        : @selector
                                                    (bogusSelector)
                                  keyEquivalent:@""];
                              item.target = weakSelf;
                              iTermGitMenuItemContext *context =
                                  [[iTermGitMenuItemContext alloc] init];
                              context.state = state;
                            ...
                            addItem(@"Add & Commit", @selector(addAndCommit:),
                                    state.dirty);
                            addItem(@"Stash", @selector(stash:), state.dirty);
                            addItem(@"Log", @selector(log:), YES);
                            addItem(
                                [NSString stringWithFormat:@"Push origin %@",
                                                           state.branch],
                                @selector(push:),
                                state.pushArrow.intValue > 0 ||
                                    [state.pushArrow isEqualToString:@"error"]);
                            addItem(
                                [NSString stringWithFormat:@"Pull origin %@",
                                                           state.branch],
                                @selector(pull:), !state.dirty);
                            [menu addItem:[NSMenuItem separatorItem]];
                            for (NSString *branch in
                                 [branches it_arrayByKeepingFirstN:7]) {
                              if (branch.length == 0) {
                                continue;
                              }
                              ...
                              addItem(
                                  [NSString
                                      stringWithFormat:@"Check out %@", branch],
                                  @selector(checkout:), YES)
                                  .userData = branch;
                            }
                            [menu popUpMenuPositioningItem:menu.itemArray
                                                               .firstObject
                                                atLocation:NSMakePoint(0, 0)
                                                    inView:containingView];
                          }];
...
- (void)runGitCommandWithArguments:(NSArray<NSString *> *)args
  ...
  runner.completion = ^(int status) {
    ...
    NSString *output = [[NSString alloc] initWithData:strongRunner.output
...
static NSArray<NSString *> *NonEmptyLinesInString(NSString *output) {
  ...
  return [branches mapWithBlock:^id(NSString *branch) {
    NSString *trimmed =
        [branch stringByTrimmingCharactersInSet:[NSCharacterSet
...
- (void)fetchRecentBranchesWithTimeout:(NSTimeInterval)timeout
                            completion:(void (^)(NSArray<NSString *> *branches))
                                           completion {
  NSArray *args = @[
    @"for-each-ref", @"--count=30", @"--sort=-committerdate", @"refs/heads/",
    @"--format=%(refname:short)"
  ];
  [self runGitCommandWithArguments:args
                           timeout:timeout
                        completion:^(NSString *_Nullable output, int status) {
...
- (void)showPopover {
  _popoverVC = [[iTermTextPopoverViewController alloc]
      initWithNibName:@"iTermTextPopoverViewController"
  ...
  _popoverVC.textView.font =
      [self.delegate statusBarComponentTerminalFont:self];
  NSRect frame = _popoverVC.view.frame;
  NSSize inset = _popoverVC.textView.textContainerInset;
  frame.size.width =
      [FontSizeEstimator fontSizeEstimatorForFont:_popoverVC.textView.font]
              .size.width *
          60 +
...
- (void)log:(id)sender {
  ...
  _logRunner.outputHandler = ^(NSData *data) {
    ...
    [popoverVC
        appendString:[[NSString alloc] initWithData:data
                                           encoding:NSUTF8StringEncoding]];
...
- (void)commit:(id)sender {
  NSMenuItem *menuItem = sender;
  iTermGitMenuItemContext *context = menuItem.representedObject;
  [self runGitInWindowWithArguments:@[ @"commit" ]
...
- (void)addAndCommit:(id)sender {
  NSMenuItem *menuItem = sender;
  iTermGitMenuItemContext *context = menuItem.representedObject;
  [self runGitInWindowWithArguments:@[ @"commit", @"-a" ]
                                pwd:context.directory
                             status:@"Committing…"
                               bury:NO];
}
...
- (void)stash:(id)sender {
  NSMenuItem *menuItem = sender;
  iTermGitMenuItemContext *context = menuItem.representedObject;
  [self runGitInWindowWithArguments:@[ @"stash" ]
                                pwd:context.directory
                             status:@"Stashing…"
                               bury:YES];
}
...
- (void)push:(id)sender {
  ...
  iTermGitMenuItemContext *context = menuItem.representedObject;
  [self
      runGitInWindowWithArguments:@[ @"push", @"origin", context.state.branch ]
                              pwd:context.directory
                           status:@"Pushing…"
                             bury:YES];
}
...
- (void)pull:(id)sender {
  ...
  iTermGitMenuItemContext *context = menuItem.representedObject;
  [self
      runGitInWindowWithArguments:@[ @"pull", @"origin", context.state.branch ]
                              pwd:context.directory
                           status:@"Pulling…"
                             bury:YES];
}
...
- (void)runGitOnRemoteHost:(NSArray<NSString *> *)args {
  ...
  NSString *command = [NSString
      stringWithFormat:@"git %@", [quotedArgs componentsJoinedByString:@" "]];
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:@"Looks like you're sshed somewhere. OK "
                                     @"to send the command “%@”?",
                                     command]
                   actions:@[ @"OK", @"Cancel" ]
                 accessory:nil
  ...
  if (selection == kiTermWarningSelection0) {
    [self.delegate statusBarComponent:self
                          writeString:[command stringByAppendingString:@"\n"]];
  }
}
...
- (void)runGitInWindowWithArguments:(NSArray<NSString *> *)args
  ...
  NSString *gitWrapper = [[NSBundle bundleForClass:self.class]
      pathForResource:@"iterm2_git_wrapper"
               ofType:@"sh"];
  __weak __typeof(self) weakSelf = self;
