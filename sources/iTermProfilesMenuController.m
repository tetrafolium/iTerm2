...
+ (void)addOpenAllToMenu:(NSMenu *)menu
  ...
  NSMenuItem *openAll = [menu addItemWithTitle:@"Open All"
                                        action:params.openAllSelector
                                 keyEquivalent:@""];
  [openAll setTarget:params.target];

  // Add alternate open all menu
  NSMenuItem *altOpenAll =
      [[NSMenuItem alloc] initWithTitle:@"Open All in New Window"
...
- (int)positionOfBookmark:(Profile *)b
           startingAtItem:(int)skip
                   inMenu:(NSMenu *)menu {
...
- (int)positionOfBookmarkWithIndex:(int)theIndex
                    startingAtItem:(int)skip
                            inMenu:(NSMenu *)menu {
  // Find position of bookmark in menu
  int N = [menu numberOfItems];
...
+ (NSString *)identifierForMenuItem:(NSMenuItem *)item {
  NSString *prefix = item.isAlternate
                         ? iTermProfileModelNewWindowMenuItemIdentifierPrefix
...
- (void)addBookmark:(Profile *)b
  ...
  NSMenuItem *item = [[NSMenuItem alloc]
      initWithTitle:[b objectForKey:KEY_NAME]
  ...
  if ([shortcut length]) {
    ...
    [item setKeyEquivalentModifierMask:NSEventModifierFlagCommand |
                                       ...
                                       (isAlternate ? NSEventModifierFlagOption
                                                    : 0)];
  } else if (isAlternate) {
    [item setKeyEquivalentModifierMask:NSEventModifierFlagOption];
  }
  [item setTarget:params.target];
  NSString *guid = [NSString castFrom:[b objectForKey:KEY_GUID]]
                       ?: [[NSUUID UUID] UUIDString];
  [item setRepresentedObject:[guid copy]];
  [item setTag:tag];
  if ([[NSString castFrom:b[KEY_CUSTOM_COMMAND]]
...
- (void)addBookmark:(Profile *)b
  ...
  if (theIndex == -1) {
    ...
    pos = [self positionOfBookmarkWithIndex:theIndex
                             startingAtItem:skip
                                     inMenu:menu];
  }
  ...
  if ([[self class] menuHasMultipleItemsExcludingAlternates:menu
                                                  fromIndex:skip] &&
...
+ (void)applyRemoveJournalEntry:(BookmarkJournalEntry *)e
  ...
  for (NSString *tag in e.tags) {
    ...
    for (int i = items.count - 1; i >= 0; i--) {
      ...
      if (submenu) {
        if (i == items.count - 1) {
          [self applyRemoveJournalEntry:e
                                 toMenu:submenu
                         startingAtItem:0
                                 params:params];
        }
        if ([submenu numberOfItems] == 0) {
          ...
          if ([self.class menuHasOpenAll:submenu] &&
              submenu.numberOfItems <= 5) {
...
+ (void)applyJournal:(NSDictionary *)journalDict
  ...
  for (BookmarkJournalEntry *e in journal) {
    switch (e.action) {
    case JOURNAL_ADD:
      [self.class applyAddJournalEntry:e
                                toMenu:menu
                        startingAtItem:skip
                                params:params];
      break;
    ...
    case JOURNAL_REMOVE:
      [self.class applyRemoveJournalEntry:e
                                   toMenu:menu
                           startingAtItem:skip
                                   params:params];
      break;
    ...
    case JOURNAL_REMOVE_ALL:
      [self.class applyRemoveAllJournalEntry:e
                                      toMenu:menu
                              startingAtItem:skip
                                      params:params];
      break;
    ...
    case JOURNAL_SET_DEFAULT:
      [self.class applySetDefaultJournalEntry:e
                                       toMenu:menu
                               startingAtItem:skip
                                       params:params];
      break;
...
+ (void)applyJournal:(NSDictionary *)journal
              toMenu:(NSMenu *)menu
              params:(iTermProfileModelJournalParams *)params {
  [self.class applyJournal:journal toMenu:menu startingAtItem:0 params:params];
}
