...
- (NSInteger)indexContainingValue:(NSInteger)value {
  // Subtract the offset because the offset is negative and our values are
  // higher than what is exposed by the owner's interface.
...
- (NSInteger)verboseIndexContainingValue:(NSInteger)value {
  ...
  // higher than what is exposed by the owner's interface.
  const NSInteger adjustedValue = value - _offset;
  DLog(@"adjusted value is %@ because offset is %@", @(adjustedValue),
       @(_offset));
  auto it = std::lower_bound(_sums.begin(), _sums.end(), adjustedValue);
  if (it == _sums.end()) {
...
- (void)setFirstValue:(NSInteger)value {
  ...
  if (_sums.size() > 1) {
    // Only ok to _drop_ lines from the first block when there are others after
    // it.
    assert(delta <= 0);
  }
  _offset += delta;
