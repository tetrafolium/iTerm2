...
// This is a hack to draw attributed strings because NSTextField randomly shifts
// the baseline up one point when using an attributed string. When AppKit is
// dead and buried I'll revisit this.
...
- (void)drawString:(NSString *)string
  ...
  const CGFloat maxWidth =
      reallyDraw ? self.bounds.size.width - point.x : INFINITY;
  if (reallyDraw) {
    NSColor *textColor = attrs[NSForegroundColorAttributeName];
    if (!textColor) {
      attrs = [attrs dictionaryBySettingObject:self.textColor
    ...
    NSRect rect = {
        ...
        .size = {.width = self.bounds.size.width - point.x, .height = height}};
    [string drawInRect:rect withAttributes:attrs];
  }
  *width =
      [self retinaRound:MIN(maxWidth, [string sizeWithAttributes:attrs].width)];
}
...
- (void)drawRect:(NSRect)dirtyRect width:(out nullable CGFloat *)widthOut {
  const BOOL reallyDraw =
      (dirtyRect.size.width > 0 && dirtyRect.size.height > 0);
  ...
  [_attributedStringValue
      enumerateAttributesInRange:NSMakeRange(0, _attributedStringValue.length)
                         options:0
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id>
                                       *_Nonnull attrs,
                                   NSRange range, BOOL *_Nonnull stop) {
                        CGFloat width = 0;
                        [self drawString:[self->_attributedStringValue.string
...
- (NSArray<iTermStatusBarComponentKnob *> *)statusBarComponentKnobs {
  iTermStatusBarComponentKnob *backgroundColorKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Background Color:"
  ...
  iTermStatusBarComponentKnob *textColorKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Text Color:"
                       type:iTermStatusBarComponentKnobTypeColor
                placeholder:nil
               defaultValue:nil
                        key:iTermStatusBarSharedTextColorKey];
  return [@[ backgroundColorKnob, textColorKnob ]
      arrayByAddingObjectsFromArray:[super statusBarComponentKnobs]];
}

- (NSColor *)textColor {
  NSDictionary *knobValues =
      self.configuration[iTermStatusBarComponentConfigurationKeyKnobValues];
  NSColor *configuredColor =
      [knobValues[iTermStatusBarSharedTextColorKey] colorValue];
  if (configuredColor) {
...
- (NSColor *)backgroundColor {
  ...
  return [knobValues[iTermStatusBarSharedBackgroundColorKey] colorValue]
             ?: [super statusBarBackgroundColor];
}

- (BOOL)shouldUpdateValue:(NSAttributedString *)proposed
                  inField:(id<iTermTextFieldish>)textField {
  ...
  if (textFieldHasString != iHaveString) {
    DLog(@"%@ updating because nilness changed. textfield=%@ proposed=%@", self,
         textField.attributedStringValue, proposed);
    return YES;
  }
  if (textFieldHasString || iHaveString) {
    BOOL result = ![NSObject object:textField.attributedStringValue
                    isEqualToObject:proposed];
    if (result) {
      DLog(@"%@ updating because %@ != %@", self,
           textField.attributedStringValue, proposed);
    }
    return result;
  }
...
- (BOOL)setValueInField:(id<iTermTextFieldish>)textField
...
- (NSArray<iTermTuple<NSAttributedString *, NSNumber *> *> *)
    widthAttributedStringTuples {
  return [self.attributedStringVariants
      mapWithBlock:^id(NSAttributedString *anObject) {
...
- (nullable NSAttributedString *)attributedStringForWidth:(CGFloat)width {
  ...
  NSArray<iTermTuple<NSAttributedString *, NSNumber *> *> *tuples =
      [self widthAttributedStringTuples];
  tuples = [tuples filteredArrayUsingBlock:^BOOL(
                       iTermTuple<NSAttributedString *, NSNumber *> *anObject) {
    return ceil(anObject.secondObject.doubleValue) <= ceil(width);
  }];
  return [tuples maxWithBlock:^NSComparisonResult(
                     iTermTuple<NSAttributedString *, NSNumber *> *obj1,
                     iTermTuple<NSAttributedString *, NSNumber *> *obj2) {
...
- (CGFloat)statusBarComponentVerticalOffset {
  ...
  const CGFloat frameY =
      (containerHeight - _attributedStringView.frame.size.height) / 2;
...
- (CGFloat)statusBarComponentMinimumWidth {
  NSArray<iTermTuple<NSAttributedString *, NSNumber *> *> *tuples =
      [self widthAttributedStringTuples];
  NSNumber *number =
      [tuples minWithBlock:^NSComparisonResult(
                  iTermTuple<NSAttributedString *, NSNumber *> *obj1,
                  iTermTuple<NSAttributedString *, NSNumber *> *obj2) {
        return [obj1.secondObject compare:obj2.secondObject];
      }].secondObject;
...
- (NSView *)statusBarComponentView {
  if (!_attributedStringView) {
    ...
    _attributedStringView.drawsBackground =
        (self.backgroundColor.alphaComponent > 0);
  }
