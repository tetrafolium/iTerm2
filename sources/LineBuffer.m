...
static NSString *const kLineBufferMayHaveDWCKey =
    @"May Have Double Width Character";
...
@implementation LineBuffer {
  ...
  // The default storage for a LineBlock (some may be larger to accommodate very
  // long lines).
  int block_size;

  // If a cursor size is saved, this gives its offset from the start of its
  // line.
  int cursor_x;
  ...
  // The maximum number of lines to store. In truth, more lines will be stored,
  // but no more than max_lines will be exposed by the interface.
  int max_lines;
...
- (instancetype)init {
  // I picked 8k because it's a multiple of the page size and should hold about
  // 100-200 lines on average. Very small blocks make finding a wrapped line
  // expensive because caching the number of wrapped lines is spread out over
  // more blocks. Very large blocks are expensive because of the linear search
  // through a block for the start of a wrapped line. This is in the middle.
  // Ideally, the number of blocks would equal the number of wrapped lines per
...
// The designated initializer. We prefer not to expose the notion of block sizes
// to clients, so this is internal.
...
- (LineBuffer *)initWithDictionary:(NSDictionary *)dictionary {
  ...
  if (self) {
    ...
    _mayHaveDoubleWidthCharacter =
        [dictionary[kLineBufferMayHaveDWCKey] boolValue];
...
- (int)dropExcessLinesWithWidth:(int)width {
  ...
  if (max_lines != -1 && nl > max_lines) {
    ...
    while (total_lines > max_lines) {
      ...
      int charsDropped;
      int dropped = [block dropLines:toDrop
                           withWidth:width
                               chars:&charsDropped];
...
- (NSString *)debugString {
  ...
  return [s length] ? [s substringToIndex:s.length - 1]
...
- (NSString *)compactLineDumpWithWidth:(int)width
                  andContinuationMarks:(BOOL)continuationMarks {
  NSMutableString *s = [NSMutableString string];
  int n = [self numLinesWithWidth:width];
  for (int i = 0; i < n; i++) {
    screen_char_t continuation;
    ScreenCharArray *line = [self wrappedLineAtIndex:i
                                               width:width
    ...
    [s appendFormat:@"%@",
                    ScreenCharArrayToStringDebug(line.line, line.length)];
  ...
  if (![block appendLine:buffer
    ...
    if ([block hasPartial]) {
      ...
      BOOL ok = [block popLastLineInto:&temp
                          ...
                          continuation:NULL];
      assert(ok);
      prefix = (screen_char_t *)iTermMalloc(MAX(1, prefix_len) *
                                            sizeof(screen_char_t));
      memcpy(prefix, temp, prefix_len * sizeof(screen_char_t));
      ITAssertWithMessage(ok, @"hasPartial but pop failed.");
    }
    if ([block isEmpty]) {
      // The buffer is empty but it's not large enough to hold a whole line. It
      // must be grown.
      if (partial) {
        // The line is partial so we know there's more coming. Allocate enough
        // space to hold the current line plus the usual block size (this is the
        // case when the line is freaking huge). We could double the size to
        // ensure better asymptotic runtime but you'd run out of memory faster
        // with huge lines.
      ...
      // The existing buffer can't hold this line, but it has preceding line(s).
      // Shrink it and allocate a new buffer that is large enough to hold this
      // line.
      [block shrinkToFit];
    ...
    // Append the prefix if there is one (the prefix was a partial line that
    // we're moving out of the last block into the new block)
    if (prefix) {
    ...
    // Finally, append this line to the new block. We know it'll fit because we
    // made enough room for it.
...
- (int)copyLineToBuffer:(screen_char_t *)buffer
  ...
  LineBlock *block = [_lineBlocks blockContainingLineNumber:lineNum
                                                      width:width
                                                  remainder:&remainder];
  ITBetaAssert(remainder >= 0,
               @"Negative lineNum BEFORE consuming block_lines");
  if (!block) {
  ...
  screen_char_t *p = [block getWrappedLineWithWrapWidth:width
                                      ...
                                      includesEndOfLine:&eol
                                           continuation:&continuation];
  if (p == nil) {
    ITAssertWithMessage(NO, @"Nil wrapped line %@ for block with width %@",
                        @(requestedLine), @(width));
    return NO;
  }
  ...
  [self extendContinuation:continuation
                  inBuffer:buffer
                  ofLength:length
                   toWidth:width];
...
- (ScreenCharArray *)wrappedLineAtIndex:(int)lineNum
  ...
  if (!block) {
    ITAssertWithMessage(
        NO, @"Failed to find line %@ with width %@. Cache is: %@", @(lineNum),
        @(width),
        [[[[_lineBlocks dumpForCrashlog] dataUsingEncoding:NSUTF8StringEncoding]
            it_compressedData] it_hexEncoded]);
    return nil;
  }
...
- (NSArray<ScreenCharArray *> *)wrappedLinesFromIndex:(int)lineNum
  ...
  [_lineBlocks
      enumerateLinesInRange:NSMakeRange(lineNum, count)
                      width:width
                      block:^(screen_char_t *_Nonnull chars, int length,
                              int eol, screen_char_t continuation,
                              BOOL *_Nonnull stop) {
                        ScreenCharArray *lineResult =
                            [[[ScreenCharArray alloc] init] autorelease];
...
- (BOOL)getCursorInLastLineWithWidth:(int)width atX:(int *)x {
  ...
  if (cursor_rawline == total_raw_lines - 1) {
    ...
    int num_overflow_lines = [block numberOfFullLinesFromBuffer:lastRawLine
                                                         length:last_line_length
                                                          width:width];
#if BETA
    const int legacy_num_overflow_lines = iTermLineBlockNumberOfFullLinesImpl(
        lastRawLine, last_line_length, width, _mayHaveDoubleWidthCharacter);
    assert(num_overflow_lines == legacy_num_overflow_lines);
#endif

    int min_x =
        OffsetOfWrappedLine(lastRawLine, num_overflow_lines, last_line_length,
                            width, _mayHaveDoubleWidthCharacter);
    // int num_overflow_lines = (last_line_length-1) / width;
    // int min_x = num_overflow_lines * width;
    int max_x = min_x + width; // inclusive because the cursor wraps to the next
                               // line on the last line in the buffer
...
- (BOOL)_findPosition:(LineBufferPosition *)start
             ...
             inOffset:(int *)offset {
  LineBlock *block =
      [_lineBlocks blockContainingPosition:start.absolutePosition - droppedChars
                                     width:-1
                                 remainder:offset
                               blockOffset:NULL
                                     index:block_num];
...
- (void)findSubstring:(FindContext *)context
               stopAt:(LineBufferPosition *)stopPosition {
  NSInteger blockIndex = context.absBlockNum - num_dropped_blocks;
  const NSInteger numBlocks =
      _lineBlocks
          .count; // This avoids involving unsigned integers in comparisons
  if (context.dir > 0) {
    // Search forwards
    ...
    if (blockIndex >= numBlocks) {
      DLog(@"Out of bounds index %@ (>=%@) in backward search", @(blockIndex),
           @(numBlocks));
      context.status = NotFound;
      return;
    }
  ...
  if (blockIndex == 0 && context.offset != -1 &&
    ...
    if (context.dir > 0) {
      ...
      // NSLog(@"offset=%d, block's startOffset=%d. give up", context.offset,
      // [block startOffset]);
      context.status = NotFound;
      return;
    }
  ...
  // NSLog(@"search block %d starting at offset %d", context.absBlockNum -
  // num_dropped_blocks, context.offset);
  ...
  NSMutableArray *filtered =
      [NSMutableArray arrayWithCapacity:[context.results count]];
  BOOL haveOutOfRangeResults = NO;
  int blockPosition =
      [self _blockPosition:context.absBlockNum - num_dropped_blocks];
...
- (NSArray *)convertPositions:(NSArray *)resultRanges withWidth:(int)width {
  ...
  NSMutableArray *unsortedPositions =
      [NSMutableArray arrayWithCapacity:[resultRanges count] * 2];
  for (ResultRange *rr in resultRanges) {
    [unsortedPositions addObject:[NSNumber numberWithInt:rr->position]];
    [unsortedPositions
        addObject:[NSNumber numberWithInt:rr->position + rr->length - 1]];
  }

  // Walk blocks and positions in parallel, converting each position in order.
  // Store in intermediate dict, mapping position->NSPoint(x,y)
  NSArray *positionsArray =
      [unsortedPositions sortedArrayUsingSelector:@selector(compare:)];
  int i = 0;
  int yoffset = 0;
  ...
  NSMutableDictionary *intermediate =
      [NSMutableDictionary dictionaryWithCapacity:[resultRanges count] * 2];
  int prev = -1;
  ...
  // Walk the positions array and populate results by looking up points in
  // intermediate dict.
  NSMutableArray *result =
      [NSMutableArray arrayWithCapacity:[resultRanges count]];
  for (ResultRange *rr in resultRanges) {
    NSValue *start =
        [intermediate objectForKey:[NSNumber numberWithInt:rr->position]];
    NSValue *end = [intermediate
        objectForKey:[NSNumber numberWithInt:rr->position + rr->length - 1]];
    if (start && end) {
...
- (LineBufferPosition *)positionForCoordinate:(VT100GridCoord)coord
  ...
  int line = y;
  NSInteger index = [_lineBlocks indexOfBlockContainingLineNumber:y
                                                            width:width
                                                        remainder:&line];
  if (index == NSNotFound) {
    return nil;
  ...
  long long absolutePosition =
      droppedChars +
      [_lineBlocks rawSpaceUsedInRangeOfBlocks:NSMakeRange(0, index)];
  ...
  pos = [block getPositionOfLine:&line
                       ...
                       withWidth:width
                         ...
                         extends:&extends];
  if (pos == 0 && yOffset == 1) {
    // getPositionOfLine:… will set yOffset to 1 when returning the first cell
    // of a raw line to disambiguate the position. That's the right thing to do
    // except for the very first cell in a block, where the position is
    // unambiguous.
    yOffset = 0;
  }
  if (pos < 0) {
  ...
  [self coordinateForPosition:result
                        width:width
                 extendsRight:YES // doesn't matter for deciding if the result
                                  // is valid
                           ok:&ok];
  if (ok) {
    return result;
...
- (VT100GridCoord)coordinateForPosition:(LineBufferPosition *)position
  ...
  if (position.absolutePosition == self.lastPosition.absolutePosition) {
    ...
    result.y = MAX(0, [self numLinesWithWidth:width] - 1 -
                          [_lineBlocks.lastBlock numberOfTrailingEmptyLines]);
  ...
  int p;
  int yoffset;
  LineBlock *block = [_lineBlocks
      blockContainingPosition:position.absolutePosition - droppedChars
                        width:width
                    remainder:&p
                  blockOffset:&yoffset
                        index:NULL];
  ...
  BOOL positionIsValid =
      [block convertPosition:p
                   withWidth:width
                   wrapOnEOL:NO //  using extendsRight here is wrong because
                                //  extension happens below
                         toX:&x
                         toY:&y];
  if (ok) {
    *ok = positionIsValid;
...
- (long long)absPositionOfFindContext:(FindContext *)findContext {
  if (findContext.absBlockNum < 0) {
    ...
    return findContext.offset + droppedChars +
           [_lineBlocks
               rawSpaceUsedInRangeOfBlocks:NSMakeRange(0, _lineBlocks.count)];
  }
  const int numBlocks =
      MIN(_lineBlocks.count, findContext.absBlockNum - num_dropped_blocks);
  const NSInteger rawSpaceUsed =
      numBlocks > 0
          ? [_lineBlocks rawSpaceUsedInRangeOfBlocks:NSMakeRange(0, numBlocks)]
          : 0;
...
- (long long)absPositionOfAbsBlock:(int)absBlockNum {
  ...
  return droppedChars +
         [_lineBlocks
             rawSpaceUsedInRangeOfBlocks:NSMakeRange(0,
                                                     absBlockNum -
                                                         num_dropped_blocks)];
}
      ...
      [iTermOrderedDictionary
          byMappingEnumerator:_lineBlocks.blocks.reverseObjectEnumerator
                        block:^id _Nonnull(NSUInteger index,
                          ...
                          DLog(@"Maybe encode block %p with guid %@", block,
              ...
              generation:iTermGenerationAlwaysEncode
                 options:iTermGraphEncoderArrayOptionsReverse
                   block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder,
                     ...
                     DLog(@"Encode %@ with identifier %@ and generation %@",
                          block, identifier, @(block.generation));
                     return [encoder
                         encodeDictionaryWithKey:kLineBufferBlockWrapperKey
                                      generation:block.generation
                                           block:^BOOL(
                                             ...
                                             assert(!truncated);
                                             DLog(@"Really encode block %p "
                                                  @"with guid %@",
                                                  block,
                                                  block.stringUniqueIdentifier);
                                             [encoder mergeDictionary:
                                                          block.dictionary];
                                             // This caps the amount of data at
                                             // a reasonable but arbitrary size.
                                             numLines += [block
                                                 getNumLinesWithWrapWidth:80];
...
- (LineBuffer *)appendOnlyCopyWithMinimumLines:(int)minLines
                                       atWidth:(int)width {
  // Calculate how many blocks to keep.
  const int numBlocks = [self numBlocksAtEndToGetMinimumLines:minLines
                                                        width:width];
  ...
  theCopy->droppedChars +=
      [self numCharsInRangeOfBlocks:NSMakeRange(0, numDroppedBlocks)];
