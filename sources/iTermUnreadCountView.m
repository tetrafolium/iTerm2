...
- (NSView *)newBubbleViewWithTextField:(NSTextField *)textField {
  ...
  shapeLayer.fillColor = [[NSColor colorWithSRGBRed:0.976
                                              green:0.243
                                               blue:0.223
                                              alpha:0.92] CGColor];
  if (@available(macOS 10.14, *)) {
  ...
  textField.frame =
      NSMakeRect((NSWidth(view.frame) - NSWidth(textField.frame)) / 2.0,
                 (NSHeight(view.frame) - NSHeight(textField.frame)) / 2.0,
                 NSWidth(textField.frame), NSHeight(textField.frame));
  return view;
}
