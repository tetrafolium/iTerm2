...
- (instancetype)initWithTime:(NSTimeInterval)t
                         age:(NSTimeInterval)age NS_DESIGNATED_INITIALIZER;
...
- (NSString *)description {
  ...
  return [NSString stringWithFormat:@"<%@: %p %@>",
                                    NSStringFromClass(self.class), self, data];
}
...
- (void)setCurrentLatency:(NSTimeInterval)age forPane:(int)wp {
  ...
  if (now - series.lastUpdateTime > 0.5) {
    iTermTmuxBufferSizeDataPoint *dataPoint =
        [[iTermTmuxBufferSizeDataPoint alloc] initWithTime:now age:age];
    [series addDataPoint:dataPoint];
  }
}

- (NSTimeInterval)ttlForSeries:(iTermTimeSeries *)series
...
- (void)update:(NSTimer *)timer {
  const NSTimeInterval now = [iTermTmuxBufferSizeDataPoint now];
  [_series enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull wp,
                                               iTermTimeSeries *_Nonnull series,
                                               BOOL *_Nonnull stop) {
    if (series.count < 4) {
    ...
    [self maybeWarnPane:wp.intValue
                    ttl:ttl
                lastTTL:series.lastTTL
                 series:series];
    series.lastTTL = ttl;
  }];
}
...
- (void)maybeWarnPane:(int)wp
  ...
  [self.delegate tmuxBufferSizeMonitor:self
                            updatePane:wp
                                   ttl:ttl
                               redzone:redzone];
}

- (BOOL)shouldWarnPaneWithTTL:(NSTimeInterval)ttl
                      lastTTL:(NSTimeInterval)lastTTL
