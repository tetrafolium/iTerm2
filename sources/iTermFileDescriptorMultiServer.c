...
// 2: A pipe that can be used to detect this process's termination. Do nothing
// with it. 3: A pipe we can read on. 4: Advisory lock on
// iterm2-daemon-1.socket.lock. Remains open for the lifetime of the process.
typedef enum {
...
static void LogChild(const iTermMultiServerChild *child) {
  FDLog(
      LOG_DEBUG,
      "masterFd=%d, pid=%d, willTerminate=%d, terminated=%d, status=%d, tty=%s",
      child->masterFd, child->pid, child->willTerminate, child->terminated,
      child->status, child->tty ?: "(null)");
}

static void AddChild(const iTermMultiServerRequestLaunch *launch, int masterFd,
                     const char *tty, const iTermForkState *forkState) {
  if (!children) {
    ...
    children = realloc(children,
                       (numberOfChildren + 1) * sizeof(iTermMultiServerChild));
  }
  const int i = numberOfChildren;
  numberOfChildren += 1;
  iTermMultiServerClientOriginatedMessage tempClientMessage = {
      .type = iTermMultiServerRPCTypeLaunch, .payload = {.launch = *launch}};

  // Copy the launch request into children[i].messageWithLaunchRequest. This is
  // done because we need to own our own pointers to arrays of strings.
  iTermClientServerProtocolMessage tempMessage;
  iTermClientServerProtocolMessageInitialize(&tempMessage);
  int status;
  status = iTermMultiServerProtocolEncodeMessageFromClient(&tempClientMessage,
                                                           &tempMessage);
  assert(status == 0);
  status = iTermMultiServerProtocolParseMessageFromClient(
      &tempMessage, &children[i].messageWithLaunchRequest);
  assert(status == 0);
...
static void RemoveChild(int i) {
  ...
  if (numberOfChildren == 1) {
    ...
    memmove(children + i, children + i + 1, sizeof(*children) * afterCount);
...
static int Launch(const iTermMultiServerRequestLaunch *launch,
                  iTermForkState *forkState, iTermTTYState *ttyStatePtr,
                  int *errorPtr) {
  iTermTTYStateInit(
      ttyStatePtr, iTermTTYCellSizeMake(launch->columns, launch->rows),
  ...
  forkState->pid =
      forkpty(&fd, ttyStatePtr->tty, &ttyStatePtr->term, &ttyStatePtr->win);
  if (forkState->pid == (pid_t)0) {
    // Child
    iTermExec(launch->path, (const char **)launch->argv,
              1, /* close file descriptors */
              0, /* restore resource limits */
              forkState, launch->pwd, launch->envp, fd);
  }
  if (forkState->pid == 1) {
    *errorPtr = errno;
...
static int SendLaunchResponse(int fd, int status, pid_t pid, int masterFd,
                              const char *tty, unsigned long long uniqueId) {
  ...
  iTermMultiServerServerOriginatedMessage message = {
      ...
      .payload = {
          .launch = {
              .status = status, .pid = pid, .uniqueId = uniqueId, .tty = tty}}};
  const int rc =
  ...
  if (masterFd >= 0) {
    ...
    int error = 0;
    result = iTermFileDescriptorServerWriteLengthAndBufferAndFileDescriptor(
        fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len, masterFd,
        &error);
    if (result < 0) {
      FDLog(LOG_ERR,
            "ERROR: SendLaunchResponse: Failed to send master FD with %s",
            strerror(error));
    }
  } else {
    ...
    int error;
    result = iTermFileDescriptorServerWriteLengthAndBuffer(
        fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len, &error);
    if (result < 0) {
...
static int HandleLaunchRequest(int fd,
                               const iTermMultiServerRequestLaunch *launch) {
  ...
  if (masterFd < 0) {
    return SendLaunchResponse(fd, -1 /* status */, 0 /* pid */,
                              -1 /* masterFd */, "" /* tty */,
                              launch->uniqueId);
  }
  ...
  return SendLaunchResponse(fd, 0 /* status */, forkState.pid, masterFd,
                            ttyState.tty, launch->uniqueId);
}
...
static int ReportTermination(int fd, pid_t pid) {
  ...
  int error;
  ssize_t result = iTermFileDescriptorServerWriteLengthAndBuffer(
      fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len, &error);
  if (result < 0) {
    FDLog(LOG_ERR, "SendMsg failed with %s", strerror(error));
  }
...
static void PopulateReportChild(const iTermMultiServerChild *child, int isLast,
...
static int ReportChild(int fd, const iTermMultiServerChild *child, int isLast) {
  ...
  int theError = 0;
  ssize_t bytes =
      iTermFileDescriptorServerWriteLengthAndBufferAndFileDescriptor(
          fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len,
          child->masterFd, &theError);
  if (bytes < 0) {
...
static int WaitForAllProcesses(int connectionFd) {
  ...
  for (int i = 0; i < numberOfChildren; i++) {
    ...
    if (pid > 0) {
      FDLog(LOG_DEBUG, "Child with pid %d exited with status %d", (int)pid,
            children[i].status);
      children[i].terminated = 1;
      if (!children[i].willTerminate && connectionFd >= 0 &&
...
static int ReportChildren(int fd) {
  ...
  // Iterate backwards because ReportAndRemoveDeadChild deletes the index passed
  // to it.
  ...
  for (int i = numberOfChildren - 1; i >= 0; i--) {
    ...
    if (ReportChild(fd, &children[i],
                    numberSent + 1 == numberOfReportableChildren)) {
...
static int HandleHandshake(int fd,
                           iTermMultiServerRequestHandshake *handshake) {
  FDLog(LOG_DEBUG, "Handle handshake maximumProtocolVersion=%d",
  ...
  if (handshake->maximumProtocolVersion < iTermMultiServerProtocolVersion2) {
    FDLog(LOG_ERR, "Maximum protocol version is too low: %d",
          handshake->maximumProtocolVersion);
    return -1;
  }
  iTermMultiServerServerOriginatedMessage message = {
      ...
      .payload = {
          .handshake = {.protocolVersion = iTermMultiServerProtocolVersion2,
  ...
  int error;
  ssize_t bytes = iTermFileDescriptorServerWriteLengthAndBuffer(
      fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len, &error);
  if (bytes < 0) {
    FDLog(LOG_ERR, "SendMsg failed with %s", strerror(error));
  }
...
static int HandleWait(int fd, iTermMultiServerRequestWait *wait) {
  FDLog(LOG_DEBUG, "Handle wait request for pid=%d preemptive=%d", wait->pid,
        wait->removePreemptively);
  ...
  iTermMultiServerResponseWaitResultType resultType =
      iTermMultiServerResponseWaitResultTypeStatusIsValid;
  if (childIndex < 0) {
...
static void HexDump(iTermClientServerProtocolMessage *message) {
  ...
  const unsigned char *bytes =
      (const unsigned char *)message->message.msg_iov[0].iov_base;
  int addr = 0;
  int offset = 0;
...
static void AcceptAndReject(int socket) {
  ...
  FDLog(LOG_DEBUG,
        "Received connection attempt while already connected. Send rejection.");
  ...
  int error;
  const ssize_t result = iTermFileDescriptorServerWriteLengthAndBuffer(
      fd, obj.ioVectors[0].iov_base, obj.ioVectors[0].iov_len, &error);
  if (result < 0) {
    FDLog(LOG_ERR, "SendMsg failed with %s", strerror(error));
  }
...
// There is a client connected. Respond to requests from it until it
// disconnects, then return.
  ...
  while (1) {
    ...
    if (results[2]) {
      ...
      if (ReadAndHandleRequest(readFd, writeFd)) {
        ...
        if (results[0]) {
          FDLog(LOG_DEBUG, "Client hung up and also have SIGCHLD to deal with. "
                           "Wait for processes.");
...
static int MakeAndSendPipe(int unixDomainSocketFd) {
  ...
  const ssize_t rc =
      iTermFileDescriptorServerWriteLengthAndBufferAndFileDescriptor(
          unixDomainSocketFd, obj.ioVectors[0].iov_base,
          obj.ioVectors[0].iov_len, writePipe, &sendFDError);
  iTermClientServerProtocolMessageFree(&obj);
  if (rc == -1) {
    FDLog(LOG_ERR, "Failed to send write file descriptor: %s",
          strerror(sendFDError));
    close(readPipe);
    readPipe = -1;
  }
...
static int iTermMultiServerAccept(int socketFd) {
  ...
  while (1) {
    ...
    if (results[1]) {
      FDLog(LOG_DEBUG, "SIGCHLD pipe became readable while waiting for "
                       "connection. Calling wait...");
      WaitForAllProcesses(-1);
    ...
    FDLog(LOG_DEBUG, "accept() returned %d error=%s", connectionFd,
          strerror(errno));
  }
  return connectionFd;
}
...
static void MainLoop(char *path, int acceptFd, int initialWriteFd,
                     int initialReadFd) {
  ...
  do {
    ...
    if (GetNumberOfReportableChildren() == 0) {
      // Not attached and no children? Quit rather than leave a useless daemon
      // running.
      FDLog(LOG_DEBUG,
            "Exiting because no reportable children remain. %d terminating.",
            numberOfChildren);
      return;
    }
...
static int MakeBlocking(int fd) {
  ...
  FDLog(LOG_DEBUG, "MakeBlocking(%d) returned %d (%s)", fd, rc,
        strerror(errno));
  return rc == -1;
}
...
static int MakePipe(void) {
  ...
  for (int i = 0; i < 2; i++) {
    if (MakeNonBlocking(gPipe[i])) {
      FDLog(LOG_ERR, "Failed to set gPipe[%d] nonblocking: %s", i,
            strerror(errno));
      return 2;
    }
  }
...
static int InitializeSignals(void) {
  ...
  if (sigprocmask(SIG_UNBLOCK, &signal_set, NULL) == -1) {
    FDLog(LOG_ERR, "sigprocmask(SIG_UNBLOCK, &signal_set, NULL) failed with %s",
          strerror(errno));
    return 1;
  }
  ...
  if (rc == SIG_ERR) {
    FDLog(LOG_ERR, "signal(SIGCHLD, SigChildHandler) failed with %s",
          strerror(errno));
    return 1;
  }
...
// NOTE: If I am ever forced to use a CFRunLoop in this process, then I can get
// a callback when the port is invalidated by using CFMachPortCreateWithPort,
// CFMachPortSetInvalidationCallBack, CFRunLoopAddSource, and CFRunLoopRun. See
// TN2050 mirrored at:
...
static void CheckIfBootstrapPortIsDead(void) {
  ...
  if (mach_port_type(mach_task_self(), port, &type) != KERN_SUCCESS) {
    FDLog(LOG_ERR, "Unable to get the type of the bootstrap port! errno=%d",
          errno);
    QuitCleanly();
  }
  if (type == MACH_PORT_TYPE_DEAD_NAME) {
    FDLog(LOG_ERR,
          "The bootstrap port has type DEAD. This indicates the user's session "
          "has died and this process is unusable. This can happen after a "
          "logout-login sequence. iTermServer will now terminate.");
    QuitCleanly();
  }
...
static int iTermFileDescriptorMultiServerRun(char *path, int socketFd,
...
// There should be a single command-line argument, which is the path to the
// unix-domain socket I'll use.
