...
+ (void)addTriggerForText:(NSString *)text
  ...
  NSPanel *panel =
      [[iTermFocusablePanel alloc] initWithContentRect:NSZeroRect
                                             styleMask:NSWindowStyleMaskTitled
                                               backing:NSBackingStoreBuffered
                                                 defer:NO
                                                screen:nil];
  // List of characters to escape comes from ICU's documentation for the
  // backslash meta-character. Some characters like - only need to be escaped
  // inside [sets] but it's safe to escape them outside sets as well.
  ...
  iTermAddTriggerViewController *vc = [[iTermAddTriggerViewController alloc]
      ...
      defaultBackgroundColor:defaultBackgroundColor
                  completion:^(NSDictionary *_Nullable dict,
                               BOOL updateProfile) {
                    [window endSheet:panel
                          returnCode:dict ? NSModalResponseOK
                                          : NSModalResponseCancel];
                    completion(dict, updateProfile);
                  }];
  [panel it_setAssociatedObject:vc forKey:"AddTriggerVC"];
  [panel setFrame:[NSPanel frameRectForContentRect:vc.view.bounds
...
- (instancetype)initWithName:(NSString *)name
      ...
      defaultBackgroundColor:(NSColor *)defaultBackgroundColor
                  completion:
                      (void (^)(NSDictionary *_Nullable, BOOL))completion {
...
- (void)viewDidLoad {
  ...
  _instantButton.state = [iTermUserDefaults addTriggerInstant]
                             ? NSControlStateValueOn
                             : NSControlStateValueOff;
  _updateProfileButton.state = [iTermUserDefaults addTriggerUpdateProfile]
                                   ? NSControlStateValueOn
                                   : NSControlStateValueOff;
  _triggers =
      [[TriggerController triggerClasses] mapWithBlock:^id(Class triggerClass) {
        return [[triggerClass alloc] init];
      }];
  [_triggers enumerateObjectsUsingBlock:^(Trigger *_Nonnull trigger,
  ...
  NSInteger i = [_triggers
      indexOfObjectPassingTest:^BOOL(Trigger *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
...
- (IBAction)ok:(id)sender {
  ...
  const BOOL updateProfile =
      _updateProfileButton.state == NSControlStateValueOn;
...
- (void)updateCustomViewForTrigger:(Trigger *)trigger value:(id)value {
  NSView *view = [TriggerController
      viewForParameterForTrigger:self.currentTrigger
                            size:NSMakeSize(
                                     _paramContainerView.frame.size.width, 21)
                           value:value
                        receiver:self
             interpolatedStrings:_interpolatedStrings
                     wellFactory:^CPKColorWell *(NSRect frame, NSColor *color) {
                       CPKColorWell *well =
                           [[CPKColorWell alloc] initWithFrame:frame];
