...
- (void)setProfiles:(NSMutableDictionary *)aDict {
  ...
  } else // if we don't have any profile, create a default profile
  {
    ...
    defaultName = NSLocalizedStringFromTableInBundle(
...
- (void)addProfileWithName:(NSString *)newProfile
               copyProfile:(NSString *)sourceProfile {
...
- (void)setEncoding:(NSStringEncoding)encoding
  ...
  [aProfile setObject:[NSNumber numberWithUnsignedInt:(unsigned int)encoding]
               forKey:@"Encoding"];
}
...
- (void)setScrollbackLines:(int)lines forProfile:(NSString *)profileName {
  ...
  [aProfile setObject:[NSNumber numberWithInt:(lines < 0 ? -1 : lines)]
               forKey:@"Scrollback"];
}
...
- (void)setCloseOnSessionEnd:(BOOL)shouldClose
                  forProfile:(NSString *)profileName {
  NSMutableDictionary *aProfile;
  ...
  [aProfile setObject:[NSNumber numberWithBool:shouldClose]
               forKey:@"Auto Close"];
}
...
- (void)setDoubleWidth:(BOOL)doubleWidth forProfile:(NSString *)profileName {
  ...
  [aProfile setObject:[NSNumber numberWithBool:doubleWidth]
               forKey:@"Double Width"];
}
...
- (void)setXtermMouseReporting:(BOOL)xtermMouseReporting
                    forProfile:(NSString *)profileName {
  NSMutableDictionary *aProfile;
  ...
  [aProfile setObject:[NSNumber numberWithBool:xtermMouseReporting]
               forKey:@"Xterm Mouse Reporting"];
}
...
- (void)setAppendTitle:(BOOL)appendTitle forProfile:(NSString *)profileName {
  ...
  [aProfile setObject:[NSNumber numberWithBool:appendTitle]
               forKey:@"Append Title"];
}
...
- (void)setNoResizing:(BOOL)noResizing forProfile:(NSString *)profileName {
  ...
  [aProfile setObject:[NSNumber numberWithBool:noResizing]
               forKey:@"No Resizing"];
}

- (void)updateBookmarkNode:(TreeNode *)node
                forProfile:(NSString *)oldProfile
                      with:(NSString *)newProfile {
  int i;
  TreeNode *child;
  NSDictionary *aDict;
  ...
  for (i = 0; i < n; i++) {
    ...
    if ([child isLeaf]) {
      ...
      if ([[aDict objectForKey:KEY_TERMINAL_PROFILE]
              isEqualToString:oldProfile]) {
        NSMutableDictionary *newBookmark =
            [[NSMutableDictionary alloc] initWithDictionary:aDict];
...
- (void)updateBookmarkProfile:(NSString *)oldProfile
                         with:(NSString *)newProfile {
  [self updateBookmarkNode:[[ITAddressBookMgr sharedInstance] rootNode]
                forProfile:oldProfile
                      with:newProfile];
