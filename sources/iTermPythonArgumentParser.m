...
- (BOOL)argsLookLikeRepl:(NSArray<NSString *> *)args {
  if ([args[0] isEqualToString:@"aioconsole"]) {
    if (args.count == 1) {
      return YES;
    } else if (args.count == 3 && [args[1] isEqualToString:@"--no-readline"] &&
               [args[2] hasPrefix:@"--banner="]) {
      return YES;
    } else if (args.count == 4 && [args[1] isEqualToString:@"--no-readline"] &&
...
- (void)parse {
  ...
  for (NSString *arg in _args) {
    ...
    if ([arg isEqualToString:@"-m"]) {
      ...
      NSArray *moduleArgs = [@[ [arg substringFromIndex:2] ]
          arrayByAddingObjectsFromArray:[_args subarrayFromIndex:i + 1]];
      [self handleModule:moduleArgs];
      return;
    } else if ([arg isEqualToString:@"-Q"] || [arg isEqualToString:@"-W"]) {
...
- (void)handleModule:(NSArray *)args {
  // If a module is specified that changes how Python parses its command line so
  // we cannot go on.
  if ([self argsLookLikeRepl:args]) {
    // Except for when it's aioconsole with known arguments. That's just the
    // REPL.
    _repl = YES;
    return;
  }
...
- (NSString *)escapedScript {
  return
      [_script stringWithBackslashEscapedShellCharactersIncludingNewlines:YES];
}
