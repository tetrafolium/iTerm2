...
- (instancetype)
    initWithRegularExpression:(NSString *)regex
                   willExpect:(void (^)(iTermExpectation *))willExpect
                   completion:
                       (void (^)(iTermExpectation *,
                                 NSArray<NSString *> *_Nonnull))completion {
  self = [super init];
  if (self) {
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p regex=%@ hasCompleted=%@ successors=%@>",
                       NSStringFromClass(self.class), self, _regex,
                       @(self.hasCompleted), _successor];
}
...
- (iTermExpectation *)
    expectRegularExpression:(NSString *)regex
                 completion:
                     (void (^)(NSArray<NSString *> *captureGroups))completion {
...
- (iTermExpectation *)
    expectRegularExpression:(NSString *)regex
                 ...
                 completion:(void (^_Nullable)(NSArray<NSString *> *_Nonnull))
                                completion {
  __weak __typeof(self) weakSelf = self;
  void (^internalWillExpect)(iTermExpectation *) =
      ^(iTermExpectation *expectation) {
        if (willExpect) {
          willExpect();
        }
        [weakSelf addExpectation:expectation];
      };
  void (^internalCompletion)(iTermExpectation *, NSArray<NSString *> *) =
      ^(iTermExpectation *expectation, NSArray<NSString *> *captureGroups) {
  ...
  iTermExpectation *expectation =
      [[iTermExpectation alloc] initWithRegularExpression:regex
...
- (void)setTimeout:(NSTimeInterval)timeout
    forExpectation:(iTermExpectation *)expectation {
  __weak __typeof(self) weakSelf = self;
  dispatch_after(
