...
- (void)setCurrentTerminal:(PseudoTerminal *)thePseudoTerminal {
  _generation++;
  DLog(@"setCurrentTerminal:%@ generation<-%@", thePseudoTerminal,
       @(_generation));
...
- (BOOL)shouldDeferSetCurrentTerminal:(PseudoTerminal *)thePseudoTerminal {
  // When "minimize to application icon" is on then making a miniaturized window
  // key puts it into a broken state where it's visible but does not accept
  // keypresses.
...
- (void)scheduleSetCurrentTerminalRetry:(PseudoTerminal *)thePseudoTerminal
...
- (void)setCurrentTerminal:(PseudoTerminal *)thePseudoTerminal
  ...
  if ([self shouldDeferSetCurrentTerminal:thePseudoTerminal]) {
    [self scheduleSetCurrentTerminalRetry:thePseudoTerminal
