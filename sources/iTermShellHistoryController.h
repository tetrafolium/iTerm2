...
// Switch between in-memory and on-disk backing store based on global
// preference.
...
// Has a command ever been saved to history? Suggests user already knows about
// shell integration.
...
// Just like commandHistoryEntriesWithPrefix:onHost, but returns command uses
// rather than entries. Only the most recent use is returned.
...
// Record that a directory was entered. Set isChange to YES if the directory has
// just changed, or NO if it's a repeat of the last path on this host.
...
// Indicate which components in entry's path may be safely abbreviated because
// they aren't needed to disambiguate.
