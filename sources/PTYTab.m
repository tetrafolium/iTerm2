...
NSString *const iTermTabDidChangeWindowNotification =
    @"iTermTabDidChangeWindowNotification";
NSString *const iTermSessionBecameKey = @"iTermSessionBecameKey";

// No user output/idle alerts for a few seconds after a window is resized
// because there will be bogus bg activity
...
static NSString *TAB_ARRANGEMENT_COLOR =
    @"Tab color"; // DEPRECATED - Each PTYSession has its own tab color now
...
// The "grain" runs perpendicular to the splitters. An example with
// isVertical==YES:
// +----------------+
// |     |     |    |
// |     |     |    |
...
@property(nonatomic, strong)
    NSMapTable<SessionView *, PTYSession *> *viewToSessionMap;
@end
...
@implementation PTYTab {
  ...
  __weak NSWindowController<iTermWindowController>
      *realParentWindow_;        // non-nil only if parent is PseudoTerminal*.
                                 // Implements optional methods of protocol.
  ...
  // The root of a tree of split views whose leaves are SessionViews. The root
  // is the view of the NSTabViewItem.
  //
  // NSTabView -> NSTabViewItem -> NSSplitView (root) -> ... -> SessionView ->
  // PTYScrollView -> etc.
  NSSplitView *root_;

  // The active pane is maximized, meaning there are other panes that are
  // hidden.
  BOOL isMaximized_;

  // Holds references to invisible session views. The key is a number that
  // corresponds to the TAB_ARRANGEMENT_ID in a saved arrangement. It is used
  // when a split pane is maximized to hold on to the SessionView's that are not
  // currently visible. It used to be necessary to prevent SessionView's from
  // getting released, but now that PTYSession has a strong reference to
  // SessionView I'm not sure if we still need this.
...
+ (NSImage *)idleImageWithAppearance:(NSAppearance *)appearance {
  // There was a separate idle graphic, but I prefer NewOutput. The distinction
  // is already drawn because a spinner is present only while new output is
  // being received. It's still in the git repo, named "Idle.png".
...
+ (void)_recursiveRegisterSessionsInArrangement:(NSDictionary *)arrangement {
  if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE]
          isEqualToString:VIEW_TYPE_SPLITTER]) {
    ...
    // Is a session view
    [PTYSession
        registerSessionInArrangement:arrangement[TAB_ARRANGEMENT_SESSION]];
  }
}

+ (void)registerSessionsInArrangement:(NSDictionary *)arrangement {
  [self _recursiveRegisterSessionsInArrangement:arrangement
...
+ (NSSize)cellSizeForBookmark:(Profile *)bookmark {
  ...
  const CGFloat hspace =
      [[bookmark objectForKey:KEY_HORIZONTAL_SPACING] doubleValue];
  const CGFloat vspace =
      [[bookmark objectForKey:KEY_VERTICAL_SPACING] doubleValue];
...
+ (NSSize)sizeForTmuxWindowWithAffinity:(NSString *)affinity
  ...
  const int columns =
      MIN(iTermMaxInitialSessionSize, [profile[KEY_COLUMNS] intValue]);
  const int rows =
      MIN(iTermMaxInitialSessionSize, [profile[KEY_ROWS] intValue]);
...
- (instancetype)initWithSession:(PTYSession *)session
                   parentWindow:(NSWindowController<iTermWindowController> *)
                                    parentWindow {
  self = [super init];
  if (self) {
...
- (instancetype)initWithRoot:(NSSplitView *)root
                    sessions:
                        (NSMapTable<SessionView *, PTYSession *> *)sessions {
  self = [super init];
  if (self) {
    ...
    for (SessionView *sessionView in [self sessionViews]) {
      [self.viewToSessionMap setObject:[sessions objectForKey:sessionView]
                                forKey:sessionView];
    }
  }
  return self;
...
- (void)commonInit {
  ...
  _variables =
      [[iTermVariables alloc] initWithContext:iTermVariablesSuggestionContextTab
                                        owner:self];
  ...
  [self.variablesScope setValue:[iTermVariables globalInstance]
               forVariableNamed:iTermVariableKeyGlobalScopeName];
  [self.variablesScope setValue:[@(self.uniqueId) stringValue]
               forVariableNamed:iTermVariableKeyTabID];
  ...
  _tabTitleOverrideSwiftyString = [[iTermSwiftyString alloc]
...
- (void)encodeWithCoder:(NSCoder *)coder {
  // NSTabViewItem called this in one crash. We don't have a decoder, but at
  // least we can avoid dying by implementing a do-nothing method. It doesn't
  // seem to be common.
}
...
- (NSString *)description {
  if (self.tmuxTab) {
    return [NSString
        stringWithFormat:
            @"<%@: %p label=%@ objectCount=%@ tmuxWindow=%@ tmuxController=%@>",
            NSStringFromClass([self class]), self, tabViewItem_.label,
            @(objectCount_), @(self.tmuxWindow), self.tmuxController];
  }
...
- (BOOL)updatePaneTitles {
  ...
  const BOOL statusBarsOnTop =
      (([iTermPreferences
       ...
       perPaneStatusBars);
  const BOOL anySessionHasTopStatusBar =
      statusBarsOnTop && [sessions anyWithBlock:^BOOL(PTYSession *session) {
        return [iTermProfilePreferences boolForKey:KEY_SHOW_STATUS_BAR
                                         inProfile:session.profile];
      }];
  const BOOL shouldShowTitles =
      (showTitles && [sessions count] > 1) || anySessionHasTopStatusBar;
  for (PTYSession *aSession in sessions) {
    const BOOL shouldShowBottomStatusBar =
        (perPaneStatusBars && !statusBarsOnTop &&
    ...
    const BOOL changedBottomStatusBar =
        [aSession.view setShowBottomStatusBar:shouldShowBottomStatusBar
                             adjustScrollView:!self.isTmuxTab];
...
- (void)updateIcon {
  if (_state & kPTYTabDeadState) {
    [self setIcon:[PTYTab deadImageWithAppearance:self.realParentWindow.window
                                                      .effectiveAppearance]];
  } else if (_state & kPTYTabBellState) {
    [self setIcon:[PTYTab bellImage]];
  } else if ([iTermPreferences
                 boolForKey:kPreferenceKeyShowNewOutputIndicator] &&
             (_state & (kPTYTabNewOutputState))) {
    [self
        setIcon:[PTYTab
                    imageForNewOutputWithAppearance:self.realParentWindow.window
                                                        .effectiveAppearance]];
  } else if ([iTermPreferences
                 boolForKey:kPreferenceKeyShowNewOutputIndicator] &&
             (_state & kPTYTabIdleState)) {
    [self setIcon:[PTYTab idleImageWithAppearance:self.realParentWindow.window
                                                      .effectiveAppearance]];
  } else {
    [self setIcon:nil];
  }
...
- (void)updateTabTitleForCurrentSessionName:(NSString *)newName {
  ...
  if (value.length == 0) {
    if (self.tmuxTab) {
      NSString *tmuxWindowName = [self.variablesScope
          valueForVariableName:iTermVariableKeyTabTmuxWindowName];
  ...
  [self.variablesScope setValue:value
               forVariableNamed:iTermVariableKeyTabTitle];
  [tabViewItem_
      setLabel:value]; // PSM uses bindings to bind the label to its title
...
- (BOOL)sessionInitiatedResize:(PTYSession *)session
                         width:(int)width
                        height:(int)height {
  return [parentWindow_ sessionInitiatedResize:session
                                         width:width
                                        height:height];
}

- (void)addSession:(PTYSession *)session
    toRestorableSession:(iTermRestorableSession *)restorableSession {
...
- (void)setActiveSession:(PTYSession *)session
    updateActivityCounter:(BOOL)updateActivityCounter {
  ...
  if (activeSession_ == nil) {
    ...
    [self.variablesScope setValue:nil
                 forVariableNamed:iTermVariableKeyTabCurrentSession];
    return;
  }
  if (changed) {
    [parentWindow_ setWindowTitle];
  ...
  [self.variablesScope setValue:activeSession_.variables
               forVariableNamed:iTermVariableKeyTabCurrentSession];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermSessionBecameKey
                    object:activeSession_
                  userInfo:@{@"changed" : @(changed)}];
  // If the active session changed in the active tab in the key window then
  // update the focused state of all sessions in that window.
...
// Do a depth-first search for a leaf with viewId==requestedId. Returns nil if
// not found under 'node'.
- (SessionView *)_recursiveSessionViewWithId:(int)requestedId
                                      atNode:(NSSplitView *)node {
  for (NSView *v in [node subviews]) {
    if ([v isKindOfClass:[NSSplitView class]]) {
      SessionView *sv = [self _recursiveSessionViewWithId:requestedId
                                                   atNode:(NSSplitView *)v];
      if (sv) {
        return sv;
      }
    } else {
...
- (NSColor *)flexibleViewColor {
  if ([realParentWindow_ anyFullScreen] &&
      [iTermAdvancedSettingsModel useBlackFillerColorForTmuxInFullScreen]) {
    return [NSColor blackColor];
  }

  NSColor *backgroundColor =
      [self.activeSession.colorMap colorForKey:kColorMapBackground];
  ...
  const CGFloat alpha = self.realParentWindow.useTransparency
                            ? (1.0 - self.activeSession.transparency)
                            : 1.0;
...
- (void)updateFlexibleViewColors {
  ...
  const NSSize delta =
      NSMakeSize(flexibleView_.frame.size.width - root_.frame.size.width,
                 flexibleView_.frame.size.height - root_.frame.size.height);
  if (![realParentWindow_ anyFullScreen] && delta.width >= 0 &&
      delta.width < cellSize.width && delta.height >= 0 &&
      delta.height < cellSize.height) {
    DLog(@"Root is slightly smaller than flexible view so use terminal "
         @"background color for flexible view's background");
    // Root is just slightly smaller than flexibleView, by less than the size of
    // a character. Set flexible view's color to the default background color
    // for tmux tabs.
    NSColor *bgColor;
    bgColor = [ITAddressBookMgr decodeColor:profile[KEY_BACKGROUND_COLOR]];
    if ([self.delegate tabShouldUseTransparency:self]) {
      CGFloat alpha =
          1.0 - [iTermProfilePreferences floatForKey:KEY_TRANSPARENCY
                                           inProfile:profile];
      if (alpha < 1) {
...
- (void)setParentWindow:(NSWindowController<iTermWindowController> *)theParent {
  ...
  if (parentWindow_ && theParent && parentWindow_ != theParent) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermTabDidChangeWindowNotification
                      object:self];
  }
...
- (void)setTabViewItem:(NSTabViewItem *)theTabViewItem {
  ...
  // The tab view item holds a reference to us. So we don't hold a reference to
  // it.
...
// This is KVO-observed by PSMTabBarControl and determines whether the activity
// indicator is visible.
- (BOOL)isProcessing {
  return (![iTermPreferences hideTabActivityIndicator] && isProcessing_ &&
          ![self isForegroundTab]);
}

- (void)setIsProcessing:(BOOL)aFlag {
  if (aFlag != isProcessing_) {
    DLog(@"Set processing flag of %@ from %@ to %@", self, @(isProcessing_),
         @(aFlag));
    isProcessing_ = aFlag;
...
- (NSArray<PTYSession *> *)sessionsAtTop {
  ...
  return [self.sessions
      minimumsWithComparator:^NSComparisonResult(
          PTYSession *_Nonnull session1, PTYSession *_Nonnull session2) {
        const CGFloat y1 = round(NSMinY([root_ convertRect:session1.view.bounds
                                                  fromView:session1.view]));
        const CGFloat y2 = round(NSMinY([root_ convertRect:session2.view.bounds
                                                  fromView:session2.view]));
...
- (NSArray<PTYSession *> *)sessionsAtLeft {
  ...
  return [self.sessions
      minimumsWithComparator:^NSComparisonResult(
          PTYSession *_Nonnull session1, PTYSession *_Nonnull session2) {
        const CGFloat y1 = round(NSMinX([root_ convertRect:session1.view.bounds
                                                  fromView:session1.view]));
        const CGFloat y2 = round(NSMinX([root_ convertRect:session2.view.bounds
                                                  fromView:session2.view]));
        return [@(y1) compare:@(y2)];
      }];
}
...
- (NSArray<PTYSession *> *)sessionsAtBottom {
  ...
  return [self.sessions
      maximumsWithComparator:^NSComparisonResult(
          PTYSession *_Nonnull session1, PTYSession *_Nonnull session2) {
        const CGFloat y1 = round(NSMaxY([root_ convertRect:session1.view.bounds
                                                  fromView:session1.view]));
        const CGFloat y2 = round(NSMaxY([root_ convertRect:session2.view.bounds
                                                  fromView:session2.view]));
        return [@(y1) compare:@(y2)];
      }];
}
...
//     C.right < B.left, AND
//
// To select adjacent-right-of instead of adjacent-left-of, multiply use
// -1*right where it says left and -1*left where it says right. To select
// adjacent-below instead of adjacent-right-of, swap all axes. To select
// adjacent-above instead of adjacent-right-of, first swap all axes and then use
// -right and -left in place of left and right.
//
// As an example:
//
// +-------+------+----+
...
- (NSArray<PTYSession *> *)sessionsSatisfying:
    (BOOL (^)(PTYSession *otherSession))condition {
...
- (NSArray<PTYSession *> *)sessionsInProjectionOfSession:(PTYSession *)aSession
                                       verticalDirection:(BOOL)verticalDirection
                                                   after:(BOOL)after {
  NSRect aRect = [root_ convertRect:aSession.view.frame
                           fromView:aSession.view.superview];
  ...
  return [self sessionsSatisfying:^BOOL(PTYSession *b) {
    ...
    NSRect bRect = [self->root_ convertRect:b.view.frame
                                   fromView:b.view.superview];
    if (verticalDirection) {
      SwapSize(&bRect.size);
    ...
    return (bBottom >= aTop + safetyMargin && bTop + safetyMargin < aBottom &&
            bLeft >= aRight);
  }];
}
...
- (NSArray<PTYSession *> *)sessionsAdjacentToSession:(PTYSession *)aSession
                                         verticalDir:(BOOL)verticalDir
                                               after:(BOOL)after {
  NSArray<PTYSession *> *bCandidates =
      [self sessionsInProjectionOfSession:aSession
  ...
  return [bCandidates filteredArrayUsingBlock:^BOOL(PTYSession *b) {
    BOOL cExists = [bCandidates anyWithBlock:^BOOL(PTYSession *cCandidate) {
      return [self session:cCandidate
               sitsBetween:aSession
                       and:b
...
- (BOOL)session:(PTYSession *)c
  ...
  return (cBottom >= aTop && cTop < aBottom && cBottom >= bTop &&
          cTop < bBottom && cLeft >= aRight && cRight < bLeft);
}

- (PTYSession *)sessionAdjacentTo:(PTYSession *)session
                      verticalDir:(BOOL)verticalDir
                            after:(BOOL)after {
  NSArray<PTYSession *> *sessions = [self sessionsAdjacentToSession:session
                                                        verticalDir:verticalDir
                                                              after:after];
  if (sessions.count || ![iTermAdvancedSettingsModel wrapFocus]) {
    return [sessions
        maxWithComparator:^NSComparisonResult(PTYSession *a, PTYSession *b) {
          return [a.activityCounter compare:b.activityCounter];
        }];
  } else {
    sessions = [self sessionsInProjectionOfSession:session
                                 verticalDirection:verticalDir
                                             after:!after];
    NSArray<PTYSession *> *wraparounds =
        [sessions minimumsWithComparator:^NSComparisonResult(PTYSession *a,
                                                             PTYSession *b) {
          NSRect aRect = [self->root_ convertRect:a.view.frame
                                         fromView:a.view.superview];
          NSRect bRect = [self->root_ convertRect:b.view.frame
                                         fromView:b.view.superview];
          if (verticalDir) {
            SwapSize(&aRect.size);
...
- (NSArray *)sessions {
  if (idMap_) {
    ...
    NSMutableArray *result =
        [NSMutableArray arrayWithCapacity:[sessionViews count]];
    for (SessionView *sessionView in sessionViews) {
      [result addObject:[self sessionForSessionView:sessionView]];
    }
    return result;
  } else {
    return [self _recursiveSessions:[NSMutableArray arrayWithCapacity:1]
                             atNode:root_];
  }
}

- (NSArray<PTYSession *> *)_recursiveSessions:
                               (NSMutableArray<PTYSession *> *)sessions
                                       atNode:(NSSplitView *)node {
...
- (NSArray<SessionView *> *)sessionViews {
  if (idMap_) {
    ...
    return [self _recursiveSessionViews:[NSMutableArray arrayWithCapacity:1]
                                 atNode:root_];
  }
}

- (NSArray<SessionView *> *)_recursiveSessionViews:
                                (NSMutableArray<SessionView *> *)sessionViews
                                            atNode:(NSSplitView *)node {
  for (id subview in [node subviews]) {
    if ([subview isKindOfClass:[NSSplitView class]]) {
...
- (void)session:(PTYSession *)synthetic setLiveSession:(PTYSession *)live {
  ...
  [self setFakeParentWindow:[[FakeWindow alloc]
                                initFromRealWindow:realParentWindow_
                                           session:live]];
...
// 2. The root splitview may have exactly one child iff that child is a
// SessionView
...
//       Move grandchildren into splitView's parent (orientation -1^(n-1), same
//       as child)
...
- (void)checkInvariants:(NSSplitView *)node when:(NSString *)when {
  if (node != root_) {
    if ([node isKindOfClass:[NSSplitView class]]) {
      // 1. A non-root splitview must have at least two children.
      ITCriticalError(
          [[node subviews] count] > 1,
          @"A non-root splitview must have at least two children. %@", when);
      NSSplitView *parentSplit = (NSSplitView *)[node superview];
      // 3. A non-root splitview's orientation must be the opposite of its
      // parent's.
      ITCriticalError([node isVertical] != [parentSplit isVertical],
                      @"A non-root splitview's orientation must be the "
                      @"opposite of its parent's. %@",
                      when);
    } else {
      if ([[node subviews] count] == 1) {
        NSView *onlyChild = [[node subviews] objectAtIndex:0];
        // The root splitview may have exactly one child iff that child is a
        // SessionView.
        ITCriticalError([onlyChild isKindOfClass:[SessionView class]],
                        @"The root splitview may have exactly one child iff "
                        @"that child is a SessionView. %@",
                        when);
      }
    }
  }
  ...
  if (initialNumberOfSubviews == 1) {
    ...
    if ([onlyChild isKindOfClass:[NSSplitView class]]) {
      if (splitView == root_) {
        ...
        NSUInteger splitViewIndex =
            [[splitViewParent subviews] indexOfObjectIdenticalTo:splitView];
        assert(splitViewIndex != NSNotFound);
        NSView *referencePoint =
            splitViewIndex > 0
                ? [[splitViewParent subviews] objectAtIndex:splitViewIndex - 1]
                : nil;
        ...
        for (NSView *grandchild in [[onlyChild subviews] copy]) {
          [grandchild removeFromSuperview];
          [splitViewParent addSubview:grandchild
...
- (void)_recursiveRemoveView:(NSView *)theView {
  ...
  if (parentSplit) {
    // When a session is in instant replay, both the live session (which has no
    // superview) and the fakey DVR session are called here. If parentSplit is
    // null the it's the live session and there's nothing to do here. Otherwise,
    // it's the one that is visible and we take this path.
...
- (PTYSession *)_recursiveSessionAtPoint:(NSPoint)point
                              relativeTo:(__kindof NSView *)node {
  ...
  if ([node isKindOfClass:[SessionView class]]) {
    ...
    for (NSView *child in [splitView subviews]) {
      PTYSession *theSession = [self _recursiveSessionAtPoint:point
                                                   relativeTo:child];
      if (theSession) {
        return theSession;
      }
...
- (void)fitSubviewsToRoot {
  ...
  // Make scrollbars the right size and put them at the tops of their session
  // views.
  for (PTYSession *theSession in [self sessions]) {
    NSSize theSize =
        [theSession idealScrollViewSizeWithStyle:[parentWindow_ scrollerStyle]];
    [[theSession.view scrollview]
        setFrame:NSMakeRect(0, 0, theSize.width, theSize.height)];
...
- (void)reallyChangeSessionFontSize:(PTYSession *)session
  ...
  if (adjustWindow && ![[self parentWindow] anyFullScreen] &&
      [iTermPreferences
          boolForKey:kPreferenceKeyAdjustWindowForFontSizeChange]) {
...
- (SessionView *)nearestNeighborOfSession:(PTYSession *)aSession {
  ...
  if ([[parentSplit subviews] count] > 1) {
    // Do a depth-first search to find the first descendent of the neighbor that
    // is a SessionView and make it active.
    int theIndex =
        [[parentSplit subviews] indexOfObjectIdenticalTo:[aSession view]];
...
- (void)removeSession:(PTYSession *)aSession {
  ...
  // Grab the nearest neighbor (arbitrarily, the subview before if there is on
  // or after if not) to make its earliest descendent that is a session active.
...
- (BOOL)canSplitVertically:(BOOL)isVertical withSize:(NSSize)newSessionSize {
  ...
  if (isVertical == [parentSplit isVertical]) {
    ...
    // This is a slightly bogus heuristic: if any sibling of the active session
    // has a violated min size constraint then splits are no longer possible.
    ...
    // Another bogus heuristic: if the active session's constraints have been
    // violated then you can't split.
...
- (void)arrangeSplitPanesEvenlyInSplitView:(NSSplitView *)splitView {
  ...
  if (splitView.vertical) {
    size = splitView.frame.size.width -
           splitView.subviews.count * splitView.dividerThickness;
  } else {
...
- (void)arrangeTmuxSplitPanesEvenly {
  [tmuxController_ setLayoutInWindowPane:self.tmuxWindow
                           toLayoutNamed:@"tiled"];
}
...
- (void)splitVertically:(BOOL)isVertical
  ...
  SessionView *newView =
      [[SessionView alloc] initWithFrame:[targetSessionView frame]];
  ...
  if ([[parentSplit subviews] count] == 1) {
    ...
    // The parent has the opposite orientation splits and has many children. We
    // need to do this:
    // 1. Remove the active SessionView from its parent
    // 2. Replace it with an 'isVertical'-orientation NSSplitView
    // 3. Add two children to the 'isVertical'-orientation NSSplitView: the
    // active session and the new view.
    NSSplitView *newSplit =
        [[PTYSplitView alloc] initWithFrame:[targetSessionView frame]];
    ...
    [[targetSessionView superview] replaceSubview:targetSessionView
                                             with:newSplit];
...
+ (NSSize)_sessionSizeWithCellSize:(NSSize)cellSize
  ...
  DLog(@"    cell size is %@",
       NSStringFromSize(NSMakeSize(charWidth, lineHeight)));
  size.width = columns * charWidth +
               [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2;
  size.height = rows * lineHeight +
  ...
  DLog(@"    term=%@, hasScrollbar=%@, scrollerStyle=%@, "
       @"NSScroller.preferredScrollerStyle=%@",
       term, @(hasScrollbar), @([term scrollerStyle]),
       @([NSScroller preferredScrollerStyle]));
  ...
  DLog(@"session size, including space for the scrollview's decoration, is %@",
       NSStringFromSize(outerSize));
  return outerSize;
}
...
- (NSSize)_sessionSize:(SessionView *)sessionView {
  ...
  return [PTYTab
      _sessionSizeWithCellSize:NSMakeSize([[session textview] charWidth],
...
- (NSSize)_minSessionSize:(SessionView *)sessionView {
  ...
  NSSize scrollViewSize = [PTYScrollView
      frameSizeForContentSize:size
      horizontalScrollerClass:nil
        verticalScrollerClass:hasScrollbar ? [PTYScroller class] : nil
                   borderType:NSNoBorder
...
- (NSSize)_recursiveSize:(NSSplitView *)node
            containsLock:(BOOL *)containsLockOut {
  ...
  *containsLockOut = NO;
  ...
  // Iterate over each subview and add up the width/height of each plus
  // dividers. If there is a discrepancy in height/width, prefer the subview
  // that contains a locked session; else, take the max.
  for (id subview in [node subviews]) {
    NSSize subviewSize;
    if (first) {
      first = NO;
    ...
    if ([subview isKindOfClass:[NSSplitView class]]) {
      ...
      subviewSize = [self _recursiveSize:(NSSplitView *)subview
                            containsLock:&subviewContainsLock];
    ...
    if ([node isVertical]) {
      // Vertical splitters have their subviews arranged horizontally so widths
      // add and height goes to the tallest.
      if (size.height == 0) {
      ...
      // Nonvertical splitters have subviews arranged vertically so heights add
      // and width goes to the widest.
      size.height += subviewSize.height;
      if (size.width == 0) {
        // Take the cross-grain size of the first subview.
        size.width = subviewSize.width;
...
// Return the minimum size of a tree of splits so that no session is smaller
// than MIN_SESSION_COLUMNS columns by MIN_SESSION_ROWS rows.
...
// This returns the content size that would best fit the existing panes. It is
// the minimum size that fits them without having to resize downwards.
- (NSSize)size {
  BOOL ignore;
...
- (NSSize)currentSize {
  if (_reportIdeal) {
    DLog(@"Reporting ideal size %@ for tab %@", NSStringFromSize(self.size),
         self);
    return [self size];
  } else {
    DLog(@"Reporting size %@ of root frame for tab %@",
         NSStringFromSize(root_.frame.size), self);
...
- (void)setSize:(NSSize)newSize {
  if ([self isTmuxTab]) {
    ...
    PtyLog(@"PTYTab setSize:%fx%f", (float)newSize.width,
...
- (void)_drawSession:(PTYSession *)session
...
- (void)_recursiveDrawSplit:(NSSplitView *)splitView
  ...
  for (NSView *subview in [splitView subviews]) {
    if (first) {
      ...
      NSRectFill(NSMakeRect(origin.x, myHeight - origin.y - (dy + hy), dx + hx,
                            dy + hy));
      [viewImage unlockFocus];
    ...
    if ([subview isKindOfClass:[NSSplitView class]]) {
      [self _recursiveDrawSplit:(NSSplitView *)subview
                        inImage:viewImage
                       atOrigin:origin];
    } else {
      SessionView *sessionView = (SessionView *)subview;
...
- (NSImage *)image:(BOOL)withSpaceForFrame {
  ...
  [self _recursiveDrawSplit:root_
                    inImage:viewImage
                   atOrigin:NSMakePoint(xOrigin, yOrigin)];
...
- (void)recompact {
  ...
  DLog(@"Change size of root frame from %@ to %@",
       NSStringFromSize(root_.frame.size), NSStringFromSize(size));
...
- (NSSize)sessionSizeForViewSize:(PTYSession *)aSession {
  ...
  int width = (size.width -
               [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2) /
              [[aSession textview] charWidth];
  int height =
      (size.height -
       [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2) /
      [[aSession textview] lineHeight];
  PtyLog(@"fitSessionToCurrentViewSize %@ gives %d rows",
         [NSValue valueWithSize:size], height);
  if (width <= 0) {
  ...
  PtyLog(@"PTYTab sessionSizeForViewSize: view is %fx%f, set screen to %dx%d",
...
- (BOOL)fitSessionToCurrentViewSize:(PTYSession *)aSession {
  ...
  return [self resizeSession:aSession
                      toSize:VT100GridSizeMake(temp.width, temp.height)];
}
...
- (BOOL)resizeSession:(PTYSession *)aSession toSize:(VT100GridSize)newSize {
  ...
  if (newSize.width == [aSession columns] &&
      newSize.height == [aSession rows]) {
    PtyLog(@"fitSessionToWindow - terminating early because session size "
           @"doesn't change");
    return NO;
  }

  PtyLog(@"fitSessionToCurrentViewSize -  calling setSize:%@",
         VT100GridSizeDescription(newSize));
...
- (CGFloat)_positionOfDivider:(int)theIndex
...
// NOTE: This has a side effect of updating idMap. It maps a number (the
// "arrangement ID") to a sessionView. It also calls -[SessionView
// saveFrameSize].
- (BOOL)_recursiveEncodeArrangementForView:(NSView *)view
                                     idMap:
                                         (NSMutableDictionary<
                                             NSNumber *, SessionView *> *)idMap
  ...
  if ([view isKindOfClass:[NSSplitView class]]) {
    ...
    iTermOrderedDictionary<NSString *, __kindof NSView *> *index =
        [iTermOrderedDictionary
            byMapping:splitView.subviews
                block:^id _Nonnull(NSUInteger i,
                                   __kindof NSView *_Nonnull view) {
    ...
    [encoder
        encodeArrayWithKey:SUBVIEWS
               identifiers:index.keys
                generation:iTermGenerationAlwaysEncode
                     block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder,
                                 NSInteger i, NSString *_Nonnull identifier,
                                 BOOL *stop) {
                       return [self
                           _recursiveEncodeArrangementForView:index[identifier]
                                                        idMap:idMap
  ...
  encoder[TAB_ARRANGEMENT_SESSIONVIEW_FRAME] =
      [PTYTab frameToDict:[view frame]];
  [encoder
      encodeDictionaryWithKey:TAB_ARRANGEMENT_SESSION
                   generation:iTermGenerationAlwaysEncode
                        block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                          return
                              [session encodeArrangementWithContents:contents
                                                             encoder:encoder];
                        }];
...
+ (void)_recursiveDrawArrangementPreview:(NSDictionary *)arrangement
                                   frame:(NSRect)frame {
  if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
          isEqualToString:VIEW_TYPE_SPLITTER]) {
    BOOL isVerticalSplitter =
        [[arrangement objectForKey:SPLITTER_IS_VERTICAL] boolValue];
    float xExtent = 0;
    float yExtent = 0;
    float dx = 0;
    float dy = 0;
    ...
    [PTYSession drawArrangementPreview:[arrangement
...
+ (__kindof NSView *)
    _recursiveRestoreSplitters:(NSDictionary<NSString *, id> *)arrangement
  ...
  if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
          isEqualToString:VIEW_TYPE_SPLITTER]) {
    NSRect frame = [PTYTab
        dictToFrame:[arrangement objectForKey:TAB_ARRANGEMENT_SPLITTER_FRAME]];
    NSSplitView *splitter = [[PTYSplitView alloc]
           initWithFrame:frame
        uniqueIdentifier:arrangement[TAB_ARRANGEMENT_SPLITTER_ID]];
    if (USE_THIN_SPLITTERS) {
      [splitter setDividerStyle:NSSplitViewDividerStyleThin];
    }
    [splitter setVertical:[[arrangement objectForKey:SPLITTER_IS_VERTICAL]
                              boolValue]];
    ...
    if (idMap || sessionMap) {
      ...
      if (tabArrangementId && idMap[tabArrangementId]) {
        // Exiting a maximized-pane state, so we can get a session view from
        // theMap, a map from arrangement id -> SessionView* where arrangement
        // IDs are stored in the arrangement dict.
        sessionView =
            [idMap objectForKey:[arrangement objectForKey:TAB_ARRANGEMENT_ID]];
        [sessionView restoreFrameSize];
        return sessionView;
      }

      NSNumber *windowPaneNumber =
          [arrangement objectForKey:TAB_ARRANGEMENT_TMUX_WINDOW_PANE];
      NSString *uniqueId =
          [PTYSession guidInArrangement:arrangement[TAB_ARRANGEMENT_SESSION]];
      ...
      NSRect frame = [PTYTab
          dictToFrame:[arrangement
                          objectForKey:TAB_ARRANGEMENT_SESSIONVIEW_FRAME]];
      if (sessionView) {
...
+ (NSDictionary *)recursiveRepairedArrangementNode:(NSDictionary *)arrangement
  ...
  if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
    ...
    for (NSDictionary<NSString *, id>
             *subArrangement in arrangement[SUBVIEWS]) {
      [repairedSubviews
          addObject:[PTYTab recursiveRepairedArrangementNode:subArrangement
    ...
    NSDictionary *repairedSession =
        [PTYSession repairedArrangement:arrangement[TAB_ARRANGEMENT_SESSION]
               replacingProfileWithGUID:badGuid
                            withProfile:goodProfile];
    NSMutableDictionary *result = [arrangement mutableCopy];
...
+ (NSDictionary *)recursiveRepairedArrangementNode:(NSDictionary *)arrangement
                  replacingOldCWDOfSessionWithGUID:(NSString *)guid
                                        withOldCWD:
                                            (NSString *)replacementOldCWD {
  if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE]
          isEqualToString:VIEW_TYPE_SPLITTER]) {
    NSArray *subviews = arrangement[SUBVIEWS];
    NSArray *repairedSubviews = [subviews
        mapWithBlock:^id(NSDictionary<NSString *, id> *subArrangement) {
    ...
    return [arrangement dictionaryBySettingObject:repairedSubviews
                                           forKey:SUBVIEWS];
  }
...
+ (NSDictionary *)
    recursiveFindSessionArrangementWithGUID:(NSString *)sessionGUID
  ...
  if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
    ...
    for (NSDictionary<NSString *, id>
             *subArrangement in arrangement[SUBVIEWS]) {
      NSDictionary *result =
          [PTYTab recursiveFindSessionArrangementWithGUID:sessionGUID
                                                     node:subArrangement];
      if (result) {
        return result;
      }
  ...
  return [sessionGUID isEqualToString:guid]
             ? arrangement[TAB_ARRANGEMENT_SESSION]
             : nil;
}

- (PTYSession *)_recursiveRestoreSessions:
                    (NSDictionary<NSString *, id> *)arrangement
  ...
  if ([[arrangement objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
    ...
    NSArray<NSDictionary<NSString *, id> *> *subArrangements =
        [arrangement objectForKey:SUBVIEWS];
    PTYSession *active = nil;
    iTermObjectType subObjectType = objectType;
    for (NSInteger i = 0;
         i < [subArrangements count] && i < splitter.subviews.count; ++i) {
      NSDictionary<NSString *, id> *subArrangement = subArrangements[i];
      PTYSession *session =
          [self _recursiveRestoreSessions:subArrangement
    ...
    if (uniqueId &&
        [self
            sessionForSessionView:sessionView]) { // TODO: Is it right to check
                                                  // if session exists here?
      ...
      session = [self sessionForSessionView:sessionView];
      [session
          setSizeFromArrangement:[arrangement
                                     objectForKey:TAB_ARRANGEMENT_SESSION]];
    } else {
      session = [PTYSession
          sessionFromArrangement:[arrangement
                                     objectForKey:TAB_ARRANGEMENT_SESSION]
                           named:arrangementName
                          inView:view
                    withDelegate:theTab
...
+ (void)drawArrangementPreview:(NSDictionary *)arrangement frame:(NSRect)frame {
  [PTYTab
      _recursiveDrawArrangementPreview:[arrangement
                                           objectForKey:TAB_ARRANGEMENT_ROOT]
                                 frame:frame];
}
...
- (void)replaceWithContentsOfTab:(PTYTab *)tabToGut {
  ...
  for (SessionView *sessionView in tabToGut.viewToSessionMap) {
    [self.viewToSessionMap
        setObject:[tabToGut.viewToSessionMap objectForKey:sessionView]
           forKey:sessionView];
  }
...
- (void)enableFlexibleView {
  ...
  // Interpose a vew between the tab and the root so the root can be smaller
  // than the tab.
  flexibleView_ =
      [[iTermFlexibleView alloc] initWithFrame:root_.frame
...
+ (void)_recursiveOpenPartialAttachments:(NSDictionary *)arrangement
  ...
  if ([arrangement[TAB_ARRANGEMENT_VIEW_TYPE]
    ...
    for (NSDictionary *subArrangement in subArrangements) {
      ...
      [self _recursiveOpenPartialAttachments:subArrangement
                                  completion:^(NSDictionary *dict) {
                                    DLog(@"_recursiveOpenPartialAttachments: "
                                         @"got a result from a subview");
    ...
    [PTYSession openPartialAttachmentsForArrangement:
                    arrangement[TAB_ARRANGEMENT_SESSION]
...
// iTermController.terminals because they are siblings of a window that is still
// being created. It's passed in to ensure that tab GUIDs are truly globally
// unique.
+ (PTYTab *)
    tabWithArrangement:(NSDictionary *)arrangement
                 named:(NSString *)arrangementName
  ...
  NSSplitView *newRoot = (NSSplitView *)[PTYTab
      _recursiveRestoreSplitters:[arrangement objectForKey:TAB_ARRANGEMENT_ROOT]
                       fromIdMap:viewMap
                      sessionMap:sessionMap
                 revivedSessions:revivedSessions];
  ...
  // Give the tab a mapping from view to session for sessions that were revived.
  // These come from
  // _recursiveRestoreSplitters:fromIdMap:sessionMap:revivedSession: matching up
  // sessions in revivedSessions with their uinque IDs in the arrangement.
  for (PTYSession *session in revivedSessions) {
    [theTab.viewToSessionMap setObject:session forKey:session.view];
  }
  [theTab
      setActiveSession:[theTab
                           _recursiveRestoreSessions:
                               [arrangement objectForKey:TAB_ARRANGEMENT_ROOT]
                                               named:arrangementName
                                              atNode:theTab->root_
                                               inTab:theTab
                                       forObjectType:objectType
                                  partialAttachments:partialAttachments]];
  theTab.titleOverride =
      [arrangement[TAB_ARRANGEMENT_TITLE_OVERRIDE] nilIfNull];
...
+ (NSDictionary *)repairedArrangement:(NSDictionary *)arrangement
  ...
  NSDictionary *newRoot =
      [PTYTab recursiveRepairedArrangementNode:arrangement[TAB_ARRANGEMENT_ROOT]
                      replacingProfileWithGUID:badGuid
                                   withProfile:goodProfile];
  NSMutableDictionary *result = [arrangement mutableCopy];
...
+ (NSDictionary *)arrangementForSessionWithGUID:(NSString *)sessionGUID
  ...
  return [PTYTab
      recursiveFindSessionArrangementWithGUID:sessionGUID
                                         node:arrangement
                                                  [TAB_ARRANGEMENT_ROOT]];
}
...
- (void)didAddToTerminal:(NSWindowController<iTermWindowController> *)term
  ...
  if (tabColor) {
    ...
    [session setSessionSpecificProfileValues:@{
      KEY_TAB_COLOR : [tabColor dictionaryValue],
...
// Uses idMap_ to reconstitute the TAB_ARRANGEMENT_SESSION elements of an
// arrangement including their contents.
- (BOOL)encodeArrangementNodeWithContents:(BOOL)includeContents
  ...
  if ([node[TAB_ARRANGEMENT_VIEW_TYPE] isEqual:VIEW_TYPE_SPLITTER]) {
    ...
    [node enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                              BOOL *_Nonnull stop) {
        ...
        [iTermOrderedDictionary
            byMapping:node[SUBVIEWS]
                block:^id _Nonnull(NSUInteger i,
                  ...
                  if ([subnode[TAB_ARRANGEMENT_VIEW_TYPE]
                          isEqual:VIEW_TYPE_SPLITTER]) {
                    return subnode[TAB_ARRANGEMENT_SPLITTER_ID]
                               ?: [@(i) stringValue];
                  } else {
                    return
                        [PTYSession
                            guidInArrangement:subnode[TAB_ARRANGEMENT_SESSION]]
                            ?: [@(i) stringValue];
                  }
                }];
    __block BOOL ok = YES;
               ...
               identifiers:index.keys
                generation:iTermGenerationAlwaysEncode
                     block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder,
                       ...
                       if (![self
                               encodeArrangementNodeWithContents:includeContents
                                             fromArrangementNode:subnode
                                                         encoder:encoder]) {
                         // If one leaf fails to encode, toss the whole tab out
                         // because it'll be a disaster trying to restore it.
                         ok = NO;
                         return NO;
                       }
                       return YES;
...
- (NSDictionary *)arrangementConstructingIdMap:(BOOL)constructIdMap
  ...
  iTermMutableDictionaryEncoderAdapter *adapter =
      [[iTermMutableDictionaryEncoderAdapter alloc]
          initWithMutableDictionary:dict];
...
- (BOOL)encodeWithContents:(BOOL)contents
  ...
  encoder[TAB_ARRANGEMENT_TITLE_OVERRIDE] =
      self.titleOverride.length ? self.titleOverride : nil;
  ...
  if (isMaximized_) {
    ...
    return [encoder
        encodeDictionaryWithKey:TAB_ARRANGEMENT_ROOT
                     generation:iTermGenerationAlwaysEncode
                          block:^BOOL(
                              id<iTermEncoderAdapter> _Nonnull encoder) {
                            return [self
                                encodeArrangementNodeWithContents:contents
                                              fromArrangementNode:
                                                  mutableRootNode
                                                          encoder:encoder];
                          }];
  ...
  return [encoder
      encodeDictionaryWithKey:TAB_ARRANGEMENT_ROOT
                   generation:iTermGenerationAlwaysEncode
                        block:^BOOL(id<iTermEncoderAdapter> _Nonnull encoder) {
                          return [self
                              _recursiveEncodeArrangementForView:root_
                                                           idMap:constructIdMap
                                                                     ? idMap_
                                                                     : nil
                                                     isMaximized:NO
                                                        contents:contents
...
+ (BOOL)_recursiveBuildSessionMap:
...
+ (NSDictionary<NSString *, PTYSession *> *)
    sessionMapWithArrangement:(NSDictionary *)arrangement
                     sessions:(NSArray *)sessions {
  NSMutableDictionary<NSString *, PTYSession *> *sessionMap =
...
+ (iTermSizeRange)
    _recursiveSetSizesInTmuxParseTree:(NSMutableDictionary *)parseTree
                           showTitles:(BOOL)showTitles
                  showBottomStatusBar:(BOOL)showBottomStatusBar
                           ...
                           inTerminal:
                               (NSWindowController<iTermWindowController> *)
                                   term {
  CGFloat splitterSize = 1; // hack: should use -[NSSplitView dividerThickness],
                            // but don't have an instance yet.
  ...
  case kLeafLayoutNode: {
    ...
    const NSSize size = [PTYTab
        _sessionSizeWithCellSize:cellSize
                      dimensions:NSMakeSize(
                                     [parseTree[kLayoutDictWidthKey] intValue],
    ...
    const NSSize maximumSize =
        NSMakeSize(size.width + cellSize.width, size.height + cellSize.height);
  ...
  case kHSplitLayoutNode: {
    ...
    for (NSMutableDictionary *node in parseTree[kLayoutDictChildrenKey]) {
      ...
      SetWithGrainDim(isVertical, &totalSize.minimumSize,
                      (WithGrainDim(isVertical, totalSize.minimumSize) +
                       ...
                       splitter));
      SetAgainstGrainDim(
          isVertical, &totalSize.minimumSize,
      ...
      SetWithGrainDim(isVertical, &totalSize.maximumSize,
                      (WithGrainDim(isVertical, totalSize.maximumSize) +
                       ...
                       splitter));
      SetAgainstGrainDim(
          isVertical, &totalSize.maximumSize,
    ...
    parseTree[kLayoutDictMaximumPixelHeightKey] =
        @(totalSize.maximumSize.height);
    break;
  }
  }
  return totalSize;
...
+ (NSDictionary *)
    _recursiveArrangementForDecoratedTmuxParseTree:(NSDictionary *)parseTree
  ...
  case kLeafLayoutNode: {
    ...
    frame.origin = origin;
    frame.size.width =
        [[parseTree objectForKey:kLayoutDictPixelWidthKey] intValue];
    frame.size.height =
        [[parseTree objectForKey:kLayoutDictPixelHeightKey] intValue];
    [dict setObject:[PTYTab frameToDict:frame]
  ...
  case kVSplitLayoutNode: {
    ...
    [dict setObject:[NSNumber numberWithBool:isVertical]
             forKey:SPLITTER_IS_VERTICAL];
    NSRect frame;
    frame.origin = origin;
    frame.size.width =
        [[parseTree objectForKey:kLayoutDictPixelWidthKey] intValue];
    frame.size.height =
        [[parseTree objectForKey:kLayoutDictPixelHeightKey] intValue];
    [dict setObject:[PTYTab frameToDict:frame]
             forKey:TAB_ARRANGEMENT_SPLITTER_FRAME];
    ...
    for (NSDictionary *child in children) {
      NSDictionary *childDict =
          [PTYTab _recursiveArrangementForDecoratedTmuxParseTree:child
                                                        bookmark:bookmark
                                                          origin:childOrigin
      ...
      NSRect childFrame = [PTYTab
          dictToFrame:[childDict
                          objectForKey:TAB_ARRANGEMENT_SESSIONVIEW_FRAME]];
      if (isVertical) {
...
+ (NSDictionary *)arrangementForDecoratedTmuxParseTree:(NSDictionary *)parseTree
  ...
  [arrangement
      setObject:
          [PTYTab _recursiveArrangementForDecoratedTmuxParseTree:parseTree
                                                        bookmark:bookmark
                                                          origin:NSZeroPoint
                                                activeWindowPane:activeWp
                                                  tmuxController:tmuxController
  ...
  while ([[temp objectForKey:TAB_ARRANGEMENT_VIEW_TYPE]
      isEqualToString:VIEW_TYPE_SPLITTER]) {
...
- (void)setTmuxWindow:(int)window {
  ...
  [[self variablesScope] setValue:@(window)
                 forVariableNamed:iTermVariableKeyTabTmuxWindow];
}
...
// Note this is to inform of us of the window name, not to initiate a change of
// it.
- (void)setTmuxWindowName:(NSString *)tmuxWindowName {
  [[self realParentWindow] setWindowTitle];
  [self.variablesScope setValue:tmuxWindowName
               forVariableNamed:iTermVariableKeyTabTmuxWindowName];
...
+ (void)setSizesInTmuxParseTree:(NSMutableDictionary *)parseTree
  ...
  const BOOL showTitles =
      perPaneTitleBarEnabled && (zoomed || haveMultipleSessions);
...
+ (NSMutableDictionary *)parseTreeWithInjectedRootSplit:
    (NSMutableDictionary *)parseTree {
  if ([[parseTree objectForKey:kLayoutDictNodeType] intValue] ==
      kLeafLayoutNode) {
    // Inject a splitter at the root to follow our convention even if there is
    // only one session.
    NSMutableDictionary *newRoot = [NSMutableDictionary dictionary];
    [newRoot setObject:[NSNumber numberWithInt:kVSplitLayoutNode]
    ...
    [newRoot setObject:[parseTree objectForKey:kLayoutDictWidthKey]
                forKey:kLayoutDictWidthKey];
    [newRoot setObject:[parseTree objectForKey:kLayoutDictHeightKey]
                forKey:kLayoutDictHeightKey];
    [newRoot setObject:[parseTree objectForKey:kLayoutDictPixelWidthKey]
                forKey:kLayoutDictPixelWidthKey];
    [newRoot setObject:[parseTree objectForKey:kLayoutDictPixelHeightKey]
                forKey:kLayoutDictPixelHeightKey];
    [newRoot setObject:[NSMutableArray arrayWithObject:parseTree]
                forKey:kLayoutDictChildrenKey];
    return newRoot;
  } else {
    return parseTree;
  }
...
+ (PTYTab *)openTabWithTmuxLayout:(NSMutableDictionary *)parseTree
  ...
  NSSize rootSize =
      NSMakeSize([[parseTree objectForKey:kLayoutDictPixelWidthKey] intValue],
                 [[parseTree objectForKey:kLayoutDictPixelHeightKey] intValue]);
  [term fitWindowToTabSize:rootSize];

  // Now we can make an arrangement and restore it.
  NSDictionary *arrangement =
      [PTYTab arrangementForDecoratedTmuxParseTree:parseTree
                                          bookmark:profile
                                  activeWindowPane:0
                                    tmuxController:tmuxController
                                            window:tmuxWindow];
  ...
  // You have to update the scrollbar style before calling -appendTab: or else
  // the calculated size of the tmux client will be wrong in fullscreen when the
  // system is configured for legacy scrollers.
...
- (void)addSplitter:(NSSplitView *)splitter
  ...
  int minPos, size;
  NSSize cellSize =
      [PTYTab cellSizeForBookmark:[self.tmuxController
                                      profileForWindow:self.tmuxWindow]];
  for (NSView *view in [splitter subviews]) {
    ...
    if ([view isKindOfClass:[NSSplitView class]]) {
      ...
      NSSize contentSize = [NSScrollView
          contentSizeForFrameSize:sessionViewFrame.size
          horizontalScrollerClass:nil
            verticalScrollerClass:[realParentWindow_ scrollbarShouldBeVisible]
                                      ? [[session.view.scrollview
...
- (int)tmuxSizeForHeight:(BOOL)forHeight {
  ...
  // intervalMap maps (min x pixel, max x pixel) -> number of cells (plus 1 for
  // each splitter) Then the largest value is the last row/column.
...
// Measure the decoration size of a view. The view is either a split view or a
// session view.
//
// For a vertical split view:
//  points.width = (number of dividers) * (divider thickness) + Σ(child
//  decoration point width) points.height = max(child decoration point height)
...
//  points.height = (number of dividers) * (divider thickness) + Σ(child
//  decoration point height) cells.width = max(child decoration cell width)
  ...
  if ([view isKindOfClass:[NSSplitView class]]) {
    ...
    if (splitView.vertical) {
      ...
      for (NSView *childView in splitView.subviews) {
        const PTYTabDecorationSize childSize =
            [self _recursiveDecorationSize:childView];
        result.points.width += childSize.points.width;
        result.cells.width += childSize.cells.width;
        result.points.height =
            MAX(result.points.height, childSize.points.height);
    ...
    SessionView *sessionView = view;
    const NSSize scrollViewDecorationSize = [NSScrollView
        frameSizeForContentSize:NSMakeSize(0, 0)
        horizontalScrollerClass:nil
          verticalScrollerClass:sessionView.scrollview.hasVerticalScroller
                                    ? sessionView.scrollview.verticalScroller
                                          .class
                                    : nil
    ...
    const CGFloat titleBarHeight =
        sessionView.showTitle ? SessionView.titleHeight : 0;
    // NOTE: At the time of writing tmux tabs can’t have per-pane status bars.
    // Should that ever change, this line of code might prevent a bug.
    const CGFloat statusBarHeight =
        sessionView.showBottomStatusBar ? iTermGetStatusBarHeight() : 0;
    const CGSize margins = NSMakeSize(
        [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
        [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2);
...
// contents. Because our dividers+margins+titlebars may be either smaller or
// larger than tmux's dividers, we need to pick the largest value that fits our
// window. When you have a tab like this:
//
//  ┌───┬───┐
//  │   │ B │
//  │ A ├───┤
...
// If the divider between B and C is really thick, then you should report
// floor((B*h+C*h+1)/h) rows.
...
// * Slice the view up into columns. Slices are made at the extents of
// horizontal dividers and the x
//   coordinate of vertical dividers. All the sessions in a slice are stacked on
//   top of each other. There are never two horizontally adjacent in a column.
// * Vmax = number of horizontal dividers in column with the most horizontal
// dividers
// * Cmax = number of rows that would fit in that column
// * Vmin = number of horizontal dividers in column with the fewest horizontal
// dividers
// * Cmin = number of rows that would fit in that column
// * The height to give tmux is the smalles MIN(Vmin + Cmin, Vmax + Cmax among
// across all columns
//   slices.
//
...
- (NSSize)variableTmuxSize {
  // The size in points we need to get it to (at most). Only the current tab
  // will have the proper frame, but during window creation there might not be a
  // current tab.
  ...
  // Size we will grow to. Note that we don't use the tabView's frame. That is
  // because when the tabbar is a titlebar accessory and it's about to be
  // removed, we need to include its size in the target size. The tabView hasn't
  // been resized yet because AppKit.
  const NSSize targetSizePoints = [currentTab.delegate tabExpectedSize];

  // Current size
  const NSSize currentSize = root_.frame.size;

  DLog(@"Target size in points is %@. Current size is %@",
       NSStringFromSize(targetSizePoints), NSStringFromSize(currentSize));

  // The size of a cell.
  NSSize cellSize =
      [PTYTab cellSizeForBookmark:[self.tmuxController
                                      profileForWindow:self.tmuxWindow]];
  NSArray<SessionView *> *allSessionViews =
      self.isMaximized ? @[ self.activeSession.view ] : [self sessionViews];

  const CGSize margins = NSMakeSize(
      [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
      [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] * 2);
  const CGFloat dividerThickness = root_.dividerThickness;
  DLog(@"Margins are %@, divider thickness is %@, cell size is %@",
       NSStringFromSize(margins), @(dividerThickness),
  ...
  NSArray<PTYSplitViewDividerInfo *> *horizontalInfos =
      [root transitiveDividerLocationsVertical:NO];
  DLog(@"Horizontal divider info: %@", horizontalInfos);

  // Get frames of vertical dividers
  NSArray<PTYSplitViewDividerInfo *> *verticalInfos =
      [root transitiveDividerLocationsVertical:YES];
  ...
  [horizontalInfos
      enumerateObjectsUsingBlock:^(PTYSplitViewDividerInfo *_Nonnull obj,
  ...
  [columnSlicePoints enumerateIndexesUsingBlock:^(NSUInteger point,
                                                  BOOL *_Nonnull stop) {
    if (last == -1) {
      last = point;
      return;
    ...
    const NSRect columnFrame =
        NSMakeRect(last, 0, point - last, currentSize.height);
    ...
    NSArray<SessionView *> *sessionViewsInSlice =
        [allSessionViews filteredArrayUsingBlock:^BOOL(SessionView *view) {
          const NSRect sessionViewFrame = [root convertRect:view.bounds
                                                   fromView:view];
    ...
    NSArray<NSNumber *> *decorationSizes =
        [sessionViewsInSlice mapWithBlock:^id(SessionView *sessionView) {
          DLog(@"%@ showTitle=%@ showBototmStatusBar=%@", sessionView,
               @(sessionView.showTitle), @(sessionView.showBottomStatusBar));
          const CGFloat titleBarHeight =
              sessionView.showTitle ? SessionView.titleHeight : 0;
          // NOTE: At the time of writing tmux tabs can’t have per-pane status
          // bars. Should that ever change, this line of code might prevent a
          // bug.
          const CGFloat statusBarHeight =
              sessionView.showBottomStatusBar ? iTermGetStatusBarHeight() : 0;
    ...
    const CGFloat totalDecorationSize =
        [decorationSizes sumOfNumbers] + numberOfDividers * dividerThickness;
    DLog(@"Total decoration size is %@", @(totalDecorationSize));
    const CGFloat C =
        floor((currentSize.height - totalDecorationSize) / cellSize.height);
  ...
  last = -1;
  [rowSlicePoints enumerateIndexesUsingBlock:^(NSUInteger point,
                                               BOOL *_Nonnull stop) {
    if (last == -1) {
      last = point;
      return;
    ...
    const NSRect rowFrame =
        NSMakeRect(0, last, currentSize.width, point - last);
    ...
    const CGFloat C =
        floor((currentSize.width - totalDecorationSize) / cellSize.width);
  ...
  // The pixel growth (+ for growth, - for shrinkage) needed to attain the
  // target
  const NSSize sizeDiff =
      NSMakeSize(targetSizePoints.width - currentSize.width,
  ...
  //  If it needs to grow, we certainly don't want it to be too big. We expect
  //  there to be a bit of extra margin sometimes. If it needs to shrink, we'd
  //  rather it shrink too much than to little, for the same reason.
  ...
  const NSSize tmuxSize =
      NSMakeSize(floor(baseTmuxSize.width + charsDiff.width),
                 floor(baseTmuxSize.height + charsDiff.height));
  DLog(@"RETURN: tmuxSize = %@ currentSize=%@ targetSize=%@",
       NSStringFromSize(tmuxSize), NSStringFromSize(currentSize),
...
- (NSSize)fixedTmuxSize {
  DLog(@"Compute size in characters of the window that fits this tab's "
       @"contents");

  // The current size of the sessions in this tab in characters
  // ** BUG **
  // This rounds off fractional parts. We really need to know the maximum
  // capacity, and fractional parts can add up to more than one whole char.
  // Here's a real world example. When scrollbars come into being, every pane
  // interior shrinks by 15px width. In this case, there are two panes
  // side-by-side [|], 345 and 338px. The sizeDiff is 30px. It should add up to
  // 99 chars: (gdb) p (345.0 - 10.0)/7.0 + (338.0 - 10.0)/7.0 + 30.0/7.0 $28 =
  // 99 But rounding errors shrink it to 97 chars: (gdb) p (345 - 10)/7 + (338 -
  // 10)/7 + 30/7 $29 = 97 For now, we work around this problem with respect to
  // scrollbars by handling them specially.
  NSSize rootSizeChars =
      NSMakeSize([self tmuxSizeForHeight:NO], [self tmuxSizeForHeight:YES]);

  // The size in pixels we need to get it to (at most). Only the current tab
  // will have the proper frame, but during window creation there might not be a
  // current tab.
  PTYTab *currentTab = [realParentWindow_ currentTab];
  if (!currentTab) {
  ...
  DLog(@"tmuxSize: rootSizeChars=%@, targetSizePixels=%@, rootSizePixels=%@, "
       @"sizeDiff=%@, charSize=%@, charsDiff=%@, tmuxSize=%@",
       NSStringFromSize(rootSizeChars), NSStringFromSize(targetSizePixels),
       NSStringFromSize(rootSizePixels), NSStringFromSize(sizeDiff),
       NSStringFromSize(charSize), NSStringFromSize(charsDiff),
       NSStringFromSize(tmuxSize));

  return tmuxSize;
}

- (BOOL)_recursiveParseTree:(NSMutableDictionary *)parseTree
       matchesViewHierarchy:(NSView *)view {
  LayoutNodeType layoutNodeType =
      [[parseTree objectForKey:kLayoutDictNodeType] intValue];
...
- (BOOL)parseTree:(NSMutableDictionary *)parseTree
    matchesViewHierarchy:(NSView *)view {
  DLog(@"Checking if a parse tree matches a view hierarchy.\nParse "
       @"tree:\n%@\nView hierarchy:\n%@",
       parseTree, [view iterm_recursiveDescription]);
...
- (void)_recursiveResizeViewsInViewHierarchy:(NSView *)view
  ...
  if ([view isKindOfClass:[NSSplitView class]]) {
    NSDictionary *frameDict =
        [arrangement objectForKey:TAB_ARRANGEMENT_SPLITTER_FRAME];
    ...
    for (NSView *child in [view subviews]) {
      [self _recursiveResizeViewsInViewHierarchy:child
                                  forArrangement:[subarrangements
                                                     objectAtIndex:i]];
      i++;
    }
  } else {
...
- (iTermTmuxLayoutBuilderLeafNode *)layoutBuilderLeafNodeForLink:
    (ITMSplitTreeNode_SplitTreeLink *)link {
  PTYSession *session = [self sessionWithGUID:link.session.uniqueIdentifier];
  if (!session) {
    return nil;
  }
  return [[iTermTmuxLayoutBuilderLeafNode alloc]
      initWithSessionOfSize:VT100GridSizeMake(link.session.gridSize.width,
                                              link.session.gridSize.height)
                 windowPane:session.tmuxPane];
}

- (iTermTmuxLayoutBuilderNode *)layoutBuilderNodeForSplitTreeNode:
    (ITMSplitTreeNode *)node {
  if (node.linksArray.count == 1 &&
      node.linksArray[0].childOneOfCase ==
          ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_Node) {
  ...
  iTermTmuxLayoutBuilderInteriorNode *result =
      [[iTermTmuxLayoutBuilderInteriorNode alloc]
          initWithVerticalDividers:node.vertical];
  for (ITMSplitTreeNode_SplitTreeLink *link in node.linksArray) {
    switch (link.childOneOfCase) {
    case ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_Node: {
      iTermTmuxLayoutBuilderNode *childNode =
          [self layoutBuilderNodeForSplitTreeNode:link.node];
      if (!childNode) {
        return nil;
      }
    ...
    case ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_Session: {
      iTermTmuxLayoutBuilderLeafNode *leafNode =
          [self layoutBuilderLeafNodeForLink:link];
      if (!leafNode) {
        return nil;
      }
...
- (void)setTmuxSizesFromSplitTreeNode:(ITMSplitTreeNode *)node {
  iTermTmuxLayoutBuilderNode *root =
      [self layoutBuilderNodeForSplitTreeNode:node];
  iTermTmuxLayoutBuilder *builder =
      [[iTermTmuxLayoutBuilder alloc] initWithRootNode:root];
  ...
  [self.tmuxController setLayoutInWindow:self.tmuxWindow
                                toLayout:builder.layoutString];
}
...
- (CGSize)setSizesFromSplitTreeNode:(ITMSplitTreeNode *)node
  ...
  for (NSInteger i = 0; i < splitView.subviews.count; i++) {
    ...
    if (splitView) {
      newSubviewSize = [self setSizesFromSplitTreeNode:link.node
                                             splitView:splitView];
    } else if (sessionView) {
      PTYSession *session = (PTYSession *)sessionView.delegate;
      newSubviewSize = [PTYTab
          _sessionSizeWithCellSize:[PTYTab cellSizeForBookmark:session.profile]
    ...
    subview.frame =
        NSMakeRect(x, y, newSubviewSize.width, newSubviewSize.height);
    if (node.vertical) {
...
- (void)resizeViewsInViewHierarchy:(NSView *)view
  ...
  NSDictionary *arrangement = [PTYTab
      _recursiveArrangementForDecoratedTmuxParseTree:parseTree
                                            bookmark:bookmark
                                              origin:NSZeroPoint
...
- (void)installTmuxTitleMonitor {
  ...
  _tmuxTitleMonitor = [[iTermTmuxOptionMonitor alloc]
           initWithGateway:tmuxController_.gateway
                     scope:self.variablesScope
      fallbackVariableName:iTermVariableKeySessionWindowName
                    format:@"#{T:set-titles-string}"
                    target:[NSString
                               stringWithFormat:@"@%@", @(self.tmuxWindow)]
              variableName:iTermVariableKeyTabTmuxWindowTitle
                     block:nil];
  [_tmuxTitleMonitor updateOnce];
  if (self.titleOverride.length == 0) {
    // Show the tmux window title if both the tmux option set-titles is on and
    // the user hasn't already set a title override.
    self.variablesScope.tabTitleOverrideFormat = [NSString
        stringWithFormat:@"↣ \\(%@?)", iTermVariableKeyTabTmuxWindowTitle];
  }
}
...
// Adjust the parse tree to minimize "holes" by allowing sessions to grow by
// just under one cell in size.
+ (NSMutableDictionary *)tweakedParseTree:(NSDictionary *)parseTree
  ...
  const NSSize actualSize =
      NSMakeSize([parseTree[kLayoutDictPixelWidthKey] intValue],
  ...
  case kLeafLayoutNode: {
    const NSSize maximumSize =
        NSMakeSize([parseTree[kLayoutDictMaximumPixelWidthKey] intValue],
  ...
  case kHSplitLayoutNode: {
    ...
    NSSize tweakedSize = actualSize;
    for (NSMutableDictionary *node in parseTree[kLayoutDictChildrenKey]) {
      // Figure out how much this node would need to grow by and set
      // `exaggeratedSize` to that size.
      const NSSize originalSize =
          NSMakeSize([node[kLayoutDictPixelWidthKey] intValue],
      ...
      NSMutableDictionary *replacement =
          [self tweakedParseTree:node fillingRootOfSize:exaggeratedSize];

      // Deduct its actual growth from the desired growth. It may take many
      // siblings’ growth to satisfy all of the desired growth.
      const NSSize updatedSize =
          NSMakeSize([replacement[kLayoutDictPixelWidthKey] intValue],
                     [replacement[kLayoutDictPixelHeightKey] intValue]);
      const NSSize growth =
          NSMakeSize(updatedSize.width - originalSize.width,
...
- (void)replaceViewHierarchyWithParseTree:(NSMutableDictionary *)parseTree
                           tmuxController:(TmuxController *)tmuxController {
  SessionView *nearestNeighbor =
      [self nearestNeighborOfSession:self.activeSession];

  NSMutableDictionary *arrangement = [NSMutableDictionary dictionary];
  parseTree = [PTYTab parseTreeWithInjectedRootSplit:parseTree];
  if (tmuxController.variableWindowSize) {
    parseTree = [PTYTab tweakedParseTree:parseTree
                       fillingRootOfSize:self.rootViewSize];
                                                    ...
                                                    bookmark:
                                                        [self.tmuxController
                                                            profileForWindow:
                                                                self.tmuxWindow]
                                                      origin:NSZeroPoint
                                            activeWindowPane:[activeSession_
                                                                 tmuxPane]
                                              tmuxController:tmuxController
                                                      window:self.tmuxWindow]
         forKey:TAB_ARRANGEMENT_ROOT];
  ...
  PTYSession *activeSession = [self
      _recursiveRestoreSessions:[arrangement objectForKey:TAB_ARRANGEMENT_ROOT]
                          named:nil
                         atNode:newRoot
                          inTab:self
                  forObjectType:objectType
...
- (void)maximizeAfterApplyingTmuxParseTree:(NSMutableDictionary *)parseTree
  ...
  // TODO: For tmux 2.2, we can use window_visible_layout to fix up the parse
  // tree earlier. The approach below is to construct a fake parse tree with a
  // single session whose size equals that of the window. See issue 5233.
...
- (void)setTmuxLayout:(NSMutableDictionary *)parseTree
  ...
  if (shouldZoom) {
    [self maximizeAfterApplyingTmuxParseTree:parseTree
                              tmuxController:tmuxController];
  }
  [realParentWindow_ tabDidChangeTmuxLayout:self];
}

// Find a session that is not "senior" to a tmux pane getting split by the user
// and make it active.
- (void)activateJuniorSession {
  ...
  // Reset the flag so layout changes in the future because of resizing,
  // dragging a split pane divider, etc. won't change active session.
  for (PTYSession *aSession in self.sessions) {
...
- (BOOL)updatedTmuxLayoutRequiresAdjustment {
  ...
  if ([iTermAdvancedSettingsModel disableTmuxWindowResizing]) {
    ...
    const NSSize cellSize =
        [PTYTab cellSizeForBookmark:[self.tmuxController
                                        profileForWindow:self.tmuxWindow]];
    DLog(@"dx=%@ dy=%@ cellSize=%@ root.frame=%@ flexibleView.frame=%@", @(dx),
         @(dy), NSStringFromSize(cellSize), NSStringFromRect(root_.frame),
  ...
  DLog(@"Using too-large check only. root %@ has size %@ vs flexible view %@ "
       @"with size %@",
       root_, NSStringFromSize(root_.frame.size), flexibleView_,
       NSStringFromSize(flexibleView_.frame.size));
...
- (void)maximize {
  DLog(@"maximize %@", self);
  for (PTYSession *session in [self sessions]) {
    session.savedRootRelativeOrigin =
        [self rootRelativeOriginOfSession:session];
  }
  ...
  _orderedGUIDs =
      [[self orderedSessions] mapWithBlock:^id(PTYSession *session) {
        return session.guid;
      }];
  ...
  if ([self isTmuxTab]) {
    ...
    VT100GridSize gridSize =
        VT100GridSizeMake([parseTree_[kLayoutDictWidthKey] intValue],
    ...
    [self resizeMaximizedTmuxSessionView:self.activeSession.view
                              toGridSize:gridSize];
  }
}

- (void)resizeMaximizedTmuxSessionView:(SessionView *)sessionView
                            toGridSize:(VT100GridSize)gridSize {
  DLog(@"resize view %@ to grid size %@", sessionView,
       VT100GridSizeDescription(gridSize));
  const BOOL perPanelTitleBarsEnabled =
      [iTermPreferences boolForKey:kPreferenceKeyShowPaneTitles];
  const BOOL showTitles = perPanelTitleBarsEnabled;
  NSSize size = [PTYTab
      _sessionSizeWithCellSize:
          [PTYTab cellSizeForBookmark:[self.tmuxController
                                          profileForWindow:self.tmuxWindow]]
...
- (void)unmaximize {
  ...
  SessionView *formerlyMaximizedSessionView =
      [[root_ subviews] objectAtIndex:0];

  // I'm not convinced this is necessary but I'm afraid to remove it. idMap_
  // should hold refs to all SessionViews that matter.
  ...
  NSSplitView *newRoot =
      [PTYTab _recursiveRestoreSplitters:[savedArrangement_
                                             objectForKey:TAB_ARRANGEMENT_ROOT]
                               fromIdMap:idMap_
                              sessionMap:nil
                         revivedSessions:nil];
  ...
  for (SessionView *sessionView in self.sessionViews) {
    // I don't know why, but this doesn't get called automatically and so focus
    // follows mouse breaks. Issue 4810.
...
- (BOOL)canMoveCurrentSessionDividerBy:(int)direction
                          horizontally:(BOOL)horizontally {
  SessionView *view = [[self activeSession] view];
  PTYSplitView *split = (PTYSplitView *)[view superview];
  if (horizontally) {
    if ([split isVertical]) {
      return [self canMoveView:view
                       inSplit:split
                  horizontally:YES
                            by:direction];
    } else {
      return [self canMoveView:split
                       inSplit:[split superview]
                  horizontally:YES
                            by:direction];
    }
  } else {
...
- (void)moveCurrentSessionDividerBy:(int)direction
                       horizontally:(BOOL)horizontally {
  SessionView *view = [[self activeSession] view];
  PTYSplitView *split = (PTYSplitView *)[view superview];
...
- (void (^)(void))blockToMoveView:(NSView *)view
  ...
  const CGFloat step = [self stepForMovementOfDividerIndex:splitterIndex
                                               ofSplitView:split];
  ...
  const CGFloat proposed =
      horizontally ? NSMaxX(beforeFrame) : NSMaxY(beforeFrame);
  ...
  // It would be ok to move the divider. Return a block that updates views'
  // frames.
...
- (void)swapSession:(PTYSession *)session1 withSession:(PTYSession *)session2 {
  ...
  if (((PTYTab *)session1.delegate)->lockedSession_ ||
      ((PTYTab *)session2.delegate)->lockedSession_) {
    return;
  }
  if (session1.isTmuxClient && session2.isTmuxClient &&
      session1.tmuxController == session2.tmuxController) {
    [session1.tmuxController swapPane:session1.tmuxPane
                             withPane:session2.tmuxPane];
    return;
  }
  if ([session1 isTmuxClient] || [session2 isTmuxClient] ||
      [session1 isTmuxGateway] || [session2 isTmuxGateway]) {
    return;
  }

  DLog(@"Before swap, %@ has superview %@ and %@ has superview %@",
       session1.view, session1.view.superview, session2.view,
       session2.view.superview);
  ...
  NSUInteger session1Index =
      [[session1Superview subviews] indexOfObject:session1.view];
  PTYSplitView *session2Superview = (PTYSplitView *)session2.view.superview;
  NSUInteger session2Index =
      [[session2Superview subviews] indexOfObject:session2.view];
...
- (void)_recursivePopulateSplitTreeNode:(ITMSplitTreeNode *)node
  ...
  for (__kindof NSView *view in splitview.subviews) {
    if ([view isKindOfClass:[NSSplitView class]]) {
      ...
      ITMSplitTreeNode_SplitTreeLink *link =
          [[ITMSplitTreeNode_SplitTreeLink alloc] init];
      link.node = child;
...
- (void)updateTabTitle {
  NSString *sessionName = [self.activeSession.variablesScope
      valueForVariableName:iTermVariableKeySessionPresentationName];
...
- (void)splitViewDidChangeSubviews:(PTYSplitView *)splitView {
  for (PTYSession *session in self.sessions) {
    // Pane number may have changed for all sessions by adding or removing a
    // split pane anywhere.
    [session didMoveSession];
  }
}
...
- (void)splitView:(PTYSplitView *)splitView
    draggingWillBeginOfSplit:(int)splitterIndex {
...
- (void)splitView:(PTYSplitView *)splitView
  ...
  NSView *theView =
      [subviews objectAtIndex:splitterIndex]; // the view right of or below the
                                              // dragged splitter.
...
- (CGFloat)splitView:(NSSplitView *)splitView
  ...
  PtyLog(@"PTYTab constrainMin:%f divider:%d", (float)proposedMin,
         (int)dividerIndex);
  CGFloat dim;
  NSSize minSize =
      [self _minSizeOfView:[[splitView subviews] objectAtIndex:dividerIndex]];
  if ([splitView isVertical]) {
    dim = minSize.width;
  } else {
    dim = minSize.height;
...
- (CGFloat)splitView:(NSSplitView *)splitView
  ...
  PtyLog(@"PTYTab constrainMax:%f divider:%d", (float)proposedMax,
         (int)dividerIndex);
  CGFloat dim;
  NSSize minSize = [self
      _minSizeOfView:[[splitView subviews] objectAtIndex:dividerIndex + 1]];
  if ([splitView isVertical]) {
    dim = minSize.width;
  } else {
    dim = minSize.height;
  }
  return [self _positionOfDivider:dividerIndex + 1 inSplitView:splitView] -
         dim - [splitView dividerThickness];
}

- (void)_resizeSubviewsOfSplitViewWithLockedGrandchild:
    (NSSplitView *)splitView {
  BOOL isVertical = [splitView isVertical];
  double unlockedSize = 0;
  double minUnlockedSize = 0;
  double lockedSize =
      WithGrainDim(isVertical, [self _sessionSize:[lockedSession_ view]]);
  ...
  for (NSView *subview in [splitView subviews]) {
    if ([[lockedSession_ view] superview] != subview) {
      ...
      if ([subview isKindOfClass:[NSSplitView class]]) {
        // Get size of child tree at this subview.
        minUnlockedSize += WithGrainDim(
            isVertical, [self _recursiveMinSize:(NSSplitView *)subview]);
      } else {
        // Get size of session at this subview.
        SessionView *sessionView = (SessionView *)subview;
        minUnlockedSize +=
            WithGrainDim(isVertical, [self _minSessionSize:sessionView]);
      }
    }
  }

  // Check that we can respect the lock without allowing any subview to become
  // smaller than its minimum wgd.
  double overflow = minUnlockedSize + lockedSize -
                    WithGrainDim(isVertical, [splitView frame].size);
  if (overflow > 0) {
    // We can't maintain the locked size without making some other subview
    // smaller than its allowed min. Ignore the lockedness of the session.
    XLog(@"Warning: locked session doesn't leave enough space for other views. "
         @"overflow=%lf",
         overflow);
    [splitView adjustSubviews];
    [self _splitViewDidResizeSubviews:splitView];
  } else {
    // Locked size can be respected. Adjust the size of every subview so that
    // unlocked ones keep their original relative proportions and the locked
    // subview takes on its mandated size.
    double x = 0;
    double overage = 0; // If subviews ended up larger than their proportional
                        // size would give, this is the sum of the extra wgds.
    double newSize =
        WithGrainDim(isVertical, [splitView frame].size) -
        [splitView dividerThickness] * ([[splitView subviews] count] - 1);
    for (NSView *subview in [splitView subviews]) {
      ...
      SetAgainstGrainDim(isVertical, &newRect.size,
                         AgainstGrainDim(isVertical, [splitView frame].size));
      if ([[lockedSession_ view] superview] != subview) {
        double fractionOfUnlockedSpace =
            WithGrainDim(isVertical, [subview frame].size) / unlockedSize;
        SetWithGrainDim(isVertical, &newRect.size,
                        (newSize - lockedSize - overage) *
                            fractionOfUnlockedSpace);
      } else {
        SetWithGrainDim(isVertical, &newRect.size, lockedSize);
      }
      double minSize;
      if ([subview isKindOfClass:[NSSplitView class]]) {
        // Get size of child tree at this subview.
        minSize = WithGrainDim(isVertical,
                               [self _recursiveMinSize:(NSSplitView *)subview]);
      } else {
        // Get size of session at this subview.
        SessionView *sessionView = (SessionView *)subview;
...
- (void)_resizeSubviewsOfSplitViewWithLockedChild:(NSSplitView *)splitView
  ...
  if ([[splitView subviews] count] == 1) {
    ...
    if ([splitView isVertical]) {
      ...
      double originalSizeWithoutLockedSession =
          oldSize.width - [[lockedSession_ view] frame].size.width -
          ([[splitView subviews] count] - 1) * [splitView dividerThickness];
      ...
      // Resize each child's width so that the space left excluding the locked
      // session is divided up in the same proportions as it was before the
      // resize.
      ...
      for (NSView *subview in mySubviews) {
        if (subview != [lockedSession_ view]) {
          ...
          double fractionOfOriginalSize =
              subviewSize.width / originalSizeWithoutLockedSession;
      ...
      // Set height of locked session and then resize all others
      // proportionately.
      CGFloat availableSize = [splitView frame].size.height;

      // This also does not include dividers.
      double originalSizeWithoutLockedSession =
          oldSize.height - [[lockedSession_ view] frame].size.height -
          ([[splitView subviews] count] - 1) * [splitView dividerThickness];
      NSArray *mySubviews = [splitView subviews];
      NSSize lockedSize = [self _sessionSize:[lockedSession_ view]];
      ...
      // Resize each child's height so that the space left excluding the locked
      // session is divided up in the same proportions as it was before the
      // resize.
      NSRect newFrame = NSZeroRect;
      newFrame.size.width = lockedSize.width;
      for (NSView *subview in mySubviews) {
        if (subview != [lockedSession_ view]) {
          ...
          NSSize subviewSize = [subview frame].size;
          double fractionOfOriginalSize =
              subviewSize.height / originalSizeWithoutLockedSession;
        ...
        [subview setFrame:newFrame];
        newFrame.origin.y +=
            newFrame.size.height + [splitView dividerThickness];
      }
    }
  }
}
...
//
// min of splitter with locked session s[x] = wtg: sum(minSize(i) for i != x) +
// sessionSize(x)
//                                            atg: sessionSize(x)
...
// min of splitter with locked grandchild s[x1][x2] = wtg: sum(minSize(i) for
// all i)
//                                                    atg: max(minSize(i) for
//                                                    all i)
// max of splitter with locked grandchild s[x1][x2] = wtg: sum(maxSize(i))
//                                                    atg: min(maxSize(i) for
//                                                    all i)
...
- (void)_recursiveLockedSize:(NSView *)theSubview
  ...
  if ([theSubview isKindOfClass:[SessionView class]]) {
    // This must be the locked session. Its min and max size are exactly its
    // ideal size.
    ...
    *minSize = NSZeroSize;
    ...
    for (NSView *aView in [splitView subviews]) {
      ...
      SetWithGrainDim(isVertical, minSize,
                      WithGrainDim(isVertical, *minSize) +
                          WithGrainDim(isVertical, viewMin) + thickness);
      // minSize.atg := MAX(viewMin)
      SetAgainstGrainDim(isVertical, minSize,
                         MAX(AgainstGrainDim(isVertical, *minSize),
                             AgainstGrainDim(isVertical, viewMin)));
      // maxSizeOut.wtg := sum(viewMax)
      SetWithGrainDim(isVertical, maxSizeOut,
                      WithGrainDim(isVertical, *maxSizeOut) +
                          WithGrainDim(isVertical, viewMax) + thickness);
      // maxSizeOut.atg := MIN(viewMax)
      SetAgainstGrainDim(isVertical, maxSizeOut,
                         MIN(AgainstGrainDim(isVertical, *maxSizeOut),
                             AgainstGrainDim(isVertical, viewMax)));
    }
  }
}
...
- (void)_redistributeQuantizationError:(const double)targetSize
  ...
  // In case quantization caused some rounding error, randomly adjust subviews
  // by plus or minus one pixel.
  ...
  // First redistribute error while respecting min and max constraints until
  // that is no longer possible.
  while (error != 0) {
    BOOL anyChange = NO;
    for (int i = 0; i < [sizes count] && error != 0; ++i) {
      ITCriticalError(sizes.count > 0,
                      @"Size of sizes array changed, is now %@",
                      @(sizes.count));
      ITCriticalError(sizes.count == minSizes.count &&
                          sizes.count == maxSizes.count,
                      @"Mismatch in sizes array materialized from thin air. "
      ...
      if (proposedSize >= theMin && proposedSize <= theMax) {
        [sizes replaceObjectAtIndex:i
                         withObject:[NSNumber numberWithDouble:proposedSize]];
        error += change;
        anyChange = YES;
      }
    }
  ...
  while (error != 0) {
    ...
    for (int i = 0; i < [sizes count] && error != 0; ++i) {
      ...
      if (size + change > 0) {
        [sizes replaceObjectAtIndex:i
                         withObject:[NSNumber numberWithDouble:size + change]];
        error += change;
        anyChange = YES;
      }
    }
    if (!anyChange) {
      XLog(@"Failed to redistribute quantization error. Change=%d, sizes=%@.",
           change, sizes);
      return;
    }
  }
...
// Called after a splitter has been resized. This adjusts session sizes
// appropriately, with special attention paid to the "locked" session, which
// never resizes.
- (void)splitView:(NSSplitView *)splitView
  ...
  NSMutableArray *sizes =
      [NSMutableArray arrayWithCapacity:[[splitView subviews] count]];
  NSMutableArray *minSizes =
      [NSMutableArray arrayWithCapacity:[[splitView subviews] count]];
  NSMutableArray *maxSizes =
      [NSMutableArray arrayWithCapacity:[[splitView subviews] count]];

  // This is the sum of the with-the-grain sizes excluding dividers that we need
  // to attain.
  const double targetSize = WithGrainDim(isVertical, [splitView frame].size) -
                            ([splitView dividerThickness] * (n - 1));
  PtyLog(@"splitView:resizeSubviewsWithOldSize - target size is %lf",
         targetSize);

  // Add up the existing subview sizes to come up with the previous total size
  // excluding dividers.
  ...
  if (oldTotalSize == 0) {
    ...
    PtyLog(@"splitView:resizeSubviewsWithOldSize. initial coeff=%lf",
           sizeChangeCoeff);
  }
  if (!ignoreConstraints) {
    // Set the min and max size for each subview. Assign an initial guess to
    // sizes.
    ...
    for (NSView *aSubview in [splitView subviews]) {
      double theMinSize;
      double theMaxSize;
      if (aSubview == [lockedSession_ view] ||
          [ancestors containsObject:aSubview]) {
        ...
        if ([aSubview isKindOfClass:[NSSplitView class]]) {
          theMinSize = WithGrainDim(
              isVertical, [self _recursiveMinSize:(NSSplitView *)aSubview]);
        } else {
          theMinSize = WithGrainDim(
              isVertical, [self _minSessionSize:(SessionView *)aSubview]);
        }
        theMaxSize = targetSize;
        PtyLog(
            @"splitView:resizeSubviewsWithOldSize - this subview is unlocked");
      }
      PtyLog(@"splitView:resizeSubviewsWithOldSize - range of %p is [%lf,%lf]",
             aSubview, theMinSize, theMaxSize);
      ...
      const double initialGuess =
          sizeChangeCoeff * WithGrainDim(isVertical, [aSubview frame].size);
      const double size =
          lround(MIN(MAX(initialGuess, theMinSize), theMaxSize));
      PtyLog(@"splitView:resizeSubviewsWithOldSize - initial guess of %p is "
             @"%lf (based on size of %lf), clamped is %lf",
             aSubview, initialGuess,
             (double)WithGrainDim(isVertical, [aSubview frame].size), size);
    ...
    // Refine sizes while we're more than half a pixel away from the target
    // size.
    const double kEpsilon = 0.5;
    while (fabs(currentSumOfSizes - targetSize) > kEpsilon) {
      PtyLog(@"splitView:resizeSubviewsWithOldSize - refining. "
             @"currentSumOfSizes=%lf vs target %lf",
      ...
      // Set a coefficient that will be applied only to subviews that aren't
      // clamped. If we're able to resize all currently unclamped subviews by
      // this coefficient then we should hit exactly the target size.
      const double coeff = desiredNewSizeForUnclamped / currentSumOfUnclamped;
      PtyLog(@"splitView:resizeSubviewsWithOldSize - coeff %lf to bring "
             @"current %lf unclamped size to %lf",
             coeff, currentSumOfUnclamped, desiredNewSizeForUnclamped);

      // Try to resize every subview by the coefficient. Clamped subviews won't
      // be able to change.
      currentSumOfSizes = 0;
      ...
      if (!anyChanges) {
        PtyLog(@"splitView:resizeSubviewsWithOldSize - nothing changed in this "
               @"round");
        if (fabs(currentSumOfSizes - targetSize) >
            [[splitView subviews] count]) {
          // I'm not sure this will ever happen, but just in case quantization
          // prevents us from converging give up and ignore constraints.
          XLog(@"No changes! Ignoring constraints!");
          ignoreConstraints = YES;
        } else {
          PtyLog(@"splitView:resizeSubviewsWithOldSize - redistribute "
                 @"quantization error");
          SetPinnedDebugLogMessage(@"Current split view for redistribution",
                                   @"oldTotalSize=%@ ignoreConstraints=%@ "
                                   @"splitView.recursiveDescription=%@",
                                   @(oldTotalSize), @(ignoreConstraints),
  ...
  if (ignoreConstraints) {
    ...
    // Not all the constraints could be satisfied. Set every subview to its
    // ideal size and hope for the best.
    double currentSumOfSizes = 0;
    if (sizeChangeCoeff == 0) {
      // Original size was 0 so make all subviews equal.
      const int numSubviews = [[splitView subviews] count];
      for (int subviewNumber = 0; subviewNumber < numSubviews;
           subviewNumber++) {
  ...
  // If all subviews are leaf nodes, redistribute extra pixels among the
  // subviews so that as few as possible of them has extra space "with the
  // grain".
  ...
  if (allSubviewsAreLeafNodes) {
    ...
    for (int i = 0; i < sizes.count; i++) {
      ...
      SessionView *sessionView =
          (SessionView *)[[splitView subviews] objectAtIndex:i];
      PTYSession *aSession = [self sessionForSessionView:sessionView];
      int ou = [aSession overUnder:WithGrainDim(isVertical, size)
               inVerticalDimension:!isVertical];
      if (ou > 0) {
        [over setObject:[NSNumber numberWithInt:ou]
                 forKey:[NSNumber numberWithInt:i]];
      } else if (ou < 0) {
        [under setObject:[NSNumber numberWithInt:-ou]
                  forKey:[NSNumber numberWithInt:i]];
      }
    }

    // Get indices of subviews with extra/lacking pixels
    NSMutableArray *overKeys = [NSMutableArray
        arrayWithArray:[[over allKeys]
                           sortedArrayUsingSelector:@selector(compare:)]];
    NSMutableArray *underKeys = [NSMutableArray
        arrayWithArray:[[under allKeys]
                           sortedArrayUsingSelector:@selector(compare:)]];
    while (overKeys.count && underKeys.count) {
      // Pick the last over and under values and cancel the out as much as
      // possible.
      ...
      int mostUnderValue =
          [[under objectForKey:[underKeys lastObject]] intValue];
      ...
      [sizes replaceObjectAtIndex:mostOverIndex
                       withObject:[NSNumber numberWithDouble:overSize -
                                                             currentValue]];
      [sizes replaceObjectAtIndex:mostUnderIndex
                       withObject:[NSNumber numberWithDouble:underSize +
                                                             currentValue]];
      if (!mostOverValue) {
      ...
      [over setObject:[NSNumber numberWithInt:mostOverValue]
               forKey:[NSNumber numberWithInt:mostOverIndex]];
      [under setObject:[NSNumber numberWithInt:mostUnderValue]
                forKey:[NSNumber numberWithInt:mostUnderIndex]];
    }
  }

  NSRect frame = NSZeroRect;
  SetAgainstGrainDim(isVertical, &frame.size,
                     AgainstGrainDim(isVertical, [splitView frame].size));
  for (int i = 0; i < [sizes count]; ++i) {
    SetWithGrainDim(isVertical, &frame.size,
                    [[sizes objectAtIndex:i] doubleValue]);
...
// Inform sessions about their new sizes. This is called after views have
// finished being resized.
- (void)splitViewDidResizeSubviews:(NSNotification *)aNotification {
  ...
  PtyLog(@"splitViewDidResizeSubviews notification received. new height is %lf",
...
// This is the implementation of splitViewDidResizeSubviews. The delegate method
// isn't called when views are added or adjusted, so we often have to call this
// ourselves.
- (void)_splitViewDidResizeSubviews:(NSSplitView *)splitView {
  ...
  for (NSView *subview in [splitView subviews]) {
    if ([subview isKindOfClass:[SessionView class]]) {
      ...
      if (session) {
        PtyLog(@"splitViewDidResizeSubviews - view is %fx%f, ignore=%d",
               [subview frame].size.width, [subview frame].size.height,
               (int)[session ignoreResizeNotifications]);
...
- (CGFloat)_recursiveStepSize:(__kindof NSView *)theView
                    wantWidth:(BOOL)wantWidth {
...
- (CGFloat)stepForMovementOfDividerIndex:(NSInteger)dividerIndex
  ...
  CGFloat beforeStep = [self _recursiveStepSize:childBefore
                                      wantWidth:[splitView isVertical]];
  CGFloat afterStep = [self _recursiveStepSize:childAfter
                                     wantWidth:[splitView isVertical]];
...
- (CGFloat)splitView:(NSSplitView *)splitView
  ...
  PtyLog(@"PTYTab splitView:constraintSplitPosition%f divider:%d case ",
         (float)proposedPosition, (int)dividerIndex);
  NSArray<NSView *> *subviews = [splitView subviews];
  ...
  const CGFloat step = [self stepForMovementOfDividerIndex:dividerIndex
                                               ofSplitView:splitView];
...
- (void)setLabelAttributesForIdleTab {
  BOOL isBackgroundTab =
      [[tabViewItem_ tabView] selectedTabViewItem] != [self tabViewItem];
  ...
  for (PTYSession *session in [self sessions]) {
    if ([session newOutput]) {
      ...
      if (session.isIdle &&
          [[NSDate date] timeIntervalSinceDate:[SessionView lastResizeDate]] >
              POST_WINDOW_RESIZE_SILENCE_SEC) {
        // Idle after new output

        // See if a notification should be posted.
        if (!session.havePostedIdleNotification &&
            [session shouldPostUserNotification]) {
          NSString *theDescription =
              [NSString stringWithFormat:@"Session %@ in tab #%d became idle.",
                                         [session name], [self tabNumber]];
          if ([iTermProfilePreferences boolForKey:KEY_SEND_IDLE_ALERT
                                        inProfile:session.profile]) {
  ...
  if (isBackgroundTab) {
    if (anySessionHasNewOutput) {
      ...
      // No new output (either we got foregrounded or nothing has happened to a
      // background tab)
...
- (void)setLabelAttributesForActiveTab:(BOOL)notify {
  BOOL isBackgroundTab =
      [[tabViewItem_ tabView] selectedTabViewItem] != [self tabViewItem];
  [self
      setIsProcessing:[self anySessionIsProcessing] && ![self isForegroundTab]];

  if (![[self activeSession] havePostedNewOutputNotification] &&
      [[self realParentWindow] broadcastMode] == BROADCAST_OFF && notify &&
      [[NSDate date] timeIntervalSinceDate:[SessionView lastResizeDate]] >
          POST_WINDOW_RESIZE_SILENCE_SEC) {
    if ([iTermProfilePreferences boolForKey:KEY_SEND_NEW_OUTPUT_ALERT
                                  inProfile:self.activeSession.profile]) {
      [[iTermNotificationController sharedInstance]
          ...
          withDescription:[NSString
                              stringWithFormat:
                                  @"New output was received in %@, tab #%d.",
                                  [[self activeSession] name], [self tabNumber]]
  ...
  if ([self _windowResizedRecently]) {
    // Reset new output flag for all sessions because it may have been caused by
    // an app redrawing itself in response to the window resizing.
    for (PTYSession *session in [self sessions]) {
...
- (void)resetLabelAttributesIfAppropriate {
  ...
  for (PTYSession *aSession in [self sessions]) {
    if (!amProcessing && !aSession.havePostedNewOutputNotification &&
        !aSession.newOutput) {
      // Avoid calling the potentially expensive -shouldPostUserNotification if
      // there's nothing to do here, which is normal.
      continue;
    }
  ...
  if (shouldResetLabel && [self isForegroundTab]) {
    ...
    [self
        setState:0
           reset:(kPTYTabIdleState | kPTYTabNewOutputState | kPTYTabDeadState)];
  }
}
...
- (void)updateUseMetal NS_AVAILABLE_MAC(10_11) {
  ...
  __block iTermMetalUnavailableReason sessionReason =
      iTermMetalUnavailableReasonNone;
  NSArray<PTYSession *> *nonHiddenSessions =
      [self.sessions filteredArrayUsingBlock:^BOOL(PTYSession *session) {
        if (!self->isMaximized_) {
          // Invisible sessions in a maximized tab aren't in the view hierarchy
          // and so will always say Metal is disallowed.
          return YES;
        }
        return session == self.activeSession;
      }];
  const BOOL allSessionsAllowMetal =
      [nonHiddenSessions allWithBlock:^BOOL(PTYSession *anObject) {
        return [anObject metalAllowed:&sessionReason];
      }];
  const BOOL allSessionsIdle =
  ...
  // Limit the number of split panes using metal because each gets its own
  // thread and I've seen some crazy stuff where people have over 50 split
  // panes.
  const NSInteger maxNumberOfSplitPanesForMetal = 6;
  const BOOL numberOfSplitPanesIsReasonable =
      self.sessions.count < maxNumberOfSplitPanesForMetal;

  iTermMetalUnavailableReason reason = iTermMetalUnavailableReasonNone;
  BOOL allowed = NO;
  // Note: we turn off metal when dragging a split in a tmux tab because it's
  // hard to keep the frame of the MTKView correct without resizing it.
  if ([self.delegate tabAnyDragInProgress:self] || _isDraggingSplitInTmuxTab) {
    ...
    dispatch_once(&onceToken, ^{
      tooManyPanesReason = [NSString
          stringWithFormat:@"there are more than %@ split frames in this tab.",
  ...
  if (allowed && satisfiesKeyRequirement && foregroundTab) {
    ...
    if (!useMetal) {
      ...
      _metalUnavailableReason =
          iTermMetalUnavailableReasonContextAllocationFailure;
    }
  }
  DLog(@"_metalUnavailableReason = %@",
       iTermMetalUnavailableReasonDescription(_metalUnavailableReason));
  [self.sessions
      enumerateObjectsUsingBlock:^(PTYSession *_Nonnull obj, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
        if (self->isMaximized_) {
...
- (void)preferenceDidChange:
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    keysAffectingMetal = [[NSSet alloc] initWithArray:@[
      kPreferenceKeyPerPaneBackgroundImage, kPreferenceKeyUseMetal
    ]];
  });
...
- (BOOL)session:(PTYSession *)session
    shouldAllowDrag:(id<NSDraggingInfo>)sender {
  if ([[[sender draggingPasteboard] types]
    ...
    PTYTab *theTab = (PTYTab *)[[[[PSMTabDragAssistant sharedDragAssistant]
        draggedCell] representedObject] identifier];
    if ([[theTab sessions] containsObject:session] ||
        [[theTab sessions] count] > 1) {
      return NO;
    }
  ...
  if ([[[sender draggingPasteboard] types]
    ...
    if ([[MovePaneController sharedInstance] isMovingSession:session]) {
      if (self.sessions.count == 1 && !self.realParentWindow.anyFullScreen &&
        ...
        // If you dragged a session from a tab with split panes onto itself then
        // do nothing. But if you drag a session onto itself in a tab WITHOUT
        // split panes, then move the whole window.
        [[MovePaneController sharedInstance]
            moveWindowBy:[sender draggedImageLocation]];
      }
      // Regardless, we must say the drag failed because otherwise
      // draggedImage:endedAt:operation: will try to move the session to its own
      // window.
      [[MovePaneController sharedInstance] setDragFailed:YES];
      return NO;
    }
    return [[MovePaneController sharedInstance]
...
- (VT100GridSize)sessionTmuxSizeWithProfile:(Profile *)profile {
  if ([iTermPreferences useTmuxProfile]) {
    ...
    NSSize contentSize = [NSScrollView
        contentSizeForFrameSize:frameSize
        horizontalScrollerClass:nil
          verticalScrollerClass:[realParentWindow_ scrollbarShouldBeVisible]
                                    ? [[anySession.view.scrollview
                                              verticalScroller] class]
                                    : nil
...
- (void)sessionDidChangeMetalViewAlphaValue:(PTYSession *)session
                                         to:(CGFloat)newValue {
...
- (PTYSession *)sessionWithGUID:(NSString *)guid {
  return [self.sessions objectPassingTest:^BOOL(PTYSession *session,
...
- (void)sessionDraggingExited:(PTYSession *)session {
  if (_temporarilyUnmaximizedSessionGUID) {
    PTYSession *session =
        [self sessionWithGUID:_temporarilyUnmaximizedSessionGUID];
...
- (iTermBuiltInFunctions *)objectMethodRegistry {
  if (!_methods) {
    ...
    method = [[iTermBuiltInMethod alloc]
             initWithName:@"select_pane_in_direction"
            defaultValues:@{}
                    types:@{@"direction" : [NSString class]}
        optionalArguments:[NSSet set]
                  context:iTermVariablesSuggestionContextSession
                   target:self
                   action:@selector(selectPaneInDirectionWithCompletion:
                                                              direction:)];
    [_methods registerFunction:method namespace:@"iterm2"];
  }
  return _methods;
...
- (void)selectPaneInDirectionWithCompletion:(void (^)(id, NSError *))completion
  ...
  if ([direction isEqualToString:@"left"]) {
    ...
    NSError *error = [NSError
        errorWithDomain:@"com.iterm2.select-pane-in-direction"
                   code:0
               userInfo:@{
                 NSLocalizedDescriptionKey : @"Invalid direction. Should be "
