...
@interface PseudoTerminal () {
  ...
  // out if it's safe to toggle Lion full screen since only one can go at a
  // time.
  ...
  // DO NOT ACCESS DIRECTLY - USE ACCESSORS INSTEAD
  // Window type before entering fullscreen. Only relevant if in/entering
  // fullscreen.
...
@property(nonatomic, retain)
    NSCandidateListTouchBarItem<NSString *> *autocompleteCandidateListItem;
...
// This is a reference to the window's content view, here for convenience
// because it has the right type.
@property(nonatomic, readonly)
    __unsafe_unretained iTermRootTerminalView *contentView;
