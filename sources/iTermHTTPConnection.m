...
- (BOOL)sendResponseWithCode:(int)code
...
- (void)unacceptable {
  [self sendResponseWithCode:406
                      reason:@"Unacceptable (upgrade Python library)"
                     headers:@{}];
}
...
- (NSString *)nextLine {
  ...
  while (bytes.length < 4096) {
    ...
    if (data) {
      ...
      if (bytes.length >= 2 &&
          [[bytes subdataWithRange:NSMakeRange(bytes.length - 2, 2)]
              isEqualToData:crlfData]) {
        return [[NSString alloc]
            initWithData:[bytes
                             subdataWithRange:NSMakeRange(0, bytes.length - 2)]
...
- (NSMutableData *)nextBytes:(int64_t)length {
  ...
  while (bytes.length < length) {
    ...
    if (_buffer.length > 0) {
      ...
      [_buffer replaceBytesInRange:NSMakeRange(0, chunkSize)
                         withBytes:""
                            length:0];
    } else {
      return nil;
...
- (BOOL)waitForIO:(BOOL)read {
  ...
  @synchronized(_fdSync) {
    ...
    if (!isinf(_deadline)) {
      ...
      if (!read) {
        // The deadline for writes is extended so we can send an error after
        // timing out.
        dt += 5;
      }
      if (dt < 0) {
