...
- (BOOL)eventIsControlCodeWithOption:(NSEvent *)event {
  const NSEventModifierFlags allEventModifierFlags =
      (NSEventModifierFlagControl | NSEventModifierFlagOption |
       NSEventModifierFlagShift | NSEventModifierFlagCommand);
  const NSEventModifierFlags controlOption =
      (NSEventModifierFlagControl | NSEventModifierFlagOption);
  ...
  if ([[NSString stringWithCharacters:&controlCode length:1]
          isEqualTo:event.charactersIgnoringModifiers]) {
    // On US keyboards, when you just press control+opt+<char> you get:
    //  event.characters="<control code>"
    //  event.charactersIgnoringModifiers="<char>"
    // On Spanish ISO (and presumably all others like it) when you press
    // control+opt+<char> you can get:
    //  event.characters="<control code>"
    //  event.charactersIgnoringModifiers="<some random other thing on the key>"
    // This code path prevents control-opt-char from ignoring the Option
    // modifier on US-style keyboards. Those should not be treated as control
    // keys. The reason I think this is correct is that on a keyboard that
    // *requires* you to press option to get a control, it must be because the
    // default character for the key is not the one that goes with the control.
    // For example, on Spanish ISO the key labeled + becomes ] when you press
    // option. So to send C-] you have to press C-Opt-], and modifyOtherKeys
    // should treat it as C-].
    return NO;
  }
  // This is a control key. We can't just send it in modifyOtherKeys=2 mode. For
  // example, in issue 9279 @elias.baixas notes that on a Spanish ISO keyboard
  // you press control-alt-+ to get control-]. characters="<0x1d>".
  return YES;
}
...
- (UTF32Char)codePointForEvent:(NSEvent *)event {
  ...
  if ([self eventIsControlCodeWithOption:event]) {
    // Keyboards that require you to press control+option+something to generate
    // a control code take this path.
    const unichar controlCode = [event.characters characterAtIndex:0];
    return controlCode + '@';
  }
...
- (NSString *)stringForEvent:(NSEvent *)event {
  ...
  if (charactersIgnoringModifiers.length > 1) {
    DLog(@"Got multiple characters for keystroke: %@",
         charactersIgnoringModifiers);
    return charactersIgnoringModifiers;
  }
  const UTF32Char codePoint = [self codePointForEvent:event];
  const NSEventModifierFlags maybeFunction =
      CodePointInPrivateUseArea(codePoint) ? NSEventModifierFlagFunction : 0;
  ...
  if ((event.it_modifierFlags & allEventModifierFlagsExShift) ==
  ...
  if ([self eventIsControlCodeWithOption:event]) {
    // This is intended for keyboards like Spanish ISO that require you to press
    // option to get certain control codes (like ctrl+opt++ for C-]).
...
- (NSString *)stringWhenOptionPressedForEvent:(NSEvent *)event {
  ...
  const NSEventModifierFlags modifiers =
      [event it_modifierFlags] & allEventModifierFlags;
  return
      [self stringForCodePoint:event.charactersIgnoringModifiers.firstCharacter
                     modifiers:modifiers];
}
...
- (iTermOptionKeyBehavior)optionKeyBehaviorForEvent:(NSEvent *)event {
  ...
  const BOOL leftAltPressed =
      (modflag & NSEventModifierFlagOption) == NSEventModifierFlagOption &&
      !rightAltPressed;
  assert(leftAltPressed || rightAltPressed);

  iTermOptionKeyBehavior left, right;
  [self.delegate modifyOtherKeys:self
        getOptionKeyBehaviorLeft:&left
                           right:&right];
  if (leftAltPressed) {
    return left;
  } else {
...
- (NSString *)stringForCodePoint:(UTF32Char)codePoint
  ...
  case NSHelpFunctionKey: // On Apple keyboards help is where insert belongs.
    return [self sequenceForNonUnicodeKeypress:@"2"
                                eventModifiers:eventModifiers];
  case NSDeleteFunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"3"
                                eventModifiers:eventModifiers];
  case NSPageUpFunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"5"
                                eventModifiers:eventModifiers];
  case NSPageDownFunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"6"
                                eventModifiers:eventModifiers];
  case NSF1FunctionKey:
    return [self sequenceForFunctionKeyWithCode:@"P"
                                 eventModifiers:eventModifiers];
  case NSF2FunctionKey:
    return [self sequenceForFunctionKeyWithCode:@"Q"
                                 eventModifiers:eventModifiers];
  case NSF3FunctionKey:
    return [self sequenceForFunctionKeyWithCode:@"R"
                                 eventModifiers:eventModifiers];
  case NSF4FunctionKey:
    return [self sequenceForFunctionKeyWithCode:@"S"
                                 eventModifiers:eventModifiers];
  case NSF5FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"15"
                                eventModifiers:eventModifiers];
  case NSF6FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"17"
                                eventModifiers:eventModifiers];
  case NSF7FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"18"
                                eventModifiers:eventModifiers];
  case NSF8FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"19"
                                eventModifiers:eventModifiers];
  case NSF9FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"20"
                                eventModifiers:eventModifiers];
  case NSF10FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"21"
                                eventModifiers:eventModifiers];
  case NSF11FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"23"
                                eventModifiers:eventModifiers];
  case NSF12FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"24"
                                eventModifiers:eventModifiers];
  case NSF13FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"25"
                                eventModifiers:eventModifiers];
  case NSF14FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"26"
                                eventModifiers:eventModifiers];
  case NSF15FunctionKey:
    return [self sequenceForNonUnicodeKeypress:@"28"
                                eventModifiers:eventModifiers];
  ...
  case NSEndFunctionKey:
    return [self reallySpecialSequenceWithCode:codePoint
                                eventModifiers:eventModifiers];
  case '\t':
    if (eventModifiers == NSEventModifierFlagShift) {
      // Issue 9202 - hack to make vim work. xterm does this on linux but not
      // macos. See also https://github.com/vim/vim/issues/7189 Private email
      // thread with Thomas Dickey subject line "Shift-tab and
      // modifyOtherKeys=2"
...
- (NSString *)sequenceForFunctionKeyWithCode:(NSString *)code
                              eventModifiers:
                                  (NSEventModifierFlags)eventModifiers {
...
- (NSString *)reallySpecialSequenceWithCode:(UTF32Char)code
  ...
  const BOOL screenlike =
      [self.delegate modifyOtherKeysTerminalIsScreenlike:self];
  switch (code) {
  case NSUpArrowFunctionKey:
  ...
  case NSLeftArrowFunctionKey:
    return [[NSString alloc] initWithData:[output keyArrowLeft:eventModifiers]
                                 encoding:NSISOLatin1StringEncoding];
  case NSHomeFunctionKey:
    return [[NSString alloc] initWithData:[output keyHome:eventModifiers
                                              screenlikeTerminal:screenlike]
                                 encoding:NSISOLatin1StringEncoding];
  case NSEndFunctionKey:
    return [[NSString alloc] initWithData:[output keyEnd:eventModifiers
                                              screenlikeTerminal:screenlike]
                                 encoding:NSISOLatin1StringEncoding];
  }

  ITCriticalError(NO, @"Unexpected code %@",
                  [NSString stringWithLongCharacter:code]);
  const int csiModifiers = [self csiModifiersForEventModifiers:eventModifiers];
  if (csiModifiers == 1) {
    // esc code
    return [NSString
        stringWithFormat:@"%c[%@", 27, [NSString stringWithLongCharacter:code]];
  } else {
    // CSI 1 ; mods code
    return [NSString stringWithFormat:@"%c[1;%d%@", 27, csiModifiers,
                                      [NSString stringWithLongCharacter:code]];
  }
}
...
// Handle control modifier when it's alone or in concert with option, provided
// that sends a control.
- (nullable NSString *)keyMapperStringForPreCocoaEvent:(NSEvent *)event {
  ...
  if (event.keyCode == kVK_Space &&
    ...
    // Shift+Space is special. No other unicode character + shift reports a
    // control sequence.
...
- (nullable NSData *)keyMapperDataForPostCocoaEvent:(NSEvent *)event {
  ...
  const NSStringEncoding encoding =
      [self.delegate modifiyOtherKeysDelegateEncoding:self];
...
- (BOOL)keyMapperShouldBypassPreCocoaForEvent:(NSEvent *)event {
  ...
  const BOOL isNonEmpty = [[event charactersIgnoringModifiers] length] >
                          0; // Dead keys have length 0
  const BOOL rightAltPressed =
      (modifiers & NSRightAlternateKeyMask) == NSRightAlternateKeyMask;
  const BOOL leftAltPressed =
      (modifiers & NSEventModifierFlagOption) == NSEventModifierFlagOption &&
      !rightAltPressed;
  iTermOptionKeyBehavior left, right;
  [self.delegate modifyOtherKeys:self
        getOptionKeyBehaviorLeft:&left
  ...
  const BOOL optionModifiesKey =
      (leftOptionModifiesKey || rightOptionModifiesKey);
  ...
  if (willSendOptionModifiedKey) {
    // Meta+key or Esc+ key
    DLog(@"isNonEmpty=%@ rightAltPressed=%@ leftAltPressed=%@ "
         @"leftOptionModifiesKey=%@ rightOptionModifiesKey=%@ "
         @"optionModifiesKey=%@ willSendOptionModifiedKey=%@ -> bypass "
         @"pre-cocoa",
         @(isNonEmpty), @(rightAltPressed), @(leftAltPressed),
         @(leftOptionModifiesKey), @(rightOptionModifiesKey),
