  ...
  if (self) {
    _identities = [[SIGIdentity allSigningIdentities]
        sortedArrayUsingComparator:^NSComparisonResult(
            SIGIdentity *_Nonnull obj1, SIGIdentity *_Nonnull obj2) {
          return [obj1.signingCertificate.longDescription
              compare:obj2.signingCertificate.longDescription];
        }];
    ...
    _identityButton =
        [[NSPopUpButton alloc] initWithFrame:NSMakeRect(22, 0, 400, 22)
                                   pullsDown:NO];
    _identityButton.translatesAutoresizingMaskIntoConstraints = NO;
    _identityButton.enabled = NO;
    [self addSubview:_identityButton];
    [_identities
        enumerateObjectsUsingBlock:^(SIGIdentity *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
          NSMenuItem *menuItem = [[NSMenuItem alloc]
              initWithTitle:obj.signingCertificate.longDescription
                     action:nil
              keyEquivalent:@""];
          menuItem.tag = idx;
    ...
    NSDictionary *views =
        NSDictionaryOfVariableBindings(_signButton, _identityButton);
    [self
        addConstraints:[NSLayoutConstraint
                           constraintsWithVisualFormat:
                               @"H:|-10-[_signButton]-10-[_identityButton]-10-|"
                                               options:0
                                               metrics:@{}
                                                 views:views]];
    [self addConstraints:
              [NSLayoutConstraint
                  constraintsWithVisualFormat:@"V:|-8-[_identityButton]-8-|"
                                      options:0
                                      metrics:@{}
                                        views:views]];
    [self addConstraint:[NSLayoutConstraint
                            constraintWithItem:_signButton
...
- (void)didToggleSignButton:(id)sender {
  _identityButton.enabled =
      (_identities.count > 0 && _signButton.state == NSControlStateValueOn);
}
