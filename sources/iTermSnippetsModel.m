...
- (BOOL)titleEqualsValueUpToLength:(NSInteger)maxLength {
  return [[self trimmedTitle:maxLength]
      isEqualToString:[self trimmedValue:maxLength]];
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _snippets = [[[NSArray castFrom:[[NSUserDefaults standardUserDefaults]
                                        objectForKey:kPreferenceKeySnippets]]
...
- (void)addSnippet:(iTermSnippet *)snippet {
  ...
  [[iTermSnippetsDidChangeNotification
      notificationWithMutationType:iTermSnippetsDidChangeMutationTypeInsertion
...
- (void)replaceSnippet:(iTermSnippet *)snippetToReplace
  ...
  [self save];
  [[iTermSnippetsDidChangeNotification
      notificationWithMutationType:iTermSnippetsDidChangeMutationTypeEdit
                             index:index] post];
}

- (NSInteger)indexOfSnippetWithIdentifier:(NSInteger)identifier {
  return [_snippets
      indexOfObjectPassingTest:^BOOL(iTermSnippet *_Nonnull snippet,
...
- (void)moveSnippetsWithIdentifiers:(NSArray<NSNumber *> *)identifiers
  ...
  NSArray<iTermSnippet *> *snippets =
      [_snippets filteredArrayUsingBlock:^BOOL(iTermSnippet *snippet) {
        return [identifiers containsObject:@(snippet.identifier)];
      }];
  NSInteger countBeforeRow =
      [[snippets filteredArrayUsingBlock:^BOOL(iTermSnippet *snippet) {
  ...
  [self save];
  [[iTermSnippetsDidChangeNotification
      moveNotificationWithRemovals:removals
...
+ (instancetype)notificationWithMutationType:
                    (iTermSnippetsDidChangeMutationType)mutationType
...
+ (void)subscribe:(NSObject *)owner
            block:(void (^)(iTermSnippetsDidChangeNotification
                                *_Nonnull notification))block {
