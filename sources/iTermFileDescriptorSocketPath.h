...
// The part of the filename between the directory and the process ID in a socket
// path.
extern const char *iTermFileDescriptorSocketNamePrefix;

// Fills in |buffer| with the path to a file descriptor server socket for
// process id |pid|.
...
// Extracts the process ID from a socket filename. Returns -1 if it is
// ill-formed.
