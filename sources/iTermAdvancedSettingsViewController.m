...
- (iTermAdvancedSettingType)advancedSettingType {
  return (iTermAdvancedSettingType)[
      [self objectForKey:kAdvancedSettingType] intValue];
}

- (NSComparisonResult)compareAdvancedSettingDicts:(NSDictionary *)other {
  return [self[kAdvancedSettingDescription]
      compare:other[kAdvancedSettingDescription]];
}
...
+ (NSArray<NSDictionary *> *)sortedAdvancedSettings {
  ...
  dispatch_once(&onceToken, ^{
    ...
    sortedAdvancedSettings = [advancedSettings
        sortedArrayUsingSelector:@selector(compareAdvancedSettingDicts:)];
  });
...
- (void)dealloc {
  ...
  // For reasons I don't understand the tableview outlives this view by a small
  // amount. To reproduce, select a row in advanced prefs. Switch to the
  // profiles tab. Press esc to close the prefs window. Doesn't reproduce all
  // the time.
...
- (NSMutableAttributedString *)attributedStringForString:(NSString *)string
                                               ...
                                               topMargin:(CGFloat)topMargin
                                                selected:(BOOL)selected
                                                    bold:(BOOL)bold {
  NSDictionary *spacerAttributes =
      @{NSFontAttributeName : [NSFont systemFontOfSize:topMargin]};
  NSAttributedString *topSpacer =
  ...
  if (@available(macOS 10.14, *)) {
    ...
    textColor = (selected && self.view.window.isKeyWindow)
                    ? [NSColor whiteColor]
                    : [NSColor blackColor];
  }
...
- (NSAttributedString *)attributedStringForGroupNamed:(NSString *)groupName {
  return [self attributedStringForString:groupName
                                    size:20
                               topMargin:8
                                selected:NO
                                    bold:YES];
...
- (iTermTableViewTextField *)
    viewForImmutableAttributedString:(NSAttributedString *)attributedString
                            selected:
                                (NSAttributedString *)selectedAttributedString {
  iTermTableViewTextField *textField =
      [_tableView makeViewWithIdentifier:@"attributedstring" owner:self]
          ?: [[iTermTableViewTextField alloc] init];
...
- (NSView *)onOffViewWithValue:(BOOL)on row:(int)row {
  NSPopUpButton *button = [_tableView makeViewWithIdentifier:@"onoff"
                                                       owner:self]
                              ?: [[NSPopUpButton alloc] init];
...
- (void)toggleOnOff:(NSPopUpButton *)sender {
  const int row = [[sender
      it_associatedObjectForKey:&iTermAdvancedSettingsTableKey] intValue];
...
- (NSView *)tristateViewWithValue:(int)tristate row:(int)row {
  NSPopUpButton *button = [_tableView makeViewWithIdentifier:@"tristate"
                                                       owner:self]
                              ?: [[NSPopUpButton alloc] init];
  [button it_setAssociatedObject:@(row) forKey:&iTermAdvancedSettingsTableKey];
  [button setTarget:self];
...
- (iTermTableViewTextFieldWrapper *)viewForMutableString:(NSString *)string
                                                     row:(int)row {
  iTermTableViewTextFieldWrapper *wrapper =
      [_tableView makeViewWithIdentifier:@"mutablestring" owner:self]
          ?: [[iTermTableViewTextFieldWrapper alloc]
                 initWithFrame:NSMakeRect(0, 0, 100, 100)];
  wrapper.ignoreBackgroundStyle = YES;
  wrapper.frame = NSMakeRect(0, 0, 100, 100);
  iTermTableViewTextField *textField =
      wrapper.subviews.firstObject ?: [[iTermTableViewTextField alloc] init];
  [wrapper addSubview:textField];
  [textField it_setAssociatedObject:@(row)
                             forKey:&iTermAdvancedSettingsTableKey];
  ...
  textField.frame = NSMakeRect(
      0, (wrapper.frame.size.height - textField.frame.size.height) / 2.0,
...
- (BOOL)description:(NSString *)description matchesQuery:(NSArray *)queryWords {
  for (NSString *word in queryWords) {
    ...
    if ([description rangeOfString:word options:NSCaseInsensitiveSearch]
            .location == NSNotFound) {
      return NO;
    }
  }
...
- (NSArray *)filteredAdvancedSettings {
  if (!_filteredAdvancedSettings) {
    ...
    if (_searchField.stringValue.length == 0) {
      ...
      NSMutableArray *result = [NSMutableArray array];
      NSArray *parts =
          [_searchField.stringValue componentsSeparatedByString:@" "];
    ...
    if (_excludeDefaults.state == NSControlStateValueOn) {
      settings = [settings filteredArrayUsingBlock:^BOOL(id obj) {
        ...
        NSString *identifier = dict[kAdvancedSettingIdentifier];
        NSObject *value =
            [[NSUserDefaults standardUserDefaults] objectForKey:identifier];
        if (value == nil || [NSObject object:defaultValue
                                isApproximatelyEqualToObject:value
                                                     epsilon:0.0001]) {
          return NO;
        }
        return YES;
    ...
    _filteredAdvancedSettings =
        [[self class] groupedSettingsArrayFromSortedArray:settings];
  }
...
- (void)advancedSettingsDidChange:(NSNotification *)notification {
  ...
  if ([firstResponder isKindOfClass:[NSTextView class]]) {
    if ([[firstResponder delegate]
            isKindOfClass:[iTermTableViewTextField class]]) {
      return;
    }
  }
...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
  ...
  if ([obj isKindOfClass:[NSString class]]) {
    ...
    NSAttributedString *attributedString =
        [self attributedStringForSettingAtRow:row selected:NO];
    CGFloat height =
        [attributedString heightForWidth:tableView.tableColumns[0].width] + 8;
    return MAX(30, height);
  }
}

- (NSAttributedString *)attributedStringForSettingWithName:
  ...
  if (subtitle) {
    ...
    if (@available(macOS 10.14, *)) {
      if (selected) {
        color =
            [[NSColor selectedMenuItemTextColor] colorWithAlphaComponent:0.5];
      } else {
        color = [NSColor secondaryLabelColor];
      }
    } else {
      color = (selected && self.view.window.isKeyWindow) ? [NSColor whiteColor]
                                                         : [NSColor grayColor];
    }
    NSDictionary *attributes = @{
...
- (NSAttributedString *)attributedStringForSettingAtRow:(NSInteger)row
                                               selected:(BOOL)selected {
  NSArray *settings = [self filteredAdvancedSettings];
  ...
  return [self attributedStringForSettingWithName:description
                                         subtitle:subtitle
                                         selected:selected];
}
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  if ([obj isKindOfClass:[NSString class]]) {
    return [self
        viewForImmutableAttributedString:[self
                                             attributedStringForGroupNamed:obj]
                                selected:
                                    [self attributedStringForGroupNamed:obj]];
  }

  if (tableColumn == _settingColumn) {

    iTermTableViewTextField *textField = [self
        viewForImmutableAttributedString:[self
                                             attributedStringForSettingAtRow:row
                                                                    selected:NO]
                                selected:
                                    [self attributedStringForSettingAtRow:row
                                                                 selected:YES]];
    ...
    case kiTermAdvancedSettingTypeInteger: {
      iTermTableViewTextFieldWrapper *wrapper =
          [self viewForMutableString:[NSString stringWithFormat:@"%@", value]
                                 row:row];
      NSTableRowView *rowView = [tableView rowViewAtRow:row makeIfNecessary:NO];
      if (rowView) {
    ...
    case kiTermAdvancedSettingTypeString: {
      iTermTableViewTextFieldWrapper *wrapper =
          [self viewForMutableString:(NSString *)value row:row];
      NSTableRowView *rowView = [tableView rowViewAtRow:row makeIfNecessary:NO];
      if (rowView) {
        wrapper.backgroundStyle = [rowView interiorBackgroundStyle];
      }
...
- (void)controlTextDidChange:(NSNotification *)aNotification {
  if ([aNotification object] == _searchField) {
    ...
    NSNumber *associatedObject =
        [textField it_associatedObjectForKey:&iTermAdvancedSettingsTableKey];
    if (!associatedObject) {
      return;
    }
...
- (void)controlTextDidEndEditing:(NSNotification *)obj {
  ...
  case kiTermAdvancedSettingTypeFloat:
    textField.stringValue =
        [NSString stringWithFormat:@"%@", @([string doubleValue])];
    break;
...
- (NSArray<iTermPreferencesSearchDocument *> *)
  ...
  if (!_docs) {
    _docs = [[iTermAdvancedSettingsViewController
        sortedAdvancedSettings] mapWithBlock:^id(NSDictionary *dict) {
      iTermPreferencesSearchDocument *doc = [iTermPreferencesSearchDocument
          documentWithDisplayName:
              @"Advanced Preferences…" // dict[kAdvancedSettingDescription]
                       identifier:
                           @"Advanced Preferences" // dict[kAdvancedSettingIdentifier]
...
- (NSInteger)indexOfIdentifier:(NSString *)identifier {
  return [self.filteredAdvancedSettings
      indexOfObjectPassingTest:^BOOL(NSDictionary *_Nonnull dict,
