...
- (instancetype)initWithName:(NSString *)name
                   variables:(iTermVariables *)variables
...
@property(nonatomic, readonly)
    NSArray<iTermVariablesNonterminalProxy *> *children;
- (instancetype)initWithScope:(iTermVariableScope *)scope;
@end

static id iTermVariablesNewProxy(NSString *name, id value, BOOL isAlias,
                                 NSString *debugInfo) {
  iTermVariables *nested = [iTermVariables castFrom:value];
  if (nested) {
    iTermVariablesNonterminalProxy *proxy =
        [[iTermVariablesNonterminalProxy alloc] initWithName:name
                                                   variables:nested];
  ...
  if (weak) {
    ...
    iTermVariablesWeakNonterminalProxy *proxy =
        [[iTermVariablesWeakNonterminalProxy alloc] initWithName:name
  ...
  iTermVariablesTerminalProxy *proxy =
      [[iTermVariablesTerminalProxy alloc] initWithName:name value:value];
  proxy.debugInfo = debugInfo;
  return proxy;
}
...
- (NSArray<id<iTermVariablesProxy>> *)children {
  ...
  _children = [_variables.allNames mapWithBlock:^id(NSString *name) {
    ...
    const BOOL isAlias =
        [[iTermWeakVariables castFrom:value] variables] == self->_variables;
    return iTermVariablesNewProxy(name, value, isAlias,
                                  self->_variables.debugInfo);
  }];
  return _children;
}
...
- (instancetype)initWithScope:(iTermVariableScope *)scope {
  ...
  if (self) {
    _scope = scope;
    NSArray<iTermTuple<NSString *, iTermVariables *> *> *sortedFrames =
        [scope.frames
            sortedArrayUsingComparator:^NSComparisonResult(
                iTermTuple<NSString *, iTermVariables *> *_Nonnull obj1,
    ...
    _children =
        [sortedFrames flatMapWithBlock:^id(
                          iTermTuple<NSString *, iTermVariables *> *tuple) {
          if (tuple.firstObject) {
            return @[ [[iTermVariablesNonterminalProxy alloc]
                initWithName:tuple.firstObject
                   variables:tuple.secondObject] ];
          } else {
            return [[tuple.secondObject.allNames
                sortedArrayUsingSelector:@selector(compare:)]
                mapWithBlock:^id(NSString *name) {
                  id value = [tuple.secondObject rawValueForVariableName:name];
                  iTermWeakVariables *weakVariables =
                      [iTermWeakVariables castFrom:value];
                  const BOOL isAlias =
                      (tuple.firstObject == nil &&
                       tuple.secondObject == weakVariables.variables);
                  return iTermVariablesNewProxy(
                      name, value, isAlias,
                      weakVariables.variables.debugInfo
                          ?: [tuple.secondObject debugInfo]);
                }];
          }
        }]
            ?: @[];
...
- (void)copyPath:(id)sender {
  NSOutlineView *outlineView =
      [[iTermVariablesOutlineMenu castFrom:[sender menu]] outlineView];
  NSPasteboard *pboard = [NSPasteboard generalPasteboard];
  [pboard declareTypes:@[ NSPasteboardTypeString ] owner:NSApp];
  [pboard setString:[self pathForRow:outlineView.clickedRow
...
- (void)copyValue:(id)sender {
  NSOutlineView *outlineView =
      [[iTermVariablesOutlineMenu castFrom:[sender menu]] outlineView];
  id<iTermVariablesProxy> proxy =
      [outlineView itemAtRow:outlineView.clickedRow];
  NSPasteboard *pboard = [NSPasteboard generalPasteboard];
  [pboard declareTypes:@[ NSPasteboardTypeString ] owner:NSApp];
