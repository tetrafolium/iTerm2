...
- (instancetype)initWithSemanticHistoryController:
...
- (void)urlActionForClickAtCoord:(VT100GridCoord)coord
  ...
  iTermImageInfo *imageInfo = [self.delegate urlActionHelper:self
                                                 imageInfoAt:coord];
  if (imageInfo) {
  ...
  iTermTextExtractor *extractor =
      [self.delegate urlActionHelperNewTextExtractor:self];
  ...
  [self.delegate
             urlActionHelper:self
      workingDirectoryOnLine:coord.y
                  completion:^(NSString *workingDirectory) {
                    [iTermURLActionFactory
                                 urlActionAtCoord:coord
                              respectHardNewlines:respectHardNewlines
                                 workingDirectory:workingDirectory ?: @""
                                       remoteHost:[self.delegate
                                                       urlActionHelper:self
                                                      remoteHostOnLine:coord.y]
                                        selectors:
                                            [self.delegate
                                            ...
                                            rules:
                                                [self.delegate
                                                    urlActionHelperSmartSelectionRules:
                                                        self]
                                        extractor:extractor
                        semanticHistoryController:self.semanticHistoryController
                                      pathFactory:^SCPPath *(NSString *path,
                                                             int line) {
                                        return [self.delegate
                                                  urlActionHelper:self
                                            secureCopyPathForFile:path
                                                           onLine:line];
                                      }
                                       completion:completion];
                  }];
...
- (void)openTargetWithEvent:(NSEvent *)event
  ...
  const VT100GridCoord coord = [self.delegate urlActionHelper:self
                                                coordForEvent:event
                                     allowRightMarginOverflow:NO];
  __weak __typeof(self) weakSelf = self;
  const NSInteger generation = ++_openTargetGeneration;
  DLog(@"Look up URL action for coord %@, generation %@",
       VT100GridCoordDescription(coord), @(generation));
  // I tried respecting hard newlines if that is a legal URL, but that's such a
  // broad definition that it doesn't work well. Hard EOLs mid-url are very
  // common. Let's try always ignoring them.
  [self urlActionForClickAtCoord:coord
          respectingHardNewlines:![self ignoreHardNewlinesInURLs]
...
- (void)downloadFileAtSecureCopyPath:(SCPPath *)scpPath
  ...
  NSPoint point = [self.delegate urlActionHelper:self
                                   pointForCoord:range.start];
...
- (SmartMatch *)smartSelectAtAbsoluteCoord:(VT100GridAbsCoord)absCoord
  ...
  const long long totalScrollbackOverflow =
      [self.delegate urlActionTotalScrollbackOverflow:self];
  BOOL ok;
  const VT100GridCoord coord =
      VT100GridCoordFromAbsCoord(absCoord, totalScrollbackOverflow, &ok);
  if (!ok) {
    *rangePtr = VT100GridAbsWindowedRangeMake(
  ...
  SmartMatch *result = [extractor
      smartSelectionAt:coord
             withRules:[self.delegate urlActionHelperSmartSelectionRules:self]
        actionRequired:actionRequired
                 range:&relativeRange
      ignoringNewlines:ignoringNewlines];
  if (result) {
    *rangePtr = VT100GridAbsWindowedRangeFromWindowedRange(
        relativeRange, totalScrollbackOverflow);
  } else {
    *rangePtr = VT100GridAbsWindowedRangeMake(
...
- (void)openSemanticHistoryPath:(NSString *)path
  ...
  NSDictionary *subs =
      [self semanticHistorySubstitutionsWithPrefix:prefix
                                            suffix:suffix
                                              path:path
                                  workingDirectory:workingDirectory
                                        lineNumber:lineNumber
                                      columnNumber:columnNumber];
  [self.semanticHistoryController
           openPath:path
...
// If iTerm2 is the handler for the scheme, then the profile is launched
// directly. Otherwise it's passed to the OS to launch.
- (void)openURL:(NSURL *)url
  ...
  Profile *profile =
      [[iTermLaunchServices sharedInstance] profileForScheme:[url scheme]];
  if (profile) {
  ...
  if ([url.scheme isEqualToString:@"file"] && url.fragment) {
    NSArray<NSString *> *parts =
        [url.fragment componentsSeparatedByString:@":"];
    NSString *lineNumber = (parts.count > 0) ? parts[0] : nil;
    NSString *columnNumber = (parts.count > 1) ? parts[1] : nil;
    NSDictionary *subs = [self
        semanticHistorySubstitutionsWithPrefix:@""
                                        suffix:url.path
                                          path:url.path
                              workingDirectory:workingDirectory
                                    lineNumber:lineNumber.isNumeric ? lineNumber
                                                                    : nil
                                  columnNumber:columnNumber.isNumeric &&
                                                       lineNumber.isNumeric
                                                   ? columnNumber
                                                   : nil];
    [self.semanticHistoryController
             openPath:url.path
...
- (void)finishOpeningTargetWithEvent:(NSEvent *)event
  ...
  if ([iTermAdvancedSettingsModel
          restrictSemanticHistoryPrefixAndSuffixToLogicalWindow]) {
    [extractor restrictToLogicalWindowIncludingCoord:coord];
  }

  DLog(@"openTargetWithEvent generation %@ has action=%@", @(generation),
       action);
  if (action) {
    switch (action.actionType) {
    case kURLActionOpenExistingFile: {
      iTermLocatedString *locatedPrefix = [extractor
          wrappedLocatedStringAt:coord
                         forward:NO
      ...
      iTermLocatedString *locatedSuffix = [extractor
          wrappedLocatedStringAt:coord
                         forward:YES
             respectHardNewlines:![self ignoreHardNewlinesInURLs]
                        maxChars:[iTermAdvancedSettingsModel
    ...
    case kURLActionOpenURL: {
      ...
      if ([url.scheme isEqualToString:@"file"] && url.host.length > 0 &&
    ...
    case kURLActionSmartSelectionAction: {
      DLog(@"Run smart selection selector %@",
           NSStringFromSelector(action.selector));
      [self.smartSelectionActionTarget
          it_performNonObjectReturningSelector:action.selector
                                    withObject:action];
      break;
    }
    ...
    case kURLActionOpenImage:
      ...
      [[NSWorkspace sharedWorkspace]
          openFile:[(iTermImageInfo *)
                           action.identifier nameForNewSavedTempFile]];
      break;
...
- (NSDictionary *)
    semanticHistorySubstitutionsWithPrefix:(NSString *)prefix
                                    suffix:(NSString *)suffix
                                      path:(NSString *)path
                          workingDirectory:(NSString *)workingDirectory
                                lineNumber:(NSString *)lineNumber
  ...
  return @{
    ...
    kSemanticHistoryWorkingDirectorySubstitutionKey :
            [workingDirectory stringEscapedForBash] ?: @"",
...
+ (NSString *)usernameToDownloadFileOnHost:(NSString *)host {
  ...
  alert.messageText = [NSString
      stringWithFormat:@"Enter username for host %@ to download file with scp",
                       host];
...
- (BOOL)smartSelectAtAbsoluteCoord:(VT100GridAbsCoord)coord
                  ignoringNewlines:(BOOL)ignoringNewlines {
  VT100GridAbsWindowedRange range;
  SmartMatch *smartMatch =
      [self smartSelectAtAbsoluteCoord:coord
                                    to:&range
                      ignoringNewlines:ignoringNewlines
                        actionRequired:NO
                       respectDividers:
  ...
  if (!ignoringNewlines) {
    // TODO(georgen): iTermSelection doesn't have a mode for smart selection
    // ignoring newlines. If that flag is set, it's better to leave the
    // selection in character mode because you can still extend a selection with
    // shift-click. If we put it in smart mode, extending would get confused.
