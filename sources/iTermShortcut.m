...
// IMPORTANT: When adding to this list also update the short string class
// methods.
...
static NSString *const kCharactersIgnoringModifiers =
    @"charactersIgnoringModifiers";

// Only append to this string! Its indexes count. Never make it longer than 10
// characters. backslash -> \0, comma -> \1, space -> \2.
...
+ (NSString *)escapedString:(NSString *)input {
  ...
  for (NSUInteger i = 0; i < sCharsToEscape.length; i++) {
    ...
    escaped = [escaped
        stringByReplacingOccurrencesOfString:[sCharsToEscape
...
+ (NSString *)shortStringForDictionary:(NSDictionary *)dict {
  return [NSString
      stringWithFormat:@"%@,%@,%@,%@", dict[kKeyCode], dict[kModifiers],
...
+ (NSArray<iTermShortcut *> *)shortcutsForProfile:(Profile *)profile {
  ...
  main.keyCode =
      [iTermProfilePreferences unsignedIntegerForKey:KEY_HOTKEY_KEY_CODE
                                           inProfile:profile];
  main.modifiers =
      [iTermProfilePreferences unsignedIntegerForKey:KEY_HOTKEY_MODIFIER_FLAGS
                                           inProfile:profile];
  main.characters = [iTermProfilePreferences stringForKey:KEY_HOTKEY_CHARACTERS
                                                inProfile:profile];
  main.charactersIgnoringModifiers = [iTermProfilePreferences
      stringForKey:KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS
         inProfile:profile];

  NSMutableArray *result = [NSMutableArray array];
  [result addObject:main];
  NSArray<NSDictionary *> *additional =
      (NSArray *)[profile objectForKey:KEY_HOTKEY_ALTERNATE_SHORTCUTS];
  [result
      addObjectsFromArray:[additional mapWithBlock:^id(NSDictionary *anObject) {
...
+ (instancetype)shortcutWithDictionary:(NSDictionary *)dictionary {
  // Empty dict is the default for a profile; can't specify nil default because
  // objc.
  ...
  shortcut.charactersIgnoringModifiers =
      dictionary[kCharactersIgnoringModifiers];
  return shortcut;
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p keyCode=%@ modifiers=%@ (%@) characters=“%@” "
                       @"(0x%@) charactersIgnoringModifiers=“%@” (0x%@)>",
                       NSStringFromClass([self class]), self, @(self.keyCode),
                       @(self.modifiers),
                       [NSString stringForModifiersWithMask:self.modifiers],
                       self.characters, [self.characters hexEncodedString],
                       self.charactersIgnoringModifiers,
                       [self.charactersIgnoringModifiers hexEncodedString]];
}
...
- (BOOL)isEqualToShortcut:(iTermShortcut *)object {
  return (object.keyCode == self.keyCode &&
          ...
          [object.charactersIgnoringModifiers
              isEqual:self.charactersIgnoringModifiers]);
}

- (NSUInteger)hash {
  NSArray *components = @[
    @(self.keyCode), @(self.modifiers), self.characters ?: @"",
    self.charactersIgnoringModifiers ?: @""
  ];
...
- (iTermKeystroke *)keystroke {
  return [[iTermKeystroke alloc]
      initWithVirtualKeyCode:self.keyCode
               modifierFlags:self.modifiers
                   character:[self.charactersIgnoringModifiers firstCharacter]];
}

- (NSString *)stringValue {
  // Dead keys can have characters without charactersIgnoringModifiers. For
  // example, option+` on a German keyboard enters an apostrophe ('). The ` key
  // is a dead key, so if you ignore modifiers it's nothing. So if there are
  // either characters or characters ignoring modifiers, it's a formattable
  // shortcut. If you press just the dead key then both characters and
  // charactersIgnoringModifiers will be empty.
  const BOOL valid = (self.charactersIgnoringModifiers.length > 0 ||
                      self.characters.length > 0 || self.keyCode != 0);
...
- (iTermHotKeyDescriptor *)descriptor {
  return _charactersIgnoringModifiers.length > 0
             ? [NSDictionary descriptorWithKeyCode:self.keyCode
...
- (NSEventModifierFlags)modifiers {
  ...
  if (self.keyCode >= NSUpArrowFunctionKey &&
      self.keyCode <= NSRightArrowFunctionKey) {
...
- (BOOL)eventIsShortcutPress:(NSEvent *)event {
  ...
  return (([event it_modifierFlags] & kHotKeyModifierMask) ==
              (_modifiers & kHotKeyModifierMask) &&
