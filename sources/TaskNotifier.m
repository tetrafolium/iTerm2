...
NSString *const kCoprocessStatusChangeNotification =
    @"kCoprocessStatusChangeNotification";
...
- (void)pipeDidBreakForExternalProcessID:(pid_t)pid status:(int)status {
  assert(pid >= 0);
  [tasksLock lock];
  const NSInteger i = [_tasks
      indexOfObjectPassingTest:^BOOL(id<iTermTask> _Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
        return obj.pid == pid;
      }];
  if (i == NSNotFound) {
    [tasksLock unlock];
    // This is the normal case — usually there'll be an error status on the FD
    // and -deregisterTask: will take care of it.
    return;
  }
...
- (BOOL)handleReadOnFileDescriptor:(int)fd
...
- (BOOL)handleWriteOnFileDescriptor:(int)fd
                               task:(id<iTermTask>)task
                              fdSet:(fd_set *)fdSet {
  if (FD_ISSET(fd, fdSet)) {
...
- (BOOL)handleErrorOnFileDescriptor:(int)fd
                               task:(id<iTermTask>)task
                              fdSet:(fd_set *)fdSet {
  if (FD_ISSET(fd, fdSet)) {
...
- (void)run {
  ...
  for (;;) {
    ...
    // Unblock pipe to interrupt select() whenever a PTYTask
    // register/unregisters
    highfd = unblockPipeR;
    ...
    if ([deadpool count] > 0) {
      ...
      NSMutableSet *newDeadpool =
          [NSMutableSet setWithCapacity:[deadpool count]];
      for (NSNumber *pid in deadpool) {
        ...
        if (waitresult == 0) {
          ...
          if (errno != ECHILD) {
            PtyTaskDebugLog(
                @"  wait failed with %d (%s), adding back to deadpool", errno,
                strerror(errno));
            [newDeadpool addObject:pid];
          } else {
            PtyTaskDebugLog(
                @"  wait failed with ECHILD, I guess we already waited on it.");
          }
        }
      }
      [deadpool release];
    ...
    if (select(highfd + 1, &rfds, &wfds, &efds, NULL) <= 0) {
      ...
      default:
        goto breakloop;
        // If the file descriptor is closed in the main thread there's a race
        // where sometimes you'll get an EBADF.
      }
    }
    ...
    while ((task = [iter nextObject])) {
      ...
      if (fd >= 0) {
        ...
        if ([task fd] >= 0 &&
            ![task hasBrokenPipe]) { // Make sure the pipe wasn't just broken.
          @synchronized(task) {
            Coprocess *coprocess = [task coprocess];
            if (coprocess) {
              ...
              [self handleReadOnFileDescriptor:fd
                                          task:task
                                 withCoprocess:coprocess
                                         fdSet:&rfds];
              [self handleErrorOnFileDescriptor:fd
                                  withCoprocess:coprocess
                                          fdSet:&efds];
              ...
              [self handleWriteOnFileDescriptor:coprocessWriteFd
                                  withCoprocess:coprocess
                                          fdSet:&wfds];
...
- (void)notifyCoprocessChange {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kCoprocessStatusChangeNotification
                    object:nil];
}
