...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  [_fractions
      enumerateObjectsUsingBlock:^(NSNumber *_Nonnull fractionNumber,
...
- (void)updateEvents {
  ...
  for (long long i = _firstTimestamp; i > 0 && i <= lastTimestamp;
       i = [_delegate instantReplayTimestampAfter:i]) {
...
- (void)setState:(iTermInstantReplayState)destinationState
    byCancelling:(BOOL)cancel {
  if (_state == iTermInstantReplayStateSetStart &&
    ...
    _eventsView.endFraction = _eventsView.startFraction;
             ...
             _state == iTermInstantReplayStateSetEnd && !cancel) {
...
- (NSSize)windowWillResize:(NSWindow *)sender toSize:(NSSize)frameSize {
  ...
  return NSMakeSize(MAX(kMinWidth, frameSize.width),
...
- (void)updateInstantReplayView {
  ...
  // Adjust the width of the "earliest time" label, and keep the margin between
  // it and the slider the same.
  NSRect labelFrame = _earliestTimeLabel.frame;
  CGFloat margin =
      _slider.frame.origin.x - labelFrame.origin.x - labelFrame.size.width;
  ...
  _slider.frame =
      NSMakeRect(newXOrigin, _slider.frame.origin.y,
  ...
  float newX =
      [_slider floatValue] * sf.size.width + sf.origin.x - f.size.width / 2;
