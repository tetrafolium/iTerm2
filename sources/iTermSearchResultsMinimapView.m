...
static NSString *const iTermBaseMinimapViewInvalidateNotification =
    @"iTermBaseMinimapViewInvalidateNotification";
...
static inline void iTermSearchResultsMinimapViewDrawItem(CGFloat offset,
                                                         CGFloat width,
                                                         CGContextRef context) {
  const CGRect boundingRect =
      CGRectMake(0, offset, width, iTermSearchResultsMinimapViewItemHeight);
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx {
  ...
  for (NSInteger i = 0; i < self.numberOfSeries; i++) {
    ...
    const CGFloat height =
        layerHeight - iTermSearchResultsMinimapViewItemHeight;
    ...
    DLog(@"Draw %@ indexes in %@ lines with height %@ fill color %@",
         ...
         @(rangeOfVisibleLines.length), @(height), series.fillColor);
    [indexes enumerateIndexesInRange:rangeOfVisibleLines
                             options:0
                          usingBlock:^(NSUInteger idx, BOOL *_Nonnull stop) {
                            const CGFloat fraction =
                                (CGFloat)(idx - rangeOfVisibleLines.location) /
                                numberOfLines;
                            const CGFloat flippedFraction = 1.0 - fraction;
                            const CGFloat pointOffset =
                                round(flippedFraction * height);
                            if (pointOffset + 2 > lastPointOffset) {
                              return;
                            }
                            iTermSearchResultsMinimapViewDrawItem(pointOffset,
                                                                  width, ctx);
...
- (void)invalidate {
  ...
  _invalid = YES;
  [self.rateLimit
      performRateLimitedSelector:@selector(postInvalidateNotification)
...
+ (void)postInvalidateNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermBaseMinimapViewInvalidateNotification
                    object:nil];
}
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _series.outlineColor = [[NSColor colorWithRed:0.5 green:0.5 blue:0
                                            alpha:1] CGColor];
    CFRetain(_series.outlineColor);
    _series.fillColor = [[NSColor colorWithRed:1 green:1 blue:0
                                         alpha:1] CGColor];
...
- (void)performInvalidate {
  ...
  _rangeOfVisibleLines =
      [self.delegate searchResultsMinimapViewRangeOfVisibleLines:self];
  const NSUInteger count =
      [_series.indexes countOfIndexesInRange:_rangeOfVisibleLines];
  DLog(@"Count is %@", @(count));
  [self setHasData:count > 0];
}

- (NSIndexSet *)indexSet {
  return _series.indexes
             ?: [self.delegate searchResultsMinimapViewLocations:self];
}
...
- (instancetype)initWithColors:
  ...
  if (self) {
    ...
    [colors enumerateObjectsUsingBlock:^(
                iTermTuple<NSColor *, NSColor *> *_Nonnull obj, NSUInteger idx,
                BOOL *_Nonnull stop) {
