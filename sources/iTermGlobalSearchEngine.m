    ...
    _cursors = [[sessions mapWithBlock:^id(PTYSession *session) {
      ...
      [session.screen setFindString:query
                   forwardDirection:NO
                               mode:mode
                        startingAtX:0
                        startingAtY:session.screen.numberOfLines + 1 +
                                    session.screen.totalScrollbackOverflow
                         withOffset:0
                          inContext:findContext
                    multipleResults:YES];
      iTermGlobalSearchEngineCursor *cursor =
          [[iTermGlobalSearchEngineCursor alloc] init];
...
- (NSAttributedString *)snippetFromExtractor:(iTermTextExtractor *)extractor
                                      result:(SearchResult *)result {
  const VT100GridAbsCoordRange range = VT100GridAbsCoordRangeMake(
      result.startX, result.absStartY, result.endX + 1, result.absEndY);
  NSDictionary *matchAttributes = @{
    NSUnderlineStyleAttributeName : @(NSUnderlineStyleSingle),
    NSBackgroundColorAttributeName : [NSColor colorWithRed:1
                                                     green:1
                                                      blue:0
                                                     alpha:0.35],
...
- (BOOL)searchWithCursor:(iTermGlobalSearchEngineCursor *)cursor {
  ...
  const BOOL more =
      [cursor.session.screen continueFindAllResults:results
                                          inContext:cursor.findContext];
  iTermTextExtractor *extractor =
      [[iTermTextExtractor alloc] initWithDataSource:cursor.session.screen];
  NSArray<iTermGlobalSearchResult *> *mapped =
      [results mapWithBlock:^id(SearchResult *anObject) {
        iTermGlobalSearchResult *result =
            [[iTermGlobalSearchResult alloc] init];
...
- (double)progressIncludingCursor:
    (iTermGlobalSearchEngineCursor *)additionalCursor {
  double done = _retiredLines;

  for (iTermGlobalSearchEngineCursor *cursor in
       [_cursors arrayByAddingObject:additionalCursor]) {
