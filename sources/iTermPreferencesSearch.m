...
+ (instancetype)documentWithDisplayName:(NSString *)displayName
  ...
  return [[iTermPreferencesSearchDocument alloc]
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p docid=%@ identifier=%@ displayName=%@>",
                       self.class, self, self.docid, self.identifier,
                       self.displayName];
}
...
- (BOOL)isEqual:(id)object {
  iTermPreferencesSearchDocument *other =
      [iTermPreferencesSearchDocument castFrom:object];
  if (!other) {
    return NO;
  }
...
@property(nonatomic, strong)
    NSArray<iTermPreferencesSearchDocument *> *documents;
@property(nonatomic, readonly) NSSet<NSNumber *> *docids;

- (instancetype)initWithKeyword:(NSString *)keyword
                       document:(iTermPreferencesSearchDocument *)document
...
- (void)dump {
  [_index enumerateObjectsUsingBlock:^(
              iTermPreferencesSearchIndexEntry *_Nonnull obj, NSUInteger idx,
              BOOL *_Nonnull stop) {
    ...
    NSLog(
        ...
        [[obj.documents mapWithBlock:^id(iTermPreferencesSearchDocument *doc) {
          return [NSString
              stringWithFormat:@"[%@: %@]", doc.docid, doc.identifier];
...
- (void)addDocumentToIndex:(iTermPreferencesSearchDocument *)document {
  for (NSString *keyword in document.allKeywords) {
    iTermPreferencesSearchIndexEntry *entry =
        [[iTermPreferencesSearchIndexEntry alloc] initWithKeyword:keyword
                                                         document:document];
...
- (NSArray<iTermPreferencesSearchDocument *> *)documentsMatchingQuery:
    (NSString *)query {
  ...
  NSArray<iTermPreferencesSearchCursor *> *cursors =
      [tokens mapWithBlock:^id(NSString *token) {
        if (token.length == 0) {
          return nil;
        }
        iTermPreferencesSearchCursor *cursor =
            [[iTermPreferencesSearchCursor alloc] initWithToken:token
                                                          index:NSNotFound];
  ...
  docids =
      [NSSet setWithArray:[identifierToDocids.allValues
                              mapWithBlock:^id(NSArray<NSNumber *> *docids) {
...
- (NSArray<iTermPreferencesSearchDocument *> *)
    documentsSortedByDisplayNameWithDocIDs:(NSSet<NSNumber *> *)docids {
  return [[docids.allObjects mapWithBlock:^id(NSNumber *docid) {
    return self->_docs[docid];
  }]
      sortedArrayUsingComparator:^NSComparisonResult(
          iTermPreferencesSearchDocument *_Nonnull doc1,
          iTermPreferencesSearchDocument *_Nonnull doc2) {
        if (doc1.queryIndependentScore != doc2.queryIndependentScore) {
          return [@(doc2.queryIndependentScore)
              compare:@(doc1.queryIndependentScore)];
        }
        return
            [doc1.displayName localizedCaseInsensitiveCompare:doc2.displayName];
      }];
}

- (NSSet<NSNumber *> *)commonDocIDsAmongCursors:
    (NSArray<iTermPreferencesSearchCursor *> *)cursors {
...
- (NSInteger)firstIndexForKeyword:(iTermPreferencesSearchKeyword *)keyword {
  NSArray<iTermPreferencesSearchKeyword *> *keywords =
      (NSArray<iTermPreferencesSearchKeyword *> *)_index;
  NSInteger index = [keywords
        indexOfObject:keyword
        inSortedRange:NSMakeRange(0, _index.count)
              options:NSBinarySearchingInsertionIndex
