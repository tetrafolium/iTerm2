...
- (void)setShouldLoadRemotePrefs:(BOOL)value {
  [iTermPreferences setBool:value
                     forKey:kPreferenceKeyLoadPrefsFromCustomFolder];
}
...
- (NSString *)prefsFilenameWithBaseDir:(NSString *)base {
  return [NSString stringWithFormat:@"%@/%@.plist", base,
...
- (BOOL)preferenceKeyIsSyncable:(NSString *)key {
  ...
  NSArray *exemptKeys = @[
    kPreferenceKeyLoadPrefsFromCustomFolder, kPreferenceKeyCustomFolder,
    @"Secure Input", @"moveToApplicationsFolderAlertSuppress",
    kPreferenceKeyAppVersion, @"CGFontRenderingFontSmoothingDisabled",
    @"PreventEscapeSequenceFromChangingProfile",
    @"PreventEscapeSequenceFromClearingHistory"
  ];
  return ![exemptKeys containsObject:key] && ![key hasPrefix:@"NS"] &&
         ![key hasPrefix:@"SU"] && ![key hasPrefix:@"NoSync"] &&
         ![key hasPrefix:@"UK"];
}
...
- (NSDictionary *)freshCopyOfRemotePreferences {
  ...
  if ([filename stringIsUrlLike]) {
    ...
    if (!data || error) {
      ...
      alert.messageText =
          @"Failed to load preferences from URL. Falling back to local copy.";
      alert.informativeText = [NSString
          stringWithFormat:@"HTTP request failed: %@",
      ...
      if (response == NSAlertSecondButtonReturn) {
        [[PreferencePanel sharedInstance]
            openToPreferenceWithKey:kPreferenceKeyLoadPrefsFromCustomFolder];
      }
      return nil;
    }
    ...
    if (![data writeToFile:tempFile options:0 error:&error]) {
      NSAlert *alert = [[NSAlert alloc] init];
      alert.messageText = @"Failed to write to temp file while getting remote "
                          @"prefs. Falling back to local copy.";
      alert.informativeText =
          [NSString stringWithFormat:@"Error on file %@: %@", tempFile,
  ...
  if (!remotePrefs.count) {
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Failed to load preferences from custom directory. "
                        @"Falling back to local copy.";
    alert.informativeText =
        [NSString stringWithFormat:@"Missing or malformed file at \"%@\"",
...
- (void)saveLocalUserDefaultsToRemotePrefs {
  if ([self remotePrefsHaveChanged]) {
    NSString *theTitle = [NSString
        stringWithFormat:@"Preferences at %@ changed since iTerm2 started. "
                         @"Overwrite it?",
                         [self customFolderOrURL]];
    if ([iTermWarning
            showWarningWithTitle:theTitle
                         actions:@[ @"Overwrite", @"Discard Local Changes" ]
                      identifier:nil
  ...
  NSDictionary *myDict = [userDefaults
  ...
  if (!isOk) {
    ...
    alert.informativeText =
        [NSString stringWithFormat:@"Tried to copy %@ to %@",
...
- (void)copyRemotePrefsToLocalUserDefaultsPreserving:
  ...
  NSDictionary *localPrefs =
      [NSDictionary dictionaryWithContentsOfFile:theFilename];
  ...
  for (NSString *key in remotePrefs) {
    if ([self preferenceKeyIsSyncable:key]) {
      [[NSUserDefaults standardUserDefaults]
          setObject:[remotePrefs objectForKey:key]
             forKey:key];
    }
  }
  return;
...
- (BOOL)localPrefsDifferFromSavedRemotePrefs {
  ...
  if (_savedRemotePrefs && [_savedRemotePrefs count]) {
    ...
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *localPrefs = [userDefaults
        persistentDomainForName:[[NSBundle mainBundle] bundleIdentifier]];
    // Iterate over each set of prefs and validate that the other has the same
    // value for each key.
    for (NSString *key in localPrefs) {
      if ([self preferenceKeyIsSyncable:key] &&
          ![[_savedRemotePrefs objectForKey:key]
              isEqual:[localPrefs objectForKey:key]]) {
        return YES;
      }
    }
...
- (void)applicationWillTerminate {
  if ([self localPrefsDifferFromSavedRemotePrefs]) {
    if (self.remoteLocationIsURL) {
      ...
      NSString *theTitle = [NSString
          stringWithFormat:@"Changes made to preferences will be lost when "
                           @"iTerm2 is restarted "
      ...
      // Not a URL
      NSString *theTitle = [NSString
          stringWithFormat:@"Preferences have changed. Copy them to %@?",
                           [self customFolderOrURL]];
