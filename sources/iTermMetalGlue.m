...
- (nullable id<iTermMetalDriverDataSourcePerFrameState>)
  ...
  ITBetaAssert(self.delegate.metalGlueContext != nil,
               @"Nil metal glue context");
  return [[iTermMetalPerFrameState alloc]
      initWithTextView:self.textView
                screen:self.screen
                  glue:self
...
- (void)metalDidFindImages:(NSSet<NSString *> *)foundImages
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    for (NSNumber *absoluteLine in animatedLines) {
      ...
      if (abs >= offset) {
        int row = abs - offset;
        [self->_textView.dataSource
            setRangeOfCharsAnimated:NSMakeRange(0, width)
                             onLine:row];
      }
    }
...
- (void)metalDriverDidDrawFrame:
  ...
  if (!VT100GridSizeEquals(state.gridSize,
                           VT100GridSizeMake(_textView.dataSource.width,
                                             _textView.dataSource.height))) {
    return;
  }
  if (!CGSizeEqualToSize(state.cellSize, CGSizeMake(_textView.charWidth,
...
- (void)metalDriverDidProduceDebugInfo:(nonnull NSData *)archive {
  ...
  [[NSWorkspace sharedWorkspace]
      activateFileViewerSelectingURLs:@[ [NSURL fileURLWithPath:filename] ]];
}
