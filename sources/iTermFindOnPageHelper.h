...
@interface iTermFindOnPageHelper
    : NSObject <iTermSearchResultsMinimapViewDelegate>
...
// direction: YES to search forwards, NO to search backwards. In practice the
...
// offset: Amount to add to findCursor for where to begin searching (used for
// "find next" to begin
...
// totalScrollbackOverflow: Number of lines lost to scrollback history (used to
// calculate absolute
//   line numbers).
...
// Reset the copied find context. This will prevent tail search from running in
// the future.
...
// Search the next block (calling out to the delegate to do the real work) and
// update highlights and search results.
...
// Sets the location to start searching. TODO: Currently this only works for
// find next/prev.
