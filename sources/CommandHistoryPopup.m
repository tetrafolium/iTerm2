...
- (NSArray *)commandsForHost:(VT100RemoteHost *)host
  ...
  if (expand) {
    return [history autocompleteSuggestionsWithPartialCommand:partialCommand
                                                       onHost:host];
  } else {
...
- (void)loadCommands:(NSArray *)commands
  ...
  for (id obj in commands) {
    CommandHistoryPopupEntry *popupEntry =
        [[[CommandHistoryPopupEntry alloc] init] autorelease];
    if ([obj isKindOfClass:[iTermCommandHistoryCommandUseMO class]]) {
      ...
      popupEntry.date = [NSDate
          dateWithTimeIntervalSinceReferenceDate:commandUse.time.doubleValue];
    } else {
      iTermCommandHistoryEntryMO *entry = obj;
      popupEntry.command = entry.command;
      popupEntry.date =
          [NSDate dateWithTimeIntervalSinceReferenceDate:entry.timeOfLastUse
                                                             .doubleValue];
    }
...
- (id)tableView:(NSTableView *)aTableView
  ...
  CommandHistoryPopupEntry *entry =
...
- (void)rowSelected:(id)sender {
  if ([_tableView selectedRow] >= 0) {
    ...
    CommandHistoryPopupEntry *entry = [[self model]
        objectAtIndex:[self convertIndex:[_tableView selectedRow]]];
    const NSEventModifierFlags mask =
        NSEventModifierFlagShift | NSEventModifierFlagOption;
    NSString *const string =
        [entry.command substringFromIndex:_partialCommandLength];
