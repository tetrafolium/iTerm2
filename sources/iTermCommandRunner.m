...
- (instancetype)initWithCommand:(NSString *)command
  ...
  if (self) {
    ...
    _readingQueue =
        dispatch_queue_create("com.iterm2.crun-reading", DISPATCH_QUEUE_SERIAL);
    _writingQueue =
        dispatch_queue_create("com.iterm2.crun-writing", DISPATCH_QUEUE_SERIAL);
    _waitingQueue =
        dispatch_queue_create("com.iterm2.crun-waiting", DISPATCH_QUEUE_SERIAL);
    assert(command);
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p pid=%@>", self.class, self,
...
- (void)terminate {
  @try {
    ...
    if (pid) {
      ...
      DLog(@"kill -%@ %@ returned %@", @(SIGKILL), @(_task.processIdentifier),
           @(rc));
    } else {
      DLog(@"command runner %@ process ID is 0. Should terminate after launch.",
           self);
    }
...
- (BOOL)launchTask {
  ...
  @try {
    ...
    NSLog(@"Task failed with %@. launchPath=%@, pwd=%@, args=%@", e,
          _task.launchPath, _task.currentDirectoryPath, _task.arguments);
    DLog(@"Task failed with %@. launchPath=%@, pwd=%@, args=%@", e,
         _task.launchPath, _task.currentDirectoryPath, _task.arguments);
    if (self.completion) {
    ...
    @autoreleasepool {
      ...
      if (!self.outputHandler) {
        DLog(@"%@: %@", [task.arguments componentsJoinedByString:@" "],
             [[NSString alloc] initWithData:inData
                                   encoding:NSUTF8StringEncoding]);
      }
      DLog(@"runCommand: Reading");
    }
...
- (void)write:(NSData *)data completion:(void (^)(size_t, int))completion {
  ...
  dispatch_data_t dispatchData =
      dispatch_data_create(data.bytes, data.length, _writingQueue, ^{
        [data length]; // just ensure data is retained
      });
  dispatch_write(fd, dispatchData, _writingQueue,
                 ^(dispatch_data_t _Nullable data, int error) {
                   if (completion) {
                     dispatch_async(dispatch_get_main_queue(), ^{
                       completion(data ? dispatch_data_get_size(data) : 0,
                                  error);
                     });
                   }
                 });
