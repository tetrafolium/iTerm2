...
NSString *const iTermPowerManagerStateDidChange =
    @"iTermPowerManagerStateDidChange";
NSString *const iTermPowerManagerMetalAllowedDidChangeNotification =
    @"iTermPowerManagerMetalAllowedDidChangeNotification";
...
- (instancetype)initPrivate {
  ...
  if (self) {
    ...
    _runLoopSource = IOPSCreateLimitedPowerNotification(
        iTermPowerManagerSourceDidChange, (__bridge void *)(self));
...
- (void)setConnected:(BOOL)connected {
  if (_connectedToPower != connected) {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermPowerManagerStateDidChange
                      object:nil];
...
- (BOOL)metalAllowed {
  ...
  const BOOL connectionToPowerRequired =
      [iTermPreferences boolForKey:kPreferenceKeyDisableMetalWhenUnplugged];
...
- (NSDictionary *)
    infoForFirstBatteryInPowerSourcesList:(CFArrayRef)powerSourcesList
  ...
  for (NSInteger i = 0; i < CFArrayGetCount(powerSourcesList); i++) {
    ...
    NSDictionary *dict = (__bridge NSDictionary *)IOPSGetPowerSourceDescription(
        powerSourcesInfo, powerSource);
    NSString *type = dict[(__bridge NSString *)CFSTR(kIOPSTypeKey)];
    if ([type isEqualToString:(__bridge NSString *)CFSTR(
...
- (iTermPowerState *)computedPowerStateWithList:(CFArrayRef)powerSourcesList
                                           info:(CFTypeRef)powerSourcesInfo {
  CFDictionaryRef info = (__bridge CFDictionaryRef)
      [self infoForFirstBatteryInPowerSourcesList:powerSourcesList
                                             info:powerSourcesInfo];
  if (!info) {
    return nil;
  }
  ...
  CFNumberRef number =
      (CFNumberRef)CFDictionaryGetValue(info, CFSTR(kIOPSCurrentCapacityKey));
  int percentage = -1;
  if (number) {
  ...
  iTermPowerState *state = [[iTermPowerState alloc] init];
  if ((CFBooleanRef)CFDictionaryGetValue(info, CFSTR(kIOPSIsChargingKey)) ==
      kCFBooleanTrue) {
    state.charging = YES;
    number = (CFNumberRef)CFDictionaryGetValue(info,
                                               CFSTR(kIOPSTimeToFullChargeKey));
  } else {
    state.charging = NO;
    number =
        (CFNumberRef)CFDictionaryGetValue(info, CFSTR(kIOPSTimeToEmptyKey));
  }
  int time = -1;
  if (number) {
  ...
  state.powerStatus = (__bridge NSString *)CFDictionaryGetValue(
      info, CFSTR(kIOPSPowerSourceStateKey));
...
- (void)addPowerStateSubscriber:(id)subscriber
                          block:(void (^)(iTermPowerState *))block {
  [_publisher addSubscriber:subscriber
                      block:^(iTermPowerState *_Nonnull payload) {
                        block(payload);
                      }];
