...
static void DecodeUTF8Bytes(unsigned char *datap, int datalen, int *rmlen,
                            VT100Token *token) {
...
static void DecodeEUCCNBytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeBIG5Bytes(unsigned char *datap, int datalen, int *rmlen,
                            VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeEUCJPBytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeSJISBytes(unsigned char *datap, int datalen, int *rmlen,
                            VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeEUCKRBytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeCP949Bytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static void DecodeOtherBytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
...
static NSString *SetReplacementCharInArray(unsigned char *datap, int *lenPtr,
                                           int badIndex) {
...
static void DecodeASCIIBytes(unsigned char *datap, int datalen, int *rmlen,
                             VT100Token *token) {
  unsigned char *p = datap;
  int len = datalen;
  ...
  // and although this while loop completed faster, the overall benchmark speed
  // on spam.cc did not improve.
  while (len > 0) {
    if (*p >= 0x20 && *p <= 0x7f) {
      p++;
      len--;
...
void ParseString(unsigned char *datap, int datalen, int *rmlen,
                 VT100Token *result, NSStringEncoding encoding) {
  *rmlen = 0;
  ...
  if (result->type == VT100_INVALID_SEQUENCE) {
    ...
    DLog(@"Parsed an invalid sequence of length %d for encoding %@: %.*s",
         *rmlen, @(encoding), datalen, datap);
    ...
    if (result.string == nil) {
      ...
      if (encoding == NSUTF8StringEncoding) {
        ...
        // Replace every byte with ?, the replacement char for non-unicode
        // encodings.
        for (i = *rmlen - 1; i >= 0 && !result.string; i--) {
