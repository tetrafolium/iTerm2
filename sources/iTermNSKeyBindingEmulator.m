...
//
//
// The purpose of this file is to parse DefaultKeyBindings.dict and to try to
// predict when keystrokes will be handled by a key binding. The first responder
// should call -handlesEvent: on each keystroke.
...
// Entries map a "normalized key" (as produced by
// dictionaryKeyForCharacters:andFlags:) to either a dictionary subtree, or to
// an array with a selector and its arguments.
...
// Special characters may be used in the key bindings dictionary to define
// buckybits required for the keystroke. They are always in a prefix of the key.
// Although they can appear in any order in the user's DefaultKeyBindings.dict
// file, once normalized they will always appear in this order in our keys.
static struct {
  unichar c;
  NSUInteger mask;
...
+ (void)initialize {
  if (self == [iTermNSKeyBindingEmulator self]) {
    ...
    AppendPinnedDebugLogMessage(@"NSKeyBindingEmulator",
                                @"Key bindings are:\n%@",
...
+ (void)loadKeyBindingsDictionary {
  ...
  if (![paths count]) {
    AppendPinnedDebugLogMessage(@"NSKeyBindingEmulator",
                                @"Failed to find library directory.");
    return;
  }
  NSString *bindPath = [paths[0]
      stringByAppendingPathComponent:@"KeyBindings/DefaultKeyBinding.dict"];
  ...
  NSDictionary *keyBindingDictionary =
      [self keyBindingDictionaryByNormalizingModifiersInKeys:theDict];
  NSMutableSet<NSString *> *temp = [NSMutableSet set];
  gRootKeyBindingsDictionary =
      [[self keyBindingDictionaryByPruningUselessBranches:keyBindingDictionary
                                      pointlessKeystrokes:temp] retain];
  gPointlessFirstKeystrokes = [temp retain];
}

+ (NSDictionary *)
    keyBindingDictionaryByPruningUselessBranches:(NSDictionary *)node
                             pointlessKeystrokes:
                                 (NSMutableSet<NSString *> *)pointless {
  ...
  for (NSString *key in node) {
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
      ...
      AppendPinnedDebugLogMessage(@"NSKeyBindingEmulator",
                                  @"Reject non-container leaf with key %@",
                                  key);
      value = nil;
    }
    if (value) {
...
// Returns the characters in a dictionary key such as "^A" (that is, the stuff
// following the special characters. Escaping backslashes in the character part
// are removed.
...
// Returns YES if |s| consists of a single upper case ASCII character, such as
// 'A' (but not '0' or 'a').
+ (BOOL)stringIsOneUpperCaseAsciiCharacter:(NSString *)s {
  return (s.length == 1 && iswascii([s characterAtIndex:0]) &&
          iswupper([s characterAtIndex:0]));
}

// Returns a version of |input| with normalized keys. Each key will consist of 0
// or more special characters in the prescribed order followed by [code %d]
// where %d is a decimal value for the keystroke ignoring modifiers. There's a
// known bug here for nonascii keystrokes modified with shift--I'm not quite
// sure how to safely lowercase them and I lack a non-US keyboard to test with.
+ (NSDictionary *)keyBindingDictionaryByNormalizingModifiersInKeys:
    (NSDictionary *)input {
  ...
  for (NSString *key in input) {
    ...
    if ([self stringIsOneUpperCaseAsciiCharacter:characters]) {
      // A key of "A" is different than a key of "a". In fact, $a isn't
      // recognized by apple's parser as a capital A. This is probably not quite
      // right (what about non-ascii characters?).
    ...
    if (!normalizedKey) {
      ...
      output[normalizedKey] =
          [self keyBindingDictionaryByNormalizingModifiersInKeys:subDict];
    } else {
...
+ (NSString *)normalizedCharacters:(NSString *)input {
  ...
  } else if (![self parseOctal:input toValue:&value] ||
             value > kMaxOctalValue) {
    return nil;
  }
...
- (BOOL)handlesEvent:(NSEvent *)event
  ...
  *pointlessly = NO;
  if (!gRootKeyBindingsDictionary && [gPointlessFirstKeystrokes count] == 0) {
    DLog(@"Short-circuit DefaultKeyBindings handling because no bindings are "
         @"defined and there are no pointless leaders");
  ...
  // Not (or no longer) in a multi-keystroke binding. Move to the root of the
  // tree.
  self.currentDict = gRootKeyBindingsDictionary;
  ...
  for (NSString *theKey in possibleKeys) {
    if ([gPointlessFirstKeystrokes containsObject:theKey]) {
      *pointlessly = YES;
      DLog(@"Keystroke is pointless. Caller should not pass to handleEvent or "
           @"interpretKeyEvents.");
      return YES;
    }
  }
...
- (NSString *)charactersIgnoringAllModifiersInEvent:(NSEvent *)event {
  ...
  NSString *theString = (NSString *)CFStringCreateWithCharacters(
...
// If shift is pressed, then the result is [ "A", "$A" ] or [ "$<esc>" ] for
// chars that don't have an uppercase version.
- (NSArray *)dictionaryKeysForEvent:(NSEvent *)event {
  NSString *charactersIgnoringModifiersExceptShift =
      [event charactersIgnoringModifiers];
  NSUInteger flags = [event it_modifierFlags];
  NSMutableArray *result = [NSMutableArray array];

  NSString *theKey = [self.class dictionaryKeyForCharacters:
                                     charactersIgnoringModifiersExceptShift
                                                   andFlags:flags];
  if (theKey) {
  ...
  NSString *charactersIgnoringAllModifiers =
      [self charactersIgnoringAllModifiersInEvent:event];
  if (charactersIgnoringAllModifiers && (flags & NSEventModifierFlagShift) &&
      [charactersIgnoringAllModifiers
          isEqualToString:charactersIgnoringAllModifiers]) {
    // The shifted version differs from the unshifted version (e.g., A vs a) so
    // add "A" since we already have "$A" ("A" is a lower priority than "$A").
    theKey =
        [self.class dictionaryKeyForCharacters:
                        charactersIgnoringModifiersExceptShift
                                      andFlags:(flags &
                                                ~NSEventModifierFlagShift)];
    if (theKey) {
