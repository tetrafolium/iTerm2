...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p dir=%@ last=%@>", self.class,
                                    self, _currentDirectory, _lastPollTime];
}
...
- (void)poll {
  ...
  iTermGitPollWorker *worker =
      [iTermGitPollWorker instanceForPath:self.currentDirectory];
...
- (void)setCurrentDirectory:(NSString *)currentDirectory {
  ...
  if (currentDirectory) {
    ...
    [_rateLimit performRateLimitedBlock:^{
      ...
      iTermGitPollWorker *worker =
          [iTermGitPollWorker instanceForPath:currentDirectory];
