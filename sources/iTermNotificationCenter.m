...
// This may outlive the object it is associated with, but that's ok because the
// block verifies the owner still exists before calling its block.
...
+ (void)subscribe:(NSObject *)owner selector:(SEL)selector {
  ...
  [self internalSubscribe:owner
                withBlock:^(id notification) {
                  [weakOwner it_performNonObjectReturningSelector:selector
...
+ (void)internalSubscribe:(NSObject *)owner
  ...
  __weak NSObject *weakOwner = owner;
  // This prevents infinite recursion if you cause the notification to be sent
  // while handling it.
  __block BOOL handling = NO;
  id token = [[NSNotificationCenter defaultCenter]
      addObserverForName:iTermInternalNotification
                  object:[self class]
                   queue:nil
  ...
  [owner
      it_setAssociatedObject:[[iTermNotificationCenterObserverUnregisterer
                                 alloc] initWithToken:token]
                      forKey:(void *)&iTermNotificationTokenAssociatedObject];
}

- (void)post {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermInternalNotification
                    object:[self class]
