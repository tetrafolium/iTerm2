...
NSString *const iTermFileDescriptorMultiClientErrorDomain =
    @"iTermFileDescriptorMultiClientErrorDomain";

@implementation iTermFileDescriptorMultiClient {
  NSString *_socketPath; // Thread safe because this is only assigned to in
                         // -initWithPath:
  ...
  if (self) {
    ...
    _thread = [iTermThread withLabel:@"com.iterm2.multi-client"
                        stateFactory:^iTermSynchronizedState *_Nullable(
                          ...
                          return [[iTermFileDescriptorMultiClientState alloc]
                              initWithQueue:queue];
                        }];
  }
  return self;
...
- (pid_t)serverPID {
  __block pid_t pid;
  [_thread
      dispatchSync:^(iTermFileDescriptorMultiClientState *_Nullable state) {
        pid = state.serverPID;
      }];
  return pid;
}

- (void)attachToOrLaunchNewDaemonWithCallback:
    (iTermCallback<id, NSNumber *> *)callback {
  [_thread
      dispatchAsync:^(iTermFileDescriptorMultiClientState *_Nonnull state) {
...
- (void)attachWithCallback:(iTermCallback<id, NSNumber *> *)callback {
  ...
  [_thread dispatchAsync:^(iTermFileDescriptorMultiClientState *state) {
    [self tryAttachWithState:state
                    callback:[thread newCallbackWithBlock:^(
                                         iTermFileDescriptorMultiClientState
                                             *_Nonnull state,
                                         NSNumber *_Nullable value) {
                      const iTermFileDescriptorMultiClientAttachStatus status =
...
// Connect on the unix domain socket. If successful, handshake and start
// read-dispatch loop and run callback. Otherwise, launch a server, handshake,
// start the read-dispatch loop, and then run the callback.
- (void)attachToOrLaunchNewDaemonServerWithState:
            (iTermFileDescriptorMultiClientState *)state
                                        callback:
                                            (iTermCallback<id, NSNumber *> *)
                                                callback {
  [state check];
  __weak __typeof(self) weakSelf = self;
  [self
      tryAttachWithState:state
                callback:[_thread newCallbackWithBlock:^(
                  ...
                  [strongSelf didAttachWithState:state
...
// Connect to the unix domain socket. Read the hello message. Then run the
// callback. NOTE: Sets _readFD and_writeFD as a side-effect. Result will be
// @(enum iTermFileDescriptorMultiClientAttachStatus).
- (void)tryAttachWithState:(iTermFileDescriptorMultiClientState *)state
  ...
  if (status != iTermFileDescriptorMultiClientAttachStatusSuccess) {
    ...
    DLog(@"tryAttachWithState(%@): Server dead or already connected",
         _socketPath);
  ...
  __weak __typeof(self) weakSelf = self;
  [self
      readMessageWithState:state
                  callback:[_thread newCallbackWithBlock:^(
                                        iTermFileDescriptorMultiClientState
                                            *_Nonnull state,
                                        iTermResult<
                                            iTermClientServerProtocolMessageBox
                                                *> *_Nullable result) {
                    __strong __typeof(self) strongSelf = weakSelf;
                    if (!strongSelf) {
                      [callback
                    ...
                    __block BOOL ok = NO;
                    [result
                        handleObject:^(iTermClientServerProtocolMessageBox
                                           *_Nonnull messageBox) {
                          if (messageBox.message.fileDescriptor) {
                            state.writeFD =
                                messageBox.message.fileDescriptor.intValue;
                            DLog(@"tryAttachWithState(%@): Success",
                            ...
                            DLog(@"tryAttachWithState(%@): Failed to get a "
                                 @"file descriptor",
                                 strongSelf->_socketPath);
                          }
                        }
                        error:^(NSError *_Nonnull error) {
                          DLog(@"tryAttachWithState(%@): Error reading file "
                               @"descriptor: %@",
                    ...
                    if (!ok) {
                      close(state.readFD);
                      state.readFD = -1;
                      // You can get here if the server crashes right after
                      // accepting the connection. It can also happen if the
                      // server already has a connected client and rejects.
                      DLog(@"tryAttachWithState(%@): Fatal error (see above)",
                           strongSelf->_socketPath);
                      [callback
                          invokeWithObject:
                              @(iTermFileDescriptorMultiClientAttachStatusFatalError)];
...
- (void)handshakeWithState:(iTermFileDescriptorMultiClientState *)state
  ...
  iTermCallback<id, NSNumber *> *innerCallback = [_thread
      newCallbackWithBlock:^(iTermFileDescriptorMultiClientState *state,
  ...
  __weak __typeof(self) weakSelf = self;
  [self
      sendHandshakeRequestWithState:state
                           callback:[_thread newCallbackWithBlock:^(
                                                 iTermFileDescriptorMultiClientState
                                                     *state,
                                                 NSNumber *sendResult) {
                             [weakSelf
                                 didSendHandshakeRequestWithSuccess:
                                     sendResult.boolValue
                                                              state:state
                                                           callback:
                                                               innerCallback
                                                childDiscoveryBlock:^(
                                                    ...
                                                    iTermMultiServerReportChild
                                                        *report) {
                                                  // Report must be consumed
                                                  // synchronously!
                                                  [weakSelf
                                                      didDiscoverChild:report
                                                                 state:state];
                                                }];
                           }]];
}
...
- (void)
    sendHandshakeRequestWithState:(iTermFileDescriptorMultiClientState *)state
                         callback:(iTermCallback<id, NSNumber *> *)callback {
  iTermMultiServerClientOriginatedMessage message = {
      .type = iTermMultiServerRPCTypeHandshake,
      .payload = {.handshake = {.maximumProtocolVersion =
...
- (void)
    didSendHandshakeRequestWithSuccess:(BOOL)sendOK
                                 state:(iTermFileDescriptorMultiClientState *)
                                           state
                              callback:(iTermCallback<id, NSNumber *> *)callback
                   childDiscoveryBlock:
                       (void (^)(iTermFileDescriptorMultiClientState *state,
  ...
  [self
      readHandshakeResponseWithState:state
                          completion:^(
                              iTermFileDescriptorMultiClientState *state,
                              BOOL ok, int numberOfChildren, pid_t pid) {
                            if (!ok) {
                              [callback invokeWithObject:@NO];
                              return;
                            }
                            state.serverPID = pid;
                            [self
                                readInitialChildReports:numberOfChildren
                                                  state:state
                                                  block:block
                                             completion:^(
                                                 iTermFileDescriptorMultiClientState
                                               ...
                                               // Ensure the completion callback
                                               // runs before the read loop
                                               // starts producing events.
                                               [callback invokeWithObject:@YES];
                                               [self
                                                   readAndDispatchNextMessageWhenReadyWithState:
                                                       state];
                                             }];
                          }];
}
...
- (void)readHandshakeResponseWithState:
            (iTermFileDescriptorMultiClientState *)state
                            completion:
                                (void (^)(
                                    iTermFileDescriptorMultiClientState *state,
                                    BOOL ok, int numberOfChildren,
  ...
  [self
      readMessageWithState:state
                  callback:[_thread newCallbackWithBlock:^(
                    ...
                    [result
                        handleObject:^(iTermClientServerProtocolMessageBox
                                           *_Nonnull boxedMessage) {
                          DLog(@"Got a response for %@", socketPath);
                          if (!boxedMessage.decoded) {
                            DLog(@"Failed to decode message for %@",
                                 socketPath);
                            completion(state, NO, 0, -1);
                            return;
                          }
                          if (boxedMessage.decoded->type !=
                              iTermMultiServerRPCTypeHandshake) {
                            DLog(@"Got an unexpected response for %@",
                                 socketPath);
                            completion(state, NO, 0, -1);
                            return;
                          }
                          DLog(@"Got a valid handshake response for %@",
                               socketPath);
                          if (boxedMessage.decoded->payload.handshake
                                  .protocolVersion !=
                              iTermMultiServerProtocolVersion2) {
                            completion(state, NO, 0, -1);
                            return;
                          ...
                          completion(
                              state, YES,
                              boxedMessage.decoded->payload.handshake
                                  .numChildren,
                              boxedMessage.decoded->payload.handshake.pid);
                        }
                        error:^(NSError *_Nonnull error) {
                          DLog(@"FAILED: Invalid handshake response for %@",
                               socketPath);
                          completion(state, NO, 0, -1);
                        }];
                  }]];
...
- (void)readInitialChildReports:(int)numberOfChildren
                     ...
                     completion:
                         (void (^)(iTermFileDescriptorMultiClientState *state,
                                   BOOL ok))completion {
  DLog(@"Read initial child reports (%@) for %@", @(numberOfChildren),
       _socketPath);
  ...
  [self
      readMessageWithState:state
                  callback:[_thread newCallbackWithBlock:^(
                    ...
                    [result
                        handleObject:^(
                            iTermClientServerProtocolMessageBox *_Nonnull box) {
                          __strong __typeof(self) strongSelf = weakSelf;
                          ...
                          if (box.decoded->type !=
                              iTermMultiServerRPCTypeReportChild) {
                            DLog(@"Unexpected message type when reading child "
                                 @"reports for %@",
                                 socketPath);
                            completion(state, NO);
                            return;
                          ...
                          const BOOL foundLast =
                              message->payload.reportChild.isLast;
                          if (!foundLast) {
                            ...
                            [strongSelf->_thread
                                dispatchAsync:^(
                                    iTermFileDescriptorMultiClientState
                                        *_Nullable state) {
                                  DLog(@"Want another child report for %@",
                                       socketPath);
                                  [strongSelf
                                      readInitialChildReports:numberOfChildren -
                                                              1
                        ...
                        error:^(NSError *_Nonnull error) {
                          DLog(@"FAILED to read initial child report %@: %@",
                               socketPath, error);
                          completion(state, NO);
                        }];
                  }]];
...
- (void)didDiscoverChild:(iTermMultiServerReportChild *)report
  ...
  iTermFileDescriptorMultiClientChild *child =
      [[iTermFileDescriptorMultiClientChild alloc]
...
// These C pointers live until the callback is run.
- (void)
    launchChildWithExecutablePath:(const char *)path
                             argv:(const char **)argv
                      environment:(const char **)environment
                              pwd:(const char *)pwd
                         ttyState:(iTermTTYState)ttyState
  ...
  iTermMultiServerClientOriginatedMessage message = {
      ...
      .payload = {.launch = {.path = path,
                             .argv = argv,
                             .argc = LengthOfNullTerminatedPointerArray(
                                 (const void **)argv),
                             .envp = environment,
                             .envc = LengthOfNullTerminatedPointerArray(
  ...
  iTermMultiServerClientOriginatedMessage messageCopy =
      [self copyLaunchRequest:message];

  DLog(@"Add pending launch %@ for %@", @(message.payload.launch.uniqueId),
       _socketPath);
  state.pendingLaunches[@(message.payload.launch.uniqueId)] =
      [[iTermFileDescriptorMultiClientPendingLaunch alloc]
          initWithRequest:messageCopy.payload.launch
                 callback:callback
                   thread:_thread];
      ...
      callback:[_thread newCallbackWithBlock:^(
        ...
        if (state.pendingLaunches[@(uniqueID)]) {
          ...
          [callback invokeWithObject:[iTermResult
                                         withError:[self connectionLostError]]];
        }
      }]];
}

// Called on job manager's queue via [self launchChildWithExecutablePath:…]
- (iTermMultiServerClientOriginatedMessage)copyLaunchRequest:
    (iTermMultiServerClientOriginatedMessage)original {
  ...
  {
    const int status =
        iTermMultiServerProtocolEncodeMessageFromClient(&original, &temp);
    assert(status == 0);
  }
  ...
  {
    const int status =
        iTermMultiServerProtocolParseMessageFromClient(&temp, &messageCopy);
    assert(status == 0);
  }
...
// Handle a launch response, telling us about a child that was hopefully forked
// and execed. Add a new child and run the callback for the pending launch.
...
- (void)handleLaunch:(iTermMultiServerResponseLaunch)launch
  ...
  iTermFileDescriptorMultiClientPendingLaunch *pendingLaunch =
      state.pendingLaunches[@(launch.uniqueId)];
  if (!pendingLaunch) {
    ITBetaAssert(NO, @"No pending launch for %@ in %@", @(launch.uniqueId),
  ...
  if (launch.status != 0) {
    DLog(@"handleLaunch: error status %@ for %@", @(launch.status),
         _socketPath);
    [pendingLaunch.launchCallback
        invokeWithObject:[iTermResult withError:self.forkError]];
  ...
  iTermFileDescriptorMultiClientChild *child =
      [[iTermFileDescriptorMultiClientChild alloc] initWithReport:&fakeReport
                                                           thread:_thread];
  [self addChild:child state:state attached:YES];
  DLog(@"handleLaunch: Success for pid %@ from %@", @(launch.pid), _socketPath);
  [pendingLaunch.launchCallback
      invokeWithObject:[iTermResult withObject:child]];
      ...
      [_thread newCallbackWithBlock:^(
        ...
        [waitResult
            handleObject:^(NSNumber *_Nonnull statusNumber) {
              DLog(@"wait for %@ returned termination status %@",
                   @(weakChild.pid), statusNumber);
...
- (void)
    didWriteWaitRequestWithStatus:(BOOL)sendOK
...
- (void)handleWait:(iTermMultiServerResponseWait)wait
             state:(iTermFileDescriptorMultiClientState *)state {
  DLog(@"handleWait for socket %@ pid %@ with error %@, status %@", _socketPath,
       @(wait.pid), @(wait.resultType), @(wait.status));
...
- (void)handleTermination:(iTermMultiServerReportTermination)termination
  ...
  iTermFileDescriptorMultiClientChild *child =
      [self childWithPID:termination.pid state:state];
  if (child) {
    [child didTerminate];
...
// Launch a new daemon, handshake with it, start the read-dispatch loop, and run
// the callback.
- (void)launchNewDaemonAndHandshakeWithState:
            (iTermFileDescriptorMultiClientState *)state
                                    callback:(iTermCallback<id, NSNumber *> *)
                                                 callback {
  assert(state.readFD < 0);
  assert(state.writeFD < 0);
...
- (NSString *)serverPath {
  ...
  NSString *filename =
      [NSString stringWithFormat:@"iTermServer-%@", versionNumber];
  NSString *appSupport =
...
// Copy iTermServer to a safe location where Autoupdate won't delete it. See
// issue 9022 for wild speculation on why this is important.
- (NSString *)serverPathCopyingIfNeeded {
  ...
  if (!desiredPath) {
    [self
        showError:nil
          message:[NSString
                      stringWithFormat:
                          @"Neither ~/Library/Application Support/iTerm2 nor "
                          @"~/.iterm2 are writable directories. This prevents "
                          @"the session restoration server from running. "
                          @"Please correct the problem and restart iTerm2."]
           badURL:nil];
    return nil;
  }
  ...
  if (![fileManager fileExistsAtPath:desiredPath]) {
    NSString *sourcePath = [[NSBundle bundleForClass:self.class]
        pathForAuxiliaryExecutable:@"iTermServer"];
    NSError *error = nil;
    ...
    if (error) {
      [self showError:error
              message:[NSString stringWithFormat:@"Could not copy %@ to %@: %@",
  ...
  {
    NSError *error = nil;
    NSDictionary *attributes = [fileManager attributesOfItemAtPath:desiredPath
                                                             error:&error];
    if (error) {
      [self showError:error
              message:[NSString
                          stringWithFormat:@"Could not check permissions on %@",
                                           desiredPath]
               badURL:[NSURL fileURLWithPath:desiredPath]];
      return nil;
    }
    ...
    if (error) {
      [self showError:error
              message:[NSString stringWithFormat:
                                    @"Could not set 0700 permissions on %@",
                                    desiredPath]
               badURL:[NSURL fileURLWithPath:desiredPath]];
      return nil;
    }
...
- (BOOL)launchNewDaemonWithState:(iTermFileDescriptorMultiClientState *)state {
  ...
  state.daemonProcessSource =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, forkState.pid,
                             DISPATCH_PROC_EXIT, state.queue);
...
- (void)closeWithState:(iTermFileDescriptorMultiClientState *)state {
  ...
  NSDictionary<NSNumber *, iTermFileDescriptorMultiClientPendingLaunch *>
      *pendingLaunches = [state.pendingLaunches copy];
  ...
  [state.children enumerateObjectsUsingBlock:^(
    ...
    [child invokeAllWaitCallbacks:[iTermResult
                                      withError:[self connectionLostError]]];
  }];
...
// This is the read-dispatch loop. readAndDispatchNextMessageWithState will call
// this method after it's done reading a message.
- (void)readAndDispatchNextMessageWhenReadyWithState:
    (iTermFileDescriptorMultiClientState *)state {
  if (state.readFD < 0 || state.writeFD < 0) {
    DLog(@"readAndDispatchNextMessageWhenReadyWithState: aborting because "
         @"files are closed");
    return;
  }
  DLog(@"readAndDispatchNextMessageWhenReadyWithState: registring read "
       @"callback");
  __weak __typeof(self) weakSelf = self;
...
- (void)readAndDispatchNextMessageWithState:
    (iTermFileDescriptorMultiClientState *)state {
                  ...
                  callback:[_thread newCallbackWithBlock:^(
                    ...
                    [result
                        handleObject:^(iTermClientServerProtocolMessageBox
                          ...
                          if (!ok) {
                            DLog(@"Close connection because dispatch failed %@",
                                 socketPath);
                          }
                        }
                        error:^(NSError *_Nonnull error) {
                          DLog(
                              @"FAILED to read message to be dispatched %@: %@",
                              socketPath, error);
                          ok = NO;
                        }];
                    if (!ok) {
...
- (BOOL)dispatch:(iTermClientServerProtocolMessageBox *)box
  ...
  case iTermMultiServerRPCTypeReportChild:
    DLog(@"Close %@ because of unexpected message of type %@", _socketPath,
         @(decoded->type));
    [self closeWithState:state];
    break;
  }
...
- (void)
    readMessageWithState:(iTermFileDescriptorMultiClientState *)state
                callback:
                    (iTermCallback<
                        id,
                        iTermResult<iTermClientServerProtocolMessageBox *> *> *)
                        callback {
           ...
           callback:[_thread
                        newCallbackWithBlock:^(
                            ...
                            iTermResult<iTermMultiServerMessage *>
                                *_Nullable result) {
                          [result
                              handleObject:^(iTermMultiServerMessage
                                                 *_Nonnull lengthMessage) {
                                __strong __typeof(self) strongSelf = weakSelf;
                                if (!strongSelf) {
                                  DLog(@"Looks like I've been dealloced. %@",
                                       socketPath);
                                  [callback
                                      invokeWithObject:
                                          [iTermResult
                                ...
                                assert(lengthMessage.data.length ==
                                       sizeof(size_t));
                                size_t length;
                                memmove(&length, lengthMessage.data.bytes,
                                        sizeof(size_t));
                                DLog(@"Next message length is %@. %@",
                                     @(length), socketPath);
                                static const NSInteger MAX_MESSAGE_SIZE =
                                    1024 * 1024;
                                if (length > MAX_MESSAGE_SIZE) {
                                  DLog(@"Max length exceeded, return io error "
                                       @"%@",
                                       socketPath);
                                  [callback
                                      invokeWithObject:
                                          [iTermResult
                                              withError:strongSelf.ioError]];
                                  return;
                                }

                                DLog(@"Will read payload of length %@. %@",
                                     @(length), socketPath);
                                // Now read the payload including a possible
                                // file descriptor.
                                                                                 ...
                                                                                 ^(
                                                                                   ...
                                                                                   [result
                                                                                       handleObject:^(
                                                                                         ...
                                                                                         if (!strongSelf) {
                                                                                           DLog(
                                                                                               @"I've been dealloced while reading %@",
                                                                                               socketPath);
                                                                                           [callback
                                                                                               invokeWithObject:
                                                                                                   [iTermResult
                                                                                         ...
                                                                                         iTermClientServerProtocolMessageBox
                                                                                             *encodedBox = [iTermClientServerProtocolMessageBox
                                                                                                 withMessage:
                                                                                                     payload];

                                                                                         if (!encodedBox) {
                                                                                           DLog(
                                                                                               @"Decoding: recv failed for %@",
                                                                                               strongSelf
                                                                                                   ->_socketPath);
                                                                                           [callback
                                                                                               invokeWithObject:
                              ...
                              error:^(NSError *_Nonnull error) {
                                DLog(@"Failed to read next message's length "
                                     @"%@: %@",
                                     socketPath, error);
                                [callback
                                    invokeWithObject:[iTermResult
                                                         withError:error]];
                              }];
                        }]];
}
...
- (void)readWithState:(iTermFileDescriptorMultiClientState *)state
             ...
             callback:
                 (iTermCallback<id, iTermResult<iTermMultiServerMessage *> *> *)
                     callback {
  iTermMultiServerMessageBuilder *builder =
      [[iTermMultiServerMessageBuilder alloc] init];
  [state whenReadable:^(iTermFileDescriptorMultiClientState *state) {
    [self partialReadWithState:state
                   totalLength:length
                       builder:builder
                      callback:callback];
  }];
...
- (void)
    partialReadWithState:(iTermFileDescriptorMultiClientState *)state
  ...
  if (state.readFD < 0) {
    ...
    [callback
        invokeWithObject:[iTermResult withError:self.connectionLostError]];
    return;
  }

  iTermClientServerProtocolMessage message;
  ssize_t bytesRead = iTermMultiServerReadMessage(state.readFD, &message,
                                                  totalLength - builder.length);
  if (bytesRead < 0) {
    if (errno == EAGAIN) {
      DLog(@"Nothing to read %@. Will wait for the socket to become readable "
           @"and try again later.",
           _socketPath);
      __weak __typeof(self) weakSelf = self;
      [state whenReadable:^(iTermFileDescriptorMultiClientState *state) {
  ...
  __weak __typeof(self) weakSelf = self;
  DLog(@"Have read %@/%@ from %@. Wait for socket to be readable again.",
       @(builder.length), @(totalLength), _socketPath);
  [state whenReadable:^(iTermFileDescriptorMultiClientState *state) {
    [weakSelf partialReadWithState:state
                       totalLength:totalLength
                           builder:builder
...
- (void)send:(iTermMultiServerClientOriginatedMessage *)message
  ...
  if (iTermMultiServerProtocolEncodeMessageFromClient(
          message, &clientServerProtocolMessage)) {
...
- (void)tryWrite:(NSData *)data
  ...
  const ssize_t bytesWritten =
      iTermFileDescriptorClientWrite(state.writeFD, data.bytes, data.length);
  const NSInteger dataLength = data.length;
  const int savedErrno = errno;
  ITAssertWithMessage(bytesWritten <= dataLength,
                      @"Data length is %@ but wrote %@. errno is %d",
                      @(dataLength), @(bytesWritten), savedErrno);
  DLog(@"Wrote %@/%@ error=%s", @(bytesWritten), @(dataLength),
       strerror(savedErrno));
  ...
  [state whenWritable:^(iTermFileDescriptorMultiClientState *_Nullable state) {
    ITAssertWithMessage(bytesWritten <= dataLength,
                        @"Data length is %@ but wrote %@.", @(dataLength),
                        @(bytesWritten));
    [weakSelf
        tryWrite:[data
                     subdataFromOffset:MAX(0,
                                           bytesWritten)] // NOTE: bytesWritten
                                                          // can be negative if
                                                          // we get EAGAIN
           state:state
        callback:callback];
...
- (NSError *)waitError:(int)errorNumber {
  iTermFileDescriptorMultiClientErrorCode code =
      iTermFileDescriptorMultiClientErrorCodeUnknown;
  switch (errorNumber) {
  case 2:
