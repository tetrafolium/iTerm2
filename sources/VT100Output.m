...
- (NSData *)keyBackspace {
  if (_keyStrings[TERMINFO_KEY_BACKSPACE]) {
    ...
    return [NSData dataWithBytes:KEY_BACKSPACE
                          length:STATIC_STRLEN(KEY_BACKSPACE)];
  }
}
...
- (NSData *)keyPageDown:(unsigned int)modflag {
  ...
  if (_keyStrings[TERMINFO_KEY_PAGEDOWN]) {
    theSuffix =
        [NSData dataWithBytes:_keyStrings[TERMINFO_KEY_PAGEDOWN]
...
- (NSData *)keypadData:(unichar)unicode keystr:(NSString *)keystr {
  ...
  case '-':
    theData = [NSData dataWithBytes:ALT_KP_MINUS
                             length:STATIC_STRLEN(ALT_KP_MINUS)];
    break;
  case '+':
    theData = [NSData dataWithBytes:ALT_KP_PLUS
                             length:STATIC_STRLEN(ALT_KP_PLUS)];
    break;
  case '.':
    theData = [NSData dataWithBytes:ALT_KP_PERIOD
                             length:STATIC_STRLEN(ALT_KP_PERIOD)];
    break;
  case '/':
    theData = [NSData dataWithBytes:ALT_KP_SLASH
                             length:STATIC_STRLEN(ALT_KP_SLASH)];
    break;
  case '*':
    theData = [NSData dataWithBytes:ALT_KP_STAR
                             length:STATIC_STRLEN(ALT_KP_STAR)];
    break;
  case '=':
    theData = [NSData dataWithBytes:ALT_KP_EQUALS
                             length:STATIC_STRLEN(ALT_KP_EQUALS)];
    break;
  case 0x03:
    theData = [NSData dataWithBytes:ALT_KP_ENTER
                             length:STATIC_STRLEN(ALT_KP_ENTER)];
    break;
  default:
...
- (char *)mouseReport:(int)button atX:(int)x Y:(int)y {
  ...
  case MOUSE_FORMAT_XTERM_EXT:
    // TODO: This doesn' thandle positions greater than 223 correctly. It should
    // use UTF-8.
    snprintf(buf, sizeof(buf), "\033[M%c%lc%lc", (wint_t)(32 + button),
             (wint_t)(32 + x), (wint_t)(32 + y));
    break;
  ...
  case MOUSE_FORMAT_SGR:
    if (button & MOUSE_BUTTON_SGR_RELEASE_FLAG) {
      ...
      snprintf(buf, sizeof(buf), "\033[<%d;%d;%dm",
               button ^ MOUSE_BUTTON_SGR_RELEASE_FLAG, x, y);
    } else {
  ...
  default:
    snprintf(buf, sizeof(buf), "\033[M%c%c%c", 32 + button, MIN(255, 32 + x),
             MIN(255, 32 + y));
    break;
  }
  return buf;
...
- (NSData *)mousePress:(int)button
         withModifiers:(unsigned int)modflag
...
- (NSData *)mouseRelease:(int)button
           withModifiers:(unsigned int)modflag
                      at:(VT100GridCoord)coord {
  int cb;
...
- (NSData *)reportiTerm2Version {
  ...
  // The [ must never be followed by a 0 (see the isiterm2.sh script for
  // justification).
  NSString *version =
      [NSString stringWithFormat:@"%c[ITERM2 %@n", ESC,
...
- (NSData *)reportDeviceAttribute {
  // VT220 + sixel
  // For a very long time we returned 1;2, like most other terms, but we need to
  // advertise sixel support. Let's see what happens! New in version 3.3.0.
  //
  // Update: Per issue 7803, VT200 must accept 8-bit CSI. Allow users to elect
  // VT100 reporting by setting $TERM to VT100.
  switch (_vtLevel) {
...
- (NSData *)reportExtendedDeviceAttribute {
  NSString *versionString = [[[NSBundle mainBundle] infoDictionary]
      objectForKey:@"CFBundleShortVersionString"];
  NSString *reportString = [NSString
      stringWithFormat:@"%cP>|iTerm2 %@%c\\", ESC, versionString, ESC];
  return [reportString dataUsingEncoding:NSUTF8StringEncoding];
}

- (NSData *)reportColor:(NSColor *)color
                atIndex:(int)index
                 prefix:(NSString *)prefix {
  NSString *string = nil;
  if ([iTermAdvancedSettingsModel oscColorReport16Bits]) {
    string =
        [NSString stringWithFormat:@"%c]%@%d;rgb:%04x/%04x/%04x%c", ESC, prefix,
                                   index, (int)([color redComponent] * 65535.0),
                                   (int)([color greenComponent] * 65535.0),
                                   (int)([color blueComponent] * 65535.0), 7];
  } else {
    string =
        [NSString stringWithFormat:@"%c]%@%d;rgb:%02x/%02x/%02x%c", ESC, prefix,
                                   index, (int)([color redComponent] * 255.0),
                                   (int)([color greenComponent] * 255.0),
                                   (int)([color blueComponent] * 255.0), 7];
  }
...
- (NSData *)reportChecksum:(int)checksum withIdentifier:(int)identifier {
  // DCS Pid ! ~ D..D ST
  NSString *string =
      [NSString stringWithFormat:@"%cP%d!~%04x%c\\", ESC, identifier,
                                 (short)checksum, ESC];
  return [string dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSData *)reportSGRCodes:(NSArray<NSString *> *)codes {
  NSString *string = [NSString
      stringWithFormat:@"%c[%@m", ESC, [codes componentsJoinedByString:@";"]];
  return [string dataUsingEncoding:NSUTF8StringEncoding];
}
...
- (int)cursorModifierParamForEventModifierFlags:(NSEventModifierFlags)modflag {
  ...
  int *modValues =
      _optionIsMetaForSpecialKeys ? metaModifierValues : altModifierValues;
