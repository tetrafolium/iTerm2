...
- (void)beginProbeWithBackspace:(NSData *)backspace
  ...
  if (backspace) {
    // Try to figure out if we're at a shell prompt. Send a space character and
    // immediately backspace over it. If no output is received within a
    // specified timeout, then go ahead and send the password. Otherwise, ask
    // for confirmation.
    ...
    // Rare case: we don't know how to send a backspace. Just enter the
    // password.
    [self enterPassword];
  }
}
  ...
  @synchronized(self) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (_state != previousState) {
        DLog(@"%@ went %@->%@ because of token %@", self, @(previousState),
             @(_state), token);
      }
      if (_state == iTermEchoProbeOff || _state == iTermEchoProbeFailed) {
        break;
...
iTermEchoProbeState iTermEchoProbeGetNextState(iTermEchoProbeState state,
                                               VT100Token *token) {
  switch (state) {
