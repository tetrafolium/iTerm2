...
// Can't check on release because autorelease pools can be drained on a
// different thread.
...
+ (instancetype)uncheckedSharedInstance {
  ...
  dispatch_once(&onceToken, ^{
    instance =
        [[iTermMainThreadState alloc] initWithQueue:dispatch_get_main_queue()];
  });
  return instance;
}
...
+ (instancetype)main {
  ...
  dispatch_once(&onceToken, ^{
    instance =
        [[self alloc] initWithQueue:dispatch_get_main_queue()
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p queue=%@>",
...
#if CHECK_DOUBLE_INVOKES
  NSArray *stacks =
      [[iTermThread currentThread] currentStacks]
...
- (void)dispatchSync:(void (^NS_NOESCAPE)(id))block {
  [self retain];
  assert(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL) !=
         dispatch_queue_get_label(_queue));
...
- (iTermCallback *)newCallbackWithWeakTarget:(id)target
...
#if CHECK_DOUBLE_INVOKES
    ITAssertWithMessage(!self->_invokeStack,
                        @"Previously invoked from:\n%@\n\nNow invoked "
                        @"from:\n%@\n\nCreated from:\n%@\n%@",
