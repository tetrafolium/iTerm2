...
void iTermFunctionCallSplitFullyQualifiedName(NSString *fqName,
                                              NSString **namespacePtr,
                                              NSString **relativeNamePtr) {
...
- (NSString *)description {
  NSString *params =
      [[_argToExpression.allKeys mapWithBlock:^id(NSString *name) {
        return [NSString
            stringWithFormat:@"%@: %@", name, self->_argToExpression[name]];
      }] componentsJoinedByString:@", "];
  NSString *value =
      [NSString stringWithFormat:@"%@(%@)", self.fullyQualifiedName, params];
  return [NSString stringWithFormat:@"<Func %@>", value];
}

- (NSString *)signature {
  return iTermFunctionSignatureFromNamespaceAndNameAndArguments(
      self.namespace, self.name, _argToExpression.allKeys);
}
...
- (BOOL)isEqual:(id)object {
  ...
  return ([NSObject object:self.fullyQualifiedName
              isEqualToObject:other.fullyQualifiedName] &&
          [NSObject object:_argToExpression
              isEqualToObject:other->_argToExpression]);
}

#pragma mark - APIs

+ (iTermParsedExpression *)
    callMethod:(NSString *)invocation
...
+ (iTermParsedExpression *)
    callFunction:(NSString *)invocation
         timeout:(NSTimeInterval)timeout
    ...
    callFunction:(NSString *)invocation
        receiver:(NSString *)receiver
         timeout:(NSTimeInterval)timeout
           scope:(iTermVariableScope *)scope
      completion:(void (^)(id, NSError *, NSSet<NSString *> *))completion {
  iTermVariableScope *placeholderScope =
      scope ? nil : [[iTermVariablePlaceholderScope alloc] init];
...
- (void)performMethodCallFromInvocation:(NSString *)invocation
                             ...
                             completion:
                                 (void (^)(id, NSError *,
                                           NSSet<NSString *> *))completion {
  id<iTermObject> object =
      [[iTermVariablesIndex sharedInstance] variablesForKey:receiver].owner;
  if (!object) {
    NSString *reason = [NSString
        stringWithFormat:@"Object with identifier “%@” not found", receiver];
    completion(nil,
               [NSError errorWithDomain:@"com.iterm2.call"
                                   code:5
                               userInfo:@{NSLocalizedDescriptionKey : reason}],
...
- (void)functionCallDidTimeOutAfter:(NSTimeInterval)timeout
                         ...
                         completion:(void (^)(id, NSError *,
                                              NSSet<NSString *> *))completion {
  NSString *reason;
  if (_remainingArgs.count) {
    reason = [NSString
        stringWithFormat:
            @"Timeout (%@ sec) while evaluating invocation “%@”. The timeout "
            @"occurred while evaluating the following arguments to %@: %@",
            @(timeout), invocation, self.fullyQualifiedName,
            [_remainingArgs.allObjects componentsJoinedByString:@", "]];
  } else {
    reason =
        [NSString stringWithFormat:
                      @"Timeout (%@ sec) while evaluating invocation “%@”. The "
                      @"timeout occurred while waiting for %@ to return.",
                      @(timeout), invocation, self.name];
  }
  NSDictionary *userInfo = @{NSLocalizedDescriptionKey : reason};
  if (self.connectionKey) {
    userInfo = [userInfo
        dictionaryBySettingObject:self.connectionKey
                           forKey:
                               iTermAPIHelperFunctionCallErrorUserInfoKeyConnection];
  }
  iTermScriptHistoryEntry *entry = [[iTermAPIHelper sharedInstance]
      scriptHistoryEntryForConnectionKey:self.connectionKey];
...
- (void)performFunctionCallFromInvocation:(NSString *)invocation
  ...
  if (timeout > 0) {
    // NOTE: The timer's block retains the completion block which is what keeps
    // the caller from being dealloc'ed when it is an iTermExpressionEvaluator.
    __weak __typeof(self) weakSelf = self;
    timer = [NSTimer
        it_scheduledTimerWithTimeInterval:timeout
                                  repeats:NO
                                    block:^(NSTimer *_Nonnull theTimer) {
                                      ...
                                      timer = nil;
                                      [weakSelf
                                          functionCallDidTimeOutAfter:timeout
...
- (void)addParameterWithName:(NSString *)name
            parsedExpression:(iTermParsedExpression *)expression {
  ...
  [self evaluateParametersWithScope:scope
                        ...
                        synchronous:synchronous
                         completion:^(
                             ...
                             NSError *depError, NSSet<NSString *> *missing) {
                           __strong __typeof(self) strongSelf = weakSelf;
                           if (!strongSelf) {
                             return;
                           }
                           [strongSelf
                               didEvaluateParametersWithScope:scope
                                                  synchronous:synchronous
...
- (void)didEvaluateParametersWithScope:(iTermVariableScope *)scope
  ...
  if (receiver) {
    iTermCallMethodByIdentifier(receiver, self.fullyQualifiedName,
                                parameterValues, ^(id object, NSError *error) {
  ...
  if (synchronous) {
    // This is useful because it causes the scope to be called for all
    // depended-upon variables even if the function can't be executed. This
    // makes it possible for the session name controller to build up its set of
    // dependencies.
        ...
        errorWithDomain:iTermAPIHelperErrorDomain
               ...
               userInfo:@{
                 NSLocalizedDescriptionKey : [NSString
                     stringWithFormat:@"Undefined function with signature “%@”",
                                      signature]
               }];
    completion(nil, error, nil);
    return;
  ...
  self->_connectionKey =
      [[apiHelper connectionKeyForRPCWithName:self.fullyQualifiedName
...
- (void)evaluateParametersWithScope:(iTermVariableScope *)scope
  ...
  __weak __typeof(self) weakSelf = self;
  [_argToExpression enumerateKeysAndObjectsUsingBlock:^(
                        NSString *_Nonnull key,
                        iTermParsedExpression *_Nonnull parsedExpression,
                        BOOL *_Nonnull stop) {
    ...
    [self evaluateArgumentWithParsedExpression:parsedExpression
                                   ...
                                   synchronous:synchronous
                                    completion:^(
                                        NSError *error, id value,
                                        NSSet<NSString *> *innerMissing) {
...
- (void)evaluateArgumentWithParsedExpression:
            (iTermParsedExpression *)parsedExpression
                                  invocation:(NSString *)invocation
                                       scope:(iTermVariableScope *)scope
                                 synchronous:(BOOL)synchronous
                                  completion:
                                      (void (^)(NSError *error, id value,
  ...
  [parameterEvaluator
      evaluateWithTimeout:synchronous ? 0 : INFINITY
               completion:^(iTermExpressionEvaluator *evaluator) {
                 ...
                 NSError *error = nil;
                 if (parsedExpression.expressionType ==
                     iTermParsedExpressionTypeFunctionCall) {
                   error =
                       [self errorForDependentCall:parsedExpression.functionCall
...
- (NSError *)errorForDependentCall:(iTermScriptFunctionCall *)call
  ...
  NSString *reason = [NSString stringWithFormat:@"In call to %@: %@", call.name,
  ...
  if (traceback) {
    userInfo =
        [userInfo dictionaryBySettingObject:traceback
                                     forKey:NSLocalizedFailureReasonErrorKey];
  }

  return [NSError errorWithDomain:@"com.iterm2.call" code:1 userInfo:userInfo];
}
