...
@property(nonatomic, retain)
    NSMutableArray<iTermWeakReference<id<iTermHotKeyNavigableSession>> *>
        *weakSessions;
...
- (NSUInteger)indexOfSession:(id<iTermHotKeyNavigableSession>)session {
  ...
  [_weakSessions
      enumerateObjectsUsingBlock:^(
          iTermWeakReference<id<iTermHotKeyNavigableSession>> *_Nonnull obj,
...
- (NSArray<iTermHotKey *> *)hotkeyPressed:(NSDictionary *)userData
  ...
  NSUInteger index = [self.weakSessions
      indexOfObjectPassingTest:^BOOL(
          ...
          NSUInteger idx, BOOL *_Nonnull stop) {
        return
            [obj.weaklyReferencedObject sessionHotkeyIsAlreadyFirstResponder];
      }];

  id<iTermHotKeyNavigableSession> session =
      self.weakSessions.firstObject.weaklyReferencedObject;
  if (index != NSNotFound) {
    ...
    index = [self.weakSessions
        indexOfObjectPassingTest:^BOOL(
            iTermWeakReference<id<iTermHotKeyNavigableSession>> *_Nonnull obj,
            NSUInteger idx, BOOL *_Nonnull stop) {
          return [obj.weaklyReferencedObject
                      sessionHotkeyIsAlreadyActiveInNonkeyWindow];
        }];
    if (index != NSNotFound) {
...
- (iTermSessionHotkeyRegistration *)registrationForSession:
                                        (id<iTermHotKeyNavigableSession>)session
...
- (iTermSessionHotkeyRegistration *)registrationForShortcut:
    (iTermShortcut *)shortcut {
  NSUInteger index =
      [_registrations indexOfObjectPassingTest:^BOOL(
                          iTermSessionHotkeyRegistration *_Nonnull obj,
                          NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)setShortcut:(iTermShortcut *)shortcut
  ...
  if (shortcut) {
    iTermSessionHotkeyRegistration *registration =
        [self registrationForShortcut:shortcut];
    if (!registration) {
      registration = [[[iTermSessionHotkeyRegistration alloc]
          initWithShortcut:shortcut] autorelease];
...
- (void)removeSession:(id<iTermHotKeyNavigableSession>)session {
  NSUInteger index;
  iTermSessionHotkeyRegistration *registration =
      [self registrationForSession:session index:&index];
  if (registration) {
