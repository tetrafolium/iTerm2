...
NSString *const iTermStatusBarElementPasteboardType =
    @"com.iterm2.status-bar-element";
...
- (instancetype)
    initWithComponentFactory:(id<iTermStatusBarComponentFactory>)factory
             layoutAlgorithm:
                 (iTermStatusBarLayoutAlgorithmSetting)layoutAlgorithm
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p %@>",
                                    ...
                                    NSStringFromClass(_component.class)];
}

- (NSAttributedString *)exemplarWithBackgroundColor:
                            (NSColor *)defaultBackgroundColor
                                          textColor:(NSColor *)defaultTextColor
                                        defaultFont:(NSFont *)defaultFont {
  NSColor *backgroundColor =
      self.component.statusBarBackgroundColor ?: defaultBackgroundColor;
  NSColor *textColor = self.component.statusBarTextColor;
  if (textColor == [NSColor labelColor] || textColor == nil) {
    textColor = defaultTextColor;
  }
  id object = [self.component
      statusBarComponentExemplarWithBackgroundColor:backgroundColor
                                          textColor:textColor];
...
- (id)copyWithZone:(nullable NSZone *)zone {
  ...
  iTermStatusBarAdvancedConfiguration *advancedConfiguration =
      [iTermStatusBarAdvancedConfiguration
          advancedConfigurationFromDictionary:dict];
  return [[iTermStatusBarSetupElement alloc]
      initWithComponent:[_component.statusBarComponentFactory
                            newComponentWithKnobs:knobs
                                  layoutAlgorithm:advancedConfiguration
                                                      .layoutAlgorithm
                                            scope:nil]];
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
  id<iTermStatusBarComponentFactory> factory =
      [aDecoder decodeObjectForKey:@"componentFactory"];
  if (!factory) {
    return nil;
  }
  NSDictionary *knobs = [aDecoder decodeObjectOfClass:[NSDictionary class]
                                               forKey:@"knobs"];
  return [self
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:_component.statusBarComponentFactory
                forKey:@"componentFactory"];
  NSDictionary *knobs =
      _component
              .configuration[iTermStatusBarComponentConfigurationKeyKnobValues]
          ?: @{};
  [aCoder encodeObject:knobs forKey:@"knobs"];
  NSDictionary *dict =
      _component.configuration
          [iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue];
  iTermStatusBarAdvancedConfiguration *advancedConfiguration =
      [iTermStatusBarAdvancedConfiguration
          advancedConfigurationFromDictionary:dict];
  [aCoder encodeInteger:advancedConfiguration.layoutAlgorithm
                 forKey:@"layoutAlgorithm"];
}
...
- (nullable instancetype)initWithPasteboardPropertyList:(id)propertyList
                                                 ofType:(NSString *)type {
  // I haven't tested this because I think it's unreachable. If it breaks then
  // it's because whatever this decodes doesn't conform to secure coding.
  NSKeyedUnarchiver *unarchiver =
      [[NSKeyedUnarchiver alloc] initForReadingFromData:propertyList error:nil];
