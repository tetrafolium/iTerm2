...
// Key in window arrangement that gives the GUID for the profile that created
// the window. Used for restoring legacy hotkey window state.
...
@property(nonatomic, readonly)
    NSArray<PseudoTerminal *> *visibleWindowControllers;
...
// Reveal the hotkey window, creating it if needed. If url is non-nil and the
// window doesn't exist yet its initial session will be opened to this URL.
// Returns YES if a new window was created, or NO if an existing window was
// used.
- (BOOL)showWindowForProfileHotKey:(iTermProfileHotKey *)profileHotKey
                               url:(NSURL *)url;

// Indicates if the event is a hotkey event. Assumes the event is a keydown
// event.
...
- (NSInteger)createHiddenWindowsFromRestorableStates:
    (NSArray *)states; // legacy
- (BOOL)createHiddenWindowsByDecoding:
    (iTermEncoderGraphRecord *)record; // sqlite
...
// Auto hide a single hotkey window, if needed and possible. Called when
// `windowController` resigns key.
- (void)autoHideHotKeyWindows:
...
// Auto hide all hotkey windows but for `exception`. Called when a window
// belonging to `exceptions` becomes key.
...
// Use when undoing the close of a window to indicate that it is a hotkey
// window. Returns NO if there is already a hotkey window for this role, in
// which case it should be made visible and resume life as a regular window.
- (BOOL)addRevivedHotkeyWindowController:(PseudoTerminal *)windowController
                      forProfileWithGUID:(NSString *)guid;

// Use this when creating a window that might be a hotkey window. Generally
// iTermProfileHotkey creates its own windows, but that's not the case for e.g.
// tmux windows. Returns the iTermProfileHotKey if the window controller was
// assigned a shotkey or nil if not.
