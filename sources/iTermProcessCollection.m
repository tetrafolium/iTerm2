...
- (NSString *)description {
  return [NSString
      stringWithFormat:
          @"<%@: %p pid=%@ name=%@ children.count=%@ haveDeepest=%@ isFg=%@>",
          self.class, self, @(self.processID), _name, @(_childProcessIDs.count),
          @(_haveDeepestForegroundJob), _isForegroundJob];
}
...
- (BOOL)isEqual:(id)object {
  ...
  return self.processID == other.processID &&
         ...
         self.parentProcessID == self.parentProcessID;
}
...
- (NSString *)treeStringWithIndent:(NSString *)indent {
  ...
  NSString *children = [[childArray mapWithBlock:^id(id anObject) {
    return
        [anObject treeStringWithIndent:[indent stringByAppendingString:@"  "]];
  }] componentsJoinedByString:@"\n"];
  _computingTreeString = NO;
  if (childArray.count > 0) {
    children = [@"\n" stringByAppendingString:children];
  }
  return [NSString stringWithFormat:@"%@pid=%@ name=%@ fg=%@%@", indent,
                                    @(self.processID), self.name,
                                    @(self.isForegroundJob), children];
}
...
- (iTermProcessInfo *)deepestForegroundJob {
  if (!_haveDeepestForegroundJob) {
    ...
    BOOL cycle = NO;
    return [self deepestForegroundJob:&level
                              visited:visitedPids
                                cycle:&cycle
                                depth:0];
  }
...
- (iTermProcessInfo *)deepestForegroundJob:(NSInteger *)levelInOut
  ...
  for (iTermProcessInfo *child in self.children) {
    ...
    iTermProcessInfo *candidate = [child deepestForegroundJob:&level
                                                      visited:visited
                                                        cycle:cycle
                                                        depth:depth + 1];
    if (*cycle) {
...
- (void)resolveAsynchronously {
  ...
  dispatch_once(&onceToken, ^{
    queue =
        dispatch_queue_create("com.iterm2.pid-lookup", DISPATCH_QUEUE_SERIAL);
  });
...
- (void)commit {
  [_processes enumerateKeysAndObjectsUsingBlock:^(
                  NSNumber *_Nonnull processID, iTermProcessInfo *_Nonnull info,
                  BOOL *_Nonnull stop) {
