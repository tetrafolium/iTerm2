...
+ (NSDictionary *)metadataInContainer:(NSString *)container
                                 name:(NSString *)name {
  NSString *path = [[container stringByAppendingPathComponent:name]
      stringByAppendingPathComponent:@"metadata.json"];
  ...
  return [NSDictionary
      castFrom:[NSJSONSerialization it_objectForJsonString:stringValue]];
}

+ (NSArray<NSString *> *)absolutePathsOfNonDotFilesIn:(NSString *)container {
  NSFileManager *fileManager = [NSFileManager defaultManager];
  NSArray<NSString *> *topLevelItems =
      [fileManager it_itemsInDirectory:container];
  topLevelItems =
      [topLevelItems filteredArrayUsingBlock:^BOOL(NSString *anObject) {
...
+ (instancetype)archiveFromContainer:(NSString *)container
  ...
  NSFileManager *fileManager = [NSFileManager defaultManager];
  NSArray<NSString *> *topLevelItems =
      [self absolutePathsOfNonDotFilesIn:container];
  if (topLevelItems.count != 1) {
    return nil;
  }

  NSString *topLevelItem = topLevelItems.firstObject;
  const BOOL topLevelItemIsDirectory =
      [fileManager itemIsDirectory:topLevelItem];
  if ([[topLevelItem pathExtension] isEqualToString:@"py"] &&
      !topLevelItemIsDirectory &&
      [topLevelItem
          isEqualToString:[container stringByAppendingPathComponent:
                                         topLevelItem.lastPathComponent]]) {
    // Basic script
    return [iTermScriptArchive
        archiveForScriptIn:container
                     named:topLevelItems[0].lastPathComponent
           fullEnvironment:NO];
  }
  ...
  NSArray<NSString *> *innerItems =
      [self absolutePathsOfNonDotFilesIn:topLevelItem];
  if (innerItems.count < 2) {
    return nil;
  }
  // Maps a boolean number to an item name. True key = setup.cfg, false key =
  // not setup.py
  NSString *setupCfg =
      [topLevelItem stringByAppendingPathComponent:iTermScriptSetupCfgName];
  NSString *deprecatedSetupPy = [topLevelItem
      stringByAppendingPathComponent:iTermScriptDeprecatedSetupPyName];
  NSString *metadata =
      [topLevelItem stringByAppendingPathComponent:iTermScriptMetadataName];
  ...
  NSString *mainPy = [[topLevelItem stringByAppendingPathComponent:name]
  ...
  return [iTermScriptArchive archiveForScriptIn:container
                                          named:name
                                fullEnvironment:YES];
}
...
- (BOOL)userAcceptsTrustedScriptAutoLaunchInstall {
  NSString *body = [NSString
      stringWithFormat:@"“%@” would like to launch automatically when iTerm2 "
                       @"starts. Would you like to allow that?",
                       self.name];
...
- (BOOL)userAcceptsExplicitAutoLaunchInstall {
  NSString *body =
      [NSString stringWithFormat:@"“%@” can launch automatically when iTerm2 "
                                 @"starts. Would you like to allow that?",
                                 self.name];
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:body
...
- (void)installBasicTrusted:(BOOL)trusted
  ...
  NSString *to;
  if ([self shouldAutoLaunchWhenTrusted:trusted
                        offerAutoLaunch:offerAutoLaunch]) {
    to = [[[NSFileManager defaultManager] autolaunchScriptPath]
        stringByAppendingPathComponent:self.name];
  } else {
    to = [[[NSFileManager defaultManager] scriptsPath]
        stringByAppendingPathComponent:self.name];
  }
  NSError *error = nil;
...
- (BOOL)shouldAutoLaunchWhenTrusted:(BOOL)trusted
                    offerAutoLaunch:(BOOL)offerAutoLaunch {
  if (![self shouldOfferAutoLaunchWhenTrusted:trusted
                              offerAutoLaunch:offerAutoLaunch]) {
    return NO;
  }
  if (offerAutoLaunch) {
...
- (void)installFullEnvironmentTrusted:(BOOL)trusted
  ...
  NSString *setupCfg =
      [from stringByAppendingPathComponent:iTermScriptSetupCfgName];
  iTermSetupCfgParser *setupParser =
      [[iTermSetupCfgParser alloc] initWithPath:setupCfg];
  if (!setupParser) {
    NSDictionary *userInfo =
        @{NSLocalizedDescriptionKey : @"Cannot find setup.cfg"};
  ...
  // Create a symlink from the final location to the temporary location so
  // shebangs will work. First try to remove a dangling link.
  if ([[[[NSFileManager defaultManager] attributesOfItemAtPath:to error:nil]
          fileType] isEqualToString:NSFileTypeSymbolicLink]) {
    NSString *expanded =
        [[NSFileManager defaultManager] destinationOfSymbolicLinkAtPath:to
                                                                  error:nil];
    if (expanded &&
  ...
  if (error) {
    NSDictionary *userInfo = @{
      NSLocalizedDescriptionKey :
          [NSString stringWithFormat:@"Could not write to %@", to]
    };
    NSError *error = [NSError errorWithDomain:@"com.iterm2.scriptarchive"
                                         code:1
                                     userInfo:userInfo];
  ...
  [[iTermPythonRuntimeDownloader sharedInstance] downloadOptionalComponentsIfNeededWithConfirmation:
                                                     YES
                                                                                      pythonVersion:
                                                                                          setupParser
                                                                                              .pythonVersion
                                                                                     ...
                                                                                     withCompletion:
                                                                                         ^(iTermPythonRuntimeDownloaderStatus
                                                                                           ...
                                                                                           case iTermPythonRuntimeDownloaderStatusError: {
                                                                                             ...
                                                                                             NSString *reason =
                                                                                                 [self
                                                                                                     errorReasonForRuntimeDownloaderStatus:
                                                                                                         status];
                                                                                             NSString *description =
                                                                                                 [NSString
                                                                                                     stringWithFormat:
                                                                                                         @"Python Runtime not downloaded: %@",
                                                                                                         reason];
                                                                                             NSDictionary
                                                                                                 *userInfo = @{
                                                                                                   NSLocalizedDescriptionKey :
                                                                                           ...
                                                                                           NSURL *toURL =
                                                                                               [NSURL
                                                                                                   fileURLWithPath:
                                                                                                       to];
                                                                                           [[iTermPythonRuntimeDownloader
                                                                                               sharedInstance] installPythonEnvironmentTo:
                                                                                                                   [NSURL
                                                                                                                       fileURLWithPath:
                                                                                                                           from]
                                                                                                                         eventualLocation:
                                                                                                                       ...
                                                                                                                       environmentVersion:
                                                                                                                           ...
                                                                                                                           createSetupCfg:
                                                                                                                               NO
                                                                                                                               completion:
                                                                                                                                   ^(iTermInstallPythonStatus
                                                                                                                                         status) {
                                                                                                                                     [self
                                                                                                                                         didInstallPythonRuntime:
                                                                                                                                             status
                                                                                                                                                            from:
                                                                                                                                                                from
                                                                                                                                                              to:to
...
- (NSString *)errorReasonForRuntimeDownloaderStatus:
...
- (void)didInstallPythonRuntime:(iTermInstallPythonStatus)status
  ...
  case iTermInstallPythonStatusGeneralFailure: {
    NSDictionary *userInfo =
        @{NSLocalizedDescriptionKey : @"Failed to install Python Runtime"};
    NSError *error = [NSError errorWithDomain:@"com.iterm2.scriptarchive"
                                         code:1
                                     userInfo:userInfo];
    completion(error);
  ...
  case iTermInstallPythonStatusDependencyFailed: {
    NSDictionary *userInfo =
        @{NSLocalizedDescriptionKey : @"Failed to install Python package"};
    NSError *error = [NSError errorWithDomain:@"com.iterm2.scriptarchive"
                                         code:2
                                     userInfo:userInfo];
    completion(error);
