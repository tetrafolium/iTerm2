...
- (instancetype)initWithInvocationSuggester {
  self = [super init];
  if (self) {
    _suggester = [[iTermFunctionCallTextFieldDelegate alloc]
        initWithPathSource:
...
- (void)sizeToFit {
  const CGFloat marginBetweenLabelAndField =
      NSMinX(_textField.frame) - NSMaxX(_label.frame);
  ...
  if (_helpButton) {
    [_helpButton sizeToFit];
    _helpButton.frame = NSMakeRect(maxX + 4, -4, _helpButton.frame.size.width,
...
- (BOOL)control:(NSControl *)control
  ...
  if ([self respondsToSelector:commandSelector]) {
    [self it_performNonObjectReturningSelector:commandSelector
                                    withObject:control];
    return YES;
  } else {
    return NO;
