...
+ (NSTextView *)newTextViewWithFrame:(NSRect)frame
                          scrollview:(out NSScrollView **)scrollViewPtr {
...
- (CGFloat)heightWhenOpen {
  ...
  NSAttributedString *attributedString =
      [[NSAttributedString alloc] initWithString:self.textView.string
                                      attributes:attributes];
  const CGFloat height =
      [attributedString heightForWidth:self.textView.frame.size.width];
...
- (instancetype)initWithFrame:(NSRect)frameRect
  ...
  if (self) {
    _disclosureButton =
        [[NSButton alloc] initWithFrame:NSMakeRect(0, 2, 24, 24)];
    ...
    _labelField = [[NSTextField alloc]
        initWithFrame:NSMakeRect(NSMaxX(_disclosureButton.frame), 0,
                                 frameRect.size.width -
                                     NSMaxX(_disclosureButton.frame),
                                 18)];
    ...
    _textView =
        [self.class newTextViewWithFrame:NSMakeRect(
                                             8,
                                             NSMaxY(_disclosureButton.frame) +
                                                 3,
                                             100, 100)
    ...
    CGFloat height = [_textView.attributedString
        heightForWidth:iTermDisclosableViewTextViewWidth];
...
- (NSSize)intrinsicContentSize {
  NSSize size =
      NSMakeSize(_disclosureButton.state == NSControlStateValueOn
                     ? MAX(_headerWidth, iTermDisclosableViewTextViewWidth)
                     : _headerWidth,
                 _disclosureButton.state == NSControlStateValueOn
                     ? [self heightWhenOpen]
                     : [self heightWhenClosed]);
...
- (void)disclosureButtonPressed:(id)sender {
  ...
  self.frame = NSMakeRect(NSMinX(myFrame), NSMinY(myFrame),
