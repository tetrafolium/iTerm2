...
@interface iTermController () <iTermSetCurrentTerminalHelperDelegate,
                               iTermPresentationControllerDelegate>
@end
...
+ (NSString *)installationId {
  ...
  NSString *installationId =
      [[NSUserDefaults standardUserDefaults] stringForKey:kInstallationIdKey];
  if (!installationId) {
    installationId = [NSString uuid];
    [[NSUserDefaults standardUserDefaults] setObject:installationId
                                              forKey:kInstallationIdKey];
  }
...
- (instancetype)init {
  ...
  if (self) {
    ...
    // create the "~/Library/Application Support/iTerm2" directory if it does
    // not exist
...
- (BOOL)shouldLeaveSessionsRunningOnQuit {
  ...
  const BOOL sessionsWillRestore =
...
- (void)dealloc {
  ...
  if (self.shouldLeaveSessionsRunningOnQuit) {
    ...
    //
    // 1. Sparkle is restarting the app. Because jobs are run in servers and
    // window
    //    restoration is on, we don't want to close term windows because that
    //    will send SIGHUP to the job processes. Normally this path is taken
    //    during a user-initiated quit, so we want the jobs killed, but not in
    //    this case.
    // 2. The user has set a pref to not kill jobs on quit.
    //
    // In either case, we only get here if we're pretty sure everything will get
    // restored nicely.
    ...
    ITAssertWithMessage([_terminalWindows count] == 0,
                        @"Expected terminals to be gone");
...
- (void)updateProcessType {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermProcessTypeDidChangeNotification
                    object:nil];
}
...
- (void)newWindow:(id)sender possiblyTmux:(BOOL)possiblyTmux {
  ...
  if (possiblyTmux && _frontTerminalWindowController &&
...
- (void)newSessionInTabAtIndex:(id)sender {
  Profile *bookmark = [[ProfileModel sharedInstance]
      bookmarkWithGuid:[sender representedObject]];
  if (bookmark) {
...
- (BOOL)terminalIsObscured:(id<iTermWindowController>)terminal
  ...
  // The occlusionState tells if you if you're on another space or another app's
  // window is occluding yours, but for some reason one terminal window can
  // occlude another without it noticing, so we compute that ourselves.
  windowIsObscured = !(occlusionState & NSWindowOcclusionStateVisible);
  if (!windowIsObscured) {
    // Try to refine the guess by seeing if another terminal is covering this
    // one.
    if ([(iTermTerminalWindow *)terminal.window approximateFractionOccluded] >
        threshold) {
...
- (void)newSessionWithSameProfile:(id)sender {
  ...
  BOOL divorced = ([[ProfileModel sessionsInstance]
                       bookmarkWithGuid:bookmark[KEY_GUID]] != nil);
  if (divorced) {
...
- (void)newSession:(id)sender possiblyTmux:(BOOL)possiblyTmux {
  DLog(@"newSession:%@ possiblyTmux:%d from %@", sender, (int)possiblyTmux,
...
- (void)repairSavedArrangementNamed:(NSString *)savedArrangementName
  ...
  NSArray *terminalArrangements =
      [WindowArrangements arrangementWithName:savedArrangementName];
  Profile *goodProfile =
      [[ProfileModel sharedInstance] bookmarkWithGuid:replacementGuid];
  if (goodProfile) {
    NSMutableArray *repairedArrangements = [NSMutableArray array];
    for (NSDictionary *terminalArrangement in terminalArrangements) {
      [repairedArrangements
          addObject:[PseudoTerminal repairedArrangement:terminalArrangement
                               replacingProfileWithGUID:guidToReplace
                                            withProfile:goodProfile]];
    }
    [WindowArrangements setArrangement:repairedArrangements
                              withName:savedArrangementName];
  }
}
...
- (BOOL)arrangementWithName:(NSString *)arrangementName
         hasSessionWithGUID:(NSString *)guid
                        pwd:(NSString *)pwd {
  NSArray *windowArrangements =
      [WindowArrangements arrangementWithName:arrangementName];
  if (!windowArrangements) {
    return NO;
  }
  return [windowArrangements anyWithBlock:^BOOL(id anObject) {
    NSDictionary *dict =
        [PseudoTerminal arrangementForSessionWithGUID:guid
                                  inWindowArrangement:anObject];
    if (!dict) {
      return NO;
    }
    NSString *actualPWD =
        [PTYSession initialWorkingDirectoryFromArrangement:dict];
...
- (void)repairSavedArrangementNamed:(NSString *)arrangementName
  ...
  NSArray *terminalArrangements =
      [WindowArrangements arrangementWithName:arrangementName];
  NSArray *repairedArrangements = [terminalArrangements
      mapWithBlock:^id(NSDictionary *terminalArrangement) {
...
- (void)saveWindowArrangementForAllWindows:(BOOL)allWindows
                                      name:(NSString *)name {
  if (allWindows) {
    NSMutableArray *terminalArrangements =
        [NSMutableArray arrayWithCapacity:[_terminalWindows count]];
    for (PseudoTerminal *terminal in _terminalWindows) {
...
- (void)saveWindowArrangementForWindow:(PseudoTerminal *)currentTerminal
                                  name:(NSString *)name {
  NSMutableArray *terminalArrangements =
      [NSMutableArray arrayWithCapacity:[_terminalWindows count]];
...
- (void)tryOpenArrangement:(NSDictionary *)terminalArrangement
  ...
  if (shouldDelay) {
    ...
    DLog(@"Opening it.");
    PseudoTerminal *term =
        [PseudoTerminal terminalWithArrangement:terminalArrangement
                                          named:arrangementName
...
- (BOOL)loadWindowArrangementWithName:(NSString *)theName
                     asTabsInTerminal:(PseudoTerminal *)term {
  BOOL ok = NO;
  NSArray *terminalArrangements =
      [WindowArrangements arrangementWithName:theName];
...
- (NSArray *)terminalsInScreen:(NSScreen *)screen {
  ...
  for (PseudoTerminal *term in _terminalWindows) {
    if (![term isHotKeyWindow] && [[term window] deepestScreen] == screen) {
...
- (void)arrangeTerminals:(NSArray *)terminals inFrame:(NSRect)frame {
  ...
  // Find the window whose top is nearest the top of the screen. That will be
  // the new top of all the windows in the first row.
  int highestTop = 0;
  ...
  // Ensure the bottom of the last row of windows will be above the bottom of
  // the screen.
  ...
  if (highestTop > frame.origin.y + frame.size.height) {
    // Don't let the top of the first row go above the top of the screen. This
    // is just paranoia.
  ...
  while ([terminalsCopy count] > 0) {
    ...
    if (rows > 1) {
      // The first row can be a bit ragged vertically but subsequent rows line
      // up at the tops of the windows.
      y = frame.origin.y + frame.size.height - h;
    }
    NSDictionary *dict = @{
      NSViewAnimationTargetKey : [terminal window],
      NSViewAnimationStartFrameKey :
          [NSValue valueWithRect:[[terminal window] frame]],
      NSViewAnimationEndFrameKey :
          [NSValue valueWithRect:NSMakeRect(x, y + yOffset, w, h)]
    };
    x += w;
    ...
    NSViewAnimation *theAnim = [[[NSViewAnimation alloc]
        initWithViewAnimations:@[ dict ]] autorelease];
  ...
  for (PseudoTerminal *windowController in _terminalWindows) {
    if ([windowController anyFullScreen]) {
      if (windowController.window.isFullScreen) {
        ...
        [[iTermFullScreenWindowManager sharedInstance]
            makeWindowExitFullScreen:windowController.ptyWindow];
...
- (void)_addBookmark:(Profile *)bookmark
  ...
  NSMenuItem *aMenuItem =
      [[NSMenuItem alloc] initWithTitle:[bookmark objectForKey:KEY_NAME]
                                 action:selector
                          keyEquivalent:@""];
  if (withShortcuts) {
  ...
  if (alternateSelector) {
    ...
    [aMenuItem
        setKeyEquivalentModifierMask:modifierMask | NSEventModifierFlagOption];
...
- (void)_addBookmarksForTag:(NSString *)tag
  ...
  NSMenuItem *aMenuItem = [[NSMenuItem alloc] initWithTitle:tag
  ...
  for (int i = 0; i < [[ProfileModel sharedInstance] numberOfBookmarks]; ++i) {
    ...
    for (int j = 0; j < [tags count]; ++j) {
      if ([tag localizedCaseInsensitiveCompare:[tags objectAtIndex:j]] ==
          NSOrderedSame) {
        ++count;
  ...
  if ([[ProfileModel sharedInstance] numberOfBookmarks] > MAX_MENU_ITEMS) {
    int overflow =
        [[ProfileModel sharedInstance] numberOfBookmarks] - MAX_MENU_ITEMS;
    NSMenuItem *overflowItem = [[NSMenuItem alloc]
        initWithTitle:[NSString
                          stringWithFormat:@"[%d profiles not shown]", overflow]
               action:nil
        keyEquivalent:@""];
...
- (BOOL)shouldOpenManyProfiles:(int)count {
  NSString *theTitle =
      [NSString stringWithFormat:@"You are about to open %d profiles.", count];
...
- (void)openSessionsFromProfiles:(NSArray<Profile *> *)profiles
              inWindowController:(PseudoTerminal *)term {
  if (profiles.count == 0) {
    return;
  }
  Profile *head = profiles.firstObject;
  NSArray<Profile *> *tail = [profiles subarrayFromIndex:1];
  [iTermSessionLauncher launchBookmark:head
                            inTerminal:term
                    respectTabbingMode:NO
                            completion:^(PTYSession *_Nonnull session) {
                              ...
                              [self openSessionsFromProfiles:tail
                                          inWindowController:nextTerm];
                            }];
}
...
// Recursively descends the menu, finding all bookmarks that aren't already in
// usedGuids (which should be empty when called from outside).
- (void)getBookmarksInMenu:(NSMenu *)parent
  ...
  for (NSMenuItem *item in [parent itemArray]) {
    if (![item isSeparatorItem] && ![item submenu] && ![item isAlternate]) {
      ...
      [self getBookmarksInMenu:sub usedGuids:usedGuids bookmarks:bookmarks];
    }
  }
}
...
- (iTermWindowType)windowTypeForBookmark:(Profile *)aDict {
  if ([aDict objectForKey:KEY_WINDOW_TYPE]) {
    const iTermWindowType windowType =
        iTermThemedWindowType([[aDict objectForKey:KEY_WINDOW_TYPE] intValue]);
...
- (PseudoTerminal *)openTmuxIntegrationWindowUsingProfile:(Profile *)profile {
  ...
  if ([iTermAdvancedSettingsModel serializeOpeningMultipleFullScreenWindows]) {
    ...
    windowType =
        iTermThemedWindowType([iTermProfilePreferences intForKey:KEY_WINDOW_TYPE
                                                       inProfile:profile]);
  }
  PseudoTerminal *term = [[[PseudoTerminal alloc]
      initWithSmartLayout:YES
               windowType:windowType
          savedWindowType:windowType
                   screen:[iTermProfilePreferences intForKey:KEY_SCREEN
                                                   inProfile:profile]
         hotkeyWindowType:iTermHotkeyWindowTypeNone
                  profile:profile] autorelease];
  if ([iTermProfilePreferences boolForKey:KEY_HIDE_AFTER_OPENING
                                inProfile:profile]) {
  ...
  if (term.windowType != WINDOW_TYPE_LION_FULL_SCREEN) {
    iTermProfileHotKey *profileHotKey = [[iTermHotKeyController sharedInstance]
        didCreateWindowController:term
                      withProfile:profile];
...
- (void)didFinishCreatingTmuxWindow:(PseudoTerminal *)windowController {
  [[[iTermHotKeyController sharedInstance]
      profileHotKeyForWindowController:windowController] showHotKeyWindow];
}

- (void)makeTerminalWindowFullScreen:
    (NSWindowController<iTermWindowController> *)term {
  [[iTermFullScreenWindowManager sharedInstance]
      makeWindowEnterFullScreen:term.ptyWindow];
}

- (PseudoTerminal *)
    windowControllerForNewTabWithProfile:(Profile *)profile
                               candidate:
                                   (PseudoTerminal *)preferredWindowController
  ...
  if (!respectTabbingMode ||
      [iTermAdvancedSettingsModel
          disregardDockSettingToOpenTabsInsteadOfWindows]) {
...
- (void)maybeWarnAboutOpeningInTab {
  ...
  const BOOL haveUsedOlderVersion =
      [[iTermPreferences allAppVersionsUsedOnThisMachine].allObjects
          anyWithBlock:^BOOL(NSString *version) {
            return [comparator compareVersion:firstVersionRespectingSetting
  ...
  [[iTermNotificationController sharedInstance]
      postNotificationWithTitle:@"Creating a tab"
                         detail:@"The system preference to open a tab instead "
                                @"of a window is now respected in iTerm2."
                            URL:[NSURL
                                    URLWithString:
...
- (int)allocateWindowNumber {
  NSMutableSet *numbers =
      [NSMutableSet setWithCapacity:[self numberOfTerminals]];
  for (PseudoTerminal *term in [self terminals]) {
...
- (PTYTab *)tabWithID:(NSString *)tabID {
  ...
  if ([tabID rangeOfCharacterFromSet:nonNumericCharacterSet].location !=
      NSNotFound) {
    return nil;
  }
...
- (void)dumpViewHierarchy {
  for (PseudoTerminal *term in [self terminals]) {
    DebugLog([NSString
        stringWithFormat:@"Terminal %@ at %@", [term window],
                         [NSValue valueWithRect:[[term window] frame]]]);
...
- (void)refreshSoftwareUpdateUserDefaults {
  BOOL checkForTestReleases =
      [iTermPreferences boolForKey:kPreferenceKeyCheckForTestReleases];
  NSString *appCast =
      checkForTestReleases
          ? [[NSBundle mainBundle]
                objectForInfoDictionaryKey:@"SUFeedURLForTesting"]
          : [[NSBundle mainBundle]
                objectForInfoDictionaryKey:@"SUFeedURLForFinal"];
  NSURL *url = [NSURL URLWithString:appCast];
  NSNumber *shard = @([iTermController shard]);
  url =
      [url URLByAppendingQueryParameter:[NSString stringWithFormat:@"shard=%@",
                                                                   shard]];
...
- (BOOL)selectionRespectsSoftBoundaries {
  return [[NSUserDefaults standardUserDefaults]
      boolForKey:kSelectionRespectsSoftBoundariesKey];
}

- (void)setSelectionRespectsSoftBoundaries:
    (BOOL)selectionRespectsSoftBoundaries {
  [[NSUserDefaults standardUserDefaults]
      setBool:selectionRespectsSoftBoundaries
...
- (void)pushCurrentRestorableSession:(iTermRestorableSession *)session {
  [_currentRestorableSessionsStack insertObject:session atIndex:0];
}
...
- (iTermRestorableSession *)currentRestorableSession {
  return _currentRestorableSessionsStack.count
             ? _currentRestorableSessionsStack[0]
             : nil;
}

- (void)removeSessionFromRestorableSessions:(PTYSession *)session {
  NSInteger index = [_restorableSessions
...
- (iTermRestorableSession *)popRestorableSession {
  ...
  iTermRestorableSession *restorableSession =
      [[[_restorableSessions lastObject] retain] autorelease];
...
// This exists because I don't trust -[NSApp keyWindow]. I've seen all kinds of
// weird behavior from it.
- (BOOL)anyVisibleWindowIsKey {
  DLog(@"Searching for key window...");
  for (NSWindow *window in
       [(iTermApplication *)NSApp orderedWindowsPlusVisibleHotkeyPanels]) {
...
- (void)workspaceWillPowerOff:(NSNotification *)notification {
  if ([iTermAdvancedSettingsModel killSessionsOnLogout] &&
      [iTermAdvancedSettingsModel runJobsInServers]) {
    _willPowerOff = YES;
...
- (NSInteger)numberOfDecodesPending {
  return
      [[self.terminals filteredArrayUsingBlock:^BOOL(PseudoTerminal *anObject) {
...
- (NSString *)shCommandLineWithCommand:(NSString *)command
  ...
  NSArray<NSString *> *const escapedArguments =
      [arguments mapWithBlock:^id(NSString *anObject) {
        if (escapeArguments) {
          return [anObject
  ...
  NSString *const escapedCommand =
      escapeArguments
          ? [command
                stringWithBackslashEscapedShellCharactersIncludingNewlines:YES]
          : command;
  NSArray<NSString *> *const combinedArray =
      [@[ escapedCommand ] arrayByAddingObjectsFromArray:escapedArguments];
  NSString *const commandLine = [combinedArray componentsJoinedByString:@" "];
  return [NSString stringWithFormat:@"sh -c \"%@\"", commandLine];
}

// This is meant for standalone command lines when used with DoNotEscape, like:
// man date || sleep 3
- (void)openSingleUseWindowWithCommand:(NSString *)rawCommand
                        ...
                        didMakeSession:
                            (void (^)(PTYSession *session))didMakeSession
...
- (void)openSingleUseWindowWithCommand:(NSString *)rawCommand
  ...
  MutableProfile *windowProfile =
      [[[self defaultBookmark] mutableCopy] autorelease];
  if ([windowProfile[KEY_WINDOW_TYPE] integerValue] ==
          WINDOW_TYPE_TRADITIONAL_FULL_SCREEN ||
      [windowProfile[KEY_WINDOW_TYPE] integerValue] ==
  ...
  NSString *command =
      [self shCommandLineWithCommand:rawCommand
                           arguments:arguments ?: @[]
                     escapeArguments:
                         !(options &
  ...
  void (^makeSession)(Profile *, PseudoTerminal *, void (^)(PTYSession *)) = ^(
      Profile *profile, PseudoTerminal *term,
      void (^makeSessionCompletion)(PTYSession *)) {
    profile = [profile dictionaryBySettingObject:@"" forKey:KEY_INITIAL_TEXT];
    const BOOL closeSessionsOnEnd =
        !!(options & iTermSingleUseWindowOptionsCloseOnTermination);
    profile =
        [profile dictionaryBySettingObject:@(closeSessionsOnEnd
                                                 ? iTermSessionEndActionClose
                                                 : iTermSessionEndActionDefault)
    ...
    [term
        asyncCreateTabWithProfile:profile
                      withCommand:command
                      environment:environment
                   didMakeSession:^(PTYSession *session) {
                     ...
                     if (completion) {
                       ...
                       [[NSNotificationCenter defaultCenter]
                           addObserverForName:PTYSessionTerminatedNotification
                                       object:session
                                        queue:nil
...
- (void)reallySetCurrentTerminal:(PseudoTerminal *)thePseudoTerminal {
  ...
  if ([thePseudoTerminal windowInitialized] &&
      [[thePseudoTerminal window] isKeyWindow] == NO) {
...
- (NSArray<id<iTermPresentationControllerManagedWindowController>> *)
    presentationControllerManagedWindows {
  return _terminalWindows;
}
