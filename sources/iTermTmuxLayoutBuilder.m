...
// leaf-info ::= width 'x' height ',' xoff ',' yoff ',' 'wp-id'
- (NSString *)info {
  return [NSString stringWithFormat:@"%@x%@,%@,%@,%@", @(self.size.width),
                                    @(self.size.height), @(self.offset.x),
                                    @(self.offset.y), @(_windowPane)];
}
...
- (NSString *)info {
  ...
  return [NSString stringWithFormat:@"%@x%@,%@,%@%@", @(size.width),
                                    @(size.height), @(offset.x), @(offset.y),
                                    children];
}
