...
@property(nonatomic, readonly)
    BOOL haveScrollRegion; // is there a left-right or top-bottom margin?
...
// Size of the grid if the cursor is outside the scroll region. Otherwise, size
...
// Set both x and y coord of cursor at once. Cursor positions are clamped to
// legal values. The cursor may extend into the right edge (cursorX ==
// size.width is allowed).
- (void)setCursor:(VT100GridCoord)coord;

// Mark a specific character dirty. If updateTimestamp is set, then the line's
// last-modified time is set to the current time.
...
// Returns the count of lines excluding totally empty lines at the bottom, and
// always including the line the cursor is on.
- (int)numberOfLinesUsed;

// Like numberOfLinesUsed, but it doesn't care about where the cursor is. Also,
// if `includeWhitespace` is YES then spaces and tabs are considered empty.
- (int)numberOfNonEmptyLinesIncludingWhitespaceAsEmpty:(BOOL)whitespaceIsEmpty;

// Append the first numLines to the given line buffer. Returns the number of
// lines appended.
...
// Advances the cursor down one line and scrolls the screen, or part of the
// screen, if necessary. Returns the number of lines dropped from lineBuffer.
// lineBuffer may be nil. If a scroll region is present, the lineBuffer is only
// added to if useScrollbackWithRegion is set. willScroll is called if the
// region will need to scroll up by one line.
...
// Move cursor to the left by n steps. Does not wrap around when it hits the
// left margin. If it starts left of the scroll region, clamp it to the left. If
// it starts right of the scroll region, don't move it.  TODO: This is probably
// wrong w/r/t scroll region logic.
- (void)moveCursorLeft:(int)n;

// Move cursor to the right by n steps. Does not wrap around when it hits the
// right margin. The cursor is not permitted to extend into the width'th column,
// which setCursorX: allows. It has a similar same logic oddity as
// moveCursorLeft:, but slightly different (if the cursor's moved position is
// inside the scroll region, then it can change).
- (void)moveCursorRight:(int)n;

// Move cursor up by n steps. If there is a scroll region, it won't go past the
// top. Also clamps the cursor's x position to be valid.
- (void)moveCursorUp:(int)n;

// Move cursor down by n steps. If there is a scroll region, it won't go past
// the bottom. Also clamps the cursor's x position to be valid.
- (void)moveCursorDown:(int)n;

// Move cursor up one line.
// Scroll the screen or a region of the screen up by one line. If lineBuffer is
// set, a line scrolled off the top will be moved into the line buffer. If a
// scroll region is present, the lineBuffer is only added to if
// useScrollbackWithRegion is set.

// When scrolling vertically within a region, the |softBreak| flag is used.
// If |softBreak| is YES then the soft line break on the top line (when
// scrolling down) or bottom line (when scrolling up) is preserved. Otherwise it
// is made hard.
...
// Scroll the whole screen into the line buffer by one line. Returns the number
// of lines dropped. Scroll regions are ignored.
...
// Clear scroll region, clear screen, move cursor and saved cursor to origin,
// leaving only the last non-empty line at the top of the screen. Some lines may
// be left behind by giving a positive value for |leave|.
...
// Move the grid contents up, leaving only the whole wrapped line the cursor is
// on at the top.
- (void)moveWrappedCursorLineToTopOfGrid;

// Set chars in a rectangle, inclusive of from and to. It will clean up orphaned
// DWCs.
...
// Delete some number of chars starting at a given location, moving chars to the
// right of them back.
- (void)deleteChars:(int)num startingAt:(VT100GridCoord)startCoord;

// Scroll a rectangular area of the screen down (positive direction) or up
// (negative direction). Clears the left-over region. If |softBreak| is YES then
// the soft line break on the top line (when scrolling down) or bottom line
// (when scrolling up) is preserved. Otherwise it is made hard.
...
// Converts a range relative to the start of a row into a grid run. If row is
// negative, a smaller- than-range.length (but valid!) grid run will be
// returned.
- (VT100GridRun)gridRunFromRange:(NSRange)range relativeToRow:(int)row;

// Pop lines out of the line buffer and on to the screen. Up to maxLines will be
// restored. Before popping, lines to be modified will first be filled with
// defaultChar. Returns whether the cursor position was set.
...
// Returns a human-readable string with the screen contents and dirty lines
// interspersed.
...
// Reset scroll regions to whole screen. NOTE: It does not reset
// useScrollRegionCols.
- (void)resetScrollRegions;

// Returns a rect describing the current scroll region. Takes
// useScrollRegionCols into account.
- (VT100GridRect)scrollRegionRect;

// If a DWC is present at (offset, lineNumber), then both its cells are erased.
// They're replaced with c (normally -defaultChar). If there's a DWC_SKIP +
// EOL_DWC on the preceding line when offset==0 then those are converted to a
// null and EOL_HARD. Returns true if a DWC was erased.
- (BOOL)erasePossibleDoubleWidthCharInLineNumber:(int)lineNumber
                                startingAtOffset:(int)offset
                                        withChar:(screen_char_t)c;

// Moves the cursor to the left margin (either 0 or scrollRegionCols.location,
// depending on useScrollRegionCols).
...
// Returns the coordinate of the cell before this one. It respects scroll
// regions and double-width characters. Returns (-1,-1) if there is no previous
// cell.
...
// Returns an array of NSData for lines in order (corresponding with lines on
// screen).
