...
static NSString *const kGridUseScrollRegionColumnsKey =
    @"Use Scroll Region Columns";
static NSString *const kGridSizeKey = @"Size";

@interface VT100Grid ()
@property(nonatomic, readonly)
    NSArray *lines; // Warning: not in order found on screen!
@end

@implementation VT100Grid {
  VT100GridSize size_;
  int screenTop_; // Index into lines_ and dirty_ of first line visible in the
                  // grid.
  NSMutableArray<NSMutableData *> *lines_; // Array of NSMutableData. Each data
                                           // has size_.width+1 screen_char_t's.
...
- (instancetype)initWithSize:(VT100GridSize)size
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary
  ...
  if (self) {
    ...
    [[NSArray castFrom:dictionary[@"timestamps"]]
        enumerateObjectsUsingBlock:^(NSNumber *timestamp, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
    ...
    scrollRegionRows_ =
        [NSDictionary castFrom:dictionary[@"scrollRegionRows"]].gridRange;
    scrollRegionCols_ =
        [NSDictionary castFrom:dictionary[@"scrollRegionCols"]].gridRange;
    useScrollRegionCols_ =
        [NSNumber castFrom:dictionary[@"useScrollRegionCols"]].boolValue;
...
- (void)markCharDirty:(BOOL)dirty
                   at:(VT100GridCoord)coord
      updateTimestamp:(BOOL)updateTimestamp {
  DLog(@"Mark %@ dirty=%@ delegate=%@", VT100GridCoordDescription(coord),
       @(dirty), delegate_);
...
- (void)markCharsDirty:(BOOL)dirty
            inRectFrom:(VT100GridCoord)from
                    to:(VT100GridCoord)to {
  DLog(@"Mark rect from %@ to %@ dirty=%@ delegate=%@",
       VT100GridCoordDescription(from), VT100GridCoordDescription(to), @(dirty),
       delegate_);
  assert(from.x <= to.x);
  if (!dirty) {
    allDirty_ = NO;
...
- (void)markCharsDirty:(BOOL)dirty inRun:(VT100GridRun)run {
  DLog(@"Mark chars in run (origin=%@, length=%@) dirty=%@ delegate=%@",
       VT100GridCoordDescription(run.origin), @(run.length), @(dirty),
       delegate_);
...
- (NSIndexSet *)dirtyIndexesOnLine:(int)line {
  if (allDirty_) {
    return
        [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, self.size.width)];
  }
...
- (int)numberOfLinesUsed {
  return MAX(MIN(size_.height, cursor_.y + 1),
             [self numberOfNonEmptyLinesIncludingWhitespaceAsEmpty:NO]);
}
...
- (int)appendLines:(int)numLines toLineBuffer:(LineBuffer *)lineBuffer {
  ...
  // The maximum number of lines of scrollback are temporarily ignored because
  // this loop doesn't call dropExcessLinesWithWidth.
  ...
  for (i = 0; i < numLines; ++i) {
    ...
    if (i + 1 < size_.height) {
      lengthOfNextLine =
          [self lengthOfLine:[self screenCharsAtLineNumber:i + 1]];
    } else {
      lengthOfNextLine = -1;
    }
    ...
    } else if ((cursor_.x == 0) && (i == cursor_.y - 1) &&
               (lengthOfNextLine == 0) && line[size_.width].code != EOL_HARD) {
      // This line is continued, the next line is empty, and the cursor is
      // on the first column of the next line. Pull it up.
      // NOTE: This was cursor_.x + 1, but I'm pretty sure that's wrong as it
      // would always be 1.
...
- (int)scrollWholeScreenUpIntoLineBuffer:(LineBuffer *)lineBuffer
  ...
  // Mark the cursor's previous location dirty. This fixes a rare race condition
  // where the cursor is not erased.
  ...
  // Increment screenTop_, effectively scrolling the lines & dirty up by one
  // line.
  ...
  if (lineBuffer) {
    ...
    // TODO: Test what happens when the alt screen scrolls while it has a
    // selection.
    [self markCharsDirty:YES
              inRectFrom:VT100GridCoordMake(0, 0)
                      to:VT100GridCoordMake(size_.width - 1, size_.height - 1)];
...
- (int)scrollUpIntoLineBuffer:(LineBuffer *)lineBuffer
  ...
  if (![self haveScrollRegion]) {
    ...
    if (scrollTop == 0 && useScrollbackWithRegion &&
        ![self haveColumnScrollRegion]) {
    ...
    // TODO: formerly, scrollTop==scrollBottom was a no-op but I think that's
    // wrong. See what other terms do.
    [self scrollRect:VT100GridRectMake(scrollLeft, scrollTop,
...
- (int)resetWithLineBuffer:(LineBuffer *)lineBuffer
  ...
  [self setCharsFrom:VT100GridCoordMake(
                         0, preserveCursorLine ? 1 + additionalLinesToSave : 0)
                  to:VT100GridCoordMake(size_.width - 1, size_.height - 1)
...
- (void)moveWrappedCursorLineToTopOfGrid {
  if (cursor_.y < 0) {
    return; // Not sure how this would happen, but the old code in -[VT100Screen
            // clearScreen] had this check.
  }
...
- (void)moveCursorLeft:(int)n {
  if ([self haveColumnScrollRegion]) {
    ...
    // column scroll region. If the cursor begins at/right of the left margin,
    // it stops at the left margin. If the cursor begins left of the left
    // margin, it stops at the left edge.
    int x = cursor_.x - n;
  ...
  while (n > 0) {
    if (self.cursorX == 0 && self.cursorY == 0) {
      ...
      case EOL_SOFT:
      case EOL_DWC:
        // Wrap around to the end of the previous line, even if this leaves the
        // cursor on a DWC_RIGHT.
        n--;
...
- (void)setCharsFrom:(VT100GridCoord)from
  ...
  for (int y = MAX(0, from.y); y <= MIN(to.y, size_.height - 1); y++) {
    ...
    [self erasePossibleDoubleWidthCharInLineNumber:y
                                  startingAtOffset:from.x - 1
                                          withChar:c];
    [self erasePossibleDoubleWidthCharInLineNumber:y
                                  startingAtOffset:to.x
                                          withChar:c];
...
- (int)appendCharsAtCursor:(screen_char_t *)buffer
  ...
  for (idx = 0; idx < len;) {
    ...
    if (buffer[idx].code == DWC_SKIP) {
      // I'm pretty sure this can never happen and that this code is just a
      // historical leftover. This is an invalid unicode character that iTerm2
      // has appropriated for internal use. Change it to something invalid but
      // safe.
    ...
    if (useScrollRegionCols_ && cursor_.x <= scrollRight + 1) {
      ...
      //
      // NOTE:
      //    Above behavior is compatible with xterm, but incompatible with
      //    VT525. VT525 has curious glitch:
      //        If a text run which starts from left of the left margin
      //        wraps or returns by CR, the next line starts from column 1, but
      //        not left margin. (see Mr. IWAMOTO's gist
      //        https://gist.github.com/ttdoda/5902671)
    ...
    if (cursor_.x >= rightMargin - widthOffset) {
      if (wraparound) {
        ...
        numDropped += [self
            moveCursorDownOneLineScrollingIntoLineBuffer:lineBuffer
                                     unlimitedScrollback:unlimitedScrollback
                                 useScrollbackWithRegion:useScrollbackWithRegion
                                              willScroll:nil];
...
#endif
    if (charsToInsert <= 0) {
      // NSLog(@"setASCIIString: output
      // length=0?(%d+%d)%d+%d",cursor_.x,charsToInsert,idx2,len);
      break;
    }
    ...
    if (newx == size_.width) {
      // The cursor is ending at the right margin. See if there's a
      // DWC_SKIP/EOL_DWC pair there which may be affected.
      mayStompSplitDwc =
          (!useScrollRegionCols_ && aLine[size_.width].code == EOL_DWC &&
           aLine[size_.width - 1].code == DWC_SKIP);
    } else if (!wraparound && rightMargin < size_.width &&
               useScrollRegionCols_ && newx >= rightMargin &&
               cursor_.x < rightMargin) {
      // Prevent the cursor from going past the right margin when wraparound is
      // off.
      newx = rightMargin - 1;
    }
...
#endif
        // Shift the old line contents to the right by 'charsToInsert'
        // positions.
...
#ifdef VERBOSE_STRING
      NSLog(@"Wiping out the right-half DWC at the cursor before writing to "
            @"screen");
      ITAssertWithMessage(cursor_.x > 0,
                          @"DWC split"); // there should never be the second
                                         // half of a DWC at x=0
#endif
    ...
    // This is an ugly little optimization--if we're inserting just one
    // character, see if it would change anything (because the memcmp is really
    // cheap). In particular, this helps vim out because it really likes
    // redrawing pane separators when it doesn't need to.
    if (charsToInsert > 1 || memcmp(aLine + cursor_.x, buffer + idx,
                                    charsToInsert * sizeof(screen_char_t))) {
      // copy charsToInsert characters into the line and set them dirty.
      memcpy(aLine + cursor_.x, buffer + idx,
             charsToInsert * sizeof(screen_char_t));
      [self markCharsDirty:YES
                inRectFrom:VT100GridCoordMake(cursor_.x, lineNumber)
                        to:VT100GridCoordMake(cursor_.x + charsToInsert - 1,
                                              lineNumber)];
    }
    if (wrapDwc) {
    ...
    if (mayStompSplitDwc && aLine[size_.width - 1].code != DWC_SKIP &&
        aLine[size_.width].code == EOL_DWC) {
      // The line no longer ends in a DWC_SKIP, but the continuation mark is
      // still EOL_DWC. Change the continuation mark to EOL_SOFT since there's
      // presumably still a DWC at the start of the next line.
      aLine[size_.width].code = EOL_SOFT;
    }
    ...
    if (cursor_.x >= effective_width && ansi) {
      if (wraparound) {
        ...
        aLine[size_.width] = [self defaultChar];
        aLine[size_.width].code =
            ((effective_width == size_.width) ? EOL_SOFT : EOL_DWC);
        self.cursorX = leftMargin;
        numDropped += [self
            moveCursorDownOneLineScrollingIntoLineBuffer:lineBuffer
                                     unlimitedScrollback:unlimitedScrollback
...
- (void)deleteChars:(int)n startingAt:(VT100GridCoord)startCoord {
  ...
  if (startCoord.x >= leftMargin && startCoord.x < rightMargin &&
      startCoord.y >= 0 && startCoord.y < size_.height) {
    ...
    if (n > 0 && startCoord.x + n <= rightMargin) {
      // Erase a section in the middle of a line. Shift the stuff to the right
      // of the deletion region left to the startCoord.
      ...
      // When there's a scroll region, are we moving the left half of a DWC w/o
      // right half?
      [self erasePossibleDoubleWidthCharInLineNumber:startCoord.y
      ...
      if (rightMargin == size_.width - 1 &&
          aLine[size_.width].code == EOL_DWC) {
        // When the previous if statement is true, this one should also always
        // be true.
        aLine[size_.width].code = EOL_HARD;
      }

      memmove(aLine + startCoord.x, aLine + startCoord.x + n,
              numCharsToMove * sizeof(screen_char_t));
      [self markCharsDirty:YES
                inRectFrom:VT100GridCoordMake(startCoord.x, lineNumber)
                        to:VT100GridCoordMake(startCoord.x + numCharsToMove - 1,
                                              lineNumber)];
...
- (void)scrollRect:(VT100GridRect)rect
            downBy:(int)distance
         softBreak:(BOOL)softBreak {
  DLog(@"scrollRect:%d,%d %dx%d downBy:%d", rect.origin.x, rect.origin.y,
       rect.size.width, rect.size.height, distance);
  if (distance == 0) {
    return;
  }
  ...
  if (rect.size.width > 0 && rect.size.height > 0) {
    ...
    int sourceIndex =
        (direction > 0 ? bottomIndex - distance : rect.origin.y - distance);
    ...
    if (continuation) {
      // The last line scrolled may have had a split-dwc. Replace its
      // continuation mark with a hard newline.
    ...
    // Move lines.
    for (int iteration = 0;
         (iteration < sourceHeight && sourceIndex < size_.height &&
          destIndex < size_.height && sourceIndex >= 0 && destIndex >= 0);
         iteration++) {
      ...
      memmove(targetLine + rect.origin.x, sourceLine + rect.origin.x,
    ...
    // Fix up broken soft or dwc_skip continuation marks. It could occur on line
    // just above the scroll region.
    if (lineNumberAboveScrollRegion >= 0 &&
        lineNumberAboveScrollRegion < size_.height && rect.origin.x == 0) {
    ...
    if (rect.origin.x + rect.size.width == size_.width && !softBreak) {
      // Clean up continuation mark on last line inside scroll region when
      // scrolling down, or last last preserved line when scrolling up, unless
      // we were asked to preserve soft breaks.
      int lastLineOfScrollRegion;
      if (direction > 0) {
        lastLineOfScrollRegion = rect.origin.y + rect.size.height - 1;
      } else {
        lastLineOfScrollRegion =
            rect.origin.y + rect.size.height - 1 + distance;
      }
      // Affecting continuation mark on first/last line in block
      if (lastLineOfScrollRegion >= 0 &&
          lastLineOfScrollRegion < size_.height) {
    ...
    if (direction > 0) {
      [self setCharsFrom:rect.origin
                      to:VT100GridCoordMake(
                             rightIndex,
                             MIN(bottomIndex, rect.origin.y + distance - 1))
                  toChar:defaultChar];
    } else {
      [self setCharsFrom:VT100GridCoordMake(
                             rect.origin.x,
                             MAX(rect.origin.y, bottomIndex + distance + 1))
                      to:VT100GridCoordMake(rightIndex, bottomIndex)
                  toChar:defaultChar];
    }

    if ((rect.origin.x == 0) ^ continuation) {
      // It's possible that either a continuation mark is being moved or a DWC
      // was moved (but not both!), causing a split-dwc continuation mark to
      // become invalid.
      for (int iteration = 0; iteration < rect.size.height; iteration++) {
    ...
    // Clean up split-dwc continuation marker on last line if scrolling down, or
    // line before rectangle if scrolling up.
    if (continuation && rect.origin.x == 0) {
      // Moving whole lines
      if (direction > 0) {
        // scrolling down
        [self erasePossibleSplitDwcAtLineNumber:rect.origin.y +
                                                rect.size.height - 1];
      } else {
        // scrolling up
        [self erasePossibleSplitDwcAtLineNumber:rect.origin.y - 1];
...
- (void)setContentsFromDVRFrame:(screen_char_t *)s info:(DVRFrameInfo)info {
  ...
  for (int y = 0; y < MIN(info.height, size_.height); y++) {
    ...
    if (charsToCopyPerLine < info.width &&
        src[charsToCopyPerLine].code == DWC_RIGHT) {
      dest[charsToCopyPerLine - 1].code = 0;
      dest[charsToCopyPerLine - 1].complexChar = NO;
    }
    if (charsToCopyPerLine - 1 < info.width &&
        src[charsToCopyPerLine - 1].code == TAB_FILLER) {
...
- (NSString *)debugString {
  ...
  for (y = 0; y < size_.height; ++y) {
    ...
    [result appendFormat:@"%04d: %s %@\n", y, line,
                         [self stringForContinuationMark:p[size_.width].code]];
...
- (VT100GridRun)gridRunFromRange:(NSRange)range relativeToRow:(int)row {
  ...
  const NSInteger location =
      (NSInteger)range.location + longRow * (NSInteger)size_.width;
  ...
  return VT100GridRunMake(adjustedLocation % size_.width,
                          adjustedLocation / size_.width, adjustedLength);
}
...
- (BOOL)restoreScreenFromLineBuffer:(LineBuffer *)lineBuffer
  ...
  while (destLineNumber >= 0) {
    ...
    if (!foundCursor) {
      ...
      foundCursor = [lineBuffer getCursorInLastLineWithWidth:size_.width
                                                         atX:&tempCursor];
      if (foundCursor) {
        VT100GridCoord newCursorCoord =
            VT100GridCoordMake(tempCursor % size_.width,
    ...
    if (cont && dest[size_.width - 1].code == 0 &&
...
- (NSArray *)rectsForRun:(VT100GridRun)run {
  ...
  for (int y = run.origin.y; length > 0; y++) {
    ...
    [rects addObject:[NSValue valueWithGridRect:VT100GridRectMake(
                                                    x, y, endX - x + 1, 1)]];
    length -= (endX - x + 1);
    x = 0;
  }
...
- (int)rightMargin {
  return useScrollRegionCols_ ? VT100GridRangeMax(scrollRegionCols_)
                              : size_.width - 1;
}
...
- (VT100GridRect)scrollRegionRect {
  return VT100GridRectMake(self.leftMargin, self.topMargin,
...
- (NSString *)compactLineDumpWithTimestamps {
  ...
  for (int y = 0; y < size_.height; y++) {
    ...
    NSDate *date = [NSDate
        dateWithTimeIntervalSinceReferenceDate:[[self lineInfoAtLineNumber:y]
                                                   timestamp]];
...
- (void)insertChar:(screen_char_t)c at:(VT100GridCoord)pos times:(int)n {
  ...
  if (self.rightMargin == size_.width - 1 &&
    ...
    // The line shifting means a split DWC is gone. The wrapping changes to hard
    // if the last code is 0 because soft wrapping doesn't make sense across a
    // null code.
    line[size_.width].code = line[size_.width - 1].code ? EOL_SOFT : EOL_HARD;
  }

  if (size_.width > 0 && self.rightMargin == size_.width - 1 &&
      line[size_.width].code == EOL_SOFT && line[size_.width - 1].code == 0) {
...
- (NSDictionary *)dictionaryValue {
  return @{
    ...
    kGridScrollRegionRowsKey :
        [NSDictionary dictionaryWithGridRange:scrollRegionRows_],
    kGridScrollRegionColumnsKey :
        [NSDictionary dictionaryWithGridRange:scrollRegionCols_],
...
- (void)setStateFromDictionary:(NSDictionary *)dict {
  ...
  // Saved values only make sense if the size is at least as large as when the
  // state was saved. When restoring from a saved arrangement, the initial grid
  // size is a guess which is a bit too wide when legacy scrollbars are in use.
...
- (void)restorePreferredCursorPositionIfPossible {
  if (_preferredCursorPosition.x >= 0 && _preferredCursorPosition.y >= 0 &&
    ...
    DLog(@"Restore preferred cursor position to %@",
         VT100GridCoordDescription(_preferredCursorPosition));
...
- (VT100GridSize)sizeRespectingRegionConditionally {
  if (self.cursor.x >= self.leftMargin && self.cursor.x <= self.rightMargin &&
      self.cursor.y >= self.topMargin && self.cursor.y <= self.bottomMargin) {
...
- (void)encode:(id<iTermEncoderAdapter>)encoder {
  NSArray<NSNumber *> *timestamps =
      [lineInfos_ mapWithBlock:^id(VT100LineInfo *anObject) {
        return @(anObject.timestamp);
      }];
  NSArray<NSData *> *lines =
      [[NSArray sequenceWithRange:NSMakeRange(0, size_.height)]
          mapWithBlock:^id(NSNumber *i) {
  ...
  [encoder mergeDictionary:@{
    ...
    @"savedDefaultCharData" : [NSData dataWithBytes:&savedDefaultChar_
                                             length:sizeof(savedDefaultChar_)]
  }];
}
...
- (NSMutableData *)lineOfWidth:(int)width filledWithChar:(screen_char_t)c {
  NSMutableData *data =
      [NSMutableData dataWithLength:sizeof(screen_char_t) * (width + 1)];
...
- (NSMutableData *)defaultLineOfWidth:(int)width {
  ...
  screen_char_t *existingCache =
      (screen_char_t *)[cachedDefaultLine_ mutableBytes];
  screen_char_t currentDefaultChar = [self defaultChar];
  if (cachedDefaultLine_ && [cachedDefaultLine_ length] == length &&
      length > 0 &&
...
- (void)clearScreenChars:(screen_char_t *)chars inRange:(VT100GridRange)range {
  if (cachedDefaultLine_) {
    // Only do this if there is a cached line; otherwise there's an infinite
    // recursion since -defaultLineOfWidth indirectly calls this method.
    NSData *defaultLine = [self defaultLineOfWidth:size_.width];
    memcpy(chars + range.location, defaultLine.bytes,
...
- (void)clearLineData:(NSMutableData *)line {
  ...
  [self clearScreenChars:[line mutableBytes]
                 inRange:VT100GridRangeMake(0, length)];
  screen_char_t *chars = (screen_char_t *)[line mutableBytes];
  int width = [line length] / sizeof(screen_char_t) - 1;
  chars[width].code = EOL_HARD;
}

// Returns number of lines dropped from line buffer because it exceeded its size
// (always 0 or 1).
...
- (BOOL)haveScrollRegion {
  const BOOL haveScrollRows =
      !(self.topMargin == 0 && self.bottomMargin == size_.height - 1);
...
- (void)setSize:(VT100GridSize)newSize {
  if (newSize.width != size_.width || newSize.height != size_.height) {
    ...
    scrollRegionRows_.location =
        MIN(scrollRegionRows_.location, size_.width - 1);
    scrollRegionRows_.length =
        MIN(scrollRegionRows_.length, size_.width - scrollRegionRows_.location);
    scrollRegionCols_.location =
        MIN(scrollRegionCols_.location, size_.height - 1);
...
- (VT100GridCoord)coordinateBefore:(VT100GridCoord)coord
          movedBackOverDoubleWidth:(BOOL *)dwc {
  ...
  if (cx == self.leftMargin || cx == 0) {
    ...
    if (![self haveColumnScrollRegion]) {
      ...
      case EOL_SOFT:
        ...
        break;
      case EOL_DWC:
        // Ok to wrap around, but move back over presumed DWC_SKIP at
        // line[rightMargin-1].
        cx--;
        if (cx < 0) {
          return invalid;
  ...
  --cx;
  if (cx < 0 || cy < 0) {
    // can't affect characters above screen so have it stand alone. cx really
    // should never be less than zero, but paranoia.
    return invalid;
  }
...
- (screen_char_t)characterAt:(VT100GridCoord)coord {
  if (coord.y < 0 || coord.y >= self.size.height || coord.x < 0 ||
      coord.x > self.size.width) {
    ITBetaAssert(NO, @"Asked for %@ in %@", VT100GridCoordDescription(coord),
                 self);
...
- (void)erasePossibleSplitDwcAtLineNumber:(int)lineNumber {
  ...
  if (line[size_.width].code == EOL_DWC) {
    line[size_.width].code = EOL_HARD;
    if (line[size_.width - 1].code ==
        DWC_SKIP) { // This really should always be the case.
...
// Number of cells between two coords, including both endpoints For example,
// with a grid of xxyy yyxx The number of cells from (2,0) to (1,1) is 4 (the
// cells containing "y").
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p size=%d x %d, cursor @ (%d,%d)>",
                                    [self class], self, size_.width,
                                    size_.height, cursor_.x, cursor_.y];
}

// Returns NSString representation of line. This exists to facilitate debugging
// only.
