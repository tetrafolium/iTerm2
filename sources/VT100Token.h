...
typedef enum {
  // Any control character between 0-0x1f inclusive can by a token type. For
  // these, the value
  ...
  VT100CC_DC3 = 19,  // Causes terminal to stop transmitting all codes except
                     // XOFF and XON (XOFF).
  ...
  VT100CSI_XDA,     // Extended device attributes
  ...
  VT100CSI_SET_MODIFIERS,   // CSI > Ps; Pm m (Whether to set modifiers for
                            // different kinds of key presses; no official name)
  VT100CSI_RESET_MODIFIERS, // CSI > Ps n (Set all modifiers values to -1,
                            // disabled)
  ...
  XTERMCC_SET_PALETTE,
  XTERMCC_SET_KVP,
  // OSC 1337;File=(args):(data) gets changed by the parser from XTERMCC_SET_KVP
  // to a
  // series of incidental tokens beginning with
  ...
  // If a sequence is split into multiple tokens, the first will be one of the
  // above whose name
  // includes MULTITOKEN_HEADER, then zero or more of these, and then
  ...
  // ISO-2022 codes for choosing character encoding. There are a bunch of other
  // encodings that
  // there are escape codes for but they're really old-fashioned, so only these
  // two are supported
  // so far.
...
// A preinitialized array of screen_char_t. When ASCII data is present, it will
// have the codes populated and all other fields zeroed out.
