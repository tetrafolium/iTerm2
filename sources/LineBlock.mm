...
struct iTermNumFullLinesCacheKey {
  ...
  iTermNumFullLinesCacheKey(const int &xOffset, const int &xLength,
                            const int &xWidth)
...
@implementation LineBlock {
  ...
  screen_char_t
      *buffer_start; // usable start of buffer (stuff before this is dropped)
  ...
  // There will be as many entries in this array as there are lines in
  // raw_buffer. The ith value is the length of the ith line plus the value of
  ...
  // If true, then the last raw line does not include a logical newline at its
  // terminus.
  BOOL is_partial;
  ...
  // This is -1 if the cache is invalid; otherwise it specifies the width for
  // which cached_numlines is correct.
  int cached_numlines_width;

  // Keys are (offset from raw_buffer, length to examine, width).
  std::unordered_map<iTermNumFullLinesCacheKey, int,
                     iTermNumFullLinesCacheKeyHasher>
...
NS_INLINE void
iTermLineBlockDidChange(__unsafe_unretained LineBlock *lineBlock) {
  lineBlock->_generation += 1;
  for (auto &observer : lineBlock->_observers) {
    __unsafe_unretained id<iTermLineBlockObserver> obj =
        static_cast<id<iTermLineBlockObserver>>(observer);
...
- (LineBlock *)initWithRawBufferSize:(int)size {
  ...
  if (self) {
    ...
    buffer_size = size;
    // Allocate enough space for a bunch of 80-character lines. It can grow if
    // needed.
...
- (void)commonInit {
  ...
  if (cll_capacity > 0) {
    metadata_ = (LineBlockMetadata *)iTermCalloc(sizeof(LineBlockMetadata),
                                                 cll_capacity);
  }
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
  ...
  if (self) {
    NSArray *requiredKeys = @[
      kLineBlockRawBufferKey, kLineBlockBufferStartOffsetKey,
      kLineBlockStartOffsetKey, kLineBlockFirstEntryKey,
      kLineBlockBufferSizeKey, kLineBlockCLLKey, kLineBlockMetadataKey,
      kLineBlockIsPartialKey, kLineBlockMayHaveDWCKey
    ];
    ...
    raw_buffer =
        (screen_char_t *)iTermMalloc(buffer_size * sizeof(screen_char_t));
    memmove(raw_buffer, data.bytes, data.length);
    buffer_start =
        raw_buffer + [dictionary[kLineBlockBufferStartOffsetKey] intValue];
    ...
    for (int i = 0; i < cll_capacity; i++) {
      ...
      metadata_[i].continuation.backgroundColor =
          [components[j++] unsignedCharValue];
    ...
    _mayHaveDoubleWidthCharacter =
        [dictionary[kLineBlockMayHaveDWCKey] boolValue];
  }
  return self;
}
...
- (LineBlock *)copyWithZone:(NSZone *)zone {
  ...
  theCopy->raw_buffer =
      (screen_char_t *)iTermMalloc(sizeof(screen_char_t) * buffer_size);
  ...
  theCopy->metadata_ = (LineBlockMetadata *)iTermMalloc(
      sizeof(LineBlockMetadata) * cll_capacity);
  memmove(theCopy->metadata_, metadata_,
          sizeof(LineBlockMetadata) * cll_capacity);
  for (int i = 0; i < cll_capacity; i++) {
...
- (void)_appendCumulativeLineLength:(int)cumulativeLength
  ...
  if (cll_entries == cll_capacity) {
    ...
    cumulative_line_lengths = (int *)iTermRealloc(
        (void *)cumulative_line_lengths, cll_capacity, sizeof(int));
    metadata_ = (LineBlockMetadata *)iTermRealloc(
        (void *)metadata_, cll_capacity, sizeof(LineBlockMetadata));
    if (gEnableDoubleWidthCharacterLineCache) {
      memset(metadata_ + cll_entries, 0,
...
- (void)dump:(int)rawOffset toDebugLog:(BOOL)toDebugLog {
  ...
  for (i = first_entry; i < cll_entries; ++i) {
    ...
    NSString *message = [NSString
        stringWithFormat:@"Line %d, length %d, offset from raw=%d, abs pos=%d, "
                         @"continued=%s: %s\n",
                         i, cumulative_line_lengths[i] - prev, prev,
                         prev + rawOffset, iscont ? "yes" : "no",
                         formatsct(buffer_start + prev - start_offset,
                                   cumulative_line_lengths[i] - prev, temp)];
    if (toDebugLog) {
      DLog(@"%@", message);
...
- (int)numberOfFullLinesFromOffset:(int)offset
  ...
  if (wasInserted) {
    result = iTermLineBlockNumberOfFullLinesImpl(
        raw_buffer + offset, length, width, _mayHaveDoubleWidthCharacter);
    it->second = result;
  } else {
    result = it->second;
  }
...
extern "C" int
iTermLineBlockNumberOfFullLinesImpl(screen_char_t *buffer, int length,
                                    int width,
...
- (void)checkAndResetCachedNumlines:(char *)methodName width:(int)width {
  ...
  if (was_valid && old_cached != new_cached) {
    NSLog(@"%s: cached_numlines updated to %d, but should be %d!", methodName,
...
- (BOOL)appendLine:(screen_char_t *)buffer
  ...
  // There's an edge case here. In the else clause, the line buffer looks like
  // this originally:
  //   |xxxx| EOL_SOFT
  // Then append an empty line with EOL_HARD. The desired result is
  //   |xxxx| EOL_SOFT
  //   ||     EOL_HARD
  // It's an edge case because even though the line buffer is in the
  // "is_partial" state, we can't just increment the last line's length.
  //
  // This can happen in practice if the now-empty line being appended formerly
  // had some stuff but that stuff was erased and the EOL_SOFT was left behind.
  if (is_partial && !(!partial && length == 0)) {
    // append to an existing line
    ITAssertWithMessage(cll_entries > 0, @"is_partial but has no entries");
    // update the numlines cache with the new number of full lines that the
    // updated line has.
    if (width != cached_numlines_width) {
      cached_numlines_width = -1;
    } else {
      int prev_cll =
          cll_entries > first_entry + 1
              ? cumulative_line_lengths[cll_entries - 2] - start_offset
              : 0;
      int cll = cumulative_line_lengths[cll_entries - 1] - start_offset;
      int old_length = cll - prev_cll;
      int oldnum = [self
          numberOfFullLinesFromOffset:(buffer_start - raw_buffer) + prev_cll
                               length:old_length
                                width:width];
      int newnum = [self
          numberOfFullLinesFromOffset:(buffer_start - raw_buffer) + prev_cll
  ...
  if (!p) {
    ...
    if (length > 0 && (!isStartOfWrappedLine || x > 0)) {
      *yOffsetPtr = 0;
      ...
      //   abc.
      //   d...
      // The cell after c has position 3, as does the cell with d. The
      // difference is that d has a yOffset=1 and the null cell after c has
      // yOffset=0.
      //
...
- (void)populateDoubleWidthCharacterCacheInMetadata:
...
- (int)offsetOfWrappedLineInBuffer:(screen_char_t *)p
  ...
  if (_mayHaveDoubleWidthCharacter) {
    ...
    [metadata->double_width_characters
        enumerateIndexesInRange:NSMakeRange(0, MAX(0, n + 1))
                        options:0
                     usingBlock:^(
                         NSUInteger indexOfLineThatWouldStartWithRightHalf,
                         BOOL *_Nonnull stop) {
                       int numberOfLines =
                           indexOfLineThatWouldStartWithRightHalf - lastIndex;
                       lines += numberOfLines;
...
// TODO: Reduce use of this function in favor of the optimized method once I am
...
- (NSTimeInterval)timestampForLineNumber:(int)lineNum width:(int)width {
  ...
  for (i = first_entry; i < cll_entries; ++i) {
    ...
    length = cll - prev;
    const int spans =
        [self numberOfFullLinesFromOffset:(buffer_start - raw_buffer) + prev
                                   length:length
                                    width:width];
    if (lineNum > spans) {
  ...
  for (int i = first_entry; i < cll_entries; ++i) {
    ...
    if (*lineNum > spans) {
      ...
      *lineNum -= consume;
      ...
      // We found the raw line that includes the wrapped line we're searching
      // for. eat up *lineNum many width-sized wrapped lines from this start of
      // the current full line
      int offset;
      if (gEnableDoubleWidthCharacterLineCache) {
        ...
        offset = OffsetOfWrappedLine(buffer_start + prev, *lineNum, length,
                                     width, _mayHaveDoubleWidthCharacter);
      }

      *lineNum = 0;
      // offset: the relevant part of the raw line begins at this offset into it
      *lineLength =
          length - offset; // the length of the suffix of the raw line,
                           // beginning at the wrapped line we want
      if (*lineLength > width) {
        // return an infix of the full line
        if (width > 1 &&
            buffer_start[prev + offset + width].code == DWC_RIGHT) {
        ...
        if (i == cll_entries - 1 && is_partial) {
          // If this is the last line and it's partial then it doesn't have an
          // end-of-line.
          *includesEndOfLine = EOL_SOFT;
        } else {
      ...
      if (yOffsetPtr) {
        // Set *yOffsetPtr to the number of consecutive empty lines just before
        // the requested line.
...
- (int)getNumLinesWithWrapWidth:(int)width {
  ...
  int prev = 0;
  int i;
  // Count the number of wrapped lines in the block by computing the sum of the
  // number of wrapped lines each raw line would use.
  for (i = first_entry; i < cll_entries; ++i) {
    int cll = cumulative_line_lengths[i] - start_offset;
    int length = cll - prev;
  ...
  // Save the result so it doesn't have to be recalculated until some relatively
  // rare operation occurs that invalidates the cache.
...
- (BOOL)popLastLineInto:(screen_char_t **)ptr
  ...
  if (available_len > width) {
    // The last raw line is longer than width. So get the last part of it after
    // wrapping. If the width is four and the last line is "0123456789" then
    // return "89". It would wrap as: 0123/4567/89. If there are double-width
    // characters, this ensures they are not split across lines when computing
    // the wrapping.
    const int numLines =
        [self numberOfFullLinesFromOffset:(buffer_start - raw_buffer) + start
                                   length:available_len
                                    width:width];
    int offset_from_start =
        OffsetOfWrappedLine(buffer_start + start, numLines, available_len,
                            width, _mayHaveDoubleWidthCharacter);
...
- (void)changeBufferSize:(int)capacity {
  ITAssertWithMessage(capacity >= [self rawSpaceUsed],
                      @"Truncating used space");
  capacity = MAX(1, capacity);
  raw_buffer = (screen_char_t *)iTermRealloc((void *)raw_buffer,
                                             sizeof(screen_char_t), capacity);
  ...
  *charsDropped = 0;
  ...
  for (i = first_entry; i < cll_entries; ++i) {
    ...
    if (n > spans) {
      ...
      // We found the raw line that includes the wrapped line we're searching
      // for. Set offset to the offset into the raw line where the nth wrapped
      // line begins.
...
static NSString *RewrittenRegex(NSString *originalRegex) {
  // Convert ^ in a context where it refers to the start of string to
  // kPrefixChar Convert $ in a context where it refers to the end of string to
  // kSuffixChar ^ is NOT start-of-string when:
  //   - it is escaped
  ...
  //   - it is escaped
  //
  // It might be possible to write this as a regular substitution but it would
  // be a crazy mess.

  NSMutableString *rewritten =
      [NSMutableString stringWithCapacity:[originalRegex length]];
  BOOL escaped = NO;
  BOOL inSet = NO;
...
static int CoreSearch(NSString *needle, screen_char_t *rawline,
                      int raw_line_length, int start, int end,
                      FindOptions options, iTermFindMode mode,
                      int *resultLength, NSString *haystack,
                      unichar *charHaystack, int *deltas, int deltaOffset) {
  ...
  if (regex) {
    ...
    NSString *sanitizedHaystack = [haystack
        stringByReplacingOccurrencesOfString:[NSString
                                                 stringWithFormat:@"%c",
                                                                  kPrefixChar]
                                  withString:[NSString
                                                 stringWithFormat:@"%c", 3]];
    sanitizedHaystack = [sanitizedHaystack
        stringByReplacingOccurrencesOfString:[NSString
                                                 stringWithFormat:@"%c",
                                                                  kSuffixChar]
                                  withString:[NSString
    ...
    if (end == raw_line_length) {
      if (start == 0) {
        sandwich = [NSString stringWithFormat:@"%C%@%C", kPrefixChar,
                                              sanitizedHaystack, kSuffixChar];
      } else {
        hasPrefix = NO;
        sandwich =
            [NSString stringWithFormat:@"%@%C", sanitizedHaystack, kSuffixChar];
      }
    } else {
      hasSuffix = NO;
      sandwich =
          [NSString stringWithFormat:@"%C%@", kPrefixChar, sanitizedHaystack];
    }
    ...
    if (backwards) {
      ...
      // keep searching from one char after the start of the match until we
      // don't find anything. regexes aren't good at searching backwards.
      while (!regexError && temp.location != NSNotFound &&
             temp.location + locationAdjustment < [sandwich length]) {
        if (temp.length != 0) {
          range = temp;
        }
        temp.location += MAX(1, temp.length);
        temp = [sandwich
            rangeOfRegex:rewrittenRegex
                 options:apiOptions
                 inRange:NSMakeRange(temp.location,
                                     [sandwich length] - temp.location)
                 capture:0
                   error:&regexError];
      }
    }
    ...
    if (mode == iTermFindModeSmartCaseSensitivity &&
        [needle rangeOfCharacterFromSet:[NSCharacterSet
                                            uppercaseLetterCharacterSet]]
                .location == NSNotFound) {
      caseInsensitive = YES;
    }
    if (caseInsensitive) {
      apiOptions = static_cast<RKLRegexOptions>(
          apiOptions | NSCaseInsensitiveSearch | NSDiacriticInsensitiveSearch |
...
static int Search(NSString *needle, screen_char_t *rawline, int raw_line_length,
                  int start, int end, int options, iTermFindMode mode,
                  int *resultLength) {
  NSString *haystack;
  unichar *charHaystack;
  int *deltas;
  haystack =
      ScreenCharArrayToString(rawline, start, end, &charHaystack, &deltas);
  // screen_char_t[i + deltas[i]] begins its run at charHaystack[i]
  int result =
      CoreSearch(needle, rawline, raw_line_length, start, end, options, mode,
                 resultLength, haystack, charHaystack, deltas, deltas[0]);
...
- (void)_findInRawLine:(int)entry
  ...
  if (options & FindOptBackwards) {
    ...
    int *deltas;
    haystack =
        ScreenCharArrayToString(rawline, 0, limit, &charHaystack, &deltas);
    ...
    do {
      ...
      if ([haystack length] >= kMaxSaneStringLength) {
        // There's a bug in OS 10.9.0 (and possibly other versions) where the
        // string
        // @"a⃑" reports a length of 0x7fffffffffffffff, which causes this loop
        // to never terminate.
        break;
      }
      tempPosition =
          CoreSearch(needle, rawline, raw_line_length, 0, limit, options, mode,
    ...
    while (skip < raw_line_length) {
      tempPosition = Search(needle, rawline, raw_line_length, skip,
                            raw_line_length, options, mode, &tempResultLength);
      if (tempPosition != -1) {
        ResultRange *r = [[[ResultRange alloc] init] autorelease];
        r->position = tempPosition;
        r->length = tempResultLength;
...
- (void)findSubstring:(NSString *)substring
  ...
  while (entry != limit) {
    ...
    // Don't search arbitrarily long lines. If someone has a 10 million
    // character long line then it'll hang for a long time.
    ...
    [self _findInRawLine:entry
                 ...
                 options:options
                    mode:mode
                    skip:skipped
                  length:MIN(MAX_SEARCHABLE_LINE_LENGTH,
                             [self _lineLength:entry])
  ...
  for (i = first_entry; i < cll_entries; ++i) {
    ...
    if ((wrapOnEOL && position >= eol) || (!wrapOnEOL && position > eol)) {
      ...
      *y += spans + 1;
      ...
      // If the position is the left half of a double width char then include
      // the right half in the following call to
      // iTermLineBlockNumberOfFullLinesImpl.
      ...
      if (bytes_to_consume_in_this_line < line_length &&
        ...
        if (width > 1 &&
            raw_buffer[prev + bytes_to_consume_in_this_line + 1].code ==
                DWC_RIGHT) {
          ++dwc_peek;
        }
      }
      ...
      *y += consume;
      if (consume > 0) {
        ...
        int offset =
            OffsetOfWrappedLine(raw_buffer + prev, consume, line_length, width,
                                _mayHaveDoubleWidthCharacter);
...
- (NSArray *)metadataArray {
  ...
  for (int i = 0; i < cll_entries; i++) {
    [metadataArray addObject:@[
      @(metadata_[i].continuation.code),
      @(metadata_[i].continuation.backgroundColor),
      @(metadata_[i].continuation.bgGreen), @(metadata_[i].continuation.bgBlue),
      @(metadata_[i].continuation.backgroundColorMode),
...
- (NSDictionary *)dictionary {
  NSData *rawBufferData =
      [NSData dataWithBytes:raw_buffer
                     length:[self rawSpaceUsed] * sizeof(screen_char_t)];
  return @{
    kLineBlockRawBufferKey : rawBufferData,
...
- (int)lengthOfLine:(int)lineNumber {
  if (lineNumber == 0) {
    ...
    return cumulative_line_lengths[lineNumber] -
           cumulative_line_lengths[lineNumber - 1];
  }
}
