...
NSString *const iTermWebSocketConnectionLibraryVersionTooOldString =
...
//
// *WARNING*****************************************************************************************
// *WARNING* Think carefully before changing this. It will break existing
// full-environment scripts.*
// *WARNING*****************************************************************************************
//
static NSString *const iTermWebSocketConnectionMinimumPythonLibraryVersion =
    @"0.24";
...
+ (instancetype)
    newWebSocketConnectionForRequest:(NSURLRequest *)request
                          connection:(iTermHTTPConnection *)connection
                              reason:(out NSString *__autoreleasing *)reason {
  if (![request.HTTPMethod isEqualToString:@"GET"]) {
    *reason =
        [NSString stringWithFormat:@"HTTP method %@ not accepted (must be GET)",
                                   request.HTTPMethod];
    return nil;
  }
  ...
  for (NSString *key in requiredValues) {
    if ([headers[key] caseInsensitiveCompare:requiredValues[key]] !=
        NSOrderedSame) {
      *reason = [NSString
          stringWithFormat:@"Header %@ has value <%@> but I require <%@>", key,
                           headers[key], requiredValues[key]];
      return nil;
    }
  }

  NSArray<NSString *> *requiredKeys =
      @[ @"sec-websocket-key", @"sec-websocket-version", @"host", @"origin" ];
  for (NSString *key in requiredKeys) {
    if ([headers[key] length] == 0) {
      *reason = [NSString
          stringWithFormat:@"Empty or missing value for required header %@",
                           key];
      return nil;
    }
  }
  ...
  if (![originURL.host isEqualToString:@"localhost"]) {
    *reason = [NSString
        stringWithFormat:
            @"Origin's host is not localhost: is %@ (string value is %@)",
            originURL.host, headers[@"origin"]];
    return nil;
  }

  NSString *host = headers[@"host"];
  NSInteger colon =
      [host rangeOfString:@":" options:NSBackwardsSearch].location;
  if (colon != NSNotFound) {
    host = [host substringToIndex:colon];
  }
  NSArray<NSString *> *loopbackNames =
      @[ @"localhost", @"127.0.0.1", @"[::1]" ];
  if (![loopbackNames containsObject:host]) {
    *reason = [NSString
        stringWithFormat:
            @"Host header is %@, but must be localhost, 127.0.01, or [::1].",
            host];
    return nil;
  }
  ...
  if ([version integerValue] < kWebSocketVersion) {
    *reason = [NSString
        stringWithFormat:@"sec-websocket-version of %@ is older than %@",
                         version, @(kWebSocketVersion)];
    return nil;
  }
  ...
  if (cookie) {
    if ([[iTermWebSocketCookieJar sharedInstance] consumeCookie:cookie]) {
      ...
      *reason = [NSString
          stringWithFormat:@"x-iterm2-cookie of %@ not recognized", cookie];
      cookie = nil;
    }
  }

  {
    ...
    if (!libver) {
      *reason = @"The “x-iterm2-library-version” header was not set. You may "
                @"need to update your Python runtime to a newer version.";
      return nil;
    }
    ...
    if (parts.count != 2) {
      *reason = [NSString
          stringWithFormat:
              @"The “x-iterm2-library-version” header was malformed: ”%@”",
              libver];
      return nil;
    }

    NSDictionary *minimums = @{
      @"python" : [NSDecimalNumber
          decimalNumberWithString:
              iTermWebSocketConnectionMinimumPythonLibraryVersion]
    };
    NSString *name = parts[0];
    NSDecimalNumber *min = minimums[name];
    NSDecimalNumber *version =
        [NSDecimalNumber decimalNumberWithString:parts[1]];
    NSComparisonResult result = [min compare:version];
    if (result == NSOrderedDescending) {
      *reason = [NSString
          stringWithFormat:@"%@. %@ library version reported as %@. Minimum "
                           @"supported by this version of iTerm2 is %@",
                           iTermWebSocketConnectionLibraryVersionTooOldString,
                           name, version, min];
      return nil;
    }
  }
  ...
  if (conn) {
    ...
    conn->_key = [[iTermAPIConnectionIdentifierController sharedInstance]
...
- (void)handleRequest:(NSURLRequest *)request
  ...
  dispatch_async(_queue, ^{
    [self reallyHandleRequest:request
                   completion:^{
...
// queue
- (void)reallyHandleRequest:(NSURLRequest *)request
                 completion:(void (^)(void))completion {
  DLog(@"Handling websocket request %@", request);
  ITAssertWithMessage(_state == iTermWebSocketConnectionStateConnecting,
                      @"Request already handled");

  [self
      sendUpgradeResponseWithKey:request
                                     .allHTTPHeaderFields[@"sec-websocket-key"]
                         version:[request.allHTTPHeaderFields
...
- (void)didUpgradeSuccessfully:(BOOL)upgradeOK {
  ...
  // I tried using dispatch_read but it didn't work reliably. Seems to work OK
  // for writing.
...
- (void)sendData:(NSData *)data {
  dispatch_data_t dispatchData =
      dispatch_data_create(data.bytes, data.length, _queue, ^{
        DLog(@"Disposing of data %p", data);
        [data length]; // Keep a reference to data
      });

  __weak __typeof(self) weakSelf = self;
  dispatch_io_write(_channel, 0, dispatchData, _queue,
                    ^(bool done, dispatch_data_t _Nullable data, int error) {
                      DLog(@"Write progress: done=%d error=%d", (int)done,
                           (int)error);
                      if (error) {
                        [weakSelf reallyAbort];
                      }
  ...
  case iTermWebSocketOpcodeText:
    if (_state == iTermWebSocketConnectionStateOpen) {
      if (frame.fin) {
        ...
        DLog(@"Already have a fragmented frame started. Opcode should have "
             @"been Continuation");
        [self reallyAbort];
      }
    } else {
  ...
  if (CC_SHA1([data bytes], [data length], hash)) {
    ...
    NSDictionary<NSString *, NSString *> *headers = @{
      ...
      @"Sec-WebSocket-Accept" :
          [sha1 stringWithBase64EncodingWithLineBreak:@""],
      @"Sec-WebSocket-Protocol" : kProtocolName,
    ...
    dispatch_async(_connection.queue, ^{
      BOOL result =
          [self->_connection sendResponseWithCode:101
