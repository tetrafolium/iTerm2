...
- (void)awakeFromNib {
  ...
  info.observer = ^() {
    ...
    if (unlimited) {
      ...
      strongSelf->_numScrollbackLines.intValue =
          [strongSelf intForKey:KEY_SCROLLBACK_LINES];
    }
  };
  ...
  info.observer = ^() {
    ...
    [strongSelf->_xtermMouseReportingAllowMouseWheel
  ...
  [self addViewToSearchIndex:_filterAlertsButton
                 displayName:@"Filter alerts"
                     phrases:@[
                       @"bell", @"idle", @"session ended", @"new output"
                     ]
                         key:nil];
}
...
- (void)populateEncodings {
  ...
  for (NSNumber *anEncoding in [self sortedEncodings]) {
    NSString *theTitle =
        [NSString localizedNameOfStringEncoding:[anEncoding unsignedIntValue]];
...
- (NSArray *)sortedEncodings {
  ...
  for (encodingPtr = [NSString availableStringEncodings]; *encodingPtr;
       ++encodingPtr) {
