...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p s=%@ prefix=%@ score=%f hitMult=%f>",
                                 [self class], self, _mainValue, _prefix,
                                 _score, _hitMultiplier];
}
...
- (NSComparisonResult)compare:(id)otherObject {
  return [[NSNumber numberWithDouble:_score]
      compare:[NSNumber numberWithDouble:[otherObject score]]];
}
