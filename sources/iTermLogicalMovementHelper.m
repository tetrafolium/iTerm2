...
- (VT100GridAbsCoordRange)
    moveSelectionEndpoint:(PTYTextViewSelectionEndpoint)endpoint
  ...
  if (![self unitIsValid:unit]) {
    XLog(
        @"ERROR: Unrecognized unit enumerated value %@, treating as character.",
        @(unit));
  ...
  if (!sub) {
    ...
    existingRange = VT100GridAbsWindowedRangeMake(
        VT100GridAbsCoordRangeMake(coord.x, coord.y, coord.x, coord.y),
        columnWindow.location, columnWindow.length);
  } else {
  ...
  VT100GridAbsWindowedRange newRange =
      [self absRangeByExtendingRange:existingRange
                            endpoint:endpoint
                           direction:direction
                           extractor:_textExtractor
                                unit:unit];

  // Convert the mode into an iTermSelectionMode. Only a subset of
  // iTermSelectionModes are possible which is why this uses its own enum.
  ...
  if (!sub) {
    ...
    [_selection endLiveSelection];
  } else if ([_selection absCoord:newRange.coordRange.start
                 isBeforeAbsCoord:newRange.coordRange.end]) {
    // Is a valid range
    [_selection setLastAbsRange:newRange mode:mode];
  } else {
    // Select a single character if the range is empty or flipped. This lets you
    // move the selection around like a cursor.
    switch (endpoint) {
    case kPTYTextViewSelectionEndpointStart: {
      newRange.coordRange.end = [_textExtractor
          successorOfAbsCoordSkippingContiguousNulls:newRange.coordRange.start];
      break;
    }
    case kPTYTextViewSelectionEndpointEnd:
      newRange.coordRange.start = [_textExtractor
          predecessorOfAbsCoordSkippingContiguousNulls:newRange.coordRange.end];
      break;
    }
    [_selection setLastAbsRange:newRange mode:mode];
  }
  ...
  case kPTYTextViewSelectionEndpointEnd:
    start = end;
        ...
        kExtraLinesToMakeVisible + 1; // plus one because of the excess region
    break;
  }

  return VT100GridAbsCoordRangeMake(range.start.x, start, range.end.x, end);
}

- (VT100GridAbsWindowedRange)
    absRangeByMovingStartOfRange:(VT100GridAbsWindowedRange)existingRange
              toTopWithExtractor:(iTermTextExtractor *)extractor {
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.start =
      VT100GridAbsCoordMake(0, _totalScrollbackOverflow);
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
  ...
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.start.x =
      [extractor lengthOfAbsLine:newRange.coordRange.start.y];
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
  ...
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.start.x =
      [extractor startOfIndentationOnAbsLine:existingRange.coordRange.start.y];
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
  ...
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.start.y =
      MAX(_totalScrollbackOverflow, existingRange.coordRange.start.y - 1);
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
  ...
  newRange.coordRange.start.y =
      MIN(maxY - 1, existingRange.coordRange.start.y + 1);
  return newRange;
}

- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRange:(VT100GridAbsWindowedRange)existingRange
            toTopWithExtractor:(iTermTextExtractor *)extractor {
  VT100GridAbsWindowedRange newRange = existingRange;
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRange:(VT100GridAbsWindowedRange)existingRange
         toBottomWithExtractor:(iTermTextExtractor *)extractor {
  VT100GridAbsWindowedRange newRange = existingRange;
  const long long maxY = MAX(_totalScrollbackOverflow,
                             _totalScrollbackOverflow + _numberOfLines - 1);
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRange:(VT100GridAbsWindowedRange)existingRange
      toEndOfLineWithExtractor:(iTermTextExtractor *)extractor {
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.end.x =
      [extractor lengthOfAbsLine:newRange.coordRange.end.y];
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
  ...
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.end.x =
      [extractor startOfIndentationOnAbsLine:existingRange.coordRange.end.y];
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRange:(VT100GridAbsWindowedRange)existingRange
               upWithExtractor:(iTermTextExtractor *)extractor {
  VT100GridAbsWindowedRange newRange = existingRange;
  newRange.coordRange.end.y =
      MAX(_totalScrollbackOverflow, existingRange.coordRange.end.y - 1);
  return newRange;
}
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingStartOfRangeBack:(VT100GridAbsWindowedRange)existingRange
                           extractor:(iTermTextExtractor *)extractor
                                unit:(PTYTextViewSelectionExtensionUnit)unit {
  VT100GridAbsCoord coordBeforeStart =
      [extractor predecessorOfAbsCoordSkippingContiguousNulls:
                     VT100GridAbsWindowedRangeStart(existingRange)];
  switch (unit) {
  ...
  case kPTYTextViewSelectionExtensionUnitWord: {
    VT100GridAbsWindowedRange rangeWithWordBeforeStart =
        [extractor rangeForWordAtAbsCoord:coordBeforeStart
  ...
  case kPTYTextViewSelectionExtensionUnitBigWord: {
    VT100GridAbsWindowedRange rangeWithWordBeforeStart =
        [extractor rangeForBigWordAtAbsCoord:coordBeforeStart
                               maximumLength:kLongMaximumWordLength];
    rangeWithWordBeforeStart.coordRange.end = existingRange.coordRange.end;
    rangeWithWordBeforeStart.columnWindow = existingRange.columnWindow;
    return rangeWithWordBeforeStart;
  ...
  case kPTYTextViewSelectionExtensionUnitLine: {
    ...
    if (rangeWithLineBeforeStart.coordRange.start.y >
        _totalScrollbackOverflow) {
      if (rangeWithLineBeforeStart.coordRange.start.x >
          rangeWithLineBeforeStart.columnWindow.location) {
        rangeWithLineBeforeStart.coordRange.start.x =
            rangeWithLineBeforeStart.columnWindow.location;
      } else {
  ...
  case kPTYTextViewSelectionExtensionUnitMark: {
    ...
    if (rangeWithLineBeforeStart.coordRange.start.y >
        _totalScrollbackOverflow) {
      long long previousMark =
          [self absoluteLineNumberOfMarkBeforeAbsLine:existingRange.coordRange
                                                          .start.y];
      if (previousMark != -1) {
        ...
        if (rangeWithLineBeforeStart.coordRange.start.y ==
          ...
          previousMark = [self
              absoluteLineNumberOfMarkBeforeAbsLine:existingRange.coordRange
                                                        .start.y -
                                                    1];
          if (previousMark != -1) {
            rangeWithLineBeforeStart.coordRange.start.y = previousMark + 1;
      ...
      rangeWithLineBeforeStart.coordRange.start.x =
          existingRange.columnWindow.location;
    }
    return rangeWithLineBeforeStart;
  }
  }
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingStartOfRangeForward:(VT100GridAbsWindowedRange)existingRange
                              extractor:(iTermTextExtractor *)extractor
                                   unit:
                                       (PTYTextViewSelectionExtensionUnit)unit {
  const VT100GridAbsCoord coordAfterStart = [extractor
      successorOfAbsCoordSkippingContiguousNulls:VT100GridAbsWindowedRangeStart(
                                                     existingRange)];
  switch (unit) {
  case kPTYTextViewSelectionExtensionUnitCharacter: {
  ...
  case kPTYTextViewSelectionExtensionUnitWord: {
    VT100GridAbsCoord startCoord =
        VT100GridAbsWindowedRangeStart(existingRange);
    const BOOL startWasOnNull =
        [extractor characterAtAbsCoord:startCoord].code == 0;
    ...
    rangeExcludingWordAtStart.coordRange.start =
        [extractor rangeForWordAtAbsCoord:startCoord
                            maximumLength:kLongMaximumWordLength]
            .coordRange.end;
    // If the start of range moved from a null to a null, skip to the end of the
    // line or past all the nulls.
    if (startWasOnNull &&
        [extractor
            characterAtAbsCoord:rangeExcludingWordAtStart.coordRange.start]
                .code == 0) {
      rangeExcludingWordAtStart.coordRange.start = [extractor
          successorOfAbsCoordSkippingContiguousNulls:rangeExcludingWordAtStart
                                                         .coordRange.start];
    }
  ...
  case kPTYTextViewSelectionExtensionUnitLine: {
    ...
    rangeExcludingFirstLine.coordRange.start.x =
        existingRange.columnWindow.location;
  ...
  case kPTYTextViewSelectionExtensionUnitMark: {
    ...
    const long long nextMark =
        [self absoluteLineNumberOfMarkAfterAbsLine:rangeExcludingFirstLine
                                                       .coordRange.start.y -
                                                   1];
    if (nextMark != -1) {
      rangeExcludingFirstLine.coordRange.start.y =
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRangeBack:(VT100GridAbsWindowedRange)existingRange
                         extractor:(iTermTextExtractor *)extractor
                              unit:(PTYTextViewSelectionExtensionUnit)unit {
  const VT100GridAbsCoord coordBeforeEnd = [extractor
      predecessorOfAbsCoordSkippingContiguousNulls:VT100GridAbsWindowedRangeEnd(
                                                       existingRange)];
  switch (unit) {
  case kPTYTextViewSelectionExtensionUnitCharacter: {
  ...
  case kPTYTextViewSelectionExtensionUnitLine: {
    ...
    if (existingRange.coordRange.end.x > existingRange.columnWindow.location) {
      rangeExcludingLastLine.coordRange.end.x =
          existingRange.columnWindow.location;
    } else {
      rangeExcludingLastLine.coordRange.end.x =
          existingRange.columnWindow.location;
      rangeExcludingLastLine.coordRange.end.y =
          MAX(1 + _totalScrollbackOverflow, existingRange.coordRange.end.y - 1);
    }
  ...
  case kPTYTextViewSelectionExtensionUnitMark: {
    ...
    long long n =
        [self absoluteLineNumberOfMarkBeforeAbsLine:rangeExcludingLastLine
                                                        .coordRange.end.y +
                                                    1];
    if (n != -1) {
      rangeExcludingLastLine.coordRange.end.y =
          MAX(1 + _totalScrollbackOverflow, n - 1);
    }
    return rangeExcludingLastLine;
  }
  }
...
- (VT100GridAbsWindowedRange)
    absRangeByMovingEndOfRangeForward:(VT100GridAbsWindowedRange)existingRange
                            extractor:(iTermTextExtractor *)extractor
                                 unit:(PTYTextViewSelectionExtensionUnit)unit {
  const VT100GridAbsCoord endCoord =
      VT100GridAbsWindowedRangeEnd(existingRange);
  ...
  case kPTYTextViewSelectionExtensionUnitWord: {
    ...
    if (endCoord.x > VT100GridRangeMax(existingRange.columnWindow)) {
      rangeWithWordAfterEnd =
          [extractor rangeForWordAtAbsCoord:coordAfterEnd
                              maximumLength:kLongMaximumWordLength];
    } else {
      rangeWithWordAfterEnd =
          [extractor rangeForWordAtAbsCoord:endCoord
                              maximumLength:kLongMaximumWordLength];
    }
  ...
  case kPTYTextViewSelectionExtensionUnitBigWord: {
    ...
    if (endCoord.x > VT100GridRangeMax(existingRange.columnWindow)) {
      rangeWithWordAfterEnd =
          [extractor rangeForBigWordAtAbsCoord:coordAfterEnd
                                 maximumLength:kLongMaximumWordLength];
    } else {
      rangeWithWordAfterEnd =
          [extractor rangeForBigWordAtAbsCoord:endCoord
                                 maximumLength:kLongMaximumWordLength];
    }
    rangeWithWordAfterEnd.coordRange.start = existingRange.coordRange.start;
    rangeWithWordAfterEnd.columnWindow = existingRange.columnWindow;
  ...
  case kPTYTextViewSelectionExtensionUnitMark: {
    ...
    rangeWithLineAfterEnd.coordRange.end.x = rightMargin;
    const long long nextMark =
        [self absoluteLineNumberOfMarkAfterAbsLine:rangeWithLineAfterEnd
                                                       .coordRange.end.y];
    if (nextMark != -1) {
      rangeWithLineAfterEnd.coordRange.end.y =
          MIN(_numberOfLines + _totalScrollbackOverflow, nextMark - 1);
    }
    if (rangeWithLineAfterEnd.coordRange.end.y ==
...
- (VT100GridAbsWindowedRange)
    absRangeByExtendingRange:(VT100GridAbsWindowedRange)existingRange
...
- (iTermSelectionMode)selectionModeForExtensionUnit:
    (PTYTextViewSelectionExtensionUnit)unit {
  switch (unit) {
  case kPTYTextViewSelectionExtensionUnitCharacter:
