...
// Timer period between updates when adaptive frame rate is enabled and
// throughput is low but not 0.
...
@implementation iTermUpdateCadenceController {
  ...
  // This timer fires periodically to redraw textview, update the scroll
  // position, tab appearance, etc.
  NSTimer *_updateTimer;

  // This is the experimental GCD version of the update timer that seems to have
  // more regular refreshes.
  ...
  // Timer period between updates when active (not idle, tab is visible or title
  // bar is changing, etc.)
...
- (instancetype)initWithThroughputEstimator:
    (iTermThroughputEstimator *)throughputEstimator {
  self = [super init];
  if (self) {
    ...
    _activeUpdateCadence =
        1.0 / MAX(1, [iTermAdvancedSettingsModel activeUpdateCadence]);
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p delegate=%@ active=%@ cadence=%@>",
                                 NSStringFromClass([self class]), self,
                                 self.delegate, @(self.isActive), @(_cadence)];
}
...
- (void)didHandleInput {
  ...
  const NSInteger estimatedThroughput =
      [_throughputEstimator estimatedThroughput];
...
- (void)changeCadenceIfNeeded:(BOOL)force {
  ...
  DLog(@"%@ state: active=%@, idle=%@, visible=%@, useAdaptiveFrameRate=%@, "
       @"adaptiveFrameRateThroughputThreshold=%@, slowFrameRate=%@, "
       @"liveResizing=%@",
       self, @(state.active), @(state.idle), @(state.visible),
       @(state.useAdaptiveFrameRate),
       @(state.adaptiveFrameRateThroughputThreshold), @(state.slowFrameRate),
       @(state.liveResizing));

  // state.active means that it needs periodic redraws OR the tab label is
  // changing. idle means no input has been received on the PTY in a while (3
  // seconds by default). assignment to self.isActive is used to update whether
  // Metal is in use, when it's disabled while idle.
  self.isActive = (state.active || !state.idle);

  if (!self.isActive) {
    // Periodic redraws not needed (i.e., nothing is blinking) and the session
    // is idle. It doesn't matter if the app itself is active because there's
    // nothing to do so use the background update cadence.
  ...
  if (!state.visible) {
    // Although self.isActive is true, the session is not visible so there's no
    // point redrawing it.
  ...
  if (!state.useAdaptiveFrameRate) {
    // The session is visible and self.active is true (it needs redraws or it's
    // not idle).
    DLog(@"select active update cadence");
    [self setUpdateCadence:_activeUpdateCadence
              liveResizing:state.liveResizing
                     force:force];
  }
  ...
  if (estimatedThroughput < kThroughputLimit && estimatedThroughput > 0) {
    ...
    [self setUpdateCadence:1.0 / state.slowFrameRate
              liveResizing:state.liveResizing
                     force:force];
  }
}
...
- (void)setUpdateCadence:(NSTimeInterval)cadence
            liveResizing:(BOOL)liveResizing
                   force:(BOOL)force {
  if (_useGCDUpdateTimer) {
...
- (void)setTimerUpdateCadence:(NSTimeInterval)cadence
  ...
  if (liveResizing) {
    ...
    // I'm worried about the possible side effects it might have since there's
    // no way to know all the tracking event loops.
    [_updateTimer invalidate];
    ...
    if (!force && _updateTimer && cadence > _updateTimer.timeInterval) {
      ...
      [_updateTimer invalidate];
      _updateTimer =
          [NSTimer scheduledWeakTimerWithTimeInterval:cadence
                                               target:self
                                             selector:@selector(updateDisplay)
                                             userInfo:nil
                                              repeats:YES];
...
- (void)setGCDUpdateCadence:(NSTimeInterval)cadence
  ...
  _gcdUpdateTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,
                                           dispatch_get_main_queue());
  dispatch_source_set_timer(
      _gcdUpdateTimer, dispatch_time(DISPATCH_TIME_NOW, period * NSEC_PER_SEC),
      period * NSEC_PER_SEC, 0.0005 * NSEC_PER_SEC);
...
- (void)maybeUpdateDisplay {
  if ([iTermWarning showingWarning] || [NSApp modalWindow] ||
      [self.delegate updateCadenceControllerWindowHasSheet]) {
    return;
  }
  [self updateDisplay];
}
