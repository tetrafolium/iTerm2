...
- (BOOL)respondsToSelector:(SEL)aSelector {
  if ([NSStringFromSelector(aSelector)
          isEqualToString:NSStringFromSelector(@selector
                                               (weaklyReferencedObject))]) {
    return YES;
  } else {
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector {
  ...
  // Prefer to use the object's class in case it got dynamically changed, but if
  // the object has already been deallocated used its cached class since we need
  // to provide a non-nil signature.
