...
@interface iTermDependencyEditorWindowController () <NSTableViewDataSource,
                                                     NSTableViewDelegate>
...
+ (instancetype)sharedInstance {
  ...
  dispatch_once(&onceToken, ^{
    instance = [[self alloc]
        initWithWindowNibName:@"iTermDependencyEditorWindowController"];
  });
  return instance;
}
...
- (void)fetchVersionOfPackage:(NSString *)packageName
                   completion:(void (^)(BOOL ok, NSString *result))completion {
...
- (void)loadPackageAtIndex:(NSInteger)index
  ...
  [self fetchVersionOfPackage:tuple.firstObject
                   completion:^(BOOL ok, NSString *result) {
                     const BOOL accepted =
                         [weakSelf didFetchVersionOfPackageAtIndex:index
                                                     originalTuple:tuple
                                                         container:container
                                                                ok:ok
                                                            result:result];
...
- (BOOL)didFetchVersionOfPackageAtIndex:(NSInteger)index
  ...
  if (!ok) {
    ...
    DLog(@"Error running pip3 show %@ in %@: %@", tuple.firstObject,
  ...
  [self->_tableView
      reloadDataForRowIndexes:[NSIndexSet indexSetWithIndex:index]
                columnIndexes:[NSIndexSet
                                  indexSetWithIndex:1]]; // Index 1 is version
  [self->_tableView endUpdates];
  return YES;
}

- (NSString *)versionInPipOutput:(NSData *)output {
  NSString *string = [[NSString alloc] initWithData:output
                                           encoding:NSUTF8StringEncoding];
  NSArray<NSString *> *lines = [string componentsSeparatedByString:@"\n"];
  NSString *const versionPrefix = @"Version: ";
  NSString *versionLine =
      [lines filteredArrayUsingBlock:^BOOL(NSString *aLine) {
  ...
  _packageTuples = [[parser.dependencies mapWithBlock:^id(NSString *dep) {
    ...
    return tuple;
      ...
      sortedArrayUsingComparator:^NSComparisonResult(
          iTermTuple *_Nonnull tuple1, iTermTuple *_Nonnull tuple2) {
        return [tuple1.firstObject compare:tuple2.firstObject];
      }];
  [self
      loadPythonVersionsSelecting:parser.pythonVersion.it_twoPartVersionNumber];
...
- (void)loadPythonVersionsSelecting:(NSString *)selectedVersion {
  NSString *const env =
      [[_selectedScriptPath stringByAppendingPathComponent:@"iterm2env"]
          stringByAppendingPathComponent:@"versions"];
  _pythonVersion =
      selectedVersion ?: [iTermPythonRuntimeDownloader bestPythonVersionAt:env];
  NSArray<NSString *> *versions = [[[[iTermPythonRuntimeDownloader
      pythonVersionsAt:env] mapWithBlock:^id(NSString *version) {
  ...
  [_pythonVersionButton
      selectItemAtIndex:[versions indexOfObject:_pythonVersion]];
}
...
- (void)populateScripts {
  ...
  [self addScriptItems:[itad.scriptsMenuController scriptItems]
           breadcrumbs:@[]];
}

- (void)addScriptItems:(NSArray<iTermScriptItem *> *)scriptItems
           breadcrumbs:(NSArray<NSString *> *)breadcrumbs {
  for (iTermScriptItem *item in [scriptItems
    ...
    if (item.isFolder) {
      [self addScriptItems:item.children
               breadcrumbs:[breadcrumbs arrayByAddingObject:item.name]];
      continue;
    }
    ...
    NSMenuItem *menuItem = [[NSMenuItem alloc]
        initWithTitle:[[breadcrumbs arrayByAddingObject:item.name]
...
- (void)pip3UninstallDidFinish:(iTermTuple<NSString *, NSString *> *)tuple
  ...
  __weak __typeof(self) weakSelf = self;
  [self
      fetchVersionOfPackage:tuple.firstObject
                 completion:^(BOOL ok, NSString *result) {
                   [weakSelf
                       uninstallDidFetchPackageVersionSuccessfully:ok
                                                           package:
                                                               tuple.firstObject
                                                selectedScriptPath:
                                                    selectedScriptPath];
...
- (void)uninstallDidFetchPackageVersionSuccessfully:(BOOL)ok
  ...
  NSString *path =
      [selectedScriptPath stringByAppendingPathComponent:@"setup.cfg"];
  iTermSetupCfgParser *parser = [[iTermSetupCfgParser alloc] initWithPath:path];
  [iTermSetupCfgParser
      writeSetupCfgToFile:path
                     name:parser.name
...
- (NSString *)requestDependencyName {
  ...
  do {
    ...
    newDependencyName = [textField.stringValue
...
- (void)runPip3WithArguments:(NSArray<NSString *> *)arguments
                  completion:(void (^)(void))completion {
  NSURL *container = [NSURL fileURLWithPath:_selectedScriptPath];
  NSString *pip3 = [[iTermPythonRuntimeDownloader sharedInstance]
             pip3At:[container.path stringByAppendingPathComponent:@"iterm2env"]
      pythonVersion:_pythonVersion];
  NSString *command = [[pip3
  ...
  iTermWarningSelection selection =
      [iTermWarning showWarningWithTitle:command
                                 actions:@[ @"OK", @"Cancel" ]
                               accessory:nil
  ...
  [[iTermController sharedInstance]
      openSingleUseWindowWithCommand:pip3
                           arguments:arguments
                              inject:nil
                         environment:nil
                                 pwd:nil
                             options:
                                 iTermSingleUseWindowOptionsCommandNotSwiftyString
                      didMakeSession:nil
                          completion:^{
                            completion();
                          }];
...
- (void)pip3InstallDidFinish:(NSString *)selectedScriptPath
...
- (void)uninstallDidFailForPackage:(NSString *)package {
  [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:@"Uninstall of %@ failed. Check the "
                                     @"output of pip3 for errors.",
                                     package]
                   actions:@[ @"OK" ]
                 accessory:nil
  ...
  [iTermSetupCfgParser
      writeSetupCfgToFile:path
             ...
             dependencies:[parser.dependencies
                              arrayByAddingObject:newDependencyName]
      ensureiTerm2Present:NO
            pythonVersion:parser.pythonVersion
       environmentVersion:parser.minimumEnvironmentVersion];
...
- (IBAction)checkForUpdates:(id)sender {
  ...
  iTermTuple<NSString *, NSString *> *tuple =
      _packageTuples[_tableView.selectedRow];
  NSString *selectedDependencyName = tuple.firstObject;
  __weak __typeof(self) weakSelf = self;
  [self
      runPip3WithArguments:@[ @"install", selectedDependencyName, @"--upgrade" ]
                completion:^{
...
- (void)install:(NSString *)newDependencyName
  ...
  [self runPip3WithArguments:@[ @"install", newDependencyName ]
                  completion:^{
                    [weakSelf pip3InstallDidFinish:selectedScriptPath
                                 newDependencyName:newDependencyName];
                    if (completion) {
                      completion();
                    }
...
- (IBAction)remove:(id)sender {
  ...
  [self runPip3WithArguments:@[ @"uninstall", selectedDependencyName ]
                  completion:^{
                    [weakSelf pip3UninstallDidFinish:tuple
                                               index:index
                                  selectedScriptPath:selectedScriptPath];
                  }];
}
...
- (IBAction)pythonVersionChanged:(id)sender {
  ...
  if ([comparator compareVersion:selectedVersion
                       toVersion:_pythonVersion] == NSOrderedAscending) {
    iTermWarning *warning = [[iTermWarning alloc] init];
    warning.title = @"You have asked to downgrade to an older Python version. "
                    @"Dependencies will need to be reinstalled. This may go "
                    @"badly. Are you sure you want to do this?";
    ...
    iTermWarning *warning = [[iTermWarning alloc] init];
    warning.title =
        @"You have asked to upgrade to a newer Python version. Dependencies "
        @"will need to be reinstalled. OK to continue?";
  ...
  NSString *path =
      [_selectedScriptPath stringByAppendingPathComponent:@"setup.cfg"];
  iTermSetupCfgParser *parser = [[iTermSetupCfgParser alloc] initWithPath:path];
...
- (void)installPackages:(NSArray<NSString *> *)packages
     selectedScriptPath:(NSString *)selectedScriptPath {
  if (packages.count == 0) {
    return;
  }
  ...
  [self install:newDependencyName
      selectedScriptPath:selectedScriptPath
              completion:^{
                [self installPackages:[packages subarrayFromIndex:1]
                    selectedScriptPath:selectedScriptPath];
              }];
}
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  if (!view) {
    ...
    NSTextField *textField =
        [NSTextField it_textFieldForTableViewWithIdentifier:identifier];
