...
- (NSString *)triggerOptionalParameterPlaceholderWithInterpolation:
    (BOOL)interpolation {
  return [self
      triggerOptionalDefaultParameterValueWithInterpolation:interpolation];
}
      ...
      paramWithBackreferencesReplacedWithValues:capturedStrings
                               ...
                               useInterpolation:useInterpolation
                                     completion:^(NSString *urlString) {
                                       [self
                                           performActionWithURLString:urlString
                                                                range:
                                                                    rangeInString
                                                              session:aSession
                                                   absoluteLineNumber:
...
- (void)performActionWithURLString:(NSString *)urlString
  ...
  unsigned short code = [[iTermURLStore sharedInstance] codeForURL:url
                                                        withParams:@""];
  ...
  iTermURLMark *mark =
      [aSession.screen addMarkStartingAtAbsoluteLine:lineNumber
                                             oneLine:YES
