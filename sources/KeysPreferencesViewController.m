...
- (void)awakeFromNib {
  ...
  info.onChange = ^() {
    ...
    [weakSelf
        ensureUniquenessOfModifierForButton:strongSelf
                                                ->_switchPaneModifierButton
                                  inButtons:@[
                                    strongSelf->_switchTabModifierButton,
  ...
  info.onChange = ^() {
    ...
    [weakSelf
        ensureUniquenessOfModifierForButton:strongSelf->_switchTabModifierButton
                                  inButtons:@[
...
- (void)updateDuplicateWarning {
  NSArray<iTermHotKeyDescriptor *> *descriptors =
      [[iTermHotKeyController sharedInstance]
          descriptorsForProfileHotKeysExcept:nil];
  _shortcutOverloaded.hidden =
      ![descriptors containsObject:[self hotkeyDescriptor]];
}
...
- (void)generateHotkeyWindowProfile {
  ...
  if (profileHotKeys.count > 0) {
    NSArray<NSString *> *names =
        [profileHotKeys mapWithBlock:^id(iTermProfileHotKey *profileHotKey) {
          return [NSString
              stringWithFormat:@"“%@”", profileHotKey.profile[KEY_NAME]];
        }];
    ...
    if (profileHotKeys.count == 1) {
      namesSentence = [NSString
          stringWithFormat:
              @"You already have a Profile with a Hotkey Window named %@",
              joinedNames];
      ...
      namesSentence = [NSString
          stringWithFormat:
              @"You already have Profiles with Hotkey Windows named %@",
              joinedNames];
    }
    namesSentence = [namesSentence stringByInsertingTerminalPunctuation:@"."];

    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:[NSString stringWithFormat:@"%@", namesSentence]
                     actions:actions
                   accessory:nil
    ...
    if (selection == cancel) {
      ...
      [[PreferencePanel sharedInstance]
          configureHotkeyForProfile:[profileHotKeys.firstObject profile]];
    }
  }
  ...
  [_hotkeyPanel
      setExplanation:
          @"This panel helps you configure a new profile that will be bound to "
          @"a keystroke you assign. Pressing the hotkey (even when iTerm2 is "
          @"not active) will toggle a special window."];
  ...
  [self.view.window
             beginSheet:_hotkeyPanel.window
      completionHandler:^(NSModalResponse returnCode) {
        if (returnCode == NSModalResponseOK) {
          ...
          NSMutableDictionary *dict =
              [[[ProfileModel sharedInstance] defaultBookmark] mutableCopy];
          ...
          NSInteger number = 1;
          while (
              [[ProfileModel sharedInstance] bookmarkWithName:newProfileName]) {
            newProfileName =
          ...
          NSMutableDictionary *keyboardMap =
              [dict[KEY_KEYBOARD_MAP] ?: @{} mutableCopy];
          iTermKeyBindingAction *action =
              [iTermKeyBindingAction withAction:KEY_ACTION_NEW_TAB_WITH_PROFILE
                                      parameter:dict[KEY_GUID]];
          iTermKeystroke *keystroke =
              [iTermKeystroke withCharacter:'t'
          ...
          [model.dictionaryValue enumerateKeysAndObjectsUsingBlock:^(
                                     NSString *_Nonnull key, id _Nonnull obj,
                                     BOOL *_Nonnull stop) {
          ...
          alert.informativeText = [NSString
              stringWithFormat:@"A new profile called “%@” was created for "
                               @"you. It is tuned to work well "
                               @"for the Hotkey Window feature and it can be "
                               @"customized in the Profiles tab.",
                               newProfileName];
...
- (void)hotkeyEnabledDidChange {
  if ([iTermPreferences boolForKey:kPreferenceKeyHotkeyEnabled]) {
    // Hotkey was enabled but might be unassigned; give it a default value if
    // needed.
    int theChar = [iTermPreferences intForKey:kPreferenceKeyHotkeyCharacter];
    int modifiers = [iTermPreferences intForKey:kPreferenceKeyHotkeyModifiers];
    int code = [iTermPreferences intForKey:kPreferenceKeyHotKeyCode];
...
- (void)updateHotkeyViews {
  ...
  if (code || theChar) {
    iTermKeystroke *keystroke =
        [[iTermKeystroke alloc] initWithVirtualKeyCode:code
                                         modifierFlags:modifiers
                                             character:theChar];
    _hotkeyField.stringValue =
        [iTermKeystrokeFormatter stringForKeystroke:keystroke];
  } else {
    _hotkeyField.stringValue = @"";
  }
...
// Set the local copy of the hotkey, update the pref panel, and register it
// after a delay.
...
- (void)postModifierChangedNotification {
  NSDictionary *userInfo = @{
    kPSMTabModifierKey : @([iTermPreferences
        maskForModifierTag:[iTermPreferences
                               intForKey:kPreferenceKeySwitchTabModifier]])
  };
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kPSMModifierChangedNotification
                    object:nil
                  userInfo:userInfo];
}
...
- (IBAction)emulateUsKeyboardHelp:(id)sender {
  ...
  alert.informativeText =
      @"Some keyboard layouts (such as AZERTY) require a modifier to press a "
      @"number key. This causes problems for switching to a window, tab, or "
      @"split pane by pressing modifier+number: you might need other modifiers "
      @"or conflicting modifiers. When “Emulate US Keyboard” is enabled, you "
      @"can press the configured modifier plus the key on the top row that "
      @"corresponds to a number key on a US keyboard. For example, on AZERTY, "
      @"the & key would act as the 1 key.";
  [alert runModal];
}
...
- (BOOL)anyProfileHasMappingForKeystroke:(iTermKeystroke *)keystroke {
  for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
    if ([iTermKeyMappings haveKeyMappingForKeystroke:keystroke
                                           inProfile:profile]) {
      return YES;
    }
  }
...
- (BOOL)warnAboutPossibleOverride {
  switch ([iTermWarning
      showWarningWithTitle:@"The global keyboard shortcut you have set is "
                           @"overridden by at least one profile. "
                           @"Check your profiles’ keyboard settings if it "
...
- (BOOL)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  NSSet<iTermKeystroke *> *keystrokesInGlobalMapping =
      [iTermKeyMappings keystrokesInGlobalMapping];
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  [item
      whenFirst:^(iTermKeystroke *_Nonnull keystroke) {
        NSMutableDictionary *dict =
            [[iTermKeyMappings globalKeyMap] mutableCopy];
      ...
      second:^(iTermTouchbarItem *_Nonnull touchbarItem) {
        NSMutableDictionary *dict =
            [[iTermTouchbarMappings globalTouchBarMap] mutableCopy];
...
- (void)maybeExplainHowToEditTouchBarControls {
  ...
  [[iTermNotificationController sharedInstance]
               notify:@"Touch Bar Item Added"
      withDescription:@"Select View > Customize Touch Bar to enable your new "
                      @"touch bar item."];
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  [keystrokes enumerateObjectsUsingBlock:^(iTermKeystroke *_Nonnull keystroke,
                                           BOOL *_Nonnull stop) {
    NSUInteger index =
        [[iTermKeyMappings sortedGlobalKeystrokes] indexOfObject:keystroke];
    assert(index != NSNotFound);
    [iTermKeyMappings
        setGlobalKeyMap:[iTermKeyMappings
                            removeMappingAtIndex:index
                                    inDictionary:[iTermKeyMappings
                                                     globalKeyMap]]];
  }];
  [touchbarItems
      enumerateObjectsUsingBlock:^(iTermTouchbarItem *_Nonnull touchbarItem,
                                   BOOL *_Nonnull stop) {
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  NSSet<iTermKeystroke *> *keystrokesThatWillChange =
      [iTermPresetKeyMappings keystrokesInGlobalPreset:presetName];
  NSSet<iTermKeystroke *> *keystrokesInGlobalMapping =
      [iTermKeyMappings keystrokesInGlobalMapping];
  BOOL replaceAll = YES;
  if (![keystrokesInGlobalMapping isSubsetOfSet:keystrokesThatWillChange]) {
  ...
  [iTermPresetKeyMappings setGlobalKeyMappingsToPreset:presetName
...
- (NSNumber *)removeBeforeLoading:(NSString *)thing {
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString
              stringWithFormat:@"Remove all key mappings before loading %@?",
                               thing]
