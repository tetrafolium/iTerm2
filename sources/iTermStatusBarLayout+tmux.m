...
- (NSArray<NSString *> *)stringVariants {
  ...
  for (NSInteger i = count; i > 0; i--) {
    [variants
        addObject:[[parts subarrayToIndex:i] componentsJoinedByString:@" | "]];
  };
  return variants;
}
...
@interface iTermStatusBarTmuxRightComponent
...
- (NSArray<NSString *> *)stringVariants {
  ...
  for (NSInteger i = 0; i < count; i++) {
    [variants addObject:[[parts subarrayFromIndex:i]
...
+ (NSDictionary *)tmux_configurationWithInterpolatedStringExpression:
    (NSString *)expression {
  NSString *interpolatedString =
...
+ (instancetype)tmuxLayoutWithController:(TmuxController *)controller
  ...
  leftConfiguration = [self tmux_configurationWithInterpolatedStringExpression:
                                iTermVariableKeySessionTmuxStatusLeft];
  rightConfiguration = [self tmux_configurationWithInterpolatedStringExpression:
                                 iTermVariableKeySessionTmuxStatusRight];

  id<iTermStatusBarComponent> leftComponent =
      [[iTermStatusBarTmuxLeftComponent alloc]
          initWithConfiguration:leftConfiguration
                          scope:scope];
  id<iTermStatusBarComponent> rightComponent =
      [[iTermStatusBarTmuxRightComponent alloc]
          initWithConfiguration:rightConfiguration
                          scope:scope];
  id<iTermStatusBarComponent> springComponent =
      [[iTermStatusBarSpringComponent alloc] initWithConfiguration:@{}
                                                             scope:scope];

  NSDictionary *layoutDict = [iTermProfilePreferences
      objectForKey:KEY_STATUS_BAR_LAYOUT
         inProfile:[controller profileForWindow:window]];
  iTermStatusBarLayout *layout =
      [[iTermStatusBarLayout alloc] initWithDictionary:layoutDict scope:scope];
