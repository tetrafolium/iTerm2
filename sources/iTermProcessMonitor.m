...
- (instancetype)initWithQueue:(dispatch_queue_t)queue
                     callback:(void (^)(iTermProcessMonitor *,
                                        dispatch_source_proc_flags_t))callback {
  self = [super init];
  if (self) {
...
- (BOOL)setProcessInfo:(iTermProcessInfo *)processInfo {
  ...
  if (_processInfo == nil) {
    ...
    _source =
        dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, processInfo.processID,
                               DISPATCH_PROC_EXIT | DISPATCH_PROC_FORK |
                                   DISPATCH_PROC_EXEC | DISPATCH_PROC_SIGNAL,
                               _queue);
    if (!_source) {
      [self invalidate];
      return NO;
  ...
  ;
  NSMutableArray<iTermProcessMonitor *> *childrenToRemove =
      [_children mutableCopy];

  [processInfo.children
      enumerateObjectsUsingBlock:^(iTermProcessInfo *_Nonnull childInfo,
...
- (iTermProcessMonitor *)childForProcessInfo:(iTermProcessInfo *)info {
  ...
  return [_children objectPassingTest:^BOOL(iTermProcessMonitor *element,
...
// Called on _queue
- (void)handleEvent {
  const dispatch_source_proc_flags_t flags =
      (dispatch_source_proc_flags_t)dispatch_source_get_data(_source);
...
- (void)invalidate {
  ...
  [children enumerateObjectsUsingBlock:^(iTermProcessMonitor *_Nonnull child,
                                         NSUInteger idx, BOOL *_Nonnull stop) {
    [child invalidate];
  }];
  _processInfo = nil;
