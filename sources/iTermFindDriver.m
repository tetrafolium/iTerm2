...
+ (void)loadUserDefaults {
  NSNumber *mode =
      [[NSUserDefaults standardUserDefaults] objectForKey:@"findMode_iTerm"];
  if (!mode) {
    // Migrate legacy value.
    NSNumber *ignoreCase = [[NSUserDefaults standardUserDefaults]
        objectForKey:@"findIgnoreCase_iTerm"];
    BOOL caseSensitive = ignoreCase ? ![ignoreCase boolValue] : NO;
    BOOL isRegex =
        [[NSUserDefaults standardUserDefaults] boolForKey:@"findRegex_iTerm"];
    ...
    if (caseSensitive && isRegex) {
      ...
      gFindMode = iTermFindModeSmartCaseSensitivity; // Upgrade case-insensitive
                                                     // substring to smart case
                                                     // sensitivity.
    }
  } else {
    // Modern value
...
- (void)userDidEditSearchQuery:(NSString *)updatedQuery
  ...
  // A query becomes stale when it is 1 or 2 chars long and it hasn't been
  // edited in 3 seconds (or the search field has lost focus since the last char
  // was entered).
  static const CGFloat kStaleTime = 3;
  BOOL isStale =
      (([NSDate timeIntervalSinceReferenceDate] - _lastEditTime) > kStaleTime &&
       updatedQuery.length > 0 && [self queryIsShort:updatedQuery]);

  void (^search)(void) = ^{
    [[iTermSearchHistory sharedInstance] addQuery:updatedQuery];
    [self doSearch];
  };
  // This state machine implements a delay before executing short (1 or 2 char)
  // queries. The delay is incurred again when a 5+ char query becomes short.
  // It's kind of complicated so the delay gets inserted at appropriate but
  // minimally annoying times. Plug this into graphviz to see the full state
  // machine:
  //
  // digraph g {
  ...
  case kFindViewDelayStateActiveMedium:
    ...
    // This state intentionally does not transition to ActiveShort. If you
    // backspace over the whole query, the delay must be done again.
    search();
    break;
...
- (void)owningViewDidBecomeFirstResponder {
  ...
  DLog(@"owning view became first responder, needs update on focus, and is "
       @"visible. delegate = %@",
       self.delegate);
  self.needsUpdateOnFocus = NO;
  _savedState = nil;
...
- (void)loadFindStringFromSharedPasteboard:(NSString *)value {
  DLog(@"[%p loadFindStringFromSharedPasteboard:%@] in window with frame %@",
       self, value, NSStringFromRect(_viewController.view.window.frame));
...
- (void)setGlobalMode:(iTermFindMode)set {
  if (!_savedState) {
    gFindMode = set;
    // The user defaults key got recycled to make it clear whether the legacy
    // (number) or modern value (dict) is in use, but the key doesn't reflect
    // its true meaning any more.
    [[NSUserDefaults standardUserDefaults] setObject:@(set)
                                              forKey:@"findMode_iTerm"];
  }
}
...
- (void)doSearch {
  NSString *theString =
      _savedState ? _state.string : _viewController.findString;
  if (!_savedState) {
    [self loadFindStringIntoSharedPasteboard:_viewController.findString];
  }
  // Search.
...
- (NSArray<NSString *> *)completionsForText:(NSString *)text
                                      range:(NSRange)range {
  return [[[iTermSearchHistory sharedInstance] queries]
      filteredArrayUsingBlock:^BOOL(NSString *historyEntry) {
        return [[historyEntry localizedLowercaseString]
            it_hasPrefix:[text localizedLowercaseString]];
      }];
}
...
- (void)moveDown:(id)sender {
  NSTextView *fieldEditor = [NSTextView
      castFrom:[_viewController.view.window fieldEditor:YES
