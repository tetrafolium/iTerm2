...
static NSString *const kXtermParserSavedStateDataKey =
    @"kXtermParserSavedStateDataKey";
static NSString *const kXtermParserSavedStateBytesUsedKey =
    @"kXtermParserSavedStateBytesUsedKey";
static NSString *const kXtermParserSavedStateModeKey =
    @"kXtermParserSavedStateModeKey";
static NSString *const kXtermParserSavedStateStateKey =
    @"kXtermParserSavedStateStateKey";
static NSString *const kXtermParserMultitokenHeaderEmittedkey =
    @"kXtermParserMultitokenHeaderEmittedkey";

// Nonstandard Linux OSC P nrrggbb ST to change color palette
// entry. Since no mode number exists for this code, we use -1 as a temporary
// placeholder until it can be translated into XTERMCC_SET_PALETTE.
static const int kLinuxSetPaletteMode = -1;

// If this is APC ... ST instead of OSC ... ST, use this for the mode since APC
// doesn't have modes.
...
typedef enum {
  // Reading the mode at the start. Either a number followed by a semicolon, or
  // the letter P.
  ...
  // Reading the string after the mode number and semicolon, up to and including
  // the terminator.
  kXtermParserParsingStringState,

  // Have encountered an illegal character (e.g., bogus mode). Continue until
  // the code ends as a
  // normal OSC would.
  ...
  // Save state into the savedState dictionary, backtrack all consumed data, and
  // have client try
...
+ (iTermXtermParserState)parseModeFromContext:(iTermParserContext *)context
                                         mode:(int *)mode {
  BOOL overflow = NO;
  ...
  if (iTermParserConsumeInteger(context, mode, &overflow)) {
    ...
    // Read an integer. Either out of data or a semicolon should follow;
    // anything else is a malformed input.
    if (iTermParserCanAdvance(context)) {
      ...
      // Out of data. Backtrack to the start of the int because when this is
      // resumed later with more data, we must begin parsing the mode all over
      // again.
      *context = saved;
      return kXtermParserOutOfDataState;
    }
  } else {
    // Failed to read an integer. Could be out of data, could be the nonstandard
    // 'P' code, or could be a malformed input.
    if (iTermParserCanAdvance(context)) {
      ...
      if (c == 'P') {
        ...
        *mode = kLinuxSetPaletteMode;
        ...
        // This isn't documented AFAICT but if you leave out the mode then it
        // defaults to 0. This is how xterm works and some users expect it (see
        // bug 3371).
        iTermParserAdvance(context);
        *mode = 0;
        return kXtermParserParsingStringState;
...
+ (iTermXtermParserState)parsePFromContext:(iTermParserContext *)context
...
// Read the next characters and append them to data. Various substrings may end
// parsing:
//   ESC ]                This is ignored
...
//   CAN or SUB           Fails
//
// When 8-bit control sequences are supported, the following behaviors are
// added:
...
+ (iTermXtermParserState)
    parseNextCharsInStringFromContext:(iTermParserContext *)context
    ...
    if (!iTermParserCanAdvance(context)) {
      ...
      case VT100CC_ESC:
        if (iTermParserTryConsume(context, &c)) {
          ...
          // Ended after ESC. Backtrack over the ESC so it can be parsed again
      ...
      case ':':
        if ((mode == 50 || mode == 1337) &&
          ...
          // This is a wonky special case for file downloads. The OSC code can
          // be really, really big. So we mark it as ended at the colon, and the
          // client is responsible for handling this properly.
...
+ (VT100TerminalTokenType)tokenTypeForMode:(int)mode {
  ...
  dispatch_once(&onceToken, ^{
    theMap = @{
      @(kAPCMode) : @(XTERMCC_WIN_TITLE), // tmux treats APC like OSC 2. We must
                                          // as well for tmux integration.
      ...
      // xterm since started using it for setting the font, so 1337 is the
      // preferred code for this in iTerm2.
      @50 : @(XTERMCC_SET_KVP),
      @52 : @(XTERMCC_PASTE64),
...
+ (void)decodeFromContext:(iTermParserContext *)context
  ...
  BOOL support8BitControlCharacters = (encoding == NSASCIIStringEncoding ||
                                       encoding == NSISOLatin1StringEncoding);

  if (savedState.count) {
    data = savedState[kXtermParserSavedStateDataKey];
    iTermParserAdvanceMultiple(
        context, [savedState[kXtermParserSavedStateBytesUsedKey] intValue]);
    mode = [savedState[kXtermParserSavedStateModeKey] intValue];
    state = [savedState[kXtermParserSavedStateStateKey] intValue];
    multitokenHeaderEmitted =
        [savedState[kXtermParserMultitokenHeaderEmittedkey] boolValue];
  } else {
    unsigned char peek = iTermParserPeek(context);
    BOOL apc = NO;
    if (support8BitControlCharacters &&
        (peek == VT100CC_C1_OSC || peek == VT100CC_C1_APC)) {
      ...
      // 99% of the time the next byte is a ], but it could be a _ which is APC
      // (used by tmux).
  ...
  while (1) {
    ...
    case kXtermParserParsingStringState:
      state =
          [self parseNextCharsInStringFromContext:context
    ...
    case kXtermParserHeaderEndState:
      // There's currently only one multitoken mode. Emit a header for it as an
      // incidental.
    ...
    case kXtermParserOutOfDataState:
      ...
      savedState[kXtermParserSavedStateBytesUsedKey] =
          @(iTermParserNumberOfBytesConsumed(context));
      savedState[kXtermParserSavedStateModeKey] = @(mode);
      savedState[kXtermParserSavedStateStateKey] = @(previousState);
      savedState[kXtermParserMultitokenHeaderEmittedkey] =
          @(multitokenHeaderEmitted);
