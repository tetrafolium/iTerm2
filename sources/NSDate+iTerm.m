...
+ (BOOL)isAprilFools {
  ...
  if (now - lastComputation > 3600) {
    // Check no more than once an hour. This could be much more efficient, but
    // doing anything with time is fraught with bugs and peril, so we'll just
    // keep it simple.
...
+ (NSTimeInterval)it_timeSinceBoot {
  ...
  const double nanoseconds =
      elapsed * sTimebaseInfo.numer / sTimebaseInfo.denom;
