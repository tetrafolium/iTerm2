...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p frame=%@ isHidden=%@ alphaValue=%@>",
                       [self class], self, NSStringFromRect(self.frame),
                       @(self.isHidden), @(self.alphaValue)];
}
...
- (void)resizeWithOldSuperviewSize:(NSSize)oldSize {
  DLog(@"%@ resized %@ -> %@:\n%@", self, NSStringFromSize(oldSize),
       NSStringFromSize(self.frame.size), [NSThread callStackSymbols]);
