...
- (NSString *)message {
  NSArray<iTermPromptOnCloseReason *> *sortedReasons =
      [_children sortedArrayUsingComparator:^NSComparisonResult(
                     iTermPromptOnCloseReason *_Nonnull obj1,
                     iTermPromptOnCloseReason *_Nonnull obj2) {
        return [obj1.priority compare:obj2.priority];
      }];

  __block NSString *previousMessage = nil;
  NSArray *uniqueReasons = [sortedReasons
      filteredArrayUsingBlock:^BOOL(iTermPromptOnCloseReason *reason) {
  ...
  NSArray *prettyReasons =
      [uniqueReasons flatMapWithBlock:^id(iTermPromptOnCloseReason *reason) {
        NSString *formattedMessage =
            [@"• " stringByAppendingString:reason.message];
  ...
  if ([previousClass groupFooter]) {
    prettyReasons =
        [prettyReasons arrayByAddingObject:[previousClass groupFooter]];
  }
...
- (void)addReason:(iTermPromptOnCloseReason *)reason {
  if ([reason isKindOfClass:[iTermPromptOnCloseCompoundReason class]]) {
    iTermPromptOnCloseCompoundReason *compound =
        (iTermPromptOnCloseCompoundReason *)reason;
...
- (NSString *)message {
  return [NSString
      stringWithFormat:@"The profile “%@” always requires confirmation.",
                       _name];
}
...
- (NSString *)message {
  ...
  if (_jobs.count <= maxJobsToList) {
    return [NSString
        stringWithFormat:@"A session with profile “%@” is running %@.", _name,
                         [_jobs componentsJoinedWithOxfordComma]];
  } else {
    return [NSString
        stringWithFormat:
            @"A session with profile “%@” is running %@, and %@ other %@.",
            _name,
            [[_jobs subarrayWithRange:NSMakeRange(0, maxJobsToList)]
...
+ (instancetype)profileAlwaysPrompts:(Profile *)profile {
  return [[[iTermPromptOnCloseAlwaysReason alloc]
      initWithProfileName:profile[KEY_NAME]] autorelease];
}

+ (instancetype)profile:(Profile *)profile
          blockedByJobs:(NSArray<NSString *> *)jobs {
  return
      [[[iTermPromptOnCloseBlockedReason alloc] initWithName:profile[KEY_NAME]
                                                        jobs:jobs] autorelease];
}

+ (instancetype)alwaysConfirmQuitPreferenceEnabled {
  return [[[iTermPromptOnCloseMessageReason alloc]
      initWithMessage:@"“Prefs > General > Closing > Confirm Quit iTerm2” is "
                      @"enabled and there is at least one terminal window."
             priority:100] autorelease];
}

+ (instancetype)alwaysConfirmQuitPreferenceEvenIfThereAreNoWindowsEnabled {
  return [[[iTermPromptOnCloseMessageReason alloc]
      initWithMessage:@"“Prefs > General > Closing > Confirm Quit iTerm2” and "
                      @"“Even if there are no windows” is enabled."
             priority:100] autorelease];
}

+ (instancetype)closingMultipleSessionsPreferenceEnabled {
  return [[[iTermPromptOnCloseMessageReason alloc]
      initWithMessage:@"“Prefs > General > Closing > Confirm closing multiple "
                      @"sessions” is enabled."
             priority:90] autorelease];
}

+ (instancetype)tmuxClientsAlwaysPromptBecauseJobsAreNotExposed {
  return [[[iTermPromptOnCloseMessageReason alloc]
      initWithMessage:@"A tmux session is configured to prompt if jobs are "
                      @"running, but tmux doesn’t expose the process tree."
             priority:80] autorelease];
}
