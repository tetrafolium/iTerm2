...
- (void)_setOverlayScrollerState:(unsigned long long)arg1
                forceImmediately:(BOOL)arg2;
@end
...
- (void)dismemberForScrollerStyle:(NSScrollerStyle)scrollerStyle
  ...
  if (self.scrollerStyle != scrollerStyle &&
    ...
    if (preferredSuperview) {
      ...
      NSScrollView *scrollview =
          [self.ptyScrollerDelegate ptyScrollerScrollView];
      if (preferredSuperview != scrollview && scrollview != nil) {
        DLog(@"Scroller style changing to overlay. Remove self from %@, add to "
             @"%@",
             preferredSuperview, scrollview);
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"%@ visibleRect:%@", [super description],
...
- (CGFloat)accumulateVerticalScrollFromEvent:(NSEvent *)theEvent {
  ...
  if ([iTermAdvancedSettingsModel useModernScrollWheelAccumulator]) {
    return [self.ptyVerticalScroller.accumulator deltaYForEvent:theEvent
                                                     lineHeight:lineHeight];
  } else {
    return
        [self.ptyVerticalScroller.accumulator legacyDeltaYForEvent:theEvent
                                                        lineHeight:lineHeight];
  }
}
...
- (void)detectUserScroll {
  ...
  scrollRect = [self documentVisibleRect];
  verticalScroller.userScroll = scrollRect.origin.y + scrollRect.size.height <
