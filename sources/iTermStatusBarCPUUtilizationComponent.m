...
- (instancetype)
  ...
  if (self) {
    ...
    [[iTermCPUUtilization sharedInstance] addSubscriber:self
                                                  block:^(double value) {
...
- (NSImage *)statusBarComponentIcon {
  return [NSImage it_cacheableImageNamed:@"StatusBarIconCPU"
                                forClass:[self class]];
}
...
- (int)currentEstimate {
  double alpha = 0.7;
  NSArray<NSNumber *> *const samples =
      [[iTermCPUUtilization sharedInstance] samples];
  ...
  if (lastSamples.count > maxSamplesToUse) {
    lastSamples = [lastSamples
        subarrayWithRange:NSMakeRange(lastSamples.count - maxSamplesToUse,
                                      maxSamplesToUse)];
  }
