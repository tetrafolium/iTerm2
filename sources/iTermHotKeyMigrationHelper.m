...
- (void)migrateSingleHotkeyToMulti {
  if ([iTermPreferences
          boolForKey:kPreferenceKeyHotkeyMigratedFromSingleToMulti]) {
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeyHotkeyEnabled] &&
      [iTermPreferences
          boolForKey:kPreferenceKeyHotKeyTogglesWindow_Deprecated]) {
    DLog(@"Legacy preferences have a dedicated window");

    NSString *guid = [iTermPreferences
        stringForKey:kPreferenceKeyHotkeyProfileGuid_Deprecated];
    // There is something to migrate
    Profile *profile =
        guid ? [[ProfileModel sharedInstance] bookmarkWithGuid:guid] : nil;
    if (profile) {
      ...
      if ([profile profileIsDynamic]) {
        ...
        [[ProfileModel sharedInstance]
            setObjectsFromDictionary:[self hotkeyDictionary]
                           inProfile:profile];
        [[ProfileModel sharedInstance] flush];
      }
      if ([self warnAboutChildrenOfHotkeyProfileIfNeeded:profile]) {
        DLog(@"Reloading dynamic profiles");
        // We changed the parent of a profile so reload them and the children
        // will pick up the (terrible) behavior we warned you about.
        [[iTermDynamicProfileManager sharedInstance] reloadDynamicProfiles];
      }
    }

    // Erase the legacy prefs so the keystroke doesn't show up as the global app
    // toggle.
  ...
  _didMigration = YES;
  [iTermPreferences setBool:YES
                     forKey:kPreferenceKeyHotkeyMigratedFromSingleToMulti];
}
...
- (BOOL)warnAboutChildrenOfHotkeyProfileIfNeeded:(Profile *)profile {
  ...
  if (childrensNames.count) {
    ...
    NSString *concatenatedNames =
        [childrensNames componentsJoinedWithOxfordComma];
    NSString *title = [NSString
        stringWithFormat:
            @"You have dynamic profiles whose “Dynamic Profile Parent Name” is "
            @"set to your hotkey window's profile, “%@.” Because multiple "
            @"hotkey windows are now supported, the hotkey will now toggle a "
            @"separate window for each of these profiles. Please update your "
...
- (void)migrateDynamicProfileHotKeySettings:(Profile *)profile {
  ...
  NSString *title =
      [NSString stringWithFormat:
                    @"Your hotkey window‘s profile is a dynamic profile named "
                    @"“%@.” It needs to be updated for this version of iTerm2 "
                    @"because hotkey settings are now stored in the profile.",
                    profile[KEY_NAME]];

  NSArray *actions;
  NSData *replacementFile =
      [self modifiedDynamicProfileFileWithNewHotKeySettingsFromProfile:profile];
  ...
  // There's nothing to do for the "remove" case. Setting the Migrated flag will
  // silently ignore the existing hotkey settings forever.
}
...
- (NSString *)jsonEntriesInDictionary:(NSDictionary *)dictionary {
  ...
  [dictionary enumerateKeysAndObjectsUsingBlock:^(
    ...
    [linesArray addObject:[NSString stringWithFormat:@"%@: %@", encodedKey,
                                                     encodedValue]];
  }];
  return [linesArray componentsJoinedByString:@",\n"];
}

// Note: this isn't totally general. It only supports string, real, and integer
// values. No containers, etc.
- (NSString *)xmlEntriesInDictionary:(NSDictionary *)dictionary {
  ...
  [dictionary enumerateKeysAndObjectsUsingBlock:^(
    ...
    assert(encodedValue);
    [linesArray
        addObject:[NSString stringWithFormat:@"<key>%@</key><%@>%@</%@>",
                                             encodedKey, type, encodedValue,
                                             type]];
  }];
  return [linesArray componentsJoinedByString:@"\n"];
}
...
- (void)showNeededChangesForHotKeyMigrationOfDynamicProfile:(Profile *)profile {
  ...
  [[iTermDynamicProfileManager sharedInstance] profilesInFile:filename
                                                     fileType:&fileType];
  NSString *lines = nil;
  switch (fileType) {
  ...
  alert.informativeText = [NSString
      stringWithFormat:
          @"Add these settings to the profile named “%@” in “%@”:\n%@",
...
- (NSData *)modifiedDynamicProfileFileWithNewHotKeySettingsFromProfile:
    (Profile *)profile {
  NSString *filename = profile[KEY_DYNAMIC_PROFILE_FILENAME];
  ...
  NSArray<Profile *> *profiles =
      [[iTermDynamicProfileManager sharedInstance] profilesInFile:filename
                                                         fileType:&fileType];
  NSUInteger index = [profiles
      indexOfObjectPassingTest:^BOOL(NSDictionary *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
  ...
  if (index != NSNotFound) {
    ...
    NSMutableArray<Profile *> *modifiedProfilesArray =
        [[profiles mutableCopy] autorelease];
    modifiedProfilesArray[index] = modifiedProfile;

    NSDictionary *dict = [[iTermDynamicProfileManager sharedInstance]
        dictionaryForProfiles:modifiedProfilesArray];
    switch (fileType) {
    case kDynamicProfileFileTypeJSON: {
      NSError *error = nil;
      NSData *data =
          [NSJSONSerialization dataWithJSONObject:dict
                                          options:NSJSONWritingPrettyPrinted
                                            error:&error];
      if (error) {
        XLog(@"Failed to create JSON data from dictionary %@ with error %@",
             dict, error);
      }
      return data;
    }
...
- (NSDictionary *)hotkeyDictionary {
  NSUInteger keyCode =
      [iTermPreferences unsignedIntegerForKey:kPreferenceKeyHotKeyCode];
  unichar character =
      [iTermPreferences unsignedIntegerForKey:kPreferenceKeyHotkeyCharacter];
  NSString *characters =
      character ? [NSString stringWithFormat:@"%C", character] : @"";
  NSEventModifierFlags modifiers =
      [iTermPreferences unsignedIntegerForKey:kPreferenceKeyHotkeyModifiers];
  BOOL autohides =
      [iTermPreferences boolForKey:kPreferenceKeyHotkeyAutoHides_Deprecated];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  BOOL dockIconTogglesWindow =
      [iTermAdvancedSettingsModel dockIconTogglesWindow];
#pragma clang diagnostic pop
  iTermHotKeyDockPreference dockAction =
      dockIconTogglesWindow ? iTermHotKeyDockPreferenceShowIfNoOtherWindowsOpen
