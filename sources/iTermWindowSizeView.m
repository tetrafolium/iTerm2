...
- (void)setWindowSize:(VT100GridSize)size {
  _label.stringValue =
      [NSString stringWithFormat:@"%@ ⨉ %@", @(size.width), @(size.height)];
  [_label sizeToFit];
...
- (void)layoutSelf {
  ...
  const CGFloat width =
      MAX(_label.frame.size.width, _detailLabel.frame.size.width);
  ...
  self.frame = NSMakeRect(center.x - size.width / 2, center.y - size.height / 2,
...
- (void)layoutSubviews {
  ...
  NSRect frame =
      NSInsetRect(bounds, iTermWindowSizeViewMargin, iTermWindowSizeViewMargin);
  if (self.detail) {
    frame.size.height = _detailLabel.frame.size.height;
    _detailLabel.frame = frame;
    frame.origin.y +=
        _detailLabel.frame.size.height + iTermWindowSizeViewMargin;
    frame.size.height = _label.frame.size.height;
    _label.frame = frame;
  } else {
    frame.size.height = _label.frame.size.height;
    frame.origin.y =
        [self retinaRound:(NSHeight(bounds) - NSHeight(frame)) / 2.0];
    _label.frame = frame;
  }
}
