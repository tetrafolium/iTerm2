...
@interface iTermScriptConsole () <NSTabViewDelegate, NSTableViewDataSource,
                                  NSTableViewDelegate>
...
- (instancetype)initWithWindowNibName:(NSNibName)windowNibName {
  ...
  if (self) {
    ...
    _dateFormatter.dateFormat =
        [NSDateFormatter dateFormatFromTemplate:@"Ld jj:mm:ss"
                                        options:0
...
- (IBAction)inspector:(id)sender {
  if (!_inspector) {
    _inspector = [[iTermScriptInspector alloc]
        initWithWindowNibName:@"iTermScriptInspector"];
  } else {
...
- (void)revealTailOfHistoryEntry:(iTermScriptHistoryEntry *)entry {
  ...
  if (index == NSNotFound) {
    ...
    if (index != NSNotFound) {
      [_scriptFilterControl
          selectSegmentWithTag:iTermScriptFilterControlTagAll];
      [_tableView reloadData];
    }
  }
  if (index == NSNotFound) {
...
- (void)tableViewSelectionDidChange:(NSNotification *)notification {
  ...
  if (!_tableView.numberOfSelectedRows) {
    ...
    _token = [[NSNotificationCenter defaultCenter]
        addObserverForName:iTermScriptHistoryEntryDidChangeNotification
                    object:entry
                     queue:nil
                usingBlock:^(NSNotification *_Nonnull note) {
                  ...
                  if (note.userInfo) {
                    NSString *delta =
                        note.userInfo[iTermScriptHistoryEntryDelta];
                    NSString *property =
                        note.userInfo[iTermScriptHistoryEntryFieldKey];
...
- (void)scrollLogsToBottomIfNeeded {
  if (_scrollToBottomOnUpdate.state == NSControlStateValueOn &&
      _tabView.selectedTabViewItem.view == _logsView.enclosingScrollView) {
...
- (void)scrollCallsToBottomIfNeeded {
  if (_scrollToBottomOnUpdate.state == NSControlStateValueOn &&
      _tabView.selectedTabViewItem.view == _callsView.enclosingScrollView) {
...
- (BOOL)line:(NSString *)line
    containsString:(NSString *)filter
...
- (void)updateFilteredValue {
  ...
  iTermScriptHistoryEntry *entry =
      [[self filteredEntries] objectAtIndex:_tableView.selectedRow];
  ...
  BOOL caseSensitive =
      [filter
          ...
          .location != NSNotFound;
  NSString *newValue =
      [[entry.logLines filteredArrayUsingBlock:^BOOL(NSString *line) {
        return unfiltered || [self line:line
...
- (NSString *)formatPIDs:(NSArray<NSNumber *> *)pids {
  ...
  return [NSString
      stringWithFormat:@"PIDs %@", [pids componentsJoinedByString:@", "]];
}
...
- (void)connectionRejected:(NSNotification *)notification {
  ...
  if (key) {
    ...
    key = [[NSUUID UUID] UUIDString]; // Just needs to be something unique to
                                      // identify this now-immutable log
  }
  if (!entry) {
...
- (void)connectionAccepted:(NSNotification *)notification {
  ...
  [entry
      addOutput:[NSString stringWithFormat:@"Connection accepted: %@\n",
...
- (void)connectionClosed:(NSNotification *)notification {
  ...
  assert(key);
  iTermScriptHistoryEntry *entry =
      [[iTermScriptHistory sharedInstance] entryWithIdentifier:key];
  if (!entry) {
    return;
  }
  [entry addOutput:@"\nConnection closed.\n"];
