...
static NSString *const kColorGalleryURL =
...
+ (NSArray<NSString *> *)presetNames {
  NSArray<NSString *> *builtInNames =
      [[iTermColorPresets builtInColorPresets] allKeys];
  NSArray<NSString *> *customNames =
      [[iTermColorPresets customColorPresets] allKeys];
  return [builtInNames arrayByAddingObjectsFromArray:customNames];
}

+ (iTermColorPreset *)presetWithName:(NSString *)name {
  iTermColorPreset *dict =
      [[iTermColorPresets builtInColorPresets] objectForKey:name];
  if (dict) {
    return dict;
  }
...
+ (NSString *)nameOfPresetUsedByProfile:(Profile *)profile {
  for (NSString *presetName in [self presetNames]) {
    ...
    for (NSString *colorName in [ProfileModel colorKeys]) {
      iTermColorDictionary *presetColorDict =
          [preset iterm_presetColorWithName:colorName];
      NSDictionary *profileColorDict =
          [iTermProfilePreferences objectForKey:colorName inProfile:profile];
      if (![presetColorDict isEqual:profileColorDict] &&
          presetColorDict != profileColorDict) {
        ok = NO;
        break;
      }
    }
...
- (void)awakeFromNib {
  ...
  for (NSString *key in colorWellDictionary) {
    ...
    [self defineControl:colorWell
                    key:key
            relatedView:nil
            displayName:[NSString stringWithFormat:@"%@ color",
                                                   relatedView.stringValue]
...
- (void)updateColorControlsEnabled {
  ...
  const BOOL smartCursorColorSelected =
      [self boolForKey:KEY_SMART_CURSOR_COLOR];
  const BOOL shouldEnableSmartCursorColor =
      ([self intForKey:KEY_CURSOR_TYPE] == CURSOR_BOX);
  const BOOL shouldEnableCursorColor =
      !(smartCursorColorSelected && shouldEnableSmartCursorColor);
...
- (NSDictionary *)colorWellRelatedViews {
  return @{
    KEY_ANSI_0_COLOR : _ansi0ColorLabel,
...
- (void)rebuildColorPresetsMenu {
  ...
  [self addPresetItemWithTitle:@"Import..."
                        action:@selector(importColorPreset:)];
  [self addPresetItemWithTitle:@"Export..."
                        action:@selector(exportColorPreset:)];
  [self addPresetItemWithTitle:@"Delete Preset..."
                        action:@selector(deleteColorPreset:)];
  [self addPresetItemWithTitle:@"Visit Online Gallery"
                        action:@selector(visitGallery:)];
  _presetsMenu.delegate = self;
}

- (void)addPresetItemWithTitle:(NSString *)title action:(SEL)action {
  NSMenuItem *item = [_presetsMenu addItemWithTitle:title
                                             action:action
                                      keyEquivalent:@""];
  item.target = self;
}

- (void)addColorPresetsInDict:(iTermColorPresetDictionary *)presetsDict
                       toMenu:(NSMenu *)theMenu {
  for (NSString *key in
       [[presetsDict allKeys] sortedArrayUsingSelector:@selector(compare:)]) {
...
- (void)deleteColorPreset:(id)sender {
  ...
  if (!customPresets || [customPresets count] == 0) {
    ...
    alert.informativeText = @"You cannot erase the built-in presets and no "
                            @"custom presets have been imported.";
  ...
  for (NSString *key in
       [[customPresets allKeys] sortedArrayUsingSelector:@selector(compare:)]) {
...
- (void)exportColorPresetToFile:(NSString *)filename {
  NSMutableDictionary *theDict =
      [NSMutableDictionary dictionaryWithCapacity:24];
  NSDictionary *colorWellDictionary = [self colorWellDictionary];
  for (NSString *key in colorWellDictionary) {
...
- (void)visitGallery:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:kColorGalleryURL]];
}
...
- (void)restoreColors {
  [_savedColors enumerateKeysAndObjectsUsingBlock:^(NSString *_Nonnull key,
                                                    id _Nonnull obj,
                                                    BOOL *_Nonnull stop) {
...
// If the current color settings exactly match a preset, place a check mark next
// to it and uncheck all others. If multiple presets match, check the first
// matching one.
...
- (void)menu:(NSMenu *)menu willHighlightItem:(nullable NSMenuItem *)item {
  if (item.action == @selector(loadColorPreset:)) {
    ...
    _timer = [NSTimer scheduledTimerWithTimeInterval:0.4
                                              target:self
                                            selector:@selector(previewColors:)
                                            userInfo:item
                                             repeats:NO];
