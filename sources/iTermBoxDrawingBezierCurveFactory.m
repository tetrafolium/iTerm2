  ...
  dispatch_once(&onceToken, ^{
    if ([iTermAdvancedSettingsModel disableCustomBoxDrawing]) {
      ...
      NSMutableCharacterSet *temp =
          [[self boxDrawingCharactersWithBezierPathsExcludingPowerline]
              mutableCopy];
...
+ (void)drawPowerlineCode:(unichar)code
  ...
  switch (code) {
  case 0xE0A0:
    [self drawPDFWithName:@"PowerlineVersionControlBranch"
                 cellSize:cellSize
                  stretch:NO
                    color:color
              antialiased:YES];
  ...
  case 0xE0A1:
    [self drawPDFWithName:@"PowerlineLN"
                 cellSize:cellSize
                  stretch:NO
                    color:color
              antialiased:NO];
  ...
  case 0xE0A2:
    [self drawPDFWithName:@"PowerlinePadlock"
                 cellSize:cellSize
                  stretch:NO
                    color:color
              antialiased:YES];
    break;
  case 0xE0B0:
    [self drawPDFWithName:@"PowerlineSolidRightArrow"
                 cellSize:cellSize
                  stretch:YES
                    color:color
              antialiased:YES];
    break;
  case 0xE0B2:
    [self drawPDFWithName:@"PowerlineSolidLeftArrow"
                 cellSize:cellSize
                  stretch:YES
                    color:color
              antialiased:YES];
    break;
  case 0xE0B1:
    [self drawPDFWithName:@"PowerlineLineRightArrow"
                 cellSize:cellSize
                  stretch:YES
                    color:color
              antialiased:YES];
    break;
  case 0xE0B3:
    [self drawPDFWithName:@"PowerlineLineLeftArrow"
                 cellSize:cellSize
                  stretch:YES
                    color:color
              antialiased:YES];
...
+ (NSImage *)newImageForPDFNamed:(NSString *)pdfName
  ...
  NSString *pdfPath = [[NSBundle bundleForClass:self] pathForResource:pdfName
                                                               ofType:@"pdf"];
  NSData *pdfData = [NSData dataWithContentsOfFile:pdfPath];
  NSPDFImageRep *pdfImageRep = [NSPDFImageRep imageRepWithData:pdfData];
  NSImage *image =
      [[NSImage alloc] initWithSize:NSMakeSize(pdfImageRep.size.width * 2,
...
+ (NSRect)drawingDestinationForImageOfSize:(NSSize)imageSize
  ...
  if (pdfAspectRatio > cellAspectRatio) {
    ...
    const CGFloat letterboxHeight =
        (destinationSize.height - destinationSize.width / pdfAspectRatio) / 2;
    return NSMakeRect(0, letterboxHeight, destinationSize.width,
                      destinationSize.height - letterboxHeight * 2);
  }

  // PDF is taller than cell so pillarbox left and right
  const CGFloat pillarboxWidth =
      (destinationSize.width - destinationSize.height * pdfAspectRatio) / 2;
  return NSMakeRect(pillarboxWidth, 0,
                    destinationSize.width - pillarboxWidth * 2,
...
+ (void)drawCodeInCurrentContext:(unichar)code
  ...
  if (useNativePowerlineGlyphs && [self isPowerlineGlyph:code]) {
    [self drawPowerlineCode:code cellSize:cellSize color:colorRef];
    return;
  }
  ...
  NSArray<NSBezierPath *> *paths =
      [iTermBoxDrawingBezierCurveFactory bezierPathsForBoxDrawingCode:code
                                                             cellSize:cellSize
                                                                scale:scale
                                                               offset:offset
                                                                solid:&solid];
...
+ (NSArray<NSBezierPath *> *)bezierPathsForBoxDrawingCode:(unichar)code
  ...
  NSArray<NSBezierPath *> *solidBoxPaths =
      [self bezierPathsForSolidBoxesForCode:code cellSize:cellSize scale:scale];
  if (solidBoxPaths) {
    if (solid) {
      *solid = YES;
    }
  ...
  while (i + 4 <= length) {
    ...
    if (x1 != lastX || y1 != lastY) {
      [path moveToPoint:makePoint((xs[x1]), (ys[y1]))];
    }
