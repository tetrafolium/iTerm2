...
- (instancetype)recordByPrependingPath:(NSString *)path {
  ...
  if (self.isTerminal) {
    ...
    return [[iTermRecordedVariable alloc]
        initNonterminalWithName:name
                        context:_nonterminalContext];
  }
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
  ...
  if (isTerminal) {
    ...
    return [self initNonterminalWithName:dict[@"name"]
                                 context:[dict[@"nonterminalContext"]
...
- (NSString *)description {
  if (_isTerminal) {
    return [NSString stringWithFormat:@"<%@: %p name=%@ terminal>",
                                      NSStringFromClass(self.class), self,
                                      self.name];
  } else {
    NSString *context =
        [iTermVariableHistory stringForContext:_nonterminalContext];
    return [NSString stringWithFormat:@"<%@: %p name=%@ nonterminal %@>",
                                      NSStringFromClass(self.class), self,
                                      self.name, context];
  }
}
