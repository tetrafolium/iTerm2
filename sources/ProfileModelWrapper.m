...
- (void)dump {
  for (int i = 0; i < [self numberOfBookmarks]; ++i) {
    NSLog(@"Dump of %p: At %d: %@", self, i,
          [[self profileTableRowAtIndex:i] name]);
  }
}
...
- (void)sync {
  ...
  NSArray *filteredBookmarks =
      [underlyingModel bookmarkIndicesMatchingFilter:filter
                                              orGuid:self.lockedGuid];
  for (NSNumber *n in filteredBookmarks) {
    int i = [n intValue];
    [bookmarks addObject:[[[ProfileTableRow alloc]
                             initWithBookmark:[underlyingModel profileAtIndex:i]
