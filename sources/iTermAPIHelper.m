...
NSString *const iTermRemoveAPIServerSubscriptionsNotification =
    @"iTermRemoveAPIServerSubscriptionsNotification";
NSString *const iTermAPIRegisteredFunctionsDidChangeNotification =
    @"iTermAPIRegisteredFunctionsDidChangeNotification";
...
NSString *const iTermAPIHelperDidStopNotification =
    @"iTermAPIHelperDidStopNotification";
static NSString *const iTermAPIHelperEnablePythonAPIWarningIdentifier =
    @"NoSyncEnableAPIServer";
NSString *const iTermAPIHelperErrorDomain = @"com.iterm2.api";
static NSString *const iTermAPIHelperDisableApplescriptAuthMagic =
    @"61DF88DC-3423-4823-B725-22570E01C027";
...
- (instancetype)initWithNotificationRequest:(ITMNotificationRequest *)req {
  self = [super init];
  if (self) {
    if (req.rpcRegistrationRequest.role !=
        ITMRPCRegistrationRequest_Role_SessionTitle) {
      return nil;
    }
    _invocation =
        [self invocationOfRegistrationRequest:req.rpcRegistrationRequest];
    if (!_invocation) {
      return nil;
    }
    _displayName =
        [req.rpcRegistrationRequest.sessionTitleAttributes.displayName copy];
    if (!_displayName) {
      return nil;
    }
    _uniqueIdentifier =
        req.rpcRegistrationRequest.sessionTitleAttributes.uniqueIdentifier;
...
- (NSString *)invocationOfRegistrationRequest:(ITMRPCRegistrationRequest *)req {
  return
      [iTermAPIHelper invocationWithFullyQualifiedName:req.it_fullyQualifiedName
...
- (BOOL)it_rpcRegistrationRequestValidWithError:(out NSError **)error {
  NSCharacterSet *ascii =
      [NSCharacterSet characterSetWithRange:NSMakeRange(0, 128)];
  NSMutableCharacterSet *invalidIdentifierCharacterSet =
  ...
  if ([self.name rangeOfCharacterFromSet:invalidIdentifierCharacterSet]
          .location != NSNotFound) {
    if (error) {
      *error = newErrorWithReason(
          [NSString stringWithFormat:@"Function name '%@' contains an invalid "
                                     @"character. Must match /[A-Za-z0-9_]/",
                                     self.name]);
    }
    return NO;
  }
  NSMutableSet<NSString *> *args = [NSMutableSet set];
  for (ITMRPCRegistrationRequest_RPCArgumentSignature *arg in self
           .argumentsArray) {
    ...
    if ([name rangeOfCharacterFromSet:invalidIdentifierCharacterSet].location !=
        NSNotFound) {
      if (error) {
        *error = newErrorWithReason([NSString
            stringWithFormat:@"Argument name '%@' contains an invalid "
                             @"character. Must match /[A-Za-z0-9_]/",
                             name]);
      }
      return NO;
    ...
    if ([args containsObject:name]) {
      if (error) {
        *error = newErrorWithReason(
            [NSString stringWithFormat:@"Two arguments share the name '%@'. "
  ...
  case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_SessionTitleAttributes:
    return [NSString
        stringWithFormat:@"title.%@",
                         [self.sessionTitleAttributes.uniqueIdentifier
                             stringByReplacingOccurrencesOfString:@"-"
                                                       withString:@"_"]];
  case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_StatusBarComponentAttributes:
    return [NSString
        stringWithFormat:@"statusbar.%@",
                         [self.statusBarComponentAttributes.uniqueIdentifier
                             stringByReplacingOccurrencesOfString:@"-"
                                                       withString:@"_"]];
  case ITMRPCRegistrationRequest_RoleSpecificAttributes_OneOfCase_ContextMenuAttributes:
    return [NSString
        stringWithFormat:@"contextMenu.%@",
                         [self.contextMenuAttributes.uniqueIdentifier
                             stringByReplacingOccurrencesOfString:@"-"
                                                       withString:@"_"]];
  }
}
...
- (NSString *)it_stringRepresentation {
  NSArray<NSString *> *argNames = [self.argumentsArray
      mapWithBlock:^id(
          ITMRPCRegistrationRequest_RPCArgumentSignature *anObject) {
        return anObject.name;
      }];
  NSString *namespace = self.it_namespace;
  return iTermFunctionSignatureFromNamespaceAndNameAndArguments(
      namespace, self.name, argNames);
}
...
- (NSSet<NSString *> *)it_allArgumentNames {
  return
      [NSSet setWithArray:[self.argumentsArray
                              mapWithBlock:^id(
                                  ITMRPCRegistrationRequest_RPCArgumentSignature
                                      *anObject) {
                                return anObject.name;
                              }]];
}
...
- (NSSet<NSString *> *)it_argumentsWithDefaultValues {
  return [NSSet
      setWithArray:[self.defaultsArray
                       mapWithBlock:^id(
                           ITMRPCRegistrationRequest_RPCArgument *anObject) {
                         return anObject.name;
                       }]];
}
...
- (BOOL)it_satisfiesExplicitParameters:
            (NSDictionary<NSString *, id> *)explicitParameters
                                 scope:(iTermVariableScope *)scope
                        fullParameters:(out NSDictionary<NSString *, id> **)
                                           fullParameters {
  NSSet<NSString *> *providedArguments =
      [NSSet setWithArray:explicitParameters.allKeys];
  ...
  NSMutableDictionary<NSString *, id> *params =
      [explicitParameters mutableCopy];
  for (ITMRPCRegistrationRequest_RPCArgument *defaultArgument in self
           .defaultsArray) {
...
- (NSString *)it_stringRepresentationWithNamespace:(NSString *)namespace {
  NSArray<NSString *> *argNames = [self.argumentsArray
      mapWithBlock:^id(ITMServerOriginatedRPC_RPCArgument *anObject) {
        return anObject.name;
      }];
  return iTermFunctionSignatureFromNamespaceAndNameAndArguments(
      namespace, self.name, argNames);
...
@implementation iTermAPIHelper {
  ...
  // Saves the last one to avoid sending changed notifications when nothing
  // changed.
  ITMBroadcastDomainsChangedNotification *_lastBroadcastChangeNotification;

  // When adding a new dictionary of subscriptions update
  // -removeAllSubscriptionsForConnectionKey: and -stop.
  NSMutableDictionary<id, ITMNotificationRequest *> *_newSessionSubscriptions;
  NSMutableDictionary<id, ITMNotificationRequest *>
      *_terminateSessionSubscriptions;
  ...
  NSMutableDictionary<id,
                      NSMutableArray<iTermTuple<ITMNotificationRequest *,
                                                iTermVariableReference *> *> *>
  ...
  NSMutableArray<iTermAllObjectsSubscription *>
      *_allSessionsSubscriptions; // Has one entry per "all" subscription for
                                  // sessions
  NSMutableArray<iTermAllObjectsSubscription *>
      *_allWindowsSubscriptions; // Has one entry per "all" subscription for
                                 // windows
  NSMutableDictionary<NSString *, iTermServerOriginatedRPCCompletionBlock>
      *_serverOriginatedRPCCompletionBlocks;
  // connectionKey -> RPC ID (RPC ID is key in
  // _serverOriginatedRPCCompletionBlocks) WARNING: These can exist after the
  // block has been removed from _serverOriginatedRPCCompletionBlocks if it
  // times out.
...
+ (instancetype)sharedInstanceIfEnabled {
  if (![NSApp isRunningUnitTests] &&
      ![iTermPreferences boolForKey:kPreferenceKeyEnableAPIServer]) {
    return nil;
  }
...
+ (BOOL)requireApplescriptAuth {
  ...
  const BOOL changed =
      (result != iTermAPIHelperLastApplescriptAuthRequiredSetting);
  iTermAPIHelperLastApplescriptAuthRequiredSetting = result;
  if (changed) {
...
+ (BOOL)internalRequireApplescriptAuth {
  ...
  NSString *actualContents =
      [NSString stringWithContentsOfFile:[self noauthPath]
                                encoding:NSUTF8StringEncoding
                                   error:nil];
  const BOOL contentsCorrect =
      [[self noauthMagic] isEqualToString:actualContents];
  if (!contentsCorrect) {
    if (valueForLastWarning &&
        [valueForLastWarning isEqualToString:actualContents]) {
      return YES;
    }
    ...
    const iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            @"The location of your Application Support directory appears to "
            @"have moved or its contents have changed unexpectedly. As a "
            @"precaution, the authentication mechanism for Python API scripts "
            @"for iTerm2 has been reverted to always require Automation "
            @"permission."
    ...
    if (selection == kiTermWarningSelection1) {
      [[PreferencePanel sharedInstance]
          openToPreferenceWithKey:kPreferenceKeyAPIAuthentication];
    }
  }
...
+ (BOOL)createNoAuthFile:(NSWindow *)window {
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          @"Do you want to allow all apps running on this machine to use the "
          @"Python API?\n\nThis will disable the check for Automation "
          @"permission. If you agree, you’ll be prompted for administrator "
  ...
  case kiTermWarningSelection2:
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL
                    URLWithString:@"https://iterm2.com/python-api-auth.html"]];
    return NO;
  default:
    assert(NO);
  }
...
+ (BOOL)removeNoAuthFile:(NSWindow *)window {
  ...
  const BOOL ok = [[NSFileManager defaultManager] removeItemAtPath:path
                                                             error:&error];
  if (ok) {
    return YES;
  }
  ...
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:
                        ...
                        @"this file manually to require Automation permission "
                        @"for the Python API.\n\nThe Python API has been "
...
+ (BOOL)reallyCreateNoAuthFile:(NSWindow *)window {
  ...
  NSString *sourceCode = [NSString
      stringWithFormat:
          @"do shell script \"umask 077; TF=$(mktemp); printf '%%s' '%@' > "
          @"\\\"$TF\\\" && chmod a+r \\\"$TF\\\" && mv \\\"$TF\\\" \\\"%@\\\" "
          @"|| rm -f \\\"$TF\\\"\" with administrator privileges",
  ...
  [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:@"The setting could not be changed: %@",
...
- (void)
    setServerOriginatedRPCSubscriptionWithSignature:(NSString *)signatureString
                                      connectionKey:(NSString *)connectionKey
                                            request:(ITMNotificationRequest *)
                                                        request {
  _internalServerOriginatedRPCSubscriptions[signatureString] =
      [iTermTuple tupleWithObject:connectionKey andObject:request];
}

- (void)removeServerOriginatedRPCSubscriptionWithSignature:
    (NSString *)signatureString {
  [_internalServerOriginatedRPCSubscriptions
      removeObjectForKey:signatureString];
}
...
- (NSInteger)removeServerOriginatedRPCSubscriptionsPasstingTest:
  ...
  return [_internalServerOriginatedRPCSubscriptions
      removeObjectsPassingTest:block];
}
...
+ (BOOL)confirmShouldStartServerAndUpdateUserDefaultsForced:(BOOL)forced {
  // It was not enabled in preferences. Ask the user. If they permanently
  // silence this they'll need to go into prefs to enable it.
  ...
  warning.warningType = forced ? kiTermWarningTypePersistent
                               : kiTermWarningTypePermanentlySilenceable;
  warning.title = @"The Python API allows scripts you run to control iTerm2 "
                  @"and access all its data.";
  static BOOL showing;
  if (showing) {
    // This can happen because the call to -runModal below starts a runloop and
    // a delayed perform can then call this.
  ...
  if (self) {
    if (![NSApp isRunningUnitTests]) {
      if (![iTermPreferences boolForKey:kPreferenceKeyEnableAPIServer]) {
        if (![iTermAPIHelper
                confirmShouldStartServerAndUpdateUserDefaultsForced:force]) {
          return nil;
        }
      }
...
+ (void)setEnabled:(BOOL)enabled {
  [iTermWarning
      unsilenceIdentifier:iTermAPIHelperEnablePythonAPIWarningIdentifier
...
- (void)stop {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermAPIHelperDidStopNotification
                    object:nil];
}
...
- (void)didCreateTerminalWindow:(NSNotification *)notification {
  ...
  for (iTermAllObjectsSubscription *sub in _allWindowsSubscriptions) {
    if (sub.request.notificationType ==
      ...
      [self monitorVariableChangesForConnectionKey:sub.connectionKey
                              ...
                              mutableVariablesDict:
                                  _allWindowVariableSubscriptions];
      continue;
    }
  ...
  for (iTermAllObjectsSubscription *sub in _allSessionsSubscriptions) {
    if (sub.request.notificationType ==
      ...
      [self monitorVariableChangesForConnectionKey:sub.connectionKey
                              ...
                              mutableVariablesDict:
                                  _allSessionVariableSubscriptions];
      continue;
    }
    [session handleAPINotificationRequest:sub.request
                            connectionKey:sub.connectionKey];
  }
  [_newSessionSubscriptions
      enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key,
        ...
        notification.newSessionNotification =
            [[ITMNewSessionNotification alloc] init];
  ...
  [_terminateSessionSubscriptions
      enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key,
        ...
        notification.terminateSessionNotification =
            [[ITMTerminateSessionNotification alloc] init];
...
- (void)handleLayoutChange {
  _layoutChanged = NO;
  [_layoutChangeSubscriptions
      enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key,
                                          ITMNotificationRequest *_Nonnull obj,
                                          BOOL *_Nonnull stop) {
        ITMNotification *notification = [[ITMNotification alloc] init];
        notification.layoutChangedNotification.listSessionsResponse =
            [self newListSessionsResponse];
        [self postAPINotification:notification toConnectionKey:key];
      }];
}
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
  ITMFocusChangedNotification *focusChange =
      [[ITMFocusChangedNotification alloc] init];
...
- (void)windowDidBecomeKey:(NSNotification *)notification {
  ...
  if (term) {
    ...
    if (!term) {
      // Non-terminal window became key and no terminal is current (e.g., you
      // opened prefs). Not very interesting.
      return;
    }
    focusChange.window.windowId = [term terminalGuid];
    focusChange.window.windowStatus =
...
- (void)profileDidChange:(NSNotification *)notification {
  ...
  NSSet<NSString *> *guids = [NSSet
      setWithArray:[entries mapWithBlock:^id(BookmarkJournalEntry *entry) {
        return entry.guid;
      }]];
  for (NSString *guid in guids) {
    [_profileChangeSubscriptions
        enumerateKeysAndObjectsUsingBlock:^(
            id _Nonnull key, ITMNotificationRequest *_Nonnull request,
            BOOL *_Nonnull stop) {
          ITMNotification *notification = [[ITMNotification alloc] init];
          notification.profileChangedNotification =
              [[ITMProfileChangedNotification alloc] init];
...
- (void)handleFocusChange:(ITMFocusChangedNotification *)notif {
  ...
  if (_layoutChanged) {
    // Let the layout change go through first so the app state can be up-to-date
    // when processing the focus change notification.
...
- (void)handleBroadcastChange {
  ...
  if ([NSObject object:broadcastSubNotification
          isEqualToObject:_lastBroadcastChangeNotification]) {
    return;
  }
...
- (NSString *)connectionKeyForRPCWithName:(NSString *)name
  ...
  for (NSString *signature in self.serverOriginatedRPCSubscriptions) {
    iTermTuple<id, ITMNotificationRequest *> *tuple =
        self.serverOriginatedRPCSubscriptions[signature];
    ITMNotificationRequest *request = tuple.secondObject;
    if (![request.rpcRegistrationRequest.it_fullyQualifiedName
            isEqualToString:name]) {
      continue;
    }
    if ([request.rpcRegistrationRequest
            it_satisfiesExplicitParameters:explicitParameters
                                     scope:scope
...
- (void)splitFullyQualifiedRPCName:(NSString *)fqName
  ...
  iTermFunctionCallSplitFullyQualifiedName(fqName, namespacePtr,
                                           relativeNamePtr);
}

- (ITMServerOriginatedRPC *)
    serverOriginatedRPCWithName:(NSString *)fullyQualifiedName
  ...
  [self splitFullyQualifiedRPCName:fullyQualifiedName
                         namespace:&namespace
                      relativeName:&relativeName];

  rpc.name = relativeName;
  for (NSString *argumentName in
       [arguments.allKeys sortedArrayUsingSelector:@selector(compare:)]) {
    ...
    if ([NSNull castFrom:argumentValue]) {
      ...
      if (!jsonValue) {
        NSString *reason =
            [NSString stringWithFormat:@"Could not JSON encode value “%@”",
                                       arguments[argumentName]];
        NSString *signature =
            iTermFunctionSignatureFromNamespaceAndNameAndArguments(
                namespace, relativeName, arguments.allKeys);
        NSString *connectionKey =
            [self connectionKeyForRPCWithSignature:signature];
        NSDictionary *userinfo = @{NSLocalizedDescriptionKey : reason};
        if (connectionKey) {
          userinfo = [userinfo
              dictionaryBySettingObject:connectionKey
                                 forKey:
                                     iTermAPIHelperFunctionCallErrorUserInfoKeyConnection];
        }
        if (error) {
    ...
    ITMServerOriginatedRPC_RPCArgument *argument =
        [[ITMServerOriginatedRPC_RPCArgument alloc] init];
...
- (void)dispatchRPCWithName:(NSString *)fullyQualifiedName
                  arguments:(NSDictionary *)arguments
                 completion:
                     (iTermServerOriginatedRPCCompletionBlock)completion {
  NSError *error = nil;
  ITMServerOriginatedRPC *rpc =
      [self serverOriginatedRPCWithName:fullyQualifiedName
                              arguments:arguments
                                  error:&error];
  if (error) {
  ...
  [self splitFullyQualifiedRPCName:fullyQualifiedName
                         namespace:&namespace
                      relativeName:&relativeName];
  [self dispatchServerOriginatedRPC:rpc
                          namespace:namespace
...
- (NSString *)signatureOfAnyRegisteredFunctionWithName:(NSString *)name
                                             namespace:(NSString *)namespace {
  NSString *fqName = [self fullyQualifiedNameFromRelativeName:name
                                                    namespace:namespace];
  for (NSString *key in self.serverOriginatedRPCSubscriptions) {
    iTermTuple<id, ITMNotificationRequest *> *sub =
        self.serverOriginatedRPCSubscriptions[key];
  ...
  return [iTermBuiltInFunctions.sharedInstance
      signatureOfAnyRegisteredFunctionWithName:fqName];
}
...
- (void)dispatchServerOriginatedRPC:(ITMServerOriginatedRPC *)rpc
                          namespace:(NSString *)namespace
                         completion:(iTermServerOriginatedRPCCompletionBlock)
                                        completion {
  ...
  if (!connectionKey) {
    NSString *reason = [NSString
        stringWithFormat:
            @"No function registered for invocation “%@”. Ensure the script is "
            @"running and the function name and argument names are correct.",
            signature];
    NSString *bestMatch =
        [self signatureOfAnyRegisteredFunctionWithName:rpc.name
                                             namespace:namespace];
    if (bestMatch) {
      reason = [reason
          stringByAppendingFormat:@" There is a similarly named function "
                                  @"available with a different signature: %@",
                                  bestMatch];
    }
    completion(nil, [self rpcDispatchError:reason
                                    detail:nil
                              unregistered:YES
                             connectionKey:nil]);
    return;
...
- (NSError *)rpcDispatchError:(NSString *)reason
  ...
  if (detail) {
    userInfo =
        [userInfo dictionaryBySettingObject:detail
  ...
  return [NSError
      errorWithDomain:iTermAPIHelperErrorDomain
...
- (void)dispatchRPC:(ITMServerOriginatedRPC *)rpc
  ...
  [outstanding
      addObject:notification.serverOriginatedRpcNotification.requestId];
  ...
  const NSTimeInterval timeoutSeconds =
      handler.rpcRegistrationRequest.hasTimeout
          ? handler.rpcRegistrationRequest.timeout
          : 5;
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{
        [weakSelf
            checkForRPCTimeout:notification.serverOriginatedRpcNotification
                                   .requestId
                 connectionKey:connectionKey];
      });
}
...
- (void)checkForRPCTimeout:(NSString *)requestID
             connectionKey:(NSString *)connectionKey {
  iTermServerOriginatedRPCCompletionBlock completion =
      _serverOriginatedRPCCompletionBlocks[requestID];
  if (!completion) {
    return;
  }
...
- (NSString *)nextServerOriginatedRPCRequestIDWithCompletion:
    (iTermServerOriginatedRPCCompletionBlock)completion {
...
- (NSDictionary<NSString *, NSArray<NSString *> *> *)
  ...
  for (NSString *stringSignature in self.serverOriginatedRPCSubscriptions
           .allKeys) {
    ITMNotificationRequest *req =
        self.serverOriginatedRPCSubscriptions[stringSignature].secondObject;
    if (!req) {
      continue;
    }
    ITMRPCRegistrationRequest *sig = req.rpcRegistrationRequest;
    NSString *functionName = [sig it_fullyQualifiedName];
    NSArray<NSString *> *args = [sig.argumentsArray
        mapWithBlock:^id(
            ITMRPCRegistrationRequest_RPCArgumentSignature *anObject) {
          return anObject.name;
        }];
...
+ (NSString *)
    invocationWithFullyQualifiedName:(NSString *)fullyQualifiedName
                            defaults:
                                (NSArray<ITMRPCRegistrationRequest_RPCArgument
                                             *> *)defaultsArray {
  NSArray<ITMRPCRegistrationRequest_RPCArgument *> *sortedDefaults =
      [defaultsArray sortedArrayUsingComparator:^NSComparisonResult(
                         ITMRPCRegistrationRequest_RPCArgument *_Nonnull obj1,
                         ITMRPCRegistrationRequest_RPCArgument *_Nonnull obj2) {
        return [obj1.name compare:obj2.name];
      }];
  NSArray<NSString *> *defaults = [sortedDefaults
      mapWithBlock:^id(ITMRPCRegistrationRequest_RPCArgument *def) {
        return [NSString stringWithFormat:@"%@:%@", def.name, def.path];
      }];
  defaults = [defaults sortedArrayUsingSelector:@selector(compare:)];
  return [NSString stringWithFormat:@"%@(%@)", fullyQualifiedName,
                                    [defaults componentsJoinedByString:@","]];
}

+ (NSString *)userDefaultsKeyForNameOfScriptVendingStatusBarComponentWithID:
    (NSString *)uniqueID {
  return [NSString
      stringWithFormat:@"NoSyncScriptNameForStatusBarComponent_%@", uniqueID];
}
...
+ (NSString *)nameOfScriptVendingStatusBarComponentWithUniqueIdentifier:
  ...
  return [[NSUserDefaults standardUserDefaults]
      objectForKey:
          [self userDefaultsKeyForNameOfScriptVendingStatusBarComponentWithID:
                    uniqueID]];
}

- (NSArray<iTermSessionTitleProvider *> *)sessionTitleFunctions {
  return [self.serverOriginatedRPCSubscriptions.allKeys
      mapWithBlock:^id(NSString *signature) {
        ITMNotificationRequest *req =
            self.serverOriginatedRPCSubscriptions[signature].secondObject;
        if (!req) {
          return nil;
        }
        return
            [[iTermSessionTitleProvider alloc] initWithNotificationRequest:req];
      }];
}
...
+ (ITMRPCRegistrationRequest *)
  ...
  return [[[self sharedInstance] statusBarComponentProviderRegistrationRequests]
      objectPassingTest:^BOOL(ITMRPCRegistrationRequest *request,
                              NSUInteger index, BOOL *stop) {
        return [request.statusBarComponentAttributes.uniqueIdentifier
            isEqualToString:uniqueIdentifier];
      }];
}
...
- (BOOL)haveRegisteredFunctionWithName:(NSString *)name
  ...
  NSString *stringSignature =
      iTermFunctionSignatureFromNamespaceAndNameAndArguments(namespace, name,
                                                             arguments);
...
- (BOOL)haveRegisteredFunctionWithSignature:(NSString *)stringSignature {
  return self.serverOriginatedRPCSubscriptions[stringSignature].secondObject !=
         nil;
}
...
- (void)logToConnectionHostingFunctionWithSignature:(NSString *)signatureString
  ...
  va_end(arguments);
  NSString *connectionKey =
      [self.serverOriginatedRPCSubscriptions[signatureString] firstObject];
...
- (iTermScriptHistoryEntry *)scriptHistoryEntryForConnectionKey:
    (NSString *)connectionKey {
  NSString *key = connectionKey
                      ? [_apiServer websocketKeyForConnectionKey:connectionKey]
                      : nil;
  iTermScriptHistoryEntry *entry =
      key ? [[iTermScriptHistory sharedInstance] entryWithIdentifier:key] : nil;
  return entry;
}
...
- (void)logToConnectionWithKey:(NSString *)connectionKey
                        string:(NSString *)string {
  iTermScriptHistoryEntry *entry =
      [self scriptHistoryEntryForConnectionKey:connectionKey];
  if (!entry) {
...
- (NSMenuItem *)menuItemWithTitleParts:(NSArray<NSString *> *)titleParts
                                inMenu:(NSMenu *)menu
    NS_DEPRECATED_MAC(10_10, 10_11, "Use menuItemWithIdentifier:") {
...
- (NSMenuItem *)menuItemWithIdentifier:(NSString *)identifier
  ...
  for (NSMenuItem *item in [menu itemArray]) {
    if ([item hasSubmenu]) {
      NSMenuItem *result = [self menuItemWithIdentifier:identifier
                                                 inMenu:item.submenu];
      if (result) {
        return result;
      }
...
- (BOOL)apiServerAuthorizeProcesses:(NSArray<NSNumber *> *)pids
                      ...
                      disableAuthUI:(BOOL)disableAuthUI
                       advisoryName:(NSString *)advisoryName
                        ...
                        displayName:
                            (out NSString *__autoreleasing *)displayName {
  *displayName =
      advisoryName ? [@"≈" stringByAppendingString:advisoryName] : @"Unknown";
  ...
  if (![iTermAPIHelper requireApplescriptAuth]) {
    *reason = @"All apps are allowed to use the API, per “Prefs > General > "
              @"Magic > Allow all apps to connect”.";
    return YES;
  }
  if (disableAuthUI) {
  ...
  NSString *message = @"Another process is trying to use the iTerm2 API. The "
                      @"API allows a script to control iTerm2 and view and "
                      @"modify its contents. Allow the connection?";

  if ([iTermAdvancedSettingsModel setCookie]) {
    message = [NSString
        stringWithFormat:
            @"%@\n\nAlthough you have chosen to allow connections "
            @"automatically, this script has not presented a valid cookie.",
            message];
  }
  ...
  case kiTermWarningSelection3:
    if ([iTermWarning showWarningWithTitle:
                          @"New sessions will contain an environment variable "
                          @"that allows scripts to run without confirmation. "
...
- (PseudoTerminal *)windowControllerWithID:(NSString *)windowID {
  return [[[iTermController sharedInstance] terminals]
      objectPassingTest:^BOOL(PseudoTerminal *windowController,
                              NSUInteger index, BOOL *stop) {
...
- (BOOL)rpcNotificationRequestIsValid:(ITMNotificationRequest *)request
                        connectionKey:(NSString *)connectionKey {
  if (request.argumentsOneOfCase !=
      ITMNotificationRequest_Arguments_OneOfCase_RpcRegistrationRequest) {
    [self logToConnectionWithKey:connectionKey
                          format:@"Expected an RPC registration request, but "
                                 @"got:\n%@",
                                 request];
    return NO;
  }
  NSError *error = nil;
  if (![request.rpcRegistrationRequest
          it_rpcRegistrationRequestValidWithError:&error]) {
    [self logToConnectionWithKey:connectionKey
                          format:@"Malformed RPC function signature: %@",
...
- (NSMutableDictionary<
    id, NSMutableArray<
            iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *> *>
...
- (iTermVariableScope *)scopeForCategory:(ITMVariableScope)category
  ...
  case ITMVariableScope_Session:
    return [[self sessionForAPIIdentifier:identifier
...
- (void)monitorVariableChangesForConnectionKey:(NSString *)connectionKey
                          ...
                          mutableVariablesDict:
                              (NSMutableDictionary<
                                  id, NSMutableArray<iTermTuple<
                                          ITMNotificationRequest *,
                                          iTermVariableReference *> *> *> *)
                                  dict {
  NSString *name = request.variableMonitorRequest.name;
  iTermVariableReference *ref =
      [[iTermVariableReference alloc] initWithPath:name vendor:scope];
  __weak __typeof(ref) weakRef = ref;
  __weak __typeof(self) weakSelf = self;
  ref.onChangeBlock = ^{
    ITMNotification *notification = [weakSelf
        variableChangeNotificationWithScope:request.variableMonitorRequest.scope
  ...
  [dict it_addObject:[iTermTuple tupleWithObject:request andObject:ref]
      toMutableArrayForKey:connectionKey];
}
...
- (BOOL)monitorVariableChangesForConnectionKey:(NSString *)connectionKey
  ...
  iTermVariableScope *scope =
      [self scopeForCategory:request.variableMonitorRequest.scope
                  identifier:identifier];
  if (scope == nil) {
    return NO;
  }
...
- (ITMNotificationResponse *)
    handleVariableChangeNotificationRequest:(ITMNotificationRequest *)request
                              connectionKey:(NSString *)connectionKey {
  ...
  if ([identifier isEqualToString:@"all"]) {
    ...
    case ITMVariableScope_Session:
      return [self
          handleSubscriptionRequestForAllSessionsFromConnectionKey:connectionKey
                                                           request:request];
    case ITMVariableScope_Window:
      return [self
          handleSubscriptionRequestForAllWindowsFromConnectionKey:connectionKey
                                                          request:request];
    case ITMVariableScope_App:
    case ITMVariableScope_Tab:
      // TODO
  ...
  NSMutableDictionary<id, NSMutableArray<iTermTuple<ITMNotificationRequest *,
                                                    iTermVariableReference *> *>
                              *> *subscriptions = [self
      subscriptionsForVariableChangeScope:request.variableMonitorRequest.scope];
  NSMutableArray *array = subscriptions[connectionKey];
  const NSInteger index =
      [array indexOfObjectPassingTest:^BOOL(
                 iTermTuple<ITMNotificationRequest *, iTermVariableReference *>
                     *_Nonnull tuple,
  ...
  if (request.subscribe) {
    ...
    iTermTuple<ITMNotificationRequest *, iTermVariableReference *> *tuple =
        array[index];
...
- (ITMNotification *)variableChangeNotificationWithScope:(ITMVariableScope)scope
  ...
  notification.variableChangedNotification.jsonNewValue =
      [NSJSONSerialization it_jsonStringForObject:newValue];
  return notification;
}

- (void)didRegisterStatusBarComponent:
            (ITMRPCRegistrationRequest_StatusBarComponentAttributes *)attributes
                         onConnection:(NSString *)connectionKey {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermAPIDidRegisterStatusBarComponentNotification
  ...
  return [[NSUserDefaults standardUserDefaults]
      setObject:fullPath
         forKey:
             [self.class
                 userDefaultsKeyForNameOfScriptVendingStatusBarComponentWithID:
                     uniqueID]];
}
...
- (ITMNotificationResponse *)
  ...
  if (request.notificationType == ITMNotificationType_NotifyOnNewSession) {
    ...
    NSString *signatureString =
        request.rpcRegistrationRequest.it_stringRepresentation;
    if (request.subscribe) {
      ...
      [self setServerOriginatedRPCSubscriptionWithSignature:signatureString
                                              connectionKey:connectionKey
                                                    request:request];
      [[NSNotificationCenter defaultCenter]
          postNotificationName:iTermAPIRegisteredFunctionsDidChangeNotification
                        object:nil];
      ...
      case ITMRPCRegistrationRequest_Role_StatusBarComponent:
        [self didRegisterStatusBarComponent:request.rpcRegistrationRequest
                                                .statusBarComponentAttributes
...
- (NSArray<PTYSession *> *)allSessions {
  return [[[iTermController sharedInstance] terminals]
      flatMapWithBlock:^id(PseudoTerminal *windowController) {
...
- (BOOL)
    unsubscribeFromVariableChangeNotificationsForAllObjects:
        (NSArray<id<iTermSubscribable>> *)objects
                                                mutableSubs:
                                                    (NSMutableDictionary<
                                                        id,
                                                        NSMutableArray<iTermTuple<
                                                            ITMNotificationRequest
                                                                *,
                                                            iTermVariableReference
                                                                *> *> *> *)subs
  ...
  NSIndexSet *indexes =
      [array indexesOfObjectsPassingTest:^BOOL(
                 iTermTuple<ITMNotificationRequest *, iTermVariableReference *>
                     *_Nonnull tuple,
                 NSUInteger idx, BOOL *_Nonnull stop) {
        return ([NSObject object:tuple.firstObject.variableMonitorRequest
...
- (void)
    subscribeToVariableChangeNotificationsForAllObjects:
        (NSArray<id<iTermSubscribable>> *)objects
                                   mutableVariablesDict:
                                       (NSMutableDictionary<
                                           id, NSMutableArray<iTermTuple<
...
- (ITMNotificationResponse *)
    handleVariableSubscriptionRequestForAllObjects:
        (NSArray<id<iTermSubscribable>> *)objects
                              mutableVariablesDict:
                                  (NSMutableDictionary<
                                      id, NSMutableArray<iTermTuple<
  ...
  const BOOL wasSubscribed = [self
      unsubscribeFromVariableChangeNotificationsForAllObjects:objects
                                                  mutableSubs:dict
                                             forConnectionKey:connectionKey
                                                      request:request];
  ITMNotificationResponse *response = [[ITMNotificationResponse alloc] init];
...
- (ITMNotificationResponse *)
    handleSubscriptionRequestForAllObjects:
        (NSArray<id<iTermSubscribable>> *)objects
                               mutableSubs:(NSMutableArray<
                                               iTermAllObjectsSubscription *> *)
                                               subscriptions
                      mutableVariablesDict:
                          (NSMutableDictionary<
                              id, NSMutableArray<iTermTuple<
  ...
  if (request.notificationType == ITMNotificationType_NotifyOnVariableChange) {
    ITMNotificationResponse *response =
        [self handleVariableSubscriptionRequestForAllObjects:objects
                                        mutableVariablesDict:dict
    ...
    for (id<iTermSubscribable> object in objects) {
      ITMNotificationResponse *response =
          [object handleAPINotificationRequest:request
                                 connectionKey:connectionKey];
  ...
  if (request.subscribe) {
    iTermAllObjectsSubscription *sub =
        [[iTermAllObjectsSubscription alloc] init];
...
- (ITMNotificationResponse *)
    handleSubscriptionRequestForAllSessionsFromConnectionKey:
        (NSString *)connectionKey
                                                     request:
                                                         (ITMNotificationRequest
                                                              *)request {
  return [self
      handleSubscriptionRequestForAllObjects:[self allSessions]
...
- (ITMNotificationResponse *)
    handleSubscriptionRequestForAllWindowsFromConnectionKey:
        (NSString *)connectionKey
                                                    request:
                                                        (ITMNotificationRequest
                                                             *)request {
  return [self
      handleSubscriptionRequestForAllObjects:[[iTermController sharedInstance]
                                                 terminals]
...
- (void)apiServerNotification:(ITMNotificationRequest *)request
  ...
  if (request.notificationType == ITMNotificationType_NotifyOnNewSession ||
    ...
    ITMNotificationResponse *response = [self
        handleSubscriptionRequestForAllSessionsFromConnectionKey:connectionKey
                                                         request:request];
    handler(response);
  } else {
...
- (void)removeAllSubscriptionsForConnectionKey:(id)connectionKey {
  ...
  // RPCs are special.
  NSInteger rpcsRemoved =
      [self removeServerOriginatedRPCSubscriptionsPasstingTest:^BOOL(
                NSString *signature,
                iTermTuple<id, ITMNotificationRequest *> *tuple) {
  ...
  NSArray<NSMutableDictionary<id, ITMNotificationRequest *> *> *dicts = @[
    ...
    _layoutChangeSubscriptions ?: empty, _focusChangeSubscriptions ?: empty,
    _broadcastDomainChangeSubscriptions ?: empty,
    _allSessionVariableSubscriptions ?: empty,
    _allWindowVariableSubscriptions ?: empty
  ];
  [dicts enumerateObjectsUsingBlock:^(
             NSMutableDictionary<id, ITMNotificationRequest *> *_Nonnull dict,
             NSUInteger idx, BOOL *_Nonnull stop) {
    [dict removeObjectForKey:connectionKey];
  }];
  [_allSessionsSubscriptions
      removeObjectsPassingTest:^BOOL(iTermAllObjectsSubscription *sub) {
        return [sub.connectionKey isEqual:connectionKey];
      }];
  [_allWindowsSubscriptions
      removeObjectsPassingTest:^BOOL(iTermAllObjectsSubscription *sub) {
        return [sub.connectionKey isEqual:connectionKey];
      }];
  if (rpcsRemoved) {
...
- (void)apiServerDidCloseConnectionWithKey:(id)connectionKey {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermRemoveAPIServerSubscriptionsNotification
  ...
  for (NSString *requestID in requestIDs) {
    ...
    if (completion) {
      [_serverOriginatedRPCCompletionBlocks removeObjectForKey:requestID];
      completion(
          nil, [self rpcDispatchError:@"Script terminated during function call"
                               detail:nil
                         unregistered:YES
                        connectionKey:connectionKey]);
    }
...
- (void)apiServerRegisterTool:(ITMRegisterToolRequest *)request
  ...
  [iTermToolbeltView
      registerDynamicToolWithIdentifier:request.identifier
                                   name:request.name
                                    URL:request.URL
...
- (void)apiServerSetProfileProperty:(ITMSetProfilePropertyRequest *)request
                            handler:(void (^)(ITMSetProfilePropertyResponse *))
                                        handler {
  ITMSetProfilePropertyResponse *response =
      [[ITMSetProfilePropertyResponse alloc] init];
  ITMSetProfilePropertyResponse_Status (^setter)(
      id object, NSArray<iTermTuple<NSString *, id> *> *assignments) = nil;
  ...
  case ITMSetProfilePropertyRequest_Target_OneOfCase_GuidList: {
    setter = ^ITMSetProfilePropertyResponse_Status(
        id object, NSArray<iTermTuple<NSString *, id> *> *assignments) {
  ...
  case ITMSetProfilePropertyRequest_Target_OneOfCase_Session: {
    setter = ^ITMSetProfilePropertyResponse_Status(
      ...
      return [(PTYSession *)object
          handleSetProfilePropertyForAssignments:assignments
  ...
  if (request.hasKey && request.hasJsonValue) {
    // DEPRECATED CODE PATH - REMOVE THIS AFTER ENSURING EVERYONE HAS BEEN
    // FORCED TO UPGRADE TO 0.69
    assignments = @[ [iTermTuple tupleWithObject:request.key
                                       andObject:request.jsonValue] ];
  } else {
    assignments = [request.assignmentsArray
        mapWithBlock:^id(ITMSetProfilePropertyRequest_Assignment *assignment) {
          return [iTermTuple tupleWithObject:assignment.key
                                   andObject:assignment.jsonValue];
        }];
  }

  __block NSError *error = nil;
  assignments = [assignments mapWithBlock:^id(
                                 iTermTuple<NSString *, id> *tuple) {
    if (error) {
      return nil;
    }
    id value = [NSJSONSerialization
        JSONObjectWithData:[tuple.secondObject
                               dataUsingEncoding:NSUTF8StringEncoding]
...
- (void)apiServerGetProfileProperty:(ITMGetProfilePropertyRequest *)request
                            handler:(void (^)(ITMGetProfilePropertyResponse *))
                                        handler {
  PTYSession *session = [self sessionForAPIIdentifier:request.session
                                includeBuriedSessions:YES];
  if (!session) {
    ITMGetProfilePropertyResponse *response =
        [[ITMGetProfilePropertyResponse alloc] init];
...
- (ITMListSessionsResponse *)newListSessionsResponse {
  ...
  for (PseudoTerminal *window in [[iTermController sharedInstance] terminals]) {
    ITMListSessionsResponse_Window *windowMessage =
        [[ITMListSessionsResponse_Window alloc] init];
    ...
    for (PTYTab *tab in window.tabs) {
      ITMListSessionsResponse_Tab *tabMessage =
          [[ITMListSessionsResponse_Tab alloc] init];
...
- (void)apiServerCreateTab:(ITMCreateTabRequest *)request
  ...
  if (request.hasProfileName) {
    profile =
        [[ProfileModel sharedInstance] bookmarkWithName:request.profileName];
    if (!profile) {
      ITMCreateTabResponse *response = [[ITMCreateTabResponse alloc] init];
  ...
  iTermSessionLauncher *launcher =
      [[iTermSessionLauncher alloc] initWithProfile:profile
                                   windowController:term];
  launcher.canActivate = NO;
  launcher.makeSession =
      ^(NSDictionary *_Nonnull profile, PseudoTerminal *_Nonnull term,
        void (^_Nonnull didMakeSession)(PTYSession *_Nullable)) {
        profile =
            [self profileByCustomizing:profile
                        withProperties:request.customProfilePropertiesArray];
...
- (void)apiServerSplitPane:(ITMSplitPaneRequest *)request
  ...
  profile = [self profileByCustomizing:profile
                        withProperties:request.customProfilePropertiesArray];
  if (!profile) {
    ITMSplitPaneResponse *response = [[ITMSplitPaneResponse alloc] init];
    response.status =
  ...
  for (PTYSession *session in sessions) {
    PseudoTerminal *term =
        [[iTermController sharedInstance] terminalWithSession:session];
    const BOOL vertical =
        request.splitDirection == ITMSplitPaneRequest_SplitDirection_Vertical;
    ...
    [term asyncSplitVertically:vertical
                 ...
                 targetSession:session
                    completion:^(PTYSession *newSession, BOOL ok) {
                      if (newSession &&
                          newSession.guid) { // The test for newSession.guid is
                                             // just to quiet the analyzer
...
- (Profile *)profileByCustomizing:(Profile *)profile
                   withProperties:(NSArray<ITMProfileProperty *> *)
                                      customProfilePropertiesArray {
...
- (void)apiServerSetProperty:(ITMSetPropertyRequest *)request
  ...
  NSError *error = nil;
  id value = [NSJSONSerialization
      JSONObjectWithData:[request.jsonValue
                             dataUsingEncoding:NSUTF8StringEncoding]
                 options:NSJSONReadingAllowFragments
                   error:&error];
  if (!value || error) {
  ...
  case ITMSetPropertyRequest_Identifier_OneOfCase_WindowId: {
    PseudoTerminal *term =
        [[iTermController sharedInstance] terminalWithGuid:request.windowId];
    if (!term) {
      response.status = ITMSetPropertyResponse_Status_InvalidTarget;
      handler(response);
      return;
    }
    [self setPropertyInWindow:term
                         name:request.name
                        value:value
                   completion:handler];
    return;
  ...
  case ITMSetPropertyRequest_Identifier_OneOfCase_SessionId: {
    if ([request.sessionId isEqualToString:@"all"]) {
      for (PTYSession *session in [self allSessions]) {
        response.status = [self setPropertyInSession:session
                                                name:request.name
                                               value:value];
        handler(response);
        return;
...
- (void)setPropertyInWindow:(PseudoTerminal *)term
  ...
  void (^setFrame)(void) = ^{
    ...
    NSRect rect = NSMakeRect(x.doubleValue, y.doubleValue, width.doubleValue,
                             height.doubleValue);
  ...
  NSDictionary<NSString *, void (^)(void)> *handlers =
      @{@"frame" : setFrame, @"fullscreen" : setFullScreen};
...
- (ITMSetPropertyResponse_Status)setPropertyInSession:(PTYSession *)session
                                                 name:(NSString *)name
                                                value:(id)value {
  ...
  SetSessionPropertyBlock setGridSize = ^ITMSetPropertyResponse_Status {
    ...
    BOOL ok = [controller sessionInitiatedResize:session
  ...
  SetSessionPropertyBlock setBuried = ^ITMSetPropertyResponse_Status {
    ...
    const BOOL isBuried = [[[iTermBuriedSessions sharedInstance] buriedSessions]
        containsObject:session];
...
- (void)apiServerGetProperty:(ITMGetPropertyRequest *)request
  ...
  case ITMGetPropertyRequest_Identifier_OneOfCase_SessionId: {
    ...
    NSString *jsonValue = [self getPropertyFromSession:session
                                                  name:request.name];
    if (jsonValue) {
      response.jsonValue = jsonValue;
      response.status = ITMGetPropertyResponse_Status_Ok;
...
- (NSString *)getPropertyFromWindow:(PseudoTerminal *)term
                               name:(NSString *)name {
  ...
  NSDictionary<NSString *, GetWindowPropertyBlock> *handlers =
      @{@"frame" : getFrame, @"fullscreen" : getFullScreen};
...
- (NSString *)getPropertyFromSession:(PTYSession *)session
                                name:(NSString *)name {
  ...
  GetSessionPropertyBlock getBuried = ^NSString * {
    BOOL isBuried = [[[iTermBuriedSessions sharedInstance] buriedSessions]
        containsObject:session];
    ...
    if ([sessionID isEqualToString:@"all"]) {
      ...
      if (session) {
        ...
        [response.statusArray
            addValue:ITMInjectResponse_Status_SessionNotFound];
      }
    }
  }
  handler(response);
...
- (void)apiServerActivate:(ITMActivateRequest *)request
  ...
  if (request.identifierOneOfCase ==
      ITMActivateRequest_Identifier_OneOfCase_TabId) {
    for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
  ...
  } else if (request.identifierOneOfCase ==
             ITMActivateRequest_Identifier_OneOfCase_WindowId) {
    windowController = [[[iTermController sharedInstance] terminals]
        objectPassingTest:^BOOL(PseudoTerminal *element, NSUInteger index,
                                BOOL *stop) {
    ...
    if (!windowController) {
      ...
      return;
             ...
             ITMActivateRequest_Identifier_OneOfCase_SessionId) {
...
- (void)apiServerVariable:(ITMVariableRequest *)request
  ...
  const BOOL allSetNamesLegal =
      [request.setArray allWithBlock:^BOOL(ITMVariableRequest_Set *setRequest) {
        return ([setRequest.name hasPrefix:userPrefix] &&
                [[setRequest.name substringFromIndex:userPrefix.length]
                    rangeOfString:@"."]
                        .location == NSNotFound);
      }];
  if (!allSetNamesLegal) {
...
- (void)handleAppScopeVariableRequest:(ITMVariableRequest *)request
  ...
  ITMVariableResponse *response = [[ITMVariableResponse alloc] init];
  BOOL ok =
      [self handleVariableSetsInRequest:request
                                  scope:[iTermVariableScope globalsScope]];
  [self handleVariableGetsInRequest:request
                           response:response
                              scope:[iTermVariableScope globalsScope]];
  response.status = ok ? ITMVariableResponse_Status_Ok
...
- (void)handleTabScopeVariableRequest:(ITMVariableRequest *)request
  ...
  if ([request.tabId isEqualToString:@"all"]) {
    NSArray<iTermVariableScope *> *scopes =
        [self.allTabs mapWithBlock:^id(PTYTab *anObject) {
...
- (void)handleWindowScopeVariableRequest:(ITMVariableRequest *)request
                                 handler:
                                     (void (^)(ITMVariableResponse *))handler {
  if ([request.sessionId isEqualToString:@"all"]) {
    NSArray<iTermVariableScope *> *scopes =
        [[iTermController sharedInstance].terminals
            mapWithBlock:^id(PseudoTerminal *anObject) {
  ...
  ITMVariableResponse *response = [[ITMVariableResponse alloc] init];
  PseudoTerminal *windowController =
      [self windowControllerWithID:request.windowId];
  if (!windowController) {
...
- (void)handleSessionScopeVariableRequest:(ITMVariableRequest *)request
  ...
  if ([request.sessionId isEqualToString:@"all"]) {
    NSArray<iTermVariableScope *> *scopes =
        [self.allSessions mapWithBlock:^id(PTYSession *anObject) {
          return anObject.variablesScope;
        }];
    handler([self handleVariableMultiSetRequest:request scopes:scopes]);
    return;
...
- (BOOL)handleVariableSetsInRequest:(ITMVariableRequest *)request
                              scope:(iTermVariableScope *)scope {
  __block BOOL result = YES;
  [request.setArray
      enumerateObjectsUsingBlock:^(ITMVariableRequest_Set *_Nonnull setRequest,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        id value;
...
- (void)handleVariableGetsInRequest:(ITMVariableRequest *)request
                           response:(ITMVariableResponse *)response
                              scope:(iTermVariableScope *)scope {
  [request.getArray
      enumerateObjectsUsingBlock:^(NSString *_Nonnull name, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
        if ([name isEqualToString:@"*"]) {
          NSDictionary *dict = scope.dictionaryWithStringValues;
          [response.valuesArray
              addObject:[NSJSONSerialization it_jsonStringForObject:dict]];
        } else {
          id obj = [NSJSONSerialization
              it_jsonStringForObject:[scope valueForVariableName:name]];
...
- (ITMVariableResponse *)
    handleVariableMultiSetRequest:(ITMVariableRequest *)request
...
- (void)apiServerSavedArrangement:(ITMSavedArrangementRequest *)request
                          handler:
                              (void (^)(ITMSavedArrangementResponse *))handler {
  switch (request.action) {
  case ITMSavedArrangementRequest_Action_Save:
    [self saveArrangementNamed:request.name
                      windowID:request.windowId
                       handler:handler];
    return;

  case ITMSavedArrangementRequest_Action_Restore:
    [self restoreArrangementNamed:request.name
                         windowID:request.windowId
                          handler:handler];
    return;
  ...
  ITMSavedArrangementResponse *response =
      [[ITMSavedArrangementResponse alloc] init];
...
- (void)saveArrangementNamed:(NSString *)name
  ...
  if (windowID.length) {
    PseudoTerminal *term =
        [[iTermController sharedInstance] terminalWithGuid:windowID];
    if (!term) {
    ...
    [[iTermController sharedInstance] saveWindowArrangementForWindow:term
                                                                name:name];
  } else {
    [[iTermController sharedInstance] saveWindowArrangementForAllWindows:YES
                                                                    name:name];
  }
...
- (void)restoreArrangementNamed:(NSString *)name
  ...
  BOOL ok =
      [[iTermController sharedInstance] loadWindowArrangementWithName:name
                                                     asTabsInTerminal:term];
  if (ok) {
    response.status = ITMSavedArrangementResponse_Status_Ok;
  } else {
...
- (void)listSavedArrangementsWithHandler:
    (void (^)(ITMSavedArrangementResponse *))handler {
  ITMSavedArrangementResponse *response =
      [[ITMSavedArrangementResponse alloc] init];
  response.status = ITMSavedArrangementResponse_Status_Ok;
...
- (void)apiServerListProfiles:(ITMListProfilesRequest *)request
                      handler:(void (^)(ITMListProfilesResponse *))handler {
  ...
  for (Profile *profile in [[ProfileModel sharedInstance] bookmarks]) {
    ...
    if (matches) {
      ITMListProfilesResponse_Profile *responseProfile =
          [[ITMListProfilesResponse_Profile alloc] init];
...
- (void)handleServerOriginatedRPCResult:
            (ITMServerOriginatedRPCResultRequest *)result
                          connectionKey:(NSString *)connectionKey {
  ...
  iTermServerOriginatedRPCCompletionBlock block =
      _serverOriginatedRPCCompletionBlocks[key];
  if (!block) {
  ...
  case ITMServerOriginatedRPCResultRequest_Result_OneOfCase_JsonValue:
    value = [NSJSONSerialization
        JSONObjectWithData:[result.jsonValue
                               dataUsingEncoding:NSUTF8StringEncoding]
                   options:NSJSONReadingAllowFragments
                     error:&error];
    if (!value || error) {
      value = nil;
      exception = @{
        @"reason" : [NSString stringWithFormat:@"Undecodable value: %@",
                                               error.localizedDescription]
      };
    }
    break;
  ...
  case ITMServerOriginatedRPCResultRequest_Result_OneOfCase_JsonException:
    exception = [NSJSONSerialization
        JSONObjectWithData:[result.jsonException
                               dataUsingEncoding:NSUTF8StringEncoding]
                   options:NSJSONReadingAllowFragments
                     error:&error];
    if (error) {
      exception = @{
        @"reason" : [NSString stringWithFormat:@"Undecodable exception: %@",
                                               error.localizedDescription]
      };
    } else {
      exception = [NSDictionary castFrom:exception]
                      ?: @{@"reason" : @"Malformed exception"};
    }
    break;
...
- (void)
    apiServerServerOriginatedRPCResult:
        (ITMServerOriginatedRPCResultRequest *)request
                         connectionKey:(NSString *)connectionKey
                               handler:
                                   (void (^)(
  ...
  ITMServerOriginatedRPCResultResponse *response =
      [[ITMServerOriginatedRPCResultResponse alloc] init];
  handler(response);
}
...
- (void)apiServerRestartSession:(ITMRestartSessionRequest *)request
  ...
  PTYSession *session = [self sessionForAPIIdentifier:request.sessionId
                                includeBuriedSessions:YES];
  ITMRestartSessionResponse *response =
      [[ITMRestartSessionResponse alloc] init];
  if (!session) {
...
- (void)apiServerMenuItem:(ITMMenuItemRequest *)request
                  handler:(void (^)(ITMMenuItemResponse *))handler {
  ITMMenuItemResponse *response = [[ITMMenuItemResponse alloc] init];
  NSMenuItem *menuItem = nil;
  menuItem = [self menuItemWithIdentifier:request.identifier
...
static BOOL iTermCheckSplitTreesIsomorphic(ITMSplitTreeNode *node1,
                                           ITMSplitTreeNode *node2) {
  ...
  for (NSInteger i = 0; i < node1.linksArray_Count; i++) {
    ...
    if (link1.childOneOfCase ==
        ITMSplitTreeNode_SplitTreeLink_Child_OneOfCase_GPBUnsetOneOfCase) {
      return NO;
    }
...
- (void)apiServerSetTabLayout:(ITMSetTabLayoutRequest *)request
                      handler:(void (^)(ITMSetTabLayoutResponse *))handler {
...
- (void)apiServerGetBroadcastDomains:(ITMGetBroadcastDomainsRequest *)request
                             handler:
                                 (void (^)(ITMGetBroadcastDomainsResponse *))
                                     handler {
  ITMGetBroadcastDomainsResponse *response =
      [[ITMGetBroadcastDomainsResponse alloc] init];
  [self enumerateBroadcastDomains:^(NSArray<PTYSession *> *sessions) {
    ITMBroadcastDomain *domain = [[ITMBroadcastDomain alloc] init];
    for (PTYSession *session in sessions) {
      [domain.sessionIdsArray addObject:session.guid];
...
- (void)handleTmuxSendCommand:(ITMTmuxRequest_SendCommand *)request
  ...
  TmuxController *controller = [[TmuxControllerRegistry sharedInstance]
      controllerForClient:request.connectionId];
  if (!controller) {
  ...
  iTermBlockTargetActionForwarder *forwarder =
      [[iTermBlockTargetActionForwarder alloc]
          initWithBlock:^(NSString *result) {
            response.sendCommand.output = result;
            handler(response);
          }];
  [controller.gateway sendCommand:request.command
                   responseTarget:forwarder
                 responseSelector:@selector(selector:)];
...
- (void)handleTmuxListConnections:(ITMTmuxRequest_ListConnections *)request
  ...
  for (NSString *clientName in
    ...
    TmuxController *controller = [[TmuxControllerRegistry sharedInstance]
        controllerForClient:clientName];
    connection.owningSessionId =
        [controller.gateway.delegate tmuxOwningSessionGUID];
...
- (void)handleTmuxCreateWindow:(ITMTmuxRequest_CreateWindow *)request
                       handler:(void (^)(ITMTmuxResponse *))handler {
  ITMTmuxResponse *response = [[ITMTmuxResponse alloc] init];
  TmuxController *controller = [[TmuxControllerRegistry sharedInstance]
      controllerForClient:request.connectionId];
  ...
  [controller
      newWindowWithAffinity:request.hasAffinity ? request.affinity : nil
                       size:[PTYTab
                                sizeForTmuxWindowWithAffinity:
                                    request.hasAffinity ? request.affinity : nil
                                                   controller:controller]
           initialDirectory:[iTermInitialDirectory
                                initialDirectoryFromProfile:controller
                                                                .sharedProfile
                 ...
                 completion:^(int newWindowId) {
                   if (newWindowId >= 0) {
                     ...
                     response.createWindow.tabId =
                         [NSString stringWithFormat:@"%d", tab.uniqueId];
                   }
                   handler(response);
                 }];
}
...
- (ITMReorderTabsResponse_Status)validateReorderTabsRequest:
    (ITMReorderTabsRequest *)request {
  NSMutableSet<NSString *> *windowIds = [NSMutableSet set];
  NSMutableSet<NSString *> *tabIds = [NSMutableSet set];
  for (ITMReorderTabsRequest_Assignment *assignment in request
           .assignmentsArray) {
    if ([windowIds containsObject:assignment.windowId]) {
      return ITMReorderTabsResponse_Status_InvalidAssignment;
    }
    PseudoTerminal *term =
        [[iTermController sharedInstance] terminalWithGuid:assignment.windowId];
    if (!term) {
...
- (void)performReorderAssignment:
    (ITMReorderTabsRequest_Assignment *)assignment {
  PseudoTerminal *destination =
      [[iTermController sharedInstance] terminalWithGuid:assignment.windowId];
  assert(destination);
...
- (void)apiServerReorderTabsRequest:(ITMReorderTabsRequest *)request
                            handler:
                                (void (^)(ITMReorderTabsResponse *))handler {
...
- (void)apiServerPreferencesRequest:(ITMPreferencesRequest *)request
                            handler:
                                (void (^)(ITMPreferencesResponse *))handler {
  ITMPreferencesResponse *response = [[ITMPreferencesResponse alloc] init];

  [request.requestsArray
      enumerateObjectsUsingBlock:^(ITMPreferencesRequest_Request *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        ITMPreferencesResponse_Result *result =
            [self handlePreferencesRequest:obj];
...
- (ITMPreferencesResponse_Result *)handlePreferencesRequest:
    (ITMPreferencesRequest_Request *)request {
  ITMPreferencesResponse_Result *result =
      [[ITMPreferencesResponse_Result alloc] init];
  switch (request.requestOneOfCase) {
  case ITMPreferencesRequest_Request_Request_OneOfCase_GetPreferenceRequest:
    result.getPreferenceResult = [self
        handleGetPreferenceRequestForKey:request.getPreferenceRequest.key];
    break;

  case ITMPreferencesRequest_Request_Request_OneOfCase_GPBUnsetOneOfCase:
    result.unrecognizedRequest =
        [[ITMPreferencesResponse_Result_UnrecognizedResult alloc] init];
    break;
  ...
  case ITMPreferencesRequest_Request_Request_OneOfCase_SetPreferenceRequest:
    result.setPreferenceResult =
        [self handleSetPreferenceRequestWithKey:request.setPreferenceRequest.key
                                      jsonValue:request.setPreferenceRequest
                                                    .jsonValue];
    break;
  case ITMPreferencesRequest_Request_Request_OneOfCase_SetDefaultProfileRequest:
    result.setDefaultProfileResult = [self
        handleSetDefaultProfileWithGUID:request.setDefaultProfileRequest.guid];
    break;
...
- (ITMPreferencesResponse_Result_GetPreferenceResult *)
    handleGetPreferenceRequestForKey:(NSString *)key {
...
- (ITMPreferencesResponse_Result_SetPreferenceResult *)
    handleSetPreferenceRequestWithKey:(NSString *)key
...
- (ITMPreferencesResponse_Result_GetDefaultProfileResult *)
    handleGetDefaultProfile {
  ...
  result.guid =
      [[[ProfileModel sharedInstance] defaultBookmark] objectForKey:KEY_GUID];
  return result;
}

- (ITMPreferencesResponse_Result_SetDefaultProfileResult *)
    handleSetDefaultProfileWithGUID:(NSString *)guid {
...
- (void)apiServerColorPresetRequest:(ITMColorPresetRequest *)request
                            handler:
                                (void (^)(ITMColorPresetResponse *))response {
  switch (request.requestOneOfCase) {
...
- (void)handleGetPreset:(ITMColorPresetRequest_GetPreset *)request
  ...
  for (NSString *key in preset) {
    ITMColorPresetResponse_GetPreset_ColorSetting *colorSetting =
        [self colorSettingForDictionary:preset[key] key:key];
...
- (ITMColorPresetResponse_GetPreset_ColorSetting *)
    colorSettingForDictionary:(iTermColorDictionary *)dict
                          key:(NSString *)key {
  ...
  obj = dict[kEncodedColorDictionaryAlphaComponent]
            ?: @([NSDictionary defaultAlphaForColorPresetKey:key]);
  setting.alpha = [obj doubleValue];

  NSString *colorSpace = dict[kEncodedColorDictionaryColorSpace]
                             ?: kEncodedColorDictionaryCalibratedColorSpace;
...
- (void)apiServerSelectionRequest:(ITMSelectionRequest *)request
                          handler:(void (^)(ITMSelectionResponse *))completion {
  switch (request.requestOneOfCase) {
  case ITMSelectionRequest_Request_OneOfCase_GetSelectionRequest:
    [self handleGetSelectionRequest:request.getSelectionRequest
                         completion:completion];
    return;

  case ITMSelectionRequest_Request_OneOfCase_SetSelectionRequest:
    [self handleSetSelectionRequest:request.setSelectionRequest
                         completion:completion];
    return;
...
- (void)handleGetSelectionRequest:
            (ITMSelectionRequest_GetSelectionRequest *)request
  ...
  for (iTermSubSelection *sub in selection.allSubSelections) {
    ...
    const VT100GridWindowedRange relativeRange =
        VT100GridWindowedRangeFromAbsWindowedRange(sub.absRange, overflow);
    subProto.windowedCoordRange.coordRange.start.x =
        relativeRange.coordRange.start.x;
    subProto.windowedCoordRange.coordRange.start.y =
        absoluteOffset + relativeRange.coordRange.start.y;
    subProto.windowedCoordRange.coordRange.end.x =
        relativeRange.coordRange.end.x;
    subProto.windowedCoordRange.coordRange.end.y =
        absoluteOffset + relativeRange.coordRange.end.y;
    subProto.connected = sub.connected;
    if (relativeRange.columnWindow.length > 0) {
      subProto.windowedCoordRange.columns.location =
          relativeRange.columnWindow.location;
      subProto.windowedCoordRange.columns.length =
          relativeRange.columnWindow.length;
    }
    ...
    [response.getSelectionResponse.selection.subSelectionsArray
        addObject:subProto];
  }
...
- (void)handleSetSelectionRequest:
            (ITMSelectionRequest_SetSelectionRequest *)request
                       completion:(void (^)(ITMSelectionResponse *))completion {
  PTYSession *session = [self sessionForAPIIdentifier:request.sessionId
                                includeBuriedSessions:YES];
  ITMSelectionResponse *response = [[ITMSelectionResponse alloc] init];
  ...
  NSArray<iTermSubSelection *> *subSelections =
      [request.selection.subSelectionsArray
          mapWithBlock:^id(ITMSubSelection *subProto) {
            ...
            VT100GridCoordRange coordRange = VT100GridCoordRangeMake(
                subProto.windowedCoordRange.coordRange.start.x,
                MAX(0, subProto.windowedCoordRange.coordRange.start.y -
                           absoluteOffset),
                subProto.windowedCoordRange.coordRange.end.x,
                MAX(0, subProto.windowedCoordRange.coordRange.end.y -
                           absoluteOffset));
            VT100GridWindowedRange range = VT100GridWindowedRangeMake(
                coordRange, subProto.windowedCoordRange.columns.location,
                subProto.windowedCoordRange.columns.length);
            ...
            VT100GridAbsWindowedRange absRange =
                VT100GridAbsWindowedRangeFromWindowedRange(range, overflow);
...
- (void)
    apiServerStatusBarComponentRequest:(ITMStatusBarComponentRequest *)request
                               handler:
                                   (void (^)(ITMStatusBarComponentResponse *))
                                       completion {
  switch (request.requestOneOfCase) {
  ...
  ITMStatusBarComponentResponse *response =
      [[ITMStatusBarComponentResponse alloc] init];
  response.status = ITMStatusBarComponentResponse_Status_RequestMalformed;
  completion(response);
}

- (void)handleOpenStatusBarPopoverRequest:
            (ITMStatusBarComponentRequest_OpenPopover *)request
                               identifier:(NSString *)identifier
                               completion:
                                   (void (^)(ITMStatusBarComponentResponse *))
                                       completion {
  ...
  id<iTermStatusBarComponent> component =
      [session.statusBarViewController componentWithIdentifier:identifier];
  if (!component) {
  ...
  [component
      statusBarComponentOpenPopoverWithHTML:request.html
                                     ofSize:NSMakeSize(request.size.width,
                                                       request.size.height)];
...
- (void)
    apiServerSetBroadcastDomainsRequest:(ITMSetBroadcastDomainsRequest *)request
                                handler:
                                    (void (^)(ITMSetBroadcastDomainsResponse *))
                                        completion {
  ITMSetBroadcastDomainsResponse *response =
      [self handleSetBroadcastDomains:request];
  completion(response);
}

- (ITMSetBroadcastDomainsResponse *)handleSetBroadcastDomains:
    (ITMSetBroadcastDomainsRequest *)request {
  ITMSetBroadcastDomainsResponse *response =
      [[ITMSetBroadcastDomainsResponse alloc] init];
  ...
  for (ITMBroadcastDomain *domain in request.broadcastDomainsArray) {
    ...
    for (NSString *sessionID in domain.sessionIdsArray) {
      ...
      id<iTermWindowController> thisWindowController =
          [session.delegate realParentWindow];
      if (!windowController) {
  ...
  for (PseudoTerminal *term in [[iTermController sharedInstance] terminals]) {
    NSArray<NSArray<PTYSession *> *> *sessionGroupsForWindow = [sessionGroups
        filteredArrayUsingBlock:^BOOL(NSArray<PTYSession *> *sessionGroup) {
          return [PseudoTerminal castFrom:sessionGroup.firstObject.delegate
                                              .realParentWindow] == term;
        }];
...
- (void)apiServerCloseRequest:(ITMCloseRequest *)request
                      handler:(void (^)(ITMCloseResponse *))completion {
...
- (void)apiServerInvokeFunctionRequest:(ITMInvokeFunctionRequest *)request
  ...
  case ITMInvokeFunctionRequest_Context_OneOfCase_Method:
    if (!request.method.hasReceiver) {
      ITMInvokeFunctionResponse *response =
          [[ITMInvokeFunctionResponse alloc] init];
...
- (void)functionInvocationDidCompleteWithObject:(id)object
                                          error:(NSError *)error
                                     completion:
                                         (void (^)(ITMInvokeFunctionResponse *))
                                             completion {
  ITMInvokeFunctionResponse *response =
      [[ITMInvokeFunctionResponse alloc] init];
  if (error) {
