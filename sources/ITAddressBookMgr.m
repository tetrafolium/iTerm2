...
NSString *const iTermUnicodeVersionDidChangeNotification =
    @"iTermUnicodeVersionDidChangeNotification";
...
- (instancetype)init {
  ...
  if (self) {
    ...
    if ([prefs objectForKey:KEY_DEPRECATED_BOOKMARKS] &&
        [[prefs objectForKey:KEY_DEPRECATED_BOOKMARKS]
            isKindOfClass:[NSDictionary class]] &&
        ![prefs objectForKey:KEY_NEW_BOOKMARKS]) {
      // Have only old-style bookmarks. Load them and convert them to new-style
      // bookmarks.
      [iTermMigrationHelper
          recursiveMigrateBookmarks:[prefs
                                        objectForKey:KEY_DEPRECATED_BOOKMARKS]
                               path:@[]];
      [prefs removeObjectForKey:KEY_DEPRECATED_BOOKMARKS];
      [prefs setObject:[[ProfileModel sharedInstance] rawData]
                forKey:KEY_NEW_BOOKMARKS];
      [[ProfileModel sharedInstance] removeAllBookmarks];
    }

    iTermProfilesMenuController *menuController =
        [[iTermProfilesMenuController alloc] init];
    ...
    NSString *originalDefaultGuid =
        [[prefs objectForKey:KEY_DEFAULT_GUID] copy];
    if ([newBookmarks isKindOfClass:[NSArray class]]) {
      ...
      if ([_dynamicProfileManager
              loadDynamicProfilesFromFile:(NSString *)newBookmarks
                                intoArray:profiles
                                    guids:guids] &&
          [profiles count] > 0) {
    ...
    BOOL bookmarkWithDefaultGuidExisted =
        ([[ProfileModel sharedInstance] bookmarkWithGuid:originalDefaultGuid] !=
         nil);
...
- (void)locateBonjourServices {
  if (!bonjourServices) {
    ...
    [telnetBonjourBrowser searchForServicesOfType:@"_telnet._tcp."
                                         inDomain:@""];
  }
}
...
- (BOOL)verbose {
  return
      [[NSUserDefaults standardUserDefaults] boolForKey:@"iTermDebugBonjour"];
}
...
#ifdef SUPPORT_SFTP
  NSString *sftpName =
      [NSString stringWithFormat:@"%@-sftp", [aNetService name]];
#endif
  for (NSNumber *n in [[ProfileModel sharedInstance]
           bookmarkIndicesMatchingFilter:@"tag:^bonjour$"]) {
    int i = [n intValue];
...
+ (void)setDefaultsInBookmark:(NSMutableDictionary *)aDict {
  ...
  aName = NSLocalizedStringFromTableInBundle(
      @"Default", @"iTerm", [NSBundle bundleForClass:[self class]],
      @"Terminal Profiles");
  [aDict setObject:aName forKey:KEY_NAME];
  [aDict setObject:kProfilePreferenceCommandTypeLoginShellValue
...
- (BOOL)usernameIsSafe:(NSString *)username {
  NSCharacterSet *unsafeSet = [[NSCharacterSet
      characterSetWithCharactersInString:
          @"abcdefghijklmnopqrstuvwxyz_1234567890"] invertedSet];
  const NSRange range =
      [username rangeOfCharacterFromSet:unsafeSet
...
- (void)_addBonjourHostProfileWithName:(NSString *)serviceName
  ...
  [newBookmark
      setObject:[NSString stringWithFormat:@"%@ %@%@%@", serviceType,
                                           userNameArg, optionalPortArg,
                                           destination]
  ...
  [newBookmark setObject:[NSArray arrayWithObjects:@"bonjour", nil]
                  forKey:KEY_TAGS];
  ...
  if ([serviceType isEqualToString:@"ssh"]) {
    [newBookmark setObject:[NSString stringWithFormat:@"%@-sftp", serviceName]
                    forKey:KEY_NAME];
    [newBookmark setObject:[NSArray arrayWithObjects:@"bonjour", @"sftp", nil]
                    forKey:KEY_TAGS];
    [newBookmark setObject:[ProfileModel freshGuid] forKey:KEY_GUID];
    [newBookmark
        setObject:[NSString stringWithFormat:@"sftp %@", ipAddressString]
           forKey:KEY_COMMAND_LINE];
    [[ProfileModel sharedInstance] addBookmark:newBookmark];
  }
#endif
...
- (NSString *)usernameFromTXTRecord:(NSData *)txtData {
  NSDictionary<NSString *, NSData *> *txtFields =
      [NSNetService dictionaryFromTXTRecordData:txtData];
  // https://kodi.wiki/view/Avahi_Zeroconf
  NSData *usernameData = txtFields[@"u"] ?: txtFields[@"username"];
  NSString *usernameString =
      [[NSString alloc] initWithData:usernameData
...
- (void)netServiceDidResolveAddress:(NSNetService *)sender {
  ...
  if ([bonjourServices containsObject:sender] == NO) {
    if ([self verbose]) {
      NSLog(@"netServiceDidResolveAddress sender not in services %@",
            bonjourServices);
    }
    return;
  }
  ...
  if ([self verbose]) {
    NSLog(@"netServiceDidResolveAddress type=%@ name=%@ address=%@",
          serviceType, serviceName, address);
  }
  ...
  if (strAddr) {
    if ([self verbose]) {
      NSLog(@"netServiceDidResolveAddress add profile with address %s",
            strAddr);
    }
...
+ (NSString *)sanitizedCustomShell:(NSString *)customShell {
  NSArray<NSString *> *parts = [customShell
...
+ (NSString *)shellLauncherCommandWithCustomShell:(NSString *)customShell {
  ...
  return [NSString
      stringWithFormat:
          @"/usr/bin/login -f%@pl %@ %@ --launch_shell%@",
          [self hushlogin] ? @"q" : @"",
          [NSUserName()
...
+ (NSString *)loginShellCommandForBookmark:(Profile *)profile
  ...
  if ([profile[KEY_CUSTOM_DIRECTORY]
  ...
  if ([customDirectoryString
          isEqualToString:kProfilePreferenceInitialDirectoryHomeValue] &&
      [[self customShellForProfile:profile] length] == 0) {
    // Run login without -l argument: this is a login session and will use the
    // home dir.
    return [self standardLoginCommand];
  } else {
    // Not using the home directory/default shell. This requires some trickery.
    // Run iTerm2's executable with a special flag that makes it run the shell
    // as a login shell (with "-" inserted at the start of argv[0]). See
    // shell_launcher.c for more details.
    NSString *launchShellCommand = [self
        shellLauncherCommandWithCustomShell:[self
                                                customShellForProfile:profile]];
...
+ (BOOL)hushlogin {
  NSString *path =
      [NSHomeDirectory() stringByAppendingPathComponent:@".hushlogin"];
...
+ (NSString *)standardLoginCommand {
  ...
  //
  // Avoid using standard escaping which is wrong for a quoted string. I don't
  // know why this is in quotes, but I'm afraid to change it because it's been
  // that way for so long and the original commit message was lost.
  //
  // The returned value gets parsed into an argument array using
  // -componentsInShellCommand by
  // computeArgvForCommand:substitutions:completion:.
  userName = [userName stringByReplacingOccurrencesOfString:@"\\"
                                                 withString:@"\\\\"];
  userName = [userName stringByReplacingOccurrencesOfString:@"\""
                                                 withString:@"\\\""];
  return [NSString stringWithFormat:@"login -f%@p \"%@\"",
                                    [self hushlogin] ? @"q" : @"", userName];
}
...
+ (void)computeCommandForProfile:(Profile *)profile
  ...
  BOOL custom = [profile[KEY_CUSTOM_COMMAND]
      isEqualToString:kProfilePreferenceCommandTypeCustomValue];
  NSString *swifty = [self bookmarkCommandSwiftyString:profile
  ...
  [evaluator
      evaluateWithTimeout:5
               completion:^(iTermExpressionEvaluator *_Nonnull evaluator) {
                 ...
                 if (!string.length) {
                   string = [ITAddressBookMgr
                       loginShellCommandForBookmark:profile
                                      forObjectType:objectType];
                 }
...
+ (NSString *)bookmarkCommandSwiftyString:(Profile *)bookmark
                            forObjectType:(iTermObjectType)objectType {
  BOOL custom = [bookmark[KEY_CUSTOM_COMMAND]
      isEqualToString:kProfilePreferenceCommandTypeCustomValue];
  if (custom) {
    NSString *command = bookmark[KEY_COMMAND_LINE];
    if ([[command stringByTrimmingCharactersInSet:
                      [NSCharacterSet whitespaceAndNewlineCharacterSet]]
            length] > 0) {
      return command;
    }
...
+ (BOOL)canRemoveProfile:(NSDictionary *)profile
               fromModel:(ProfileModel *)model {
...
+ (BOOL)removeProfile:(NSDictionary *)profile fromModel:(ProfileModel *)model {
  ...
  [self postNotificationName:kProfileWasDeletedNotification
                      object:nil
                    userInfo:nil];
  [model flush];
  return YES;
...
+ (void)removeKeyMappingsReferringToGuid:(NSString *)badRef {
  for (NSString *guid in [[ProfileModel sharedInstance] guids]) {
    ...
    profile = [iTermKeyMappings removeKeyMappingsReferencingGuid:badRef
                                                     fromProfile:profile];
    if (profile) {
  ...
  [self postNotificationName:kKeyBindingsChangedNotification
                      object:nil
                    userInfo:nil];
}
  ...
  if (sDelayedNotifications) {
    for (NSNotification *existing in sDelayedNotifications) {
      if ([existing.name isEqualToString:notification.name] &&
          (existing.object == notification.object ||
           [existing.object isEqual:notification.object]) &&
          (existing.userInfo == notification.userInfo ||
           [existing.userInfo isEqual:notification.userInfo])) {
...
+ (BOOL)shortcutIdentifier:(NSString *)identifier
                     title:(NSString *)title
               matchesItem:(NSMenuItem *)item {
