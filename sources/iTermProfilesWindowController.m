...
static NSString *const kCloseBookmarksWindowAfterOpeningKey =
    @"CloseBookmarksWindowAfterOpening";
...
// In issue 6770 some people saw the key equivalent stop working. My guess is
// that view-based table views are responsible. This function cuts the gordian
// knot.
- (BOOL)performKeyEquivalent:(NSEvent *)event {
  ...
  DLog(@"iTermProfileWindowContentView: Perform key equivalent returns %@",
       @(result));
  return result;
}
...
+ (void)restoreWindowWithIdentifier:(NSString *)identifier
  ...
  iTermProfilesWindowController *windowController =
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
  ...
  BOOL result = [super performKeyEquivalent:event];
  DLog(@"iTermOpenProfileInTabButton: performKeyEquivalent result is %@",
       @(result));
  return result;
}
...
- (instancetype)initWithWindowNibName:(NSString *)windowNibName {
  ...
  if (self) {
    ...
    [closeAfterOpeningBookmark_ setState:[n boolValue]
...
- (void)windowDidLoad {
  ((iTermProfileWindowContentView *)self.window.contentView).windowController =
      self;
}
...
- (void)_openBookmarkInTab:(BOOL)inTab
             firstInWindow:(BOOL)firstInWindow
                    inPane:(PaneMode)inPane {
...
- (IBAction)openBookmarkInVerticalPane:(id)sender {
  ...
  [self _openBookmarkInTab:YES
             firstInWindow:!windowExists
                    inPane:VERTICAL_PANE];
...
- (void)profileTableSelectionDidChange:(id)profileTable {
  ...
  if ([guids count]) {
    ...
    // tabButton is enabled even if windowExists==false because its shortcut is
    // enter and we don't want to break that.
...
- (IBAction)closeAfterOpeningChanged:(id)sender {
  NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
  [prefs
      setObject:[NSNumber numberWithBool:[closeAfterOpeningBookmark_ state] ==
                                         NSControlStateValueOn]
