...
- (instancetype)initWithBlock:(void (^)(NSString *,
...
@implementation TmuxGateway {
  // Set to YES when the remote host closed the connection. We won't send
  ...
  BOOL
      acceptNotifications_; // Initially NO. When YES, respond to notifications.
  NSMutableString *strayMessages_;

  // When we get the first %begin-%{end,error} we notify the delegate. Until
  // that happens, this is set to NO.
  BOOL _initialized;
  NSMutableDictionary<NSString *, iTermTmuxSubscriptionHandle *>
      *_subscriptions;
}
...
- (void)abortWithErrorMessage:(NSString *)message title:(NSString *)title {
  ...
  [delegate_ tmuxHostDisconnected:[[_dcsID copy]
                                      autorelease]]; // Force the client to quit
}
    ...
    if (c == '\\') {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        if (bytes[i] < '0' || bytes[i] > '7') {
          c = '?';
          i--; // Back up in case bytes[i] is a null; we don't want to go off
               // the end.
          break;
        }
        c *= 8;
...
- (void)parseExtendedOutputCommandData:(NSData *)input {
  ...
  // This one is tricky to parse because the string version of the command could
  // have bogus UTF-8. 3.1 and earlier:
  ...
  TmuxLog(@"Run tmux command: \"%%extended-output \"%%%d\" %@ %.*s", windowPane,
          ms, (int)[decodedData length], [decodedData bytes]);
...
error:
  [self
      abortWithErrorMessage:[NSString
                                stringWithFormat:@"Malformed command (expected "
                                                 @"%%num data): \"%s\"",
                                                 command]];
}
...
- (void)parseOutputCommandData:(NSData *)input {
  ...
  TmuxLog(@"Run tmux command: \"%%output \"%%%d\" %.*s", windowPane,
          (int)[decodedData length], [decodedData bytes]);
...
- (void)parseLayoutChangeCommand:(NSString *)command {
  ...
  NSArray *components = [command
      captureComponentsMatchedByRegex:@"^%layout-change @([0-9]+) (.*)"];
  if (components.count != 3) {
    [self
        abortWithErrorMessage:
            [NSString
                stringWithFormat:@"Malformed command (expected %%layout-change "
                                 @"<window> <layout>): \"%@\"",
                                 command]];
    return;
  }
...
- (void)parseWindowAddCommand:(NSString *)command {
  NSArray *components =
      [command captureComponentsMatchedByRegex:@"^%window-add @([0-9]+)$"];
  if (components.count != 2) {
    [self abortWithErrorMessage:
              [NSString
                  stringWithFormat:
                      @"Malformed command (expected %%window-add id): \"%@\"",
                      command]];
    return;
  }
...
- (void)parseWindowCloseCommand:(NSString *)command {
  NSArray *components =
      [command captureComponentsMatchedByRegex:
                   @"^%(?:unlinked-)?window-close @([0-9]+)$"];
  if (components.count != 2) {
    [self abortWithErrorMessage:
              [NSString
                  stringWithFormat:
                      @"Malformed command (expected %%window-close id): \"%@\"",
                      command]];
    return;
  }
...
- (void)parseWindowRenamedCommand:(NSString *)command {
  ...
  if (components.count != 3) {
    [self
        abortWithErrorMessage:
            [NSString stringWithFormat:@"Malformed command (expected "
                                       @"%%window-renamed id new_name): \"%@\"",
                                       command]];
    return;
  }
...
- (void)parseSessionRenamedCommand:(NSString *)command {
  NSArray *components = [command
      captureComponentsMatchedByRegex:@"^%session-renamed \\$([0-9]+) (.+)$"];
  if (components.count != 3) {
    [self abortWithErrorMessage:
              [NSString stringWithFormat:@"Malformed command (expected "
  ...
  [delegate_ tmuxSession:[[components objectAtIndex:1] intValue]
                 renamed:[components objectAtIndex:2]];
}

- (void)parseSessionChangeCommand:(NSString *)command {
  NSArray *components = [command
      captureComponentsMatchedByRegex:@"^%session-changed \\$([0-9]+) (.+)$"];
  if (components.count != 3) {
    [self abortWithErrorMessage:
              [NSString stringWithFormat:@"Malformed command (expected "
  ...
  [delegate_ tmuxSessionChanged:[components objectAtIndex:2]
                      sessionId:[[components objectAtIndex:1] intValue]];
}

- (void)parseSessionsChangedCommand:(NSString *)command {
  NSArray *components =
      [command captureComponentsMatchedByRegex:@"^%sessions-changed$"];
  if (components.count != 1) {
    [self
        abortWithErrorMessage:
            [NSString
                stringWithFormat:
                    @"Malformed command (expected %%sessions-changed): \"%@\"",
                    command]];
    return;
  }
    ...
    [self
        abortWithErrorMessage:
            [NSString stringWithFormat:
                          @"Malformed command (expected %%window-pane-changed "
                          @"@window-id %%pane-id): \"%@\"",
                          command]];
    return;
  }
  [delegate_ tmuxActiveWindowPaneDidChangeInWindow:[components[1] intValue]
                                      toWindowPane:[components[2] intValue]];
}

- (void)parsePauseCommand:(NSString *)command {
  NSArray<NSString *> *components =
      [command captureComponentsMatchedByRegex:@"^%pause %([0-9]+)$"];
  if (components.count != 2) {
    [self
        abortWithErrorMessage:
            [NSString stringWithFormat:
                          @"Malformed command (expected %%pause %%wp): \"%@\"",
                          command]];
    return;
  }
  [delegate_ tmuxWindowPaneDidPause:components[1].intValue notification:YES];
}

// %subscription-changed name $a @b x %c : value
// Where a = session, b = window, x = index, c = pane and any can be - if they
// are not appropriate to that subscription (so a session subscription will not
// include b,x,c and a window not include c.
- (void)parseSubscriptionChangedCommand:(NSString *)command {
  ...
  if (components.count != 3) {
    [self
        abortWithErrorMessage:
            [NSString stringWithFormat:
                          @"Malformed command (expected %%subscription-changed "
                          @"sid [...] : value): \"%@\"",
                          command]];
    return;
  }
...
- (SEL)currentCommandSelector {
  NSString *theString =
      [self objectConvertingNullInDictionary:currentCommand_
...
- (void)abortWithErrorForCurrentCommand {
  if ([self commandIsTmux21Quirk]) {
    [self abortWithErrorMessage:
              [NSString stringWithFormat:
                            @"Error: %@.\n\nTmux 2.1 and earlier will refuse "
                            @"to create a new window pane with a nonexistent "
                            @"initial working directory.\n\nInfo:\n%@",
                            currentCommandResponse_, currentCommand_]];
  } else {
    [self abortWithErrorMessage:[NSString
...
- (void)invokeCurrentCallbackWithError:(BOOL)withError {
  ...
  if (withError) {
    [target performSelector:selector withObject:nil withObject:obj];
    return;
  }
  if (_tmuxLogging) {
    [delegate_
        tmuxPrintLine:[NSString
                          stringWithFormat:@"[Normal response to “%@”]",
                                           currentCommand_[kCommandString]]];
  }
...
- (BOOL)shouldAutofailSubsequentCommands {
  ...
  // Remove subsequent commands belonging to the same list so we can go back to
  // life as usual.
  ...
  while (YES) {
    if (withError) {
      if (_tmuxLogging) {
        [delegate_
            tmuxPrintLine:
                [NSString stringWithFormat:@"[Error “%@” in response to “%@”]",
                                           currentCommandResponse_,
                                           currentCommand_[kCommandString]]];
      }
      const BOOL shouldTolerateError = (
          [self currentCommandFlags] & kTmuxGatewayCommandShouldTolerateErrors);
      if (!shouldTolerateError &&
          (_initialized || currentCommand_[kCommandString])) {
...
- (void)parseBegin:(NSString *)command {
  ...
  NSArray *components = [command
      captureComponentsMatchedByRegex:@"^%begin ([0-9]+) [0-9]+( [0-9]+)?$"];
  if (components.count < 3) {
    [self abortWithErrorMessage:
              [NSString stringWithFormat:@"Malformed command (expected %%begin "
                                         @"command_id [flags]): \"%@\"",
                                         command]];
    return;
  }

  NSString *flagStr = [[components objectAtIndex:2]
  ...
  if (!(flags & 1)) {
    ...
    currentCommand_ = [[NSMutableDictionary
        dictionaryWithObjectsAndKeys:[components objectAtIndex:1], kCommandId,
                                     nil] retain];
    [currentCommandResponse_ release];
    [currentCommandData_ release];
    ...
    if (_tmuxLogging) {
      TmuxLog(@"Begin response to %@",
              [currentCommand_ objectForKey:kCommandString]);
      [delegate_
          tmuxPrintLine:[NSString
                            stringWithFormat:@"[Begin response for %@]",
                                             currentCommand_[kCommandString]]];
    }
  }
}
...
- (void)stripLastNewline {
  if ([currentCommandResponse_ hasSuffix:@"\n"]) {
    ...
    [currentCommandResponse_ replaceCharactersInRange:theRange withString:@""];
...
- (void)executeToken:(VT100Token *)token {
  ...
  if (![command hasPrefix:@"%output "] && !currentCommand_) {
  ...
  // received.
  if (currentCommand_ &&
      ([command hasPrefix:@"%exit "] || [command isEqualToString:@"%exit"])) {
  ...
  NSString *endCommand = [NSString
      stringWithFormat:@"%%end %@", [currentCommand_ objectForKey:kCommandId]];
  NSString *errorCommand =
      [NSString stringWithFormat:@"%%error %@",
                                 [currentCommand_ objectForKey:kCommandId]];
  ...
  if (currentCommand_ && [command hasPrefix:endCommand]) {
    ...
    // New in tmux 2.5
    if (acceptNotifications_)
      [self parseWindowPaneChangedCommand:command];
    ...
    // New in tmux 3.2
    if (acceptNotifications_)
      [self parseSubscriptionChangedCommand:command];
  } else if ([command hasPrefix:@"%continue"]) {
    // New in tmux 3.2. Don't care.
  } else if ([command
                 hasPrefix:@"%window-pane-changed"] || // active pane changed
             [command hasPrefix:@"%session-window-changed"] || // active window
                                                               // changed
             [command hasPrefix:@"%client-session-changed"] || // client is now
                                                               // attached to a
                                                               // new session
    ...
    if ([strayMessages_ length] > 0) {
      ...
      for (NSString *line in
           [strayMessages_ componentsSeparatedByString:@"\n"]) {
        if ([line length] > 0) {
    ...
    if (![iTermAdvancedSettingsModel tolerateUnrecognizedTmuxCommands]) {
      [delegate_ tmuxPrintLine:@"Unrecognized command from tmux. Did your ssh "
                               @"session die? The command was:"];
      [delegate_ tmuxPrintLine:command];
      [self hostDisconnected];
      return;
    }
...
- (void)sendKeys:(NSString *)string toWindowPane:(int)windowPane {
  if ([self serverSupportsUTF8]) {
    ...
    // Send each byte of UTF-8 as a separate "keystroke". For tmux 2.1 and
    // earlier.
...
- (NSString *)firstSupplementaryPlaneCharacterInArray:
    (NSArray<NSNumber *> *)codePoints {
  NSUInteger index = [codePoints
      indexOfObjectPassingTest:^BOOL(NSNumber *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
...
- (void)sendCodePoints:(NSArray<NSNumber *> *)codePoints
  ...
  if (![self serverAcceptsSurrogatePairs]) {
    NSString *string =
        [self firstSupplementaryPlaneCharacterInArray:codePoints];
    if (string) {
      [delegate_ tmuxCannotSendCharactersInSupplementaryPlanes:string
                                                    windowPane:windowPane];
      return;
    }
  }

  // Send multiple small send-keys commands because commands longer than 1024
  // bytes crash tmux 1.8.
  ...
  for (NSUInteger start = 0; start < codePoints.count; start += stride) {
    ...
    [commands
        addObject:[self dictionaryForSendKeysCommandWithCodePoints:subarray
                                                        windowPane:windowPane]];
  }
...
- (BOOL)doubleValue:(double)value1
    isGreaterOrEqualTo:(double)value2
...
- (BOOL)serverSupportsUTF8 {
  return (self.minimumServerVersion != nil &&
          [self.minimumServerVersion
              compare:[NSDecimalNumber decimalNumberWithString:@"2.2"]] !=
              NSOrderedAscending);
}

- (BOOL)serverAcceptsSurrogatePairs {
  NSDecimalNumber *version2_2 =
      [NSDecimalNumber decimalNumberWithString:@"2.2"];
  return !([self.minimumServerVersion isEqual:version2_2] &&
           [self.maximumServerVersion isEqual:version2_2]);
}

- (NSDictionary *)dictionaryForSendKeysCommandWithCodePoints:
                      (NSArray<NSNumber *> *)codePoints
                                                  windowPane:(int)windowPane {
  NSString *command =
      [NSString stringWithFormat:@"send-keys -t \"%%%d\" %@", windowPane,
                                 [codePoints numbersAsHexStrings]];
  NSDictionary *dict =
      [self dictionaryForCommand:command
                  responseTarget:self
...
- (NSDictionary *)dictionaryForCommand:(NSString *)command
                      ...
                      responseSelector:(SEL)selector
                        responseObject:(id)obj
                                 flags:(int)flags {
  return [NSDictionary
      dictionaryWithObjectsAndKeys:command, kCommandString,
                                   target ? target : [NSNull null],
                                   kCommandTarget,
                                   selector ? (id)NSStringFromSelector(selector)
                                            : (id)[NSNull null],
                                   kCommandSelector, obj ? obj : [NSNull null],
                                   kCommandObject,
...
- (void)enqueueCommandDict:(NSDictionary *)dict {
  if ([dict[kCommandFlags] intValue] &
      kTmuxGatewayCommandOfferToDetachIfLaggyDuplicate) {
    if ([self havePendingCommandEqualTo:dict[kCommandString]] &&
        [self isTmuxUnresponsive]) {
      [delegate_ tmuxGatewayDidTimeOut];
      if (disconnected_) {
        return;
      }
...
- (void)sendCommand:(NSString *)command
  ...
  [self sendCommand:command
      ...
      responseSelector:selector
        responseObject:nil
                 flags:(target == nil) ? kTmuxGatewayCommandShouldTolerateErrors
                                       : 0];
}
...
- (iTermTmuxSubscriptionHandle *)
    subscribeToFormat:(NSString *)format
               target:(NSString *)target
                block:(void (^)(NSString *, NSArray<NSString *> *))block {
  iTermTmuxSubscriptionHandle *handle =
      [[[iTermTmuxSubscriptionHandle alloc] initWithBlock:block] autorelease];
  NSString *subscribe =
      [NSString stringWithFormat:@"refresh-client -B '%@:%@:%@'",
...
- (void)unsubscribe:(iTermTmuxSubscriptionHandle *)handle {
  ...
  // succeed.
  NSString *subscribe =
      [NSString stringWithFormat:@"refresh-client -B '%@'", handle.identifier];
