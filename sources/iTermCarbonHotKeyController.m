...
+ (UInt32)carbonModifiersForCocoaModifiers:(NSEventModifierFlags)cocoa {
  ...
  [map enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
...
+ (NSEventModifierFlags)cocoaModifiersForCarbonModifiers:
...
- (BOOL)hotKeyIDEquals:(EventHotKeyID)hotKeyID {
  return hotKeyID.signature == _hotKeyID.signature &&
         hotKeyID.id == _hotKeyID.id;
}
...
- (instancetype)init {
  ...
  if (self) {
    ...
    if (InstallEventHandler(
            GetEventDispatcherTarget(), // target
            EventHandler,               // handler function
            sizeof(events) /
                sizeof(*events), // number of events registering for
...
- (void)sessionDidBecomeActive:(NSNotification *)notification {
  ...
  NSDictionary<iTermTuple<NSNumber *, NSNumber *> *, NSArray<iTermHotKey *> *>
      *hotkeysByShortcut =
          [_suspendedHotKeys classifyWithBlock:^id(iTermHotKey *hotkey) {
            return [iTermTuple tupleWithObject:@(hotkey.shortcut.keyCode)
                                     andObject:@(hotkey.shortcut.modifiers)];
          }];
  for (iTermTuple<NSNumber *, NSNumber *> *key in hotkeysByShortcut) {
    ...
    const UInt32 carbonModifiers = [NSEvent
        carbonModifiersForCocoaModifiers:representative.shortcut.modifiers];
    OSStatus status = RegisterEventHotKey(
        (UInt32)representative.shortcut.keyCode, carbonModifiers,
        representative.hotKeyID, GetEventDispatcherTarget(), 0, &eventHotKey);
    if (status != noErr) {
      ELog(@"Failed to register event hotkey when session resumed active: %@",
           @(status));
    } else {
      eventHotKey = NULL;
    }
...
- (void)sessionDidResignActive:(NSNotification *)notification {
  ...
  NSDictionary<iTermTuple<NSNumber *, NSNumber *> *, NSArray<iTermHotKey *> *> *
      hotkeysByShortcut = [_hotKeys classifyWithBlock:^id(iTermHotKey *hotkey) {
        return [iTermTuple tupleWithObject:@(hotkey.shortcut.keyCode)
                                 andObject:@(hotkey.shortcut.modifiers)];
      }];
  for (iTermTuple<NSNumber *, NSNumber *> *key in hotkeysByShortcut) {
...
- (iTermHotKey *)registerShortcut:(iTermShortcut *)shortcut
  ...
  const UInt32 carbonModifiers =
      [NSEvent carbonModifiersForCocoaModifiers:shortcut.modifiers];
  iTermHotKey *existingHotKey =
      [[self hotKeysWithKeyCode:shortcut.keyCode
                      modifiers:shortcut.modifiers] firstObject];
  DLog(@"Registering shortcut %@. Existing hotkeys: %@", shortcut,
       existingHotKey);
  EventHotKeyID hotKeyID;
  if (!existingHotKey) {
    hotKeyID = [self nextHotKeyID];
    if (!_suspended) {
      if (RegisterEventHotKey((UInt32)shortcut.keyCode, carbonModifiers,
                              hotKeyID, GetEventDispatcherTarget(), 0,
                              &eventHotKey)) {
        return nil;
      }
    }
  ...
  iTermHotKey *hotkey =
      [[[iTermHotKey alloc] initWithEventHotKey:eventHotKey
                                             id:hotKeyID
                                       shortcut:shortcut
                                       userData:userData
                                         target:target
...
- (EventHotKeyID)nextHotKeyID {
  ...
  EventHotKeyID keyID = {.signature = iTermCarbonHotKeyControllerSignature,
                         .id = ++_nextHotKeyID};
...
- (NSArray<iTermHotKey *> *)hotKeysWithKeyCode:(NSUInteger)keyCode
  ...
  for (iTermHotKey *hotkey in _suspended ? _suspendedHotKeys : _hotKeys) {
    if (hotkey.shortcut.keyCode == keyCode &&
        hotkey.shortcut.modifiers == modifiers) {
...
static OSStatus EventHandler(EventHandlerCallRef inHandler, EventRef inEvent,
                             void *inUserData) {
...
- (BOOL)handleEvent:(EventRef)event {
  ...
  if ([NSApp isActive] &&
    ...
    iTermShortcutInputView *shortcutInputView =
  ...
  for (iTermHotKey *hotkey in hotkeys) {
    ...
    // The target returns an array of siblings that were handled at the same
    // time.
    NSArray *handledSiblings =
