...
@interface iTermGraphDatabaseState
    : iTermSynchronizedState <iTermGraphDatabaseState *>
@property(nonatomic, strong) id<iTermDatabase> db;
// Load complete means that the initial attempt to load the DB has finished. It
// may have failed, leaving us without any state, but it is now safe to proceed
// to use the graph DB as it won't change out from under you after this point.
@property(nonatomic) BOOL loadComplete;
- (instancetype)initWithQueue:(dispatch_queue_t)queue
                     database:(id<iTermDatabase>)db;
...
- (void)setLoadComplete:(BOOL)loadComplete {
  ...
  if (loadComplete && !_loadComplete) {
    ...
    [blocks enumerateObjectsUsingBlock:^(void (^_Nonnull block)(void),
...
- (instancetype)initWithDatabase:(id<iTermDatabase>)db {
  ...
  if (self) {
    ...
    _thread = [[iTermThread alloc]
        initWithLabel:@"com.iterm2.graph-db"
         stateFactory:^iTermSynchronizedState *_Nonnull(
...
- (BOOL)
  ...
  DLog(@"beginUpdate");
  [self beginUpdate];
  iTermGraphDeltaEncoder *encoder =
      [[iTermGraphDeltaEncoder alloc] initWithPreviousRevision:self.record];
  block(encoder);
  __weak __typeof(self) weakSelf = self;

  void (^perform)(iTermGraphDatabaseState *) =
      ^(iTermGraphDatabaseState *state) {
...
- (void)trySaveEncoder:(iTermGraphDeltaEncoder *)originalEncoder
  ...
  @try {
    ...
    DLog(@"save failed: %@ with recovery count %@", state.db.lastError,
         @(_recoveryCount));
    if (_recoveryCount >= 3) {
    ...
    _recoveryCount += 1;
    // Create a fresh encoder that's not in a partially broken state. Replace
    // the encoder pointer so we can get its record below, as a successful
    // recovery mutates the record by setting rowids in it.
    encoder =
        [[iTermGraphDeltaEncoder alloc] initWithRecord:originalEncoder.record];
    ...
    if (ok) {
      // If we were able to save, then use this record as the new baseline. Note
      // that we very carefully take it from encoder, not originalEncoder,
      // because regardless of whether a recovery was attempted `encoder.record`
      // has the correct rowids.
...
- (BOOL)reallySave:(iTermGraphDeltaEncoder *)encoder
  ...
  const BOOL ok = [encoder enumerateRecords:^(
                               iTermEncoderGraphRecord *_Nullable before,
                               iTermEncoderGraphRecord *_Nullable after,
                               NSNumber *parent, NSString *path, BOOL *stop) {
    if (before && !before.rowid) {
      @throw [NSException
          exceptionWithName:@"MissingRowID"
                     reason:[NSString
                                stringWithFormat:@"Before lacking a rowid: %@",
                                                 before]
                   userInfo:nil];
    }
    if (before && !after) {
      if (![state.db
              executeUpdate:@"delete from Node where rowid=?", before.rowid]) {
        *stop = YES;
        return;
      }
      return;
    ...
    if (!before && after) {
      if (![state.db executeUpdate:@"insert into Node (key, identifier, "
                                   @"parent, data) values (?, ?, ?, ?)",
                                   after.key, after.identifier, parent,
                                   after.data ?: [NSData data]]) {
        *stop = YES;
        return;
      }
      ...
      @try {
        ...
        @throw [exception
            it_rethrowWithMessage:@"after.key=%@ after.identifier=%@",
    ...
    if (before && after) {
      ...
      if (after.rowid == nil) {
        ...
        if (before.generation == after.generation &&
          ...
          DLog(@"Don't update rowid %@ %@[%@] because it is unchanged",
               before.rowid, after.key, after.identifier);
          return;
        }
      }
      ...
      if (![state.db executeUpdate:@"update Node set data=? where rowid=?",
                                   after.data, before.rowid]) {
        *stop = YES;
      }
      return;
    }
...
- (BOOL)createTables:(iTermGraphDatabaseState *)state {
  ...
  if (![state.db
          executeUpdate:
              @"create table if not exists Node (key text not null, identifier "
              @"text not null, parent integer not null, data blob)"]) {
    return NO;
  }
  [state.db executeUpdate:
                @"create index if not exists parent_index on Node (parent)"];
...
- (iTermEncoderGraphRecord *_Nullable)load:(iTermGraphDatabaseState *)state
  ...
  NSMutableArray<NSArray *> *nodes = [NSMutableArray array];
  {
    ...
    FMResultSet *rs = [state.db
        executeQuery:@"select key, identifier, parent, rowid, data from Node"];
    while ([rs next]) {
      DLog(@"Read row");
      [nodes addObject:@[
        [rs stringForColumn:@"key"], [rs stringForColumn:@"identifier"],
