...
@interface iTermFunctionCallTextFieldDelegate () <NSControlTextEditingDelegate>
...
- (instancetype)initWithPathSource:
                    (NSSet<NSString *> * (^)(NSString *))pathSource
  ...
  if (self) {
    ...
    NSDictionary<NSString *, NSArray<NSString *> *> *bifSignatures =
        [[iTermBuiltInFunctions sharedInstance]
            registeredFunctionSignatureDictionary];
    NSDictionary<NSString *, NSArray<NSString *> *> *combinedSignatures =
        [registeredSignatures dictionaryByMergingDictionary:bifSignatures];
    Class suggesterClass = functionsOnly ? [iTermFunctionCallSuggester class]
                                         : [iTermSwiftyStringSuggester class];
    _suggester =
        [[suggesterClass alloc] initWithFunctionSignatures:combinedSignatures
...
- (NSArray *)control:(NSControl *)control
  ...
  *index = -1; // Don't select anything. Doing so causes pathological behavior
               // when you press period.
  return suggestions;
}
...
- (NSArray<NSString *> *)suggestionsForRange:(NSRange)charRange {
  ...
  NSArray<NSString *> *suggestions = [_suggester
      suggestionsForString:[self.lastEntry
                               substringToIndex:NSMaxRange(charRange)]];
...
- (void)focusReportingTextFieldWillBecomeFirstResponder:
  ...
  NSTextView *fieldEditor =
      [NSTextView castFrom:[[sender window] fieldEditor:YES forObject:sender]];
  if (self.isAutocompleting == NO && !self.backspaceKey) {
    self.isAutocompleting = YES;
    self.lastEntry = [[fieldEditor string] copy];
    [fieldEditor complete:nil];
