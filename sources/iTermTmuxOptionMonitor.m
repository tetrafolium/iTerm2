...
- (instancetype)initWithGateway:(TmuxGateway *)gateway
  ...
  if (self) {
    ...
    if ([_gateway versionAtLeastDecimalNumberWithString:@"3.2"]) {
      ...
      _subscriptionHandle = [_gateway
          subscribeToFormat:self.escapedFormat
                     target:target
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p format=%@ target=%@ sub=%@ scope=%@>",
                       NSStringFromClass(self.class), self, _format, _target,
                       _subscriptionHandle, _scope];
}
...
- (void)startTimer {
  if (_subscriptionHandle.isValid) {
    DLog(@"Not starting timer because there is a valid subscription for %@",
         self);
    return;
  }
  [_timer invalidate];
...
- (NSString *)escapedFormat {
  return [[_format stringByReplacingOccurrencesOfString:@"\\"
                                             withString:@"\\\\"]
...
- (NSString *)command {
  return [NSString stringWithFormat:@"display-message -t '%@' -p '%@'", _target,
                                    self.escapedFormat];
}
...
- (void)updateOnce {
  ...
  if (_fallbackVariableName &&
      self.gateway.minimumServerVersion.doubleValue <= 2.9) {
