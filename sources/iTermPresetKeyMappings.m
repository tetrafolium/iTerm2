...
+ (NSDictionary *)readPresetKeyMappingsFromPlist:(NSString *)thePlist {
  NSString *plistFile =
      [[NSBundle bundleForClass:[self class]] pathForResource:thePlist
                                                       ofType:@"plist"];
  ...
  for (NSString *name in builtins) {
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id _Nonnull serializedKeystroke,
      ...
      iTermKeystroke *keystroke =
          [[iTermKeystroke alloc] initWithSerialized:serializedKeystroke];
      if (!keystroke) {
        return;
      }
      iTermKeyBindingAction *action =
          [iTermKeyBindingAction withDictionary:mapping];
      if (!action) {
        return;
      }
      [result addObject:[iTermTuple tupleWithObject:keystroke
                                          andObject:action]];
    }];
  }
  return result;
...
+ (Profile *)profileByLoadingPresetNamed:(NSString *)presetName
  ...
  if (replaceAll) {
    return [sourceProfile dictionaryBySettingObject:preset
                                             forKey:KEY_KEYBOARD_MAP];
  }
  NSDictionary *sourceMap = sourceProfile[KEY_KEYBOARD_MAP] ?: @{};
  NSDictionary *updated = [sourceMap
      it_dictionaryByMergingSerializedKeystrokeKeyedDictionary:preset];
  return [sourceProfile dictionaryBySettingObject:updated
                                           forKey:KEY_KEYBOARD_MAP];
}
...
+ (void)setGlobalKeyMappingsToPreset:(NSString *)presetName
  ...
  if (!replaceAll) {
    ...
    globalKeyMap =
        [globalKeyMap it_dictionaryByMergingSerializedKeystrokeKeyedDictionary:
                          self.defaultGlobalKeyMap];
