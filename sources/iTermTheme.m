...
- (id<PSMTabStyle>)tabStyleWithDelegate:(id<PSMMinimalTabStyleDelegate>)delegate
  ...
  case TAB_STYLE_DARK_HIGH_CONTRAST:
    // Use the stated style. it_tabStyle assumes you want a style based on the
    // current appearance but this is the one case where that is not true. If
    // there is only one tab and it has a dark tab color the style will be
    // adjusted later in the call to updateTabColors.
    break;
  }
  switch (tabStyle) {
  case TAB_STYLE_AUTOMATIC:
...
- (NSColor *)
    backgroundColorForDecorativeSubviewsInSessionWithTabColor:
        (NSColor *)tabColor
                                          effectiveAppearance:
                                              (NSAppearance *)
                                                  effectiveAppearance
  ...
  if (self.useMinimalStyle) {
    ...
    undimmedColor = [self
        backgroundColorForDecorativeSubviewsForTabColor:undimmedColor
                                               tabStyle:[effectiveAppearance
                                                            it_tabStyle:
                                                                preferredStyle]
                                 sessionBackgroundColor:sessionBackgroundColor];
  }
...
- (nullable NSColor *)
    statusBarContainerBackgroundColorForTabColor:(NSColor *)tabColor
                             effectiveAppearance:
  ...
  if ([iTermPreferences boolForKey:kPreferenceKeySeparateStatusBarsPerPane]) {
    return [self
        backgroundColorForDecorativeSubviewsInSessionWithTabColor:tabColor
                                              effectiveAppearance:
                                                  effectiveAppearance
                                           sessionBackgroundColor:
                                               sessionBackgroundColor
    ...
    return [self tabBarBackgroundColorForTabColor:tabColor style:tabStyle];
  }
}

#pragma mark Status Bar Text Color

- (NSColor *)statusBarTextColorForEffectiveAppearance:
                 (NSAppearance *)effectiveAppearance
  ...
  if (self.useMinimalStyle) {
    NSColor *color = [self
        terminalWindowDecorationTextColorForBackgroundColor:nil
    ...
    return [colorMap
        colorByDimmingTextColor:[color
                                    colorUsingColorSpace:[NSColorSpace
                                                             sRGBColorSpace]]];
  } else if (@available(macOS 10.14, *)) {
    return [NSColor labelColor];
  } else if ([effectiveAppearance.name
...
- (NSColor *)
    terminalWindowDecorationTextColorForBackgroundColor:
...
- (NSColor *)
    backgroundColorForDecorativeSubviewsForTabColor:(NSColor *)tabColor
                                           tabStyle:(iTermPreferencesTabStyle)
                                                        tabStyle
                             sessionBackgroundColor:
                                 (NSColor *)sessionBackgroundColor {
  if (self.useMinimalStyle) {
...
- (NSColor *)
    dimmedBackgroundColorWithAppearance:(NSAppearance *)appearance
