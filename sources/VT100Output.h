...
typedef NS_ENUM(NSInteger, MouseFormat) {
  ...
  MOUSE_FORMAT_XTERM_EXT =
      1, // UTF-8 1005 mode (does not pass through luit unchanged)
  MOUSE_FORMAT_URXVT = 2, // rxvt's 1015 mode (outputs csi codes, that if echoed
                          // to the term, mess up the display)
...
// This class produces data to send for special keys (arrow keys, function keys,
// etc.) It has a small amount of state that is copied from VT100Terminal. This
// object is 1:1 with VT100Terminal.
