...
static NSString *const iTermStatusBarCompressionResistanceKey =
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    NSString *className =
        [aDecoder decodeObjectOfClass:[NSString class] forKey:@"class"] ?: @"";
...
@interface iTermStatusBarBaseComponent () <iTermWebViewDelegate,
                                           NSPopoverDelegate>
@end
...
+ (NSString *)statusBarComponentIdentifier {
  return [NSString
      stringWithFormat:@"com.iterm2.%@", NSStringFromClass(self.class)];
}
...
- (id<iTermStatusBarComponentFactory>)statusBarComponentFactory {
  return
      [[iTermStatusBarBuiltInComponentFactory alloc] initWithClass:self.class];
}
...
- (BOOL)isEqualToComponent:(id<iTermStatusBarComponent>)other {
  ...
  iTermStatusBarBaseComponent *otherBase =
      [iTermStatusBarBaseComponent castFrom:other];
...
- (NSArray<iTermStatusBarComponentKnob *> *)minMaxWidthKnobs {
  iTermStatusBarComponentKnob *maxWidthKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Maximum Width:"
  ...
  iTermStatusBarComponentKnob *minWidthKnob =
      [[iTermStatusBarComponentKnob alloc]
          initWithLabelText:@"Minimum Width:"
                       type:iTermStatusBarComponentKnobTypeDouble
                placeholder:@""
...
- (CGFloat)clampedWidth:(CGFloat)width {
  ...
  CGFloat max =
      [knobValues[iTermStatusBarMaximumWidthKey] ?: @(INFINITY) doubleValue];
  CGFloat min = [knobValues[iTermStatusBarMinimumWidthKey]
                     ?: @(self.defaultMinimumWidth) doubleValue];
  return MIN(max, MAX(min, width));
}

+ (NSDictionary *)defaultMinMaxWidthKnobValues {
  return @{
    iTermStatusBarMaximumWidthKey : @(INFINITY),
...
- (iTermStatusBarComponentKnob *)newPriorityKnob {
  return [[iTermStatusBarComponentKnob alloc]
      initWithLabelText:@"Priority:"
                   type:iTermStatusBarComponentKnobTypeDouble
            placeholder:@""
           defaultValue:self.class.statusBarComponentDefaultKnobs
  ...
  if ([self statusBarComponentCanStretch]) {
    ...
    compressionResistanceKnob = [[iTermStatusBarComponentKnob alloc]
        initWithLabelText:title
              ...
              placeholder:@""
             defaultValue:self.class.statusBarComponentDefaultKnobs
                              [iTermStatusBarCompressionResistanceKey]
...
- (void)statusBarComponentSetKnobValues:(NSDictionary *)knobValues {
  NSMutableSet<NSString *> *keys =
      [NSMutableSet setWithArray:[knobValues allKeys]];
  for (NSString *key in _configuration.allKeys) {
    [keys addObject:key];
  }
  NSDictionary *replacement = [_configuration
      dictionaryBySettingObject:knobValues
                         forKey:
                             iTermStatusBarComponentConfigurationKeyKnobValues];
  NSMutableSet<NSString *> *updatedKeys = [NSMutableSet set];
  NSDictionary *replacementKnobs =
      replacement[iTermStatusBarComponentConfigurationKeyKnobValues];
  NSDictionary *originalKnobs =
      _configuration[iTermStatusBarComponentConfigurationKeyKnobValues];
  for (NSString *key in keys) {
    // The color picker tends to slightly perturb values during colorspace
    // conversion so use a fuzzy comparison for floating point values.
    if (![NSObject object:originalKnobs[key]
            isApproximatelyEqualToObject:replacementKnobs[key]
                                 epsilon:0.0001]) {
...
- (void)statusBarComponentOpenPopoverWithHTML:(NSString *)html
                                       ofSize:(NSSize)size {
  WKWebView *webView =
      [[iTermWebViewFactory sharedInstance] webViewWithDelegate:self];
  if (!webView) {
    return;
  }
...
- (void)itermWebViewScriptInvocation:(NSString *)invocation
  ...
  [[iTermAPIHelper sharedInstance]
      logToConnectionHostingFunctionWithSignature:invocation
