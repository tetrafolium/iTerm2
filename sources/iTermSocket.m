...
- (void)setReuseAddr:(BOOL)reuse {
  ...
  int rc = setsockopt(_fd, SOL_SOCKET, SO_REUSEADDR, (const void *)&optionValue,
                      sizeof(optionValue));
  if (rc) {
...
- (BOOL)listenWithBacklog:(int)backlog
                   accept:(void (^)(int, iTermSocketAddress *,
                                    NSNumber *))acceptBlock {
  if (!_boundAddress) {
    return NO;
  }
  if (_acceptQueue) {
  ...
  dispatch_async(_acceptQueue, ^{
    while (1) {
      @autoreleasepool {
        ...
        //   "The argument s must be a UNIX-domain socket (unix(4)) of type
        //   SOCK_STREAM on
        //    which either connect(2) or listen(2) have been called"
        // Well, we called listen on `fd`, but `getpeereid` gives "Socket is not
        // connected" when you pass that to it. If you give it `acceptFd`, it
        // works fine. ¯\_(ツ)_/¯
        NSNumber *uidNumber = nil;
        if (getpeereid(acceptFd, &euid, &egid) == 0) {
          uidNumber = @(euid);
        }
        acceptBlock(acceptFd,
                    [iTermSocketAddress socketAddressWithSockaddr:sockaddr],
                    uidNumber);
      }
    }
  });
