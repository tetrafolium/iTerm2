...
/*
   ...
   warningActionWithLabel:@"Cancel" block:nil]; iTermWarningAction *doStuff =
        [iTermWarningAction warningActionWithLabel:@"Do Stuff"
                                             block:^(iTermWarningSelection
   selection) { DoStuff();
        }];
    iTermWarning *warning = [[[iTermWarning alloc] init] autorelease];
    warning.title = @"This is the main text for the warning.";      // TODO:
   CUSTOMIZE THIS warning.warningActions = @[ doStuff, cancel ]; // TODO:
   CUSTOMIZE THIS warning.identifier = @"NoSyncSuppressDoStuffWarning"; // TODO:
   CUSTOMIZE THIS warning.warningType = kiTermWarningTypePermanentlySilenceable;
   // TODO: CUSTOMIZE THIS [warning runModal];
*/

@interface iTermWarning : NSObject

// Used to unsilence a particular selection (e.g., when you have a bug and
// silence the Cancel selection).
...
// actionToSelectionMap gives the iTermWarningSelection that should be returned
// for each entry in actions. It must be in 1:1 correspondence with actions. It
// is useful because it allows you to add a new action in the middle of the
// actions array without invalidating a saved selection. If nil then the first
// selection is Selection0, second is Selection1, etc. For example, if you
// originally had actions = [ "Hide", "Kill" ] and a user saved "Kill" as their
// default, then NSUserDefaults would store a value of kiTermWarningSelection1.
// If you then change actions to [ "Hide", "Cancel", "Kill" ], you want Kill to
// still be iTermWarningSelection1, even though Cancel is in the second
// position, so the saved preference will be respected. In that case, you'd use
// an actionToSelectionMap of [ kiTermWarningSelection0,
// kiTermWarningSelection2, kItermWarningSelection1 ], which has the effect of
// making Cancel return Selection2 even though it's in the second position.
+ (iTermWarningSelection)showWarningWithTitle:(NSString *)title
                                      actions:(NSArray *)actions
                                actionMapping:
...
// cancelLabel is the action name to treat like "Cancel". It won't be
// remembered.
+ (iTermWarningSelection)showWarningWithTitle:(NSString *)title
                                      actions:(NSArray *)actions
                                actionMapping:
...
// If you prefer you can set the properties you care about and then invoke
// runModal.
...
// Optional. Should be 1:1 with actions. Provides a mapping from the index of
// the button actually pressed to the index runModal reports.
...
// If set then a "help" button is added to the alert box and this block is
// invoked when it is clicked.
