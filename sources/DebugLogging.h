...
#define STOPWATCH_LAP(name)                                                    \
  do {                                                                         \
    ...
    if (count_##name % 10000 == 0) {                                           \
      NSLog(@"%s: %fms (%d)", #name, 1000.0 * sum_##name / count_##name,       \
            count_##name);                                                     \
    }                                                                          \
  } while (0)
#else
...
// I use a variadic macro here because of an apparent compiler bug in XCode 4.2
// that thinks a variadic objc call as an argument is not a single value.
...
// Write to crash log immediately and unconditionally. Use this when you expect
// to crash right away.
#define CrashLog(args...)                                                      \
  do {                                                                         \
    LogForNextCrash(__FILE__, __LINE__, __FUNCTION__,                          \
                    [NSString stringWithFormat:args], YES);                    \
  } while (0)
...
#define ILog(args...)                                                          \
  do {                                                                         \
    DLog(args);                                                                \
    LogForNextCrash(__FILE__, __LINE__, __FUNCTION__,                          \
                    [NSString stringWithFormat:args], NO);                     \
  } while (0)
...
// Private error log. May contain private info. Logged to syslog, but not crash
// reports.
#define XLog(args...)                                                          \
  do {                                                                         \
    DLog(args);                                                                \
  ...
  do {                                                                         \
    if (!(condition)) {                                                        \
      ...
      if (TurnOffDebugLoggingSilently()) {                                     \
        NSRunAlertPanel(                                                       \
            @"Critical Error",                                                 \
            @"A critical error occurred and a debug log was created. Please "  \
            @"send /tmp/debuglog.txt to the developers.",                      \
            @"OK", nil, nil);                                                  \
      }                                                                        \
      ...
      if (TurnOffDebugLoggingSilently()) {                                     \
        dispatch_async(dispatch_get_main_queue(), ^{                           \
          ...
          alert.informativeText =                                              \
              @"A critical error occurred and a debug log was created. "       \
  ...
  do {                                                                         \
    if (!(condition)) {                                                        \
      ...
      __assert_rtn(                                                            \
          ...
          [[NSString stringWithFormat:@ #condition ": " args] UTF8String]);    \
    }                                                                          \
  } while (0)
