...
@interface iTermStatusBarViewController () <iTermStatusBarComponentDelegate,
...
@interface iTermStatusBarViewController () <
    iTermStatusBarAutoRainbowControllerDelegate>
@end
...
- (instancetype)initWithLayout:(iTermStatusBarLayout *)layout
  ...
  if (self) {
    _scope = scope;
    _layout = layout;
    _autoRainbowController = [[iTermStatusBarAutoRainbowController alloc]
        initWithStyle:layout.advancedConfiguration.autoRainbowStyle];
...
- (void)loadView {
  iTermStatusBarView *view =
      [[iTermStatusBarView alloc] initWithFrame:NSZeroRect];
...
- (nullable iTermStatusBarContainerView *)mandatoryView {
  ...
  return [_containerViews
      objectPassingTest:^BOOL(iTermStatusBarContainerView *containerView,
...
- (iTermStatusBarLayoutAlgorithm *)layoutAlgorithm {
  return [iTermStatusBarLayoutAlgorithm
      layoutAlgorithmWithContainerViews:_containerViews
                          mandatoryView:self.mandatoryView
                         statusBarWidth:self.view.frame.size.width
                                setting:_layout.advancedConfiguration
                                            .layoutAlgorithm
                  removeEmptyComponents:_layout.advancedConfiguration
...
- (NSArray<NSNumber *> *)desiredSeparatorOffsets {
  ...
  for (iTermStatusBarContainerView *anObject in
       [_visibleContainerViews it_arrayByDroppingLastN:1]) {
    const BOOL isSpring = [anObject.component
        isKindOfClass:[iTermStatusBarSpringComponent class]];
    const CGFloat margin =
        isSpring ? 0 : iTermStatusBarViewControllerMargin / 2.0;
    NSNumber *offset =
        @(anObject.desiredOrigin + anObject.desiredWidth + margin);
    if ([anObject.component
            isKindOfClass:[iTermStatusBarFixedSpacerComponent class]]) {
      // Consecutive fixed spacers do not get separators. Rather than adding a
      // separator just record that we saw a fixed spacer and add the separator
      // if we see a subsequent non-spacer.
    ...
    if (lastFixedSpacerOffset && haveFoundNonSpacer) {
      // This is a non-spacer following a spacer, but not the very first
      // non-spacer. That second clause is to prevent adding a separator between
      // the spacer and nonspacer in this example:
      //     |[spacer][nonspacer]:[whatever]|
      // While we do want separators here here (referring to the second
      // separator, obvs):
...
- (NSArray<iTermTuple<NSColor *, NSNumber *> *> *)desiredBackgroundColors {
  return [_visibleContainerViews
      mapWithBlock:^id(iTermStatusBarContainerView *containerView) {
        NSColor *color = containerView.backgroundColor;
        const BOOL isSpring = [containerView.component
            isKindOfClass:[iTermStatusBarSpringComponent class]];
        const CGFloat margin =
            isSpring ? 0 : iTermStatusBarViewControllerMargin / 2.0;
        NSNumber *offset = @(containerView.desiredOrigin +
                             containerView.desiredWidth + margin + 0.5);
        return [iTermTuple tupleWithObject:color andObject:offset];
      }];
}

- (void)viewWillLayout {
  NSArray<iTermStatusBarContainerView *> *previouslyVisible =
      _visibleContainerViews.copy;
...
- (void)setTemporaryLeftComponent:
    (nullable id<iTermStatusBarComponent>)temporaryLeftComponent {
  _temporaryLeftComponent = temporaryLeftComponent;
  [self updateViews];
  [self.view layoutSubtreeIfNeeded];
}

- (void)setTemporaryRightComponent:
    (nullable id<iTermStatusBarComponent>)temporaryRightComponent {
...
- (NSViewController<iTermFindViewController> *)searchViewController {
  return
      [_containerViews mapWithBlock:^id(
...
- (iTermStatusBarContainerView *)containerViewForComponent:
...
- (void)updateViews {
  NSMutableArray<iTermStatusBarContainerView *> *updatedContainerViews =
      [NSMutableArray array];
  NSMutableArray<id<iTermStatusBarComponent>> *components =
      [_layout.components mutableCopy];
  if (_temporaryLeftComponent) {
    [components insertObject:_temporaryLeftComponent atIndex:0];
  }
  if (_temporaryRightComponent) {
    iTermStatusBarSpringComponent *spring = [iTermStatusBarSpringComponent
        springComponentWithCompressionResistance:1];
  ...
  if (!components.count) {
    NSDictionary *placeholderConfiguration = @{
      iTermStatusBarComponentConfigurationKeyLayoutAdvancedConfigurationDictionaryValue :
          _layout.advancedConfiguration.dictionaryValue
    };
    [components addObject:[[iTermStatusBarPlaceholderComponent alloc]
                              initWithConfiguration:placeholderConfiguration
                                              scope:nil]];
  }
  for (id<iTermStatusBarComponent> component in components) {
    iTermStatusBarContainerView *view =
        [self containerViewForComponent:component];
    if (view) {
  ...
  NSString *const sessionID =
      [_scope valueForVariableName:iTermVariableKeySessionID] ?: @"";

  [_containerViews enumerateObjectsUsingBlock:^(
                       iTermStatusBarContainerView *_Nonnull view,
                       NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    [view setUnreadCount:[[iTermStatusBarUnreadCountController sharedInstance]
                             unreadCountForComponentWithIdentifier:identifier
                                                         sessionID:sessionID]];
  }];
  // setDelegate: may have side effects that expect _containerViews to be
  // populated.
  for (id<iTermStatusBarComponent> component in components) {
...
- (void)updateColors {
  for (iTermStatusBarContainerView *view in _containerViews) {
    NSColor *tintColor = [view.component statusBarTextColor]
                             ?: [self statusBarComponentDefaultTextColor];
  ...
  [[iTermStatusBarView castFrom:self.view]
      setSeparatorColor:[self.delegate statusBarSeparatorColor]];
  [[iTermStatusBarView castFrom:self.view]
      setBackgroundColor:[self.delegate statusBarBackgroundColor]];
  _autoRainbowController.darkBackground =
      [self.delegate statusBarHasDarkBackground];
...
- (nullable id<iTermStatusBarComponent>)componentWithIdentifier:
    (NSString *)identifier {
  return
      [_containerViews objectPassingTest:^BOOL(
                           iTermStatusBarContainerView *element,
                           NSUInteger index, BOOL *stop) {
        return
            [element.component.statusBarComponentIdentifier isEqual:identifier];
      }].component;
}
...
- (nullable __kindof id<iTermStatusBarComponent>)visibleComponentWithIdentifier:
    (NSString *)identifier {
  return
      [_visibleContainerViews objectPassingTest:^BOOL(
                                  iTermStatusBarContainerView *element,
                                  NSUInteger index, BOOL *stop) {
        return
            [element.component.statusBarComponentIdentifier isEqual:identifier];
...
- (BOOL)statusBarComponentTerminalBackgroundColorIsDark:
    (id<iTermStatusBarComponent>)component {
  return [[self.delegate statusBarTerminalBackgroundColor]
...
- (void)statusBarComponent:(id<iTermStatusBarComponent>)component
  ...
  [self.delegate statusBarReportScriptingError:error
...
- (void)statusBarContainerView:(iTermStatusBarContainerView *)sender
                 hideComponent:(id<iTermStatusBarComponent>)component {
  iTermStatusBarLayout *layout = [[iTermStatusBarLayout alloc]
      initWithDictionary:[self.layout dictionaryValue]
                   scope:_scope];
  layout.components =
      [layout.components it_arrayByRemovingObjectsPassingTest:^BOOL(
                             id<iTermStatusBarComponent> anObject) {
        return anObject.class == component.class;
      }];
  [self.delegate statusBarSetLayout:layout];
}

- (void)statusBarContainerViewDisableStatusBar:
    (iTermStatusBarContainerView *)sender {
  [self.delegate statusBarDisable];
}

- (void)statusBarContainerViewConfigureStatusBar:
    (iTermStatusBarContainerView *)sender {
  [self.delegate statusBarOpenPreferencesToComponent:nil];
}

- (void)statusBarContainerView:(iTermStatusBarContainerView *)sender
            configureComponent:(id<iTermStatusBarComponent>)component {
...
- (void)updateAutoRainbowColorsIfNeeded {
  ...
  [_autoRainbowController
      enumerateColorsWithCount:_visibleContainerViews.count
                         block:^(NSInteger i, NSColor *_Nonnull color) {
                           id<iTermStatusBarComponent> component =
                               _visibleContainerViews[i].component;
                           ...
                           if ([knobValues[iTermStatusBarSharedTextColorKey]
                                   isEqualToDictionary:colorDict]) {
                             return;
                           }
                           knobValues[iTermStatusBarSharedTextColorKey] =
                               colorDict;
                           [component
                               statusBarComponentSetKnobValues:knobValues];
                         }];
}
