...
NSString *const iTermTmuxWindowCacheDidChange =
    @"iTermTmuxWindowCacheDidChange";
...
@property(nonatomic, strong)
    NSMutableArray<iTermTmuxCacheWindow *> *mutableWindows;
@end

@implementation iTermTmuxCacheSession
- (void)setMutableWindows:
    (NSMutableArray<iTermTmuxCacheWindow *> *)mutableWindows {
...
- (NSArray<iTermTmuxWindowCacheWindowInfo *> *)hiddenWindows {
  ...
  [_clients enumerateObjectsUsingBlock:^(iTermTmuxCacheClient *_Nonnull client,
                                         NSUInteger idx, BOOL *_Nonnull stop) {
    TmuxController *controller = [[TmuxControllerRegistry sharedInstance]
        controllerForClient:client.identifier];
    [client.sessions
        enumerateObjectsUsingBlock:^(iTermTmuxCacheSession *_Nonnull session,
                                     NSUInteger idx, BOOL *_Nonnull stop) {
          if (session.sessionObject.number != controller.sessionId) {
            return;
          }
          [session.mutableWindows enumerateObjectsUsingBlock:^(
                                      iTermTmuxCacheWindow *_Nonnull window,
                                      NSUInteger idx, BOOL *_Nonnull stop) {
            // Windows that haven't been opened because there were too many at
            // attach time will not be labeled as hidden. Instead, they simply
            // won't have windows. Therefore we don't use windowIsHidden:.
            if ([controller window:window.number] != nil) {
              return;
            }
            [results addObject:[[iTermTmuxWindowCacheWindowInfo alloc]
                                   initWitCacheWindow:window
                                              session:session
...
- (void)tmuxControllerSessionsDidChange:(NSNotification *)notification {
  ...
  client.sessions = [[controller.sessionObjects
      mapWithBlock:^id(iTermTmuxSessionObject *sessionObject) {
...
- (void)tmuxControllerWindowsDidChange:(NSNotification *)notification {
  TmuxController *controller = notification.object;
  iTermTmuxCacheSession *session =
      [self sessionInClient:[self clientNamed:controller.clientName]
...
- (void)tmuxControllerWindowOpened:(NSNotification *)notification {
  ...
  iTermTmuxCacheWindow *window =
      [self windowInClient:[self clientNamed:controller.clientName] number:wid];
  if (!window) {
    return;
  }
  [self invalidate];
...
- (void)tmuxControllerSessionWasRenamed:(NSNotification *)notification {
  ...
  TmuxController *controller = params[2];
  iTermTmuxCacheSession *session =
      [self sessionWithNumber:sessionID.intValue
...
- (void)tmuxControllerRegistryDidChange:(NSNotification *)notification {
  ...
  for (NSString *clientName in
    ...
    for (iTermTmuxSessionObject *sessionObject in controller.sessionObjects) {
      ...
      [controller listWindowsInSessionNumber:sessionObject.number
                                      target:self
                                    selector:@selector(didListWindows:
                                                           forSession:)
                                      object:session];
    }
...
- (iTermTmuxCacheSession *)sessionInClient:(iTermTmuxCacheClient *)client
                                    number:(int)number {
...
- (iTermTmuxCacheSession *)sessionInClient:(iTermTmuxCacheClient *)client
                                withWindow:(iTermTmuxCacheWindow *)window {
  for (iTermTmuxCacheSession *session in client.sessions) {
...
- (iTermTmuxCacheWindow *)windowInClient:(iTermTmuxCacheClient *)client
                                  number:(int)wid {
  for (iTermTmuxCacheSession *session in client.sessions) {
...
- (iTermTmuxCacheSession *)sessionWithNumber:(int)number
                                    inClient:(iTermTmuxCacheClient *)client {
  for (iTermTmuxCacheSession *session in client.sessions) {
    if (session.sessionObject.number == number) {
      return session;
    }
...
- (void)reallyInvalidate {
  _needsInvalidate = NO;
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermTmuxWindowCacheDidChange
                    object:nil];
}

#pragma mark - Callbacks

- (void)didListWindows:(TSVDocument *)doc
            forSession:(iTermTmuxCacheSession *)session {
  ...
  for (NSArray *record in doc.records) {
    ...
    window.number =
        [[[doc valueInRecord:record
                    forField:@"window_id"] substringFromIndex:1] intValue];
...
- (void)setWindows:(TSVDocument *)doc
        forSession:(iTermTmuxCacheSession *)session {
  if (![self haveSession:session]) {
    return;
  }
