...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  NSRectFill(
      NSMakeRect(0, self.bounds.size.height - 1, self.bounds.size.width, 1));
...
+ (id)announcementViewWithTitle:(NSString *)title
  ...
  iTermAnnouncementView *view =
      [[self alloc] initWithFrame:NSMakeRect(0, 0, 1000, 44)];
  view.style = style;
  [view setTitle:title];
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    if (@available(macOS 10.14, *)) {
      // This forces the button text to be dark. This view is always light
      // regardless of theme.
      self.appearance =
    ...
    _internalView =
        [[iTermAnnouncementInternalView alloc] initWithFrame:frameRect];
    _internalView.autoresizingMask = NSViewWidthSizable | NSViewHeightSizable;
    [self addSubview:_internalView];

    NSImage *closeImage = [NSImage it_imageNamed:@"closebutton"
                                        forClass:self.class];
    NSSize closeSize = closeImage.size;
    _buttonWidth = ceil(closeSize.width + kMargin);
    NSButton *closeButton = [[NSButton alloc]
        initWithFrame:NSMakeRect(
                          frameRect.size.width - _buttonWidth,
...
- (void)createButtonsFromActions:(NSArray *)actions
                           block:(void (^)(int index))block {
  _block = [block copy];
  ...
  // If there are <=2 buttons, create all as buttons. Otherwise create first as
  // button.
  int start;
  int limit;
  int step;
  ...
  if (shouldCreatePopup) {
    pullDown = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(0, 0, 0, 0)
                                          pullsDown:YES];
    ...
    pullDown.frame =
        NSMakeRect(rect.size.width - _buttonWidth,
                   floor((rect.size.height - pullDown.frame.size.height) / 2),
                   pullDown.frame.size.width, pullDown.frame.size.height);
...
- (NSString *)stringByAddingShortcutInString:(NSString *)original {
  ...
  if (index == NSNotFound) {
    ...
    NSString *shortcut = [NSString
        stringWithLongCharacter:[original characterAtIndex:index + 1]];
    NSString *modifiedOriginal =
        [original stringByReplacingCharactersInRange:NSMakeRange(index, 1)
                                          withString:@""];
...
- (void)setTitle:(NSString *)title {
  ...
  CGFloat y =
      floor((_internalView.frame.size.height - iconImage.size.height) / 2);
  {
    [_icon removeFromSuperview];
    _icon = [[NSImageView alloc]
        initWithFrame:NSMakeRect(kMargin, y, iconImage.size.width,
...

  {
    ...
    _textView.frame = NSMakeRect(
        rect.origin.x, floor((_internalView.frame.size.height - height) / 2),
...
- (void)updateTrackingAreas {
  if (self.window) {
    ...
    NSTrackingArea *trackingArea = [[NSTrackingArea alloc]
        initWithRect:_internalView.frame
             options:NSTrackingInVisibleRect | NSTrackingActiveInKeyWindow |
...
- (void)updateTextViewFrame {
  ...
  NSRect textRect = NSMakeRect(
      rect.origin.x, floor((_internalView.frame.size.height - height) / 2),
      _internalView.frame.size.width - _buttonWidth - NSMaxX(_icon.frame) -
          kMargin,
      height);
...
- (void)resizeSubviewsWithOldSize:(NSSize)oldSize {
  ...
  _closeButton.frame = NSMakeRect(
      self.frame.size.width - closeSize.width - kMargin,
  ...
  CGFloat y =
      floor((_internalView.frame.size.height - iconFrame.size.height) / 2);
  _icon.frame =
      NSMakeRect(kMargin, y, iconFrame.size.width, iconFrame.size.height);
}
