...
// Returns YES on success or NO if it failed to create a socket (out of file
// descriptors maybe?)
...
- (void)waitForChild:(iTermFileDescriptorMultiClientChild *)child
    removePreemptively:(BOOL)removePreemptively
              callback:(iTermCallback<id, iTermResult<NSNumber *> *> *)
                           callback; // number is integer status
