...
static int32_t SetIntermediateByteInPackedCommand(int32_t command,
                                                  unsigned char c) {
...
// Note that only a single intermediate byte is supported. I don't know of any
// CSI codes that use more than one. The integer packing format is very
// efficient, so it's worth the limitation.
...
static BOOL ParseCSIPrologue(iTermParserContext *context,
                             BOOL support8BitControlCharacters,
                             CVector *incidentals) {
  if (support8BitControlCharacters &&
      iTermParserPeek(context) == VT100CC_C1_CSI) {
  ...
  return AdvanceAndEatControlChars(context, support8BitControlCharacters,
                                   incidentals);
}

static BOOL ParseCSIPrefix(iTermParserContext *context,
                           BOOL support8BitControlCharacters,
                           CVector *incidentals, CSIParam *param) {
  // Now we parse Parameter Bytes (ECMA-48, 5.4 - (b))
  //
  // CSI P...P I...I F
  //     ^
  //
  // Parameter Bytes, which, if present, consist of bit combinations from \x30
  // to \x3f;
  //
  ...
  //
  //          note: ECMA-48 is to write ascii codes as (decimal top
  //          nibble)/(decimal lower nibble),
  //                and that a value like 03/15 = 0x3f (see ECMA-48, 4.1).
  //
  //        This description suggests that if the first byte of parameter bytes
  //        is one of
  //        '<', '=', '>', '?' (\x3c-\x3f), it's well-formed and could be
  //        considered as private CSI extension.
  //
  //        Example:
  //
  //          In the DEC VT-series, the '?' prefix is used, such as by
  //          DEC-specific private modes. "CSI > Ps c" is interpreted as a
  //          Secondary Device attributes (DA2) request. Higher versions of the
  //          DEC VT treat "CSI = Ps c" as a Tertiary Device attributes (DA3)
  //          request. Tera Term and RLogin use '<'-prefixed extensions for IME
  //          support. For example, "CSI < Ps t" means "change the IME
  //          open/close state".
  ...
  if (iTermParserCanAdvance(context)) {
    ...
    case '?':
      ...
      if (!AdvanceAndEatControlChars(context, support8BitControlCharacters,
                                     incidentals)) {
        return NO;
      }
      break;
...
static BOOL ParseCSIParameters(iTermParserContext *context,
                               ...
                               BOOL *unrecognized) {
  //     2. parse parameters
  //        Typically, it consists of '0'-'9' or ';'. If there are sub
  //        parameters, they'll be colon-delimited. <parameter>:<sub 1>:<sub
  //        2>:<sub 3>...:<sub N>
  //        '<', '=', '>', '?' should be ignored, but if current sequence
  //        contains them, this sequence should be mark as unrecognized.
  BOOL isSub = NO;
  ...
  while (iTermParserTryPeek(context, &c) && c >= 0x30 && c <= 0x3f) {
    ...
    case '9': {
      ...
      if (isSub && param->count > 0) {
        // This implementation is not really well aligned with the spec. In
        // ECMA-48 section 5.4, the format of a CSI code is described. The
        // parameter string, which follows CSI, is a semicolon-delimited list of
        // parameter substrings A parameter substring is a sequence of digits
        // with colon separators. The data structure we use treats each
        // parameter string up to the first colon (if any) as the parameter, and
        // parts after the first colon as sub-parameters. That doesn't really
        // make sense if a parameter string starts with a colon, which is
        // allowed but not defined in the spec. Since that never should happen
        // in practice, we'll just ignore a parameter string that starts with a
        // colon.
    ...
    case ':':
      ...
      // ECMA-48 suggests it may be used as a separator in a parameter
      // sub-string (5.4.2 - (b)), but it seems the usage of ":" around SGR is
      // confused a little.
      //
...
static BOOL ParseCSIIntermediate(iTermParserContext *context,
  ...
  // CSI P...P I...I F
  //           ^
  // Intermediate Bytes, if present, consist of bit combinations from 02/00 to
  // 02/15.
  //
  unsigned char c;
...
static BOOL ParseCSIGarbage(iTermParserContext *context,
                            BOOL support8BitControlCharacters,
                            CVector *incidentals, BOOL *unrecognized) {
  // compatibility HACK:
  //
  // CSI P...P I...I (G...G) F
  //                  ^
  ...
  while (iTermParserTryPeek(context, &c)) {
    if (c >= 0x40 && c <= 0x7e) { // final byte
      ...
      if (c > 0x1f && c != 0x7f) {
        ...
        // mark current sequence as "unrecognized". The only way to get here is
        // to have a character in the range [0x20,0x3f] occur after an 0x7f.
        *unrecognized = YES;
      }
      if (!AdvanceAndEatControlChars(context, support8BitControlCharacters,
                                     incidentals)) {
...
static void ParseCSIFinal(iTermParserContext *context, CSIParam *param,
                          BOOL *unrecognized) {
...
static void ParseCSISequence(iTermParserContext *context,
                             BOOL support8BitControlCharacters, CSIParam *param,
                             CVector *incidentals) {
  // A CSI sequence consists of a prefix byte, zero or more parameters
  // (optionally with sub- parameters), zero or more intermediate bytes, and a
  // final byte.
  //
  // The prefix, intermediate, and final bytes are packed into an integer and
  // stored in param->cmd. The parameters and sub-parameters are stored in
  // param->p and param->sub.
  //
  // - Parameter Prefix Byte (if present, range: \x3a-\x3f)
  // - Intermediate Bytes (actually, just the last one) (if present, range:
  // \x20-\x2f)
  ...
  //
  // Each (prefix, intermediate, final) 3-tuple has a unique packed
  // representation.
  ...
  if (ParseCSIPrologue(context, support8BitControlCharacters, incidentals) &&
      ParseCSIPrefix(context, support8BitControlCharacters, incidentals,
                     param) &&
      ParseCSIParameters(context, support8BitControlCharacters, incidentals,
                         param, &unrecognized) &&
      ParseCSIIntermediate(context, support8BitControlCharacters, incidentals,
                           param) &&
      ParseCSIGarbage(context, support8BitControlCharacters, incidentals,
                      &unrecognized)) {
...
static void SetCSITypeAndDefaultParameters(CSIParam *param,
                                           VT100Token *result) {
  switch (param->cmd) {
