...
- (void)updateValues {
  ...
  if (_bytesPerCallKey &&
      [[NSUserDefaults standardUserDefaults] objectForKey:_bytesPerCallKey]) {
    _bytesPerCall =
        [[NSUserDefaults standardUserDefaults] integerForKey:_bytesPerCallKey];
  }
  if (_delayBetweenCallsKey && [[NSUserDefaults standardUserDefaults]
                                   objectForKey:_delayBetweenCallsKey]) {
    _delayBetweenCalls = [[NSUserDefaults standardUserDefaults]
        floatForKey:_delayBetweenCallsKey];
  }
}
...
- (void)setBytesPerCall:(int)newBytesPerCall {
  ...
  if (_bytesPerCallKey) {
    [[NSUserDefaults standardUserDefaults] setInteger:_bytesPerCall
                                               forKey:_bytesPerCallKey];
  }
}
