...
static NSString *const kPreferenceDidChangeFromOtherPanel =
    @"kPreferenceDidChangeFromOtherPanel";

// key for userInfo dictionary of kPreferenceDidChangeFromOtherPanel
// notification having key of changed preference.
NSString *const kPreferenceDidChangeFromOtherPanelKeyUserInfoKey = @"key";

@interface iTermPreferencesBaseViewController ()
// If set to YES, then controls won't be updated with values from backing store
// when it changes.
...
- (IBAction)settingChanged:(id)sender {
  ...
  if (info.customSettingChangedHandler) {
    ...
    case kPreferenceInfoTypeDoubleTextField: {
      NSScanner *scanner =
          [NSScanner localizedScannerWithString:[sender stringValue]];
      double value;
      if ([scanner scanDouble:&value]) {
        // A double value may take a legal but non-canonical form during
        // editing. For example, "1." is a valid way of specifying 1.0. But we
        // don't want to replace the control's value with "1", or there's no way
        // of entering "1.2", for example. We turn on disableUpdates so
        // -setDouble:forKey: doesn't have the side-effect of updating the
        // control with its canonical value.
    ...
    case kPreferenceInfoTypeUnsignedIntegerTextField:
      ...
      [self setUnsignedInteger:[sender separatorTolerantUnsignedIntegerValue]
                        forKey:info.key];
      break;
  ...
  if ([self shouldUpdateOtherPanels]) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kPreferenceDidChangeFromOtherPanel
                      object:self
                    userInfo:@{
                      kPreferenceDidChangeFromOtherPanelKeyUserInfoKey :
                          info.key
                    }];
  }
}
...
- (PreferenceInfo *)defineControl:(NSControl *)control
  ...
  if (!settingChanged || !update) {
    ...
    assert(type !=
           kPreferenceInfoTypeRadioButton); // This is just a modernized matrix
  }
...
- (void)updateValueForInfo:(PreferenceInfo *)info {
  ...
  case kPreferenceInfoTypeCheckbox: {
    ...
    button.state = [self boolForKey:info.key] ? NSControlStateValueOn
  ...
  case kPreferenceInfoTypeInvertedCheckbox: {
    assert([info.control isKindOfClass:[NSButton class]]);
    NSButton *button = (NSButton *)info.control;
    button.state = [self boolForKey:info.key] ? NSControlStateValueOff
  ...
  case kPreferenceInfoTypeUnsignedIntegerTextField: {
    assert([info.control isKindOfClass:[NSTextField class]]);
    NSTextField *field = (NSTextField *)info.control;
    field.stringValue = [NSString
        stringWithFormat:@"%lu", [self unsignedIntegerForKey:info.key]];
  }
...
- (void)associateStepper:(NSStepper *)stepper
          withPreference:(PreferenceInfo *)info {
...
- (void)postRefreshNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kRefreshTerminalNotification
                    object:nil
                  userInfo:nil];
}
...
- (void)saveDeferredUpdates {
  [_keys enumerateObjectsUsingBlock:^(id _Nonnull key, BOOL *_Nonnull stop) {
    ...
    if (info.deferUpdate) {
      switch (info.type) {
      case kPreferenceInfoTypeIntegerTextField:
        [self setInt:[[NSTextField castFrom:info.control]
                         separatorTolerantIntValue]
              forKey:key];
        break;
...
- (void)applyIntegerConstraints:(PreferenceInfo *)info {
  // NSNumberFormatter seems to have lost its mind on Lion. See a description of
  // the problem here:
...
- (void)applyUnsignedIntegerConstraints:(PreferenceInfo *)info {
  assert([info.control isKindOfClass:[NSTextField class]]);
  NSTextField *textField = (NSTextField *)info.control;
  NSUInteger iv = [self unsignedIntegerForString:[textField stringValue]
                                         inRange:info.range];
...
// This is a notification signature but it gets called because we're the
// delegate of text fields.
...
- (void)controlTextDidEndEditing:(NSNotification *)aNotification {
  ...
  if (info.controlTextDidEndEditing) {
    ...
    case kPreferenceInfoTypeIntegerTextField:
      if (info.deferUpdate) {
        const int intValue =
            [[NSTextField castFrom:info.control] separatorTolerantIntValue];
        [self setInt:intValue forKey:info.key];
      }
      break;

    case kPreferenceInfoTypeDoubleTextField:
      // Replace the control with its canonical value. Only floating point text
      // fields can temporarily take illegal values, which are tolerated until
      // editing ends. See the comments in -settingChanged: for more details on
      // why doubles are special.
...
- (void)preferenceDidChangeFromOtherPanel:(NSNotification *)notification {
  NSString *key =
      notification.userInfo[kPreferenceDidChangeFromOtherPanelKeyUserInfoKey];
...
- (NSRect)windowFrameForTabViewSize:(NSSize)tabViewSize
                            tabView:(NSTabView *)tabView {
  NSWindow *window = self.view.window;
  NSView *contentView = window.contentView;
  const CGFloat left = NSMinX([tabView convertRect:tabView.bounds
                                            toView:contentView]);
  const CGFloat right =
      NSWidth(contentView.frame) - NSMaxX([tabView convertRect:tabView.bounds
                                                        toView:contentView]);
  const CGFloat below = NSMinY([tabView convertRect:tabView.bounds
                                             toView:contentView]);
  const CGFloat above =
      NSHeight(contentView.frame) - NSMaxY([tabView convertRect:tabView.bounds
                                                         toView:contentView]);
  const NSRect contentViewFrame =
      NSMakeRect(0, 0, left + tabViewSize.width + right,
                 below + tabViewSize.height + above);
  const NSSize windowSize =
      [window frameRectForContentRect:contentViewFrame].size;
  const NSPoint windowTopLeft =
      NSMakePoint(NSMinX(window.frame), NSMaxY(window.frame));
  return NSMakeRect(windowTopLeft.x, windowTopLeft.y - windowSize.height,
                    windowSize.width, windowSize.height);
}

- (void)resizeWindowForView:(NSView *)theView
                    tabView:(NSTabView *)tabView
                   animated:(BOOL)animated {
  const CGFloat minimumWidthForOuterTabView =
      644; // Reserve space for general, appearance, profiles, etc. buttons
  const CGFloat kTabViewMinWidth =
      MAX(minimumWidthForOuterTabView, self.minimumWidth);
  const CGFloat inset =
      NSWidth(tabView.bounds) - NSWidth(theView.superview.bounds);
  const CGFloat bottomMargin = 36;
  CGSize tabViewSize =
      NSMakeSize(MAX(kTabViewMinWidth, theView.bounds.size.width) + inset,
