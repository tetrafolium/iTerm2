...
// macOS sends a *LOT* of screenParametersDidChange notifications for all kinds
// of unexpected reasons. For example, changing desktops will do it. So will
// miniaturizing. I've even seen it simply because the app got activated. We
// need to destroy and re-create metal views when a display is detached and
// re-attached. That is rare compared to all the other stuff. This is an attempt
// to detect screen removal/additions. There are still a lot of false positives,
// but maybe it will help. This doesn't really belong in this file but I don't
// have a better place for it yet.
NSNotificationName const iTermScreenParametersDidChangeNontrivally =
    @"iTermScreenParametersDidChangeNontrivally";
static _Atomic int gShouldPostNontrivialScreenParametersChange;
static void
iTermDisplayReconfigurationCallback(CGDirectDisplayID display,
                                    CGDisplayChangeSummaryFlags flags,
                                    void *userInfo) {
  DLog(@"iTermDisplayReconfigurationCallback display=%@ flags=%@", @(display),
       @(flags));
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      if (gShouldPostNontrivialScreenParametersChange) {
        ...
        [[NSNotificationCenter defaultCenter]
            postNotificationName:iTermScreenParametersDidChangeNontrivally
                          object:nil];
      }
    });
  }
  ...
  if (self) {
    ...
    dispatch_once(&onceToken, ^{
      CGDisplayRegisterReconfigurationCallback(
          iTermDisplayReconfigurationCallback, nil);
    });
  }
  return self;
}
...
- (void)updateWithSanityCheck:(BOOL)sanityCheck {
  ...
  NSScreen *currentScreenWithDock =
      [self screenWithDockFromScreens:screensToHideDock];

  const BOOL shouldHideDock =
      currentScreenWithDock != nil ||
      [self haveFullScreenWindowOnSameScreenWhereDockWasLastHidden];
  // If hiding the sock, set screenWithDock to the best guess of the screen that
  // has the dock. It could be that currentScreenWithDock is nil because our
  // presentation is hiding the dock. In that case, carry forward our best guess
  // from _lastScreen. This value becomes the new _lastScreen, provided
  // shouldHideDock is true.
  NSScreen *screenWithDock =
      shouldHideDock ? (currentScreenWithDock ?: _lastScreen) : nil;

  if (sanityCheck && !shouldHideDock && ![self anyScreenHasDock] &&
      [self dockIsCurrentlyHidden] && screensToHideDock.count > 0) {
    // This happens when -update is called when a fullscreen window is causing
    // the dock to be hidden. The easiest way to reproduce it is to turn off
    // input broadcasting.
    DLog(@"Schedule sanity check for next spin of the runlooop. Showing the "
         @"dock while hidden and no screen has the dock and there is a full "
         @"screen window.");
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)logScreens {
  for (NSScreen *screen in [NSScreen screens]) {
    DLog(@"Screen %@ frame=%@ visibleFrame=%@: has menu bar=%@ has dock=%@",
         screen, NSStringFromRect(screen.frame),
         NSStringFromRect(screen.visibleFrame),
         @([self screenHasMenuBar:screen]), @(screen.hasDock));
  }
}

// When the dock is hidden because we have set the auto-hide dock presentation
// option, we can't figure out which screen *would* have the dock because their
// frames equal their visibleFrames. Instead, try to figure out which of the
// current screens is like the last screen where we saw the dock, just before
// hiding it. This can go wrong if the screen configuration changes. That is
...
- (BOOL)haveFullScreenWindowOnSameScreenWhereDockWasLastHidden {
  DLog(@"Checking if there's still a full screen window on the same screen "
       @"where we last saw the dock. The last such screen had frame %@",
  ...
  NSArray<id<iTermPresentationControllerManagedWindowController>>
      *windowControllers = [self.delegate presentationControllerManagedWindows];
  for (id<iTermPresentationControllerManagedWindowController>
           windowController in windowControllers) {
    NSScreen *screen = nil;
    if (![self windowControllerIsWorthyOfConsideration:windowController
                                                screen:&screen]) {
      continue;
    }
    NSWindow *window =
        [windowController presentationControllerManagedWindowControllerWindow];
    DLog(@"  Considering fullscreen window controller %@ whose screen has "
         @"frame %@",
         windowController, NSStringFromRect(window.screen.frame));
    if (window.screen &&
        NSIntersectsRect(window.screen.frame, _lastScreen.frame)) {
      DLog(@"  > Yup");
      return YES;
    }
  }
...
- (BOOL)dockIsCurrentlyHidden {
  return (NSApp.presentationOptions & NSApplicationPresentationAutoHideDock) !=
         0;
}
...
- (void)forceShowMenuBarAndDock {
  [self setApplicationPresentationFlagsWithHiddenDock:NO
                                              menuBar:NO
                                       screenWithDock:nil];
}
...
NSString *PODescription(NSApplicationPresentationOptions presentationOptions) {
  ...
  if ((presentationOptions & (1 << 12))) {
    [array
        addObject:@"NSApplicationPresentationDisableCursorLocationAssistance"];
  }
...
- (void)setApplicationPresentationFlagsWithHiddenDock:(BOOL)shouldHideDock
                                       ...
                                       screenWithDock:
                                           (NSScreen *)screenWithDock {
  DLog(@"setting options: hide dock=%@ hide menu bar=%@", @(shouldHideDock),
       @(shouldHideMenuBar));
  ...
  NSApplicationPresentationOptions presentationOptions =
      (NSApp.presentationOptions & ~mask);
  if (shouldHideDock) {
  ...
  if (presentationOptions & NSApplicationPresentationFullScreen) {
    // Do not remove auto-hide dock/menubar when in full screen or else you
    // don't get a title bar w/ title bar view controller. A new feature of
    // macOS 10.15.6. Issue 9164
  ...
  DLog(@"Set presentation options from %@ to %@",
       PODescription(NSApp.presentationOptions),
...
- (BOOL)windowControllerIsWorthyOfConsideration:
            (id<iTermPresentationControllerManagedWindowController>)
                windowController
                                         screen:(out NSScreen **)screenPtr {
  DLog(@"Checking if %@ is worthy of consideration", windowController);
  BOOL lion = NO;
  const BOOL fullscreen = [windowController
      presentationControllerManagedWindowControllerIsFullScreen:&lion];
  if (!fullscreen) {
...
// 5. Become actgive
//
// For some reason the screen visibleFrame is wrong at this point. Another cycle
// of resign & become active fixes it.
- (NSScreen *)screenWithDockFromScreens:(NSArray<NSScreen *> *)screens {
  DLog(@"Checking if any screen has dock in %@", screens);
  return [screens
      objectPassingTest:^BOOL(NSScreen *screen, NSUInteger index, BOOL *stop) {
        // We need to check both the screen we were given as well as the current
        // "real" screen, because they can have different visibleFrames. My
        // theory is that NSScreen is immutable and copies of it proliferate
...
- (BOOL)shouldHideMenuForWindowController:
    (id<iTermPresentationControllerManagedWindowController>)windowController {
  ...
  DLog(@"  %@: based on hide menu bar in fullscreen setting",
       result ? @"YES" : @"NO");
  return result;
}
...
- (void)screenParametersDidChange:(NSNotification *)notification {
  ...
  DLog(@"screen parameters did change. Set lastScreen to nil and update. This "
       @"could cause the dock to spuriously appear.");
  _lastScreen = nil;
  [self update];
}
