...
- (instancetype)initWithRestorableState:(NSDictionary *)restorableState {
  self = [super init];
  if (self) {
    self.sessions = [restorableState[@"sessionFrameTuples"]
        mapWithBlock:^id(NSArray *tuple) {
    ...
    self.windowType = restorableState[@"windowType"]
                          ? [restorableState[@"windowType"] intValue]
                          : iTermWindowDefaultType();
    self.savedWindowType = restorableState[@"savedWindowType"]
                               ? [restorableState[@"savedWindowType"] intValue]
                               : iTermWindowDefaultType();
    self.screen =
        restorableState[@"screen"] ? [restorableState[@"screen"] intValue] : -1;
  }
  return self;
}
...
- (NSDictionary *)restorableState {
  ...
  return @{
    @"sessionFrameTuples" : maybeSessionFrameTuples ?: @[],
