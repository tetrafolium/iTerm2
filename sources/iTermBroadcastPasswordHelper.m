...
static const char
    iTermBroadcastPasswordHelperEchoProbeSessionAssociatedObjectKey;
...
@implementation iTermBroadcastPasswordHelper {
  ...
  NSArray<PTYSession *> * (^_completion)(NSArray<PTYSession *> *,
                                         NSArray<PTYSession *> *);
  BOOL _cleaningUp;
}

static NSMutableArray<iTermBroadcastPasswordHelper *>
    *sBroadcastPasswordHelpers;

+ (void)tryToSendPassword:(NSString *)password
               toSessions:(NSArray<PTYSession *> *)sessions
               completion:(NSArray<PTYSession *> *_Nonnull (^)(
                              NSArray<PTYSession *> *_Nonnull,
                              NSArray<PTYSession *> *_Nonnull))completion {
  static dispatch_once_t onceToken;
  iTermBroadcastPasswordHelper *helper =
      [[self alloc] initWithPassword:password
                            sessions:sessions
...
- (instancetype)initWithPassword:(NSString *)password
                      ...
                      completion:(NSArray<PTYSession *> * (^)(
                                     NSArray<PTYSession *> *,
                                     NSArray<PTYSession *> *))completion {
  if (sessions.count == 0) {
    return nil;
  }
  ...
  if (self) {
    ...
    _probes = [sessions mapWithBlock:^id(PTYSession *session) {
      if (session.backspaceData) {
        ...
        [probe
            it_setAssociatedObject:session
                            forKey:(void *)&
                            iTermBroadcastPasswordHelperEchoProbeSessionAssociatedObjectKey];
        return probe;
      } else {
...
- (PTYSession *)sessionForProbe:(iTermEchoProbe *)probe {
  return
      [probe it_associatedObjectForKey:(void *)&
             iTermBroadcastPasswordHelperEchoProbeSessionAssociatedObjectKey];
}
...
- (void)checkIfFinished {
  ...
  DLog(@"Finished with successes=%@ failures=%@ indeterminate=%@", _successes,
       _failures, @(_indeterminate));
