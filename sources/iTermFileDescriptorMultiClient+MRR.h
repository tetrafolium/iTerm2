...
typedef NS_ENUM(NSUInteger, iTermFileDescriptorMultiClientAttachStatus) {
  ...
  iTermFileDescriptorMultiClientAttachStatusFatalError, // includes rejection,
                                                        // unexpected errors
  iTermFileDescriptorMultiClientAttachStatusInProgress  // connecting
                                                        // asynchronously
};

iTermFileDescriptorMultiClientAttachStatus
iTermConnectToUnixDomainSocket(NSString *path, int *fdOut, int async);
...
typedef struct {
  ...
  int readFD;
  // as long as this fd is open, nobody else can use the socket. It is an
  // advisory lock using O_EXLOCK.
  int lockFD;

} iTermUnixDomainSocketConnectResult;

iTermUnixDomainSocketConnectResult
iTermCreateConnectedUnixDomainSocket(NSString *path, int closeAfterAccept);
