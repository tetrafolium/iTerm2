...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p files=%@ folders=%@>",
...
- (void)startWatching {
  ...
  _tokens = [_paths.files.allObjects mapWithBlock:^id(NSString *file) {
    return [[NSFileManager defaultManager] monitorFile:file
                                                 block:^(long flags) {
...
- (iTermFilesAndFolders *)pathsToWatch {
  ...
  if ([fileManager itemIsSymlink:path]) {
    NSString *resolved = [fileManager destinationOfSymbolicLinkAtPath:path
                                                                error:nil];
...
- (void)reportError:(NSString *)error file:(NSString *)file {
  ...
  [[iTermScriptHistoryEntry dynamicProfilesEntry]
      addOutput:[error stringByAppendingString:@"\n"]];
...
- (void)reallyReportError:(NSString *)error file:(NSString *)file {
  ...
  if (selection == 1) {
    [[iTermScriptConsole sharedInstance]
        revealTailOfHistoryEntry:[iTermScriptHistoryEntry
                                     dynamicProfilesEntry]];
  }
}
...
- (void)reveal:(id)sender {
  ...
  [[NSWorkspace sharedWorkspace]
      activateFileViewerSelectingURLs:@[ [NSURL fileURLWithPath:file] ]];
}
...
- (NSString *)unresolvedDynamicProfilesPath {
  if ([[iTermAdvancedSettingsModel dynamicProfilesPath] length]) {
    return [[iTermAdvancedSettingsModel dynamicProfilesPath]
        stringByExpandingTildeInPath];
  }
  NSFileManager *fileManager = [NSFileManager defaultManager];
  NSString *appSupport = [fileManager applicationSupportDirectory];
  NSString *thePath =
      [appSupport stringByAppendingPathComponent:@"DynamicProfiles"];
...
- (NSString *)dynamicProfilesPath {
  ...
  return [[NSFileManager defaultManager] destinationOfSymbolicLinkAtPath:thePath
                                                                   error:nil]
             ?: thePath;
}
...
- (void)reallyReloadDynamicProfiles {
  ...
  for (NSString *file in fileNames) {
    ...
    if (![self loadDynamicProfilesFromFile:fullName
                                 intoArray:newProfiles
                                     guids:guids]) {
      [self reportError:[NSString
                            stringWithFormat:@"Ignoring dynamic profiles in "
                                             @"“%@” because of an error.",
                                             fullName]
                   file:fullName];
    }
  ...
  for (Profile *profile in newProfiles) {
    Profile *existingProfile = [self profileWithGuid:profile[KEY_GUID]
                                             inArray:oldProfiles];
    if (existingProfile) {
  ...
  for (Profile *profile in oldProfiles) {
    DLog(@"Check profile name=%@ guid=%@", profile[KEY_NAME],
         profile[KEY_GUID]);
...
- (NSArray<Profile *> *)profilesInFile:(NSString *)filename
  ...
  if (dict) {
    ...
    if (!data) {
      [self
          reportError:[NSString
                          stringWithFormat:
                              @"Dynamic Profiles file %@ is not readable: %@.",
    ...
    if (!dict) {
      [self reportError:[NSString stringWithFormat:@"Dynamic Profiles file %@ "
    ...
    if (!dict) {
      [self reportError:[NSString
                            stringWithFormat:@"Dynamic Profiles file %@ does "
                                             @"not have an Object (i.e., a "
  ...
  if (!entries) {
    ...
    [self
        reportError:[NSString
                        stringWithFormat:@"Dynamic Profiles file %@ does not "
                                         @"have a “Profiles” key at the root.",
                                         filename]
               file:filename];
    return nil;
  }
  ...
  for (Profile *profile in entries) {
    if (![profile[KEY_GUID] isKindOfClass:[NSString class]]) {
      [self
          reportError:[NSString stringWithFormat:@"Dynamic profile is missing "
                                                 @"the Guid field in file %@",
                                                 filename]
                 file:filename];
      continue;
    }
    if (![profile[KEY_NAME] isKindOfClass:[NSString class]]) {
      [self reportError:[NSString
                            stringWithFormat:@"Dynamic profile with Guid %@ is "
    ...
    if ([self nonDynamicProfileHasGuid:profile[KEY_GUID]]) {
      [self
          reportError:[NSString stringWithFormat:
                                    @"Dynamic profile with Guid %@ conflicts "
                                    @"with non-dynamic profile with same Guid",
                                    profile[KEY_GUID]]
                 file:filename];
      continue;
    }
...
- (BOOL)loadDynamicProfilesFromFile:(NSString *)filename
  ...
  for (Profile *profile in allProfiles) {
    if ([guids containsObject:profile[KEY_GUID]]) {
      [self reportError:[NSString
                            stringWithFormat:
                                @"Two dynamic profiles have the same Guid: %@",
                                profile[KEY_GUID]]
                   file:filename];
      continue;
    }
    ...
    {
      Profile *amendedProfile =
          [profile dictionaryBySettingObject:filename
                                      forKey:KEY_DYNAMIC_PROFILE_FILENAME];
...
- (void)updateDynamicProfile:(Profile *)newProfile {
  DLog(@"Updating dynamic profile name=%@ guid=%@", newProfile[KEY_NAME],
       newProfile[KEY_GUID]);
  ...
  [[ProfileModel sharedInstance] setBookmark:merged withGuid:merged[KEY_GUID]];
}
...
- (Profile *)prototypeForDynamicProfile:(Profile *)profile {
  ...
  if (parentName) {
    ...
    if (!prototype) {
      [self reportError:[NSString
                            stringWithFormat:
                                @"Dynamic profile %@ references unknown parent "
                                @"name %@. Using default profile as parent.",
...
- (void)addDynamicProfile:(Profile *)profile {
  DLog(@"Add dynamic profile name=%@ guid=%@", profile[KEY_NAME],
       profile[KEY_GUID]);
  Profile *prototype = [self prototypeForDynamicProfile:profile];
  NSMutableDictionary *merged = [self profileByMergingProfile:profile
                                                  intoProfile:prototype];
  // Don't inherit the deprecated KEY_DEFAULT_BOOKMARK value, which in issue
  // 4115 we learn can cause a dynamic profile to become the default profile!
...
- (BOOL)removeDynamicProfile:(Profile *)profile {
  DLog(@"Remove dynamic profile name=%@ guid=%@", profile[KEY_NAME],
       profile[KEY_GUID]);
