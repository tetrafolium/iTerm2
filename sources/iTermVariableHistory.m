...
+ (void)recordBuiltInVariables {
  ...
  [names enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx,
                                      BOOL *_Nonnull stop) {
    [self recordUseOfVariableNamed:obj
                         inContext:iTermVariablesSuggestionContextSession];
  }];
  [self
      recordUseOfNonterminalVariableNamed:iTermVariableKeyGlobalScopeName
                                inContext:iTermVariablesSuggestionContextSession
                         leadingToContext:iTermVariablesSuggestionContextApp];
  [self
      recordUseOfNonterminalVariableNamed:iTermVariableKeySessionTab
                                inContext:iTermVariablesSuggestionContextSession
                         leadingToContext:iTermVariablesSuggestionContextTab];
  [self
      recordUseOfNonterminalVariableNamed:iTermVariableKeySessionParent
                                inContext:iTermVariablesSuggestionContextSession
                         leadingToContext:
                             iTermVariablesSuggestionContextSession];

  // Tab context
  [self recordUseOfVariableNamed:iTermVariableKeyTabTitleOverride
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfVariableNamed:iTermVariableKeyTabTitleOverrideFormat
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfVariableNamed:iTermVariableKeyTabTmuxWindowName
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfVariableNamed:iTermVariableKeyTabTmuxWindowTitle
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfVariableNamed:iTermVariableKeyTabTmuxWindow
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfVariableNamed:iTermVariableKeyTabID
                       inContext:iTermVariablesSuggestionContextTab];
  [self recordUseOfNonterminalVariableNamed:iTermVariableKeyTabWindow
                                  inContext:iTermVariablesSuggestionContextTab
                           leadingToContext:
                               iTermVariablesSuggestionContextWindow];
  ...
  [self recordUseOfVariableNamed:iTermVariableKeyWindowID
                       inContext:iTermVariablesSuggestionContextWindow];
  [self
      recordUseOfNonterminalVariableNamed:iTermVariableKeyWindowCurrentTab
                                inContext:iTermVariablesSuggestionContextWindow
                         leadingToContext:iTermVariablesSuggestionContextTab];
  [self
      recordUseOfNonterminalVariableNamed:iTermVariableKeyGlobalScopeName
  ...
  // App context
  [self recordUseOfVariableNamed:iTermVariableKeyApplicationPID
...
+ (NSSet<NSString *> *_Nonnull (^)(NSString *_Nonnull))pathSourceForContext:
    (iTermVariablesSuggestionContext)context {
  return [self pathSourceForContext:context augmentedWith:[NSSet set]];
}

+ (NSSet<NSString *> *_Nonnull (^)(NSString *_Nonnull))
    pathSourceForContext:(iTermVariablesSuggestionContext)context
...
+ (void)recordUseOfVariableNamed:(NSString *)namePossiblyContainingNonterminals
                       inContext:
                           (iTermVariablesSuggestionContext)originalContext {
  iTermVariablesSuggestionContext context = originalContext;
  NSString *name =
      [self pathByConsumingNonterminalsInPath:namePossiblyContainingNonterminals
                                      context:originalContext
                                   contextOut:&context];
  assert(name);

  iTermRecordedVariable *record =
      [[iTermRecordedVariable alloc] initTerminalWithName:name];
  NSMutableSet<iTermRecordedVariable *> *records =
      [self mutableRecordedVariableNamesInContext:context];
  if (![records containsObject:record]) {
    DLog(@"Record %@ in context %@", name,
         [iTermVariableHistory stringForContext:context]);
...
+ (NSMutableDictionary<NSNumber *, NSMutableSet<iTermRecordedVariable *> *> *)
    mutableRecordedNames {
  static NSMutableDictionary<NSNumber *,
                             NSMutableSet<iTermRecordedVariable *> *> *records;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults]
                             dictionaryForKey:@"NoSyncRecordedVariables"]
                             ?: @{};
    ...
    for (id key in dict) {
      ...
      NSArray<iTermRecordedVariable *> *names =
          [[NSArray castFrom:dict[key]] mapWithBlock:^id(id anObject) {
...
+ (void)synchronizeRecordedNames {
  NSDictionary *plist = [[self mutableRecordedNames]
      mapValuesWithBlock:^id(NSNumber *key,
                             NSMutableSet<iTermRecordedVariable *> *object) {
        return [object.allObjects
            mapWithBlock:^id(iTermRecordedVariable *anObject) {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:plist
                                            forKey:@"NoSyncRecordedVariables"];
}

+ (NSMutableSet<iTermRecordedVariable *> *)
    mutableRecordedVariableNamesInContext:
        (iTermVariablesSuggestionContext)context {
  NSMutableSet<iTermRecordedVariable *> *set =
      self.mutableRecordedNames[@(context)];
  if (!set) {
...
+ (NSSet<NSString *> *)
    recordedVariableNamesInContext:(iTermVariablesSuggestionContext)context
                     augmentedWith:(NSSet<NSString *> *)augmentations
...
+ (NSSet<NSString *> *)
    recordedVariableNamesInContext:(iTermVariablesSuggestionContext)context
                     augmentedWith:(NSSet<NSString *> *)augmentations
                         excluding:(NSSet<NSString *> *)exclusions
                            prefix:(NSString *)prefix
                     allowUserVars:(BOOL)allowUserVars {
  NSMutableSet<NSString *> *terminalCandidates =
      [[self recordedTerminalVariableNamesInContext:context] mutableCopy];
  ...
  for (NSString *candidate in [terminalCandidates copy]) {
    ...
    NSSet<NSString *> *paths = [self recordedVariableNamesInContext:context
  ...
  NSSet<NSString *> *nonterminalCandidates =
      [self recordedNonterminalVariableNamesInContext:context];
...
+ (NSString *)
    pathByConsumingNonterminalsInPath:(NSString *)prefix
                              context:(iTermVariablesSuggestionContext)context
                           contextOut:(out iTermVariablesSuggestionContext *)
                                          contextPtr {
  NSSet<NSString *> *candidates =
      [self recordedTerminalVariableNamesInContext:context];
...
// Consume leading terminals from prefix beginning with candidate. For example,
// given the prefix "tab.currentSession.tab.foo" in the context of a session and
// candidate "tab", return "foo" in the context of a tab.
+ (NSString *)
    pathByConsumingNonterminalsInPath:(NSString *)prefix
  ...
  NSArray<NSString *> *candidateParts =
      [candidate componentsSeparatedByString:@"."];
  ...
  const BOOL isNonterminal = [self pathIsNonterminal:accumulatedPath
  ...
  return [self pathByConsumingNonterminalsInPath:updatedPrefix
                                         context:currentContext
                                      contextOut:contextPtr];
}

+ (NSSet<NSString *> *)recordedVariableNamesInContext:
                           (iTermVariablesSuggestionContext)context
  ...
  if (candidateParts.count >= prefixParts.count) {
    // Candidate is same length or longer than prefix. Use it but don't expand
    // it.
    return [NSSet setWithObject:candidate];
  }

  // Prefix is longer than candidate. That's OK if the path ends in a
  // nonterminal.
  NSString *accumulatedPath = [accum componentsJoinedByString:@"."];
  iTermVariablesSuggestionContext currentContext = context;
  const BOOL isNonterminal = [self pathIsNonterminal:accumulatedPath
                                           inContext:&currentContext];
  if (!isNonterminal) {
    // Prefix is longer than path and traverses a nonterminal so we have to
    // stop.
    return [NSSet set];
  }

  NSString *updatedPrefix = [prefixParts componentsJoinedByString:@"."];
  NSSet<NSString *> *innerNames =
      [self recordedVariableNamesInContext:currentContext
                             augmentedWith:[NSSet set]
                                    prefix:updatedPrefix];
  NSString *commonPrefix = [accumulatedPath stringByAppendingString:@"."];
  return [NSSet setWithArray:[innerNames.allObjects mapWithBlock:^id(
                                                        NSString *innerName) {
                  return [commonPrefix stringByAppendingString:innerName];
                }]];
}

+ (BOOL)pathIsNonterminal:(NSString *)path
                inContext:(inout iTermVariablesSuggestionContext *)contextPtr {
  NSSet<iTermRecordedVariable *> *vars =
      [self recordedVariablesInContext:*contextPtr];
  iTermRecordedVariable *record =
      [[vars allObjects] objectPassingTest:^BOOL(iTermRecordedVariable *record,
...
+ (NSSet<iTermRecordedVariable *> *)recordedVariablesInContext:
    (iTermVariablesSuggestionContext)context {
  ...
  for (int bit = 0; bit < 64; bit++) {
    ...
    if (mask & context) {
      NSSet<iTermRecordedVariable *> *records =
          self.mutableRecordedNames[@(mask)] ?: [NSSet set];
      result = [result setByAddingObjectsFromSet:records];
    }
  }
  if ((context & (iTermVariablesSuggestionContextSession |
                  iTermVariablesSuggestionContextTab)) &&
    ...
    result = [NSSet
        setWithArray:[result.allObjects
                         arrayByAddingObjectsFromArray:
                             [appVariables.allObjects
                                 mapWithBlock:^id(
                                     iTermRecordedVariable *appVariable) {
                                   return [appVariable
                                       recordByPrependingPath:@"iterm2."];
                                 }]]];
  }
  return result;
}

+ (NSSet<NSString *> *)recordedTerminalVariableNamesInContext:
    (iTermVariablesSuggestionContext)context {
  return [NSSet
      setWithArray:[[[self recordedVariablesInContext:context] allObjects]
...
+ (NSSet<NSString *> *)recordedNonterminalVariableNamesInContext:
    (iTermVariablesSuggestionContext)context {
  return [NSSet
      setWithArray:[[[self recordedVariablesInContext:context] allObjects]
                       mapWithBlock:^id(iTermRecordedVariable *record) {
...
+ (void)
    recordUseOfNonterminalVariableNamed:(NSString *)name
                              inContext:(iTermVariablesSuggestionContext)context
                       leadingToContext:
                           (iTermVariablesSuggestionContext)leadingToContext {
  iTermRecordedVariable *record =
      [[iTermRecordedVariable alloc] initNonterminalWithName:name
