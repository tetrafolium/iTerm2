...
@interface ProfilesKeysPreferencesViewController () <
    iTermKeyMappingViewControllerDelegate>
@end
...
- (void)awakeFromNib {
  ...
  [self defineControl:_optionKeySends
      ...
      settingChanged:^(id sender) {
        [self optionKeySendsDidChangeForControl:sender];
      }
      update:^BOOL {
        __strong __typeof(weakSelf) strongSelf = weakSelf;
        if (strongSelf) {
          [strongSelf
              updateOptionKeySendsForControl:strongSelf->_optionKeySends];
          return YES;
        } else {
          return NO;
        }
  ...
  [self defineControl:_rightOptionKeySends
      ...
      update:^BOOL {
        ...
        if (strongSelf) {
          [strongSelf
              updateOptionKeySendsForControl:strongSelf->_rightOptionKeySends];
          return YES;
        } else {
          return NO;
        }
  ...
  info.customSettingChangedHandler = ^(id sender) {
    if ([[self stringForKey:KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS] length]) {
      [self setBool:([sender state] == NSControlStateValueOn)
             forKey:KEY_HAS_HOTKEY];
    } else {
  ...
  info.observer = ^() {
    __strong __typeof(weakSelf) strongSelf = weakSelf;
    if (strongSelf) {
      strongSelf->_configureHotKey.enabled =
          strongSelf->_hasHotkey.state == NSControlStateValueOn;
    }
  };
  ...
  [self addViewToSearchIndex:_keyMappingViewController.view
                 displayName:@"Profile key bindings"
                     phrases:@[
                       @"mapping", @"shortcuts", @"touch bar", @"preset",
                       @"xterm", @"natural", @"terminal.app compatibility",
                       @"numeric keypad"
                     ]
                         key:nil];
...
- (void)layoutSubviewsForEditCurrentSessionMode {
  ...
  iTermSizeRememberingView *sizeRememberingView =
      (iTermSizeRememberingView *)self.view;
...
- (NSArray *)keysForBulkCopy {
  NSArray *keys = @[
    KEY_KEYBOARD_MAP, KEY_TOUCHBAR_MAP, KEY_OPTION_KEY_SENDS,
...
- (NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *)
    incompatibleKeystrokeOrTouchbarBindings {
  ...
  // (Combo) -> (Combo, Action)
  NSDictionary<iTermKeystroke *,
               NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *>
                   *> *inProfileDict = [inProfile
      classifyWithBlock:^id(
          iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *tuple) {
        return [tuple.firstObject keystrokeWithoutVirtualKeyCode];
      }];
  NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *>
      *inAnyPreset = [iTermPresetKeyMappings keystrokeTuplesInAllPresets];

  return [inAnyPreset mapWithBlock:^id(
                          iTermTuple<iTermKeystroke *, iTermKeyBindingAction *>
                              *inPreset) {
    iTermKeystroke *key = [inPreset.firstObject keystrokeWithoutVirtualKeyCode];
    NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *> *actions =
        inProfileDict[key];
    if (actions.count == 0) {
      return nil;
    }
...
- (void)removeKeystrokeBindings:
  ...
  [bindingsToRemove
      enumerateObjectsUsingBlock:^(
          iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *_Nonnull tuple,
          NSUInteger idx, BOOL *_Nonnull stop) {
        const NSUInteger index =
            [[iTermKeyMappings sortedKeystrokesForProfile:profile]
                indexOfObject:tuple.firstObject];
        if (index != NSNotFound) {
...
- (void)commitChangesToProfile:(NSDictionary *)profile {
  [[self.delegate profilePreferencesCurrentModel]
      setBookmark:profile
...
- (void)didToggleLibtickit {
  ...
  NSArray<iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *>
      *incompatibles = [self incompatibleKeystrokeOrTouchbarBindings];
  if (incompatibles.count == 0) {
    return;
  }
  NSArray<NSString *> *descriptions = [incompatibles
      mapWithBlock:^id(
          iTermTuple<iTermKeystroke *, iTermKeyBindingAction *> *tuple) {
        NSString *formattedCombo =
            [iTermKeystrokeFormatter stringForKeystroke:tuple.firstObject];
        iTermKeyBindingAction *action = tuple.secondObject;
        NSString *formattedAction = action.displayName;
        return [NSString
            stringWithFormat:@"%@\t%@", formattedCombo, formattedAction];
      }];
  warning.title = [NSString
      stringWithFormat:@"This profile has some key bindings from a preset that "
                       @"conflict with CSI u. Remove them?"];
  NSString *message = [descriptions componentsJoinedByString:@"\n"];

  iTermScrollingDisclosableView *accessory =
      [[iTermScrollingDisclosableView alloc]
  ...
  paragraphStyle.tabStops = @[ [[NSTextTab alloc] initWithType:NSLeftTabStopType
                                                      location:100] ];

  NSMutableAttributedString *attributedString =
      [accessory.textView.attributedString mutableCopy];
  [attributedString
      enumerateAttributesInRange:NSMakeRange(0, attributedString.string.length)
                         options:0
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id>
                                       *_Nonnull attrs,
                                   NSRange range, BOOL *_Nonnull stop) {
                        attrs = [attrs
                            dictionaryBySettingObject:paragraphStyle
                                               forKey:
...
- (IBAction)csiuHelp:(id)sender {
  NSURL *url =
      [NSURL URLWithString:@"https://iterm2.com/documentation-csiu.html"];
  [[NSWorkspace sharedWorkspace] openURL:url];
}

- (IBAction)openHotKeyPanel:(id)sender {
  iTermHotkeyPreferencesModel *model =
      [[iTermHotkeyPreferencesModel alloc] init];
  model.hasModifierActivation =
      [self boolForKey:KEY_HOTKEY_ACTIVATE_WITH_MODIFIER];
  model.modifierActivation =
      [self unsignedIntegerForKey:KEY_HOTKEY_MODIFIER_ACTIVATION];
  model.primaryShortcut = [[iTermShortcut alloc]
                  initWithKeyCode:[self
                                      unsignedIntegerForKey:KEY_HOTKEY_KEY_CODE]
                        modifiers:[self unsignedIntegerForKey:
                                            KEY_HOTKEY_MODIFIER_FLAGS]
                       characters:[self stringForKey:KEY_HOTKEY_CHARACTERS]
      charactersIgnoringModifiers:
          [self stringForKey:KEY_HOTKEY_CHARACTERS_IGNORING_MODIFIERS]];
  model.autoHide = [self boolForKey:KEY_HOTKEY_AUTOHIDE];
  model.showAutoHiddenWindowOnAppActivation =
      [self boolForKey:KEY_HOTKEY_REOPEN_ON_ACTIVATION];
  ...
  [model setAlternateShortcutDictionaries:
             (id)[self objectForKey:KEY_HOTKEY_ALTERNATE_SHORTCUTS]];

  _hotkeyPanel = [[iTermHotkeyPreferencesWindowController alloc] init];
  _hotkeyPanel
      .descriptorsInUseByOtherProfiles = [[iTermHotKeyController sharedInstance]
      descriptorsForProfileHotKeysExcept:self.delegate
                                             .profilePreferencesCurrentProfile];
  _hotkeyPanel.model = model;

  __weak __typeof(self) weakSelf = self;
  [self.view.window beginSheet:_hotkeyPanel.window
             completionHandler:^(NSModalResponse returnCode) {
               ...
               if (returnCode == NSModalResponseOK) {
                 ...
                 strongSelf->_hasHotkey.state =
                     [strongSelf boolForKey:KEY_HAS_HOTKEY]
    ...
    [iTermKeyMappings
        setMappingAtIndex:0
             forKeystroke:[iTermKeystroke backspace]
                   action:[iTermKeyBindingAction
                              withAction:KEY_ACTION_SEND_C_H_BACKSPACE
                               parameter:@""]
                createNew:YES
...
- (void)updateDeleteSendsCtrlH {
  ...
  iTermKeyBindingAction *action =
      [iTermKeyMappings localActionForKeystroke:[iTermKeystroke backspace]
                                    keyMappings:profile[KEY_KEYBOARD_MAP]];
  const BOOL sendCH = (action.keyAction == KEY_ACTION_SEND_C_H_BACKSPACE);
  _deleteSendsCtrlHButton.state =
      (sendCH ? NSControlStateValueOn : NSControlStateValueOff);
}

#pragma mark - Option Key Sends

- (void)optionKeySendsDidChangeForControl:(NSMatrix *)sender {
  if (sender == _optionKeySends &&
      [[_optionKeySends selectedCell] tag] == OPT_META) {
    [self maybeWarnAboutMeta];
  } else if (sender == _rightOptionKeySends &&
             [[_rightOptionKeySends selectedCell] tag] == OPT_META) {
    [self maybeWarnAboutMeta];
  }
...
- (BOOL)keyMapping:(iTermKeyMappingViewController *)viewController
    shouldImportKeystrokes:(NSSet<iTermKeystroke *> *)keystrokesThatWillChange {
  Profile *profile = [self.delegate profilePreferencesCurrentProfile];
  NSSet<iTermKeystroke *> *keystrokesInProfile =
      [iTermKeyMappings keystrokesInKeyMappingsInProfile:profile];
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  [item
      whenFirst:^(iTermKeystroke *_Nonnull theKeystroke) {
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  [keystrokes enumerateObjectsUsingBlock:^(iTermKeystroke *_Nonnull keystroke,
                                           BOOL *_Nonnull stop) {
    NSUInteger index = [[iTermKeyMappings sortedKeystrokesForProfile:dict]
        indexOfObject:keystroke];
    assert(index != NSNotFound);

    [iTermKeyMappings removeMappingAtIndex:index fromProfile:dict];
  }];

  // Ignore touch bar items because we don't support profile-specific touch bar
  // items.
...
- (void)keyMapping:(iTermKeyMappingViewController *)viewController
  ...
  NSSet<iTermKeystroke *> *keystrokesThatWillChange =
      [iTermPresetKeyMappings keystrokesInKeyMappingPresetWithName:presetName];
  NSSet<iTermKeystroke *> *keystrokesInProfile =
      [iTermKeyMappings keystrokesInKeyMappingsInProfile:profile];
  BOOL replaceAll = NO;
  if (![keystrokesInProfile isSubsetOfSet:keystrokesThatWillChange]) {
  ...
  Profile *updatedProfile =
      [iTermPresetKeyMappings profileByLoadingPresetNamed:presetName
                                              intoProfile:profile
                                           byReplacingAll:replaceAll];
  [[self.delegate profilePreferencesCurrentModel]
      setBookmark:updatedProfile
         withGuid:profile[KEY_GUID]];
  [[self.delegate profilePreferencesCurrentModel] flush];
