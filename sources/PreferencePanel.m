...
/*
 * Preferences in iTerm2 are complicated, to say the least. Here is how the
 * classes are organized.
 *
 * - PreferencePanel: There are two instances of this class: -sharedInstance and
 * -sessionsInstance. The sharedInstance is the app settings panel, while
 * sessionsInstance is for editing a single session (View>Edit Current Session).
 *     - GeneralPreferencesViewController:    View controller for Prefs>General
 *     - AppearancePreferencesViewController: View controller for
 * Prefs>Appearance
 ...
 *     - iTermShortcutsViewController:        View controller for
 * Prefs>Shortcuts
 ...
 *
 *  View controllers of tabs in PreferencePanel derive from
 * iTermPreferencesBaseViewController. iTermPreferencesBaseViewController
 * provides a map from NSControl* to PreferenceInfo. PreferenceInfo stores a
 * pref's type, user defaults key, can constrain its value, and stores pointers
 * to blocks that are run when a value is changed or a field needs to be updated
 *  for customizing how controls are bound to storage. Each view controller
 * defines these bindings in its -awakeFromNib method.
 *
 *  User defaults are accessed through iTermPreferences, which assigns string
 * constants to user defaults keys, defines default values for each key, and
 * provides accessors. It also allows the exposed values to be computed from
 * underlying values. (Currently, iTermPreferences is not used by advanced
 * settings, but that should change).
 *
 *  Because per-profile preferences are similar, a parallel class structure
 * exists for them. The following classes are view controllers for tabs in
 * Prefs>Profiles:
 *
 ...
 *
 *  These derive from iTermProfilePreferencesBaseViewController, which is just
 * like iTermPreferencesBaseViewController, but its methods for accessing
 * preference values take an additional profile: parameter. The analog of
 * iTermPreferences is iTermProfilePreferences.
 *  */
...
NSString *const kKeyBindingsChangedNotification =
    @"kKeyBindingsChangedNotification";
NSString *const kPreferencePanelDidUpdateProfileFields =
    @"kPreferencePanelDidUpdateProfileFields";
NSString *const kSessionProfileDidChange = @"kSessionProfileDidChange";
NSString *const kPreferencePanelDidLoadNotification =
    @"kPreferencePanelDidLoadNotification";
NSString *const kPreferencePanelWillCloseNotification =
    @"kPreferencePanelWillCloseNotification";

static NSString *const iTermPreferencePanelSearchFieldToolbarItemIdentifier =
    @"iTermPreferencePanelSearchFieldToolbarItemIdentifier";
static NSString *const iTermPrefsScrimMouseUpNotification =
    @"iTermPrefsScrimMouseUpNotification";
...
CGFloat iTermPreferencePanelGetWindowMinimumWidth(BOOL session) {
  ...
  if (@available(macOS 10.16, *)) {
    ...
    // Use 760 if you are OK with the search field hiding tabs when focused (I
    // don't like it because it stays hidden after the search field loses focus
    // if there's a query).
    return 785;
  }
  return 560;
...
- (void)click:(NSGestureRecognizer *)gestureRecognizer {
  if (gestureRecognizer.state == NSGestureRecognizerStateRecognized) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermPrefsScrimMouseUpNotification
                      object:nil];
  }
}
...
- (NSDictionary *)dictForFrame:(NSRect)frame onScreen:(NSScreen *)screen {
  ...
  return @{
    @"topLeft" : NSStringFromPoint(topLeft),
...
// Animated window changes call this a lot of times fast. Dumber than iOS, but
...
@interface PreferencePanel () <
    iTermPrefsPanelDelegate,
    iTermPreferencesSearchEngineResultsWindowControllerDelegate,
    NSSearchFieldDelegate, NSTabViewDelegate, iTermPreferencePanelSizing>
...
@implementation PreferencePanel {
  ...
  IBOutlet AppearancePreferencesViewController
  ...
  NSDictionary<NSString *, id<iTermSearchableViewController>>
      *_keywordToViewController;
...
+ (instancetype)sharedInstance {
  if (!gSharedPreferencePanel) {
    gSharedPreferencePanel = [[PreferencePanel alloc]
          initWithProfileModel:[ProfileModel sharedInstance]
...
+ (instancetype)sessionsInstance {
  if (!gSessionsPreferencePanel) {
    gSessionsPreferencePanel = [[PreferencePanel alloc]
          initWithProfileModel:[ProfileModel sessionsInstance]
...
- (void)awakeFromNib {
  ITAssertWithMessage(self.isWindowLoaded, @"window not loaded in %@",
  ...
  if (@available(macOS 10.16, *)) {
    ...
    _globalToolbarItem.image = [NSImage it_imageForSymbolName:@"gearshape"
                                     accessibilityDescription:@"General"];
    _appearanceToolbarItem.image =
        [NSImage it_imageForSymbolName:@"eye"
              accessibilityDescription:@"Appearance"];
    _keyboardToolbarItem.image = [NSImage it_imageForSymbolName:@"keyboard"
                                       accessibilityDescription:@"Keys"];
    _arrangementsToolbarItem.image =
        [NSImage it_imageForSymbolName:@"macwindow.on.rectangle"
              accessibilityDescription:@"Arrangements"];
    _bookmarksToolbarItem.image = [NSImage it_imageForSymbolName:@"person"
                                        accessibilityDescription:@"Profiles"];
    _mouseToolbarItem.image =
        [NSImage it_imageForSymbolName:@"cursorarrow.motionlines"
              accessibilityDescription:@"Pointer"];
    _advancedToolbarItem.image = [NSImage it_imageForSymbolName:@"gearshape.2"
                                       accessibilityDescription:@"Advanced"];
    _shortcutsToolbarItem.image = [NSImage it_imageForSymbolName:@"bolt.circle"
                                        accessibilityDescription:@"Shortcuts"];
  }
#endif
...
- (void)openToPreferenceWithKey:(NSString *)key {
  ...
  iTermPreferencesSearchDocument *document =
      [gSearchEngine documentWithKey:key];
  if (!document) {
    return;
  }
  [self run];
  // Let it become first responder. Then show the scrim. Becoming first
  // responder hides the scrim.
  [self performSelector:@selector(revalDocument:)
             withObject:document
             afterDelay:0];
}
...
- (void)revalDocument:(iTermPreferencesSearchDocument *)document {
  ...
  [self
      fadeOutScrimAfterDelay:[self
                                 delayToWaitForTabToSwitch:switchingTabs
...
- (NSWindow *)window {
  ...
  if (shouldPostWindowLoadNotification) {
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kPreferencePanelDidLoadNotification
                      object:self];
  }
  return window;
}
...
- (void)windowWillLoad {
  ...
  // We finally set our autosave window frame name and restore the one from the
  // user's defaults.
  [self setShouldCascadeWindows:NO];
}

- (void)windowDidLoad {
  // We shouldn't use setFrameAutosaveName: because this window controller
  // controls two windows with different frames (besides, I tried it and it
  // doesn't work here for some reason).
  if (![(iTermPrefsPanel *)self.window
          haveSavedFrameForFrameWithName:self.nameForFrame]) {
    [self.window center];
  } else {
...
- (void)windowDidBecomeKey:(NSNotification *)aNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kNonTerminalWindowBecameKeyNotification
                    object:nil
                  userInfo:nil];
}
...
- (void)postWillCloseNotification {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kPreferencePanelWillCloseNotification
                    object:self];
}

- (void)updateSERPOrigin {
  NSPoint point = [self.window
      convertPointToScreen:[self.searchField
                               convertPoint:NSMakePoint(
                                                0, NSHeight(
                                                       self.searchField.bounds))
                                     toView:nil]];
  point.y -= 1;
...
- (NSSearchToolbarItem *)bigSurSearchFieldToolbarItem NS_AVAILABLE_MAC(10_16) {
  if (!_bigSurSearchFieldToolbarItem) {
    _bigSurSearchFieldToolbarItem = [[NSSearchToolbarItem alloc]
...
- (NSArray *)orderedToolbarIdentifiers {
  ...
  NSArray *result = @[
    ...
    [_mouseToolbarItem itemIdentifier], [_shortcutsToolbarItem itemIdentifier],
...
#endif
  _searchFieldToolbarItem = [[NSToolbarItem alloc]
      initWithItemIdentifier:
          iTermPreferencePanelSearchFieldToolbarItemIdentifier];
...
// This is used by iTermHotKeyController to not activate the hotkey while the
// field for typing the hotkey into is the first responder.
...
- (iTermPreferencesBaseViewController *)viewControllerForTabViewItem:
...
- (void)resizeWindowForTabViewItem:(NSTabViewItem *)tabViewItem
  ...
  iTermPreferencesBaseViewController *viewController =
      [self viewControllerForTabViewItem:tabViewItem];
  if (viewController.tabView != nil) {
    [viewController resizeWindowForCurrentTabAnimated:animated];
    return;
  }

  iTermSizeRememberingView *theView =
      (iTermSizeRememberingView *)tabViewItem.view;
...
- (NSArray<id<iTermSearchableViewController>> *)searchableViewControllers {
  return @[
    _generalPreferencesViewController, _appearancePreferencesViewController,
    _keysViewController, _profilesViewController, _pointerViewController,
    _advancedViewController, _shortcutsViewController
  ];
}
...
- (void)buildSearchEngineIfNeeded {
  ...
  for (id<iTermSearchableViewController> viewController in self
           .searchableViewControllers) {
    for (iTermPreferencesSearchDocument *doc in
         [viewController searchableViewControllerDocuments]) {
...
- (void)showScrimAndSERP {
  ...
  [_serpWindowController close];
  _serpWindowController =
      [[iTermPreferencesSearchEngineResultsWindowController alloc]
          initWithWindowNibName:
              @"iTermPreferencesSearchEngineResultsWindowController"];
...
- (void)selectTabForViewController:
    (id<iTermSearchableViewController>)viewController {
  NSTabViewItem *tabViewItem =
      [self tabViewItemForViewController:viewController];
  if (tabViewItem) {
    [self selectTabViewItem:tabViewItem];
    return;
  }
  if ([viewController
          isKindOfClass:[iTermProfilePreferencesBaseViewController class]]) {
    [self selectTabViewItem:_bookmarksTabViewItem];
    if (_profilesViewController.selectedProfile == nil) {
      [_profilesViewController
          openToProfileWithGuid:[[ProfileModel sharedInstance]
                                    defaultBookmark][KEY_GUID]
               selectGeneralTab:NO
                          scope:nil];
    }
  }
...
- (id<iTermSearchableViewController>)viewControllerForDocumentOwnerIdentifier:
    (NSString *)ownerIdentifier {
  for (id<iTermSearchableViewController> vc in
       [self searchableViewControllers]) {
    if ([vc.documentOwnerIdentifier isEqualToString:ownerIdentifier]) {
      return vc;
    }
  }
  return [_profilesViewController
      viewControllerWithOwnerIdentifier:ownerIdentifier];
}
...
- (BOOL)revealDocument:(iTermPreferencesSearchDocument *)document
  ...
  id<iTermSearchableViewController> viewController =
      [self viewControllerForDocumentOwnerIdentifier:document.ownerIdentifier];
  if (!viewController) {
    return NO;
  }
  NSTabViewItem *tabViewItemBefore = _tabView.selectedTabViewItem;
  [self selectTabForViewController:viewController];
  const BOOL waitForTabToSwitch =
      (tabViewItemBefore != _tabView.selectedTabViewItem);
  BOOL waitForInnerTabToSwitch = NO;
  _scrim.cutoutView = [viewController
      searchableViewControllerRevealItemForDocument:document
...
- (void)preferencesSearchEngineResultsDidSelectDocument:
    (iTermPreferencesSearchDocument *)document {
...
- (NSTabViewItem *)innerTabViewItem {
  return [self viewControllerForTabViewItem:_tabView.selectedTabViewItem]
...
- (void)preferencesSearchEngineResultsDidActivateDocument:
    (iTermPreferencesSearchDocument *)document {
