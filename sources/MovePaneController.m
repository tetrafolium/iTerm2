...
NSString *const iTermSessionDidChangeTabNotification =
    @"iTermSessionDidChangeTabNotification";
...
- (void)moveWindowBy:(NSPoint)point {
  ...
  origin.x += point.x;
  origin.y += point.y;
  DLog(@"Move window from %@ to %@", NSStringFromPoint(window.frame.origin),
...
- (void)moveSessionToNewWindow:(PTYSession *)movingSession
  ...
  PTYTab *theTab =
      [movingSession.delegate.realParentWindow tabForSession:movingSession];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:iTermSessionDidChangeTabNotification
                    object:movingSession];
  [movingSession didMoveSession];
}

+ (void)moveTab:(PTYTab *)tab
       toWindow:(PseudoTerminal *)term
  ...
  [sourceTerm.tabBarControl moveTabAtIndex:sourceIndex
                                  toTabBar:term.tabBarControl
                                   atIndex:index];
}
...
- (BOOL)dropTab:(PTYTab *)tab
      inSession:(PTYSession *)dest
        ...
        atPoint:(NSPoint)point {
  _dropping = YES;
  const BOOL result = [self reallyDropTab:tab
                                inSession:dest
                                     half:half
                                  atPoint:point];
  _dropping = NO;
...
- (BOOL)reallyDropTab:(PTYTab *)tab
  ...
  if ([[tab sessions] count] != 1) {
    // This sometimes can't be done at all, and it usually can't be done right
    // if tabs' sizes differ, etc. Just wimp out, basically.
    return NO;
  }
...
// If only one call is made, then dest will be nil and the session will be moved
// to a new window. If two calls are made, the both have dest non-null but only
// the first call will perform a split. The second call will do nothing. It
// isn't called at all if a session is dragged into an existing tab bar.
...
- (BOOL)reallyDropInSession:(PTYSession *)dest
                    ...
                    atPoint:(NSPoint)point {
  if ((dest &&
       ![session_ isCompatibleWith:dest]) || // Would create hetero-tmuxual
                                             // splits in tab
  ...
  if ([self.session isTmuxClient]) {
    ...
    // here.
    if (isMove_) {
      [[self.session tmuxController]
            movePane:[self.session tmuxPane]
  ...
  if (isMove_) {
    ...
    if (![[destinationTab realParentWindow]
            canSplitPaneVertically:isVertical
    ...
    [[destinationTab realParentWindow]
        splitVertically:isVertical
                 before:(half == kNorthHalf || half == kWestHalf)
...
- (void)beginDrag:(PTYSession *)session {
  ...
  NSRect rect = [[[session view] superview] convertRect:[[session view] frame]
                                                 toView:nil];
