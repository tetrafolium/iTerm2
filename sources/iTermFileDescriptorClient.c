...
static ssize_t ReceiveMessageAndFileDescriptor(int fd, void *buffer,
                                               size_t bufferCapacity,
                                               int *receivedFileDescriptorPtr,
                                               int deadMansPipeReadEnd) {
  // Loop because sometimes the dynamic loader spews warnings (for example, when
  // malloc logging is enabled)
  while (1) {
    ...
    do {
      ...
      if (deadMansPipeReadEnd >= 0) {
        ...
        if (readable[1]) {
          FDLog(LOG_DEBUG, "Server was dead before recvmsg. Did the shell "
...
iTermFileDescriptorServerConnection iTermFileDescriptorClientRun(pid_t pid) {
  ...
  iTermFileDescriptorServerConnection result =
      iTermFileDescriptorClientRead(socketFd, -1);
...
iTermFileDescriptorClientRead(int socketFd, int deadMansPipeReadEnd) {
  iTermFileDescriptorServerConnection result = {0};
  int rc = ReceiveMessageAndFileDescriptor(
      socketFd, &result.childPid, sizeof(result.childPid), &result.ptyMasterFd,
      deadMansPipeReadEnd);
