...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p cursor=%@ range=%@>",
...
@implementation iTermMetalDriver {
  ...
  // be nonnil and holds the state needed by those calls. Will bet set to nil if
  // the frame will be drawn by reallyDrawInMTKView:.
  iTermMetalDriverAsyncContext *_context;
#if ENABLE_UNFAMILIAR_TEXTURE_WORKAROUND
  // Used to work around a bug where presentDrawable: sometimes doesn't work. It
  // only seems to happen with a never-before-seen texture. Holds weak refs to
  // drawables' textures.
...
- (nullable instancetype)initWithDevice:(nonnull id<MTLDevice>)device {
  ...
  if (self) {
    ...
    _backgroundImageRenderer =
        [[iTermBackgroundImageRenderer alloc] initWithDevice:device];
    _textRenderer = [[iTermTextRenderer alloc] initWithDevice:device];
    _backgroundColorRenderer =
        [[iTermBackgroundColorRenderer alloc] initWithDevice:device];
    _markRenderer = [[iTermMarkRenderer alloc] initWithDevice:device];
    _badgeRenderer = [[iTermBadgeRenderer alloc] initWithDevice:device];
    _flashRenderer =
        [[iTermFullScreenFlashRenderer alloc] initWithDevice:device];
    _timestampsRenderer =
        [[iTermTimestampsRenderer alloc] initWithDevice:device];
    _indicatorRenderer = [[iTermIndicatorRenderer alloc] initWithDevice:device];
    _broadcastStripesRenderer =
        [[iTermBroadcastStripesRenderer alloc] initWithDevice:device];
    _cursorGuideRenderer =
        [[iTermCursorGuideRenderer alloc] initWithDevice:device];
    _highlightRowRenderer =
        [[iTermHighlightRowRenderer alloc] initWithDevice:device];
    _imageRenderer = [[iTermImageRenderer alloc] initWithDevice:device];
    _underlineCursorRenderer =
        [iTermCursorRenderer newUnderlineCursorRendererWithDevice:device];
    _barCursorRenderer =
        [iTermCursorRenderer newBarCursorRendererWithDevice:device];
    _imeCursorRenderer =
        [iTermCursorRenderer newIMECursorRendererWithDevice:device];
    _blockCursorRenderer =
        [iTermCursorRenderer newBlockCursorRendererWithDevice:device];
    _frameCursorRenderer =
        [iTermCursorRenderer newFrameCursorRendererWithDevice:device];
    _copyModeCursorRenderer =
        [iTermCursorRenderer newCopyModeCursorRendererWithDevice:device];
    _keyCursorRenderer =
        [iTermCursorRenderer newKeyCursorRendererWithDevice:device];
    _copyBackgroundRenderer =
        [[iTermCopyBackgroundRenderer alloc] initWithDevice:device];
    if (iTermTextIsMonochrome()) {
    } else {
      _copyToDrawableRenderer =
          [[iTermCopyToDrawableRenderer alloc] initWithDevice:device];
    }
    ...
    _statHistograms = [[NSArray
        sequenceWithRange:NSMakeRange(0, iTermMetalFrameDataStatCount)]
...
- (void)dealloc {
  ...
  // This reference to _mainThreadState is *possibly* unsafe! dealloc gets
  // called off the main queue. But there should not be any remaining references
  // to mainThreadState once you get to dealloc.
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
  ...
  dispatch_async(_queue, ^{
    ...
    [self->_statHistograms
        enumerateObjectsUsingBlock:^(iTermHistogram *_Nonnull obj,
...
- (void)setCellSize:(CGSize)cellSize
  ...
  DLog(@"Cell size is now %@x%@, grid size is now %@x%@", @(cellSize.width),
       @(cellSize.height), @(gridSize.width), @(gridSize.height));
...
- (void)drawInMTKView:(nonnull MTKView *)view {
  ...
  iTermPreciseTimerSaveLog(
      [NSString stringWithFormat:@"%@: Dropped frames", _identifier],
      [NSString stringWithFormat:@"%0.1f%%\n",
                                 100.0 * ((double)_dropped / (double)_total)]);
  if (_total % 10 == 1) {
    iTermPreciseTimerSaveLog(
        [NSString
            stringWithFormat:@"%@: Start-to-Start Time (ms)", _identifier],
        [_startToStartHistogram stringValue]);
    iTermPreciseTimerSaveLog(
        [NSString
            stringWithFormat:@"%@: Frames In Flight at Start", _identifier],
...
- (int)maximumNumberOfFramesInFlight {
  ...
  if (_currentDrawableTime.numberOfMeasurements > 5 && _maxFramesInFlight > 1 &&
    ...
    DLog(@"Moving average of currentDrawable latency of %0.2fms with %@ "
         @"measurements with mff of %@ is too high. Decrease mff to %@",
         _currentDrawableTime.value * 1000,
         @(_currentDrawableTime.numberOfMeasurements), @(_maxFramesInFlight),
         @(_maxFramesInFlight - 1));
    _maxFramesInFlight -= 1;
    [_currentDrawableTime reset];
  } else if (_currentDrawableTime.numberOfMeasurements > 10 &&
             _maxFramesInFlight <
                 iTermMetalDriverMaximumNumberOfFramesInFlight &&
             _currentDrawableTime.value < fastEnoughToUpgradeThreshold) {
    DLog(@"Moving average of currentDrawable latency of %0.2fms with %@ "
         @"measurements with mff of %@ is low. Increase mff to %@",
         _currentDrawableTime.value * 1000,
         @(_currentDrawableTime.numberOfMeasurements), @(_maxFramesInFlight),
         @(_maxFramesInFlight + 1));
    _maxFramesInFlight += 1;
...
- (iTermMetalDriverAsyncContext *)newContextForDrawInView:(MTKView *)view
                                                    count:(int)count {
  iTermMetalDriverAsyncContext *context =
      [[iTermMetalDriverAsyncContext alloc] init];
  _context = context;
  context.count = count;
...
- (void)drawAsynchronouslyInView:(MTKView *)view
...
- (BOOL)reallyDrawInMTKView:(nonnull MTKView *)view
...
#warning DO NOT SUBMIT - FLAKY MODE ENABLED
  ...
  @synchronized(self) {
    ...
    const BOOL shouldDrop =
        (framesInFlight >= [self maximumNumberOfFramesInFlight]);
    if (shouldDrop) {
      DLog(@"  abort: busy (dropped %@%%, number in flight: %d)",
           @((_dropped * 100) / _total), (int)framesInFlight);
      DLog(@"  current frames:\n%@", _currentFrames);
      _dropped++;
      self.needsDraw = YES;
      return NO;
  ...
  if (frameData.group) {
    DLog(@"Frame %@ has a group. The context's count is %d", frameData,
         _context.count);
  }
  _context = nil;
...
- (iTermMetalFrameData *)newFrameDataForView:(MTKView *)view {
  ...
  iTermMetalFrameData *frameData =
      [[iTermMetalFrameData alloc] initWithView:view
  ...
  [frameData
      measureTimeForStat:iTermMetalFrameDataStatMtExtractFromApp
                 ofBlock:^{
                   ...
                   frameData.perFrameState =
                       [self->_dataSource metalDriverWillBeginDrawingFrame];
                   ...
                   frameData.cellSize =
                       rescale(frameData.perFrameState.cellSize);
                   frameData.cellSizeWithoutSpacing =
                       rescale(frameData.perFrameState.cellSizeWithoutSpacing);
                   frameData.glyphSize = self.mainThreadState->glyphSize;

                   frameData.scale = scale;
                   frameData.hasBackgroundImage =
                       frameData.perFrameState.hasBackgroundImage;
                   const NSEdgeInsets pointInsets =
                       frameData.perFrameState.extraMargins;
                   frameData.extraMargins = NSEdgeInsetsMake(
                       pointInsets.top * scale, pointInsets.left * scale,
...
- (void)performPrivateQueueSetupForFrameData:(iTermMetalFrameData *)frameData
  ...
  if ([iTermAdvancedSettingsModel showMetalFPSmeter]) {
    [frameData.perFrameState
        setDebugString:[self
                           fpsMeterStringForFrameNumber:frameData.frameNumber]];
  }
  ...
  // If we're rendering to an intermediate texture because there's something
  // complicated behind text and we need to use the fancy subpixel antialiasing
  // algorithm, create it now. This has to be done before updates so the
  // copyBackgroundRenderer's `enabled` flag can be set properly.
  if (!iTermTextIsMonochrome()) {
    [frameData createIntermediateRenderPassDescriptor];
    [frameData createTemporaryRenderPassDescriptor];
  }

  // Set properties of the renderers for values that tend not to change very
  // often and which are used to create transient states. This must happen
  // before creating transient states since renderers use this info to decide if
  // they should return a nil transient state.
  ...
  [frameData measureTimeForStat:iTermMetalFrameDataStatPqCreateTransientStates
                        ofBlock:^{
                          ...
                          [self
                              createTransientStatesWithFrameData:frameData
                                                            view:view
  ...
  [frameData
      measureTimeForStat:iTermMetalFrameDataStatPqPopulateTransientStates
                 ofBlock:^{
...
- (void)addRowDataToFrameData:(iTermMetalFrameData *)frameData {
  for (int y = 0; y < frameData.gridSize.height; y++) {
    ...
    rowData.y = y;
    rowData.keysData =
        [iTermGlyphKeyData dataOfLength:sizeof(iTermMetalGlyphKey) * columns];
    rowData.attributesData = [iTermAttributesData
        dataOfLength:sizeof(iTermMetalGlyphAttributes) * columns];
    rowData.backgroundColorRLEData = [iTermBackgroundColorRLEsData
        dataOfLength:sizeof(iTermMetalBackgroundColorRLE) * columns];
    rowData.lineData = [frameData.perFrameState lineForRow:y];
    iTermMetalGlyphKey *glyphKeys =
        (iTermMetalGlyphKey *)rowData.keysData.mutableBytes;
    int drawableGlyphs = 0;
    int rles = 0;
    iTermMarkStyle markStyle;
    NSDate *date;
    [frameData.perFrameState
        metalGetGlyphKeys:glyphKeys
           ...
           drawableGlyphs:&drawableGlyphs
                     date:&date];
    rowData.backgroundColorRLEData.length =
        rles * sizeof(iTermMetalBackgroundColorRLE);
    rowData.date = date;
    rowData.numberOfBackgroundRLEs = rles;
    rowData.numberOfDrawableGlyphs = drawableGlyphs;
    ITConservativeBetaAssert(
        drawableGlyphs <= rowData.keysData.length / sizeof(iTermMetalGlyphKey),
...
- (BOOL)shouldCreateIntermediateRenderPassDescriptor:
    (iTermMetalFrameData *)frameData {
...
- (void)updateRenderersForNewFrameData:(iTermMetalFrameData *)frameData {
  ...
  [self.cellRenderers
      enumerateObjectsUsingBlock:^(id<iTermMetalCellRenderer> _Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)createTransientStatesWithFrameData:(iTermMetalFrameData *)frameData
  ...
  for (id<iTermMetalRenderer> renderer in self.nonCellRenderers) {
    ...
    [frameData measureTimeForStat:renderer.createTransientStateStat
                          ofBlock:^{
                            __kindof iTermMetalRendererTransientState
                                *_Nonnull tState = [renderer
                                    createTransientStateForConfiguration:
                                        cellConfiguration
                                                           commandBuffer:
                                                               commandBuffer];
                            if (tState) {
                              [frameData setTransientState:tState
  ...
  for (id<iTermMetalCellRenderer> renderer in self.cellRenderers) {
    ...
    [frameData
        measureTimeForStat:renderer.createTransientStateStat
                   ofBlock:^{
...
- (void)populateTransientStatesWithFrameData:(iTermMetalFrameData *)frameData
  ...
  [self
      populateTextAndBackgroundRenderersTransientStateWithFrameData:frameData];
...
- (id<MTLTexture>)destinationTextureForFrameData:
    (iTermMetalFrameData *)frameData {
  if (frameData.debugInfo) {
    // Render to offscreen first
    MTLTextureDescriptor *textureDescriptor = [MTLTextureDescriptor
        texture2DDescriptorWithPixelFormat:MTLPixelFormatBGRA8Unorm
                                     width:frameData.destinationDrawable.texture
                                               .width
                                    height:frameData.destinationDrawable.texture
                                               .height
                                 mipmapped:NO];
    id<MTLTexture> texture =
        [frameData.device newTextureWithDescriptor:textureDescriptor];
    texture.label = @"Offscreen destination";
    [iTermTexture
         setBytesPerRow:frameData.destinationDrawable.texture.width * 4
...
// Main thread
- (void)acquireScarceResources:(iTermMetalFrameData *)frameData
                          view:(MTKView *)view {
  if (frameData.debugInfo) {
    [frameData
        measureTimeForStat:iTermMetalFrameDataStatMtGetRenderPassDescriptor
                   ofBlock:^{
                     frameData.renderPassDescriptor =
                         [frameData newRenderPassDescriptorWithLabel:
                                        @"Offscreen debug texture"
                                                                fast:NO];
        ...
        measureTimeForStat:iTermMetalFrameDataStatMtGetCurrentDrawable
                   ofBlock:^{
                     ...
                     frameData.destinationTexture =
                         frameData.renderPassDescriptor.colorAttachments[0]
                             .texture;
                   }];
    ...
    frameData.deferCurrentDrawable =
        ([iTermPreferences boolForKey:kPreferenceKeyMetalMaximizeThroughput] &&
         !synchronousDraw);
#else
      ...
      NSTimeInterval duration = [frameData
          measureTimeForStat:iTermMetalFrameDataStatMtGetCurrentDrawable
                     ofBlock:^{
                       frameData.destinationDrawable = view.currentDrawable;
                       frameData.destinationTexture =
                           [self destinationTextureForFrameData:frameData];
#if ENABLE_UNFAMILIAR_TEXTURE_WORKAROUND
                       frameData.textureIsFamiliar =
                           [self textureIsFamiliar:frameData.destinationDrawable
                                                       .texture];
                       if (!frameData.textureIsFamiliar) {
                         [_familiarTextures
                             addPointer:(__bridge void *)frameData
                                            .destinationDrawable.texture];
                       }
...
- (void)enqueueDrawCallsForFrameData:(iTermMetalFrameData *)frameData
  ...
  // If we're using an intermediate render pass, copy from it to the view for
  // final steps.
  if (frameData.intermediateRenderPassDescriptor) {
...
- (void)updateTextRendererForFrameData:(iTermMetalFrameData *)frameData {
  ...
  [_textRenderer
      ...
      creationIdentifier:
          [frameData.perFrameState
              metalASCIICreationIdentifierWithOffset:frameData.asciiOffset]
                creation:^NSDictionary<NSNumber *, iTermCharacterBitmap *> *(
                    char c, iTermASCIITextureAttributes attributes) {
                  __typeof(self) strongSelf = weakSelf;
                  iTermMetalFrameData *strongFrameData = weakFrameData;
                  if (strongSelf && strongFrameData) {
                    return
                        [strongSelf dictionaryForASCIICharacter:c
...
- (NSDictionary<NSNumber *, iTermCharacterBitmap *> *)
    dictionaryForASCIICharacter:(char)c
                 withAttributes:(iTermASCIITextureAttributes)attributes
                      frameData:(iTermMetalFrameData *)frameData
                      glyphSize:(CGSize)glyphSize
                          scale:(CGFloat)scale {
  static const int typefaceMask =
      ((1 << iTermMetalGlyphKeyTypefaceNumberOfBitsNeeded) - 1);
  ...
  BOOL emoji = NO;
  // Don't need to pass predecessor or successor because ASCII never has
...
- (void)updateBackgroundImageRendererForFrameData:
    (iTermMetalFrameData *)frameData {
  ...
  NSImage *backgroundImage =
      [frameData.perFrameState metalBackgroundImageGetMode:&mode];
  [_backgroundImageRenderer
...
- (void)updateCopyBackgroundRendererForFrameData:
    (iTermMetalFrameData *)frameData {
  if (_copyBackgroundRenderer.rendererDisabled) {
    return;
  }
  _copyBackgroundRenderer.enabled =
...
- (void)updateIndicatorRendererForFrameData:(iTermMetalFrameData *)frameData {
  ...
  NSRect frame = NSMakeRect(0, 0, frameData.viewportSize.x / scale,
                            frameData.viewportSize.y / scale);
  [_indicatorRenderer reset];
  [frameData.perFrameState
      enumerateIndicatorsInFrame:frame
                           block:^(
                               iTermIndicatorDescriptor *_Nonnull indicator) {
...
- (void)updateBroadcastStripesRendererForFrameData:
    (iTermMetalFrameData *)frameData {
  if (_broadcastStripesRenderer.rendererDisabled) {
    return;
  }
  _broadcastStripesRenderer.enabled =
      frameData.perFrameState.showBroadcastStripes;
}
...
- (void)populateCopyBackgroundRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  if (_copyBackgroundRenderer.rendererDisabled) {
    return;
  }
  ...
  copyState.sourceTexture =
      frameData.intermediateRenderPassDescriptor.colorAttachments[0].texture;

  iTermCopyToDrawableRendererTransientState *dState =
      [frameData transientStateForRenderer:_copyToDrawableRenderer];
  dState.sourceTexture =
      frameData.temporaryRenderPassDescriptor.colorAttachments[0].texture;
}

- (void)populateCursorRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  ...
  iTermMetalCursorInfo *cursorInfo =
      [frameData.perFrameState metalDriverCursorInfo];
  if (!cursorInfo.frameOnly && cursorInfo.cursorVisible &&
      cursorInfo.shouldDrawText && cursorInfo.coord.y >= 0 &&
    ...
    glyphAttributes[cursorInfo.coord.x].backgroundColor =
        simd_make_float4(cursorInfo.cursorColor.redComponent,
  ...
  if (cursorInfo.cursorVisible && cursorInfo.password) {
    iTermCursorRendererTransientState *tState =
        [frameData transientStateForRenderer:_keyCursorRenderer];
    ...
    case CURSOR_BOX: {
      iTermCursorRendererTransientState *tState =
          [frameData transientStateForRenderer:_blockCursorRenderer];
      tState.coord = cursorInfo.coord;
      tState.color = cursorInfo.cursorColor;
      tState.doubleWidth = cursorInfo.doubleWidth;
    ...
    case CURSOR_VERTICAL: {
      iTermCursorRendererTransientState *tState =
          [frameData transientStateForRenderer:_barCursorRenderer];
      tState.coord = cursorInfo.coord;
      tState.color = cursorInfo.cursorColor;
      break;
    }
  ...
  if (imeInfo) {
    iTermCursorRendererTransientState *tState =
        [frameData transientStateForRenderer:_imeCursorRenderer];
...
- (void)populateBadgeRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  if (_badgeRenderer.rendererDisabled) {
    return;
  }
  iTermBadgeRendererTransientState *tState =
      [frameData transientStateForRenderer:_badgeRenderer];
...
- (void)populateTextAndBackgroundRenderersTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  if (_textRenderer.rendererDisabled &&
  ...
  iTermTextRendererTransientState *textState =
      [frameData transientStateForRenderer:_textRenderer];

  // Set the background texture if one is available.
  textState.backgroundTexture =
      frameData.intermediateRenderPassDescriptor.colorAttachments[0].texture;
  ...
  [frameData.perFrameState
      metalGetUnderlineDescriptorsForASCII:&asciiUnderlineDescriptor
  ...
  textState.defaultBackgroundColor =
      frameData.perFrameState.defaultBackgroundColor;
  ...
  backgroundState.defaultBackgroundColor =
      frameData.perFrameState.processedDefaultBackgroundColor;

  iTermMetalIMEInfo *imeInfo = frameData.perFrameState.imeInfo;

  [frameData.rows enumerateObjectsUsingBlock:^(
                      iTermMetalRowData *_Nonnull rowData, NSUInteger idx,
                      BOOL *_Nonnull stop) {
    NSRange markedRangeOnLine = NSMakeRange(NSNotFound, 0);
    if (imeInfo && rowData.y >= imeInfo.markedRange.start.y &&
      ...
      if (rowData.y == imeInfo.markedRange.start.y) {
        ...
        if (rowData.y == imeInfo.markedRange.end.y) {
          ...
          markedRangeOnLine = NSMakeRange(imeInfo.markedRange.start.x,
                                          imeInfo.markedRange.end.x -
                                              imeInfo.markedRange.start.x);
        } else {
          // Marked line begins on this line and ends later
          markedRangeOnLine = NSMakeRange(imeInfo.markedRange.start.x,
                                          frameData.gridSize.width -
                                              imeInfo.markedRange.start.x);
        }
      } else {
      ...
      [textState setGlyphKeysData:rowData.keysData
           ...
           backgroundColorRLEData:rowData.backgroundColorRLEData
                markedRangeOnLine:markedRangeOnLine
                         ...
                         creation:^NSDictionary<
                             NSNumber *,
                             iTermCharacterBitmap *> *_Nonnull(int x,
                                                               BOOL *emoji) {
                           return [frameData.perFrameState
                               metalImagesForGlyphKey:&glyphKeys[x]
                                          asciiOffset:frameData.asciiOffset
                                                 size:glyphSize
                                                scale:scale
                                                emoji:emoji];
  ...
  if (!_backgroundColorRenderer.rendererDisabled) {
    BOOL (^comparator)(iTermMetalRowData *obj1, iTermMetalRowData *obj2) =
        ^BOOL(iTermMetalRowData *obj1, iTermMetalRowData *obj2) {
          ...
          const iTermMetalBackgroundColorRLE *array1 =
              (const iTermMetalBackgroundColorRLE *)
                  obj1.backgroundColorRLEData.mutableBytes;
          const iTermMetalBackgroundColorRLE *array2 =
              (const iTermMetalBackgroundColorRLE *)
                  obj2.backgroundColorRLEData.mutableBytes;
    ...
    [frameData.rows
        enumerateCoalescedObjectsWithComparator:comparator
                                          block:^(iTermMetalRowData *rowData,
                                                  NSUInteger count) {
                                            [backgroundState
                                                 setColorRLEs:
                                                     (const iTermMetalBackgroundColorRLE
...
- (NSString *)fpsMeterStringForFrameNumber:(int)frameNumber {
  ...
  return [NSString
      stringWithFormat:@" [Frame %d: %d fps] ", frameNumber, (int)round(fps)];
}

- (void)populateMarkRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermMarkRendererTransientState *tState =
      [frameData transientStateForRenderer:_markRenderer];
  [frameData.rows
      enumerateObjectsUsingBlock:^(iTermMetalRowData *_Nonnull rowData,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        [tState setMarkStyle:rowData.markStyle row:idx];
      }];
}

- (void)populateHighlightRowRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermHighlightRowRendererTransientState *tState =
      [frameData transientStateForRenderer:_highlightRowRenderer];
  [frameData.perFrameState
      metalEnumerateHighlightedRows:^(vector_float3 color, NSTimeInterval age,
                                      int row) {
...
- (void)populateCursorGuideRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermCursorGuideRendererTransientState *tState =
      [frameData transientStateForRenderer:_cursorGuideRenderer];
  iTermMetalCursorInfo *cursorInfo =
      frameData.perFrameState.metalDriverCursorInfo;
...
- (void)populateTimestampsRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermTimestampsRendererTransientState *tState =
      [frameData transientStateForRenderer:_timestampsRenderer];
  if (frameData.perFrameState.timestampsEnabled) {
    ...
    tState.timestamps =
        [frameData.rows mapWithBlock:^id(iTermMetalRowData *row) {
          return row.date;
        }];
  }
}

- (void)populateFlashRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermFullScreenFlashRendererTransientState *tState =
      [frameData transientStateForRenderer:_flashRenderer];
  tState.color = frameData.perFrameState.fullScreenFlashColor;
}

- (void)populateMarginRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermMarginRendererTransientState *tState =
      [frameData transientStateForRenderer:_marginRenderer];
  vector_float4 color = frameData.perFrameState.processedDefaultBackgroundColor;
  [tState setColor:color];
}

- (void)populateImageRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermImageRendererTransientState *tState =
      [frameData transientStateForRenderer:_imageRenderer];
  tState.firstVisibleAbsoluteLineNumber =
      frameData.perFrameState.firstVisibleAbsoluteLineNumber;
  [frameData.rows
      enumerateObjectsUsingBlock:^(iTermMetalRowData *_Nonnull row,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        [row.imageRuns
            enumerateObjectsUsingBlock:^(iTermMetalImageRun *_Nonnull imageRun,
                                         NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)populateBackgroundImageRendererTransientStateWithFrameData:
    (iTermMetalFrameData *)frameData {
  iTermBackgroundImageRendererTransientState *tState =
      [frameData transientStateForRenderer:_backgroundImageRenderer];
  tState.computedAlpha = iTermAlphaValueForBottomView(
      1 - frameData.perFrameState.transparencyAlpha,
...
- (id<MTLBuffer>)bufferWithContentsOfDestinationTextureInFrameData:
                     (iTermMetalFrameData *)frameData
                                                             label:(NSString *)
                                                                       label {
  // Blit to shared buffer so CPU can see it
  id<MTLBlitCommandEncoder> blitter =
      [frameData.commandBuffer blitCommandEncoder];
  ...
  id<MTLBuffer> buffer =
      [frameData.device newBufferWithLength:length
                                    options:MTLResourceStorageModeShared];
  [blitter copyFromTexture:frameData.destinationTexture
                   sourceSlice:0
                   sourceLevel:0
                  sourceOrigin:MTLOriginMake(0, 0, 0)
                    sourceSize:MTLSizeMake(frameData.destinationTexture.width,
...
- (void)copyToDrawableFromTexture:(id<MTLTexture>)sourceTexture
         withRenderPassDescriptor:
             (MTLRenderPassDescriptor *)renderPassDescriptor
                            label:(NSString *)label
                        frameData:(iTermMetalFrameData *)frameData {
  // Copy from texture to drawable
  [frameData
      updateRenderEncoderWithRenderPassDescriptor:renderPassDescriptor
                                             stat:iTermMetalFrameDataStatNA
                                            label:label];
  if (!_copyOffscreenRenderer) {
    _copyOffscreenRenderer =
        [[iTermCopyOffscreenRenderer alloc] initWithDevice:frameData.device];
    _copyOffscreenRenderer.enabled = YES;
  }

  iTermCopyOffscreenRendererTransientState *tState = [_copyOffscreenRenderer
      createTransientStateForConfiguration:frameData.cellConfiguration
...
// frameData's renderEncoder must have just had -endEncoding called on it at
// this point. It will be left in the same state.
- (void)copyOffscreenTextureToDrawableInFrameData:
    (iTermMetalFrameData *)frameData {
...
- (void)drawRenderer:(id<iTermMetalRenderer>)renderer
  ...
  [frameData measureTimeForStat:stat
                        ofBlock:^{
                          iTermMetalRendererTransientState *state =
                              [frameData transientStateForRenderer:renderer];
                          state.sequenceNumber =
                              frameData.numberOfRenderersDrawn;
                          frameData.numberOfRenderersDrawn =
                              frameData.numberOfRenderersDrawn + 1;

                          // NOTE: State may be nil if we determined it should
                          // be skipped early on.
                          if (state != nil && !state.skipRenderer) {
                            [renderer drawWithFrameData:frameData
                                         transientState:state];
                          }
                        }];

  const NSUInteger numberOfDraws =
      frameData.debugInfo.numberOfRecordedDraws - before;
...
- (void)saveRenderOutputForDebuggingIfNeeded:(iTermMetalFrameData *)frameData
  ...
  if (frameData.debugInfo && frameData.debugRealRenderPassDescriptor) {
    ...
    id<MTLBuffer> pixelBuffer = [self
        bufferWithContentsOfDestinationTextureInFrameData:frameData
                                                    label:NSStringFromClass(
                                                              [state class])];
    iTermMetalDebugInfo *debugInfo = frameData.debugInfo;
    CGSize size =
        CGSizeMake(frameData.viewportSize.x, frameData.viewportSize.y);
    [frameData.commandBuffer
        addCompletedHandler:^(id<MTLCommandBuffer> _Nonnull commandBuffer) {
          void (^block)(void) = ^{
            NSMutableData *data =
                [NSMutableData dataWithBytes:pixelBuffer.contents
...
- (void)updateRenderEncoderForCurrentPass:(iTermMetalFrameData *)frameData
  ...
  if (useTemporaryTexture) {
    ...
    descriptors = @[
      frameData.intermediateRenderPassDescriptor
  ...
  [frameData updateRenderEncoderWithRenderPassDescriptor:descriptors[pass]
                                                   ...
                                                   label:label];
  frameData.destinationTexture =
      [descriptors[pass].colorAttachments[0] texture];
}
...
- (void)drawCursorBeforeTextWithFrameData:(iTermMetalFrameData *)frameData {
  ...
  if (!cursorInfo.copyMode && !cursorInfo.password &&
...
- (BOOL)deferredRequestCurrentDrawableWithCommandBuffer:
            (id<MTLCommandBuffer>)commandBuffer
                                              frameData:(iTermMetalFrameData *)
                                                            frameData {
  __block BOOL shouldCopyToDrawable = NO;
  [frameData
      measureTimeForStat:iTermMetalFrameDataStatPqBlockOnSynchronousGetDrawable
                 ofBlock:^{
                   ...
                   if (!self.waitingOnSynchronousDraw) {
                         ...
                         [frameData
                             measureTimeForStat:
                                 iTermMetalFrameDataStatMtGetCurrentDrawable
                                        ofBlock:^{
                                          drawable =
                                              frameData.view.currentDrawable;
                                          DLog(@"%p DEFERRED PATH: got "
                                               @"drawable %@ for frame %@",
                                               self, drawable,
                     ...
                     // TODO: This could avoid false positives by polling every
                     // millisecond to see if waitingOnSynchronousDraw has
                     // become true.
                     ...
                     @synchronized(self) {
                       ...
                       frameData.destinationTexture.label =
...
#if ENABLE_DEFER_CURRENT_DRAWABLE
  if (frameData.deferCurrentDrawable) {
    if ([self deferredRequestCurrentDrawableWithCommandBuffer:commandBuffer
                                                    frameData:frameData]) {
      shouldCopyToDrawable = YES;
    }
    if (frameData.destinationTexture == nil) {
      ...
      measureTimeForStat:iTermMetalFrameDataStatPqEnqueueDrawPresentAndCommit
                 ofBlock:^{
                   ...
                   if (frameData.destinationDrawable) {
                     ...
                     [commandBuffer
                         presentDrawable:frameData.destinationDrawable];
                   }
#endif
                   ...
                   iTermPreciseTimerStatsStartTimer(
                       &frameData
                            .stats[iTermMetalFrameDataStatGpuScheduleWait]);
                   ...
                   void (^scheduledBlock)(void) = [^{
                     ...
                     [frameData
                         class]; // force a reference to frameData to be kept
                   } copy];
                   [commandBuffer
                       addScheduledHandler:^(
                   ...
                   void (^completedBlock)(void) = [^{
                     completed = [self didComplete:completed
                                     withFrameData:frameData];
                   } copy];

                   [commandBuffer addCompletedHandler:^(
                                      id<MTLCommandBuffer> _Nonnull buffer) {
#if ENABLE_PRIVATE_QUEUE
                     [frameData dispatchToQueue:self->_queue
                                  forCompletion:completedBlock];
#else
...
- (BOOL)didComplete:(BOOL)completed
  ...
  if (!completed) {
    ...
    [weakSelf dispatchAsyncToMainQueue:^{
      if (!self->_imageRenderer.rendererDisabled) {
        iTermImageRendererTransientState *tState =
            [frameData transientStateForRenderer:self->_imageRenderer];
        [weakSelf.dataSource
            metalDidFindImages:tState.foundImageUniqueIdentifiers
...
- (NSArray<id<iTermMetalCellRenderer>> *)cellRenderers {
  return @[
    _marginRenderer, _textRenderer, _backgroundColorRenderer, _markRenderer,
    _cursorGuideRenderer, _highlightRowRenderer, _imageRenderer,
    _underlineCursorRenderer, _barCursorRenderer, _imeCursorRenderer,
    _blockCursorRenderer, _frameCursorRenderer, _copyModeCursorRenderer,
    _keyCursorRenderer, _timestampsRenderer
  ];
}

- (NSArray<id<iTermMetalRenderer>> *)nonCellRenderers {
  NSArray *shared = @[
    _backgroundImageRenderer, _badgeRenderer, _broadcastStripesRenderer,
    _copyBackgroundRenderer, _indicatorRenderer, _flashRenderer
  ];
