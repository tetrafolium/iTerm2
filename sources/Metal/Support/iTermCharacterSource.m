...
static const CGFloat iTermCharacterSourceAntialiasedRetinaFakeBoldShiftPoints =
    0.5;
static const CGFloat
    iTermCharacterSourceAntialiasedNonretinaFakeBoldShiftPoints = 0;
...
+ (instancetype)
    characterSourceDescriptorWithAsciiFont:(PTYFontInfo *)asciiFontInfo
  ...
  iTermCharacterSourceDescriptor *descriptor =
      [[iTermCharacterSourceDescriptor alloc] init];
...
+ (instancetype)characterSourceAttributesWithThinStrokes:(BOOL)useThinStrokes
  ...
  iTermCharacterSourceAttributes *attributes =
      [[iTermCharacterSourceAttributes alloc] init];
...
@implementation iTermCharacterSource {
  ...
  BOOL _fakeItalic;
  NSFont *_font;
  // Large enough to hold glyphSize * maxParts in both horizontal and vertical
  // direction.
  CGSize _size;
  BOOL _boxDrawing;
...
+ (NSRect)boundingRectForCharactersInRange:(NSRange)range
  ...
  if (useNonAsciiFont && nonAsciiFontInfo) {
    pointSize =
        MAX(asciiFontInfo.font.pointSize, nonAsciiFontInfo.font.pointSize);
  } else {
    pointSize = asciiFontInfo.font.pointSize;
  }
  NSArray *key = @[
    NSStringFromRange(range), asciiFontInfo.font.fontName ?: @"",
    nonAsciiFontInfo.font.fontName ?: @"", @(pointSize),
    @(asciiFontInfo.baselineOffset), @(scale), @(useBoldFont), @(useItalicFont),
    @(useNonAsciiFont)
  ];
  if (cache[key]) {
  ...
  const CGSize bigSize =
      CGSizeMake(round(pointSize * 10), round(pointSize * 10));
  iTermCharacterSourceDescriptor *descriptor = [iTermCharacterSourceDescriptor
      characterSourceDescriptorWithAsciiFont:asciiFontInfo
  ...
  iTermCharacterSourceAttributes *attributes = [iTermCharacterSourceAttributes
      characterSourceAttributesWithThinStrokes:NO
                                          bold:YES
                                        italic:YES];
  ...
  for (NSInteger i = 0; i < range.length; i++) {
    @autoreleasepool {
      ...
      iTermCharacterSource *source = [[iTermCharacterSource alloc]
                 initWithCharacter:[NSString stringWithLongCharacter:c]
...
- (instancetype)initWithCharacter:(NSString *)string
  ...
  if (self) {
    ...
    _antialiased =
        _isAscii ? descriptor.asciiAntiAliased : descriptor.nonAsciiAntiAliased;
    _font = [descriptor fontForASCII:_isAscii
                          attributes:attributes
                          renderBold:&_fakeBold
                        renderItalic:&_fakeItalic];
    DLog(@"%p initialize with descriptor %@, isAscii=%@", self, descriptor,
         @(_isAscii));
    ITAssertWithMessage(_font, @"Nil font for string=%@ attributes=%@", string,
                        attributes);
    ...
    for (int i = 0; i < 4; i++) {
      _attributedStrings[i] = [[NSAttributedString alloc]
          initWithString:string
              attributes:[self attributesForIteration:i]];
      DLog(@"Create lineref %@ with attributed string %@", @(i),
           _attributedStrings[i]);
      _lineRefs[i] = CTLineCreateWithAttributedString(
          (CFAttributedStringRef)_attributedStrings[i]);
    }
  }
  return self;
}
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p font=%@>",
                                 NSStringFromClass([self class]), self, _font];
}
...
- (void)logStringRepresentationOfAlphaChannelOfBitmapDataBytes:
    (unsigned char *)data {
...
- (void)performPostProcessing {
  // Be conservative and allocate more space than needed if there's any
  // imprecision.
  const NSInteger destinationLength =
      ceil(_size.width) * 4 * ceil(_size.height);
  ...
  _postprocessed = YES;
  [_postprocessedData
      checkForOverrunWithInfo:[NSString
                                  stringWithFormat:@"Size is %@",
...
- (iTermCharacterBitmap *)bitmapForPart:(int)part {
  ...
  if (saveBitmapsForDebugging) {
    NSImage *image =
        [NSImage imageWithRawData:[NSData dataWithBytes:bitmapBytes
                                                 length:bitmap.data.length]
                             size:bitmap.size
                    bitsPerSample:8
                  samplesPerPixel:4
                         hasAlpha:YES
  ...
  ssize_t sourceOffset =
      (dx * 4 * glyphWidth) + (dy * glyphHeight * sourceRowSize);
  for (int i = 0; (destOffset >= 0 && i < glyphHeight &&
...
- (NSArray<NSNumber *> *)newParts {
  ...
  for (int y = 0; y < self.maxParts; y++) {
    for (int x = 0; x < self.maxParts; x++) {
      CGRect partRect = CGRectMake(
          x * _descriptor.glyphSize.width, y * _descriptor.glyphSize.height,
...
- (CGRect)frameFlipped:(BOOL)flipped {
  ...
  if (_boxDrawing) {
    ...
    rect.size =
        NSMakeSize(_descriptor.cellSize.width * _descriptor.scale + inset * 2,
  ...
  if (_fakeItalic) {
    // Unfortunately it looks like CTLineGetImageBounds ignores the context's
    // text matrix so we have to guess what the frame's width would be when
    // skewing it.
    const CGFloat heightAboveBaseline =
        NSMaxY(frame) + _descriptor.baselineOffset * _descriptor.scale;
  ...
  CGPoint min =
      CGPointMake(floor(CGRectGetMinX(frame)), floor(CGRectGetMinY(frame)));
  CGPoint max =
      CGPointMake(ceil(CGRectGetMaxX(frame)), ceil(CGRectGetMaxY(frame)));
  frame = CGRectMake(min.x, min.y, max.x - min.x, max.y - min.y);
  DLog(@"%@ Bounding box for character '%@' in font %@ is %@ at scale %@", self,
       _string, _font, NSStringFromRect(frame), @(_descriptor.scale));
...
- (void)drawWithOffset:(CGPoint)offset iteration:(NSInteger)iteration {
  ...
  [self drawRuns:runs
       ...
       iteration:iteration];
  _haveDrawn = YES;
  const NSUInteger length = CGBitmapContextGetBytesPerRow(_context) *
                            CGBitmapContextGetHeight(_context);
  NSMutableData *data =
      [NSMutableData dataWithBytes:CGBitmapContextGetData(_context)
                            length:length];
...
- (void)initializeStateIfNeededWithFont:(CTFontRef)runFont {
  ...
  ITAssertWithMessage(_context, @"context is null for size %@",
                      NSStringFromSize(_size));
...
- (void)setTextColorForIteration:(NSInteger)iteration
                         context:(CGContextRef)context {
  ITAssertWithMessage(context, @"nil context for iteration %@/%@", @(iteration),
...
- (void)initializeIteration:(NSInteger)iteration
  ...
  if (_antialiased) {
    ...
    if (shouldSmooth) {
      if (_attributes.useThinStrokes) {
        // This seems to be available at least on 10.8 and later. The only
        // reference to it is in WebKit. This causes text to render just a
        // little lighter, which looks nicer. It does not work in Mojave without
        // subpixel AA.
        style = 16;
      } else {
        style = 0;
...
- (void)drawBoxInContext:(CGContextRef)context
                  offset:(CGPoint)offset
               iteration:(int)iteration {
  assert(context);
  [iTermBoxDrawingBezierCurveFactory
      drawCodeInCurrentContext:[_string characterAtIndex:0]
                      cellSize:NSMakeSize(_descriptor.cellSize.width *
                                              _descriptor.scale,
                                          _descriptor.cellSize.height *
                                              _descriptor.scale)
...
- (void)drawBoxAtOffset:(CGPoint)offset iteration:(int)iteration {
  NSGraphicsContext *graphicsContext =
      [NSGraphicsContext graphicsContextWithCGContext:_context flipped:YES];
  [NSGraphicsContext saveGraphicsState];
  [NSGraphicsContext setCurrentContext:graphicsContext];
  NSAffineTransform *transform = [NSAffineTransform transform];

  const CGFloat scaledCellHeight =
      _descriptor.cellSize.height * _descriptor.scale;
  const CGFloat scaledCellHeightWithoutSpacing =
      _descriptor.cellSizeWithoutSpacing.height * _descriptor.scale;
  const float verticalShift =
      round((scaledCellHeight - scaledCellHeightWithoutSpacing) /
            (2 * _descriptor.scale)) *
      _descriptor.scale;

  [transform
      translateXBy:offset.x
               yBy:offset.y +
                   (_descriptor.baselineOffset + _descriptor.cellSize.height) *
                       _descriptor.scale -
                   verticalShift];
...
- (void)drawRuns:(CFArrayRef)runs
  ...
  for (CFIndex j = 0; j < CFArrayGetCount(runs); j++) {
    ...
    DLog(@"%@ For run %@, run is %@ and runFont is %@", self, @(j), run,
         runFont);
    [self prepareToDrawRunAtIteration:iteration
                               offset:offset
                              runFont:runFont
                                 skew:skew
    ...
    if (_boxDrawing) {
      [self drawBoxAtOffset:offset iteration:iteration];
    } else if (_isEmoji) {
      ...
      if (_fakeBold) {
        [self
            initializeTextMatrixInContext:context
                                 withSkew:skew
                                   offset:CGPointMake(offset.x +
                                                          self.fakeBoldShift *
                                                              _descriptor.scale,
                                                      offset.y)];
      ...
      CGContextStrokeRect(_contexts[iteration],
                          CGRectMake(offset.x + positions[0].x,
...
- (void)drawEmojiWithFont:(CTFontRef)runFont
  ...
  [self initializeCTMWithFont:runFont
                       offset:offset
                    iteration:iteration
                      context:context];
...
- (const CGGlyph *)glyphsInRun:(CTRunRef)run length:(size_t)length {
  ...
  CTRunGetGlyphs(run, CFRangeMake(0, length),
                 (CGGlyph *)_glyphsData.mutableBytes);
...
- (CGPoint *)positionsInRun:(CTRunRef)run length:(size_t)length {
  ...
  CTRunGetPositions(run, CFRangeMake(0, length),
                    (CGPoint *)_positionsBuffer.mutableBytes);
...
- (void)initializeTextMatrixInContext:(CGContextRef)cgContext
  ...
  if (!_isEmoji) {
    ...
    CGAffineTransform textMatrix =
        CGAffineTransformMake(_descriptor.scale, 0.0, skew * _descriptor.scale,
...
- (NSDictionary *)attributesForIteration:(NSInteger)iteration {
  ...
  return @{
    ...
    (NSString *)kCTForegroundColorAttributeName :
        (id)[self textColorForIteration:iteration],
