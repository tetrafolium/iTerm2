...
public:
  ...
  NSString *description() const {
    return [NSString
        stringWithFormat:
            @"[GlyphKey: code=%@ combiningSuccessor=%@ complex=%@ "
            @"boxdrawing=%@ thinstrokes=%@ drawable=%@ typeface=%@]",
            @(_repr.code), @(_repr.combiningSuccessor), @(_repr.isComplex),
            @(_repr.boxDrawing), @(_repr.thinStrokes), @(_repr.drawable),
            @(_repr.typeface)];
  }
...
namespace std {
template <> struct hash<iTerm2::GlyphKey> {
