...
// Maybe I could increase this in the future but it's easier to reason about
// issues during development when it's 1. This is exposed because it's used to
// set the capacity of mixed-size buffer pools.
...
// You don't generally need to assign to this unless you plan to make more than
// one draw call. You can get a pipeline state from the
// iTermMetal[Cell]Renderer. See its comments for details.
...
// Returns the pipeline state based on the current value of
// `fragmentFunctionName`, which you can change whenever you please.
...
- (id<MTLBuffer>)
    newQuadWithFrame:
        (CGRect)quad // pixel coordinates on viewport with 0,0 at bottom left
        textureFrame:(CGRect)textureFrame // normalized coordinates
         poolContext:(iTermMetalBufferPoolContext *)poolContext;

// Things in Metal are randomly upside down for no good reason. So make it easy
// to flip them back.
...
- (nullable id<MTLTexture>)
    textureFromImage:(NSImage *)image
             context:(nullable iTermMetalBufferPoolContext *)context
                pool:(nullable iTermTexturePool *)pool;

- (id<MTLRenderPipelineState>)
    newPipelineWithBlending:(nullable iTermMetalBlending *)blending
