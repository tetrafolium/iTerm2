...
- (NSString *)debugDescription {
  return [NSString
      stringWithFormat:
          @"<%@: %p viewportSize=%@x%@ scale=%@ cellSize=%@ "
          @"cellSizeWithoutSpacing=%@ gridSize=%@ usingIntermediatePass=%@>",
          NSStringFromClass([self class]), self, @(self.viewportSize.x),
...
- (void)setPIUValue:(void *)valuePointer coord:(VT100GridCoord)coord {
  const size_t index =
      coord.x + coord.y * self.cellConfiguration.gridSize.width;
  memcpy(self.pius.contents + index * _piuElementSize, valuePointer,
         _piuElementSize);
}
...
- (NSEdgeInsets)margins {
  ...
  if (@available(macOS 10.14, *)) {
    ...
    MARGIN_WIDTH = [iTermPreferences intForKey:kPreferenceKeySideMargins] *
                   self.configuration.scale;
    MARGIN_HEIGHT =
        [iTermPreferences intForKey:kPreferenceKeyTopBottomMargins] *
        self.configuration.scale;
  } else {
    // MTKView inset on sides and top to avoid overlapping rounded corners too
    // much.
    MARGIN_WIDTH =
        MAX(0, [iTermPreferences intForKey:kPreferenceKeySideMargins] - 1) *
        self.configuration.scale;
    MARGIN_HEIGHT = 0;
  }
  ...
  const CGSize usableSize =
      CGSizeMake(self.cellConfiguration.viewportSize.x - MARGIN_WIDTH * 2 -
                     extraMargins.left - extraMargins.right,
                 self.cellConfiguration.viewportSize.y - MARGIN_HEIGHT * 2 -
                     extraMargins.top - extraMargins.bottom);
  return NSEdgeInsetsMake(
      fmod(usableSize.height, self.cellConfiguration.cellSize.height) +
          MARGIN_HEIGHT + extraMargins.bottom,
      MARGIN_WIDTH, MARGIN_HEIGHT + extraMargins.top,
      fmod(usableSize.width, self.cellConfiguration.cellSize.width) +
          MARGIN_WIDTH);
}
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  NSString *info = [NSString
      stringWithFormat:@"margins=(top=%@, left=%@, bottom=%@,right= %@)",
                       @(self.margins.top), @(self.margins.left),
                       @(self.margins.bottom), @(self.margins.right)];
  [info
      writeToURL:[folder URLByAppendingPathComponent:@"CellTransientState.txt"]
      atomically:NO
...
- (nullable __kindof iTermMetalRendererTransientState *)
  ...
  __kindof iTermMetalRendererTransientState *transientState =
      [super createTransientStateForConfiguration:configuration
  ...
  const vector_float2 offset = {margins.left, margins.top};
  tState.offsetBuffer =
      [_offsetBuffers requestBufferFromContext:tState.poolContext
                                     withBytes:&offset
                                checkIfChanged:YES];
  return tState;
}
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer
  ...
  if (index == iTermVertexInputIndexOffset &&
      buffer.length == sizeof(vector_float2)) {
    vector_float2 *v = (vector_float2 *)buffer.contents;
    NSString *s =
        [NSString stringWithFormat:@"offset=(%@, %@)\n", @(v->x), @(v->y)];
    [s writeToURL:[folder URLByAppendingPathComponent:
                              @"vertexBuffer.iTermVertexInputIndexOffset.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:nil];
  } else {
