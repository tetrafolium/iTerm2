...
static void *iTermTexturePoolAssociatedObjectKeyGeneration =
    "iTermTexturePoolAssociatedObjectKeyGeneration";
...
- (instancetype)init {
  ...
  if (self) {
    ...
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(powerManagerMetalAllowedDidChange:)
               name:iTermPowerManagerMetalAllowedDidChangeNotification
             object:nil];
  }
  return self;
}
...
- (void)returnTexture:(id<MTLTexture>)texture {
  @synchronized(self) {
    if (texture.width == _size.x && texture.height == _size.y) {
      NSNumber *generation = [(NSObject *)texture
...
- (void)stampTextureWithGeneration:(id<MTLTexture>)texture {
  [(NSObject *)texture
      it_setAssociatedObject:_generation
                      forKey:iTermTexturePoolAssociatedObjectKeyGeneration];
}
