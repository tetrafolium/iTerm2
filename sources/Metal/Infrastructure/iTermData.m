...
- (instancetype)initWithLength:(NSUInteger)length {
  ...
  if (self) {
    unsigned char *buffer =
        iTermMalloc(length + sizeof(iTermDataGuardRegionValue));
    memmove(buffer + length, iTermDataGuardRegionValue,
...
- (void)checkForOverrun {
  if (_mutableBytes) {
    ...
    const int comparisonResult =
        memcmp(buffer + _originalLength, iTermDataGuardRegionValue,
               sizeof(iTermDataGuardRegionValue));
