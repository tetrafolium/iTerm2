...
- (void)writeToFolder:(NSURL *)folder {
  [_vertexBuffers enumerateKeysAndObjectsUsingBlock:^(
                      NSNumber *_Nonnull key, id<MTLBuffer> _Nonnull obj,
                      BOOL *_Nonnull stop) {
    [self->_formatter writeVertexBuffer:obj
                                  index:key.integerValue
                               toFolder:folder];
  }];
  [_fragmentBuffers enumerateKeysAndObjectsUsingBlock:^(
                        NSNumber *_Nonnull key, id<MTLBuffer> _Nonnull obj,
                        BOOL *_Nonnull stop) {
    [self->_formatter writeFragmentBuffer:obj
                                    index:key.integerValue
                                 toFolder:folder];
  }];
  [_fragmentTextures enumerateKeysAndObjectsUsingBlock:^(
                         NSNumber *_Nonnull key, id<MTLTexture> _Nonnull obj,
                         BOOL *_Nonnull stop) {
    [self->_formatter writeFragmentTexture:obj
                                     index:key.integerValue
                                  toFolder:folder];
    NSString *description = [obj debugDescription];
    NSString *file =
        [NSString stringWithFormat:@"texture-description.%@.txt", key];
  ...
  NSString *description = [NSString
      stringWithFormat:@"vertex count: %@\ninstance count: %@\nFragment "
                       @"function: %@\nVertex function: %@\n",
                       @(_vertexCount), @(_instanceCount),
                       self.fragmentFunctionName, self.vertexFunctionName];
  [description
      writeToURL:[folder URLByAppendingPathComponent:@"description.txt"]
      atomically:NO
        encoding:NSUTF8StringEncoding
           error:nil];
}
...
- (void)setPostmultipliedRenderPassDescriptor:
    (MTLRenderPassDescriptor *)renderPassDescriptor {
  _postmultipliedRenderPassDescriptor = renderPassDescriptor;
}

- (void)setIntermediateRenderPassDescriptor:
    (MTLRenderPassDescriptor *)renderPassDescriptor {
  _intermediateRenderPassDescriptor = renderPassDescriptor;
}

- (void)setTemporaryRenderPassDescriptor:
    (MTLRenderPassDescriptor *)renderPassDescriptor {
...
- (void)addRenderOutputData:(NSData *)data
  ...
  tState.renderedOutputForDebugging =
      [NSImage imageWithRawData:data
                           size:size
                  bitsPerSample:8
                samplesPerPixel:4
                       hasAlpha:YES
...
- (iTermMetalDebugDrawInfo *)newDrawWithFormatter:
    (id<iTermMetalDebugInfoFormatter>)formatter {
...
- (NSData *)newArchive {
  ...
  NSURL *root = [[[NSURL fileURLWithPath:NSTemporaryDirectory()]
      ...
      URLByAppendingPathExtension:@"iterm2-metal-frame"];
  NSError *error = nil;
  ...
  if (_postmultipliedRenderPassDescriptor) {
    [self
        writeRenderPassDescriptor:_postmultipliedRenderPassDescriptor
                               to:[self
                                      newFolderNamed:
                                          @"postmultipliedRenderPassDescriptor"
                                                root:root]];
  }
  if (_intermediateRenderPassDescriptor) {
    [self
        writeRenderPassDescriptor:_intermediateRenderPassDescriptor
                               to:[self newFolderNamed:
                                            @"IntermediateRenderPassDescriptor"
                                                  root:root]];
  }
  if (_temporaryRenderPassDescriptor) {
    [self writeRenderPassDescriptor:_temporaryRenderPassDescriptor
                                 to:[self newFolderNamed:
                                              @"TemporaryRenderPassDescriptor"
                                                    root:root]];
  }
  NSURL *rowDataFolder = [self newFolderNamed:@"RowData" root:root];
  [_rowData enumerateObjectsUsingBlock:^(iTermMetalRowData *_Nonnull rowData,
  ...
  [_transientStates enumerateObjectsUsingBlock:^(
                        iTermMetalRendererTransientState *_Nonnull obj,
                        NSUInteger idx, BOOL *_Nonnull stop) {
    Class theClass = [obj class];
    NSURL *folder = [self
        newFolderNamed:[NSString stringWithFormat:@"state-%04d-%@",
                                                  (int)obj.sequenceNumber,
    ...
    if (obj.renderedOutputForDebugging) {
      NSString *filename = [NSString
          stringWithFormat:@"%04d-output.png", (int)obj.sequenceNumber];
      [obj.renderedOutputForDebugging
          saveAsPNGTo:[[folder URLByAppendingPathComponent:filename] path]];
    }
  }];

  [_cellRenderers enumerateObjectsUsingBlock:^(
                      id<iTermMetalCellRenderer> _Nonnull obj, NSUInteger idx,
                      BOOL *_Nonnull stop) {
    ...
    Class theClass = [obj class];
    NSURL *folder = [self
        newFolderNamed:[NSString stringWithFormat:@"renderer-%04d-%@", (int)idx,
                                                  NSStringFromClass(theClass)]
                  root:root];
    [obj writeDebugInfoToFolder:folder];
  }];

  [_draws enumerateObjectsUsingBlock:^(iTermMetalDebugDrawInfo *_Nonnull obj,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
    NSString *name =
        [NSString stringWithFormat:@"draw-%04d-%@", (int)idx, obj.name];
...
- (void)writeRenderPassDescriptor:(MTLRenderPassDescriptor *)rpd
                               to:(NSURL *)folder {
  [[rpd debugDescription]
      writeToURL:[folder URLByAppendingPathComponent:@"DebugDescription.txt"]
      atomically:NO
        encoding:NSUTF8StringEncoding
           error:nil];
  MTLRenderPassColorAttachmentDescriptor *attachment = rpd.colorAttachments[0];
  [[attachment debugDescription]
      writeToURL:[folder URLByAppendingPathComponent:@"ColorAttachment0.txt"]
      atomically:NO
        encoding:NSUTF8StringEncoding
           error:nil];
}
