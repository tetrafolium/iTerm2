...
#if __cplusplus
static_assert(iTermASCIITextureAttributesBold == iTermMetalGlyphKeyTypefaceBold,
              "Bold flags differ");
static_assert(iTermASCIITextureAttributesItalic ==
                  iTermMetalGlyphKeyTypefaceItalic,
              "Italic flags differ");
#endif

NS_INLINE iTermASCIITextureAttributes
iTermASCIITextureAttributesFromGlyphKeyTypeface(
    iTermMetalGlyphKeyTypeface typeface, BOOL thinStrokes) {
...
// Convert a drawable ASCII character into an index into the texture array. Note
// that control character, SPACE, and DEL (127) are not accepted.
NS_INLINE int iTermASCIITextureIndexOfCode(char code,
  ...
  return offset + iTermASCIITextureOffsetCount *
                      (code - iTermASCIITextureMinimumCharacter);
}
...
@property(nonatomic, copy, readonly)
    NSDictionary<NSNumber *, iTermCharacterBitmap *> * (^creation)
        (char, iTermASCIITextureAttributes);
