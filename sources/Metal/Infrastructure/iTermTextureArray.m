...
+ (CGSize)atlasSizeForUnitSize:(CGSize)unitSize
  ...
  return CGSizeMake(unitSize.width * cellsPerRow,
                    unitSize.height *
                        ceil((double)length / (double)cellsPerRow));
}
...
- (instancetype)initWithTextureWidth:(uint32_t)width
  ...
  if (self) {
    ...
    _arrayLength = length;
    CGSize atlasSize =
        [iTermTextureArray atlasSizeForUnitSize:CGSizeMake(width, height)
                                    arrayLength:length
...
- (BOOL)setSlice:(NSUInteger)slice withContentsOfFile:(NSString *)path {
  return [self setSlice:slice
              withImage:[[NSImage alloc] initWithContentsOfFile:path]];
}
...
- (BOOL)setSlice:(NSUInteger)slice withImage:(NSImage *)nsimage {
  ...
  NSBitmapImageRep *bitmapRepresentation =
      [[NSBitmapImageRep alloc] initWithData:[nsimage TIFFRepresentation]];
  ...
  CGContextRef context =
      CGBitmapContextCreate(NULL, width, height, 8, rowBytes, colorSpace,
