...
+ (instancetype)compositeSourceOver {
  ...
  // I tried to make this the same as NSCompositingOperationSourceOver. It's not
  // quite right but I have no idea why.
  blending.rgbBlendOperation = MTLBlendOperationAdd;
  blending.sourceRGBBlendFactor =
      MTLBlendFactorOne; // because it's premultiplied
...
- (NSString *)debugDescription {
  return [NSString stringWithFormat:@"<%@: %p viewportSize=%@x%@ scale=%@>",
                                    ...
                                    @(self.viewportSize.x),
                                    @(self.viewportSize.y), @(self.scale)];
}
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  [[_pipelineState debugDescription]
      writeToURL:[folder URLByAppendingPathComponent:@"PipelineState.txt"]
      atomically:NO
        encoding:NSUTF8StringEncoding
           error:NULL];
  [[_configuration debugDescription]
      writeToURL:[folder URLByAppendingPathComponent:@"Configuration.txt"]
      atomically:NO
        encoding:NSUTF8StringEncoding
           error:NULL];
  NSString *state = [NSString
      stringWithFormat:@"skip=%@", self.skipRenderer ? @"YES" : @"NO"];
...
@implementation iTermMetalRenderer {
  ...
  NSMutableDictionary<NSDictionary *, id<MTLRenderPipelineState>>
      *_pipelineStates;
...
- (NSDictionary *)keyForPipelineState {
  // This must contain all inputs to picking the pipeline state. These can be
  // changed at runtime.
  return @{
    @"fragment function" : _fragmentFunctionName ?: @"",
...
- (id<MTLRenderPipelineState>)pipelineState {
  ...
  if (_pipelineStates[key] == nil) {
    ...
    id<MTLFunction> vertexShader =
        [defaultLibrary newFunctionWithName:_vertexFunctionName];
    ITDebugAssert(vertexShader);
    id<MTLFunction> fragmentShader =
        [defaultLibrary newFunctionWithName:_fragmentFunctionName];
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer
  ...
  NSString *name = [NSString
      stringWithFormat:@"vertexBuffer.%@.%@.bin", @(index), buffer.label];
  ...
  if ([_formatterDelegate
          respondsToSelector:@selector(writeVertexBuffer:index:toFolder:)]) {
    [_formatterDelegate writeVertexBuffer:buffer index:index toFolder:folder];
  }
}

- (void)writeFragmentBuffer:(id<MTLBuffer>)buffer
                      index:(NSUInteger)index
                   toFolder:(NSURL *)folder {
  NSData *data = [NSData dataWithBytesNoCopy:buffer.contents
                                      length:buffer.length
                                freeWhenDone:NO];
  NSString *name = [NSString
      stringWithFormat:@"fragmentBuffer.%@.%@.bin", @(index), buffer.label];
  [data writeToURL:[folder URLByAppendingPathComponent:name] atomically:NO];

  if ([_formatterDelegate
          respondsToSelector:@selector(writeFragmentBuffer:index:toFolder:)]) {
    [_formatterDelegate writeFragmentBuffer:buffer index:index toFolder:folder];
  }
}

- (void)writeFragmentTexture:(id<MTLTexture>)texture
                       index:(NSUInteger)index
                    toFolder:(NSURL *)folder {
  ...
  [texture getBytes:storage.mutableBytes
        ...
        mipmapLevel:0];
  NSImage *image =
      [NSImage imageWithRawData:storage
  ...
  NSString *name =
      [NSString stringWithFormat:@"texture.%@.%@.png", @(index), texture.label];
  [image saveAsPNGTo:[folder URLByAppendingPathComponent:name].path];

  if ([_formatterDelegate
          respondsToSelector:@selector(writeFragmentTexture:index:toFolder:)]) {
    [_formatterDelegate writeFragmentTexture:texture
                                       index:index
                                    toFolder:folder];
  }
}
...
- (nullable __kindof iTermMetalRendererTransientState *)
  ...
  iTermMetalRendererTransientState *tState =
      [[self.transientStateClass alloc] initWithConfiguration:configuration];
...
- (id<MTLBuffer>)newQuadWithFrame:(CGRect)quad
  ...
  const iTermVertex vertices[] = {
      ...
      {{CGRectGetMaxX(quad), CGRectGetMinY(quad)},
       {CGRectGetMaxX(textureFrame), CGRectGetMinY(textureFrame)}},
      {{CGRectGetMinX(quad), CGRectGetMinY(quad)},
       {CGRectGetMinX(textureFrame), CGRectGetMinY(textureFrame)}},
      {{CGRectGetMinX(quad), CGRectGetMaxY(quad)},
       {CGRectGetMinX(textureFrame), CGRectGetMaxY(textureFrame)}},
      ...
      {{CGRectGetMaxX(quad), CGRectGetMaxY(quad)},
       {CGRectGetMaxX(textureFrame), CGRectGetMaxY(textureFrame)}},
  };
...
- (id<MTLBuffer>)newQuadOfSize:(CGSize)size
  ...
  const iTermVertex vertices[] = {
      // Pixel Positions             Texture Coordinates
      {{size.width, 0}, {1.f, 0.f}},           {{0, 0}, {0.f, 0.f}},
      {{0, size.height}, {0.f, 1.f}},

      {{size.width, 0}, {1.f, 0.f}},           {{0, size.height}, {0.f, 1.f}},
...
- (id<MTLBuffer>)newFlippedQuadOfSize:(CGSize)size
  ...
  const iTermVertex vertices[] = {
      // Pixel Positions, Texture Coordinates
      {{size.width, 0}, {1.f, 1.f}},           {{0, 0}, {0.f, 1.f}},
      {{0, size.height}, {0.f, 0.f}},

      {{size.width, 0}, {1.f, 1.f}},           {{0, size.height}, {0.f, 0.f}},
...
- (id<MTLRenderPipelineState>)
    newPipelineWithBlending:(iTermMetalBlending *)blending
             vertexFunction:(id<MTLFunction>)vertexFunction
           fragmentFunction:(id<MTLFunction>)fragmentFunction {
  // Set up a descriptor for creating a pipeline state object
  MTLRenderPipelineDescriptor *pipelineStateDescriptor =
      [[MTLRenderPipelineDescriptor alloc] init];
  pipelineStateDescriptor.label = [NSString
      stringWithFormat:@"Pipeline for %@", NSStringFromClass([self class])];
  ...
  if (blending) {
    ...
    renderbufferAttachment.destinationRGBBlendFactor =
        blending.destinationRGBBlendFactor;

    renderbufferAttachment.sourceAlphaBlendFactor =
        blending.sourceAlphaBlendFactor;
    renderbufferAttachment.destinationAlphaBlendFactor =
        blending.destinationAlphaBlendFactor;
  }
...
- (nullable id<MTLTexture>)
    textureFromImage:(NSImage *)image
...
- (void)convertWidth:(NSUInteger)width
  ...
  if (height > width) {
    [self convertWidth:height
                height:width
               toWidth:heightOut
                height:widthOut
          notExceeding:maxSize];
    return;
  }
...
- (nullable id<MTLTexture>)textureFromImage:(NSImage *)image
                                    context:
  ...
  CGImageRef imageRef = [image CGImageForProposedRect:&imageRect
                                              context:NULL
                                                hints:nil];
  ...
  uint8_t *rawData =
      (uint8_t *)iTermCalloc(height * width * 4, sizeof(uint8_t));
...
- (id<MTLBuffer>)vertexBufferForViewportSize:(vector_uint2)viewportSize {
  if (!simd_equal(viewportSize, _cachedViewportSize)) {
    ...
    _cachedViewportSizeBuffer =
        [_device newBufferWithBytes:&viewportSize
...
- (void)drawWithTransientState:(iTermMetalRendererTransientState *)tState
              ...
              numberOfVertices:(NSInteger)numberOfVertices
                 ...
                 vertexBuffers:(NSDictionary<NSNumber *, id<MTLBuffer>> *)
                                   clientVertexBuffers
               fragmentBuffers:
                   (NSDictionary<NSNumber *, id<MTLBuffer>> *)fragmentBuffers
                      textures:
                          (NSDictionary<NSNumber *, id<MTLTexture>> *)textures {
  iTermMetalDebugDrawInfo *debugDrawInfo =
      [tState.debugInfo newDrawWithFormatter:self];
  ...
  vertexBuffers = [clientVertexBuffers
      dictionaryBySettingObject:
          [self vertexBufferForViewportSize:tState.configuration.viewportSize]
                         forKey:@(iTermVertexInputIndexViewportSize)];

  [vertexBuffers enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
                                                     id<MTLBuffer> _Nonnull obj,
                                                     BOOL *_Nonnull stop) {
  ...
  [fragmentBuffers enumerateKeysAndObjectsUsingBlock:^(
                       NSNumber *_Nonnull key, id<MTLBuffer> _Nonnull obj,
                       BOOL *_Nonnull stop) {
  ...
  [textures enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
                                                id<MTLTexture> _Nonnull obj,
                                                BOOL *_Nonnull stop) {
  ...
  [debugDrawInfo drawWithVertexCount:numberOfVertices
