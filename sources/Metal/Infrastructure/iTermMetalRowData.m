...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  NSString *info =
      [NSString stringWithFormat:@"y=%@\n"
                                 ...
                                 @"markStyle=%@\n"
                                 @"date=%@\n",
                                 @(self.y), @(self.numberOfBackgroundRLEs),
                                 @(self.numberOfDrawableGlyphs),
                                 @(self.markStyle), self.date];
  ...
  @autoreleasepool {
    ...
    [glyphKeysString
        writeToURL:[folder URLByAppendingPathComponent:@"GlyphKeys.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:NULL];
  }
  ...
  @autoreleasepool {
    ...
    iTermMetalGlyphAttributes *attributes =
        (iTermMetalGlyphAttributes *)_attributesData.mutableBytes;
    for (int i = 0;
         i < _attributesData.length / sizeof(iTermMetalGlyphAttributes); i++) {
      NSString *attribute =
          iTermMetalGlyphAttributesDescription(&attributes[i]);
      [attributesString appendFormat:@"%4d: %@\n", i, attribute];
    }
    [attributesString
        writeToURL:[folder URLByAppendingPathComponent:@"Attributes.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:NULL];
  }
  ...
  @autoreleasepool {
    ...
    iTermMetalBackgroundColorRLE *bg =
        (iTermMetalBackgroundColorRLE *)_backgroundColorRLEData.mutableBytes;
    ...
    [bgColorsString
        writeToURL:[folder URLByAppendingPathComponent:@"BackgroundColors.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:NULL];
  }
  ...
  @autoreleasepool {
    ...
    [lineString
        writeToURL:[folder URLByAppendingPathComponent:@"ScreenChars.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:NULL];
  }
...
- (NSString *)formatChar:(screen_char_t)c {
  return [NSString
      stringWithFormat:
          @"code=%x (%@) foregroundColor=%@ fgGreen=%@ fgBlue=%@ "
          @"backgroundColor=%@ bgGreen=%@ bgBlue=%@ foregroundColorMode=%@ "
          @"backgroundColorMode=%@ complexChar=%@ bold=%@ faint=%@ italic=%@ "
          @"blink=%@ underline=%@ underlineStyle=%@ strikethrough=%@ image=%@ "
          @"unused=%@ urlCode=%@",
          (int)c.code, ScreenCharToStr(&c), @(c.foregroundColor), @(c.fgGreen),
          @(c.fgBlue), @(c.backgroundColor), @(c.bgGreen), @(c.bgBlue),
          @(c.foregroundColorMode), @(c.backgroundColorMode), @(c.complexChar),
          @(c.bold), @(c.faint), @(c.italic), @(c.blink), @(c.underline),
          @(c.underlineStyle), @(c.strikethrough), @(c.image), @(c.unused),
          @(c.urlCode)];
}
