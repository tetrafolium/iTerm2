...
namespace iTerm2 {
// A PIUArray is an array of arrays of PIU structs. This avoids giant
// allocations. It is append-only.
template <class T> class PIUArray {
public:
  ...
  void push_back(const T &piu) { memmove(get_next(), &piu, sizeof(piu)); }

  size_t get_number_of_segments() const { return _arrays.size(); }
