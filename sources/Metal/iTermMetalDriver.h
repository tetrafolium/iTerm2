...
// Initialize sketchPtr to 0. The number of set bits estimates the unique number
...
// Returns the background image or nil. If there's a background image, fill in
// mode.
- (NSImage *)metalBackgroundImageGetMode:
    (nullable iTermBackgroundImageMode *)mode;

// An object that compares as equal if ascii characters produced by
// metalImagesForGlyph would produce the same bitmap.
...
- (void)enumerateIndicatorsInFrame:(NSRect)frame
                             block:(void (^)(iTermIndicatorDescriptor *))block;

- (void)metalEnumerateHighlightedRows:
    (void (^)(vector_float3 color, NSTimeInterval age, int row))block;
...
- (nullable id<iTermMetalDriverDataSourcePerFrameState>)
    metalDriverWillBeginDrawingFrame;

- (void)metalDriverDidDrawFrame:
    (id<iTermMetalDriverDataSourcePerFrameState>)perFrameState;

- (void)metalDidFindImages:(NSSet<NSString *> *)foundImages
             missingImages:(NSSet<NSString *> *)missingImages
             animatedLines:
                 (NSSet<NSNumber *> *)animatedLines; // absolute line numbers
...
// The arg to completion is YES on success and NO if the draw was aborted for
// lack of resources.
