...
namespace iTerm2 {
// Holds a collection of iTerm2::TexturePages. Provides an interface for finding
// a GlyphEntry for a GlyphKey, adding a new glyph, and pruning disused texture
// pages. Tries to be fast.
class TexturePageCollection : TexturePageOwner {
public:
  TexturePageCollection(id<MTLDevice> device, const vector_uint2 cellSize,
                        const int pageCapacity, const int maximumNumberOfPages)
      : _device(device), _cellSize(cellSize), _pageCapacity(pageCapacity),
        _maximumNumberOfPages(maximumNumberOfPages), _openPage(NULL) {}
  ...
  // Returns a collection of glyph entries for a glyph key, or NULL if none
  // exists.
  ...
  // Adds a collection of glyph entries for a glyph key, allocating a new
  // texture page if needed.
  std::vector<const GlyphEntry *> *
  add(int column, const GlyphKey &glyphKey,
      iTermMetalBufferPoolContext *context,
      NSDictionary<NSNumber *, iTermCharacterBitmap *> * (^creator)(int,
                                                                    BOOL *)) {
    BOOL emoji;
    NSDictionary<NSNumber *, iTermCharacterBitmap *> *images =
        creator(column, &emoji);
    std::vector<const GlyphEntry *> *result =
        new std::vector<const GlyphEntry *>();
    ...
    for (NSNumber *partNumber in images) {
      ...
      const GlyphEntry *entry =
          internal_add(partNumber.intValue, glyphKey, image, emoji, context);
      result->push_back(entry);
    }
    DLog(@"Added %@. Count is now %@", glyphKey.description(),
         @(_allPages.size() * _pageCapacity));

    return result;
  }

  const vector_uint2 &get_cell_size() const { return _cellSize; }

  // Discard least-recently used texture pages.
  void prune_if_needed() {
    if (is_over_maximum_size()) {
      ELog(@"Pruning. Have %@ pages. Each page stores up to %@ glyphs. Max "
           @"pages is %@",
           @(_allPages.size()), @(_pageCapacity), @(_maximumNumberOfPages));
      // Create a copy of texture pages sorted by recency of use (from least
      // recent to most).
      std::vector<TexturePage *> pages;
      std::copy(_allPages.begin(), _allPages.end(), std::back_inserter(pages));
      std::sort(pages.begin(), pages.end(),
                TexturePageCollection::LRUComparison);
...
private:
  const GlyphEntry *internal_add(int part, const GlyphKey &key,
                                 iTermCharacterBitmap *image, bool is_emoji,
                                 iTermMetalBufferPoolContext *context) {
    if (!_openPage) {
      _openPage = new TexturePage(this, _device, _pageCapacity,
                                  _cellSize); // Retains this for _openPage
    ...
    const GlyphEntry *result = new GlyphEntry(
        part, key, openPage, openPage->add_image(image, is_emoji), is_emoji);
  ...
  // Remove all references to `pageToPrune` and all glyph entries that reference
  // the page.
  void internal_prune(TexturePage *pageToPrune) {
    ...
    ITOwnershipLog(@"OWNERSHIP: page %p has %d owners", pageToPrune,
                   (int)owners.size());
    for (auto pair : owners) {
      auto owner = pair.first;  // TexturePageOwner *
      auto count = pair.second; // int
      ITOwnershipLog(@"OWNERSHIP: remove all %d references by owner %p", count,
                     owner);
      for (int j = 0; j < count; j++) {
        if (owner->texture_page_owner_is_glyph_entry()) {
          ...
          if (it != _pages.end()) {
            // Remove from _pages as soon as the first part is found for this
            // glyph key. Subsequent parts won't need to remove an entry from
            // _pages.
