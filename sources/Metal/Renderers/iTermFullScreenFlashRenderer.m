...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  NSString *s = [NSString stringWithFormat:@"color=(%@, %@, %@, %@)",
                                           @(self.color.x), @(self.color.y),
                                           @(self.color.z), @(self.color.w)];
...
- (vector_float4)premultipliedColor {
  return simd_make_float4(_color.x * _color.w, _color.y * _color.w,
                          _color.z * _color.w, _color.w);
}
  ...
  if (self) {
    _metalRenderer = [[iTermMetalRenderer alloc]
        ...
        fragmentFunctionName:@"iTermFullScreenFlashFragmentShader"
         ...
         transientStateClass:[iTermFullScreenFlashRendererTransientState
                                 class]];
    _colorBufferPool =
...
- (void)drawWithFrameData:(nonnull iTermMetalFrameData *)frameData
  ...
  if (color.w > 0) {
    ...
    id<MTLBuffer> vertexBuffer =
        [_metalRenderer newQuadOfSize:size poolContext:tState.poolContext];
    id<MTLBuffer> colorBuffer =
        [_colorBufferPool requestBufferFromContext:tState.poolContext
                                         withBytes:&color
                                    checkIfChanged:YES];
    [_metalRenderer
        drawWithTransientState:tState
              ...
              numberOfVertices:6
               ...
               fragmentBuffers:@{
                 @(iTermFragmentBufferIndexFullScreenFlashColor) : colorBuffer
               }
                      textures:@{}];
  }
}
...
- (nullable __kindof iTermMetalRendererTransientState *)
    createTransientStateForConfiguration:
        (nonnull iTermRenderConfiguration *)configuration
                           commandBuffer:
