...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  for (auto h : _highlights) {
    [s appendFormat:@"color=(%@, %@, %@, %@) row=%@\n", @(h.color.x),
                    @(h.color.y), @(h.color.z), @(h.color.w), @(h.row)];
  }
...
- (void)setOpacity:(CGFloat)opacity color:(vector_float3)color row:(int)row {
  iTerm2::Highlight h = {.color = simd_make_float4(color.x * opacity,
                                                   color.y * opacity,
                                                   color.z * opacity, opacity),
                         .row = row};
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  [tState enumerateDraws:^(vector_float4 color, int row) {
    id<MTLBuffer> vertexBuffer = [self->_cellRenderer
        newQuadWithFrame:CGRectMake(
                             0,
    ...
    id<MTLBuffer> colorBuffer =
        [self->_colorPool requestBufferFromContext:tState.poolContext
                                         withBytes:&color
                                    checkIfChanged:YES];
