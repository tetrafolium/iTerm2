...
@property(nonatomic, strong)
    NSArray<iTermIndicatorDescriptor *> *indicatorDescriptors;
@end
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  NSString *descriptors = [[_indicatorDescriptors
      mapWithBlock:^id(iTermIndicatorDescriptor *descriptor) {
...
- (void)initializeTransientState:
    (iTermIndicatorRendererTransientState *)tState {
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  [tState.indicatorDescriptors
      enumerateObjectsUsingBlock:^(
          iTermIndicatorDescriptor *_Nonnull descriptor, NSUInteger idx,
          BOOL *_Nonnull stop) {
  ...
  [_metalRenderer
      drawWithTransientState:tState
            ...
            numberOfVertices:6
             ...
             fragmentBuffers:@{
               @(iTermFragmentBufferIndexIndicatorAlpha) : alphaBuffer
             }
                    textures:@{
                      @(iTermTextureIndexPrimary) : descriptor.texture
                    }];
}
...
- (id<MTLBuffer>)vertexBufferForFrame:(NSRect)frame
  ...
  CGRect quad =
      CGRectMake(CGRectGetMinX(frame) * scale, CGRectGetMinY(frame) * scale,
