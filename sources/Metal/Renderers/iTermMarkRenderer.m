...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  [[NSString stringWithFormat:@"marks=%@", _marks]
...
- (nonnull NSData *)newMarkPerInstanceUniforms {
  NSMutableData *data = [[NSMutableData alloc]
      initWithLength:sizeof(iTermMarkPIU) * _marks.count];
  iTermMarkPIU *pius = (iTermMarkPIU *)data.mutableBytes;
  __block size_t i = 0;
  [_marks enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull rowNumber,
                                              NSNumber *_Nonnull styleNumber,
                                              BOOL *_Nonnull stop) {
    MTLOrigin origin =
        [self->_marksArrayTexture offsetForIndex:styleNumber.integerValue];
    pius[i] = (iTermMarkPIU){
        .offset = {0, (self.cellConfiguration.gridSize.height -
                       rowNumber.intValue - 1) *
                              self.cellConfiguration.cellSize.height +
                          self.margins.top +
                          self.cellConfiguration.cellSize.height -
                          self.markSize.height - self.markOffset.y},
...
- (void)initializeTransientState:(iTermMarkRendererTransientState *)tState {
  ...
  CGRect leftMarginRect = CGRectMake(
      1, 0,
      ([iTermPreferences intForKey:kPreferenceKeySideMargins] - 1) * scale,
      tState.cellConfiguration.cellSize.height);
  CGRect markRect = [iTermTextDrawingHelper
      frameForMarkContainedInRect:leftMarginRect
                         cellSize:tState.cellConfiguration.cellSize
           cellSizeWithoutSpacing:tState.cellConfiguration
  ...
  if (!CGSizeEqualToSize(markRect.size, _markSize)) {
    ...
    if (_markSize.width > 0 && _markSize.height > 0) {
      _marksArrayTexture =
          [[iTermTextureArray alloc] initWithTextureWidth:_markSize.width
      ...
      [_marksArrayTexture
          addSliceWithImage:[self newImageWithMarkOfColor:successColor
                                                     size:_markSize]];
      [_marksArrayTexture
          addSliceWithImage:[self newImageWithMarkOfColor:failureColor
                                                     size:_markSize]];
      [_marksArrayTexture
          addSliceWithImage:[self newImageWithMarkOfColor:otherColor
                                                     size:_markSize]];
    }
  }
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  const CGFloat scale = tState.configuration.scale;
  const CGRect quad = CGRectMake(round(tState.markOffset.x / scale) * scale, 0,
                                 tState.markSize.width, tState.markSize.height);
  const CGRect textureFrame =
      CGRectMake(0, 0, tState.markSize.width, tState.markSize.height);
  ...
  tState.vertexBuffer =
      [_cellRenderer.verticesPool requestBufferFromContext:tState.poolContext
                                                 withBytes:vertices
                                            checkIfChanged:YES];
  ...
  [_cellRenderer
      drawWithTransientState:tState
            ...
            numberOfVertices:6
             ...
             fragmentBuffers:@{}
                    textures:@{
                      @(iTermTextureIndexPrimary) :
                          tState.marksArrayTexture.texture
                    }];
}
