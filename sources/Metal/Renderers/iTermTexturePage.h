...
namespace iTerm2 {
enum dontcare { magic = 0xdeadbeef };
class TexturePage;

class TexturePageOwner {
public:
  virtual bool texture_page_owner_is_glyph_entry() { return false; }
};
...
public:
  ...
  TexturePage(TexturePageOwner *owner, id<MTLDevice> device, int capacity,
              vector_uint2 cellSize)
      : _magic(magic), _capacity(capacity), _cell_size(cellSize), _count(0),
        _emoji(capacity) {
    retain(owner);
    ...
    _reciprocal_atlas_size =
        1.0f / simd_make_float2(_atlas_size.x, _atlas_size.y);
  }
  ...
  void assert_valid() const { assert(_magic == magic); }

  int get_available_count() const { return _capacity - _count; }
  ...
  id<MTLTexture> get_texture() const { return _textureArray.texture; }

  iTermTextureArray *get_texture_array() const { return _textureArray; }

  bool get_is_emoji(const int index) const { return _emoji[index]; }

  const vector_uint2 &get_cell_size() const { return _cell_size; }

  const vector_uint2 &get_atlas_size() const { return _atlas_size; }
  ...
  void retain(TexturePageOwner *owner) {
    _owners[owner]++;
    ITOwnershipLog(@"OWNERSHIP: retain %p as owner of %p with refcount %d",
                   owner, this, (int)_owners[owner]);
  }

  bool release(TexturePageOwner *owner) {
    ITOwnershipLog(@"OWNERSHIP: release %p as owner of %p. New refcount for "
                   @"this owner will be %d",
                   owner, this, (int)_owners[owner] - 1);
  ...
  long long get_last_used() const { return _last_used; }
