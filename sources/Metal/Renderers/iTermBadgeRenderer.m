...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  CGRect quad = CGRectMake(scale * tState.destinationRect.origin.x,
                           tState.configuration.viewportSize.y -
                               scale * CGRectGetMaxY(tState.destinationRect) -
                               MARGIN_HEIGHT -
                               transientState.configuration.extraMargins.top,
                           scale * size.width, scale * size.height);
  // The destinationRect is clipped to the visible area.
  const CGFloat textureHeight =
      tState.textureSizeInPoints.height * tState.configuration.scale;
  ...
  tState.vertexBuffer =
      [_metalRenderer.verticesPool requestBufferFromContext:tState.poolContext
                                                  withBytes:vertices
                                             checkIfChanged:YES];
  [_metalRenderer
