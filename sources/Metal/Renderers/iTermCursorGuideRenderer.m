...
- (void)initializeVerticesWithPool:(iTermMetalBufferPool *)verticesPool {
  ...
  const CGRect quad = CGRectMake(
      self.margins.left,
      self.margins.top + (gridSize.height - self.row - 1) * cellSize.height,
      cellSize.width * gridSize.width, cellSize.height);
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  [[NSString stringWithFormat:@"row=%@", @(_row)]
...
- (id<MTLTexture>)newCursorGuideTextureWithTransientState:
    (iTermCursorGuideRendererTransientState *)tState {
  NSImage *image =
      [[NSImage alloc] initWithSize:tState.cellConfiguration.cellSize];

  [image lockFocus];
  {
    [_color set];
    NSRect rect = NSMakeRect(0, 0, tState.cellConfiguration.cellSize.width,
    ...
    rect.origin.y += tState.cellConfiguration.cellSize.height -
