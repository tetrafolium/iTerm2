...
- (NSString *)debugDescription {
  NSString *info = [NSString
      stringWithFormat:@"startCoordInImage=%@ startCoordOnScreen=%@ length=%@ "
                       @"code=%@ size=%@ uniqueIdentifier=%@",
  ...
  return
      [NSString stringWithFormat:@"<%@: %p %@>",
                                 NSStringFromClass([self class]), self, info];
}
...
// Counts the number of times each texture key is in use. Shared by all
// transient states.
...
- (void)addRun:(iTermMetalImageRun *)imageRun {
  ...
  // Check if the image got loaded asynchronously. This happens when decoding an
  // image takes a while.
  if ([iTermTexture metadataForTexture:texture]
          [iTermImageRendererTextureMetadataKeyImageMissing] &&
  ...
  if (_textures[key] == nil) {
    ...
    [_foundImageUniqueIdentifiers
        addObject:imageRun.imageInfo.uniqueIdentifier];
  }
  if (imageRun.imageInfo.animated) {
    [_animatedLines addObject:@(imageRun.startingCoordOnScreen.y +
...
- (id<MTLTexture>)newTextureForImageRun:(iTermMetalImageRun *)run {
  ...
  NSImage *image = [run.imageInfo imageWithCellSize:cellSize
  ...
  if (missing) {
    [iTermTexture
        setMetadataObject:@YES
                   forKey:iTermImageRendererTextureMetadataKeyImageMissing
                onTexture:texture];
  }
  return texture;
}

- (void)enumerateDraws:(void (^)(NSNumber *, id<MTLBuffer>,
                                 id<MTLTexture>))block {
  ...
  [_runs enumerateObjectsUsingBlock:^(iTermMetalImageRun *_Nonnull run,
    ...
    NSSize chunkSize =
        NSMakeSize(textureSize.width / run.imageInfo.size.width,
                   textureSize.height / run.imageInfo.size.height);
    const CGRect textureFrame = NSMakeRect(
        (chunkSize.width * run.startingCoordInImage.x) / textureSize.width,
        (textureSize.height -
         chunkSize.height * (run.startingCoordInImage.y + 1)) /
            textureSize.height,
    ...
    const CGFloat spacing =
        round((self.cellConfiguration.cellSizeWithoutSpacing.height -
               cellSize.height) /
              (2.0 * scale)) *
        scale;
    const CGRect destinationFrame = CGRectMake(
        run.startingCoordOnScreen.x * cellSize.width + offset.x,
        height - (run.startingCoordOnScreen.y + 1) * cellSize.height -
            offset.y - spacing,
        run.length * cellSize.width, cellSize.height);

    id<MTLBuffer> vertexBuffer =
        [self->_cellRenderer newQuadWithFrame:destinationFrame
...
@implementation iTermImageRenderer {
  ...
  // If a texture's reference count (stored in `_counts`) goes to 0 it doesn't
  // get removed from the dictionary immediately because if only one frame is
  // ever in flight then all textures would get removed at the end of each
  // frame. When the count goes to 0, put it on notice. If it's not used in the
  // next frame, remove it.
  //
  // TODO: Consider keeping a counted set and removing textures after N frames
  // of disuse, so that short animated GIFs can avoid churning textures.
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  [tState enumerateDraws:^(id key, id<MTLBuffer> vertexBuffer,
    ...
    [self->_cellRenderer drawWithTransientState:tState
        ...
        numberOfPIUs:0
        vertexBuffers:@{
          @(iTermVertexInputIndexVertices) : vertexBuffer
        }
        fragmentBuffers:@{}
        textures:@{
  ...
  [texturesToRemove enumerateObjectsUsingBlock:^(NSNumber *_Nonnull key,
                                                 BOOL *_Nonnull stop) {
