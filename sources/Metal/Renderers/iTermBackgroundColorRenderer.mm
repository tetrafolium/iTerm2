...
- (NSUInteger)sizeOfNewPIUBuffer {
  return sizeof(iTermBackgroundColorPIU) *
...
- (void)setColorRLEs:(const iTermMetalBackgroundColorRLE *)rles
  ...
  vector_float2 cellSize =
      simd_make_float2(self.cellConfiguration.cellSize.width,
...
- (void)enumerateSegments:(void (^)(const iTermBackgroundColorPIU *,
                                    size_t))block {
...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
  ...
  if (self) {
    ...
    if (iTermTextIsMonochrome()) {
      _nonblendingRenderer = [[iTermMetalCellRenderer alloc]
          ...
          fragmentFunctionName:@"iTermBackgroundColorFragmentShader"
           ...
           transientStateClass:[iTermBackgroundColorRendererTransientState
                                   class]];
      _nonblendingRenderer.formatterDelegate = self;

      _compositeOverRenderer = [[iTermMetalCellRenderer alloc]
                initWithDevice:device
            vertexFunctionName:@"iTermBackgroundColorVertexShader"
          fragmentFunctionName:@"iTermBackgroundColorFragmentShader"
...
#endif
    _blendingRenderer = [[iTermMetalCellRenderer alloc]
              initWithDevice:device
          vertexFunctionName:@"iTermBackgroundColorVertexShader"
        fragmentFunctionName:@"iTermBackgroundColorFragmentShader"
    ...
    // TODO: The capacity here is a total guess. But this would be a lot of rows
    // to have.
    _piuPool = [[iTermMetalMixedSizeBufferPool alloc]
        initWithDevice:device
              capacity:512
                  name:@"background color PIU"];
    _infoPool = [[iTermMetalBufferPool alloc]
        initWithDevice:device
...
- (iTermMetalCellRenderer *)rendererForConfiguration:
...
- (nullable __kindof iTermMetalRendererTransientState *)
  ...
  iTermMetalCellRenderer *renderer =
      [self rendererForConfiguration:configuration];
...
- (void)initializeTransientState:
    (iTermBackgroundColorRendererTransientState *)tState {
  tState.vertexBuffer =
      [[self rendererForConfiguration:tState.cellConfiguration]
          newQuadOfSize:tState.cellConfiguration.cellSize
            poolContext:tState.poolContext];
  tState.vertexBuffer.label = @"Vertices";
}

- (id<MTLBuffer>)infoBufferForTransientState:
    (iTermBackgroundColorRendererTransientState *)tState {
  iTermMetalBackgroundColorInfo info;
  memset(&info, 0, sizeof(info));
  info.defaultBackgroundColor = tState.defaultBackgroundColor;
  id<MTLBuffer> buffer =
      [self->_infoPool requestBufferFromContext:tState.poolContext
                                      withBytes:&info
                                 checkIfChanged:YES];
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  [tState enumerateSegments:^(const iTermBackgroundColorPIU *pius,
                              size_t numberOfInstances) {
    id<MTLBuffer> piuBuffer = [self->_piuPool
  ...
  if (index == iTermVertexInputIndexPerInstanceUniforms) {
    ...
    for (int i = 0; i < buffer.length / sizeof(*pius); i++) {
      [s appendFormat:@"offset=(%@, %@) runLength=%@ numRows=%@ color=(%@, %@, "
                      @"%@, %@)\n",
                      @(pius[i].offset.x), @(pius[i].offset.y),
                      @(pius[i].runLength), @(pius[i].numRows),
                      @(pius[i].color.x), @(pius[i].color.y),
                      @(pius[i].color.z), @(pius[i].color.w)];
    }
    NSURL *url = [folder
