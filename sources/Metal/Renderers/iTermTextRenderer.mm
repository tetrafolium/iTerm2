...
// This seems like a good number 🤷. It lets you draw this many *
// iTermTextAtlasCapacity distinct non-ascii characters at one time without
// having to constantly prune and redraw glyphs. That's 64k chars under the
// current values of 16 and 4096.
static const int iTermTextRendererMaximumNumberOfTexturePages = 4096;

// True for macOS 10.14+. Means no subpixel antialiasing, so text blending is
// very simple.
...
- (BOOL)isEqual:(id)object {
  iTermTextRendererCachedQuad *other =
      [iTermTextRendererCachedQuad castFrom:object];
  if (!other) {
    return NO;
  }
...
+ (NSData *)transformedData:(NSData *)input {
  ...
  unsigned char (^load)(int, int, int) =
      ^unsigned char(int text, int background, int bwSample) {
    const size_t i = 256 * (18 * text + background) + bwSample;
    return inputBytes[i];
  };

  NSMutableData *output =
      [NSMutableData uninitializedDataWithLength:18 * 18 * 256];
  unsigned char *outputBytes =
      reinterpret_cast<unsigned char *>(output.mutableBytes);
  void (^store)(int, int, int, unsigned char) =
      ^void(int text, int background, int bwSample, unsigned char value) {
...
+ (void)enumerateGridOfSubpixelModels:(void (^)(float, float,
                                                iTermSubpixelModel *))block {
  ...
  for (float textColor = 0; textColor < 256; textColor += stride) {
    for (float backgroundColor = 0; backgroundColor < 256;
         backgroundColor += stride) {
...
+ (NSData *)subpixelModelData {
  ...
  dispatch_once(&onceToken, ^{
    ...
    [self
        enumerateGridOfSubpixelModels:^(float textColor, float backgroundColor,
                                        iTermSubpixelModel *model) {
...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
  ...
  if (self) {
    ...
    // NOTE: The vertex and fragment function names get changed later. These
    // aren't used but must be valid.
    ...
    _emptyBuffers = [[iTermMetalBufferPool alloc] initWithDevice:device
                                                      bufferSize:1];
    _verticesPool =
        [[iTermMetalBufferPool alloc] initWithDevice:device
                                          bufferSize:sizeof(iTermVertex) * 6];
    _dimensionsPool = [[iTermMetalBufferPool alloc]
        initWithDevice:device
            bufferSize:sizeof(iTermTextureDimensions)];
    _textInfoPool = [[iTermMetalBufferPool alloc]
        initWithDevice:device
            bufferSize:sizeof(iTermVertexTextInfo)];

    // Allow the pool to reserve up to this many bytes. Work backward to find
    // the largest number of buffers we are OK with keeping permanently
    // allocated. By having enough characters on screen you could easily go over
    // this, which will cost a major performance penalty because vertex buffers
    // are slow to create.
    ...
    if (iTermTextIsMonochrome()) {
      ...
      MTLTextureDescriptor *textureDescriptor = [MTLTextureDescriptor
          texture2DDescriptorWithPixelFormat:MTLPixelFormatR8Unorm
                                   ...
                                   mipmapped:NO];
      _models =
          [_cellRenderer.device newTextureWithDescriptor:textureDescriptor];
...
- (nullable __kindof iTermMetalRendererTransientState *)
  ...
  if (!CGSizeEqualToSize(configuration.cellSize, _cellSizeForQuadCache)) {
    // All quads depend on cell size. No point keeping usesless entries in the
    // cache.
    [_quadCache removeAllObjects];
    _cellSizeForQuadCache = configuration.cellSize;
  }
  // NOTE: Any time a glyph overflows its bounds into a neighboring cell it's
  // possible the strokes will intersect. I haven't thought of a way to make
  // that look good yet without having to do one draw pass per overflow glyph
  // that blends using the output of the preceding passes.
...
- (void)initializeTransientState:(iTermTextRendererTransientState *)tState
  ...
  if (_texturePageCollectionSharedPointer != NULL) {
    const vector_uint2 &oldSize =
        _texturePageCollectionSharedPointer.object->get_cell_size();
  ...
  if (!_texturePageCollectionSharedPointer) {
    iTerm2::TexturePageCollection *collection =
        new iTerm2::TexturePageCollection(
            _cellRenderer.device,
    ...
    _texturePageCollectionSharedPointer =
        [[iTermTexturePageCollectionSharedPointer alloc]
            initWithObject:collection];
  }

  tState.device = _cellRenderer.device;
  tState.asciiTextureGroup = _asciiTextureGroup;
  tState.texturePageCollectionSharedPointer =
      _texturePageCollectionSharedPointer;
  tState.numberOfCells = tState.cellConfiguration.gridSize.width *
...
- (id<MTLBuffer>)quadOfSize:(CGSize)size
  ...
  iTermTextRendererCachedQuad *entry =
      [[iTermTextRendererCachedQuad alloc] init];
  entry.cellSize = size;
  ...
  const iTermVertex vertices[] = {
      ...
      {{vw, 0}, {w, 0}}, {{0, 0}, {0, 0}},  {{0, vh}, {0, h}},

      {{vw, 0}, {w, 0}}, {{0, vh}, {0, h}}, {{vw, vh}, {w, h}},
  };
  entry.quad = [_verticesPool requestBufferFromContext:poolContext
                                             withBytes:vertices
                                        checkIfChanged:YES];
  entry.quad.label = @"Text Quad";
  // I cache quads myself, so I don't want them returned to the pool where they
  // could change out from under my cache.
...
static NSString *const FragmentFunctionName(const BOOL &underlined,
  ...
  static NSArray<NSString *> *names = @[
    ...
    @"iTermTextFragmentShaderMonochromeUnderlined", // mono underlined regular
                                                    // text
    @"iTermTextFragmentShaderMonochromeUnderlinedEmoji"
  ]; // mono underlined emoji
  int index =
      (gMonochromeText ? 4 : 0) | (underlined ? 2 : 0) | (emoji ? 1 : 0);
  return names[index];
}
...
- (void)blitFromTemporaryToIntermediateTexture:
    (iTermMetalFrameData *_Nonnull)frameData {
  ...
  [frameData
      updateRenderEncoderWithRenderPassDescriptor:
          frameData.temporaryRenderPassDescriptor
                                             stat:iTermMetalFrameDataStatNA
                                            label:@"Draw more text"];
}

- (id<MTLBuffer>)textInfoBufferForTransientState:
    (iTermTextRendererTransientState *)tState {
  ...
  id<MTLBuffer> buffer =
      [self->_textInfoPool requestBufferFromContext:tState.poolContext
                                          withBytes:&textInfo
                                     checkIfChanged:YES];
...
- (void)drawWithFrameData:(iTermMetalFrameData *)frameData
  ...
  const simd_float2 cellSize =
      simd_make_float2(frameData.cellConfiguration.cellSize.width,
                       frameData.cellConfiguration.cellSize.height);

  // The vertex buffer's texture coordinates depend on the texture map's atlas
  // size so it must be initialized after the texture map.
  ...
  [tState
      enumerateDraws:^(const iTermTextPIU *pius, NSInteger instances,
                       id<MTLTexture> texture, vector_uint2 textureSize,
                       ...
                       BOOL underlined, BOOL emoji) {
        totalInstances += instances;
        __block id<MTLBuffer> vertexBuffer;
        [tState
            measureTimeForStat:iTermTextRendererStatNewQuad
                       ofBlock:^{
                         vertexBuffer = [self
                              quadOfSize:CGSizeMake(glyphSize.x, glyphSize.y)
                             textureSize:CGSizeMake(textureSize.x,
                                                    textureSize.y)
        ...
        if (iTermTextIsMonochrome()) {
          ...
          textures = @{
            @(iTermTextureIndexPrimary) : texture,
        ...
        const float strikethroughThickness =
            strikethroughDescriptor.thickness * scale;
        [tState
            measureTimeForStat:iTermTextRendererStatNewDims
                       ofBlock:^{
                         iTermTextureDimensions textureDimensions = {
                             .textureSize =
                                 simd_make_float2(textureSize.x, textureSize.y),
                             .glyphSize =
                                 simd_make_float2(glyphSize.x, glyphSize.y),
                             .cellSize = cellSize,
                             .underlineOffset =
                                 MAX(underlineThickness,
                                     glyphSize.y -
                                         (underlineDescriptor.offset * scale)),
                             .underlineThickness = underlineThickness,
                             .strikethroughOffset =
                                 glyphSize.y -
                                 strikethroughDescriptor.offset * scale,
                             .strikethroughThickness = strikethroughThickness,
                             .scale = scale,
                         };

                         // These tend to get reused so avoid changing the
                         // buffer if it is the same as the last one.
                         if (previousTextureDimensionsBuffer != nil &&
                             !memcmp(&textureDimensions,
                                     &previousTextureDimensions,
                                     sizeof(textureDimensions))) {
                           textureDimensionsBuffer =
                               previousTextureDimensionsBuffer;
                         } else {
                           textureDimensionsBuffer = [self->_dimensionsPool
                               requestBufferFromContext:tState.poolContext
                                              withBytes:&textureDimensions
                                         checkIfChanged:YES];
                           textureDimensionsBuffer.label =
                               @"Texture dimensions";
                           previousTextureDimensionsBuffer =
                               textureDimensionsBuffer;
        ...
        [tState
            measureTimeForStat:iTermTextRendererStatDraw
                       ofBlock:^{
                         // Change the pipeline state just before drawing so we
                         // get the right underlined/not underlined state.
                         self->_cellRenderer.fragmentFunctionName =
                             FragmentFunctionName(underlined, emoji);
                         self->_cellRenderer.vertexFunctionName =
                             VertexFunctionName(underlined, emoji);
                         tState.pipelineState =
                             [self->_cellRenderer pipelineState];
                         NSDictionary *fragmentBuffers;
                         fragmentBuffers = @{
                           @(iTermFragmentInputIndexTextureDimensions) :
                               textureDimensionsBuffer
                         };
                         [self->_cellRenderer
...
- (void)setASCIICellSize:(CGSize)cellSize
  ...
  iTermASCIITextureGroup *replacement =
      [[iTermASCIITextureGroup alloc] initWithDevice:_cellRenderer.device
                                  creationIdentifier:(id)creationIdentifier
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  NSMutableData *data = [NSMutableData data];
  [self.class enumerateGridOfSubpixelModels:^(float textColor,
                                              float backgroundColor,
                                              iTermSubpixelModel *model) {
    NSString *name = [NSString
        stringWithFormat:@"SubpixelModelGridBwGlyph.t_%02x.b_%02x.dat",
...
- (void)writeVertexBuffer:(id<MTLBuffer>)buffer
  ...
  if (index == iTermVertexInputIndexPerInstanceUniforms) {
    ...
    NSURL *url = [folder
        URLByAppendingPathComponent:
            @"vertexBuffer.iTermVertexInputIndexPerInstanceUniforms.txt"];
