...
NS_INLINE NSString *
iTermMetalUnderlineDescriptorDescription(iTermMetalUnderlineDescriptor *d) {
  return [NSString
      stringWithFormat:@"offset=%@, thickness=%@, color=(%@, %@, %@, %@)",
                       @(d->offset), @(d->thickness), @(d->color.x),
                       @(d->color.y), @(d->color.z), @(d->color.w)];
}

struct iTermMetalBackgroundColorRLE {
  vector_float4 color;
  unsigned short origin; // Not strictly needed but this is needed to binary
                         // search the RLEs
  ...
  bool operator<(const int &other) const { return origin < other; }
#endif
};
#if __cplusplus
inline bool operator<(const int &origin,
                      const iTermMetalBackgroundColorRLE &other) {
  return origin < other.origin;
}
#endif
...
NS_INLINE NSString *
iTermMetalBackgroundColorRLEDescription(const iTermMetalBackgroundColorRLE *c) {
  return [NSString
      stringWithFormat:@"color=(%0.2f, %0.2f, %0.2f, %0.2f) origin=%d count=%d",
