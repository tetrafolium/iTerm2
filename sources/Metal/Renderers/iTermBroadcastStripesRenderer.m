...
@interface iTermBroadcastStripesRendererTransientState
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  [[NSString stringWithFormat:@"size=%@", NSStringFromSize(self.size)]
  ...
  if (self) {
    _metalRenderer = [[iTermMetalRenderer alloc]
        ...
        fragmentFunctionName:@"iTermBroadcastStripesFragmentShader"
         ...
         transientStateClass:[iTermBroadcastStripesRendererTransientState
                                 class]];
    NSImage *image = [[NSBundle bundleForClass:self.class]
        imageForResource:@"BackgroundStripes"];
    _size = image.size;
...
- (void)initializeTransientState:
    (iTermBroadcastStripesRendererTransientState *)tState {
