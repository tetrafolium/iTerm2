...
- (void)enumerateRows:(void (^)(int row, iTermTimestampKey *key,
                                NSRect frame))block;
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  NSMutableString *s =
      [NSMutableString stringWithFormat:@"backgroundColor=%@\ntextColor=%@\n",
                                        _backgroundColor, _textColor];
  [_timestamps enumerateObjectsUsingBlock:^(
                   NSDate *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
    [s appendFormat:@"%@\n", obj];
  }];
...
- (void)enumerateRows:(void (^)(int row, iTermTimestampKey *key,
  ...
  if (!_drawHelper) {
    _drawHelper = [[iTermTimestampDrawHelper alloc]
        initWithBackgroundColor:_backgroundColor
  ...
  const vector_float4 textColor =
      simd_make_float4(_textColor.redComponent, _textColor.greenComponent,
                       _textColor.blueComponent, _textColor.alphaComponent);
  const vector_float4 backgroundColor = simd_make_float4(
      _backgroundColor.redComponent, _backgroundColor.greenComponent,
  ...
  [_timestamps enumerateObjectsUsingBlock:^(
                   NSDate *_Nonnull date, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    key.date = [self->_drawHelper rowIsRepeat:idx]
                   ? -1
                   : round(date.timeIntervalSinceReferenceDate);
    block(idx, key,
          NSMakeRect(self.configuration.viewportSize.x / scale - visibleWidth,
                     self.configuration.viewportSize.y / scale -
                         ((idx + 1) * rowHeight) - vmargin,
                     visibleWidth, rowHeight));
  }];
}
...
- (NSImage *)imageForRow:(int)row {
  NSSize size = NSMakeSize(_drawHelper.suggestedWidth,
                           self.cellConfiguration.cellSize.height /
                               self.cellConfiguration.scale);
  assert(size.width * size.height > 0);
  NSImage *image = [[NSImage alloc] initWithSize:size];
  [image lockFocusFlipped:YES];
  NSGraphicsContext *context = [NSGraphicsContext currentContext];
  iTermSetSmoothing(context.CGContext, NULL, self.useThinStrokes,
                    self.antialiased);
  ...
  [tState enumerateRows:^(int row, iTermTimestampKey *key, NSRect frame) {
    ...
    if (!pooledTexture) {
      ...
      id<MTLTexture> texture =
          [self->_cellRenderer textureFromImage:image
                                        context:context
                                           pool:self->_texturePool];
      assert(texture);
      pooledTexture =
          [[iTermPooledTexture alloc] initWithTexture:texture
                                                 pool:self->_texturePool];
    ...
    if (pooledTexture.texture.width <
        tState.configuration.viewportSize.x + slop) {
      overflow = 0;
    } else {
      overflow = pooledTexture.texture.width -
                 tState.configuration.viewportSize.x - slop;
    }
    tState.vertexBuffer = [self->_cellRenderer
        newQuadWithFrame:CGRectMake(frame.origin.x * scale + overflow,
    ...
    [self->_cellRenderer drawWithTransientState:tState
        ...
        fragmentBuffers:@{}
        textures:@{
          @(iTermTextureIndexPrimary) : pooledTexture.texture
        }];
  }];
}
