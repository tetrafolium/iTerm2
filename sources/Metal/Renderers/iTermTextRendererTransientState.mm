...
const vector_float4 iTermAnnotationUnderlineColor =
...
static const size_t iTermPIUArrayIndexUnderlined =
    1; // This can be used as the Underlined bit in this bitmask
static const size_t iTermPIUArrayIndexNotUnderlinedEmoji =
    2; // This can be used as the Emoji bit in this bitmask
...
@implementation iTermTextRendererTransientState {
  // Data's bytes contains a C array of iTermMetalBackgroundColorRLE with
  // background colors.
  ...
  iTerm2::PIUArray<iTermTextPIU> _asciiPIUArrays[iTermPIUArraySize]
                                                [iTermNumberOfPIUArrays];
  iTerm2::PIUArray<iTermTextPIU> _asciiOverflowArrays[iTermPIUArraySize]
                                                     [iTermNumberOfPIUArrays];

  // Array of PIUs for each texture page.
  std::map<iTerm2::TexturePage *, iTerm2::PIUArray<iTermTextPIU> *>
      _pius[iTermPIUArraySize];

  iTermPreciseTimerStats _stats[iTermTextRendererStatCount];
}

- (instancetype)initWithConfiguration:
    (__kindof iTermRenderConfiguration *)configuration {
...
+ (NSString *)formatTextPIU:(iTermTextPIU)a {
  return [NSString
      stringWithFormat:@"offset=(%@, %@) "
                       @"textureOffset=(%@, %@) "
                       @"backgroundColor=(%@, %@, %@, %@) "
                       @"textColor=(%@, %@, %@, %@) "
                       @"underlineStyle=%@ "
                       @"underlineColor=(%@, %@, %@, %@)\n",
                       @(a.offset.x), @(a.offset.y), @(a.textureOffset.x),
                       @(a.textureOffset.y), @(a.backgroundColor.x),
                       @(a.backgroundColor.y), @(a.backgroundColor.z),
                       @(a.backgroundColor.w), @(a.textColor.x),
                       @(a.textColor.y), @(a.textColor.z), @(a.textColor.w),
                       @(a.underlineStyle), @(a.underlineColor.x),
                       @(a.underlineColor.y), @(a.underlineColor.z),
                       @(a.underlineColor.w)];
}
...
- (void)writeDebugInfoToFolder:(NSURL *)folder {
  ...
  [_modelData writeToURL:[folder URLByAppendingPathComponent:@"model.bin"]
              atomically:NO];

  @autoreleasepool {
    NSMutableString *s = [NSMutableString string];
    [_backgroundColorRLEDataArray
        enumerateObjectsUsingBlock:^(iTermData *_Nonnull data, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
          const iTermMetalBackgroundColorRLE *rle =
              (const iTermMetalBackgroundColorRLE *)data.bytes;
          [s appendFormat:@"%4d: %@\n", (int)idx,
                          iTermMetalBackgroundColorRLEDescription(rle)];
        }];
    [s writeToURL:[folder URLByAppendingPathComponent:@"backgroundColors.txt"]
        atomically:NO
  ...
  @autoreleasepool {
    ...
    [s writeToURL:[folder URLByAppendingPathComponent:@"fixups.txt"]
        atomically:NO
          encoding:NSUTF8StringEncoding
             error:nil];
  }
  ...
  @autoreleasepool {
    for (int k = 0; k < iTermPIUArraySize; k++) {
      for (int i = 0;
           i < sizeof(_asciiPIUArrays[k]) / sizeof(**_asciiPIUArrays); i++) {
        NSMutableString *s = [NSMutableString string];
        ...
        NSMutableString *name =
            [NSMutableString stringWithFormat:@"asciiPIUs.CenterPart."];
  ...
  @autoreleasepool {
    for (int k = 0; k < iTermPIUArraySize; k++) {
      for (int i = 0;
           i < sizeof(_asciiOverflowArrays[k]) / sizeof(**_asciiOverflowArrays);
           i++) {
        NSMutableString *s = [NSMutableString string];
        ...
        NSMutableString *name =
            [NSMutableString stringWithFormat:@"asciiPIUs.Overflow."];
        if (i & iTermASCIITextureAttributesBold) {
          [name appendString:@"B"];
        }
        if (i & iTermASCIITextureAttributesItalic) {
  ...
  NSString *s = [NSString
      stringWithFormat:
          ...
          iTermMetalUnderlineDescriptorDescription(
              &_nonAsciiUnderlineDescriptor),
          iTermMetalUnderlineDescriptorDescription(
              &_strikethroughUnderlineDescriptor),
          @(_defaultBackgroundColor.x), @(_defaultBackgroundColor.y),
          @(_defaultBackgroundColor.z), @(_defaultBackgroundColor.w)];
...
- (NSString *)nameForStat:(int)i {
  return [@[
    @"text.newQuad", @"text.newPIU", @"text.newDims", @"text.info",
    @"text.subpixel", @"text.draw"
  ] objectAtIndex:i];
}
...
- (void)
    enumerateASCIIDrawsFromArrays:(iTerm2::PIUArray<iTermTextPIU> *)piuArrays
                       underlined:(BOOL)underlined
                            emoji:(BOOL)emoji
                            block:(void (^)(const iTermTextPIU *, NSInteger,
                                            id<MTLTexture>, vector_uint2,
                                            vector_uint2,
  ...
  for (int i = 0; i < iTermNumberOfPIUArrays; i++) {
    ...
    iTermASCIITexture *asciiTexture = [_asciiTextureGroup
        asciiTextureForAttributes:(iTermASCIITextureAttributes)i];
    ...
    for (int j = 0; j < n; j++) {
      if (piuArrays[i].size_of_segment(j) > 0) {
        block(piuArrays[i].start_of_segment(j), piuArrays[i].size_of_segment(j),
              ...
              _asciiUnderlineDescriptor, _strikethroughUnderlineDescriptor,
              underlined, emoji);
      }
    }
  }
...
- (size_t)enumerateNonASCIIDraws:
    (void (^)(const iTermTextPIU *, NSInteger, id<MTLTexture>, vector_uint2,
  ...
  for (size_t k = 0; k < iTermPIUArraySize; k++) {
    for (auto const &mapPair : _pius[k]) {
      ...
      for (size_t i = 0; i < piuArray->get_number_of_segments(); i++) {
        ...
        if (count > 0) {
          sum += count;
          block(piuArray->start_of_segment(i), count,
                texturePage->get_texture(), texturePage->get_atlas_size(),
                texturePage->get_cell_size(), _nonAsciiUnderlineDescriptor,
                _strikethroughUnderlineDescriptor,
...
- (void)enumerateASCIIDrawsFromArrays:
            (iTerm2::PIUArray<iTermTextPIU>[iTermPIUArraySize]
                                           [iTermNumberOfPIUArrays])piuArrays
                                block:(void (^)(const iTermTextPIU *, NSInteger,
                                                id<MTLTexture>, vector_uint2,
                                                vector_uint2,
                                                iTermMetalUnderlineDescriptor,
  ...
  [self enumerateASCIIDrawsFromArrays:piuArrays[iTermPIUArrayIndexUnderlined]
                           underlined:true
                                ...
                                block:block];
  [self
      enumerateASCIIDrawsFromArrays:piuArrays[iTermPIUArrayIndexUnderlinedEmoji]
                         underlined:true
                              emoji:true
                              block:block];
  [self enumerateASCIIDrawsFromArrays:piuArrays[iTermPIUArrayIndexNotUnderlined]
                           underlined:false
                                emoji:false
                                block:block];
  [self enumerateASCIIDrawsFromArrays:piuArrays
                                          [iTermPIUArrayIndexNotUnderlinedEmoji]
                           underlined:false
                                emoji:true
                                block:block];
}
...
- (void)willDraw {
  DLog(@"WILL DRAW %@", self);
  // Fix up the background color of parts of glyphs that are drawn outside their
  // cell. Add to the correct page's PIUs.
  ...
  for (auto pair : _fixups) {
    ...
    for (auto fixup : *fixups) {
      iTerm2::PIUArray<iTermTextPIU> &piuArray =
          *_pius[fixup.outerPIUIndex][page];
      iTermTextPIU &piu = piuArray.get(fixup.piu_index);

      // Set fields in piu
      if (fixup.y >= 0 && fixup.y < numRows && fixup.x >= 0 &&
          fixup.x < width) {
        iTermData *data = _backgroundColorRLEDataArray[fixup.y];
        const iTermMetalBackgroundColorRLE *backgroundRLEs =
            (iTermMetalBackgroundColorRLE *)data.bytes;
        ...
        //
        // Upper bound always gives you one past what you wanted. Lower bound is
        // not consistent.
        auto it = std::upper_bound(backgroundRLEs, backgroundRLEs + rleCount,
...
NS_INLINE iTermTextPIU *iTermTextRendererTransientStateAddASCIIPart(
    iTermTextPIU *piu, char code, float w, float h, iTermASCIITexture *texture,
    float cellWidth, int x, CGSize asciiOffset, iTermASCIITextureOffset offset,
    vector_float4 textColor, vector_float4 backgroundColor,
    iTermMetalGlyphAttributesUnderline underlineStyle,
    vector_float4 underlineColor) {
  piu->offset =
      simd_make_float2(x * cellWidth + asciiOffset.width, asciiOffset.height);
  const int index = iTermASCIITextureIndexOfCode(code, offset);
  MTLOrigin origin =
      iTermTextureArrayOffsetForIndex(texture.textureArray, index);
...
static inline int iTermOuterPIUIndex(const bool &annotation,
                                     ...
                                     const bool &emoji) {
  const int underlineBit =
      (annotation || underlined) ? iTermPIUArrayIndexUnderlined : 0;
...
- (void)addASCIICellToPIUsForCode:(char)code
  ...
  // When profiling, objc_retain and objc_release took 20% of the time in this
  // function, which is called super frequently. It should be safe not to retain
  // the texture because the texture group retains it. Once a texture is set in
  // the group it won't be removed or replaced.
  __unsafe_unretained iTermASCIITexture *texture =
      _asciiTextureGroup->_textures[asciiAttrs];
  if (!texture) {
  ...
  const int outerPIUIndex =
      iTermOuterPIUIndex(hasAnnotation, hasUnderline, false);
  if (hasAnnotation) {
    underlineColor = iTermAnnotationUnderlineColor;
  } else if (hasUnderline) {
    underlineColor = _asciiUnderlineDescriptor.color.w > 0
                         ? _asciiUnderlineDescriptor.color
  ...
  if (iTermTextIsMonochrome()) {
    // There is only a center part for ASCII on Mojave because the glyph size is
    // increased to contain the largest ASCII glyph.
    iTermTextRendererTransientStateAddASCIIPart(
        _asciiPIUArrays[outerPIUIndex][asciiAttrs].get_next(), code, w, h,
        texture, cellWidth, x, asciiOffset, iTermASCIITextureOffsetCenter,
        textColor, attributes[x].backgroundColor, underlineStyle,
        underlineColor);
    return;
  }
  // Pre-10.14, ASCII glyphs can get chopped up into multiple parts. This is
  // necessary so subpixel AA will work right.
  ...
  if (parts & iTermASCIITexturePartsLeft) {
    if (x > 0) {
      // Normal case
      iTermTextRendererTransientStateAddASCIIPart(
          _asciiOverflowArrays[outerPIUIndex][asciiAttrs].get_next(), code, w,
          h, texture, cellWidth, x - 1, asciiOffset,
      ...
      iTermTextRendererTransientStateAddASCIIPart(
          _asciiOverflowArrays[outerPIUIndex][asciiAttrs].get_next(), code, w,
          h, texture, cellWidth, x - 1, asciiOffset,
          iTermASCIITextureOffsetLeft, textColor, _defaultBackgroundColor,
          iTermMetalGlyphAttributesUnderlineNone, underlineColor);
    }
  }
  ...
  iTermTextRendererTransientStateAddASCIIPart(
      _asciiPIUArrays[outerPIUIndex][asciiAttrs].get_next(), code, w, h,
      texture, cellWidth, x, asciiOffset, iTermASCIITextureOffsetCenter,
      textColor, attributes[x].backgroundColor, underlineStyle, underlineColor);
  ...
  if (parts & iTermASCIITexturePartsRight) {
    ...
    if (x < lastColumn) {
      ...
      iTermTextRendererTransientStateAddASCIIPart(
          _asciiOverflowArrays[outerPIUIndex][asciiAttrs].get_next(), code, w,
          h, texture, cellWidth, x + 1, asciiOffset,
          iTermASCIITextureOffsetRight, attributes[x].foregroundColor,
          attributes[x + 1].backgroundColor,
          iTermMetalGlyphAttributesUnderlineNone, underlineColor);
    } else {
      ...
      iTermTextRendererTransientStateAddASCIIPart(
          ...
          h, texture, cellWidth, x + 1, asciiOffset,
          iTermASCIITextureOffsetRight, attributes[x].foregroundColor,
          _defaultBackgroundColor, iTermMetalGlyphAttributesUnderlineNone,
          underlineColor);
    }
  }
}
...
- (void)setGlyphKeysData:(iTermGlyphKeyData *)glyphKeysData
  ...
  const iTermMetalGlyphKey *glyphKeys =
      (iTermMetalGlyphKey *)glyphKeysData.bytes;
  const iTermMetalGlyphAttributes *attributes =
      (iTermMetalGlyphAttributes *)attributesData.bytes;
  ...
  const float verticalShift =
      round(
          (cellHeight - self.cellConfiguration.cellSizeWithoutSpacing.height) /
          (2 * self.configuration.scale)) *
      self.configuration.scale;
  const float yOffset =
      (self.cellConfiguration.gridSize.height - row - 1) * cellHeight +
      verticalShift;
  ...
  for (int x = 0; x < count; x++) {
    ...
    if (GlyphKeyCanTakeASCIIFastPath(glyphKeys[x])) {
      ...
      iTermASCIITextureAttributes asciiAttrs =
          iTermASCIITextureAttributesFromGlyphKeyTypeface(
              glyphKeys[x].typeface, glyphKeys[x].thinStrokes);
      [self addASCIICellToPIUsForCode:glyphKeys[x].code
                                    x:x
                               offset:CGSizeMake(asciiXOffset,
                                                 yOffset + asciiYOffset)
      ...
      std::vector<const iTerm2::GlyphEntry *> *entries =
          _texturePageCollectionSharedPointer.object->find(glyphKey);
      if (!entries) {
        entries = _texturePageCollectionSharedPointer.object->add(
            x, glyphKey, context, creation);
        if (!entries) {
          continue;
        }
      ...
      const bool &hasAnnotation = attributes[x].annotation;
      const bool hasUnderline = attributes[x].underlineStyle !=
                                iTermMetalGlyphAttributesUnderlineNone;
      const iTerm2::GlyphEntry *firstGlyphEntry = (*entries)[0];
      const int outerPIUIndex = iTermOuterPIUIndex(hasAnnotation, hasUnderline,
      ...
      for (auto entry : *entries) {
        ...
        if (it == _pius[outerPIUIndex].end()) {
          array = _pius[outerPIUIndex][entry->_page] =
              new iTerm2::PIUArray<iTermTextPIU>(_numberOfCells);
        } else {
          array = it->second;
        }
        ...
        vector_float2 reciprocal_atlas_size =
            entry->_page->get_reciprocal_atlas_size();
        piu->textureOffset =
            simd_make_float2(origin.x * reciprocal_atlas_size.x,
        ...
        if (attributes[x].annotation) {
          ...
          piu->underlineStyle = iTermMetalGlyphAttributesUnderlineSingle;
          piu->underlineColor = _nonAsciiUnderlineDescriptor.color.w > 1
                                    ? _nonAsciiUnderlineDescriptor.color
                                    : piu->textColor;
        } else {
        ...
        if (part != iTermTextureMapMiddleCharacterPart &&
          ...
          // Only underline center part and its right neighbor of the character.
          // There are weird artifacts otherwise, such as floating underlines
          // (for parts above and below) or doubly drawn underlines.
        ...
        if (entry->_part == iTermTextureMapMiddleCharacterPart) {
          ...
          iTermTextFixup fixup = {.piu_index = array->size() - 1,
                                  .x = x + dx,
                                  .y = row + dy,
...
- (void)didComplete {
  ...
  _texturePageCollectionSharedPointer.object
      ->prune_if_needed(); // The static analyzer wrongly says this is a
                           // use-after-free.
...
- (nonnull NSMutableData *)modelData {
  if (_modelData == nil) {
    _modelData = [[NSMutableData alloc]
        initWithUninitializedLength:sizeof(iTermTextPIU) *
