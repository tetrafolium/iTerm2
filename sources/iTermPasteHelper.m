...
+ (NSMutableCharacterSet *)unsafeControlCodeSet {
  ...
  [controlSet
      removeCharactersInRange:NSMakeRange(12,
                                          2)]; // Form feed and carriage return
  return controlSet;
}
...
+ (void)togglePromptToConvertTabsToSpacesWhenPasting {
  if (![self promptToConvertTabsToSpacesWhenPasting]) {
    ...
    [iTermWarning setIdentifier:@"AboutToPasteTabsWithCancel"
             permanentSelection:kiTermWarningSelection0];
  }
}
...
- (void)showPasteOptionsInWindow:(NSWindow *)window
  ...
  int chunkSize =
      [iTermPreferences intForKey:kPreferenceKeyPasteSpecialChunkSize];
  NSTimeInterval chunkDelay =
      [iTermPreferences floatForKey:kPreferenceKeyPasteSpecialChunkDelay];
  [iTermPasteSpecialWindowController
      showAsPanelInWindow:window
                chunkSize:chunkSize
...
+ (void)sanitizePasteEvent:(PasteEvent *)pasteEvent
  ...
  if (flags & kPasteFlagsRemovingNewlines) {
    theString = [[theString stringByReplacingOccurrencesOfString:@"\r"
                                                      withString:@""]
  ...
  if (flags & kPasteFlagsConvertUnicodePunctuation) {
    theString = [theString
        stringByReplacingOccurrencesOfString:
            kPasteSpecialViewControllerUnicodeDoubleQuotesRegularExpression
                                  withString:@"\""
                                     options:NSRegularExpressionSearch
                                       range:NSMakeRange(0, theString.length)];
    theString = [theString
        stringByReplacingOccurrencesOfString:
            kPasteSpecialViewControllerUnicodeSingleQuotesRegularExpression
                                  withString:@"'"
                                     options:NSRegularExpressionSearch
                                       range:NSMakeRange(0, theString.length)];
    theString = [theString
        stringByReplacingOccurrencesOfString:
            kPasteSpecialViewControllerUnicodeDashesRegularExpression
                                  withString:@"-"
                                     options:NSRegularExpressionSearch
                                       range:NSMakeRange(0, theString.length)];
  }
  ...
  if (flags & kPasteFlagsRemovingUnsafeControlCodes) {
    // All control codes except tab (9), newline (10), form feed (12), carriage
    // return (13), and shell escape (22) are removed. This must be done before
    // transforming tabs since that transformation might add ^Vs.
    theString = [[theString
        componentsSeparatedByCharactersInSet:[iTermPasteHelper
                                                 unsafeControlCodeSet]]
  ...
  if (flags & kPasteFlagsEscapeSpecialCharacters) {
    ...
    if (pasteEvent.tabTransform != kTabTransformEscapeWithCtrlV) {
      theString =
          [theString stringWithEscapedShellCharactersIncludingNewlines:NO];
    } else {
      // Avoid double-escaping tabs with both ^V and backslash.
      theString =
          [theString stringWithEscapedShellCharactersExceptTabAndNewline];
    }
  }

  if (pasteEvent.flags & kPasteFlagsUseRegexSubstitution &&
      pasteEvent.regex.length > 0) {
    NSString *replacement = nil;
    @try {
      replacement =
          [theString
              stringByReplacingOccurrencesOfRegex:pasteEvent.regex ?: @""
                                       withString:
                                           [pasteEvent.substitution
      ...
      NSLog(@"Exception with s/%@/%@/g: %@", pasteEvent.regex ?: @"",
            pasteEvent.substitution ?: @"", exception);
    }
  }
...
- (void)pasteString:(NSString *)theString stringConfig:(NSString *)jsonConfig {
  PasteEvent *pasteEvent =
      [iTermPasteSpecialViewController pasteEventForConfig:jsonConfig
                                                    string:theString];
...
- (PasteEvent *)pasteEventWithString:(NSString *)theString
  ...
  NSUInteger bracketFlag =
      [_delegate pasteHelperShouldBracket] ? kPasteFlagsBracket : 0;
  NSUInteger flags = (kPasteFlagsSanitizingNewlines |
                      kPasteFlagsRemovingUnsafeControlCodes | bracketFlag);
  if (escapeShellChars) {
...
// this needs to take the delay, chunk size, key names, and the exact flags it
// wants
- (void)tryToPasteEvent:(PasteEvent *)pasteEvent {
  DLog(@"-[iTermPasteHelper pasteString:flags:");
  DLog(@"length=%@, flags=%@", @(pasteEvent.string.length),
       @(pasteEvent.flags));
  ...
  if (!(pasteEvent.flags &
        (kPasteFlagsCommands | kPasteFlagsDisableWarnings))) {
...
- (void)pasteEventImmediately:(PasteEvent *)pasteEvent {
  // A queued up paste command might have wanted bracketing but the host might
  ...
  [iTermPasteHelper sanitizePasteEvent:pasteEvent
                              encoding:[_delegate pasteHelperEncoding]];
  ...
  if ([pasteEvent.string length] == 0) {
    DLog(@"Beep: Tried to paste 0-byte string (became 0 length after removing "
         @"controls). Beep.");
    NSBeep();
    return;
  }
...
// Outputs 16 bytes every 125ms so that clients that don't buffer input can
// handle pasting large buffers. Override the constants by setting defaults
// SlowPasteBytesPerCall and SlowPasteDelayBetweenCalls
- (void)pasteSlowly:(NSString *)theString {
  DLog(@"pasteSlowly length=%@", @(theString.length));
  PasteEvent *pasteEvent =
      [PasteEvent pasteEventWithString:theString
                                 flags:0
                      defaultChunkSize:16
                              chunkKey:@"SlowPasteBytesPerCall"
                          defaultDelay:0.125
...
- (void)pasteNormally:(NSString *)aString {
  ...
  // thing? See bug 1031.
  PasteEvent *pasteEvent =
      [PasteEvent pasteEventWithString:aString
                                 flags:0
                      defaultChunkSize:iTermQuickPasteBytesPerCallDefaultValue
                              chunkKey:@"QuickPasteBytesPerCall"
                          defaultDelay:0.01
...
- (NSInteger)normalChunkSize {
  NSNumber *n = [[NSUserDefaults standardUserDefaults]
      objectForKey:@"QuickPasteBytesPerCall"];
  if (!n) {
...
- (NSTimeInterval)normalDelay {
  NSNumber *n = [[NSUserDefaults standardUserDefaults]
      objectForKey:@"QuickPasteDelayBetweenCalls"];
  if (!n) {
    return 0.01;
  } else {
...
- (void)pasteNextChunkAndScheduleTimer {
  ...
  if (range.length > 0) {
    if (_pasteContext.blockAtNewline) {
      // If there is a newline in the range about to be pasted, only paste up to
      // and including it and the block to YES.
...
- (void)scheduleNextPasteForCurrentPasteContext {
  [_timer invalidate];
  _timer = [self
      scheduledTimerWithTimeInterval:_pasteContext.delayBetweenCalls
                              target:self
...
- (void)pasteLiteralEventUnconditionallyImmediately:(PasteEvent *)pasteEvent {
  ...
  const NSTimeInterval sumOfDelays = _pasteContext.delayBetweenCalls *
                                     _buffer.length /
                                     _pasteContext.bytesPerCall;
  ...
  if (!pasteEvent.isUpload) {
    if ((sumOfDelays + timeSpentWriting >
         kMinEstimatedPasteTimeToShowIndicator) ||
        _pasteContext.blockAtNewline) {
      [self showPasteIndicatorInView:[_delegate pasteHelperViewForIndicator]
             statusBarViewController:[_delegate
                                         pasteHelperStatusBarViewController]];
    }
  }

  if (_pasteContext.blockAtNewline &&
      [_delegate pasteHelperShouldWaitForPrompt]) {
...
- (BOOL)maybeWarnAboutMultiLinePaste:(PasteEvent *)pasteEvent {
  const int limit =
      [iTermAdvancedSettingsModel alwaysWarnBeforePastingOverSize];
  if (limit >= 0) {
    if (pasteEvent.string.length > limit) {
      ...
      const iTermWarningSelection selection = [iTermWarning
          showWarningWithTitle:
              [NSString
                  stringWithFormat:@"OK to paste %@ characters?",
                                   [numberFormatter
                                       stringFromNumber:@(pasteEvent.string
                                                              .length)]]
                   ...
                   silenceable:kiTermWarningTypePersistent
                       heading:@"Paste Limit Exceeded"
                        window:self.delegate.pasteHelperViewForIndicator
                                   .window];
      switch (selection) {
  ...
  NSRange rangeOfFirstNewline =
      [pasteEvent.string rangeOfCharacterFromSet:newlineCharacterSet];
  if (rangeOfFirstNewline.length == 0) {
    return YES;
  }
  if ([iTermAdvancedSettingsModel
          suppressMultilinePasteWarningWhenPastingOneLineWithTerminalNewline] &&
      rangeOfFirstNewline.location == pasteEvent.string.length - 1) {
    return YES;
  }
  BOOL atShellPrompt = [_delegate pasteHelperIsAtShellPrompt];
  if ([iTermAdvancedSettingsModel
          suppressMultilinePasteWarningWhenNotAtShellPrompt] &&
      !atShellPrompt) {
    return YES;
  }
  ...
  NSArray *lines =
      [pasteEvent.string componentsSeparatedByRegex:@"(?:\r\n)|(?:\r)|(?:\n)"];
  NSString *theTitle;
  NSMutableArray<iTermWarningAction *> *actions = [NSMutableArray array];

  __block BOOL result = YES;
  iTermWarningAction *cancel = [iTermWarningAction
      warningActionWithLabel:@"Cancel"
                       block:^(iTermWarningSelection selection) {
                         result = NO;
                       }];
  iTermWarningAction *paste = [iTermWarningAction
      warningActionWithLabel:@"Paste"
                       block:^(iTermWarningSelection selection) {
                         result = YES;
                       }];
  iTermWarningAction *pasteWithoutNewline = [iTermWarningAction
      warningActionWithLabel:@"Paste Without Newline"
                       block:^(iTermWarningSelection selection) {
  ...
  if (lines.count > 1) {
    if (atShellPrompt) {
      theTitle =
          [NSString stringWithFormat:@"OK to paste %d lines at shell prompt?",
                                     (int)[lines count]];
    } else {
      prompt = [iTermAdvancedSettingsModel promptForPasteWhenNotAtPrompt];
      theTitle = [NSString
          stringWithFormat:@"OK to paste %d lines?", (int)[lines count]];
    }
  } else {
    [actions insertObject:pasteWithoutNewline atIndex:1];
    if (atShellPrompt) {
      identifier = [iTermAdvancedSettingsModel
  ...
  [iTermWarning unsilenceIdentifier:identifier
                  ifSelectionEquals:[actions indexOfObjectIdenticalTo:cancel]];
  [actions
      addObject:[iTermWarningAction
                    warningActionWithLabel:@"Advanced…"
                                     block:^(iTermWarningSelection selection) {
                                       PTYSessionPasteFlags flags = 0;
                                       if (pasteEvent.slow) {
                                         flags |= kPTYSessionPasteSlowly;
                                         // The other two flags do not appear to
                                         // be used
                                       }
...
- (void)showAdvancedPasteWithFlags:(PTYSessionPasteFlags)flags {
  PasteEvent *temporaryEvent = [self
      pasteEventWithString:@""
          ...
          escapeShellChars:!!(flags & kPTYSessionPasteEscapingSpecialCharacters)
              ...
              spacesPerTab:4
                  progress:nil];
  PasteContext *temporaryContext =
      [[PasteContext alloc] initWithPasteEvent:temporaryEvent];
  [iTermPasteSpecialWindowController
      showAsPanelInWindow:self.delegate.pasteHelperViewForIndicator.window
...
- (int)numberOfSpacesToConvertTabsTo:(NSString *)source {
  if ([source rangeOfString:@"\t"].location != NSNotFound) {
    ...
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:@"You're about to paste a string with tabs."
                     actions:@[
                       @"OK", @"Cancel", @"Convert tabs to spaces", @"Advanced…"
                     ]
