...
NSString *const iTermWindowOcclusionDidChange =
    @"iTermWindowOcclusionDidChange";
...
- (void)invalidateCachedOcclusion:(NSNotification *)notification {
  DLog(@"Invalidate occlusion cache because of notification %@",
       notification.name);
...
- (void)invalidateCachedOcclusion {
  ...
  [_rateLimit performRateLimitedBlock:^{
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermWindowOcclusionDidChange
                      object:nil];
  }];
}
