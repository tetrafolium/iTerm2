...
// A LineBuffer represents an ordered collection of strings of screen_char_t.
// Each string forms a logical line of text plus color information. Logic is
// provided for the following major functions:
//   - If the lines are wrapped onto a screen of some width, find the Nth
//   wrapped line
//   - Append a line
//   - If the lines are wrapped onto a screen of some width, remove the last
//   wrapped line
//   - Store and retrieve a cursor position
//   - Store an unlimited or a fixed number of wrapped lines
// The implementation uses an array of small blocks that hold a few kb of
// unwrapped lines. Each block caches some information to speed up repeated
// lookups with the same screen width.
...
// Call this immediately after init. Otherwise the buffer will hold unlimited
// lines (until you run out of memory).
- (void)setMaxLines:(int)maxLines;

// Add a line to the buffer. Set partial to true if there's more coming for this
// line: that is to say, this buffer contains only a prefix or infix of the
// entire line.
//
// NOTE: call dropExcessLinesWithWidth after this if you want to limit the
// buffer to max_lines.
...
// If more lines are in the buffer than max_lines, call this function. It will
// adjust the count of excess lines and try to free the first block(s) if they
// are unused. Because this could happen after any call to appendLines, you
// should always call this after appendLines.
//
...
// Returns the timestamp associated with a line when wrapped to the specified
// width.
...
// the first 'width' characters will be modified. 0 <= lineNum <
// numLinesWithWidth:width Returns EOL code. DEPRECATED, use
// wrappedLineAtIndex:width: instead.
- (int)copyLineToBuffer:(screen_char_t *)buffer
                  width:(int)width
                lineNum:(int)lineNum
           continuation:(screen_char_t *)continuationPtr;

// Like the above but with a saner way of holding the returned data. Callers are
// advised not to modify the screen_char_t's returned, but the ScreenCharArray
// is otherwise safe to mutate. |continuation| is optional and if set will be
// filled in with the continuation character.
...
// Copy up to width chars from the last line into *ptr. The last line will be
// removed or truncated from the buffer. Sets *includesEndOfLine to true if this
// line should have a continuation marker.
...
// Save the cursor position. Call this just before appending the line the cursor
// is in. x gives the offset from the start of the next line appended. The
// cursor position is invalidated if dropExcessLinesWithWidth is called.
- (void)setCursor:(int)x;

// If the last wrapped line has the cursor, return true and set *x to its
// horizontal position. 0 <= *x <= width (if *x == width then the cursor is
// actually on the next line). Call this just before
// popAndCopyLastLineInto:width:includesEndOfLine:timestamp:continuation.
...
// Performs a search. Use prepareToSearchFor:startingAt:options:withContext: to
// initialize the FindContext prior to calling this.
- (void)findSubstring:(FindContext *)context
               stopAt:(LineBufferPosition *)stopAt;
...
// Returns a dictionary with the contents of the line buffer. If it is more than
// 10k lines @ 80 columns then it is truncated. The data is a weak reference and
// will be invalid if the line buffer is changed.
...
// Make a copy of the last |minLines| at width |width|. May copy more than
// |minLines| for speed.
