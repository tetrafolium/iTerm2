...
- (void)update {
  [_publisher
      publish:@((double)self.memoryUsage / (double)self.availableMemory)];
}
...
- (long long)memoryUsage {
  ...
  kern_return_t status = host_statistics64(host_port, HOST_VM_INFO64,
                                           (host_info64_t)&vm_stat, &host_size);
  if (status != KERN_SUCCESS) {
    return 0;
  }

  return self.pageSize * (vm_stat.internal_page_count -
                          vm_stat.purgeable_count + vm_stat.wire_count);
}
