...
// and not selectable. It is not copied into the clipboard. A line ending in
// this character should always have EOL_DWC. These are stripped when adding a
// line to the scrollback buffer.
...
// If DWC_SKIP appears in the input, we convert it to this to avoid causing
// confusion. NOTE: I think this isn't used because DWC_SKIP is caught early and
// converted to a '?'.
...
#define ONECHAR_UNKNOWN                                                        \
  ('?') // Replacement character for encodings other than utf-8.
...
// Use default foreground/background, but use default background for foreground
// and default foreground for background (reverse video).
...
typedef struct screen_char_t {
  ...
  // If the current text is part of a hypertext link, this gives an index into
  // the URL store.
...
@property(nonatomic, assign)
    screen_char_t *line; // Assume const unless instructed otherwise
...
static inline BOOL ScreenCharacterAttributesEqual(screen_char_t *c1,
                                                  screen_char_t *c2) {
  return (c1->foregroundColor == c2->foregroundColor &&
          c1->fgGreen == c2->fgGreen && c1->fgBlue == c2->fgBlue &&
          c1->backgroundColor == c2->backgroundColor &&
          c1->bgGreen == c2->bgGreen && c1->bgBlue == c2->bgBlue &&
          c1->foregroundColorMode == c2->foregroundColorMode &&
          c1->backgroundColorMode == c2->backgroundColorMode &&
          c1->bold == c2->bold && c1->faint == c2->faint &&
          c1->italic == c2->italic && c1->blink == c2->blink &&
          c1->underline == c2->underline &&
          c1->underlineStyle == c2->underlineStyle &&
          c1->strikethrough == c2->strikethrough &&
          !c1->urlCode ==
              !c2->urlCode && // Only tests if urlCode is zero/nonzero in both
          c1->image == c2->image);
}

// Copy foreground color from one char to another.
static inline void CopyForegroundColor(screen_char_t *to,
                                       const screen_char_t from) {
...
static inline void CopyBackgroundColor(screen_char_t *to,
                                       const screen_char_t from) {
...
static inline BOOL BackgroundColorsEqual(const screen_char_t a,
  ...
  if (a.backgroundColorMode == b.backgroundColorMode) {
    if (a.backgroundColorMode != ColorMode24bit) {
      ...
      return a.backgroundColor == b.backgroundColor && a.bgGreen == b.bgGreen &&
             a.bgBlue == b.bgBlue;
    }
  } else {
...
static inline BOOL ForegroundAttributesEqual(const screen_char_t a,
                                             const screen_char_t b) {
  if (a.bold != b.bold || a.faint != b.faint || a.italic != b.italic ||
      a.blink != b.blink || a.underline != b.underline ||
      a.underlineStyle != b.underlineStyle ||
      a.strikethrough != b.strikethrough || !a.urlCode != !b.urlCode) {
    return NO;
  }
  if (a.foregroundColorMode == b.foregroundColorMode) {
    if (a.foregroundColorMode != ColorMode24bit) {
      ...
      // RGB must all be equal for 24bit color
      return a.foregroundColor == b.foregroundColor && a.fgGreen == b.fgGreen &&
             a.fgBlue == b.fgBlue;
    }
  } else {
    // different ColorMode == different colors
...
static inline BOOL
ScreenCharHasDefaultAttributesAndColors(const screen_char_t s) {
  return (s.backgroundColor == ALTSEM_DEFAULT &&
          s.foregroundColor == ALTSEM_DEFAULT &&
          s.backgroundColorMode == ColorModeAlternate &&
          s.foregroundColorMode == ColorModeAlternate && !s.complexChar &&
          !s.bold && !s.faint && !s.italic && !s.blink && !s.underline &&
          s.underlineStyle == VT100UnderlineStyleSingle && !s.strikethrough &&
          !s.urlCode);
}
...
// This is not how you'd normally construct a string line, since it's supposed
// to come from screen characters. It's useful if you need a string line that
// doesn't represent actual characters on the screen, though.
...
// In other words:
// part or all of [result characterAtIndex:i] refers to all or part of
// screenChars[i - (*deltasPtr)[i]].
...
// A good choice for ambiguousIsDoubleWidth is [SESSION
// treatAmbiguousWidthAsDoubleWidth]. If not null, *cursorIndex gives an index
// into s and is changed into the
...
// Copy attributes from fg and bg, and zero out other fields. Text attributes
// like bold, italic, etc. come from fg.
void InitializeScreenChar(screen_char_t *s, screen_char_t fg, screen_char_t bg);

// Translates normal characters into graphics characters, as defined in
// charsets.h. Must not contain complex characters.
...
// not distorted. Insets should be specified as a fraction of cell size (all
// inset values should be in [0, 1] and will be multiplied by cell width and
...
// Assigns an image to a code allocated by ImageCharForNewImage. data is
// optional and only used for animated gifs.
void SetDecodedImage(unichar code, iTermImage *image, NSData *data);

// Releases all memory associated with an image. The code comes from
// ImageCharForNewImage.
