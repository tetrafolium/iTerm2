...
// Takes a string containing a path to a file, plus the string that follows the
// filename, and tries to improve it:
...
// - Ensure the file actually exists; if it does not, try again by removing an
// a/ or b/ prefix
//   (as seen in diff output)
// - Standardize the path by removing . and .. anywhere in the path,
//   turning /bar/baz/../foo into /bar/foo
//
// Furthermore, the line and column number are extracted from either the suffix
// or the path after removing enclosing punctuation, wherever they may appear.
//
// tl;dr: Given a mess like "(a/bar/baz/../foo:1:2)." convert it to $PWD/bar/foo
// and extracts the line and column number into properties, or returns nil if
// $PWD/bar/foo does not exist.
