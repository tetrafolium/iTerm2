...
- (IBAction)performBulkCopy:(id)sender {
  ...
  for (NSString *destGuid in destGuids) {
    ...
    if ([_copyColors state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyColors
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copyText state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyText
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copyWindow state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyWindow
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copyTerminal state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyTerminal
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copyKeyboard state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyKeyboard
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copySession state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopySession
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
    if ([_copyAdvanced state] == NSControlStateValueOn) {
      [self copyAttributes:BulkCopyAdvanced
          fromProfileWithGuid:_sourceGuid
            toProfileWithGuid:destGuid];
    }
  }
...
- (void)updateLabel {
  Profile *profile =
      [[ProfileModel sharedInstance] bookmarkWithGuid:_sourceGuid];
  [_bulkCopyLabel
      setStringValue:
          [NSString stringWithFormat:@"Copy these settings from profile “%@”:",
                                     profile[KEY_NAME]]];
}
