...
NSString *const kRebuildColorPresetsMenuNotification =
    @"kRebuildColorPresetsMenuNotification";

@implementation iTermColorPresets

+ (iTermColorPresetDictionary *)customColorPresets {
  return [[NSUserDefaults standardUserDefaults]
      objectForKey:kCustomColorPresetsKey];
}

+ (iTermColorPresetDictionary *)builtInColorPresets {
  NSString *plistFile =
      [[NSBundle bundleForClass:[self class]] pathForResource:@"ColorPresets"
                                                       ofType:@"plist"];
  return [NSDictionary dictionaryWithContentsOfFile:plistFile];
}

+ (iTermColorPresetDictionary *)allColorPresets {
  return [[self builtInColorPresets]
      dictionaryByMergingDictionary:[self customColorPresets]];
}
...
+ (BOOL)importColorPresetFromFile:(NSString *)filename {
  ...
  if (!aDict) {
    ...
    alert.informativeText = @"The selected file could not be read or did not "
                            @"contain a valid color scheme.";
    ...
    if (dup) {
      ...
      alert.informativeText = [NSString
          stringWithFormat:@"The color preset “%@” is the same as the preset "
                           @"you're trying to add. Really add it?",
                           dup];
...
+ (void)deletePresetWithName:(NSString *)name {
  ...
  NSMutableDictionary *newCustom =
      [NSMutableDictionary dictionaryWithDictionary:customPresets];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kRebuildColorPresetsMenuNotification
                    object:nil];
}
...
+ (NSString *)nameOfPresetsEqualTo:(NSDictionary *)dict {
  NSDictionary *presets = [[NSUserDefaults standardUserDefaults]
      objectForKey:kCustomColorPresetsKey];
...
+ (void)addColorPreset:(NSString *)presetName
            withColors:(NSDictionary *)theDict {
  DLog(@"Add color preset with name %@ and dictionary %@", presetName, theDict);
  NSDictionary *presets = [[NSUserDefaults standardUserDefaults]
      objectForKey:kCustomColorPresetsKey];
  NSMutableDictionary *customPresets =
  ...
  [[NSUserDefaults standardUserDefaults] setObject:customPresets
                                            forKey:kCustomColorPresetsKey];
...
// This is an abuse of objectForKey:inProfile:, which expects the second arg to
// be a profile. The preset dictionary looks just enough like a profile for this
// to work.
- (iTermColorDictionary *)iterm_presetColorWithName:(NSString *)colorName {
  // If the preset is missing an entry, the default color will be used for that
  // entry.
...
+ (NSArray *)colorKeys {
  return @[
    KEY_ANSI_0_COLOR,        KEY_ANSI_1_COLOR,     KEY_ANSI_2_COLOR,
    KEY_ANSI_3_COLOR,        KEY_ANSI_4_COLOR,     KEY_ANSI_5_COLOR,
    KEY_ANSI_6_COLOR,        KEY_ANSI_7_COLOR,     KEY_ANSI_8_COLOR,
    KEY_ANSI_9_COLOR,        KEY_ANSI_10_COLOR,    KEY_ANSI_11_COLOR,
    KEY_ANSI_12_COLOR,       KEY_ANSI_13_COLOR,    KEY_ANSI_14_COLOR,
    KEY_ANSI_15_COLOR,       KEY_FOREGROUND_COLOR, KEY_BACKGROUND_COLOR,
    KEY_BOLD_COLOR,          KEY_LINK_COLOR,       KEY_SELECTION_COLOR,
    KEY_SELECTED_TEXT_COLOR, KEY_CURSOR_COLOR,     KEY_CURSOR_TEXT_COLOR,
    KEY_TAB_COLOR,           KEY_UNDERLINE_COLOR,  KEY_CURSOR_GUIDE_COLOR,
    KEY_BADGE_COLOR
  ];
}
...
- (BOOL)addColorPresetNamed:(NSString *)presetName
  ...
  for (NSString *colorName in [ProfileModel colorKeys]) {
    ...
    if (colorDict) {
      ...
      [newDict
          removeObjectForKey:colorName]; // Can happen for tab color and
                                         // underline color, which are optional
    }
  }
