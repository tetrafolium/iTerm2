...
- (void)awakeFromNib {
  ...
  footerView_.frame = NSMakeRect(
      0, self.footerMargin,
      self.window.contentView.frame.size.width - footerHeight, footerHeight);
  ...
  _footerBackground.layer.backgroundColor =
      [[NSColor colorWithWhite:0.5 alpha:1] CGColor];
  _footerBackground.layer.opacity = 0.2;
  _footerBackground.layer.cornerRadius = 4;
  _footerBackground.layer.maskedCorners =
      (kCALayerMinXMinYCorner | kCALayerMaxXMinYCorner);
  _footerBackground.frame =
      NSMakeRect(0, 0, self.window.contentView.frame.size.width,
                 footerHeight + self.footerMargin * 1.5);
...
// When a popup window is open we will forward some actions to the "real"
// responder in the owning window. We have to implement the methods in question
// so that the SDK will enable them in the menu.
...
- (void)reloadData:(BOOL)canChangeSide {
  ...
  for (PopupEntry *s in unfilteredModel_) {
    if ([self _word:[self truncatedMainValueForEntry:s]
            matchesFilter:substring_]) {
      [model_ addObject:s];
    }
  }
  ...
  if (!haveChangedSelection_ && [tableView_ numberOfRows] > 0) {
    ...
    if (i >= 0) {
      NSIndexSet *indexes =
          [NSIndexSet indexSetWithIndex:[self convertIndex:i]];
      [tableView_ selectRowIndexes:indexes byExtendingSelection:NO];
    }
  }
...
- (CGFloat)desiredHeight {
  CGFloat height =
      [[tableView_ headerView] frame].size.height +
      MIN(20, [model_ count]) *
          ([tableView_ rowHeight] + [tableView_ intercellSpacing].height);
...
- (void)setPosition:(BOOL)canChangeSide {
  ...
  NSRect monitorFrame =
      [[[[self.delegate popupWindowController] window] screen] visibleFrame];

  if (canChangeSide) {
    // p.y gives the bottom of the frame relative to the bottom of the screen,
    // assuming it's below the cursor.
    float bottomOverflow = monitorFrame.origin.y - p.y;
    float topOverflow = p.y + 2 * frame.size.height + [tv lineHeight] -
                        (monitorFrame.origin.y + monitorFrame.size.height);
  ...
  if (canChangeSide) {
    ...
    if (flip) {
      ...
      NSIndexSet *indexes = [NSIndexSet
          indexSetWithIndex:[self convertIndex:[tableView_ selectedRow]]];
      [tableView_ selectRowIndexes:indexes byExtendingSelection:NO];
      reloading_ = oldReloading;
    }
  }
...
- (void)moveDown:(id)sender {
  ...
  [tableView_ selectRowIndexes:[NSIndexSet indexSetWithIndex:row + 1]
...
- (void)insertText:(NSString *)insertString {
  ...
  timer_ = [NSTimer scheduledTimerWithTimeInterval:4
                                            target:self
                                          selector:@selector
                                          (_setClearFilterOnNextKeyDownFlag:)
                                          userInfo:nil
                                           repeats:NO];
}
...
- (BOOL)passKeyEventToDelegateForSelector:(SEL)selector
  ...
  if ([_delegate respondsToSelector:@selector
                 (popupHandleSelector:string:currentValue:)]) {
    PopupEntry *entry = nil;
    if ([tableView_ selectedRow] >= 0) {
      entry = [[self model]
          objectAtIndex:[self convertIndex:[tableView_ selectedRow]]];
    }
    if ([_delegate popupHandleSelector:selector
                                string:string
                          currentValue:entry.mainValue]) {
      return YES;
    }
  }
  return NO;
...
- (NSAttributedString *)attributedStringForEntry:(PopupEntry *)entry
  ...
  for (int i = 0; i < [substring_ length]; ++i) {
    ...
    if (prefix.length > 0) {
      ...
      attributedSubstr = [[[NSAttributedString alloc]
          initWithString:substr
    ...
    attributedSubstr = [[[NSAttributedString alloc]
        initWithString:[NSString stringWithCharacters:&matchChar length:1]
  ...
  if ([temp length] > 0) {
    NSAttributedString *attributedSubstr = [[[NSAttributedString alloc]
        initWithString:temp
            attributes:plainAttributes] autorelease];
    [as appendAttributedString:attributedSubstr];
  }

  return [self shrunkToFitAttributedString:as
                                   inEntry:entry
                            baseAttributes:plainAttributes];
}
...
- (id)tableView:(NSTableView *)aTableView
  ...
  return [self attributedStringForEntry:e
                             isSelected:[aTableView selectedRow] == rowIndex];
}
