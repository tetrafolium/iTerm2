...
- (void)setActive:(BOOL)active {
  ...
  if (active) {
    __weak __typeof(self) weakSelf = self;
    _leftMonitor = [[iTermTmuxOptionMonitor alloc]
             initWithGateway:_gateway
                       scope:_scope
        fallbackVariableName:nil
                      format:@"#{T:status-left}"
                      target:[NSString
                                 stringWithFormat:@"@%@", _scope.tab.tmuxWindow]
                variableName:nil
                       block:^(NSString *_Nonnull left) {
                         [weakSelf handleStatusLeftValueExpansionResponse:left];
                       }];
    _rightMonitor = [[iTermTmuxOptionMonitor alloc]
             initWithGateway:_gateway
                       scope:_scope
        fallbackVariableName:nil
                ...
                variableName:nil
                       block:^(NSString *_Nonnull right) {
                         [weakSelf
                             handleStatusRightValueExpansionResponse:right];
                       }];
...
- (void)handleStatusLeftValueExpansionResponse:(NSString *)string {
  ...
  [self.scope setValue:[self sanitizedString:string] ?: @""
...
- (void)accelerateUpdateIfStringContainsNotReady:(NSString *)string {
  ...
  if ([string containsString:@"' not ready>"]) {
    if (_leftMonitor.interval > _acceleratedInterval) {
      _accelerated = YES;
      DLog(@"%@: Schedule accelerated upate with interval %@", self,
           @(_acceleratedInterval));
      dispatch_after(
          dispatch_time(DISPATCH_TIME_NOW,
                        (int64_t)(_acceleratedInterval * NSEC_PER_SEC)),
