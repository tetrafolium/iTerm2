...
static NSString *const kMultitouchSupportFramework =
...
static void *GetFunctionByName(NSString *library, char *func) {
  ...
  CFURLRef bundleURL = CFURLCreateWithFileSystemPath(
      kCFAllocatorDefault, (CFStringRef)library, kCFURLPOSIXPathStyle, true);
...
CPSStealKeyFocusFunction *GetCPSStealKeyFocusFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function =
        GetFunctionByName(kApplicationServicesFramework, "CPSStealKeyFocus");
  });
  return function;
}
...
CPSReleaseKeyFocusFunction *GetCPSReleaseKeyFocusFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function =
        GetFunctionByName(kApplicationServicesFramework, "CPSReleaseKeyFocus");
  });
  return function;
}

CGSSetWindowBackgroundBlurRadiusFunction *
GetCGSSetWindowBackgroundBlurRadiusFunction(void) {
...
MTActuatorCreateFromDeviceIDFunction *
iTermGetMTActuatorCreateFromDeviceIDFunction(void) {
  static dispatch_once_t onceToken;
...
MTActuatorOpenFunction *iTermGetMTActuatorOpenFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function = GetFunctionByName(kMultitouchSupportFramework, "MTActuatorOpen");
  });
  return function;
}
...
MTActuatorCloseFunction *iTermGetMTActuatorCloseFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function =
        GetFunctionByName(kMultitouchSupportFramework, "MTActuatorClose");
  });
  return function;
}
...
MTActuatorActuateFunction *iTermGetMTActuatorActuateFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function =
        GetFunctionByName(kMultitouchSupportFramework, "MTActuatorActuate");
  });
  return function;
}
...
MTActuatorIsOpenFunction *iTermGetMTActuatorIsOpenFunction(void) {
  ...
  dispatch_once(&onceToken, ^{
    function =
        GetFunctionByName(kMultitouchSupportFramework, "MTActuatorIsOpen");
  });
  return function;
}
...
+ (NSValue *)futureValueWithEdgeInsets:(NSEdgeInsets)edgeInsets {
  return [[[NSValue alloc] initWithBytes:&edgeInsets
                                objCType:@encode(NSEdgeInsets)] autorelease];
}
