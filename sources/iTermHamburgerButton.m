...
- (BOOL)trackMouse:(NSEvent *)event
  ...
  if (![controlView
          conformsToProtocol:@protocol(iTermHamburgerButtonCellControl)]) {
    return [super trackMouse:event
                      inRect:cellFrame
                      ofView:controlView
                untilMouseUp:untilMouseUp];
  }
  id<iTermHamburgerButtonCellControl> control =
      (id<iTermHamburgerButtonCellControl>)controlView;
  const NSPoint centerPoint = [controlView
      convertPoint:NSMakePoint(NSMidX(cellFrame), NSMidY(cellFrame))
            toView:nil];
  ...
  [NSMenu popUpContextMenu:[control hamburgerMenu]
                 withEvent:fakeEvent
                   forView:controlView];
