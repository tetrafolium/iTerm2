...
NSString *const iTermProfileModelNewWindowMenuItemIdentifierPrefix =
    @"NewWindow:";
...
- (Profile *)tmuxProfile {
  ...
  if (!profile) {
    ...
    NSMutableDictionary *tmuxProfile =
        [[defaultBookmark mutableCopy] autorelease];
...
+ (BOOL)doesProfileWithName:(NSString *)name
  ...
  NSArray *nameWords =
      [name componentsSeparatedByCharactersInSet:[NSCharacterSet
  ...
  for (int i = 0; i < [tokens count]; ++i) {
    ...
    for (int j = 0; !found && j < [tags count]; ++j) {
      ...
      NSArray *tagWords = [[tags objectAtIndex:j]
    ...
    if (!found && name != nil) {
      // No tag had token i as a prefix. If name is nil then we don't really
      // care about the answer and we just want index sets.
      return NO;
    }
  }
  return YES;
...
+ (NSArray *)parseFilter:(NSString *)filter {
  ...
  for (NSString *phrase in phrases) {
    iTermProfileSearchToken *token =
        [[[iTermProfileSearchToken alloc] initWithPhrase:phrase] autorelease];
...
- (NSArray *)bookmarkIndicesMatchingFilter:(NSString *)filter
  ...
  NSMutableArray *result =
      [NSMutableArray arrayWithCapacity:[bookmarks_ count]];
...
+ (void)migratePromptOnCloseInMutableBookmark:(NSMutableDictionary *)dict {
  ...
  if ([dict objectForKey:KEY_PROMPT_CLOSE_DEPRECATED]) {
    ...
    if (value != PROMPT_ALWAYS) {
      [dict setObject:[NSNumber numberWithInt:value] forKey:KEY_PROMPT_CLOSE];
    }
  }
  if (![dict objectForKey:KEY_PROMPT_CLOSE]) {
    BOOL promptOnClose = [[[NSUserDefaults standardUserDefaults]
        objectForKey:@"PromptOnClose"] boolValue];
    NSNumber *onlyWhenNumber = [[NSUserDefaults standardUserDefaults]
        objectForKey:@"OnlyWhenMoreTabs"];
  ...
  if (![dict objectForKey:KEY_JOBS]) {
    [dict setObject:[NSArray arrayWithObjects:@"rlogin", @"ssh", @"slogin",
                                              @"telnet", nil]
             forKey:KEY_JOBS];
    gMigrated = YES;
  }
...
- (void)addBookmark:(Profile *)bookmark inSortedOrder:(BOOL)sort {
  ...
  if (![newBookmark objectForKey:KEY_CUSTOM_COMMAND]) {
    [newBookmark setObject:kProfilePreferenceCommandTypeLoginShellValue
  ...
  int theIndex;
  if (sort) {
    // Insert alphabetically. Sort so that objects with the "bonjour" tag come
    // after objects without.
    ...
    for (int i = 0; i < [bookmarks_ count]; ++i) {
      ...
      if (order == NSOrderedSame) {
        order = [[[bookmarks_ objectAtIndex:i] objectForKey:KEY_NAME]
            caseInsensitiveCompare:newName];
      }
    ...
    if (insertionPoint == -1) {
      ...
      [bookmarks_ insertObject:[NSDictionary dictionaryWithDictionary:bookmark]
  ...
  NSString *isDeprecatedDefaultBookmark =
      [bookmark objectForKey:KEY_DEFAULT_BOOKMARK];

  // The call to setDefaultByGuid may add a journal entry so make sure this one
  // comes first.
  ...
  if (![self defaultBookmark] ||
      (isDeprecatedDefaultBookmark &&
       [isDeprecatedDefaultBookmark isEqualToString:@"Yes"])) {
    [self setDefaultByGuid:[bookmark objectForKey:KEY_GUID]];
  }
  [self postChangeNotification];
  [[self debugHistoryForGuid:bookmark[KEY_GUID]]
      addObject:[NSString stringWithFormat:@"%@: Add bookmark with guid %@",
                                           self, bookmark[KEY_GUID]]];
}
...
- (void)removeBookmarksAtIndices:(NSArray *)indices {
  ...
  for (int j = [sorted count] - 1; j >= 0; j--) {
    ...
    [journal_ addObject:[BookmarkJournalEntry
                            journalWithAction:JOURNAL_REMOVE
                                     bookmark:[bookmarks_ objectAtIndex:i]
                                        model:self]];
    [[self debugHistoryForGuid:bookmarks_[i][KEY_GUID]]
        addObject:[NSString
                      stringWithFormat:@"%@: Remove bookmark with guid %@",
                                       self, bookmarks_[i][KEY_GUID]]];
    [bookmarks_ removeObjectAtIndex:i];
    if (![self defaultBookmark] && [bookmarks_ count]) {
      [self setDefaultByGuid:[[bookmarks_ objectAtIndex:0]
  ...
  [journal_ addObject:[BookmarkJournalEntry
                          journalWithAction:JOURNAL_REMOVE
                                   bookmark:[bookmarks_ objectAtIndex:i]
                                      model:self]];
  [[self debugHistoryForGuid:bookmarks_[i][KEY_GUID]]
      addObject:[NSString stringWithFormat:@"%@: Remove bookmark with guid %@",
                                           self, bookmarks_[i][KEY_GUID]]];
  [bookmarks_ removeObjectAtIndex:i];
  DLog(@"Number of profiles is now %d", (int)bookmarks_.count);
  if (![self defaultBookmark] && [bookmarks_ count]) {
    [self
        setDefaultByGuid:[[bookmarks_ objectAtIndex:0] objectForKey:KEY_GUID]];
  }
  [self postChangeNotification];
}
...
// A change in bookmarks is journal-worthy only if the name, shortcut, tags, or
// guid changes.
- (BOOL)bookmark:(Profile *)a differsJournalablyFrom:(Profile *)b {
  // Any field that is shown in a view (profiles window, menus, bookmark list
  // views, etc.) must be a criteria for journalability for it to be updated
  // immediately.
  if (![[a objectForKey:KEY_NAME] isEqualToString:[b objectForKey:KEY_NAME]] ||
      ![[a objectForKey:KEY_SHORTCUT]
          isEqualToString:[b objectForKey:KEY_SHORTCUT]] ||
      ...
      ![[a objectForKey:KEY_COMMAND_LINE]
          isEqualToString:[b objectForKey:KEY_COMMAND_LINE]] ||
      ![[a objectForKey:KEY_CUSTOM_COMMAND]
          isEqualToString:[b objectForKey:KEY_CUSTOM_COMMAND]]) {
    return YES;
  } else {
    return NO;
  }
...
- (void)setBookmark:(Profile *)bookmark atIndex:(int)i {
  ...
  [[self debugHistoryForGuid:bookmark[KEY_GUID]]
      addObject:[NSString stringWithFormat:
                              @"%@: Replace bookmark at index %@ (%@) with %@",
                              self, @(i), bookmarks_[i][KEY_GUID],
                              bookmark[KEY_GUID]]];
  [bookmarks_ replaceObjectAtIndex:i withObject:bookmark];
  if (needJournal) {
    BookmarkJournalEntry *e =
        [BookmarkJournalEntry journalWithAction:JOURNAL_ADD
                                       bookmark:bookmark
                                          model:self
                                          index:i];
    [journal_ addObject:e];
...
- (void)removeAllBookmarks {
  [[self debugHistoryForGuid:@"na"]
      addObject:[NSString stringWithFormat:@"%@: Remove all bookmarks", self]];
  [bookmarks_ removeAllObjects];
  defaultBookmarkGuid_ = @"";
  [journal_ addObject:[BookmarkJournalEntry journalWithAction:JOURNAL_REMOVE_ALL
                                                     bookmark:nil
                                                        model:self]];
  [self postChangeNotification];
}
...
- (void)load:(NSArray *)prefs {
  ...
  [[self debugHistoryForGuid:@"na"]
      addObject:[NSString stringWithFormat:@"%@: Load bookmarks. Now have %@.",
                                           self, [self guids]]];
}
...
- (int)indexOfProfileWithGuid:(NSString *)guid withFilter:(NSString *)filter {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if ([[[bookmarks_ objectAtIndex:i] objectForKey:KEY_GUID]
...
- (Profile *)bookmarkWithName:(NSString *)name {
  int count = [bookmarks_ count];
  for (int i = 0; i < count; ++i) {
    if ([[[bookmarks_ objectAtIndex:i] objectForKey:KEY_NAME]
...
- (Profile *)setObject:(id)object
  ...
  NSMutableDictionary *newDict =
      [NSMutableDictionary dictionaryWithDictionary:bookmark];
  if (object == nil) {
  ...
  [self setBookmark:newBookmark withGuid:guid];
  return newBookmark;
}
...
- (void)setDefaultByGuid:(NSString *)guid {
  ...
  [journal_
      addObject:[BookmarkJournalEntry journalWithAction:JOURNAL_SET_DEFAULT
...
- (void)setProfilePreservingGuidWithGuid:(NSString *)origGuid
  ...
  Profile *preDivorceProfile =
      [[ProfileModel sharedInstance] bookmarkWithGuid:preDivorceGuid];
  ...
  // Change the dict in the sessions bookmarks so that if you copy it back, it
  // gets copied to the new profile.
...
- (void)moveGuid:(NSString *)guid toRow:(int)destinationRow {
  ...
  [bookmark retain];
  [[self debugHistoryForGuid:bookmark[KEY_GUID]]
      addObject:[NSString stringWithFormat:@"%@: Moving guid %@ to row %@. "
                                           @"First, remove it from row %@",
                                           self, guid, @(destinationRow),
                                           @(sourceRow)]];
  ...
  [[self debugHistoryForGuid:bookmark[KEY_GUID]]
      addObject:[NSString stringWithFormat:@"%@: Now insert it %@ at row %@",
                                           self, guid, @(destinationRow)]];
...
- (void)postNotification:(NSNotification *)notification {
  if (_delayedNotifications) {
    ...
    if ([notification.name isEqualToString:kReloadAddressBookNotification] &&
      ...
      // Special hack to combine journals to avoid sending a million
      // notifications each with a small journal.
      NSArray *lastArray = last.userInfo[@"array"] ?: @[];
      NSArray *thisArray = notification.userInfo[@"array"] ?: @[];
      NSArray *combinedArray =
          [lastArray arrayByAddingObjectsFromArray:thisArray];
      NSNotification *combined =
          [NSNotification notificationWithName:notification.name
                                        object:nil
                                      userInfo:@{@"array" : combinedArray}];
...
- (void)postChangeNotification {
  ...
  if (postChanges_) {
    ...
    [self postNotificationName:kReloadAddressBookNotification
                        object:nil
                      userInfo:@{@"array" : journal_}];
  }
  [journal_ release];
  journal_ = [[NSMutableArray alloc] init];
}
...
- (void)dump {
  for (int i = 0; i < [self numberOfBookmarks]; ++i) {
    ...
    NSLog(@"%d: %@ %@", i, [bookmark objectForKey:KEY_NAME],
          [bookmark objectForKey:KEY_GUID]);
  }
}
...
- (void)flush {
  ...
  // If KEY_NEW_BOOKMARKS is a string then it was overridden at the command line
  // to point at a file and we shouldn't save it to user defaults. If it wasn't
  // overridden it'll be an array.
  if (![[prefs_ objectForKey:KEY_NEW_BOOKMARKS]
