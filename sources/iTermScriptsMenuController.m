...
- (instancetype)initFullEnvironmentWithPath:(NSString *)path
                                     parent:(nullable iTermScriptItem *)parent {
...
- (instancetype)initFolderWithPath:(NSString *)path
                            parent:(nullable iTermScriptItem *)parent {
  self = [super init];
  if (self) {
    _isFolder = YES;
...
@interface iTermScriptsMenuController () <NSOpenSavePanelDelegate,
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
  ...
  const BOOL isRunning =
      path && !![[iTermScriptHistory sharedInstance] runningEntryWithPath:path];
...
- (NSInteger)separatorIndex {
  return [_scriptsMenu.itemArray
      indexOfObjectPassingTest:^BOOL(NSMenuItem *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
...
- (NSArray<iTermScriptItem *> *)scriptItems {
  iTermScriptItem *root = [[iTermScriptItem alloc]
      initFolderWithPath:[[NSFileManager defaultManager] scriptsPath]
                  parent:nil];
...
- (void)populateScriptItem:(iTermScriptItem *)parentFolderItem {
  ...
  NSSet<NSString *> *scriptExtensions =
      [NSSet setWithArray:@[ @"scpt", @"app", @"py" ]];
    ...
    if (isDirectory) {
      ...
      if ([workspace isFilePackageAtPath:path] ||
          [iTermAPIScriptLauncher environmentForScript:path
                                         ...
                                         checkForSaved:YES]) {
        [parentFolderItem
            addChild:[[iTermScriptItem alloc]
                         initFullEnvironmentWithPath:path
                                              parent:parentFolderItem]];
        continue;
      }
      iTermScriptItem *folderItem =
          [[iTermScriptItem alloc] initFolderWithPath:path
                                               parent:parentFolderItem];
      ...
      [parentFolderItem
          addChild:[[iTermScriptItem alloc] initFileWithPath:path
                                                      parent:parentFolderItem]];
    } else if ([file.pathExtension isEqualToString:@"its"]) {
      [self didFindScriptArchive:path
                      autolaunch:parentFolderItem.isAutoLaunchFolderItem];
    }
  }
}
  ...
  const iTermWarningSelection selection = [iTermWarning
      showWarningWithTitle:
          [NSString stringWithFormat:
                        @"A script archive named “%@” was found in the Scripts "
                        @"directory. Would you like to install it?",
...
- (void)addMenuItemsForScriptItems:
            (NSArray<iTermScriptItem *> *)unsortedScriptItems
                            toMenu:(NSMenu *)containingMenu {
  NSArray<iTermScriptItem *> *const scriptItems =
      [unsortedScriptItems sortedArrayUsingSelector:@selector(compare:)];
  for (iTermScriptItem *scriptItem in scriptItems) {
    if (scriptItem.isFolder) {
      ...
      [self addMenuItemsForScriptItems:scriptItem.children
                                toMenu:submenuItem.submenu];
      continue;
    }

    [self addFile:scriptItem.name
        withFullPath:scriptItem.path
        toScriptMenu:containingMenu];
  }
}
...
- (void)revealScriptsInFinder {
  ...
  [[NSFileManager defaultManager] createDirectoryAtPath:scriptsPath
                            withIntermediateDirectories:YES
                                             attributes:nil
                                                  error:nil];
  [[NSWorkspace sharedWorkspace] openFile:scriptsPath
...
- (void)setInstallRuntimeMenuItem:(NSMenuItem *)installRuntimeMenuItem {
  ...
  if ([[iTermPythonRuntimeDownloader sharedInstance]
              ...
              signingIdentity:signingIdentity
                   completion:^(NSString *errorMessage, NSURL *zipURL) {
                     ...
                     [[NSWorkspace sharedWorkspace]
                         activateFileViewerSelectingURLs:@[ zipURL ]];
                   }];
      }];
}
...
- (BOOL)scriptShouldAutoLaunchWithFullPath:(NSString *)fullPath {
  return [fullPath
      hasPrefix:[[self autolaunchScriptPath] stringByAppendingString:@"/"]];
}

- (NSString *)autoLaunchPathIfFullPathWereMovedToAutoLaunch:
    (NSString *)fullPath {
  return [[self autolaunchScriptPath]
      stringByAppendingPathComponent:fullPath.lastPathComponent];
}
...
- (BOOL)couldMoveScriptToAutoLaunch:(NSString *)fullPath {
  ...
  [[NSFileManager defaultManager]
            createDirectoryAtPath:[self autolaunchScriptPath]
      withIntermediateDirectories:YES
                       attributes:nil
                            error:nil];
  if (![[NSFileManager defaultManager]
          fileExistsAtPath:[self autolaunchScriptPath]]) {
    return NO;
  }

  NSString *destination =
      [self autoLaunchPathIfFullPathWereMovedToAutoLaunch:fullPath];
...
- (void)launchOrTerminateScript:(NSMenuItem *)sender {
  ...
  iTermScriptHistoryEntry *entry =
      [[iTermScriptHistory sharedInstance] runningEntryWithPath:fullPath];
  if (entry) {
    [entry kill];
  } else {
...
- (void)revealScript:(NSMenuItem *)sender {
  ...
  [[NSWorkspace sharedWorkspace]
      activateFileViewerSelectingURLs:@[ [NSURL fileURLWithPath:fullPath] ]];
}

- (void)launchScriptWithRelativePath:(NSString *)path
                           arguments:(NSArray<NSString *> *)arguments
                  explicitUserAction:(BOOL)explicitUserAction {
  NSString *fullPath = [[[NSFileManager defaultManager] scriptsPath]
...
// NOTE: This logic needs to be kept in sync with
// -couldLaunchScriptWithAbsolutePath
- (void)launchScriptWithAbsolutePath:(NSString *)fullPath
  ...
  if (venv) {
    ...
    NSString *mainPyPath = [[[fullPath stringByAppendingPathComponent:name]
        stringByAppendingPathComponent:name]
        stringByAppendingPathExtension:@"py"];
    [iTermAPIScriptLauncher
              launchScript:mainPyPath
                  fullPath:fullPath
                 arguments:arguments
            withVirtualEnv:venv
              setupCfgPath:[fullPath
                               stringByAppendingPathComponent:@"setup.cfg"]
        explicitUserAction:explicitUserAction];
    return;
  }
  ...
  if ([[fullPath pathExtension] isEqualToString:@"scpt"]) {
    ...
    NSUserAppleScriptTask *script =
        [[NSUserAppleScriptTask alloc] initWithURL:aURL error:&error];
    if (!script) {
  ...
  if ([[NSFileManager defaultManager] itemIsDirectory:fullPath]) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:
            [NSString stringWithFormat:@"The script “%@” is malformed.",
...
- (BOOL)couldLaunchScriptWithAbsolutePath:(NSString *)fullPath {
  ...
  if ([[fullPath pathExtension] isEqualToString:@"scpt"]) {
    ...
    script = [[NSAppleScript alloc] initWithContentsOfURL:aURL
                                                    error:&errorInfo];
    return script != nil;
  } else {
    return NO;
...
- (NSString *)pathToTemplateForPicker:
    (iTermScriptTemplatePickerWindowController *)picker {
  NSArray<NSString *> *environmentNamePart = @[ @"na", @"basic", @"pyenv" ];
  NSArray<NSString *> *templateNamePart = @[ @"na", @"simple", @"daemon" ];
  NSString *templateName = [NSString
      stringWithFormat:@"template_%@_%@",
                       environmentNamePart[picker.selectedEnvironment],
                       templateNamePart[picker.selectedTemplate]];
  NSString *templatePath =
      [[NSBundle bundleForClass:self.class] pathForResource:templateName
                                                     ofType:@"py"];
  return templatePath;
}
...
- (void)newPythonScript {
  ...
  iTermPythonRuntimeDownloader *downloader =
      [iTermPythonRuntimeDownloader sharedInstance];
  [downloader
                               ...
                               minimumEnvironmentVersion:0
                                      requiredToContinue:YES
                                          withCompletion:^(
                                            ...
                                            [weakSelf
                                                reallyCreateNewPythonScript];
                                          }];
}

- (void)reallyCreateNewPythonScript {
  iTermScriptTemplatePickerWindowController *picker =
      [[iTermScriptTemplatePickerWindowController alloc]
                                 ...
                                 minimumEnvironmentVersion:0
                                        requiredToContinue:YES
                                            withCompletion:^(
                                              ...
                                              [self
                                                  reallyCreateNewPythonScriptAtURL:
                                                      url
                                                                            picker:
                                                                                picker
                                                                      dependencies:
...
- (void)
    reallyCreateNewPythonScriptAtURL:(NSURL *)url
                              picker:
                                  (iTermScriptTemplatePickerWindowController *)
                                      picker
  ...
  if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
    NSURL *folder =
        [NSURL fileURLWithPath:[self folderForFullEnvironmentSavePanelURL:url]];
                     ...
                     pythonVersion:pythonVersion
                        completion:^(BOOL ok) {
                          if (!ok) {
                            ...
                            alert.informativeText =
                                @"Remove ~/Library/Application "
                                @"Support/iTerm2/iterm2env and try again.";
                            [alert runModal];
                            return;
                          }
                          [self finishInstallingNewPythonScriptForPicker:picker
                                                                     url:url];
                          [self build];
                        }];
  } else {
...
- (void)finishInstallingNewPythonScriptForPicker:
            (iTermScriptTemplatePickerWindowController *)picker
                                             url:(NSURL *)url {
  // destinationTemplatePath is a full path to the main.py file, e.g.
  // foo/bar/bar/bar.py
  NSString *destinationTemplatePath =
      [self destinationTemplatePathForPicker:picker url:url];
  NSString *template = [self templateForPicker:picker url:url];
  if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
    [[NSFileManager defaultManager]
              createDirectoryAtPath:[url.path stringByAppendingPathComponent:
  ...
  NSString *type;
  BOOL ok =
      [[NSWorkspace sharedWorkspace] getInfoForFile:destinationTemplatePath
                                        application:&app
                                               type:&type];
  if (ok) {
    iTermWarningSelection selection = [iTermWarning
        showWarningWithTitle:[NSString
                                 stringWithFormat:@"Open new script in %@?",
                                                  app]
  ...
  [[NSWorkspace sharedWorkspace] selectFile:destinationTemplatePath
                   inFileViewerRootedAtPath:@""];
}

- (NSPopUpButton *)newPythonVersionPopup {
  NSPopUpButton *popUpButton =
      [[NSPopUpButton alloc] initWithFrame:NSMakeRect(10, 0, 50, 50)];
  ...
  for (NSString *pythonVersion in
       [iTermPythonRuntimeDownloader pythonVersionsAt:path]) {
    NSMenuItem *menuItem = [[NSMenuItem alloc] initWithTitle:pythonVersion
                                                      action:NULL
                                               keyEquivalent:@""];
...
- (NSTokenField *)newTokenFieldForDependencies {
  NSTokenField *tokenField =
      [[NSTokenField alloc] initWithFrame:NSMakeRect(0, 0, 100, 22)];
...
- (NSView *)newAccessoryViewForSavePanelWithTokenField:
                (NSTokenField *)tokenField
                                    pythonVersionPopup:
                                        (NSPopUpButton *)pythonVersionPopup {
  NSTextField *label =
      [[NSTextField alloc] initWithFrame:NSMakeRect(0, 5, 60, 22)];
  ...
  NSTextField *pythonVersionLabel =
      [[NSTextField alloc] initWithFrame:NSMakeRect(0, 5, 60, 22)];
  ...
  NSView *accessoryView =
      [[NSView alloc] initWithFrame:NSMakeRect(0, 0,
                                               NSMaxX(tokenField.frame) +
                                                   margin + tokenFieldWidth,
                                               32)];
  ...
  tokenField.frame =
      NSMakeRect(NSMaxX(label.frame) + margin, 5, tokenFieldWidth, 22);
  ...
  NSDictionary *metrics =
      @{@"sideMargin" : @(sideMargin), @"verticalMargin" : @(verticalMargin)};
  [accessoryView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:@"H:|-sideMargin-[label]"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  [accessoryView
      addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:
                                             @"H:[tokenField]-sideMargin-|"
                                                             options:0
                                                             metrics:metrics
                                                               views:views]];
  [accessoryView addConstraints:[NSLayoutConstraint
                                    constraintsWithVisualFormat:
                                        @"H:|-sideMargin-[pythonVersionLabel]"
                                                        options:0
                                                        metrics:metrics
                                                          views:views]];
  [accessoryView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:
                             @"V:|-verticalMargin-[label]-verticalMargin-["
                             @"pythonVersionPopup]-verticalMargin-|"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  ...
  [accessoryView
      addConstraint:[NSLayoutConstraint
                        constraintWithItem:pythonVersionPopup
                                 attribute:NSLayoutAttributeLeading
                                 relatedBy:NSLayoutRelationGreaterThanOrEqual
  ...
  [accessoryView addConstraint:[NSLayoutConstraint
                                   constraintWithItem:tokenField
  ...
  NSLayoutConstraint *constraint =
      [NSLayoutConstraint constraintWithItem:tokenField
...
- (nullable NSURL *)
    runSavePanelForNewScriptWithPicker:
        (iTermScriptTemplatePickerWindowController *)picker
  ...
  if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
    ...
    savePanel.accessoryView =
        [self newAccessoryViewForSavePanelWithTokenField:tokenField
                                      pythonVersionPopup:pythonVersionPopup];
  } else {
    savePanel.allowedFileTypes = @[ @"py" ];
  }
  savePanel.directoryURL =
      [NSURL fileURLWithPath:[[NSFileManager defaultManager] scriptsPath]];
  ...
  if ([savePanel runModal] == NSModalResponseOK) {
    ...
    if ([filename isEqualToString:safeFilename]) {
      ...
      *pythonVersionOut = pythonVersionPopup.selectedItem.title;
      ...
      alert.informativeText =
          @"Scripts can't have space characters in their filenames.";
      ...
      if ([alert runModal] == NSAlertFirstButtonReturn) {
        return [[url URLByDeletingLastPathComponent]
            URLByAppendingPathComponent:safeFilename];
      } else {
...
- (NSString *)templateForPicker:
  ...
  NSMutableString *template =
      [NSMutableString stringWithContentsOfFile:templatePath
                                       encoding:NSUTF8StringEncoding
                                          error:nil];
  [subs enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj,
                                            BOOL *_Nonnull stop) {
    [template replaceOccurrencesOfString:key
                              withString:obj
                                 options:0
...
- (NSString *)relativePathFrom:(NSString *)possibleSuper
...
- (nullable NSString *)relativePathFrom:(NSString *)possibleSuper
  ...
  return [self relativePathFrom:possibleSuper
                       ...
                       relative:[possibleSub.lastPathComponent
...
- (NSString *)folderForFullEnvironmentSavePanelURL:(NSURL *)url {
  ...
  NSString *scriptsRoot = [[[NSURL fileURLWithPath:noSpacesScriptsRoot]
  ...
  if (relative) {
    ...
    NSString *folder =
        [noSpacesScriptsRoot stringByAppendingPathComponent:name];
    return folder;
  }
}

- (NSString *)destinationTemplatePathForPicker:
                  (iTermScriptTemplatePickerWindowController *)picker
                                           url:(NSURL *)url {
  if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
    ...
    return [[folder stringByAppendingPathComponent:name]
        stringByAppendingPathComponent:
            [url.path.lastPathComponent stringByAppendingPathExtension:@"py"]];
  } else {
    return url.path;
  }
}

- (NSString *)pythonForPicker:
                  (iTermScriptTemplatePickerWindowController *)picker
                          url:(NSURL *)url {
  NSString *fullPath;
  if (picker.selectedEnvironment == iTermScriptEnvironmentPrivateEnvironment) {
    fullPath = [iTermAPIScriptLauncher
        prospectivePythonPathForPyenvScriptNamed:url.lastPathComponent];
  } else {
    fullPath = [[iTermPythonRuntimeDownloader sharedInstance]
        pathToStandardPyenvPythonWithPythonVersion:nil];
  }
  return fullPath;
}
...
- (void)addFile:(NSString *)file
    ...
    toScriptMenu:(NSMenu *)scriptMenu {
  NSMenuItem *scriptItem =
  ...
  NSMenuItem *altItem = [[NSMenuItem alloc]
      initWithTitle:[NSString stringWithFormat:@"Reveal %@", file]
...
- (BOOL)shouldRunAutoLaunchScripts {
  ...
  return ([[NSFileManager defaultManager]
              fileExistsAtPath:self.legacyAutolaunchScriptPath] ||
          [[NSFileManager defaultManager]
              fileExistsAtPath:self.autolaunchScriptPath]);
}
...
- (void)panel:(NSSavePanel *)sender
    didChangeToDirectoryURL:(nullable NSURL *)url {
...
- (BOOL)panel:(id)sender validateURL:(NSURL *)url error:(NSError **)outError {
  ...
  NSString *message =
      [NSString stringWithFormat:
                    @"Full-environment scripts must be located under in your "
                    @"Application Support/iTerm2/Scripts directory:\n%@",
