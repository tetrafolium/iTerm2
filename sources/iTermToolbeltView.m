...
NSString *const iTermToolbeltDidRegisterDynamicToolNotification =
    @"iTermToolbeltDidRegisterDynamicToolNotification";

static NSString *const iTermToolbeltProportionsUserDefaultsKey =
...
+ (void)initialize {
  ...
  [iTermToolbeltView registerToolWithName:kActionsToolName
                                withClass:[iTermToolActions class]];
  [iTermToolbeltView registerToolWithName:kCapturedOutputToolName
                                withClass:[ToolCapturedOutputView class]];
  [iTermToolbeltView registerToolWithName:kCommandHistoryToolName
                                withClass:[ToolCommandHistoryView class]];
  [iTermToolbeltView registerToolWithName:kRecentDirectoriesToolName
                                withClass:[ToolDirectoriesView class]];
  [iTermToolbeltView registerToolWithName:kJobsToolName
                                withClass:[ToolJobs class]];
  [iTermToolbeltView registerToolWithName:kNotesToolName
                                withClass:[ToolNotes class]];
  [iTermToolbeltView registerToolWithName:kPasteHistoryToolName
                                withClass:[ToolPasteHistory class]];
  [iTermToolbeltView registerToolWithName:kProfilesToolName
                                withClass:[ToolProfiles class]];
  [iTermToolbeltView registerToolWithName:kSnippetsToolName
                                withClass:[iTermToolSnippets class]];

  NSDictionary<NSString *, NSDictionary *> *dynamicTools =
      [[NSUserDefaults standardUserDefaults] objectForKey:kDynamicToolsKey];
  [dynamicTools enumerateKeysAndObjectsUsingBlock:^(
                    NSString *_Nonnull identifier, NSDictionary *_Nonnull dict,
                    BOOL *_Nonnull stop) {
    [iTermToolbeltView registerToolWithName:dict[kDynamicToolName]
                                  withClass:[ToolWebView class]];
  }];
}

+ (NSArray<NSString *> *)builtInToolNames {
  return
      [gRegisteredTools.allKeys filteredArrayUsingBlock:^BOOL(NSString *key) {
...
+ (void)registerDynamicToolWithIdentifier:(NSString *)identifier
  ...
  NSDictionary *registry =
      [[NSUserDefaults standardUserDefaults] objectForKey:kDynamicToolsKey];
  NSString *oldName =
      [[registry[identifier][kDynamicToolName] retain] autorelease];
  if ([registry[identifier][kDynamicToolURL] isEqualToString:url] &&
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:iTermToolbeltDidRegisterDynamicToolNotification
                      object:identifier];
    return;
  }
  NSMutableDictionary *mutableRegistry =
      [[registry mutableCopy] autorelease] ?: [NSMutableDictionary dictionary];
  mutableRegistry[identifier] =
      @{kDynamicToolName : name, kDynamicToolURL : url};
  [[NSUserDefaults standardUserDefaults] setObject:mutableRegistry
                                            forKey:kDynamicToolsKey];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kDynamicToolsDidChange
                    object:nil];
}
...
+ (NSArray *)configuredTools {
  NSArray *tools =
      [[NSUserDefaults standardUserDefaults] objectForKey:kToolbeltPrefKey];
  if (!tools) {
...
+ (void)addToolsToMenu:(NSMenu *)menu {
  NSArray *names = [[iTermToolbeltView allTools]
      sortedArrayUsingSelector:@selector(compare:)];
  for (NSString *theName in names) {
    NSMenuItem *i =
        [[[NSMenuItem alloc] initWithTitle:theName
                                    action:@selector(toggleToolbeltTool:)
                             keyEquivalent:@""] autorelease];
    [i setState:[iTermToolbeltView shouldShowTool:theName]
...
+ (void)toggleShouldShowTool:(NSString *)theName {
  NSMutableArray *tools =
      [[[iTermToolbeltView configuredTools] mutableCopy] autorelease];
  if (!tools) {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:tools
                                            forKey:kToolbeltPrefKey];
...
- (instancetype)initWithFrame:(NSRect)frame
  ...
  if (self) {
    ...
    _splitter = [[iTermToolbeltSplitView alloc]
    ...
    [_splitter setDividerColor:[NSColor colorWithCalibratedWhite:122 / 255.0
                                                           alpha:0.25]];
    ...
    _dragHandle = [[[iTermDragHandleView alloc]
        initWithFrame:NSMakeRect(0, 0, 3, frame.size.height)] autorelease];
    _dragHandle.delegate = self;
    _dragHandle.autoresizingMask = (NSViewHeightSizable | NSViewMaxXMargin);
    [self addSubview:_dragHandle];

    _menuButton =
        [[[iTermHamburgerButton alloc] initWithMenuProvider:^NSMenu *_Nonnull {
          NSMenu *menu =
...
- (NSRect)menuButtonFrame {
  ...
  return NSMakeRect(self.bounds.size.width - hamburger.size.width - 4, 4.5,
                    hamburger.size.width, hamburger.size.height);
}
...
- (NSColor *)backgroundColor {
  ...
  NSColor *lightColor = [NSColor colorWithCalibratedWhite:237.0 / 255.0
                                                    alpha:1];
  NSColor *darkColor = [NSColor colorWithCalibratedWhite:0.12 alpha:1.00];
  switch ([self.effectiveAppearance
      it_tabStyle:[iTermPreferences intForKey:kPreferenceKeyTabStyle]]) {
...
+ (NSDictionary *)savedProportions {
  return [[NSUserDefaults standardUserDefaults]
      objectForKey:iTermToolbeltProportionsUserDefaultsKey];
}
...
- (void)windowBackgroundColorDidChange {
  for (iTermToolWrapper *wrapper in _tools.allValues) {
    if ([wrapper.tool
            respondsToSelector:@selector(windowBackgroundColorDidChange)]) {
...
- (void)forceSplitterSubviewsToRespectSizeConstraints {
  ...
  // Calculate the total amount of slop (height beyond the minimum) and deficit
  // (height less than minimum) in all views.
  ...
  if (totalDeficit > 0) {
    // One or more views is under the minimum height. Steal a fraction from each
    // view that is over the minimum.
    ...
    for (int i = 0; i < subviews.count; i++) {
      ...
      if (i == subviews.count - 1) {
        // Last view always fills out the remainder. This takes care of
...
- (void)addTool:(NSView<ToolbeltTool> *)theTool
      toWrapper:(iTermToolWrapper *)wrapper {
...
- (void)addToolWithName:(NSString *)toolName {
  if (![gRegisteredTools objectForKey:toolName]) {
    // User could have a plist from a future version with a tool that doesn't
    // exist here.
    return;
  }
  iTermToolWrapper *wrapper = [[[iTermToolWrapper alloc]
      initWithFrame:NSMakeRect(
                        ...
                        self.frame.size.height /
                            MAX(1, [iTermToolbeltView numberOfVisibleTools] -
                                       1))] autorelease];
  ...
  if (c) {
    ...
    NSRect frame = NSMakeRect(0, 0, wrapper.container.frame.size.width,
                              wrapper.container.frame.size.height);
    if ([c instancesRespondToSelector:@selector(initWithFrame:
                                                          URL:identifier:)]) {
      NSDictionary *registry =
          [[NSUserDefaults standardUserDefaults] objectForKey:kDynamicToolsKey];
      NSString *identifier = [registry.allKeys
          objectPassingTest:^BOOL(NSString *key, NSUInteger index, BOOL *stop) {
      ...
      if (url && identifier) {
        theTool = [[[c alloc] initWithFrame:frame URL:url
...
- (void)hideToolbelt {
  [[NSNotificationCenter defaultCenter] postNotificationName:kToolbeltShouldHide
                                                      object:nil
                                                    userInfo:nil];
}
...
- (NSDictionary *)proportions {
  ...
  NSArray<NSDictionary<NSString *, id> *> *proportions =
      [[names zip:fractions] mapWithBlock:^id(iTermTuple *tuple) {
        return @{
          @"name" : tuple.firstObject,
...
- (void)setProportions:(NSDictionary *)dict {
  ...
  NSArray<NSString *> *names = [iTermToolbeltView configuredTools];
  NSArray<NSNumber *> *currentHeights =
      [names mapWithBlock:^id(NSString *name) {
        return @(_tools[name].frame.size.height);
      }];
  ...
  NSArray<iTermTuple<NSString *, NSNumber *> *> *tuples =
      [proportions mapWithBlock:^id(id anObject) {
  ...
  const BOOL consistent = [[tuples zip:names]
      allWithBlock:^BOOL(
          iTermTuple<iTermTuple<NSString *, NSNumber *> *, NSString *> *tuple) {
        return [tuple.firstObject.firstObject
                   isEqualToString:tuple.secondObject] &&
  ...
  NSArray<NSNumber *> *desiredHeights =
      [tuples mapWithBlock:^id(iTermTuple<NSString *, NSNumber *> *tuple) {
        iTermToolWrapper *wrapper = self->_tools[tuple.firstObject];
        const CGFloat proposed =
            tuple.secondObject.doubleValue * sumOfCurrentHeights;
  ...
  if (addedHeight > 0) {
    NSArray<NSNumber *> *wiggleRoom = [[tuples zip:desiredHeights]
        mapWithBlock:^id(iTermTuple<iTermTuple<NSString *, NSNumber *> *,
                                    NSNumber *> *tuple) {
    ...
    desiredHeights = [[desiredHeights zip:wiggleRoom]
  ...
  CGFloat y = 0;
  for (iTermTuple<NSString *, NSNumber *> *tuple in
       [names zip:desiredHeights]) {
