...
+ (instancetype)it_textFieldForTableViewWithIdentifier:(NSString *)identifier {
  NSTextField *result =
      [[[self alloc] initWithFrame:NSMakeRect(0, 0, 0, 0)] autorelease];
...
- (BOOL)textFieldIsFirstResponder {
  ...
  inFocus =
      ([[[self window] firstResponder] isKindOfClass:[NSTextView class]] &&
       [[self window] fieldEditor:NO forObject:nil] != nil &&
       [self isEqualTo:(id)[(NSTextView *)[[self window]
...
- (void)setLabelEnabled:(BOOL)enabled {
  if (@available(macOS 10.14, *)) {
    ...
    self.textColor =
        enabled ? [NSColor blackColor] : [NSColor disabledControlTextColor];
  }
}

- (int)separatorTolerantIntValue {
  NSString *digits =
      [[self stringValue] stringByReplacingOccurrencesOfRegex:@"[^0-9]"
                                                   withString:@""];
...
- (NSTextField *)replaceWithHyperlinkTo:(NSURL *)url {
  NSTextField *link =
      [[[NSTextField alloc] initWithFrame:self.frame] autorelease];
  ...
  NSDictionary *attributes = @{
    ...
    NSForegroundColorAttributeName : [NSColor colorWithCalibratedRed:0
                                                               green:0
                                                                blue:0.93
                                                               alpha:1],
