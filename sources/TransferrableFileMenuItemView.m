...
// This is used as part of the bug workaround in sanityCheckSiblings to ensure
// we don't try to redraw a view more than once.
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    _progressIndicator = [[iTermProgressIndicator alloc]
        initWithFrame:NSMakeRect(leftMargin, 17,
                                 frameRect.size.width - leftMargin -
                                     rightMargin,
...
// Works around an OS bug. If the menu is closed with an item selected and then
// reopened, the item remains selected. If you then select a different item, the
// originally selected item won't get redrawn!
- (void)sanityCheckSiblings {
  for (NSMenuItem *item in [[[self enclosingMenuItem] menu] itemArray]) {
    ...
    if ([item.view isKindOfClass:[TransferrableFileMenuItemView class]]) {
      TransferrableFileMenuItemView *view =
          (TransferrableFileMenuItemView *)item.view;
      if (!view.drawPending && view.lastDrawnHighlighted &&
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  NSDictionary *smallGrayAttributes = @{
    NSForegroundColorAttributeName : grayColor,
    NSParagraphStyleAttributeName : leftAlignStyle,
    NSFontAttributeName : smallFont
  };
  const CGFloat textHeight =
      [_filename sizeWithAttributes:filenameAttributes].height;
  NSString *sizeString;
  if (_size >= 0) {
    sizeString =
  ...
  const CGFloat smallTextHeight =
      [sizeString sizeWithAttributes:sizeAttributes].height;
  ...
  // Draw file name
  NSRect filenameRect =
      NSMakeRect(leftMargin, topY,
                 self.bounds.size.width - leftMargin - rightMargin, textHeight);

  [_filename drawInRect:filenameRect withAttributes:filenameAttributes];

  // Draw subheading
  NSRect subheadingRect = NSMakeRect(
      leftMargin, topY - smallTextHeight - 1,
      self.bounds.size.width - leftMargin - rightMargin, smallTextHeight);
  [_subheading drawInRect:subheadingRect withAttributes:smallGrayAttributes];

  // Draw status label
  if (_statusMessage) {
    [_statusMessage drawInRect:NSMakeRect(leftMargin, bottomY,
  ...
  // Draw size
  [sizeString drawInRect:NSMakeRect(leftMargin, bottomY,
