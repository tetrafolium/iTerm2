...
- (void)mouseDown:(NSEvent *)event {
  [self setFillColor:[NSColor selectedMenuItemColor]
...
- (void)mouseUp:(NSEvent *)event {
  if (event.clickCount == 1) {
    ...
    [self setFillColor:[NSColor gridColor]
...
@interface iTermScriptTemplatePickerWindowController () <
    iTermSelectableBoxDelegate>
...
- (void)windowDidLoad {
  ...
  [_basic addTrackingRect:_basic.bounds
                    owner:_basic
                 userData:NULL
             assumeInside:NO];
  [_pyenv addTrackingRect:_pyenv.bounds
                    owner:_pyenv
                 userData:NULL
             assumeInside:NO];
}
