...
+ (Interval *)intervalWithLocation:(long long)location
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p [%lld, %lld)>", self.class, self,
                                    self.location, self.limit];
}
...
- (NSDictionary *)dictionaryValue {
  return
      @{kIntervalLocationKey : @(_location), kIntervalLengthKey : @(_length)};
}
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p interval=%@ object=%@>", self.class,
                                 self, self.interval, self.object];
}
@end
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"<%@: %p maxLimitAtSubtree=%lld entries=[%@]>",
                       self.class, self, self.maxLimitAtSubtree, entriesString];
}
...
- (Interval *)spanningInterval {
  return [Interval intervalWithLocation:self.location
                                 length:[self maxLimit] - self.location];
}
  ...
  if (self) {
    for (NSDictionary *entry in dict[kIntervalTreeEntriesKey]) {
      ...
      if (intervalDict && objectDict && className) {
        ...
        if ([theClass
                instancesRespondToSelector:@selector(initWithDictionary:)]) {
          id<IntervalTreeObject> object =
              [[[theClass alloc] initWithDictionary:objectDict] autorelease];
          if (object) {
...
- (instancetype)init {
  ...
  if (self) {
    _tree = [[AATree alloc]
        initWithKeyComparator:^(NSNumber *key1, NSNumber *key2) {
...
- (void)dealloc {
  for (id<IntervalTreeObject> obj in
       [self objectsInInterval:[Interval maxInterval]]) {
    obj.entry = nil;
  }
  _tree.delegate = nil;
  [_tree release];
...
- (void)addObject:(id<IntervalTreeObject>)object
  ...
  if (!value) {
    IntervalTreeValue *newValue =
        [[[IntervalTreeValue alloc] init] autorelease];
...
- (void)removeObject:(id<IntervalTreeObject>)object {
  ...
  if (entry) {
    assert(object.entry == entry); // Was object added to another tree before
                                   // being removed from this one?
    object.entry = nil;
...
- (void)addObjectsInInterval:(Interval *)interval
  ...
  if (node.left) {
    // The requested interval includes points before this node's interval so we
    // must search intervals that start before this node.
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"<%@: %p tree=%@>", self.class, self, _tree];
}

- (BOOL)containsObject:(id<IntervalTreeObject>)object {
  IntervalTreeValue *value =
      [_tree objectForKey:@(object.entry.interval.location)];
...
- (NSArray *)objectsWithLargestLimitFromNode:(AATreeNode *)node {
  ...
  long long leftMaxLimit =
      ((IntervalTreeValue *)node.left.data).maxLimitAtSubtree;
  long long rightMaxLimit =
      ((IntervalTreeValue *)node.right.data).maxLimitAtSubtree;
  ...
  if (leftMaxLimit == bestLimit) {
    [objects
        addObjectsFromArray:[self objectsWithLargestLimitFromNode:node.left]];
  }
  if (rightMaxLimit == bestLimit) {
    [objects
        addObjectsFromArray:[self objectsWithLargestLimitFromNode:node.right]];
  }
...
- (NSArray *)objectsWithSmallestLimitFromNode:(AATreeNode *)node {
  ...
  // Searching for the smallest limit among node, node.left subtree, and
  // node.right subtree If node's key >= node.left's first object, don't search
  // right subtree
  ...
  if (node.right && (!objectsFromLeft.count ||
                     nodeInterval.location < leftValue.entry.interval.limit)) {
    objectsFromRight = [self objectsWithSmallestLimitFromNode:node.right];
  }

  id<IntervalTreeObject> rightValue = objectsFromRight[0];
  long long selfLimit = LLONG_MAX, leftLimit = LLONG_MAX,
            rightLimit = LLONG_MAX;
  if (nodeInterval) {
...
- (NSArray *)objectsWithLargestLocationBefore:(long long)location
  ...
  if (key >= location) {
    // If there is a left subtree, search it. If not, this will return an empty
    // array.
  ...
  // key < location
  if (node.right) {
    // There is a larger value in the right subtree, but maybe there's nothing <
    // location?
    NSArray *result = [self objectsWithLargestLocationBefore:location
                                                      atNode:node.right];
    if (result) {
      return result;
    }
  ...
  // If you get here then the there was no right subtree or the whole right
  // subtrees had keys > location. Return the objects in this node.
  IntervalTreeValue *value = node.data;
  NSMutableArray *objects = [NSMutableArray array];
  for (IntervalTreeEntry *entry in value.entries) {
    [objects addObject:entry.object];
...
- (NSArray *)objectsWithSmallestLimitAfter:(long long)bound
                                  fromNode:(AATreeNode *)node {
  if (!node) {
    return nil;
  }
  ...
  for (IntervalTreeEntry *entry in nodeValue.entries) {
    if (entry.interval.limit > bound &&
        (!nodeInterval || entry.interval.limit < nodeInterval.limit)) {
      if (myObjects) {
        [myObjects removeAllObjects];
      } else {
  ...
  // ignore right subtree if node's location > left subtree's smallest limit and
  // left subtree's smallest limit < bound (because every interval in the right
  // subtree will have a limit larger than this node's location, and the left
  // subtree has an interval that ends before that location).
  id<IntervalTreeObject> bestLeft = leftObjects[0];
  const BOOL thisNodesLocationIsAfterLeftSubtreesSmallestLimitAfterBound =
      (bestLeft && thisLocation > bestLeft.entry.interval.limit &&
  ...
  if (node.right && rightValue.maxLimitAtSubtree > bound &&
    ...
    rightObjects = [self objectsWithSmallestLimitAfter:bound
                                              fromNode:node.right];
  }
...
- (NSArray *)objectsWithLargestLimitBelow:(long long)bound
                                 fromNode:(AATreeNode *)node {
  if (!node) {
    return nil;
  }
  ...
  if (node.left) {
    // Try to eliminate the left subtree from consideration if this node or the
    // right subtree is superior to it.
    const BOOL thisNodeBeatsWholeLeftSubtree =
        (nodeInterval && nodeInterval.limit < bound &&
    ...
    if (!thisNodeBeatsWholeLeftSubtree && !rightSubtreeBeatsWholeLeftSubtree) {
      leftObjects = [self objectsWithLargestLimitBelow:bound
                                              fromNode:node.left];
    }
  }
  id<IntervalTreeObject> bestLeft = leftObjects[0];
  if (thisLocation < bound && node.right) {
    // Try to eliminate the right subtree from consideration if this node or the
    // left subtree is superior to it.
    const BOOL thisNodeBeatsWholeRightSubtree =
        (nodeInterval && nodeInterval.limit < bound &&
         nodeInterval.limit > rightValue.maxLimitAtSubtree);
    const BOOL leftSubtreeBeatsWholeRightSubtree =
        (leftValue && leftValue.maxLimitAtSubtree < bound &&
    ...
    if (!thisNodeBeatsWholeRightSubtree && !leftSubtreeBeatsWholeRightSubtree) {
      rightObjects = [self objectsWithLargestLimitBelow:bound
                                               fromNode:node.right];
    }
  }
  id<IntervalTreeObject> bestRight = rightObjects[0];
...
- (NSDictionary *)dictionaryValueWithOffset:(long long)offset {
  ...
  for (id<IntervalTreeObject> object in self.allObjects) {
    ...
    [objectDicts addObject:@{
      kIntervalTreeIntervalKey : object.entry.interval.dictionaryValue,
