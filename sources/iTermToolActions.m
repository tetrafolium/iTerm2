...
static NSString *const iTermToolActionsPasteboardType =
    @"iTermToolActionsPasteboardType";
...
static NSButton *iTermToolActionsNewButton(NSString *imageName, NSString *title,
...
- (instancetype)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    if (@available(macOS 10.16, *)) {
      _applyButton = iTermToolActionsNewButton(@"play", @"Apply", self,
                                               @selector(apply:), frame);
      _addButton = iTermToolActionsNewButton(@"plus", @"Add", self,
                                             @selector(add:), frame);
      _removeButton = iTermToolActionsNewButton(@"minus", @"Remove", self,
                                                @selector(remove:), frame);
      _editButton = iTermToolActionsNewButton(@"switch.2", @"Edit", self,
                                              @selector(edit:), frame);
    } else {
      _applyButton = iTermToolActionsNewButton(nil, @"Apply", self,
                                               @selector(apply:), frame);
      _addButton = iTermToolActionsNewButton(NSImageNameAddTemplate, nil, self,
                                             @selector(add:), frame);
      _removeButton = iTermToolActionsNewButton(
          NSImageNameRemoveTemplate, nil, self, @selector(remove:), frame);
      _editButton =
          iTermToolActionsNewButton(nil, @"✐", self, @selector(edit:), frame);
    }
    ...
    [_tableView setColumnAutoresizingStyle:
    ...
    [iTermActionsDidChangeNotification
        subscribe:self
...
- (void)add:(id)sender {
  _editActionWindowController =
      [self newEditKeyActionWindowControllerForAction:nil];
}
...
- (void)edit:(id)sender {
  ...
  if (action) {
    _editActionWindowController =
        [self newEditKeyActionWindowControllerForAction:action];
  }
}
...
- (iTermEditKeyActionWindowController *)
    newEditKeyActionWindowControllerForAction:(iTermAction *)action {
  iTermEditKeyActionWindowController *windowController =
      [[iTermEditKeyActionWindowController alloc]
...
- (void)editActionDidComplete:(iTermAction *)original {
  if (_editActionWindowController.ok) {
    [self pushUndo];
    if (original) {
      [[iTermActionsModel sharedInstance]
          replaceAction:original
             withAction:_editActionWindowController.unboundAction];
    } else {
      [[iTermActionsModel sharedInstance]
          addAction:_editActionWindowController.unboundAction];
    }
  }
...
- (BOOL)tableView:(NSTableView *)tableView
  ...
  [pboard declareTypes:@[ iTermToolActionsPasteboardType ] owner:self];
  ...
  [pboard setPropertyList:plist forType:iTermToolActionsPasteboardType];
  return YES;
}
...
- (BOOL)tableView:(NSTableView *)aTableView
  ...
  NSArray<NSNumber *> *identifiers =
      [pboard propertyListForType:iTermToolActionsPasteboardType];
