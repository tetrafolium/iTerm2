...
- (NSArray<ObjectType> *)it_arrayByRemovingObjectsAtIndexes:
    (NSIndexSet *)indexes;
...
- (NSArray *)mapEnumeratedWithBlock:(id (^NS_NOESCAPE)(NSUInteger i, id object,
                                                       BOOL *stop))block;
...
- (id)reduceWithFirstValue:(id)firstValue
                     block:(id (^)(id first, ObjectType second))block;

// Returns those elements of the array for which block(element) returns YES.
// block is called on every element in order.
- (NSArray *)filteredArrayUsingBlock:(BOOL(NS_NOESCAPE ^)(ObjectType anObject))
                                         block;
- (ObjectType)objectPassingTest:(BOOL (^)(ObjectType element, NSUInteger index,
                                          BOOL *stop))block;

// Returns the first object that is a kind of `theClass` for which block returns
// YES.
...
- (ObjectType)maxWithComparator:
    (NSComparisonResult (^)(ObjectType a, ObjectType b))comparator;
...
- (NSArray<ObjectType> *)uniqWithComparator:(BOOL (^)(ObjectType obj1,
                                                      ObjectType obj2))block;
...
// Given a collection of file URLs return the file URL that is their deepest
// common ancestor. For example, given an input of: /a/b/c /a/b/c/d /a/b/x
//
// returns /a/b
...
- (void)enumerateCoalescedObjectsWithComparator:
            (BOOL (^)(ObjectType obj1, ObjectType obj2))comparator
                                          block:
                                              (void (^)(ObjectType object,
                                                        NSUInteger count))block;

- (NSArray<iTermTuple *> *)tuplesWithFirstObjectEqualTo:(id)firstObject;
- (NSDictionary<id, NSArray<ObjectType> *> *)classifyWithBlock:
    (id (^)(ObjectType))block;
- (NSDictionary<id, ObjectType> *)classifyUniquelyWithBlock:
    (id (^)(ObjectType))block;
- (ObjectType)uncheckedObjectAtIndex:(NSInteger)index;

- (NSUInteger)indexOfMaxWithBlock:
    (NSComparisonResult (^)(ObjectType obj1, ObjectType obj2))block;
- (ObjectType)maxWithBlock:(NSComparisonResult (^)(ObjectType obj1,
                                                   ObjectType obj2))block;
- (ObjectType)minWithBlock:(NSComparisonResult (^)(ObjectType obj1,
                                                   ObjectType obj2))block;
...
- (NSDictionary *)keyValuePairsWithBlock:
    (iTermTuple * (^)(ObjectType object))block;
- (id)it_jsonSafeValue;
- (instancetype)it_arrayByRemovingObjectsPassingTest:
    (BOOL (^)(ObjectType anObject))block;
