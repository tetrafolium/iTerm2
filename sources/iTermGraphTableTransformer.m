...
static iTermEncoderGraphRecord *
iTermGraphDeltaEncoderMakeGraphRecord(NSNumber *nodeID, NSDictionary *nodes,
  ...
  NSString *tail = [NSString
      stringWithFormat:@"%@[%@]", nodeDict[@"key"], nodeDict[@"identifier"]];
  NSArray<iTermEncoderGraphRecord *> *childGraphRecords =
      [childNodeIDs mapWithBlock:^id(NSNumber *childNodeID) {
        return iTermGraphDeltaEncoderMakeGraphRecord(
            childNodeID, nodes, [path arrayByAddingObject:tail]);
      }];
  ...
  DLog(@"key=%@ id=%@ rowid=%@ children=%@ pod=%@", nodeDict[@"key"],
       nodeDict[@"identifier"], nodeDict[@"rowid"], childNodeIDs,
       [pod tastefulDescription]);
...
- (NSDictionary<NSNumber *, NSMutableDictionary *> *)nodes:
    (out NSNumber **)rootNodeIDOut {
  // Create nodes
  ...
  for (NSArray *row in _nodeRows) {
    if (row.count != 5) {
      ...
      _lastError = [NSError
          errorWithDomain:@"com.iterm2.graph-transformer"
                     code:1
                 userInfo:@{
                   NSLocalizedDescriptionKey : [NSString
                       stringWithFormat:@"Wrong number of items in row: %@",
                                        row]
                 }];
      return nil;
    }
            ...
            errorWithDomain:@"com.iterm2.graph-transformer"
                   ...
                   userInfo:@{
                     NSLocalizedDescriptionKey :
                         [NSString stringWithFormat:@"Two roots found"]
                   }];
        return nil;
      }
      *rootNodeIDOut = rowid;
    ...
    nodes[rowid] = [@{
      @"pod" : [NSMutableDictionary dictionary],
      @"key" : key,
...
- (BOOL)attachChildrenToParents:
            (NSDictionary<NSNumber *, NSMutableDictionary *> *)nodes
              ignoringRootRowID:(NSNumber *)rootRowID {
  __block BOOL ok = YES;
  [nodes enumerateKeysAndObjectsUsingBlock:^(
             NSNumber *_Nonnull nodeid, NSMutableDictionary *_Nonnull nodeDict,
             BOOL *_Nonnull stop) {
    if ([nodeid isEqualToNumber:rootRowID]) {
      return;
    }
    NSMutableDictionary<NSString *, id> *parentDict =
        nodes[nodeDict[@"parent"]];
    if (!parentDict) {
      ok = NO;
      ...
      _lastError = [NSError
          errorWithDomain:@"com.iterm2.graph-transformer"
                 ...
                 userInfo:@{
                   NSLocalizedDescriptionKey : [NSString
                       stringWithFormat:@"Dangling parent pointer %@ from %@",
