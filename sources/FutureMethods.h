...
// This is for the args to CGSSetWindowBackgroundBlurRadiusFunction, which is
// used for window-blurring using undocumented APIs.
#import "iTermCPS.h"
#import <CGSInternal/CGSInternal.h>

typedef CGError CGSSetWindowBackgroundBlurRadiusFunction(CGSConnectionID cid,
                                                         CGSWindowID wid,
                                                         NSUInteger blur);
...
// Grabs keyboard focus; key events are directed to the key window although the
// app is inactive.
...
typedef IOReturn MTActuatorActuateFunction(CFTypeRef actuatorRef,
                                           SInt32 actuationID, UInt32 unknown1,
                                           Float32 unknown2, Float32 unknown3);
...
NS_INLINE BOOL iTermTextIsMonochromeOnMojave(void) NS_AVAILABLE_MAC(10_14) {
  ...
  dispatch_once(&onceToken, ^{
    NSNumber *number = [[NSUserDefaults standardUserDefaults]
        objectForKey:@"CGFontRenderingFontSmoothingDisabled"];
    if (!number) {
