...
- (void)drawFocusRingMaskWithFrame:(NSRect)cellFrame
  ...
  if (controlView.frame.origin.y >= 0) {
    [super
        drawFocusRingMaskWithFrame:NSInsetRect(cellFrame, kFocusRingInset.width,
                                               kFocusRingInset.height)
                            inView:controlView];
  }
}
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
  ...
  const BOOL focused =
      [self shouldUseFocusedAppearanceWithControlView:controlView];
  ...
  if (indexAndCountString) {
    ...
    NSRect rect =
        NSMakeRect(NSMaxX(textRect) - size.width - self.marginForIndexAndCount,
                   textRect.origin.y, size.width, textRect.size.height);
    [indexAndCountString drawInRect:rect
                     withAttributes:self.attributesForIndexAndCount];
  }
...
// Work around a macOS bug that prevents updating the text rect while the search
...
- (void)reallyUpdateKeyboardClipView:(NSTextField *)textField {
  NSView *keyboardClipView = [self.controlView.subviews
      objectPassingTest:^BOOL(__kindof NSView *element, NSUInteger index,
                              BOOL *stop) {
        return [NSStringFromClass([element class])
            isEqualToString:@"_NSKeyboardFocusClipView"];
      }];
  if (!keyboardClipView) {
    return;
  }
  ...
  if (shouldUpdate) {
    ...
    NSTextView *textView = [NSTextView
        castFrom:[textField.window fieldEditor:YES forObject:textField]];
    [textView scrollRangeToVisible:[[[textView selectedRanges] firstObject]
                                       rangeValue]];
...
- (NSString *)indexAndCountString {
  ...
  id<iTermSearchFieldControl> control =
      (id<iTermSearchFieldControl>)controlView;
  if (![control searchFieldControlHasCounts:self]) {
    return nil;
  }
  const iTermSearchFieldCounts counts =
      [control searchFieldControlGetCounts:self];
  ...
  return [NSString stringWithFormat:@"%@/%@", @(counts.currentIndex),
                                    @(counts.numberOfResults)];
}
...
- (NSDictionary *)attributesForIndexAndCount {
  ...
  NSMutableDictionary *attributes =
      [[textField.attributedStringValue attributesAtIndex:0
                                           effectiveRange:nil] mutableCopy];
  attributes[NSForegroundColorAttributeName] =
      [attributes[NSForegroundColorAttributeName] colorWithAlphaComponent:0.5];
  return attributes;
}
...
- (void)drawProgressBarInFrame:(NSRect)cellFrame
                          path:(NSBezierPath *)fieldPath {
  ...
  NSRect blueRect = NSMakeRect(
      0, 0, maximumWidth * [self fraction] + kEdgeWidth, cellFrame.size.height);

  const CGFloat alpha = 0.3 * _alphaMultiplier;
  [[NSColor colorWithCalibratedRed:0.6 green:0.6 blue:1.0 alpha:alpha] set];
