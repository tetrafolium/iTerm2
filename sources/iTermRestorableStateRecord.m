...
+ (void)createWithIndexEntry:(id)indexEntry
                  completion:
                      (void (^)(iTermRestorableStateRecord *record))completion {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   iTermRestorableStateRecord *record =
                       [[self alloc] initWithIndexEntry:indexEntry];
...
// You might wonder why we bother to encrypt the file and then save the key in
// the same directory. I do it only because Apple did it, just in case there's a
// good reason for it that I haven't thought of yet.
- (NSData *)ciphertext {
  NSData *iv = [[NSMutableData alloc] initWithLength:self.key.length];
  return [self.plaintext aesCBCEncryptedDataWithPCKS7PaddingAndKey:self.key
                                                                iv:iv];
}
...
- (NSData *)ciphertextFromBlob:(NSData *)blob {
  ...
  for (NSData *expected in expectedValues) {
    if (blob.length < offset + expected.length ||
        ![[blob subdataWithRange:NSMakeRange(offset, expected.length)]
            isEqualToData:expected]) {
      return nil;
    }
  ...
  NSData *lengthWord =
      [blob subdataWithRange:NSMakeRange(offset, sizeof(NSUInteger))];
...
- (NSKeyedUnarchiver *)unarchiver {
  ...
  NSError *error = nil;
  NSKeyedUnarchiver *unarchiver =
      [[NSKeyedUnarchiver alloc] initForReadingFromData:self.plaintext
                                                  error:&error];
