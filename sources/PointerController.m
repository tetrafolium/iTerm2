...
- (void)performAction:(NSString *)action
  ...
  if ([action isEqualToString:kPasteFromClipboardPointerAction]) {
    if (argument.length) {
      [delegate_ advancedPasteWithConfiguration:argument
                                  fromSelection:NO
                                      withEvent:event];
    } else {
    ...
    [delegate_ smartSelectAndMaybeCopyWithEvent:event ignoringNewlines:NO];
  } else if ([action isEqualToString:
                         kSmartSelectionIgnoringNewlinesPointerAction]) {
    [delegate_ smartSelectAndMaybeCopyWithEvent:event ignoringNewlines:YES];
    ...
    [delegate_ newTabWithProfile:argument withEvent:event];
                 ...
                 isEqualToString:kNewVerticalSplitWithProfilePointerAction]) {
    [delegate_ newVerticalSplitWithProfile:argument withEvent:event];
  } else if ([action
                 isEqualToString:kNewHorizontalSplitWithProfilePointerAction]) {
...
- (NSString *)argumentForEvent:(NSEvent *)event
  ...
  if (numTouches <= 2) {
    ...
    return [PointerPrefsController
        argumentForTapWithTouches:numTouches
                        modifiers:[event it_modifierFlags]];
  }
}
...
- (BOOL)threeFingerTap:(NSEvent *)event {
  ...
  NSNumber *number = [[NSUserDefaults standardUserDefaults]
      objectForKey:@"com.apple.trackpad.threeFingerTapGesture"];
...
- (BOOL)mouseDown:(NSEvent *)event
  ...
  return [self actionForEvent:event clicks:clicks_
...
- (BOOL)pressureChangeWithEvent:(NSEvent *)event {
  if ([event respondsToSelector:@selector(stage)]) {
    ...
    if (event.stage == 2 && previousStage < 2) {
      NSString *action =
          [PointerPrefsController actionForGesture:kForceTouchSingleClick
                                         modifiers:[event it_modifierFlags]];
      NSString *argument =
          [PointerPrefsController argumentForGesture:kForceTouchSingleClick
                                           modifiers:[event it_modifierFlags]];
      if (action) {
        [self performAction:action forEvent:event withArgument:argument];
        return YES;
        ...
        NSNumber *number = [[NSUserDefaults standardUserDefaults]
            objectForKey:@"com.apple.trackpad.forceClick"];
        if (number && number.boolValue) {
          // This hack stolen from Firefox:
          // https://searchfox.org/mozilla-central/source/widget/cocoa/nsChildView.mm
          // I have no idea why -quicklookWithEvent: doesn't get called, but at
...
- (void)swipeWithEvent:(NSEvent *)event {
  ...
  NSString *action =
      [PointerPrefsController actionForGesture:gesture
                                     modifiers:[event it_modifierFlags]];
  NSString *argument =
      [PointerPrefsController argumentForGesture:gesture
                                       modifiers:[event it_modifierFlags]];
  if (action) {
    [self performAction:action forEvent:event withArgument:argument];
  }
