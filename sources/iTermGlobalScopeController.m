...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:NSStringFromSelector(@selector
                                                    (effectiveAppearance))]) {
...
- (instancetype)init {
  ...
  if (self) {
    ...
    [[NSUserDefaults standardUserDefaults]
        it_addObserverForKey:kPreferenceKeyTabStyle
                       block:^(id _Nonnull newValue) {
                         [weakSelf themeDidChange];
                       }];
    [[iTermVariableScope globalsScope] setValue:@(getpid())
                               forVariableNamed:iTermVariableKeyApplicationPID];
    _userVariables = [[iTermVariables alloc]
        initWithContext:iTermVariablesSuggestionContextNone
                  owner:self];
    [[iTermVariableScope globalsScope] setValue:_userVariables
                               forVariableNamed:@"user"];
    [[iTermVariableScope globalsScope]
                setValue:[self effectiveTheme]
                               ...
                               repeats:YES
                                 block:^(NSTimer *_Nonnull timer) {
                                   [[iTermVariableScope globalsScope]
                                       ...
                                       forVariableNamed:
                                           iTermVariableKeyApplicationLocalhostName];
                                 }];
  }
  return self;
}
