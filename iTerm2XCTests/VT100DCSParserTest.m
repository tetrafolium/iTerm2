...
- (void)testDCSControl {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP%c", VT100CC_ESC, VT100CC_LF];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateEntry);
}
...
- (void)testDCSBackspace {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP%c", VT100CC_ESC, VT100CC_DEL];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateEntry);
}
...
- (void)testDCSIntermediateIgnoreMany {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP 0%c%c%c0", VT100CC_ESC, VT100CC_LF,
                                   VT100CC_EM, VT100CC_FS];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateIgnore);
}
...
- (void)testDCSIntermediateIgnoreIgnoreEsc {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP 01%c", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
...
- (void)testDCSIntermediateIgnoreIgnoreEscAsciiST {
  // Enter ignore, then dcs escape, then passthrough, then ground; should still
  // be invalid.
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP 01%cabc%c\\", VT100CC_ESC, VT100CC_ESC,
                                   VT100CC_ESC];
  XCTAssert(token->type == VT100_INVALID_SEQUENCE);
  XCTAssert(_parser.state == kVT100DCSStateGround);
}
...
- (void)testDCSIntermediatePassthroughEsc {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP x%c", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateDCSEscape);
}
...
- (void)testDCSMultipleParameters {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP12%c3;45%c6;;0", VT100CC_ESC,
                                   VT100CC_LF, VT100CC_DEL];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateParam);
...
- (void)testDCSCatchesBinaryGarbage {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cPAbc%c%c%c%c~", VT100CC_ESC, VT100CC_LF,
                                   VT100CC_EM, VT100CC_FS, VT100CC_DEL];
...
- (void)testDCSPassthroughEsc {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cPAbcd%c", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_parser.state == kVT100DCSStateDCSEscape);
...
- (void)testDCSEverything {
  VT100Token *token = [self
      tokenForDataWithFormat:@"%cP<0;1;!\"abc%c\\", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == VT100_NOTSUPPORT);
  XCTAssert(_parser.state == kVT100DCSStateGround);
...
- (void)testDCSRequestTermcapTerminfo {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP+q%@%c\\", VT100CC_ESC,
                                   [self hexEncodedString:@"TN"], VT100CC_ESC];
...
- (void)testDCSEnterTmuxIntegration {
  ...
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP1000p%c\\", VT100CC_ESC, VT100CC_ESC];
...
- (void)testDCSTmuxHook {
  ...
  // Technically DCS should take ESC ESC as input to produce ESC as output, but
  // that's not how tmux works so we have a hack that in tmux mode only a single
  // ESC is treated as an ESC.
...
- (void)testDCSTmuxWrap {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cPtmux;%c%c[1m%c\\", VT100CC_ESC,
...
- (void)testDCSSavedState {
  NSString *wholeSequence = [NSString
      stringWithFormat:@"%cP<0;1;!\"abc%c\\", VT100CC_ESC, VT100CC_ESC];

  for (int i = 2; i < wholeSequence.length; i++) {
    VT100Token *token =
        [self tokenForDataWithFormat:@"%@", [wholeSequence substringToIndex:i]];
    XCTAssert(token->type == VT100_WAIT);

    token = [self tokenForDataWithFormat:@"%@%@", [@"-" stringRepeatedTimes:i],
                                         [wholeSequence substringFromIndex:i]];
    XCTAssert(_parser.state == kVT100DCSStateGround);
    XCTAssert([_parser.privateMarkers isEqualToString:@"<"]);
    NSArray *expected = @[ @"0", @"1", @"" ];
    XCTAssertEqualObjects(_parser.parameters, expected);
...
- (void)testDECRQSS {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%cP$q q%c\\", VT100CC_ESC, VT100CC_ESC];
...
- (void)testIssue9070 {
  VT100Parser *parser = [[[VT100Parser alloc] init] autorelease];
  parser.encoding = NSUTF8StringEncoding;
  unsigned char preambleBytes[8] = {0x1B, 0x50, 0x30, 0x3B,
                                    0x30, 0x3B, 0x38, 0x71};
