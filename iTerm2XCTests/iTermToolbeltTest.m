...
- (void)setUp {
  ...
  [[iTermShellHistoryController sharedInstance]
      eraseCommandHistoryForHost:host];
  ...
  XCTestExpectation *expectation =
      [[XCTestExpectation alloc] initWithDescription:@"launch session"];
  ...
  [_windowController.window
      setFrame:_windowController.window.screen.visibleFrame
       display:YES];
...
- (void)sendPrompt {
  NSString *promptLine = [NSString
      stringWithFormat:@"%c]1337;RemoteHost=user@hostname%c"
                       @"%c]1337;CurrentDir=%@%c"
                       @"%c]133;A%c"
                       @"> "
                       @"%c]133;B%c",
...
- (void)sendPromptAndStartCommand:(NSString *)command
                        toSession:(PTYSession *)session {
  NSString *promptLine = [NSString
      stringWithFormat:@"%c]1337;RemoteHost=user@hostname%c"
                       @"%c]1337;CurrentDir=%@%c"
                       @"%c]133;A%c"
                       ...
                       VT100CC_ESC, VT100CC_BEL,              // FinalTerm A
                       VT100CC_ESC, VT100CC_BEL,              // FinalTerm B
                       command, VT100CC_ESC, VT100CC_BEL];    // FinalTerm C
  [self sendData:[promptLine dataUsingEncoding:NSUTF8StringEncoding]
...
- (void)endCommand {
  NSString *promptLine =
      [NSString stringWithFormat:@"%c]133;D;1%c", VT100CC_ESC, VT100CC_BEL];
  [self sendData:[promptLine dataUsingEncoding:NSUTF8StringEncoding]
      toTerminal:_session.terminal];
}
...
- (void)testCapturedOutputShowsLineOnClick {
  ...
  XCTAssert(!NSIntersectsRect(
      _session.textview.enclosingScrollView.documentVisibleRect,
      rectForFirstCellOfCapturedLine));

  [tool.tableView selectRowIndexes:[NSIndexSet indexSetWithIndex:0]
...
- (void)testCapturedOutputActivatesTriggerOnDoubleClick {
  ...
  [tool.tableView.delegate performSelector:tool.tableView.doubleAction
                                withObject:nil];
...
- (NSAttributedString *)attributedStringInTableView:(NSTableView *)tableView
                                                row:(NSInteger)row {
...
- (void)testCommandHistoryBoldsCommandsForCurrentSession {
  ...
  ToolCommandHistoryView *tool = (ToolCommandHistoryView *)[_view.toolbelt
  ...
  NSArray *values = @[
    [self attributedStringInTableView:tool.tableView row:0],
...
- (void)testCommandHistoryEntersCommandOnDoubleClick {
  ...
  [tool.tableView
          selectRowIndexes:[NSIndexSet
                               indexSetWithIndex:tool.tableView.numberOfRows -
                                                 1]
      byExtendingSelection:NO];

  tool.toolWrapper.delegate.delegate = self;
  [tool.tableView.delegate performSelector:tool.tableView.doubleAction
                                withObject:tool.tableView];
...
- (void)testCommandHistoryWritesCdOnOptionDoubleClick {
  ...
  [tool.tableView.delegate performSelector:tool.tableView.doubleAction
                                withObject:tool.tableView.target];
  XCTAssertEqualObjects(_insertedText,
                        [@"cd " stringByAppendingString:_currentDir]);
}
...
- (void)testCommandHistoryLinkedToCapturedOutput {
  ...
  ToolCapturedOutputView *capturedOutputTool =
      (ToolCapturedOutputView *)[_view.toolbelt
          toolWithName:kCapturedOutputToolName];
  ToolCommandHistoryView *commandHistoryTool =
      (ToolCommandHistoryView *)[_view.toolbelt
          toolWithName:kCommandHistoryToolName];

  // Select first command
  [commandHistoryTool.tableView
          selectRowIndexes:[NSIndexSet indexSetWithIndex:0]
      byExtendingSelection:NO];
  NSString * (^getObject)(int) = ^NSString *(int row) {
    NSTextField *rowView = [[NSTableCellView
        castFrom:[capturedOutputTool.tableView.delegate
                              tableView:capturedOutputTool.tableView
                     viewForTableColumn:capturedOutputTool.tableView
                                            .tableColumns[0]
                                    row:0]] textField];
...
- (void)testDirectoriesUpdatesOnCd {
  ...
  ToolDirectoriesView *tool = (ToolDirectoriesView *)[_view.toolbelt
      toolWithName:kRecentDirectoriesToolName];
  XCTAssertEqual(tool.tableView.numberOfRows, 2);

  NSAttributedString *object = [self attributedStringInTableView:tool.tableView
                                                             row:0];
...
- (void)testToolbeltImpingesOnWindowWhenNearRightEdge {
  ...
  XCTAssertEqual(_windowController.window.frame.size.width,
                 originalWindowFrame.size.width);

  // TabView's frame should change
  XCTAssertNotEqual(_view.tabView.frame.size.width,
                    originalTabViewFrame.size.width);
}
...
- (void)testToolbeltGrowsWhenSpaceIsAvailableOnRight {
  ...
  XCTAssertNotEqual(_windowController.window.frame.size.width,
                    originalWindowFrame.size.width);

  // TabView's frame should not
  XCTAssertEqual(_view.tabView.frame.size.width,
                 originalTabViewFrame.size.width);
}
