...
- (void)testSimpleCase {
  iTermWeaklyReferenceableObject *object =
      [[iTermWeaklyReferenceableObject alloc] init];
...
- (void)testWeaklyReferencedObjectMethodBeforeDealloc {
  iTermWeaklyReferenceableObject *object =
      [[iTermWeaklyReferenceableObject alloc] init];
  iTermWeaklyReferenceableObject<iTermWeakReference> *weakReference =
      [object weakSelf];
...
- (void)testReleaseWeakReferenceBeforeObject {
  iTermWeaklyReferenceableObject *object =
      [[iTermWeaklyReferenceableObject alloc] init];
  iTermWeakReference *weakReference =
      [[iTermWeakReference alloc] initWithObject:object];
  [weakReference release];
  XCTAssert(object.retainCount == 1);
  [object release];
}
...
- (void)testReleaseTwoWeakReferencesBeforeObject {
  iTermWeaklyReferenceableObject *object =
      [[iTermWeaklyReferenceableObject alloc] init];
  iTermWeakReference *weakReference1 =
      [[iTermWeakReference alloc] initWithObject:object];
  iTermWeakReference *weakReference2 =
      [[iTermWeakReference alloc] initWithObject:object];
  [weakReference1 release];
  [weakReference2 release];
  XCTAssert(object.retainCount == 1);
...
- (void)testNullfiedAtStartOfDealloc {
  ...
  [object release];
  XCTAssert(((iTermWeakReference *)ref).weaklyReferencedObject == nil,
            @"Reference's object not nullified");
  XCTAssert(ok,
            @"Reference's object nullified after start of object's dealloc");
}
...
// This is a nondeterministic test that tries to trigger crashy race conditions.
// If it passes, you learn nothing, but if it crashes you have a bug :). It has
// caught a few problems so I'll keep it around with a low number of iterations.
- (void)testRaceConditions {
  dispatch_queue_t q1 =
      dispatch_queue_create("com.iterm2.WeakReferenceTest1", NULL);
  dispatch_queue_t q2 =
      dispatch_queue_create("com.iterm2.WeakReferenceTest2", NULL);
