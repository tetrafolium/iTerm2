...
typedef NSModalResponse (^WarningBlockType)(NSAlert *alert,
                                            NSString *identifier);
...
@interface iTermPasteHelperTest () <iTermPasteHelperDelegate,
                                    iTermWarningHandler>
@end
  ...
  [self sanitizeString:kTestString
                expect:expected
                 flags:(kPasteFlagsEscapeSpecialCharacters |
                        ...
                        kPasteFlagsBracket | kPasteFlagsBase64Encode |
...
- (void)testBasicPasteString {
  ...
  XCTAssert([[[[PasteboardHistory sharedInstance] entries][0] mainValue]
      isEqualToString:kHelloWorld]);
}
...
- (void)testMultilineWarningWithOverride {
  [iTermSelectorSwizzler
      swizzleSelector:@selector(promptForPasteWhenNotAtPrompt)
...
// You still get a close bracket even if you change your mind about wanting it
// unless the whole paste is queued.
- (void)testDelegateChangesItsMindAboutBracketingNoQueue {
  ...
  _shouldBracket = NO;
  [self runTimer];
  NSString *expected = [[@"\x1b[200~" stringByAppendingString:test]
      stringByAppendingString:@"\x1b[201~"];
  XCTAssert([_writeBuffer isEqualToString:expected]);
...
- (void)testDelegateChangesItsMindAboutBracketingWithQueue {
  ...
  [self runTimer];
  NSString *expected = [[[@"\x1b[200~" stringByAppendingString:test1]
      stringByAppendingString:@"\x1b[201~"] stringByAppendingString:test2];
  XCTAssert([_writeBuffer isEqualToString:expected]);
}
...
- (void)testPasteQueued {
  ...
  [self runTimer];
  XCTAssert(
      [_writeBuffer isEqualToString:[test1 stringByAppendingString:test2]]);
  NSTimeInterval expectedDuration = 4 * 0.01;
  XCTAssert(fabs(_helper.duration - expectedDuration) <
            kFloatingPointTolerance);
  XCTAssert([[[[PasteboardHistory sharedInstance] entries][0] mainValue]
      isEqualToString:test1]);
  XCTAssert([[[[PasteboardHistory sharedInstance] entries][1] mainValue]
      isEqualToString:test2]);
}
...
- (void)testQueuedKeystrokeAndPaste {
  ...
  [_helper pasteString:test2
      ...
      escapeShellChars:NO
          ...
          spacesPerTab:0];
  [self runTimer];
  XCTAssert([_writeBuffer isEqualToString:[[test1 stringByAppendingString:@"x"]
