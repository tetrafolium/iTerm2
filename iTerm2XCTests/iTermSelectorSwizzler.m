...
static NSString *const iTermSelectorSwizzlerContexts =
    @"iTermSelectorSwizzlerContexts";
...
- (void)unswizzle {
  method_setImplementation(self.originalMethod,
                           self.originalMethodImplementation);
}
...
- (IMP)swizzleInstanceMethodSelector:(SEL)selector withBlock:(id)fakeBlock {
  iTermSelectorSwizzlerContext *context =
      [iTermSelectorSwizzlerContext currentContext];
  assert(context);
  iTermSwizzledSelector *swizzledSelector =
      [[[iTermSwizzledSelector alloc] init] autorelease];
  swizzledSelector.originalMethod =
      class_getInstanceMethod([self class], selector);
  swizzledSelector.originalMethodImplementation =
      method_getImplementation(swizzledSelector.originalMethod);
  if (fakeBlock && context) {
    ...
    method_setImplementation(swizzledSelector.originalMethod,
                             fakeMethodImplementation);
  }
