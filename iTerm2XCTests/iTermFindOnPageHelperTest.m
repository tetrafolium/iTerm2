...
- (void)testFindRangeOfSearchResults_Random {
  for (int j = 0; j < 10000; j++) {
    ...
    [values sortUsingComparator:^NSComparisonResult(NSNumber *_Nonnull obj1,
    ...
    for (int k = 0; k < 100; k++) {
      ...
      XCTAssertTrue(NSEqualRanges(actual, expected),
                    @"Unequal ranges for values %@ and query location=%@ "
                    @"length=%@. Actual=%@, expected=%@",
                    values, @(range.location), @(range.length),
                    NSStringFromRange(actual), NSStringFromRange(expected));
    }
  }
}
...
- (void)testFindRangeOfSearchResults_Basic {
  ...
  NSRange actual =
      [helper rangeOfSearchResultsInRangeOfLines:NSMakeRange(30, 11)];
...
- (void)testFindRangeOfSearchResults_InexactMin {
  ...
  NSRange actual =
      [helper rangeOfSearchResultsInRangeOfLines:NSMakeRange(25, 11)];
...
- (void)testFindRangeOfSearchResults_InexactMax {
  ...
  NSRange actual =
      [helper rangeOfSearchResultsInRangeOfLines:NSMakeRange(30, 15)];
  NSRange expected = NSMakeRange(2, 3);
  XCTAssertTrue(NSEqualRanges(actual, expected));
}
...
- (void)testFindRangeOfSearchResults_NoResults {
  ...
  NSRange actual =
      [helper rangeOfSearchResultsInRangeOfLines:NSMakeRange(11, 5)];
