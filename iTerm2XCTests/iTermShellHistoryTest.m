...
static NSString *const kFakeCommandHistoryPlistPath =
    @"/tmp/fake_command_history.plist";
static NSString *const kFakeDirectoriesPlistPath =
    @"/tmp/fake_directories.plist";
...
@interface iTermShellHistoryControllerWithRAMStoreForTesting
    : iTermShellHistoryControllerForTesting
@end
...
- (instancetype)initWithGuid:(NSString *)guid
            shouldSaveToDisk:(BOOL)shouldSaveToDisk;
...
@implementation
    iTermShellHistoryControllerWithConfigurableStoreDefaultingToDiskForTesting
...
- (void)setUp {
  ...
  [[NSFileManager defaultManager]
      removeItemAtPath:[kSqlitePathForTest stringByAppendingString:_guid]
                 error:nil];
}
  ...
  NSDictionary *dictionary = @{
    @"user1@host1" : @[
      ...
      @{
        ...
        @"last used" : @(_now),
        @"use times" : @[
          @[ @(_now), @"/path4", @"mark-guid-4", @"command2", @1 ],
        ],
      },
    ],
    @"user2@host2" : @[ @{
      ...
      @"last used" : @(_now),
      @"use times" : @[
        @[ @(_now), @"/path5", @"mark-guid-5", @"command3", @1 ],
        @[ @(_now), @"/path5", @"mark-guid-5", @"command3", @1 ],
      ],
    } ]
  };
  NSString *plistPath =
      [kFakeCommandHistoryPlistPath stringByAppendingString:_guid];
  [NSKeyedArchiver archiveRootObject:dictionary toFile:plistPath];
  XCTAssert([[NSFileManager defaultManager] fileExistsAtPath:plistPath
                                                 isDirectory:nil]);
  iTermShellHistoryControllerWithConfigurableStoreDefaultingToDiskForTesting
      *historyController =
          [[[iTermShellHistoryControllerWithConfigurableStoreDefaultingToDiskForTesting
              alloc] initWithGuid:_guid shouldSaveToDisk:YES] autorelease];
  XCTAssert(![[NSFileManager defaultManager] fileExistsAtPath:plistPath
                                                  isDirectory:nil]);
  XCTAssert([[NSFileManager defaultManager]
      fileExistsAtPath:[kSqlitePathForTest stringByAppendingString:_guid]
           isDirectory:nil]);
  ...
  for (int iteration = 0; iteration < 2; iteration++) {
    ...
    for (NSString *key in dictionary) {
      VT100RemoteHost *remoteHost =
          [[[VT100RemoteHost alloc] init] autorelease];
      ...
      for (NSInteger i = 0; i < expectedEntries.count; i++) {
        ...
        XCTAssertEqualObjects(actualEntry.timeOfLastUse,
                              expectedEntry[@"last used"]);
        XCTAssertEqual(actualEntry.uses.count,
                       [expectedEntry[@"use times"] count]);

        NSOrderedSet<iTermCommandHistoryCommandUseMO *> *actualUses =
            actualEntry.uses;
...
- (void)testAddFirstCommandOnNewHost {
  iTermShellHistoryControllerForTesting *historyController =
      [[[iTermShellHistoryControllerForTesting alloc] initWithGuid:_guid]
          autorelease];
  ...
  [historyController addCommand:@"command1"
                    ...
                    inDirectory:@"/directory1"
                       withMark:mark];
  entries = [historyController commandHistoryEntriesWithPrefix:@""
                                                        onHost:remoteHost];
...
- (VT100RemoteHost *)addEntriesWithCommonPrefixes:
    (iTermShellHistoryControllerForTesting *)historyController {
  const NSTimeInterval now = 1000000;
  historyController.currentTime = now;
...
- (void)testSearchCommandEntriesByPrefix {
  iTermShellHistoryControllerForTesting *historyController =
      [[[iTermShellHistoryControllerForTesting alloc] initWithGuid:_guid]
          autorelease];
  VT100RemoteHost *remoteHost =
      [self addEntriesWithCommonPrefixes:historyController];
  NSArray *entries;
  entries = [historyController commandHistoryEntriesWithPrefix:@""
                                                        onHost:remoteHost];
  XCTAssertEqual(entries.count, self.commandWithCommonPrefixes.count - 1);
  entries = [historyController commandHistoryEntriesWithPrefix:@"a"
                                                        onHost:remoteHost];
  XCTAssertEqual(entries.count, 3);
  entries = [historyController commandHistoryEntriesWithPrefix:@"b"
                                                        onHost:remoteHost];
  XCTAssertEqual(entries.count, 1);
  entries = [historyController commandHistoryEntriesWithPrefix:@"c"
                                                        onHost:remoteHost];
  XCTAssertEqual(entries.count, 0);
}
...
- (void)testSearchCommandUsesByPrefix {
  ...
  NSArray *uses;
  uses =
      [historyController autocompleteSuggestionsWithPartialCommand:@""
                                                            onHost:remoteHost];
  ...
  uses =
      [historyController autocompleteSuggestionsWithPartialCommand:@"a"
                                                            onHost:remoteHost];
  XCTAssertEqual(uses.count, 3);
  uses =
      [historyController autocompleteSuggestionsWithPartialCommand:@"b"
                                                            onHost:remoteHost];
  XCTAssertEqual(uses.count, 1);
  uses =
      [historyController autocompleteSuggestionsWithPartialCommand:@"c"
                                                            onHost:remoteHost];
...
- (void)testEraseCommandHistoryWhenSavingToDisk {
  ...
  historyController = [[[iTermShellHistoryControllerForTesting alloc]
      initWithGuid:_guid] autorelease];
  XCTAssertFalse([historyController haveCommandsForHost:remoteHost]);
}

- (void)testEraseCommandHistoryInMemoryOnly {
  iTermShellHistoryControllerWithRAMStoreForTesting *historyController =
      [[[iTermShellHistoryControllerWithRAMStoreForTesting alloc]
          initWithGuid:_guid] autorelease];
  VT100RemoteHost *remoteHost = [[[VT100RemoteHost alloc] init] autorelease];
  [historyController addCommand:@"command"
                         onHost:remoteHost
...
- (void)testFindCommandUseByMark {
  ...
  iTermCommandHistoryCommandUseMO *use =
      [historyController commandUseWithMarkGuid:mark.guid onHost:remoteHost];
  XCTAssertEqualObjects(use.directory, @"/directory2");
}
...
// There was a bug where migrating directories would stop on migrated commands.
// This tests migrating both to ensure this isn't an issue.
- (void)testCommandHistoryAndDirectoryMigration {
  ...
  NSString *commandPlistPath =
      [kFakeCommandHistoryPlistPath stringByAppendingString:_guid];
  [NSKeyedArchiver archiveRootObject:commandDictionary toFile:commandPlistPath];
  XCTAssert([[NSFileManager defaultManager] fileExistsAtPath:commandPlistPath
                                                 isDirectory:nil]);
  ...
  NSString *directoriesPlistPath =
      [kFakeDirectoriesPlistPath stringByAppendingString:_guid];
  [NSKeyedArchiver archiveRootObject:directoryDictionary
                              toFile:directoriesPlistPath];
  XCTAssert([[NSFileManager defaultManager]
      fileExistsAtPath:directoriesPlistPath
           isDirectory:nil]);
...
// If you migrate from plist when there's already a Core Data database, it
// should just merge it in without causing duplicate host records.
- (void)testNoDuplicateHostRecordsAfterDoubleMigration {
  ...
  [NSKeyedArchiver archiveRootObject:commandDictionary2
                              toFile:commandPlistPath];
  XCTAssert([[NSFileManager defaultManager] fileExistsAtPath:commandPlistPath
                                                 isDirectory:nil]);
  ...
  [NSKeyedArchiver archiveRootObject:directoryDictionary2
                              toFile:directoriesPlistPath];
  XCTAssert([[NSFileManager defaultManager]
      fileExistsAtPath:directoriesPlistPath
           isDirectory:nil]);
...
- (void)testCorruptDatabase {
  ...
  XCTAssertTrue([historyController haveCommandsForHost:remoteHost]);
  NSMutableData *data =
      [NSMutableData dataWithContentsOfFile:[kSqlitePathForTest
                                                stringByAppendingString:_guid]];
  for (int i = 1024; i < data.length; i += 16) {
    ((char *)data.mutableBytes)[i] = i & 0xff;
  }
  [data writeToFile:[kSqlitePathForTest stringByAppendingString:_guid]
         atomically:NO];
...
- (void)testSuccessfulDirectoriesMigration {
  ...
  for (int iteration = 0; iteration < 2; iteration++) {
    ...
    NSArray<iTermRecentDirectoryMO *> *directories =
        [historyController directoriesSortedByScoreOnHost:host];
...
- (void)testAddFirstDirectoryToNewHost {
  ...
  [historyController recordUseOfPath:@"/test/path"
                              onHost:remoteHost
                            isChange:YES];
...
- (void)testAbbreviationSafeIndexes {
  ...
  for (NSInteger i = 0; i < directories.count; i++) {
    ...
    if ([directory.path isEqualToString:paths[0]]) {
      ...
      NSIndexSet *actual = [historyController
          abbreviationSafeIndexesInRecentDirectory:directory];
...
- (void)testSortDirectoriesByScore {
      ...
      for (NSNumber *date in @[ @1, @2 ]) {
        ...
        for (int i = 0; i < useCount.intValue; i++) {
          NSString *path = [NSString
              stringWithFormat:@"/%@/uses%@/date%@",
                               starred.boolValue ? @"starred" : @"unstarred",
                               useCount, date];
          iTermRecentDirectoryMO *directory =
              [historyController recordUseOfPath:path
                                          onHost:remoteHost
                                        isChange:YES];
  ...
  // Number of uses will be considered equivalent for these so they'll sort by
  // date.
  historyController.currentTime = 2;
  for (int i = 0; i < 9; i++) {
    [historyController recordUseOfPath:@"/unstarred/uses9/date2"
                                onHost:remoteHost
                              isChange:YES];
  }
  historyController.currentTime = 1;
  for (int i = 0; i < 10; i++) {
    [historyController recordUseOfPath:@"/unstarred/uses10/date1"
                                onHost:remoteHost
                              isChange:YES];
  }
  ...
  NSArray *expected = @[
    @"/starred/uses16/date2", @"/starred/uses16/date1", @"/starred/uses8/date2",
    @"/starred/uses8/date1", @"/starred/uses1/date2", @"/starred/uses1/date1",
    @"/unstarred/uses16/date2", @"/unstarred/uses16/date1",
    @"/unstarred/uses9/date2", @"/unstarred/uses8/date2",
    @"/unstarred/uses10/date1", @"/unstarred/uses8/date1",
    @"/unstarred/uses1/date2", @"/unstarred/uses1/date1"
  ];
  NSArray<iTermRecentDirectoryMO *> *directories =
      [historyController directoriesSortedByScoreOnHost:remoteHost];
...
- (void)testBackingStoreDidChange {
  iTermShellHistoryControllerWithConfigurableStoreDefaultingToDiskForTesting
      *historyController =
          [[[iTermShellHistoryControllerWithConfigurableStoreDefaultingToDiskForTesting
              alloc] initWithGuid:_guid] autorelease];
  ...
  [historyController recordUseOfPath:@"/test2/path2"
                              onHost:remoteHost
                            isChange:YES];
  [historyController addCommand:@"command1"
                         onHost:remoteHost
