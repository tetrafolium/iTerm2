...
- (void)tearDown {
  NSFileHandle *fileHandle =
      [NSFileHandle fileHandleForWritingAtPath:kDiffScriptPath];
...
- (void)launchProfileInCurrentTerminal:(NSDictionary *)profile
...
- (void)invokeMenuItemWithSelector:(SEL)selector tag:(NSInteger)tag {
  NSMenuItem *fakeMenuItem =
      [[[NSMenuItem alloc] initWithTitle:@"Fake Menu Item"
                                  action:selector
...
- (void)testPaste {
  ...
  [[NSPasteboard generalPasteboard] setString:@"test"
...
- (void)testPasteSelection {
  ...
  XCTAssert(
      [_methodsCalled[@"textViewPasteFromSessionWithMostRecentSelection:1"]
          intValue] == 1);
}
...
- (PTYSession *)sessionWithProfileOverrides:(NSDictionary *)profileOverrides
  ...
  NSString *plistFile =
      [[NSBundle bundleForClass:[self class]] pathForResource:@"DefaultBookmark"
                                                       ofType:@"plist"];
  ...
  NSRect theFrame = NSMakeRect(
      0, 0,
      size.width * session.textview.charWidth +
          [iTermPreferences intForKey:kPreferenceKeySideMargins] * 2,
      size.height * session.textview.lineHeight +
...
- (NSImage *)imageForInput:(NSString *)input
          ...
          profileOverrides:(NSDictionary *)profileOverrides
                      size:(VT100GridSize)size {
  PTYSession *session = [self sessionWithProfileOverrides:profileOverrides
                                                     size:size];
...
- (NSString *)shortNameForGolden:(NSString *)name {
  ...
  if ([[[iTermApplication sharedApplication] delegate] isRunningOnTravis]) {
    // Travis runs in a VM that renders text a little differently than a retina
    // device running the app in low-res mode.
    domain = @"travis-";
  } else if ([[NSScreen mainScreen] backingScaleFactor] == 1.0) {
    domain = @"nonretina-";
  }
  return [NSString
      stringWithFormat:@"PTYTextViewTest-golden-%@%@.png", domain, name];
}
...
// Minor differences in anti-aliasing on different machines (even running the
// same version of the OS) cause false failures with golden images, so we'll
// ignore tiny differences in brightness.
- (BOOL)image:(NSData *)image1
    approximatelyEqualToImage:(NSData *)image2
                         size:(NSSize)size
                        stats:(iTermDiffStats *)stats
  ...
  for (int i = 0; i < image1.length; i += 4) {
    CGFloat brightness1 = PerceivedBrightness(
        bytes1[i + 0] / 255.0, bytes1[i + 1] / 255.0, bytes1[i + 2] / 255.0);
    CGFloat brightness2 = PerceivedBrightness(
        bytes2[i + 0] / 255.0, bytes2[i + 1] / 255.0, bytes2[i + 2] / 255.0);
  ...
  NSData *encodedDiffImage =
      [diffImage dataForFileOfType:NSBitmapImageFileTypePNG];
...
- (void)doGoldenTestForInput:(NSString *)input
  ...
  if (createGolden) {
    ...
    if (!golden) {
      golden = [[NSBundle mainBundle]
          imageForResource:[self shortNameForGolden:name]];
    }
    XCTAssertNotNil(golden,
                    @"Failed to load golden image with name %@, short name %@",
                    goldenName, [self shortNameForGolden:name]);
    ...
    XCTAssertEqual(goldenData.length, actualData.length,
                   @"Different number of pixels between %@ and %@", golden,
                   actual);
    iTermDiffStats stats = {0};
    NSString *diffPath = [NSString stringWithFormat:@"/tmp/diff-%@.png", name];
    BOOL ok = [self image:goldenData
        approximatelyEqualToImage:actualData
                             size:actual.size
                            stats:&stats
                         diffPath:diffPath];
    if (ok) {
      NSLog(@"Tests “%@” ok with variance: %f. Max diff: %f", name,
            stats.variance, stats.maxDiff);
    } else {
      char *projectDir = STRINGIFY_MACRO(PROJECT_DIR);
      NSString *sourceFolder = [NSString stringWithUTF8String:projectDir];
      if (sourceFolder && ![[[iTermApplication sharedApplication] delegate]
                              isRunningOnTravis]) {
        NSData *pngData = [actual dataForFileOfType:NSBitmapImageFileTypePNG];
        NSString *sourceName =
            [[[[sourceFolder stringByAppendingPathComponent:@"tests/Goldens"]
                stringByAppendingPathComponent:@"PTYTextViewTest-golden-"]
                stringByAppendingString:name] stringByAppendingString:@".png"];
        [pngData writeToFile:sourceName atomically:NO];
        NSLog(@"Wrote to golden file at %@", sourceName);
      }

      NSString *failPath =
          [NSString stringWithFormat:@"/tmp/failed-%@.png", name];
      [[actual dataForFileOfType:NSBitmapImageFileTypePNG] writeToFile:failPath
                                                            atomically:NO];
    ...
    XCTAssert(ok, @"variance=%f maxdiff=%f deciles=%@", stats.variance,
              stats.maxDiff, [self decilesInStats:stats]);
  }
}
...
- (NSString *)sgrSequenceWithSubparams:(NSArray *)values {
  return [NSString stringWithFormat:@"%c[%@m", VT100CC_ESC,
  ...
  [self doGoldenTestForInput:@"abcd"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100GridWindowedRange range =
                              VT100GridWindowedRangeMake(
                                  VT100GridCoordRangeMake(1, 0, 3, 0), 0, 0);
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
...
// A 2x2 box should be selected. The selection color is grayed because the
// window is unfocused.
- (void)testBoxSelection {
  [self doGoldenTestForInput:@"abcd\r\nefgh\r\nijkl\r\nmnop"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100GridWindowedRange range =
                              VT100GridWindowedRangeMake(
                                  VT100GridCoordRangeMake(1, 1, 3, 2), 0, 0);
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
...
// ab, fg, and jk should be selected. The selection color is grayed because the
// window is unfocused.
  ...
  [self doGoldenTestForInput:@"abcd\r\nefgh\r\nijkl\r\nmnop"
                        ...
                        hook:^(PTYTextView *textView) {
                          ...
                          range = VT100GridWindowedRangeMake(
                              VT100GridCoordRangeMake(0, 0, 2, 0), 0, 0);
                          subSelection = [iTermSubSelection
                              subSelectionWithRange:range
                                               mode:kiTermSelectionModeCharacter
                                              width:textView.dataSource.width];
                          [textView.selection addSubSelection:subSelection];
...
// The middle two letters should be selected. The selection color is grayed
// because the window is unfocused.
- (void)testWindowedCharacterSelection {
  [self doGoldenTestForInput:@"abcd\r\nefgh\r\nijkl\r\nmnop"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100GridWindowedRange range =
                              VT100GridWindowedRangeMake(
                                  VT100GridCoordRangeMake(1, 0, 3, 3), 1, 2);
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
                                               mode:kiTermSelectionModeBox
                                              width:textView.dataSource.width];
...
// A cell cells after the a are selected. The selection color is grayed because
...
// The area between a and b is selected, and so is b. The selection color is
// grayed because the window is unfocused.
- (void)testSelectedTab {
  [self doGoldenTestForInput:@"a\tb"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100GridWindowedRange range =
                              VT100GridWindowedRangeMake(
                                  VT100GridCoordRangeMake(7, 0, 9, 0), 0, 0);
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
                                               mode:kiTermSelectionModeCharacter
                                              width:textView.dataSource.width];
...
// Although one of the tab fillers after a is selected, only a should appear
// selected. The selection color is grayed because the window is unfocused.
- (void)testSelectedTabFillerWithoutTab {
  [self doGoldenTestForInput:@"a\tb"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100GridWindowedRange range =
                              VT100GridWindowedRangeMake(
                                  VT100GridCoordRangeMake(0, 0, 3, 0), 0, 0);
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
                                               mode:kiTermSelectionModeCharacter
                                              width:textView.dataSource.width];
...
// By default, the text view is not the first responder. Ensure the color is
// correct when it is FR.
                        ...
                        hook:^(PTYTextView *textView) {
                          ...
                          iTermSubSelection *subSelection = [iTermSubSelection
                              subSelectionWithRange:range
                                              ...
                                              width:textView.dataSource.width];
                          [textView.selection addSubSelection:subSelection];
                          textView.drawingHook =
                              ^(iTermTextDrawingHelper *helper) {
...
- (void)testBadge {
  [self doGoldenTestForInput:@"\n\n\n\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
                             @"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\x1b[42mabc"
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
...
- (void)test256Colors {
  ...
  NSArray *names =
      @[ @"black", @"gray", @"red", @"green", @"blue", @"purple", @"muddy" ];
  [input appendString:@"        "];
...
- (NSString *)sequenceForForegroundColorWithRed:(CGFloat)red
                                          green:(CGFloat)green
                                           blue:(CGFloat)blue {
  return [NSString stringWithFormat:@"\e[38:2:%d:%d:%dm", (int)(red * 255),
...
- (NSString *)sequenceForBackgroundColorWithRed:(CGFloat)red
                                          green:(CGFloat)green
                                           blue:(CGFloat)blue {
  return [NSString stringWithFormat:@"\e[48:2:%d:%d:%dm", (int)(red * 255),
                                    (int)(green * 255), (int)(blue * 255)];
}
...
// Red diagonal stripes should fill the background including side margins, top,
// and excess. It should be visible over the badge and nondefault background
// colors.
...
- (void)testUseNonAsciiFont {
  [self doGoldenTestForInput:@"aé"
                        ...
                        hook:nil
            profileOverrides:@{
              KEY_NON_ASCII_FONT : [[NSFont fontWithName:@"Courier"
                                                    size:8] stringValue],
              KEY_USE_NONASCII_FONT : @YES
            }
                createGolden:NO
                        ...
                        hook:^(PTYTextView *textView) {
                          textView.drawingHook =
                              ^(iTermTextDrawingHelper *helper) {
                                ...
                                helper.cursorBlinking =
                                    YES; // PTYTextView sets this based on if
                                         // the window is key
                              };
                        }
...
- (void)testScrollbackOverflow {
  // Tests receiving input between -refresh (which calls setNeedsDisplay) and
  // -drawRect. The most up-to-date model should be drawn. This is a departure
  // from how 2.0 worked, which tried to draw how things "were" at the time
  // refresh was called (but didn't really succeed).
  [self doGoldenTestForInput:@"a\r\nb\r\n"
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
                          [textView refresh];
...
// Red with 50% black should show through background and text should be normal
// gray..
- (void)testTransparency {
  [self doGoldenTestForInput:@"a\r\nb\r\n"
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
                          // Change the session's class to one that always
                          // returns YES for use transparency.
                          PTYSession *session = (PTYSession *)textView.delegate;
                          object_setClass(
                              session,
                              [iTermFakeSessionForPTYTextViewTest class]);
                          textView.drawingHook =
                              ^(iTermTextDrawingHelper *helper) {
...
- (void)testIMEWithAmbiguousIsDoubleWidth {
  [self
      doGoldenTestForInput:@"x"
                      name:NSStringFromSelector(_cmd)
                      hook:^(PTYTextView *textView) {
                        [textView
                               setMarkedText:@"Γ" // U+0393 (hex), Greek capital
                                                  // gamma, is ambiguous width
...
- (void)testIMEWrapsDoubleWidthAtEndOfLine {
  [self doGoldenTestForInput:@"x"
                        ...
                        hook:^(PTYTextView *textView) {
                          ...
                          [textView setMarkedText:@"aᄀ" // U+1100 (hex), HANGUL
                                                         // CHOSEONG KIYEOK, is
                                                         // double width.
...
// Background image low blending - less of background shows through on default
// bg color Background noise should be subtle.
- (void)testBackgroundImageLowBlending {
  ...
  [self doGoldenTestForInput:@"a\e[31mb\e[41mc"
                        ...
                        hook:nil
            profileOverrides:@{
              KEY_BACKGROUND_IMAGE_LOCATION : pathToImage,
              KEY_BLEND : @0.3
            }
                createGolden:NO
...
// Background image high blending - more of background shows through on default
// bg color Background noise should be quite visible (annoyingly so).
...
// The first line is fg/bg swapped vs the second line. a and b on the second
// line should have transparent (default) backgrounds; all others should have
...
// Default bg becomes gray. Default fg over default bg (a in second row) becomes
// black.
- (void)testBackgroundImageWithGloballyInvertedColors {
  NSString *pathToImage = [self pathForTestResourceNamed:@"TestBackground.png"];
  [self
      doGoldenTestForInput:@"\e[7ma\e[31mb\e[42mc\r\n" // reversed
                           @"\e[0ma\e[31mb\e[42mc"     // regular
                           @"\e[?5h" // invert colors globally (affects default
                                     // bg and default fg when over default bg)
                      name:NSStringFromSelector(_cmd)
                      hook:nil
          profileOverrides:@{
            KEY_BACKGROUND_IMAGE_LOCATION : pathToImage,
...
// Default background color is transparent on a and b. Nondefault red background
// is opaque on c (which you can't see because it's also red).
...
// Smart cursor color
// All white neighbors->cursor will be black because text color is too close to
// white.
...
// Dark red is distant from the default cursor color so the center cell should
// be white on dark red.
- (void)testSmartCursorColor_allWhiteDarkRedFore {
  NSString *white = [self sequenceForBackgroundColorWithRed:1 green:1 blue:1];
  NSString *input = [NSString
      stringWithFormat:@"%@%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H",
                       [self sequenceForForegroundColorWithRed:0.5
                                                         green:0
                                                          blue:0],
                       white, white, white, white, white, white, white, white,
                       white];
...
// All neighbors are black so cursor will be black on default foreground gray
// color.
...
// All neighbors are dark gray (close to text color) so cursor will be black on
// white.
- (void)testSmartCursorColor_allDarkGray {
  NSString *darkGray = [self sequenceForBackgroundColorWithRed:0.4
                                                         green:0.4
                                                          blue:0.4];
  NSString *input = [NSString
      stringWithFormat:@"%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H", darkGray,
...
// Cursor should be visible when all text is default fg/bg with globally
// inverted colors.
- (void)testSmartCursorColor_reverseVideo {
  NSString *input =
      [NSString stringWithFormat:@"\e[?5hxxx\r\nxxx\r\nxxx\e[2;2H"];
...
// Cursor should be visible when all text is non-default fg/default bg with
// globally inverted colors.
- (void)testSmartCursorColor_reverseVideoNondefaultForeground {
  NSString *input =
      [NSString stringWithFormat:@"\e[31m\e[?5hxxx\r\nxxx\r\nxxx\e[2;2H"];
...
// Colors are globally inverted and the cursor is on text that is red on default
// bg (black). Without a cursor, it would render as red on gray. A naïve color
// swap for the cursor would give gray on red. Since that is terrible contrast,
// the cursor renders as white on red.
...
// Without a cursor, the center character would be default fg (gray) on red. It
// should render the cursor as red on gray.
- (void)testSmartCursorColor_reverseVideoNondefaultBackground {
  NSString *input =
      [NSString stringWithFormat:@"\e[41m\e[?5hxxx\r\nxxx\r\nxxx\e[2;2H"];
...
// The char the cursor is on has a red background. It should render as white on
// default bg (black) because red on black wouldn't have enough contrast.
...
// The char under the cursor is red over light gray. Global reverse video
// doesn't affect nondefault background colored chars. So the cursor renders as
// white on red (white for contrast).
- (void)
    testSmartCursorColor_reverseVideoNondefaultForegroundAndBackgroundUnderCursor {
  NSString *input = [NSString
      stringWithFormat:@"\e[?5hxxx\r\nx\e[47m\e[31mx\e[mx\r\nxxx\e[2;2H"];
...
// All neighbors are the same as the default text color. Cursor should be black
// on white or white on black (either is ok; one must be chosen arbitrarily).
- (void)testSmartCursorColor_allCursorColor {
  NSString *gray = [self sequenceForBackgroundColorWithRed:0.5
                                                     green:0.5
                                                      blue:0.5];
  NSString *input = [NSString
      stringWithFormat:@"%@%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H",
...
// The frame is visible (light gray) but smart cursor color doesn't ensure text
// is visible when unfocused.
...
- (void)testSmartCursorColor_whiteCrossGrayCorners {
  ...
  NSString *input = [NSString
      stringWithFormat:@"%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H", gray,
                       white, gray, white, white, white, gray, white, gray];
  [self doGoldenTestForInput:input
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
                          textView.drawingHook =
...
- (void)testSmartCursorColor_manyGrayOneWhite {
  ...
  NSString *input = [NSString
      stringWithFormat:@"%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H", gray,
                       gray, gray, white, gray, gray, gray, gray, gray];
  [self doGoldenTestForInput:input
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
                          textView.drawingHook =
...
// With gray, white, and black neighbors, the cursor should be black on light
// gray.
- (void)testSmartCursorColor_manyGrayOneWhiteOneBlack {
  ...
  NSString *input = [NSString
      stringWithFormat:@"%@x%@x%@x\r\n%@x%@x%@x\r\n%@x%@x%@x\e[2;2H", gray,
                       gray, gray, white, gray, gray, gray, black, gray];
  [self doGoldenTestForInput:input
                        name:NSStringFromSelector(_cmd)
                        hook:^(PTYTextView *textView) {
                          textView.drawingHook =
...
// Cursor frame should be a medium gray, between the background color of the
// cell it's on and its white neighbor. Dark gray would also be ok (actually,
// would be better).
...
// The center cell is gray on black. Inverted, it would be black on gray. With
// smart cursor color, it should become white on black. That's kind of
// confusing, but this is a weird edge case.
...
// Cursor boost should dim everything but the cursor as compared to
// testSmartCursorColor_manyGrayOneWhiteOneBlack
...
- (void)testUnderlineHost {
  [self doGoldenTestForInput:@"abcdefg"
                        ...
                        hook:^(PTYTextView *textView) {
                          textView.drawingHook =
                              ^(iTermTextDrawingHelper *helper) {
                                helper.underlinedRange =
                                    VT100GridAbsWindowedRangeMake(
                                        VT100GridAbsCoordRangeMake(2, 0, 2, 1),
                                        0, 5);
                              };
                        }
            profileOverrides:nil
...
- (void)testMinimumContrast {
  NSString *input = [NSString
      stringWithFormat:
          @"%@%@x",
          [self sequenceForForegroundColorWithRed:.51
                                            green:.59
                                             blue:.85], // Puke green (should
                                                        // render as same color)
          [self sequenceForBackgroundColorWithRed:.45
                                            green:.64
                                             blue:.39]]; // Similar-brightness
                                                         // blue (should render
                                                         // black)
...
- (void)testCursorBoost {
  NSString *input =
      [NSString stringWithFormat:@"a%@b%@c\e[m ",
                                 [self sequenceForForegroundColorWithRed:.51
                                                                   green:.59
                                                                    blue:.85],
                                 [self sequenceForBackgroundColorWithRed:.45
  ...
  [self doGoldenTestForInput:input
            ...
            profileOverrides:@{
              KEY_CURSOR_BOOST : @0.5,
              KEY_CURSOR_COLOR :
                  [[NSColor colorWithCalibratedRed:.45
                                             green:.64
                                              blue:.39
                                             alpha:1] dictionaryValue]
            }
...
// Everything is washed out. The c is as dark as can be, but still washed out.
// The cursor is lightish green.
...
// Various different date formats (as described in comments below) should be
// seen on the right.
- (void)testTimestamps {
  [self doGoldenTestForInput:@"\e[41mabcdefghijklmn"
                        ...
                        hook:^(PTYTextView *textView) {
                          VT100Screen *screen =
                              (VT100Screen *)textView.dataSource;
                          NSTimeInterval now = 449711536;
                          const NSTimeInterval day = 86400;
                          int line = 0;
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - 1]; // HH:MM:SS
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - day - 1]; // DOW HH:MM:SS
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - 6 * day]; // DOW HH:MM:SS
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - 6 * day - 1]; // MM/DD HH:MM:SS
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - 180 * day]; // MM/DD HH:MM:SS
                          [[screen.currentGrid lineInfoAtLineNumber:line++]
                              setTimestamp:now - 180 * day -
                                           1]; // MM/DD/YYYY HH:MM:SS
                          textView.drawingHook =
                              ^(iTermTextDrawingHelper *helper) {
                                helper.showTimestamps = YES;
                                helper.now = now;
                                helper.useTestingTimezone =
                                    YES; // Use GMT so test can pass anywhere.
...
- (void)testMark {
  [self doGoldenTestForInput:@"abc\r\ndef\r\nghi"
                        ...
                        hook:^(PTYTextView *textView) {
                          ...
                          VT100ScreenMark *mark = [session
                              markAddedAtCursorOfClass:[VT100ScreenMark class]];
                          mark.code = 1;
                        }
            profileOverrides:nil
                createGolden:NO
...
- (void)testFindMatches {
  [self
      doGoldenTestForInput:@"abxxfghxxxxl"
                      name:NSStringFromSelector(_cmd)
                      hook:^(PTYTextView *textView) {
                        // Need to call refresh to clear dirty flags, otherwise
                        // find matches get reset when refresh gets called.
                        [textView refresh];
                        ...
                        [textView findString:@"xx"
                               forwardDirection:NO
                                           mode:
                                               iTermFindModeCaseSensitiveSubstring
                                     withOffset:0
...
// Stacked acute accents and cedillas should draw over green backgrounds on line
// above and below the blue arrow, which also runs into the cell with the a.
// Nothing should be clipped.
- (void)testOversizeGlyphs {
  // Combining accents that draw into line above and line below. The arrow glyph
  // is single-width but occupies two cells. Although other text may draw over
  // the out-of-range bits the background should not obscure any of it.
...
// Various emoji, as seen below should be drawn. The second row has a red
// background.
- (void)testEmoji {
  // The exclamation point tests the case where CTRunGetGlyphsPtr returns nil.
  // It has a combining mark that colors it.
...
// Double width lorem ipsum. e and m have yellow bg. i has green bg. m and i are
// red. Acute accent on p.
- (void)testDoubleWidthCharacter {
  [self doGoldenTestForInput:
            @"123456789Ｌｏｒ\e[43mｅ\e[31mｍ\e[42mｉ\e[mｐ\u0301ｓｕｍ"
                        name:NSStringFromSelector(_cmd)
                        hook:nil
            profileOverrides:nil
                createGolden:NO
...
// A box divided into four parts. The middle row is red on green. The row above
// is red on black. There's a frame cursor.
- (void)testBoxDrawing {
...
// Faint regular text and faint bold text, as compared to their non-faint
// versions. The faint ones should be darker.
- (void)testFaintText {
...
// Faint regular text and faint bold text, as compared to their non-faint
// versions. The faint ones should be darker. The background is 50% red due to
// transparency.
...
// It can happen that a line like [double-width character][dwc-right]["a"] will
// be drawn starting with the dwc-right. There was a bug where no advance was
// given for the dwc-right because it was assumed that the DWC itself was always
// drawn. That ceased to be the case when the "draw an extra ring of glyphs"
// algorithm was implemented (it accommodates oversize glyphs outside the
// drawing rect).
//
// In this test, the first column should be empty and the second column should
// hold an "a".
...
// A 16x16 grid of the ansi colors with columns having similar bg's and rows
// having similar fg's.
- (void)testAnsiColors {
  // Test every combination of foreground and background.
  NSMutableString *input = [NSMutableString string];

  for (NSNumber *fgNumber in @[
         @30, @31, @32, @33, @34, @35, @36, @37, @90, @91, @92, @93, @94, @95,
         @96, @97
       ]) {
    for (NSNumber *bgNumber in @[
           @40, @41, @42, @43, @44, @45, @46, @47, @100, @101, @102, @103, @104,
           @105, @106, @107
         ]) {
      ...
      [input
          appendFormat:@"%@%@x", [self sgrSequence:fg], [self sgrSequence:bg]];
    }
    [input appendFormat:@"\r\n"];
  }
...
- (void)testSelectedTextVeryBasic {
  // Given
  PTYSession *session =
      [self sessionWithProfileOverrides:@{} size:VT100GridSizeMake(10, 2)];
  _textView.dataSource = session.screen;
  NSString *text = @"123456789";
  [session synchronousReadTask:text];
  iTermFakeUserDefaults *fakeDefaults =
      [[[iTermFakeUserDefaults alloc] init] autorelease];
  ...
  [iTermSelectorSwizzler swizzleSelector:@selector(standardUserDefaults)
      ...
      forBlock:^{
        ...
        NSAttributedString *selectedAttributedText =
            [_textView selectedTextWithStyle:iTermCopyTextStyleAttributed
                                cappedAtSize:0
                           minimumLineNumber:0];
...
- (void)testSelectedTextWithSizeCap {
  ...
  NSString *selectedText =
      [_textView selectedTextWithStyle:iTermCopyTextStylePlainText
                          cappedAtSize:5
                     minimumLineNumber:0];
...
- (void)testSelectedAttributedTextIncludesBoldAttribute {
  PTYSession *session =
      [self sessionWithProfileOverrides:@{} size:VT100GridSizeMake(20, 2)];
  _textView.dataSource = session.screen;
  NSString *text = @"regular\e[1mbold";
  [session synchronousReadTask:text];
  [_textView selectAll:nil];
  NSAttributedString *selectedAttributedText =
      [_textView selectedTextAttributed:YES
                           cappedAtSize:11
                      minimumLineNumber:0];
  ...
  NSDictionary *regularAttributes =
      [selectedAttributedText attributesAtIndex:0 effectiveRange:&range];
  ...
  NSDictionary *boldAttributes =
      [selectedAttributedText attributesAtIndex:kRegularLength
  ...
  NSFont *regularFont = [[selectedAttributedText attributesAtIndex:0
                                                    effectiveRange:nil]
