...
- (void)testTypeFunctions {
  ...
  VT100GridRun runFromCoords = VT100GridRunFromCoords(
      VT100GridCoordMake(1, 2), VT100GridCoordMake(2, 4), 5);
  // .....
  // .....
  // .1234
  // 56789
...
- (void)testMarkAndClearDirty {
  // This test assumes that underlying implementation of dirty chars is a range
  // per line.
  ...
  [grid markCharsDirty:YES
            inRectFrom:VT100GridCoordMake(1, 1)
                    to:VT100GridCoordMake(5, 1)];
  XCTAssert(![grid isCharDirtyAt:VT100GridCoordMake(0, 1)]);
  XCTAssert([grid isCharDirtyAt:VT100GridCoordMake(1, 1)]);
  XCTAssert([grid isCharDirtyAt:VT100GridCoordMake(2, 1)]);
  XCTAssert([grid isCharDirtyAt:VT100GridCoordMake(3, 1)]);
  ...
  [grid markCharsDirty:NO
            inRectFrom:VT100GridCoordMake(2, 1)
  ...
  [grid markCharsDirty:NO
            inRectFrom:VT100GridCoordMake(3, 1)
                    to:VT100GridCoordMake(7, 1)];
  XCTAssert(![grid isCharDirtyAt:VT100GridCoordMake(0, 1)]);
  XCTAssert([grid isCharDirtyAt:VT100GridCoordMake(1, 1)]);
  XCTAssert([grid isCharDirtyAt:VT100GridCoordMake(2, 1)]);
...
- (void)testMarkCharsDirtyInRect {
  ...
  XCTAssert(
      [[grid compactDirtyDump] isEqualToString:@"cccc\ncccc\ncccc\ncccc"]);
  [grid markCharsDirty:YES
            inRectFrom:VT100GridCoordMake(1, 1)
...
- (VT100Grid *)gridFromCompactLines:(NSString *)compact {
  ...
  VT100Grid *grid = [[VT100Grid alloc]
      initWithSize:VT100GridSizeMake([[lines objectAtIndex:0] length],
                                     [lines count])
          delegate:self];
  int i = 0;
...
- (VT100Grid *)gridFromCompactLinesWithContinuationMarks:(NSString *)compact {
  NSArray *lines = [compact componentsSeparatedByString:@"\n"];
  VT100Grid *grid = [[VT100Grid alloc]
      initWithSize:VT100GridSizeMake([[lines objectAtIndex:0] length] - 1,
                                     [lines count])
          delegate:self];
  int i = 0;
  for (NSString *line in lines) {
    ...
    for (j = 0; j < [line length] - 1; j++) {
      ...
      if (c == '-')
        c = DWC_RIGHT;
      if (c == '>' && j == [line length] - 2 &&
          [line characterAtIndex:j + 1] == '>')
        c = DWC_SKIP;
      s[j].code = c;
    }
...
- (void)testAppendLineToLineBuffer {
  ...
  LineBuffer *lineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
  [grid appendLines:2 toLineBuffer:lineBuffer];
  XCTAssert([[lineBuffer debugString] isEqualToString:@"abcd!\nefgh!"]);

  grid = [self
      gridFromCompactLinesWithContinuationMarks:@"abcd!\nefgh+\n....!\n....!"];
  ...
  // Test that the cursor gets hoisted from the start of a blank line following
  // a soft-eol to the end of the preceding line.
  grid = [self
      gridFromCompactLinesWithContinuationMarks:@"abcd+\nefgh+\n....!\n....!"];
  lineBuffer = [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
  grid.cursorX = 0;
...
- (void)testMoveCursorDownOneLineNoScroll {
  ...
  // Test scrolling when there's a full-width region touching the top of the
  // screen
  grid = [self gridFromCompactLines:@"abcd\nefgh\nijkl\nmnop"];
  lineBuffer = [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
...
- (void)testMoveCursorLeftNotWrappingAroundHardEOL {
  ...
  XCTAssert([[grid compactLineDump] isEqualToString:@"abc.\n"
                                                    @"d...\n"
                                                    @"....\n"
                                                    @"...."]);
...
- (void)testScrollWholeScreenUpIntoLineBuffer {
  ...
  LineBuffer *lineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
  [lineBuffer setMaxLines:1];
  XCTAssert([grid scrollWholeScreenUpIntoLineBuffer:lineBuffer
                                unlimitedScrollback:NO] == 0);
  XCTAssert([[grid compactLineDump] isEqualToString:@"efgh\nijkl\nmnop\n...."]);
  XCTAssert(
      [[grid compactDirtyDump] isEqualToString:@"cccc\nccdc\ncccc\ndddd"]);
  [self setLine:3 ofGrid:grid toString:@"qrst"];
  XCTAssert([grid scrollWholeScreenUpIntoLineBuffer:lineBuffer
                                unlimitedScrollback:NO] == 1);
  XCTAssert([[grid compactLineDump] isEqualToString:@"ijkl\nmnop\nqrst\n...."]);
  XCTAssert([[lineBuffer debugString] isEqualToString:@"efgh!"]);
  XCTAssert(
      [[grid compactDirtyDump] isEqualToString:@"ccdc\ncccc\ndddd\ndddd"]);
}

// No test for scrollDown because it's just a wafer thin wrapper around
// scrollRect:downBy:.
...
- (NSString *)compactLineDumpForRectScrolledDownBy:(int)downBy
  ...
  return [NSString stringWithFormat:@"%@\n\n%@", [grid compactLineDump],
                                    [grid compactDirtyDump]];
}
...
- (void)testScrollRectDownBy {
  ...
  XCTAssert([s isEqualToString:@"abcde\n"
                               @"f...j\n"
                               @"k...o\n"
                               @"pghit\n"
                               @"uvwxy\n"
  ...
  NSString *multiSplitDwcValue = @"ab>\n"
                                 @"c-d\n"
                                 @"ef>\n"
                                 @"gh-";
  // Test that a split-dwc at bottom when separated from its dwc is handled
  // correctly.
  s = [self compactLineDumpForRectScrolledDownBy:-1
                                      scrollRect:VT100GridRectMake(0, 1, 3, 2)
                                    initialValue:multiSplitDwcValue];
  XCTAssert([s isEqualToString:@"ab.\n"
                               @"ef.\n"
                               @"...\n"
                               @"gh-\n"
                               @"\n"
  ...
  // Test that a split-dwc at top when separated from its dwc is handled
  // correctly.
  NSString *singleSplitDwcOnTopValue = @"ab>\n"
                                       @"c-d\n"
                                       @"efg";
  s = [self compactLineDumpForRectScrolledDownBy:1
                                      scrollRect:VT100GridRectMake(0, 1, 3, 2)
  ...
  NSString *orphansValue =
      ...
      @"ij-k>\n" // j- can move up, but the split-dwc is broken since l- doesn't
                 // move as a whole
      @"l-mno\n" // l- gets erased
      @"p-qr-\n" // p- and r- get erased
      @"stuvw";
  ...
  XCTAssert([s isEqualToString:@"a.g.e\n"
                               @".j-k.\n"
                               @"i.mn.\n"
                               @"..q.o\n"
                               @".tuv.\n"
  ...
  NSString *edgeCaseyOrphans = @"abcd>\n"
                               @"e-fgh\n"
                               @"ijklm\n"
                               @"nopq>\n"
                               @"r-stu";
  ...
  // Test scrolling a region where scrollRight=right margin, scrollLeft=1, and
  // there's a split-dwc
  s = [self compactLineDumpForRectScrolledDownBy:-1
                                      scrollRect:VT100GridRectMake(1, 0, 4, 5)
                                    initialValue:edgeCaseyOrphans];
  XCTAssert([s isEqualToString:@"a.fgh\n"
                               @".jklm\n"
                               @"iopq.\n"
                               @"n.stu\n"
                               @".....\n"
  ...
  // Test scrolling a region where scrollRight=right margin-1, scrollLeft=0, and
  // there's a split-dwc
  s = [self compactLineDumpForRectScrolledDownBy:-1
                                      scrollRect:VT100GridRectMake(0, 0, 4, 5)
                                    initialValue:edgeCaseyOrphans];
  XCTAssert([s isEqualToString:@"e-fg.\n"
                               @"ijklh\n"
                               @"nopqm\n"
                               @"r-st.\n"
                               @"....u\n"
  ...
  VT100Grid *grid = [self gridFromCompactLinesWithContinuationMarks:@"abcd+\n"
                                                                    @"efgh+\n"
                                                                    @"ijkl+\n"
                                                                    @"mnop+\n"
                                                                    @"qrst!"];
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"abcd!\n"
                                                                   @"....!\n"
                                                                   @"efgh+\n"
                                                                   @"ijkl!\n"
                                                                   @"qrst!"]);
  ...
  // With DWC_SKIP
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abc>>\n"
                                                         @"E-gh+\n"
                                                         @"ijkl+\n"
                                                         @"mno>>\n"
                                                         @"Q-st!"];
  [grid scrollRect:VT100GridRectMake(1, 1, 3, 3) downBy:-1 softBreak:NO];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"abc.!\n"
                                                                   @".jkl+\n"
                                                                   @"ino.!\n"
                                                                   @"m...!\n"
                                                                   @"Q-st!"]);
...
- (void)testSetContentsFromDVRFrame {
  ...
  testGrid = [[[VT100Grid alloc] initWithSize:VT100GridSizeMake(3, 3)
                                     delegate:self] autorelease];
  [testGrid setContentsFromDVRFrame:frame info:info];
  NSString *truncatedCompactLines = @"efg\n"
                                    @"ijk\n"
                                    @"mno";
  ...
  testGrid = [[[VT100Grid alloc] initWithSize:VT100GridSizeMake(5, 5)
                                     delegate:self] autorelease];
  [testGrid setContentsFromDVRFrame:frame info:info];
  NSString *paddedCompactLines = @"abcd.\n"
                                 @"efgh.\n"
                                 @"ijkl.\n"
                                 @"mnop.\n"
                                 @".....";
...
- (void)doTestDefaultLine {
  ...
  XCTAssert(data.length ==
            sizeof(screen_char_t) *
                (w + 1)); // w+1 because it adds one for continuation marker
  for (int i = 0; i < w; i++) {
...
- (VT100Grid *)coverageForRuns:(NSArray *)runs inGrid:(VT100Grid *)grid {
  VT100Grid *coverage = [[[VT100Grid alloc] initWithSize:grid.size
                                                delegate:self] autorelease];
...
- (screen_char_t *)screenCharLineForString:(NSString *)string {
  NSMutableData *data =
      [NSMutableData dataWithLength:string.length * sizeof(screen_char_t)];
  screen_char_t *line = data.mutableBytes;
...
// Put a * in a string to designate the char after it as the cursor's location.
// The * will not be added to the linebuffer.
...
- (void)testRestoreScreenFromLineBuffer {
  VT100Grid *grid = [self largeGrid];
  LineBuffer *lineBuffer =
      [self lineBufferWithStrings:@"test", @"hello wor*ld", nil];
  [grid restoreScreenFromLineBuffer:lineBuffer
                    withDefaultChar:[grid defaultChar]
                  maxLinesToRestore:1];
  XCTAssert([[grid compactLineDump] isEqualToString:@"rld.....\n"
                                                    @"........\n"
                                                    @"........\n"
                                                    @"........\n"
                                                    @"........\n"
  ...
  XCTAssert([[grid compactLineDump] isEqualToString:@"test....\n"
                                                    @"hello wo\n"
                                                    @"rld.....\n"
                                                    @"........\n"
                                                    @"........\n"
  ...
  [grid restoreScreenFromLineBuffer:lineBuffer
                    withDefaultChar:dc
                  maxLinesToRestore:100];
  XCTAssert([[grid compactLineDump] isEqualToString:@"rl\n"
                                                    @"d."]);
  XCTAssert(grid.cursorX == 1);
  XCTAssert(grid.cursorY == 0);
...
- (void)testEraseDwc {
  ...
  XCTAssert([grid erasePossibleDoubleWidthCharInLineNumber:1
                                          startingAtOffset:0
                                                  withChar:dc]);
  XCTAssert([[grid compactLineDump] isEqualToString:@"ab.\n..."]);
  XCTAssert([[grid compactDirtyDump]
      isEqualToString:@"ccc\nddc"]); // Don't need to set DWC_SKIP->NULL char to
                                     // dirty
}
...
- (void)testMoveWrappedCursorLineToTopOfGrid {
  ...
  XCTAssert([[grid compactLineDump] isEqualToString:@"hijk\n"
                                                    @"lmno\n"
                                                    @"pq..\n"
                                                    @"rstu\n"
                                                    @"vwx.\n"
...
- (void)
    doAppendCharsAtCursorTestWithInitialBuffer:(NSString *)initialBuffer
  ...
  VT100Grid *grid =
      [self gridFromCompactLinesWithContinuationMarks:initialBuffer];
  LineBuffer *lineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
  if (scrollRect.size.width >= 0) {
    grid.scrollRegionCols =
        VT100GridRangeMake(scrollRect.origin.x, scrollRect.size.width);
    grid.useScrollRegionCols = YES;
  }
  if (scrollRect.size.height >= 0) {
    grid.scrollRegionRows =
        VT100GridRangeMake(scrollRect.origin.y, scrollRect.size.height);
  }
  ...
  int numLinesDropped = [grid appendCharsAtCursor:line
                          ...
                          useScrollbackWithRegion:useScrollbackWithRegion
                                       wraparound:wraparoundMode_
                                             ansi:isAnsi_
                                           insert:insertMode_];
  XCTAssert([[grid compactLineDumpWithContinuationMarks]
      isEqualToString:expectedLines]);
...
- (void)testAppendCharsAtCursor {
  ...
  // no scrolling into line buffer with scrollTop/Bottom +
  ...
  // cursor starts outside scrollTop/Bottom region and scrolls (not sure what
  // should happen!)
  // TODO

  // no scrolling into line buffer with h-region + v-region +
  // useScrollbackWithRegion
  [self doAppendCharsAtCursorTestWithInitialBuffer:@"abc!\n"
                                                   @"d..!"
  ...
  // plain text truncated bc wraparoundmode is off (cont marker should go to
  // hard)
  wraparoundMode_ = NO;
  [self
      doAppendCharsAtCursorTestWithInitialBuffer:@"zyxwv+\n"
                                                 @"u....!"
                                    scrollRegion:VT100GridRectMake(0, 0, -1, -1)
                         useScrollbackWithRegion:YES
                             unlimitedScrollback:NO
  ...
  // Appending long string with wraparound off that orphans a dwc |abC-d| ->
  // |abCxz|
  wraparoundMode_ = NO;
  [self
      doAppendCharsAtCursorTestWithInitialBuffer:@"zyxwv+\n"
  ...
  // insert really long string, causing truncation at end of line and of
  // inserted string and wraparound
  insertMode_ = YES;
  [self
  ...
  // with ansi terminal, placing cursor at right margin wraps it around in
  // wraparound mode
  ...
  // with ansi terminal, placing cursor at right margin moves it back one space
  // if wraparoundmode is off
  // TODO: vsplits aren't treated the same way; should they be?
  isAnsi_ = YES;
  wraparoundMode_ = NO;
  [self
...
- (void)testDeleteChars {
  ...
  XCTAssert([[grid compactLineDump] isEqualToString:@"acd.\n"
                                                    @"efg."]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"a...!\n"
                                                                   @"efg.!"]);
  ...
  grid = [self gridFromCompactLinesWithContinuationMarks:@"aB-d!\n"
                                                         @"efg.!"];
  [grid deleteChars:1 startingAt:VT100GridCoordMake(1, 0)];
  XCTAssert([[grid compactLineDump] isEqualToString:@"a.d.\n"
                                                    @"efg."]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"bc..!\n"
                                                                   @"D-ef!"]);

  // Scroll region
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abcde+\n"
                                                         @"fghi.!"];
  grid.scrollRegionCols = VT100GridRangeMake(1, 3);
  grid.useScrollRegionCols = YES;
  [grid deleteChars:1 startingAt:VT100GridCoordMake(2, 0)];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"abd.e+\n"
                                                                   @"fghi.!"]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab..e+\n"
                                                                   @"fghi.!"]);

  // Scroll region, creating orphan dwc by deleting right half
  grid = [self gridFromCompactLinesWithContinuationMarks:@"aB-cd+\n"
                                                         @"fghi.!"];
  grid.scrollRegionCols = VT100GridRangeMake(1, 3);
  grid.useScrollRegionCols = YES;
  [grid deleteChars:1 startingAt:VT100GridCoordMake(2, 0)];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"a.c.d+\n"
                                                                   @"fghi.!"]);

  // Scroll region right boundary overlaps half a DWC, orphaning its right half
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abC-e+"];
  grid.scrollRegionCols = VT100GridRangeMake(0, 3);
  grid.useScrollRegionCols = YES;
  [grid deleteChars:1 startingAt:VT100GridCoordMake(0, 0)];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"b...e+"]);

  // Scroll region right boundary overlaps half a DWC, orphaning its left half
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abC-efg+"];
  grid.scrollRegionCols = VT100GridRangeMake(3, 2);
  grid.useScrollRegionCols = YES;
  [grid deleteChars:1 startingAt:VT100GridCoordMake(3, 0)];
  XCTAssert([[grid compactLineDumpWithContinuationMarks]
      isEqualToString:@"ab.e.fg+"]);
...
- (void)testInsertChar {
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"a.bc+\n"
                                                                   @"efg.!"]);
  ...
  [grid insertChar:c at:VT100GridCoordMake(1, 0) times:100];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"axxx+\n"
                                                                   @"efg.!"]);
  c.code = 0;
  ...
  grid = [self gridFromCompactLinesWithContinuationMarks:@"aB-de+\n"
                                                         @"fghi.!"];
  c.code = 'x';
  [grid insertChar:c at:VT100GridCoordMake(2, 0) times:1];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"a.x.d+\n"
                                                                   @"fghi.!"]);
  c.code = 0;
  ...
  [grid insertChar:c at:VT100GridCoordMake(2, 0) times:1];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab.cd+\n"
                                                                   @"E-fgh!"]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab..c+\n"
                                                                   @"E-fgh!"]);
  ...
  [grid insertChar:c at:VT100GridCoordMake(2, 0) times:2];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab...!\n"
                                                                   @"E-fgh!"]);

  // Scroll region
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abcdef+"];
  grid.scrollRegionCols = VT100GridRangeMake(1, 4);
  grid.useScrollRegionCols = YES;
  [grid insertChar:c at:VT100GridCoordMake(2, 0) times:1];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab.cdf+"]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"ab...f+"]);

  // Make orphan by inserting into scroll region that overlaps left half of dwc
  grid = [self gridFromCompactLinesWithContinuationMarks:@"abcD-f+"];
  grid.scrollRegionCols = VT100GridRangeMake(1, 3);
  grid.useScrollRegionCols = YES;
  [grid insertChar:c at:VT100GridCoordMake(1, 0) times:1];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"a.bc.f+"]);

  // Make orphan by inserting into scroll region that overlaps right half of dec
  grid = [self gridFromCompactLinesWithContinuationMarks:@"A-cdef+"];
  grid.scrollRegionCols = VT100GridRangeMake(1, 3);
  grid.useScrollRegionCols = YES;
  [grid insertChar:c at:VT100GridCoordMake(1, 0) times:1];
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@"...cef+"]);
  ...
  XCTAssert(
      [[grid compactLineDumpWithContinuationMarks] isEqualToString:@".aC->>\n"
                                                                   @"E-fgh!"]);
...
// Issue 4308
// There was a scroll region because screen was being used. The user appended a
// long line at the bottom of the region (just above the status bar). When it
// scrolled up, a hard linebreak replaced the soft one.
...
- (void)testGridRunFromRange_basic {
  ...
  VT100Grid *grid = [[[VT100Grid alloc] initWithSize:VT100GridSizeMake(10, 10)
                                            delegate:self] autorelease];
  VT100GridRun actual = [grid gridRunFromRange:range relativeToRow:0];
  VT100GridRun expected = VT100GridRunMake(0, 0, 5);
  XCTAssertTrue(VT100GridRunEquals(actual, expected), "actual=%@, expected=%@",
                VT100GridRunDescription(actual),
                VT100GridRunDescription(expected));
}
...
- (void)testSingleColumnLineBuffer {
  VT100Grid *grid = [[VT100Grid alloc] initWithSize:VT100GridSizeMake(1, 24)
                                           delegate:self];
  LineBuffer *lineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:1000] autorelease];
  [grid appendLines:5 toLineBuffer:lineBuffer];
  screen_char_t c;
  ScreenCharArray *sca = [lineBuffer wrappedLineAtIndex:0
                                                  width:1
                                           continuation:&c];
  XCTAssertNotNil(sca);
