...
- (void)testASCIIWordSelection {
  ...
  NSArray *words = @[
    @"word",    @"word",    @"word",    @"word",    @" ",       @"123",
    @"123",     @"123",     @"   ",     @"   ",     @"   ",     @"abc",
    @"abc",     @"abc",     @"/",       @"def",     @"def",     @"def",
    @" ",       @"afl-cio", @"afl-cio", @"afl-cio", @"afl-cio", @"afl-cio",
    @"afl-cio", @"afl-cio", @" ",       @"!",       @"@",       @"-"
  ];
...
- (void)performTestForWordSelectionUsingLine:(NSString *)line
  ...
  [fakeDefaults
      setFakeObject:extraWordCharacters
             forKey:kPreferenceKeyCharactersConsideredPartOfAWordForSelection];
  _lines = @[ line ];
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
      ...
      forBlock:^{
        ...
        for (int i = 0; i < line.length; i++) {
          ...
          XCTAssertEqualObjects(actual, expected[i],
                                @"For click at %@ got a range of %@ giving "
                                @"“%@”, while I expected “%@”",
                                @(i), VT100GridWindowedRangeDescription(range),
                                actual, expected[i]);
        }
      }];
}
...
- (void)testDoubleWidthCharacterSmartSelection {
  ...
  VT100GridWindowedRange range;
  NSDictionary *rule =
      @{kRegexKey : @"\\S+", kPrecisionKey : kVeryHighPrecision};
...
- (void)testRangeByTrimmingWhitespace_TrimBothEnds {
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridAbsCoordRange actual = [extractor
      rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 7,
                                                                    0)];
...
- (void)testRangeByTrimmingWhitespace_TrimLeft {
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridAbsCoordRange actual = [extractor
      rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 5,
                                                                    0)];
  VT100GridAbsCoordRange expected = VT100GridAbsCoordRangeMake(2, 0, 5, 0);
  XCTAssertEqual(actual.start.x, expected.start.x);
  XCTAssertEqual(actual.start.y, expected.start.y);
...
- (void)testRangeByTrimmingWhitespace_NothingToTrim {
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridAbsCoordRange actual = [extractor
      rangeByTrimmingWhitespaceFromRange:VT100GridAbsCoordRangeMake(0, 0, 3,
                                                                    0)];
  VT100GridAbsCoordRange expected = VT100GridAbsCoordRangeMake(0, 0, 3, 0);
  XCTAssertEqual(actual.start.x, expected.start.x);
  XCTAssertEqual(actual.start.y, expected.start.y);
...
- (void)testContentInRange_TruncateHeadSearchingBackwards_Huge {
  ...
  VT100GridWindowedRange range = VT100GridWindowedRangeMake(
      VT100GridCoordRangeMake(0, 0, 3, _lines.count), 0, 0);
...
- (void)testContentInRange_RemoveTabFillers {
  NSString *line = @"a\uf001\uf001\tb";
  NSMutableData *data =
      [NSMutableData dataWithLength:(line.length + 1) * sizeof(screen_char_t)];

  screen_char_t color = {0};
  int len = 0;
  StringToScreenChars(line, data.mutableBytes, color, color, &len, NO, NULL,
                      NULL, NO, kUnicodeVersion);
  screen_char_t *buffer = data.mutableBytes;
  // Turn replacement characters into tab fillers. StringToScreenChars removes
  // private range codes.
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridWindowedRange range =
      VT100GridWindowedRangeMake(VT100GridCoordRangeMake(0, 0, 5, 1), 0, 0);
  NSString *actual = [extractor
              contentInRange:range
           attributeProvider:nil
                  nullPolicy:
...
- (void)appendWrappedLine:(NSString *)line width:(int)width eol:(int)eol {
  NSMutableData *data =
      [NSMutableData dataWithLength:(width + 1) * sizeof(screen_char_t)];
  screen_char_t color = {0};
  int len = 0;
  StringToScreenChars(line, data.mutableBytes, color, color, &len, NO, NULL,
                      NULL, NO, kUnicodeVersion);
...
- (void)testRangeForWrappedLine_MaxChars {
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridWindowedRange range =
      [extractor rangeForWrappedLineEncompassing:VT100GridCoordMake(5, 5)
...
- (void)testRangeForWrappedLine_EOL_DWC {
  ...
  [self appendWrappedLine:
            [NSString stringWithFormat:@"111111111111111111111111111中%C%C",
                                       DWC_RIGHT, DWC_SKIP]
                    width:30
                      eol:EOL_DWC];
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridWindowedRange range =
      [extractor rangeForWrappedLineEncompassing:VT100GridCoordMake(5, 1)
                            respectContinuations:NO
                                        maxChars:1000];
  XCTAssertEqual(range.coordRange.start.x, 0);
...
- (void)testRangeForWrappedLine_EOL_SOFT {
  [self appendWrappedLine:@"asdf" width:30 eol:EOL_HARD];
  [self
      appendWrappedLine:[NSString
                            stringWithFormat:@"111111111111111111111111111xyz"]
                  width:30
                    eol:EOL_SOFT];
...
- (void)testHaveNonWhitespaceInFirstLineOfRange_OneLineOfWhitespace {
  ...
  iTermTextExtractor *extractor =
      [iTermTextExtractor textExtractorWithDataSource:self];
  VT100GridWindowedRange range = VT100GridWindowedRangeMake(
      VT100GridCoordRangeMake(0, 0, 0, _lines.count), 0, 0);
...
- (void)
    testHaveNonWhitespaceInFirstLineOfRange_ComplexNonWhitespaceOnFirstLine {
  // u+2003 is em space
  [self appendWrappedLine:@"  \t😀 \u2003 " width:30 eol:EOL_HARD];
  [self appendWrappedLine:@"x" width:30 eol:EOL_HARD];
  iTermTextExtractor *extractor =
...
- (int)lengthOfLineAtIndex:(int)theIndex withBuffer:(screen_char_t *)buffer {
  if ([_lines[0] isKindOfClass:[NSData class]]) {
    ...
    screen_char_t color = {0};
    int len = 0;
    StringToScreenChars(_lines[0], buffer, color, color, &len, NO, NULL, NULL,
                        NO, kUnicodeVersion);
    return len;
  }
}
...
- (screen_char_t *)getLineAtIndex:(int)theIndex {
  ...
  if ([_lines[0] isKindOfClass:[NSData class]]) {
    ...
    screen_char_t color = {0};
    int len = 0;
    StringToScreenChars(_lines[theIndex], _buffer, color, color, &len, NO, NULL,
                        NULL, NO, kUnicodeVersion);
