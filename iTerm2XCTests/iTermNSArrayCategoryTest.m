...
- (void)testObjectsOfClasses {
  NSArray *objects =
      @[ [NSNull null], @0, [[[NSObject alloc] init] autorelease], @1 ];
  ...
  NSArray *numbersAndNull =
      [objects objectsOfClasses:@[ [NSNumber class], [NSNull class] ]];
...
- (void)testAttributedComponentsJoinedByAttributedString {
  ...
  NSDictionary *joinAttributes =
      @{NSForegroundColorAttributeName : [NSColor redColor]};

  NSAttributedString *string1 =
      [[[NSAttributedString alloc] initWithString:@"one"
                                       attributes:attributes1] autorelease];
  NSAttributedString *string2 =
      [[[NSAttributedString alloc] initWithString:@"two"
                                       attributes:attributes2] autorelease];
  NSAttributedString *joiner =
      [[[NSAttributedString alloc] initWithString:@","
                                       attributes:joinAttributes] autorelease];

  NSArray *array = @[ string1, string2 ];
  NSAttributedString *joined =
      [array attributedComponentsJoinedByAttributedString:joiner];
