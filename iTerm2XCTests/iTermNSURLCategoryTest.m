...
- (void)testURLByReplacingFormatSpecifier_User {
  NSString *string =
      @"https://%@:password@example.com/a/c/b?a=1&b=2&c=3#fragment";
  NSURL *url = [NSURL urlByReplacingFormatSpecifier:@"%@"
                                           inString:string
                                          withValue:@"value"];
  XCTAssertEqualObjects(
...
- (void)testURLWithUserSuppliedString_NonAsciiPath {
  NSString *string = @"http://wiki.teamliquid.net/commons/images/thumb/a/af/"
                     @"Torbjörn-Barbarossa.jpg/580px-Torbjörn-Barbarossa.jpg";
  ...
  XCTAssertEqualObjects(
      url.absoluteString,
      @"http://wiki.teamliquid.net/commons/images/thumb/a/af/"
      @"Torbj%C3%B6rn-Barbarossa.jpg/580px-Torbj%C3%B6rn-Barbarossa.jpg");
}
...
- (void)testURLWithUserSuppliedString_Acid {
  ...
  NSString *urlString =
      [NSString stringWithFormat:@"%@://%@:%@@%@:%@/%@?%@#%@", scheme, user,
...
- (void)testURLWithUserSuppliedString_ManyParts {
  NSString *urlString =
      @"https://example.com:6088/projects/repos/applications/"
...
- (void)testUrlInQuery {
  ...
  NSURL *url = [NSURL URLWithUserSuppliedString:urlString];
  XCTAssertEqualObjects(url.absoluteString,
                        @"https://google.com/search?q=http://google.com/");
}
