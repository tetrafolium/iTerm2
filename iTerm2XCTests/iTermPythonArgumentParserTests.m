...
- (void)testStatement {
  iTermPythonArgumentParser *parser = [[iTermPythonArgumentParser alloc]
      initWithArgs:@[ @"python", @"-c", @"statement", @"script" ]];
...
- (void)testCompoundStatement {
  iTermPythonArgumentParser *parser = [[iTermPythonArgumentParser alloc]
      initWithArgs:@[ @"python", @"-cstatement", @"script" ]];
  XCTAssertEqualObjects(parser.statement, @"statement");
  XCTAssertNil(parser.script);
}
...
- (void)testModule {
  iTermPythonArgumentParser *parser = [[iTermPythonArgumentParser alloc]
      initWithArgs:@[ @"python", @"-m", @"module", @"arg" ]];
...
- (void)testCompoundModule {
  iTermPythonArgumentParser *parser = [[iTermPythonArgumentParser alloc]
      initWithArgs:@[ @"python", @"-mmodule", @"arg" ]];
  XCTAssertEqualObjects(parser.module, @"module arg");
  XCTAssertNil(parser.script);
}
