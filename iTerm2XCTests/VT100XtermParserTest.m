...
- (void)testNoModeYet {
  ...
  token =
      [self tokenForDataWithFormat:@"%c]0;title%c", VT100CC_ESC, VT100CC_BEL];
...
- (void)testWellFormedSetWindowTitleTerminatedByST {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0;title%c\\", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == XTERMCC_WINICON_TITLE);
  XCTAssert([token.string isEqualToString:@"title"]);
}
...
- (void)testIgnoreEmbeddedOSC {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0;ti%c]tle%c", VT100CC_ESC, VT100CC_ESC,
                                   VT100CC_BEL];
  XCTAssert(token->type == XTERMCC_WINICON_TITLE);
  XCTAssert([token.string isEqualToString:@"title"]);
}
...
- (void)testIgnoreEmbeddedOSCTwoPart_OutOfDataAfterBracket {
  ...
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0;ti%c]", VT100CC_ESC, VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);

  token = [self tokenForDataWithFormat:@"%c]0;ti%c]tle%c", VT100CC_ESC,
...
- (void)testFailOnEmbeddedEscapePlusCharacter {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0;ti%cc", VT100CC_ESC, VT100CC_ESC];
...
- (void)testUnsupportedFirstParameter {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]x%c", VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == VT100_NOTSUPPORT);
}
...
- (void)testCancelAbortsOSC {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0%c", VT100CC_ESC, VT100CC_CAN];
  XCTAssert(token->type == VT100_NOTSUPPORT);
}

- (void)testSubstituteAbortsOSC {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]0%c", VT100CC_ESC, VT100CC_SUB];
  XCTAssert(token->type == VT100_NOTSUPPORT);
}

- (void)testUnfinishedMultitoken {
  VT100Token *token = [self
      tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar:abc", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
...
- (void)testCompleteMultitoken {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar:abc%c",
                                   VT100CC_ESC, VT100CC_BEL];
...
- (void)testCompleteMultitokenInMultiplePasses {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]1337;File=blah;", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(CVectorCount(&_incidentals) == 0);

  // Give it some more header
  token =
      [self tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(CVectorCount(&_incidentals) == 0);
  ...
  // Give it some body.
  token =
      [self tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar:a", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(CVectorCount(&_incidentals) == 2);
  ...
  // Start finishing up
  token = [self tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar:abc%c",
...
- (void)testLateFailureMultitokenInMultiplePasses {
  ...
  token = [self tokenForDataWithFormat:@"%c]1337;File=blah;foo=bar:abc%c",
                                       VT100CC_ESC, VT100CC_SUB];
  XCTAssert(token->type == VT100_NOTSUPPORT);
}

- (void)testUnfinishedMultitokenWithDeprecatedMode {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]50;File=blah;foo=bar:abc", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(CVectorCount(&_incidentals) == 2);
...
- (void)testMultiPartOSC {
  // Pass in a partial escape code. The already-parsed data should be saved in
  // the saved-state dictionary.
  VT100Token *token = [self tokenForDataWithFormat:@"%c]0;foo", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_savedState.allKeys.count > 0);

  // Give it a more-formed code. The first three characters have changed.
  // Normally they would be the same, but it's done here to ensure that they are
  // ignored.
  token = [self tokenForDataWithFormat:@"%c]0;XXXbar", VT100CC_ESC];
  XCTAssert(token->type == VT100_WAIT);
  XCTAssert(_savedState.allKeys.count > 0);

  // Now a fully-formed code. The entire string value must come from saved
  // state.
  token =
      [self tokenForDataWithFormat:@"%c]0;XXXXXX%c", VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == XTERMCC_WINICON_TITLE);
...
- (void)testEmbeddedColon {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]1;foo:bar%c", VT100CC_ESC, VT100CC_BEL];
...
- (void)testUnsupportedMode {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c]999;foo%c", VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == VT100_NOTSUPPORT);
}

- (void)testBelAfterEmbeddedOSC {
  VT100Token *token = [self tokenForDataWithFormat:@"%c]1;%c]%c", VT100CC_ESC,
                                                   VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == XTERMCC_ICON_TITLE);
...
- (void)testIgnoreEmbeddedOSCWhenFailing {
  VT100Token *token = [self tokenForDataWithFormat:@"%c]x%c]%c", VT100CC_ESC,
                                                   VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == VT100_NOTSUPPORT);
...
- (void)testDefaultModeForDtermCodes {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c];Foo%c", VT100CC_ESC, VT100CC_BEL];
  XCTAssert(token->type == XTERMCC_WINICON_TITLE);
...
- (void)testOverflow {
  VT100Token *token = [self
      tokenForDataWithFormat:@"%c]9999999999;foo%c", VT100CC_ESC, VT100CC_BEL];
