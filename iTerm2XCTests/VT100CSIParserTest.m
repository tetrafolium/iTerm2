...
- (void)testSubParameter {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c[38:2:255:128:64:0:5:1m", VT100CC_ESC];
  ...
  int numberOfSubparameters =
      iTermParserGetAllCSISubparametersForParameter(token.csi, 0, subs);
...
- (void)testIntermediateByte {
  // DECSCUSR with parameter 3 (set cursor to "blink underline"), which has an
  // intermediate byte of the space character.
...
- (void)testGarbageIgnored {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c[1%cm", VT100CC_ESC, 0x7f];
  XCTAssert(token->type == VT100CSI_SGR);
}

- (void)testBadGarbageCausesFailure {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c[1%c 2m", VT100CC_ESC, 0x7f];
  XCTAssert(token->type == VT100_UNKNOWNCHAR);
}
...
- (void)testDefaultParameterValues {
  struct {
    ...
    int p3;
      ...
      // ` not supported (Character Position Absolute [column] (default =
      // [row,1]) (HPA)) a not supported (Character Position Relative [columns]
      // (default = [row,col+1]) (HPR)) b not supported (Repeat the preceding
      // graphic character P s times (REP))
      ...
      // >p not supported (Set resource value pointerMode. This is used by xterm
      // to decide whether to hide the pointer cursor as the user types.)
      ...
      // ${ not supported (Selective Erase Rectangular Area (DECSERA), VT400 and
      // up)
      // '| not supported (Request Locator Position (DECRQLP))
      // '} not supported (Insert P s Column(s) (default = 1) (DECIC), VT420 and
      // up)
      // '~ not supported (Delete P s Column(s) (default = 1) (DECDC), VT420 and
      // up)
...
// This test is here to remind you to write a test when implementing support for
// a new CSI code.
- (void)testUnsupportedCodes {
  char *unsupported[] = {
      ...
      "?1i",         ">0p",         "61;0\"p",
      "q",           "\"q",         "1;2;3;4;0$r",
      "?1s",         "1;2;3;4;0$t", ">1;60t",
      "0 t",         "1 u",         "1;2;3;4;5;6;7;8$v",
      "1;2;3;4'w",   "x",           "0*x",
      "0;1;2;3;4$x", "0;0'z",       "1;2;3;4$z",
      "'{",          "1;2;3;4${",   "'|",
      "'}",          "'~",
  };
  ...
  for (int i = 0; i < n; i++) {
    VT100Token *token =
        [self tokenForDataWithFormat:@"%c[%s", VT100CC_ESC, unsupported[i]];
...
- (void)testWindowManipulationCodes {
  ...
  for (int i = 0; i < n; i++) {
    VT100Token *token =
        [self tokenForDataWithFormat:@"%c[%dt", VT100CC_ESC, codes[i].p0];
...
- (void)testParameterOverflow {
  VT100Token *token =
      [self tokenForDataWithFormat:@"%c[9999999999m", VT100CC_ESC];
  XCTAssert(token->type == VT100_UNKNOWNCHAR);
}
