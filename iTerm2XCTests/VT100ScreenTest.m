...
// This macro can be used in tests to document a known bug. The first expression
// would evaluate to true if the bug were fixed. Until then, the second
// expression unfortunately does evaluate to true.
#define ITERM_TEST_KNOWN_BUG(expressionThatShouldBeTrue, expressionThatIsTrue) \
  do {                                                                         \
    ...
    NSLog(@"Known bug: %s should be true, but %s is.",                         \
          #expressionThatShouldBeTrue, #expressionThatIsTrue);                 \
  } while (0)

@interface VT100ScreenTest
    : XCTestCase <iTermSelectionDelegate, VT100ScreenDelegate>
@end
...
- (VT100Screen *)screen {
  VT100Screen *screen =
      [[[VT100Screen alloc] initWithTerminal:terminal_] autorelease];
...
- (void)testInit {
  ...
  // Make sure the screen is initialized to a positive size with the cursor at
  // the origin
  ...
  s = ScreenCharArrayToStringDebug([screen getLineAtIndex:0], [screen width]);
  XCTAssert([s isEqualToString:@"Line 0"]);
...
- (VT100Screen *)fiveByFourScreenWithThreeLinesOneWrapped {
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"abcde\n"
                                                      @"fgh..\n"
                                                      @"ijkl.\n"
                                                      @"....."]);
  return screen;
...
- (VT100Screen *)fiveByFourScreenWithFourLinesOneWrappedAndOneInLineBuffer {
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"ijkl.\n"
                                                      @"mnopq\n"
                                                      @"rst..\n"
                                                      @"....."]);
  return screen;
...
- (void)setSelectionRange:(VT100GridCoordRange)range width:(int)width {
  ...
  VT100GridWindowedRange theRange = VT100GridWindowedRangeMake(range, 0, 0);
...
- (VT100Screen *)screenFromCompactLinesWithContinuationMarks:
    (NSString *)compactLines {
  NSArray *lines = [compactLines componentsSeparatedByString:@"\n"];
  VT100Screen *screen =
      [self screenWithWidth:[[lines objectAtIndex:0] length] - 1
                     height:[lines count]];
  int i = 0;
  for (NSString *line in lines) {
    screen_char_t *s = [screen getLineAtScreenIndex:i++];
...
- (NSString *)selectedStringInScreen:(VT100Screen *)screen {
  ...
  [selection_ enumerateSelectedRanges:^(VT100GridWindowedRange range,
                                        BOOL *stop, BOOL eol) {
    ...
    for (int y = range.coordRange.start.y; y <= range.coordRange.end.y; y++) {
      ...
      int x;
      int ex =
          y == range.coordRange.end.y ? range.coordRange.end.x : [screen width];
      BOOL newline = NO;
      for (x = sx; x < ex; x++) {
        if (line[x].code) {
...
- (NSData *)screenCharLineForString:(NSString *)s {
  NSMutableData *data =
      [NSMutableData dataWithLength:s.length * sizeof(screen_char_t)];
  int len;
  StringToScreenChars(s, (screen_char_t *)[data mutableBytes],
                      [terminal_ foregroundColorCode],
                      [terminal_ backgroundColorCode], &len, NO, NULL, NULL, NO,
                      kUnicodeVersion);
  return data;
}
...
- (void)sendStringToTerminalWithFormat:(NSString *)formatString, ... {
  ...
  NSString *string = [[[NSString alloc] initWithFormat:formatString
...
- (void)screenDidAppendAsciiDataToCurrentLine:(AsciiData *)asciiData {
  [self screenDidAppendStringToCurrentLine:
...
- (VT100GridWindowedRange)selectionRangeForParentheticalAt:
...
- (VT100GridWindowedRange)selectionRangeForSmartSelectionAt:
    (VT100GridCoord)coord {
  return VT100GridWindowedRangeMake(VT100GridCoordRangeMake(0, 0, 0, 0), 0, 0);
}
...
- (void)testSetSizeHeight {
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"abcd\n"
                                                      @"efgh\n"
                                                      @"ijkl\n"
                                                      @"...."]);
  XCTAssert(screen.cursorX == 1);
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"abcdefgh.\n"
                                                      @"ijkl.....\n"
                                                      @".........\n"
                                                      @"........."]);
  XCTAssert(screen.cursorX == 1);
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"gh....\n"
                                                      @"ijkl..\n"
                                                      @"mnopqr\n"
                                                      @"st....\n"
                                                      @"......"]);
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"ijk\n"
                                                      @"l..\n"
                                                      @"..."]);
  XCTAssert(screen.cursorX == 1);
  XCTAssert(screen.cursorY == 3);
  ...
  s = ScreenCharArrayToStringDebug([screen getLineAtIndex:0], [screen width]);
  XCTAssert([s isEqualToString:@"abc"]);
  s = ScreenCharArrayToStringDebug([screen getLineAtIndex:1], [screen width]);
  XCTAssert([s isEqualToString:@"def"]);
  s = ScreenCharArrayToStringDebug([screen getLineAtIndex:2], [screen width]);
  XCTAssert([s isEqualToString:@"gh"]);
  s = ScreenCharArrayToStringDebug([screen getLineAtIndex:3], [screen width]);
  XCTAssert([s isEqualToString:@"ijk"]);

  // Starting in primary with selection, it shrinks, but selection stays on
  // screen abcde+ fgh..! ijkl.!
  // .....!
  screen = [self fiveByFourScreenWithThreeLinesOneWrapped];
  ...
  // Starting in primary with selection, it shrinks, selection is pushed off top
  // completely abcde+ fgh..! ijkl.!
  // .....!
  screen = [self fiveByFourScreenWithThreeLinesOneWrapped];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  // select "abcd"
  ...
  // Starting in alt with selection and screen shrinks but selection stays on
  // screen
  screen = [self fiveByFourScreenWithThreeLinesOneWrapped];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self showAltAndUppercase:screen];
  ...
  // Starting in alt with selection and selection is pushed off the top
  // partially
  screen = [self fiveByFourScreenWithThreeLinesOneWrapped];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self showAltAndUppercase:screen];
  ...
  XCTAssert([[self selectedStringInScreen:screen] isEqualToString:@"GH\nIJ"]);
  [screen setSize:VT100GridSizeMake(6, 5)];
  XCTAssert([[screen compactLineDump] isEqualToString:@"ABCDEF\n"
                                                      @"GH....\n"
                                                      @"IJKL..\n"
                                                      @"......\n"
                                                      @"......"]);
  ...
  // Starting in alt with selection and screen grows, pulling lines out of line
  // buffer into primary grid.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  // abcde
  // fgh..
  ...
  // uvwxy
  // z....
  [self appendLines:@[ @"abcdefgh", @"ijkl", @"mnopqrst", @"uvwxyz" ]
           toScreen:screen];
  [self showAltAndUppercase:screen];
  XCTAssert([[screen compactLineDump] isEqualToString:@"MNOPQ\n"
                                                      @"RST..\n"
                                                      @"UVWXY\n"
                                                      @"Z....\n"
                                                      @"....."]);
  // select everything
  // TODO There's a bug when the selection is at the very end (5,6). It is
  // deselected.
  [self setSelectionRange:VT100GridCoordRangeMake(0, 0, 1, 6)
                    width:screen.width];
  XCTAssert([[self selectedStringInScreen:screen]
      isEqualToString:@"abcdefgh\nijkl\nMNOPQRST\nUVWXYZ"]);
  [screen setSize:VT100GridSizeMake(6, 6)];
  XCTAssert([[screen compactLineDump] isEqualToString:@"MNOPQR\n"
                                                      @"ST....\n"
                                                      @"UVWXYZ\n"
                                                      @"......\n"
                                                      @"......\n"
  ...
  // Starting in alt with selection and screen grows, pulling lines out of line
  // buffer into primary grid. Selection goes to very end of screen
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  // abcde
  // fgh..
  ...
  XCTAssert([[self selectedStringInScreen:screen]
      isEqualToString:@"abcdefgh\nijkl\nMNOPQRST\nUVWXYZ\n"]);
  [screen setSize:VT100GridSizeMake(6, 6)];
  ITERM_TEST_KNOWN_BUG([[self selectedStringInScreen:screen]
                           isEqualToString:@"abcdefgh\nMNOPQRST\nUVWXYZ\n"],
                       [[self selectedStringInScreen:screen]
                           isEqualToString:@"abcdefgh\nMNOPQRST\nUVWXYZ"]);
  ...
  // TODO
  // This is kind of questionable. We strip nulls in
  // -convertCurrentSelectionToWidth..., while it would be better to preserve
  // the selection.
  XCTAssertTrue(
      [[self selectedStringInScreen:screen] isEqualToString:@"\nabcdef"]);

  // In alt screen with selection that begins in history and ends in history
  // just above the visible screen. The screen grows, moving lines from history
  // into the primary screen. The end of the selection has to move back because
  // some of the selected text is no longer around in the alt screen.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
           toScreen:screen];
  XCTAssert([[screen compactLineDumpWithHistory]
      isEqualToString:@"abcde\n"
                      @"fgh..\n"
                      @"ijklm\n"
                      @"nopqr\n" // top line of screen
                      @"st...\n"
  ...
  [screen setSize:VT100GridSizeMake(6, 6)];
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"abcdef\n"
                                                                 @"NOPQRS\n"
                                                                 @"T.....\n"
                                                                 @"UVWXYZ\n"
                                                                 @"......\n"
  ...
  // In alt screen with selection that begins in history just above the visible
  // screen and ends onscreen. The screen grows, moving lines from history into
  // the primary screen. The start of the selection has to move forward because
  // some of the selected text is no longer around in the alt screen.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
  ...
  // In alt screen with selection that begins and ends onscreen. The screen is
  // grown and some history is deleted.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
           toScreen:screen];
  ...
  // In alt screen with selection that begins in history just above the visible
  // screen and ends there too. The screen grows, moving lines from history into
  // the primary screen. The selection is lost because none of its characters
  // still exist.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
  ...
  // In alt screen with selection that begins in history and ends in history
  // just above the visible screen. The screen grows, moving lines from history
  // into the primary screen. The end of the selection is exactly at the last
  // character before those that are lost.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
           toScreen:screen];
  ...
  // Starting in primary but with content on the alt screen. It is properly
  // restored.
  screen = [self screenWithWidth:5 height:5];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijklmnopqrst", @"uvwxyz" ]
           toScreen:screen];
  XCTAssert([[screen compactLineDump] isEqualToString:@"nopqr\n"
                                                      @"st...\n"
                                                      @"uvwxy\n"
                                                      @"z....\n"
                                                      @"....."]);
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"NOPQR\n"
                                                      ...
                                                      @"....."]);
  [screen setSize:VT100GridSizeMake(6, 6)];
  XCTAssert([[screen compactLineDump] isEqualToString:@"NOPQRS\n"
                                                      @"T.....\n"
                                                      @"UVWXYZ\n"
                                                      @"......\n"
                                                      @"......\n"
...
- (void)testRunByTrimmingNullsFromRun {
  // Basic test
  VT100Screen *screen = [self screenFromCompactLines:@"..1234\n"
                                                     @"56789a\n"
                                                     @"bc...."];
  ...
  // Test no nulls
  screen = [self screenFromCompactLines:@"1234\n"
                                        @"5678"];
...
- (void)testTerminalResetPreservingPrompt {
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@"ijkl.\n"
                                                      @".....\n"
                                                      @"....."]);
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"fgh..\n"
                                                                 @"ijkl.\n"
                                                                 @".....\n"
                                                                 @"....."]);
  ...
  XCTAssert([[screen compactLineDump] isEqualToString:@".....\n"
                                                      @".....\n"
                                                      @"....."]);
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"ijkl.\n"
                                                                 @".....\n"
                                                                 @".....\n"
                                                                 @"....."]);
...
- (void)testClearBuffer {
  ...
  [self appendLines:@[ @"abcdefgh", @"ijkl", @"mnopqrstuvwxyz" ]
           toScreen:screen];
  [screen clearBuffer];
  ...
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"wxyz.\n"
                                                                 @".....\n"
                                                                 @".....\n"
                                                                 @"....."]);
  ...
  [screen clearBuffer];
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"rstuv\n"
                                                                 @".....\n"
                                                                 @".....\n"
                                                                 @"....."]);
  XCTAssert(screen.cursorX == 4);
...
- (void)testClearScrollbackBuffer {
  ...
  [self appendLines:@[ @"abcdefgh", @"ijkl", @"mnopqrstuvwxyz" ]
           toScreen:screen];
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"abcde\n"
                                                                 @"fgh..\n"
                                                                 @"ijkl.\n"
                                                                 @"mnopq\n"
                                                                 @"rstuv\n"
  ...
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"mnopq\n"
                                                                 @"rstuv\n"
                                                                 @"wxyz.\n"
                                                                 @"....."]);
...
// Most of the work is done by VT100Grid's appendCharsAtCursor, which is heavily
// tested already. This only tests the extra work not included therein.
- (void)testAppendStringAtCursorAscii {
  ...
  [self
      sendEscapeCodes:@"^[[1m^[[3m^[[4m^[[5m^[[9m"]; // Bold, italic, blink,
                                                     // underline, strikethrough
  [screen appendStringAtCursor:@"Hello world"];

  XCTAssert([[screen compactLineDump] isEqualToString:@"Hello\n"
                                                      @" worl\n"
                                                      @"d....\n"
                                                      @"....."]);
...
- (void)testAppendComposedCharactersPiecewise {
  ...
  struct {
    ...
    BOOL doubleWidth;
      ...
      {@[ @('a'), @0x301 ], // a + accent
       [NSString stringWithCharacters:aaccent length:2], NO},
      {@[ @0xD800, @0xDD50 ], // surrogate pair
       @"𐅐", NO},
      {@[ @0xff25, @0x301 ], // double-width e + accent
       @"Ｅ́", YES},
      /*
       This test fails but you can't hit this case in real life, unless your
      terminal's encoding is UTF-16. In UTF-8, surrogate pairs are not used, so
      they'll always appear together.
      {
          @[ @0xD83D, @0xDD95, @0xD83C, @0xDFFE ],  // Middle finger + dark skin
      tone
          @"🖕🏾",
          NO
      },
...
- (void)testUnicode12Emoji {
  int32_t codePoints[] = {0x1F468, 0x1F3FF, 0x200D, 0x1F91D,
                          0x200D,  0x1F468, 0x1F3FB};
  NSData *expectedData = [NSData dataWithBytes:codePoints
                                        length:sizeof(codePoints)];
  NSString *expectedString =
      [[NSString alloc] initWithData:expectedData
  ...
  NSString *actualString =
      [ScreenCharToStr(line) decomposedStringWithCompatibilityMapping];
  XCTAssertEqualObjects(expectedString, actualString);
  NSData *actualData =
      [actualString dataUsingEncoding:NSUTF32LittleEndianStringEncoding];
...
- (void)testAppendStringAtCursorNonAscii {
  ...
  NSMutableString *s =
      [NSMutableString stringWithCharacters:chars
  ...
  int i = 0;
  NSString *a =
      [ScreenCharToStr(line + i++) decomposedStringWithCompatibilityMapping];
  ...
  XCTAssert([ScreenCharToStr(line + i++)
      isEqualToString:@"\u200b"]); // zero-width space advances cursor by
                                   // default.
  if (@available(macOS 10.13, *)) {
    XCTAssert([ScreenCharToStr(line + i++)
        isEqualToString:@"\u200c\u200d"]); // Funny way macoS 10.13 works
  }
  ...
  if (lettersHaveSkin) {
    XCTAssert([ScreenCharToStr(line + i++)
        isEqualToString:@"g\U0001F3FE"]); // macOS 10.14 does a silly thing
  } else {
    XCTAssert([ScreenCharToStr(line + i++) isEqualToString:@"g"]);
    XCTAssert([ScreenCharToStr(line + i++)
        isEqualToString:@"🏾"]); // Skin tone modifier only combines with
                                   // certain emoji
  }
...
- (void)testSetHistory {
  NSArray *lines = @[
    [self screenCharLineForString:@"abcdefghijkl"],
  ...
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
...
// Perform a search, append some stuff, and continue searching from the end of
// scrollback history prior to the appending, finding a match in the stuff that
// was appended. This is what PTYSession does for tail-find.
- (void)testAPIsUsedByTailFind {
  VT100Screen *screen = [self screenWithWidth:5 height:2];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self appendLines:@[ @"abcdefgh", @"ijkl", @"mnopqrstuvwxyz", @"012" ]
           toScreen:screen];
  /* abcde
   fgh..
   ijkl.
  ...
  XCTAssert([screen continueFindAllResults:results inContext:ctx]);
  ...
  // Now add some stuff to the bottom and search again from where we previously
  // stopped.
  ...
  [screen setFindString:@"mnop"
       forwardDirection:NO
                   mode:iTermFindModeCaseSensitiveSubstring
            startingAtX:0
            startingAtY:[screen numberOfLines] + 1 +
  ...
  SearchResult *expectedResult = [SearchResult searchResultFromX:0
                                                               y:3
                                                             toX:3
                                                               y:3];
...
- (void)testGetLineAtIndex {
  VT100Screen *screen = [self screenFromCompactLines:@"abcde>\n"
                                                     @"F-ghi.\n"];
  ...
  // Scroll the DWC_SPLIT off the screen. getLineAtIndex: will restore it, even
  // though line buffers don't store those.
...
- (void)assertSearchInScreen:(VT100Screen *)screen
  ...
  [screen setFindString:pattern
       forwardDirection:forward
        ...
        multipleResults:YES];
  NSMutableArray *results = [NSMutableArray array];
  while ([screen continueFindAllResults:results
                              inContext:[screen findContext]]) {
    if (block) {
      block(screen);
    }
...
- (void)assertSearchInScreenLines:(NSString *)compactLines
                 ...
                 forwardDirection:(BOOL)forward
                   ...
                   matchesResults:(NSArray *)expected {
  VT100Screen *screen =
      [self screenFromCompactLinesWithContinuationMarks:compactLines];
...
- (void)testFind {
  ...
  [self assertSearchInScreenLines:lines
                 ...
                 forwardDirection:YES
                      ...
                      startingAtY:0
                       withOffset:1
                   matchesResults:@[ [SearchResult searchResultFromX:0
                                                                   y:2
                                                                 toX:1
                                                                   y:2] ]];
  ...
  [self assertSearchInScreenLines:lines
                 ...
                 forwardDirection:NO
                      ...
                      startingAtY:2
                       withOffset:1
                   matchesResults:@[ [SearchResult searchResultFromX:3
                                                                   y:0
                                                                 toX:0
                                                                   y:1] ]];
  ...
  [self assertSearchInScreenLines:lines
                 ...
                 forwardDirection:YES
                      ...
                      startingAtY:0
                       withOffset:0
                   matchesResults:@[ [SearchResult searchResultFromX:2
                                                                   y:3
                                                                 toX:2
                                                                   y:4] ]];
  ...
  VT100Screen *screen = [self screenWithWidth:5 height:2];
  LineBuffer *smallBlockLineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:10] autorelease];
  ...
  [self assertSearchInScreen:screen
                ...
                forwardDirection:NO
                     ...
                     startingAtY:12
                      withOffset:0
                  matchesResults:@[
                    [SearchResult searchResultFromX:1 y:10 toX:3 y:10],
...
- (void)testScrollingInAltScreen {
  // When in alt screen and scrolling and !saveToScrollbackInAlternateScreen_,
  // then the whole screen must be marked dirty.
  ...
  [screen terminalLineFeed];
  XCTAssert([screen scrollbackOverflow] == 1);
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"1.\n"
                                                                 @"2.\n"
                                                                 @"3.\n"
                                                                 @"4.\n"
                                                                 @"..\n"
                                                                 @".."]);
  XCTAssert([[[screen currentGrid] compactDirtyDump] isEqualToString:@"cc\n"
                                                                     @"dc\n"
                                                                     @"dd"]);
  [screen resetScrollbackOverflow];
  ...
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"1.\n"
                                                                 ...
                                                                 @"..\n"
                                                                 @".."]);
  XCTAssert([[[screen currentGrid] compactDirtyDump] isEqualToString:@"dd\n"
                                                                     @"dd\n"
                                                                     @"dd"]);
  ...
  // When this bug is fixed, also test truncation with and without scroll
  // regions, as well as deselection because the whole selection scrolled off
...
- (void)testSaveToDvr {
  ...
  NSString *s;
  s = ScreenCharArrayToStringDebug(frame, [screen width]);
  XCTAssert([s isEqualToString:@"Line 1"]);
...
- (void)testPrinting {
  ...
  [screen terminalPrintBuffer];
  XCTAssert(!printed_);
  XCTAssert([ScreenCharArrayToStringDebug(
      [screen getLineAtScreenIndex:0], screen.width) isEqualToString:@"test"]);
...
- (void)testTabStops {
  ...
  XCTAssert([ScreenCharArrayToStringDebug([screen getLineAtScreenIndex:0],
                                          screen.width)
      isEqualToString:@"0123456789"]);

  // Tabbing over all nils replaces them with tab fillers and a tab character at
  // the end
  screen = [self screenWithWidth:20 height:3];
  [screen terminalAppendTabAtCursor:NO];
  screen_char_t *line = [screen getLineAtScreenIndex:0];
...
- (void)testMoveCursor {
  ...
  // In origin mode, coord is relative to origin and cursor is forced inside
  // scroll region
...
- (void)testSaveAndRestoreCursorAndCharset {
  ...
  // Restore without saving. Should use default charsets and move cursor to
  // origin. Terminal doesn't do anything in this case, but xterm does what we
  // do.
...
- (void)testSetTopBottomScrollRegion {
  ...
  XCTAssert(
      [ScreenCharArrayToStringDebug([screen getLineAtScreenIndex:15],
                                    screen.width) isEqualToString:@"Hello"]);
  [screen terminalLineFeed];
  XCTAssert(
      [ScreenCharArrayToStringDebug([screen getLineAtScreenIndex:14],
                                    screen.width) isEqualToString:@"Hello"]);
...
- (VT100Screen *)screenForEraseInDisplay {
  ...
  [self appendLines:@[ @"abcdefghij", @"klmnopqrst", @"0123456789" ]
           toScreen:screen];
  XCTAssert([[screen compactLineDump] isEqualToString:@"abcdefghij\n"
                                                      @"klmnopqrst\n"
                                                      @"0123456789\n"
                                                      @".........."]);
...
- (void)testEraseInDisplay {
  ...
  // Before and after should clear screen and move all nonempty lines into
  // history
  VT100Screen *screen = [self screenForEraseInDisplay];
  [screen terminalEraseInDisplayBeforeCursor:YES afterCursor:YES];
  XCTAssert(
      [[screen compactLineDumpWithHistory] isEqualToString:@"abcdefghij\n"
                                                           @"klmnopqrst\n"
                                                           @"0123456789\n"
                                                           @"..........\n"
                                                           @"..........\n"
...
- (void)testIndex {
  // We don't implement index separately from linefeed. As far as I can tell
  // they are the same. Both respect vsplits.
  ...
  [screen terminalLineFeed];
  [screen terminalLineFeed];
  XCTAssert([[screen compactLineDump] isEqualToString:@"klmnopqrst\n"
                                                      @"0123456789\n"
                                                      @"..........\n"
                                                      @".........."]);
...
- (void)testResetPreservingPrompt {
  ...
  [screen terminalMoveCursorToX:4 y:2];
  [screen terminalResetPreservingPrompt:YES];
  XCTAssert([[screen compactLineDump] isEqualToString:@"klm.......\n"
                                                      @"..........\n"
                                                      @"..........\n"
                                                      @".........."]);
...
- (void)testEraseCharactersAfterCursor {
  ...
  screen = [self screenFromCompactLinesWithContinuationMarks:@"abcD-fghij+\n"
                                                             @"klm.......!"];
  [screen terminalMoveCursorToX:5 y:1]; // '-'
  [screen terminalEraseCharactersAfterCursor:2];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
      isEqualToString:@"abc...ghij+\n"
                      @"klm.......!"]);

  // Break dwc after cursor
  screen = [self screenFromCompactLinesWithContinuationMarks:@"abcdeF-hij+\n"
                                                             @"klm.......!"];
  [screen terminalMoveCursorToX:5 y:1]; // 'e'
  [screen terminalEraseCharactersAfterCursor:2];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
      isEqualToString:@"abcd...hij+\n"
                      @"klm.......!"]);

  // Break split dwc
  screen = [self screenFromCompactLinesWithContinuationMarks:@"abcdefghi>>\n"
                                                             @"J-klm.....!"];
  [screen terminalMoveCursorToX:5 y:1]; // 'e'
  [screen terminalEraseCharactersAfterCursor:6];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
...
- (void)testSetTitle {
  ...
  XCTAssert(dirlog_.count == 1);
  entry = dirlog_[0];
  XCTAssert([entry[0] intValue] ==
            29); // 20 lines of scrollback + 10th line of display
  XCTAssertEqualObjects(entry[1], @"/");
...
- (void)testInsertEmptyCharsAtCursor {
  ...
  screen = [self screenFromCompactLinesWithContinuationMarks:@"abcdE-fghi+\n"
                                                             @"jkl.......!\n"
                                                             @"..........!"];
  [screen terminalMoveCursorToX:6 y:1]; // 'E'
  [screen terminalInsertEmptyCharsAtCursor:1];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
      isEqualToString:@"abcd...fgh+\n"
                      @"jkl.......!\n"
                      @"..........!"]);

  // Insert breaking DWC at cursor, which is on right half of dwc
  screen = [self screenFromCompactLinesWithContinuationMarks:@"abcD-efghi+\n"
                                                             @"jkl.......!\n"
                                                             @"..........!"];
  [screen terminalMoveCursorToX:5 y:1]; // '-'
  [screen terminalInsertEmptyCharsAtCursor:1];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
      isEqualToString:@"abc...efgh+\n"
                      @"jkl.......!\n"
                      @"..........!"]);
  ...
  [screen terminalInsertEmptyCharsAtCursor:1];
  XCTAssert([[screen compactLineDumpWithHistoryAndContinuationMarks]
      isEqualToString:@"abcd.efghj+\n"
                      @"klm.......!\n"
                      @"..........!"]);

  // There are a few more tests of insertChar in VT100GridTest, no sense
  // duplicating them all here.
}
...
- (void)testPasting {
  VT100Screen *screen = [self screen];
  screen.delegate = (id<VT100ScreenDelegate>)self;
  [self sendEscapeCodes:
            @"^[]50;CopyToClipboard=general^GHello world^[]50;EndCopy^G"];
  XCTAssert([pasteboard_ isEqualToString:@"general"]);
  XCTAssert(
      !memcmp(pbData_.mutableBytes, "Hello world", strlen("Hello world")));
  XCTAssert(pasted_);
}
...
- (void)testCursorReporting {
  ...
  NSString *s =
      [[[NSString alloc] initWithData:write_
...
- (void)testResizeNotes {
  // Put a note on the primary grid, switch to alt, resize width, swap back to
  // primary. Note should still be there.
  VT100Screen *screen = [self fiveByFourScreenWithThreeLinesOneWrapped];
  XCTAssert([[screen compactLineDump] isEqualToString:@"abcde\n"
                                                      @"fgh..\n"
                                                      @"ijkl.\n"
...
- (void)testRestoreWithNoteOnEmptyLineAtTop {
  ...
  PTYNoteViewController *note =
      [[[PTYNoteViewController alloc] init] autorelease];
  [screen addNote:note
          inRange:VT100GridCoordRangeMake(0, 3, 2,
                                          3)]; // First two chars on last line
}
...
- (void)testResizeNoteInPrimaryWhileInAltAndSomeHistory {
  ...
  XCTAssert(
      [[screen compactLineDumpWithHistory] isEqualToString:@"abcde\n" // history
                                                           @"fgh..\n" // history
                                                           @"ijkl.\n"
                                                           @"hello\n"
                                                           @" worl\n"
  ...
  [screen terminalShowPrimaryBuffer];
  XCTAssert(
      [[screen compactLineDumpWithHistory] isEqualToString:@"abcd\n" // history
                                                           @"efgh\n" // history
                                                           @"ijkl\n"
                                                           @"hell\n"
                                                           @"o wo\n"
...
- (void)
  ...
  [screen terminalShowPrimaryBuffer];
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"abc\n"
                                                                 @"def\n"
                                                                 @"gh.\n"
                                                                 @"ijk\n"
                                                                 @"l..\n"
...
- (void)
  ...
  PTYNoteViewController *note =
      [[[PTYNoteViewController alloc] init] autorelease];
  [screen addNote:note
          inRange:VT100GridCoordRangeMake(0, 2, 5, 3)]; // ijkl\nhello
  [screen terminalShowAltBuffer];
  [screen setSize:VT100GridSizeMake(3, 4)];
  [screen terminalShowPrimaryBuffer];
  XCTAssert([[screen compactLineDumpWithHistory] isEqualToString:@"abc\n"
...
- (void)testNoteTruncatedOnSwitchingToAlt {
  ...
  PTYNoteViewController *note =
      [[[PTYNoteViewController alloc] init] autorelease];
  [screen addNote:note
          inRange:VT100GridCoordRangeMake(0, 1, 5, 3)]; // fgh\nijkl\nhello
  [screen terminalShowAltBuffer];
  [screen terminalShowPrimaryBuffer];
...
- (void)commonAnnotationRestorationWithRange:(VT100GridCoordRange)range {
  ...
  [screen restoreFromDictionary:state
  ...
  note = notes[0];
  VT100GridCoordRange rangeAfterResize =
      [screen coordRangeForInterval:note.entry.interval];
...
- (void)testResizeWithNoteFirstLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 0, 5, 0)];
}
- (void)testResizeWithNoteFirstLinePlusFirstCharacterOfSecondLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 0, 2, 1)];
}
- (void)testResizeWithNoteFirstTwoCharactersOfSecondLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 1, 3, 1)];
}
- (void)testResizeWithNoteSecondLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 1, 5, 1)];
}
- (void)testResizeWithNoteLastFourCharactersOfSecondLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(2, 1, 5, 1)];
}
- (void)
    testResizeWithNoteSecondCharacterOfSecondLineToSecondCharacterOfThirdLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(2, 1, 2, 2)];
}
- (void)testResizeWithNoteSecondAndThirdLines {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 1, 5, 2)];
}
- (void)testResizeWithNoteSecondThroughFourthLines {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 1, 5, 3)];
}
- (void)testResizeWithNoteSecondThroughFifthLines {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 1, 5, 4)];
}
- (void)
    testResizeWithNoteSecondCharacterOfSecondLineThroughFirstCharacterOfFifthLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(2, 1, 2, 4)];
}
- (void)testResizeWithNoteThirdLineThroughFifthLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 3, 5, 4)];
}
- (void)testResizeWithNoteThirdLineThroughMiddleOfFifthLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 3, 3, 4)];
}
- (void)testResizeWithNoteFifthLine {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 4, 5, 4)];
}
- (void)testResizeWithNoteAllLines {
  [self
      commonAnnotationRestorationWithRange:VT100GridCoordRangeMake(0, 0, 5, 4)];
}
...
- (void)testResizeWithBlanksBeforeAnnotation {
  ...
  [self appendLines:@[
    @"Last login: Mon Dec  9 23:22:07 on ttys011", @"You have mail.",
    @"Georges-iMac:/Users/gnachman% echo;echo xxxxxxxxxx", @"", @"xxxxxxxxxx",
  ...
  VT100GridCoordRange rangeAfterResize =
      [screen coordRangeForInterval:note.entry.interval];
  XCTAssertTrue(
      VT100GridCoordRangeEqualsCoordRange(rangeAfterResize, expected));
}

- (void)
    commonNoteResizeRegressionTestWithInitialRange:(VT100GridCoordRange)range1
  ...
  {
    NSArray *notes =
        [screen notesInRange:VT100GridCoordRangeMake(0, 0, 80, 25)];
  ...
  [screen setSize:VT100GridSizeMake(80, 25)];
  {
    ...
    VT100GridCoordRange rangeAfterResize =
        [screen coordRangeForInterval:note.entry.interval];
    XCTAssertTrue(
        VT100GridCoordRangeEqualsCoordRange(rangeAfterResize, range1));
  }
}

- (void)testNoteResizeRegression1 {
  [self commonNoteResizeRegressionTestWithInitialRange:VT100GridCoordRangeMake(
                                                           0, 0, 80, 0)
                                     intermediateRange:VT100GridCoordRangeMake(
                                                           0, 0, 77, 0)];
}
...
- (void)testEmptyLineRestoresBackgroundColor {
  ...
  [lineBuffer copyLineToBuffer:buffer
                         width:3
                       lineNum:0
                  continuation:&continuation];
...
- (void)testRemoteHostOnTrailingEmptyLineNotLostDuringResize {
  // Append some text, then a newline, then set a remote host, then resize.
  // Ensure the remote host is still there.
...
- (void)testWrappedLinesFromIndexAtBoundary {
  ...
  LineBuffer *lineBuffer =
      [[[LineBuffer alloc] initWithBlockSize:blockSize] autorelease];
  ...
  // This works because the old version is, as far as I can tell, written
  // defensively rather than correctly.
  ...
  [lineBuffer copyLineToBuffer:buffer
                  ...
                  continuation:&continuation];
  for (int i = 0; i < linesPerBlock * 2; i++) {
    NSArray *lines = [lineBuffer wrappedLinesFromIndex:i width:n * 2 count:1];
...
- (void)testCSI_CUD {
  ...
  // This control function moves the cursor down a specified number of lines in
  // the same column. The cursor stops at the bottom margin. If the cursor is
  // already below the bottom margin, then the cursor stops at the bottom line.
...
- (void)testCSI_CUF {
  ...
  // This control function moves the cursor to the right by a specified number
  // of columns. The cursor stops at the right border of the page.
...
/*
 ...
 // I not supported (Cursor Forward Tabulation P s tab stops (default = 1)
 (CHT)) { 0, 0, 'J', VT100CSI_ED, 0, -1 },
 ...
 // ` not supported (Character Position Absolute [column] (default = [row,1])
 (HPA))
 // a not supported (Character Position Relative [columns] (default =
 [row,col+1]) (HPR))
 ...
 // >p not supported (Set resource value pointerMode. This is used by xterm to
 decide whether
