...
+ (NSString *)_punycodeDecode:(NSString *)aString;
{
    ...
    ([aString compare:ACEPrefix options:NSCaseInsensitiveSearch range:(NSRange) {
            0,acePrefixLength
        }] != NSOrderedSame))
    return aString;
    ...
    NSRange delimiter = [aString rangeOfString:@"-"
            ...
            range:(NSRange) {
                acePrefixLength, labelLength-acePrefixLength
            }];
    if (delimiter.length > 0) {
        decoded = [[aString substringWithRange:(NSRange) {
            acePrefixLength, delimiter.location - acePrefixLength
        }] mutableCopy];
        deltas = (NSRange) {
            NSMaxRange(delimiter), labelLength - NSMaxRange(delimiter)
        };
    } else {
    ...
    /* now use the decoded integers to insert characters into the decoded string */
    {
        ...
        for (deltaIndex = 0; deltaIndex < deltaCount; deltaIndex ++) {
            ...
            [decoded replaceCharactersInRange:(NSRange) {
                        position, 0
                    }
