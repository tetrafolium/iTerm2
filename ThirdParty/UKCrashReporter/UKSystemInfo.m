...
unsigned UKPhysicalRAMSize(void) {
  return (unsigned)(([NSProcessInfo.processInfo physicalMemory] / 1024ULL) /
                    1024ULL);
}
...
void UKGetSystemVersionComponents(SInt32 *outMajor, SInt32 *outMinor,
                                  SInt32 *outBugfix) {
  NSArray *sysVersionComponents =
      [UKSystemVersionString() componentsSeparatedByString:@"."];
...
NSString *UKMachineName(void) {
  ...
  if (!sysctlbyname("hw.model", temp, &tempLen, 0, 0)) {
    ...
    NSDictionary *translationDictionary = [[NSDictionary alloc]
        initWithObjectsAndKeys:
            @"PowerMac 8500/8600", @"AAPL,8500", @"PowerMac 9500/9600",
            @"AAPL,9500", @"PowerMac 7200", @"AAPL,7200", @"PowerMac 7200/7300",
            @"AAPL,7300", @"PowerMac 7500", @"AAPL,7500",
            @"Apple Network Server", @"AAPL,ShinerESB",
            @"Alchemy(Performa 6400 logic-board design)", @"AAPL,e407",
            @"Gazelle(5500)", @"AAPL,e411", @"PowerBook 3400",
            @"AAPL,3400/2400", @"PowerBook 3500", @"AAPL,3500",
            @"PowerMac G3 (Gossamer)", @"AAPL,Gossamer", @"PowerMac G3 (Silk)",
            @"AAPL,PowerMac G3", @"PowerBook G3 (Wallstreet)",
            @"AAPL,PowerBook1998", @"Yikes! Old machine", @"AAPL", // generic.

            @"iMac (first generation)", @"iMac,1", @"iMac", @"iMac", // generic.

            @"PowerBook G3 (Lombard)", @"PowerBook1,1", @"iBook (clamshell)",
            @"PowerBook2,1", @"iBook FireWire (clamshell)", @"PowerBook2,2",
            ...
            @"iBook (12in May 2001)", @"PowerBook4,1", @"iBook (May 2002)",
            @"PowerBook4,2", @"iBook 2 rev. 2 (w/ or w/o 14in LCD) (Nov 2002)",
            @"PowerBook4,3", @"iBook 2 (w/ or w/o 14in LDC)", @"PowerBook4,4",
            ...
            @"PowerBook G4 (Aluminum 12in)", @"PowerBook6,2", @"iBook G4",
            @"PowerBook6,3", @"PowerBook or iBook", @"PowerBook", // generic.

            @"Blue & White G3", @"PowerMac1,1", @"PowerMac G4 PCI Graphics",
            @"PowerMac1,2", @"iMac FireWire (CRT)", @"PowerMac2,1",
            @"iMac FireWire (CRT)", @"PowerMac2,2", @"PowerMac G4 AGP Graphics",
            @"PowerMac3,1", @"PowerMac G4 AGP Graphics", @"PowerMac3,2",
            ...
            @"iMac (Flower Power)", @"PowerMac4,1", @"iMac (Flat Panel 15in)",
            @"PowerMac4,2", @"eMac", @"PowerMac4,4", @"iMac (Flat Panel 17in)",
            @"PowerMac4,5", @"PowerMac G4 Cube", @"PowerMac5,1",
            @"PowerMac G4 Cube", @"PowerMac5,2", @"iMac (Flat Panel 17in)",
            @"PowerMac6,1", @"PowerMac G5", @"PowerMac7,2", @"PowerMac G5",
            @"PowerMac7,3", @"PowerMac", @"PowerMac", // generic.

            @"XServe", @"RackMac1,1", @"XServe rev. 2", @"RackMac1,2",
            @"XServe G5", @"RackMac3,1", @"XServe", @"RackMac",
    ...
    while ((aKey = [e nextObject])) {
      ...
      if (r.location != NSNotFound) {
        if (humanReadableName == nil ||
            [foundKey length] !=
                [internalName length]) // We didn't have an exact match yet?
        {
    ...
    // If it was a generic name, include the ugly name so we can add it to the
    // list:
    if ([foundKey rangeOfString:@","].location == NSNotFound)
      humanReadableName = [[NSString
          stringWithFormat:@"%@ (%@)", humanReadableName, foundKey] retain];
    // If nothing was found, at least show the ugly name so we have some hint:
    if (humanReadableName == nil)
      cpuName =
          [[NSString stringWithFormat:@"Unknown (%@)", internalName] retain];
    else
...
NSString *UKCPUName(void) { return UKAutoreleasedCPUName(NO); }
