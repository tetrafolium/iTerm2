...
- (NSColor *)tabBarColor {
  ...
  DLog(@"Computing tab bar color. delegate=%@ minimalStyleColor=%@",
       self.delegate, minimalStyleColor);
  return [self colorByDimmingColor:minimalStyleColor
...
- (NSColor *)textColorDefaultSelected:(BOOL)selected
  ...
  CGFloat backgroundBrightness = backgroundColor
                                     ? backgroundColor.it_hspBrightness
                                     : self.tabBarColor.it_hspBrightness;
  if (!backgroundColor) {
    DLog(@"Choose background brightness form tab bar color of %@",
         self.tabBarColor);
  }
  ...
  DLog(@"selected=%@ backgroundColor=%@ backgroundBrightness=%@ delta=%@ "
       @"value=%@",
       @(selected), backgroundColor, @(backgroundBrightness), @(delta),
       @(value));
  CGFloat alpha = 1;
  if (mainAndActive) {
    alpha = 0.75;
...
- (NSColor *)nonSelectedTabColor {
  const double difference = [[self.tabBar.delegate
            tabView:self.tabBar
      valueOfOption:PSMTabBarControlOptionMinimalStyleBackgroundColorDifference]
      doubleValue];
  return [self
      colorByDimmingColor:[self.tabBarColor
                              psm_nonSelectedColorWithDifference:difference]];
}
...
- (NSColor *)colorByDimmingColor:(NSColor *)color {
  const CGFloat dimmingAmount = [[self.tabBar.delegate
            tabView:self.tabBar
...
- (NSColor *)accessoryTextColor {
  ...
  NSColor *result =
      [self textColorDefaultSelected:YES
                     backgroundColor:nil
          windowIsMainAndAppIsActive:self.windowIsMainAndAppIsActive];
...
- (void)drawPostHocDecorationsOnSelectedCell:(PSMTabBarCell *)cell
  ...
  if (self.anyTabHasColor) {
    ...
    if (isHorizontal) {
      if (bar.cells.firstObject == cell &&
          self.treatLeftInsetAsPartOfFirstTab) {
        containingFrame = NSMakeRect(0, 0, NSMaxX(containingFrame),
                                     NSHeight(containingFrame));
      }
    ...
    CGFloat prominence = [[self.tabBar.delegate
              tabView:self.tabBar
        valueOfOption:
...
- (NSColor *)outlineColor {
  ...
  const CGFloat alpha = [[self.tabBar.delegate
            tabView:self.tabBar
      valueOfOption:PSMTabBarControlOptionColoredMinimalOutlineStrength]
      doubleValue];
  CGFloat value;
  if (backgroundBrightness < 0.5) {
    value = 1;
...
- (void)drawCellBackgroundSelected:(BOOL)selected
  ...
  [super drawCellBackgroundSelected:selected
                             inRect:insetCellFrame
...
- (NSColor *)selectedTabColor {
  ...
  PSMTabBarControl *bar = self.tabBar;
  BOOL selected = (bar.orientation == PSMTabBarHorizontalOrientation) ||
                  [self firstTabIsSelected];
...
- (NSRect)endInsetFrame {
  ...
  PSMTabBarControl *bar = self.tabBar;
  if (bar.orientation == PSMTabBarHorizontalOrientation) {
    return NSMakeRect(NSMaxX(cell.frame), 0,
    ...
    if (!self.tabBar.overflowPopUpButton.isHidden) {
      ...
      return NSMakeRect(0, NSHeight(self.tabBar.frame) - NSHeight(cell.frame),
                        NSWidth(cell.frame), NSHeight(cell.frame));
    }
    return NSMakeRect(0, NSMaxY(cell.frame), NSWidth(cell.frame),
...
- (void)drawTabBar:(PSMTabBarControl *)bar
  ...
  [super drawTabBar:bar
             inRect:rect
           clipRect:clipRect
         horizontal:horizontal
       withOverflow:withOverflow];
  const BOOL horizontalOrientation =
      bar.orientation == PSMTabBarHorizontalOrientation;
...
- (void)drawOutlineAroundBottomTabBarWithFirstTabSelected:
    (PSMTabBarControl *)bar {
  [self drawOutlineAfterSelectedTabInBottomTabBar:bar];
}

- (void)drawOutlineAroundBottomTabBarWithLastTabSelected:
    (PSMTabBarControl *)bar {
  [self drawOutlineBeforeSelectedTabInBottomTabBar:bar];
  [self drawOutlineAfterSelectedTabInBottomTabBar:bar];
}

- (void)drawOutlineAroundBottomTabBarWithInteriorTabSelected:
    (PSMTabBarControl *)bar {
  [self drawOutlineBeforeSelectedTabInBottomTabBar:bar];
  [self drawOutlineAfterSelectedTabInBottomTabBar:bar];
}
...
- (void)drawOutlineBeforeSelectedTabInBottomTabBar:(PSMTabBarControl *)bar {
  ...
  if (!cell || cell.isInOverflowMenu) {
    frame = NSMakeRect(
        NSMaxX(bar.frame) -
            [self
                rightMarginForTabBarControlWithOverflow:bar.lainOutWithOverflow
...
- (void)drawOutlineAroundTopTabBarWithInteriorTabSelected:
    (PSMTabBarControl *)bar {
  [self drawOutlineBeforeSelectedTabInTopTabBar:bar];
  [self drawOutlineAfterSelectedTabInTopTabBar:bar];
}
...
- (void)drawOutlineAroundVerticalTabBarWithFirstTabSelected:
    (PSMTabBarControl *)bar {
  if (!self.treatLeftInsetAsPartOfFirstTab) {
    [self drawOutlineAboveSelectedTabInVerticalTabBar:bar];
  }
...
- (void)drawOutlineAboveSelectedTabInVerticalTabBar:(PSMTabBarControl *)bar {
  ...
  NSRect frame = cell.frame;
  if (!cell || cell.isInOverflowMenu) {
    frame = NSMakeRect(0, NSMaxY(bar.frame) - bar.height, NSWidth(bar.frame),
                       bar.height);
  }
  const CGFloat top = 0.5;
...
- (void)drawOutlineAroundVerticalTabBarWithInteriorTabSelected:
    (PSMTabBarControl *)bar {
  [self drawOutlineAboveSelectedTabInVerticalTabBar:bar];
  [self drawOutlineUnderSelectedTabInVerticalTabBar:bar];
}
