...
@implementation PSMTabDragAssistant {
  ...
  // A window that shows a ghost pane while a tab is being dragged out of its
  // tab bar
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _sineCurveWidths =
        [[NSMutableArray alloc] initWithCapacity:kPSMTabDragAnimationSteps];
  }
...
- (void)addSineCurveWidthsWithOrientation:(PSMTabBarOrientation)orientation
                                     size:(NSSize)size {
  float cellStepSize = (orientation == PSMTabBarHorizontalOrientation)
                           ? (size.width + 6)
                           : (size.height + 1);
  for (int i = 0; i < kPSMTabDragAnimationSteps - 1; i++) {
    int thisWidth =
        (int)(cellStepSize -
              ((cellStepSize / 2.0) +
               ((sin((PI / 2.0) +
                     ((float)i / (float)kPSMTabDragAnimationSteps) * PI) *
                 cellStepSize) /
                2.0)));
    [_sineCurveWidths addObject:@(thisWidth)];
  }
  int width = (orientation == PSMTabBarHorizontalOrientation) ? size.width
                                                              : size.height;
...
- (void)startDraggingCell:(PSMTabBarCell *)cell
  ...
  [self addSineCurveWidthsWithOrientation:[control orientation]
                                     size:cellFrame.size];
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:PSMTabDragDidBeginNotification
                    object:nil];

  // Retain the control in case the drag operation causes the control to be
  // released
  [control retain];
  ...
  _dragTabWindow = [[PSMTabDragWindow
      dragWindowWithTabBarCell:cell
                         image:dragImage
  ...
  draggingRect = NSMakeRect(cellFrame.origin.x, cellFrame.origin.y, 1, 1);
  ...
  NSPoint cellOriginInWindow = [control convertPoint:cellFrame.origin
                                              toView:nil];
  ...
  NSDraggingSession *draggingSession =
      [control beginDraggingSessionWithItems:@[ dragItem ]
                                       event:event
                                      source:control];
...
- (void)draggingEnteredTabBar:(PSMTabBarControl *)control
  ...
  [[control overflowPopUpButton] setHidden:YES];
  [[control addTabButton] setHidden:YES];
  if ([[control cells] count] == 0 ||
      ![[[control cells] objectAtIndex:0] isPlaceholder]) {
  ...
  if (_dragViewWindow) {
    ...
    _fadeTimer =
        [NSTimer scheduledTimerWithTimeInterval:1.0 / 30.0
                                         target:self
                                       selector:@selector(fadeOutDragWindow:)
                                       userInfo:nil
                                        repeats:YES];
  }
}
...
- (void)draggingExitedTabBar:(PSMTabBarControl *)control {
  ...
  if (_fadeTimer) {
    ...
    if (!_dragViewWindow) {
      ...
      if ([control delegate] &&
          [[control delegate] respondsToSelector:@selector
                              (tabView:imageForTabViewItem:styleMask:)]) {
        // get a custom image representation of the view to drag from the
        // delegate
        ...
        NSPoint drawOffset = [control convertPoint:draggedCellFrame.origin
                                            toView:nil];
        drawOffset.y = control.window.frame.size.height - drawOffset.y;
        const NSRect controlFrameInWindowCoords =
            [control convertRect:control.bounds toView:nil];

        NSSize distanceFromWindowTopLeftToControlTopLeft;
        if ([control orientation] == PSMTabBarHorizontalOrientation) {
          distanceFromWindowTopLeftToControlTopLeft = NSMakeSize(
              controlFrameInWindowCoords.origin.x,
              control.window.frame.size.height - NSMaxY(
                                                     controlFrameInWindowCoords) /* - control.frame.size.height*/);
        } else {
          distanceFromWindowTopLeftToControlTopLeft = NSZeroSize;
        }
        _dragWindowOffset = NSMakeSize(
            draggedCellFrame.origin.x + // Position of cell relative to tab bar
                _dragTabOffset.width +
                distanceFromWindowTopLeftToControlTopLeft.width,
            draggedCellFrame.origin.y - _dragTabOffset.height +
                distanceFromWindowTopLeftToControlTopLeft.height);

        // _dragWindowOffset.height gives distance from top of window to mouse
        [viewImage lockFocus];

        // draw the tab into the returned window, that way we don't have two
        // windows being dragged (this assumes the tab will be on the window)
        drawPoint =
            NSMakePoint(drawOffset.x, [viewImage size].height - drawOffset.y);
        ...
        if ([control orientation] == PSMTabBarHorizontalOrientation) {
          ...
          case PSMTab_LeftTab:
            drawPoint.y =
                viewImage.size.height - self.draggedCell.frame.size.height;
            break;
        ...
        // The delegate doesn't give a custom image, so use an image of the
        // view.
        NSView *tabView = [[[self draggedCell] representedObject] view];
        viewImage =
            [[[NSImage alloc] initWithSize:[tabView frame].size] autorelease];
        [viewImage lockFocus];
      ...
      _dragViewWindow = [[PSMTabDragWindow
          dragWindowWithTabBarCell:[self draggedCell]
                             image:viewImage
                         styleMask:NSWindowStyleMaskBorderless] retain];
    ...
    const NSPoint windowOrigin =
        NSMakePoint(bottomLeftOfTabWindow.x + _dragWindowOffset.width,
                    bottomLeftOfTabWindow.y + _dragWindowOffset.height);

    [_dragViewWindow setFrameTopLeftPoint:windowOrigin];
    [_dragViewWindow orderWindow:NSWindowBelow
                      relativeTo:[_dragTabWindow windowNumber]];

    // Set the window's alpha mask to zero if the last tab is being dragged.
    // Don't fade out the old window if the delegate doesn't respond to the new
    // tab bar method, just to be safe.
    if ([[[self sourceTabBar] tabView] numberOfTabViewItems] == 1 &&
        [self sourceTabBar] == control &&
        [[[self sourceTabBar] delegate]
            respondsToSelector:@selector(tabView:
                                   newTabBarForDraggedTabViewItem:atPoint:)]) {
...
- (void)reallyPerformDragOperation:(id<NSDraggingInfo>)sender {
  ...
  int destinationIndex =
      [[[self destinationTabBar] cells] indexOfObject:[self targetCell]];

  // there is the slight possibility of the targetCell now being set properly,
  // so avoid errors
  ...
  if (![self draggedCell]) {
    ...
    NSTabViewItem *newTabViewItem = [[[self destinationTabBar] delegate]
                        tabView:[[self destinationTabBar] tabView]
    ...
    if (newTabViewItem) {
      [[[self destinationTabBar] tabView] insertTabViewItem:newTabViewItem
                                                    atIndex:insertIndex];
    ...
    if ([self sourceTabBar] != [self destinationTabBar]) {
      ...
      [[self sourceTabBar]
          removeTrackingRect:[[self draggedCell] closeButtonTrackingTag]];
      [[self sourceTabBar]
          removeTrackingRect:[[self draggedCell] cellTrackingTag]];
      ...
      for (i = 0, insertIndex = 0;
           (i < [cells count]) &&
           ([cells objectAtIndex:i] != [self draggedCell]);
           i++, insertIndex++) {
      ...
      if ([[[self sourceTabBar] delegate]
              respondsToSelector:@selector(tabView:
                                     willDropTabViewItem:inTabBar:)]) {
        [[[self sourceTabBar] delegate]
                        tabView:[[self sourceTabBar] tabView]
            willDropTabViewItem:[[self draggedCell] representedObject]
                       inTabBar:[self destinationTabBar]];
      }

      [[[self sourceTabBar] tabView]
          removeTabViewItem:[[self draggedCell] representedObject]];
      [[[self destinationTabBar] tabView]
          insertTabViewItem:[[self draggedCell] representedObject]
                    atIndex:insertIndex];

      // rebind the cell to the new control
      [[self destinationTabBar] initializeStateForCell:[self draggedCell]];
      [[self destinationTabBar]
          bindPropertiesForCell:[self draggedCell]
                 andTabViewItem:[[self draggedCell] representedObject]];

      // select the newly moved item in the destination tab view
      [[[self destinationTabBar] tabView]
          selectTabViewItem:[[self draggedCell] representedObject]];
    } else {
      // have to do this before checking the index of a cell otherwise
      // placeholders will be counted
      ...
      // find the index of where the dragged cell was just dropped
      for (theIndex = 0; theIndex < [cells count] &&
                         [cells objectAtIndex:theIndex] != [self draggedCell];
           theIndex++)
        ;

      if ([[[self sourceTabBar] cells] indexOfObject:[self draggedCell]] !=
              _draggedCellIndex &&
          [[[self sourceTabBar] delegate]
              respondsToSelector:@selector(tabView:
                                     willDropTabViewItem:inTabBar:)]) {
      ...
      // temporarily disable the delegate in order to move the tab to a
      // different index
  ...
  PSMTabBarControl *destination =
      [[[self destinationTabBar] retain] autorelease];
...
- (BOOL)shouldCreateNewWindowOnDrop:(BOOL *)moveWindow {
  ...
  return ([self destinationTabBar] == nil &&
          ...
          [sourceDelegate respondsToSelector:@selector
                          (tabView:newTabBarForDraggedTabViewItem:atPoint:)]);
}

- (void)createNewWindowWithTabBar:(PSMTabBarControl *)control
                           height:(CGFloat)height
                           origin:(const NSPoint)origin {
  id sourceDelegate = [[self sourceTabBar] delegate];
  if (!control) {
  ...
  if ([sourceDelegate respondsToSelector:@selector(tabView:
                                             willDropTabViewItem:inTabBar:)]) {
    [sourceDelegate tabView:[[self sourceTabBar] tabView]
        willDropTabViewItem:[[self draggedCell] representedObject]
                   inTabBar:control];
...
- (void)cancelDrag {
  [[[self sourceTabBar] cells] insertObject:[self draggedCell]
                                    atIndex:[self draggedCellIndex]];
  [[[self sourceTabBar] window]
      setAlphaValue:1]; // Make the window visible again.
...
- (void)draggedImageEndedAt:(NSPoint)aPoint
  ...
  const NSPoint origin =
      [self topLeftPointOfDragViewWindowForMouseLocation:aPoint];
  ...
  if ([self shouldCreateNewWindowOnDrop:&moveSourceWindow]) {
    ...
    PSMTabBarControl *control = [sourceDelegate
                               tabView:[[self sourceTabBar] tabView]
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:PSMTabDragDidEndNotification
                    object:nil];
...
- (void)moveDragTabWindowForMouseLocation:(NSPoint)aPoint {
  [_dragTabWindow
      setFrameTopLeftPoint:NSMakePoint(aPoint.x - _dragTabOffset.width,
                                       aPoint.y - _dragTabOffset.height)];
}

- (void)draggingBeganAt:(NSPoint)aPoint {
  ILog(@"Drag of %p began with current event %@ in window with frame %@ "
       @"from\n%@",
       [self sourceTabBar], [NSApp currentEvent],
       NSStringFromRect(self.sourceTabBar.window.frame),
       [NSThread callStackSymbols]);
  if (_dragTabWindow) {
...
- (void)draggingMovedTo:(NSPoint)aPoint {
  if (_dragTabWindow) {
    ...
    if (_dragViewWindow) {
      ...
      // depending on the position of the tab bar relative to the controlled tab
      // view
  ...
  for (int i = 0; i < [objects count]; ++i) {
    ...
    if ([_participatingTabBars containsObject:tabBar]) {
      ...
      [[NSRunLoop currentRunLoop]
          performSelector:@selector(display)
                   target:tabBar
                 argument:nil
                    order:1
                    modes:@[
...
- (void)calculateDragAnimationForTabBar:(PSMTabBarControl *)control {
  ...
  float position = [control orientation] == PSMTabBarHorizontalOrientation
  ...
  if ([self destinationTabBar] == control) {
    ...
    if (mouseLoc.x < [[control style] leftMarginForTabBarControl]) {
      ...
      NSRect overCellRect;
      PSMTabBarCell *overCell = [control cellForPoint:mouseLoc
                                            cellFrame:&overCellRect];
      if (overCell) {
        ...
        if ([overCell isPlaceholder]) {
          ...
          if (mouseLoc.x <
            ...
            [self setTargetCell:[cells
                                    objectAtIndex:([cells
                                                       indexOfObject:overCell] -
                                                   1)]];
          } else {
  ...
  for (i = 0; i < cellCount; i++) {
    ...
    if (![cell isInOverflowMenu]) {
      if ([cell isPlaceholder]) {
        ...
        if ([control orientation] == PSMTabBarHorizontalOrientation) {
          newRect.size.width =
              [[_sineCurveWidths objectAtIndex:[cell currentStep]] intValue];
        } else {
          newRect.size.height =
              [[_sineCurveWidths objectAtIndex:[cell currentStep]] intValue];
        }
    ...
    if ([cell indicator])
      [[cell indicator]
          setFrame:[[control style] indicatorRectForTabCell:cell]];
  }
  if (removeFlag) {
...
- (void)distributePlaceholdersInTabBar:(PSMTabBarControl *)control
  ...
  PSMTabBarCell *pc = [[[PSMTabBarCell alloc]
      initPlaceholderWithFrame:[[self draggedCell] frame]
                      expanded:YES
...
- (void)distributePlaceholdersInTabBar:(PSMTabBarControl *)control {
  ...
  for (i = 0; i < numVisibleTabs; i++) {
    PSMTabBarCell *pc =
        [[[PSMTabBarCell alloc] initPlaceholderWithFrame:draggedCellFrame
                                                expanded:NO
                                           inControlView:control] autorelease];
...
- (void)removeAllPlaceholdersFromTabBar:(PSMTabBarControl *)control {
  ...
  // redraw
  [[NSRunLoop currentRunLoop]
      performSelector:@selector(update)
               target:control
             argument:nil
                order:1
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    if ([aDecoder allowsKeyedCoding]) {
      ...
      _destinationTabBar =
          [[aDecoder decodeObjectForKey:@"destinationTabBar"] retain];
      _participatingTabBars =
          [[aDecoder decodeObjectForKey:@"participatingTabBars"] retain];
      ...
      _animationTimer =
          [[aDecoder decodeObjectForKey:@"animationTimer"] retain];
      _sineCurveWidths =
          [[aDecoder decodeObjectForKey:@"sineCurveWidths"] retain];
