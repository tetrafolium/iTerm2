...
NSString *const kPSMModifierChangedNotification =
    @"kPSMModifierChangedNotification";
NSString *const kPSMTabModifierKey = @"TabModifier";
NSString *const PSMTabDragDidEndNotification = @"PSMTabDragDidEndNotification";
NSString *const PSMTabDragDidBeginNotification =
    @"PSMTabDragDidBeginNotification";
...
PSMTabBarControlOptionKey
    PSMTabBarControlOptionColoredUnselectedTabTextProminence =
        @"PSMTabBarControlOptionColoredUnselectedTabTextProminence";
PSMTabBarControlOptionKey PSMTabBarControlOptionDimmingAmount =
    @"PSMTabBarControlOptionDimmingAmount";
PSMTabBarControlOptionKey
...
PSMTabBarControlOptionKey PSMTabBarControlOptionHighVisibility =
    @"PSMTabBarControlOptionHighVisibility";
PSMTabBarControlOptionKey
...
PSMTabBarControlOptionKey
    PSMTabBarControlOptionMinimalSelectedTabUnderlineProminence =
        @"PSMTabBarControlOptionMinimalSelectedTabUnderlineProminence";
PSMTabBarControlOptionKey PSMTabBarControlOptionDragEdgeHeight =
    @"PSMTabBarControlOptionDragEdgeHeight";
PSMTabBarControlOptionKey PSMTabBarControlOptionAttachedToTitleBar =
    @"PSMTabBarControlOptionAttachedToTitleBar";
PSMTabBarControlOptionKey PSMTabBarControlOptionHTMLTabTitles =
    @"PSMTabBarControlOptionHTMLTabTitles";
...
- (float)availableCellWidthWithOverflow:(BOOL)withOverflow {
  ...
  width =
      width - [_style leftMarginForTabBarControl] -
      [_style rightMarginForTabBarControlWithOverflow:withOverflow
                                         addTabButton:self.showAddTabButton] -
      _resizeAreaCompensation;
  return width;
}
...
- (id)initWithFrame:(NSRect)frame {
  ...
  if (self) {
    ...
    NSRect overflowButtonRect = NSMakeRect(
        [self frame].size.width -
            [_style
                rightMarginForTabBarControlWithOverflow:YES
                                           addTabButton:self.showAddTabButton] +
            1,
        0,
    ...
    _overflowPopUpButton =
        [[PSMOverflowPopUpButton alloc] initWithFrame:overflowButtonRect
                                            pullsDown:YES];
    if (_overflowPopUpButton) {
      // configure
      [_overflowPopUpButton
          setAutoresizingMask:NSViewNotSizable | NSViewMinXMargin];
      _overflowPopUpButton.accessibilityLabel = @"More tabs";
    }

    // new tab button
    NSRect addTabButtonRect = NSMakeRect(
        [self frame].size.width -
            [_style
                rightMarginForTabBarControlWithOverflow:YES
                                           addTabButton:self.showAddTabButton],
    ...
    [self registerForDraggedTypes:
              [NSArray arrayWithObjects:@"com.iterm2.psm.controlitem", nil]];
...
- (void)sanityCheckFailedWithCallsite:(NSString *)callsite
                               reason:(NSString *)reason {
  ILog(@"Sanity check failed from %@ for reason %@. Cells=%@. "
       @"tabView.tabViewItems=%@ stack:\n%@",
       callsite, reason, self.cells, self.tabView.tabViewItems,
  ...
  if (self.tabView.tabViewItems.count != self.cells.count) {
    ...
    for (NSInteger i = 0; i < self.cells.count; i++) {
      ...
      if (cell.representedObject != tabViewItem) {
        [self sanityCheckFailedWithCallsite:callsite
                                     reason:@"cells[i].representedObject != "
                                            @"tabView.tabViewItems[i]."
                                            @"representedObject"];
      }
    }
    DLog(@"Sanity check passed. cells=%@. tabView.tabViewITems=%@", self.cells,
...
- (void)setDelegate:(id<PSMTabBarControlDelegate>)object {
  ...
  NSMutableArray *types =
      [NSMutableArray arrayWithObject:@"com.iterm2.psm.controlitem"];

  // Update the allowed drag types
  if ([[self delegate]
          respondsToSelector:@selector(allowedDraggedTypesForTabView:)]) {
    [types addObjectsFromArray:[[self delegate]
                                   allowedDraggedTypesForTabView:_tabView]];
  }
  [self unregisterDraggedTypes];
...
- (NSLineBreakMode)truncationStyle {
  ...
  NSCountedSet *suffixIgnoringParenthesizedPartCounts =
      [[[NSCountedSet alloc] init] autorelease];
  ...
  for (PSMTabBarCell *cell in _cells) {
    ...
    NSString *suffix = [title
        substringFromIndex:(NSInteger)title.length - kPrefixOrSuffixLength];

    [prefixCounts addObject:prefix];
    [suffixCounts addObject:suffix];

    if (self.ignoreTrailingParentheticalsForSmartTruncation &&
        [title hasSuffix:@")"]) {
      NSInteger openParen =
          [title rangeOfString:@" (" options:NSBackwardsSearch].location;
      if (openParen != NSNotFound && openParen > kPrefixOrSuffixLength) {
        suffix = [title
            substringWithRange:NSMakeRange(openParen - kPrefixOrSuffixLength,
                                           kPrefixOrSuffixLength)];
  ...
  NSUInteger suffixCount =
      MAX(suffixCounts.count, suffixIgnoringParenthesizedPartCounts.count);
...
- (void)hideTabBar:(BOOL)hide animate:(BOOL)animate {
  ...
  float partnerOriginalSize, partnerOriginalOrigin, myOriginalSize,
      myOriginalOrigin, partnerTargetSize, partnerTargetOrigin, myTargetSize,
      myTargetOrigin;
  ...
  NSDictionary *userInfo = [NSDictionary
      dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:myOriginalOrigin],
                                   @"myOriginalOrigin",
                                   [NSNumber
                                       numberWithFloat:partnerOriginalOrigin],
                                   @"partnerOriginalOrigin",
                                   [NSNumber numberWithFloat:myOriginalSize],
                                   @"myOriginalSize",
                                   [NSNumber
                                       numberWithFloat:partnerOriginalSize],
                                   @"partnerOriginalSize",
                                   [NSNumber numberWithFloat:myTargetOrigin],
                                   @"myTargetOrigin",
                                   [NSNumber
                                       numberWithFloat:partnerTargetOrigin],
                                   @"partnerTargetOrigin",
                                   [NSNumber numberWithFloat:myTargetSize],
                                   @"myTargetSize",
                                   [NSNumber numberWithFloat:partnerTargetSize],
                                   @"partnerTargetSize", nil];
  [NSTimer scheduledTimerWithTimeInterval:(1.0 / 20.0)
                                   target:self
                                 selector:@selector(animateShowHide:)
                                 userInfo:userInfo
                                  repeats:YES];
}

- (void)animateShowHide:(NSTimer *)timer {
  // moves the frame of the tab bar and window (or partner view) linearly to
  // hide or show the tab bar
  NSRect myFrame = [self frame];
  NSDictionary *userInfo = [timer userInfo];
  float myCurrentOrigin =
      ([[userInfo objectForKey:@"myOriginalOrigin"] floatValue] +
       (([[userInfo objectForKey:@"myTargetOrigin"] floatValue] -
         [[userInfo objectForKey:@"myOriginalOrigin"] floatValue]) *
        (_currentStep / kPSMHideAnimationSteps)));
  float myCurrentSize =
      ([[userInfo objectForKey:@"myOriginalSize"] floatValue] +
       (([[userInfo objectForKey:@"myTargetSize"] floatValue] -
         [[userInfo objectForKey:@"myOriginalSize"] floatValue]) *
        (_currentStep / kPSMHideAnimationSteps)));
  float partnerCurrentOrigin =
      ([[userInfo objectForKey:@"partnerOriginalOrigin"] floatValue] +
       (([[userInfo objectForKey:@"partnerTargetOrigin"] floatValue] -
         [[userInfo objectForKey:@"partnerOriginalOrigin"] floatValue]) *
        (_currentStep / kPSMHideAnimationSteps)));
  float partnerCurrentSize =
      ([[userInfo objectForKey:@"partnerOriginalSize"] floatValue] +
       (([[userInfo objectForKey:@"partnerTargetSize"] floatValue] -
         [[userInfo objectForKey:@"partnerOriginalSize"] floatValue]) *
        (_currentStep / kPSMHideAnimationSteps)));

  NSRect myNewFrame;
  if ([self orientation] == PSMTabBarHorizontalOrientation) {
    myNewFrame = NSMakeRect(myFrame.origin.x, myCurrentOrigin,
                            myFrame.size.width, myCurrentSize);
  } else {
    myNewFrame = NSMakeRect(myCurrentOrigin, myFrame.origin.y, myCurrentSize,
                            myFrame.size.height);
  }
    ...
    if ([self orientation] == PSMTabBarHorizontalOrientation) {
      resizeRect =
          NSMakeRect([partnerView frame].origin.x, partnerCurrentOrigin,
                     [partnerView frame].size.width, partnerCurrentSize);
    } else {
      resizeRect =
          NSMakeRect(partnerCurrentOrigin, [partnerView frame].origin.y,
                     partnerCurrentSize, [partnerView frame].size.height);
    }
    ...
    if ([self orientation] == PSMTabBarHorizontalOrientation) {
      resizeRect =
          NSMakeRect([[self window] frame].origin.x, partnerCurrentOrigin,
                     [[self window] frame].size.width, partnerCurrentSize);
    } else {
      resizeRect =
          NSMakeRect(partnerCurrentOrigin, [[self window] frame].origin.y,
                     partnerCurrentSize, [[self window] frame].size.height);
    }
  ...
  if (_currentStep == kPSMHideAnimationSteps + 1) {
    ...
    if (_isHidden) {
      if ([[self delegate] respondsToSelector:@selector(tabView:
                                                  tabBarDidHide:)]) {
...
- (void)update:(BOOL)animate {
  // This method handles all of the cell layout, and is called when something
  // changes to require the refresh.  This method is not called during drag and
  // drop. See the PSMTabDragAssistant's calculateDragAnimationForTabBar:
  // method, which does layout in that case.
  ...
  if ([self orientation] == PSMTabBarHorizontalOrientation) {
    if ((animate || _animationTimer != nil) && cellCount > 0) {
      ...
      _animationTimer = [NSTimer
          scheduledTimerWithTimeInterval:1.0 / 30.0
                                ...
                                userInfo:
                                    [self
                                        cellWidthsForHorizontalArrangementWithOverflow:
                                            _lainOutWithOverflow]
                                 repeats:YES];
      return;
    } else {
      [self finishUpdateWithRegularWidths:
                [self cellWidthsForHorizontalArrangementWithOverflow:NO]
                       widthsWithOverflow:
                           [self cellWidthsForHorizontalArrangementWithOverflow:
                                     YES]];
    }
  } else {
    ...
    NSRect cellRect =
        [self genericCellRectWithOverflow:(NO || _showAddTabButton)];
    ...
    for (int i = 0; i < cellCount; ++i) {
      ...
      if (currentOrigin + cellRect.size.height <= [self frame].size.height) {
        ...
        if ([newOrigins count] > 0 &&
            [self frame].size.height - currentOrigin < cellRect.size.height) {
          [newOrigins removeLastObject];
        }
        break;
      }
    }
    [self finishUpdateWithRegularWidths:newOrigins
                     widthsWithOverflow:newOrigins];
  }

  [self setNeedsDisplay:YES];
}

// Tab widths may vary. Calculate the widths and see if this will work. Only
// allow sizes to vary if all tabs fit in the allotted space.
- (NSArray<NSNumber *> *)variableCellWidthsWithOverflow:(BOOL)withOverflow {
  const CGFloat availableWidth =
      [self availableCellWidthWithOverflow:withOverflow];
  CGFloat totalDesiredWidth = 0.0;
  NSMutableArray *desiredWidths = [NSMutableArray array];
  for (PSMTabBarCell *cell in _cells) {
    const CGFloat width =
        MAX(_cellMinWidth, MIN([cell desiredWidthOfCell], _cellMaxWidth));
...
- (BOOL)shouldUseOptimalWidthWithOverflow:(BOOL)withOverflow {
  ...
  BOOL canFitAllCellsOptimally =
      (self.cellOptimumWidth * _cells.count <= availableWidth);
  return !self.stretchCellsToFit && canFitAllCellsOptimally;
}

- (NSArray<NSNumber *> *)cellWidthsForHorizontalArrangementWithOverflow:
    (BOOL)withOverflow {
  ...
  if (self.sizeCellsToFit) {
    NSArray<NSNumber *> *widths =
        [self variableCellWidthsWithOverflow:withOverflow];
    if (widths) {
      return widths;
    }
  }
  ...
  if ([self shouldUseOptimalWidthWithOverflow:withOverflow]) {
    // Use the client-specified size, even if that leaves unused space on the
    // right.
    for (int i = 0; i < cellCount; i++) {
      [newWidths addObject:@(_cellOptimumWidth)];
    }
  } else {
    // Divide up the space evenly, but don't allow cells to be smaller than the
    // minimum width.
    CGFloat widthRemaining = availableWidth;
    // If all cells are the smallest allowed size, do they fit?
    const BOOL canFitAllCellsMinimally =
        (self.cellMinWidth * cellCount <= availableWidth);
    NSInteger numberOfVisibleCells =
        canFitAllCellsMinimally ? cellCount : (availableWidth / _cellMinWidth);
...
- (void)_animateCells:(NSTimer *)timer {
  ...
  if ([_cells count] > 0) {
    // compare our target widths with the current widths and move towards the
    // target
...
- (void)finishUpdateWithRegularWidths:(NSArray *)regularWidths
  ...
  if (!overflowMenu && _showAddTabButton) {
    ...
    if ([self orientation] == PSMTabBarHorizontalOrientation) {
      cellRect.origin.y = 0;
      cellRect.origin.x +=
          [[newValues valueForKeyPath:@"@sum.floatValue"] floatValue];
...
- (NSMenu *)_setupCells:(NSArray *)newValues {
  ...
  NSRect cellRect =
      [self genericCellRectWithOverflow:(_showAddTabButton ||
    ...
    if (i < numberOfVisibleCells) {
      ...
      if ([cell hasCloseButton] &&
          ([[cell representedObject]
               isEqualTo:[_tabView selectedTabViewItem]] ||
           [self allowsBackgroundTabClosing])) {
        NSPoint mousePoint = [self
            convertPoint:[[self window]
                             pointFromScreenCoords:[NSEvent mouseLocation]]
                fromView:nil];
        ...
        tag = [self addTrackingRect:closeRect
                              owner:cell
                           userData:nil
                       assumeInside:NO];
        [cell setCloseButtonTrackingTag:tag];

        // highlight the close button if the currently selected tab has the
        // mouse over it this will happen if the user clicks a close button in a
        // tab and all the tabs are rearranged
      ...
      tag = [self addTrackingRect:cellRect
                            owner:cell
                         userData:nil
                     assumeInside:NO];
      ...
      if ([[cell representedObject] isEqualTo:[_tabView selectedTabViewItem]]) {
        ...
        // previous cell
        if (i > 0) {
          [[_cells objectAtIndex:i - 1]
              setTabState:([(PSMTabBarCell *)[_cells objectAtIndex:i - 1]
                               tabState] |
                           PSMTab_RightIsSelectedMask)];
        }
        // next cell - see below
      } else {
      ...
      // more tab states
      if (cellCount == 1) {
        tabState |= PSMTab_PositionLeftMask | PSMTab_PositionRightMask |
                    PSMTab_PositionSingleMask;
      } else if (i == 0) {
      ...
      if (overflowMenu == nil) {
        ...
        [overflowMenu insertItemWithTitle:@"FIRST"
                                   action:nil
                            keyEquivalent:@""
                                  atIndex:0]; // Because the overflowPupUpButton
                                              // is a pull down menu
      }
      ...
      if ([cell count] > 0) {
        [menuItem setTitle:[[menuItem title]
                               stringByAppendingFormat:@" (%d)", [cell count]]];
      }
...
- (void)_setupOverflowMenu:(NSMenu *)overflowMenu {
  ...
  if ([self orientation] == PSMTabBarHorizontalOrientation) {
    ...
    cellRect.origin.x =
        ...
        [_style rightMarginForTabBarControlWithOverflow:YES
                                           addTabButton:self.showAddTabButton] +
        (_resizeAreaCompensation ? -_resizeAreaCompensation : 1);
  } else {
    cellRect.origin.x = 0;
  ...
  if (overflowMenu) {
    // Have a candidate for new overflow menu. Does it contain the same
    // information as the current one? If they're equal, we don't want to update
    // the menu since this happens several times per second while the user is
    // visiting the menu. But reading it is fine.
    BOOL equal = YES;
    equal = [_overflowPopUpButton menu] &&
            [[_overflowPopUpButton menu] numberOfItems] ==
                [overflowMenu numberOfItems];
...
- (void)mouseDown:(NSEvent *)theEvent {
  ...
  // what cell?
  NSPoint mousePt = [self convertPoint:[theEvent locationInWindow]
                              fromView:nil];
  NSRect frame = [self frame];

  if ([self orientation] == PSMTabBarVerticalOrientation &&
      [self allowsResizing] && partnerView &&
  ...
  if (cell) {
    BOOL overClose = NSMouseInRect(
        mousePt, [cell closeButtonRectForFrame:cellFrame], [self isFlipped]);
    if (overClose && cell.closeButtonVisible &&
        ([self allowsBackgroundTabClosing] ||
...
- (void)mouseDragged:(NSEvent *)theEvent {
  ...
  if (_resizing) {
    ...
    if ((currentPoint.x > frame.size.width && resizeAmount > 0) ||
        (currentPoint.x < frame.size.width && resizeAmount < 0)) {
      [[NSCursor resizeLeftRightCursor] push];

      NSRect partnerFrame = [partnerView frame];

      // do some bounds checking
      if ((frame.size.width + resizeAmount > [self cellMinWidth]) &&
          (frame.size.width + resizeAmount < [self cellMaxWidth])) {
  ...
  NSRect cellFrame;
  NSPoint trackingStartPoint = [self convertPoint:_initialDragLocation
                                         fromView:nil];
  PSMTabBarCell *cell = [self cellForPoint:trackingStartPoint
                                 cellFrame:&cellFrame];
  if (cell) {
    ...
    if (_closeClicked &&
        NSMouseInRect(trackingStartPoint, iconRect, [self isFlipped]) &&
        ([self allowsBackgroundTabClosing] ||
         [[cell representedObject] isEqualTo:[_tabView selectedTabViewItem]])) {
      [cell setCloseButtonPressed:NSMouseInRect(currentPoint, iconRect,
                                                [self isFlipped])];
      [self setNeedsDisplay:YES];
      return;
    }
    ...
    if (distance >= self.minimumTabDragDistance && !_didDrag &&
      ...
      _didDrag = YES;
      ILog(
          @"Start dragging with mouse down event %@ in window %p with frame %@",
          [self lastMouseDownEvent], self.window,
          NSStringFromRect(self.window.frame));
      [[PSMTabDragAssistant sharedDragAssistant]
           startDraggingCell:cell
                  fromTabBar:self
...
- (void)otherMouseUp:(NSEvent *)theEvent {
  ...
  if ([theEvent buttonNumber] == 2 && !_resizing) {
    ...
    PSMTabBarCell *mouseDownCell =
        [self cellForPoint:[self convertPoint:[[self lastMiddleMouseDownEvent]
                                                  locationInWindow]
                                     fromView:nil]
...
- (void)handleMouseUp:(NSEvent *_Nonnull)theEvent {
  const NSPoint clickPoint = [self convertPoint:[theEvent locationInWindow]
                                       fromView:nil];
  NSRect cellFrame;
  ...
  NSRect mouseDownCellFrame;
  PSMTabBarCell *mouseDownCell = [self
      cellForPoint:[self
                       convertPoint:[[self lastMouseDownEvent] locationInWindow]
                           fromView:nil]
         cellFrame:&mouseDownCellFrame];
  const NSRect iconRect =
      [mouseDownCell closeButtonRectForFrame:mouseDownCellFrame];
  const BOOL clickedInCloseButton =
      NSMouseInRect(clickPoint, iconRect, [self isFlipped]);

  if (clickedInCloseButton && cell.closeButtonVisible &&
  ...
  const BOOL mouseUpInSameCellAsMouseDown =
      NSMouseInRect(clickPoint, mouseDownCellFrame, [self isFlipped]);
  const NSPoint trackingStartPoint =
      [self convertPoint:[[self lastMouseDownEvent] locationInWindow]
                fromView:nil];
  const BOOL mouseDownWasInCloseButton =
      NSMouseInRect(trackingStartPoint,
                    [cell closeButtonRectForFrame:cellFrame], [self isFlipped]);
  const BOOL closeButtonDoesNotInterfere =
      (!mouseDownWasInCloseButton || [self disableTabClose] ||
       ![self allowsBackgroundTabClosing]);
  ...
  // Weird cases we don't care about, like mouse down in one cell and mouse up
  // in another.
  [mouseDownCell setCloseButtonPressed:NO];
...
- (NSMenu *)menuForEvent:(NSEvent *)event {
  NSMenu *menu = nil;
  NSTabViewItem *item = [[self
      cellForPoint:[self convertPoint:[event locationInWindow] fromView:nil]
         cellFrame:nil] representedObject];

  if (item && [[self delegate] respondsToSelector:@selector(tabView:
                                                      menuForTabViewItem:)]) {
    menu = [[self delegate] tabView:_tabView menuForTabViewItem:item];
  } else if (!item) {
    // when the "LSUIElement hack" (issue #954) is enabled, the menu bar is
    // inaccessible, so show it as a context menu when right-clicking empty
    // tabBar region
...
- (void)resetCursorRects {
  ...
  if ([self orientation] == PSMTabBarVerticalOrientation) {
    NSRect frame = [self frame];
    [self
        addCursorRect:NSMakeRect(frame.size.width - 2, 0, 2, frame.size.height)
    ...
    case PSMTab_TopTab:
      [self
          addCursorRect:NSMakeRect(0, 0, self.bounds.size.width, edgeDragHeight)
                 cursor:[NSCursor openHandCursor]];
      break;
    case PSMTab_BottomTab:
      [self
          addCursorRect:NSMakeRect(0, self.bounds.size.height - edgeDragHeight,
                                   self.bounds.size.width, edgeDragHeight)
                 cursor:[NSCursor openHandCursor]];
      break;
...
- (NSDraggingSession *)
    beginDraggingSessionWithItems:(NSArray<NSDraggingItem *> *)items
                            event:(NSEvent *)event
...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender {
  if ([[[sender draggingPasteboard] types]
    ...
    if ([[self delegate]
            ...
            (tabView:shouldDropTabViewItem:inTabBar:moveSourceWindow:)] &&
        ![[self delegate] tabView:[[sender draggingSource] tabView]
            shouldDropTabViewItem:[[[PSMTabDragAssistant sharedDragAssistant]
                                      draggedCell] representedObject]
                         inTabBar:self
    ...
    [[PSMTabDragAssistant sharedDragAssistant]
        draggingEnteredTabBar:self
                      atPoint:[self convertPoint:[sender draggingLocation]
                                        fromView:nil]];
    return NSDragOperationMove;
  } else if ([[self delegate] respondsToSelector:@selector
                              (tabView:draggingEnteredTabBarForSender:)]) {
    NSDragOperation op = [[self delegate] tabView:_tabView
                   draggingEnteredTabBarForSender:sender];
    if (op != NSDragOperationNone) {
      [[PSMTabDragAssistant sharedDragAssistant]
          startAnimationWithOrientation:_orientation
...
- (NSDragOperation)draggingUpdated:(id<NSDraggingInfo>)sender {
  PSMTabBarCell *cell = [self
      cellForPoint:[self convertPoint:[sender draggingLocation] fromView:nil]
         cellFrame:nil];
  ...
  if ([[[sender draggingPasteboard] types]
    ...
    [[PSMTabDragAssistant sharedDragAssistant]
        draggingUpdatedInTabBar:self
                        atPoint:[self convertPoint:[sender draggingLocation]
                                          fromView:nil]];
    return NSDragOperationMove;
  } else if ([[self delegate] respondsToSelector:@selector
                              (tabView:shouldAcceptDragFromSender:)] &&
             [[self delegate] tabView:_tabView
                 shouldAcceptDragFromSender:sender]) {
    [[PSMTabDragAssistant sharedDragAssistant]
        draggingUpdatedInTabBar:self
                        atPoint:[self convertPoint:[sender draggingLocation]
                                          fromView:nil]];
...
- (BOOL)prepareForDragOperation:(id<NSDraggingInfo>)sender {
  ...
  BOOL badType = [[[sender draggingPasteboard] types]
                     indexOfObject:@"com.iterm2.psm.controlitem"] == NSNotFound;
  if (badType &&
      [[self delegate] respondsToSelector:@selector(tabView:
                                              shouldAcceptDragFromSender:)] &&
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  if ([[[sender draggingPasteboard] types]
    ...
    [[PSMTabDragAssistant sharedDragAssistant] performDragOperation:sender];
                              ...
                              (tabView:acceptedDraggingInfo:onTabViewItem:)]) {
    // forward the drop to the delegate
    [[self delegate] tabView:_tabView
        acceptedDraggingInfo:sender
               onTabViewItem:
                   [[self
                       cellForPoint:[self convertPoint:[sender draggingLocation]
                                              fromView:nil]
                          cellFrame:nil] representedObject]];
  }
  return YES;
...
- (void)draggingSession:(NSDraggingSession *)session
  ...
  if (operation != NSDragOperationNone) {
    [self removeTabForCell:[[PSMTabDragAssistant sharedDragAssistant]
                               draggedCell]];
    [[PSMTabDragAssistant sharedDragAssistant] finishDrag];
  } else {
    [[PSMTabDragAssistant sharedDragAssistant] draggedImageEndedAt:aPoint
                                                         operation:operation];
  }
}
...
- (void)tabDoubleClick:(id)sender {
  if ([[self delegate] respondsToSelector:@selector(tabView:
                                              doubleClickTabViewItem:)]) {
    [[self delegate] tabView:[self tabView]
        doubleClickTabViewItem:[sender representedObject]];
  }
}

- (void)tabBarDoubleClick {
  if ([[self delegate]
          respondsToSelector:@selector(tabViewDoubleClickTabBar:)]) {
...
- (void)addTab:(id)sender {
  if ([self.delegate
          respondsToSelector:@selector(tabViewDidClickAddTabButton:)]) {
...
- (void)frameDidChange:(NSNotification *)notification {
  // figure out if the new frame puts the control in the way of the resize
  // widget
  ...
  if ([[self window] showsResizeIndicator] &&
      NSIntersectsRect([self frame], resizeWidgetFrame)) {
    // the resize widgets are larger on metal windows
    _resizeAreaCompensation =
        [[self window] styleMask] & NSWindowStyleMaskTexturedBackground ? 20
                                                                        : 8;
  } else {
  ...
  [self update];
  // trying to address the drawing artifacts for the progress indicators -
  // hackery follows this one fixes the "blanking" effect when the control hides
  // and shows itself
  for (PSMTabBarCell *cell in _cells) {
    ...
    if ([self orientation] == PSMTabBarHorizontalOrientation) {
      if (partnerView) {
        ...
        if (myFrame.origin.y - 22 > [partnerView frame].origin.y) {
          // partner is below me
          [self setFrame:NSMakeRect(myFrame.origin.x, myFrame.origin.y + 21,
                                    myFrame.size.width,
                                    myFrame.size.height - 21)];
          [partnerView
              setFrame:NSMakeRect(partnerFrame.origin.x, partnerFrame.origin.y,
                                  partnerFrame.size.width,
                                  partnerFrame.size.height + 21)];
        } else {
          // partner is above me
          [self setFrame:NSMakeRect(myFrame.origin.x, myFrame.origin.y,
                                    myFrame.size.width,
                                    myFrame.size.height - 21)];
          [partnerView setFrame:NSMakeRect(partnerFrame.origin.x,
                                           partnerFrame.origin.y - 21,
                                           partnerFrame.size.width,
                                           partnerFrame.size.height + 21)];
        }
        [partnerView setNeedsDisplay:YES];
        ...
        [[self window]
            setFrame:NSMakeRect(windowFrame.origin.x, windowFrame.origin.y + 21,
                                windowFrame.size.width,
                                windowFrame.size.height - 21)
             display:YES];
        [self
            setFrame:NSMakeRect(myFrame.origin.x, myFrame.origin.y,
                                myFrame.size.width, myFrame.size.height - 21)];
      }
    } else {
      if (partnerView) {
        NSRect partnerFrame = [partnerView frame];
        ...
        if (myFrame.origin.x < [partnerView frame].origin.x) {
          // partner is to the left
          [self setFrame:NSMakeRect(myFrame.origin.x, myFrame.origin.y, 1,
                                    myFrame.size.height)];
          [partnerView
              setFrame:NSMakeRect(
                           partnerFrame.origin.x - myFrame.size.width + 1,
                           partnerFrame.origin.y,
                           partnerFrame.size.width + myFrame.size.width - 1,
                           partnerFrame.size.height)];
        } else {
          // partner to the right
          [self setFrame:NSMakeRect(myFrame.origin.x + myFrame.size.width,
                                    myFrame.origin.y, 1, myFrame.size.height)];
          [partnerView
              setFrame:NSMakeRect(partnerFrame.origin.x, partnerFrame.origin.y,
                                  partnerFrame.size.width + myFrame.size.width,
                                  partnerFrame.size.height)];
        }
        ...
        NSRect windowFrame = [[self window] frame];
        [[self window]
            setFrame:NSMakeRect(windowFrame.origin.x + myFrame.size.width - 1,
                                windowFrame.origin.y,
                                windowFrame.size.width - myFrame.size.width + 1,
...
- (BOOL)validateMenuItem:(NSMenuItem *)sender {
  return [[self delegate] respondsToSelector:@selector
                          (tabView:validateOverflowMenuItem:forTabViewItem:)]
             ? [[self delegate] tabView:[self tabView]
                   validateOverflowMenuItem:sender
                             forTabViewItem:[sender representedObject]]
             : YES;
}
...
- (void)tabView:(NSTabView *)aTabView
  ...
  if ([[self delegate] respondsToSelector:@selector
                       (tabView:willInsertTabViewItem:atIndex:)]) {
    [[self delegate] tabView:aTabView
...
- (void)tabView:(NSTabView *)aTabView
  ...
  // here's a weird one - this message is sent before the
  // "aDidChangeNumberOfTabViewItems" message, thus I can end up updating when
  // there are no cells, if no tabs were (yet) present
  if ([_cells count] > 0) {
    [self update];
  }
  if ([[self delegate] respondsToSelector:@selector(tabView:
...
- (BOOL)tabView:(NSTabView *)aTabView
  ...
  if ([[self delegate] respondsToSelector:@selector(tabView:
    ...
    return (BOOL)[[self delegate] tabView:aTabView
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)aTabView {
  ...
  // go through cells, remove any whose representedObjects are not in [tabView
  // tabViewItems]
  ...
  for (PSMTabBarCell *cell in cellsToRemove) {
    if ([[self delegate] respondsToSelector:@selector(tabView:
                                                didCloseTabViewItem:)]) {
      [[self delegate] tabView:aTabView
           didCloseTabViewItem:[cell representedObject]];
    }
...
- (NSDragOperation)tabView:(NSTabView *)tabView
    draggingEnteredTabBarForSender:(id<NSDraggingInfo>)tagViewItem {
  return NSDragOperationNone;
}

- (BOOL)tabView:(NSTabView *)tabView
    shouldAcceptDragFromSender:(id<NSDraggingInfo>)tagViewItem {
  return NO;
}
...
- (NSString *)view:(NSView *)view
  ...
  if ([[self delegate] respondsToSelector:@selector(tabView:
                                              toolTipForTabViewItem:)]) {
    return
        [[self delegate] tabView:[self tabView]
            toolTipForTabViewItem:[[self cellForPoint:point
                                            cellFrame:nil] representedObject]];
  }
  return @"";
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  if ([aCoder allowsKeyedCoding]) {
    ...
    [aCoder encodeBool:_allowsBackgroundTabClosing
                forKey:@"PSMallowsBackgroundTabClosing"];
    [aCoder encodeBool:_allowsResizing forKey:@"PSMallowsResizing"];
    [aCoder encodeBool:_selectsTabsOnMouseDown
                forKey:@"PSMselectsTabsOnMouseDown"];
    ...
    [aCoder encodeObject:_lastMiddleMouseDownEvent
                  forKey:@"PSMlastMiddleMouseDownEvent"];
    [aCoder encodeObject:_delegate forKey:@"PSMdelegate"];
    [aCoder encodeBool:_useOverflowMenu forKey:@"PSMuseOverflowMenu"];
    [aCoder encodeBool:_automaticallyAnimates
                forKey:@"PSMautomaticallyAnimates"];
  }
}
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    if ([aDecoder allowsKeyedCoding]) {
      ...
      _overflowPopUpButton =
          [[aDecoder decodeObjectForKey:@"PSMoverflowPopUpButton"] retain];
      ...
      _allowsBackgroundTabClosing =
          [aDecoder decodeBoolForKey:@"PSMallowsBackgroundTabClosing"];
      _allowsResizing = [aDecoder decodeBoolForKey:@"PSMallowsResizing"];
      _selectsTabsOnMouseDown =
          [aDecoder decodeBoolForKey:@"PSMselectsTabsOnMouseDown"];
      ...
      _lastMouseDownEvent =
          [[aDecoder decodeObjectForKey:@"PSMlastMouseDownEvent"] retain];
      _lastMiddleMouseDownEvent =
          [[aDecoder decodeObjectForKey:@"PSMlastMiddleMouseDownEvent"] retain];
      _useOverflowMenu = [aDecoder decodeBoolForKey:@"PSMuseOverflowMenu"];
      _automaticallyAnimates =
          [aDecoder decodeBoolForKey:@"PSMautomaticallyAnimates"];
...
- (id)cellForPoint:(NSPoint)point cellFrame:(NSRectPointer)outFrame {
  if ([self orientation] == PSMTabBarHorizontalOrientation &&
      !NSPointInRect(point,
                     [self genericCellRectWithOverflow:_lainOutWithOverflow])) {
    return nil;
  }
...
- (void)modifierChanged:(NSNotification *)aNotification {
  NSUInteger mask = ([[[aNotification userInfo] objectForKey:kPSMTabModifierKey]
...
- (void)moveTabAtIndex:(NSInteger)sourceIndex
  ...
  [[destinationTabBar tabView] insertTabViewItem:[movingCell representedObject]
                                         atIndex:destinationIndex];

  // Rebind the cell to the new control.
  [destinationTabBar initializeStateForCell:movingCell];
  [destinationTabBar bindPropertiesForCell:movingCell
                            andTabViewItem:[movingCell representedObject]];

  // Select the newly moved item in the destination tab view.
  [[destinationTabBar tabView]
      selectTabViewItem:[movingCell representedObject]];

  if ([self.delegate respondsToSelector:@selector(tabView:
                                            didDropTabViewItem:inTabBar:)]) {
    [self.delegate tabView:self.tabView
  ...
  if ([self.tabView numberOfTabViewItems] == 0 &&
      [self.delegate respondsToSelector:@selector(tabView:
                                            closeWindowForLastTabViewItem:)]) {
    [self.delegate tabView:self.tabView
        closeWindowForLastTabViewItem:[movingCell representedObject]];
  }
}
