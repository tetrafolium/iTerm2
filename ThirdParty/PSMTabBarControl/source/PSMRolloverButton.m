...
- (void)setTargetAlpha:(CGFloat)targetAlpha {
  if (@available(macOS 10.14, *)) {
    if (fabs(targetAlpha - _targetAlpha) <
        PSMRolloverButtonDifferenceThreshold) {
      return;
    }
    _targetAlpha = targetAlpha;
    if (!_timer) {
      _timer = [NSTimer
          scheduledTimerWithTimeInterval:1.0 / PSMRolloverButtonFramesPerSecond
                                  target:self
...
// macOS 10.14+ because it needs layers to properly composite the background
// color.
- (void)updateBackgroundAlphaTimer:(NSTimer *)timer NS_AVAILABLE_MAC(10_14) {
  ...
  if (_alpha < _targetAlpha) {
    _alpha = MIN(_targetAlpha,
                 _alpha + changePerSecond / PSMRolloverButtonFramesPerSecond);
  } else {
    _alpha = MAX(_targetAlpha,
                 _alpha - changePerSecond / PSMRolloverButtonFramesPerSecond);
  }
  ...
  self.layer.backgroundColor = [[NSColor colorWithWhite:0.5
                                                  alpha:_alpha] CGColor];
}

// Must override mouseDown and mouseUp and not call super for mouseDragged: to
// work.
