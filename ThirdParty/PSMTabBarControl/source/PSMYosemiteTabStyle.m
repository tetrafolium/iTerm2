...
- (void)setPreviousAttributedString:
    (NSAttributedString *)previousAttributedString {
  self.psm_yosemiteAssociatedDictionary[@"attributedString"] =
      [[previousAttributedString copy] autorelease];
}

- (CGFloat)previousWidthOfAttributedString {
  return [self.psm_yosemiteAssociatedDictionary[@"attributedStringWidth"]
      doubleValue];
}

- (void)setPreviousWidthOfAttributedString:
    (CGFloat)previousWidthOfAttributedString {
  self.psm_yosemiteAssociatedDictionary[@"attributedStringWidth"] =
      @(previousWidthOfAttributedString);
}
...
- (id)init {
  if ((self = [super init])) {
    // Load close buttons
    _closeButton = [[[NSBundle bundleForClass:self.class]
        imageForResource:@"TabClose_Front"] retain];
    _closeButton.template = YES;
    _closeButtonDown = [[[NSBundle bundleForClass:self.class]
        imageForResource:@"TabClose_Front_Pressed"] retain];
    _closeButtonDown.template = YES;
    _closeButtonOver = [[[NSBundle bundleForClass:self.class]
        imageForResource:@"TabClose_Front_Rollover"] retain];
    ...
    _addTabButtonImage = [[NSImage alloc]
        initByReferencingFile:[[PSMTabBarControl bundle]
                                  pathForImageResource:addTabImageName]];
    _addTabButtonPressedImage = [[NSImage alloc]
        initByReferencingFile:[[PSMTabBarControl bundle]
                                  pathForImageResource:addTabImageName]];
    _addTabButtonRolloverImage = [[NSImage alloc]
        initByReferencingFile:[[PSMTabBarControl bundle]
                                  pathForImageResource:addTabImageName]];
    if (@available(macOS 10.16, *)) {
...
- (BOOL)windowIsMainAndAppIsActive {
  return (self.tabBar.window.isMainWindow && [NSApp isActive]);
}
...
- (CGFloat)edgeDragHeight {
  NSNumber *size =
      [self.tabBar.delegate tabView:self.tabBar
                      valueOfOption:PSMTabBarControlOptionDragEdgeHeight];
...
- (NSRect)closeButtonRectForTabCell:(PSMTabBarCell *)cell {
  ...
  result.origin.y = cellFrame.origin.y +
                    floor((cellFrame.size.height - result.size.height) / 2.0);
...
- (NSRect)iconRectForTabCell:(PSMTabBarCell *)cell {
  ...
  if ([cell count]) {
    ...
    minX = NSMinX([self indicatorRectForTabCell:cell]) -
           kSPMTabBarCellInternalXMargin;
  } else {
...
- (NSRect)objectCounterRectForTabCell:(PSMTabBarCell *)cell {
  ...
  float countWidth = [self
      retinaRoundUpCell:cell
                  value:[[self attributedObjectCountValueForTabCell:cell] size]
                            .width];
  ...
  NSRect result;
  result.size =
      NSMakeSize(countWidth, 2 * kPSMMetalObjectCounterRadius); // temp
  result.origin.x = cellFrame.origin.x + cellFrame.size.width -
                    kSPMTabBarCellInternalXMargin - result.size.width;
  result.origin.y = cellFrame.origin.y +
                    floor((cellFrame.size.height - result.size.height) / 2.0);
...
- (CGFloat)widthOfRightMatterInCell:(PSMTabBarCell *)cell {
  ...
  // object counter?
  if ([cell count] > 0) {
    resultWidth += [self objectCounterRectForTabCell:cell].size.width +
...
- (float)minimumWidthOfTabCell:(PSMTabBarCell *)cell {
  return ceil([self widthOfLeftMatterInCell:cell] + kPSMMinimumTitleWidth +
...
- (CGFloat)widthOfAttributedStringInCell:(PSMTabBarCell *)cell {
  ...
  if (![cell.previousAttributedString
...
- (NSAttributedString *)attributedObjectCountValueForTabCell:
    (PSMTabBarCell *)cell {
...
- (NSColor *)textColorDefaultSelected:(BOOL)selected
  ...
  return [NSColor colorWithSRGBRed:value / 255.0
                             green:value / 255.0
                              blue:value / 255.0
                             alpha:1];
}
...
- (NSColor *)textColorForCell:(PSMTabBarCell *)cell {
  ...
  if ([self anyTabHasColor]) {
    ...
    if (selected) {
      ...
      CGFloat prominence = [[_tabBar.delegate
                tabView:_tabBar
          valueOfOption:
    ...
    if (selected) {
      ...
      DLog(@"not selected");
      return [self textColorDefaultSelected:NO
                            backgroundColor:nil
                 windowIsMainAndAppIsActive:mainAndActive];
    }
  }
...
- (PSMCachedTitleInputs *)cachedTitleInputsForTabCell:(PSMTabBarCell *)cell {
  const BOOL parseHTML = [[_tabBar.delegate
            tabView:_tabBar
      valueOfOption:PSMTabBarControlOptionHTMLTabTitles] boolValue];
  PSMCachedTitleInputs *inputs = [[[PSMCachedTitleInputs alloc]
        initWithTitle:cell.stringValue
...
- (CGFloat)fontSize {
  NSNumber *override =
      [_tabBar.delegate tabView:_tabBar
                  valueOfOption:PSMTabBarControlOptionFontSizeOverride];
  if (override) {
...
- (NSColor *)topLineColorSelected:(BOOL)selected {
  ...
  if (@available(macOS 10.14, *)) {
    if (keyMainAndActive) {
      ...
      return [NSColor colorWithSRGBRed:209.0 / 255.0
                                 green:209.0 / 255.0
                                  blue:209.0 / 255.0
                                 alpha:1];
    }
    ...
    if (keyMainAndActive) {
      if (selected) {
        return [NSColor colorWithSRGBRed:189 / 255.0
                                   green:189 / 255.0
                                    blue:189 / 255.0
                                   alpha:1];
      } else {
        return [NSColor colorWithSRGBRed:160 / 255.0
                                   green:160 / 255.0
                                    blue:160 / 255.0
                                   alpha:1];
      }
    } else {
      return [NSColor colorWithSRGBRed:219 / 255.0
                                 green:219 / 255.0
                                  blue:219 / 255.0
                                 alpha:1];
    }
...
- (NSColor *)verticalLineColorSelected:(BOOL)selected {
  ...
  if (@available(macOS 10.14, *)) {
    if (keyMainAndActive) {
      return [NSColor colorWithSRGBRed:174.0 / 255.0
                                 green:174.0 / 255.0
                                  blue:174.0 / 255.0
                                 alpha:1];
    } else {
...
- (NSColor *)bottomLineColorSelected:(BOOL)selected {
  if (@available(macOS 10.16, *)) {
    ...
    if (keyMainAndActive) {
      ...
      return [NSColor colorWithSRGBRed:210 / 255.0
                                 green:210 / 255.0
                                  blue:210 / 255.0
                                 alpha:1];
    }
...
- (NSColor *)bigSurBackgroundColorSelected:(BOOL)selected
...
- (NSColor *)mojaveBackgroundColorSelected:(BOOL)selected
                           highlightAmount:(CGFloat)highlightAmount
...
- (NSColor *)backgroundColorSelected:(BOOL)selected
                     highlightAmount:(CGFloat)highlightAmount {
  if (@available(macOS 10.16, *)) {
    return [self bigSurBackgroundColorSelected:selected
                               highlightAmount:highlightAmount];
  } else {
    return [self mojaveBackgroundColorSelected:selected
                               highlightAmount:highlightAmount];
  }
}
...
- (void)drawVerticalLineInFrame:(NSRect)rect x:(CGFloat)x {
  ...
  NSRect modifiedRect = NSMakeRect(x, NSMinY(rect) + topInset, 1,
...
- (NSColor *)cellBackgroundColorForTabColor:(NSColor *)tabColor
  ...
  // Alpha the non-key window's tab colors a bit to make it clearer which window
  // is key.
  CGFloat alpha;
  const BOOL keyMainAndActive = self.windowIsMainAndAppIsActive;
  if (keyMainAndActive) {
  ...
  NSColor *color = [NSColor colorWithColorSpace:tabColor.colorSpace
...
- (NSColor *)effectiveBackgroundColorForTabWithTabColor:(NSColor *)tabColor
  ...
  DLog(@"Computing effective background color for tab with color %@ "
       @"selected=%@ highlight=%@",
       tabColor, @(selected), @(highlightAmount));
  NSColor *base = [[self backgroundColorSelected:selected
                                 highlightAmount:highlightAmount]
      it_srgbForColorInWindow:window];
  DLog(@"base=%@", base);
  if (tabColor) {
    NSColor *cellbg = [self cellBackgroundColorForTabColor:tabColor
                                                  selected:selected];
    DLog(@"cellbg=%@", cellbg);
    ...
    NSColor *result = [NSColor colorWithColorSpace:tabColor.colorSpace
                                        components:components
                                             count:4];
    DLog(@"return %@", result);
    return result;
...
- (void)drawCellBackgroundSelected:(BOOL)selected
  ...
  NSEdgeInsets backgroundInsets =
      [self backgroundInsetsWithHorizontalOrientation:horizontal];
  ...
  if (tabColor) {
    NSColor *color = [self cellBackgroundColorForTabColor:tabColor
                                                 selected:selected];
    // Alpha the inactive tab's colors a bit to make it clear which tab is
    // active.
    [color set];
  ...
  if (horizontal) {
    ...
    if (@available(macOS 10.14, *)) {
      if (isFirst && NSMinX(cellFrame) > 1) {
        ...
        // Because alpha is less than 1, we don't want to double-draw. I don't
        // think drawing the left line is necessary in earlier macOS versions
        // either but I don't feel like adding any risk at the moment.
    ...
    [self drawVerticalLineInFrame:cellFrame
                                x:NSMaxX(cellFrame) - rightAdjustment];
    ...
    if (@available(macOS 10.16, *)) {
      ...
      const BOOL drawBottomLine = [[_tabBar.delegate
                tabView:_tabBar
          valueOfOption:
...
- (void)drawTabCell:(PSMTabBarCell *)cell
    highlightAmount:(CGFloat)highlightAmount {
  // TODO: Test hidden control, whose height is less than 2. Maybe it happens
  // while dragging?
  [self drawCellBackgroundAndFrameHorizontallyOriented:
            (_orientation == PSMTabBarHorizontalOrientation)
                                          ...
                                          withTabColor:[cell tabColor]
                                               isFirst:cell ==
                                                       _tabBar.cells.firstObject
                                                isLast:cell ==
                                                       _tabBar.cells.lastObject
                                       highlightAmount:highlightAmount];
  [self drawInteriorWithTabCell:cell
                         inView:[cell controlView]
                highlightAmount:highlightAmount];
}

- (CGFloat)tabColorBrightness:(PSMTabBarCell *)cell {
  return
      [[self effectiveBackgroundColorForTabWithTabColor:cell.tabColor
                                               selected:(cell.state ==
                                                         NSControlStateValueOn)
                                        highlightAmount:0
...
- (BOOL)anyTabHasColor {
  return
      [_tabBar.cells
          indexOfObjectPassingTest:^BOOL(PSMTabBarCell *_Nonnull cell,
...
- (void)drawPostHocDecorationsOnSelectedCell:(PSMTabBarCell *)cell
  ...
  if (self.anyTabHasColor) {
    ...
    NSColor *outerColor;
    NSColor *innerColor;
    NSNumber *strengthNumber =
        [bar.delegate tabView:bar
                valueOfOption:
    ...
    const BOOL keyMainAndActive = self.windowIsMainAndAppIsActive;
    const CGFloat alpha =
        MIN(MAX(strength, 0), 1) * (keyMainAndActive ? 1 : 0.6);
    if (brightness > 0.5) {
...
- (void)drawInteriorWithTabCell:(PSMTabBarCell *)cell
  ...
  if ([cell hasCloseButton]) {
    if (cell.isCloseButtonSuppressed &&
        _orientation == PSMTabBarHorizontalOrientation) {
      // Do not use this much space on the left for the label, but the label is
      // centered as though it is not reserved if it's not too long.
      //
      //                Center
      //                   V
  ...
  // Draw graphic icon (i.e., the app icon, not new-output indicator icon) over
  // close button.
  if (cachedTitle.inputs.graphic) {
    const CGFloat width =
        [self drawGraphicWithCellFrame:cellFrame
  ...
  if ([cell hasIcon]) {
    // There is an icon. Draw it as long as the amount of space left for the
    // label is more than the size of the icon. This is a heuristic to roughly
    // prioritize the label over the icon.
    ...
    const CGFloat minimumLabelWidth = [[self.tabBar.delegate
              tabView:self.tabBar
    ...
    if (labelWidth > minimumLabelWidth) {
      ...
      // center in available space (in case icon image is smaller than
      // kPSMTabBarIconWidth)
  ...
  if ([cell count] > 0) {
    ...
    NSAttributedString *counterString =
        [self attributedObjectCountValueForTabCell:cell];
    counterStringRect.size = [counterString size];
    counterStringRect.origin.x =
        myRect.origin.x +
        floor((myRect.size.width - counterStringRect.size.width) / 2.0);
    counterStringRect.origin.y =
        myRect.origin.y +
        floor((myRect.size.height - counterStringRect.size.height) / 2.0);
  ...
  if (!cachedTitle.isEmpty) {
    ...
    labelRect.origin.y =
        cellFrame.origin.y +
        floor((cellFrame.size.height - boundingSize.height) / 2.0);
    labelRect.size.height = boundingSize.height;

    NSAttributedString *attributedString =
        [cachedTitle attributedStringForcingLeftAlignment:truncate
...
- (CGFloat)drawGraphicWithCellFrame:(NSRect)cellFrame
  ...
  NSRect rect = NSMakeRect(NSMinX(cellFrame) + 6,
                           NSMinY(cellFrame) +
                               (NSHeight(cellFrame) - image.size.height) / 2.0,
...
- (CGFloat)widthForLabelInCell:(PSMTabBarCell *)cell
  ...
  NSRect labelRect =
      NSMakeRect(labelPosition, 0,
                 cellFrame.size.width - (labelPosition - cellFrame.origin.x) -
                     kPSMTabBarCellPadding,
  ...
  if (drewIcon) {
    ...
    labelRect.size.width -=
        cell.indicator.frame.size.width + kPSMTabBarCellIconPadding;
  }

  if ([cell count] > 0) {
    labelRect.size.width -=
        ([self objectCounterRectForTabCell:cell].size.width +
         kPSMTabBarCellPadding);
  }
  ...
  BOOL truncate = NO;
  if (_orientation == PSMTabBarHorizontalOrientation) {
    CGFloat effectiveLeftMargin =
        (labelRect.size.width - boundingSize.width) / 2;
...
- (void)drawBackgroundInRect:(NSRect)rect
  ...
  if (_orientation == PSMTabBarHorizontalOrientation) {
    [NSBezierPath
        strokeLineFromPoint:NSMakePoint(rect.origin.x,
    ...
    // this looks ok with tabs on top but doesn't appear w/ tabs on bottom for
    // some reason
    [NSBezierPath
        strokeLineFromPoint:NSMakePoint(rect.origin.x, rect.origin.y - 0.5)
                    toPoint:NSMakePoint(rect.origin.x + rect.size.width,
                                        rect.origin.y - 0.5)];
  } else {
    [NSBezierPath
        strokeLineFromPoint:NSMakePoint(rect.origin.x, rect.origin.y + 0.5)
                    toPoint:NSMakePoint(rect.origin.x, rect.origin.y +
                                                           rect.size.height +
                                                           0.5)];

    [NSBezierPath
        strokeLineFromPoint:NSMakePoint(rect.origin.x + rect.size.width,
                                        rect.origin.y + 0.5)
                    toPoint:NSMakePoint(rect.origin.x + rect.size.width,
                                        rect.origin.y + rect.size.height +
                                            0.5)];
...
- (void)drawTabBar:(PSMTabBarControl *)bar
  ...
  [self drawBackgroundInRect:clipRect
  ...
  if (@available(macOS 10.16, *)) {
    ...
    const NSRect insetClipIntersection =
        NSIntersectionRect(clipRect, insetRect);
  ...
  if (![bar tabView]) {
    ...
    NSMutableAttributedString *attrStr = [[[NSMutableAttributedString alloc]
        initWithString:contents] autorelease];
  ...
  const BOOL attachedToTitleBar = [[bar.delegate
            tabView:bar
      valueOfOption:PSMTabBarControlOptionAttachedToTitleBar] boolValue];
  // draw cells
  for (int i = 0; i < 2; i++) {
    NSInteger stateToDraw =
        (i == 0 ? NSControlStateValueOn : NSControlStateValueOff);
    for (PSMTabBarCell *cell in [bar cells]) {
      if (![cell isInOverflowMenu] &&
        ...
        if (cell.state == stateToDraw) {
          ...
          if (@available(macOS 10.16, *)) {
            if ([self shouldDrawTopLineSelected:(stateToDraw ==
                                                 NSControlStateValueOn)
                                       attached:attachedToTitleBar
                                       position:bar.tabLocation]) {
              [topLineColor set];
              NSRectFill(
                  NSMakeRect(NSMinX(cell.frame), 0, NSWidth(cell.frame), 1));
            }
          }
  ...
  for (PSMTabBarCell *cell in [bar cells]) {
    if (![cell isInOverflowMenu] && NSIntersectsRect([cell frame], clipRect) &&
...
- (BOOL)shouldDrawTopLineSelected:(BOOL)selected
  ...
  case PSMTab_TopTab:
    ...
    // Leave out the line on the selected tab when it's attached to the tabbar
    // so it looks like it's the same surface.
    return NO;
  }
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  if ([aCoder allowsKeyedCoding]) {
    ...
    [aCoder encodeObject:_addTabButtonPressedImage
                  forKey:@"addTabButtonPressedImage"];
    [aCoder encodeObject:_addTabButtonRolloverImage
                  forKey:@"addTabButtonRolloverImage"];
  }
}
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    if ([aDecoder allowsKeyedCoding]) {
      ...
      _closeButtonDown =
          [[aDecoder decodeObjectForKey:@"metalCloseButtonDown"] retain];
      _closeButtonOver =
          [[aDecoder decodeObjectForKey:@"metalCloseButtonOver"] retain];
      _addTabButtonImage =
          [[aDecoder decodeObjectForKey:@"addTabButtonImage"] retain];
      _addTabButtonPressedImage =
          [[aDecoder decodeObjectForKey:@"addTabButtonPressedImage"] retain];
      _addTabButtonRolloverImage =
