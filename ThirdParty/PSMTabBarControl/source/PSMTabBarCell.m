...
@interface PSMTabAccessibilityElement
    : PSMTabAccessibilityElementPrototype <NSAccessibilityRadioButton>
@end
...
- (NSRect)accessibilityFrame {
  ...
  return PSMConvertAccessibilityFrameToScreen(cell.psmTabControlView,
                                              cell.frame);
}
...
- (BOOL)accessibilityPerformPress {
  ...
  return YES; // we don't actually know if -tabClick: succeeded, but for now,
              // let's pretend it did
}

@end

@interface PSMTabCloseButtonAccessibilityElement
    : PSMTabAccessibilityElementPrototype <NSAccessibilityButton>
@end
...
- (NSRect)accessibilityFrame {
  ...
  return PSMConvertAccessibilityFrameToScreen(
      controlView, [[controlView style] closeButtonRectForTabCell:cell]);
}

- (BOOL)accessibilityPerformPress {
  PSMTabBarCell *cell = self.cell;
  [cell.psmTabControlView closeTabClick:cell];
  return YES; // we don't actually know if -closeTabClick: succeeded, but for
              // now, let's pretend it did
}
...
- (id)initWithControlView:(PSMTabBarControl *)controlView {
  ...
  if (self) {
    ...
    _indicator = [[PSMProgressIndicator alloc]
        initWithFrame:NSMakeRect(0, 0, kPSMTabBarIndicatorWidth,
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:
          @"<%@: %p representedObject=%@ identifier=%@ objectCount=%@>",
          NSStringFromClass([self class]), self, self.representedObject,
...
- (PSMCachedTitle *)cachedTitle {
  ...
  NSInteger index = [_titleCache
      indexOfObjectPassingTest:^BOOL(PSMCachedTitle *_Nonnull obj,
  ...
  PSMCachedTitle *title =
      [[[PSMCachedTitle alloc] initWith:inputs] autorelease];
  [_titleCache insertObject:title atIndex:0];
...
- (void)updateStringValue:(NSTimer *)timer {
  ...
  [[self psmTabControlView]
      update:[[self psmTabControlView] automaticallyAnimates]];
}
...
- (void)setHasIcon:(BOOL)value {
  _hasIcon = value;
  [[self psmTabControlView]
      update:[[self psmTabControlView]
                 automaticallyAnimates]]; // binding notice is too fast
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  // the progress indicator, label, icon, or count has changed - redraw the
  // control view
  [[self psmTabControlView]
      update:[[self psmTabControlView] automaticallyAnimates]];
}
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
  ...
  [[[self psmTabControlView] style] drawTabCell:self
                                highlightAmount:[self highlightAmount]];
}
...
- (CGFloat)highlightAmount {
  NSTimeInterval timeSinceChange =
...
- (void)mouseEntered:(NSEvent *)theEvent {
  ...
  [[self psmTabControlView]
      setNeedsDisplayInRect:NSInsetRect([self frame], -2, -2)];
}
...
- (NSImage *)dragImage {
  NSRect cellFrame = [[[self psmTabControlView] style]
      dragRectForTabCell:self
             orientation:[[self psmTabControlView] orientation]];

  NSBitmapImageRep *rep;
  rep =
      [self.psmTabControlView bitmapImageRepForCachingDisplayInRect:cellFrame];
  [self.psmTabControlView cacheDisplayInRect:cellFrame toBitmapImageRep:rep];
  NSImage *image = [[[NSImage alloc] initWithSize:[rep size]] autorelease];
  [image addRepresentation:rep];
  NSImage *returnImage =
      [[[NSImage alloc] initWithSize:[rep size]] autorelease];
  ...
  if (![[self indicator] isHidden]) {
    ...
    NSImage *piImage =
        [[NSImage alloc] initByReferencingFile:[[PSMTabBarControl bundle]
                                                   pathForImageResource:@"pi"]];
    [returnImage lockFocus];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  if ([aCoder allowsKeyedCoding]) {
    ...
    [aCoder encodeBool:_isCloseButtonSuppressed
  ...
  if (self) {
    if ([aDecoder allowsKeyedCoding]) {
      ...
      _isCloseButtonSuppressed =
          [aDecoder decodeBoolForKey:@"isCloseButtonSuppressed"];
...
- (void)setUpAccessibilityElement {
  if (!_element) {
    _element = [[PSMTabAccessibilityElement alloc]
        initWithCell:self
                role:NSAccessibilityRadioButtonRole];
    if (_element) {
      PSMTabCloseButtonAccessibilityElement *closeButtonElement =
          [[PSMTabCloseButtonAccessibilityElement alloc]
              initWithCell:self
...
- (void)updateHighlight {
  if (self.isHighlighted) {
    ...
    NSPoint mouseLocationInWindowCoords =
        [self.controlView.window convertRectFromScreen:rectInScreenCoords]
            .origin;
    NSPoint mouseLocationInViewCoords =
        [self.controlView convertPoint:mouseLocationInWindowCoords
                              fromView:nil];
...
- (CGFloat)highlightAnimationDuration {
  if ([(PSMTabBarControl *)self.controlView orientation] ==
...
- (void)redrawHighlight:(NSTimer *)timer {
  [self.controlView setNeedsDisplayInRect:self.frame];
  if ([NSDate timeIntervalSinceReferenceDate] - _highlightChangeTime >
      [self highlightAnimationDuration]) {
    [timer invalidate];
  }
}
