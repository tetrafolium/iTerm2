...
- (instancetype)initWithTitle:(NSString *)title
  ...
  self = [super init];
  if (self) {
    _title = title.length < 256
                 ? [title copy]
                 : [[title substringToIndex:255] stringByAppendingString:@"…"];
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p title=%@ trunc=%@ color=%@ graphic=%@ "
                       @"orientation=%@ fontSize=%@ parseHTML=%@>",
                       NSStringFromClass([self class]), self, self.title,
                       @(self.truncationStyle), self.color, self.graphic,
                       @(self.orientation), @(self.fontSize), @(_parseHTML)];
}
...
- (NSAttributedString *)attributedString {
  ...
  if (_inputs.parseHTML) {
    textAttributedString =
        [NSAttributedString newAttributedStringWithHTML:_inputs.title
                                             attributes:attributes];
  } else {
    textAttributedString =
        [[NSAttributedString alloc] initWithString:_inputs.title
                                        attributes:attributes];
  }
...
- (NSAttributedString *)attributedStringForcingLeftAlignment:(BOOL)forceLeft
  ...
  if (needsComputed) {
    NSAttributedString *value =
        [self truncatedAttributedStringForWidth:truncatingWidth
...
- (NSAttributedString *)leftAlignedAttributedString {
  if (!_leftAlignedAttributedString) {
    _leftAlignedAttributedString = [self.attributedString
        attributedStringWithTextAlignment:NSTextAlignmentLeft];
  }
...
- (NSAttributedString *)truncatedAttributedStringForWidth:(CGFloat)width
  ...
  NSAttributedString *attributedString =
      forceLeft ? self.leftAlignedAttributedString : self.attributedString;
