...
- (id)initWithFrame:(NSRect)frameRect pullsDown:(BOOL)flag {
  ...
  if (self) {
    ...
    _PSMTabBarOverflowPopUpImage = [[NSImage alloc]
        initByReferencingFile:[[PSMTabBarControl bundle]
                                  pathForImageResource:@"overflowImage"]];
    _PSMTabBarOverflowDownPopUpImage =
        [[NSImage alloc] initByReferencingFile:
                             [[PSMTabBarControl bundle]
                                 pathForImageResource:@"overflowImagePressed"]];
...
- (void)drawRect:(NSRect)rect {
  ...
  NSImage *image =
      (_down) ? _PSMTabBarOverflowDownPopUpImage : _PSMTabBarOverflowPopUpImage;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  if ([aCoder allowsKeyedCoding]) {
    [aCoder encodeObject:_PSMTabBarOverflowPopUpImage
                  forKey:@"PSMTabBarOverflowPopUpImage"];
    [aCoder encodeObject:_PSMTabBarOverflowDownPopUpImage
                  forKey:@"PSMTabBarOverflowDownPopUpImage"];
    [aCoder encodeBool:_animatingAlternateImage
  ...
  if ((self = [super initWithCoder:aDecoder])) {
    if ([aDecoder allowsKeyedCoding]) {
      _PSMTabBarOverflowPopUpImage =
          [[aDecoder decodeObjectForKey:@"PSMTabBarOverflowPopUpImage"] retain];
      _PSMTabBarOverflowDownPopUpImage = [[aDecoder
          decodeObjectForKey:@"PSMTabBarOverflowDownPopUpImage"] retain];
      [self setAnimatingAlternateImage:
                [aDecoder decodeBoolForKey:
                              @"PSMTabBarOverflowAnimatingAlternateImage"]];
    }
  }
  return self;
}
