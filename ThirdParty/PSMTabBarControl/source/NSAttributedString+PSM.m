...
- (NSAttributedString *)attributedStringWithTextAlignment:
  ...
  NSDictionary *immutableAttributes =
      [self attributesAtIndex:representativeIndex effectiveRange:nil];
  ...
  [self
      enumerateAttributesInRange:NSMakeRange(0, self.length)
                      ...
                      usingBlock:^(NSDictionary<NSAttributedStringKey, id>
                        ...
                        updatedAttrs[NSParagraphStyleAttributeName] =
                            paragraphStyle;
...
+ (instancetype)newAttributedStringWithHTML:(NSString *)html
  ...
  NSDictionary * (^attributesForOptions)(PSMTextOption) =
      ^NSDictionary *(PSMTextOption options) {
    if (options == 0) {
      return attributes;
    }
    ...
    if (options & PSMTextOptionUnderline) {
      modifiedAttributes[NSUnderlineStyleAttributeName] =
  ...
  void (^appendWithTagRange)(NSRange) = ^(NSRange tagRange) {
    NSString *string = [html
        substringWithRange:NSMakeRange(cursor, tagRange.location - cursor)];
    string = [string stringByReplacingOccurrencesOfString:@"&lt;"
                                               withString:@"<"];
    string = [string stringByReplacingOccurrencesOfString:@"&amp;"
                                               withString:@"&"];
    NSAttributedString *part = [[NSAttributedString alloc]
  ...
  NSString *regex = [[openTags arrayByAddingObjectsFromArray:closeTags]
