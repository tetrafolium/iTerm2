...
/*! Creates a new "blank" login session. Switches to the LoginWindow. This does
 * NOT check to see if fast user switching is enabled! */
CG_EXTERN CGError CGSCreateLoginSession(CGSSessionID *outSession);

/*! Releases a session. */
CG_EXTERN CGError CGSReleaseSession(CGSSessionID session);

/*! Gets a list of sessions. Each session dictionary is in the format returned
 * by `CGSCopyCurrentSessionDictionary`. */
