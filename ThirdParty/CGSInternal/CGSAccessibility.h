...
/*! Gets whether the display is zoomed. I'm not sure why there's two calls that
 * appear to do the same thing - I think CGSIsZoomed calls through to
 * CGSDisplayIsZoomed. */
CG_EXTERN bool CGSDisplayIsZoomed(void);
CG_EXTERN CGError CGSIsZoomed(CGSConnectionID cid, bool *outIsZoomed);

/*! Gets and sets the cursor scale. The largest the Universal Access prefpane
 * allows you to go is 4.0. */
...
/*! Sets the display's contrast. There doesn't seem to be a get version of this
 * function. */
