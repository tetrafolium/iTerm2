 ...
 * it uses. This in turn is used for deadlock detection by seeing if inDatabase:
 * is called on the queue's dispatch queue, which should not happen and causes a
 * deadlock.
 */
...
- (FMDatabase *)database {
  if (![_db isOpen]) {
    if (!_db) {
      _db = FMDBReturnRetained(
          [[[self class] databaseClass] databaseWithPath:_path]);
    }
...
- (void)inDatabase:(__attribute__((noescape))void (^)(FMDatabase *db))block {
   ...
   * theory could be another DB queue and then check it against self to make
   * sure we're not about to deadlock. */
  FMDatabaseQueue *currentSyncQueue =
      (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
  assert(currentSyncQueue != self &&
         "inDatabase: was called reentrantly on the same queue, which would "
         "lead to a deadlock");
#endif
...
- (void)beginTransaction:(FMDBTransaction)transaction
...
- (void)inImmediateTransaction:
    (__attribute__((noescape))void (^)(FMDatabase *_Nonnull,
                                       BOOL *_Nonnull))block {
...
- (BOOL)checkpoint:(FMDBCheckpointMode)mode
...
- (BOOL)checkpoint:(FMDBCheckpointMode)mode
  ...
  FMDBRetain(self);
  dispatch_sync(_queue, ^() {
    result = [self.database checkpoint:mode
                                  name:name
