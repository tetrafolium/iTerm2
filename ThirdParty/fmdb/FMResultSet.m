    ...
    if (outErr) {
      if (_parentDB) {
        *outErr = [_parentDB lastError];
        ...
        // closed, we need to return the appropriate error.
...
- (BOOL)bindWithArray:(NSArray *)array
         orDictionary:(NSDictionary *)dictionary
             orVAList:(va_list)args {
  [_statement reset];
  return [_parentDB bindStatement:_statement.statement
             WithArgumentsInArray:array
                     orDictionary:dictionary
                         orVAList:args];
}
