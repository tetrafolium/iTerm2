...
- (FMResultSet *_Nullable)executeQuery:(NSString *)sql
                  withArgumentsInArray:(NSArray *_Nullable)arrayArgs
                          orDictionary:(NSDictionary *_Nullable)dictionaryArgs
...
- (BOOL)open {
  ...
  // if we previously tried to open and it failed, make sure to close it before
  // we try again
...
- (FMStatement *)cachedStatementForQuery:(NSString *)query {
  ...
  return [[statements
      objectsPassingTest:^BOOL(FMStatement *statement, BOOL *stop) {
...
- (void)setCachedStatement:(FMStatement *)statement forQuery:(NSString *)query {
  ...
  if (!query) {
    NSLog(@"API misuse, -[FMDatabase setCachedStatement:forQuery:] query must "
          @"not be nil");
    return;
  }
...
- (int)bindObject:(id)obj toColumn:(int)idx inStatement:(sqlite3_stmt *)pStmt {
  ...
  // FIXME - someday check the return codes on these binds.
    ...
    return sqlite3_bind_blob(pStmt, idx, bytes, (int)[obj length],
                             SQLITE_TRANSIENT);
  } else if ([obj isKindOfClass:[NSDate class]]) {
    if (self.hasDateFormatter)
      return sqlite3_bind_text(pStmt, idx,
                               [[self stringFromDate:obj] UTF8String], -1,
                               SQLITE_TRANSIENT);
    else
    ...
    if (strcmp([obj objCType], @encode(char)) == 0) {
      ...
      return sqlite3_bind_int64(pStmt, idx,
                                (long long)[obj unsignedLongLongValue]);
      ...
      return sqlite3_bind_text(pStmt, idx, [[obj description] UTF8String], -1,
                               SQLITE_TRANSIENT);
    }
  }
...
- (FMResultSet *)executeQuery:(NSString *)sql
  ...
  // we should only autoclose if we're binding automatically when the statement
  // is prepared
...
- (BOOL)executeStatements:(NSString *)sql
          withResultBlock:(__attribute__((noescape))
                           FMDBExecuteStatementsCallbackBlock)block {
...
- (BOOL)startSavePointWithName:(NSString *)name
  ...
  NSString *errorMessage = NSLocalizedStringFromTable(
      @"Save point functions require SQLite 3.7", @"FMDB", nil);
  if (self.logsErrors)
    NSLog(@"%@", errorMessage);
  return NO;
#endif
...
- (BOOL)checkpoint:(FMDBCheckpointMode)checkpointMode
             error:(NSError *__autoreleasing *)error {
  return [self checkpoint:checkpointMode
                     name:nil
            logFrameCount:NULL
...
#else
  NSLog(@"sqlite3_wal_checkpoint_v2 unavailable before sqlite 3.7.6. Ignoring "
        @"checkpoint mode: %d",
        mode);
...
// deprecated because "arguments" parameter is not maximum argument count, but
// actual argument count.

- (void)makeFunctionNamed:(NSString *)name
         maximumArguments:(int)arguments
                withBlock:
...
#if !__has_feature(objc_arc)
  sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments,
                          SQLITE_UTF8, (void *)b,
