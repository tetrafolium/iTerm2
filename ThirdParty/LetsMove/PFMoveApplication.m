...
#define kStrMoveApplicationCouldNotMove                                        \
  _I10NS(@"Could not move to Applications folder")
#define kStrMoveApplicationQuestionTitle _I10NS(@"Move to Applications folder?")
#define kStrMoveApplicationQuestionTitleHome                                   \
  _I10NS(@"Move to Applications folder in your Home folder?")
#define kStrMoveApplicationQuestionMessage                                     \
  _I10NS(@"I can move myself to the Applications folder if you'd like.")
...
#define kStrMoveApplicationQuestionInfoWillRequirePasswd                       \
  _I10NS(@"Note that this will require an administrator password.")
#define kStrMoveApplicationQuestionInfoInDownloadsFolder                       \
  _I10NS(@"This will allow auto-update to function.")
...
void PFMoveToApplicationsFolderIfNecessary(void) {
  ...
  NSString *applicationsDirectory =
      PreferredInstallLocation(&installToUserApplications);
  ...
  BOOL needAuthorization =
      ([fm isWritableFileAtPath:applicationsDirectory] == NO);

  // Check if the destination bundle is already there but not writable
  needAuthorization |= ([fm fileExistsAtPath:destinationPath] &&
                        ![fm isWritableFileAtPath:destinationPath]);
  ...
  NSAlert *alert = [[[NSAlert alloc] init] autorelease];
  {
    ...
    [alert setMessageText:(installToUserApplications
                               ? kStrMoveApplicationQuestionTitleHome
    ...
    if (needAuthorization) {
      ...
      informativeText = [informativeText
          stringByAppendingString:
              kStrMoveApplicationQuestionInfoWillRequirePasswd];
    } else if (IsInDownloadsFolder(bundlePath)) {
      // Don't mention this stuff if we need authentication. The informative
      // text is long enough as it is in that case.
      informativeText = [informativeText stringByAppendingString:@" "];
      informativeText = [informativeText
          stringByAppendingString:
              kStrMoveApplicationQuestionInfoInDownloadsFolder];
    }
    ...
    NSButton *cancelButton =
        [alert addButtonWithTitle:kStrMoveApplicationButtonDoNotMove];
    [cancelButton
  ...
  if ([alert runModal] == NSAlertFirstButtonReturn) {
    ...
    if (needAuthorization) {
      ...
      if (!AuthorizedInstall(bundlePath, destinationPath,
        ...
        if (authorizationCanceled) {
          ...
          NSLog(@"ERROR -- Could not copy myself to /Applications with "
                @"authorization");
          goto fail;
        }
      }
    } else {
      // If a copy already exists in the Applications folder, put it in the
      // Trash
    ...
    // NOTE: This final delete does not work if the source bundle is in a
    // network mounted volume.
    //       Calling rm or file manager's delete method doesn't work either.
    //       It's unlikely to happen but it'd be great if someone could fix
    //       this.
    if (!isNestedApplication && diskImageDevice == nil &&
        !DeleteOrTrash(bundlePath)) {
      NSLog(@"WARNING -- Could not delete application after moving it to "
            @"Applications folder");
    }
    ...
    // Launched from within a disk image? -- unmount (if no files are open after
    // 5 seconds, otherwise leave it mounted).
...
static NSString *PreferredInstallLocation(BOOL *isUserDirectory) {
  ...
  NSArray *userApplicationsDirs = NSSearchPathForDirectoriesInDomains(
  ...
  if ([userApplicationsDirs count] > 0) {
    ...
    if ([fm fileExistsAtPath:userApplicationsDir isDirectory:&isDirectory] &&
        isDirectory) {
      // User Applications directory exists. Get the directory contents.
      NSArray *contents = [fm contentsOfDirectoryAtPath:userApplicationsDir
                                                  error:NULL];
  ...
  // No user Applications directory in use. Return the machine local
  // Applications directory
  if (isUserDirectory)
    *isUserDirectory = NO;

  return [[NSSearchPathForDirectoriesInDomains(
      NSApplicationDirectory, NSLocalDomainMask, YES) lastObject]
...
static BOOL IsInDownloadsFolder(NSString *path) {
  NSArray *downloadDirs = NSSearchPathForDirectoriesInDomains(
      NSDownloadsDirectory, NSAllDomainsMask, YES);
...
static BOOL IsApplicationAtPathRunning(NSString *bundlePath) {
  ...
  // Use the new API on 10.6 or higher to determine if the app is already
  // running
...
#endif
  // Use the shell to determine if the app is already running on systems 10.5 or
  // lower
  NSString *script =
      [NSString stringWithFormat:@"/bin/ps ax -o comm | /usr/bin/grep %@/ | "
                                 @"/usr/bin/grep -v grep >/dev/null",
                                 ShellQuotedString(bundlePath)];
  NSTask *task = [NSTask
      launchedTaskWithLaunchPath:@"/bin/sh"
                       arguments:[NSArray arrayWithObjects:@"-c", script, nil]];
  [task waitUntilExit];

  // If the task terminated with status 0, it means that the final grep produced
  // 1 or more lines of output. Which means that the app is already running
...
static BOOL DeleteOrTrash(NSString *path) {
  ...
  if ([[NSFileManager defaultManager] removeItemAtPath:path error:&error]) {
    ...
    NSLog(@"WARNING -- Could not delete '%@': %@", path,
...
static BOOL AuthorizedInstall(NSString *srcPath, NSString *dstPath,
  ...
  // running 'sudo rm -rf' so we really don't want to fuck this up.
...
static void Relaunch(NSString *destinationPath) {
  ...
  if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_5) {
    ...
    preOpenCmd =
        [NSString stringWithFormat:@"/usr/bin/xattr -d com.apple.quarantine %@",
                                   quotedDestinationPath];
  }

  NSString *script = [NSString
      stringWithFormat:@"(while /bin/kill -0 %d >&/dev/null; do /bin/sleep "
                       @"0.1; done; %@; /usr/bin/open %@) &",
