...
- (NSMutableDictionary *)rulesByNonTerminal {
  return (NSMutableDictionary *)objc_getAssociatedObject(
      self, &rulesByNonTerminalKey);
}

- (void)setRulesByNonTerminal:(NSMutableDictionary *)newRulesByNonTerminal {
  objc_setAssociatedObject(self, &rulesByNonTerminalKey, newRulesByNonTerminal,
...
- (void)setFollowCache:(NSMutableDictionary *)newFollowCache {
  objc_setAssociatedObject(self, &followCacheKey, newFollowCache,
                           OBJC_ASSOCIATION_RETAIN);
}

- (NSArray *)rules {
  NSMutableArray *rs =
      [NSMutableArray arrayWithCapacity:[[self rulesByNonTerminal] count]];
...
- (void)setRules:(NSArray *)newRules {
  @synchronized(self) {
    [self setRulesByNonTerminal:[NSMutableDictionary
                                    dictionaryWithCapacity:[newRules count]]];
...
- (NSArray *)orderedRules {
  return [[[self allRules] allObjects]
      sortedArrayUsingComparator:^NSComparisonResult(CPRule *r1, CPRule *r2) {
        NSComparisonResult t =
            [r1 tag] < [r2 tag]
                ? NSOrderedDescending
                : [r1 tag] > [r2 tag] ? NSOrderedAscending : NSOrderedSame;
        NSComparisonResult r =
            NSOrderedSame != t ? t : [[r1 name] compare:[r2 name]];
        return NSOrderedSame != r
                   ? r
                   : ([[r1 rightHandSideElements] count] <
                              [[r2 rightHandSideElements] count]
                          ? NSOrderedAscending
                          : ([[r1 rightHandSideElements] count] >
                                     [[r2 rightHandSideElements] count]
                                 ? NSOrderedDescending
