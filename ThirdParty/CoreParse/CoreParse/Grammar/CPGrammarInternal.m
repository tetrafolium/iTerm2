...
- (CPGrammar *)augmentedGrammar {
  return [[[CPGrammar alloc]
      initWithStart:@"s'"
              rules:[[self rules]
                        arrayByAddingObject:
                            [CPRule ruleWithName:@"s'"
...
- (NSSet *)lr0Closure:(NSSet *)i {
  ...
  while ([processingQueue count] > 0) {
    ...
    if (![nextSymbol isTerminal]) {
      [[self rulesForNonTerminalWithName:[nextSymbol name]]
          enumerateObjectsUsingBlock:^(CPRule *rule, NSUInteger ix, BOOL *s) {
...
- (NSArray *)lr0Kernels {
  CPRule *startRule =
      [[self rulesForNonTerminalWithName:[self start]] objectAtIndex:0];
  NSSet *initialKernel = [NSSet setWithObject:[CPItem itemWithRule:startRule
                                                          position:0]];
...
- (NSSet *)lr1Closure:(NSSet *)i {
  ...
  while ([processingQueue count] > 0) {
    ...
    CPGrammarSymbol *nextSymbol =
        [followingSymbols count] > 0 ? [followingSymbols objectAtIndex:0] : nil;
    if (![nextSymbol isTerminal]) {
      NSArray *rs = [self
          rulesForNonTerminalWithName:[(CPGrammarSymbol *)nextSymbol name]];
      for (CPRule *r in rs) {
        NSArray *afterNext = [followingSymbols
            subarrayWithRange:NSMakeRange(1, [followingSymbols count] - 1)];
        NSArray *afterNextWithTerminal =
            [afterNext arrayByAddingObject:[item terminal]];
        NSSet *firstAfterNext = [self first:afterNextWithTerminal];
        for (NSString *o in firstAfterNext) {
          CPLR1Item *newItem =
              [CPLR1Item lr1ItemWithRule:r
                                position:0
                                terminal:[CPGrammarSymbol terminalWithName:o]];
          if (![j containsObject:newItem]) {
            [processingQueue addObject:newItem];
            [j addObject:newItem];
          }
...
- (NSSet *)follow:(NSString *)name {
  ...
  if (nil == follows) {
    ...
    for (CPRule *rule in [self allRules]) {
      ...
      [rightHandSide enumerateObjectsUsingBlock:^(CPGrammarSymbol *rhsE,
                                                  NSUInteger idx, BOOL *s) {
        if (![rhsE isTerminal] && [[rhsE name] isEqualToString:name]) {
          if (idx + 1 < numElements) {
            NSSet *first = [self
                first:[rightHandSide
                          subarrayWithRange:NSMakeRange(idx + 1,
                                                        [rightHandSide count] -
                                                            idx - 1)]];
            NSSet *firstMinusEmpty = [first
                objectsPassingTest:^BOOL(NSString *symbolName, BOOL *fstop) {
...
- (NSArray *)tidyRightHandSides:(NSArray *)oldRules error:(NSError **)error {
  ...
  for (CPRule *r in oldRules) {
    [rhsElements
        unionSet:[self
                     collectRHSElementsForNewRules:[r rightHandSideElements]]];
  }
              ...
              errorWithDomain:CPEBNFParserErrorDomain
                     ...
                     userInfo:[NSDictionary
                                  dictionaryWithObjectsAndKeys:
                                      [NSString
                                          stringWithFormat:
                                              @"Duplicate tag names %@ in same "
                                              @"part of alternative is not "
                                              @"allowed in \"%@\".",
                                              duplicateTags, rule],
                ...
                errorWithDomain:CPEBNFParserErrorDomain
                       ...
                       userInfo:[NSDictionary
                                    dictionaryWithObjectsAndKeys:
                                        [NSString
                                            stringWithFormat:
                                                @"Duplicate tag names (%@) in "
                                                @"same part of alternative is "
                                                @"not allowed in \"%@\".",
                                                intersection, rule],
                                        NSLocalizedDescriptionKey, nil]];
          }
...
- (NSDictionary *)nameNewRules:(NSSet *)rhsElements
  ...
  NSUInteger name = 0;
  NSMutableDictionary *namedRules =
      [NSMutableDictionary dictionaryWithCapacity:[rhsElements count]];
  for (CPRHSItem *item in rhsElements) {
    [namedRules
        setObject:[self
                      symbolNameNotInSet:symbolNames
                             basedOnName:[NSString stringWithFormat:@"RHS%ld",
                                                                    (long)name]]
           forKey:item];
    name++;
  }
...
- (NSArray *)addRHSRules:(NSDictionary *)newRules toRules:(NSArray *)oldRules {
  NSMutableArray *rules =
      [[[NSMutableArray alloc] initWithArray:oldRules] autorelease];
  ...
  for (CPRHSItem *item in newRules) {
    ...
    if ([[item alternatives] count] == 1) {
      ...
      if ([item mayNotExist]) {
        rule = [[[CPRule alloc] initWithName:ruleName
                       rightHandSideElements:[NSArray array]] autorelease];
        [rule setTag:0];
      } else {
        rule =
            [[[CPRule alloc] initWithName:ruleName
                    rightHandSideElements:[[item alternatives] objectAtIndex:0]]
                autorelease];
        [rule setTag:1];
      }
      ...
      if ([item repeats]) {
        rule = [[[CPRule alloc]
                     initWithName:ruleName
            rightHandSideElements:
                [[[item alternatives] objectAtIndex:0]
                    arrayByAddingObject:[CPGrammarSymbol
                                            nonTerminalWithName:ruleName]]]
            autorelease];
        [rule setTag:2];
      ...
      for (NSArray *contents in [item alternatives]) {
        CPRule *rule = [[[CPRule alloc] initWithName:ruleName
                               rightHandSideElements:contents] autorelease];
        [rule setTag:3 + i];
        [rule setRepresentitiveClass:rhsItemClass];
        [rule setShouldCollapse:[item shouldCollapse]];
        [rule setTagNames:[item tags]];
  ...
  for (CPRule *rule in rules) {
    ...
    NSMutableArray *newRightHandSideElements =
        [NSMutableArray arrayWithCapacity:[rhsElements count]];
    for (id element in rhsElements) {
      [newRightHandSideElements
          addObject:[element isRHSItem]
                        ? [CPGrammarSymbol
                              nonTerminalWithName:[newRules
                                                      objectForKey:element]]
                        : element];
    }
