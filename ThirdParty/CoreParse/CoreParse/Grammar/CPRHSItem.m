...
- (NSUInteger)hash {
  return [[self alternatives] hash] ^
         ([self repeats] ? 0x2 : 0x0) + ([self mayNotExist] ? 0x1 : 0x0);
}
...
- (BOOL)isEqual:(id)object {
  return ([object isRHSItem] &&
          ...
          ([self tags] == nil ||
           [[self tags] isEqualToSet:[(CPRHSItem *)object tags]]));
}
...
- (NSString *)description {
  ...
  if ([[self alternatives] count] != 1 ||
      [[[self alternatives] objectAtIndex:0] count] != 1) {
    [desc appendString:@"("];
  }
  NSUInteger i = 0;
  ...
  [desc appendString:[self repeats] ? ([self mayNotExist] ? @"*" : @"+")
                                    : ([self mayNotExist] ? @"?" : @"")];
  return desc;
}
  ...
  for (NSArray *components in [self alternatives]) {
    ...
    for (id comp in components) {
      if ([comp isRHSItem]) {
        ...
        NSMutableSet *duplicateTags =
            [[tagNamesInAlternative mutableCopy] autorelease];
                             ...
                             [NSDictionary
                                 dictionaryWithObjectsAndKeys:
                                     [NSString
                                         stringWithFormat:
                                             @"Duplicate tag names (%@) in "
                                             @"same part of alternative is not "
                                             @"allowed in \"%@\".",
                                             intersection, self],
                                     NSLocalizedDescriptionKey, nil]];
            }
          ...
          errorWithDomain:CPEBNFParserErrorDomain
                 ...
                 userInfo:[NSDictionary
                              dictionaryWithObjectsAndKeys:
                                  [NSString
                                      stringWithFormat:
                                          @"Tag names are not allowed within "
                                          @"repeating section of rule \"%@\".",
                                          self],
                                  NSLocalizedDescriptionKey, nil]];
    }
    return nil;
