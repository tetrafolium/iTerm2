...
/**
 * The CPParseResult protocol declares a method that a class must implement so
 * that instances can be created as the result of parsing a token stream.
 */
...
/**
 * The delegate of a CPParser must adopt the CPParserDelegate protocol.  This
 * allows you to replace the produced syntax trees with data structures of your
 * choice.
 *
 * Significant processing can be performed in a parser delegate.  For example, a
 * parser for numeric expressions could replace each syntax tree with an
 * NSNumber representing the resultant value of evaluating the expression.  This
 * would allow you to parse, and compute the result of the expression in one
 * pass.
 */
...
/**
 * Called when the parser encounters a token for which it can not shift, reduce
 * or accept.
 *
 * @param parser           The parser which produced the syntax tree.
 * @param inputStream      The input stream containing the token the parser
 * could not cope with.
 * @return An action to take to recover from the parse error or nil.  If the
 * action is nil, and the problematic token is a CPErrorToken the parse stack is
 * unwound a step for the parent rule to deal with the error.
 * @bug Warning this method is deprecated, use
 * -parser:didEncounterErrorOnInput:expecting: instead.
 */
- (CPRecoveryAction *)parser:(CPParser *)parser
    didEncounterErrorOnInput:(CPTokenStream *)inputStream
    __attribute__((deprecated(
        "use -parser:didEncounterErrorOnInput:expecting: instead.")));
 ...
 * @param inputStream      The input stream containing the token the parser
 * could not cope with.
 * @param acceptableTokens A set of token names that would have allowed the
 * parser to continue in its current state.
 * @return An action to take to recover from the parse error or nil.  If the
 * action is nil, and the problematic token is a CPErrorToken the parse stack is
 * unwound a step for the parent rule to deal with the error.
 */
 ...
 *
 * Parsers are built by constructing a grammar, and then using it to create a
 * parser.  The parser delegate may be used to monitor and replace output from
 * the parser.
 *
 * @warning Note that CPParser is an abstract superclass.  Use one of its
 * subclasses to construct your parser.
 */
 ...
 * @return Returns a parser that parses the input grammar, or nil if no such
 * parser could be created.
 */
 ...
 *
 * Currently if errors are generated, `nil` is returned and the error Logged
 * using NSLog.  This behaviour may change in the future to return the error in
 * a more usable form.
 *
 * @param tokenStream The token stream to parse.
 * @return Returns the parsed syntax tree for the whole stream or `nil` if the
 * token stream could not be parsed.
 */
