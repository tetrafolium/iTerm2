...
- (CPShiftReduceAction *)actionForState:(NSUInteger)state
                                  token:(CPToken *)token;
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  self = [super
      initWithGrammar:[aDecoder
                          decodeObjectForKey:CPShiftReduceParserGrammarKey]];

  if (nil != self) {
    [self setActionTable:
              [aDecoder decodeObjectForKey:CPShiftReduceParserActionTableKey]];
    [self setGotoTable:[aDecoder
                           decodeObjectForKey:CPShiftReduceParserGotoTableKey]];
  }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  [aCoder encodeObject:[self actionTable]
                forKey:CPShiftReduceParserActionTableKey];
...
- (BOOL)constructShiftReduceTables {
  NSLog(@"CPShiftReduceParser is abstract, use one of it's concrete subclasses "
        @"instead");
  return NO;
}
...
- (id)parse:(CPTokenStream *)tokenStream {
  @try {
    NSMutableArray *stateStack = [NSMutableArray
        arrayWithObject:[CPShiftReduceState shiftReduceStateWithObject:nil
                                                                 state:0]];
    CPToken *nextToken = [[tokenStream peekToken] retain];
    BOOL hasErrorToken = NO;
    while (1) {
      @autoreleasepool {
        CPShiftReduceAction *action = [self
            actionForState:[(CPShiftReduceState *)[stateStack lastObject] state]
                     token:nextToken];

        if ([action isShiftAction]) {
          [stateStack
              addObject:[CPShiftReduceState
                            shiftReduceStateWithObject:nextToken
          ...
          NSUInteger numElements =
              [[reductionRule rightHandSideElements] count];
          NSMutableArray *components =
              [NSMutableArray arrayWithCapacity:numElements];
          NSRange stateStackRange =
              NSMakeRange([stateStack count] - numElements, numElements);
          NSMutableDictionary *tagValues = [NSMutableDictionary dictionary];
          [stateStack
              enumerateObjectsAtIndexes:
                  [NSIndexSet indexSetWithIndexesInRange:stateStackRange]
                                options:NSEnumerationReverse
                             usingBlock:^(CPShiftReduceState *state,
                               ...
                               if ([o isRHSItemResult]) {
                                 ...
                                 if ([o shouldCollapse]) {
                                   ...
                                   if ([r tagNames] != nil &&
                                     ...
                                     for (NSString *tagName in [r tagNames]) {
                                       [tagValues
                                           setObject:[comps objectAtIndex:0]
                                              forKey:tagName];
                                     }
                                   }
                                 } else {
                                   [components insertObject:[r contents]
                                                    atIndex:0];
                                   if ([r tagNames] != nil) {
                                     for (NSString *tagName in [r tagNames]) {
                                       [tagValues setObject:[r contents]
                                                     forKey:tagName];
                                     }
                                   }
                                 }
                                 ...
                                 [tagValues
                                     addEntriesFromDictionary:[r tagValues]];
                               } else {
          ...
          CPSyntaxTree *tree = [CPSyntaxTree syntaxTreeWithRule:reductionRule
                                                       children:components
                                                      tagValues:tagValues];
          id result = nil;
          ...
          NSUInteger newState = [self
              gotoForState:[(CPShiftReduceState *)[stateStack lastObject] state]
                      rule:reductionRule];
          [stateStack addObject:[CPShiftReduceState
                                    shiftReduceStateWithObject:result
                                                         state:newState]];
        } else if ([action isAccept]) {
          [nextToken release];
          return [(CPShiftReduceState *)[stateStack lastObject] object];
        } else {
          CPRecoveryAction *recoveryAction = [self
                  error:tokenStream
              expecting:[self
                            acceptableTokenNamesForState:[(CPShiftReduceState *)
                                                                 [stateStack
                                                                     lastObject]
                                                             state]]];
...
- (CPRecoveryAction *)error:(CPTokenStream *)tokenStream
  ...
  if (delegateRespondsTo.didEncounterErrorOnInputExpecting) {
    return [[self delegate] parser:self
          didEncounterErrorOnInput:tokenStream
                         expecting:acceptableTokens];
  } else if (delegateRespondsTo.didEncounterErrorOnInput) {
    return [[self delegate] performSelector:@selector(parser:
                                                didEncounterErrorOnInput:)
                                 withObject:self
                                 withObject:tokenStream];
    //        return [[self delegate] parser:self
    //        didEncounterErrorOnInput:tokenStream];
  } else {
    CPToken *t = [tokenStream peekToken];
    NSLog(@"%ld:%ld: parse error.  Expected %@, found %@",
          (long)[t lineNumber] + 1, (long)[t columnNumber] + 1,
          acceptableTokens, t);
    return nil;
  }
}
