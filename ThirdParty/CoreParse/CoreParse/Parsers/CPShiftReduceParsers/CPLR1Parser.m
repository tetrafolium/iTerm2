...
- (BOOL)constructShiftReduceTables {
  ...
  for (NSSet *kernel in kernels) {
    ...
    for (CPLR1Item *item in itemsSet) {
      ...
      if (nil == next) {
        if ([[[item rule] name] isEqualToString:startSymbol]) {
          BOOL success =
              [[self actionTable] setAction:[CPShiftReduceAction acceptAction]
                                   forState:idx
                                       name:@"EOF"];
          if (!success) {
            return NO;
        ...
        BOOL success =
            [[self actionTable] setAction:[CPShiftReduceAction shiftAction:ix]
                                 forState:idx
                                     name:[next name]];
        if (!success) {
          return NO;
    ...
    for (NSString *nonTerminalName in allNonTerminalNames) {
      NSSet *g =
          [aug lr1GotoKernelWithItems:itemsSet
...
- (NSArray *)kernelsForGrammar:(CPGrammar *)aug {
  CPRule *startRule =
      [[aug rulesForNonTerminalWithName:[aug start]] objectAtIndex:0];
  NSSet *initialKernel = [NSSet
      setWithObject:[CPLR1Item lr1ItemWithRule:startRule
                                      position:0
                                      terminal:[CPGrammarSymbol
                                                   terminalWithName:@"EOF"]]];
