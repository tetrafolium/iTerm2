...
- (BOOL)constructShiftReduceTables {
  ...
  [self setActionTable:[[[CPShiftReduceActionTable alloc]
                           initWithCapacity:itemCount] autorelease]];
  [self setGotoTable:[[[CPShiftReduceGotoTable alloc]
                         initWithCapacity:itemCount] autorelease]];
  ...
  for (NSSet *kernel in kernels) {
    @autoreleasepool {
        ...
        @autoreleasepool {
          ...
          if (nil == next) {
            if ([[[item rule] name] isEqualToString:startSymbol]) {
              BOOL success = [[self actionTable]
                  setAction:[CPShiftReduceAction acceptAction]
                   forState:idx
                       name:@"EOF"];
              if (!success) {
                NSLog(@"Could not insert shift in action table for state %lu, "
                      @"token %@",
                      (unsigned long)idx, @"EOF");
                return NO;
              }
            } else {
              BOOL success = [[self actionTable]
                  setAction:[CPShiftReduceAction reduceAction:[item rule]]
                   forState:idx
                       name:[[item terminal] name]];
              if (!success) {
                NSLog(@"Could not insert reduce in action table for state %lu, "
                      @"token %@",
                      (unsigned long)idx, [[item terminal] name]);
                return NO;
              }
            }
            ...
            BOOL success = [[self actionTable]
                setAction:[CPShiftReduceAction shiftAction:ix]
                 forState:idx
                     name:[next name]];
            if (!success) {
              NSLog(@"Could not insert shift in action table for state %lu, "
                    @"token %@",
                    (unsigned long)idx, [next name]);
              return NO;
            }
          }
        }
      ...
      for (NSString *nonTerminalName in allNonTerminalNames) {
        NSSet *g = [aug
            lr0GotoKernelWithItems:itemsSet
                            symbol:[CPGrammarSymbol
                                       nonTerminalWithName:nonTerminalName]];
        NSSet *lr0G = [g cp_map:^id(CPLR1Item *i) {
          return [CPItem itemWithRule:[i rule] position:[i position]];
        }];
        NSUInteger indx = 0;
        ...
        BOOL success = [[self gotoTable] setGoto:gotoIndex
                                        forState:idx
                                nonTerminalNamed:nonTerminalName];
        if (!success) {
          NSLog(@"Could not insert into goto table for state %lu, token %@",
                (unsigned long)idx, nonTerminalName);
          return NO;
        }
      }
    }
...
- (NSArray *)kernelsForGrammar:(CPGrammar *)aug {
  ...
  NSMutableArray *lr1Kernels =
      [NSMutableArray arrayWithCapacity:[lr0Kernels count]];
  ...
  for (NSSet *kernel in lr0Kernels) {
    for (CPItem *item in kernel) {
      if ([[[item rule] name] isEqualToString:grammarStartSymbol] &&
          0 == [item position]) {
        NSMutableDictionary *gotoSpontaneous =
            [spontaneous objectForKey:kernel];
      ...
      NSSet *j = [aug
          lr1Closure:
              [NSSet setWithObject:[CPLR1Item
                                       lr1ItemWithRule:[item rule]
                                              position:[item position]
                                              terminal:[CPGrammarSymbol
                                                           terminalWithName:
                                                               uniqueName]]]];
      for (CPLR1Item *lr1Item in j) {
        ...
        if (nil != nextSymbol) {
          NSSet *g = [aug lr0GotoKernelWithItems:[aug lr0Closure:kernel]
                                          symbol:nextSymbol];
          if ([uniqueName isEqualToString:name]) {
            NSMutableDictionary *fromKernelPropogations =
                [propogations objectForKey:kernel];
            ...
            NSMutableDictionary *itemPropogations =
                [fromKernelPropogations objectForKey:item];
            ...
            [gotoPropogations
                addObject:[CPItem itemWithRule:[lr1Item rule]
                                      position:[lr1Item position] + 1]];
          } else {
            CPItem *lr0Item = [CPItem itemWithRule:[lr1Item rule]
                                          position:[lr1Item position] + 1];
            NSMutableDictionary *gotoSpontaneous = [spontaneous objectForKey:g];
            NSMutableSet *itemSpontaneous =
                [gotoSpontaneous objectForKey:lr0Item];
            if (nil == itemSpontaneous) {
              itemSpontaneous = [NSMutableSet set];
  ...
  while (!finishedPropogating) {
    ...
    for (NSSet *fromKernel in propogations) {
      NSDictionary *fromKernelPropogations =
          [propogations objectForKey:fromKernel];
      for (CPItem *from in fromKernelPropogations) {
        NSDictionary *itemPropogatiotns =
            [fromKernelPropogations objectForKey:from];
        for (NSSet *toKernel in itemPropogatiotns) {
          NSDictionary *gotoPropogations =
              [itemPropogatiotns objectForKey:toKernel];
          for (CPItem *toItem in gotoPropogations) {
            NSDictionary *gotoSpontaneous =
                [spontaneous objectForKey:fromKernel];
            NSSet *symbols = [gotoSpontaneous objectForKey:from];
            for (CPGrammarSymbol *symbol in symbols) {
              NSMutableDictionary *toGotoSpontaneous =
                  [spontaneous objectForKey:toKernel];
              ...
              NSMutableSet *toSpontaneous =
                  [toGotoSpontaneous objectForKey:toItem];
              if (nil == toSpontaneous) {
                [toGotoSpontaneous setObject:[NSMutableSet setWithObject:symbol]
                                      forKey:toItem];
  ...
  for (NSSet *kernel in lr0Kernels) {
    ...
    for (CPItem *item in kernel) {
      ...
      for (CPGrammarSymbol *symbol in symbols) {
        if (nil != [symbol name]) {
          [lr1Kernel addObject:[CPLR1Item lr1ItemWithRule:[item rule]
                                                 position:[item position]
                                                 terminal:symbol]];
        }
      }
