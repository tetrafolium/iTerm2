...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (nil != self) {
    NSArray *rows =
        [aDecoder decodeObjectForKey:CPShiftReduceActionTableTableKey];
    capacity = [rows count];
    table = malloc(capacity * sizeof(NSMutableDictionary *));
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:[NSArray arrayWithObjects:table count:capacity]
                forKey:CPShiftReduceActionTableTableKey];
}
...
- (BOOL)setAction:(CPShiftReduceAction *)action
         forState:(NSUInteger)state
             name:(NSString *)token {
  NSMutableDictionary *row = table[state];
  if (nil != [row objectForKey:token] &&
      ![[row objectForKey:token] isEqualToShiftReduceAction:action]) {
    return NO;
  }
