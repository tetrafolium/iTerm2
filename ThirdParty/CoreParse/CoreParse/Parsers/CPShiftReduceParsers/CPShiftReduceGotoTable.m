...
- (id)initWithCapacity:(NSUInteger)initCapacity {
  ...
  if (nil != self) {
    ...
    for (NSUInteger buildingState = 0; buildingState < capacity;
         buildingState++) {
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (nil != self) {
    NSArray *rows =
        [aDecoder decodeObjectForKey:CPShiftReduceGotoTableTableKey];
    capacity = [rows count];
    table = malloc(capacity * sizeof(NSMutableDictionary *));
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:[NSArray arrayWithObjects:table count:capacity]
                forKey:CPShiftReduceGotoTableTableKey];
}
...
- (BOOL)setGoto:(NSUInteger)gotoIndex
  ...
  if (nil != [row objectForKey:nonTerminalName] &&
      [[row objectForKey:nonTerminalName] unsignedIntegerValue] != gotoIndex) {
    return NO;
  }
  [row setObject:[NSNumber numberWithUnsignedInteger:gotoIndex]
          forKey:nonTerminalName];
  return YES;
}

- (NSUInteger)gotoForState:(NSUInteger)state rule:(CPRule *)rule {
  return [(NSNumber *)[table[state] objectForKey:[rule name]]
