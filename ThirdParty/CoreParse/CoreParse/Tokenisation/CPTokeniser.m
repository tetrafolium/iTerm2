...
- (void)advanceLineNumber:(NSUInteger *)ln
             columnNumber:(NSUInteger *)cn
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (nil != self) {
    [self setTokenRecognisers:
              [aDecoder decodeObjectForKey:CPTokeniserTokenRecognisersKey]];
  }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:[self tokenRecognisers]
                forKey:CPTokeniserTokenRecognisersKey];
}
...
- (void)insertTokenRecogniser:(id<CPTokenRecogniser>)recogniser
  ...
  if (NSNotFound == idx) {
    [NSException raise:NSInvalidArgumentException
                format:@"Token recogniser to insert before was not found"];
  }
  ...
  while (currentTokenOffset < inputLength) {
    @autoreleasepool {
      ...
      for (id<CPTokenRecogniser> recogniser in recs) {
        ...
        CPToken *tok = [recogniser recogniseTokenInString:input
        ...
        if (nil != tok) {
          ...
          if (delegateRespondsTo.shouldConsumeToken) {
            if ([delegate tokeniser:self shouldConsumeToken:tok]) {
              ...
              [self advanceLineNumber:&currentLineNumber
                         columnNumber:&currentColumnNumber
                            withInput:input
                                range:CFRangeMake(lastTokenOffset,
                                                  currentTokenOffset -
                                                      lastTokenOffset)];
              recognised = YES;
              break;
      ...
      if (!recognised) {
        if (delegateRespondsTo.didNotFindTokenOnInputPositionError) {
          NSString *err = nil;
          currentTokenOffset = [delegate tokeniser:self
                            didNotFindTokenOnInput:input
                                          position:currentTokenOffset
                                             error:&err];
          ...
          CPErrorToken *t = [CPErrorToken
              errorWithMessage:
                  [NSString stringWithFormat:
                                @"The tokeniser encountered an invalid input "
                                @"\"%@\", and could not handle it.  Implement "
                                @"-tokeniser:didNotFindTokenAtInputPosition:"
                                @"error: to make this do something more useful",
                                [input substringWithRange:
                                           NSMakeRange(
                                               currentTokenOffset,
                                               MIN((NSUInteger)10,
                                                   [input length] -
...
- (void)addToken:(CPToken *)tok toStream:(CPTokenStream *)stream {
  if (delegateRespondsTo.requestsPush) {
    ...
    [stream pushTokens:[delegate performSelector:@selector(tokeniser:
                                                     willProduceToken:)
                                      withObject:self
                                      withObject:tok]];
    //        [stream pushTokens:[delegate tokeniser:self
    //        willProduceToken:tok]];
  } else {
...
- (void)advanceLineNumber:(NSUInteger *)ln
  ...
  dispatch_once(&onceToken, ^{
    newlineCharset = (CFCharacterSetRef)[
  ...
  CFRange foundRange;
  BOOL found = CFStringFindCharacterFromSet((CFStringRef)input, newlineCharset,
                                            searchRange, 0L, &foundRange);
  NSUInteger lastNewLineLocation = NSNotFound;
  while (found) {
    *ln += foundRange.length;
    lastNewLineLocation = foundRange.location + foundRange.length;
    searchRange =
        CFRangeMake(lastNewLineLocation, rangeEnd - lastNewLineLocation);
    found = CFStringFindCharacterFromSet((CFStringRef)input, newlineCharset,
                                         searchRange, 0L, &foundRange);
  }
...
- (void)setDelegate:(id<CPTokeniserDelegate>)aDelegate {
  if (delegate != aDelegate) {
    ...
    delegateRespondsTo.shouldConsumeToken =
        [delegate respondsToSelector:@selector(tokeniser:shouldConsumeToken:)];
    delegateRespondsTo.requestsPush =
        [delegate respondsToSelector:@selector(tokeniser:
                                           requestsToken:pushedOntoStream:)];
    delegateRespondsTo.willProduceToken =
        [delegate respondsToSelector:@selector(tokeniser:willProduceToken:)];
    delegateRespondsTo.didNotFindTokenOnInputPositionError = [delegate
        respondsToSelector:@selector(tokeniser:
                               didNotFindTokenOnInput:position:error:)];
    delegateRespondsTo.willFinish =
        [delegate respondsToSelector:@selector(tokeniserWillFinish:stream:)];
  }
}
