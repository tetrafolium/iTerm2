 ...
 * inform parsers that the stream is complete.
 */
 ...
 *
 * This method will not block waiting for a token to become available.  If no
 * token is available the method returns `nil`.  If the token stream is closed
 ...
 *
 * This method will block waiting for a token to become available if the token
 * stream is empty.  If the token stream is closed the method returns `nil`.
 *
 * @return The first CPToken in the stream.
 * @see peekToken
 */
...
/**
 * Closes the token stream, causing popToken to return `nil` when all tokens
 * have been exhausted rather than blocking waiting for new input.
 */
