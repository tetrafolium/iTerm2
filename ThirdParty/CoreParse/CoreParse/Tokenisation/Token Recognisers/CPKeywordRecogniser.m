...
+ (id)recogniserForKeyword:(NSString *)keyword
  ...
  return
      [[[self alloc] initWithKeyword:keyword
          invalidFollowingCharacters:invalidFollowingCharacters] autorelease];
}

- (id)initWithKeyword:(NSString *)initKeyword
    invalidFollowingCharacters:
        (NSCharacterSet *)initInvalidFollowingCharacters {
  self = [super init];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (nil != self) {
    [self
        setKeyword:[aDecoder decodeObjectForKey:CPKeywordRecogniserKeywordKey]];
    [self setInvalidFollowingCharacters:
              [aDecoder decodeObjectForKey:
                            CPKeywordRecogniserInvalidFollowingCharactersKey]];
  }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  [aCoder encodeObject:[self invalidFollowingCharacters]
                forKey:CPKeywordRecogniserInvalidFollowingCharactersKey];
}
...
- (CPToken *)recogniseTokenInString:(NSString *)tokenString
  ...
  if (remainingChars >= kwLength) {
    if (CFStringFindWithOptions((CFStringRef)tokenString, (CFStringRef)keyword,
                                CFRangeMake(*tokenPosition, kwLength),
                                kCFCompareAnchored, NULL)) {
      if (remainingChars == kwLength || nil == invalidFollowingCharacters ||
          !CFStringFindCharacterFromSet(
              (CFStringRef)tokenString,
              (CFCharacterSetRef)invalidFollowingCharacters,
              CFRangeMake(*tokenPosition + kwLength, 1), kCFCompareAnchored,
              NULL)) {
