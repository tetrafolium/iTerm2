...
- (id)initWithRegexp:(NSRegularExpression *)initRegexp
        matchHandler:(CPRegexpKeywordRecogniserMatchHandler)initMatchHandler {
  self = [super init];
  if (self) {
...
+ (id)recogniserForRegexp:(NSRegularExpression *)regexp
             matchHandler:
                 (CPRegexpKeywordRecogniserMatchHandler)initMatchHandler {
  return [[[self alloc] initWithRegexp:regexp
                          matchHandler:initMatchHandler] autorelease];
}
...
- (CPToken *)recogniseTokenInString:(NSString *)tokenString
  ...
  NSRange searchRange =
      NSMakeRange(*tokenPosition, inputLength - *tokenPosition);
  NSTextCheckingResult *result =
      [[self regexp] firstMatchInString:tokenString
                                options:NSMatchingAnchored
                                  range:searchRange];
  if (nil != result && nil != matchHandler &&
      result.range.location == *tokenPosition && result.range.length > 0) {
