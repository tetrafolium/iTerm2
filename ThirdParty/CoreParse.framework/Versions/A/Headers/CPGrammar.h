...
/**
 * The CPGrammar class represents a context free grammar.  Grammars can be used
 * later to construct parsers.
 */
 ...
 *
 * see initWithStart:backusNaurForm: for a description of the syntax used for
 * BNF.
 *
 * @param start The non-terminal that all parses must reduce to.
 ...
 * -grammarWithStart:backusNaurForm:error: instead.
 * @see grammarWithStart:backusNaurForm:error:
 */
+ (id)grammarWithStart:(NSString *)start
        backusNaurForm:(NSString *)bnf __attribute__((deprecated(
                           "will simply print any errors that occur, use "
                           "+grammarWithStart:backusNaurForm:error: instead")));
 ...
 * @param start The non-terminal that all parses must reduce to.
 * @param bnf   BNF for the grammar.
 * @param error A pointer to an error object which will be filled if the method
 * returns nil.
 * @return Returns a CPGrammar based on the BNF and starting non-terminal.
 *
...
/**
 * Initialises a grammar based on a starting non-terminal and some backus naur
 * form.
 *
 * The BNF is expressed using rules in the form `nonTerminal ::=
 * <subNonTerminal> "subTerminal" <subNonTerminal>;`.  Rules may optionally be
 * prefixed with a number indicating their tag. This allows you to quickly
 * construct grammars in a readable form. You may also use EBNF to construct
 * grammars.  This allows you to use the symbols "*", "+", and "?" to indicate
 * that a construction may appear 0 or more; 1 or more; and 0 or 1 times
 * respectively. You may also parenthesise subrules.
 *
 * When you use any of the above EBNF constructs or parentheses, the parser will
 * return the contents in an NSArray.
 *
 * You may use tags to identify sections of rules to be extracted in your result
 * classes or parser delegate.  To do this, use the syntax `nonTerminal ::=
 * foo@<subNonTerminal> bar@"subTerminal";`.  You may use multiple tags to
 * identify the same section of a rule.  For example `range ::= min@'Number' '-'
 * max@'Number' | min@'Number' '-' | '-' max@'Number' | min@max@'Number'`.
 *
 ...
 *
 * @bug Warning this method is deprecated, use
 * -initWithStart:backusNaurForm:error: instead.
 * @see initWithStart:backusNaurForm:error:
 */
- (id)initWithStart:(NSString *)start
     backusNaurForm:(NSString *)bnf
    __attribute__((deprecated("will simply print any errors that occur, use "
                              "-initWithStart:backusNaurForm:error: instead")));
