...
static int move_to_user_namespace__100600(void) {
  FIND_SYMBOL(_vprocmgr_move_subset_to_user, void *,
              (uid_t, const char *, uint64_t))
...
static int move_to_user_namespace__101000(void) {
  ...
  FIND_SYMBOL(bootstrap_look_up_per_user, kern_return_t,
              (mach_port_t, const char *, uid_t, mach_port_t *))

  if (f_bootstrap_get_root(bootstrap_port, &rootbs) != KERN_SUCCESS) {
    warn("%s failed", fn_bootstrap_get_root);
    return -1;
  }
  if (f_bootstrap_look_up_per_user(rootbs, NULL, getuid(), &puc) !=
      KERN_SUCCESS) {
