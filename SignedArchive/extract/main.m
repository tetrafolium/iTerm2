...
static NSError *Extract(NSString *source, NSString *destination) {
  ...
  if (!destinationURL) {
    return [SIGError errorWithCode:SIGErrorCodeIOWrite
                            detail:@"Destination filename malformed."];
  }
  SIGArchiveVerifier *verifier =
      [[SIGArchiveVerifier alloc] initWithURL:[NSURL fileURLWithPath:source]];
  __block BOOL result;
  __block NSError *errorResult = nil;
  dispatch_group_t group = dispatch_group_create();
  dispatch_group_enter(group);
  [verifier verifyAndWritePayloadToURL:destinationURL
