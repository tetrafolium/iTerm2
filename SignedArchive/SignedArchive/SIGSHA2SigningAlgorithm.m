...
- (NSData *)signatureForInputStream:(NSInputStream *)readStream
  ...
  _readTransform = SecTransformCreateReadTransformWithReadStream(
      (__bridge CFReadStreamRef)readStream);
  if (!_readTransform) {
    if (error) {
  ...
  _dataDigestTransform = SecDigestTransformCreate(kSecDigestSHA2, 256, &err);
  if (!_dataDigestTransform) {
    if (error) {
      *error =
          [SIGError errorWrapping:(__bridge NSError *)err
                             code:SIGErrorCodeAlgorithmCreationFailed
  ...
  _dataSignTransform = SecSignTransformCreate(privateKey.secKey, &err);
  if (!_dataSignTransform) {
    if (error) {
      *error =
          [SIGError errorWrapping:(__bridge NSError *)err
  ...
  SecTransformSetAttribute(_dataSignTransform, kSecInputIsAttributeName,
                           kSecInputIsDigest, &err);
  if (err) {
    if (error) {
      *error = [SIGError errorWrapping:(__bridge NSError *)err
                                  code:SIGErrorCodeAlgorithmCreationFailed
  ...
  SecTransformSetAttribute(_dataSignTransform, kSecDigestTypeAttribute,
                           kSecDigestSHA2, &err);
  if (err) {
    if (error) {
      *error = [SIGError errorWrapping:(__bridge NSError *)err
  ...
  SecTransformSetAttribute(_dataSignTransform, kSecDigestLengthAttribute,
                           (__bridge CFTypeRef _Nonnull) @256, &err);
  if (err) {
    if (error) {
      *error =
          [SIGError errorWrapping:(__bridge NSError *)err
  ...
  SecTransformConnectTransforms(
      _readTransform, kSecTransformOutputAttributeName, _dataDigestTransform,
      kSecTransformInputAttributeName, _group, &err);
  if (err != nil) {
    if (error) {
      *error = [SIGError
          errorWrapping:(__bridge NSError *)err
                   code:SIGErrorCodeAlgorithmCreationFailed
  ...
  SecTransformConnectTransforms(
      ...
      _dataSignTransform, kSecTransformInputAttributeName, _group, &err);
  if (err != nil) {
    if (error) {
      *error = [SIGError
          errorWrapping:(__bridge NSError *)err
  ...
  NSData *signature =
      (__bridge_transfer NSData *)SecTransformExecute(_group, &err);
  if (err != nil) {
    if (error) {
      *error =
          [SIGError errorWrapping:(__bridge NSError *)err
