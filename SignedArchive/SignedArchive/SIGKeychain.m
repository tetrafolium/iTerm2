...
+ (NSString *)path {
  NSString *const keychainsDirectory =
      [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Keychains"];
  NSString *const keychainName = @"login";
  NSString *const fullPath =
      [keychainsDirectory stringByAppendingPathComponent:keychainName];
  return fullPath;
}
...
- (instancetype)initPrivate {
  self = [super init];
  if (self) {
    OSErr err = SecKeychainOpen([SIGKeychain.path UTF8String], &_secKeychain);
    if (err != noErr) {
