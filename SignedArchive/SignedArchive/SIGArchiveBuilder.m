...
- (BOOL)writeToURL:(NSURL *)url
  ...
  if (!combinedOutputStream) {
    if (error) {
      *error = [SIGError errorWithCode:SIGErrorCodeIOWrite
                                detail:@"Could not create write stream"];
    }
    return NO;
  }
...
#if ENABLE_SIGARCHIVE_MIGRATION_CREATION
  if (![self writeSignature:signature
                   toStream:combinedOutputStream
                      error:error]) {
    return NO;
  }
#endif
  if (![self writeCertificate:certificate
                     toStream:combinedOutputStream
                        error:error]) {
    return NO;
  }

  // NOTE: The signing certificate must be first. This is a requirement of
  // SecTrustCreateWithCertificates which is implicit in the file format.
  SIGCertificate *issuerCertificate = _identity.signingCertificate.issuer;
  while (issuerCertificate != nil) {
    if (![self writeCertificate:issuerCertificate.data
                       toStream:combinedOutputStream
                          error:error]) {
      return NO;
    }
  ...
  NSData *combinedData = [combinedOutputStream
  ...
  NSOutputStream *writeStream = [NSOutputStream outputStreamWithURL:url
                                                             append:NO];
  [writeStream open];
  const long long length = [writeStream write:combinedData.bytes
                                    maxLength:combinedData.length];
...
- (BOOL)writeHeaderToStream:(NSOutputStream *)stream
                      error:(out NSError *_Nullable __autoreleasing *)error {
  NSData *data =
      [SIGArchiveHeaderMagicString dataUsingEncoding:NSUTF8StringEncoding];
  const NSInteger desiredLength = data.length;
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagHeader
                                          length:desiredLength
                                          offset:_offset];
  const BOOL ok = [chunkWriter writeData:data toStream:stream error:error];
  _offset += chunkWriter.chunkLength;
  return ok;
}

- (BOOL)writePayloadToStream:(NSOutputStream *)writeStream
                       error:
                           (out NSError *_Nullable __autoreleasing *)errorOut {
  NSInputStream *readStream =
      [[NSInputStream alloc] initWithURL:_payloadFileURL];
  if (!readStream) {
    return NO;
  }
  [readStream open];
  ...
  NSError *error = nil;
  NSInteger length = [[NSFileManager defaultManager]
                         attributesOfItemAtPath:_payloadFileURL.path
                                          error:&error]
                         .fileSize;
  ...
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagPayload
                                          length:length
                                          offset:_offset];
...
- (NSString *)keyValuePairsFromDictionary:
...
- (NSString *)producedVersion {
  // Version 1 signed only the payload. Version 2 signs the container except the
  // signature's chunk.
#if ENABLE_SIGARCHIVE_MIGRATION_CREATION
  return @"1";
#else
...
- (BOOL)writeMetadataToStream:(NSOutputStream *)writeStream
                        error:(out NSError *_Nullable __autoreleasing *)error {
  NSDictionary *const fieldDict = @{
    SIGArchiveMetadataKeyVersion : [self producedVersion],
  ...
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagMetadata
                                          length:data.length
                                          offset:_offset];
  const BOOL ok = [chunkWriter writeData:data toStream:writeStream error:error];
  _offset += chunkWriter.chunkLength;
  return ok;
}

#if ENABLE_SIGARCHIVE_MIGRATION_CREATION
- (BOOL)writeSignature:(NSData *)signature
              toStream:(NSOutputStream *)writeStream
                 error:(out NSError *_Nullable __autoreleasing *)error {
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagSignature
...
- (BOOL)writeSignature2:(NSData *)signature
               toStream:(NSOutputStream *)writeStream
                  error:(out NSError *_Nullable __autoreleasing *)error {
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagSignature2
                                          length:signature.length
                                          offset:_offset];
  const BOOL ok = [chunkWriter writeData:signature
                                toStream:writeStream
...
- (BOOL)writeCertificate:(NSData *)certificate
                toStream:(NSOutputStream *)writeStream
                   error:(out NSError *_Nullable __autoreleasing *)error {
  SIGArchiveChunkWriter *chunkWriter =
      [[SIGArchiveChunkWriter alloc] initWithTag:SIGArchiveTagCertificate
...
- (id<SIGSigningAlgorithm>)signingAlgorithm:(out NSError **)error {
  id<SIGSigningAlgorithm> algorithm =
      [[[self signingAlgorithmClass] alloc] init];
  if (!algorithm) {
    if (error) {
...
- (NSData *)signature:(out NSError **)error {
  ...
  NSInputStream *readStream =
      [NSInputStream inputStreamWithFileAtPath:_payloadFileURL.path];
  if (!readStream) {
    if (error) {
