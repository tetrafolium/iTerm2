...
- (void)verifyAndWritePayloadToURL:(NSURL *)url
  ...
  [self verifyWithCompletion:^(BOOL ok, NSError *_Nullable error) {
    ...
    const BOOL copiedOK = [self copyPayloadToURL:url error:&copyError];
    completion(copiedOK, copyError);
  }];
}

- (BOOL)copyPayloadToURL:(NSURL *)url error:(out NSError **)errorOut {
  if (!_verified) {
    if (errorOut) {
      *errorOut =
          [SIGError errorWithCode:SIGErrorCodeConsistency
                           detail:@"Application error: archive not verified"];
    }
    return NO;
  }
  NSError *error = nil;
  ...
  NSOutputStream *writeStream = [[NSOutputStream alloc] initWithURL:url
                                                             append:NO];
  if (!writeStream) {
    if (errorOut) {
  ...
  while ([readStream hasBytesAvailable]) {
    ...
    const NSInteger numberOfBytesWritten =
        [writeStream write:buffer maxLength:numberOfBytesRead];
    if (numberOfBytesWritten != numberOfBytesRead) {
      if (errorOut) {
        *errorOut = [SIGError errorWrapping:writeStream.streamError
...
- (NSDictionary<NSString *, NSString *> *)
    metadataDictionaryFromString:(NSString *)metadata
  ...
  for (NSString *row in rows) {
    ...
    NSString *value =
        [row substringFromIndex:SIGAddNonnegativeInt64s(index, 1)];
...
- (BOOL)verifyMetadata:(NSString *)metadata error:(out NSError **)error {
  ...
  NSArray<NSString *> *const supportedDigestTypes =
      SIGVerificationDigestAlgorithmNames();
  if (![supportedDigestTypes containsObject:digestType]) {
    if (error) {
      NSString *detail = [NSString
          stringWithFormat:@"Algorithm \"%@\" not supported", digestType];
      *error = [SIGError errorWithCode:SIGErrorCodeUnsupportedAlgorithm
                                detail:detail];
    }
    return NO;
  }
...
- (BOOL)prepareToVerify:(out NSError **)error {
  ...
  for (NSData *certificateData in certificateDatas) {
    SIGCertificate *certificate =
        [[SIGCertificate alloc] initWithData:certificateData];
    if (!certificate) {
      if (error) {
...
- (id<SIGVerificationAlgorithm>)verificationAlgorithm:(out NSError **)error {
  id<SIGVerificationAlgorithm> algorithm =
      [[SIGSHA2VerificationAlgorithm alloc] init];
  if (!algorithm) {
    if (error) {
...
- (BOOL)verify:(out NSError **)error {
  ...
  if (_signature2Data) {
    return
        [algorithm verifyInputStream:_payload2InputStream
...
- (void)createReader {
  ...
  if (!_reader) {
    _readerLoadError =
        [SIGError errorWithCode:SIGErrorCodeUnknown
