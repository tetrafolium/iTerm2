...
- (NSString *)name {
  CFStringRef value;
  const OSStatus status = SecCertificateCopyCommonName(_secCertificate, &value);
...
- (NSString *)longDescription {
  ...
  NSString *value =
      (__bridge_transfer NSString *)SecCertificateCopyLongDescription(
          NULL, _secCertificate, &error);
...
- (NSData *)serialNumber {
  ...
  if (@available(macOS 10.13, *)) {
    value = (__bridge_transfer NSData *)SecCertificateCopySerialNumberData(
        _secCertificate, &error);
  } else {
    value = (__bridge_transfer NSData *)SecCertificateCopySerialNumber(
        _secCertificate, &error);
  }
#else
  value = (__bridge_transfer NSData *)SecCertificateCopySerialNumberData(
...
+ (BOOL)certificateInArray:(CFArrayRef)array
                   atIndex:(NSInteger)i
                   hasName:(CFDataRef)name {
  SecCertificateRef secCertificate =
      (SecCertificateRef)CFArrayGetValueAtIndex(array, i);

  CFDataRef subjectContent;
#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_12_4
  if (@available(macOS 10.12.4, *)) {
    subjectContent =
        SecCertificateCopyNormalizedSubjectSequence(secCertificate);
  } else {
    subjectContent =
        SecCertificateCopyNormalizedSubjectContent(secCertificate, NULL);
  }
#else
...
- (SIGCertificate *)issuer {
  ...
  CFDataRef issuerName =
      SecCertificateCopyNormalizedIssuerSequence(_secCertificate);
  if (!issuerName) {
    return nil;
  }
  SecCertificateRef secCertificate =
      [SIGCertificate secCertificateWithName:issuerName];
