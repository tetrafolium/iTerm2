...
+ (instancetype)
    chunkFromFileHandle:(NSFileHandle *)fileHandle
               atOffset:(long long)offset
                  error:
                      (out NSError *_Nullable __autoreleasing *_Nullable)error {
  @try {
  ...
  SIGArchiveTag tag;
  if (![self readNonnegativeIntegerFromFileHandle:fileHandle
                                       fromOffset:offset
                                            value:(long long *)&tag
                                            error:error]) {
    return nil;
  ...
  long long length;
  if (![self readNonnegativeIntegerFromFileHandle:fileHandle
                                       fromOffset:SIGAddNonnegativeInt64s(
                                                      offset, sizeof(long long))
                                            value:(long long *)&length
                                            error:error]) {
    return nil;
  }

  SIGArchiveChunk *chunk = [[SIGArchiveChunk alloc]
      initWithTag:tag
           length:length
...
+ (NSData *)
    readDataFromFileHandle:(NSFileHandle *)fileHandle
            expectedLength:(long long)expectedLength
                     error:(out NSError *_Nullable __autoreleasing *_Nullable)
                               error {
...
+ (BOOL)
    readNonnegativeIntegerFromFileHandle:(NSFileHandle *)fileHandle
                              fromOffset:(NSInteger)offset
...
- (BOOL)writeData:(NSData *)data
  ...
  const long long length = [stream write:data.bytes maxLength:data.length];
...
- (BOOL)writeStream:(NSInputStream *)readStream
  ...
  if (![self writeInteger:self.payloadLength
  ...
  while (readStream.hasBytesAvailable) {
    ...
    totalNumberOfBytes =
        SIGAddNonnegativeInt64s(totalNumberOfBytes, numberOfBytesRead);
    ...
    NSInteger numberOfBytesWritten = [writeStream write:buffer
                                              maxLength:numberOfBytesRead];
...
- (BOOL)writeInteger:(long long)integer
  ...
  const NSInteger length = [stream write:buffer maxLength:sizeof(buffer)];
  if (error) {
    if (length < 0) {
