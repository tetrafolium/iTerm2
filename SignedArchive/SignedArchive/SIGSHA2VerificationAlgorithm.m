...
- (BOOL)verifyInputStream:(NSInputStream *)payloadInputStream
  ...
  _dataReadTransform = SecTransformCreateReadTransformWithReadStream(
      (__bridge CFReadStreamRef)payloadInputStream);
  if (!_dataReadTransform) {
    if (error) {
  ...
  _dataVerifyTransform = SecVerifyTransformCreate(
      publicKey, (__bridge CFDataRef)signatureData, &secError);
  if (!_dataVerifyTransform || secError) {
    if (error) {
      *error = [SIGError
          errorWrapping:(__bridge NSError *)secError
  ...
  NSNumber *result =
      (__bridge_transfer NSNumber *)SecTransformExecute(_group, &secError);
  if (secError) {
    if (error) {
  ...
  if (!result.boolValue) {
    if (error) {
      *error =
          [SIGError errorWithCode:SIGErrorCodeSignatureDoesNotMatchPayload];
    }
    return NO;
  }
