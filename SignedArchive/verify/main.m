...
static NSString *Details(SIGArchiveVerifier *verifier) {
  ...
  for (NSData *data in [verifier.reader signingCertificates:nil]) {
    ...
    NSString *line =
        [NSString stringWithFormat:@"       Certificate “%@”", name];
    if (first) {
    ...
    if (cert.issuer) {
      NSString *name =
          ((cert.issuer.name ?: cert.issuer.longDescription) ?: @"Unknown");
  ...
  [detailLines
      addObject:[NSString stringWithFormat:@"    Payload length: %@",
                                           @(verifier.reader.payloadLength)]];
  return [detailLines componentsJoinedByString:@"\n"];
}

static NSError *Verify(NSString *path, BOOL requireV2, NSString **detailsPtr) {
  SIGArchiveVerifier *verifier =
      [[SIGArchiveVerifier alloc] initWithURL:[NSURL fileURLWithPath:path]];
  __block BOOL result;
  ...
  return [SIGError errorWithCode:SIGErrorCodeUnknown
                          detail:@"Verification failed for an unknown reason"];
}
  ...
  @autoreleasepool {
    ...
    for (int i = first; i < argc; i++) {
      ...
      NSError *error = Verify([NSString stringWithUTF8String:argv[i]],
                              requireV2, verbose ? &details : NULL);
      if (error) {
        errors++;
