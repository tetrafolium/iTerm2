...
SIGIdentity *FindSigningIdentity(NSString *query) {
  for (SIGIdentity *identity in [SIGIdentity allSigningIdentities]) {
    if ([identity.signingCertificate.longDescription
...
NSURL *TemporaryURLOfExtractedUnverifiedPayload(NSURL *archiveURL) {
  ...
  NSURL *url =
      CreateTemporaryURL([NSURL fileURLWithPath:NSTemporaryDirectory()]);
  if (!url) {
  ...
  while ([readStream hasBytesAvailable]) {
    ...
    if (numberOfBytesRead < 0) {
      fprintf(stderr, "Error %s reading from %s\n",
              readStream.streamError.localizedDescription.UTF8String,
              archiveURL.path.UTF8String);
      return nil;
    }
    ...
    if (numberOfBytesWritten != numberOfBytesRead) {
      fprintf(stderr, "Error %s while writing to %s\n",
              writeStream.streamError.localizedDescription.UTF8String,
              url.path.UTF8String);
      return nil;
    }
...
static BOOL ReSignExtracted(NSURL *oldArchiveURL, NSString *identityName,
                            NSURL *outputURL, NSURL *payloadURL) {
  if (!payloadURL) {
...
static BOOL ReSign(NSURL *oldArchiveURL, NSString *identityName,
                   NSURL *outputURL) {
  NSURL *payloadURL = TemporaryURLOfExtractedUnverifiedPayload(oldArchiveURL);
  const BOOL ok =
      ReSignExtracted(oldArchiveURL, identityName, outputURL, payloadURL);
...
static void Move(NSURL *source, NSURL *dest) {
  ...
  if (error) {
    fprintf(stderr, "While moving temp file %s over input file %s: %s",
            source.path.UTF8String, dest.path.UTF8String,
            error.localizedDescription.UTF8String);
  }
}
...
static int ResignMultiple(int argc, const char *argv[]) {
  ...
  for (size_t i = 3; i < argc; i++) {
    NSURL *fileURL =
        [NSURL fileURLWithPath:[NSString stringWithUTF8String:argv[i]]];
