...
- (instancetype)init {
  ...
  if (self) {
    _queue =
        dispatch_queue_create("com.iterm2.pidinfo", DISPATCH_QUEUE_CONCURRENT);
  }
  return self;
}
...
- (void)runShellScript:(NSString *)script
             ...
             withReply:
                 (void (^)(NSData *_Nullable, NSData *_Nullable, int))reply {
  [self performRiskyBlock:^(BOOL shouldPerform,
                            BOOL (^_Nullable completion)(void)) {
    if (!shouldPerform) {
      reply(nil, nil, 0);
      syslog(LOG_WARNING, "pidinfo wedged");
      return;
    }
    [self reallyRunShellScript:script
                         shell:shell
                    completion:^(NSData *output, NSData *error, int status) {
                      if (!completion()) {
                        syslog(LOG_INFO,
                               "runShellScript finished after timing out");
                        return;
                      }
...
- (NSString *)temporaryFileNameWithPrefix:(NSString *)prefix
  ...
  NSString *template =
      [NSString stringWithFormat:@"%@XXXXXX%@", prefix ?: @"", suffix ?: @""];
  ...
  int fileDescriptor =
      mkstemps(tempFileNameCString, (int)strlen(suffix.UTF8String));
...
- (void)reallyRunShellScript:(NSString *)script
                       shell:(NSString *)shell
                  completion:(void (^)(NSData *_Nullable, NSData *_Nullable,
                                       int))completion {
  @try {
    NSTask *task = [[NSTask alloc] init];
    task.launchPath = shell;

    NSString *tempfile = [self temporaryFileNameWithPrefix:@"iTerm2-script"
                                                    suffix:@"sh"];
    NSError *error = nil;
...
- (void)getProcessInfoForProcessID:(NSNumber *)pid
  ...
  [self performRiskyBlock:^(BOOL shouldPerform, BOOL (^completion)(void)) {
    if (!shouldPerform) {
      reply(@-1, [NSData data]);
      syslog(LOG_WARNING,
             "pidinfo %d detected wedged proc_pidinfo for process ID %d, "
             "flavor %d. Count is %d.",
             reqid, pid.intValue, flavor.intValue, self->_numWedged);
      return;
    }
    [self reallyGetProcessInfoForProcessID:pid
                                    flavor:flavor
                                       arg:arg
                                      size:size
                                     reqid:reqid
                                 withReply:^(NSNumber *number, NSData *data) {
                                   if (!completion()) {
                                     syslog(LOG_INFO,
                                            "pidinfo reqid %d finished after "
                                            "timing out",
                                            reqid);
                                     return;
                                   }
...
- (void)checkIfDirectoryExists:(NSString *)directory
                     withReply:(void (^)(NSNumber *_Nullable))reply {
  [self performRiskyBlock:^(BOOL shouldPerform,
                            BOOL (^_Nullable completion)(void)) {
    if (!shouldPerform) {
      reply(nil);
...
//   }];
// }];
- (void)performRiskyBlock:(void (^)(BOOL shouldPerform,
                                    BOOL (^_Nullable completion)(void)))block {
  ...
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{
        ...
        if (self->_numWedged > 128) {
          syslog(LOG_ERR,
                 "There are more than 128 wedged threads. Restarting.");
          _exit(0);
        }
      });
  dispatch_async(_queue, ^{
    block(YES, ^{
      ...
      if (wedged) {
        ...
        self->_numWedged--;
        syslog(LOG_INFO,
               "pidinfo detected slow but not wedged proc_pidinfo. Count is "
               "now %d.",
               self->_numWedged);
        return NO;
      }
...
- (void)maybeDelayWithFlavor:(int)flavor
  ...
  syslog(LOG_ERR, "pidinfo %d responding slowly because directory is root.",
         reqid);
...
- (void)reallyGetProcessInfoForProcessID:(NSNumber *)pid
  ...
  NSMutableData *result =
      [NSMutableData dataWithLength:size.unsignedIntegerValue];
#if ENABLE_VERY_VERBOSE_LOGGING
  syslog(LOG_DEBUG,
         "pidinfo %d will call proc_pidinfo(pid=%d, flavor=%d). wedged=%d "
         "count=%d",
  ...
  const int rc = proc_pidinfo(
      pid.intValue, flavor.intValue, arg.unsignedIntegerValue,
      (size.integerValue > 0) ? result.mutableBytes : NULL, safeLength);
  if (rc <= 0) {
    const int copyOfErrno = errno;
    NSString *message = [NSString
        stringWithFormat:@"proc_pidinfo flavor=%@ pid=%@ arg=%@ size=%@ "
                         @"returned %@ with errno %@",
...
#if ENABLE_SLOW_ROOT
  if (rc > 0) {
    [self maybeDelayWithFlavor:flavor.intValue reqid:reqid result:result];
  }
#endif
  struct timespec end;
  ...
  syslog(LOG_DEBUG,
         "pidinfo %d finished proc_pidinfo(pid=%d, flavor=%d) in %dms", reqid,
         pid.intValue, flavor.intValue, ms);
#endif
  dispatch_async(dispatch_get_main_queue(), ^{
    reply(@(rc), result);
  });
