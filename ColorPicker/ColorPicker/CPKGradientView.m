...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  NSRect rect = NSMakeRect(0.5, 0.5, self.bounds.size.width - 1,
  ...
  for (NSInteger i = 0; i < self.bounds.size.height; i++) {
    ...
    NSGradient *gradient = [[NSGradient alloc]
        initWithStartingColor:[self startingColorAt:progress]
...
- (NSRect)indicatorFrame {
  ...
  NSRect frame =
      NSMakeRect(self.selectedX * NSWidth(self.bounds) - halfX,
                 MAX(1 - halfY, self.selectedY * NSHeight(self.bounds) - halfY),
                 self.indicatorView.image.size.width,
                 self.indicatorView.image.size.height);
  frame.origin.x =
      MIN(MAX(frame.origin.x, 0), NSMaxX(self.bounds) - NSWidth(frame));
  frame.origin.y =
      MIN(MAX(frame.origin.y, 0), NSMaxY(self.bounds) - NSHeight(frame));
  return frame;
}
...
- (NSColor *)startingColorAt:(CGFloat)fraction {
  ...
  case kCPKGradientViewTypeRedGreen:
    return [NSColor cpk_colorWithRed:0 green:fraction blue:self.blue alpha:1];
    break;
  case kCPKGradientViewTypeGreenBlue:
    return [NSColor cpk_colorWithRed:self.red green:0 blue:fraction alpha:1];
    break;
  case kCPKGradientViewTypeBlueRed:
    return [NSColor cpk_colorWithRed:fraction green:self.green blue:0 alpha:1];
    break;
  }
...
- (NSColor *)endingColorAt:(CGFloat)fraction {
  ...
  case kCPKGradientViewTypeHueSaturation:
    ...
    break;
  case kCPKGradientViewTypeRedGreen:
    return [NSColor cpk_colorWithRed:1 green:fraction blue:self.blue alpha:1];
    break;
  case kCPKGradientViewTypeGreenBlue:
    return [NSColor cpk_colorWithRed:self.red green:1 blue:fraction alpha:1];
    break;
  case kCPKGradientViewTypeBlueRed:
    return [NSColor cpk_colorWithRed:fraction green:self.green blue:1 alpha:1];
    break;
  }
  return [NSColor blackColor];
}
...
- (CPKColor *)selectedColor {
  ...
  case kCPKGradientViewTypeRedGreen:
    return
        [[CPKColor alloc] initWithColor:[NSColor cpk_colorWithRed:self.selectedX
  ...
  case kCPKGradientViewTypeGreenBlue:
    return
        [[CPKColor alloc] initWithColor:[NSColor cpk_colorWithRed:self.red
  ...
  case kCPKGradientViewTypeBlueRed:
    return
        [[CPKColor alloc] initWithColor:[NSColor cpk_colorWithRed:self.selectedY
                                                            green:self.green
                                                             blue:self.selectedX
                                                            alpha:1]];
    break;
