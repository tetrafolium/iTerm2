...
- (instancetype)initWithFrame:(NSRect)frameRect
               noColorAllowed:(BOOL)noColorAllowed {
  ...
  if (self) {
    ...
    NSPoint origin = NSMakePoint(
        NSMaxX(self.addFavorite.frame) + kMarginBetweenButtons, kTopMargin);
    self.removeFavorite =
        [self addButtonWithImage:[self cpk_imageNamed:@"Remove"] origin:origin];
    ...
    origin = NSMakePoint(
        NSMaxX(self.removeFavorite.frame) + kMarginBetweenButtons, kTopMargin);
    self.eyedropperMode =
        [self addButtonWithImage:[self cpk_imageNamed:@"Eyedropper"]
                          origin:origin];
    ...
    origin = NSMakePoint(
        NSMaxX(self.eyedropperMode.frame) + kMarginBetweenButtons, kTopMargin);
    self.escapeHatch =
        [self addButtonWithImage:[self cpk_imageNamed:@"EscapeHatch"]
                          origin:origin];
    ...
    origin = NSMakePoint(NSMaxX(self.escapeHatch.frame) +
                             kMarginBetweenButtonsAndSwatch,
                         kTopMargin);
    self.swatch = [[CPKSwatchView alloc]
        initWithFrame:NSMakeRect(
                          NSMaxX(self.escapeHatch.frame) +
                              kMarginBetweenButtonsAndSwatch,
                          kTopMargin,
                          NSWidth(frameRect) - NSMaxX(self.escapeHatch.frame) -
                              kMarginBetweenButtonsAndSwatch - kRightMargin,
...
- (void)setUseSystemColorPicker:(BOOL)useSystemColorPicker {
  ...
  self.escapeHatch.image = self.useSystemColorPicker
                               ? [self cpk_imageNamed:@"ActiveEscapeHatch"]
