...
// Picking a color with a different hue will cause silly results. With a
// multiplier of 1, #fefb67 gets named "Hit Pink". So we use this to make hues
// span 0-5 while saturation and brightness each take values in 0-1. We will
// rarely find a nearest neighbor with a significantly different hue.
...
- (instancetype)init {
  ...
  if (self) {
    NSString *filename =
        [[NSBundle bundleForClass:[self class]] pathForResource:@"colors"
                                                         ofType:@"txt"];
        ...
        if (hex.length == 6) {
          ...
          if ([scanner scanHexInt:&value]) {
            ...
            [self.tree
                addObject:name
                   forKey:@[
                     @(color.hueComponent * kHueMultiplier),
                     @(color.saturationComponent), @(color.brightnessComponent)
                   ]];
          }
        }
      }
...
- (NSString *)nameForColor:(NSColor *)color {
  NSString *baseName = [self.tree nearestNeighborTo:@[
    @(color.hueComponent * kHueMultiplier), @(color.saturationComponent),
    @(color.brightnessComponent)
  ]];
  if (color.alphaComponent < 0.99) {
    return [baseName
        stringByAppendingFormat:@" (%d%%)", (int)(color.alphaComponent * 100)];
  } else {
    return baseName;
  }
}
