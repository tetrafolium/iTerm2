...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  [dividerColor set];
  NSRectFill(
      NSMakeRect(self.bounds.size.width - 1, 0, 1, self.bounds.size.height));
  ...
  centerColor =
      [centerColor colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
  NSString *description =
      [NSString stringWithFormat:@"#%02x%02x%02x R:%d G:%d B:%d",
  ...
  NSRect frame = NSMakeRect(floor((NSWidth(self.bounds) - size.width) / 2),
                            NSMidY(self.bounds) + 20, size.width, size.height);
  ...
  path = [NSBezierPath bezierPathWithRoundedRect:expandedFrame
                                         xRadius:3
                                         yRadius:3];
