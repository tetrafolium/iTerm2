...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  // I don't know why NSGradient is misbehaving here. If I have it go from
  // clearColor to [self.color colorWithAlphaComponent:1] it looks all wrong.
  NSMutableArray<NSColor *> *colors = [NSMutableArray array];
  int parts = 20;
  for (int i = 0; i <= parts; i++) {
    [colors
        addObject:[self.color.color colorWithAlphaComponent:i / (double)parts]];
  }
