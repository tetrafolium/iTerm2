...
NSString *const kCPKFavoritesDidChangeNotification =
    @"kCPKFavoritesDidChangeNotification";
...
@interface CPKFavoritesView () <NSTableViewDataSource, NSTableViewDelegate,
                                NSTextFieldDelegate>
...
- (instancetype)initWithFrame:(NSRect)frameRect {
  ...
  if (self) {
    ...
    dispatch_once(&onceToken, ^{
      ...
      NSArray *userDefaults = [[NSUserDefaults standardUserDefaults]
          objectForKey:kCPFavoritesUserDefaultsKey];
      if (!userDefaults) {
    ...
    self.tableView =
        [[NSTableView alloc] initWithFrame:NSMakeRect(0, 0, availableSize.width,
    ...
    self.colorColumn =
        [[NSTableColumn alloc] initWithIdentifier:kColorColumnIdentifier];
    self.colorColumn.width = 25;

    self.nameColumn =
        [[NSTableColumn alloc] initWithIdentifier:kNameColumnIdentifier];
...
- (NSArray *)cannedFavorites {
  return @[
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.0
                                                       green:0.0
                                                        blue:0.0
                                                       alpha:1]
                              name:@"Black"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.1
                                                       green:0.1
                                                        blue:0.1
                                                       alpha:1]
                              name:@"10% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.2
                                                       green:0.2
                                                        blue:0.2
                                                       alpha:1]
                              name:@"20% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.3
                                                       green:0.3
                                                        blue:0.3
                                                       alpha:1]
                              name:@"30% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.4
                                                       green:0.4
                                                        blue:0.4
                                                       alpha:1]
                              name:@"40% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.5
                                                       green:0.5
                                                        blue:0.5
                                                       alpha:1]
                              name:@"50% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.6
                                                       green:0.6
                                                        blue:0.6
                                                       alpha:1]
                              name:@"60% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.7
                                                       green:0.7
                                                        blue:0.7
                                                       alpha:1]
                              name:@"70% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.8
                                                       green:0.8
                                                        blue:0.8
                                                       alpha:1]
                              name:@"80% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:0.9
                                                       green:0.9
                                                        blue:0.9
                                                       alpha:1]
                              name:@"90% Gray"],
    [CPKFavorite favoriteWithColor:[NSColor cpk_colorWithRed:1.0
                                                       green:1.0
                                                        blue:1.0
                                                       alpha:1]
                              name:@"White"]
...
- (NSView *)colorViewForColor:(NSColor *)color {
  // The table view wants to resize its views, so to keep it centered properly,
  // I use a wrapper view that doesn't resize its subviews.
  NSView *wrapper =
      [[NSView alloc] initWithFrame:NSMakeRect(0, 0, kColorColumnWidth,
                                               self.tableView.rowHeight - 1)];
  CPKSwatchView *view = [[CPKSwatchView alloc]
      initWithFrame:NSMakeRect(0, 1, kColorColumnWidth,
                               self.tableView.rowHeight - 1)];
...
- (NSView *)nameViewWithValue:(NSString *)name
                   identifier:(NSString *)identifier {
  NSTextField *textField =
      [[NSTextField alloc] initWithFrame:NSMakeRect(0, 0, self.nameColumn.width,
...
- (void)saveFavorites {
  ...
  [self.tableView reloadData];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:kCPKFavoritesDidChangeNotification
                    object:self];
}
...
- (void)selectColor:(NSColor *)color {
  ...
  for (NSInteger i = 0; i < gFavorites.count; i++) {
    if ([[gFavorites[i] color] isApproximatelyEqualToColor:color]) {
      // Temporarily remove the block pointer to avoid notifying the client of
      // this change, since they initiated it.
...
- (NSView *)tableView:(NSTableView *)tableView
  ...
  if (tableColumn == self.colorColumn) {
    ...
    return [self nameViewWithValue:favorite.name
  ...
  [info enumerateDraggingItemsWithOptions:0
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *draggingItem,
                                 ...
                                 NSString *identifier =
                                     favoriteToAdd.identifier;
                                 NSInteger indexToRemove = [self
                                     rowForFavoriteWithIdentifier:identifier];
  ...
  [info enumerateDraggingItemsWithOptions:0
                            ...
                            searchOptions:@{}
                               usingBlock:^(NSDraggingItem *draggingItem,
                                 ...
                                 NSInteger row =
                                     [self rowForFavoriteWithIdentifier:
                                               favorite.identifier];
                                 if (row < 0) {
                                   // We don't accept drags from other processes
                                   // yet.
                                   shouldAccept = NO;
                                   *stop = YES;
                                 }
