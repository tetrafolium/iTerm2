...
/** This really should be an NSCell. It provides the implementation of the
 * CPKColorWell. */
@interface CPKColorWellView : CPKSwatchView

/** Block invoked when the user changes the color. Only called for continuous
 * controls. */
...
@interface CPKColorWellView () <NSDraggingDestination, NSDraggingSource,
                                NSPopoverDelegate>
...
- (void)mouseDragged:(NSEvent *)theEvent {
  ...
  const CGFloat kDragThreshold =
      5; // Minimum drag distance before initiating a drag.
  if (distance < kDragThreshold) {
    return;
  }
  NSColor *color = self.selectedColor ?: [NSColor clearColor];
  NSDraggingItem *dragItem =
      [[NSDraggingItem alloc] initWithPasteboardWriter:color];
  ...
  dragItem.imageComponentsProvider =
      ^NSArray<NSDraggingImageComponent *> *(void) {
    NSDraggingImageComponent *imageComponent = [NSDraggingImageComponent
...
- (void)useColorPicker:(id)sender {
  ...
  if (self.savedPresentingView.window) {
    [self openPopOverRelativeToRect:self.savedPresentationRect
                             ofView:self.savedPresentingView];
  }
}
...
- (void)showSystemColorPicker {
  ...
  container.frame = NSMakeRect(0, 0, NSMaxX(button.frame),
                               NSMaxY(button.frame) + kBottomMargin);
...
- (void)openPopOverRelativeToRect:(NSRect)presentationRect
                           ofView:(NSView *)presentingView {
  ...
  self.popover = [CPKPopover presentRelativeToRect:presentationRect
