...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p object=%@ key=%@ left=%p right=%p>",
                       NSStringFromClass([self class]), self, self.object,
                       self.keyDescription, self.leftChild, self.rightChild];
}
...
- (NSString *)dot {
  ...
  if (self.leftChild) {
    [result appendFormat:@"\"%@ @ %@\" -> \"%@ @ %@\"\n", self.object,
                         self.keyDescription, self.leftChild.object,
                         self.leftChild.keyDescription];
    [result appendString:self.leftChild.dot];
  }
  if (self.rightChild) {
    [result appendFormat:@"\"%@ @ %@\" -> \"%@ @ %@\"\n", self.object,
                         self.keyDescription, self.rightChild.object,
