...
+ (void)pickColorWithCompletion:(void (^)(NSColor *color))completion {
  ...
  eyedropperWindow.eyedropperView =
      [[CPKEyedropperView alloc] initWithFrame:rect];
...
- (NSInteger)currentScreenIndex {
  ...
  [[NSScreen screens]
      enumerateObjectsUsingBlock:^(NSScreen *_Nonnull screen, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
...
- (void)accept {
  ...
  point.x *= screen.backingScaleFactor;
  point.y *= screen.backingScaleFactor;
  self.selectedColor = [[self.currentScreenScreenshot colorAtX:point.x
                                                             y:point.y]
