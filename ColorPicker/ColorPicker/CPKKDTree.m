...
// This is where values are stored before -build is called. Its elements are
// NSArray objects. Each array has two values; the first is the associated
// object, and the second is the key.
...
// Valid only during a search. The current best distance from the target to
// |self.guess|.
...
- (CPKKDTreeNode *)buildAtDepth:(NSInteger)depth entries:(NSArray *)entries {
  ...
  NSInteger indexOfMedian = [self indexOfMedianEntryFromEntries:entries
                                                         onAxis:axis];
...
- (NSInteger)indexOfMedianEntryFromEntries:(NSArray *)entries
                                    onAxis:(NSInteger)axis {
  // Sort entries by the |axis|th value in key.
  NSArray *sortedEntries = [entries
...
- (NSArray *)entriesBefore:(double)pivot
                    onAxis:(NSInteger)axis
...
- (id)nearestNeighborTo:(NSArray *)key {
  NSAssert(self.stagedEntries == nil,
           @"You must call -build before querying the tree.");
  if (!self.root) {
    return nil;
  }
...
- (void)findNearestNeighborTo:(NSArray *)testPoint
  ...
  // If the current location is better than the best known location, update the
  // best known location. NOTE: The Stanford document uses the wrong arguments
  // to distance.
  ...
  if (a_i < curr_i) {
    DebugLog(@"Check left child because test point %f is less than splitting "
             @"plane %f",
             a_i, curr_i);
    [self findNearestNeighborTo:testPoint depth:depth + 1 node:curr.leftChild];
    otherChild = curr.rightChild;
  } else {
    DebugLog(@"Check right child because test point %f is more than splitting "
             @"plane %f",
             a_i, curr_i);
    [self findNearestNeighborTo:testPoint depth:depth + 1 node:curr.rightChild];
  ...
  if (fabs(curr_i - a_i) < self.bestDistance) {
    DebugLog(@"Hyperplane at %f for this node is close enough to the test "
             @"point on this axis "
             @"of %f to search since the distance between planes is %f and the "
             @"best distance "
             @"is %f",
             curr_i, a_i, fabs(curr_i - a_i), self.bestDistance);
    [self findNearestNeighborTo:testPoint depth:depth + 1 node:otherChild];
  } else {
    DebugLog(@"Hyperplane at %f for this node is NOT close enough to the test "
             @"point on this "
             @"axis of %f to search since the distance between planes is %f "
             @"and the best "
             @"distance is %f",
             curr_i, a_i, fabs(curr_i - a_i), self.bestDistance);
  }
