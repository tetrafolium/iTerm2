...
- (void)loadView {
  // The 100 value below is temporary and will be changed before we return to
  // the correct height.
  self.view = [[CPKFlippedView alloc]
      initWithFrame:NSMakeRect(0, 0, kDesiredWidth, 100)];
  self.view.autoresizesSubviews = NO;

  __weak __typeof(self) weakSelf = self;
  self.selectionView = [[CPKSelectionView alloc]
  ...
  self.controlsView = [[CPKControlsView alloc]
       initWithFrame:NSMakeRect(0, NSMaxY(self.selectionView.frame),
                                kDesiredWidth, [CPKControlsView desiredHeight])
  ...
  self.controlsView.addFavoriteBlock = ^() {
    ...
    input.stringValue =
        [[CPKColorNamer sharedInstance] nameForColor:weakSelf.selectedColor];
    alert.accessoryView = input;
    [alert layout];
    [[alert window] makeFirstResponder:input];
    NSInteger button = [alert runModal];
    if (button == NSAlertFirstButtonReturn) {
      [weakSelf.favoritesView
          addFavorite:[CPKFavorite favoriteWithColor:weakSelf.selectedColor
  ...
  self.controlsView.startPickingBlock = ^() {
    [CPKEyedropperWindow pickColorWithCompletion:^(NSColor *color) {
      if (color) {
        weakSelf.selectionView.selectedColor =
            [[CPKColor alloc] initWithColor:color];
      }
    }];
  };
  ...
  self.favoritesView = [[CPKFavoritesView alloc]
      initWithFrame:NSMakeRect(kLeftMargin, NSMaxY(self.controlsView.frame),
  ...
  self.favoritesView.selectionDidChangeBlock = ^(NSColor *newColor) {
    if (newColor) {
      weakSelf.selectionView.selectedColor =
          [[CPKColor alloc] initWithColor:newColor];
  ...
  self.view.subviews =
      @[ self.selectionView, self.controlsView, self.favoritesView ];
...
- (void)viewDidAppear {
  self.controlsView.useSystemColorPicker =
      [[NSUserDefaults standardUserDefaults]
          boolForKey:kCPKUseSystemColorPicker];
}
...
- (void)selectionViewContentSizeDidChange {
  ...
  self.controlsView.frame =
      NSMakeRect(0, NSMaxY(self.selectionView.frame), kDesiredWidth,
                 [CPKControlsView desiredHeight]);

  self.favoritesView.frame = NSMakeRect(
      kLeftMargin, NSMaxY(self.controlsView.frame),
      kDesiredWidth - kLeftMargin - kRightMargin, [self favoritesViewHeight]);
}
