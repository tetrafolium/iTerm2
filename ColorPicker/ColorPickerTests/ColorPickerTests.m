...
- (void)testKDTreeStanfordData {
  NSArray *points = @[
    @[ @2, @1, @3 ], @[ @2, @3, @7 ], @[ @1, @4, @4 ], @[ @2, @4, @5 ],
    @[ @3, @1, @4 ], @[ @0, @5, @7 ], @[ @6, @1, @4 ], @[ @4, @0, @6 ],
    @[ @4, @3, @4 ], @[ @5, @2, @5 ], @[ @7, @1, @6 ]
  ];
...
- (NSArray *)randomPointsWithSeed:(NSInteger)seed {
  ...
  for (NSInteger i = 0; i < numPoints; i++) {
    NSArray *point = @[
      @(abs(rand()) % kMaxValue), @(abs(rand()) % kMaxValue),
      @(abs(rand()) % kMaxValue)
    ];
    ...
    for (NSInteger y = -1; y < kMaxValue + 1; y++) {
      for (NSInteger z = -1; z < kMaxValue + 1; z++) {
        ...
        double distanceToNearestNeighbor =
            [self distanceFrom:xyz to:points[n.integerValue]];
        if (distanceToNearestNeighbor > bestDistance) {
          NSLog(@"Oops");
        }
        XCTAssertEqualWithAccuracy(distanceToNearestNeighbor, bestDistance,
                                   0.0001, @"Found wrong result");
      }
    }
  }
}
